...
//#endif
    ...
    hARMVB = ARMVB_RegAudioDevice(); // TODO: LJF TEST MOVE POSITION
...
//hch,add a2dp dev
    ...
    hAUDARMRECExp = AUD_rec_RegExpPlugger();
    ...
    hFMADDev = ARMVB_FMAd_RegAudioDevice();
#endif    
#ifdef BLUETOOTH_SUPPORT
...
#ifdef BT_AV_SNK_SUPPORT  //for bt_dialer a2dp play music 
#ifdef EQ_EXE_ON_CHIP
    #ifndef EQ_HAS_MULTI_BANDS
        AUDIO_PM_AddCodecExpress (hAdOutputCodec, hAUDPROCExp);
    #else
        AUDIO_PM_AddCodecExpress (hAdOutputCodec, hAUDENHAExp);
    #endif
#else
    AUDIO_PM_AddCodecExpress (hAdOutputCodec, hEQExp);
    AUDIO_PM_AddCodecExpress (hAdOutputCodec, hAGCExp);
#endif
#endif
...
//  Note:
/*****************************************************************************/
#define LOOPBACK_DELAY_TIME (3000)    //ms  20/400/1000/2000/3000/4000    //control LOOPBACK delay time
...
)
{
    ...
    if(NULL==hVoiceRing)
    {
        hVoiceRing    = AUDIO_CreateNormalHandle(hVoiceCodec,    SCI_NULL, hLOOPBACKDEV,     SCI_NULL, AudioDummyNotifyCallback);
        SCI_ASSERT(NULL != hVoiceRing);/*assert verified*/  
    }
    
    if(is_enable)
    {
	 //AUDIO_CP_SetBtNRECFlag(LOOPBACK_DELAY_TIME);  //reuse BT NREC Flag for add loopback delay time in audio driver
    ...
    else
    {
        ...
        AUD_EnableVoiceProcessConfig(SCI_FALSE, SCI_TRUE, PNULL);
        if(result!=AUDIO_NO_ERROR)
        {
...
{
    SCI_TRACE_LOW("[ljf] AUD_EnableVoiceProcessConfig come on in");
    return AUDIO_NO_ERROR;
#if 0
