...
//       (s_dc_info_ptr->preview_ctl_info.zoom_level == 0 || is_frame_enc == 1))
    {
    	SCI_TRACE_LOW(" david capture ENC ENC ENC jpeg_buf_ptr:0x%x malloc_buffer_ptr:0x%x",
		...
		SCI_TRACE_LOW(" david capture jpeg_buf_ptr:0x%x malloc_buffer_ptr:0x%x",
							 				pJpgEncInPara->jpeg_buf_ptr, malloc_buffer_ptr);
        ...
        {
        	SCI_TRACE_LOW("david JPEG_EncodeJpeg fail!!!");
            return DCAMERA_OP_ENCODE_ERR;
        }
...
LOCAL DCAMERA_RETURN_VALUE_E _DCAMERA_SetSensorRot(uint32 arg)
{
    ...
    else if(DCAMERA_ROTATION_180 == arg)
    {
        Sensor_Ioctl(SENSOR_IOCTL_VMIRROR_ENABLE,SCI_TRUE);
        h_mirror_eb = 1;
    }
    ...
    Sensor_Ioctl(SENSOR_IOCTL_HMIRROR_ENABLE,h_mirror_eb);
    return SCI_SUCCESS;
...
int dcamera_opencamera()
{
	...
	SCI_TRACE_LOW("davidtest open camera over");
    return ret;
}
...
PUBLIC DCAMERA_RETURN_VALUE_E DCAMERA_Init(BOOLEAN if_force_calib, DCAMERA_CAM_PARAM_T *p_Param)
{
    ...
    DCAMERA_TRACE("DCAMERA: DCAMERA_Init is force to calibration %d! nv sign 0x%x", if_force_calib, p_Param->cam_val_nv.sensor_nv_sign);
        ...
        ||(!Sensor_CheckIfHaveCamera()))
    {
        ...
        if(rtn)
        {
            DCAMERA_TRACE("DCAMERA: DCAMERA_Init - sensor_initial error! 1st path");
    ...
    else
    {
        ...
        if(rtn)
        {
            DCAMERA_TRACE("DCAMERA: DCAMERA_Init - sensor_initial error! 2nd path");
            rtn = DCAMERA_OP_NO_SENSOR_ERR;
        }
        else
        {
...
PUBLIC DCAMERA_RETURN_VALUE_E DCAMERA_Open(DCAMERA_OP_MODE_E op_mode)
{
	...
	SCI_TRACE_LOW("david %s 25 camera memAvalid=%d,totalmemAvalid=%d", __FUNCTION__,SCI_GetByteHeapAvailableSpace(DYNAMIC_HEAP_APP_MEMORY),SCI_GetHeapTotalAvailableSpace(DYNAMIC_HEAP_APP_MEMORY) );
    ...
    if(!DCAMERA_IsOpen())
    {
		...
		if(DCAMERA_OP_VIDEOCALL != op_mode)
		{
            ...
            {
                SCI_TRACE_LOW("[DC]  DCAMERA_Open :::: LCD_SetInvalidateLimit  PERMIT_GUI_INVALIDATE");
                LCD_SetInvalidateLimit(MAIN_LCD_ID, PERMIT_GUI_INVALIDATE);
            }
            else
            {
                SCI_TRACE_LOW("[DC]  DCAMERA_Open :::: LCD_SetInvalidateLimit  NO_PERMIT_GUI_INVALIDATE");
                LCD_SetInvalidateLimit(MAIN_LCD_ID, NO_PERMIT_GUI_INVALIDATE);
            }
		}
...
LOCAL DCAMERA_RETURN_VALUE_E _DCAMERA_Close(void)
{
    ...
    if(DCAMERA_IsOpen())
    {
        ...
        if(g_pic_buffer != NULL)
        {
            ...
            g_pic_buffer = NULL;
            SCI_TRACE_LOW("david free pic buffer.");
        }
 ...
 */
		...
		if(	DCAMERA_OP_VIDEOCALL != s_operation_mode)
		{
        	...
        	SCI_TRACE_LOW("[DC]  DCAMERA_StartPreview :::: LCD_SetInvalidateLimit=  NO_PERMIT_GUI_INVALIDATE");

        	LCD_DisableBlock(param_ptr->lcd_id, param_ptr->lcd_block_id);
        	SCI_TRACE_LOW("[DC]  DCAMERA_StartPreview :::: LCD_DisableBlock  %d %d ", param_ptr->lcd_id, param_ptr->lcd_block_id);
		}
    ...
    */
        ...
        while(count < DCAMERA_CAMERA_TRY_COUNT)
        {
            ...
            {
            	SCI_TRACE_LOW("david _DCAMERA_SetPreviewZoom ok");
        ...
        if(DCAMERA_OP_SUCCESS != ret_value)
        {
        	SCI_TRACE_LOW("david SetPreviewZoom error");
            //CAMERA_CloseSensor();
...
LOCAL int _DCAMERA_PreviewDisplayCallbackLowMem(void* address)
{
    ...
    DCAMERA_TRACE("_DCAMERA_PreviewDisplayCallbackLowMem, frame 0x%x, yaddr 0x%x, uaddr 0x%x", address,p_frame->yaddr,p_frame->uaddr);
    ...
    if(dc_info_ptr->display_cb)
    {
        ...
        if(!rtn)
        {
            ...
            DCAMERA_TRACE("_DCAMERA_PreviewDisplayCallbackLowMem, get proc src success block %d y_addr 0x%x u_addr 0x%x", 
                                                block_id_src, 
        ...
        {
            DCAMERA_TRACE("_DCAMERA_PreviewDisplayCallbackLowMem, havn't get the src process buffer!");
        ...
        if(!rtn)
        {
            ...
            DCAMERA_TRACE("_DCAMERA_PreviewDisplayCallbackLowMem, get proc dst success block %d y_addr 0x%x u_addr 0x%x", 
                                    block_id_dst, 
        ...
        {
            DCAMERA_TRACE("_DCAMERA_PreviewDisplayCallbackLowMem, havn't get the dst process buffer!");
            return (int)ISP_CB_PROC_QUIT;
        }
        ...
        if(rtn)                                                                     //display not success handle
        {
            ...
            DCAMERA_TRACE("_DCAMERA_PreviewDisplayCallbackLowMem, block %d state change from %d to %d", block_id_src, ISP_VT_MEMBLK_PROCING_SRC, ISP_VT_MEMBLK_IDLE);
            DCAMERA_TRACE("_DCAMERA_PreviewDisplayCallbackLowMem, block %d state change from %d to %d", block_id_dst, ISP_VT_MEMBLK_PROCING_DST, ISP_VT_MEMBLK_IDLE);
...
LOCAL DCAMERA_RETURN_VALUE_E _DCAMERA_StartPreviewEX(DCAMERA_PREVIEW_EX_PARAM_T const* param_ex_ptr)
{
    ...
    if(p_vt_enc->local_review_enable)
    {
        ...
        SCI_TRACE_LOW("[DC]  DCAMERA_StartPreviewEX :::: LCD_SetInvalidateLimit=  NO_PERMIT_GUI_INVALIDATE");

        LCD_DisableBlock(param_ex_ptr->lcd_id, param_ex_ptr->lcd_block_id);
        SCI_TRACE_LOW("[DC]  DCAMERA_StartPreviewEX :::: LCD_DisableBlock  %d %d ", param_ex_ptr->lcd_id, param_ex_ptr->lcd_block_id);
    ...
    if(DCAMERA_OP_SPECIAL == s_operation_mode)
    {
        ...
        DCAMERA_TRACE("DC: DCAMERA_StartPreviewEX, op special mem_start 0x%x, mem_size %d", mem_start, mem_size);
        ...
        if(dc_info_ptr->display_cb)
        {
            ...
            {
                DCAMERA_TRACE("DC: DCAMERA_StartPreviewEX, No enough memory to use normal case, try low memory case! Total 0x%x", mem_size);
                ...
                else
                {
                    ...
                    DCAMERA_TRACE("DC: DCAMERA_StartPreviewEX, now try low memory case!");
                }
            }
            else
            {
...
PUBLIC DCAMERA_RETURN_VALUE_E DCAMERA_StartRecord ( DCAMERA_RECORD_PARAM_T * param_ptr )
{
    ...
    LCD_SetInvalidateLimit(MAIN_LCD_ID, NO_PERMIT_GUI_INVALIDATE);
    SCI_TRACE_LOW("[DC]  DCAMERA_StartRecord :::: LCD_SetInvalidateLimit  NO_PERMIT_GUI_INVALIDATE");
...
LOCAL void _DCAMERA_BackupPreviewbuf2ReviewBuf(uint32 lcd_id, uint32 block_id)
{
    ...
    {
        DCAMERA_TRACE("_DCAMERA_BackupPreviewbuf2ReviewBuf: input parameters error!");
        return;
    }
    ...
    {
        DCAMERA_TRACE("_DCAMERA_BackupPreviewbuf2ReviewBuf: quick view buffer error");
        LCD_PutLcdCtrlSemaphore(0);
        return;
    }
    
    block_buf = (uint32)LCD_GetBlockBuffer(lcd_id, block_id);
	if (block_buf < dc_mem_start ||
		block_buf > base_addr) 
	{
        DCAMERA_TRACE("_DCAMERA_BackupPreviewbuf2ReviewBuf: block_buf error");
       ...
       blk_info.resolution != rev_fmt)
    {
        if(quick_view_size >= disp_buf_size)
        {
            if(blk_info.resolution != rev_fmt) // Preview use YUV422 always, so review must be YUV420 in this case 
            {
                ...
                if(0 == isp_frm.uaddr)
                {
                    DCAMERA_TRACE("_DCAMERA_BackupPreviewbuf2ReviewBuf: uv buffer error");
                    LCD_PutLcdCtrlSemaphore(0);
                    return;
                }       
...
//        temp_val1= (date_mark_in.target_size.w * date_mark_in.target_size.h)<<2;
        ...
        DCAMERA_TRACE(
            "_DCAMERA_handleScaleAndDate: scaling date params src w-%d,h-%d target w-%d,h-%d, buff addr-0x%x, size-%d", 
        ...
        if(PNULL != date_mark_in.target_buf.ptr)
        {
            ...
            DCAMERA_TRACE(
                "_DCAMERA_handleScaleAndDate: date mark start scaling, the size of the image to be blended: w -%d h - %d , \
                scaling data input param 0x%x, output result 0x%x", 
                target_size.w, 
                target_size.h,
                &date_mark_in,
                &date_mark_out
...
LOCAL int32 _DCAMERA_reviewDateCallback(void *frame_ptr)
{
    ...
    SCI_TRACE_LOW("david recv YUV frame");
...
static unsigned char*g_JpegTmpBuf;/*lint !1485123*/
                                     ...
                                     uint32 read_lines)
{
    ...
    DCAMERA_TRACE("_DCAMERA_JpgEncGetSliceF420T422: photo_width =%d, get_slice_width=%d, line_offset %d, read_lines %d, yaddr 0x%x, uv 0x%x",
    ...
    {
        DCAMERA_TRACE("_DCAMERA_JpgEncGetSliceF420T422: photo_height=%d,line_offset=%d", s_dc_info_ptr->jpeg_rect.w,line_offset);
        return 0;
    }

    if(read_lines > DC_SLICE_HEIGHT)
    {
        DCAMERA_TRACE("_DCAMERA_JpgEncGetSliceF420T422: dc slice height too large!");
        return 0;
    }
    ...
    {
        DCAMERA_TRACE("_DCAMERA_JpgEncGetSliceF420T422: y buffer too small!");
        return 0;
    }
    output_addr.uaddr = (uint32)DCAMERA_CaptureGetSliceBufUV(&mem_size);
    if(mem_size < s_dc_info_ptr->isp_frame.width * read_lines)
    {
        DCAMERA_TRACE("_DCAMERA_JpgEncGetSliceF420T422: uv buffer too small!");
        return 0;
    }
...
{                                                                                                                                                    
	...
	SCI_TRACE_LOW("david capture _DCAMERA_GetFrameFromIspPsl ");
    ...
    LCD_SetInvalidateLimit(MAIN_LCD_ID, NO_PERMIT_GUI_INVALIDATE);        
    SCI_TRACE_LOW("[DC]  _DCAMERA_GetFrameFromIspPsl :::: LCD_SetInvalidateLimit  NO_PERMIT_GUI_INVALIDATE");
                                      ...
                                      DCAMERA_QUICK_VIEW_CALL_BACK quickview_cb)
{
	...
	SCI_TRACE_LOW("david capture _DCAMERA_GetFrameFromIspNml ");
    ...
    if(ISP_ServiceSetJpegCaptureParam(p_cap_para))
    {
        LCD_SetCopyOsdMode(MAIN_LCD_ID,SCI_TRUE);
        _DCAMERA_PutMutex(s_callback_mutex_ptr);
		SCI_TRACE_LOW("david capture ISP_ServiceSetJpegCaptureParam error ");
        return DCAMERA_OP_ISP_ERR;
    }        
    ...
    _DCAMERA_GetMutex(s_callback_mutex_ptr);
    LCD_SetInvalidateLimit(MAIN_LCD_ID, NO_PERMIT_GUI_INVALIDATE); 
    SCI_TRACE_LOW("[DC]  _DCAMERA_GetFrameFromIspNml :::: LCD_SetInvalidateLimit  NO_PERMIT_GUI_INVALIDATE");
    ...
    LCD_SetInvalidateLimit(MAIN_LCD_ID, PERMIT_GUI_INVALIDATE);
    SCI_TRACE_LOW("[DC]  _DCAMERA_GetFrameFromIspNml :::: LCD_SetInvalidateLimit  PERMIT_GUI_INVALIDATE");
...
{                                                                                                    
    ...
    _DCAMERA_GetMutex(s_callback_mutex_ptr);
	//SCI_TRACE_LOW("david capture capture start callback");
    ...
    if(_DCAMERA_IsPreview())                                                                         
    {           
    	SCI_TRACE_LOW("david capture 2 ISP_ServiceStopPreview");
        ISP_ServiceStopPreview();                                                                    
    }     
    else
    {
...
//---------------------------------------------------------------------
    ...
    {               
    	SCI_TRACE_LOW("david capture alloc buffer mmi");
        malloc_buffer_ptr = param_ptr->data_buf;                                                     
        malloc_buffer_len = param_ptr->buf_len;                                                      
    }                                                                                                
    else                                                                                             
    {     
    	SCI_TRACE_LOW("david capture alloc buffer");
		...
		if(g_pic_buffer == NULL)
		{
			...
			{
				SCI_TRACE_LOW("david capture alloc buffer OK size:%u(k)", DCAMERA_JPEG_BUFFER_SIZE);
			}
			else
			{
				SCI_TRACE_LOW("david capture alloc buffer fail");
				_DCAMERA_CloseSensor();
        		DC_RET_ERR(DCAMERA_OP_NO_ENOUGH_MEMORY)
			}
    ...
    {   
        ...
        _DCAMERA_GetMutex(s_callback_mutex_ptr);  
        LCD_SetInvalidateLimit(MAIN_LCD_ID, NO_PERMIT_GUI_INVALIDATE);
        SCI_TRACE_LOW("[DC]  _DCAMERA_DoSnapshott :::: LCD_SetInvalidateLimit  NO_PERMIT_GUI_INVALIDATE");
	...
	SCI_TRACE_LOW("david capture alloc buffer buf:0x%x, len= %d", 
						param_ptr->data_buf,
						param_ptr->buf_len);
	...
	//SCI_TRACE_LOW("david capture snapshot_callback");
    if(param_ptr->snapshot_callback)                                                                 
    {                        
    	SCI_TRACE_LOW("david dosnapshot enter snapshot_callback time:%u", SCI_GetTickCount());
        _DCAMERA_GetMutex(s_callback_mutex_ptr);                                                     
...
LOCAL uint32 _DCAMERA_IsScalingBufferUsedForJpegEncoding(void)
{
#if (defined(PLATFORM_SC6530) || defined(PLATFORM_SC8800G)) || defined(PLATFORM_SC6531EFM) || defined(PLATFORM_UIX8910)
...
PUBLIC DCAMERA_RETURN_VALUE_E DCAMERA_DisplayOneFrame(DCAMERA_DISPALY_PARAM_T *param_ptr)
{
    ...
    if(ISP_VT_MEM_MODE_LOW == dc_info_ptr->vt_enc_param_t.mem_mode)
    {
        ...
        if(rtn)
        {
            DCAMERA_TRACE("DCAMERA_DisplayOneFrame, processing source memory block state error!");
            return DCAMERA_OP_MEMORY_ERR;
        }

        DCAMERA_TRACE("DCAMERA_DisplayOneFrame, changing block %d state from %d to %d", block_id, ISP_VT_MEMBLK_PROCING_SRC, ISP_VT_MEMBLK_IDLE);
        ...
        if(rtn)
        {
            DCAMERA_TRACE("DCAMERA_DisplayOneFrame, processing dest memory block state error!");
            return DCAMERA_OP_MEMORY_ERR;
        }

        DCAMERA_TRACE("DCAMERA_DisplayOneFrame, changing block %d state from %d to %d", block_id, ISP_VT_MEMBLK_PROCING_DST, ISP_VT_MEMBLK_INVALIDING);
...
LOCAL DCAMERA_RETURN_VALUE_E _DCAMERA_CloseISP(void)
{
	...
	SCI_TRACE_LOW("david _DCAMERA_CloseISP");
...
LOCAL int _DCAMERA_GetFrameFromIspExt(DCAMERA_EXT_GET_DATA_T *param_ptr )
{
    ...
    if(SENSOR_MODE_MAX <= s_sensor_ptr->sensor_mode_info[s_dc_info_ptr->snapshot_m].mode)
    {
        DCAMERA_TRACE("_DCAMERA_GetFrameFromIspExt, error sensor mode!");
        return DCAMERA_OP_ERROR;
    }
    ...
    if(SCI_NULL == mem_start || 0 == mem_size)
    {
        DCAMERA_TRACE("_DCAMERA_GetFrameFromIspExt, not enough memory!");
        return DCAMERA_OP_NO_ENOUGH_MEMORY;
    }
    //data capture handle
    //ISP_ServiceMemConfig((void*)mem_start, mem_size);
    ...
    if(ISP_ServiceSetJpegCaptureParam(p_cap_para))
    {
        DCAMERA_TRACE("_DCAMERA_GetFrameFromIspExt, set capture parameters error!");
        return DCAMERA_OP_ISP_ERR;
    }        
    ISP_ServiceStartCaptureJpeg();  
    ...
    if(_DCAMERA_WaitForCaptureDone())
    {
        DCAMERA_TRACE("_DCAMERA_GetFrameFromIspExt, capture timeout!");
        return DCAMERA_OP_ISP_ERR;
    }
    ...
    if(param_ptr->is_display)
    {
        ...
        LCD_SetInvalidateLimit(MAIN_LCD_ID, NO_PERMIT_GUI_INVALIDATE);  
        SCI_TRACE_LOW("[DC]  _DCAMERA_GetFrameFromIspExt :::: LCD_SetInvalidateLimit  NO_PERMIT_GUI_INVALIDATE");
        ...
        {
            DCAMERA_TRACE("_DCAMERA_GetFrameFromIspExt, set display parameter or memory error!");
            return DCAMERA_OP_PARAM_ERR;
        }
    ...
    if(DCAMERA_DATA_TYPE_JPEG == param_ptr->data_format)
    {
        ...
        {
            DCAMERA_TRACE("_DCAMERA_GetFrameFromIspExt, jpeg ecode error!");
            return DCAMERA_OP_DECODE_ERR;
        }
...
PUBLIC uint32 DCAMERA_ExtDisplay(DCAMERA_EXT_DISPLAY_T_PTR display_param )
{
    ...
    {
        DCAMERA_TRACE("DC: DCAMERA_ExtDisplay, set display parameter or memory error!");
    }
...
PUBLIC uint32 DC_DisplayAFrame(DISPLAY_DATA_FRAME_T *p_frame)
{
    ...
    {
        DCAMERA_TRACE("DC: DC_DisplayAFrame, set display parameter or memory error!");
    }
...
LOCAL void _Video_Display_SetFrame(const VIDEO_DISP_FRAME_T *p_frame, DISPLAY_DATA_FRAME_T * p_display_frameDate)
{
    ...
    {
        DCAMERA_TRACE("Video_Display_Oneframe,not support yuv%d", p_frame->data_type);
        p_display_frameDate->yuv_type = JINF_FORMAT_YUV420;
    }
...
PUBLIC int Video_Display_Oneframe(VIDEO_DISP_FRAME_T *p_frame)
{
    ...
    {
        DCAMERA_TRACE("Video_Display_Oneframe,p_frame == PNULL");
        return SCI_ERROR;
    }
    ...
    {
        DCAMERA_TRACE("Video_Display_Oneframe,w(%d) or h(%d) is zero!",  p_frame->org_w,  p_frame->org_h);
        return SCI_ERROR;
    }
    ...
    {
        DCAMERA_TRACE("Video_Display_Oneframe, trim size bigger than dec out size!");
        return SCI_ERROR;
    }
    else
    {
    ...
    {
        DCAMERA_TRACE("Video_Display_Oneframe, DC_DisplayAFrame fail");
        result = SCI_ERROR;
    }
