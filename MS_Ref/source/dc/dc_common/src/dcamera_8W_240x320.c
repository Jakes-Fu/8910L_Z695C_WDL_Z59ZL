...
PUBLIC DCAMERA_IOCTL_CFG_T  g_dcam_cfg_8W_240x320 =
{
    get_GoudaOffset,
    Gouda_config,
...
static int Gouda_config(int mode, BLOCKCFG_T* cfg, DC_CAMERA_CFG ** param)
{
    ...
    if(!cfg || !param)
    {
        SCI_PASSERT(SCI_FALSE, ("Dcamera_Gouda_config param error!"));
    }

    cfg->start_x = 0;
    cfg->start_y = 0;
    cfg->end_x = lcd_info.lcd_width-1; //239
    cfg->end_y = lcd_info.lcd_height-1; //239 
    ...
    {
        SCI_PASSERT(SCI_FALSE, ("Dcamera_Gouda_config mode Error!"));
    }
    
    SCI_TRACE_LOW("david displayoneframe config lcd(w*h=%d*%d) YUV(w*h=%d*%d) rot=%d mod=%d",
...
static uint32 get_GoudaOffset(int mode, DC_CAMERA_CFG ** param)
{
    ...
    if(!param)
    {
       SCI_PASSERT(SCI_FALSE, ("_ISP_ServiceDisplayOneFrame mode error!")); 
    }
...
/* 
 *  8W sensor 240x320. 预览输出YUV 240X320
 */
static uint32 Preview_init_config(DC_CAMERA_CFG* param)
{
	...
	pCfg->hal_cfg.spi_pixels_per_line   = 240;                 //320; //IspOutWidth;
	pCfg->hal_cfg.spi_pixels_per_column = 320;                 //IspOutHeight;
	...
	pCfg->hal_cfg.scaleEnable = FALSE;

	pCfg->hal_cfg.cropEnable     = TRUE;
	pCfg->hal_cfg.dstWinColStart = 0;
	pCfg->hal_cfg.dstWinColEnd   = 240;
	pCfg->hal_cfg.dstWinRowStart = 0;
	pCfg->hal_cfg.dstWinRowEnd   = 320;

	pCfg->preview_rot = 0;   //普通预览旋转90度
    
	pCfg->width  = 240;      
	pCfg->height = 320;

	pCfg->display_width = 240;
	pCfg->display_height =320;
...
/*
 *  8W sensor 240x320. 拍照输出YUV 240x320
 */
static uint32 Snapshot_init_config(DC_CAMERA_CFG* param)
{
	...
	pCfg->preview_rot = 0;  //拍照照片预览旋转90度
    pCfg->enc_rot     = 0;  
...
/*
 *  8W sensor 240x320. 录像输出YUV 240x320
 */
static uint32 Record_init_config(DC_CAMERA_CFG* param)
{
...
//视频通话Camera输出320x240,视频通话实际size由
//上层软件从该320x240 YUV数据中截取
static uint32 VideoCall_init_config(DC_CAMERA_CFG* param)
{
