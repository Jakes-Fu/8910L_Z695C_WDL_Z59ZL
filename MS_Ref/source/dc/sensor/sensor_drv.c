...
#include "../../image_proc/src/isp/inc/uix8910_reg_isp.h"
/*lint -save -e765 */   
/**---------------------------------------------------------------------------*
 **                         Compiler Flag                                     *
 **---------------------------------------------------------------------------*/
...
#define     SENSOR_I2C_FREQ                   (200*1000) //(400*1000)//(100*1000)
...
{
	SCI_TRACE_LOW("david _Sensor_IicHandlerRelease!");
...
PUBLIC int32 _Sensor_IicHandlerInit(SENSOR_ID_E sensor_id) 
{
	...
	SCI_TRACE_LOW("david _Sensor_IicHandlerInit id = %d", sensor_id);
...
PUBLIC void Sensor_SetVoltage(SENSOR_AVDD_VAL_E dvdd_val, SENSOR_AVDD_VAL_E avdd_val, SENSOR_AVDD_VAL_E iodd_val)
{
	...
	SCI_TRACE_LOW("david Sensor_SetVoltage dvdd:%d, avdd:%d, iovdd:%d", 
...
PUBLIC ERR_SENSOR_E Sensor_SendRegTabToSensor(SENSOR_REG_TAB_INFO_T * sensor_reg_tab_info_ptr   )
{
	...
	OS_TickDelay(2);
	SCI_TRACE_LOW("david send setting over 11: stat0-3 0x%x 0x%x 0x%x 0x%x",
		stat[0], stat[1], stat[2], stat[3]);
	SCI_TRACE_LOW("david send setting over 22: stat4-7 0x%x 0x%x 0x%x 0x%x",
		stat[4], stat[5], stat[6], stat[7]);
	SCI_TRACE_LOW("david send setting over 33: stat8-9 0x%x 0x%x, err %d",
		stat[8], stat[9], err);
...
LOCAL void _Sensor_Identify(SENSOR_ID_E sensor_id)
{
    ...
    {  
        ...
        if(PNULL!=sensor_info_ptr->ioctl_func_tab_ptr->identify)
        {
            ...
            if(SCI_SUCCESS==sensor_info_ptr->ioctl_func_tab_ptr->identify(SENSOR_ZERO_I2C))
            {
            	SCI_TRACE_LOW("david identify ok!");
                s_sensor_list_ptr[sensor_id]=sensor_info_ptr; 
                s_sensor_register_info_ptr->is_register[sensor_id]=SCI_TRUE;
                s_sensor_register_info_ptr->img_sensor_num++;
...
PUBLIC BOOLEAN Sensor_Init(void)
{
    ...
    if(Sensor_IsInit())
    {
        ...
        for(i=0; i<SENSOR_ID_MAX; i++)
        {
            SCI_TRACE_LOW("SENSOR: Sensor_Init the index of sensor %d is %d", i, s_sensor_index[i] );
        }
        return SCI_TRUE;
    }
...
PUBLIC void Sensor_SetIdentifiedState(BOOLEAN if_identified, uint8 *sensor_index)
{
    ...
    for(i = 0; i < SENSOR_ID_MAX; i++)
    {
        ...
        SCI_TRACE_LOW("Sensor_drv: Sensor_SetIdentifiedState the index of %d is %d", i, s_sensor_index[i]);
    }
}
...
PUBLIC void Sensor_GetIdentifiedState(SENSOR_ID_E sensor_id, uint8 *p_sensor_index)
{
    *p_sensor_index = s_sensor_index[sensor_id];
    SCI_TRACE_LOW("Sensor_drv: Sensor_GetIdentifiedState the index of %d is %d", sensor_id, s_sensor_index[sensor_id]);
    return;
}
...
int sensor_powerup()
{
	...
	SCI_TRACE_LOW("davidtest sensor_powerup start");
   ...
   // SCI_TRACE_ID(TRACE_TOOL_CONVERT,SENSOR_DRV_1437_112_2_18_1_24_35_625,(uint8*)"d", SCI_GetTickCount());
	...
	SCI_TRACE_LOW("davidtest sensor_powerup start over!");
    return 0; // CID37010
}
...
LOCAL BOOLEAN _Sensor_PowerUpHandle(SENSOR_ID_E sensor_id)
{
	...
	SCI_TRACE_LOW("david power up end");
...
PUBLIC int32 Sensor_I2cInit() 
{
	...
	SCI_TRACE_LOW("david _Sensor_IicHandlerInit");
