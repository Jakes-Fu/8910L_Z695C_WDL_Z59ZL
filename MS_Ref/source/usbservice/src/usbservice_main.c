...
extern void UCOM_SetWorkMode(USB_SERVICE_E service_type, uint8 calibmode);
/**---------------------------------------------------------------------------*
 **                         GLOBAL DEFINITION                                 *
 **---------------------------------------------------------------------------*/
...
LOCAL void USB_MainThread (uint32 argc, void *argv)
{
    ...
    else if ((RESTART_BY_CHARGE == restart_condition) || CHGMNG_IsChargeConnect())
    {
        UDC_HWInit();
        if (REFPARAM_GetDebugPortPhyNo() != VIR_COM1)
    ...
    for (;;)
    {
        ...
        {
            if ( (service_type==USB_SERVICE_UCOM) || (service_type==USB_SERVICE_LOG))
            {
                UCOM_SetWorkMode(service_type, calibmode);
            }
#ifdef WINUSB_SUPPORT            
            else if (service_type == USB_SERVICE_UCOM_UMS)
            {
                UCOM_SetWorkMode (service_type,calibmode);         
            }
#endif

            USBGadgetRegisterDriver (g_USBService_table[service_type]);
            USB_NotifyClientService (service_type,1);
        }
        else if ( (signal->SignalCode & STOP_SERVICE) == STOP_SERVICE)
        {
            if (g_USBService_table[service_type]==NULL)
            {
            ...
            {
                UDC_HWUNInit();
                USBGadgetUnregisterDriver (g_USBService_table[service_type]);
