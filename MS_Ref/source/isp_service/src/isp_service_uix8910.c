...
uint32 ISP_ServiceOpen(uint32 wait_option)
{
    ...
    {
    	SCI_TRACE_LOW("ISP_ServiceOpen timeout errcode:%d", s->err_code);
        if(s->err_code)
        {
...
uint32 ISP_ServiceStartPreview(void)
{
	...
	SCI_TRACE_LOW("david ISP_ServiceStartPreview status:%d",  status);	
...
LOCAL void _ISP_ServiceOpenSignalHandler(uint32 service,uint32 param,void * ptr)
{
	...
	SCI_TRACE_LOW("david 01 _ISP_ServiceOpenSignalHandler send event");
...
LOCAL void _ISP_ServiceCloseSignalHandler(uint32 service,uint32 param,void * ptr)
{
   ...
   _ISP_ServiceDeInit();// event, mutex, semaphore will be released here
	...
	{
    	SCI_TRACE_LOW("david close handler stop xfer!");
...
LOCAL void _ISP_ServiceInit(void)
{
    ...
    SCI_TRACE_LOW("david _ISP_ServiceInit");
...
LOCAL void _ISP_ServiceStartPreview(void)
{
	...
	SCI_TRACE_LOW("david start preview. display size:%u(k) dcam mode = %d YUV size = %d(k)",
...
///----------------------- Uis8910 ------------------------------------
	...
	SCI_TRACE_LOW("david capture _ISP_UixCaptureOneFrame");
...
LOCAL void _ISP_ServiceJPEGHandler(ISP_FRAME_T *p_frame)
{
	...
	SCI_TRACE_LOW("david capture _ISP_ServiceJPEGHandler");
...
PUBLIC uint32 ISP_ServiceRegister(ISP_SERVICE_CALLBACK_ID_E eid, int (*func)(void *))
{
    ...
    SCI_TRACE_LOW("david ISP_ServiceRegister eid=%d func=0x%x", eid, func);
    switch(eid)
    {
...
uint32 ISP_ServiceUnRegister(ISP_SERVICE_CALLBACK_ID_E eid)
{
    ...
    SCI_TRACE_LOW("david ISP_ServiceUnRegister eid=%d", eid);
    switch(eid)
    {
        case ISP_SERVICE_CALLBACK_ENCODE:   
...
void ISP_Uix_LinkEncFrames(void)
{
	...
	SCI_TRACE_LOW("david ISP_Uix_LinkEncFrames head=0x%x", s->uix_frame_enc_head);
}
...
int32 ISP_Uix_setEncAddr(uint32 addr)
{
	...
	SCI_TRACE_LOW("david ISP_Uix_setEncAddr yaddr:0x%x", addr);
...
int ISP_Uix_AllocEncFrame()
{
	...
	SCI_TRACE_LOW("david ISP_Uix_AllocEncFrame. ok Endaddr=0x%x, totalusesize=%d(k)", 
						end, (end-s->uix_base_addr)/1024);
...
int ISP_Uix_AllocFrameAddr()
{
	...
	SCI_TRACE_LOW("david ISP_Uix_AllocFrameAddr. dvmod=%d, dispsize=%d baseaddr=0x%x", 
		g_DcamMode, s->uix_display_size, s->uix_base_addr);
...
LOCAL void Cam_preview_start()
{
	...
	{
		SCI_TRACE_LOW("david PREVIEW 0 getcurframe yaddr NULL!!!");
		return;
	}
...
LOCAL void Cam_preview_process()
{
	...
	SCI_TRACE_LOW("david PREVIEW 1 enter tick:%u deltatime : %u", time, time-s_curtime);
	//屏蔽中断 
	hal_camera_mask_irq(CAM_IRQ_MASK);
    ...
    if (gIsCameraStartXfer == TRUE)
    {
        ...
        {
        	SCI_TRACE_LOW("david PREVIEW 1 _ISP_UixIrqHandler stage2 error!");
            hal_camera_clear_fifo();
	...
	if(!g_stopPreview)
	{
		...
		if(g_pFrame->yaddr == NULL || g_pFrame == NULL)
		{
			SCI_TRACE_LOW("david PREVIEW 1 getcurframe yaddr NULL EXIT.");
            hal_camera_clear_fifo();
			mask.fend = 1;
			//mask.dma  = 1;
...
LOCAL void Cam_snapshot_start()
{
	...
	SCI_TRACE_LOW("david capture stage 0 getcurframe g_pFrame:0x%x, tick:%u capmode:%d", 
								g_pFrame, SCI_GetTickCount(), g_DcamMode);

	if(NULL == g_pFrame->yaddr || NULL == g_pFrame)
	{
		SCI_TRACE_LOW("david capture getcurframe yaddr NULL");
		return;
	}
...
LOCAL void Cam_snapshot_process()
{
    ...
    if (gIsCameraStartXfer == TRUE)
    {
		SCI_TRACE_LOW("david capture stage 1 getcurframe tick:%u g_Capturemode:%d", 
        ...
        {
			SCI_TRACE_LOW("hch, Cam_snapshot_process hal_CameraStopXfer NOT XFER_SUCCESS");
            mask.fend = 1;
            hal_CameraIrqSetMask(mask);
            g_ispmode = CAM_CAPTURE_STAGE_START;
        }
    }
	SCI_TRACE_LOW("david capture stage 1 over tick:%u",SCI_GetTickCount());
}
...
void Cam_mpeg_start()
{
	...
	g_pFrame = hal_camera_getCurFrame();
	SCI_TRACE_LOW("david mpeg stage 1 getcurframe g_pFrame:0x%x", g_pFrame);
	if(g_pFrame->yaddr == NULL)
	{
		SCI_TRACE_LOW("david getcurframe yaddr NULL");
		return;
	}
...
void Cam_mpeg_process()
{
    ...
    if (gIsCameraStartXfer == TRUE)
    {
        ...
        {
        	SCI_TRACE_LOW("david Cam_mpeg_process stopxfer error");
            mask.fend = 1;
            hal_CameraIrqSetMask(mask);
            g_ispmode = CAM_MPEG_STAGE_START;
        }
...
void ISP_Uix_StopPreview()
{
	SCI_TRACE_LOW("david capture set stop preview!");
...
void ISP_Uix_SetBaseaddr(uint32 addr, uint32 size)
{
	...
	s->uix_mem_size  = size;
	SCI_TRACE_LOW("david set baseaddr 0x%x, size %u", addr, size);
}
...
void _ISP_UixIrqHandler(HAL_CAMERA_IRQ_CAUSE_T cause)
{
    ...
    switch (g_ispmode)
    {
        ...
        if(g_stopPreview)
        {
            SCI_TRACE_LOW("david _ISP_UixIrqHandler stop!");
            hal_camera_mask_irq(CAM_IRQ_MASK);
            hal_camera_clear_fifo();
            return;
        }  
...
void ISP_UixSetDcamMode(int mode)
{
	SCI_TRACE_LOW("david ISP_UixSetDcamMode mode=%d", mode);
...
int ISP_UixGetDcamMode()
{
    SCI_TRACE_LOW("david ISP_UixGetDcamMode mode=%d", g_DcamMode);
	return g_DcamMode;
}
...
int ISP_UixOpenCameraContoller()
{
    ...
    SCI_TRACE_LOW("david ISP_UixOpenCameraContoller mode=%d. 1 [%d.%d.%d.%d]", 
                    g_DcamMode, 
    ...
    SCI_TRACE_LOW("david ISP_UixOpenCameraContoller 2 [%d. %d. %d. %d]", 
                    pCamConfig->endianess,
                    pCamConfig->camId,     
                    pCamConfig->vsync_inv, 
                    pCamConfig->spi_pixels_per_line);

    SCI_TRACE_LOW("david ISP_UixOpenCameraContoller 3 [%d. %d. %d. %d. %d]", 
                    pCamConfig->endianess,
                    pCamConfig->camId,     
                    pCamConfig->vsync_inv, 
                    pCamConfig->spi_pixels_per_line,  
    ...
    SCI_TRACE_LOW("david ISP_UixOpenCameraContoller 4 [%d. %d. %d. %d.]", 
                    pCamConfig->spi_yuv_out,          
                    pCamConfig->spi_mode,            
                    pCamConfig->spi_little_endian_en,
                    pCamConfig->spi_ctrl_clk_div   );

    SCI_TRACE_LOW("david ISP_UixOpenCameraContoller 5 [%d. %d. %d. %d]", 
                    pCamConfig->colRatio,  
                    pCamConfig->rowRatio,   
    ...
    SCI_TRACE_LOW("david ISP_UixOpenCameraContoller 6 [%d. %d. %d. %d. %d]", 
...
//  Sensor input 640*480 -> Output 200*200
	...
	SCI_TRACE_LOW("david ISP_UixOpenCamCtrlWatchMode over!");
...
//  Sensor input 640*480 -> Output 320*240
	...
	SCI_TRACE_LOW("david ISP_UixOpenCamCtrlDVMode over!");
...
int ISP_UixOpenCameraController(int open)
{
    ...
    hal_CameraIrqSetMask(mask);	
	SCI_TRACE_LOW("david ISP_UixOpenCameraController over!");
...
LOCAL void _ISP_Uix_ServiceMpegHandler(ISP_FRAME_T *p_frame)
{
 ...
 //   SCI_TRACE_ID(TRACE_TOOL_CONVERT,ISP_SERVICE_SC6530_6586_112_2_18_1_44_7_483,(uint8*)"d",p_frame->flags);
    ...
    //    case ISP_STATE_MPEG_CAPTURE:
            ...
            {
            	SCI_TRACE_LOW("ISP_SERVICE: _ISP_ServiceMpegHandler, encode frame 0x%x not released!", s->uix_frame_enc_cur);
                //ISP_SERVICE_TRACE:"ISP_SERVICE: _ISP_ServiceMpegHandler, encode frame 0x%x not released!"
                //SCI_TRACE_ID(TRACE_TOOL_CONVERT,ISP_SERVICE_SC6530_6687_112_2_18_1_44_8_488,(uint8*)"d",s->p_frame_enc_cur);
            }
	...
	unsigned int height)
{
	...
	SCI_TRACE_LOW("david isp ISP_YUV422_UYVY_Rotate_90 width:%d, height:%d",
		width, height);
...

{
	...
	SCI_TRACE_LOW("david isp ISP_YUV422_UYVY_Rotate_270 width:%d, height:%d",
		width, height);
