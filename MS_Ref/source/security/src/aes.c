...
static const uint8 x2_sbox[256] = {
...
static const uint8 x3_sbox[256] = {
 ...
 */
static void add_round_key(uint32 *state, uint32 *key)
{
	int idx;
...
static void aes_expand_key(uint8 *key, uint8 *expkey)
{
...
static void mix_sub_columns(uint8 *state)
{
	uint8 tmp[4 * AES_STATECOLS];
...
static void aes_encrypt(uint8 *in, uint8 *expkey, uint8 *out)
{
	uint8 state[AES_STATECOLS * 4];
	uint32 round;

	memcpy(state, in, AES_STATECOLS * 4);
	add_round_key((uint32 *)state, (uint32 *)expkey);
	...
	for (round = 1; round < AES_ROUNDS + 1; round++) {
		...
		add_round_key((uint32 *)state,
			      (uint32 *)expkey + round * AES_STATECOLS);
	}

	memcpy(out, state, sizeof(state));
}

static void romapi_set_base(void)
{
...
static void get_chip_unique(struct chip_unique_id *id, int kind)
{
	//EFUSE_PHY1_Open();
...
PUBLIC void rda_encrypt_obj(unsigned char * buf, int len)
{
	...
	romapi_set_base();
	get_chip_unique(&rom_id, UNIQUE_ID_ANTI_CLONE);
	#if 0
	for (i = 0; i < 16; i++) {
	 	//rprintf("id[%d] = %x\n", i, rom_id.id[i]);
	}
	#endif
	aes_expand_key((uint8 *)&rom_id, key_schedule);
	...
	for (i = 0; i < num_aes_blocks; i++) {
		...
		aes_encrypt(src, key_schedule, dst);
