...
PUBLIC void Calc_SHA1_Hash(uint32 *start_data_ptr, uint32 data_len)
{
	SHA1Context_32 sha;
...
LOCAL EFUSE_RETURN_E WriteBootloaderHashValue()
{
	#define NOR_BOOTLOADER_ADDR 	0x0
	#define HASH_LEN_ADD		0x20
	
	EFUSE_RETURN_E result = EFUSE_RESULT_SUCCESS;
	uint32 hashLen = *(volatile uint32 *)(NOR_BOOTLOADER_ADDR + HASH_LEN_ADD);
	
	Calc_SHA1_Hash(NOR_BOOTLOADER_ADDR,hashLen);

	//result = EFUSE_HAL_WriteHash(&soft_harsh_data);
    ...
    )
{
 ...
 #ifdef DUAL_BATTERY_SUPPORT     
        if( SCI_TRUE == HandleDualBatChannelSel(adc_src-1) )
        {
            for (i = 0 ; i < 16 ; i++)
            {
                adc_result += ADC_GetResultDirectly ( (adc_src - 1), ADC_SCALE_1V2);
            }
 ...
 #endif
        {
             if(adc_src -1==ADC_CHANNEL_1 ||adc_src -1==ADC_CHANNEL_6 ||adc_src -1==ADC_CHANNEL_0){
