...
#ifndef FPGA_VERIFICATION
#if defined(FM_SUPPORT) && defined(PLATFORM_SC6531EFM) || defined(PLATFORM_UIX8910)
#ifndef WIN32
    fm_calibration_main();
#endif
#endif
...
#if defined(ATFORM_SC6531EFM) || defined(PLATFORM_UIX8910)
#ifndef WIN32
...
#if 1
#if (defined(PLATFORM_SC6531EFM) || defined(PLATFORM_UIX8910)) && (!defined(WIN32))
    if(RESTART_BY_NONE == POWER_GetRestartCondition() \
		|| (CHGMNG_ADP_USB == CHGMNG_IdentifyAdpType()))
#else
    if(RESTART_BY_NONE == POWER_GetRestartCondition())
	...
	SCI_InitRef(0);	
	 ...
	 NV_IPC_init();
        ...
        else
        {
		    ...
		    CHIP_Disable7sRstMode();      //disable 7s rst  ZDT_PRODUCT_Z685S
