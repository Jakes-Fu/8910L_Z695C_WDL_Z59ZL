...
#else
#define  RM_VCOM_MAX_WAIT_TIME      15000    /* wait max 15s for PC response when virtual com is used(ms)*/
#define  RM_VCOM_INITEND_MAXTIME    5000     /* max usb virtual com enumeration time (ms)*/        
#endif
...
PUBLIC BOOLEAN enable_deep_sleep = 0xFF;	/*lint -esym(765,enable_deep_sleep)*/	// Must be initlized as 0xFF, it will be set in run_mode.c
...
//                  SCI_FALSE: Disable
/*****************************************************************************/
PUBLIC BOOLEAN RM_GetEnableArmSleepFlag(void)   // Return the flag.
{
...
PUBLIC void RM_SetEnableArmSleepFlag(
	BOOLEAN is_deep_sleep			//deep sleep enable/disable flag
	)
{
...
#else
    SIO_InitUartLowLevel(UART_COM2, BaudRate );
#endif
}
...
PUBLIC void RM_GetRunModeResponse(void)
{
...
#else
        count = *(volatile uint32*)(RM_UART+0x20) & 0xFF;
#endif
	...
	if( count >= RM_FIXED_PROTO_LEN )
	{
		...
		{////receive first command in CALIBRATION_NV_ACCESS_MODE
			...
			{
				while((fifo_data_len =*(volatile uint32*)(RM_UART+0x0C) & 0xFF)==0){};
