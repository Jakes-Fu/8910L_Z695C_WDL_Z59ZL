                                   ...
                                   uint16 src_len)
{
    ...
    switch (msg_head->subtype)
    {
        ...
        case CURRENT_TEST_DEEP_SLEEP:
           ...
           SCI_MEMCPY(p_calibData, 
                                  (uint16 *)(src_ptr + sizeof(MSG_HEAD_T)),
                                   reqLen);
           ...
           NV_IPC_sendCmdToCp(cmdId, NV_CMD_CALIB, (uint32)p_calibData, (msg_head->len - sizeof(MSG_HEAD_T)));
        	...
        	while (g_usb_plugout_flag)
			{
				...
				mon_Event(__LINE__);
				mon_Event(usb_cnt);
//				LPS_Debug_Cnt = usb_cnt;
//				SCI_TRACE_LOW("Slp test usb plug out, count %d\n", usb_cnt);
//				fflush(0);
				SCI_Sleep(100);
			}
...
//            LPS_Debug_Step = __LINE__;
#if 1
			//移除串口输出功能保存恢复机制 确保有跟踪板也能睡眠
			//开机启动有Log输出 未读出无法睡眠 
			//所以准备底电流测试时移除串口Log输出保存恢复机制
...
#else
           *(volatile uint32*)0x8b000180 = 0x0000a1b2;
           *(volatile uint32*)0x8b0000ac |= 0x4;
#endif
			HAL_TurnOffPower();
            //lint -e716
            while (1)
