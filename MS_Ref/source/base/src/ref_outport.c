...
#if 0/*dummy here tp enable assert debug without charger on  */      
        if(!CHR_IsChargeConnect())
        {
            /*
            *  usb is pulled down, data got from uart 
            */
            if(!s_usb_is_reconfiged)
            {
                s_usb_is_reconfiged = TRUE;
                VCOM_usb_boot();
            }
...
//            s_usb_is_reconfiged = FALSE;
            ...
            if (VCOM_DEV_NO_PRESENT == VCOM_GetDevState())
            {
                if (CHGMNG_IsChargeConnect())
                {
                    ...
                    while ((new_tick - old_tick) < 3000) // wait 3s
                    {
...
#if defined(PLATFORM_UIX8910)
        {
            if (*(volatile uint32*)(0x5010464c) == 0x5ee5e55e) {
                retval = '#';
                *(volatile uint32*)(0x5010464c) = 0x0;
            }
        }
#endif
...
void OS_SendRemainLogMessage(void)
{
    ...
    if((VIR_COM0 != phyport)&&(VIR_COM1 != phyport))
    {
        ...
        * else do null
        */
#if !defined(PLATFORM_UIX8910) //temp_stub_sgq
       VCOM_usb_boot();
#endif
    }
#endif
...
{
    uint32  i;
    volatile uint32  *reg_ptr = NULL;
    uint32  seq_num;
    uint32 reg_group_len = 0; //reg_group_len is length of one register group in byte.      
    uint32 reg_group_num = CHIP_GetNumCtlRegGrp();
    CTL_REG_INFO_T *CtlRegInfoPtr = (CTL_REG_INFO_T *)CHIP_GetInfoOfCtlReg();
    CTL_REG_INFO_T *CtlRegInfoPtr_temp = NULL;
    uint32 dump_count =0;
    uint32 num, packet_num,residual_num;
    //dump number of registers group            
    //head.
    seq_num = s_mem_seq_num;
    ...
    dump_count = 4;
    // update and print Head info.
    _OS_UpdateDumpHeader(++seq_num, dump_count);        
        
    //data.
    PutUint32(reg_group_num); 
    //tail
    OS_DebugPutChar(0x7E); 

#if defined(PLATFORM_UIX8910)
        CtlRegInfoPtr_temp = CtlRegInfoPtr;
	//if modify the CHIP_GetInfoOfCtlReg
	 while(CtlRegInfoPtr_temp->reg_address_begin!=0xffffffff)
	 {
		#ifdef BLUETOOTH_SUPPORT_SPRD_BT
	       // if BT core is false , modify the BT_RISCV_RAM_START to 0x(uint32)CHIP_GetInfoOfCtlReg() && (SCI_FALSE == BT_GetState()  
	       if((BT_RISCV_RAM_START == CtlRegInfoPtr_temp->reg_address_begin) && (SCI_FALSE == BT_IsEnabled()))
	       {
	       	CtlRegInfoPtr_temp->reg_address_begin = (uint32)CtlRegInfoPtr;
			CtlRegInfoPtr_temp->reg_address_end = CtlRegInfoPtr_temp->reg_address_begin + 4;	   
	       }

		// if BT core is false , modify the BT_RISCV_RAM_START to 0x(uint32)CHIP_GetInfoOfCtlReg() 
		if((BT_RISCV_EM_START == CtlRegInfoPtr_temp->reg_address_begin) && (SCI_FALSE == BT_IsEnabled()))
		{
			CtlRegInfoPtr_temp->reg_address_begin = (uint32)CtlRegInfoPtr;
			CtlRegInfoPtr_temp->reg_address_end = CtlRegInfoPtr_temp->reg_address_begin + 4; 	
		}
...
#endif	
    //length of register value,register value.
        
    while(CtlRegInfoPtr->reg_address_begin!=0xffffffff)
    {
        if ((CtlRegInfoPtr->reg_address_begin >=  MODEM_ZITCM_START)
            && (CtlRegInfoPtr->reg_address_begin <=  MODEM_ZAXIDMA_START)) {
            CONFIRM_POWER_DOMAIN_TURNON_POLL(ZSP_PWR);
            if (((MODEM_ZDTCM_START == CtlRegInfoPtr->reg_address_begin)
                || (MODEM_ZITCM_START == CtlRegInfoPtr->reg_address_begin))
                && !(hwp_clkrst->RSTCTRL_ZSP & 0x01)) {
                hwp_sysreg->ZSP_SVTADDR = 0x10000000;
                hwp_clkrst->RSTCTRL_ZSP |= 0x01; //释放CORE
            }
        } else if ((CtlRegInfoPtr->reg_address_begin >=  MODEM_RXTX_START)
            && (CtlRegInfoPtr->reg_address_begin <=  MODEM_RXCAP_START)) {
            CONFIRM_POWER_DOMAIN_TURNON_POLL(ZSP_PWR);
            CONFIRM_POWER_DOMAIN_TURNON(LTE_PWR);
        } else if ((CtlRegInfoPtr->reg_address_begin >=  MODEM_BBAPB_START)
            && (CtlRegInfoPtr->reg_address_begin <=  MODEM_BCPUROM_START)) {
            CONFIRM_POWER_DOMAIN_TURNON(GGE_PWR);
        } else if ((CtlRegInfoPtr->reg_address_begin >=  BT_RISCV_RAM_START)
            && (CtlRegInfoPtr->reg_address_begin <=  BT_RISCV_EM_START)) {
            CONFIRM_POWER_DOMAIN_TURNON(BTFM_PWR);
        }


        reg_group_len = (CtlRegInfoPtr->reg_address_end - CtlRegInfoPtr->reg_address_begin + 4);//uint is byte

        OS_DebugPutChar(0x7E);
        
        // update and print Head info.
        _OS_UpdateDumpHeader(++seq_num, 8);        
        
        //data
        dump_count = dump_count + 4;
        PutUint32(CtlRegInfoPtr->reg_address_begin); //the start address of the reg group

        dump_count = dump_count + 4;
        PutUint32(reg_group_len); //the length of the reg group     

        OS_DebugPutChar(0x7E);
        //dump value of regiser group
        reg_ptr = (volatile uint32  *) CtlRegInfoPtr->reg_address_begin;                
        
        packet_num = (reg_group_len + DUMP_MEM_PACKET_SIZE - 1)/ DUMP_MEM_PACKET_SIZE;
        residual_num =  reg_group_len % DUMP_MEM_PACKET_SIZE;
        if((residual_num % 4) !=0)
        {
            residual_num = ((residual_num + 3) >> 2)<<2; 
        }
        ...
        for (num = 0; num < (packet_num - 1); num++)
        {
            ...
            // Data.
            for (i = 0; i < DUMP_MEM_PACKET_SIZE; i += 4)
            {
                uint32 data = DumpReadReg((uint32)(reg_ptr));
                dump_count = dump_count + 4;
                PutUint32(data);    
                reg_ptr++;
            }
        ...
        OS_DebugPutChar(0x7E);//end of one group registers data 
        //end to transfer
...
PUBLIC void OS_DumpMemory(void)
{
    ...
    uint32  dump_len = 0;
    uint32  i;
    uint32 sdram_length          = MEM_GetDumpLen(); 
    //uint32 shared_mem_length     = CHIP_GetSharedMemLen();
    uint32 inter_ram_length      = CHIP_GetInterRamLen();
  #if defined(PLATFORM_UIX8910)
     uint32 bcpu_sram_length = CHIP_GetBcpuSramLen();
  #endif
     uint32 sdram_begin_addr      = MEM_GetDumpBaseAddr();
    //uint32 shared_mem_begin_addr = CHIP_GetSharedMemBase();
    uint32 inter_ram_begin_addr  = CHIP_GetInterRamBase();
  #if defined(PLATFORM_UIX8910)
     uint32 bcpu_sram_begin_addr = CHIP_GetBcpuSramBase();
  #endif
    uint32 size_of_dump_mem = 0;
#if defined(PLATFORM_UIX8910)
     T_MEM_RELAY_ADDRESS_AREA *pt_ip_reg_dump_save_addr=NULL;
    CTL_REG_INFO_T *CtlRegInfoPtr = NULL;
#endif


#if defined(PLATFORM_UIX8910)  // be used to recover ip register  
   CtlRegInfoPtr = (CTL_REG_INFO_T *)CHIP_GetInfoOfCtlReg();
    pt_ip_reg_dump_save_addr = GET_IPRegisterDumpAreaSaveAddr();
   if(pt_ip_reg_dump_save_addr)
   {
      pt_ip_reg_dump_save_addr->ap_dump_ip_register_save_base = (sdram_begin_addr+sdram_length+inter_ram_length+bcpu_sram_length+DUMP_MEM_PACKET_SIZE);
      pt_ip_reg_dump_save_addr->ap_dump_ip_register_base=CtlRegInfoPtr->reg_address_begin;
      pt_ip_reg_dump_save_addr->ap_dump_ip_register_end=CtlRegInfoPtr->reg_address_end; 
   }
   SCI_PrintAssertString("\npt_ip_reg_dump_save_addr:0x%x\n",pt_ip_reg_dump_save_addr);
   ...
   SCI_PrintAssertString("\nRAM_RW Region:Offset=0x%08X, Length=0x%08X", 0x00, (int)sdram_length);
   
   DumpMemSection(sdram_begin_addr, sdram_length); 
   size_of_dump_mem=sdram_length;

   // Dump internal memory data.
   SCI_PrintAssertString("\nInter Ram Memory:Offset=0x%08X, Length=0x%08X", (int)size_of_dump_mem, (int)inter_ram_length);
   DumpMemSection(inter_ram_begin_addr, inter_ram_length);
   size_of_dump_mem=size_of_dump_mem+inter_ram_length;

  #if defined(PLATFORM_UIX8910)
   // Dump BCPU internal memory data.
   SCI_PrintAssertString("\nBCPU Inter Ram Memory:Offset=0x%08X, Length=0x%08X", (int)size_of_dump_mem, (int)bcpu_sram_length);
   DumpMemSection(bcpu_sram_begin_addr, bcpu_sram_length);
   size_of_dump_mem=size_of_dump_mem+bcpu_sram_length;
  #endif
    /* dump thread info */
    SCI_PrintAssertString("\nThreadInfo:Offset=0x%08X, Length=0x%08X", (int)size_of_dump_mem, (int)DUMP_MEM_PACKET_SIZE);
    DumpThreadInfo();
    size_of_dump_mem=size_of_dump_mem+DUMP_MEM_PACKET_SIZE;
...
#if defined(GPS_SUPPORT) && defined(GPS_CHIP_VER_GREENEYE2)
   // Dump GE2 memory data.
   dump_len = OutputGe2Memory(GPS_Get_Uartport());
   SCI_PrintAssertString("\nGE2 Memory:Offset=0x%08X, Length=0x%08X", (int)size_of_dump_mem,(int)dump_len);
   size_of_dump_mem=size_of_dump_mem+dump_len;  
#endif
  ...
  #if !defined(PLATFORM_UIX8910)
   //dump shared memory
   SCI_PrintAssertString("\nShare Memory:Offset=0x%08X, Length=0x%08X", (int)size_of_dump_mem, (int)CHIP_GetSharedMemLen());
   DumpSharedMem();
   size_of_dump_mem=size_of_dump_mem+CHIP_GetSharedMemLen();
  
   // Dump dsp memory
   dump_len = DumpDSPMem();  
   SCI_PrintAssertString("\nDSP Memory:Offset=0x%08X, Length=0x%08X", (int)size_of_dump_mem, (int)dump_len); 
   size_of_dump_mem=size_of_dump_mem+dump_len;  
  #endif
  ...
  #if !defined(PLATFORM_UIX8910)
   dump_len = DumpIRAMDsp();
   SCI_PrintAssertString("\nIRAM Dsp:Offset=0x%08X, Length=0x%08X", (int)size_of_dump_mem, (int)dump_len);
   size_of_dump_mem = size_of_dump_mem+dump_len;
    #endif
   ...
   // Dump BT PSRAM memory
   SCI_PrintAssertString("\nBT PSRAM memory:Offset=0x%08X, Length=0x%08X", (int)size_of_dump_mem, (int)CHIP_GetBTCodeSize());	
   DumpMemSection(CHIP_GetBTCodeAddr(), CHIP_GetBTCodeSize());
   size_of_dump_mem=size_of_dump_mem+CHIP_GetBTCodeSize();

   // Dump BT IRAM memory data.
   *(volatile uint32*)ARM7_TOP_CTL1 |= 0x8001c; //Switch BT IRAM to arm9
   SCI_PrintAssertString("\nBT IRAM Memory:Offset=0x%08X, Length=0x%08X", (int)size_of_dump_mem, (int)CHIP_GetBTIRamLen());
   DumpMemSection(CHIP_GetBTIRamBase(), CHIP_GetBTIRamLen());
   size_of_dump_mem=size_of_dump_mem+CHIP_GetBTIRamLen();

   // Dump BT share memory data.
   SCI_PrintAssertString("\nBT share Memory:Offset=0x%08X, Length=0x%08X", (int)size_of_dump_mem, (int)CHIP_GetBTShareRamLen());
   DumpMemSection(CHIP_GetBTShareRamBase(), CHIP_GetBTShareRamLen());
   size_of_dump_mem=size_of_dump_mem+CHIP_GetBTShareRamLen();
#endif
    ...
    SCI_PrintAssertString( "\nMemory Dumping Finished:begin addr=0x%08X,total size=%ldByte(0x%08x)\n",
        (int)sdram_begin_addr,size_of_dump_mem, size_of_dump_mem);
#ifdef CARD_LOG
    OS_DumpLogSaveBuf();
#endif
#endif
...
void osiReset(osiResetMode_t mode)
{
    ...
    {
        reset_cause.v = hwp_sysCtrl->Reset_Cause;//hwp_sysCtrl->Reset_Cause;
