...
LOCAL const SCI_DATE_T s_sys_default_date =
    {23,  9,  2020,  3};
/**---------------------------------------------------------------------------*
 **                         Local Variables                                  *
 **---------------------------------------------------------------------------*/
  ...
  TB_RTC_TIME_T  rtc_time;
    ...
    if (PNULL != date_ptr)
    {
      ...
      //if current rtc day is more than the max value,then restore to default value
        ...
        {
        TM_SetSysDate(s_sys_default_date);
        s_is_rtc_abnormal = TRUE;
      }
...
{
        SCI_MEMCPY( ( void * )date_ptr, ( void * )&s_sys_default_date, sizeof( SCI_DATE_T ) );
        return ERR_TM_NONE;
}
...
LOCAL  void TM_HandleAbnormRTC(void)
{
  ...
  if(NVERR_NONE != ret)
  {
        ...
        // Saved s_last_date to NV.
        if(NVERR_NONE != EFS_NvitemWrite(NV_ORIGINAL_DATE, sizeof(SCI_DATE_T), (uint8 *)&s_sys_default_date, 1))
        {
