...
static uint32 test_point_offset = 0;
    ...
    #ifdef TM_LOADING_TRACK	
    	uint16 task_start_time = 0;
    	uint16 uart_time_dur = 0;
    	uint16 uart_time_dur_t = 0;
    	uint16 uart_time_start = 0;
    	uint16 uart_time_t = 0;
    	uint16 idle_time_t = 0;
    	uint64 uart_time = 0;
    	uint64 idle_time = 0;
    	uint16 idle_time_start = 0;
    	BOOLEAN GPRS_loading_flag = TRUE;
    	TX_THREAD * last_thread_ptr;
    #endif

#else
	#ifdef TM_DSP_TRACK
		extern BOOLEAN g_is_dsp_ready;	
	#endif
#endif	
...
#else
	/* task information */
	tm_info.mem_tab[1].start_addr = 0xFFFFFFFF; /* invalid */
	tm_info.mem_tab[1].size = DUMP_MEM_PACKET_SIZE;
	tm_info.mem_tab[1].offset = tm_info.mem_tab[0].offset + tm_info.mem_tab[0].size;
	
	/* shared memory */
	tm_info.mem_tab[2].start_addr = CHIP_GetSharedMemBase();
	tm_info.mem_tab[2].size = CHIP_GetSharedMemLen();
	tm_info.mem_tab[2].offset = tm_info.mem_tab[1].offset + tm_info.mem_tab[1].size;
#endif
	/* internal ram */
...
{    
        ...
        if(SCI_ASSERT_DEBUG_MODE == assert_mode)
        {
    		    ...
    		    if (GPRS_loading_flag == TRUE)
    		    {
                    ...
                    {
                        #if defined(PLATFORM_UIX8910)
                        test_point[i].id           = (uint16) TM_IDLE_REQ;
                        test_point[i].time         = SLEEP_GetCur32kNum();
                        test_point[i].acuracy_time = 0; 
                        test_point[i].pointer      = (void *) irq_ie_state;
                        test_point[i].return_addr  = (void *) 0;
						#else
                        test_point[i].id           = (uint16) TM_IDLE_REQ;
                        test_point[i].acuracy_time = (uint16)TIMER_GetCntValue(TIMER_1); 
                        test_point[i].pointer      = (void *) irq_ie_state;
                        test_point[i].return_addr  = (void *) 0;
                        test_point[i].time         = SCI_GetTickCount();
						#endif						
            #ifdef TM_LOADING_TRACK		
                   		task_start_time            = task_start_time - test_point[i].acuracy_time - uart_time_dur;
                   		last_thread_ptr->time      += task_start_time ;
               	    	uart_time_dur = 0;
            		
           	    	    idle_time_start = test_point[i].acuracy_time;
            		
               	    	GPRS_loading_flag = FALSE;
                   	}
   	        	}
   		    SCI_RESTORE_IRQ
       		#else
	...
	#else
    uint32 SleepModeValue = SCI_Getsleepmode();
		#ifdef TM_DSP_TRACK
		    if(g_is_dsp_ready == TRUE)
		    {
		        while (!cmd_ptr->RefCmd_block.cmd_done);
		        cmd_ptr->RefCmd_block.cmd_done  = 0; 
		        cmd_ptr->RefCmd_block.Cmd_ID    = (uint16) TM_IDLE_REQ; 
		       	cmd_ptr->RefCmd_block.reserved1 = (uint16) (SleepModeValue >> 16);
		        cmd_ptr->RefCmd_block.reserved2 = (uint16) (SleepModeValue & 0x0ffff);
...
void * TM_SendTaskRequest(void *thread_ptr)
{
        ...
        if(SCI_ASSERT_DEBUG_MODE == assert_mode)
        {
            ...
            {
                #if defined(PLATFORM_UIX8910)
                test_point[i].id           = (uint16) TM_TASK_REQ;
                test_point[i].time         = SLEEP_GetCur32kNum();
                test_point[i].acuracy_time = 0; 
                test_point[i].pointer      = thread_ptr;
                test_point[i].return_addr  = (void *) 0;
				#else
                test_point[i].id           = (uint16) TM_TASK_REQ;
                test_point[i].acuracy_time = (uint16)TIMER_GetCntValue(TIMER_1); 
				...
				#endif
            #ifdef TM_LOADING_TRACK	
                task_start_time            = test_point[i].acuracy_time;   
            	last_thread_ptr            = (TX_THREAD *) thread_ptr;  
    	        idle_time_t = idle_time_start - test_point[i].acuracy_time - uart_time_dur ;
            	idle_time += idle_time_t; 
            	uart_time_dur = 0;
    	        GPRS_loading_flag = TRUE;  
            #endif
            }
        }
    #else
	    ...
	    if(g_is_dsp_ready == TRUE)
	    {
	        while (!cmd_ptr->RefCmd_block.cmd_done);
	        cmd_ptr->RefCmd_block.cmd_done  = 0; 
	        cmd_ptr->RefCmd_block.Cmd_ID    = TM_TASK_REQ; 
	        cmd_ptr->RefCmd_block.reserved1 = (uint16) (((uint32) thread_ptr) >> 16);
	        cmd_ptr->RefCmd_block.reserved2 = (uint16) (((uint32) thread_ptr) & 0x0ffff);
	        *(volatile uint32 *) (0x8b000010) |= 0x8;
	    }
	    #endif
    #endif
    ...
    // NULL
        ...
        if(SCI_ASSERT_DEBUG_MODE == assert_mode)
        {
        	...
        	if ( i < MAX_SWITCH_NUM)
	        {
                ...
                test_point[i].time         = SLEEP_GetCur32kNum();
                test_point[i].acuracy_time = 0; 
            	test_point[i].pointer     = (void *) irq_status;
        	    test_point[i].return_addr = (void *) _tx_pc_val_before_irq;		
				#else
    	        test_point[i].id          = id;
        	    test_point[i].acuracy_time = (uint16)TIMER_GetCntValue(TIMER_1); 
            	test_point[i].pointer     = (void *) irq_status;
				...
				#endif
                #ifdef TM_LOADING_TRACK	
    	    	    if (id == TM_ENTER_INT_REQ)
        		    {
        			    uart_time_start =  test_point[i].acuracy_time;
            		}
        	    	else if (id == TM_LEAVE_INT_REQ)
    	    	    {
    		    	    uart_time_dur_t = uart_time_start - test_point[i].acuracy_time;
        			    uart_time_dur += uart_time_dur_t;
            			//SCI_TRACE_LOW("uart_time_dur_t=%d %d %d",uart_time_dur_t,test_point[i].acuracy_time,uart_time_start);
            			uart_time += uart_time_dur_t;
    	        	}
    		        else
            		{
		    ...
		    if(g_is_dsp_ready == TRUE)
		    {
		        while (!cmd_ptr->RefCmd_block.cmd_done);
		        cmd_ptr->RefCmd_block.cmd_done  = 0; 
		        cmd_ptr->RefCmd_block.Cmd_ID    = id; 
		        cmd_ptr->RefCmd_block.reserved1 = (uint16) (irq_status >> 16);
		        cmd_ptr->RefCmd_block.reserved2 = (uint16) (irq_status & 0x0ffff);
		        *(volatile uint32 *) (0x8b000010) |= 0x8;
		    }
		#endif
	#endif
...
void TM_SendSleepRequest(uint8 id)
{
        ...
        if(SCI_ASSERT_DEBUG_MODE == assert_mode)
        {
            ...
            {		
                ...
                test_point[i].time         = SLEEP_GetCur32kNum();
                test_point[i].acuracy_time = 0; 
        	    test_point[i].pointer     = 0;
        	    test_point[i].return_addr = 0;
				#else
        	    test_point[i].id          = id;
            	test_point[i].acuracy_time = (uint16)TIMER_GetCntValue(TIMER_1); 
        	    test_point[i].pointer     = 0;
	        ...
	        if(g_is_dsp_ready == TRUE)
	        {
	            while (!cmd_ptr->RefCmd_block.cmd_done);
   		        cmd_ptr->RefCmd_block.cmd_done  = 0; 
    	        cmd_ptr->RefCmd_block.Cmd_ID    = id; 
	            cmd_ptr->RefCmd_block.reserved1 = 0;
	            cmd_ptr->RefCmd_block.reserved2 = 0;
	            *(volatile uint32 *) (0x8b000010) |= 0x8;
   		    }
    	#endif
    #endif
...
void TM_SendTestPointRequest(uint32 param1, uint32 param2)
{
    ...
    //null
        ...
        if(SCI_ASSERT_DEBUG_MODE == assert_mode)
        {
	        ...
	        {	
                #if defined(PLATFORM_UIX8910)
    	        test_point[i].id          = TM_TEST_POINT_REQ;
                test_point[i].time         = SLEEP_GetCur32kNum();
                test_point[i].acuracy_time = 0; 
...
LOCAL void TM_SendTestPointIDRequest(uint8 id, uint32 param1, uint32 param2)
{
    #ifdef TM_TASK_MONITOR
        uint8 assert_mode;
        OSPARAM_GetAssertMode(&assert_mode);
...
{
    #if defined TM_TASK_MONITOR && defined(PLATFORM_UIX8910)
	uint8      v_taskNameLen;
    uint32     i;
