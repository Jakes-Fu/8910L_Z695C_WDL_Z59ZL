			...
			switch(device_info->type)
			{
				case PRODUCT_CODE_T://product code contained in the NDT conifg in nv, which support read only, return directly
					...
					{	
						...
						SCI_TraceLow("PRODUCT CODE WRITE PASS");
						
					}
					else
					{
						write_res.status=ret;//failed
						SCI_TraceLow("PRODUCT CODE WRITE FAILED");
					}
					break;
				default:
					ret=ERR_NOT_ALLOWED;
		...
		case DEVICE_INFO_READ:
			switch(device_info->type)
			{
				...
				case PRODUCT_TYPE_T:
					...
					SCI_TraceLow("PRODUCT TYPE READ PASS");
					break;
				case PRODUCT_CODE_T://wait for nv id
					...
					{	
						read_res.status=0;//success
						SCI_TraceLow("PRODUCT CODE READ PASS");
						
					}
					else
					{
						read_res.status=ret;//failed
						SCI_TraceLow("PRODUCT CODE READ FAILED");
					}
					break;
				case VERISON_T:
                    str = VERSION_Read();
					...
					read_res.status=0;//success
					SCI_TraceLow("VERSION READ PASS");
					break;	
				case PRODUCT_NAME_T:
					...
					SCI_TraceLow("PRODUCT NAME READ PASS");
					break;
				default:
					ret=ERR_NOT_ALLOWED;
					read_res.status=ret;//failed
