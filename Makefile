...
ifeq ($(strip $(FLASH_SIZE)), 128MBIT)
XML_FILE = UIX8910_128MBIT
PAC_FILE = pac_UIX8910_new.pl
endif

ifeq ($(strip $(FLASH_SIZE)), 128MBITX64MBIT_OLD)
XML_FILE = UIX8910_128MBITX64MBIT_old
PAC_FILE = pac_UIX8910_128MBITX64MBIT_old.pl
endif

ifeq ($(strip $(FLASH_SIZE)), 128MBITX64MBIT_NEW)
XML_FILE = UIX8910_128MBITX64MBIT_new
PAC_FILE = pac_UIX8910_new.pl
endif
...
ifeq ($(strip $(PLATFORM)),	UIX8910)
ALLNV	= $(shell $(CAT) $(call Path_process,$(BUILD_DIR)/img/NV_GROUP$(if $(strip $(PID_LIST)),_$(strip $(PID_LIST))).txt))
endif
endif
...
ifeq ($(strip $(USE_LZMA_UZIP_BIN)),TRUE)
	-$(LZMABIN) lib/modem/$(strip $(MODEM_BIN_FILE))/cp.bin  65536
	-$(LZMABIN) lib/modem/$(strip $(MODEM_BIN_FILE))/zsp.bin  65536
else
...
ifeq ($(strip $(PLATFORM)),	UIX8910)
	...
	@$(PERL) make/perl_script/mk_dir.pl "./lib/modem/$(MODEM_BIN_FILE)" ""
endif
...
ifneq ($(strip $(VARIANT_LP_LINUX)),)
	@$(COPY_FILE) $(call Path_process,lib/modem/$(strip $(MODEM_BIN_FILE))/$(strip $(MMI_RES_DIR))_$(VARIANT_LP_LINUX).bin) $(call Path_process,$(BUILD_DIR)/img/) 
else
	@$(COPY_FILE) $(call Path_process,lib/modem/$(strip $(MODEM_BIN_FILE))/$(strip $(MMI_RES_DIR)).bin) $(call Path_process,$(BUILD_DIR)/img/) 
endif
ifeq ($(strip $(MIDI_SUPPORT)), TRUE)
   ifeq ($(strip $(SOUNDBANK_MODE)),COMPACT)
	$(COPY_FILE)  $(call Path_process,lib/modem/$(strip $(MODEM_BIN_FILE))/midi_soundbank_compact.c)  $(call Path_process,$(BUILD_DIR)/tmp/)
   endif
endif
endif
...
ifeq ($(strip $(PLATFORM)),	UIX8910)
	-$(COPY_FILE) $(call Path_process, MS_Customize\$(strip $(PLATFORM))_normal.scf  $(BUILD_DIR)\tmp\$(strip $(PLATFORM))_normal.scf)
	-$(COPY_FILE) $(call Path_process, MS_Customize\$(strip $(PLATFORM))_multi.scf   $(BUILD_DIR)\tmp\$(strip $(PLATFORM))_multi.scf)
...
ifeq ($(strip $(PLATFORM)),	UIX8910)
	-${MAKEIMAGE} -A arm -T standalone -C none -a 0x800100 -e 0x800100 -n nor_fdl1.img  -d $(call Path_process,$(BUILD_DIR)/img/nor_fdl1.bin) $(call Path_process,$(BUILD_DIR)/img/nor_fdl1.img)
endif
...
## -res is used for BZP OK, when unzip isn't used
	-$(BZP) $(BZP_CMD)  \
	-ps $(call Path_process,$(BUILD_DIR)/img/$(IMGFILENAME)/EXEC_FLASH_B_IMAGE) \
	-res  $(call Path_process,lib/modem/$(strip $(MODEM_BIN_FILE))/memory_index_list.bin) \
	-ext0 $(call Path_process,lib/modem/$(strip $(MODEM_BIN_FILE))/memory_index_list.bin) \
	-ext1 $(call Path_process,lib/modem/$(strip $(MODEM_BIN_FILE))/$(strip $(CPIMG))) \
	-ext2 $(call Path_process,lib/modem/$(strip $(MODEM_BIN_FILE))/$(strip $(ZSPIMG))) \
	-ext3 $(call Path_process,lib/modem/$(strip $(MODEM_BIN_FILE))/bcpu_gsm.bin) \
	-ext4  $(call Path_process,lib/modem/$(strip $(MODEM_BIN_FILE))/riscV.bin) \
...
## -res is used for BZP OK, when unzip isn't used
	...
	@$(PERL) make/perl_script/exist_file.pl $(call Path_process,$(BUILD_DIR)/img/$(strip $(MMI_RES_DIR))$(if $(strip $(PID_LP)),_$(strip $(PID_LP))).bin)   >> $(BUILD_DIR)/tmp/error.log	
	@$(PERL) make/perl_script/exist_file.pl $(call Path_process,$(BUILD_DIR)/img/$(IMGFILENAME).bin)   >> $(BUILD_DIR)/tmp/error.log
...
ifeq  ($(strip $(SECURE_BOOT_SUPPORT)), TRUE)
	@-$(PERL) make/perl_script/rm_file.pl $(PROJECT)$(if $(strip $(PID_LIST)),_$(strip $(PID_LIST)))_signed.pac $(BUILD_DIR)/img/
...
ifneq ($(strip $(SECURE_BOOT_MAKE_UNSIGNED_PACK)), TRUE)
	-${VLRSIGN} -pw 12345678 -pn test  -ha SHA1-32 -ipbk true -pw2 12345678 -pn2 test -img $(call Path_process,$(BUILD_DIR)/img/nor_fdl.bin) -out $(call Path_process,$(BUILD_DIR)/img/nor_fdl_signed.bin)
	-${VLRSIGN} -pw 12345678 -pn test -plen 0xbce0 -ha Blake2 -img $(call Path_process,$(BUILD_DIR)/img/nor_bootloader.img) -out $(call Path_process,$(BUILD_DIR)/img/nor_bootloader_signed.img)
	-${VLRSIGN} -pw 12345678 -pn test -plen 0xbce0 -ha Blake2 -img $(call Path_process,$(BUILD_DIR)/img/nor_fdl1.img) -out $(call Path_process,$(BUILD_DIR)/img/nor_fdl1_signed.img)
	-${VLRSIGN} -pw 12345678 -pn test  -ha SHA1-32 -slen 0x100000 -img $(BUILD_DIR)/img/$(IMGFILENAME)_stone$(if $(strip $(PID_LP)),_$(strip $(PID_LP))).bin -out $(BUILD_DIR)/img/$(IMGFILENAME)_stone$(if $(strip $(PID_LP)),_$(strip $(PID_LP)))_signed.bin
endif
else
...
ifneq ($(wildcard lib/modem/$(strip $(MODEM_BIN_FILE))/*.bin),"")
	$(PERL) make/perl_script/mk_dir.pl "$(BUILD_DIR)/img/$(strip $(MODEM_BIN_FILE))"
	$(COPY_DIR)  $(call Path_process,lib/modem/$(strip $(MODEM_BIN_FILE)))  $(call Path_process,$(BUILD_DIR)/img/$(strip $(MODEM_BIN_FILE)))	
...
ifneq ($(strip $(SECURE_BOOT_MAKE_UNSIGNED_PACK)), TRUE)
	...
	$(BUILD_DIR)/img/nor_fdl_signed.bin \
	$(BUILD_DIR)/img/$(ALLNV).bin \
	$(BUILD_DIR)/img/nor_bootloader_signed.img \
	$(BUILD_DIR)/img/$(IMGFILENAME)_stone$(if $(strip $(PID_LP)),_$(strip $(PID_LP)))_signed.bin \
