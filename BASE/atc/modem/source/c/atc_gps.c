        ...
        case APP_MN_GPS_DOWNLOAD_CNF:
        {
            ...
            if(TRUE == param_ptr->is_ok)
            {
                if((ATC_SPGPSPWR == gps_config.gps_cmd_type) ||
                    ...
                    (ATC_CGNSPWR== gps_config.gps_cmd_type))
                {
                    MNGPS_InitReq(dual_sys);

                    ATC_ADD_EXPECTED_EVENT_MUX(atc_config_ptr, 
                                  APP_MN_GPS_INIT_CNF, 0xff, ATC_NO_DOMAIN, dual_sys);
                }
        ...
        case APP_MN_GPS_INIT_CNF:
        {
            ...
            if(TRUE == param_ptr->is_ok)
            {
                    ...
                    (ATC_CGNSPWR== gps_config.gps_cmd_type))
                {
                    ...
                    ATC_ADD_EXPECTED_EVENT_MUX(atc_config_ptr, 
                                      APP_MN_GPS_START_CNF, 0xff, ATC_NO_DOMAIN, dual_sys);
                }
                else
                {
        ...
        case APP_MN_GPS_RESET_CNF:
        {
            ...
            if(gps_config.gps_cmd_type == ATC_SPGPSTTFF)
            {
                ...
                if(TRUE == param_ptr->is_ok)
                {
                    ...
                    ATC_ADD_EXPECTED_EVENT_MUX(atc_config_ptr, 
                          APP_MN_GPS_FIX_IND, 0xff, ATC_NO_DOMAIN, dual_sys);
                }
                else
                {
                    ATC_BuildResultCodeRsp(atc_config_ptr,S_ATC_RESULT_CODE_ERROR, ERR_GPS_OPERATION_FAILED);
        ...
        case ATC_CMD_TYPE_SET:
        {
            if(PARAM1_FLAG)
            {
                ...
                if(PARA1.num == 1)
                {
                    ...
                    ATC_ADD_EXPECTED_EVENT_MUX(atc_config_ptr, 
                                      APP_MN_GPS_DOWNLOAD_CNF, 0xff, ATC_NO_DOMAIN, dual_sys);                    
                }
                else if(PARA1.num == 0)
                {
                   ...
                   ATC_ADD_EXPECTED_EVENT_MUX(atc_config_ptr, 
                                      APP_MN_GPS_POWEROFF_CNF, 0xff, ATC_NO_DOMAIN, dual_sys);    
                }
        ...
        case ATC_CMD_TYPE_SET:
        {
            if(PARAM1_FLAG)
            {
                ...
                else
                {
                    ...
                    ATC_ADD_EXPECTED_EVENT_MUX(atc_config_ptr, 
                                      APP_MN_GPS_STOP_CNF, 0xff, ATC_NO_DOMAIN, dual_sys);
                }
    ...
    switch (ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            if(PARAM1_FLAG)
            {
                ...
                ATC_ADD_EXPECTED_EVENT_MUX(atc_config_ptr, 
                                     APP_MN_GPS_RESET_CNF, 0xff, ATC_NO_DOMAIN, dual_sys);
            }
            else
            {
                status = ERR_GPS_INVALID_PARAMETER;
...
AT_CMD_FUNC( ATC_ProcessSPGPSSTATUS )
{
    ...
    switch (ATC_GET_CMD_TYPE)
    {
        ...
        case ATC_CMD_TYPE_READ:
        {
           ...
           ATC_ADD_EXPECTED_EVENT_MUX(atc_config_ptr, 
                                 APP_MN_GPS_READ_STATUS_CNF, 0xff, ATC_NO_DOMAIN, dual_sys);
            break;
        }
    ...
    switch (ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            if(PARAM1_FLAG)
            {
                ...
                ATC_ADD_EXPECTED_EVENT_MUX(atc_config_ptr, 
                                  APP_MN_GPS_READ_INFO_CNF, 0xff, ATC_NO_DOMAIN, dual_sys);
            }
            else
            {
                status = ERR_GPS_INVALID_PARAMETER;
    ...
    switch (ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            if(PARAM1_FLAG)
            {
                ...
                ATC_ADD_EXPECTED_EVENT_MUX(atc_config_ptr, 
                                  APP_MN_GPS_OUTPUT_CNF, 0xff, ATC_NO_DOMAIN, dual_sys);
            }
            else
            {
                status = ERR_GPS_INVALID_PARAMETER;
        ...
        case ATC_CMD_TYPE_SET:
        {
            if(PARAM1_FLAG)
            {
                if(1 == PARA1.num)
                {
                    ...
                    ATC_ADD_EXPECTED_EVENT_MUX(atc_config_ptr, 
                                      APP_MN_ACTIVATE_PDP_CONTEXT_CNF, pdp_id_arr[0], ATC_DOMAIN_PS, dual_sys);                   
                }
                else if(0 == PARA1.num)
                {
...
{
#if defined(_MNGPS_SUPPORT_) && defined(_SCM21_ATC_DEV)
    ATC_STATUS     status = S_ATC_DEFAULT_HANDLE;
    MN_DUAL_SYS_E   dual_sys = ATC_CURRENT_SIM_ID_MUX;

    SCI_TRACE_LOW("ATC: ATC_ProcessCGNSPWR enter");  
    ...
    switch (ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            if(PARAM1_FLAG)
            {
                if(PARA1.num == 1)
                {
                    gps_config.gps_cmd_type = ATC_CGNSPWR;
                    MNGPS_DownloadReq(dual_sys);
        ...
        case ATC_CMD_TYPE_READ:
        {
            ...
            sprintf((char*)g_rsp_str, "%s: %d", g_atc_info_table[ATC_CGNSPWR].cmd_name->str_ptr, status_info.is_gps_power_on);
