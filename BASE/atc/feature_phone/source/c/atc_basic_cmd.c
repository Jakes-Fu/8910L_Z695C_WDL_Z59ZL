                ...
                )
{
    ...
    SCI_TRACE_LOW("tpm_plmn_list   validate num is %d ",tpm_plmn_list.plmn_validate_num);
    ...
    if ((sig_ptr->is_ok)&&(g_atc_uplmn_info[dual_sys].mode == ATC_UPLMN_SET))
    {
        if (tpm_plmn_list.plmn_validate_num < tpm_plmn_list.plmn_max_num)
        {
            ...
            ATC_ADD_EXPECTED_EVENT(atc_config_ptr, (uint16)APP_MN_PREFER_PLMN_CHANGE_CNF, 0xff, ATC_NO_DOMAIN);
        }
        else
        {
    ...
    else if ((sig_ptr->is_ok)&&(g_atc_uplmn_info[dual_sys].mode == ATC_UPLMN_GET))
    {
        ...
        for(i = 0; i < tpm_plmn_list.plmn_validate_num; i++)
        {
            ...
            access_type = tpm_plmn_list.rat[i];

            TransformPlmnToNum(&tpm_plmn_list.plmn_arr[i], oper_str);
    ...
    else if ((sig_ptr->is_ok)&&(g_atc_uplmn_info[dual_sys].mode == ATC_UPLMN_DEL))
    {
        uint32 total = tpm_plmn_list.plmn_validate_num;
        uint8 index = g_atc_uplmn_info[dual_sys].index;
        SCI_TRACE_LOW("atc_uplmn_info mode is ATC_UPLMN_DEL index is %d, total is %d", index, total);
        if ((index + 1) == total)
        {
            tpm_plmn_list.plmn_validate_num--;
            SCI_MEMSET(&tpm_plmn_list.plmn_arr[index], 0, sizeof(tpm_plmn_list.plmn_arr[index]));         
            tpm_plmn_list.rat[index] = 0;
            MNSIM_SetPLMNWACTEx(dual_sys, &tpm_plmn_list);
            ATC_ADD_EXPECTED_EVENT(atc_config_ptr, (uint16)APP_MN_PREFER_PLMN_CHANGE_CNF, 0xff, ATC_NO_DOMAIN);
        }
        else if (index < total)
        {
             SCI_MEMSET(&(tpm_plmn_list.plmn_arr[index]), 0, sizeof(tpm_plmn_list.plmn_arr[index]));
             SCI_MEMCPY(&(tpm_plmn_list.plmn_arr[index]), &(tpm_plmn_list.plmn_arr[total -1]), sizeof(tpm_plmn_list.plmn_arr[index]));
             tpm_plmn_list.rat[index] = tpm_plmn_list.rat[total-1];
             tpm_plmn_list.plmn_validate_num--;
             SCI_MEMSET(&(tpm_plmn_list.plmn_arr[total-1]), 0, sizeof(tpm_plmn_list.plmn_arr[total-1]));
             tpm_plmn_list.rat[total-1] = 0;
             MNSIM_SetPLMNWACTEx(dual_sys, &tpm_plmn_list);
            ATC_ADD_EXPECTED_EVENT(atc_config_ptr, (uint16)APP_MN_PREFER_PLMN_CHANGE_CNF, 0xff, ATC_NO_DOMAIN);
        }
        else
...
AT_CMD_FUNC( ATC_ProcessSADM )
{
    ...
    if (type <= ATC_GET_AUD_MODE_NV_DATA)
    {
...
AT_CMD_FUNC(ATC_ProcessCAVIMS)
{
	...
	if (ATC_GET_CMD_TYPE == ATC_CMD_TYPE_SET) 
	{
		if (PARAM1_FLAG)
		{
			...
			MMIAPISET_SetVolteInfo(dual_sys, volte_on);
		}
		else
		{
			return S_ATC_FAIL;
	...
	{
		uint8 vlolte_info;
		vlolte_info = MMIAPISET_GetVolteInfo();
 ...
 **---------------------------------------------------------------------------*/
 /*****************************************************************************/
//  Description : This function handles command AT+AUNTZ 
//
//  Global resource dependence : none
//  Author:   rong.zhang1
//  Note:
/*****************************************************************************/
ATC_STATUS ATC_ProcessAUNTZ( // return the handle result
                ATC_CONFIG_T   * atc_config_ptr, // point to the ATC_CONFIG_T struct.
                ATC_CUR_INFO_LIST_T *cur_cmd_info // point to the atc cmd info
                            )
{
    ...
    switch(ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            ...
            {
                is_auto = PARA1.num;
                MMIAPISET_SetIsAutoUpdateTime(is_auto);
                MMIAPIAUT_SetIsNeedUpdateTimeByNet(is_auto);
            }
            else
            {
        ...
        case ATC_CMD_TYPE_READ:
        {
            is_auto = MMIAPISET_GetIsAutoUpdateTime();
            sprintf((char*)g_rsp_str, "+AUNTZ: %d", is_auto);
            ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
            break;
        }
        ...
        case ATC_CMD_TYPE_TEST:
        {
             sprintf((char *)g_rsp_str, "+AUNTZ: 0-1");
             ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
            break;
        }
        default:
...
/*****************************************************************************/
//  Description : This function handles command AT+CFUQ
//
//  Global resource dependence : none
//  Author:   rong.zhang1
//  Note:
/*****************************************************************************/
ATC_STATUS ATC_ProcessCFUQ( // return the handle result
                ATC_CONFIG_T   * atc_config_ptr, // point to the ATC_CONFIG_T struct.
                ATC_CUR_INFO_LIST_T *cur_cmd_info // point to the atc cmd info
                            )
{
    ...
    switch(ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            ...
            {
                is_query = PARA1.num;
                MMIAPIENG_SetCallForwardQuery(is_query);
            }
            else
            {
                at_status = S_ATC_FAIL;
        ...
        case ATC_CMD_TYPE_READ:
        {
            is_query = MMIAPIENG_GetCallForwardQuery();
            sprintf((char *)g_rsp_str, "+CFUQ: %d", is_query);
            ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
            break;
        }
        ...
        case ATC_CMD_TYPE_TEST:
        {
             sprintf((char *)g_rsp_str, "+CFUQ: 0-1");
             ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
            break;
        }
        default:
...
/*****************************************************************************/
//  Description : This function handles command AT+FAPDP  
//
//  Global resource dependence : none
//  Author:   rong.zhang1
//  Note:
/*****************************************************************************/
ATC_STATUS ATC_ProcessFAPDP( // return the handle result
                ATC_CONFIG_T   * atc_config_ptr, // point to the ATC_CONFIG_T struct.
                ATC_CUR_INFO_LIST_T *cur_cmd_info // point to the atc cmd info
                            )
{
    ...
    switch(ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            ...
            {
                is_on = PARA1.num;
                MMIAPIENG_ForbidAllGprsData_Write(is_on);
            }else
            {
                at_status = S_ATC_FAIL;
            }
        ...
        case ATC_CMD_TYPE_READ:
        {
            ...
            sprintf((char *)g_rsp_str, "+FAPDP: %d", is_on);
            ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
            break;
        }
        ...
        case ATC_CMD_TYPE_TEST:
        {
             sprintf((char *)g_rsp_str, "+FAPDP: 0-1");
             ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
            break;
        }
        default:
...
const uint16 LTE_Band_Valu[LTE_BAND_NUM]=
{
1,2,3,4,5,7,8,12,13,17,20,28,38,39,40,41,66
};
const uint32 LTE_freq_rang[LTE_BAND_NUM][2]=
{
{1,599},
{600,1199},
{1200,1949},
...
const uint32 GSM_freq_rang[GSM_BAND_MAX+1][2]=
{
{128,251},
{1,124},
{512,885},
...
static MMIENG_BAND_FREQ_T s_band_freq_info={0};
uint16 g_lte_band_num;
extern MMIENG_NV_ITEM_ID_E;

/*void ATC_SaveBandFreqInfo(MMIENG_BAND_FREQ_T *bandfreqinfo,MN_DUAL_SYS_E  dual_sys)
{
    if(dual_sys == 0)
    {
...
MN_RETURN_RESULT_E ATC_ReadBandFreqInfo(MMIENG_BAND_FREQ_T *bandfreqinfo,MN_DUAL_SYS_E  dual_sys)
{
...
void  ATC_InitBandFreq(MN_DUAL_SYS_E  dual_sys)
{
    uint8 freq_num =0,gsm_band=0;
    MN_RETURN_RESULT_E ret=MN_RETURN_SUCCESS;
    int i;
    
    SCI_MEMSET(&s_band_freq_info,0x0,sizeof(s_band_freq_info));

    /*ret = ATC_ReadBandFreqInfo(&s_band_freq_info,dual_sys);
    ...
    MNPHONE_GetLteLockFreq(&freq_num,s_band_freq_info.Freq_list,dual_sys);
    if(freq_num == 0)
    {
        SCI_MEMSET(s_band_freq_info.Freq_list,0xFFFFFFFF,sizeof(s_band_freq_info.Freq_list));
    }
    MNPHONE_GetGsmBand(&gsm_band,dual_sys);
    if(gsm_band & MMI_NW_BAND_GSM_850)
    {
        s_band_freq_info.GSM_band[NW_BAND_GSM_850] =TRUE;
    }
    if(gsm_band & MMI_NW_BAND_GSM_900P)
    {
        s_band_freq_info.GSM_band[NW_BAND_GSM_900] =TRUE;
    }
    if(gsm_band & MMI_NW_BAND_DCS_1800)
    {
        s_band_freq_info.GSM_band[NW_BAND_DCS_1800] =TRUE;
    }
    if(gsm_band & MMI_NW_BAND_PCS_1900)
    {
        s_band_freq_info.GSM_band[NW_BAND_PCS_1900] =TRUE;
    }
    
    //ATC_SaveBandFreqInfo(&s_band_freq_info,dual_sys);
    
    SCI_TraceLow("g_lte_band_num=%d,gsmband=%d,freqnum=%d", g_lte_band_num,gsm_band,freq_num);
    
    for(i=0;i< g_lte_band_num;i++)
    {
        SCI_TraceLow("LTE_band[%d] =%d",i,s_band_freq_info.LTE_band[i]);
    }
    for(i=0;i<ENG_FREQ_MAX_NUM;i++)
    {
        SCI_TraceLow("Freq_list[%d] =%d",i,s_band_freq_info.Freq_list[i]);
        SCI_TraceLow("GSM_Freq[%d] =%d",i,s_band_freq_info.GSM_Freq[i]);
    }
    for(i=0;i<GSM_BAND_MAX;i++)
    {
        SCI_TraceLow("gsm_band[%d] =%d",i,s_band_freq_info.GSM_band[i]);
    }
}

BOOLEAN ATC_IsGsmFreqValid(uint32 freq)
{
    int i=0;
    BOOLEAN ret= FALSE;
    ...
    for(i = 0; i< 4; i++)
    {
        ...
        {
            if(freq >=GSM_freq_rang[i][0] && freq <= GSM_freq_rang[i][1])
            {
                 ret = TRUE;
                 break;
             }
             ...
             {
                 if(freq >= GSM_freq_rang[GSM_BAND_MAX][0] && freq <= GSM_freq_rang[GSM_BAND_MAX][1])
                 {
                     ret = TRUE;
                     break;
                 }
                ...
                )
{
    ...
    SCI_TraceLow("ATC_ProcessGLBAND");
    ...
    switch(ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            if (PARAM1_FLAG && PARAM2_FLAG && PARAM3_FLAG && PARAM4_FLAG)
           {
                for ( i = 0; i < 4; i++)
               {
                ...
                if(PARA1.num)
                {
                    gsm_band = gsm_band|MMI_NW_BAND_GSM_850;
                    s_band_freq_info.GSM_band[NW_BAND_GSM_850] =TRUE;
                }
                if(PARA2.num)
                {
                    gsm_band = gsm_band|MMI_NW_BAND_GSM_900E|MMI_NW_BAND_GSM_900P;
                    s_band_freq_info.GSM_band[NW_BAND_GSM_900] =TRUE;
                }
                if(PARA3.num)
                {
                    gsm_band = gsm_band|MMI_NW_BAND_DCS_1800;
                    s_band_freq_info.GSM_band[NW_BAND_DCS_1800] =TRUE;
                }
                if(PARA4.num)
                {
                    gsm_band = gsm_band|MMI_NW_BAND_PCS_1900;
                    s_band_freq_info.GSM_band[NW_BAND_PCS_1900] =TRUE;
                }
            } else
            {
             ...
             SCI_TraceLow("ATC_ProcessGLBAND  gsm_band %d", gsm_band);
             MNPHONE_SetGsmBand(gsm_band, dual_sys);
             break;
        }
        case ATC_CMD_TYPE_READ:
        {
            ptr = g_rsp_str;
            sprintf((char*)ptr,"+GLBAND: ");
            ptr = g_rsp_str + strlen((char*)g_rsp_str);

            MNPHONE_GetGsmBand(&gsm_band,dual_sys);
            if(gsm_band & MMI_NW_BAND_GSM_850)
            {
                sprintf((char*)ptr," 850 ");
                ptr = g_rsp_str + strlen((char*)g_rsp_str);
            }
            if(gsm_band & MMI_NW_BAND_GSM_900P)
            {
                sprintf((char*)ptr," 900 ");
                ptr = g_rsp_str + strlen((char*)g_rsp_str);
            }
            if(gsm_band & MMI_NW_BAND_DCS_1800)
            {
                sprintf((char*)ptr," 1800 ");
                ptr = g_rsp_str + strlen((char*)g_rsp_str);                
            }
            if(gsm_band & MMI_NW_BAND_PCS_1900)
            {
                sprintf((char*)ptr," 1900 ");
                ptr = g_rsp_str + strlen((char*)g_rsp_str); 
            }
            ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
            break;
        ...
        case ATC_CMD_TYPE_TEST:
        {
             sprintf((char *)g_rsp_str, "+GLBAND: (0-1),(0-1),(0-1),(0-1)");
             ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
            break;
        }
        default:
                ...
                )
{
    ...
    switch(ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            ...
            if (PARAM1_FLAG)
            {
                s_band_freq_info.LTE_band[count++] = PARA1.num;
            }

            if (PARAM2_FLAG)
            {
                s_band_freq_info.LTE_band[count++] = PARA2.num;
            }

            if (PARAM3_FLAG)
            {
                s_band_freq_info.LTE_band[count++] = PARA3.num;
            }

            if (PARAM4_FLAG)
            {
                s_band_freq_info.LTE_band[count++] = PARA4.num;
            }

            if (PARAM5_FLAG)
            {
                s_band_freq_info.LTE_band[count++] = PARA5.num;
            }

            if (PARAM6_FLAG)
            {
                s_band_freq_info.LTE_band[count++] = PARA6.num;
            }

            if (PARAM7_FLAG)
            {
                s_band_freq_info.LTE_band[count++] = PARA7.num;
            }

            if (PARAM8_FLAG)
            {
                s_band_freq_info.LTE_band[count++] = PARA8.num;
            }

             if (PARAM9_FLAG)
            {
                s_band_freq_info.LTE_band[count++] = PARA9.num;
            }

             if (PARAM10_FLAG)
            {
                s_band_freq_info.LTE_band[count++] = PARA10.num;
            }

             if (PARAM11_FLAG)
            {
                s_band_freq_info.LTE_band[count++] = PARA11.num;
            }

             if (PARAM12_FLAG)
            {
                s_band_freq_info.LTE_band[count++] = PARA12.num;
            }

            if (PARAM13_FLAG)
            {
                s_band_freq_info.LTE_band[count++] = PARA13.num;
            }
            g_lte_band_num = count; 
            MNPHONE_SetLteLockBand(g_lte_band_num, (s_band_freq_info.LTE_band),dual_sys);
            break;
        }
        case ATC_CMD_TYPE_READ:
        {
            ...
            ptr = g_rsp_str;
            sprintf((char*)ptr,"+TLBAND: ");
            ptr = g_rsp_str + strlen((char*)g_rsp_str);
            MNPHONE_GetLteLockBand(&g_lte_band_num,s_band_freq_info.LTE_band,dual_sys);
            for(i = 0; i <  g_lte_band_num; i++)
            {
                SCI_TraceLow("LTE_band[%d] =%d",i,s_band_freq_info.LTE_band[i]);
                sprintf((char*)ptr,"  %d  ", s_band_freq_info.LTE_band[i]);
                ptr = g_rsp_str + strlen((char*)g_rsp_str);
            }
            ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
            break;
               ...
               )
{
    ...
    SCI_TraceLow("ATC_ProcessGLFREQ");
    ...
    switch(ATC_GET_CMD_TYPE)
    {
         case ATC_CMD_TYPE_SET:
         {
             ...
             if (PARAM1_FLAG)
             {
                  if (ATC_IsGsmFreqValid(PARA1.num))
                  {
                       s_band_freq_info.GSM_Freq[0] = PARA1.num;
                       plmnlist.ArfcnL[plmnlist.nCount]= PARA1.num;
                       plmnlist.nCount++;
                  }
                  else
                  {
             ...
             if (PARAM2_FLAG)
             {
                  if (ATC_IsGsmFreqValid(PARA2.num))
                  {
                      s_band_freq_info.GSM_Freq[1] = PARA2.num;
                       plmnlist.ArfcnL[plmnlist.nCount]= PARA2.num;
                       plmnlist.nCount++;
                  }
                  else
                  {
             ...
             if (PARAM3_FLAG)
             {
                  if (ATC_IsGsmFreqValid(PARA3.num))
                  {
                      s_band_freq_info.GSM_Freq[2] = PARA3.num;
                      plmnlist.ArfcnL[plmnlist.nCount]= PARA3.num;
                      plmnlist.nCount++;
                  }
                  else
                  {
             ...
             if (PARAM4_FLAG)
             {
                  if (ATC_IsGsmFreqValid(PARA4.num))
                  {
                      s_band_freq_info.GSM_Freq[3] = PARA4.num;
                      plmnlist.ArfcnL[plmnlist.nCount]= PARA4.num;
                      plmnlist.nCount++;
                  }
                  else
                  {
             ...
             if (PARAM5_FLAG)
             {
                  if (ATC_IsGsmFreqValid(PARA5.num))
                  {
                      s_band_freq_info.GSM_Freq[4] = PARA5.num;
                      plmnlist.ArfcnL[plmnlist.nCount]= PARA5.num;
                      plmnlist.nCount++;
                  }
                  else
                  {
             ...
             if (PARAM6_FLAG)
             {
                  if (ATC_IsGsmFreqValid(PARA6.num))
                  {
                      s_band_freq_info.GSM_Freq[5] = PARA6.num;
                      plmnlist.ArfcnL[plmnlist.nCount]= PARA6.num;
                      plmnlist.nCount++;
                  }
                  else
                  {
            ...
            if (PARAM7_FLAG)
             {
                  if (ATC_IsGsmFreqValid(PARA7.num))
                  {
                      s_band_freq_info.GSM_Freq[6] = PARA7.num;
                      plmnlist.ArfcnL[plmnlist.nCount]= PARA7.num;
                      plmnlist.nCount++;
                  }
                  else
                  {
            ...
            if (PARAM8_FLAG)
             {
                  if (ATC_IsGsmFreqValid(PARA8.num))
                  {
                      s_band_freq_info.GSM_Freq[7] = PARA8.num;
                      plmnlist.ArfcnL[plmnlist.nCount]= PARA8.num;
                      plmnlist.nCount++;
                  }
                  else
                  {
            ...
            if (PARAM9_FLAG)
             {
                  if (ATC_IsGsmFreqValid(PARA9.num))
                  {
                      s_band_freq_info.GSM_Freq[8] = PARA9.num;
                      plmnlist.ArfcnL[plmnlist.nCount]= PARA9.num;
                      plmnlist.nCount++;
                  }
                  else
                  {
             ...
             MNPHOE_CfgSetStoredPlmnList(&plmnlist, dual_sys);
             MNPHONE_NwSetLockBCCH(TRUE, dual_sys);
             break;
         }
         case ATC_CMD_TYPE_READ:
         {
...
//LTE freq Handle
BOOLEAN ATC_IsLteFreqValid(uint32 freq)
{
    int i=0,index=0;
    BOOLEAN ret= FALSE;
    ...
    for (index = 0;  index< LTE_BAND_NUM; index++ )
    {
        for(i = 0;i < g_lte_band_num; i++)
        {
            if(LTE_Band_Valu[index] == s_band_freq_info.LTE_band[i])
           {
     ...
     for(i = 0; i < LTE_BAND_NUM; i++)
     {
         if(select_item[i])
         {
             if(freq >= LTE_freq_rang[i][0] && freq <= LTE_freq_rang[i][1])
             {
                 ret = TRUE;
                 break;
              }
               ...
               )
{
    ...
    switch(ATC_GET_CMD_TYPE)
    {
         case ATC_CMD_TYPE_SET:
         {
             ...
             if (PARAM1_FLAG)
             {
                  if (ATC_IsLteFreqValid(PARA1.num))
                  {
                       s_band_freq_info.Freq_list[number] = PARA1.num;
                       number++;
                  }
                  else
                  {
             ...
             if (PARAM2_FLAG)
             {
                  if (ATC_IsLteFreqValid(PARA2.num))
                  {
                      s_band_freq_info.Freq_list[number] = PARA2.num;
                      number++;
                  }
                  else
                  {
             ...
             if (PARAM3_FLAG)
             {
                  if (ATC_IsLteFreqValid(PARA3.num))
                  {
                      s_band_freq_info.Freq_list[number] = PARA3.num;
                      number++;
                  }
                  else
                  {
             ...
             if (PARAM4_FLAG)
             {
                  if (ATC_IsLteFreqValid(PARA4.num))
                  {
                      s_band_freq_info.Freq_list[number] = PARA4.num;
                      number++;
                  }
                  else
                  {
             ...
             if (PARAM5_FLAG)
             {
                  if (ATC_IsLteFreqValid(PARA5.num))
                  {
                      s_band_freq_info.Freq_list[number] = PARA5.num;
                      number++;
                  }
                  else
                  {
             ...
             if (PARAM6_FLAG)
             {
                  if (ATC_IsLteFreqValid(PARA6.num))
                  {
                      s_band_freq_info.Freq_list[number] = PARA6.num;
                      number++;
                  }
                  else
                  {
            ...
            if (PARAM7_FLAG)
             {
                  if (ATC_IsLteFreqValid(PARA7.num))
                  {
                      s_band_freq_info.Freq_list[number] = PARA7.num;
                      number++;
                  }
                  else
                  {
            ...
            if (PARAM8_FLAG)
             {
                  if (ATC_IsLteFreqValid(PARA8.num))
                  {
                      s_band_freq_info.Freq_list[number] = PARA8.num;
                      number++;
                  }
                  else
                  {
            ...
            if (PARAM9_FLAG)
             {
                  if (ATC_IsLteFreqValid(PARA9.num))
                  {
                      s_band_freq_info.Freq_list[number] = PARA9.num;
                      number++;
                  }
                  else
                  {
             ...
             MNPHONE_SetLteLockFreq(number,(s_band_freq_info.Freq_list),dual_sys);
             break;
         }
         case ATC_CMD_TYPE_READ:
         {
             uint8 *ptr= PNULL;
             MNPHONE_GetLteLockFreq(&number,s_band_freq_info.Freq_list,dual_sys);

             if (number == 0)
             {
                 SCI_MEMSET(s_band_freq_info.Freq_list,0xFFFFFFFF,sizeof(s_band_freq_info.Freq_list));
                 sprintf((char *)g_rsp_str, "+TLFREQ: empty");
                 ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
             }
             else
             {
                 ptr = g_rsp_str;
                 sprintf((char*)ptr,"+TLFREQ: ");
                 ptr = g_rsp_str + strlen((char*)g_rsp_str);
                 for (i = 0; i < number; i++)
                 {
                     sprintf((char*)ptr,"  %d  ", s_band_freq_info.Freq_list[i]);
                     ptr = g_rsp_str + strlen((char*)g_rsp_str);
                 }
                 ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
             }
                ...
                )
{
    ...
    SCI_TraceLow("ATC_ProcessSPUPLMN");
    ...
    switch(ATC_GET_CMD_TYPE)
    {
         case ATC_CMD_TYPE_SET:
         {
             if (PARAM1_FLAG && PARAM2_FLAG && PARAM3_FLAG && PARAM4_FLAG)
             {
                 ...
                 g_atc_uplmn_info[dual_sys].plmn.mcc = PARA2.num;
                 g_atc_uplmn_info[dual_sys].plmn.mnc = PARA3.num;
                 g_atc_uplmn_info[dual_sys].rat = PARA4.num;
                 if (PARA3.num > 99)
                {
                 ...
                 else
                {
                 ...
                 ATC_ADD_EXPECTED_EVENT(atc_config_ptr, (uint16)APP_MN_PREFER_PLMN_CHANGE_CNF, 0xff, ATC_NO_DOMAIN);
                 at_status = S_ATC_DEFAULT_HANDLE;
             }
             else if(PARAM1_FLAG && !PARAM2_FLAG && !PARAM3_FLAG && !PARAM4_FLAG)
             {
                  g_atc_uplmn_info[dual_sys].index = PARA1.num;
                  g_atc_uplmn_info[dual_sys].mode = ATC_UPLMN_DEL;
                  MNSIM_GetPLMNWACTEx(dual_sys);
                  ATC_ADD_EXPECTED_EVENT(atc_config_ptr, (uint16)APP_MN_PREFER_PLMN_CHANGE_CNF, 0xff, ATC_NO_DOMAIN);
                  at_status = S_ATC_DEFAULT_HANDLE;
             }
         ...
         case ATC_CMD_TYPE_READ:
         {
            SCI_TRACE_LOW("ATC: reading the plmn list");
...
static uint32 ping_times;
static uint32 ping_count;
static uint32 ping_fail;

void ATC_PingCallBack(
    ...
    char*                               ipaddr) 
{
    ...
    if (PINGRES_SUCCESS != res)
    {
        ...
        SCI_TraceLow("ping error!!!! res is %d", res);
        ping_fail++;
        ATC_BuildResultCodeRsp(atc_config_ptr ,S_ATC_RESULT_CODE_ERROR,0);
    }
    else
    {
        ping_count++;
        SCI_TraceLow("ping time_delay %d  ", time_delay);
        SCI_TraceLow("ATC_PingCallBack ttl %d ttl    ipaddr %s", ttl, ipaddr);
        sprintf((char *)g_rsp_str, "+PING:from:%s response time_delay=%d  ttl=%d", ipaddr, time_delay, ttl);
        ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
    }
    if ((ping_count + ping_fail) == ping_times)
    {
        sprintf((char *)g_rsp_str, "+PING:Finish,Packets: Sent=%d, Received=%d,Lost=%d", ping_times, ping_count, ping_times -ping_count);
        ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
        ping_count = 0;
        ping_times = 0;
        ping_fail = 0;
    ...
    switch (ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            ...
            if(PARAM1_TYPE != ATC_CMD_PARAM_TYPE_STRING)
            {
             ...
             addr = (char *)PARA1.str_ptr->str_ptr;
            if (PARAM2_FLAG)
            {
                times = PARA2.num;
                ping_times =  PARA2.num;
            }
            ...
            SCI_TraceLow("ATC_ProcessPING get nsapi is %d", nsapi);
            if (ip_type)
            {
                sci_ping6_request(addr, 0, 0, times,ATC_PingCallBack, nsapi);
            }
            else
            {
                sci_ping_request_ext(addr, 0, 0, times,ATC_PingCallBack, nsapi);
            }
            ATC_BuildResultCodeRsp(atc_config_ptr,S_ATC_RESULT_CODE_OK,0);
            sprintf((char *)g_rsp_str, "+PING: Begin %s, data size=64 bytes", addr);
            ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
...
AT_CMD_FUNC( ATC_ProcessCDMAIMSI )
{
    ...
    if(ATC_CMD_TYPE_EXECUTE == ATC_GET_CMD_TYPE)
    {
        ...
        uint8 i = 0;
        uint8 temp[3] = {0};
        nStatus = MNPHONE_GetCdmaImsi(cdmaimsi, dual_sys);
        SCI_TRACE_BUF("at cdmaimsi:", cdmaimsi, 10);
        sprintf((char*)g_rsp_str, "%d, ", nStatus);

        for(i = 0; i < 10; i++)
        {
            sprintf(temp, "%02x",cdmaimsi[i]);
            strcat(g_rsp_str, temp);
        }
