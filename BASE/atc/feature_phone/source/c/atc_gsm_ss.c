...
static BOOLEAN               g_at_is_codec_on = FALSE;
...
BOOLEAN ATC_IsVoiceCodecEnable()
{
    return g_at_is_codec_on;
}
    ...
    switch (ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            if (PARAM1_FLAG)
            {
                ...
                CFW_SsSetClip(PARA1.num, PHONE_GetFreeUti(), dual_sys);
            }
            else
            {
                return ERR_INVALID_INDEX;
        ...
        case ATC_CMD_TYPE_READ:
        {
            ...
            {
                SCI_TRACE_LOW("ATC: ATC_ProcessCLIP, MNSS_InterrogateService, failure:%d", err_code);
    ...
    switch (ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            if (PARAM1_FLAG)
            {
                ...
                CFW_SsSetColp(PARA1.num, PHONE_GetFreeUti(), dual_sys);
            }
            else
            {
                return ERR_INVALID_INDEX;
                        ...
                        )
{
    ...
    SCI_ASSERT(MN_CALL_MAX_CALL_NUM > call_index); /*assert verified*/
    ...
    ATC_UpSINDInfo(dual_sys, atc_config_ptr, PNULL,ATC_WIND_BIT8, 9);
    ...
    {   
        ...
        sprintf((char*)g_rsp_str, "+COLP: \"%s\",%d ", &(tel_str[1]),tel_str[0]);
        ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
    }   
#endif
    return S_ATC_SUCCESS;
        ...
        case AT_CUSD_ENABLE:
        {
            ...
            if (param2_flag)
            {
                if (ptr_para2_str->str_len > MN_SS_MAX_USSD_STRING_LENGTH_BEFORE_ENCODE)
                {
                 ...
                 SCI_TRACE_LOW("ATC: HandleUssdReq s_cusd_isresponse=%d", s_cusd_isresponse);
                                ...
                                )
{
    ...
    switch( dcs )
    {
        ...
        {
            default_str_len = ( input_ussd_len * 8) / 7;
            ...
            if(SCI_SUCCESS != SCI_Unpack7bitsTo8bits(input_ussd_str, input_ussd_len, 0, default_buffer_ptr, (int16 *)&default_str_len))
            {
                SCI_TRACE_LOW("ATC: HandleUssdInd convert 7 bits to 8 bits wrong!");
                SCI_FREE(default_buffer_ptr);
                return S_ATC_FAIL;
            }
           
            temp_len = ATC_Default2Ascii(default_buffer_ptr, temp_str, default_str_len);
            SCI_TRACE_LOW("ATC: HandleUssdInd temp_len=%d",temp_len);
            SCI_FREE(default_buffer_ptr);
                        ...
                        )
{
    ...
    s_cusd_isresponse = FALSE;
    SCI_TRACE_LOW("ATC: ATC_ProcessUssdServiceCnf s_cusd_isresponse=%d", s_cusd_isresponse);
    //if disable to present result,return. 
    if(AT_CUSD_DISABLE == s_cusd_presentation)
    {
        return S_ATC_FAIL;
            ...
            )
{
    ...
    switch (ccfc_reason)
    {
        ...
        case ATC_COMMUNICATION_DEFLECTION:
        {
            *ss_code_ptr = MN_SS_CD;
            break;
        }
        case ATC_COMMUNICATION_FORWARDING_NOT_LOGGEDIN:
        {
            *ss_code_ptr = MN_SS_CFNL;
            break;
        }
        default:
            break;
...
{
    SCI_TRACE_LOW("ATC: ATC_ProcessForwardStatusInd!!!");
    // enable the unsolicited result code: +CSSI
    if (atc_config_ptr->cssi_flag)
    {
        SCI_TRACE_LOW("ATC:forward_operation %d ",sig_ptr->forward_operation);
        SCI_TRACE_LOW("ATC:sig_ptr->forward_status is %d",sig_ptr->forward_status);
            ...
            case AT_CMD_CCFCU:
            {
                if (is_service_active)
                {
                    ...
                    if (sig_ptr->info_list[0].forward_num_present)
                    {
                        ...
                        if (sig_ptr->info_list[0].no_reply_time_present)
                        {
                            sprintf((char*)g_rsp_str, "%s: %d,%d,\"%s\",%d,%d",g_atc_info_table[AT_CMD_CCFCU].cmd_name->str_ptr,
                                AT_STATUS_ACTIVE, ATC_VOICE, &(tel_number[1]),tel_number[0], sig_ptr->info_list[0].no_reply_time);
                        }
                        else
                        {
                            sprintf((char*)g_rsp_str, "%s: %d,%d,\"%s\",%d",g_atc_info_table[AT_CMD_CCFCU].cmd_name->str_ptr,
                                AT_STATUS_ACTIVE, ATC_VOICE,  &(tel_number[1]),tel_number[0]);
                        }
                    }
                    else
                    {
                        tel_number[0] = '\0';
                        sprintf((char*)g_rsp_str, "%s: %d,%d,\"%s\",,",g_atc_info_table[AT_CMD_CCFCU].cmd_name->str_ptr,
                                AT_STATUS_ACTIVE, ATC_VOICE, tel_number);
                    }
                }
                else
                {
                    sprintf((char*)g_rsp_str, "%s: %d,%d",g_atc_info_table[AT_CMD_CCFCU].cmd_name->str_ptr,
                        AT_STATUS_NOT_ACTIVE, ATC_VOICE);
                }
                break;
            }
                        ...
                        )
{
    ...
    if (SetUserBusyForWaitCall(dual_sys,call_state_ptr))
    {
        return S_ATC_DEFAULT_HANDLE;
    }
    // check if there is a waiting call
    else if (RelPresentHeldCalls(dual_sys,call_state_ptr))
    {
        return S_ATC_DEFAULT_HANDLE;
    }
    else
                        ...
                        )
{
    ...
    if (CheckPresentActiveCall(call_state_ptr, &active_id))
    {
        ...
        {
            if(MN_RETURN_SUCCESS == MNCALL_HoldCallEx(dual_sys, active_id))
...
AT_CMD_FUNC( ATC_ProcessCLIR )
{
    ...
    switch (ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            ...
            CFW_SsSetClir(PARA1.num, PHONE_GetFreeUti(), dual_sys);
            break;
        }
...
AT_CMD_FUNC( ATC_ProcessCUSD )
{
    ...
    int32                   para3_num             = MN_SS_DEFAULT_DCS_VALUE;
    ...
    SCI_TRACE_LOW("ATC: ATC_ProcessCUSD in");
    switch(ATC_GET_CMD_TYPE)
    {
        // AT+CUSD?
        case ATC_CMD_TYPE_READ:
            ...
            if(PARAM1_FLAG)
            {
                if(PARAM3_FLAG)
                {
                    ...
                    switch(PARA3.num)
                    {
                        ...
                        case AT_CUSD_UCS2_72:
                        case AT_CUSD_UCS2_90:
                        {
                            data_code_scheme = MN_SS_USC2_NO_CLASS_DCS;
                            if(AT_CUSD_UCS2ALPHABET == PARA3.num)
                            {
                        ...
                        case AT_CUSD_8BITDATA_68:
                        case AT_CUSD_8BITDATA_86:
                        {
                ...
                SCI_TRACE_LOW("ATC: data_code_scheme is %d, para3_num=%d", data_code_scheme, para3_num);

                if(PARAM2_FLAG)
                {
                    if(PARAM2_TYPE != ATC_CMD_PARAM_TYPE_STRING)
                    {
                        SCI_TRACE_LOW("ATC: ATC_ProcessCUSD, param2 is not string!");
                        status = ERR_INVALID_INDEX;
                        break;
                    }
                    ...
                    {
                        SCI_TRACE_LOW("ATC: ATC_ProcessCUSD, the string is empty");
                        return ERR_INVALID_INDEX;
                    }
                
                    switch( data_code_scheme )
                    {
                        case MN_SS_DEFAULT_DCS_VALUE:
                        case MN_SS_8BIT_NO_CLASS_DCS:
                        {
                            ...
                            {
                                SCI_TRACE_LOW("ATC: UCS2-GSM PARA2.str_ptr->str_len is %d, str is %s", PARA2.str_ptr->str_len, PARA2.str_ptr->str_ptr);
                                ...
                                SCI_TRACE_LOW("ATC: ussd_str str_len is %d, str is %s", ptr_para2_str->str_len,ptr_para2_str->str_ptr);
                            }
                            else if(0 == strcmp((char*)(atc_config_ptr->cscs), "IRA"))
                            {
                                SCI_TRACE_LOW("ATC: IRA-GSM PARA2.str_ptr->str_len is %d, str is %s", PARA2.str_ptr->str_len, PARA2.str_ptr->str_ptr);
                            ...
                            else
                            {
                                ...
                                SCI_TRACE_LOW("ATC: GSM-GSM ussd_str str_len is %d, str is %s", ptr_para2_str->str_len,ptr_para2_str->str_ptr);
                            }
                        ...
                        case MN_SS_USC2_NO_CLASS_DCS:
                        {
                            ...
                            {
                                SCI_TRACE_LOW("ATC: GSM-UCS2 PARA2.str_ptr->str_len is %d, str is %s", PARA2.str_ptr->str_len, PARA2.str_ptr->str_ptr);
                                ATC_ConvertAsciiToUcs2Hex(PARA2.str_ptr->str_ptr,PARA2.str_ptr->str_len, temp_str);
                                temp_str_len = PARA2.str_ptr->str_len * 4;
                                ConvertHexToBin(temp_str,temp_str_len, ussd_str);
                                ptr_para_str.str_len = temp_str_len / 2;
                                ussd_str[ptr_para_str.str_len] = '\0';
                                ptr_para_str.str_ptr = ussd_str;
                                ptr_para2_str = &ptr_para_str;
                                SCI_TRACE_LOW("ATC: ussd_str str_len is %d, str is %s", ptr_para2_str->str_len,ptr_para2_str->str_ptr);
                            ...
                            {
                                SCI_TRACE_LOW("ATC: IRA-UCS2 PARA2.str_ptr->str_len is %d, str is %s", PARA2.str_ptr->str_len, PARA2.str_ptr->str_ptr);
                                temp_str_len = MN_SS_MAX_USSD_STRING_LENGTH_BEFORE_ENCODE + 1;
                                ATC_ConvertHEXToUCS2(PARA2.str_ptr->str_ptr,PARA2.str_ptr->str_len, temp_str, &temp_str_len);
                                ConvertHexToBin(temp_str,temp_str_len,ussd_str);
                                ptr_para_str.str_len = temp_str_len / 2;
                                ussd_str[ptr_para_str.str_len] = '\0';
                                ptr_para_str.str_ptr = ussd_str;
                            ...
                            {
                                SCI_TRACE_LOW("ATC: UCS2-UCS2 PARA2.str_ptr->str_len is %d, str is %s", PARA2.str_ptr->str_len, PARA2.str_ptr->str_ptr);
                                ConvertHexToBin(PARA2.str_ptr->str_ptr,PARA2.str_ptr->str_len, ussd_str);
                                ptr_para_str.str_len = PARA2.str_ptr->str_len / 2;
                                ussd_str[ptr_para_str.str_len] = '\0';
                                ptr_para_str.str_ptr = ussd_str;
...
#if 0
                if (PNULL == ptr_para2_str)
                {
                    SCI_TRACE_LOW("ATC ATC_ProcessCUSD ptr_para2_str is NULL");
                    status = ERR_INVALID_INDEX;
                    break;
                }
#endif
...
#define ATC_DIAL_TELSTR "tel:"
#define ATC_DIAL_SIPSTR "sip:"
#define ATC_CMD_D_OPTYPE_VOICECALL     1
#define ATC_CMD_D_OPTYPE_GPRSDIAL       2

uint32 SUL_Strlen(
    CONST uint16 *string     //The string to be sized
)
{
    const uint16 *sc;
...
int8 ATC_CDU_ParaParse(uint8 *pPara, uint8 *pDialNum)
{
    uint8 pParaStr[41] = { 0 ,};
    uint8 PWIndex = 0;
    uint8 uDialNumLen = 0;
    ...
    SCI_TRACE_LOW( "ATC:D VOICECALL: pPara:%s\n\r", pPara);

    if(strlen(pPara) > 41)
        return ERR_OPERATION_NOT_SUPPORTED;

    SCI_Memcpy(pParaStr, pPara, strlen(pPara));
    pStr = pParaStr;
    // consider the priority here. voice should be the last one.
    if (!SCI_MEMCMP(pParaStr, ATC_DIAL_SIPSTR, strlen(ATC_DIAL_SIPSTR)))
    {
        return ATC_CMD_D_OPTYPE_GPRSDIAL;
    }
    else if (!SCI_MEMCMP(pParaStr, ATC_DIAL_TELSTR, strlen(ATC_DIAL_TELSTR)))
    {
        do
        {
            uDialNumLen++;
        ...
        while(*pStr++ != 0);
        memset(pDialNum, 0, 22);
        strncpy(pDialNum, &pParaStr[4], uDialNumLen);
        SCI_TRACE_LOW("ATC: VOICECALL: pDialNum:%s\n\r", pDialNum);
        return ATC_CMD_D_OPTYPE_VOICECALL;
    }
    else
    {
...
AT_CMD_FUNC(ATC_ProcessCCFCU)
{
    ...
    ATC_STATUS                 re_value = S_ATC_DEFAULT_HANDLE;
    MN_DUAL_SYS_E dual_sys = ATC_CURRENT_SIM_ID_MUX;
    uint8 ucNType = 0, ucNumType = 0, ucClass = 0, uton = 0, ucTime = 0;
    uint8 uDialNum[41] = { 0, };
    uint8 uOpType;
    uint8 iLen = 0;
    ...
    switch (operation_mode)
    {
        ...
        {
            basic_service.service_type = MN_SS_ALL_SERVICE; 
            s_ss_context.cmd_id = AT_CMD_CCFCU;      
            MNSS_InterrogateServiceEx(dual_sys, ss_code, basic_service);
            ATC_ADD_EXPECTED_EVENT(atc_config_ptr,APP_MN_INTERROGATE_CNF,0xff,ATC_NO_DOMAIN);
            break;
        }
        ...
        case ATC_REGISTRATION:
        {
            ...
            if(ATC_CMD_PARAM_TYPE_STRING != PARAM5_TYPE)
            {
                //SCI_TRACE_LOW:"ATC command's parameter type wrong"
                SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_GSM_SS_327_112_2_17_22_17_17_629,(uint8*)"");
                re_value = ERR_INVALID_INDEX;
            ...
            if (ucNType == 0)
            {
                SCI_TRACE_LOW("ATC:number type is zero!!!");
            }
            else if (ucNType == 1)
            {
                    uOpType = ATC_CDU_ParaParse(PARA5.str_ptr->str_ptr, uDialNum);
                    if(uOpType == ATC_CMD_D_OPTYPE_VOICECALL)
                    {
                        str_len = (uint8)SUL_Strlen(uDialNum);
                            
                        SCI_TRACE_LOW("ATC:phone num = %s\n\r", uDialNum);
                        if (str_len > 41)
                            str_len = 41;
                        if ('+' == uDialNum[0])
                        {
                            str_len--;
                            iLen = 1;
                            register_param.cfnry_param.forward_num.number_type =  145;
                        }
                        else if (('0' == uDialNum[0]) && ('0' == uDialNum[1]))
                        {
                            str_len -= 2;
                            iLen = 2;
                        }
                    }
                    temp_flag = ATC_ConvertTeleStrToStruct(&(register_param.cfnry_param.forward_num),
                                    uDialNum + iLen,str_len);
                    if (!temp_flag)
                    {
                        //SCI_TRACE_LOW:"ATC:CCFC reg fail3"
                        SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_GSM_SS_360_112_2_17_22_17_17_631,(uint8*)"");
            ...
            {
                    if (((char)*PARA5.str_ptr->str_ptr) == '+')
                    {
                        str_len = PARA5.str_ptr->str_len - 1;
                        register_param.cfnry_param.forward_num.number_type = 145;
                    }
                    else
                    {
                        str_len = PARA5.str_ptr->str_len;
                    }
                    ...
                    if ((ATC_NO_REPLY == ccfc_reason) && !PARAM10_FLAG)
                    {
                        //re_value =  ERR_INVALID_INDEX;
                        //break;
                    ...
                    temp_flag = ATC_ConvertTeleStrToStruct(
                                    &(register_param.cfnry_param.forward_num),
                                    PARA5.str_ptr->str_ptr,
                                    PARA5.str_ptr->str_len);
                    if (!temp_flag)
                    {
                        //SCI_TRACE_LOW:"ATC:CCFC reg fail3"
                        SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_GSM_SS_360_112_2_17_22_17_17_631,(uint8*)"");
                    ...
                    if (ATC_NO_REPLY == ccfc_reason && PARAM10_FLAG)
                    {
                         register_param.cfnry_param.no_reply_time = (uint8)PARA10.num;
                    }
             }
            MNSS_RegisterSSInfoEx(dual_sys, ss_code, basic_service, register_param);
            ATC_ADD_EXPECTED_EVENT(atc_config_ptr,APP_MN_REGISTER_SS_CNF,0xff,ATC_NO_DOMAIN);
...
AT_CMD_FUNC( ATC_ProcessCACM )
{
    ...
    switch (ATC_GET_CMD_TYPE)
    {
        ...
        case ATC_CMD_TYPE_READ:
        {
            atc_config_ptr->operate = ATC_PIN_GET_ACM;
            MNSIM_ReadAcmUnitEx(dual_sys);
            return S_ATC_DEFAULT_HANDLE;
        }
