...
struct GPRS_Act_Wait_Att_Rsp_Info
{
    ...
    uint8 act_state;
    BOOLEAN is_all;
    uint32 cid[MAX_PDP_CONTEXT_COUNT];
};
...
static ATC_DATA_PENDING_LIST_T s_data_req_pending_list_arr[MN_GPRS_MAX_PDP_CONTEXT_COUNT] = {0};
...
/******************************************************************************/  
//    DESCRIPTION: Add data req to pending list.
//    Global resource dependence : s_data_req_pending_list_arr 
//    AUTHOR:
//    Note : 
/******************************************************************************/ 
LOCAL void AddDataReqToPendingList(
                uint8  *data_ptr, 
                uint32 data_size,
                uint8  nsapi
                );

#if defined(_GPRS_FULL_FUNCTION_)||defined(_ATC_UIX8910_ENABLE_)
/******************************************************************************/  
...
ATC_STATUS  ATC_ProcessCGDSCONT( // return the handle result
                ...
                ATC_CUR_INFO_LIST_T *cur_cmd_info) // point to the atc cmd info
{
 #if defined( _SUPPORT_GPRS_ ) || defined(_ATC_UIX8910_ENABLE_)
        ATC_SET_PDP_CONTEXT_REQ_T  mn_info;
        ERR_MNGPRS_CODE_E ret = ERR_MNGPRS_NO_ERR;
        MN_GPRS_PDP_CON_T    *pdp_con_arr =  PNULL;
        MN_GPRS_PDP_CON_T    pdp_con_pcid;
        MN_GPRS_CON_STATE_T *pdp_state_arr = PNULL;
        uint32   pdp_con_num = 0;
        ...
        ATC_TRACE("ATGPRS:ATC_ProcessCGDSCONT\n");
        ...
        switch(ATC_GET_CMD_TYPE)
        {
            case ATC_CMD_TYPE_SET:
               ...
               uP_cid = PARA2.num;
                ...
                if (0 == pdp_con_num || uP_cid > pdp_con_num)
                {
                     return ERR_OPERATION_NOT_SUPPORTED;
                } 
#else
                ret = MNGPRS_GetPdpCxt(uP_cid, &pdp_con_pcid, dual_sys);
                if (ERR_MNGPRS_NO_ERR != ret)
                {
                    return ERR_OPERATION_NOT_SUPPORTED;
                }
               ...
               if (mn_info.param_used[2])
               {
                    mn_info.d_comp =PARA3.num;
                } else
                {
#ifndef _ATC_UIX8910_ENABLE_
                    mn_info.d_comp = pdp_con_arr[uP_cid].d_comp;
#else
                    mn_info.d_comp = pdp_con_pcid.d_comp;
#endif
                 }
                //param4 ---HCOMP
                if (mn_info.param_used[3])
                {
                    mn_info.h_comp = PARA4.num;
                } else
                {
#ifndef _ATC_UIX8910_ENABLE_
                    mn_info.h_comp = pdp_con_arr[uP_cid].h_comp;
#else
                    mn_info.h_comp = pdp_con_pcid.h_comp;
#endif
                }
#ifndef _ATC_UIX8910_ENABLE_
                ret = MNGPRS_SetPdpContextEx(dual_sys, mn_info.param_used,
                                       mn_info.pdp_id,
                                       pdp_con_arr[uP_cid].pdp_addr_type,
                                       pdp_con_arr[uP_cid].apn,
                                       pdp_con_arr[uP_cid].pdp_addr_str,
                                       mn_info.d_comp,
                                       mn_info.h_comp,
                                       mn_info.pco); 
                if(PNULL != pdp_con_arr)
                ...
                ret = MNGPRS_SetPdpContextEx(dual_sys, mn_info.param_used,
                                       mn_info.pdp_id,
                                       pdp_con_pcid.pdp_addr_type,
                                       pdp_con_pcid.apn,
                                       pdp_con_pcid.pdp_addr_str,
                                       mn_info.d_comp,
                                       mn_info.h_comp,
                                       mn_info.pco);
#endif
               ...
               ret = MNGPRS_SetPdpContextPcidEx(dual_sys, mn_info.pdp_id, uP_cid);
               if (ERR_MNGPRS_NO_ERR != ret)
               {
                    return ERR_OPERATION_NOT_SUPPORTED;
               }
            ...
            case ATC_CMD_TYPE_READ:
                ret = MNGPRS_ReadPdpContextEx(dual_sys, &pdp_con_arr, &pdp_con_num);
                if (ERR_MNGPRS_NO_ERR != ret  || 0 == pdp_con_num)
                {
                    if(pdp_con_arr != PNULL)
                    {
                        SCI_FREE(pdp_con_arr);
                ...
                {
                    // There isn't PDP context.
                    sprintf((char*)g_rsp_str,"%s:", g_atc_info_table[AT_CMD_CGDSCONT].cmd_name->str_ptr); 
                }
                else
                {
#endif
                for (i = 0; i < pdp_con_num; i++)
                {
                    if (0 == i)
                    {
                        sprintf((char*)g_rsp_str,"+CGDSCONT: %d, %d, %d",
                                                 pdp_con_arr[i].pdp_id,
                                                 pdp_con_arr[i].d_comp,
                                                 pdp_con_arr[i].h_comp);
                    }
                    else
                    {
                        sprintf((char*)g_rsp_str+strlen((char*)g_rsp_str),"\r\n+CGDSCONT: %d, %d, %d",
                ...
                break;
            case ATC_CMD_TYPE_TEST:
                ret = MNGPRS_ReadPdpContextStateEx(dual_sys, &pdp_state_arr, &pdp_con_num);
                ...
                if (0 == pdp_con_num)
                {
                    sprintf((char *)g_rsp_str, "%s:  (1..11),(0),(\"IP\",\"IPV6\",\"PPP\"),(0..3),(0..4)",
                            g_atc_info_table[AT_CMD_CGDSCONT].cmd_name->str_ptr);
                    ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
                }
                else
                {
                    for (i = 0; i < pdp_con_num; i++)
                    {
                        if (MN_CONTEXT_ACTIVATED == pdp_state_arr[i].pdp_state)
                        {
                            sprintf((char *)g_rsp_str, "%s: (1..11),(%d),(\"IP\",\"IPV6\",\"PPP\"),(0..3),(0..4)",
                                    g_atc_info_table[AT_CMD_CGDSCONT].cmd_name->str_ptr, pdp_state_arr[i].pdp_id);
                            ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
                            break;
                        }
                    }
...
//  Note :
/******************************************************************************/
MNGPRS_TFT_SET g_Atc_Tft_Set[2][11] = {0,};

int32 ATC_GPRS_IPStringToPDPAddr(uint8 *pPdpAddrStr, uint8 uSize, uint8 *pPdpAddr, uint8 *pAddrSize)
{
    uint8 i       = 0;
    uint8 j       = 0;
    uint8 m       = 0;
    ...
    ATC_TRACE("ATC:pPdpAddrStr: %s, uSize: %d", pPdpAddrStr, uSize);
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        ATC_TRACE("ATC:uBuf: %s, uIpNum: %d, j: %d", uBuf, uIpNum, j);
...
int32 ATC_GPRS_IPV6StringToPDPAddr(uint8 *pPdpAddrStr, uint8 uSize, uint8 *pPdpAddr, uint8 *pAddrSize)
{
    uint8 i       = 0;
    uint8 j       = 0;
    uint8 m       = 0;
    ...
    ATC_TRACE("ATC:pPdpAddrStr(v6): %s, uSize: %d", pPdpAddrStr, uSize);
...
uint32 ATC_GetRange(uint8* StrRang, uint32* Range)
{
    uint8 i = 0;
    uint16 value = 0;
    uint8 length = strlen(StrRang);
    ATC_TRACE("ATC:StrRang = %s",StrRang);
    for(i = 0; i < length; i++)
    {
        uint8 c = StrRang[i];
        if((c >= '0') && (c <= '9'))
    ...
    *Range = *Range << 16;
    *Range |= value;
    ATC_TRACE("ATC:High 16 Range = %d",*Range >> 16);
    ATC_TRACE("ATC:Low 16 Range = %d",(*Range)& 0xFFFF);

    return 1;
}

uint8 ATC_ascii2hex(uint8* pInput, uint8* pOutput)
{
    uint16 i = 0;
...
)
{
    ...
    uint16 tos;
    uint32 ret;
    uint8 source_mask[130] = {0};
    uint8 count_dot = 0;
    uint8 is_ipv6 = 0;
    uint8 ip_dot_count = 4;
    uint8 i, k = 0;
    uint8 addr_Len = 0;
    uint8 ipsec_hex[9] = {0};
    uint8 hex_len;

    ATC_TRACE("ATC: ATC_ProcessCGTFT");
    ...
    ATC_TRACE("ATC: ATC_ProcessCGTFT  %d ", ATC_GET_CMD_TYPE);
    switch(ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
            if (PARAM1_FLAG)
            ...
            {
                 memset(&(g_Atc_Tft_Set[dual_sys][cid]), 0, sizeof(MNGPRS_TFT_SET));
                 packet_filter_id = PARA2.num;
                 eval_prece_index = PARA3.num;
                 g_Atc_Tft_Set[dual_sys][cid].Pf_ID = packet_filter_id;
                 g_Atc_Tft_Set[dual_sys][cid].Pf_BitMap |= 0x8000;
                 g_Atc_Tft_Set[dual_sys][cid].Pf_EvaPreIdx = eval_prece_index;
                 g_Atc_Tft_Set[dual_sys][cid].Pf_BitMap |= 0x4000;
            }
            ...
            if (PARAM4_FLAG)
            {
                ...
                if(PARAM4_TYPE != ATC_CMD_PARAM_TYPE_STRING ||  PARA4.str_ptr->str_len > 127)
                {
                    return ERR_OPERATION_NOT_SUPPORTED;
                }

                SCI_Memcpy(source_mask, PARA4.str_ptr->str_ptr, PARA4.str_ptr->str_len);

                for(i = 0; i < PARA4.str_ptr->str_len; i++)
                {
                    if(!((source_mask[i] >= '0'  && source_mask[i] <= '9')||source_mask[i] == '.'))
                    {
                        ATC_TRACE("ATC:invalid parameter");
                        return ERR_OPERATION_NOT_SUPPORTED;
                    }
                    if(source_mask[i] == '.')
                        count_dot++;
                }
                ATC_TRACE("ATC:CGTFT: count_dot = %d", count_dot);
                if(count_dot >= 31)
                {
                    is_ipv6 = 1;
                    ip_dot_count = 16;
                }
                for(i = 0; i < PARA4.str_ptr->str_len; i++) // try to find ip end . and set it to 0
                {
                 ...
                 if(k != 0xFF)
                {
                    ATC_TRACE("ATC:SourceMask address is invalid(%s)", source_mask);
                    return ERR_OPERATION_NOT_SUPPORTED;
                }

                if(is_ipv6 == 0)
                {
                    if(ATC_GPRS_IPStringToPDPAddr(source_mask, i, &g_Atc_Tft_Set[dual_sys][cid].Pf_RmtAddrIPv4[0], &addr_Len) != 1)
                    {
                        ATC_TRACE("ATC:source address is invalid(%s)", source_mask);
                        return ERR_OPERATION_NOT_SUPPORTED;
                    }
                    if(addr_Len > 4)
                    {
                        ATC_TRACE("ATC:source address len error (%s)", source_mask);
                        return ERR_OPERATION_NOT_SUPPORTED;
                    }

                    if(ATC_GPRS_IPStringToPDPAddr(&(source_mask[i]), strlen(source_mask + i), &g_Atc_Tft_Set[dual_sys][cid].Pf_RmtAddrIPv4[4], &addr_Len) != 1)
                    {
                        ATC_TRACE("ATC:mask address is invalid(%s)", source_mask + i);
                        return ERR_OPERATION_NOT_SUPPORTED;
                    }
                    if(addr_Len > 4)
                    {
                        ATC_TRACE("ATC:T_mask address len error (%s)", source_mask + i);
                        return ERR_OPERATION_NOT_SUPPORTED;
                    }
                    g_Atc_Tft_Set[dual_sys][cid].Pf_BitMap |= 0x2000;
                }
                else
                {
                    if(ATC_GPRS_IPV6StringToPDPAddr(source_mask, i, &g_Atc_Tft_Set[dual_sys][cid].Pf_RmtAddrIPv6[0], &addr_Len) != 1)
                    {
                        ATC_TRACE("ATC:source address is invalid(%s)", source_mask);
                        return ERR_OPERATION_NOT_SUPPORTED;
                    }
                    ...
                    if(ATC_GPRS_IPV6StringToPDPAddr(&(source_mask[i]), strlen(source_mask + i), &g_Atc_Tft_Set[dual_sys][cid].Pf_RmtAddrIPv6[16], &addr_Len) != 1)
                    {
                        ATC_TRACE("ATC:mask address is invalid(%s)", source_mask + i);
                        return ERR_OPERATION_NOT_SUPPORTED;
                    }
                    if(addr_Len > 16)
                    {
                        ATC_TRACE("ATC:mask address len error (%s)", source_mask + i);
                        return ERR_OPERATION_NOT_SUPPORTED;
                    }
                    g_Atc_Tft_Set[dual_sys][cid].Pf_BitMap |= 0x1000;
                }
            }

            if (PARAM5_FLAG)
            {
                g_Atc_Tft_Set[dual_sys][cid].Pf_PortNum_NH = PARA5.num;
                g_Atc_Tft_Set[dual_sys][cid].Pf_BitMap |= 0x0800;
            }

            if (PARAM6_FLAG)
            {                     
                if(PARA6.str_ptr == NULL || PARAM6_TYPE != ATC_CMD_PARAM_TYPE_STRING)
                {
                    return ERR_OPERATION_NOT_SUPPORTED;
                }

                if(ATC_GetRange(PARA6.str_ptr->str_ptr, &local_port_range )== 0)
                {
                    ATC_TRACE("ATC:Get sixth Parameter error!  local_port_range = %d", local_port_range);
                    return ERR_OPERATION_NOT_SUPPORTED;
                }
                ATC_TRACE("ATC:CGTFT: local_port_range = %d", local_port_range);
                g_Atc_Tft_Set[dual_sys][cid].Pf_LocalPortRangeMin = local_port_range>>16;
                g_Atc_Tft_Set[dual_sys][cid].Pf_LocalPortRangeMax = local_port_range&0xFFFF; 
                g_Atc_Tft_Set[dual_sys][cid].Pf_BitMap |= 0x0400;
            }

            if (PARAM7_FLAG)
            {
                 if(PARA7.str_ptr == NULL || PARAM7_TYPE != ATC_CMD_PARAM_TYPE_STRING)
                {
                    return ERR_OPERATION_NOT_SUPPORTED;
                }
                ret = ATC_GetRange(PARA7.str_ptr->str_ptr, &remote_port_range );
                if(!ret)
                {
                    ATC_TRACE("ATC:Get sixth Parameter error!  local_port_range = %d", remote_port_range);
                    return ERR_OPERATION_NOT_SUPPORTED;
                }
                ATC_TRACE("ATC:CGTFT: local_port_range = %d", remote_port_range);
                g_Atc_Tft_Set[dual_sys][cid].Pf_RemotPortRangeMin = remote_port_range>>16;
                g_Atc_Tft_Set[dual_sys][cid].Pf_RemotPortRangeMax = remote_port_range&0xFFFF; 
                g_Atc_Tft_Set[dual_sys][cid].Pf_BitMap |= 0x0400;
            }

            if (PARAM8_FLAG)
            {
                if(PARA8.str_ptr == NULL || PARAM8_TYPE != ATC_CMD_PARAM_TYPE_STRING)
                {
                    return ERR_OPERATION_NOT_SUPPORTED;
                }
                if ( PARA8.str_ptr->str_len > 8)
                {
                    ATC_TRACE("ATC:Get 8th Parameter error!");
                    return ERR_OPERATION_NOT_SUPPORTED;
                }
                if((hex_len = ATC_ascii2hex(PARA8.str_ptr->str_ptr, ipsec_hex)) == 0)
                {
                    ATC_TRACE("ATC:Get 8th Parameter error!format error");
                    return ERR_OPERATION_NOT_SUPPORTED;
                }
                ipsec_index = 0;
                for(i = 0; i < hex_len;i++)
                ...
                g_Atc_Tft_Set[dual_sys][cid].Pf_SPI = ipsec_index;
                g_Atc_Tft_Set[dual_sys][cid].Pf_BitMap |= 0x0100;
            }

             if (PARAM9_FLAG)
            {
                uint32 type_service = 0;
                if(PARA9.str_ptr == NULL || PARAM9_TYPE != ATC_CMD_PARAM_TYPE_STRING)
                {
                    return ERR_OPERATION_NOT_SUPPORTED;
                }

                if (ATC_GetRange(PARA9.str_ptr->str_ptr, &type_service) == 0)
                {
                    ATC_TRACE("ATC:Get 9th Parameter error! tos = %d", type_service);
                    return ERR_OPERATION_NOT_SUPPORTED;
                }
                ATC_TRACE("ATC:CGTFT: type_service = %x", type_service);

                if((( type_service>> 16) > 255) || ((type_service & 0xFFFF) > 255))
                {
                    ATC_TRACE("ATC:Get 9th Parameter error!high tos = %d, low = %d", type_service>>16, type_service&0xFFFF);
                    return ERR_OPERATION_NOT_SUPPORTED;
                }
                tos=(type_service>>8)|(type_service&0xFF);
                ATC_TRACE("ATC:CGTFT: uTypeOfService = %x", type_service);
                ATC_TRACE("ATC:CGTFT: high tos = %d, low = %d", tos>>8, tos&0xFF);

                g_Atc_Tft_Set[dual_sys][cid].Pf_TOS_TC[0] = tos>>8;
                g_Atc_Tft_Set[dual_sys][cid].Pf_TOS_TC[1] = tos&0xFF;
                g_Atc_Tft_Set[dual_sys][cid].Pf_BitMap |= 0x0080;
            }

             if (PARAM10_FLAG)
            {
                g_Atc_Tft_Set[dual_sys][cid].Pf_SPI = PARA10.num;
                g_Atc_Tft_Set[dual_sys][cid].Pf_BitMap |= 0x0100;
            }

             ret = MNGPRS_SetGprsCtxTFT(cid, g_Atc_Tft_Set[dual_sys][cid], dual_sys);
             if (ret)
             {
                 return ERR_OPERATION_NOT_SUPPORTED;
             }
            break;
        case ATC_CMD_TYPE_READ:
             ATC_TRACE("ATC:CGTFT: AT_CMD_READ");
             for(i = 0; i< 7; i++)
             {
                 if(g_Atc_Tft_Set[dual_sys][i].Pf_ID == 0)
                        continue;
                 sprintf((char *)g_rsp_str + strlen((char*)g_rsp_str), "%s: %d, %d, %d, %d.%d.%d.%d.%d.%d.%d.%d, %d ,%d.%d,%d.%d,%x,%d.%d",
                                                 g_atc_info_table[AT_CMD_CGTFT].cmd_name->str_ptr,
                                                 i,g_Atc_Tft_Set[dual_sys][i].Pf_ID,  g_Atc_Tft_Set[dual_sys][i].Pf_EvaPreIdx,
                                                 g_Atc_Tft_Set[dual_sys][i].Pf_RmtAddrIPv4[0],g_Atc_Tft_Set[dual_sys][i].Pf_RmtAddrIPv4[1],
                                                 g_Atc_Tft_Set[dual_sys][i].Pf_RmtAddrIPv4[2],g_Atc_Tft_Set[dual_sys][i].Pf_RmtAddrIPv4[3],
                                                 g_Atc_Tft_Set[dual_sys][i].Pf_RmtAddrIPv4[4],g_Atc_Tft_Set[dual_sys][i].Pf_RmtAddrIPv4[5],
                                                 g_Atc_Tft_Set[dual_sys][i].Pf_RmtAddrIPv4[6],g_Atc_Tft_Set[dual_sys][i].Pf_RmtAddrIPv4[7],
                                                 g_Atc_Tft_Set[dual_sys][i].Pf_PortNum_NH,
                                                 g_Atc_Tft_Set[dual_sys][i].Pf_LocalPortRangeMin,g_Atc_Tft_Set[dual_sys][i].Pf_LocalPortRangeMax,
                                                 g_Atc_Tft_Set[dual_sys][i].Pf_RemotPortRangeMin,g_Atc_Tft_Set[dual_sys][i].Pf_RemotPortRangeMax,
                                                 g_Atc_Tft_Set[dual_sys][i].Pf_SPI,
                                                 g_Atc_Tft_Set[dual_sys][i].Pf_TOS_TC[0],
                                                 g_Atc_Tft_Set[dual_sys][i].Pf_TOS_TC[1]);
             }
            ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
            break;
        default:
                ...
                ATC_CUR_INFO_LIST_T *cur_cmd_info)
{
    ...
    MN_DUAL_SYS_E dual_sys = ATC_CURRENT_SIM_ID_MUX;
    MNGPRS_EQOS eqos = { 0, 0, 0, 0, 0 };
    uint8*    ptr;
    uint8 cid = 0;
    uint32 ret = 0;
    ATC_TRACE("ATGPRS:ATC_ProcessCGEQOS");
    ...
    switch(ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            ...
            if (PARAM3_FLAG)
            {
                eqos.nDlGbr= PARA3.num;
            }

            if (PARAM4_FLAG)
            {
                eqos.nUlGbr= PARA4.num;
            }

            if (PARAM5_FLAG)
            {
                eqos.nDlMbr= PARA5.num;
            }

            if (PARAM6_FLAG)
            {
                eqos.nUlMbr= PARA6.num;
            }

            ret = MNGPRS_GprsSetCtxEQos(cid,eqos,dual_sys);
            if (0 != ret)
            {
                ATC_TRACE("ATGPRS:MNGPRS_GprsSetCtxEQos() failed,ret =  0x%x.",  ret);
        ...
        case ATC_CMD_TYPE_READ:
        {
            ...
            {
                ret = MNGPRS_GprsGetCtxEQos(cid, &eqos, dual_sys);
                if (0 == ret)
                {
                     sprintf((char*)ptr, "%s: %d, %d, %d, %d, %d, %d \r\n",
                                              g_atc_info_table[AT_CMD_CGEQOS].cmd_name->str_ptr, cid, 
                                              eqos.nQci,eqos.nDlGbr,eqos.nUlGbr,eqos.nDlMbr,eqos.nUlMbr);
                     ptr = g_rsp_str + strlen((char*)g_rsp_str);
                  }
                  else
                  {
                      ATC_TRACE("ATGPRS:AT+CGEQOS:MNGPRS_GprsGetCtxEQos() failed, ret = 0x%x,cid = %d.", ret, cid);
                      at_status = ERR_OPERATION_NOT_SUPPORTED;
                      break;
                  }
            }
...
#else
            g_state = MNPHONE_GetAttState(dual_sys);
            sprintf((char*)g_rsp_str, "+CGATT: %d", g_state);
#endif
            ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
            break;
        }
        ...
        case ATC_CMD_TYPE_SET:
        {
            ...
            for(i = 0; param_ptr != NULL && i < MN_GPRS_MAX_PDP_CONTEXT_COUNT; i++)
            {
                ...
                if(cur_cmd_info->param_used[i+1])
                {
                    ...
                    g_uGPRS_Act_Wait_Att_Rsp_Info.cid[i] = param_ptr->param_info.num;
                }    
            ...
            g_state = MNPHONE_GetAttState(dual_sys);
            cur_rat = MNPHONE_GetCurrentRAT(dual_sys);
            if ((1 == state) && (cur_rat != MN_GMMREG_RAT_4G) && ( 0 == g_state) )
            {  
                g_uGPRS_Act_Wait_Att_Rsp_Info.wait_state = 1;
                g_uGPRS_Act_Wait_Att_Rsp_Info.act_state  = state;
                g_uGPRS_Act_Wait_Att_Rsp_Info.is_all = mn_info.is_all;
                MNPHONE_GprsAttachEx(dual_sys, MN_PHONE_GPRS_ATTACH);
                ATC_ADD_EXPECTED_EVENT(atc_config_ptr,APP_MN_GPRS_ATTACH_RESULT,0xff,ATC_NO_DOMAIN);
                return S_ATC_DEFAULT_HANDLE;
            }
...
//    Note : 
/******************************************************************************/ 
ATC_STATUS ATC_ProcessCGCMOD( // return the handle result
                ATC_CONFIG_T   * atc_config_ptr, // point to the ATC_CONFIG_T struct.
                ATC_CUR_INFO_LIST_T *cur_cmd_info // point to the atc cmd info
                            )
{
    ...
    SCI_TRACE_LOW("ATC:ATC_ProcessCGCMOD\n");
    ...
    switch(cmd_type)
    {
        case ATC_CMD_TYPE_SET:
        {
            ...
            for(i = 0; i < param_num; i++)
            {
                ...
                if(params[i].param_len != 0)
                {
                    ...
                    #endif
                    mn_info.pdp_id[i] = params[i].param[0]; 
                    ret = MNGPRS_EpsModifyPdpContextEx(params[i].param[0], dual_sys);
                    if (ERR_MNGPRS_NO_ERR != ret)
                    {
                        break;
                    }
            ...
            if (ERR_MNGPRS_NO_ERR != ret)
            {
                 SCI_TRACE_LOW("ATC: CGDMOD() failed,iResult = 0x%x.",  ret);
                 result = (MN_GPRS_ERR_E)ERR_MNGPRS_CALL_FAILURE;
                 break;
            }
#ifndef _ATC_UIX8910_ENABLE_
            ...
            {
                SCI_TRACE_LOW("ATC: invalid parameter");
                result = (MN_GPRS_ERR_E)ERR_MNGPRS_INVALID_PDP_ID;
                break;
            }
            if (i > 0)
            {
        ...
        case ATC_CMD_TYPE_READ:
        {
             sprintf((char*)g_rsp_str, "+CGCMOD: (1,......7)");
             ATC_BuildInfoRsp(atc_config_ptr,g_rsp_str);
             result = S_ATC_SUCCESS;
             break;
        }    
                             ...
                             )
{
	...
	#ifdef PPP_USB
            SIO_ATC_WriteCmdRes((uint8*)g_rsp_str, strlen((char*)g_rsp_str),ATC_GetSioPort());
	#else
            SIO_ATC_WriteCmdRes((uint8*)g_rsp_str, strlen((char*)g_rsp_str));
	#endif	
#endif
    ...
    switch(ATC_GET_CMD_TYPE)
    {
        case ATC_CMD_TYPE_SET:
        {
            ...
            else
            {
                ...
                {
                    //SCI_TRACE_LOW:"ATC: ATC_ProcessSGPRSDATA, ERROR: MNGPRS_ReadPdpContextState failed."
                    SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_PLUS_GPRS_2025_112_2_17_22_17_52_793,(uint8*)"" );
                ...
                {
                    //SCI_TRACE_LOW:"ATC: ATC_ProcessSGPRSDATA, ERROR: invalid active pdp count=%d."
                    SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_PLUS_GPRS_2048_112_2_17_22_17_52_794,(uint8*)"d",active_pdp_count);
                    nsapi = 0;
                    return ERR_OPERATION_NOT_ALLOWED;              
                }
            }
            ...
            if (PARAM3_FLAG)    //data
            {
                ...
                {
                    //SCI_TRACE_LOW:"ATC: ATC_ProcessSGPRSDATA, too long length!"
                    SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_PLUS_GPRS_2063_112_2_17_22_17_52_795,(uint8*)"" );
                    return ERR_INVALID_INDEX;
                }
            }
            ...
            {
                //ATC_TRACE:"ATGPRS: The GPRS send data len beyond MAX_AT_GPRSDATA_SEND_LEN!\n"
                SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_PLUS_GPRS_2070_112_2_17_22_17_52_796,(uint8*)"" );
                return ERR_OPERATION_NOT_SUPPORTED;
            }
            ...
            for(i=0; i<num; i++)
            {
                data_ptr = SCI_ALLOC_BASE(GPRS_MTU); 
                ...
                {
                	AddDataReqToPendingList(data_ptr,GPRS_MTU,nsapi);
                }
                else
                {
	                //SCI_TRACE_LOW:"ATC: ATC_ProcessSGPRSDATA, send data len=%d,nsapi=%d"
	                SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_PLUS_GPRS_2089_112_2_17_22_17_52_797,(uint8*)"dd",GPRS_MTU,nsapi);
	                value = MNGPRS_SendDataReq(data_ptr, GPRS_MTU, nsapi);

	                if (ERR_MNGPRS_NO_ERR != value)
	                {
	                    //SCI_TRACE_LOW:"ATC: ATC_ProcessSGPRSDATA, MNGPRS_SendDataReq failed!"
	                    SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_PLUS_GPRS_2094_112_2_17_22_17_52_798,(uint8*)"" );
	                    SCI_FREE(data_ptr);
	                }
                }
            }            
            ...
            if (remain_len > 0)
            {
                data_ptr = SCI_ALLOC_BASE(remain_len);  
                ...
                if(MNGPRS_IsFlowCtrlOpen(nsapi))
                {
                	AddDataReqToPendingList(data_ptr,remain_len,nsapi);
                }
                else
                {
	                //SCI_TRACE_LOW:"ATC: ATC_ProcessSGPRSDATA, send data len=%d,nsapi=%d"
	                SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_PLUS_GPRS_2111_112_2_17_22_17_52_799,(uint8*)"dd",remain_len,nsapi);
	                value = MNGPRS_SendDataReq(data_ptr, remain_len, nsapi);
	            
	                if (ERR_MNGPRS_NO_ERR != value)
	                {
	                    //SCI_TRACE_LOW:"ATC: ATC_ProcessSGPRSDATA, MNGPRS_SendDataReq failed!"
	                    SCI_TRACE_ID(TRACE_TOOL_CONVERT,ATC_PLUS_GPRS_2116_112_2_17_22_17_52_800,(uint8*)"" );
	                    SCI_FREE(data_ptr);
	                }
                }
            } 
...
#ifdef _SUPPORT_GPRS_    
/******************************************************************************/  
//    DESCRIPTION: Send the data in pending list.
//    Global resource dependence : s_data_req_pending_list_arr 
//    AUTHOR:
//    Note : 
/******************************************************************************/ 
PUBLIC void ATC_HandleDataReqPendingList(uint8  nsapi)
{
	ATC_DATA_INFO_T *data_info_ptr  = NULL;

	SCI_ASSERT((nsapi >= 5) && (nsapi <= 15));/*assert verified:invalid NSAPI*/

	data_info_ptr = s_data_req_pending_list_arr[nsapi - 5].head;

	while((NULL != data_info_ptr) && (!MNGPRS_IsFlowCtrlOpen(nsapi)))
	{
		int value = MNGPRS_SendDataReq(data_info_ptr->data_ptr, data_info_ptr->data_size, nsapi);

		if(ERR_MNGPRS_NO_ERR != value)
		{
			SCI_FREE(data_info_ptr->data_ptr);
		}

		s_data_req_pending_list_arr[nsapi - 5].head = data_info_ptr->next;
	...
	if(NULL == data_info_ptr)
	{
           s_data_req_pending_list_arr[nsapi - 5].tail = NULL;
	}
}

/******************************************************************************/  
//    DESCRIPTION: Free the data req pending list.
//    Global resource dependence : s_data_req_pending_list_arr 
//    AUTHOR:
//    Note : 
/******************************************************************************/
PUBLIC void ATC_FreeDataReqPendingList(uint8  nsapi)
{
    ATC_DATA_INFO_T *data_info_ptr  = NULL;
                        ...
                        )
{
    ...
    #endif
    #ifndef _ATC_UIX8910_ENABLE_
        ATC_SendDetachReq(((APP_MN_GPRS_DEACTIVATE_IND_T*)sig_ptr)->dual_sys);//LTE  need remove
    #endif
    #ifdef _ATC_GPRS_MODEM_        
        ResetGPRSDataConf();
    #endif
...
ATC_STATUS ATC_ProcessAttResult(
                        ATC_CONFIG_T        *atc_config_ptr,
                        APP_MN_GPRS_ATTACH_RESULT_T       *sig_ptr)
{
    MN_DUAL_SYS_E dual_sys = ATC_CURRENT_SIM_ID_MUX;
    uint8 ret;
    SCI_TRACE_LOW("ATC: ATC_ProcessAttResult %d", g_uGPRS_Act_Wait_Att_Rsp_Info.wait_state );
    if( sig_ptr->is_attach_ok) 
    {
       if (g_uGPRS_Act_Wait_Att_Rsp_Info.wait_state == 1)
       {
           g_uGPRS_Act_Wait_Att_Rsp_Info.wait_state = 0;
           ret = MNGPRS_ActivatePdpContextEx(dual_sys, g_uGPRS_Act_Wait_Att_Rsp_Info.is_all,g_uGPRS_Act_Wait_Att_Rsp_Info.cid,POC_E,MN_UNSPECIFIED);
           ATC_ADD_EXPECTED_EVENT(atc_config_ptr,APP_MN_ACTIVATE_PDP_CONTEXT_CNF,g_uGPRS_Act_Wait_Att_Rsp_Info.cid[0],ATC_DOMAIN_PS);
   ...
   {
        ATC_BuildResultCodeRsp(atc_config_ptr,S_ATC_RESULT_CODE_ERROR, sig_ptr->err_cause );
   }
                        ...
                        )
{
...
#endif /* PCLINK_ENABLE */
    {
        ...
        if (TRUE == atc_global_info_ptr->atc_config_ptr->pdp_id_arr[pid-1])
        {
            if((((APP_MN_GPRS_EXT_T*)sig_ptr)->result >= MN_GPRS_ERR_SUCCESS) && (((APP_MN_GPRS_EXT_T*)sig_ptr)->result <= MN_GPRS_ERR_SUCCESS_SINGLE_ONLY))
            {
                SCI_TRACE_LOW("ATC:APP_MN_ACTIVATE_PDP_CONTEXT_CNF pid:%d",pid);

                    // for ATX4
                    if(ATC_CONNECT_WITH_BAUD == atc_global_info_ptr->atc_config_ptr->connect_result_code_mode)
                    {
                        uint32        baud_rate = 0;
                       ...
                       sprintf((char *)g_rsp_str, "CONNECT %d", baud_rate);
                       SCI_TRACE_LOW("atc: g_rsp_str: %s", g_rsp_str);
                       //ATC_BuildResultCodeRsp(atc_config_ptr,S_ATC_RESULT_CODE_OK, 0);
                       ATC_BuildInfoRsp(atc_config_ptr, g_rsp_str);
                    }
                    else
		   ...
		   SIO_ATC_ClrCmdLine(ATC_GetSioPort());
	        ...
	        SIO_ATC_ClrCmdLine();
                ...
                {
                    set_ppp_result = MPPP_SetPPPInterface(pid+4,0,pid);
                }
                else
                {
                    set_ppp_result = MPPP_SetPPPInterface(pid+4,ATC_GetLinkIdFromCid(pid),pid);
                }
                ...
                if(set_ppp_result)
                {
                    ...
                    SCI_TRACE_LOW("ATC: MPPP_SetPPPInterface() fail");

                    pdp_id_arr[0] = (uint32)pid;
                    
                    if(ERR_MNGPRS_NO_ERR != MNGPRS_DeactivatePdpContextEx(g_atc_modem_dial_sys, FALSE, pdp_id_arr))
                    {
                        SCI_TRACE_LOW("ATC: deactivate pdp fail, cid=%d", pid);
                    }
                }
            #else
                #ifdef TCPIP_SUPPORT
                    if(0 != TCPIP_SetPPPInterface(((APP_MN_GPRS_EXT_T *)sig_ptr)->nsapi))
                    {
                        uint32 pdp_id_arr[MN_GPRS_MAX_PDP_CONTEXT_COUNT] = {0};
                        
                        SCI_TRACE_LOW("ATC: TCPIP_SetPPPInterface() fail");

                        pdp_id_arr[0] = (uint32)pid;
                        
                        if(ERR_MNGPRS_NO_ERR != MNGPRS_DeactivatePdpContextEx(((APP_MN_GPRS_EXT_T*)sig_ptr)->dual_sys, FALSE, pdp_id_arr))
                        {
                            SCI_TRACE_LOW("ATC: deactivate pdp fail, cid=%d", pid);
                        }
                    }
...
#ifdef ATC_SAT_ENABLE //wuding: faturephone test case, at+cgact=1,1 will response +CME ERR: 28
#ifdef TCPIP_SUPPORT 
#ifndef _GSM_ONLY_
                ...
                )
{
    ...
    data_info_ptr = (ATC_DATA_INFO_T *)SCI_ALLOC_BASEZ(sizeof(ATC_DATA_INFO_T));

    if(data_info_ptr != NULL)
    {
        data_info_ptr->data_ptr  = data_ptr;
        data_info_ptr->data_size = data_size;
        data_info_ptr->next        = NULL;

        if(NULL == s_data_req_pending_list_arr[nsapi - 5].tail)
        {
            s_data_req_pending_list_arr[nsapi - 5].head = data_info_ptr;
        }
        else
        {
            s_data_req_pending_list_arr[nsapi - 5].tail->next = data_info_ptr;
        }

        s_data_req_pending_list_arr[nsapi - 5].tail = data_info_ptr;
    }
    else
    {
        SCI_FREE(data_ptr);
                ...
                )
{
    ...
    switch (mode)
    {
        ...
        case MN_GPRS_EVENT_REPORT_MODE_3:
        {
			...
			#ifdef PPP_USB
                    SIO_ATC_WriteCmdRes((uint8*)g_rsp_str, strlen((char*)g_rsp_str),ATC_GetSioPort());
                    SIO_ATC_WriteCmdRes(ev_report_ptr->buf, ev_report_ptr->len,ATC_GetSioPort());
			#else
                    SIO_ATC_WriteCmdRes((uint8*)g_rsp_str, strlen((char*)g_rsp_str) );
                    SIO_ATC_WriteCmdRes(ev_report_ptr->buf, ev_report_ptr->len);
			#endif					
