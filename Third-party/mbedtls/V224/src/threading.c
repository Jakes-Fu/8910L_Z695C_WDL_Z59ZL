...
{
    static unsigned int mutex_num = 0;
    char mutex_str[20] = {0};
    if (mutex == NULL)
    {
        SCI_TRACE_LOW(" mutex is NULL ");
        return;
    }
    sprintf(mutex_str, "mbedtls%d", mutex_num++);
    *mutex = SCI_CreateMutex(mutex_str,SCI_NO_INHERIT);
    //SCI_TRACE_LOW("threading_mutex_init  ---name:%s ", mutex_str);
}
