...
#if defined(PLATFORM_SC6531EFM) || defined(PLATFORM_UIX8910)
#define SRAM_START_ADDR         (0x04100000)
#else
...
LOCAL uint16 fixnv_crc = INVALID_VALUE;
...
LOCAL BOOLEAN verify_fixnv(uint32 addr)
{
    ...
    if (fixnv_address >= SRAM_START_ADDR)
    {
        fixnv_status = *(uint16 *)(fixnv_address + sector_size - SECTOR_STATUS_OFFSET);
    }
    else
    {
        fixnv_status = FDL_FlashReadWord(fixnv_address + sector_size - SECTOR_STATUS_OFFSET);
    }
    ...
    {
        //SIO_TRACE("STATUS ERR! %s %d fixnv_status=0x%x, fixnv_address=0x%x",
    ...
    if (fixnv_address >= SRAM_START_ADDR)
    {
        fixnv_length = *(uint16 *)(fixnv_address + sector_size - FIX_NV_LEN_OFFSET);
    }
    else
    {
        fixnv_length = FDL_FlashReadWord(fixnv_address + sector_size - FIX_NV_LEN_OFFSET);
    }

    calc_fixnv_len = calculate_fixNV_length(fixnv_address, fixnv_address + sector_size);

    if ((fixnv_length > sector_size - FIX_NV_LEN_OFFSET)
            || (fixnv_length <= sizeof(uint16))
            || (fixnv_length != calc_fixnv_len)
       )
    {
        SIO_TRACE("LENGTH ERROR! %s %d fixnv_length=0x%x, fixnv_address=0x%x, calc_fixnv_len=0x%x",
        ...
        )
    {
        ...
        {
            SIO_TRACE("CRC ERROR! %s %d fixnv_dedicate_crc=0x%x, fixnv_crc=0x%x, fixnv_address=0x%x",
    ...
    else
    {
        ...
        {
            SIO_TRACE("figure_crc ERROR! %s %d fixnv_dedicate_crc=0x%x, fixnv_figure_crc=0x%x, fixnv_address=0x%x",
    ...
    if (fixnv_address >= SRAM_START_ADDR)
    {
        prod_sec_dedicate_crc = *(uint16 *)(fixnv_address + sector_size - PRD_INFO_CRC_OFFSET);
        productinfo_offset = *(uint32 *)(fixnv_address + sector_size - PRD_INFO_OFFSET_ADDR);
    }
    else
    {
        prod_sec_dedicate_crc = FDL_FlashReadWord(fixnv_address + sector_size - PRD_INFO_CRC_OFFSET);
        productinfo_offset = FDL_FlashReadDWord (fixnv_address + sector_size - PRD_INFO_OFFSET_ADDR);
    }

    if (prod_sec_dedicate_crc != INVALID_VALUE)
    {    
        //productinfo_offset = Get_productinfo_offset();
        prod_crc_length = sector_size - productinfo_offset - FIX_NV_LEN_OFFSET;
        
        if ((prod_crc_length >= sector_size) || (prod_crc_length < FIX_NV_LEN_OFFSET))
        {
        ...
        {        
            prod_sec_figure_crc_old = crc16(0, fixnv_address + sizeof(uint16), sector_size - FIX_NV_LEN_OFFSET);
            ...
            SIO_TRACE("ERROR! %s %d prod_sec_dedicate_crc=0x%x prod_sec_figure_crc=0x%x fixnv_address=0x%x prod_sec_figure_crc_old=0x%x",
    ...
    else
    {
        ...
        {
            SIO_TRACE("PROD STATUS ERROR! %s %d fixnv_status=0x%x, fixnv_address=0x%x",
                          __func__, __LINE__, fixnv_status, fixnv_address);
    ...
    //SIO_TRACE("fixnv_crc=0x%x, fixnv_dedicate_crc=0x%x, fixnv_figure_crc=0x%x, fixnv_address=0x%x", 
...
uint32	update_fixnv(char * fix_nv_address, uint32 fix_nv_size)
{
    ...
    * ((uint16 *) &fix_nv_address[fix_nv_size]) = INVALID_VALUE;
    
    productinfo_offset = Get_productinfo_offset();    
    *(uint32 *)(fix_nv_address + sector_size - PRD_INFO_OFFSET_ADDR) = productinfo_offset;
    
    // 假设上层函数改写了FIXNV 但没有改写CRC 和状态位，所以只检查FIXNV 长度和结构
    // 注意: calculate_fixNV_length 函数会依赖 FIX_NV_LEN_OFFSET 写入的 fix_nv_size
    calc_fixnv_len = calculate_fixNV_length((uint32)fix_nv_address, (uint32)fix_nv_address + sector_size);
    ...
    {
        SIO_TRACE("LENGTH ERROR! %s %d fix_nv_size=0x%x, fix_nv_address=0x%x, calc_fixnv_len=0x%x",
    ...
    //SIO_TRACE("%s %d %d 0x%x 0x%x", __func__, __LINE__, is_dload_phasecheck, calc_fixnv_len, fix_nv_address);
    ...
    {     
        ...
        FDL_memset((void *)(fix_nv_address + fix_nv_size), 0xFF, sector_size - fix_nv_size - FIX_NV_LEN_OFFSET);
#if 1
        prod_crc_length = sector_size - productinfo_offset - FIX_NV_LEN_OFFSET;     
        if ((prod_crc_length >= sector_size) || (prod_crc_length < FIX_NV_LEN_OFFSET))
        {
        ...
        *(uint16 *)(fix_nv_address + sector_size - PRD_INFO_CRC_OFFSET) = prod_sec_figure_crc;
#else
        *(uint16 *)(fix_nv_address + sector_size - PRD_INFO_CRC_OFFSET) = INVALID_VALUE;
#endif            
        if (valide_fixnv_addr !=INVALID_ADDR)
        {
            valid_productinfo_crc = FDL_FlashReadWord(valide_fixnv_addr + sector_size - PRD_INFO_CRC_OFFSET);
            valid_prod_offset_addr = FDL_FlashReadDWord(valide_fixnv_addr + sector_size - PRD_INFO_OFFSET_ADDR);
        
            if( (valid_productinfo_crc !=INVALID_VALUE) 
            ...
            )
            {
                ...
                if ((uint16)productinfo_offset == valid_prod_offset_addr)
                {
                    memcpy((void *)((uint32)fix_nv_address + productinfo_offset), 
                            ...
                            sector_size - productinfo_offset - FIX_NV_LEN_OFFSET);   
                }
                else
                {         
                   

                    min_productinfo_length = MIN(sector_size - valid_prod_offset_addr, sector_size - productinfo_offset);
    ...
    //SIO_TRACE("%s %d %d", __func__, __LINE__, verify_fixnv((uint32)fix_nv_address));
    ...
    if (FDL_FlashWriteArea(slave_address, fix_nv_address, sector_size) == sector_size)
    {
        ...
        * ((uint16 *) &fix_nv_address[sector_size - SECTOR_STATUS_OFFSET]) = NV_MOK_SOK;
        ...
        if (FDL_FlashWriteArea(master_address, (char *) fix_nv_address, sector_size) == sector_size)
        {
            ...
            FDL_FlashWriteWord(slave_address + sector_size - SECTOR_STATUS_OFFSET, NV_MOK_SINVALID);
            FDL_FlashWriteWord(master_address + sector_size - SECTOR_STATUS_OFFSET, NV_MOK_SINVALID);
            FDL_FlashErase_Block(slave_address, sector_size);
            FDL_FlashWriteWord(slave_address + sector_size - SECTOR_STATUS_OFFSET, SECTOR_GOOD_STATUS);
...
void update_sector_by_slave(void)
{
    ...
    SIO_TRACE("%s %d valid_fixnv_addr=0x%x, slave_address=0x%x, master_address=0x%x",
                  __func__, __LINE__, valid_fixnv_addr, slave_address, master_address);

    if (INVALID_ADDR == valid_fixnv_addr)
    {
        SIO_TRACE("I can not find valid fixnv! %s %d", __func__, __LINE__);
        return;
    }

    valid_fixnv_status = FDL_FlashReadWord(valid_fixnv_addr + sector_size - SECTOR_STATUS_OFFSET);

    if (valid_fixnv_addr == master_address)
    {
        SIO_TRACE("%s %d valid_fixnv_addr=0x%x", __func__, __LINE__, valid_fixnv_addr);
    ...
    {      
        ...
        {
            FDL_FlashRead(valid_fixnv_addr, sector_buf_ptr, sector_size);

            *(uint16 *)(sector_buf_ptr + sector_size - SECTOR_STATUS_OFFSET) = NV_MOK_SOK;

            if (FDL_FlashWriteArea(master_address, (char *) sector_buf_ptr, sector_size) == sector_size)
            {                
                if (!verify_fixnv(master_address))
                {
                    FDL_SendErrLog("ERROR! %s %d", __func__, __LINE__);
                ...
                {
                    FDL_FlashWriteWord(valid_fixnv_addr + sector_size - SECTOR_STATUS_OFFSET, NV_MOK_SINVALID);
                }
                
                FDL_FlashWriteWord(master_address + sector_size - SECTOR_STATUS_OFFSET, NV_MOK_SINVALID);
                FDL_FlashErase_Block(valid_fixnv_addr, sector_size);
            }
        }
    }
...
//    SIO_TRACE("%s %d explore_fixnv_addr=0x%x valide_fixnv_addr=0x%x", 
