...
LOCAL uint32 SPIFLASH_GetPollStatus( int cs )
{
	...
	uint32 status = 0;
       ...
       status1 = SFCDRV_CmdWithRx(cs, CMD_RDSR1, 0, 0, 0, 1);

	if(nQEPos[cs]>=8 && nQEPos[cs]!=UNSUPPORT_CMD)
	{
              status2 = SFCDRV_CmdWithRx(cs, CMD_RDSR2, 0, 0, 0, 1);
	}
...
{	 
       SFCDRV_CmdNoRx(cs, CMD_WREN, 0, 0, 0);
}
...
LOCAL void SPIFLASH_WriteStatusReg(int cs, uint32 value, BOOLEAN bSet)
{
	...
	SFCDRV_CmdNoRx(cs, CMD_WRSR, 0, &reg_value, size);
...
LOCAL void SPIFLASH_ResetStatusReg(int cs)
{
	...
	for(i = 0; i < 3; i++)
	{
		...
		SFCDRV_CmdNoRx(cs, cmd_write_reg[i], 0, &status_reg_val[i], 1);
...
LOCAL uint8 SPIFLASH_ReadReg_VCR(int cs)
{
    ...
    reg_value = SFCDRV_CmdWithRx(cs, CMD_RDVCR, 0, 0, 0, 1);
...
LOCAL void SPIFLASH_WriteReg_VCR(int cs, uint8 reg_val)
{
    ...
    SFCDRV_CmdNoRx(cs, CMD_WRVCR, 0, &reg_val, 1);
...
LOCAL uint8 SPIFLASH_ReadReg_VECR(int cs)
{
    ...
    vecr_val = SFCDRV_CmdWithRx(cs, CMD_RDVECR, 0, 0, 0, 1);
           
    return (uint8)vecr_val;
}
...
LOCAL void SPIFLASH_WriteReg_VECR(int cs, uint8 reg_val)
{
    ...
    SFCDRV_CmdNoRx(cs, CMD_WRVECR, 0, &reg_val, 1);
...
LOCAL void SPIFLASH_EnterQPI(int cs)
{
    ...
    SFCDRV_CmdNoRx(cs, EnterQPI[cs], 0, 0, 0);
}
...
LOCAL void SPIFLASH_ExitQPI(int cs)
{
    ...
    SFCDRV_CmdNoRx(cs, ExitQPI[cs], 0, 0, 0);
}
...
LOCAL void SPIFLASH_Enter4ByteAddrMode(int cs)
{	
       SFCDRV_CmdNoRx(cs, CMD_4BYTEADDREN, 0, 0, 0);
}
...
LOCAL void SPIFLASH_Exit4ByteAddrMode(int cs)
{ 	
       SFCDRV_CmdNoRx(cs, CMD_4BYTEADDRDI, 0, 0, 0);
}
...
LOCAL void SPIFLASH_SetReadPara(int cs, uint8 para)
{
	...
	dummyclk_freq[cs] = para;
       ...
       SFCDRV_CmdNoRx(cs, CMD_SETBURST, 0, &para, 1);
}
...
LOCAL void SPIFLASH_EnterQPIRead(int cs)
{
    ...
    SFCDRV_CmdNoRx(cs, cmd_4read[cs], 0, &s_address_cycle[cs], bytenum);
...
PUBLIC BOOLEAN SPIFLASH_ReadID(int cs, uint8 *pMID, uint8 *pDID1, uint8 *pDID2)
{
	uint32 id = 0;
       ...
       id = SFCDRV_CmdWithRx(cs, CMD_RDID, 0, 0, 0, 3);
    ...
    uint32       counter)           // number of byte to write.
{
...
#if defined(PLATFORM_SC6530) || defined(CHIP_VER_UIS8910FF)
    ...
    dest_addr = addr & s_address_mask[cs];
    SFCDRV_CmdNoRx(cs, cmd, dest_addr, buf, counter);
    ...
    uint16      data)
{
    ...
    dest_addr = addr & s_address_mask[cs];
    SFCDRV_CmdNoRx(cs, cmd, dest_addr, buf, 2);
    ...
    uint32 addr)
{
	...
	do
	{
        ...
        SFCDRV_CmdNoRx(cs, CMD_SE, (addr & s_address_mask[cs]), 0, 0);
    ...
    uint32 addr)
{
	...
	do
	{
        ...
        SFCDRV_CmdNoRx(cs, CMD_32KBE, (addr & s_address_mask[cs]), 0, 0);
    ...
    uint32 addr)
{
	...
	do
	{
        ...
        SFCDRV_CmdNoRx(cs, CMD_64KBE, (addr & s_address_mask[cs]), 0, 0);
...
LOCAL BOOLEAN SPIFLASH_EraseChip(int cs)
{
	...
	do
	{
        ...
        SFCDRV_CmdNoRx(cs, CMD_CE, 0, 0, 0);
