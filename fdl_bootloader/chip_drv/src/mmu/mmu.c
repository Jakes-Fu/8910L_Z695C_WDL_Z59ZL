...
LOCAL const uint32 const_MMUTableStartAddr = 0x80000000;
LOCAL const uint32 const_MMUTableStartAddrRemap0 = 0x80000000;
LOCAL const uint32 const_MMUTableStartAddrRemap1= 0x80000000;

#else

LOCAL const uint32 const_MMUTableStartAddr       = 0x003f0000 ;//remap = 0,sdram from 0x0

LOCAL const uint32 const_MMUTableStartAddrRemap0 = 0x04400000 - 16*1024;    //remap = 0

#ifdef NOR_FDL_SC6800H
LOCAL const uint32 const_MMUTableStartAddrRemap1 = 0x34400000 - 16*1024;//remap = 1,psram from 0x3400,0000
#elif defined(PLATFORM_SC6531EFM)
LOCAL const uint32 const_MMUTableStartAddrRemap1 = 0x14400000 - 16*1024;//remap = 1,psram from 0x1400,0000
#else
LOCAL const uint32 const_MMUTableStartAddrRemap1 = 0x31600000 - 16*1024;//remap = 1,sdram from 0x3000,0000
#endif
#endif

//MMU_TABLE_ADDR must be aligned by 16K-Byte.
#define MMU_TABLE_ADDR          ((const_MMUTableStartAddrRemap0) & 0xFFFFC000 )
...
void MMU_Init (void)
{
    ...
    remap = * (volatile uint32 *) 0x205000E0;
#elif defined(PLATFORM_SC6531EFM)
    remap = * (volatile uint32 *) 0x205000E0;
#elif defined(PLATFORM_UIX8910)
    remap = * (volatile uint32 *) 0x80000000;
#else
    ...
    for (i = 0; i < 0x1000; i++)
    {
        ...
        {
         page_table[i] = (PMD_TYPE_SECT|PMD_SECT_BUFFERED|PMD_FULL_ACCESS|PMD_SECT_TEX(0)|PMD_DOMAIN(0xF)) + (i << 20);
        }
...
void MMU_Init_ext (void)
{
    ...
    if (remap&0x01)
    {
        g_mmu_page_table = (unsigned int *) MMU_TABLE_ADDR_REMAP1;
    }
    else
    {
        g_mmu_page_table = (unsigned int *) MMU_TABLE_ADDR_REMAP0;
    }
    ...
    for (i = 0; i < 0x1000; i++)
    {
#ifdef defined(PLATFORM_UIX8910)
