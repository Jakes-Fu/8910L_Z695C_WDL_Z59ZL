...
#ifdef SECURE_BOOT_SUPPORT
#define VLR_SIZE	512
    uint8 *memAddr = (uint8 *)(g_file.start_address + FILE_HEAD_SIZ + VLR_SIZE);
    uint8 *fdl_vlr_ptr =  (uint8 *)(memAddr - VLR_SIZE);	//Substracted size of vlr from end address to get vlr data address
...
{
    #define FDL_SECURE_BOOT_HEAD_SIZE (0x600)
    rprintf("NOR_FDL1_DataExec 0x%x\n", g_file.start_address + FDL_SECURE_BOOT_HEAD_SIZE);
    //BSL_REP_ACK is sent in the begin of Nor FDL2

    JumpToTarget (g_file.start_address + FDL_SECURE_BOOT_HEAD_SIZE);
