...
/*****************************************************************************/
//  Description:    AppendIntegerField
//  Author:         
//  Note:          
/*****************************************************************************/
...
char *ComposeGsv(unsigned char SatSystem, char *StringStart, int SatNumber, int NmeaVersion)
{
	...
	if (g_NmeaDataCP2AP.systemSvNum[SatSystem] == 0)
		return StringStart;
    SCI_MEMSET((void * )SvIndex, 0, sizeof(SvIndex));
    SCI_MEMSET((void * )SvPrn, 0, sizeof(SvPrn));
    SCI_MEMSET((void * )SvEl, 0, sizeof(SvEl));
    SCI_MEMSET((void * )SvCN0, 0, sizeof(SvCN0));
    SCI_MEMSET((void * )SvAz, 0, sizeof(SvAz));
	...
	for (i = 1; i <= GsvMsgNumber; i ++)
	{
		...
		*p ++ = 'V';
        ...
        //#endif  
        GREENEYE2_LOGD(("[GE2]=====>>>>>%s,SatNumber:%d,NmeaVersion:%d",StringStart,SatNumber,NmeaVersion));
...
int NmeaEncode(unsigned int NmeaMask, unsigned char NmeaVersion)
{
	...
	int TempInt;
	int SignFlag = 0;
	GE2_KINEMATIC_INFO PosVelRec;
	NLLH PosLLHRec;
	unsigned int NmeaType, OutNmeaType;
	int i, consFlag = 0;
    double geoUndu = 0.0;
    BOOLEAN bResValid = FALSE;
	uint8 llh_time_flg = 0;

	
	if(((g_startmode_flg == 1)&&(g_NmeaDataCP2AP.Quality>=1))||(g_llh_time_flg==1))
	{
	    llh_time_flg = 1;
		g_llh_time_flg = 1;
	}
	else if(g_startmode_flg != 1)
	{
	    llh_time_flg = 1;
	}
	else
   ...
   memcpy(&PosVelRec, &(g_NmeaDataCP2AP.PosVelRec), sizeof(GE2_KINEMATIC_INFO));
...
//				*p ++ = '3';
		...
		case NMEA_GSV:
          	if (g_NmeaDataCP2AP.PvtSystemUsage & (1 << GPS))
				StringStart = ComposeGsv(GPS, StringStart, g_NmeaDataCP2AP.systemSvNum[GPS], NmeaVersion);
                        GREENEYE2_LOGD(("[GE2]g_NmeaDataCP2AP.PvtSystemUsage=%d,workmode=%d",g_NmeaDataCP2AP.PvtSystemUsage,s->workmode));
        ...
        if(NmeaType != NMEA_GSV)
        {
            GREENEYE2_LOGD(("[GE2]=====>>>>>%s",nmea_string));
...
/*****************************************************************************/
//  Description:    nmea_parse_cp
//  Author:         
//  Note:          
/*****************************************************************************/
void nmea_parse_cp(unsigned char *buff,unsigned short lenth)
{
    GREENEYE2_LOGD(("[GE2]nmea parse cp begin"));
    if(lenth == sizeof(NMEA_CP2AP))
    {
        GREENEYE2_LOGD(("[GE2]received nmea message,encode it"));
    ...
    {
        GREENEYE2_LOGD(("[GE2]nmea type is include mscount,but firmwate is not send it"));
        memcpy(&g_NmeaDataCP2AP,buff,lenth);
		gpsTow = 0;
        NmeaEncode(g_NmeaDataCP2AP.NmeaMask, g_NmeaDataCP2AP.NmeaVersion);
    }
	...
	{
		GREENEYE2_LOGE(("[GE2]reived nmea lenth is error"));
	}
    return;
