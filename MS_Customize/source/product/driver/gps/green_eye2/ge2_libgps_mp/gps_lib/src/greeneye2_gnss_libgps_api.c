/******************************************************************************
 ** File Name:   greeneye2_gnss_libgps_api.c                                                *
 ** Author:                                                     *
 ** DATE:                                                         *
								 ...
								 unsigned short Len, unsigned short* pProcessedLen)
{
	...
	for(i = 0; i < Len; i++)// the magic number is 4 '~'
	{
		if(0x7E == *pStartData)
		{
			...
			{
                pStartData++;
				SCI_MEMSET(pStream->curHeader,GNSS_MAGIC_NUMBER,GNSS_MAGIC_NUMBER_LEN);
									 ...
									 unsigned short Len, unsigned short* pProcessedLen)
{
    ...
    SCI_MEMCPY(pStream->curHeader + headsize, pData, processedLen);
    ...
    *pProcessedLen = processedLen;
    ...
    {
    	TM_SendTestPointRequest(headsize,9301525);
	GE2GPS_LOGD("headsize=%d",headsize);
        return ;
    }
    ...
    if(crc == crcInFrame)  // We have got a right header
    {
		...
		//GREENEYE2_LOGE(("[GE2]GNSS_CollectHeader get the type =%d, subtype =%d\r\n",pStream->curHeader[4],pStream->curHeader[5]));
		if(0 == dataLen)
		{
		...
		else
		{
            ...
            else
            {
                ...
                GREENEYE2_LOGE(("[GE2]GNSS_CollectHeader error = %d dataLen = %d\r\n",pStream->errorNum,dataLen));
            }
		}
	...
	{
		pStream->errorNum++;
		GREENEYE2_LOGE(("[GE2]crcInFrame = 0x%x crc =0x%x\r\n",crcInFrame,crc));
		researchFlag(pStream);
		GREENEYE2_LOGE(("[GE2]GNSS_CollectHeader error = %d\r\n",pStream->errorNum));
	}
							...
							unsigned short Len, unsigned short* pProcessedLen)
{
    ...
    SCI_MEMCPY(pPacket->buff + pStream->receivedDataLen, pData, nCopy);
    pStream->receivedDataLen += nCopy;

    *pProcessedLen = nCopy;
    // Have we got the whole frame?
    TM_SendTestPointRequest(pStream->receivedDataLen,924380); 
    if(pStream->receivedDataLen == (pStream->dataLen + GNSS_MAX_DATA_CRC))
    {
        ...
        else
        {
            ...
            {
                GREENEYE2_LOGD(("[GE2]The Data CheckSum error  type = %d, subtype = %d len =%d",
                  pPacket->type,pPacket->subType, pPacket->length));
                
                GREENEYE2_LOGD(("[GE2]The Data CheckSum error CRC = %d CrcInFrame =%d",
                  dataCrc,CrcInFrame));
            }
        }
		...
		pStream->receivedDataLen = 0;
		pStream->dataLen = 0;
		//GREENEYE2_LOGD(("[GE2]recv one packet complete\n"));
    }
...
int GNSS_ParseOnePacket(TGNSS_DataProcessor_t* pStream,  unsigned char* pData, unsigned short Len)
{
	...
	while(remainLen)
	{
			TM_SendTestPointRequest(pStream->state,924477); 
	        switch(pStream->state)
            {
        	    case GNSS_RECV_SEARCH_FLAG:	    
				GNSS_searchFlag(pStream, pInput, remainLen, &processedLen);
				if(pStream->state != GNSS_RECV_COLLECT_HEAD)
			        {
			        	TM_SendTestPointRequest(pStream->state,10101813); 
			        }
				break;
        	    case GNSS_RECV_COLLECT_HEAD:	    
				GNSS_CollectHeader(pStream,pInput,remainLen, &processedLen);
		                if(pStream->state != GNSS_RECV_DATA)
			        {
			        	TM_SendTestPointRequest(pStream->state,10101814); 
			        }
				break;
        	    case GNSS_RECV_DATA:
				GNSS_CollectData(pStream, pInput, remainLen, &processedLen);
			        ...
			        {
			        	TM_SendTestPointRequest(pStream->state,10101815); 
			        }
				break;
                default: 
				break;
		...
		pInput    += processedLen;
        nRet += processedLen;
        //GE2GPS_LOGD("[%d]remainLen=%d,processedLen=%d,nRet=%d",count,remainLen,processedLen,nRet);
		//count++;
	}

	if(assertpos > 0 && (!s->happenAssert))
	{
		s->happenAssert = 1;
		GREENEYE2_LOGD(("[GE2]GNSS_ParseOnePacket  state = %d, headSize = %d receivedDataLen =%d,dataLen=%d",
                  pStream->state,pStream->headSize, pStream->receivedDataLen,pStream->dataLen));
		SCI_PASSERT(0, ("[GE2]GNSS_ParseOnePacket receive assert subtype!"));
	}
        return nRet;
}
...
int GNSS_EncodeOnePacket( TCmdData_t* pInData,unsigned char* pOutData,unsigned short outLen)
{
	...
	if(len)
	{
		SCI_MEMCPY((pOutData+GNSS_MAX_HEAD_LEN),pInData->buff,len);
