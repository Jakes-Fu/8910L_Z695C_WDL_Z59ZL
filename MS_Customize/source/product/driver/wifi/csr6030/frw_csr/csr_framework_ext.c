...
#elif defined(PLATFORM_SC6531EFM) || defined(PLATFORM_UIX8910)
#include "priority_system.h"
#ifdef CSR_WIFI_6030
#include "csr_sched_init.h"
#else
...
{
    SCI_GetSemaphore(globalMutex, SCI_WAIT_FOREVER);
}
#else
 ...
 *----------------------------------------------------------------------------*/
#ifdef CSR_WIFI_6030
CsrResult CsrThreadCreate(void (*threadFunction)(void *pointer), void *pointer, CsrUint32 stackSize, CsrUint16 priority, const CsrCharString *threadName, CsrThreadHandle *threadHandle)	
#else
...
#if !defined(PLATFORM_SC6530) && !defined(PLARFORM_SC6531EFM)
 ...
 *----------------------------------------------------------------------------*/ 
#ifdef CSR_WIFI_6030
CsrResult CsrThreadCreateExt(void (*threadFunction)(void *pointer), void *pointer, CsrUint32 stackSize, CsrUint16 priority, const CsrCharString *threadName, CsrThreadHandle *threadHandle)	
#else
...
#ifdef CSR_WIFI_6030
            // Set to CSR_THREAD_PRIORITY_HIGH
...
{
    SCI_Sleep((CsrUint32)sleepTimeInMs);
}
#else
...
#ifdef CSR_WIFI_6030
void * CsrMemAllocExt(CsrSize size, const char* file, const int line)
#else
...
#ifdef CSR_WIFI_6030
void * CsrPmemAllocExt(CsrSize size, const char* file, const int line)
{
	CsrMemAllocExt(size, file, line);
}
#endif
/*----------------------------------------------------------------------------*
 *  NAME
...
#endif
{
    void *pointer;
#ifdef CSR_WIFI_6030
    CsrSize size = numberOfElements * elementSize;
#else
