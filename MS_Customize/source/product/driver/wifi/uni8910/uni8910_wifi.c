...
#include "../../../../../../chip_drv/chip_plf/uix8910/uix8910_int_cfg.h"
...
static void uni8910_prvWifiClearReady()
{
	...
	TM_SendTestPointRequest (0x00550004, hwp_wcnWlan->config_reg);
	// enable
	conf_reg.b.apb_clear = 0;
	hwp_wcnWlan->config_reg = conf_reg.v;
	while(((hwp_wcnWlan->config_reg & 0x1) != 0x0) && (j<=20)){j++;};//avoid endless loop
	TM_SendTestPointRequest (0x00550005, hwp_wcnWlan->config_reg);
}
...
static void uni8910_prvLoadApInfo(wifiApInfo_t *info)
{
	...
	/*
	*config_reg bit1 is apb_hold, must always be 1, suggestion from asic*
	*interrupt cleared only by software when apb_hold is 1, otherwise interrupt processed automatically*
	*/

	//conf_reg.b.apb_hold = 0;
	//hwp_wcnWlan->config_reg = conf_reg.v;
	//while(((hwp_wcnWlan->config_reg & 0x2) != 0x0) && (j<=20)){j++;};//avoid endless loop
	TM_SendTestPointRequest (0x00550003, hwp_wcnWlan->config_reg);
}
/*ApInfo have handled */
...
static void uni8910_handle_irq(wifiScanRequest_t *req)
{
	...
	TM_SendTestPointRequest (0x00550001, hwp_wcnWlan->datardyint + (uni8910_irq_stat<<8));
	uni8910_prvLoadApInfo(&info);
	uni8910_prvSaveApInfo(&info, req);
	TM_SendTestPointRequest (0x00550001, 0x55);
	uni8910_prvWifiClearReady();
	TM_SendTestPointRequest (0x00550001, 0x55 + (uni8910_irq_stat<<8) + uni8910_g_chn);
}
