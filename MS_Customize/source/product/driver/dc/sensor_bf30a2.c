...
LOCAL void bf30a2_WriteReg( uint8  subaddr, uint8 data )
{
	...
	SCI_TRACE_LOW("SENSOR: bf30a2_WriteReg reg/value(%x,%x) !!", subaddr, data);
}
...
LOCAL uint8 bf30a2_ReadReg( uint8  subaddr)
{
	...
	SCI_TRACE_LOW("bf30a2_ReadReg: subaddr:%x,value:%x",subaddr,value);
...
LOCAL uint32 bf30a2_set_ae_awb_enable(uint32 ae_enable, uint32 awb_enable)
{
    ...
    SCI_TRACE_LOW("bf30a2_set_ae_awb_enable: set_ae_awb_enable: ae=%d awb=%d", ae_enable, awb_enable);
...
LOCAL uint32 set_bf30a2_ev(uint32 level)
{
    ...
    SCI_TRACE_LOW("set_bf30a2_ev: set_ev: level = %d", level);
    return 0;
}
...
{//24m->65.6 us 12m->131us
    ...
    SCI_TRACE_LOW("set_bf30a2_anti_flicker: set_banding_mode: mode = %d", mode);
	return 0;
}
...
LOCAL uint32 bf30a2_set_brightness(uint32 level)
{
	...
	SCI_TRACE_LOW("bf30a2 bf30a2_set_brightness: level = %d", level);
...
LOCAL uint32 bf30a2_set_contrast(uint32 level)
{
    ...
    OS_TickDelay(20);
    SCI_TRACE_LOW("bf30a2: bf30a2_set_contrast: level = %d", level);
    return 0;
}
...
LOCAL uint32 bf30a2_set_preview_mode(uint32 preview_mode)
{
	SCI_TRACE_LOW("bf30a2 bf30a2_set_preview_mode: preview_mode = %d", preview_mode);
...
LOCAL uint32 bf30a2_set_image_effect(uint32 effect_type)
{
	...
	SCI_TRACE_LOW("bf30a2 bf30a2_set_image_effect: effect_type = %d", effect_type);
...
LOCAL uint32 bf30a2_set_work_mode(uint32 mode)
{
    ...
    SCI_TRACE_LOW("bf30a2 set_work_mode: mode = %d", mode);
...
LOCAL uint32 _bf30a2_Power_On(uint32 power_on)
{
    ...
    SCI_TRACE_LOW("SENSOR: _bf30a2_Power_On(1:on, 0:off): %d", power_on);    
