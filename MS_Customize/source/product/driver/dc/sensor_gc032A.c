...
#define GC032A_VER_ADDR     0x2a
	...
	while(nLoop--)
	{
		...
		{
			OS_TickDelay(2);
			SCI_TRACE_LOW("david GC032A_Identify 0 ERROR 0x%x!!!", ret);
			ret = GC032A_ReadReg(reg[1]);
			if(ret != value[1])
			{
				SCI_TRACE_LOW("david GC032A_Identify 1 ERROR 0x%x!!!", ret);
			}
			else
			{
				err_cnt = 0;
				SCI_TRACE_LOW("david GC032A_Identify OKOK 0x%x!!!", ret);
				break;
			}
		}
		else
	...
	if (err_cnt != 0)
	{
		SCI_TRACE_LOW("david GC032A_Identify failed");
		return -1;
	}
	SCI_TRACE_LOW("david GC032A_Identify success");
...
LOCAL uint32 Set_GC032A_Brightness(uint32 level)
{
    ...
    SCI_TRACE_LOW("david sensor set_GC032A_brightness: level = %d", level);
...
LOCAL uint32 Set_GC032A_Contrast(uint32 level)
{
    ...
    SCI_TRACE_LOW("david sensor set_GC032A_contrast: level = %d", level);
...
LOCAL uint32 Set_GC032A_Image_Effect(uint32 effect_type)
{
    ...
    SCI_TRACE_LOW("david sensor set_GC032A_image_effect: effect_type = %d", effect_type);
...
__align(4) const SENSOR_REG_T GC032A_ev_tab[][4]=
{
    ...
    {{0xfe, 0x01}, {0x13, 0x3a}, {0xfe, 0x00}, {0xff, 0xff}},
    {{0xfe, 0x01}, {0x13, 0x4a}, {0xfe, 0x00}, {0xff, 0xff}},// level zero
    {{0xfe, 0x01}, {0x13, 0x5a}, {0xfe, 0x00}, {0xff, 0xff}},
    {{0xfe, 0x01}, {0x13, 0x6a}, {0xfe, 0x00}, {0xff, 0xff}},
...
LOCAL uint32 Set_GC032A_Ev(uint32 level)
{
	...
	SCI_TRACE_LOW("david sensor set_GC032A_ev: level = %d", level);
    SCI_ASSERT(PNULL != sensor_reg_ptr);
    SCI_ASSERT(level < 7);
...
LOCAL uint32 Set_GC032A_Anti_Flicker(uint32 mode)
{
	...
	SCI_TRACE_LOW("david sensor set_GC032A_anti_flicker-mode=%d", mode);
	switch(mode)
	{
...
LOCAL uint32 Set_GC032A_Preview_Mode(uint32 preview_mode)
{
	switch (preview_mode)
	{
		...
		{
			SCI_TRACE_LOW("david sensor set_GC032A_preview_mode: level = %d", preview_mode);
			GC032A_WriteReg(0xfe,0x01);
			GC032A_WriteReg(0x3c,0x20);
			GC032A_WriteReg(0xfe,0x00);
			break;
		}
...
LOCAL uint32 Set_GC032A_AWB(uint32 mode)
{
	...
	SCI_TRACE_LOW("david sensor set_GC032A_awb_mode: mode = %d", mode);
...
LOCAL uint32 GC032A_Before_Snapshot(uint32 para)
{
	...
	SCI_TRACE_LOW("david sensor SENSOR_GC032A: Before Snapshot");
...
LOCAL uint32 GC032A_After_Snapshot(uint32 para)
{
	...
	SCI_TRACE_LOW("david sensor SENSOR_GC032A: After Snapshot");
...
LOCAL uint32 Set_GC032A_Video_Mode(uint32 mode)
{
	...
	SCI_TRACE_LOW("david sensor set_GC032A_video_mode=%d",mode);

    sensor_reg_ptr = (SENSOR_REG_T*)GC032a_video_mode_nor_tab[mode];

    if(mode > DCAMERA_MODE_MAX ||PNULL == sensor_reg_ptr)
    {
    	SCI_TRACE_LOW("david sensor set_GC032A_video_mode error");
        return SCI_ERROR;
    }
