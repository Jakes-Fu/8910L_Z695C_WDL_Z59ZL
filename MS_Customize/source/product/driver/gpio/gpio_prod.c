...
#endif
//#define K_AMPLIFIER_SUPPORT

#ifdef K_AMPLIFIER_SUPPORT
//#define DELAY_US(DURATION)		{unsigned int cn; for(cn = 1; cn <= (DURATION*50); cn++){}}
#define GPIO_SPK_REAL_NUM 12

PUBLIC void _GPIO_OpenAudioPA2(BOOLEAN b_enable )
{
    HAL_SetGPIOVal( GPIO_SPK_REAL_NUM, SCI_TRUE );
    TIMER_DELAY_US(3);
    HAL_SetGPIOVal( GPIO_SPK_REAL_NUM, SCI_FALSE );
    TIMER_DELAY_US(3);
    HAL_SetGPIOVal( GPIO_SPK_REAL_NUM, SCI_TRUE );
    TIMER_DELAY_US(100);
}
...
PUBLIC void _GPIO_OpenAudioPA3(BOOLEAN b_enable )
{
...
PUBLIC void _GPIO_OpenAudioPA4(BOOLEAN b_enable )
{
        HAL_SetGPIOVal( GPIO_SPK_REAL_NUM, SCI_TRUE );
        TIMER_DELAY_US(2);
        HAL_SetGPIOVal( GPIO_SPK_REAL_NUM, SCI_FALSE );
...
PUBLIC void _GPIO_OpenAudioPA(BOOLEAN b_enable )
{
    static BOOLEAN s_is_pa_enable = FALSE;
    //Trace_Log_Print("_GPIO_OpenAudioPA enable=%d",b_enable);
    if(s_is_pa_enable == b_enable)
    {
        return;
    }
    else
    {
        s_is_pa_enable = b_enable;
    }
...
PUBLIC void GPIO_KeyLockIntHandler (uint32 gpio_id, uint32 gpio_state)
{
    TB_MSG  gpio_msg;
    GPIO_CFG_INFO_T_PTR cfg_ptr = GPIO_PROD_GetCfgInfo (GPIO_PROD_KEYLOCK_INT_ID);
    if (PNULL != cfg_ptr)
    {
        if (gpio_state)
        {
            ...
            gpio_msg.wparam  = SCI_VK_SLIDE;
            gpio_msg.lparam  = TB_NULL;
            _GPIO_PROD_SetInterruptSense (GPIO_PROD_KEYLOCK_INT_ID, GPIO_INT_LEVEL_LOW);
        }
        else
        {
            if (!cfg_ptr->valid_level)
            ...
            gpio_msg.wparam  = SCI_VK_SLIDE;
            gpio_msg.lparam  = TB_NULL;
            _GPIO_PROD_SetInterruptSense (GPIO_PROD_KEYLOCK_INT_ID, GPIO_INT_LEVEL_HIGH);
        }
        DRV_Callback (TB_GPIO_INT,&gpio_msg);
    }
}
...
{
    SCI_TRACE_LOW("GPIO_SetTorch: b_on=%d", b_on);
  //  _GPIO_PROD_SetVal (GPIO_PROD_FLASH_EN_ID,  b_on);
    _GPIO_PROD_SetVal (GPIO_PROD_TORCHLED_ID,  b_on);	
    s_torch_is_on = b_on;
}

PUBLIC BOOLEAN GPIO_GetTorchStatus( void )
{
    SCI_TRACE_LOW("GPIO_GetTorchStatus: s_torch_is_on=%d", s_torch_is_on);
