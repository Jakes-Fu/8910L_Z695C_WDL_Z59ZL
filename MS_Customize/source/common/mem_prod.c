...
PUBLIC void* MEM_GetMainLCDBuffer(void)
{
    ...
    {
    	s_main_lcd_buffer_ptr = SCI_ALLOC_APPZ(MAX_MAIN_LCD_BUF_SIZE + 256);
    }
    
	return (void*)LCD_ALIGN_256(s_main_lcd_buffer_ptr);
}
...
{
#ifdef PRODUCT_DRIVER_SC6800
	lenght_ptr = (uint32)vb_decode_output_buffer_size;
	return (void*)VBDecodeOuputBuf;
#else
	lenght_ptr = NULL;
	return (void*)PNULL;
#endif
...
{
    return (SRAM_TOTAL_SIZE);
}


#ifdef DSP_USB_LOG
#define  MEM_DSPTRACE_SIZE       0x00019000
#define  MEM_DSPTRACE_START_ADDR		(MEM_DSPCODE_START_PHYADDR + MEM_RESERVED_SIZE - 0xC0000)  //use the last 1M of DSP Code area as DSP USB LOG
/*****************************************************************************/
			...
			)
{
	...
	DSPLogBuff_region->start_add = (uint32*)MEM_DSPTRACE_START_ADDR;
	DSPLogBuff_region->size      = MEM_DSPTRACE_SIZE;
	DSPLogBuff_region->prev      = NULL;
	DSPLogBuff_region->next      = NULL;
	DSPLogBuff_region->attribute = MEM_RW_REGION;
