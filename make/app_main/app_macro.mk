    ...
    ifeq ($(strip $(VIDEO_PLAYER_SUPPORT)), TRUE)
        ...
        ifneq ($(strip $(VIDEOPLAYER_MINI_FUNCTION)), TRUE) 
				...
				ifneq ($(strip $(MAINLCD_DEV_SIZE)), 128X160) #MINI界面临时关闭该功能
			        ...
			        #MCFLAG_OPT		+=  -DMMI_VIDEOWALLPAPER_SUPPORT   #不支持视频墙纸
...
ifneq ($(strip $(CMCC_UI_STYLE)), TRUE)
ifneq ($(strip $(TOUCHPANEL_TYPE)), NONE)
#michael wang
...
#	MCFLAG_OPT += -DBOCA_MMI_SIM_LOCK_SUPPORT
...
#小区广播
#MCFLAG_OPT += -DMMI_SMSCB_CELL_INFO_SUPPORT
#小区广播地区位置信息
   ...
   ifeq ($(strip $(MMS_SMS_IN_1_SUPPORT)), TRUE)
	...
	# delete by zhigang.peng 2018-10-16, for low memory
...
ifeq ($(strip $(WEATHER_SUPPORT)), TRUE)
    ...
    MCFLAG_OPT += -DMMIWIDGET_WEATHER_ANIM   # 天气预报get focu时的动画
endif	  
ifneq ($(strip $(FM_SUPPORT)), NONE)
    MCFLAG_OPT		+= -DMMI_WIDGET_FM							 #Grid FM    
endif    
...
ifeq ($(strip $(PIC_VIEWER_SUPPORT)), TRUE)    
    ...
    MCFLAG_OPT		+= -DPB_SUPPORT_LOW_MEMORY             #PB support 1000.
...
#-------------------QCIF QQVGA MINI --------------------------# 
    ...
    #电池电量百分比在status bar上显示的功能宏 ，该宏与双电池功能互斥，双电池情况下，不能在状态栏上显示，还是按老方法显示
		...
		#都包在32x64，32x16,16x16之外的size里面，所以，这里也只在64x64，128x64和NONE中打开
...
ifeq ($(strip $(JAVA_SUPPORT_TYPE)), SVM)
    ...
    #(CMCC 4.1 需要disable这个宏，也就是jbed和browser同时可以run)
    #7701目前内紧张，不支持browser和java同时运行
    ifeq ($(strip $(PLATFORM)),SC7702)
...
ifeq ($(strip $(MULTI_VARIANT_SUPPORT)), TRUE)
MCFLAG_OPT		+=  -DGBK_SUPPORT
MCFLAG_OPT		+=  -DBIG5_SUPPORT
endif
...
ifeq ($(strip $(GPRS_SUPPORT)), TRUE)
	MCFLAG_OPT		+=  -DMMI_GPRS_SUPPORT
        MCFLAG_OPT		+=  -DMMI_OTA_SUPPORT ##modify by weipeng.wang for MMS recieve
...
ifeq ($(strip $(SIM_LOCK_TYPE2_SUPPORT)), TRUE)
# SIM_LOCK_TYPE2_SUPPORT and SIM_LOCK_SUPPORT must be enabled mutually exclusive.
ifneq ($(strip $(SIM_LOCK_SUPPORT)), TRUE)
	MCFLAG_OPT += -DMMI_SIM_LOCK_TYPE2_SUPPORT
	## MMI_SIM_LOCK_TYPE2_SUPPORT
	MCFLAG_OPT += -DMMI_SIM_LOCK_TYPE2_UNLOCK_SUPPORT #解锁功能
	## MMI_SIM_LOCK_TYPE2_SUPPORT
endif
endif
...
MCFLAG_OPT  	+= -DTTS_FILE_TCARD -DYT_TTS_VER_S2
endif

ifeq ($(strip $(TTS_SUPPORT)), YOUNGTONE_ROM)
MCFLAG_OPT  	+= -DYT_TTS_VER_S2
endif
...
#使用pop动画背景渐变
#MCFLAG_OPT		+= -DMMI_MENUMASKLAYER_SUPPORT
endif
...
ifeq ($(strip $(CMCC_UI_STYLE)), TRUE)
    ifeq ($(strip $(APP_DUALMODE_ENABLE)), TRUE)
        MCFLAG_OPT		+=  -DMMI_SIMPLE_NET_INDICATOR
    endif
    ifeq ($(strip $(BROWSER_SUPPORT_DORADO)), TRUE)
    ...
    ifneq ($(strip $(MMI_MULTI_SIM_SYS)), SINGLE)
        MCFLAG_OPT +=  -DMMI_GPRS_DATA_SIM_SET
    endif 

    ifneq ($(strip $(MMI_MULTI_SIM_SYS)), SINGLE)
       MCFLAG_OP   +=  -DMMIPB_UID_SUPPORT
    endif 
ifeq ($(strip $(CMCC_TEST_FLAG)), TRUE)
     MCFLAG_OPT    +=  -DMMIPB_SEARCH_CONTACT_CONTENT
endif

# FIX Send card in SMS
    MCFLAG_OPT += -DMMI_SMS_FIX_SEND_SIM
#编辑彩信，图片过大时，进行自动压缩    
    MCFLAG_OPT += -DMMIMMS_AUTO_COMPRESS_IMAGE
#彩信中打开网页是否询问
    MCFLAG_OPT += -DMMIMMS_BROWSER_ENTRY_QUERY
#彩信中搜索功能
    MCFLAG_OPT += -DMMISEARCH_SUPPORT_ONLYMMS
ifeq ($(strip $(CMCC_TEST_FLAG)), TRUE)
    MCFLAG_OPT += -DMMI_MSG_RESEND
endif
    MCFLAG_OPT += -DMMI_MMS_LIST_DISPLAY_MULTI_PEOPLE
endif
...
ifneq ($(strip $(PLATFORM)), SC7702)#内存足够支持同时使用浏览器和视频播放器
...
MCFLAG_OPT += -DMMI_DIFF_WALLPAPER_STYLE
MCFLAG_OPT +=  -DMMI_SMS_ALPHABET_SETTING_SUPPORT
#SIM卡语言支持
ifeq ($(strip $(SIM_LANGUAGE_SUPPORT)), TRUE)
    MCFLAG_OPT += -DMMI_SIM_LANGUAGE_SUPPORT
...
ifeq ($(strip $(MOCONF_SIMLOCK)), TRUE)
#    MCFLAG_OPT += -DBOCA_MMI_SIM_LOCK_SUPPORT
## BOCA_MMI_SIM_LOCK_SUPPORT
endif

ifeq ($(strip $(MMIWNS_SUPPORT)), TRUE)
    MCFLAG_OPT += -DMMIWNS_SUPPORT
endif

ifeq ($(strip $(SMART_DUAL_SIM_SUPPORT)), TRUE)
ifeq ($(strip $(MMI_MULTI_SIM_SYS)), DUAL)
    MCFLAG_OPT +=  -DMMI_SMART_DUAL_SIM_SUPPORT
    MCFLAG_OPT +=  -DMMI_GPRS_DATA_SIM_SET
    # define MMI_SMART_DUAL_SIM_SUPPORT
    # MMI_DUALSIM_CALL_DIVERT_SUPPORT must define
    # MMIPB_SIMDN_SUPPORT must define 
endif
endif
    MCFLAG_OPT += -DUNISOC_TEST_ENTRY_FOR_TESTER

#Roaming Indication功能
    MCFLAG_OPT += -DBOCA_ROAMING_INDICATION  

#强制使用复杂文本  
	MCFLAG_OPT += -DCOMPLEX_TEXT_SCRIPT

# MCFLAG_OPT += -DOMA_CP_SEC_SUPPORT
  
#mms add attachment
  MCFLAG_OPT += -DMMI_MESSAGE_MULTIFILE_FROM_GALLERY
  MCFLAG_OPT    += -DMMI_DUALSYS_PREFER_SIM
  MCFLAG_OPT += -DMMI_VIDEOPLAYER_SET_RENAME_NOT_SUPPORT
  MCFLAG_OPT += -DMMISET_GALLERY_WALLPAPER_NO_SUPPORT
  MCFLAG_OPT += -DMOBILE_DATA_CONN_USE_PERFER_SIM_ONLY