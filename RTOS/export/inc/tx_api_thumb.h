...
#define TX_OS_ASSERT(_EXP)     SCI_ASSERT(_EXP) 
#else
#define TX_OS_ASSERT(_EXP)
#endif
...
#define TX_NO_WAIT          0
#define TX_WAIT_MATCH        0xAAAAAAAAUL
...
#define TX_MAX_PRIORITIES   256  //32
...
#define TX_STACK_FILL       ((ULONG) 0x5a5a5a5a)
...
#ifdef	TX_TIMER_INT_PER_TICK
    /* Keep track of the pointer to the head of this list as well.  */
    struct TX_INTERNAL_TIMER_STRUCT
                **tx_list_head;
#else
    void        *tx_need_active;
//#ifdef SCI_DEEP_SLEEP    
    ULONG      tx_sleep_flag;
//#endif 
#endif //TX_TIMER_INT_PER_TICK
...
typedef  struct TX_THREAD_STRUCT
{
    ...
    ULONG       tx_priority_bit[MAX_TX_THREAD_CLASS_NUM];        /* Priority ID bit          */
...
typedef struct TX_BLOCK_POOL_STRUCT
{
    ...
    /* Save the real used memory size  */
    ULONG     tx_block_pool_used_size;
    
    /* Save the malloced memory size */
    ULONG     tx_block_pool_malloc_size;
                    
    /* Save the min available num  */                    
    ULONG     tx_block_pool_min_available;                
...
typedef struct TX_BYTE_POOL_STRUCT
{
   ...
   /*Define Huge Memory begin address*/	
   ULONG    tx_byte_pool_huge_memory_start;

   /* Define to save the min number of available bytes in the pool.*/
    ULONG     tx_byte_pool_min_available;

  /* Define a available memory list to save the available memory start pointer.  */
    CHAR_PTR   tx_byte_pool_available_list;

  /*Define a mutex pointer, Application Memory will use this to manager 
   memory allocate*/
    TX_MUTEX_PTR     tx_mutex_ptr;
  
 /* Define the huge memory watermark, when memory size more than this value, 
      allocate memory from bottom to top, and others alloc from top to bottom     */
    ULONG        tx_huge_memory_watermark;
} TX_BYTE_POOL;
...
#define TX_THREAD_STACK_CHECK(thread_ptr)                                                                           \
    if (((ULONG *) thread_ptr -> tx_stack_ptr) < ((ULONG *) thread_ptr -> tx_thread_stack_highest_ptr))      \
    {                                                                                                               \
        thread_ptr -> tx_thread_stack_highest_ptr =  thread_ptr -> tx_stack_ptr;                             \
    }                                                                                                               \
    if ((*((ULONG *) thread_ptr -> tx_stack_start) != TX_STACK_FILL) ||                                      \
        (((ULONG *) thread_ptr -> tx_thread_stack_highest_ptr) < ((ULONG *) thread_ptr -> tx_stack_start)))  \
    {                                                                                                               \
...
#define tx_timer_adjust             _tx_timer_adjust
#define tx_timer_lastsettick        _tx_timer_lastSetTick
...
void        tx_timer_adjust(uint32 timeTick);
UINT        tx_timer_lastSetTick(void);
	
void        _tx_timer_interrupt(void);

VOID        _tx_thread_stack_error_handler(TX_THREAD *thread_ptr);

UINT        _tx_thread_stack_error_notify(VOID (*stack_error_handler)(void *));

#ifdef	TX_WITH_ROM
	VOID  		tx_init_rom_lib(VOID);
#endif	// TX_WITH_ROM	
