...
#define SCI_SYSTICK_MAX 	(((uint64)0xFFFFFFFF * 1000) / 32768)
#else
#define SCI_SYSTICK_MAX		0xFFFFFFFF
#endif

#define TASK_PRIORITY_256
#ifdef TASK_PRIORITY_256

#define  TX_CORE_HIGH         0
#define  TX_CORE_NORMALE      7
#define  TX_CORE_LOW          15

#define  TX_BASE_HIGH         16
#define  TX_BASE_NORMALE      31
#define  TX_BASE_LOW          47 



#define  TX_SYSTEM_HIGH       48
#define  TX_SYSTEM_NORMALE    111
#define  TX_SYSTEM_LOW        175

#define  TX_USER_HIGH         176
#define  TX_USER_NORMALE      210
#define  TX_USER_LOW          250

#define SCI_MAX_PRIORITIES    255

#else

#define  TX_CORE_HIGH         0
#define  TX_CORE_NORMALE      16
#define  TX_CORE_LOW          31

#define  TX_BASE_HIGH         0
#define  TX_BASE_NORMALE      16
...
#define  TX_USER_NORMALE      16
#define  TX_USER_LOW          31

#define SCI_MAX_PRIORITIES    31
...
#define SCI_TRACE_NULL           SCI_TraceNull
    ...
    #define SCI_TRACE_DSPDATA    SCI_TraceL1DspData
#ifndef SCI_TRACE_ID
    #define SCI_TRACE_ID         SCI_TraceEx
        ...
        #define SCI_TRACE_DATA       SCI_TraceCapData
        #define SCI_TRACE_DSPDATA    SCI_TraceL1DspData
		#define SCI_DEBUG(fmt, arg...)	SCI_TRACE_LOW(("%s:%d--"fmt), __FUNCTION__, __LINE__, ##arg)
    #else
    	#define SCI_TraceBuf(...)                //
        #define SCI_TraceLow(...)               //
        #define SCI_TraceMid(...)                //
		...
		#define SCI_DEBUG(fmt, arg...)
       	#define SCI_TRACE_HIGH(...) 
       	#define SCI_TRACE_ERROR(...)  
        #define SCI_TRACE_DATA(...)
   	    #define SCI_TRACE_DSPDATA(...)
		#define SCI_PRINT_STACK_TRACE(...)
    #endif
#endif  
 ...
 **---------------------------------------------------------------------------*/
    ...
    #define SCI_ALLOCAZ(_SIZE)            malloc(_SIZE) 
	#define SCI_CALLOC(_NUM, _SIZE)       calloc(_NUM, _SIZE)
	#define SCI_REALLOC(_MEMBLOCK, _SIZE) realloc(_MEMBLOCK, _SIZE)
	#define SCI_MEMSIZE(_MEMBLOCK)        _msize(_MEMBLOCK) 
    ...
    #define SCI_DASSERT(_EXP)             assert(_EXP)
        ...
        #define SCI_ALLOCAZ(_SIZE)           SCI_MallocApp_Zero(_SIZE, __FILE__, __LINE__)
		#define SCI_CALLOC(_NUM, _SIZE)      SCI_CAlloc(_NUM, _SIZE, __FILE__, __LINE__)
	 	#define SCI_REALLOC(_MEMBLOCK,_SIZE) SCI_ReAlloc(_MEMBLOCK, _SIZE, __FILE__, __LINE__)
		#define SCI_MEMSIZE(_MEMBLOCK)       SCI_MemSize(_MEMBLOCK) 
        ...
        #define SCI_DASSERT(_EXP)            (_EXP) ? (void)0 : SCI_DAssert(#_EXP, __FILE__, __LINE__)
                ...
                #define SCI_ALLOC_BASEZ(_SIZE)   SCI_Malloc_Zero(_SIZE, DYNAMIC_HEAP_BASE_MEMORY, __MODULE__, __LINE__)
            	#define SCI_ALLOC_APP(_SIZE)    SCI_MallocEx(_SIZE, DYNAMIC_HEAP_APP_MEMORY, __MODULE__, __LINE__)      	        	    
	            #define SCI_ALLOC_APPZ(_SIZE)    SCI_Malloc_Zero(_SIZE, DYNAMIC_HEAP_APP_MEMORY, __MODULE__, __LINE__)                  
		    	...
		    	#define SCI_ALLOCAZ(_SIZE)   SCI_MallocApp_Zero(_SIZE, __MODULE__, __LINE__)
				#define SCI_CALLOC(_NUM, _SIZE)      SCI_CAlloc(_NUM, _SIZE, __MODULE__, __LINE__)
			 	#define SCI_REALLOC(_MEMBLOCK,_SIZE) SCI_ReAlloc(_MEMBLOCK, _SIZE, __MODULE__, __LINE__)
				#define SCI_MEMSIZE(_MEMBLOCK)       SCI_MemSize(_MEMBLOCK) 
		        #define SCI_FREE(_MEM_PTR)  SCI_Free_Debug(_MEM_PTR, __MODULE__, __LINE__); (_MEM_PTR) = PNULL;				
	        #else
		        #define SCI_ALLOC_CONST(_SIZE)    SCI_MallocExConst_NoDebug(_SIZE)
        	    #define SCI_ALLOC_BASE(_SIZE)     SCI_MallocExBase_NoDebug(_SIZE)
                #define SCI_ALLOC_BASEZ(_SIZE)    SCI_MallocExBase_Zero_NoDebug(_SIZE)
            	#define SCI_ALLOC_APP(_SIZE)      SCI_MallocExApp_NoDebug(_SIZE)      	        	    
	            #define SCI_ALLOC_APPZ(_SIZE)     SCI_MallocExApp_Zero_NoDebug(_SIZE)                  
    	        #define SCI_ALLOC(_SIZE)          SCI_Malloc_NoDebug(_SIZE)
        	    #define SCI_MPALLOC(_SIZE, _HANDLE_)    SCI_MemPoolMalloc_NoDebug(_SIZE, _HANDLE_)
	        	#define SCI_ALLOCA(_SIZE)    SCI_MallocApp_NoDebug(_SIZE)	  
                #define SCI_ALLOCAZ(_SIZE)   SCI_MallocApp_Zero_NoDebug(_SIZE)
				#define SCI_CALLOC(_NUM, _SIZE)      SCI_CAlloc_NoDebug(_NUM, _SIZE)
			 	#define SCI_REALLOC(_MEMBLOCK,_SIZE) SCI_ReAlloc_NoDebug(_MEMBLOCK, _SIZE)
				#define SCI_MEMSIZE(_MEMBLOCK)       SCI_MemSize(_MEMBLOCK) 
		        #define SCI_FREE(_MEM_PTR)  SCI_Free(_MEM_PTR); (_MEM_PTR) = PNULL;
	        #endif      
            ...
            #ifdef __ASSERT_RELEASE_MODE__
	            #define SCI_ASSERT(_EXP)    (_EXP) ? (void)0 : SCI_Assert_NoDebug()
	            #define SCI_PASSERT(_EXP,PRINT_STR)     (_EXP) ? (void)0 : (void )SCI_Assert_NoDebug()
	            #define SCI_DASSERT(_EXP)    (_EXP) ? (void)0 : SCI_Assert_NoDebug()
            #else
...
#if !defined SCI_MEMORY_DEBUG && !defined WIN32
#define  CONST            const
    void* SCI_Memcpy(void* _DEST_PTR, CONST  void* _SRC_PTR, uint32 _SIZE) ;

   void*  SCI_Memset(void* _DEST_PTR, uint32 _VALUE, uint32 _SIZE) ;

#define SCI_MEMCPY(_DEST_PTR, _SRC_PTR, _SIZE) SCI_Memcpy(_DEST_PTR, _SRC_PTR, _SIZE);
#define SCI_MEMSET(_DEST_PTR, _VALUE, _SIZE) SCI_Memset(_DEST_PTR, _VALUE, _SIZE);
...
#define  SCI_GetStaticSpaceMaxAvalidSpace()      SCI_GetByteHeapAvailableSpace(CONST_HEAP_MEMORY)   
#define  SCI_GetSystemSpaceMaxAvalidSpace()      SCI_GetByteHeapAvailableSpace(DYNAMIC_HEAP_APP_MEMORY)   

#define  SCI_GetStaticSpaceTotalAvalidMem()      SCI_GetHeapTotalAvailableSpace(CONST_HEAP_MEMORY)   
#define  SCI_GetSystemSpaceTotalAvalidMem()      SCI_GetHeapTotalAvailableSpace(DYNAMIC_HEAP_APP_MEMORY)   
...
/*****************************************************************************/
//  Description:    SCI_MemSize returns the size (in bytes) as an unsigned integer.
/*****************************************************************************/
PUBLIC uint32 SCI_MemSize(
	void* memory_ptr
	);

/*****************************************************************************/
//  Description:    SCI_CAlloc returns a pointer to the allocated space. The storage
//                  space pointed to by the return value is guaranteed to be uint32
//                  aligned for storage. To get a pointer to a 
//                  type other than void, use a type cast on the return value. 
/*****************************************************************************/
void *SCI_CAlloc( 
	uint32  num,        //Number of elements 
	uint32 size,        //Length in bytes of each element 
	const char * file,  // The name of the file which call this function.
	uint32 line         // The line number in the file which call this function.
	);
...
/*****************************************************************************/
//  Description:    SCI_ReAlloc returns a void pointer to the reallocated (and possibly moved)
//                  memory block. The return value is NULL if the size is zero and the 
//                  buffer argument is not NULL, or if there is not enough available
//                  memory to expand the block to the given size. In the first case, 
//                  the original block is freed. In the second, the original block is 
//                  unchanged. The return value points to a storage space that is guaranteed 
...
void * SCI_ReAlloc( 
	...
	uint32 size,        //New size in bytes 
	const char * file,  // The name of the file which call this function.
	uint32 line         // The line number in the file which call this function.
	);

/*****************************************************************************/
//  Description:    SCI_CAlloc_NoDebug returns a pointer to the allocated space. The storage
//                  space pointed to by the return value is guaranteed to be uint32
//                  aligned for storage. To get a pointer to a 
//                  type other than void, use a type cast on the return value. 
/*****************************************************************************/
void * SCI_CAlloc_NoDebug( 
	uint32 num,           //Number of elements 
	uint32 size           //Length in bytes of each element 
	);

/*****************************************************************************/
//  Description:    SCI_ReAlloc_NoDebug returns a void pointer to the reallocated (and possibly moved)
//                  memory block. The return value is NULL if the size is zero and the 
//                  buffer argument is not NULL, or if there is not enough available
//                  memory to expand the block to the given size. In the first case, 
//                  the original block is freed. In the second, the original block is 
...
PUBLIC uint32 SCI_Free_Debug(     // SCI_SUCCESS or SCI_ERROR
					   void * memory_ptr,        // Pointer to memory block to be deleted.
					   const char * file,  // The name of the file which call this function.
                       uint32 line         // The line number in the file which call this function.
					   );
...
/*****************************************************************************/
//  Description:    The SCI_MallocApp_Zero function is used to Allocates memory blocks
//                  in embedded system. It allocates memory from different 
//                  memory pool heap according to the size applications needed.
//                  This can avoid memory fragments and speed up memory 
//                  allocating, the shortcoming is that we might waste some 
...
//	Global resource dependence: 
//  Author: Richard.Yang
//	Note:           Don't use it directly. Please use SCI_ALLOCAZ
/*****************************************************************************/
PUBLIC void * SCI_MallocApp_Zero(   // Returns a void pointer to the allocated space, 
                            // or SCI_NULL if there is insufficient memory
                            // available.
    uint32 size,        // Bytes to allocate.
    const char * file,  // The name of the file which call this function.
...
/*****************************************************************************/
//  Description:    The function send signal to proper thread if not trapping on
//	Global resource dependence: 
//  Author: Pizer.Fan
//	Note:
/*****************************************************************************/
PUBLIC uint32 SCI_SendSignal_PS(   // If send successfully, return SCI_SUCCESS,
                                // else return SCI_ERROR
    xSignalHeader   signal_ptr, // Signal pointer to be sended
    BLOCK_ID        dest_id     // Dest thread ID
    );
...
/*****************************************************************************/
//  Description:    The function creates a dynamic thread.
//                  The stack and queue memory used in the thread should 
//                  be alloced before call this function.
//	Global resource dependence: 
//  Author:         Richard.Yang
//	Note:           
/*****************************************************************************/
PUBLIC BLOCK_ID SCI_CreateAppThreadEx(       // If successful, returns the thread ID
                                        // of the new created thread, 
                                        // else returns SCI_INVALID_BLOCK_ID
    const char  *thread_name,           // Name string of the thread
    void    (*entry)(uint32, void*),    // Entry function of the thread
    uint32  argc,                       // First parameter for entry function,
    void    *argv,                      // Second parameter for entry function,
    void    *stack_mem_start,       // Start pointer of the thread stack
    uint32  stack_mem_size,         // Size of the thread stack in bytes
    uint32  priority,           // Prority of the thread
    uint32  preempt,            // Indicates if the thread is preemptable.
    const char  *queue_name,    // Name of the queue
    void    *queue_mem_start,       // Start pointer of the queue.
    uint32  queue_mem_size,          // Number of messages which can be enqueued.
    uint32  auto_start          // Specifies whether the thread starts 
                                // immediately or stays in a pure suspended
                                // state. Legal options are SCI_AUTO_START
                                // and SCI_DONT_START.
...
/*****************************************************************************/
//  Description:    function is used to register callback handler for thread stack check
//	Global resource dependence: 
//  parameter:      void* : ptr of SCI_THREAD_T
//  Author:         
//	Note:           can do assert handle and debug
/*****************************************************************************/
PUBLIC void SCI_SetThreadStackErrorNotify(
   void(*entry)(void*)
   );
...
/*****************************************************************************/
//  Description:    The function retrieves information about the specified 
//                  application timer. time unit is tick, not ms
//	Global resource dependence: 
//  Author:         yong.luo
//	Note:           
/*****************************************************************************/
PUBLIC uint32 SCI_GetTimerInfoTick( // If get successful, returns SCI_SUCCESS,
                                // else return SCI_ERROR
    SCI_TIMER_PTR timer_ptr,    // Pointer to a previously created application
                                // timer.
    char    *name,              // Pointer to an SCI_MAX_NAME_SIZE character
...
/*****************************************************************************/
//  Description:    The function adjust all timer when wakeed up to add  
//                  timetick of sleeping. 
//	Global resource dependence: 
//  Author:         yong.luo
//	Note: 
/*****************************************************************************/
PUBLIC void SCI_AdjustTimer(uint32 timeTick);
...
/*******************************************************************************************/
//     Description : print current stack info
//    param : 
// 			level: '0' is print current position . '1' is where calling this. '-1' is print all info with this thread stack. MAX is 50;
//
//  Author: jun.song
//    Note:
/*********************************************************************************************/
PUBLIC void SCI_PrintStackTrace(int level);
...
/*****************************************************************************/
//  Description:    This function is to get the total availd fragment space in the special byte heap
//	Global resource dependence: 
//  Author: Thomas.Chai
//	Note:       
/*****************************************************************************/
PUBLIC uint32  SCI_GetHeapTotalAvailableSpace(uint32  heap_type);


/*****************************************************************************/
//  Description:    This function is to get the total space in the special byte heap
//	Global resource dependence: 
//  Author: Thomas.Chai
//	Note:       
/*****************************************************************************/
PUBLIC uint32  SCI_GetHeapTotalSpace(uint32  heap_type);
...
//  Description:    The SCI_MallocApp_Zero_NoDebug function is used to Allocates memory blocks
//                  in embedded system. If the system App Heap memory is not enough, return NULL
//	Global resource dependence: 
//  Author: 
//	Note:           Don't use it directly. Please use SCI_ALLOC_CONST/SCI_ALLOC_BASE/SCI_ALLOC_APP
/*****************************************************************************/
PUBLIC void * SCI_MallocApp_Zero_NoDebug(   // Returns a void pointer to the allocated space, 
                                         // or SCI_NULL if there is insufficient memory
                                         // available.
                                         uint32 size        // Bytes to allocate.
    );
...
#define BLKMEM_SUCCESS  0
#define BLKMEM_ERROR    1
#define BLKMEM_EXIST    2

typedef uint32 (*blkmem_callback)(uint32 id, void* param);

typedef struct BLKMEM_CONFIG_Tag
{
    void*  param;     //The parameter of blkmem call back
	blkmem_callback  func_callback;  //The call back function
}BLKMEM_CONFIG_T;


PUBLIC uint32 BLKMEM_Register(            //BLKMEM_SUCCESS --- successful;  BLKMEM_ERROR --fail; BLKMEM_EXIST --- module had been registed
                              uint32 id,     // the register module id
                              BLKMEM_CONFIG_T* config_ptr  // the register module operation config
                              );

PUBLIC uint32 BLKMEM_UnRegister(         //BLKMEM_SUCCESS --- successful;  BLKMEM_ERROR --fail;
                               uint32 id);  // id --- the registed module id

PUBLIC void*  BLKMEM_Malloc(    //Returns a void pointer to the allocated space; NULL ---- No Memory
                            uint32 id,    // id --- the registed module id
                            uint32 len);  // len --- The memory length want to alloc

PUBLIC uint32 BLKMEM_Free(    //BLKMEM_SUCCESS --- successful;  BLKMEM_ERROR --fail;
                          uint32 id,   // id --- the registed module id
                          void* mem_ptr); // a  pointer of the allocated space; 

PUBLIC uint32 BLKMEM_GarbageClean(void);  //BLKMEM_SUCCESS --- successful;  BLKMEM_ERROR --fail;
/**---------------------------------------------------------------------------*
 **                         Compiler Flag                                     *
 **---------------------------------------------------------------------------*/
#ifdef __cplusplus
