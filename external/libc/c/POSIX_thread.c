...
{
    .uiStackSize                = PTHREAD_STACK_MIN2,
    .usSchedPriorityDetachState = ( ( uint16 ) tskIDLE_PRIORITY & pthreadSCHED_PRIORITY_MASK ) | ( PTHREAD_CREATE_JOINABLE << pthreadDETACH_STATE_SHIFT )
};
...
static void vTaskSetApplicationTaskTag(TaskHandle_t xTaskHandle, pthread_internal_t *pxThread)
{
    ...
    #else
    pthread_id_map_t *new_node = (pthread_id_map_t *)SCI_ALLOC(sizeof(pthread_id_map_t));
...
static pthread_t xTaskGetApplicationTaskTag(TaskHandle_t xTaskHandle)
{
    ...
    {    
      ...
      #else
        pthread_id_map_t *tmp = g_pthread_id_tbl;
...
int pthread_setspecific(pthread_key_t key, const void *value)
{
    ...
    vTaskSetThreadLocalStoragePointer(0, i * 2, value);
