...
__inline fixed fixed_idiv(int a, int b)     {if (0 != b) return (a<<FIXED_PRECISION)/b;}
...
/*****************************************************************************/
//  Description: Fill Layer with specific color Except a round field.
//  Global resource dependence:
//  Author: Frank.Zhou
//  Note:
/*****************************************************************************/
PUBLIC void LCD_FillLayerExceptRoundField(
                           const GUI_LCD_DEV_INFO* 	dev_info_ptr,                           
                           int16                   	x,
                           int16                   	y,
                           uint16                  	radius,
                           ...
                           )
{
	...
	dest_rect.left = 0;
	dest_rect.top = 0;
	dest_rect.right = lcd_width+dest_rect.left-1;
	dest_rect.bottom = lcd_height+dest_rect.top-1;
    ...
    if (DATA_TYPE_ARGB888 == UILAYER_GetLayerColorType((GUI_LCD_DEV_INFO *)dev_info_ptr))
    {
   		...
   		for(i = 0; i< lcd_height; i++)
		{
			for(j = 0; j < lcd_width; j++)
			{
				...
				{
					*((u32_block_buffer_ptr) + (i) * (mem_width) + (j)) = ConvertRgb565ToArgb888(color, alpha);					
				}

				if (pixels>0 && ((j-x)*(j-x)+(i-y)*(i-y))>((radius)*(radius) )&& ((j-x)*(j-x)+(i-y)*(i-y))<=((radius+pixels)*(radius+pixels)))
				{
					*((u32_block_buffer_ptr) + (i) * (mem_width) + (j)) = ConvertRgb565ToArgb888(line_color, 0xff);	
				}
			}
		}
    }
        ...
        for(i = 0; i< lcd_height; i++)
		{
			for(j = 0; j < lcd_width; j++)
			{
				if(((j-x)*(j-x)+(i-y)*(i-y))>(radius*radius))
				{
					if( 0xFF == alpha )
					{
						*((block_buffer_ptr) + (i) * (mem_width) + (j)) = color;
					}
					else
					{
						GUI_COLOR_T *dst_ptr = (block_buffer_ptr) + (i) * (mem_width) + (j);
						...
						uint8  n = (alpha + 1) >> 3;
			                    ...
			                    _x = ( _x | (_x << 16)) & 0x7E0F81F;
			                    _y = ( _y | (_y << 16)) & 0x7E0F81F;
			                    
			                    result = ( (( (_x - _y) * n ) >> 5 ) + _y) & 0x7E0F81F;
				...
				if (pixels>0 && ((j-x)*(j-x)+(i-y)*(i-y))>((radius)*(radius) )&& ((j-x)*(j-x)+(i-y)*(i-y))<=((radius+pixels)*(radius+pixels)))
				{
					//if( 0xFF == alpha )
					{
						*((block_buffer_ptr) + (i) * (mem_width) + (j)) = line_color;
					}
					/*
					else
					{
                                 ...
                                 )
{
    ...
    if (dev_info_ptr->block_id >= GUI_BLOCK_MAX)
        return FALSE;
#else
    if (dev_info_ptr->block_id >= GUI_UILAYER_MAX)
        return FALSE;
#endif
