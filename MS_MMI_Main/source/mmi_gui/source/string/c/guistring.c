    ...
    if (is_single_line) // 单行显示的时候
    {
        ...
        {            //显示...
            if (GUISTR_GetStringWidth(style_ptr, str_ptr, state) > text_rect.right - text_rect.left + 1)
            {
                ...
                SCI_MEMSET(temp_str, 0x00, (str_ptr->wstr_len + ellipsis_len + 1) * sizeof(wchar));
                ...
                ))
            {
                ...
                if ( is_ellipsis )
                {
                    ...
                    if ( is_true )
                    {
                        ...
                        SCI_MEMSET(temp_str, 0x00, (str_data.wstr_len + ellipsis_len + 1) * sizeof(wchar));
...
//        param.buffer_width = des_buf_ptr->width;
        ...
        //for huawei
        //不需要自动调整bearing y的功能
        param.format |= TEXT_FORMAT_NO_AUTOBY;  /*Disable后整体显示效果较差，将原有修改回退，504639/506108通过去除加粗以及微调字库后解决*///Disable it to fix bugs 504639,506108
...
#ifndef WIN32
#if defined(BROWSER_SUPPORT) && defined(OPERA_MINI_SUPPORT_VER42)
                    if(!OD_SPRD_GetOperaDisplay())
#endif
#endif
                    {
                        if(TEXT_DIR_RTL == dir)
                        {
                            param.dir = GUISTR_TEXT_DIR_RTL;                    
                            param.format |= ConvertAlignment( state, GUISTR_TEXT_DIR_RTL, is_change_align, ALIGN_RIGHT );
                            // 设置文本其他状态
                            ConvertTextState(state, &param.format);
                        }
                    }
...
LOCAL GUI_RECT_T DrawTextImgInLine(
                                GUISTR_STYLE_T     *style_ptr,
                                GUISTR_BUF_T       *des_buf_ptr,
                                const GUI_RECT_T   *disp_rect_ptr,       //the fixed display area
                                const GUI_RECT_T   *clip_rect_ptr,       //用户要剪切的实际区域
...
//        param.buffer_width = des_buf_ptr->width;
        ...
        param.size = MMITHEME_PHONE_IMG_HEIGHT;
        ...
        PhImg_DrawText(style_ptr,&param, string.wstr_ptr, string.wstr_len, &metrics_data );
                          ...
                          )
{
	...
	if (str_ptr == PNULL || !CheckRectValid(rect))
    {
        //SCI_TRACE_LOW:"GetYPosition invalid input param!"
        SCI_TRACE_ID(TRACE_TOOL_CONVERT,GUISTRING_538_112_2_18_3_21_42_346,(uint8*)"");
        return y;
    }

    font_height = img_height;
    ...
    else
    {
        ...
        {
            //SCI_TRACE_LOW:"[GetYPosition guistring.c] len = %d, str_ptr = %x"
            SCI_TRACE_ID(TRACE_TOOL_CONVERT,GUISTRING_590_112_2_18_3_21_42_347,(uint8*)"dd", len, str_ptr);
        }
...
LOCAL GUI_RECT_T DrawTextImgInRect(
                                GUISTR_STYLE_T     *style_ptr,
                                GUISTR_BUF_T       *des_buf_ptr,
                                const GUI_RECT_T   *disp_rect_ptr,       //the fixed display area
                                const GUI_RECT_T   *clip_rect_ptr,       //用户要剪切的实际区域
                                ...
                                )
{
    ...
    BOOLEAN is_change_align = ( TEXT_DIR_AUTO == dir ) ? TRUE : FALSE;
    BOOLEAN is_single_line = IsSetState(state, GUISTR_STATE_SINGLE_LINE); // 是否单行显示
    BOOLEAN is_ellipsis = IsSetState(state, GUISTR_STATE_ELLIPSIS);
    wchar  *temp_str = PNULL;
    // 保存
    ...
    // 获取Y的坐标
    y = GetImgYPosition(*disp_rect_ptr, str_ptr->wstr_ptr, str_ptr->wstr_len, 
        style_ptr->char_space, font_height, 
        style_ptr->align, style_ptr->line_space, is_single_line);
    ...
    if (is_single_line) // 单行显示的时候
    {
        ...
        act_rect = DrawTextImgInLine(tmp_style_ptr, des_buf_ptr, &text_rect, &clip_rect, (const MMI_STRING_T*)&str_data, state, dir, FALSE);
...
//                     }
                ...
                // 显示字符
                line_act_rect = DrawTextImgInLine(tmp_style_ptr, des_buf_ptr, &text_rect, &clip_rect, &str_data, state, dir, is_change_align);
...
PUBLIC GUI_RECT_T GUISTR_DrawTextImgInRect( 
                                        GUISTR_BUF_T          *des_buf_ptr,
                                        const GUI_RECT_T      *disp_rect_ptr,       //the fixed display area
                                        const GUI_RECT_T      *clip_rect_ptr,       //用户要剪切的实际区域
                                        const MMI_STRING_T    *str_ptr,
                                        ...
                                        )
{
    ...
    box = DrawTextImgInRect(style_ptr, des_buf_ptr,disp_rect_ptr, clip_rect_ptr, str_ptr, state, dir);
...
PUBLIC void GUISTR_DrawTextImgToLCDInRect( 
                                       const GUI_LCD_DEV_INFO *dev_info_ptr,
                                       const GUI_RECT_T      *disp_rect_ptr,       //the fixed display area
                                       const GUI_RECT_T      *clip_rect_ptr,       //用户要剪切的实际区域
                                       const MMI_STRING_T    *str_ptr,
                                       ...
                                       )
{
    ...
    clip_rect_ptr = LCD_ConvertDispRect(GUILCD_CONVERT_POS_LCD2DISP, (GUI_LCD_DEV_INFO*)dev_info_ptr, clip_rect_ptr, &clip_rect);
#endif
    box = GUISTR_DrawTextImgInRect(&des_buf_data, disp_rect_ptr, clip_rect_ptr, str_ptr,style_ptr, state, dir);
    
    MMITHEME_StoreUpdateRect(dev_info_ptr, box);
}

/*****************************************************************************/
PUBLIC void GUISTR_DrawImgToLCDInRect( 
                                       const GUI_LCD_DEV_INFO *dev_info_ptr,
                                       const GUI_RECT_T      *disp_rect_ptr,       //the fixed display area
                                       const GUI_RECT_T      *clip_rect_ptr,       //用户要剪切的实际区域
                                       const MMI_STRING_T    *str_ptr,
                                    ...
                                    )
{
    ...
    string.wstr_len = str_ptr->wstr_len;
    string.wstr_ptr = str_ptr->wstr_ptr;
        ...
        || style_ptr->region_num > 0)
    {
        ...
        // 获取字符串的宽度
        SPMLAPI_GetTextMetrics( &param, string.wstr_ptr, string.wstr_len, &metrics );
    ...
    {
        info_ptr->width = GUI_GetStringWidthEx( style_ptr->font, string.wstr_ptr, string.wstr_len, style_ptr->char_space );
        info_ptr->height = GUI_GetStringHeightEx ( style_ptr->font, string.wstr_ptr, string.wstr_len);//param.size;
    }
...
)
{
    ...
    cur_node_ptr = *head_pptr;
	if( NULL != region_ptr ){   //for coverity.
		region = *region_ptr;
	}
                            ...
                            )
{
	...
	if (in_region_ptr == NULL) return 0;
                                ...
                                )
{
    ...
    else
    {
            ...
            || style_ptr->region_num > 0 )
        {
            ...
            *char_num = SPMLAPI_LineBreak( &param, string.wstr_ptr, string.wstr_len, width, pixel_num, is_byword );
        ...
        {
            result = GUI_GetStringInfoInLineWithDir( dir, string.wstr_ptr, string.wstr_len, 
