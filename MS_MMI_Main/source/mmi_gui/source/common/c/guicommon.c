...
/*******************************************************************************************/
// 	Description : calculate the string scroll start positon
//	Global resource dependence :
//  Author: tingting.zheng
//  param    str_width          [in]  string pixel number
//  param    display_width      [in]  the width of display rect
//  param    scroll_step        [in]  pinxel num of scroll one step
//  param    current_pos        [in]  current pixel positon
//  return   scroll start positon 
//	Note:
/*********************************************************************************************/
PUBLIC uint16 GUI_CalculateStringScrollStartPos(
                                          uint16    str_width,
...
{
    uint16 scroll_start_pos = 0;

    if ((str_width - 1 - current_pos) > display_width)
    {
        scroll_start_pos = current_pos + scroll_step;
    }
    else if ((str_width > display_width)&&((str_width - 1 - display_width) <= scroll_step))//只满足滚动一步;字符串宽度小于显示宽度，不做滚动，解决bug 1260540
    {
        if (current_pos / scroll_step == MMITHEME_TEXT_SCROLL_STEP_MIN)//但为了美观，需要滚动 LIST_SCROLL_STEP_MIN步，for bug1251377
        {
            scroll_start_pos = 0;
        }
        else
...
PUBLIC BOOLEAN GUI_GetPhImgInfoInLineWithDir(
                                               int8             dir,
                                               const wchar		*str_ptr,
                                               uint16      		str_len,
                                               uint16  img_width,
...
PUBLIC BOOLEAN GUI_GetPhImgInfoInLine(
                                       const wchar* str_ptr,    //[IN] String指针
                                       uint16       str_len,    //[IN] String长度(Byte长度)
                                       uint16  img_width,       //[IN] String的字体
                                       uint8        char_space,  //[IN] 字间距
                                       uint16       width,
                                       uint16*      char_num,
                                       uint16*      pixel_num,
...
{
    return GUI_GetPhImgInfoInLineWithDir( TEXT_DIR_AUTO, str_ptr, str_len, img_width, char_space, width, char_num, pixel_num, is_byword );
}
PUBLIC uint16 GUI_CalculatePhImgLinesByPixelNum(
                                                 uint16    pixelnum,    //[IN] 像素个数
                                                 const wchar* str_ptr,  //[IN] String指针
                                                 uint16     str_len,    //[IN] String长度(Byte长度)
                                                 uint16  img_width,       //[IN] String的字体
                                                 ...
                                                 )
{
    ...
    while(cur_len < str_len)
    {
        if( GUI_GetPhImgInfoInLine(&str_ptr[cur_len], 
            (uint16)(str_len - cur_len),
            img_width,
            char_space,
            pixelnum,
...
PUBLIC uint16 GUI_GetPhImgWidth(
                                 uint16  img_width,
                                 const wchar* str_ptr,
                                 uint16 str_len
                                 )
...
PUBLIC uint16 GUI_GetPhImgWidthEx(
                                   uint16  img_width,    
                                   const wchar* str_ptr,
                                   uint16 str_len,
                                   uint16 char_space
                                   )
{
    ...
    for( ; i < str_len; i++ )
    {
        ...
        char_width = img_width+char_space;
...
PUBLIC uint16 GUI_CalculatePhImgPiexlNum(
                                          const wchar* str_ptr,    //[IN] String指针
                                          uint16       str_len,    //[IN] String长度(Byte长度)
                                          uint16  img_width,       //[IN] String的字体
                                          uint8        char_space  //[IN] 字间距
                                          )
{
    ...
    str_piexl_num = GUI_GetPhImgWidthEx(img_width, str_ptr, str_len, char_space);
        ...
        *pixel_num = 0;
        ...
        do {
            ...
            else
            {
                ...
                // default stream will auto add space into string so that calculate line number wrong
                // Add for prevent corner case
                if (((w_char != 32)&& ((*pixel_num + char_width) > width))||((pre_char == 32)&& ((*pixel_num + char_width) > width)))
                {
                    //存在按词换行字符
                    if( is_exist_word_bread_ch )
                    {
						   ...
						   )
{
    ...
    case GUI_BG_COLOR:
        ...
        break;
    case GUI_BG_HALF_TRANS:
         LCD_FillArgbRect(lcd_dev_ptr, display_ptr->display_rect, bg_ptr->color, (uint8)MMI_IDLE_NTF_LIST_BG_ALPHA);
         break;
    default:
            ...
            if (GUI_IntersectRect(&sheen_input_ptr->rect,show_rect,sheen_input_ptr->rect))
            {
                    ...
                    (PNULL != draw_dc.lcd_buffer_ptr))
                {
                    ...
                    if (DATA_TYPE_ARGB888 == UILAYER_GetLayerColorType(&sheen_input_ptr->lcd_dev))
                    {
                        ...
                        //alloc copy buffer
                        sheen_ptr->copy_buf_ptr = SCI_ALLOCA((uint32)(rect_width*rect_height)*sizeof(uint32));
...
#endif
                    {
                        ...
                        //alloc copy buffer
                        sheen_ptr->copy_buf_ptr = SCI_ALLOCA((uint32)(rect_width*rect_height)*sizeof(GUI_COLOR_T));
                        if (PNULL != sheen_ptr->copy_buf_ptr)
                        {
