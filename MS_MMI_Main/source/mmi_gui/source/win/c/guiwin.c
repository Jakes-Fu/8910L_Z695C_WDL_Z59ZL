...
#include "../../mmi_ctrl/source/Softkey/h/ctrlsoftkey.h"
...
// Note:
/*****************************************************************************/
PUBLIC BOOLEAN GUIWIN_GetSoftkeyId(
                                       MMI_HANDLE_T    win_handle,    // 窗口句柄
                                       ...
                                       )
{
    ...
    if (0 != ctrl_id)
    {
        result = GUISOFTKEY_GetId(win_handle,ctrl_id,
                        left_id_ptr,middle_id_ptr,right_id_ptr);
    }
...
/*****************************************************************************/
// Description : 设置softkey背景的透明值
// Global resource dependence : 
// Author:
// Note:
/*****************************************************************************/
PUBLIC BOOLEAN GUIWIN_SetSoftkeyBgAlpha(
                                MMI_HANDLE_T    win_handle,                   
                                uint8         bg_alpha                            
                                )
{
    ...
    {
        result = TRUE;
        GUISOFTKEY_SetBgAlpha(win_handle,ctrl_id,bg_alpha);
    }
...
// Note:
/*****************************************************************************/
PUBLIC BOOLEAN GUIWIN_SetSoftKeyButtonfont(
                                MMI_HANDLE_T    win_handle,                   
                                GUISOFTKEY_INDEX button_num,
                                GUI_FONT_T      *font_ptr
                                )
{
    ...
    {
        result = TRUE;
        GUISOFTKEY_SetButtonFont(win_handle, ctrl_id,button_num ,font_ptr);
    }
...
/*****************************************************************************/
// Description : 获得softkey是否置灰
// Global resource dependence : 
// Author:weipeng.wang
// Note: 
/*****************************************************************************/
PUBLIC BOOLEAN  GUIWIN_GetSoftkeyBtnGray(MMI_HANDLE_T win_handle, uint16 button_index)
{
    CTRLSOFTKEY_OBJ_T	*softkey_ptr = PNULL;
    //IGUICTRL_T *ctrl_ptr = PNULL;
    MMI_CTRL_ID_T ctrl_id = MMK_GetWinSoftkeyCtrlId(win_handle);
    //ctrl_ptr = MMK_GetCtrlPtrByWin(win_id, MMIMMS_SMS_EDIT_TEXT_CTRL_ID);
    softkey_ptr = (CTRLSOFTKEY_OBJ_T*)MMK_GetCtrlPtrByWin(win_handle, ctrl_id);

    if (PNULL == softkey_ptr)
    {
        SCI_TRACE_LOW("[wwwwww]GUIWIN_GetSoftkeyBtnGray: softkey_ptr is PNULL");
        return FALSE;
    }
    SCI_TRACE_LOW("[wwwwww]GUIWIN_GetSoftkeyBtnGray: softkey_ptr->button_arr[%d].is_gray=%d", button_index, softkey_ptr->button_arr[button_index].is_gray);
    return softkey_ptr->button_arr[button_index].is_gray;
}
...
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN GUIWIN_GetStbBgIsDisplay(
                                     MMI_HANDLE_T win_handle
                                     )
{
    return GUISTATUSBAR_GetBgIsDisplay(win_handle);
}
...
PUBLIC BOOLEAN GUIWIN_HasTitle(MMI_HANDLE_T win_handle)
{
