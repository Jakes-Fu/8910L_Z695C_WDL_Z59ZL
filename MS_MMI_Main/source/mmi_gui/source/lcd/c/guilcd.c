...
//                UILAYER_InvalidateRect(lcd_id,&rect);
        ...
        else
        {
            ...
            // uix8910 use the soft blending
            if (!is_only_invalidate && PNULL != blend_param_ptr)
            {
                DISPLAY_BlendLayer(lcd_id, blend_param_ptr->dst_layer_ptr, 
                    blend_param_ptr->layer_arr, 
                    blend_param_ptr->layer_num, 
                    blend_param_ptr->rect_ptr);
#ifdef WIN32
                //模拟器显示多图层图像
                MSDev_DISPLAY_BlendLayer(lcd_id, 
                    blend_param_ptr->layer_arr, 
                    blend_param_ptr->layer_num, 
                    &invalidate_param.param_ptr->rect);
#endif
            }
