#if defined(GBK_SUPPORT) || defined(MULTI_VARIANT_SUPPORT)
/*****************************************************************************
** File Name:      gui_gbk.c                                                 *
** Author:                                                                   *
** Date:           01/21/2010                                                *
...
**                 from UCS2 string to GBK string                            *
******************************************************************************
**                         Important Edit History                            *
** --------------------------------------------------------------------------*
** DATE           NAME             DESCRIPTION                               *
** 01/21/2010    James.Zhang         Create                                  *
******************************************************************************/
...
extern int16 GUI_GetUCSOffset(//return the offset in GB2312 table.
                              uint16 ucs2	//an ucs2 code. 0x4E00 <= ucs2 < 0xA000 
                              ) ;
/**--------------------------------------------------------------------------*
 **                         STATIC DEFINITION                                *
 **--------------------------------------------------------------------------*/
...
LOCAL const uint16 s_gbkext1_2uni_page81[6080] = 
{
  /* 0x81 */
...
LOCAL const uint16 s_gbkext2_2uni_pagea8[8272] = {
  /* 0xa8 */
...
LOCAL const uint16 s_gbkext_inv_2charset[14313] = {
...
LOCAL const GBK_SUMMARY_T s_gbkext_inv_uni2indx_page02[14] = 
{
  /* 0x0200 */
...
LOCAL const GBK_SUMMARY_T s_gbkext_inv_uni2indx_page20[44] = 
{
  /* 0x2000 */
...
LOCAL const GBK_SUMMARY_T s_gbkext_inv_uni2indx_page25[17] = 
{
  /* 0x2500 */
...
LOCAL const GBK_SUMMARY_T s_gbkext_inv_uni2indx_page30[16] = 
{
  /* 0x3000 */
...
LOCAL const GBK_SUMMARY_T s_gbkext_inv_uni2indx_page32[30] = 
{
  /* 0x3200 */
...
LOCAL const GBK_SUMMARY_T s_gbkext_inv_uni2indx_page4e[1307] = 
{
  /* 0x4e00 */
...
LOCAL const GBK_SUMMARY_T s_gbkext_inv_uni2indx_pagef9[19] = 
{
  /* 0xf900 */
...
LOCAL const GBK_SUMMARY_T s_gbkext_inv_uni2indx_pagefe[31] = 
{
  /* 0xfe00 */
...
LOCAL const uint16 s_cp936ext_2uni_pagea6[181-159] = 
{
    /* 0xa6 */
    0xfe35,
...
LOCAL const uint16 s_cp936ext_2uni_pagea8[128-122] = 
{
    /* 0xa8 */
...
LOCAL const uint16 s_cp936ext_page01[16] = 
{
...
LOCAL const uint16 s_cp936ext_page02[24] = 
{
...
LOCAL const uint16 s_cp936ext_pagefe[24] = 
{
...
{
    return GUI_GBKToWstrWithEndPos(wstr_ptr, gbk_ptr, str_len, PNULL);
} 

/*****************************************************************************/
// 	Description : gbk to wstr, with returning the end of src string.
//	Global resource dependence : none
//  Author: Yintang.ren
//	Note: 
/*****************************************************************************/
									  ...
									  )
{
        ...
        else if ( ch1 >= 0x81 && ch1 < 0xff )
        {
            ...
            if ( ch1 >= 0xa1 && ch1 <= 0xf7 ) 
            {
                ...
                if ( ch2 >= 0xa1 && ch2 < 0xff ) 
                {
                    ...
                    if ( offset < 8272 )
                    {
                        unicode = sci_gb2312_to_ucs2_table[offset];
                        
                        if ( 0 == unicode )
                        {
                            unicode = UCS_CHAR_UNDEF;
                        }
                    ...
                    if ( ch1 == 0xa6 || ch1 == 0xa8 ) 
                    {
                            ...
                            || ( ch2 >= 0x80 && ch2 < 0xff ) ) 
                        {
                            ...
                            if ( i < 7410 ) 
                            {
                                if (i >= 7189 && i < 7211)
                                {
                                    unicode = s_cp936ext_2uni_pagea6[i-7189];
                                }
                            } 
                            else 
                            {
                                if ( i >= 7532 && i < 7538 )
                                {
                                    unicode = s_cp936ext_2uni_pagea8[i-7532];
                                }
                            }
            ...
            if ( ch1 >= 0x81 && ch1 <= 0xa0 )
            {
                    ...
                    || ( ch2 >= 0x80 && ch2 < 0xff ) ) 
                {
                    ...
                    if (i < 6080)
                    {
                        unicode = s_gbkext1_2uni_page81[i];
                    }
            ...
            if ( ch1 >= 0xa8 && ch1 <= 0xfe )
            {
                if ((ch2 >= 0x40 && ch2 < 0x7f) || (ch2 >= 0x80 && ch2 < 0xa1)) 
                {
                    ...
                    if (i < 12016)
                    {
                        unicode = s_gbkext2_2uni_pagea8[i-3744];
                    }
    ...
    if( PNULL != src_end_pos )
    {
        *src_end_pos = (uint32)(gbk_ptr);
    }
    
    return count;
} 
/*****************************************************************************/
// 	Description : wstr to gbk
//	Global resource dependence : none
//  Author: James.Zhang
//	Note: 
/*****************************************************************************/
PUBLIC uint32 GUI_WstrToGBK(                         // return the length of gbk_ptr
...
{ 
    ...
    if ( PNULL == gbk_ptr || PNULL == wstr_ptr || 0 == wstr_len )
    {
        return 0;
    }
    ...
    for ( ; i < wstr_len; i++ )
    {
        ...
        if ( unicode < 0x80 )
        {
            *gbk_ptr++ = (uint8)unicode;
            gb_length++;
        }
        else
        {
            BOOLEAN is_find_gb2312 = FALSE;

#ifdef IM_SIMP_CHINESE_SUPPORT                           
            //gb2312
            if( (0x4E00 <= unicode) && (0xA000 > unicode) )
            {
                offset = GUI_GetUCSOffset( unicode );
                
                if (offset >= 0)
                {
                    *gbk_ptr++ = sci_ucs2_to_gb2312_table[ offset ] >> 8;
                    *gbk_ptr++ = sci_ucs2_to_gb2312_table[ offset ] & 0xff;
                    gb_length += 2;
            ...
            else
            {
                ...
                {
                    if( unicode == sci_gb2312_to_ucs2_table[ offset ] )
                    {
                        *gbk_ptr++ = offset / 94 + 0xA0; 
                        *gbk_ptr++ = offset % 94 + 0xA1;
                        gb_length += 2;
            ...
            {
                //gbkext
                const GBK_SUMMARY_T *summary = PNULL;
                
                if (unicode >= 0x0200 && unicode < 0x02e0)
                {
                    summary = &s_gbkext_inv_uni2indx_page02[(unicode>>4)-0x020];
                }
                else if (unicode >= 0x2000 && unicode < 0x22c0)
                {
                    summary = &s_gbkext_inv_uni2indx_page20[(unicode>>4)-0x200];
                }
                else if (unicode >= 0x2500 && unicode < 0x2610)
                {
                    summary = &s_gbkext_inv_uni2indx_page25[(unicode>>4)-0x250];
                }
                else if (unicode >= 0x3000 && unicode < 0x3100)
                {
                    summary = &s_gbkext_inv_uni2indx_page30[(unicode>>4)-0x300];
                }
                else if (unicode >= 0x3200 && unicode < 0x33e0)
                {
                    summary = &s_gbkext_inv_uni2indx_page32[(unicode>>4)-0x320];
                }
                else if (unicode >= 0x4e00 && unicode < 0x9fb0)
                {
                    summary = &s_gbkext_inv_uni2indx_page4e[(unicode>>4)-0x4e0];
                }
                else if (unicode >= 0xf900 && unicode < 0xfa30)
                {
                    summary = &s_gbkext_inv_uni2indx_pagef9[(unicode>>4)-0xf90];
                }
                else if (unicode >= 0xfe00 && unicode < 0xfff0)
                {
                    summary = &s_gbkext_inv_uni2indx_pagefe[(unicode>>4)-0xfe0];
                }
                
                if (summary) 
                {
                    uint16 used = summary->used;
                    uint32 index = unicode & 0x0f;
                    
                    if (used & ((uint16) 1 << index)) 
                    {
                        /* Keep in `used' only the bits 0..index-1. */
                        used &= ((uint16) 1 << index) - 1;
                        ...
                        *gbk_ptr++ = s_gbkext_inv_2charset[summary->indx + used] >> 8;
                        *gbk_ptr++ = s_gbkext_inv_2charset[summary->indx + used] & 0xff;
                        gb_length += 2;
                        continue;
                    }
                }
                
                if (unicode >= 0x2170 && unicode <= 0x2179) 
                {
                    *gbk_ptr++ = 0xa2;
                    *gbk_ptr++ = 0xa1 + (unicode-0x2170);
                    gb_length += 2;
                    continue;
                }
                
                //cp936ext
                if (unicode >= 0x0140 && unicode < 0x0150)
                {
                    *gbk_ptr++ = s_cp936ext_page01[unicode-0x0140] >> 8;
                    *gbk_ptr++ = s_cp936ext_page01[unicode-0x0140] & 0xff;
                    gb_length += 2;
                    continue;
                }
                else if (unicode >= 0x0250 && unicode < 0x0268)
                {                   
                    *gbk_ptr++ = s_cp936ext_page02[unicode-0x0250] >> 8;
                    *gbk_ptr++ = s_cp936ext_page02[unicode-0x0250] & 0xff;
                    gb_length += 2;
                    continue;
                }
                else if (unicode >= 0xfe30 && unicode < 0xfe48)
                {                   
                    *gbk_ptr++ = s_cp936ext_pagefe[unicode-0xfe30] >> 8;
                    *gbk_ptr++ = s_cp936ext_pagefe[unicode-0xfe30] & 0xff;
                    gb_length += 2;
                    continue;
                }
