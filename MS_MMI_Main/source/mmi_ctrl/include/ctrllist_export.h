                                        ...
                                        )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        AppendListItem(ctrl_id, IMAGE_SECMENU_ICON_SET_CALL, TXT_HELLO_TEST_LIST1, STXT_SOFTKEY_OK_MK, STXT_RETURN);
        AppendListItem(ctrl_id, IMAGE_SECMENU_ICON_SET_PHONE, TXT_HELLO_TEST_LIST2, STXT_SELECT,STXT_RETURN);
        AppendListItem(ctrl_id, IMAGE_SECMENU_ICON_SET_DISPLAY, TXT_HELLO_TEST_LIST3, STXT_SOFTKEY_ANSWER_MK, STXT_RETURN);
        break;
...
enum
{
    ...
    GUIITEM_STATE_LEFT_FOCUS            = 0x00004000, //item左键focus
    GUIITEM_STATE_RIGHT_FOCUS           = 0x00008000, //item右键focus    
    GUIITEM_STATE_HAS_AVATAR_MASK       = 0x00010000, //头像带有蒙版图片
    GUIITEM_STATE_HAS_INITIAL_MASK      = 0x00020000, //头像带有蒙版图片
    GUIITEM_STATE_HAS_INITIAL_CHECKBOX  = 0x00040000, //item支持头像check box，如Choice Picker for avatars
    GUIITEM_STATE_HAS_AVATAR_CHECKBOX   = 0x00080000, //item支持头像check box，如Choice Picker for avatars
    GUIITEM_STATE_HAS_TOGGLE_MARK       = 0x00100000, //item支持toggle标记状态
    GUIITEM_STATE_HAS_SIM_SELECTION     = 0x00200000, //item支持sim selection
	GUIITEM_STATE_HAS_ACCESSORY_MASK    = 0x00400000, //item支持accessory image
...
typedef struct
{
	...
	MMI_TEXT_ID_T			   lfocus_softkey_id[GUISOFTKEY_BUTTON_NUM];   /*!< 左focus状态下软键对应的文字资源id*/
	MMI_TEXT_ID_T			   rfocus_softkey_id[GUISOFTKEY_BUTTON_NUM];   /*!< 右focus状态下软键对应的文字资源id*/
}GUILIST_ITEM_DATA_T;
...
{
    uint32                      style_id;
    uint32                      normal_style_id;/*<NormalFocus类型>*/
    uint32                      left_style_id;/*<LeftFocus类型>*/
    uint32                      right_style_id;/*<RightFocus类型>*/
...
PUBLIC void CTRLLIST_ChangeLeftQuickSoftkey(
                                        ...
                                        MMI_TEXT_ID_T       lfocus_mid_sk_id,//当处于left focus状态下，中键显示的文本 
...
PUBLIC void CTRLLIST_SetBgTransparent(
                                        MMI_CTRL_ID_T   ctrl_id,
                                        BOOLEAN         is_transpant, //是否显示背景透明
                                        uint8           alpha          //list背景透明度
                                        );
...
/*****************************************************************************/
//  Description : force set item data,see CTRLLIST_SetItemData
//  Global resource dependence :
//  Author:
//  Note:
/*****************************************************************************/
