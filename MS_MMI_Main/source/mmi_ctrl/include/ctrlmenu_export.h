...
/*---------------------------------------------------------------------------
                          TYPE AND STRUCT                                  
--------------------------------------------------------------------------*/
typedef uint32 GUIMENU_ITEM_STATE_T;              //菜单项状态
#define GUIMENU_ITEM_STATE_TOGGLE      0x00000001 //菜单项是否包含toggle
#define GUIMENU_ITEM_STATE_TOGGLE_ON   0x00000002 //toggle on/off
...
typedef enum
{
    ...
    GUIMENU_STYLE_OPTIONS     = 14, /*!< OptionsMenu */
     GUIMENU_STYLE_FULLSCREEN_POPMENU     = 15, /*!< full screen pop menu */
    GUIMENU_STYLE_MAX
} GUIMENU_STYLE_E;
...
typedef struct
{
    ...
    uint32                  node_id;        /*!< 菜单节点ID @note is_static为FALSE时有效 */
    MMI_MENU_GROUP_ID_T     group_id;       /*!< 菜单组ID @note is_static为TRUE时有效 */
    MMI_STRING_T            *text_ptr;      /*!< 指向text的指针 @note 如果为PNULL，表示忽略此项，如果指向的文本为空，表示清除text，如果指向有效文本，表示设置text */
} GUIMENU_TEXT_INFO_T;
...
typedef struct
{
    ...
    uint32                      button_id[GUISOFTKEY_BUTTON_NUM];   /*!< softkey的提示信息 */ /*!< option menu softkey 可为image/text */
...
typedef struct
{
    ...
    MMI_MENU_GROUP_ID_T     group_id;       /*!< 菜单组ID @note is_static为TRUE时有效 */
    uint32                  selected_index; /*!< selected index @note is_static为TRUE&&style为GUIMENU_STYLE_POPUP_RADIO时有效*/
