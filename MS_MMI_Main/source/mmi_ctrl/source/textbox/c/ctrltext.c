...
/*****************************************************************************/
//  Description : get tag display top
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL int32 GetTagDispTop(
                              GUITEXT_TAG_T     *tag_ptr,
                              CTRLTEXT_OBJ_T    *text_ctrl_ptr
                              );

/*****************************************************************************/
//  Description : get tag display bottom
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL int32 GetTagDispBottom(
                                 GUITEXT_TAG_T      *tag_ptr,
                                 CTRLTEXT_OBJ_T     *text_ctrl_ptr
                                 );
...
/*****************************************************************************/
// 	Description : display text dialog string
//	Global resource dependence : 
//  Author:
//	Note:
/*****************************************************************************/
LOCAL void DisplayTextDialogString(
                             CTRLTEXT_OBJ_T     *text_ctrl_ptr  //in:
                             );
                            ...
                            )
{
    ...
    text_ctrl_ptr->is_font_height_changed = TRUE;
    //get text theme
    MMITHEME_GetTextTheme(&text_ctrl_ptr->theme);

    if (TRUE == text_ctrl_ptr->is_text_dialog)
    {
        text_ctrl_ptr->theme.font.font = SONG_FONT_20;
    }
                                 ...
                                 )
{
    ...
    case MSG_CTL_LOSE_FOCUS:
        ...
        if (MSG_CTL_LOSE_ACTIVE == msg_id)
        {
			...
			if(text_ctrl_ptr->cur_tag_ptr != PNULL)
			{
			       text_ctrl_ptr->cur_tag_ptr = text_ctrl_ptr->first_tag_ptr;
				text_ctrl_ptr->cur_tag_ptr->is_highlight = FALSE;
			}
            //notify parent control
                            ...
                            )
{
    ...
    //get font height
	if(text_ctrl_ptr->is_font_height_changed)
	{
		//font_height = GUI_GetFontHeight(text_ctrl_ptr->theme.font.font,UNICODE_HANZI);
		text_ctrl_ptr->font_height = GUI_GetStringHeightEx(text_ctrl_ptr->theme.font.font,text_ctrl_ptr->str_ptr,text_ctrl_ptr->str_len);
		text_ctrl_ptr->is_font_height_changed = FALSE;
    }
    font_height = text_ctrl_ptr->font_height;
    ...
    if (!GUI_IsRectEmpty(text_rect))
    {
        ...
        if (TRUE == text_ctrl_ptr->is_text_dialog)
		{
        	page_height = (uint16)(text_rect.bottom - text_rect.top + 1);
		}
		else
		{
                                ...
                                )
{
    ...
    if (PNULL != text_ctrl_ptr)
    {
        ...
        if (TRUE == text_ctrl_ptr->is_text_dialog)
        {        
            DisplayTextDialogString(text_ctrl_ptr);
        }
        else           
        {
...
/*****************************************************************************/
//  Description : Set is_text_dialog flag
//  Global resource dependence : 
//  Author: Jassmine
//  Note:
/*****************************************************************************/
PUBLIC void GUITEXT_SetIsTextDialog(
                                MMI_CTRL_ID_T      ctrl_id,     //in:
                                BOOLEAN            is_text_dialog
                                )
{
    ...
    if (PNULL != text_ctrl_ptr)
    {
        ...
        text_ctrl_ptr->is_text_dialog = is_text_dialog;
    }
}
...
//	Note:
/*****************************************************************************/
PUBLIC void GUITEXT_SetTextDialogTitle(
                                MMI_CTRL_ID_T      ctrl_id,     //in:
                                ...
                                )
{
    ...
    text_ctrl_ptr->dialog_title_str = title_str_ptr;  
}
                             ...
                             )
{
    ...
    CTRLBASE_OBJ_T          *base_ctrl_ptr = (CTRLBASE_OBJ_T*)text_ctrl_ptr;
    BOOLEAN                 is_first_disp_line = TRUE;
        ...
        (0 < text_ctrl_ptr->str_len))
    {
        for (i=text_ctrl_ptr->top_line_index; i<=text_ctrl_ptr->bottom_line_index; i++)
        {
            ...
            {
                /*
                if (text_rect.top < cross_rect.top || text_rect.bottom > cross_rect.bottom)
                    continue;
                */ 
                
                //get line string len
                text_str.wstr_len = GUITEXT_GetLineStrLen(i,text_ctrl_ptr);
                if(is_first_disp_line)
                {
                      text_ctrl_ptr->cur_top_line_index = i;
                      is_first_disp_line = FALSE;
                }
                text_ctrl_ptr->cur_bottom_line_index = i;

                //Draw text dialog title 
				if (PNULL != text_ctrl_ptr->dialog_title_str.wstr_ptr)        
                {
                    if (TRUE == IsDisplayFirstLine(text_ctrl_ptr))
                    {
                        if (0 == i)                
                        {
                            //text rect 
                            text_rect.top += TEXT_DIALOG_TITLE_TOP_OFFSET;  //6
                            text_rect.bottom =  text_rect.top + SONG_FONT_24;
                            ...
                            if (GUI_IntersectRect(&cross_rect,base_ctrl_ptr->display_rect,text_rect))
                            {
                                ...
                                GUISTR_DrawTextToLCDInRect((const GUI_LCD_DEV_INFO*)&base_ctrl_ptr->lcd_dev_info,
                                    ...
                                    &text_ctrl_ptr->dialog_title_str,
                                    &str_style,
                                    str_state,
                ...
                if (0 < text_str.wstr_len)
                {
                    ...
                    if (PNULL != text_ctrl_ptr->dialog_title_str.wstr_ptr)
                    {
					    text_rect.top -= TEXT_DIALOG_RECT_OFFSET_WITH_TITLE;      //6
                        text_rect.bottom -= TEXT_DIALOG_RECT_OFFSET_WITH_TITLE;   //6
                    }      
                    //start from index 0 
					else 
					{
					    text_rect.top += TEXT_DIALOG_RECT_OFFSET_WITHOUT_TITLE;    //2 //2
                        text_rect.bottom += TEXT_DIALOG_RECT_OFFSET_WITHOUT_TITLE; //2//2
					}
...
#if 0 
/*****************************************************************************/
// 	Description : display text dialog title
//	Global resource dependence : 
//  Author:
//	Note:
/*****************************************************************************/
LOCAL void DisplayTextDialogTitle(
                             CTRLTEXT_OBJ_T     *text_ctrl_ptr  //in:
                             )
{
    uint16                  i = 0;
...
//        for (i=text_ctrl_ptr->top_line_index; i<=text_ctrl_ptr->bottom_line_index; i++)
        {
            ...
            if (GUI_IntersectRect(&cross_rect,base_ctrl_ptr->display_rect,text_rect))
            {
               ...
               // if (0 < text_str.wstr_len)
				if ( 0 != text_ctrl_ptr->dialog_title_str.wstr_len)
                {
                    //get display string
                    text_str.wstr_ptr = &text_ctrl_ptr->str_ptr[text_ctrl_ptr->line_info_ptr[i].start_pos];
                    ...
                    text_rect.top = base_ctrl_ptr->rect.top + 12;
                              ...
                              )
{
    ...
    if (0 < text_ctrl_ptr->tag_num)
    {
        ...
        //get tag top
        cur_tag_top = GetTagDispTop(cur_tag_ptr,text_ctrl_ptr);
        cur_tag_bottom = GetTagDispBottom(cur_tag_ptr,text_ctrl_ptr);
        //set next tag
        ...
        {
            next_tag_top    = text_ctrl_ptr->base_ctrl.rect.top;
            next_tag_bottom = next_tag_top + GUITEXT_GetLineHeight(text_ctrl_ptr);
			IsTagHighlight = FALSE;
        }
		*/
        if (PNULL != next_tag_ptr)
        {
            next_tag_top    = GetTagDispTop(next_tag_ptr,text_ctrl_ptr);
            next_tag_bottom = GetTagDispBottom(next_tag_ptr,text_ctrl_ptr);
        }

        //current tag top is not display  	
        if ((cur_tag_top < text_ctrl_ptr->base_ctrl.display_rect.top ))
        {
            //get display top by scroll
			if(cur_tag_bottom > text_ctrl_ptr->base_ctrl.display_rect.top)
			{
				//cur tag display part
				ScrollTextLines(TRUE,&display_top,text_ctrl_ptr);
			}            
            //not change tag
            next_tag_ptr = cur_tag_ptr;
			cur_tag_ptr->is_highlight = FALSE;
            IsTagHighlight = FALSE;
			text_ctrl_ptr->highlight.start_pos = text_ctrl_ptr->highlight.end_pos = 0;
        }
		else if(cur_tag_bottom <= text_ctrl_ptr->base_ctrl.display_rect.bottom
			&& !cur_tag_ptr->is_highlight)
		{
			//current tag need to highlighted
			text_ctrl_ptr->highlight.start_pos =  text_ctrl_ptr->cur_tag_ptr->start_pos;
			text_ctrl_ptr->highlight.end_pos   = (uint16)(text_ctrl_ptr->cur_tag_ptr->start_pos + text_ctrl_ptr->cur_tag_ptr->len); 
			next_tag_ptr = cur_tag_ptr;
			cur_tag_ptr->is_highlight = TRUE;
			IsTagHighlight = TRUE;
		}
        else
        {
            //change tag
            //next tag is not display
            if ((next_tag_top < text_ctrl_ptr->base_ctrl.display_rect.top))
            {
                //get display height
				IsTagHighlight = FALSE;
				if(next_tag_bottom > text_ctrl_ptr->base_ctrl.display_rect.top)
				{
					display_height = GUITEXT_GetDispHeight(text_ctrl_ptr);
					...
					{
						//set display top
						display_top = GetTagRelativeBottom(next_tag_ptr,text_ctrl_ptr) - display_height + 1;
					}
					else
					{
						//get display top by next tag top
						display_top = GetTagRelativeTop(next_tag_ptr,text_ctrl_ptr);;
					}
			...
			else if(next_tag_bottom > 0 
				&& next_tag_bottom <= text_ctrl_ptr->base_ctrl.display_rect.bottom)
			{
				//next tag need to highlighted
				//text_ctrl_ptr->highlight.start_pos =  text_ctrl_ptr->cur_tag_ptr->start_pos;
				//text_ctrl_ptr->highlight.end_pos   = (uint16)(text_ctrl_ptr->cur_tag_ptr->start_pos + text_ctrl_ptr->cur_tag_ptr->len); 
				IsTagHighlight = TRUE;
			}
        ...
        if(IsTagHighlight)
		{
			...
			text_ctrl_ptr->highlight.end_pos   = (uint16)(text_ctrl_ptr->cur_tag_ptr->start_pos + text_ctrl_ptr->cur_tag_ptr->len);			
			result = TRUE;
			text_ctrl_ptr->cur_tag_ptr->is_highlight = TRUE;
		}
		else if (PNULL == next_tag_ptr)
		{
			GUITEXT_DisplayText(TRUE,text_ctrl_ptr);
		}
    }
                                ...
                                )
{
	...
	int32           next_tag_top = 0;
    int32           next_tag_bottom = 0;
	int32           next_tag_relative_bottom = 0;
    int32           display_max_top = 0;
    GUITEXT_TAG_T   *cur_tag_ptr = PNULL;
    GUITEXT_TAG_T   *next_tag_ptr = PNULL;
    ...
    if (0 < text_ctrl_ptr->tag_num)
    {
        ...
        if(PNULL == next_tag_ptr)
        {
            //next_tag_top    = text_ctrl_ptr->base_ctrl.rect.bottom - GUITEXT_GetLineHeight(text_ctrl_ptr);
            //next_tag_bottom = text_ctrl_ptr->base_ctrl.rect.bottom;
			//IsTagHighlight = FALSE;
			//first_tag_ptr
			text_ctrl_ptr->highlight.start_pos= text_ctrl_ptr->highlight.end_pos = 0;
        }
        ...
        //current tag bottom is not display 
        if(cur_tag_top >= text_ctrl_ptr->base_ctrl.display_rect.bottom ||
		   cur_tag_bottom > text_ctrl_ptr->base_ctrl.display_rect.bottom)
        {
            //get display top by scroll
            if(cur_tag_top <text_ctrl_ptr->base_ctrl.display_rect.top)
            {
		...
		else if(cur_tag_bottom >= text_ctrl_ptr->base_ctrl.display_rect.top
			&& !cur_tag_ptr->is_highlight)
		{
			//current tag need to highted
			text_ctrl_ptr->highlight.start_pos =  text_ctrl_ptr->cur_tag_ptr->start_pos;
			text_ctrl_ptr->highlight.end_pos   = (uint16)(text_ctrl_ptr->cur_tag_ptr->start_pos + text_ctrl_ptr->cur_tag_ptr->len); 
			next_tag_ptr = cur_tag_ptr;
			cur_tag_ptr->is_highlight = TRUE;
        ...
        {
			//next tag to be hightlighted.
            if(next_tag_top >= text_ctrl_ptr->base_ctrl.display_rect.bottom ||
               next_tag_bottom > text_ctrl_ptr->base_ctrl.display_rect.bottom)

            {
				//next tag not hightlighted
                //get display height
                 if(next_tag_top <text_ctrl_ptr->base_ctrl.display_rect.top)
                    {
                        display_height = GUITEXT_GetDispHeight(text_ctrl_ptr);
                        ...
                        {
							next_tag_relative_bottom = GetTagRelativeBottom(next_tag_ptr,text_ctrl_ptr);
                            //get display top by child bottom
                            if (next_tag_relative_bottom < display_height)
                            {
                                display_top = GetTagRelativeTop(next_tag_ptr,text_ctrl_ptr);
                            }
                            else
                            {
                                display_top = next_tag_relative_bottom - display_height + 1;
                            }
                        }
			...
			else if(next_tag_bottom > 0 && next_tag_bottom <= text_ctrl_ptr->base_ctrl.display_rect.bottom)
			{
				text_ctrl_ptr->highlight.start_pos =  text_ctrl_ptr->cur_tag_ptr->start_pos;
			    text_ctrl_ptr->highlight.end_pos   = (uint16)(text_ctrl_ptr->cur_tag_ptr->start_pos + text_ctrl_ptr->cur_tag_ptr->len); 
				IsTagHighlight = TRUE;
                              ...
                              )
{
    ...
    if (PNULL != tag_ptr)
    {
        ...
        //get icon relative rect
       disp_rect = GUITEXT_GetLineRect(line_index, text_ctrl_ptr);          
    }

    return (disp_rect.top);
}
                     ...
                     )
{
    ...
    if (PNULL == text_ctrl_ptr->first_tag_ptr)
    {
        ...
        //set highlight
        text_ctrl_ptr->highlight.start_pos = new_tag_ptr->start_pos;
        text_ctrl_ptr->highlight.end_pos   = (uint16)(new_tag_ptr->start_pos + new_tag_ptr->len);
    }
    else
    {
                           ...
                           )
{
    ...
    if (TRUE == text_ctrl_ptr->is_text_dialog && PNULL != text_ctrl_ptr->dialog_title_str.wstr_ptr)
    {
        line_index = 2;        
    }
    ...
    if (!GUI_IsRectEmpty(text_rect))
    {
        ...
        //start_pos = text_ctrl_ptr->line_info_ptr[2].start_pos;
                                     ...
                                     )
{
    ...
    if (PNULL != text_ctrl_ptr)
    {
		...
		if(max_pos > min_pos)
		{
			...
			if ((min_line >= text_ctrl_ptr->cur_top_line_index  && min_line <= text_ctrl_ptr->cur_bottom_line_index)
				|| (max_line >= text_ctrl_ptr->cur_top_line_index  && max_line <= text_ctrl_ptr->cur_bottom_line_index)
			  )
			{
				if (PNULL != highlight_min_ptr)
				{
					*highlight_min_ptr = min_line;
				}

				if (PNULL != highlight_max_ptr)
				{
					*highlight_max_ptr = max_line;
				}
				result = TRUE;
			}
		}
