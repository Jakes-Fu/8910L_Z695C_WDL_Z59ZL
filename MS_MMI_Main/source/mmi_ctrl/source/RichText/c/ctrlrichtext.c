...
//  CTRLBASE_VTBL_T     *parent_vtbl_ptr = PNULL;
    ...
    case MSG_APP_WEB:
		if( (richtext_ctrl_ptr->first_item_ptr != PNULL) && (richtext_ctrl_ptr->focus_item_ptr != PNULL) )
		{
			focus_index = richtext_ctrl_ptr->focus_item_ptr->focus_index;
			if(richtext_ctrl_ptr->focus_item_ptr->focus_elem[focus_index].tag_type == GUIRICHTEXT_TAG_URL)
			{
				richtext_ctrl_ptr->focus_item_ptr->focus_elem[focus_index].is_visited = TRUE;
			}
		}
    ...
    case MSG_TP_PRESS_DOWN:
        ...
        if (GUI_PointIsInRect(point, richtext_ctrl_ptr->cont_rect))
        {
            ...
            {
                GUICTRL_PostNotify(richtext_ctrl_ptr->handle, MSG_NOTIFY_RICHTEXT_SWITCHFOCUS_BY_TP);
            }
            else if (is_tp_on_foucs)
            {
                recode = GUICTRL_PostNotify(richtext_ctrl_ptr->handle, MSG_NOTIFY_PENOK);
            }
        }
#endif
        //slide...
...
//     ((CTRLBASE_OBJ_T*)richtext_ptr)->both_rect = init_ptr->both_rect;
	...
	richtext_ptr->visited_font_color = richtext_style.visited_font_color;
...
#else
                    region_style[region_count].state |= GUISTR_STATE_BACKGROUND | GUISTR_STATE_UNDERLINE;
#endif//syy modify 2010.12.1 -end
                }
                else
                {
                    region_style[region_count].state |= GUISTR_STATE_UNDERLINE;
					if(item_ptr->focus_elem[focus_index].is_visited)
					{
						region_style[region_count].state |= GUISTR_STATE_SPEC_FONTCOLOR;
						str_style.line_color = region_style[region_count].font_color = richtext_ptr->visited_font_color;
					}
                }   
...
)
{
    ...
    SCI_MEMSET(src_str_ptr, 0x00, (uint16)src_len * sizeof(wchar));
