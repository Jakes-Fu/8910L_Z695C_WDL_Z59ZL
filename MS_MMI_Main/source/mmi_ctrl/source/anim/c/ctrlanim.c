...
#define GUIANIM_FILE_SUFFIX_LEN            10
/**--------------------------------------------------------------------------*
 **                         STATIC DEFINITION                                *
 **--------------------------------------------------------------------------*/
...
)
{
    ...
    anim_param_ptr->is_spinner     = anim_init_ptr->is_spinner;
}

/*****************************************************************************/
//  Description : spinner anim construct
//  Global resource dependence :
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN SpinnerAnimCtrlConstruct (
    CTRLBASE_OBJ_T           *base_ctrl_ptr,
    CTRLANIM_INIT_PARAM_T   *anim_param_ptr
)
{
    ...
    {
        anim_ctrl_ptr->display_info.is_bg_buf = TRUE;
        anim_ctrl_ptr->display_info.bg.bg_type = anim_ctrl_ptr->theme.spinner_bg.bg_type;
        anim_ctrl_ptr->display_info.bg.color = anim_ctrl_ptr->theme.spinner_bg.color;
        anim_ctrl_ptr->display_info.align_style = GUIANIM_ALIGN_NONE;
        anim_ctrl_ptr->display_info.is_play_once = FALSE;

        anim_ctrl_ptr->data_info.anim_id = anim_ctrl_ptr->theme.spinner_img_id;
...
)
{
    ...
    if ((PNULL != base_ctrl_ptr) && (PNULL != anim_param_ptr))
    {
        ...
        anim_ctrl_ptr->is_spinner   = anim_param_ptr->is_spinner;

        if(TRUE == anim_ctrl_ptr->is_spinner)
        {
            SpinnerAnimCtrlConstruct(base_ctrl_ptr, anim_param_ptr);
        }
...
)
{
    ...
    case MSG_CTL_PAINT:
		...
		{
			GUICTRL_PostNotifyEx(base_ctrl_ptr->handle,MSG_NOTIFY_ANIM_UPDATE_END,PNULL,0);
		}
        break;
    ...
    case MSG_CTL_LOSE_FOCUS:
        HandleAnimLoseFocus(anim_ctrl_ptr, MSG_CTL_LOSE_FOCUS);
        break;
    ...
    case MSG_CTL_NOT_IN_DISPLAYREC:
        HandleAnimLoseFocus(anim_ctrl_ptr, MSG_CTL_NOT_IN_DISPLAYREC);
        break;
...
LOCAL uint32 AnimCtrlIsJPG (
    const wchar      *suffix_name_ptr,
    uint16      suffix_name_len
)
{
    uint32 i = 0;
    uint8 gb_suffix_name[GUIANIM_FILE_SUFFIX_LEN + 1] = {0};
    uint16 gb_suffix_name_len = 0;
    const uint8* str_jpg = "jpg";
    ...
    MMI_WSTRNTOSTR (gb_suffix_name, GUIANIM_FILE_SUFFIX_LEN, suffix_name_ptr, suffix_name_len, suffix_name_len);
    ...
    if (0 == MMIAPICOM_Stricmp ( (uint8 *) gb_suffix_name, (uint8 *) str_jpg))
    {
        return TRUE;
    }
    ...
    if (GUIANIM_GetAnimRect(anim_ctrl_ptr,&anim_rect))
    {
        ...
        else
        {
            if ((GUIANIM_RESULT_SUCC != result) && 
                ...
                (GUIANIM_RESULT_BUSY !=result))//hch
            {
                SCI_TRACE_LOW("GUIANIM:DecodeImgBufOrFile: result=%d", result);
                //free target buffer
                FreeTargetBuffer(anim_ctrl_ptr);
                                         ...
                                         )
{
    ...
    if (PNULL == anim_ctrl_ptr->target_buffer)
    {
        ...
        else
        {
            ...
            SCI_TRACE_LOW("GUIANIM:SetDecEncTarget:no target buffer!");
        }
    }
    else
    {
                                   ...
                                   )
{
    ...
    if ((full_path_len + GUIANIM_MINIATURE_PATH_LEN + suffix_len) < (GUIANIM_MINIATURE_FULL_PATH_LEN + 1))
    {
        ...
        //init miniature full path
        SCI_MEMSET(miniature_full_path_ptr,0,(GUIANIM_MINIATURE_FULL_PATH_LEN + 1)*sizeof(wchar));
        ...
        if (SFS_NO_ERROR == SFS_GetFileSize(file_handle,&miniature_size))
        {
            ...
            if(SFS_NO_ERROR == SFS_SetFilePointer(file_handle,(__int64)miniature_offset,SFS_SEEK_BEGIN))
            {
                if(SFS_ERROR_NONE == SFS_ReadFile(file_handle,size_time,(GUIANIM_MINIATURE_SIZE_TIME_LEN+2),&size_time_len,PNULL))
                {
                    ...
                    {
                        SCI_TRACE_LOW("GUIANIM:IsTwoFileCorresponding: get miniature file size_time_len %d is error!");
                    }
                }
            }
        }
                                ...
                                )
{
   ...
   SCI_TRACE_LOW("GUIANIM:HandleAnimGetDataCnf:decode/encode result %d", result);
    switch (result)
    {
                               ...
                               )
{
    ...
    //lose focus
    if ((anim_ctrl_ptr->display_info.is_free_buf_lose_focus && msg_id == MSG_CTL_LOSE_FOCUS) 
        || (anim_ctrl_ptr->display_info.is_free_buf_not_in_displayrec && msg_id == MSG_CTL_NOT_IN_DISPLAYREC))
    {
...
/***************************************************************************//*!
//  Description : 擦除动画控件默认图片
//  Global resource dependence : 
//  Author: 
//  Note:
*******************************************************************************/
PUBLIC void CTRLANIM_ClearAnimDefaultIcon(MMI_HANDLE_T  ctrl_handle)
{    
    CTRLANIM_OBJ_T  *anim_ctrl_ptr = PNULL;
    ...
    if ((PNULL != anim_ctrl_ptr) &&
        (GUIANIM_TypeOf((CTRLBASE_OBJ_T *)anim_ctrl_ptr)))
    {
        anim_ctrl_ptr->theme.is_clear_anim_bg = TRUE;
        anim_ctrl_ptr->theme.fill_color = 0XFFFF;
    }
...
PUBLIC BOOLEAN CTRLANIM_SetMiniWidthAndHeightByPtr(
                                           void             *ctrl_ptr,
                                           uint16   width,
                                           uint16   height
                                           )
{
    ...
    {
        //set default image
        anim_ctrl_ptr->miniature_width = width;
        anim_ctrl_ptr->miniature_height = height;
        result = TRUE;
    }
...
//  Note:
/*****************************************************************************/
PUBLIC void CTRLANIM_SetPeriodByPtr(
                                  uint32    min_period, //in
                                  void      *ctrl_ptr
                                  )
{
...
/*****************************************************************************/
//  Description : delete the miniature file {only jpeg have Mniature File. please see GetMiniaturFileName}
//  Global resource dependence : 
//  Author:jassmine.meng
//  Note: 
/*****************************************************************************/
                                           ...
                                           )
{
    ...
    uint16 suffix_len = GUIANIM_FILE_SUFFIX_LEN;
    wchar list_suffix_name[GUIANIM_FILE_SUFFIX_LEN+1] = {0};

    
    if (MMIAPIFMM_SplitFileName (full_path_ptr, MMIAPICOM_Wstrlen(full_path_ptr), PNULL, PNULL, list_suffix_name, &suffix_len))
     {
        if (!AnimCtrlIsJPG(list_suffix_name, suffix_len)) 
        {
            SCI_TRACE_LOW("This pictue   have not Miniature file");
            return TRUE;
        }
...
#if 0 //sycn with  GetMiniaturFileName.
    //get miniature name
    SetMiniatureFullPath(full_path_ptr,
        full_path_len,
        miniature_height,
...
)
{
    ...
    if (PNULL != anim_ctrl_ptr)
    {
        GUIANIM_GetDisplayBitmap(anim_ctrl_ptr, bitmap_ptr);
    }
