 ...
 **                         MACRO DEFINITION                                 *
 **--------------------------------------------------------------------------*/
#define GUISOFTKEY_EDGE                 0
#define GUISOFTKEY_GRAY_TEXT_ALPHA      89 //灰化时字体透明值
#define GUISOFTKEY_GRAY_IMG_ALPHA       189//图片置灰
#define MMI_SOFTKEY_BIG_SPACE_V                 7//上下间隔
#define MMI_SOFTKEY_SMALL_SPACE_V                 4//上下间隔
...
/*****************************************************************************/
//  Description :判断输入类型的组合
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL GUI_SOFTKEY_TYPE_MS GetSoftkeyMSType(
                          CTRLSOFTKEY_OBJ_T    *softkey_ptr
                          );
/**--------------------------------------------------------------------------*
 **                         GLOBAL DEFINITION                                *
                          ...
                          )
{
    ...
    GUISK_DATA_TYPE_E   left_data_type = GUISK_DATA_MAX;
    GUISK_DATA_TYPE_E   right_data_type = GUISK_DATA_MAX;
    uint32 lsk_id = 0;
    BOOLEAN is_lsk_empty = FALSE;
    BOOLEAN is_rsk_empty = FALSE;
    
    if (PNULL == softkey_ptr)
    {
        return GUI_SOFTKEY_IMAGE_TWO;
    }
    left_data_type = softkey_ptr->button_arr[LEFT_BUTTON].content.data_type;

    if(GUISK_DATA_TEXT_ID == left_data_type)
    {
        lsk_id = softkey_ptr->button_arr[LEFT_BUTTON].content.data_u.text_id;
        is_lsk_empty = MMI_IsTextNull((MMI_TEXT_ID_T)lsk_id);
    }
    else if(GUISK_DATA_IMAGE_ID == left_data_type)
        {
            lsk_id = softkey_ptr->button_arr[LEFT_BUTTON].content.data_u.image_id;
            is_lsk_empty = MMI_IsImageNull((MMI_IMAGE_ID_T)lsk_id);
        }

    right_data_type = softkey_ptr->button_arr[RIGHT_BUTTON].content.data_type;
    if(GUISK_DATA_TEXT_ID == right_data_type)
    {
        lsk_id = softkey_ptr->button_arr[RIGHT_BUTTON].content.data_u.text_id;
        is_rsk_empty = MMI_IsTextNull((MMI_TEXT_ID_T)lsk_id);
    }
    else if(GUISK_DATA_IMAGE_ID == right_data_type)
        {
            lsk_id = softkey_ptr->button_arr[RIGHT_BUTTON].content.data_u.image_id;
            is_rsk_empty = MMI_IsImageNull((MMI_IMAGE_ID_T)lsk_id);
        }
    
    if(((GUISK_DATA_TEXT_ID == left_data_type)||(GUISK_DATA_TEXT_BUFFER == left_data_type))
        &&(GUISK_DATA_IMAGE_ID!= right_data_type))
    {
        return GUI_SOFTKEY_TEXT_ALL;
    }
    else if(((GUISK_DATA_TEXT_ID == right_data_type)||(GUISK_DATA_TEXT_BUFFER == right_data_type))
            &&(GUISK_DATA_IMAGE_ID!= left_data_type))
        {
            return GUI_SOFTKEY_TEXT_ALL;
        }
    //左右类型不一致，且数据不为0
    if((left_data_type != right_data_type)&&(!is_lsk_empty)&&(!is_rsk_empty))
    {
        return GUI_SOFTKEY_IMAGE_TEXT_ONE;
    }
    else
    {
        return GUI_SOFTKEY_IMAGE_TWO;
                          ...
                          )
{
    ...
    space_w = MMITHEME_GetSoftkeyButtonWidth();
        ...
        softkey_ptr->button_arr[LEFT_BUTTON].rect = GUI_CreateRect(
                (uint16)(ctrl_ptr->rect.left + space_h),
                (uint16)(ctrl_ptr->rect.top),
                (uint16)(ctrl_ptr->rect.left + space_h + space_w - 1),
                (uint16)(ctrl_ptr->rect.bottom)
                );
        // 中
        softkey_ptr->button_arr[MIDDLE_BUTTON].rect = GUI_CreateRect(
                (uint16)(ctrl_ptr->rect.left + space_h*2 + space_w -1),
                (uint16)(ctrl_ptr->rect.top),
                (uint16)(ctrl_ptr->rect.right - space_h*2 -space_w + 1),
                (uint16)(ctrl_ptr->rect.bottom)
                );
        // 右
        softkey_ptr->button_arr[RIGHT_BUTTON].rect = GUI_CreateRect(
                    (uint16)(ctrl_ptr->rect.right - space_h - space_w + 1),
                    (uint16)(ctrl_ptr->rect.top),
                    (uint16)(ctrl_ptr->rect.right - space_h),
                    (uint16)(ctrl_ptr->rect.bottom)
                    );
}
    ...
    case GUISK_DATA_TEXT_BUFFER:
        ...
        else
        {
            piexl_num = GUI_CalculateStringPiexlNum(
                ...
                GetTextFont(softkey_ptr , button_num),       //[IN] String的字体
                0  //[IN] 字间距
                );
        }
        break;
                             ...
                             )
{
    ...
    else if (softkey_ptr->button_arr[button_num].font)
    {
	font = softkey_ptr->button_arr[button_num].font;	
    }
    else
    {
...
{
    uint16            total_width = 0 , mid_softkey_width;
    GUI_RECT_T        rect = {0};
                          ...
                          )
{
    ...
    if (button_num == 0)
    {
        align_num = ALIGN_LVMIDDLE;
    }
    else if (button_num == 1)
    {
        align_num = ALIGN_HVMIDDLE;
    }
    else
    {
        align_num = ALIGN_RVMIDDLE;
    }
    // 设置文本风格
    str_style.align = align_num;
    str_style.font = button_ptr->font;
    ...
    {
        str_state = GUISTR_STATE_ALIGN | GUISTR_STATE_SINGLE_LINE| GUISTR_STATE_GRADUAL_VER | GUISTR_STATE_GRADUAL_HOR;
        str_style.begin_alpha = str_style.end_alpha = GUISOFTKEY_GRAY_TEXT_ALPHA;
    } 
    ...
    if(PNULL != text.wstr_ptr && 0 != text.wstr_len) // 显示文字
    {
        ...
        if(button_num == 1)
        {
            str_style.font = text.wstr_len > 2 ? SONG_FONT_26 : SONG_FONT_34;
        }
        //灰化时字体35%不透明
        if(softkey_ptr->button_arr[button_num].is_gray)
        {
#ifndef UI_MULTILAYER_SUPPORT
            font_color_blending_without_multilayer(&str_style);
#endif
            GUISTR_DrawSingleLineGradualInRect(&ctrl_ptr->lcd_dev_info, 
                           ...
                           )
{
    ...
    else
    {
	    ...
	    if (MMITHEME_GetSoftkeyStyle(&softkey_style))
        {
            ...
            softkey_ptr->char_space = softkey_style.char_space;
            
            softkey_ptr->button_arr[LEFT_BUTTON].font           = softkey_style.small_font;
            ...
            softkey_ptr->button_arr[MIDDLE_BUTTON].font           = softkey_style.font;
            ...
            softkey_ptr->button_arr[RIGHT_BUTTON].font           = softkey_style.small_font;
                                        ...
                                        )
{
    ...
    case MSG_CTL_UPDATE_COLOR_THEME:
        //cur_index= MMITHEME_GetAccentId();
        softkey_ptr->bg_color = MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_DEFAULT);
        break;
...
/*****************************************************************************/
// Description : 设置每个单独button字体
// Global resource dependence : 
// Author:
// Note:
/*****************************************************************************/
PUBLIC void CTRLSOFTKEY_SetButtonfont(
                                MMI_HANDLE_T    win_handle,
                                MMI_CTRL_ID_T   ctrl_id,
                                GUISOFTKEY_INDEX button_num,
                                GUI_FONT_T      *font_ptr
                                )
{
    ...
    switch (button_num)
    {
        case LEFT_BUTTON:
        {
            if (PNULL != font_ptr)
            {
                softkey_ptr->button_arr[button_num].font= *font_ptr;
            }
        }
           break; 
        case MIDDLE_BUTTON:
                                    ...
                                    )
{
    ...
    if ((PNULL != left_id_ptr) && (GUISK_DATA_TEXT_ID == softkey_ptr->button_arr[0].content.data_type))
    {
        *left_id_ptr = softkey_ptr->button_arr[0].content.data_u.text_id;
    }
    
    if ((PNULL != middle_id_ptr) && (GUISK_DATA_TEXT_ID == softkey_ptr->button_arr[1].content.data_type))
    {
        *middle_id_ptr = softkey_ptr->button_arr[1].content.data_u.text_id;
    }

    if ((PNULL != right_id_ptr) && (GUISK_DATA_TEXT_ID == softkey_ptr->button_arr[2].content.data_type))
    {
...
// Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLSOFTKEY_GetId(
                                    MMI_HANDLE_T    win_handle,     //in:
                                    uint32          ctrl_id,        //in:
                                    uint32          *left_id_ptr,   //in:may PNULL
                                    uint32          *middle_id_ptr, //in:may PNULL
                                    uint32          *right_id_ptr   //in:may PNULL
                                    )
{
    CTRLSOFTKEY_OBJ_T	*softkey_ptr = PNULL;
    ...
    {
        if(GUISK_DATA_TEXT_ID == softkey_ptr->button_arr[0].content.data_type)
        {
            *left_id_ptr = (uint32)softkey_ptr->button_arr[0].content.data_u.text_id;
        }
        else
        {
            *left_id_ptr = (uint32)softkey_ptr->button_arr[0].content.data_u.image_id;
        }
    }
    
    if (PNULL != middle_id_ptr)
    {
        if(GUISK_DATA_TEXT_ID == softkey_ptr->button_arr[1].content.data_type)
        {
            *middle_id_ptr = (uint32)softkey_ptr->button_arr[1].content.data_u.text_id;
        }
        else
        {
            *middle_id_ptr = (uint32)softkey_ptr->button_arr[1].content.data_u.image_id;
        }
    }
    ...
    if (PNULL != right_id_ptr)
    {
        if(GUISK_DATA_TEXT_ID == softkey_ptr->button_arr[1].content.data_type)
        {
            *right_id_ptr = (uint32)softkey_ptr->button_arr[1].content.data_u.text_id;
        }
        else
        {
            *right_id_ptr = (uint32)softkey_ptr->button_arr[1].content.data_u.image_id;
        }
    }
