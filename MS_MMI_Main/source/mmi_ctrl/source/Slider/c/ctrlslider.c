...
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN SliderCtrlInitVtbl(
                                 CTRLSLIDER_VTBL_T        *slider_vtbl_ptr
                                 );
...
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN SliderCtrlConstruct (
...
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN SliderCtrlDestruct (
                                  CTRLBASE_OBJ_T        *base_ctrl_ptr
                                  );
...
//  Note:
/*****************************************************************************/
LOCAL void SliderCtrlPackInitParam (
                                    GUISLIDER_INIT_DATA_T     *slider_init_ptr,   //in
...
//  Note:
/*****************************************************************************/
LOCAL CTRLSLIDER_OBJ_T* GetSliderPtr (
...
//  Note:
/*****************************************************************************/
LOCAL void UpdateSliderRect(
                            CTRLSLIDER_OBJ_T *slider_ctrl_ptr
                            );
...
//  Note:
/*****************************************************************************/
LOCAL void SliderDrawChannelFg(
                               CTRLSLIDER_OBJ_T     *slider_ctrl_ptr
                               );
...
//  Note:
/*****************************************************************************/
LOCAL void SliderDrawChannelBg(
                               CTRLSLIDER_OBJ_T     *slider_ctrl_ptr
                               );
...
//	Note:
/*****************************************************************************/
LOCAL BOOLEAN SliderCtrlGetHeightByWidth (
...
//	Note:
/*****************************************************************************/
LOCAL BOOLEAN SliderCtrlSetRect(
                                CTRLBASE_OBJ_T* base_ctrl_ptr, 
...
//	Note:
/*****************************************************************************/
LOCAL BOOLEAN SliderCtrlSetDispRect(
                                    CTRLBASE_OBJ_T* base_ctrl_ptr, 
                                    const GUI_RECT_T* rect_ptr,
                                    BOOLEAN is_update
                                    );
...
/*****************************************************************************/
LOCAL BOOLEAN SliderSetCurBySize(
                                 CTRLSLIDER_OBJ_T *slider_ctrl_ptr,
                                 int16 size
                                 );
...
//  Note: return to head or to tail
/*****************************************************************************/
LOCAL void SliderCtrlAdjuestCur(
                                CTRLSLIDER_OBJ_T *slider_ctrl_ptr
                                );
...
PUBLIC TYPE_TAG_T SLIDER_GetType()
{
    ...
    if (0 == obj_type)
    {
        obj_type = TYPEMNG_Register (
            ...
            sizeof (CTRLSLIDER_VTBL_T),
            (VTBL_INIT_FUNC) SliderCtrlInitVtbl);
    }
...
{
    CTRLBASE_INIT_PARAM_T   *base_ctrl_param_ptr = (CTRLBASE_INIT_PARAM_T*) slider_param_ptr;
                                   ...
                                   )
{
    ...
    if (PNULL != slider_ctrl_ptr)
    {
        ...
        slider_ctrl_ptr->channel_bg_info.image_id = slider_style.channel_bg_image_id;
        slider_ctrl_ptr->channel_bg_info.disable_color = slider_style.channel_disable_bg_color;
        slider_ctrl_ptr->channel_bg_info.disable_image_id = slider_style.channel_disable_bg_image_id;
        ...
        slider_ctrl_ptr->channel_fg_info.image_id = slider_style.channel_fg_image_id;
        slider_ctrl_ptr->channel_fg_info.disable_color = slider_style.channel_disable_fg_color;
        slider_ctrl_ptr->channel_fg_info.disable_image_id = slider_style.channel_disable_fg_image_id;
        ...
        slider_ctrl_ptr->thumb_info.disable_color = slider_style.disable_thumb_color;
        slider_ctrl_ptr->thumb_info.disable_image_id = slider_style.disable_thumb_image_id;
                                 ...
                                 )
{
    ...
    if (PNULL != slider_ctrl_ptr)
    {
        ...
        // 纠正
        if (slider_ctrl_ptr->cur < slider_ctrl_ptr->min)
        {
            slider_ctrl_ptr->cur = slider_ctrl_ptr->min;
            result = TRUE;
        }
        ...
        {
            // post msg to win
            MMK_PostMsg(base_ctrl_ptr->win_handle, MSG_NOTIFY_SLIDER_CHANGED, PNULL, 0);
        }
        else
        {
            result = TRUE;
...
//  Note:
/*****************************************************************************/
LOCAL int16 SliderCtrlGetMoveSize(
								  CTRLSLIDER_OBJ_T *slider_ctrl_ptr
								  )
{
    int16 size = 0;
                                     ...
                                     )
{
    ...
    if (PNULL != slider_ctrl_ptr)
    {
        ...
        if (GUI_PointIsInRect(slider_ctrl_ptr->tp_down_point, slider_ctrl_ptr->thumb_rect)) // 滑块上
        {
            slider_ctrl_ptr->is_thumb_pressed = TRUE;
        }
        else if (GUI_PointIsInRect(slider_ctrl_ptr->tp_down_point, slider_ctrl_ptr->channel_rect) &&
				 ...
				 0 != slider_ctrl_ptr->channel_height) // 槽内，但不在滑块
        {
            ...
            if (slider_ctrl_ptr->is_vertical)
            {
                ...
                space_size = slider_ctrl_ptr->tp_down_point.y - slider_ctrl_ptr->channel_rect.top;

                slider_ctrl_ptr->tp_pos = slider_ctrl_ptr->min + space_size*range_size/channel_width;

                if ( slider_ctrl_ptr->tp_down_point.y < slider_ctrl_ptr->thumb_rect.top)
                {
            ...
            else
            {
                ...
                space_size = slider_ctrl_ptr->tp_down_point.x - slider_ctrl_ptr->channel_rect.left;
                slider_ctrl_ptr->tp_pos = slider_ctrl_ptr->min + space_size*range_size/channel_width; // 

                if ( slider_ctrl_ptr->tp_down_point.x < slider_ctrl_ptr->thumb_rect.left)
                {
                    slider_ctrl_ptr->is_slide_negative = TRUE;
                }
            }

			move_size = SliderCtrlGetMoveSize(slider_ctrl_ptr); // 计算移动的size

			if (!SliderSetCurBySize(slider_ctrl_ptr, move_size)) // 有变化，启动timer
			{
				slider_ctrl_ptr->slider_timer_id = MMK_CreateWinTimer(base_ctrl_ptr->handle, CTRL_SLIDER_TIMEOUT, FALSE);
			}
    ...
    if (PNULL != slider_ctrl_ptr)
    {
            ...
            && GUI_PointIsInRect(cur_point, base_ctrl_ptr->rect))
        {
            ...
            if (slider_ctrl_ptr->is_vertical)
            {
				...
				if (slider_ctrl_ptr->thumb_rect.top < slider_ctrl_ptr->channel_rect.top - slider_ctrl_ptr->thumb_height/2)
				{
					slider_ctrl_ptr->thumb_rect.top = slider_ctrl_ptr->channel_rect.top - slider_ctrl_ptr->thumb_height/2;
					slider_ctrl_ptr->thumb_rect.bottom = slider_ctrl_ptr->thumb_rect.top + slider_ctrl_ptr->thumb_height - 1;
				}

				if (slider_ctrl_ptr->thumb_rect.bottom > slider_ctrl_ptr->channel_rect.bottom + slider_ctrl_ptr->thumb_height/2)
				{
					slider_ctrl_ptr->thumb_rect.bottom = slider_ctrl_ptr->channel_rect.bottom + slider_ctrl_ptr->thumb_height/2;
					slider_ctrl_ptr->thumb_rect.top = slider_ctrl_ptr->thumb_rect.bottom - slider_ctrl_ptr->thumb_height + 1;
				}
            }
            else
            {
				...
				if (slider_ctrl_ptr->thumb_rect.left < slider_ctrl_ptr->channel_rect.left - slider_ctrl_ptr->thumb_width/2)
				{
					slider_ctrl_ptr->thumb_rect.left = slider_ctrl_ptr->channel_rect.left - slider_ctrl_ptr->thumb_width/2;
					slider_ctrl_ptr->thumb_rect.right = slider_ctrl_ptr->thumb_rect.left + slider_ctrl_ptr->thumb_width - 1;
				}

				if (slider_ctrl_ptr->thumb_rect.right > slider_ctrl_ptr->channel_rect.right + slider_ctrl_ptr->thumb_width/2)
				{
					slider_ctrl_ptr->thumb_rect.right = slider_ctrl_ptr->channel_rect.right + slider_ctrl_ptr->thumb_width/2;
					slider_ctrl_ptr->thumb_rect.left = slider_ctrl_ptr->thumb_rect.right - slider_ctrl_ptr->thumb_width + 1;
				}
            }
            
			if (0 != slider_ctrl_ptr->max - slider_ctrl_ptr->min)
			{
				SliderSetCurBySize(slider_ctrl_ptr, move_space*(slider_ctrl_ptr->max - slider_ctrl_ptr->min)/(slider_ctrl_ptr->channel_width));
                                        ...
                                        )
{
    ...
    CTRLSLIDER_OBJ_T     *slider_ctrl_ptr = (CTRLSLIDER_OBJ_T*) base_ctrl_ptr;
    CTRLBASE_VTBL_T     *parent_vtbl_ptr = (CTRLBASE_VTBL_T*)TYPEMNG_GetParentVtbl(CTRL_SLIDER_TYPE, (OBJECT_TYPE_PTR)base_ctrl_ptr);
                            ...
                            )
{
    ...
    if (PNULL != slider_ctrl_ptr)
    {
        ...
        if (slider_ctrl_ptr->is_dirty)
        {
            ...
            {
                channel_rect = GUI_GetCenterRect(channel_rect, slider_ctrl_ptr->channel_width, slider_ctrl_ptr->channel_height);
            }
            ...
            if (slider_ctrl_ptr->is_vertical)
            {
                ...
                if (0 != slider_ctrl_ptr->max - slider_ctrl_ptr->min)
                {
                    thumb_pos.y = channel_rect.top + (slider_ctrl_ptr->channel_height*slider_ctrl_ptr->cur/(slider_ctrl_ptr->max - slider_ctrl_ptr->min));
                }
                else
                {
                ...
                thumb_pos.x = channel_rect.left + (slider_ctrl_ptr->channel_width)/2;
            }
            else
            {
                // cal thumb rect
                if (0 != slider_ctrl_ptr->max - slider_ctrl_ptr->min)
                {
                    thumb_pos.x = channel_rect.left + (slider_ctrl_ptr->channel_width*slider_ctrl_ptr->cur/(slider_ctrl_ptr->max - slider_ctrl_ptr->min));
                }
                else
                {
                ...
                thumb_pos.y = channel_rect.top + (slider_ctrl_ptr->channel_height)/2;
            }
            
            thumb_rect.left = thumb_rect.right = thumb_pos.x;
            thumb_rect.top = thumb_rect.bottom = thumb_pos.y;
            thumb_rect = GUI_GetCenterRectExt(thumb_rect, slider_ctrl_ptr->thumb_width, slider_ctrl_ptr->thumb_height);
                           ...
                           )
{
        ...
        && !GUI_IsInvalidRect(cross_rect))
    {
        ...
        if (MMITHEME_CheckImageID(image_id))
        {
            ...
            image_rect.bottom = image_height - 1;//image_rect.top + (cross_rect.bottom - cross_rect.top);
            image_rect.right  = image_width - 1;//image_rect.left + (cross_rect.right - cross_rect.left);
                           ...
                           )
{
    ...
    {
        SliderDrawImage(slider_ctrl_ptr, slider_ctrl_ptr->thumb_info, &slider_ctrl_ptr->thumb_rect);
    }
}
                               ...
                               )
{
    ...
    if (PNULL != slider_ctrl_ptr && slider_ctrl_ptr->channel_fg_info.is_true)
    {
        ...
        SliderDrawImage(slider_ctrl_ptr, slider_ctrl_ptr->channel_fg_info, &channel_fg_rect);
    }
}
                               ...
                               )
{
    ...
    if (PNULL != slider_ctrl_ptr)
    {
        SliderDrawImage(slider_ctrl_ptr, slider_ctrl_ptr->channel_bg_info, &slider_ctrl_ptr->channel_rect);
    }
}
                                    ...
                                    )
{
    ...
    parent_vtbl_ptr = (CTRLBASE_VTBL_T*)TYPEMNG_GetParentVtbl(CTRL_SLIDER_TYPE, (OBJECT_TYPE_PTR)base_ctrl_ptr);
