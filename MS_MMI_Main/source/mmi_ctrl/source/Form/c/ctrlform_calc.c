                                        ...
                                        )
{
    ...
    uint16              display_width = 0;
    uint16              display_height = 0;
	uint16              backup_display_height = 0;
    uint16              calc_auto_num = 0;
    uint16              child_height = 0;
    uint32              all_child_width = 0;
    uint32              all_child_height = 0;
    ...
    backup_display_height = display_height;
    //set child
    child_ptr = first_child_ptr;
    while (PNULL != child_ptr)
    {
        if (CTRLFORM_IsValidChild(child_ptr))
        {
                    ...
                    form_ctrl_ptr))
            {
				...
				if(form_ctrl_ptr->top_child_ptr != PNULL
					&&child_ptr != PNULL 
					&& GUIFORM_CHILD_HEIGHT_MAX_TO_BOTTOM == child_ptr->height_info.type)
				{					
					if(form_ctrl_ptr->top_child_ptr != PNULL
					  && (form_ctrl_ptr->base_ctrl.rect.bottom - form_ctrl_ptr->top_child_ptr->height -all_child_height) > 0)
					{
						display_height =  form_ctrl_ptr->base_ctrl.rect.bottom - form_ctrl_ptr->top_child_ptr->height - all_child_height;
					}
					else
					{
						display_height =  backup_display_height;
					}
                        ...
                        form_ctrl_ptr))
                {
                    ...
                    // NOTE: Bug 1381225 disable progress bar when set height as GUIFORM_CHILD_HEIGHT_MAX_TO_BOTTOM
                    if (GUIFORM_CHILD_HEIGHT_MAX_TO_BOTTOM == child_ptr->height_info.type)
                    {
                        form_ctrl_ptr->is_display_prg = FALSE;
                    }
                                   ...
                                   )
{
    ...
    case GUIFORM_CHILD_HEIGHT_LEFT_MIN:
        ...
        break;
     case GUIFORM_CHILD_HEIGHT_MAX_TO_BOTTOM:
		 if (IGUICTRL_GetHeightByWidth(child_ptr->child_ctrl_ptr,child_width,&child_height))
		 {
			 child_ptr->height = MIN(child_height, display_height);
		 }
		 else
		 {
			 child_ptr->height =((form_ctrl_ptr->base_ctrl.display_rect.bottom > child_ptr->relative_rect.top) ? (form_ctrl_ptr->base_ctrl.display_rect.bottom - child_ptr->relative_rect.top +1) :display_height);
		 }
        break;
