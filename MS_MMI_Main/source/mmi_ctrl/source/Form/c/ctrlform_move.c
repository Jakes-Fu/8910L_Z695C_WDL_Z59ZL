                                                 ...
                                                 )
{
    ...
    if ((PNULL != child_ptr) && 
        ...
        (GUIFORM_CHILD_DOCK_FLOAT !=  CTRLFORM_GetDockChildType(child_ptr))&&
                                    ...
                                    )
{
        ...
        (display_top > child_top +1))
    {
        ...
        result = TRUE;
             ...
             (GetValidOrAtvChildPtr(FALSE,TRUE,form_ctrl_ptr->theme.is_cir_handle_ud,form_ctrl_ptr,form_ctrl_ptr->cur_child_ptr,&prev_child_ptr)))
    {
         //int32  child_top = 0;
	     //child_top = CTRLFORM_GetChildTop(form_ctrl_ptr->cur_child_ptr);
        //set current child
		if(form_ctrl_ptr->active_child_ptr != PNULL && (GUIFORM_CHILD_DOCK_BOTTOM == CTRLFORM_GetDockChildType(form_ctrl_ptr->active_child_ptr)))
		{
			GUI_RECT_T content_rect = {0};
			GUI_RECT_T form_rect = {0};
			GUI_RECT_T child_rect = {0};
			GUI_RECT_T top_rect = {0};
			BOOLEAN    exist_dock_top = FALSE;
            CTRLFORM_CHILD_T *first_child_ptr = prev_child_ptr;
			BOOLEAN  valid_child = FALSE;
			CTRLFORM_CHILD_T *top_child = CTRLFORM_GetDockChildPtrByDockType(GUIFORM_CHILD_DOCK_TOP, form_ctrl_ptr);
			
			form_rect = CTRLFORM_GetContentRect(form_ctrl_ptr);
			CTRLBASE_GetRect(form_ctrl_ptr->active_child_ptr->ctrl_handle, &content_rect);
			if(top_child != PNULL)
			{
				CTRLBASE_GetRect(top_child->ctrl_handle, &top_rect);
				exist_dock_top = TRUE;
			}
			do 
			{
				CTRLBASE_GetRect(prev_child_ptr->ctrl_handle, &child_rect);
				if(GUIFORM_CHILD_DOCK_TOP == CTRLFORM_GetDockChildType(prev_child_ptr) || GUIFORM_CHILD_DOCK_FLOAT == CTRLFORM_GetDockChildType(prev_child_ptr))
				{
					break;
				}     		
				if(child_rect.bottom - child_rect.top > 0)
				{			
					if(exist_dock_top && content_rect.top - top_rect.bottom >= (child_rect.bottom - child_rect.top))
					{
						break;
					}
					if(!exist_dock_top && form_rect.top + 10 < child_rect.bottom && (child_rect.top < form_rect.bottom))
					{
						break;
					}
				}
				if(!GetValidOrAtvChildPtr(FALSE,TRUE,form_ctrl_ptr->theme.is_cir_handle_ud,form_ctrl_ptr,prev_child_ptr,&prev_child_ptr))
				{
                    prev_child_ptr = PNULL;
					break;
				}
			} while (first_child_ptr != prev_child_ptr);
			

		}
		if(prev_child_ptr != PNULL)
		{
			form_ctrl_ptr->cur_child_ptr = prev_child_ptr;

		 //Remove since the cursor position not align with edit bit
			//set current child select index is last
			//if (form_ctrl_ptr->theme.is_cir_handle_ud)
			//{
			//    IGUICTRL_SetSelectIndex(prev_child_ptr->child_ctrl_ptr,FALSE,TRUE);
			//}
			...
			if ((CTRLFORM_IsDockChild(prev_child_ptr)) ||
				...
				|| ((child_top > display_top) && (form_ctrl_ptr->active_child_ptr == prev_child_ptr)))
			{
                                      ...
                                      )
{
    ...
    if (!(CTRLFORM_IsDockChild(form_ctrl_ptr->cur_child_ptr)) && 
        (GUIFORM_CHILD_DOCK_FLOAT !=  CTRLFORM_GetDockChildType(form_ctrl_ptr->cur_child_ptr))&&
             ...
             (GetValidOrAtvChildPtr(TRUE,FALSE,form_ctrl_ptr->theme.is_cir_handle_ud,form_ctrl_ptr,form_ctrl_ptr->cur_child_ptr,&next_child_ptr)))
    {
		...
		//child_top = CTRLFORM_GetChildTop(form_ctrl_ptr->cur_child_ptr);
        //set current child
		if(form_ctrl_ptr->active_child_ptr != PNULL && (GUIFORM_CHILD_DOCK_TOP == CTRLFORM_GetDockChildType(form_ctrl_ptr->active_child_ptr)))
		{
			GUI_RECT_T content_rect = {0};
			GUI_RECT_T child_rect = {0};
			GUI_RECT_T bottom_rect = {0};
			BOOLEAN    exist_dock_bottom =FALSE;
            CTRLFORM_CHILD_T *first_child_ptr = next_child_ptr;
			BOOLEAN   middle_child_visible = FALSE;
			CTRLFORM_CHILD_T *bottom_child = CTRLFORM_GetDockChildPtrByDockType(GUIFORM_CHILD_DOCK_BOTTOM, form_ctrl_ptr);
            GUI_RECT_T form_rect = CTRLFORM_GetContentRect(form_ctrl_ptr);
			CTRLBASE_GetRect(form_ctrl_ptr->active_child_ptr->ctrl_handle, &content_rect);
			if(bottom_child != PNULL)
			{
				CTRLBASE_GetRect(bottom_child->ctrl_handle, &bottom_rect);	
				exist_dock_bottom = TRUE;				
			}

			do 
			{
				CTRLBASE_GetRect(next_child_ptr->ctrl_handle, &child_rect);
				if(GUIFORM_CHILD_DOCK_BOTTOM == CTRLFORM_GetDockChildType(next_child_ptr) || GUIFORM_CHILD_DOCK_FLOAT == CTRLFORM_GetDockChildType(next_child_ptr))
				{
					break;
				} 
				if(child_rect.bottom - child_rect.top > 0)
				{
					if(exist_dock_bottom && bottom_rect.top - content_rect.bottom >= (child_rect.bottom - child_rect.top))
					{
						 break;
					}
					if(!exist_dock_bottom && form_rect.bottom  > child_rect.top + 10)
					{
						break;
					}
				}
				if(!(GetValidOrAtvChildPtr(TRUE,FALSE,form_ctrl_ptr->theme.is_cir_handle_ud,form_ctrl_ptr, next_child_ptr,&next_child_ptr)))
				{
                    next_child_ptr = PNULL;
					break;
				}
			} while (first_child_ptr != next_child_ptr);
			
			
		}	
		if(next_child_ptr != PNULL)
		{
		 ...
		 //Remove since the cursor position not align with edit bit
			...
			//if (form_ctrl_ptr->theme.is_cir_handle_ud)
			//{
			//    IGUICTRL_SetSelectIndex(next_child_ptr->child_ctrl_ptr,TRUE,FALSE);
			//}
			...
			if ((CTRLFORM_IsDockChild(next_child_ptr)) ||
				...
				|| ((child_bottom < form_ctrl_ptr->relative_disp_rect.bottom) && (form_ctrl_ptr->active_child_ptr == next_child_ptr)))
			{
...
/*****************************************************************************/
//  Description : check the all child ctrl is in the form display rect
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC void CTRLFORM_CheckAllChildInDisplayRect(
                              CTRLFORM_OBJ_T      *form_ctrl_ptr        
                                      )
{
        CTRLFORM_CHILD_T    *next_child_ptr = form_ctrl_ptr->first_child_ptr;

        if ((CTRLFORM_IsOrderForm(form_ctrl_ptr)) && CTRLFORM_IsValidChild(next_child_ptr))
        {
            CTRLFORM_CHILD_T *first_child_ptr = next_child_ptr;
            GUI_RECT_T child_rect = {0};
            ...
            display_rect.bottom = form_ctrl_ptr->base_ctrl.display_rect.bottom;
            display_rect.left = form_ctrl_ptr->base_ctrl.display_rect.left;
            display_rect.right = form_ctrl_ptr->base_ctrl.display_rect.right;
            display_rect.top = form_ctrl_ptr->base_ctrl.display_rect.top;
            do 
            {
                CTRLBASE_GetRect(next_child_ptr->ctrl_handle, &child_rect);
                if(GUIFORM_CHILD_DOCK_BOTTOM == CTRLFORM_GetDockChildType(next_child_ptr) || GUIFORM_CHILD_DOCK_FLOAT == CTRLFORM_GetDockChildType(next_child_ptr))
                ...
                if(GUI_IntersectRect(&temp_rect, child_rect, display_rect) == FALSE)
                {
                    MMK_SendMsg(form_ctrl_ptr->base_ctrl.win_handle,MSG_CTL_NOT_IN_DISPLAYREC,&(next_child_ptr->ctrl_handle));
                }
                else
                {
                    MMK_SendMsg(form_ctrl_ptr->base_ctrl.win_handle,MSG_CTL_IN_DISPLAYREC,&(next_child_ptr->ctrl_handle));
                }
            } while (GetValidOrAtvChildPtr(TRUE,FALSE,form_ctrl_ptr->theme.is_cir_handle_ud,form_ctrl_ptr, next_child_ptr,&next_child_ptr) && (first_child_ptr != next_child_ptr));
        }
}
