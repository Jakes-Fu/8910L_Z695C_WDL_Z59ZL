                               ...
                               )
{
    ...
    case GUIFORM_CHILD_DOCK_TOP:
        ...
        //top/bottom
        if(form_ctrl_ptr->relative_rect.bottom > 0 || child_height == 0)
        {
        ...
        {
	        form_ctrl_ptr->relative_rect.top    = form_ctrl_ptr->relative_rect.bottom + child_height -1;
	        form_ctrl_ptr->relative_rect.bottom = form_ctrl_ptr->relative_rect.bottom + child_height -1 ;
        }
        break;
                                 ...
                                 )
{
    ...
    if (CTRLFORM_IsUnitForm(form_ctrl_ptr))
    {
        ...
        if (PNULL != form_ctrl_ptr->active_child_ptr)
        {
            ...
            ctrl_bg.color = MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_DEFAULT);
            ctrl_border = form_ctrl_ptr->theme.form_active_child.border;
            if (form_ctrl_ptr->is_permit_font)
            {
                ctrl_font = GetChildFont(form_ctrl_ptr->first_child_ptr,form_ctrl_ptr);
                ctrl_font.color =0xffff;
                IGUICTRL_SetFont(form_ctrl_ptr->first_child_ptr->child_ctrl_ptr,&ctrl_font); 
                ctrl_font = GetChildFont(form_ctrl_ptr->active_child_ptr,form_ctrl_ptr);
                ctrl_font.color =0xffff;
                IGUICTRL_SetFont(form_ctrl_ptr->active_child_ptr->child_ctrl_ptr,&ctrl_font);  				
            }
        }
        else
        {
			ctrl_bg = form_ctrl_ptr->theme.form_child.bg;
            ctrl_border = form_ctrl_ptr->theme.form_child.border;
            if (form_ctrl_ptr->is_permit_font)
            {
                ctrl_font = GetChildFont(form_ctrl_ptr->first_child_ptr,form_ctrl_ptr);                
                IGUICTRL_SetFont(form_ctrl_ptr->first_child_ptr->child_ctrl_ptr,&ctrl_font);  				
                ctrl_font = GetChildFont(form_ctrl_ptr->cur_child_ptr,form_ctrl_ptr);                
                IGUICTRL_SetFont(form_ctrl_ptr->cur_child_ptr->child_ctrl_ptr,&ctrl_font);  				
            } 
        }
	...
	else if(form_ctrl_ptr->is_use_form_bg && !form_ctrl_ptr->is_permit_bg)
	{
        //get bg and border
        ctrl_bg.bg_type = GUI_BG_NONE;
        if (PNULL != form_ctrl_ptr->active_child_ptr)
                          ...
                          )
{
    ...
    if ((IsDisplayBgBorder(form_ctrl_ptr)) &&
        ...
        (GUIFORM_CHILD_DISP_NORMAL == child_ptr->display_type || GUIFORM_CHILD_DISP_FROM_HEAD == child_ptr->display_type ))
    {
        if (is_form_child)
        {
            if (CTRLFORM_IsAtvFormChild(child_ptr))
                                  ...
                                  )
{
    ...
    else if (!CTRLFORM_IsUnitFormSingle(form_ctrl_ptr))
    {
        ...
        {
            if (GUIFORM_CHILD_DISP_NORMAL == child_ptr->display_type || GUIFORM_CHILD_DISP_FROM_HEAD == child_ptr->display_type )
            {
                               ...
                               )
{
    ...
    if(CTRLFORM_IsExistFloatChild(form_ctrl_ptr->base_ctrl.handle))
    {
        display_top  = form_ctrl_ptr->relative_disp_rect.top;
    }
                                  ...
                                  )
{
    ...
    CTRLFORM_CHILD_T    *child_ptr = PNULL;
    CTRLFORM_CHILD_T    *high_pro_child = PNULL;
...
#ifdef GUIF_FORM_DOCKABLE            
            //first layout active form
            form_ctrl_ptr->cur_dock_postion_valid = FALSE;
            cur_priority = 1;
            child_ptr = CTRLFORM_GetDockChildPtr(&cur_priority,form_ctrl_ptr);

            while (PNULL != child_ptr)
            {
                if(form_ctrl_ptr->active_child_ptr == child_ptr
                   || form_ctrl_ptr->cur_child_ptr == child_ptr
                   ||(!CTRLFORM_IsDockChild(form_ctrl_ptr->cur_child_ptr) 
                       && !CTRLFORM_IsDockChild(form_ctrl_ptr->active_child_ptr)
                       &&child_ptr->dock_priority == 1))
                {               
                    CTRLBASE_OBJ_T    *base_ctrl_ptr=PNULL;
                    //set child rect and display rect
                    high_pro_child = child_ptr;
                    SetDockChildRect(is_update,TRUE,form_ctrl_ptr,child_ptr);                
                    base_ctrl_ptr = (CTRLBASE_OBJ_T *)child_ptr->child_ctrl_ptr;
                    if(base_ctrl_ptr != PNULL)
                    {
                        form_ctrl_ptr->cur_top = base_ctrl_ptr->rect.top;
                        form_ctrl_ptr->cur_bottom = base_ctrl_ptr->rect.bottom;
                    }
                    else
                    {
                        form_ctrl_ptr->cur_top = child_ptr->relative_rect.top;
                        form_ctrl_ptr->cur_bottom = child_ptr->relative_rect.bottom;
                    }
                    form_ctrl_ptr->cur_dock_postion_valid = TRUE;
                    //get next dock child
                }
                child_ptr = CTRLFORM_GetDockChildPtr(&cur_priority,form_ctrl_ptr);
            }
            
            //the layout dock child
            cur_priority = 1;
            child_ptr = CTRLFORM_GetDockChildPtr(&cur_priority,form_ctrl_ptr);
            while (PNULL != child_ptr)
            {
...
//  Note:
/*****************************************************************************/
PUBLIC void CTRLFORM_SetChildRectAfterActiveChanged(
                                  BOOLEAN            is_update,      //in:
                                  CTRLFORM_OBJ_T     *form_ctrl_ptr,  //in:
                                  CTRLFORM_CHILD_T   *active_child_ptr //in:
                                  )
{
#ifdef GUIF_FORM_DOCKABLE
    uint16              cur_priority = 0;
    ...
    if (PNULL != form_ctrl_ptr)
    {
        ...
        if (GUI_IsZeroRect(form_ctrl_ptr->writeback_rect))
        {
            ...
            while (PNULL != child_ptr)
            {
                if(active_child_ptr == child_ptr
                    ||(!CTRLFORM_IsDockChild(active_child_ptr) 
                       &&child_ptr->dock_priority == 1)
                )
                {               
                    CTRLBASE_OBJ_T    *base_ctrl_ptr=PNULL;
                ...
                child_ptr = CTRLFORM_GetDockChildPtr(&cur_priority,form_ctrl_ptr);
                if(active_child_ptr == child_ptr || GUIFORM_CHILD_DISP_FROM_HEAD != active_child_ptr->display_type)
                {                    
                    form_ctrl_ptr->cur_dock_postion_valid = TRUE;
                }
            }             
            ...
            while (PNULL != child_ptr)
            {
                if(active_child_ptr != child_ptr && high_pro_child != child_ptr)
                {               
                    CTRLBASE_OBJ_T    *base_ctrl_ptr=PNULL;
                    //set child rect and display rect
                    SetDockChildRect(is_update,TRUE,form_ctrl_ptr,child_ptr);
                            ...
                            )
{
        ...
        (MMK_IsHandleValid(child_ptr->ctrl_handle)))
    {
		...
		active_child_ptr = (form_ctrl_ptr->active_child_ptr != PNULL ? form_ctrl_ptr->active_child_ptr:form_ctrl_ptr->cur_child_ptr);
        if( CTRLFORM_IsOrderForm(form_ctrl_ptr) 
            && (active_child_ptr != child_ptr && form_ctrl_ptr->cur_child_ptr != child_ptr)
            &&(form_ctrl_ptr->cur_top != 0 || form_ctrl_ptr->cur_bottom != 0)
            && form_ctrl_ptr->cur_dock_postion_valid
            /*
			&& (GUIFORM_CHILD_DOCK_TOP == CTRLFORM_GetDockChildType(active_child_ptr) 
			    || GUIFORM_CHILD_DOCK_BOTTOM == CTRLFORM_GetDockChildType(active_child_ptr)
				)
			*/
			)
        {
	        if(GUIFORM_CHILD_DOCK_TOP == child_ptr->dock_type)
            {
                //not active dock top, when it's bottom > form_ctrl_ptr->cur_top, must move to above of form's top 
                if(child_rect.bottom > form_ctrl_ptr->cur_top)
                {
	                child_rect.top = form_ctrl_ptr->cur_top - (child_rect.bottom - child_rect.top);
	                child_rect.bottom = form_ctrl_ptr->cur_top;                        
                }
            }
            else if(GUIFORM_CHILD_DOCK_BOTTOM == child_ptr->dock_type)
            {
                //dock bottom, when it's top < form_ctrl_ptr->cur_bottom, must move to below form's bottom
                if(child_rect.top < form_ctrl_ptr->cur_bottom)
                {
	                child_rect.bottom = form_ctrl_ptr->cur_bottom + (child_rect.bottom - child_rect.top);
	                child_rect.top = form_ctrl_ptr->cur_bottom;
                }
                        ...
                        )
{
        ...
        (PNULL != child_ptr))
    {
        ...
        {        
            if((form_ctrl_ptr->cur_top < child_rect.bottom && form_ctrl_ptr->cur_top > child_rect.top)
               ||(form_ctrl_ptr->cur_top <= child_rect.top && form_ctrl_ptr->cur_bottom >= child_rect.bottom))
            {
                //at dock form's half top    
                child_rect.top = form_ctrl_ptr->cur_top - (child_rect.bottom - child_rect.top);
                child_rect.bottom = form_ctrl_ptr->cur_top;
                
            }
            else if(form_ctrl_ptr->cur_bottom > child_rect.top && form_ctrl_ptr->cur_bottom < child_rect.bottom)
            {
                //at dock form's half bottom
                child_rect.bottom = form_ctrl_ptr->cur_bottom + (child_rect.bottom - child_rect.top);
                child_rect.top = form_ctrl_ptr->cur_bottom;
        ...
        {
			if(GUIFORM_CHILD_DOCK_FLOAT == CTRLFORM_GetDockChildType(child_ptr))
			{
				cross_rect = child_ptr->relative_rect;
			}			
...
#ifdef MMI_LISTBOX_SHOW_DIVIDING_LINE
