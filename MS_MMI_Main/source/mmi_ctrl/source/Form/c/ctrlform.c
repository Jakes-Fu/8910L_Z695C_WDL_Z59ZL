...
/*****************************************************************************/
//  Description : edit cursor move up/down
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleCursorUpDownMsg(
                                       CTRLFORM_OBJ_T           *form_ctrl_ptr, //IN
                                       GUIFORM_CURSOR_UPDOWN_T    *notify_ptr     //in:
                                       );
                            ...
                            )
{
    ...
    form_ctrl_ptr->is_use_form_bg =FALSE;
	form_ctrl_ptr->is_reset_top_down = FALSE;
                                 ...
                                 )
{
    ...
    case MSG_NOTIFY_UPDATE:
        ...
        break;
    case MSG_NOTIFY_CURSOR_UP_DOWN:
        result = HandleCursorUpDownMsg(form_ctrl_ptr, (GUIFORM_CURSOR_UPDOWN_T *)param);
        break;
                                     ...
                                     )
{
        ...
        (active_child_ptr != form_ctrl_ptr->active_child_ptr))
    {
        ...
        if(GUIFORM_CHILD_DOCK_TOP != 
        CTRLFORM_GetDockChildType(active_child_ptr) && 
        ...
        if (form_ctrl_ptr->layout_index == form_ctrl_ptr->all_child_num)
        {
            if (CTRLFORM_IsFormChild(active_child_ptr))
            {
				...
				else if(!form_ctrl_ptr->is_permit_bg)
				{
                                       ...
                                       )
{
    ...
    if (0 == parent_handle)
    {
        ...
        if (notify_ptr->cursor_rect.top < content_rect.top || notify_ptr->cursor_rect.bottom > content_rect.bottom)
        {
            if (CTRLFORM_GetChildPtrByHandle(notify_ptr->ctrl_handle,form_ctrl_ptr,&child_ptr))
            {
                    ...
                    (PNULL != ((CTRLFORM_OBJ_T *)active_child_ptr->child_ctrl_ptr)->active_child_ptr))
                {
                    ...
                    MMK_SendMsg(active_child_ptr->ctrl_handle,MSG_NOTIFY_CURSOR_UP_DOWN,notify_ptr);
                }
            }
        }
        else
                                          ...
                                          )
{
        ...
        (form_ctrl_ptr->is_valid_child))
    {
        ...
        {
            //scroll form
             if(active_child_ptr != PNULL 
				 && MSG_APP_UP == msg_id
				 && form_ctrl_ptr->active_child_ptr !=active_child_ptr 
				 && GUIFORM_CHILD_DISP_FROM_HEAD == active_child_ptr->display_type)
             {
				 uint16 display_height = 0;
				 uint16 child_height = 0;

				 child_height = CTRLFORM_GetChildHeight(active_child_ptr);
				 display_height = CTRLFORM_GetDisplayHeight(form_ctrl_ptr);
				 if(child_height > display_height)
				 {
					display_top = display_top - (child_height - display_height);
					if(display_top < 0)
					{
						display_top = 0;
					}
            ...
            if (CTRLFORM_SetDisplayRect(display_top,display_left,form_ctrl_ptr))
            {
				...
				if(active_child_ptr != PNULL)
				{
					CTRLFORM_SetChildRectAfterActiveChanged(TRUE,form_ctrl_ptr, active_child_ptr);
				}
            }
            else if(active_child_ptr != form_ctrl_ptr->active_child_ptr)
            {
                //set child rect
                CTRLFORM_SetChildRectAfterActiveChanged(TRUE,form_ctrl_ptr, active_child_ptr);
            }
            if ((active_child_ptr != PNULL) && (active_child_ptr != form_ctrl_ptr->active_child_ptr))
            {
                //set active control
				//IGUICTRL_SetState(active_child_ptr->child_ctrl_ptr,GUICTRL_STATE_DISABLE_ACTIVE,FALSE);
				if(form_ctrl_ptr->is_reset_top_down)
				{
					if(MSG_APP_DOWN == msg_id)
					{
						IGUICTRL_SetSelectIndex(active_child_ptr->child_ctrl_ptr, TRUE, FALSE);
					}
					else if(MSG_APP_UP == msg_id)
					{
						IGUICTRL_SetSelectIndex(active_child_ptr->child_ctrl_ptr, FALSE, TRUE);
					}
				}
                MMK_SetAtvCtrl(form_ctrl_ptr->win_handle,active_child_ptr->ctrl_handle);

                CTRLFORM_CheckAllChildInDisplayRect(form_ctrl_ptr);
            }
...
/*****************************************************************************/
//  Description :form set child up/down postion
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLFORM_SetChildUpDownPostion(
                              MMI_HANDLE_T   form_handle,//in:
                              BOOLEAN        need_set_child_postion     //in:
                              )
{
    CTRLFORM_OBJ_T  *form_ctrl_ptr = PNULL;
    BOOLEAN         result = FALSE;
    ...
    {
        //set bg
		form_ctrl_ptr->is_reset_top_down = need_set_child_postion;
...
/***************************************************************************//*!
@brief 设置正常Form_child的背景,边框或者字体
@author arvin
...
@note 
*******************************************************************************/
PUBLIC BOOLEAN CTRLFORM_SetFormChildTheme(
                                      MMI_HANDLE_T   form_handle,    //in:
                                      ...
                                      )
{
    ...
    //get form pointer by control id
    form_ctrl_ptr = CTRLFORM_GetPtr(form_handle);
    if (PNULL != form_ctrl_ptr)//should check if it's unit type or not?
    {
        //set bg
        if (PNULL != bg_ptr)
        {
            form_ctrl_ptr->theme.form_child.bg = *bg_ptr;
        }
        
        //set border
        if (PNULL != border_ptr)
        {
            form_ctrl_ptr->theme.form_child.border = *border_ptr;
        }
        ...
        while (PNULL != child_ptr)
        {
            ...
            if (CTRLFORM_IsFormChild(child_ptr))
            {
                CTRLFORM_SetFormChildTheme(child_ptr->ctrl_handle,bg_ptr,border_ptr);
            }
            child_ptr = child_ptr->next_ptr;
        }
    }
...
/***************************************************************************//*!
@brief 设置激活Form_child的背景,边框或者字体
@author arvin
@param form_handle  [in] 控件handle
@param bg_ptr       [in] 背景
@param border_ptr   [in] 边框
...
@note 
*******************************************************************************/
PUBLIC BOOLEAN CTRLFORM_SetFormActiveChildTheme(
                                      MMI_HANDLE_T   form_handle,    //in:
                                      GUI_BG_T       *bg_ptr,        //in/out:may PNULL
                                      GUI_BORDER_T   *border_ptr   //in/out:may PNULL
                                      )
{
    ...
    if (PNULL != form_ctrl_ptr)//should check if it's unit type or not?
    {
        ...
        if (PNULL != bg_ptr)
        {
            form_ctrl_ptr->theme.form_active_child.bg = *bg_ptr;
        }
        
        //set border
        if (PNULL != border_ptr)
        {
            form_ctrl_ptr->theme.form_active_child.border = *border_ptr;
        }
        ...
        while (PNULL != child_ptr)
        {
            ...
            if (CTRLFORM_IsFormChild(child_ptr))
            {
                CTRLFORM_SetFormActiveChildTheme(child_ptr->ctrl_handle,bg_ptr,border_ptr);
            }
            child_ptr = child_ptr->next_ptr;
        }
    }
...
/*****************************************************************************/
//  Description : set user form bg
//  Global resource dependence : 
//  Author: Jassmine
//  Note: only set one control dock type
/*****************************************************************************/
PUBLIC void CTRLFORM_SetUseFormBg(
                                     MMI_HANDLE_T           form_handle,
                                     BOOLEAN                  is_use_form_bg
                                     )
{
    BOOLEAN             result = FALSE;
    CTRLFORM_OBJ_T      *form_ctrl_ptr = PNULL;
    ...
    //get form pointer by control id
    form_ctrl_ptr = CTRLFORM_GetPtr(form_handle);
    if(form_ctrl_ptr != PNULL)
    {
        form_ctrl_ptr->is_use_form_bg = is_use_form_bg;
    }
...
/*****************************************************************************/
//  Description : get form exist float child
//  Global resource dependence : 
//  Author: Jassmine
//  Note: default TRUE
/*****************************************************************************/
PUBLIC BOOLEAN CTRLFORM_IsExistFloatChild(
                                      MMI_HANDLE_T   form_handle                                     
                                      )
{
    BOOLEAN             result = FALSE;
    ...
    if (PNULL != form_ctrl_ptr)
    {
        ...
        {
            //is form child
            if (GUIFORM_CHILD_DOCK_FLOAT == child_ptr->dock_type)
            {
                result = TRUE;
                break;
            }            
