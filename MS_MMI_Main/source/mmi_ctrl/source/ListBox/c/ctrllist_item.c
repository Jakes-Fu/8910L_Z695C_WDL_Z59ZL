        ...
        )
{
            ...
            && !LIST_GetItemState(item_ptr, GUIITEM_STATE_DISABLE_MARK))
    {
        ...
        {   
            dw -= (list_ctrl_ptr->display_style.tag_width - (list_ctrl_ptr->prgbox_ptr ? MMITHEME_GetScrollBarWidthByState(list_ctrl_ptr->display_style.is_slide_used) + 1 : 0));
            item_ptr->mark_left = list_ctrl_ptr->mark_left_margin;
            contents_left += list_ctrl_ptr->display_style.tag_width;
            item_ptr->mark_top = list_ctrl_ptr->mark_top_margin;

            //此处单双行list下check box图标位置不同，需要分开设置mark_top
            if(list_ctrl_ptr->is_check_used_for_multi_line)
            {
                item_ptr->mark_top = list_ctrl_ptr->mark_top_margin_for_multi;
            }   
            ...
            {
                item_ptr->mark_left = list_ctrl_ptr->avatar_checkbox_left_margin;
                contents_left = 0;
                item_ptr->mark_top = list_ctrl_ptr->avatar_checkbox_top_margin;           
            }
        }
        else
        {
            if (CTRLLIST_MARK_RADIO == list_ctrl_ptr->mark_type)
            {
                dw -= (list_ctrl_ptr->display_style.radio_tag_width - (list_ctrl_ptr->prgbox_ptr ? MMITHEME_GetScrollBarWidthByState(list_ctrl_ptr->display_style.is_slide_used) + 1 : 0));
                contents_right -= list_ctrl_ptr->display_style.radio_tag_width;
    ...
    {
        contents_right -= list_ctrl_ptr->display_style.toggle_tag_width;
        item_ptr->mark_left = contents_right + 1;           
    }
    ...
    {
        int16 backup_left = 0;
        uint16  tag_icon_width = list_ctrl_ptr->display_style.tag_width;
        int16	left_width = 0;
        ...
        if(list_ctrl_ptr->is_mark_icon_left)
        {
            tag_icon_width = list_ctrl_ptr->display_style.tag_img_width;

            //此处取头像checkbox图标宽度
            if (LIST_GetItemState(item_ptr, GUIITEM_STATE_HAS_INITIAL_CHECKBOX) 
                    ||LIST_GetItemState(item_ptr, GUIITEM_STATE_HAS_AVATAR_CHECKBOX))
            {
                tag_icon_width = list_ctrl_ptr->display_style.avatar_checkbox_width;
            }
        }
        ...
        {
            //此处取Radio图标宽度
            tag_icon_width = list_ctrl_ptr->display_style.radio_tag_width;
        }

        if (LIST_GetItemState(item_ptr, GUIITEM_STATE_HAS_TOGGLE_MARK))
        {
            //此处取toggle图标宽度
            tag_icon_width = list_ctrl_ptr->display_style.toggle_tag_width;
        }

        //only left: left(+tag_width)->right(disp_right-)->left
        //mark
        item_ptr->mark_left = (disp_right + list_rect.left) - (item_ptr->mark_left + tag_icon_width) + list_rect.left;
        //num
        item_ptr->num_left = disp_right - (item_ptr->num_left + tag_icon_width);
        ...
        {
            left_width = list_ctrl_ptr->display_style.quick_icon_bg_width + list_ctrl_ptr->display_style.quick_and_triangel_space;

            if (GUIITEM_THEME_STATE_LEFT_FOCUS == state)
            {
                for (idx = 0; idx < GUIITEM_CONTENT_MAX_NUM; idx++)
                {
                    item_ptr->content_rect[idx].left += left_width;
                    item_ptr->content_rect[idx].right += left_width;
                }
            }
        ...
        )
{
    ...
    {
        if (((uint32)(item_ptr->item.data_ptr) & 0xFFFF0000) != 0)
        {
            for (i = 0; i < GUIITEM_CONTENT_MAX_NUM; i++)
            {
                //释放text buffer
        ...
        )
{
    ...
    else
    {
        ...
        {
            icon_id = (is_selected)
                ? list_ctrl_ptr->display_style.avatar_checkbox_selected_image_id
                : list_ctrl_ptr->display_style.avatar_checkbox_unselected_image_id;
        }
...
//  Global resource dependence :
//  Author:
//  Note: add item_ptr,is_highlight param @Yanyan.an
/*****************************************************************************/
        ...
        if (GUI_IntersectRect(&inter_rect, *disp_rect_ptr, icon_disp_rect))
        {
            ...
            else if(list_ctrl_ptr->display_style.check_selected_image_id == icon_id)
            {
                ...
                GUIRES_DisplayImg(PNULL,
                        ...
                        &icon_rect,
                        list_ctrl_ptr->win_id,
                        list_ctrl_ptr->display_style.marked_image_id,//对勾icon
                        dev_info_ptr);
            }
                    ...
                    && (list_ctrl_ptr->display_style.avatar_checkbox_selected_image_id == icon_id))
            {
                ...
                GUIRES_DisplayImg(PNULL,
                        ...
                        &icon_rect,
                        list_ctrl_ptr->win_id,
                        list_ctrl_ptr->display_style.avatar_marked_image_id,//对勾icon
                        dev_info_ptr);
        ...
        )
{
            ...
            (PNULL != lcd_dev_info_ptr))
    {
        ...
        if (GUI_IntersectRect(&bg_rect, inter_rect, icon_disp_rect))
        {
            //获取bg color
        ...
        )
{
    ...
    SCI_MEMSET(item_ptr->item.data_ptr, 0x00, sizeof(GUILIST_ITEM_DATA_T));
    ...
    for (i = 0; i < GUIITEM_CONTENT_MAX_NUM; i++)
    {
        ...
        SCI_MEMSET(item_ptr->item.data_ptr->item_content[i].item_data.text_buffer.wstr_ptr, 0x00, str_len * sizeof(wchar));
        ...
        )
{
    ...
    if (item_ptr != PNULL)
    {
        ...
        for (i = 0; i < GUIITEM_CONTENT_MAX_NUM; i++)
        {
            ...
            //added by Tong Yongli. According to Coverity CID 16708
            if(PNULL == item_ptr->item.data_ptr)
            {
                continue;
            }
...
#define INVALID_VALUE 0X7FFF
    ...
    else
    {
        ...
        else
        {
            ...
            list_ctrl_ptr->is_pos_dirty = TRUE;
            //list_ctrl_ptr->is_full_dirty = TRUE;bug491524 poor performance of list.
        ...
        )
{
    ...
    if (total_item_height >= total_rect_height) // 当高度大于区域的高度时
    {
        ...
        {
            offset_y -= list_ctrl_ptr->end_item_bottom_margin;
        }
    }
    else
    {
        ...
        )
{
            ...
            && LIST_GetListStateByPtr(list_ctrl_ptr, GUILIST_STATE_NEED_HIGHTBAR))
    {
        ...
        else if(LIST_GetItemState(item_ptr, GUIITEM_STATE_LEFT_FOCUS))
        {
        ...
        )
{
...
#if 0
    if (TRUE == item_ptr->item.data_ptr->item_content[content_index].is_custom_font_color)
    {
    ...
    is_active_state = LIST_GetListStateByPtr(list_ctrl_ptr, GUILIST_STATE_ACTIVE);
    if ((item_index != list_ctrl_ptr->cur_item_idx||(!is_active_state))&&(item_style_ptr->content[content_index].state & GUIITEM_CONTENT_STATE_EFFECT_STR))
    {
...
/*****************************************************************************/
//  Description : 获取列表控件文本的custom text color
//  Global resource dependence :
//  Author:Yanyan.an
//  Note:
/*****************************************************************************/
PUBLIC GUI_COLOR_T LIST_GetItemCustomTextColor(
        CTRLLIST_OBJ_T *list_ctrl_ptr,
        uint16 item_index,
        uint16 content_index
        )
{
    ...
    //invalid
    if (GUIITEM_THEME_STATE_INVALID == theme_state
            || GUIITEM_THEME_STATE_FOCUS_INVALID == theme_state)
    {
        color = THEMELIST_GetContentTextColor(theme_state, item_ptr->item.style_ptr, content_index);
    }
    //nomal
    else if (GUIITEM_THEME_STATE_DEFAULT == theme_state)
    {
        color = list_ctrl_ptr->display_style.content_color_style[content_index].font_color;
    }
    else
    {
        color = list_ctrl_ptr->display_style.content_color_style[content_index].font_color_focus;
    }
...
/*****************************************************************************/
//  Description : get toggle icon of list
//  Global resource dependence :
//  Author:Yanyan.an
//  Note:
/*****************************************************************************/
PUBLIC MMI_IMAGE_ID_T LIST_GetToggleIconId(
        const CTRLLIST_OBJ_T *list_ctrl_ptr,
        const CTRLLIST_ITEM_INFO_T *item_ptr,
        BOOLEAN is_focusd
        )
{
    ...
    is_gray = LIST_GetItemState(item_ptr, GUIITEM_STATE_GRAY);
...
#if 1
        icon_id = (is_selected)
            ? list_ctrl_ptr->display_style.toggle_on_image_id
            : list_ctrl_ptr->display_style.toggle_off_image_id;
#else
        if(TRUE == is_gray)
        {
            icon_id = MMITHEME_GetThemeToggleIcon(is_selected, FALSE);
        }
        else
        {
            icon_id = MMITHEME_GetThemeToggleIcon(is_selected, is_focusd);
        }
#endif
    }
