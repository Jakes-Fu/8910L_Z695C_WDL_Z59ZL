...
typedef struct
{
    ...
    BOOLEAN                     is_show_left_shortcut_indicator;//当前是否需要显示左shortcut indicator，默认FALSE
    BOOLEAN                     is_show_right_shortcut_indicator;//当前是否需要显示右shortcut indicator，默认FALSE

    BOOLEAN                     is_item_handle_leftkey;         //当前是否响应左键，默认FALSE
    BOOLEAN                     is_item_handle_rightkey;        //当前是否响应右键，默认FALSE

    MMI_IMAGE_ID_T              item_left_quick_icon; //当前显示的左quick图标，默认PNULL
    MMI_IMAGE_ID_T              item_right_quick_icon;//当前显示的右quick图标，默认PNULL
...
typedef struct _list_obj_tag
{
	...
	BOOLEAN         				is_use_special_check_image; // 是否使用特定check图标
	MMI_IMAGE_ID_T  				check_selected_image_id;    // 多选选中图标
	MMI_IMAGE_ID_T  				check_unselected_image_id;  // 多选未选中图标
    //for check box
    BOOLEAN                         is_check_used_for_multi_line;  /*!< 是否check用于多行list style，默认为FALSE*/
    ...
    BOOLEAN                         is_need_speech;       /*<是否需要朗读list内容>*/
#endif

	//6pixel spec
	BOOLEAN                         is_permit_bottom_margin;   /*!<是否允许end item与softkey间有边距*/
	uint8							end_item_bottom_margin;   /*!<end item与softkey间的边距*/

    MMI_IMAGE_ID_T                  left_quick_icon_image;    //Qucik图片的image id
    MMI_IMAGE_ID_T                  right_quick_icon_image;    //Qucik图片的image id
	BOOLEAN         				is_show_left_triange_image; //是否显示左三角图标，默认不显示，仅用于Focus状态下
	BOOLEAN         				is_show_right_triange_image; //是否显示右三角图标，默认不显示，仅用于Focus状态下
    BOOLEAN                         is_handle_left_key;         //是否响应左键
    BOOLEAN                         is_handle_right_key;        //是否响应右键
