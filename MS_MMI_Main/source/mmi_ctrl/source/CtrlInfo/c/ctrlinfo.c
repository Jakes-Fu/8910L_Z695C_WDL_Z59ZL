...
LOCAL GUICTRL_CLASS_INFO_T s_ctrl_class_info[] =
{
	...
	{GUID_NAME_DEF(SPRD_GUI_HEADERANDDIVIDER_ID), HEADERANDDIVIDER_GetType},
#ifdef GUIF_SCROLLKEY
...
LOCAL const CAF_STATIC_CLASS_T s_ctrl_class_list[] = 
{
	...
	{SPRD_GUI_HEADERANDDIVIDER_ID, CtrlClassNew,0},
};
                              ...
                              )
{
	...
	SCI_TRACE_LOW("VTLCTRL_Release\n");

    if (0 < --guictrl_ptr->ref_num)
    {
    	SCI_TRACE_LOW("VTLCTRL_Release ref_num >0\n");
        ref_num = guictrl_ptr->ref_num;
    }
    else
    {
    	SCI_TRACE_LOW("VTLCTRL_Release TYPEMNG_Destroy\n");
