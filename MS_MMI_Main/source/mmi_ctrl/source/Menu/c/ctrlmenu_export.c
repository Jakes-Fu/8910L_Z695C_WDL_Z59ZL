...
/*****************************************************************************/
//  Description :set menu text
//  Global resource dependence : 
//  Author:
//  Note:
/*****************************************************************************/
PUBLIC void CTRLMENU_SetText(
                              ...
                              )
{
    ...
    if (PNULL != text_ptr)
    {
        ...
        {
            //set menu text
            menu_ctrl_ptr->text_len = (uint16)(MIN(CTRLMENU_TEXT_MAX_LEN,text_ptr->wstr_len));
            MMI_WSTRNCPY( menu_ctrl_ptr->text_wstr,CTRLMENU_TEXT_MAX_LEN,
                text_ptr->wstr_ptr,text_ptr->wstr_len,
                menu_ctrl_ptr->text_len);
        }
    }
}

/***************************************************************************//*!
//  Description :设置Text的扩展方法
//  Global resource dependence : 
//  Author:
//  Note:
*******************************************************************************/
PUBLIC BOOLEAN CTRLMENU_SetTextEx(
                                   MMI_CTRL_ID_T       ctrl_id,        // [in]
                                   GUIMENU_TEXT_INFO_T *text_info_ptr // [in]
                                   )
{
    ...
    if (PNULL != text_info_ptr)
    {
        ...
        if (PNULL != menu_ctrl_ptr)
        {
            if (menu_ctrl_ptr->is_static && text_info_ptr->is_static)
            {
                group_info_ptr = CTRLMENU_GetGroupInfo(menu_ctrl_ptr, text_info_ptr->group_id);
                if (PNULL != group_info_ptr)
                {
                    ReplaceString(&group_info_ptr->text_str, text_info_ptr->text_ptr);
                    result = TRUE;
                }
            }
            else if (!menu_ctrl_ptr->is_static && !text_info_ptr->is_static)
            {
                //get parent node pointer
                root_node_ptr = menu_ctrl_ptr->root_node_ptr;
                node_ptr = FindNodeById(text_info_ptr->node_id, root_node_ptr);

                if (PNULL != node_ptr)
                {
                    ReplaceString(&node_ptr->text_str, text_info_ptr->text_ptr);
                    result = TRUE;
                }
            }
        }
                                    ...
                                    )
{
	...
	case GUIMENU_STYLE_FULLSCREEN_POPMENU:
        is_param_ok = TRUE;
        break;
...
PUBLIC void CTRLMENU_SetDynamicMenuSoftkey(
                                           MMI_CTRL_ID_T    ctrl_id,
                                           uint32           left_softkey_id,  //option menu softkey 可设置为text/image
                                           uint32           middle_softkey_id,
                                           uint32           right_softkey_id
                                           )
{
    CTRLMENU_OBJ_T  *menu_ctrl_ptr = PNULL;
    ...
    if (PNULL != menu_ctrl_ptr)
    {
        menu_ctrl_ptr->dyna_softkey_id[0] = left_softkey_id;
        menu_ctrl_ptr->dyna_softkey_id[1] = middle_softkey_id;
        menu_ctrl_ptr->dyna_softkey_id[2] = right_softkey_id;
    }
}

/*****************************************************************************/
//  Description : Set PopMenu Softkey Type
//  Global resource dependence : 
//  Author:
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLMENU_SetPopMenuSoftkeyType(
                                         MMI_CTRL_ID_T       ctrl_id,
                                         GUISK_DATA_TYPE_E   left_softkey_type,
                                         GUISK_DATA_TYPE_E   middle_softkey_tye,
                                         GUISK_DATA_TYPE_E   right_softkey_type
                                         )
{
    BOOLEAN         result = FALSE;
    CTRLMENU_OBJ_T  *menu_ctrl_ptr  = PNULL;
    ...
    {
        menu_ctrl_ptr->pop_theme.sk_type[LEFT_BUTTON]   = left_softkey_type;
		menu_ctrl_ptr->pop_theme.sk_type[MIDDLE_BUTTON] = middle_softkey_tye;
		menu_ctrl_ptr->pop_theme.sk_type[RIGHT_BUTTON]  = right_softkey_type;
        result = TRUE;
    }
...
/*****************************************************************************/
//  Description : Set PopMenu Centered
//  Global resource dependence : 
//  Author:
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLMENU_SetPopMenuCentered(
                                         MMI_CTRL_ID_T       ctrl_id,
                                         BOOLEAN             is_centered
                                         )
{
    ...
    if ((PNULL != menu_ctrl_ptr) && 
        (CTRLMENU_IsPopMenu(menu_ctrl_ptr)))
    {
        menu_ctrl_ptr->pop_theme.is_centered = is_centered;
        result = TRUE;
    }
...
/*****************************************************************************/
//  Description : set item state, this method is for static menu
//  Global resource dependence : 
//  Author:
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLMENU_SetStaticItemState(
                                      MMI_CTRL_ID_T         ctrl_id,    // [in]
                                      MMI_MENU_GROUP_ID_T   group_id,   // [in]
                                      MMI_MENU_ID_T         menu_id,    // [in]
        ...
        if (PNULL != group_info_ptr)
        {
            ...
            for (i = 0; i < group_ptr->item_count; i++)
            {
                if (group_ptr->item_ptr[i].menu_id == menu_id)
                {
                    if(is_true)
                    {
                        item_info_ptr[i].item_state |= item_state;
                    }
				    else
				    {
				        item_info_ptr[i].item_state &= ~item_state;
...
/*****************************************************************************/
//  Description : set pop-up menu item state
//  Global resource dependence : 
//  Author:
//  Note:
/*****************************************************************************/
PUBLIC void CTRLMENU_SetItemStateEx(
                                      GUIMENU_POP_SEL_INFO_T    *selected_info_ptr,
                                      GUIMENU_ITEM_STATE_T      item_state,
                                      BOOLEAN                   is_true
                                      )
{
    CTRLMENU_OBJ_T          *menu_ctrl_ptr = PNULL;
    ...
    if (PNULL != menu_ctrl_ptr)
    {
        if(selected_info_ptr->is_static)
        {
            CTRLMENU_SetStaticItemState(selected_info_ptr->ctrl_id, selected_info_ptr->group_id, selected_info_ptr->menu_id, item_state, is_true);
        }
		else
		{
		    CTRLMENU_NODE_T     *item_node_ptr  = PNULL;

            item_node_ptr = CTRLMENU_GetNodeByNodeId(selected_info_ptr->node_id,menu_ctrl_ptr->cur_parent_node_ptr);
