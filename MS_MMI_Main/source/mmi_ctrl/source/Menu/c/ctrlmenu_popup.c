...
**                         MACRO DEFINITION                                  *
**---------------------------------------------------------------------------*/
#define CTRLMENU_CONTEXT_TOP_MARGIN          0          // 上下的
                            ...
                            )
{
    ...
    if(menu_ctrl_ptr->is_first_disp && MMITHEME_Is3DOpen() &&  CTRLMENU_IsPopMenu(menu_ctrl_ptr))
    {
        if(1 == menu_ctrl_ptr->menu_level)
        {
            ...
            MMITHEME_PopupMenuEffect(&menu_rect, &pre_menu_rect , 1, TRUE);
                                              ...
                                              )
{
    ...
    if (PopmenuIsNeedScrollBar( menu_ctrl_ptr ) )
    {
        item_rect.right = (int16)( item_rect.right - (MMITHEME_GetScrollBarWidthByState(menu_ctrl_ptr->common_theme.is_used_slide) + 2) );
                                  ...
                                  )
{
   ...
   if(item_index == menu_ctrl_ptr->cur_item_index && text_color == 0x0040)
    {
        text_color = 0xffff; //use white color
    }
                                      ...
                                      )
{
    ...
    if (MMITHEME_GetIsScrollAsPixel())
    {
        ...
        text_pixel_len = GUI_CalculateStringPiexlNum(text_str.wstr_ptr,
                            ...
                            font_type,
                            0);
        menu_ctrl_ptr->item_str_index = GUI_CalculateStringScrollStartPos(text_pixel_len,text_rect_width,
                                                menu_ctrl_ptr->pop_theme.pixel_scroll_step,
                                                menu_ctrl_ptr->item_str_index);
    }
    else
    {
