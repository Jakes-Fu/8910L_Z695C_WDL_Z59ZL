                            ...
                            )
{
    ...
    {
        if(menu_param_ptr->style == GUIMENU_STYLE_FULLSCREEN_POPMENU)
         {
            menu_param_ptr->style = GUIMENU_STYLE_POPUP;
            menu_ctrl_ptr->is_full_screen = TRUE;
         }
         else
         {
             menu_ctrl_ptr->is_full_screen = FALSE;
         }
        ...
        {
            //set transparent
            if(!menu_ctrl_ptr->is_full_screen)
            {
...
#else
        CTRLMENU_SetStatePtr(menu_ctrl_ptr, CTRLMENU_STATE_DRAW_SPLIT_LINE, FALSE);
#endif
#endif
    }
                                 ...
                                 )
{
    ...
    case MSG_APP_OK:
        {
            uint32 lsk_id = 0;
            GUIWIN_GetSoftkeyId(menu_ctrl_ptr->win_handle, &lsk_id, PNULL, PNULL);
            if(MMI_IsTextNull((MMI_TEXT_ID_T)lsk_id) || MMI_IsImageNull((MMI_IMAGE_ID_T)lsk_id))
            {
                break;
            }
        }
    ...
    case MSG_CTL_UPDATE_COLOR_THEME:
        ...
        if(GUIMENU_STYLE_POPUP == menu_ctrl_ptr->cur_style ||
           ...
           GUIMENU_STYLE_POPUP_AUTO == menu_ctrl_ptr->cur_style ||
           GUIMENU_STYLE_POPUP_RECT == menu_ctrl_ptr->cur_style)
        {
            result = CTRLMENU_PopHandleColorTheme(menu_ctrl_ptr);
        }
#endif
        break;
                         ...
                         )
{
    ...
    if (CTRLMENU_STACK_MAX_LEVEL > menu_ctrl_ptr->menu_level)
    {
        ...
        menu_ctrl_ptr->menu_stack[menu_level].text_len         = menu_ctrl_ptr->text_len;
        menu_ctrl_ptr->menu_stack[menu_level].text_height      = menu_ctrl_ptr->text_height;
                        ...
                        )
{
    ...
    if (0 < menu_ctrl_ptr->menu_level)
    {
        ...
        menu_ctrl_ptr->text_len         = menu_ctrl_ptr->menu_stack[menu_level].text_len;
        menu_ctrl_ptr->text_height      = menu_ctrl_ptr->menu_stack[menu_level].text_height;
    }
    else
    {
...
#endif
        {
            ...
            if (MMITHEME_Is3DOpen())
            {
                if (is_pop_menu)
                {
                    ...
                    MMITHEME_PopupMenuEffect(&(base_ctrl_ptr->rect), &pre_menu_rect, start_level + 1, FALSE);
                    MMIDEFAULT_FreePopMenuBgBuf();
                }
                else
                {
