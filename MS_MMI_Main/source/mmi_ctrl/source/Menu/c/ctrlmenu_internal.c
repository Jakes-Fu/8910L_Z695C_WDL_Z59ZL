...
//  Note: this method is for static menu.
/*****************************************************************************/
LOCAL CTRLMENU_ITEM_INFO_T* GetItemInfoByVisibleIndex(
                                            CTRLMENU_OBJ_T          *menu_ctrl_ptr,     //[in]
                                            uint16                  visible_index       //[in]
                                            );
...
/*****************************************************************************/
//  Description : get pop item state
//  Global resource dependence : 
//  Author: 
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN CTRLMENU_GetItemState(
                                      CTRLMENU_OBJ_T            *menu_ctrl_ptr, //[in]
                                      uint16                    item_index,     //[in]
                                      GUIMENU_ITEM_STATE_T      item_state      //[in]
                                      )
{
    BOOLEAN                result          = FALSE;
    ...
    if (PNULL != menu_ctrl_ptr)
    {
        ...
        {
            item_info_ptr = GetItemInfoByVisibleIndex(menu_ctrl_ptr, item_index);
            if(PNULL != item_info_ptr)
            {
                result = (BOOLEAN)((item_info_ptr->item_state & item_state) == item_state);
            }
        }
        else
        {
            ...
            {
                result = (BOOLEAN)((item_node_ptr->item_state & item_state) == item_state);
            }
        }
    }
...
/*****************************************************************************/
//  Description : set pop item state
//  Global resource dependence : 
//  Author: 
//  Note: 
/*****************************************************************************/
PUBLIC void CTRLMENU_SetItemState(
                                      CTRLMENU_OBJ_T            *menu_ctrl_ptr, //[in]
                                      uint16                    item_index,     //[in]
                                      GUIMENU_ITEM_STATE_T      item_state,     //[in]
                                      BOOLEAN                   is_true
    ...
    if (PNULL != menu_ctrl_ptr)
    {
        if (menu_ctrl_ptr->is_static)
        {
            ...
            if(PNULL != item_info_ptr)
            {
                if(is_true)
                {
                    item_info_ptr->item_state |= item_state;
                }
				else
				{
				    item_info_ptr->item_state &= ~item_state;
        ...
        else
        {
            ...
            if (PNULL != item_node_ptr)
            {
                if(is_true)
                {
                    item_node_ptr->item_state |= item_state;
                }
				else
				{
				    item_node_ptr->item_state &= ~item_state;
                                            ...
                                            )
{
    ...
    uint16                  cur_visible_index = 0;
    CTRLMENU_ITEM_INFO_T    *item_info_ptr = PNULL;
	CTRLMENU_ITEM_INFO_T    *out_item_info_ptr = PNULL;
    GUIMENU_GROUP_T         *menu_group_ptr = PNULL;
    ...
    for (i = 0; i < item_count; i++)
    {
        if (item_info_ptr[i].is_visible)
        {
            if (visible_index == cur_visible_index)
            {
                out_item_info_ptr = (CTRLMENU_ITEM_INFO_T*)&item_info_ptr[i];
...
//  Note:
/*****************************************************************************/
PUBLIC CTRLMENU_NODE_T* CTRLMENU_GetNodeByNodeId(
                                                       uint32           node_id,         //[in]
                                                       CTRLMENU_NODE_T  *parent_node_ptr //[in]
                                                       )
{
    ...
    while (PNULL != find_node_ptr)
    {
        if (find_node_ptr->node_id == node_id)
        {
            break;
        }
...
//  Note: this method is for menu view
/*****************************************************************************/
PUBLIC BOOLEAN CTRLMENU_GetText(
                                CTRLMENU_OBJ_T     *menu_ctrl_ptr, //[in]
                                MMI_STRING_T       *text_str_ptr   //[out]
                                )
{
    if (PNULL == menu_ctrl_ptr)
    {
    ...
    if (menu_ctrl_ptr->is_static)
    {
        ...
        {
            if (PNULL != menu_ctrl_ptr->cur_group_info_ptr->text_str.wstr_ptr
                && 0 != menu_ctrl_ptr->cur_group_info_ptr->text_str.wstr_len)
            {
                *text_str_ptr = menu_ctrl_ptr->cur_group_info_ptr->text_str;
            }
            else if (0 < menu_ctrl_ptr->text_len)
            {
                text_str_ptr->wstr_len = menu_ctrl_ptr->text_len;
                text_str_ptr->wstr_ptr = menu_ctrl_ptr->text_wstr;
            }
            else
            {
                text_str_ptr->wstr_len = 0;
            }
        }
    }
    else
    {
        if (PNULL != text_str_ptr)
        {
            if (PNULL != menu_ctrl_ptr->cur_parent_node_ptr->text_str.wstr_ptr
                && 0 != menu_ctrl_ptr->cur_parent_node_ptr->text_str.wstr_len)
            {
                *text_str_ptr = menu_ctrl_ptr->cur_parent_node_ptr->text_str;
            }
            //if it is the first level menu
            else if (PNULL == menu_ctrl_ptr->cur_parent_node_ptr->parent_node_ptr)
            {
...
/*****************************************************************************/
//  Description : set pop menu softkey param
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC void CTRLMENU_SetPopSoftkey(
                          CTRLMENU_OBJ_T    *menu_ctrl_ptr
                          )
{
    BOOLEAN                 is_null_softkey = TRUE;
    uint16                  i = 0;
	uint32                  sk_id[GUISOFTKEY_BUTTON_NUM] = {0};
    uint32                  *softkey_id_ptr = PNULL;
    GUIMENU_ITEM_T          *item_ptr = PNULL;
    ...
    if (menu_ctrl_ptr->is_static)
    {
        ...
        if (PNULL != item_ptr)
        {
            softkey_id_ptr = item_ptr->button_id;
        }
    }
    else
    {
    ...
    {
        if(GUISK_DATA_IMAGE_ID == menu_ctrl_ptr->pop_theme.sk_type[i])
        {
            is_null_softkey = MMI_IsImageNull((MMI_IMAGE_ID_T)softkey_id_ptr[i]);
        }
		else
		{
		    is_null_softkey = MMI_IsTextNull((MMI_TEXT_ID_T)softkey_id_ptr[i]);
		}
		if(!is_null_softkey)
		{
		    break;
	...
	for (i = 0; i < GUISOFTKEY_BUTTON_NUM; i++)
	{
	    if(is_null_softkey)
	    {
	        //set default softkey id
	        sk_id[i] = menu_ctrl_ptr->pop_theme.sk_id[i];
	    }
		else
		{
		    //set softkey id defined by app
		    sk_id[i] = softkey_id_ptr[i];
		}

		if(GUISK_DATA_IMAGE_ID == menu_ctrl_ptr->pop_theme.sk_type[i])
		{
		    GUIWIN_SeSoftkeytButtonIconId(menu_ctrl_ptr->win_handle, (MMI_IMAGE_ID_T)sk_id[i], i, TRUE);
		}
		else
		{
		    GUIWIN_SeSoftkeytButtonTextId(menu_ctrl_ptr->win_handle, (MMI_TEXT_ID_T)sk_id[i], i, TRUE);
		}
	}

    if(CTRLMENU_GetItemState(menu_ctrl_ptr, menu_ctrl_ptr->cur_item_index, GUIMENU_ITEM_STATE_TOGGLE))
    {
        GUIWIN_SeSoftkeytButtonTextId(menu_ctrl_ptr->win_handle, menu_ctrl_ptr->pop_theme.toggle_msk_id, MIDDLE_BUTTON, TRUE);
    }
}
                          ...
                          )
{
    ...
    if (PNULL != button_id_ptr)
    {
        if (menu_ctrl_ptr->is_static)
        {
            ...
            if (PNULL != item_ptr)
            {
                softkey_id_ptr = (MMI_TEXT_ID_T *)item_ptr->button_id;
            }
        }
        else
        {
            softkey_id_ptr = (MMI_TEXT_ID_T *)menu_ctrl_ptr->dyna_softkey_id;
        }
                                     ...
                                     )
{
    ...
    if  (menu_ctrl_ptr->is_static && menu_ctrl_ptr->cur_style == GUIMENU_STYLE_POPUP_RADIO)
    {
        sel_item_ptr = menu_ctrl_ptr->selected_list.init_item_ptr;
        while(PNULL != sel_item_ptr)
        {
            if (sel_item_ptr->item_info.group_id == menu_ctrl_ptr->cur_group_id)
            {
                break;
            }
            sel_item_ptr = sel_item_ptr->next_ptr;
        }
        if (sel_item_ptr && (uint16)sel_item_ptr->item_info.selected_index < menu_ctrl_ptr->cur_group_ptr->item_count)
        {
            menu_ctrl_ptr->cur_item_index = sel_item_ptr->item_info.selected_index;
        }
   }
