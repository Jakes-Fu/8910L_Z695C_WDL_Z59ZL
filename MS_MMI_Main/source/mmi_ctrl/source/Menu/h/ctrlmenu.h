...
#define CTRLMENU_TEXT_MAX_LEN       100 //text max length
...
typedef struct
{
    ...
    GUIMENU_ITEM_STATE_T   item_state;        // 菜单项状态，每1 bit对应一种状态
...
typedef struct _menu_obj_tag
{
    ...
    BOOLEAN                     is_full_screen; //是否为全屏的pop menu
    ...
    wchar                       text_wstr[CTRLMENU_TEXT_MAX_LEN + 1];
    uint16                      text_len;
    uint16                      text_height;
    ...
    uint32                      dyna_softkey_id[GUISOFTKEY_BUTTON_NUM]; //dynamic menu,softkey id(image/text)
...
/*****************************************************************************/
//  Description : handle pop menu color theme
//  Global resource dependence : 
//  Author:
//  Note:
/*****************************************************************************/
PUBLIC MMI_RESULT_E CTRLMENU_PopHandleColorTheme(
                                             CTRLMENU_OBJ_T     *menu_ctrl_ptr
                                             );
