...
#define SUBTEXT_LEFT_OFFSET           62 
#define SUBTEXT_RIGHT_OFFSET          2

//boda,128x160
#if defined (MAINLCD_SIZE_240X320)
#define TEXT_TOP_OFFSET                  2
#define TEXT_BOTTOM_OFFSET            0
#else
#define TEXT_TOP_OFFSET                  2
#define TEXT_BOTTOM_OFFSET            2
#endif 

#define SUBTEXT_MAX_L_MARGIN          75

#define SUBTEXT_BOTTOM_OFFSET         2
                                         ...
                                         )
{
    ...
    display_width = GetTextRectWidth(title_ptr);
    // if title string length larger then title rect width , need to circular display.
    if (pixel_length > display_width) 
    {
        title_ptr->is_text_overstep = TRUE;
        //disappeared from left byte by byte
        if (TITLE_SCROLL_LEFT_OUT == title_ptr->scroll_type)    
        { 
            title_ptr->scroll_pixel_offset = GUI_CalculateStringScrollStartPos(pixel_length,display_width,
                                title_ptr->scroll_pixel_step,
                                title_ptr->scroll_pixel_offset);
        }
...
//                 &title_ptr->lcd_dev_info);
			   ...
			   title_ptr->bg_image = MMITHEME_GetCommonBg().img_id;
#endif
