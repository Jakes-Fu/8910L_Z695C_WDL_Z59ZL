...
#define CTRL_BLACK_COLOR        0x0000
#define CTRL_WHITE_COLOR        0xFFFF
...
/*---------------------------------------------------------------------------*/
LOCAL uint16 button_x_margin		= 0;
LOCAL uint16 button_y_margin 		= 0; 
...
/*****************************************************************************/
//  Description : Create and append button text label layer
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN CreateAndAppendTransparentLayer(
                                              CTRLBUTTON_OBJ_T      *button_ctrl_ptr
                                              );

/*****************************************************************************/
//  Description : Create button text label layer
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN CreateTransparentLayer(
                                     CTRLBUTTON_OBJ_T      *button_ctrl_ptr,
                                     GUI_RECT_T            *text_dis_rect
                                    );

/*****************************************************************************/
//  Description : Release button text label layer
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL void ReleaseTransparentLayer( 
                                CTRLBUTTON_OBJ_T     *button_ctrl_ptr
                                );

/*****************************************************************************/
//  Description : display rect button 
//  Global resource dependence : 
//  Author: Jassmine
//  Note:
/*****************************************************************************/
LOCAL void RectButtonDraw(
                        CTRLBUTTON_OBJ_T           *button_ctrl_ptr
                       );
                               ...
                               )
{
    ...
    GUI_RECT_T              button_rect = {0};    
 	...
 	//Set rect button layout
	if (TRUE == button_ctrl_ptr->is_rect_button)
	{
	    button_rect.top    = button_y_margin;
	    button_rect.bottom = button_rect.top + RECT_BUTTON_HEIGHT;
	    button_rect.left   = button_x_margin;
	    button_rect.right  = button_rect.left + RECT_BUTTON_WIDTH;

	    base_ctrl_ptr->rect         = button_rect;
	    base_ctrl_ptr->display_rect = button_rect;
	    GUI_SetVOrHRect( base_ctrl_ptr->handle, (GUI_RECT_T *)&button_rect, &(base_ctrl_ptr->both_rect));
	}
	...
	if (TRUE == button_ctrl_ptr->is_rect_button)
	{
        button_ctrl_ptr->theme.font.color = CTRL_WHITE_COLOR;
		button_ctrl_ptr->label_bg_color = CTRL_BLACK_COLOR;

		//Create and append rect button layer 
		//CreateAndAppendTransparentLayer(button_ctrl_ptr);
	}
                              ...
                              )
{
	...
	if (TRUE == button_ctrl_ptr->is_rect_button) 
	{
    	//ReleaseTransparentLayer(button_ctrl_ptr);
	}
                                    ...
                                    )
{
    ...
    case MSG_CTL_GET_ACTIVE:
        ...
        CTRLBUTTON_SetButtonFocused(base_ctrl_ptr->ctrl_id, TRUE, FALSE);
    ...
    case MSG_CTL_LOSE_ACTIVE:
       ...
       CTRLBUTTON_SetButtonFocused(base_ctrl_ptr->ctrl_id, FALSE, FALSE);
    ...
    case MSG_CTL_UPDATE_COLOR_THEME:
        if(TRUE == button_ctrl_ptr->is_focused)
        {
            button_ctrl_ptr->label_bg_color = MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_DEFAULT);
             
        }
        else 
        {
            button_ctrl_ptr->label_bg_color = MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_NEG_3);
        }
        break;        
                     ...
                     )
{
    ...
    if (TRUE == button_ctrl_ptr->is_rect_button)
	{
    	RectButtonDraw(button_ctrl_ptr);
	}
    else 
    {
	    if ((VTLBASE_GetVisible((CTRLBASE_OBJ_T*)button_ctrl_ptr)) &&
                       ...
                       )
{
    ...
    if ((VTLBASE_GetVisible((CTRLBASE_OBJ_T*)button_ctrl_ptr)) &&
        (GUI_IntersectRect(&cross_rect,base_ctrl_ptr->display_rect,base_ctrl_ptr->rect)))
    {        
        //Set tranparent layer for text lebel
        /*
        UILAYER_RemoveBltLayer(&(button_ctrl_ptr->transparent_layer_info));
        append_layer_info.layer_level = UILAYER_LEVEL_NORMAL;
        append_layer_info.lcd_dev_info = button_ctrl_ptr->transparent_layer_info;
        UILAYER_AppendBltLayer(&append_layer_info);
        UILAYER_SetLayerPosition(&(button_ctrl_ptr->transparent_layer_info),base_ctrl_ptr->display_rect.left,base_ctrl_ptr->display_rect.top);
        UILAYER_SetLayerColorKey(&(button_ctrl_ptr->transparent_layer_info), TRUE, UILAYER_TRANSPARENT_COLOR);
        UILAYER_Clear(&(button_ctrl_ptr->transparent_layer_info));*/
        ...
        display_rect = base_ctrl_ptr->rect;

        // Modified by sunhongzhe to fix PR that text  is not in the center.
        //display_rect.top = base_ctrl_ptr->rect.top;
        //display_rect.bottom =  display_rect.top + bord_margin; 
        //display_rect.bottom =  display_rect.top + RECT_BUTTON_BORDER;        
        //GUI_FillRect((const GUI_LCD_DEV_INFO*) &base_ctrl_ptr->lcd_dev_info, display_rect, 0x0000);

        //display_rect.top = display_rect.bottom;
        //display_rect.bottom = display_rect.top + 34;  //label height
        content_rect = display_rect;
        GUI_FillRect((const GUI_LCD_DEV_INFO*) &base_ctrl_ptr->lcd_dev_info, display_rect, button_ctrl_ptr->label_bg_color);
        //GUI_FillRect((const GUI_LCD_DEV_INFO*) &button_ctrl_ptr->transparent_layer_info, display_rect, button_ctrl_ptr->label_bg_color);
        ...
        //Display image and label text
        button_rect = base_ctrl_ptr->rect;
        img_rect = button_rect;        
        text_rect = button_rect;
        //text_rect.top = button_rect.top + 6;        
        //text_rect.bottom = button_rect.bottom - 6;

        if (NULL == button_ctrl_ptr->theme.icon_id)
        {
            //text_rect.top = button_rect.top + TEXT_LABEL_TOP_MARGIN;        
            //text_rect.bottom = text_rect.top + TEXT_LABEL_HEIGHT;       
            text_rect.left = button_rect.left + TEXT_LABEL_LEFT_MARGIN;    
            text_rect.right = text_rect.left + TEXT_LABEL_WIDTH;     

        }
        else
        {
            str_width = GUI_CalculateStringPiexlNum(button_str.wstr_ptr,
                            button_str.wstr_len,
                            button_ctrl_ptr->theme.font.font,
                            0);
            str_height = GUI_GetStringHeight(button_ctrl_ptr->theme.font.font,button_str.wstr_ptr,button_str.wstr_len);      
            
            GUIRES_GetImgWidthHeight(&img_width,&img_height,button_ctrl_ptr->theme.icon_id,button_ctrl_ptr->win_handle);

            //Calculate image rect
            img_rect.left = content_rect.left + (uint16)((content_rect.right - content_rect.left)-(str_width + 6 + img_width))/2;        
            img_rect.right = content_rect.right;
            margin = (content_rect.bottom - content_rect.top + 1 - img_height)/2;
            img_rect.top = ((margin >=0) ?content_rect.top + margin : content_rect.top);
            img_rect.bottom = ((margin >=0) ?content_rect.bottom - margin :content_rect.bottom);           

            //Calculate text rect 
            //margin = ((content_rect.bottom - content_rect.top )- str_height)/2;
            //text_rect.top = ((margin >=0) ?content_rect.top + margin-1 :content_rect.top);        
            //text_rect.top = ((margin >=0) ?content_rect.top + margin :content_rect.top);        
            //text_rect.bottom =((margin >=0) ?content_rect.bottom - margin :content_rect.bottom);    
            text_rect.left = img_rect.left + img_width + ICON_LABEL_TEXT_ICON_SPACE;        
            text_rect.right = img_rect.right;     
        ...
        if (PNULL != button_ctrl_ptr && 0 < button_str.wstr_len)
        {
            ...
            str_style.font_color = 0xffff;
            str_state = GUISTR_STATE_ALIGN|GUISTR_STATE_SINGLE_LINE|GUISTR_STATE_ELLIPSIS;
#ifdef UI_MULTILAYER_SUPPORT	
            if(TRUE == button_ctrl_ptr->is_gray)
            {
                UILAYER_SetLayerAlpha((const GUI_LCD_DEV_INFO*)&base_ctrl_ptr->lcd_dev_info, 89);
            }
            else 
            {
                UILAYER_SetLayerAlpha((const GUI_LCD_DEV_INFO*)&base_ctrl_ptr->lcd_dev_info, 255);
            }                      
#endif
...
/*****************************************************************************/
//  Description : Set button location
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLBUTTON_SetLocation(
                                     MMI_CTRL_ID_T   ctrl_id,    
                                     uint8           x_position,
                                     uint8           y_position
                                     )
{
    ...
    if (PNULL != button_ctrl_ptr)
    {    
        button_x_margin = x_position;
        button_y_margin = y_position;
...
/*****************************************************************************/
//  Description : Set is rect button
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLBUTTON_SetIsRectButton(
                                        MMI_CTRL_ID_T   ctrl_id,    //in:
                                        BOOLEAN         is_ms_button
                                        )
{
    ...
    button_ctrl_ptr->is_rect_button = TRUE;    
...
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLBUTTON_SetButtonIcon(
                                        MMI_CTRL_ID_T   ctrl_id,    
                                        ...
                                        )
{
    ...
    if (PNULL != button_ctrl_ptr)
    {
        button_ctrl_ptr->theme.icon_id = icon_id;
		result = TRUE;
    }
...
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLBUTTON_SetButtonValid(
                                         MMI_CTRL_ID_T    ctrl_id,    
                                         BOOLEAN          is_gray,    
                                         GUI_BG_T         *gray_ptr,  
                                         BOOLEAN          is_update
                                         )
{
    ...
    if (PNULL != button_ctrl_ptr)
    {
        ...
        if (is_update)
        {
            MMK_SendMsg(button_ctrl_ptr->win_handle,MSG_FULL_PAINT,PNULL);
        }
        result = TRUE;
    }
...
/*****************************************************************************/
//  Description : set button focused
//  Global resource dependence : 
//  Author: Jassmine
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLBUTTON_SetButtonFocused(
                                         MMI_CTRL_ID_T    ctrl_id,    
                                         BOOLEAN          is_button_focused,    
                                         BOOLEAN          is_update
                                         )
{
    BOOLEAN                 result = FALSE;
    ...
    {
        //set gray
        button_ctrl_ptr->is_focused = is_button_focused;
                                              ...
                                              )
{
    ...
    CTRLBASE_OBJ_T      *base_ctrl_ptr = (CTRLBASE_OBJ_T*)button_ctrl_ptr;    
#ifdef UI_MULTILAYER_SUPPORT
    GUI_RECT_T transparent_layer_rect = {0};
    UILAYER_APPEND_BLT_T append_layer_info = {0};
#endif
...
#ifdef UI_MULTILAYER_SUPPORT
    transparent_layer_rect.top = base_ctrl_ptr->rect.top;
    transparent_layer_rect.left = base_ctrl_ptr->rect.left;
    transparent_layer_rect.bottom = base_ctrl_ptr->rect.bottom;
    transparent_layer_rect.right = base_ctrl_ptr->rect.right;

    if (GUI_IsInvalidRect(transparent_layer_rect))
    {
        return result;
    }   

    result = CreateTransparentLayer(button_ctrl_ptr, &transparent_layer_rect);

    if (TRUE == button_ctrl_ptr->is_gray)
        UILAYER_SetLayerAlpha((const GUI_LCD_DEV_INFO*)&button_ctrl_ptr->transparent_layer_info,166);
    else
        UILAYER_SetLayerAlpha((const GUI_LCD_DEV_INFO*)&button_ctrl_ptr->transparent_layer_info,255);
                                    ...
                                    )
{
    ...
    if ((PNULL == button_ctrl_ptr)||(PNULL == text_dis_rect))
    {
        return result;
    }
#ifdef UI_MULTILAYER_SUPPORT
    if(UILAYER_IsMultiLayerEnable())
    {
        ...
        create_info.lcd_id       = base_ctrl_ptr->lcd_dev_info.lcd_id;
        create_info.owner_handle = base_ctrl_ptr->handle;
        create_info.offset_x     = text_dis_rect->left;
        create_info.offset_y     = text_dis_rect->top;
        create_info.width        = text_dis_rect->right - text_dis_rect->left + 1;
        create_info.height       = text_dis_rect->bottom - text_dis_rect->top + 1;

        //create pop layer
        if(UILAYER_RESULT_SUCCESS == UILAYER_CreateLayer(&create_info, &(button_ctrl_ptr->transparent_layer_info)))
        {
            UILAYER_SetLayerColorKey(&(button_ctrl_ptr->transparent_layer_info), TRUE, UILAYER_TRANSPARENT_COLOR);
            UILAYER_Clear(&(button_ctrl_ptr->transparent_layer_info));
            result = TRUE;
                                ...
                                )
{
    ...
    if (UILAYER_IsMultiLayerEnable())
    {
        UILAYER_RELEASELAYER(&(button_ctrl_ptr->transparent_layer_info));
        button_ctrl_ptr->transparent_layer_info.block_id = GUI_BLOCK_MAIN;
    }
#endif
}
