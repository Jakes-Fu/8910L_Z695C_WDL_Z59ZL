 ...
 **                         MACRO DEFINITION                                 *
 **--------------------------------------------------------------------------*/
 #define ICONLIST_ITEM_TIME_STR_LEN      20
// iconlist type info
...
typedef struct
{
    wchar   wstr_ptr[ICONLIST_ITEM_TIME_STR_LEN];
    uint16   wstr_len;
} GUIICONLIST_STAMP_ITEM_T;

//item info
typedef struct
{
    BOOLEAN             	is_selected;    //is selected
    BOOLEAN             	is_need_refresh_time_stamp; //is need refresh
    BOOLEAN					is_display_time_stamp;    //is display time stamp 
    //MMI_STRING_T			time_stamp_str;       //time stamp str
    GUIICONLIST_STAMP_ITEM_T time_stamp_str;
    GUIICONLIST_TEXT_T  	text;           //text
    GUIICONLIST_ICON_T  	icon;           //icon
    GUIICONLIST_TEXT_T  	text_addtion;   //addtion text
    MMI_IMAGE_ID_T            ind_info_img_id;
} GUIICONLIST_ITEM_T;
...
typedef struct
{
    ...
    uint16                  	update_num;         //number of current need update item
	...
	BOOLEAN                     is_iconlist_maskable;  //icon 是否设置mask
} CTRLICONLIST_OBJ_T;
