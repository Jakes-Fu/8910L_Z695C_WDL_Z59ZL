...
#define SELECT_FLAG_WIDTH           	    6   	//select rect width

#define MIN_ITEM_MEM_ALLOC_NUM      20      //动态分配item空间时，最小的一次分配单位，item个数

#define ICONLIST_ITEM_TIME_STR_HEIGHT   24
#define GUI_CTRLLIST_BLACK_COLOR		0x0000
#define GUI_CTRLLIST_BLUE_COLOR			0x07ff
/**--------------------------------------------------------------------------*
 **                         STATIC DEFINITION                                *
 **--------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*/
PUBLIC BOOLEAN ChangedIndex( BOOLEAN flag);
PUBLIC BOOLEAN GetChangedFlag();
LOCAL void AllocItemMemoryEx(
                           CTRLICONLIST_OBJ_T   *iconlist_ctrl_ptr
                           );
LOCAL void SendMsgAppendIconEx(
                             uint16                 index,              //in
...
/*****************************************************************************/
//  Description : alloc next page item time stamp memory
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL void AllocNextPageItemTimeStampMemory( 
								   uint16              next_top_index,
								   uint16              next_bottom_index,
								   CTRLICONLIST_OBJ_T  *iconlist_ctrl_ptr
                                    );
...
//  Note:
/*****************************************************************************/
LOCAL void DisplayIconListMaskImg(
                           uint16               index,              //in:
                           CTRLICONLIST_OBJ_T   *iconlist_ctrl_ptr  //in:
                           );

/*****************************************************************************/
//  Description : display time stamp
//  Global resource dependence : 
//  Author: jinju.ma
//  Note:
/*****************************************************************************/
LOCAL void DisplayTimeStamp(
                           uint16               index,              //in:
                           CTRLICONLIST_OBJ_T   *iconlist_ctrl_ptr  //in:
                           );
...
//  Note:
/*****************************************************************************/
LOCAL void DisplayInfoIndicator(
                           uint16               index,              //in:
                           CTRLICONLIST_OBJ_T   *iconlist_ctrl_ptr  //in:
                           );
...
)
{
	...
	case MSG_CTL_ANIM_UPDATE_END:
		...
		if(iconlist_ctrl_ptr->item_ptr[iconlist_ctrl_ptr->cur_index].is_display_time_stamp
			&& iconlist_ctrl_ptr->item_ptr[iconlist_ctrl_ptr->cur_index].is_need_refresh_time_stamp
			)
		{
			DisplayTimeStamp(iconlist_ctrl_ptr->cur_index,iconlist_ctrl_ptr);
			iconlist_ctrl_ptr->item_ptr[iconlist_ctrl_ptr->cur_index].is_need_refresh_time_stamp = FALSE;
			DisplayIconListMaskImg(iconlist_ctrl_ptr->cur_index,iconlist_ctrl_ptr);
			break;
		}

		//refresh stamp
		iconlist_ctrl_ptr->update_num += 1;
		cur_page_icon_num = iconlist_ctrl_ptr->bottom_index - iconlist_ctrl_ptr->top_index + 1;
//		if(iconlist_ctrl_ptr->update_num == cur_page_icon_num)
		{
			iconlist_ctrl_ptr->update_num = 0;

			for (i=iconlist_ctrl_ptr->top_index; i<=iconlist_ctrl_ptr->bottom_index; i++)
			{		
				//display time stamp
				if (iconlist_ctrl_ptr->item_ptr[i].is_display_time_stamp
					&& !(iconlist_ctrl_ptr->item_ptr[i].is_selected)
					)
				{
					DisplayTimeStamp(i,iconlist_ctrl_ptr);
					DisplayIconListMaskImg(i,iconlist_ctrl_ptr);
				}
			}
		}
		break;
                        ...
                        )
{
	...
	{		
		...
		{
			iconlist_ctrl_ptr->col_num_page = 0;
		}
		if (item_width != 0 && (display_width % item_width) >= ICONLIST_GetItemWidth(FALSE,iconlist_ctrl_ptr))
		{
		...
		{
			iconlist_ctrl_ptr->row_num_page = 0;
		}
		iconlist_ctrl_ptr->row_max_num_page = iconlist_ctrl_ptr->row_num_page;
		
		if (item_height != 0 && 0 < (display_height % item_height))
		{
                           ...
                           )
{
	...
	if (PNULL != iconlist_ctrl_ptr)
	{
		if (0 < iconlist_ctrl_ptr->total_num)
		{
			...
			{
#if 0
				iconlist_ctrl_ptr->item_ptr[i].time_stamp_str.wstr_len = ICONLIST_ITEM_TIME_STR_LEN;
				iconlist_ctrl_ptr->item_ptr[i].time_stamp_str.wstr_ptr = SCI_ALLOC_APP(ICONLIST_ITEM_TIME_STR_LEN * sizeof(wchar));
				SCI_MEMSET(iconlist_ctrl_ptr->item_ptr[i].time_stamp_str.wstr_ptr,0,(ICONLIST_ITEM_TIME_STR_LEN * sizeof(wchar)));
#endif
				iconlist_ctrl_ptr->item_ptr[i].time_stamp_str.wstr_len = ICONLIST_ITEM_TIME_STR_LEN;
			}       
		}
                             ...
                             )
{
	...
	{		
        ...
        #if 0 
        // for mem leak pool
		if (PNULL != item_ptr->time_stamp_str.wstr_ptr)
		{
			SCI_FREE(item_ptr->time_stamp_str.wstr_ptr);
			item_ptr->time_stamp_str.wstr_len = 0;
		}
        #endif
	}
}
                                    ...
                                    )
{
    ...
    //added by Tong Yongli. according to Coverity CID 11372
    if(PNULL == iconlist_ctrl_ptr)
    {
        return;
    }
    ...
    //modified by Tong Yongli. according to Coverity CID 11372
    if (PNULL != iconlist_ctrl_ptr->item_ptr)
    {
...
{
	uint16 i = 0;
	uint16 num = next_bottom_index - next_top_index+1;
	if (PNULL != iconlist_ctrl_ptr)
	{
		if (0 < num)
		{
#if 0
			for(i = next_top_index; i <= next_bottom_index; i++)
			{	
				if (PNULL == iconlist_ctrl_ptr->item_ptr[i].time_stamp_str.wstr_ptr)
				{	
					iconlist_ctrl_ptr->item_ptr[i].time_stamp_str.wstr_len = ICONLIST_ITEM_TIME_STR_LEN;
					iconlist_ctrl_ptr->item_ptr[i].time_stamp_str.wstr_ptr = SCI_ALLOC_APP(ICONLIST_ITEM_TIME_STR_LEN * sizeof(wchar));
					SCI_MEMSET(iconlist_ctrl_ptr->item_ptr[i].time_stamp_str.wstr_ptr,0,(ICONLIST_ITEM_TIME_STR_LEN * sizeof(wchar)));	
                           ...
                           )
{
	...
	if (PNULL != iconlist_ctrl_ptr)
	{
		...
		//display time stamp
		if (iconlist_ctrl_ptr->item_ptr[index].is_display_time_stamp
			&& !(iconlist_ctrl_ptr->item_ptr[index].is_selected)
			)
		{
			DisplayTimeStamp(index, iconlist_ctrl_ptr);
			DisplayIconListMaskImg(index,iconlist_ctrl_ptr);
		}
...
#endif    
    /**Microsoft project not need to support scrollbar++**/
    BOOLEAN result = FALSE;
    /**Microsoft project not need to support scrollbar--**/
                               ...
                               )
{
		...
		(index == iconlist_ctrl_ptr->cur_index))
    {
        ...
        if (IsDisplaySelectedFlag(iconlist_ctrl_ptr))
        {
            ...
            GUI_DisplayBorder(icon_rect,//icon_rect,
                base_ctrl_ptr->display_rect,
...
/*****************************************************************************/
//  Description : get if cur icon display time stamp
//  Global resource dependence : 
//  Author: Jinju.ma
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLICONLIST_GetDisplayTimeStampFlag(
										...
										)
{
    ...
    return iconlist_ctrl_ptr->item_ptr[index].is_display_time_stamp;
}

/*****************************************************************************/
//  Description : set if cur icon display time stamp
//  Global resource dependence : 
//  Author: Jinju.ma
//  Note:
/*****************************************************************************/
PUBLIC void CTRLICONLIST_SetDisplayTimeStampFlag(
										MMI_CTRL_ID_T     ctrl_id,
										uint16 	 index,
										BOOLEAN  is_display
										)
{
    ...
    iconlist_ctrl_ptr->item_ptr[index].is_display_time_stamp= is_display;
}

/*****************************************************************************/
//  Description : 设置当前索引item的time字符串
//  Global resource dependence : 
//  Author: Jinju.ma
//  Note:
/*****************************************************************************/
PUBLIC void CTRLICONLIST_SetTimeStr(
										MMI_CTRL_ID_T     ctrl_id,
										uint16 	 index,
										...
										)
{
    ...
    iconlist_ctrl_ptr = ICONLIST_GetPtr(ctrl_id);	
	if (PNULL == iconlist_ctrl_ptr || PNULL == time_str.wstr_ptr || 0 == time_str.wstr_len)
	{
		return;
	}
	
	MMIAPICOM_Wstrncpy(iconlist_ctrl_ptr->item_ptr[index].time_stamp_str.wstr_ptr,
        time_str.wstr_ptr, 
		MIN(time_str.wstr_len, iconlist_ctrl_ptr->item_ptr[index].time_stamp_str.wstr_len)
		);
}
                              ...
                              )
{
	...
	if (PNULL != iconlist_ctrl_ptr)
	{
			...
			(iconlist_ctrl_ptr->is_loading_icon))
		{
			...
			//don't need load, refresh time stamp
			if (!result && (MMK_IsFocusWin(iconlist_ctrl_ptr->win_id)))
			{
                           ...
                           )
{
	...
	if(iconlist_ctrl_ptr->item_ptr[iconlist_ctrl_ptr->cur_index].is_display_time_stamp
		&& !is_selected)
	{
		iconlist_ctrl_ptr->item_ptr[iconlist_ctrl_ptr->cur_index].is_need_refresh_time_stamp = TRUE;
	}
    ...
    else
    {
	 ...
	 DisplayInfoIndicator(iconlist_ctrl_ptr->cur_index,iconlist_ctrl_ptr);		
    }
                             ...
                             )
{
...
#endif
	{
		...
		//get top line index
		if (min_position > margin_space&&item_distance != 0)
		{
    ...
    case GUIICONLIST_LOAD_ONE_BY_ONE:
        ...
        (pre_bottom_index != iconlist_ctrl_ptr->bottom_index))
        {
			...
			AllocNextPageItemTimeStampMemory(iconlist_ctrl_ptr->top_index,iconlist_ctrl_ptr->bottom_index,iconlist_ctrl_ptr);
        }
        break;
                      ...
                      )
{
    ...
    //CTRLANIM_SetMiniWidthAndHeightByPtr(anim_ctrl_ptr,iconlist_ctrl_ptr->theme.icon_item.width,iconlist_ctrl_ptr->theme.icon_item.height);
                            ...
                            )
{
    ...
    if (GetOperateAnim(&index,owner_draw_ptr->ctrl_handle,iconlist_ctrl_ptr))
    {
        ...
        if (index < iconlist_ctrl_ptr->total_num)
        {
			...
			DisplayIconListMaskImg(index,iconlist_ctrl_ptr);
            DisplayMarkIcon(index,iconlist_ctrl_ptr);     
			DisplayInfoIndicator(index,iconlist_ctrl_ptr);     		
        }        
    }
}
                           ...
                           )
{
    ...
    if (iconlist_ctrl_ptr->item_ptr[index].is_selected)
    {
		...
		GUIRES_GetImgWidthHeight(&img_width,
						&img_height,
						iconlist_ctrl_ptr->theme.mark_img_id,
						iconlist_ctrl_ptr->win_id);
		
		//set display rect
		icon_width = icon_rect.right - icon_rect.left + 1;
		icon_height = icon_rect.bottom - icon_rect.top + 1;
		display_rect.left   = (int16)(icon_rect.left + (icon_width - img_width) / 2);
		display_rect.right  = (int16)(display_rect.left + img_width - 1);
		display_rect.top    = (int16)(icon_rect.top + (icon_height - img_height) / 2);
                           ...
                           )
{
    ...
    if (iconlist_ctrl_ptr->is_iconlist_maskable)
    {
		if (iconlist_ctrl_ptr->item_ptr[index].is_selected)
		{
			return;
		...
		GUIRES_GetImgWidthHeight(&img_width,
			&img_height,
			iconlist_ctrl_ptr->theme.iconlist_mask_img_id,
			iconlist_ctrl_ptr->win_id);
...
{
    uint16          bg_height 		= ICONLIST_ITEM_TIME_STR_HEIGHT;
    GUI_RECT_T      icon_rect 		= {0};
    GUI_RECT_T      bg_rect 		= {0};
    GUI_RECT_T      cross_rect 		= {0};
    GUI_RECT_T      time_stamp_rect = {0};
    CTRLBASE_OBJ_T  *base_ctrl_ptr 	= PNULL;
    GUIICONLIST_ITEM_T *item_ptr 	= PNULL;
    GUISTR_STYLE_T  str_style       = {0};
    GUISTR_STATE_T  str_state   	= GUISTR_STATE_ALIGN|GUISTR_STATE_SINGLE_LINE;
    MMI_STRING_T   time_stamp_str   = {0};
    ...
    item_ptr = iconlist_ctrl_ptr->item_ptr + index;
    if (PNULL == item_ptr || PNULL == item_ptr->time_stamp_str.wstr_ptr)
    {
        return;
    }
    ...
    if (iconlist_ctrl_ptr->item_ptr[index].is_display_time_stamp)
    {
        ...
        //set bg display rect
        bg_rect.right   = (int16)(icon_rect.right - 2);
        bg_rect.left  = (int16)(icon_rect.left + 2);
        bg_rect.bottom = (int16)(icon_rect.bottom - 2);
        bg_rect.top    = (int16)(bg_rect.bottom - bg_height + 1);

        if (GUI_IntersectRect(&cross_rect,bg_rect,base_ctrl_ptr->display_rect))
        {
            //set time stamp rect
            time_stamp_rect.left   = (int16)(cross_rect.left - bg_rect.left);
            time_stamp_rect.top    = (int16)(cross_rect.top - bg_rect.top);
            time_stamp_rect.right  = (int16)(time_stamp_rect.left + cross_rect.right - cross_rect.left);
            time_stamp_rect.bottom = (int16)(time_stamp_rect.top + cross_rect.bottom - cross_rect.top);
	
            if (item_ptr->time_stamp_str.wstr_len != 0)
            {
                //display bg image
                LCD_FillArgbRect(&base_ctrl_ptr->lcd_dev_info,
								cross_rect,
								GUI_CTRLLIST_BLACK_COLOR,
								255 * 80 / 100
								);
                ...
                str_style.font_color = iconlist_ctrl_ptr->theme.font.color;
                str_style.align      = ALIGN_HVMIDDLE;
                time_stamp_str.wstr_ptr = item_ptr->time_stamp_str.wstr_ptr;
                time_stamp_str.wstr_len = item_ptr->time_stamp_str.wstr_len;
                GUISTR_DrawTextToLCDInRect((const GUI_LCD_DEV_INFO*)&base_ctrl_ptr->lcd_dev_info,
					(const GUI_RECT_T*)&cross_rect,
					(const GUI_RECT_T*)&cross_rect,
					&time_stamp_str,
                           ...
                           )
{
	...
	item_ptr = iconlist_ctrl_ptr->item_ptr + index;
	if (PNULL == item_ptr || 0 == item_ptr->ind_info_img_id)
	{
		return;
	}
    ...
    if (iconlist_ctrl_ptr->item_ptr[index].ind_info_img_id)
    {
        //get icon display rect
        icon_rect = ICONLIST_GetIconRect(index,iconlist_ctrl_ptr);
		...
		GUIRES_GetImgWidthHeight(&img_width,
						&img_height,
						iconlist_ctrl_ptr->item_ptr[index].ind_info_img_id,
						iconlist_ctrl_ptr->win_id);
		...
		display_rect.right  = (int16)(display_rect.left + img_width - 1);
		display_rect.top    = (int16)(icon_rect.top);
		display_rect.bottom = (int16)(display_rect.top + img_height - 1);
...
/*****************************************************************************/
//  Description : set icon list can maskable or not
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLICONLIST_SetIconMaskable(
										MMI_CTRL_ID_T    ctrl_id,
										BOOLEAN          is_iconlist_maskable
										)
{
    BOOLEAN             result			   = FALSE;
    CTRLICONLIST_OBJ_T  *iconlist_ctrl_ptr = PNULL;
    ...
    if (PNULL != iconlist_ctrl_ptr)
    { 
        iconlist_ctrl_ptr->is_iconlist_maskable = is_iconlist_maskable;	
        result = TRUE;
    }
    return (result);
}
...
/*****************************************************************************/
//  Description : set icon list mask image
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLICONLIST_SetIconMaskThemeImg(
										MMI_CTRL_ID_T    ctrl_id,
										...
										)
{
	...
	iconlist_ctrl_ptr = ICONLIST_GetPtr(ctrl_id);
	if (PNULL != iconlist_ctrl_ptr && PNULL != &image_id)
	{
		iconlist_ctrl_ptr->theme.iconlist_mask_img_id = image_id;
		result = TRUE;
	}
	return (result);
}
...
/*****************************************************************************/
//  Description : set icon list mark image
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLICONLIST_SetMarkThemeImg(
										MMI_CTRL_ID_T    ctrl_id,
										MMI_IMAGE_ID_T image_id
										)
{
	...
	if (PNULL != iconlist_ctrl_ptr && PNULL != &image_id)
	{
		iconlist_ctrl_ptr->theme.mark_img_id = image_id;
		result = TRUE;
	}
	return (result);
}
...
LOCAL BOOLEAN IconListCtrlAppendIcon( IGUIICONLIST_T * me_ptr, uint16 icon_index, void * data_ptr,  CAF_RESOURCE_SOURCE_E resource_source )
{
    ...
    if (PNULL != iconlist_ctrl_ptr);
    {
        ...
        { 
            ...
            SCI_MEMSET(sysfilename_ptr, 0x00, (file_info.full_path_wstr_len * 2) + 4);
...
/***************************************************************************//*!
//  Description : 封装对外接口---Set Indicate Icon
//  Global resource dependence : 
//  Author: wenny.cheng
//  Note:
*******************************************************************************/
PUBLIC BOOLEAN CTRLICONLIST_SetIndicateIcon(
                                           MMI_CTRL_ID_T    ctrl_id,
                                           uint16           index,
                                           MMI_IMAGE_ID_T          img_id)
{
    ...
    if ((PNULL != iconlist_ctrl_ptr) &&
        (index < iconlist_ctrl_ptr->total_num) ) 
    {
        iconlist_ctrl_ptr->item_ptr[index].ind_info_img_id = img_id;
        result = TRUE;
     }
								   ...
								   )
{
	...
	else 
	{
		...
		item_height = ICONLIST_GetItemHeight(TRUE,iconlist_ctrl_ptr);
                if(item_height != 0)
                {
		    line_index = group_height/item_height;
		    div_result = group_height%item_height;
                }
		//寻找在pos下的第一个index
		if (is_under_pos_first_index)
		{            
		...
		{		
			...
			uint16 ih =ICONLIST_GetItemHeight(TRUE,iconlist_ctrl_ptr);
			if (ih != 0 && 0 != group_height % ih)
			{
				if (0 == (line_index * iconlist_ctrl_ptr->col_num_page + tmp_ptr->start_item_index))
				{
					bottom_index = 0;
                           ...
                           )
{
	...
	if (PNULL != iconlist_ctrl_ptr)
	{
		if (0 < iconlist_ctrl_ptr->total_num)
		{
			if (PNULL != iconlist_ctrl_ptr->item_ptr)
			{
				iconlist_ctrl_ptr->item_ptr = SCI_REALLOC(iconlist_ctrl_ptr->item_ptr,iconlist_ctrl_ptr->total_num*sizeof(GUIICONLIST_ITEM_T));
			}
			else
			{
				iconlist_ctrl_ptr->item_ptr = SCI_REALLOC(iconlist_ctrl_ptr->item_ptr,iconlist_ctrl_ptr->total_num*sizeof(GUIICONLIST_ITEM_T));
            ...
            if (PNULL != iconlist_ctrl_ptr->item_ptr)
            {
				for(i = 0; i<iconlist_ctrl_ptr->total_num; i++)
				{
					if (0 == iconlist_ctrl_ptr->item_ptr[i].time_stamp_str.wstr_len)
					{
						iconlist_ctrl_ptr->item_ptr[i].time_stamp_str.wstr_len = ICONLIST_ITEM_TIME_STR_LEN;
					}			
				}
...
/***************************************************************************//*!
//  Description : 根据total_num 重新分配Icon的Memory
//  Global resource dependence : 
//  Author: yuguang.liu
//  Note:
*******************************************************************************/
PUBLIC void CTRLICONLIST_ReallocIconMemory(
                                     MMI_CTRL_ID_T    ctrl_id,  //in
                                     uint16           total_num //in
                                     )
{
    ...
    if (PNULL != iconlist_ctrl_ptr)
    {
        //alloc
		iconlist_ctrl_ptr->total_num = total_num;
        AllocItemMemoryEx(iconlist_ctrl_ptr);
		//set relative rect
		SetRelativeRect(iconlist_ctrl_ptr);
    }
}
...
/***************************************************************************//*!
//  Description : DispalyItemIcon
//  Global resource dependence : 
//  Author: yuguang.liu
//  Note:
*******************************************************************************/
PUBLIC void CTRLICONLIST_DispalyItemIcon(  
                           MMI_CTRL_ID_T   ctrl_id,	                    
                           uint16               index,
                           BOOLEAN              is_changed)
{
	...
	iconlist_ctrl_ptr = ICONLIST_GetPtr(ctrl_id);	
	if (PNULL != iconlist_ctrl_ptr && index < iconlist_ctrl_ptr->total_num)
    {
		if (is_changed)
		{
			//append icon
			SendMsgAppendIconEx(index,iconlist_ctrl_ptr,is_changed);
		}
    }
}

/***************************************************************************//*!
//  Description : 设置是否需要重新加载Icon
//  Global resource dependence : 
//  Author: yuguang.liu
//  Note:
*******************************************************************************/
PUBLIC void CTRLICONLIST_SetItemLoadIcon(
	MMI_CTRL_ID_T   ctrl_id,
	uint16               index,
	BOOLEAN is_load
	)
{
	...
	if (PNULL != iconlist_ctrl_ptr && index < iconlist_ctrl_ptr->total_num)
    {
		iconlist_ctrl_ptr->item_ptr[index].icon.is_load=is_load;
    }
}

/***************************************************************************//*!
//  Description : 获取IconList top index
//  Global resource dependence : 
//  Author:
//  Note:
*******************************************************************************/
PUBLIC uint16 CTRLICONLIST_TopIndex(
                                     MMI_CTRL_ID_T    ctrl_id
                                     )
{
    CTRLICONLIST_OBJ_T  *iconlist_ctrl_ptr = PNULL;
    ...
    {
       //alloc
        return iconlist_ctrl_ptr->top_index;
    }
	SCI_TRACE_LOW("ICONLIST:iconlist_ctrl_ptr==PNULL");
	return 0;
}

/***************************************************************************//*!
//  Description : 获取IconList bottom index
//  Global resource dependence : 
//  Author:
//  Note:
*******************************************************************************/
PUBLIC uint16 CTRLICONLIST_BottomIndex(
                                     MMI_CTRL_ID_T    ctrl_id
                                     )
{
    CTRLICONLIST_OBJ_T  *iconlist_ctrl_ptr = PNULL;
    ...
    {
       //alloc
        return iconlist_ctrl_ptr->bottom_index;
    }
	SCI_TRACE_LOW("ICONLIST:iconlist_ctrl_ptr==PNULL");
	return 0;
}
