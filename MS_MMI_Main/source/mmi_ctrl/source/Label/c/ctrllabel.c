...
)
{
    ...
    label_ctrl_ptr->original_align = label_param_ptr->align;	// add by lebao
...
)
{
    ...
    if (0 < label_ctrl_ptr->str_len)
    {
        ...
        if (GUI_IntersectRect (&cross_rect, base_ctrl_ptr->display_rect, str_rect))
        {
            ...
            {
				if ((GUILABEL_ALIGN_LEFT == label_ctrl_ptr->align) && MMIAP_IsMp3Tittle(label_ctrl_ptr->base_ctrl.ctrl_id))
				{
					label_ctrl_ptr->align =GUILABEL_ALIGN_MIDDLE;
				}
else
{
                label_ctrl_ptr->align = label_ctrl_ptr->original_align;	// 恢复原来的对齐方式, add by lebao
}
                str_state = str_state | GUISTR_STATE_ELLIPSIS;
            }
            
            if (GUILABEL_ALIGN_LEFT == label_ctrl_ptr->align || GUILABEL_ALIGN_FORCE_LEFT == label_ctrl_ptr->align)
            {
                str_state = str_state | GUISTR_STATE_ALIGN_BY_DIR;
            }
            if(GUILABEL_TEXT_TRANS == label_ctrl_ptr->text_effect)
            {
            	str_state = str_state | GUISTR_STATE_GRADUAL_VER | GUISTR_STATE_GRADUAL_HOR;
            }
			//set align
            ...
            if(GUILABEL_TEXT_TRANS == label_ctrl_ptr->text_effect)
            {
				str_style.begin_alpha = str_style.end_alpha = 89;//
            }
            //display string
			if(label_ctrl_ptr->text_effect)
			{
                //set label string
				...
				GUISTR_DrawSingleLineGradualInRect((const GUI_LCD_DEV_INFO*) &base_ctrl_ptr->lcd_dev_info,
            ...
            else if ( (is_scroll || label_ctrl_ptr->is_ticker) &&
                    (MMITHEME_GetIsScrollAsPixel()))
            {
                ...
                if (label_ctrl_ptr->is_ticker) //Label tiker for java tck
                {
                    GUI_RECT_T     disp_rect = {0};
                    //get label string len
                    uint16  str_width = GUI_CalculateStringPiexlNum ( label_ctrl_ptr->str_ptr,
                                                  (uint16) label_ctrl_ptr->str_len,
                                                  label_ctrl_ptr->theme.font.font,
                                                  (uint8) label_ctrl_ptr->theme.char_space);                    
                    if (cross_rect.right - label_ctrl_ptr->str_index - label_ctrl_ptr->theme.scroll_pixel + str_width > 0)
                    {
                        label_ctrl_ptr->str_index += label_ctrl_ptr->theme.scroll_pixel;
                    }
                    else
                    {
                        label_ctrl_ptr->str_index = 0;
                    ...
                    disp_rect.left = cross_rect.right - label_ctrl_ptr->str_index;
                    disp_rect.right = cross_rect.right;
                    disp_rect.top = cross_rect.top;
                    disp_rect.bottom= cross_rect.bottom;
            ...
            else
            {
                ...
                // [MS] should not pass DIR_AUTO to DisplayEngine if we need draw the current string
                // left-aligned, check the CUSTOMIZED align here and convert the dir to DIR_LTR.
                if (GUILABEL_ALIGN_FORCE_LEFT == label_ctrl_ptr->align)
                {
                    dir = GUISTR_TEXT_DIR_LTR;
                }
...
)
{
    ...
    case GUILABEL_ALIGN_FORCE_LEFT://[MS]
...
)
{
    ...
    //is active
    if (label_ctrl_ptr->is_active ||label_ctrl_ptr->is_scroll ||MMIAP_IsMp3Tittle(label_ctrl_ptr->base_ctrl.ctrl_id))
    {
        if (!label_ctrl_ptr->is_ticker)
        {
            //get string width
...
@param text_effect [in] 字体效果 :0:normal; 1:半透
@return 是否操作成功
@note 
*******************************************************************************/
PUBLIC BOOLEAN CTRLLABEL_SetFontEffect(
                                MMI_CTRL_ID_T   ctrl_id,    //in:
                                GUILABEL_TEXT_TYPE_E text_effect//in:
                                )
{
    BOOLEAN             result = FALSE;
    CTRLLABEL_OBJ_T		*label_ctrl_ptr = PNULL;
    ...
    {
        //set font
        label_ctrl_ptr->text_effect= text_effect;
...
/*****************************************************************************/
// 	Description : set is ticker for java tck
//	Global resource dependence :
//  Author: pan.yang
//	Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLLABEL_SetIsTicker(
    MMI_CTRL_ID_T    ctrl_id,
    BOOLEAN          is_ticker)
{
    BOOLEAN             result = FALSE;
    ...
    if (PNULL != label_ctrl_ptr)
    {
        label_ctrl_ptr->is_ticker = is_ticker;
        result = TRUE;
    }
...
/*****************************************************************************/
// 	Description : get label str rect
//	Global resource dependence :
//  Author: 
//	Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLLABEL_GetStrRect (
    MMI_CTRL_ID_T   ctrl_id,    //in:
    GUI_RECT_T      *rect_ptr   //in/out:
)
{
    ...
    if (PNULL != label_ctrl_ptr)
    {
        *rect_ptr = GetLabelStrRect(label_ctrl_ptr);
...
PUBLIC BOOLEAN CTRLLABEL_SetTextScroll(
    MMI_CTRL_ID_T    ctrl_id,
    BOOLEAN          is_scroll
)
{
    ...
    if (PNULL != label_ctrl_ptr)
    {
        label_ctrl_ptr->is_scroll = is_scroll;
