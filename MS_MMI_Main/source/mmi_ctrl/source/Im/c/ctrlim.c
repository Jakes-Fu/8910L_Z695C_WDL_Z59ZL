...
)
{
    ...
    //b_predictive = MMITHEME_IMCheckT9PredictveStatus();
    //if(!b_predictive)
    //    im_ptr->init_param.im_type =GUIIM_TYPE_ABC;

    //SCI_TRACE_LOW("[arvin] ImConstruct : b_predictive = %d, im_ptr->init_param.im_type = %d", b_predictive, im_ptr->init_param.im_type);
    ...
    if (GUIIM_LANG_NONE != im_ptr->method_param.input_lang)
    {
        ...
        SCI_TRACE_LOW("[arvin] ImConstruct : im_ptr->method_param.input_lang = %d", im_ptr->method_param.input_lang);
    }
    else
    {
        default_lang = MMITHEME_IMGetDefaultLang(im_ptr->method_param.limit);
        SCI_TRACE_LOW("[arvin] ImConstruct : default_lang = %d", default_lang);
    }

    //设置默认输入法
    //1 没有限制
    //2 没有指定输入语言
    //3 Init im type is smart
    if (0 == im_ptr->method_param.limit
        ...
        && GUIIM_TYPE_SMART == im_ptr->init_param.im_type)
    {
        default_method = MMITHEME_IMGetDefaultMethod();
        ResetParameter(im_ptr, default_method);

        im_ptr->is_default_method = 1;
        SCI_TRACE_LOW("[arvin] ImConstruct : default_method = %X", default_method);
    }
    else if (init_param_ptr->input_type & im_ptr->method_param.input_set)
    {
        ...
        if(PNULL != im_def_ptr)
        {
            SCI_TRACE_LOW("[arvin] ImConstruct : MMITHEME_IMGetDefBySet -- im_def_ptr->method = %x", im_def_ptr->method);
            if (MMITHEME_IMIsNotLimit(im_ptr->method_param.input_lang, im_ptr->method_param.limit, im_def_ptr))
            {
                ...
                SCI_TRACE_LOW("[arvin] ImConstruct : im_ptr->method_param.im_def_ptr = im_def_ptr");
            }
        }
        ...
        if (PNULL != im_ptr->method_param.im_def_ptr)
        {
            ...
            SCI_TRACE_LOW("[arvin] ImConstruct : im_ptr->method_param.im_set.method = %x", im_ptr->method_param.im_def_ptr->method);
        }
    }
    ...
    if (PNULL == im_ptr->method_param.im_def_ptr)
    {
        ...
        SCI_TRACE_LOW("[arvin] ImConstruct : MMITHEME_IMGetDefByLimit -- im_def_ptr->method = %x", im_def_ptr->method);
    ...
    {
        im_ptr->method_param.im_set.lang = default_lang;
        SCI_TRACE_LOW("[arvin] ImConstruct : reset language -- default_lang = %d", default_lang);
    }
...
)
{
    ...
    {
		// Forbid to commit current underline characters for T9 chs input method
#ifdef IM_ENGINE_T9
        if (GUIIM_LANG_CHS != im_ptr->method_param.im_set.lang &&
            GUIIM_LANG_CHT != im_ptr->method_param.im_set.lang &&
            GUIIM_LANG_TRAD_CHINESE_HK != im_ptr->method_param.im_set.lang)
#endif
        {
...
#ifdef IM_ENGINE_T9
        if (!MMITHEME_IMCheckT9PredictveStatus() &&
            (GUIIM_TYPE_SMART == im_ptr->method_param.im_def_ptr->type ||
             GUIIM_TYPE_STROKE == im_ptr->method_param.im_def_ptr->type))
        {
            SCI_TRACE_LOW ("[CTRLIM] Notify IM APP to switch method for DICT OFF. Current method = %x", im_ptr->method_param.im_def_ptr->method);
            (void)(im_ptr->apis_ptr->handle)(im_ptr->im_handle, GUIIM_EVENT_CUSTOM_SWITCH_METHOD, PNULL);
        }
#endif
        ...
        if (IsSwitchMsg(msg_id) && CanSwitchToKeyboard(im_ptr, &default_input))
        {
            ...
            */
            ...
            if (is_set)
            {
                ...
                MMK_SendMsg(ctrl_ptr->handle, msg_id, param);
            }
        }
        else
        {
...
#if (defined IM_ENGINE_CSTAR || defined IM_ENGINE_SOGOU || defined IM_ENGINE_IEKIE)    //可能不存在智能输入法
...
LOCAL BOOLEAN AppHandle(CTRLIM_OBJ_T *im_ptr, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
    ...
    is_im_dealt_msg = (im_ptr->apis_ptr->handle)(im_ptr->im_handle, GUIIM_EVENT_SYS_MSG, &event_data);
...
Note: 
==============================================================================*/
PUBLIC MMI_IMAGE_ID_T CTRLIM_ImGetIcon(MMI_HANDLE_T im_handle)
{
    MMI_IMAGE_ID_T icon = 0;//IMAGE_NULL;
...
PUBLIC MMI_IMAGE_ID_T CTRLIM_ImGetDefaultIcon(MMI_HANDLE_T im_handle)
{
    MMI_IMAGE_ID_T icon = 0;//IMAGE_NULL;
    ...
    return MMITHEME_ImGetDefaultIcon(im_ptr->method_param.im_def_ptr->method);
}

// Add for pen icon
PUBLIC MMI_IMAGE_ID_T CTRLIM_ImGetPenIcon(MMI_HANDLE_T im_handle)
{
    MMI_IMAGE_ID_T icon = 0;//IMAGE_NULL;
    ...
    return MMITHEME_ImGetPenIcon(im_ptr->method_param.im_def_ptr->method);
}

// Add for pen icon
PUBLIC MMI_IMAGE_ID_T CTRLIM_ImGetDefaultPenIcon(MMI_HANDLE_T im_handle)
{
    MMI_IMAGE_ID_T icon = 0;//IMAGE_NULL;
    ...
    return MMITHEME_ImGetDefaultPenIcon(im_ptr->method_param.im_def_ptr->method);
}
...
/*==============================================================================
Description: 设置输入法的符号
Global resource dependence: NONE
Author: 
Note: 在设置tag之后，简单的重建输入法应用
==============================================================================*/
PUBLIC BOOLEAN CTRLIM_GetSymbol(MMI_HANDLE_T im_handle)
{
    CTRLIM_OBJ_T *im_ptr = GetImPtr(im_handle);
    ...
    //改为根据当前的语言获取符号表
    MMIIM_SP_SymbCreateWinEx(im_handle, MMIIM_GetSymbTypeByLang(im_ptr->method_param.im_set.lang));

    
  //  MMIIM_SP_SymbCreateWinEx(im_handle, MMIIM_GetSymbType (im_ptr->method_param.im_set.method));
