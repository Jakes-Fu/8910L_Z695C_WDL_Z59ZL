...
#define GUIEDIT_ITEM_VERTICAL_MARGIN 6
#define GUIEDIT_LEFT_MARGIN 6
#define GUIEDIT_RIGHT_MARGIN 6
/**--------------------------------------------------------------------------*
 **                         TYPE AND STRUCT                                  *
 **--------------------------------------------------------------------------*/
...
//time and date editor display type 
typedef enum
{
    TIMEDATE_DEFAUL_DISPLAYTYPE,
    TIMEDATE_NORMAL_DISPLAYTYPE,
    TIMEDATE_DIALOG_DISPLAYTYPE,     
    TIMEDATE_COMBINE_DISPLAYTYPE,
    TIMEDATE_MAX_DISPLAYTYPE
}TIMEDATEEDITOR_DISPLAY_TYPE_E;
...
typedef struct
{
	...
	uint16                      cache_total_line_num;     /*!<总行数,从1开始*/
	...
	// for [MS]
    BOOLEAN                     is_disp_password;    /*!<是否显示password*/

   	MMI_TEXT_ID_T               		label_id;
	MMI_TEXT_ID_T               		hint_text_id;
	GUIEDITLABEL_TEXT_DATA_E      label_text_type;

    MMI_STRING_T                			label_str;
	MMI_STRING_T                			hint_text_str;     
	GUIEDITHINTTEXT_TEXT_DATA_E	hint_text_text_type;
    GUI_LCD_DEV_INFO            edit_layer_info; //edit layer
	GUI_LCD_DEV_INFO            label_layer_info; //label layer
    GUI_LCD_DEV_INFO            hint_text_layer_info; //hint text layer
    GUI_LCD_DEV_INFO            theme_line_layer_info; //hint text layer
    TIMEDATEEDITOR_DISPLAY_TYPE_E  timedate_editor_displaytype;    //time and date editor display type 

	GUIEDIT_SEPERATE_LINE_T seperate_line;
    MMI_IMAGE_ID_T image_id;
	BOOLEAN                  is_display_inverse;
	BOOLEAN	                 is_update_borderColor;
#ifdef ZDT_EDIT_NUM_IMG_SUPPORT
	BOOLEAN 				use_phnum_img;			// 是否用图片显示拨号字体
	uint16					   phnum_img_width;
	uint16					   phnum_img_height;
#endif
...
typedef struct
{
    ...
    /**************************************************************************/
    //  Description : the callback function on cursor changed
    /**************************************************************************/
...
//  Note:
/*****************************************************************************/
PUBLIC void BaseEditCtrlOnCursorChanged(
    CTRLBASEEDIT_OBJ_T  *baseedit_ctrl_ptr //in:
);
...
//  Description : notify im app to sync cursor
//  Global resource dependence : 
//  Author: arvin
//  Note: Define as virtual type due to we might implement by extended class
/**************************************************************************/
static __inline void VTLBASEEDIT_OnCursorChanged(
        CTRLBASEEDIT_OBJ_T  *baseedit_ctrl_ptr     // [in] ctrl ptr
)
{
    CTRLBASEEDIT_VTBL_T* baseedit_vtbl_ptr = (CTRLBASEEDIT_VTBL_T*)TYPEMNG_GetVtbl(CTRL_BASEEDIT_TYPE, baseedit_ctrl_ptr);

    if (PNULL != baseedit_vtbl_ptr->OnCursorChanged)
    {
        baseedit_vtbl_ptr->OnCursorChanged(baseedit_ctrl_ptr);
    }
}
...
/*****************************************************************************/
// Description : Set is_timedate_editor flag 
// Global resource dependence : none
// Author: 
// Note:
/*****************************************************************************/
PUBLIC void CTRLBASEEDIT_SetIsTimeDateEditorFlag(
                                     CTRLBASEEDIT_OBJ_T    *baseedit_ptr,
                                     BOOLEAN               is_timedate_editor //is_timedate_editor flag
                                     );
