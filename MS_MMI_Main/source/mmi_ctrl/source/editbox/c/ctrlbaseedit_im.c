 ...
 **                         FUNCTION DEFINITION                              *
 **--------------------------------------------------------------------------*/
//微软提供的English multitap的keymap中大于0x7F的字符集
wchar abc_enable_char_value[] =
{
    0x00E4, 0x00E6, 0x00E5, 0x00E0, 0x00E1, 0x00E2, 0x00E3, 0x00E7, 
    0x00E8, 0x00E9, 0x00EA, 0x00EB, 0x00F0,
    0x00EC, 0x00ED, 0x00EE, 0x00EF,
    0x00A3,
    0x00F6, 0x00F8, 0x00F2, 0x00F3, 0x00F4, 0x00F5, 0x00F1,
    0x00DF, 
    0x00F9, 0x00FA, 0x00FB, 0x00FC,
    0x00FD, 0x00FE,  
    0x00C4, 0x00C6, 0x00C5, 0x00C0, 0x00C1, 0x00C2, 0x00C3, 0x00C7,
    0x00C8, 0x00C9, 0x00CA, 0x00CB, 0x00D0,
    0x00CC, 0x00CD, 0x00CE, 0x00CF,
    0x00A3,
    0x00D6, 0x00D8, 0x00D2, 0x00D3, 0x00D4, 0x00D5, 0x00D1,
...
)
{
	...
	SCI_TRACE_LOW("[arvin] CTRLBASEEDIT_SetIm : allow_im = %x, init_im = %x", allow_im, init_im);
    ...
    if (PNULL != baseedit_ctrl_ptr)
    {
        ...
        //根据微软提供的keymap信息，在egnlish_gb时，multitap时可以输入大于0x7F的字符，
        //因此在强制设置输入方式为GUIIM_TYPE_ABC时将这些字符加入额外允许的字符表中，
        //这样在BASEFLEX_CheckTextStr@ctrlbaseflex_common.c中就不会限制这些被MMITHEME_IsUcs2Char排除的字符了
        if((GUIEDIT_TYPE_DATE != CTRLBASEEDIT_GetType(ctrl_id))
            && (GUIEDIT_TYPE_TIME != CTRLBASEEDIT_GetType(ctrl_id))
            &&(GUIEDIT_TYPE_IP != CTRLBASEEDIT_GetType(ctrl_id))
            &&(GUIIM_TYPE_ABC & allow_im))
        {
            CTRLBASEFLEX_ConfigAddEnableChar(ctrl_id,abc_enable_char_value,sizeof(abc_enable_char_value)/sizeof(wchar));
        }
...
{   
    ...
    if (PNULL != baseedit_ctrl_ptr)
    {
        if (0 != baseedit_ctrl_ptr->im_ctrl_handle)
        {
            ...
            CTRLIM_GetSymbol(baseedit_ctrl_ptr->im_ctrl_handle);
        }
    }
}
/*****************************************************************************/
									  ...
									  )
{
    ...
    CTRLBASEEDIT_OBJ_T *baseedit_ctrl_ptr = PNULL;
	GUIIM_EDIT_STATE_T state			  = GUIIM_EDIT_NONE;
