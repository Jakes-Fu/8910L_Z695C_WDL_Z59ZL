...
/*****************************************************************************/
//  Description : get StrNum in current line
//  Global resource dependence :
//  Author: hua.fang
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN CheckStrIsValid (
    CTRLLISTEDIT_OBJ_T      *listedit_ctrl_ptr,     // in
    uint16                  *prev_char_count_ptr    // out
);
...
)
{
    ...
    baseedit_ctrl_param_ptr->is_inline_editor = listedit_init_ptr->is_inline_editor;
...
)
{
    ...
    if (PNULL != new_edit_ctrl_ptr)
    {
        ...
        if (0 < item_num)
        {
            ...
            SCI_MEMSET(item_array_ptr, 0x00, item_num * sizeof (GUIEDIT_LIST_ITEM_T));
...
//        SCI_ASSERT (baseedit_ctrl_ptr->cursor.cur_pos == baseedit_ctrl_ptr->str_len); /*assert verified*/
        if(baseedit_ctrl_ptr->cursor.cur_pos != baseedit_ctrl_ptr->str_len) /*assert verified*/
		{
			SCI_TRACE_LOW("== AddSeparator Error == str_len = %d, cur_pos = %d ", baseedit_ctrl_ptr->str_len, baseedit_ctrl_ptr->cursor.cur_pos );
...
/*****************************************************************************/
//  Description : Check str valid or not
//  Global resource dependence :
//  Author: howie.chen
//  Note:
/*****************************************************************************/
    ...
    highlight_min_pos = BASEEDIT_GetHighlightMinPos (baseedit_ctrl_ptr);
        ...
        if (PNULL != prev_char_count_ptr)
        {
            for (i = highlight_min_pos - 1; i >= 0; i--)
            {
                ...
                // if String contains char exclude numbers/space, set as valid
                if ( ((baseedit_ctrl_ptr->str_ptr[i] < 48) || (baseedit_ctrl_ptr->str_ptr[i] >57)) && (baseedit_ctrl_ptr->str_ptr[i] != bl_char))
                {
                    is_valid =TRUE;
                    break;				
                }
...
)
{
    ...
    is_valid = CheckStrIsValid(listedit_ctrl_ptr, &current_str_len);
    ...
    while (i < add_str_ptr->commit_len + add_str_ptr->underline_len)
    {
        ...
        case EDIT_ITEM_STATE_CHAR:
            ...
            if (EDIT_ITEM_STATE_SEPARATOR == pre_item_state)
            {
				...
				//listedit_ctrl_ptr->pre_str_len = total_str_len - 1;
                // 添加回车，里面会自动加总数
                AddCommitStr (
                    listedit_ctrl_ptr,
...
)
{
    ...
    SCI_MEMSET(obj_info_ptr->user_data.wstr_ptr, 0x00, user_data_ptr->wstr_len * sizeof (wchar));
    MMI_MEMCPY (
