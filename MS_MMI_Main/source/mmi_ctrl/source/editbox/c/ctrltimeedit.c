 ...
 **                         MACRO DEFINITION                                 *
 **--------------------------------------------------------------------------*/
#define GUIEDIT_TIME_MAX_LEN                  (15)   //时间和上下午显示的最大长度目前是11，因为高棉语的上下午字符串占5个字节
#define GUIEDIT_TIME_AM_PM_HIGHLIGHT_MAX_LEN  (7) // xx:xx X
#define GUIEDIT_TIME_AM_PM_EXCEPTION_LEN      (20) //多国语言对应的Unicode长度不确定，最大长度暂设为20
 ...
 **                         GLOBAL DEFINITION                                *
 **--------------------------------------------------------------------------*/
uint16      time_editor_x_margin = 0;
uint16      time_editor_y_margin = 0;
...
/*****************************************************************************/
//  Description : Display time editor title
//  Global resource dependence :
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL void DisplayTimeEditorTitle (
                                   CTRLBASEFIXED_OBJ_T     *basefixed_ctrl_ptr
                                   );
...
)
{
    ...
    MMITHEME_GetMsTimeEditTheme(&timeedit_ctrl_ptr->theme);
...
)
{
    ...
    GUIEDIT_TIME_BIT_E  time_cursor_position;
    ...
    time_cursor_position = GetCurTimeBit(timeedit_ctrl_ptr);

    switch (msg_id)
    {
    case MSG_APP_UP:
    case MSG_KEYREPEAT_UP:
        if(time_cursor_position == GUIEDIT_TIME_BIT_AM_PM)
        {
    ...
    else
    {
        ...
        *highlight_len_ptr = 1;
        ...
        if (GUIEDIT_TIME_STYLE_12 == timeedit_ctrl_ptr->theme.time_style)
        {
            ...
            {
                if (GUIEDIT_TIME_AM_PM_EXCEPTION_LEN <= char_offset)
                {
                    char_offset = GUIEDIT_TIME_AM_PM_EXCEPTION_LEN;
                    *highlight_len_ptr = baseedit_ctrl_ptr->str_len - GUIEDIT_TIME_AM_PM_EXCEPTION_LEN;
                }
            }
        }
    }
                                ...
                                )
{
    ...
    timeedit_ctrl_ptr->display_type = display_type;
}
...
//  Note:
/*****************************************************************************/
PUBLIC GUIEDIT_TIME_DISPLAY_TYPE_E CTRLTIMEEDIT_GetTimeEditorDisplayType(
                                MMI_CTRL_ID_T                   ctrl_id
                                 )
{
    CTRLTIMEEDIT_OBJ_T      *timeedit_ctrl_ptr = PNULL;    
	...
	if (PNULL == timeedit_ctrl_ptr) 
        ...
        return timeedit_ctrl_ptr->display_type;
}

/*****************************************************************************/
//  Description : Set display type to NORMAL_DISPLAYTYPE
//  Global resource dependence :
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC void CTRLTIMEEDIT_SetIsFullTimeEditorFlag(
                                MMI_CTRL_ID_T           ctrl_id,
                                BOOLEAN                 is_full_editor
                                )
{
    CTRLTIMEEDIT_OBJ_T          *timeedit_ctrl_ptr = PNULL;    
	...
	if (TRUE == is_full_editor) 
	{
    	timeedit_ctrl_ptr->display_type = GUIEDIT_TIME_NORMAL_DISPLAYTYPE;		
	}
}


/*****************************************************************************/
//  Description : Set time editor location
//  Global resource dependence :
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLTIMEEDIT_SetLocation(
                                   MMI_CTRL_ID_T   ctrl_id,    //in:
                                   uint8           x_position,
                                   uint8           y_position
                                   )
{
    ...
    timeedit_ctrl_ptr = GetTimeEditPtr (ctrl_id);
    base_ctrl_ptr = (CTRLBASE_OBJ_T*)timeedit_ctrl_ptr;
    baseedit_ctrl_ptr = (CTRLBASEEDIT_OBJ_T*)base_ctrl_ptr;
    if (PNULL != timeedit_ctrl_ptr)
    {    
        time_editor_x_margin = x_position;
        time_editor_y_margin = y_position;
        
        both_rect.v_rect.top = both_rect.v_rect.top + time_editor_y_margin;    
        both_rect.v_rect.left = both_rect.v_rect.left + time_editor_x_margin;
        both_rect.v_rect.bottom = both_rect.v_rect.top + 66;
        both_rect.v_rect.right = both_rect.v_rect.left+240;

        base_ctrl_ptr->display_rect = both_rect.v_rect;        
        base_ctrl_ptr->rect = both_rect.v_rect;        
        GUI_SetVOrHRect( base_ctrl_ptr->handle, (GUI_RECT_T *)&both_rect,&(base_ctrl_ptr->both_rect));
        baseedit_ctrl_ptr->is_line_num_cached = FALSE;
        VTLBASEEDIT_DisplayAll (base_ctrl_ptr, FALSE);
        result = TRUE;
    }
    return (result);
}
...
/*****************************************************************************/
//  Description : set date editor title 
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
...

{
    ...
    if ( PNULL != timeedit_ctrl_ptr->title.wstr_ptr )
    {
        SCI_FREE( timeedit_ctrl_ptr->title.wstr_ptr );
        timeedit_ctrl_ptr->title.wstr_ptr = PNULL;
        timeedit_ctrl_ptr->title.wstr_len = 0;
    }

    if ( PNULL != title_str_ptr
        && PNULL != title_str_ptr->wstr_ptr
        && 0 != title_str_ptr->wstr_len )
    {
        timeedit_ctrl_ptr->title.wstr_ptr = SCI_ALLOC_APP( ( title_str_ptr->wstr_len + 1 ) * sizeof(wchar) );        
        SCI_MEMSET(  timeedit_ctrl_ptr->title.wstr_ptr , 0, (( title_str_ptr->wstr_len + 1 ) * sizeof(wchar) ));        
        MMIAPICOM_Wstrncpy(  timeedit_ctrl_ptr->title.wstr_ptr , title_str_ptr->wstr_ptr, title_str_ptr->wstr_len );        
        timeedit_ctrl_ptr->title.wstr_len = title_str_ptr->wstr_len;
    }
}
                                   ...
                                   )
{
    CTRLBASEEDIT_OBJ_T  *baseedit_ctrl_ptr = (CTRLBASEEDIT_OBJ_T*) basefixed_ctrl_ptr;
    CTRLTIMEEDIT_OBJ_T  *timeedit_ctrl_ptr = (CTRLTIMEEDIT_OBJ_T*) basefixed_ctrl_ptr;
    CTRLBASE_OBJ_T      *base_ctrl_ptr = (CTRLBASE_OBJ_T*) basefixed_ctrl_ptr;
    ...
    // Added by sunhongzhe to fixed the issue that
    // time editor title shows even if display_rect is invalid.
    if (GUI_IsInvalidRect(base_ctrl_ptr->display_rect)) return;
    // End.

    text_rect.top = base_ctrl_ptr->display_rect.top + 6 ;
    text_rect.bottom = text_rect.top + 24;
    text_rect.left = base_ctrl_ptr->display_rect.left + 6;
    text_rect.right = text_rect.left + 228;
    text_style.font_color = MAKE_BREW_RGB(102, 102, 102);
    base_ctrl_ptr = (CTRLBASE_OBJ_T*)timeedit_ctrl_ptr;


    //bug681003，显示类型为GUIEDIT_TIME_COMBINE_DATE_DISPLAYTYPE时，如果光标在date控件上，显示date控件title，time控件title不刷新
    if (GUIEDIT_TIME_COMBINE_DATE_DISPLAYTYPE == timeedit_ctrl_ptr->display_type)
    {
        uint16 lcd_height = 0;
        uint16 lcd_width  = 0;

        text_rect.left = GUIEDIT_ITEM_VERTICAL_MARGIN; //title左对齐时，将date和time控件的左坐标设为一致
        GUILCD_GetLogicWidthHeight(GUI_MAIN_LCD_ID, &lcd_width, &lcd_height);
        text_rect.right = lcd_width - GUIEDIT_ITEM_VERTICAL_MARGIN;//将date和time控件的右坐标设为一致

        if ( FALSE == base_ctrl_ptr->is_active)
        {
    ...
    {
        GUI_COLOR_T  bg_color = 0xffff;

        if (GUIEDIT_TIME_DIALOG_DISPLAYTYPE == timeedit_ctrl_ptr->display_type)
        {
            bg_color = MAKE_BREW_RGB(0x02, 0x59, 0xA2);
        }
        else if (GUIEDIT_TIME_NORMAL_DISPLAYTYPE == timeedit_ctrl_ptr->display_type ||
			GUIEDIT_TIME_COMBINE_DATE_DISPLAYTYPE == timeedit_ctrl_ptr->display_type)
        {
            bg_color = 0xffff;
        }
        GUI_FillRect((const GUI_LCD_DEV_INFO*) &baseedit_ctrl_ptr->edit_layer_info, cross_rect, bg_color);
    }

    //display title text 
    if (0 != timeedit_ctrl_ptr->title.wstr_len)
    {
        text_str = timeedit_ctrl_ptr->title;
        ...
        if (GUIEDIT_TIME_DIALOG_DISPLAYTYPE == timeedit_ctrl_ptr->display_type)
        {
            text_style.font_color = 0xFFFF;
               ...
               ||(GUIEDIT_TIME_COMBINE_DATE_DISPLAYTYPE == timeedit_ctrl_ptr->display_type))
        {
            text_style.font_color = 0x8410;  // MMI_DARK_GRAY_COLOR
        }
...
)
{
    ...
    if ( GUIEDIT_TIME_DEFAUL_DISPLAYTYPE != timeedit_ctrl_ptr->display_type)
    {
        DisplayTimeEditorTitle(basefixed_ctrl_ptr);     
        CTRLBASEEDIT_SetTimeDateEditorDisplayType(baseedit_ctrl_ptr,timeedit_ctrl_ptr->display_type);
    }
    ...
    if (0 == baseedit_ctrl_ptr->str_len)
    {
        ...
        if (GUIEDIT_TIME_STYLE_12 == timeedit_ctrl_ptr->theme.time_style)
        {
            uint32 mm = 0;
            MMI_STRING_T text = {0};
            uint32 str_max_len=baseedit_ctrl_ptr->str_max_len;

            baseedit_ctrl_ptr->str_ptr[i++] = GUIEDIT_TIME_SUFFIX_SEPARATOR;

            if (is_am)
            {
                MMI_GetLabelTextByLang(STR_CL_TIME_AM, &text);
                //for (mm = 0; mm < text.wstr_len; mm++)		// check it later, 2018-10-06, zhigang.peng
                for (mm = 0; mm < text.wstr_len; mm++)
                {
                    baseedit_ctrl_ptr->str_ptr[i++] = text.wstr_ptr[mm];
                    if((str_max_len-1) < i)
                        break;	
                }
            }
            ...
            {
                MMI_GetLabelTextByLang(STR_CL_TIME_PM, &text);
                //for (mm = 0; mm < text.wstr_len; mm++)		// check it later, 2018-10-06, zhigang.peng
                for (mm = 0; mm < text.wstr_len; mm++)
                {
                    baseedit_ctrl_ptr->str_ptr[i++] = text.wstr_ptr[mm];
...
)
{
    ...
    if (GUIEDIT_TIME_STYLE_12 == timeedit_ctrl_ptr->theme.time_style)
    {
        ...
        MMI_GetLabelTextByLang(STR_CLK_IDLE_AM, &text);
        for (mm = 0; mm < text.wstr_len; mm++)
        {
            if (text.wstr_ptr[mm] == baseedit_ctrl_ptr->str_ptr[baseedit_ctrl_ptr->str_len - text.wstr_len + mm])
            {
                is_am = TRUE;
            }
            else
...
)
{
    ...
    if ( (0 == baseedit_ctrl_ptr->cursor.cur_pos) &&
         (TRUE == is_form_type) ||(!baseedit_ctrl_ptr->common_theme.is_cir_handle_lf)
		)
    {
    ...
    {
		//dong.chunguang add for bug507670
		//if banner is open,close banner first
		if(MMIAPICOM_IsBannerWinOpen())
		{
			MMIAPICOM_CloseBannerWin(KEY_PRESSED,KEY_UP);
			return TRUE;
		}
		//end
        ...
        else
        {
            if (GUIEDIT_TIME_STYLE_12 == timeedit_ctrl_ptr->theme.time_style)
            {
                //VTLBASEEDIT_SetCursorPos(baseedit_ctrl_ptr, GUIEDIT_TIME_AM_PM_EXCEPTION_LEN);
				VTLBASEEDIT_SetCursorPos(baseedit_ctrl_ptr, GUIEDIT_TIME_AM_PM_HIGHLIGHT_MAX_LEN - 1);
                highlight_len = baseedit_ctrl_ptr->str_len - GUIEDIT_TIME_AM_PM_EXCEPTION_LEN;
            }
            else
            {
...
)
{
    ...
    if (GUIEDIT_TIME_STYLE_12 == timeedit_ctrl_ptr->theme.time_style)
    {
        time_max_len = GUIEDIT_TIME_AM_PM_HIGHLIGHT_MAX_LEN; // constant value is 7(xx:xx X)
    }
    else
    {
...
)
{
    ...
    if (baseedit_ctrl_ptr->cursor.cur_pos < (time_max_len - 1))
    {
        ...
        //AM or PM is one
        if (baseedit_ctrl_ptr->cursor.cur_pos == GUIEDIT_TIME_AM_PM_HIGHLIGHT_MAX_LEN - 1)
        {
...
)
{
    ...
    if (GUIEDIT_TIME_BIT_AM_PM == time_bit)
    {
        ...
        //reset highlight for am/pm because of the string length change
        highlight_len = baseedit_ctrl_ptr->str_len - GUIEDIT_TIME_AM_PM_EXCEPTION_LEN; // xx:xx XX(XX is the highlight part)
