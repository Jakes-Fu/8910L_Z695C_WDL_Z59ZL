...
)
{
    ...
    SCI_ASSERT ((GUIEDIT_STYLE_SINGLE == baseedit_ctrl_ptr->style)||(GUIEDIT_STYLE_SINGLE_UP== baseedit_ctrl_ptr->style)); /*assert verified*/
    ...
    while (start_pos < baseedit_ctrl_ptr->str_len)
    {
            ...
            (BASEEDIT_IsNewLineChar (baseedit_ctrl_ptr, start_pos)))
        {
            ...
            if(TRUE == MMISET_IsCurrentLanguageTextAlignRTLDirection())
            {
                ...
                {
                    if((remain_str_ptr[i] >= '0') && (remain_str_ptr[i] <= '9'))
                    {
                        baseedit_ctrl_ptr->line_info_ptr[line_index].direction = TEXT_DIR_RTL;
                    }
                }
            }
        }
        else
        {
			...
			{
				GUI_GetPhImgInfoInLineWithDir(baseedit_ctrl_ptr->line_info_ptr[line_index].direction,
					remain_str_ptr,
					remain_str_len,
...
)
{
    ...
    while (start_pos < str_len)
    {
		...
		if(baseedit_ctrl_ptr->use_phnum_img)
		{
		GUI_GetPhImgInfoInLineWithDir(TEXT_DIR_LTR,
			remain_str_ptr,
			remain_str_len,
			baseedit_ctrl_ptr->phnum_img_width,
			(uint8)baseedit_ctrl_ptr->common_theme.char_space,
