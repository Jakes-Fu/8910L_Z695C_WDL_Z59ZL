...
uint16 date_editor_x_margin = 0;
uint16 date_editor_y_margin = 0;
...
/*****************************************************************************/
//  Description : Display date editor title
//  Global resource dependence :
//  Author: Jassmine
//  Note:
/*****************************************************************************/
LOCAL void DisplayDateEditorTitle (
                                   CTRLBASEFIXED_OBJ_T     *basefixed_ctrl_ptr
                                   );
...
)
{
    ...
    baseedit_ctrl_param_ptr->is_inline_editor = dateedit_init_ptr->is_inline_editor;	
...
)
{
    ...
    MMITHEME_GetMsDateEditTheme (&dateedit_ctrl_ptr->theme);
    ...
    dateedit_ctrl_ptr->title.wstr_ptr = PNULL;
    dateedit_ctrl_ptr->title.wstr_len = 0;
...
)
{
    ...
    if ( PNULL != dateedit_ctrl_ptr->title.wstr_ptr )
    {
        SCI_FREE( dateedit_ctrl_ptr->title.wstr_ptr );
        dateedit_ctrl_ptr->title.wstr_ptr = PNULL;
        dateedit_ctrl_ptr->title.wstr_len = 0;
    }
    return (result);
...
/*****************************************************************************/
//  Description : Set date editor location
//  Global resource dependence :
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLDATEEDIT_SetLocation(
                                   MMI_CTRL_ID_T   ctrl_id,    //in:
                                   uint8           x_position,
                                   uint8           y_position
                                   )
{
    ...
    dateedit_ctrl_ptr = GetDateEditPtr (ctrl_id);
    base_ctrl_ptr = (CTRLBASE_OBJ_T*)dateedit_ctrl_ptr;
    baseedit_ctrl_ptr = (CTRLBASEEDIT_OBJ_T*) base_ctrl_ptr; 
    if (PNULL != dateedit_ctrl_ptr)
    {    
        date_editor_x_margin = x_position;
        date_editor_y_margin = y_position;
        
        both_rect.v_rect.top = both_rect.v_rect.top + date_editor_y_margin;    
        both_rect.v_rect.left = both_rect.v_rect.left + date_editor_x_margin;
...
//  Note:
/*****************************************************************************/
PUBLIC void CTRLDATEEDIT_SetTitle(
                                 MMI_CTRL_ID_T  ctrl_id,   
...

{
    ...
    {
        dateedit_ctrl_ptr->title.wstr_ptr = SCI_ALLOC_APP( ( title_str_ptr->wstr_len + 1 ) * sizeof(wchar) );        
        SCI_MEMSET(  dateedit_ctrl_ptr->title.wstr_ptr , 0, (( title_str_ptr->wstr_len + 1 ) * sizeof(wchar) ));        
        MMIAPICOM_Wstrncpy(  dateedit_ctrl_ptr->title.wstr_ptr , title_str_ptr->wstr_ptr, title_str_ptr->wstr_len );        
        dateedit_ctrl_ptr->title.wstr_len = title_str_ptr->wstr_len;
    }
}

/*****************************************************************************/
//  Description : free date editor title memory
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC void CTRLDATEEDIT_FreeTitle(
                                 MMI_CTRL_ID_T  ctrl_id,   
                                 MMI_STRING_T   *title_str_ptr
                                 )
                                   ...
                                   )
{
    ...
    // date editor title shows even if display_rect is invalid.
    ...
    //bug681003,显示类型为GUIEDIT_DATE_COMBINE_TIME_DISPLAYTYPE时，如果光标在time控件上，显示time控件title，不显示date控件的title
    if (GUIEDIT_DATE_COMBINE_TIME_DISPLAYTYPE == dateedit_ctrl_ptr->display_type)
    {
        uint16 lcd_height = 0;
        uint16 lcd_width  = 0;

        text_rect.left = GUIEDIT_ITEM_VERTICAL_MARGIN;  //将date和time控件的左坐标设为一致
    ...
    {
        GUI_COLOR_T  bg_color = 0x0000;

        if (GUIEDIT_DATE_DIALOG_DISPLAYTYPE == dateedit_ctrl_ptr->display_type)
        {
            bg_color = 0x0000;
        }
        else if (GUIEDIT_DATE_NORMAL_DISPLAYTYPE == dateedit_ctrl_ptr->display_type||
		      GUIEDIT_DATE_COMBINE_TIME_DISPLAYTYPE == dateedit_ctrl_ptr->display_type)
        {
            bg_color = 0xffff ;/// 0x0000;  /////ZDT_PRODUCT_BH
        }        
        GUI_FillRect((const GUI_LCD_DEV_INFO*) &baseedit_ctrl_ptr->edit_layer_info, cross_rect, bg_color);
    }
    
    //Display title text 
    if (0 != dateedit_ctrl_ptr->title.wstr_len)
    {
        text_str = dateedit_ctrl_ptr->title;            
       ...
       // Mirror calculate
                ...
                ||(GUIEDIT_DATE_COMBINE_TIME_DISPLAYTYPE == dateedit_ctrl_ptr->display_type))
        {
            ...
            {
                text_style.font_color =0x0000 ;// MMITHEME_GetAccentColor(MMITHEME_GetAccentId(), MMITHEME_OFFSET_COLOR_DEFAULT);///ZDT_PRODUCT_BH
            }
            else
            {
                text_style.font_color = 0x0000 ;// MAKE_BREW_RGB(102, 102, 102);
            }
        }
...
//  Note:
/*****************************************************************************/
PUBLIC void CTRLDATEEDIT_SetDateEditorDisplayType(
                                MMI_CTRL_ID_T                   ctrl_id,
                                GUIEDIT_DATE_DISPLAY_TYPE_E     display_type
                                )
{
    CTRLDATEEDIT_OBJ_T      *dateedit_ctrl_ptr = PNULL;    
    CTRLBASE_OBJ_T          *base_ctrl_ptr = PNULL;    
 ...
 baseedit_ctrl_ptr = (CTRLBASEEDIT_OBJ_T*)dateedit_ctrl_ptr;
    ...
    dateedit_ctrl_ptr->display_type = display_type;
...
//  Note:
/*****************************************************************************/
PUBLIC GUIEDIT_DATE_DISPLAY_TYPE_E CTRLDATEEDIT_GetDateEditorDisplayType(
                                MMI_CTRL_ID_T                   ctrl_id
                                 )
{
    CTRLDATEEDIT_OBJ_T      *dateedit_ctrl_ptr = PNULL;    
	...
	if (PNULL == dateedit_ctrl_ptr) 
        ...
        return dateedit_ctrl_ptr->display_type;
}
...
//  Note:
/*****************************************************************************/
PUBLIC void CTRLDATEEDIT_SetIsFullDateEditorFlag(
                                MMI_CTRL_ID_T           ctrl_id,
                                ...
                                )
{
	...
	{
    	dateedit_ctrl_ptr->display_type = GUIEDIT_DATE_NORMAL_DISPLAYTYPE;		
	}
}
...
)
{
    ...
    {
        baseedit_ctrl_ptr = (CTRLBASEEDIT_OBJ_T*) basefixed_ctrl_ptr;
        dateedit_ctrl_ptr = (CTRLDATEEDIT_OBJ_T*) basefixed_ctrl_ptr;
    }
    
    if ( GUIEDIT_DATE_DEFAUL_DISPLAYTYPE != dateedit_ctrl_ptr->display_type)
    {
        DisplayDateEditorTitle(basefixed_ctrl_ptr);     
        CTRLBASEEDIT_SetTimeDateEditorDisplayType(baseedit_ctrl_ptr,dateedit_ctrl_ptr->display_type);
    }
