...
)
{
    ...
    else
    {
		...
		GUI_GetImgCursorInfo(BASEEDIT_GetCursorAlign(baseedit_ctrl_ptr),
			&text_rect,
			text_direction,
			str_ptr,
			str_len,
...
)
{
    ...
    if (!baseedit_ctrl_ptr->is_rect_adaptive)
    {
        ...
        else if (min_top_line_offset + cursor_offset + line_height >= baseedit_ctrl_ptr->top_line_offset + display_height)
        {
			...
			{
			    baseedit_ctrl_ptr->top_line_offset = 0;
			}
...
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN SendCursorMoveUpDownMsg (
    CTRLBASEEDIT_OBJ_T  *baseedit_ctrl_ptr
)
{
    BOOLEAN                 result = FALSE;
    CTRLBASE_OBJ_T          *base_ctrl_ptr = (CTRLBASE_OBJ_T*) baseedit_ctrl_ptr;
    GUIFORM_CURSOR_UPDOWN_T cursor_notify = {0};

    //post parent control
    if (0 != MMK_GetParentCtrlHandle (base_ctrl_ptr->handle))
    {
        cursor_notify.ctrl_handle = base_ctrl_ptr->handle;
        BASEEDIT_GetCursorRect (baseedit_ctrl_ptr, PNULL, &cursor_notify.cursor_rect);
        
        CTRLMSG_SendNotifyEx (base_ctrl_ptr->handle, MSG_NOTIFY_CURSOR_UP_DOWN, &cursor_notify);
...
)
{
    ...
    if (baseedit_ctrl_ptr->highlight.start_pos != baseedit_ctrl_ptr->highlight.end_pos)
    {
        ...
        if (is_up)
        {
            ...
            if ((min_highlight_pos ==0) && (TYPEMNG_IsTypeOf(baseedit_ctrl_ptr, LISTEDIT_GetType())))
            {
                result = TRUE;
            }
            else
    ...
    if (1 < baseedit_ctrl_ptr->total_line_num)
    {
        ...
        if (is_up)
        {
            ...
            if ((!baseedit_ctrl_ptr->common_theme.is_cir_handle_ud) &&
                (0 == cursor_line_index) && (!TYPEMNG_IsTypeOf (baseedit_ctrl_ptr, LISTEDIT_GetType())))
            {
    ...
    if(result)
    {
       SendCursorMoveUpDownMsg(baseedit_ctrl_ptr);
    }
    return (result);
}
...
)
{
    ...
    str_ptr = VTLBASEEDIT_GetDispStr (
                  ...
                  str_len);
#ifdef ZDT_EDIT_NUM_IMG_SUPPORT
		char_num = GUI_GetImgCursorPos(BASEEDIT_GetCursorAlign(baseedit_ctrl_ptr),
			&text_rect,
...
)
{
            ...
            (baseedit_ctrl_ptr->cursor.bg_buf_size == cursor_size))
    {
        ...
        //store rect
        MMITHEME_StoreUpdateRect ( (const GUI_LCD_DEV_INFO*) &baseedit_ctrl_ptr->edit_layer_info, cursor_rect);
    }
}
