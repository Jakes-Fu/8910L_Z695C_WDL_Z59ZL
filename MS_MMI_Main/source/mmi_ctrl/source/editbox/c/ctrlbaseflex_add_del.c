...
)
{
        ...
        || (PNULL != list_item_ptr && list_item_count > 0))
    {
		if (PNULL != add_str_ptr)
		{
			SCI_TRACE_LOW("[504445] BASEFLEX_AddString: add_str_ptr->commit_len: %d, add_str_ptr->underline_len: %d.", add_str_ptr->commit_len, add_str_ptr->underline_len);
		}
        ...
        (BASEFLEX_CheckEditString (baseflex_ctrl_ptr, add_str_ptr->underline_ptr, &add_str_ptr->underline_len)))
    {
        ...
        if ((0 < add_str_ptr->commit_len) || (0 < add_str_ptr->underline_len))
        {
            ...
            *is_delete_ptr = DeleteStringAfterFull (baseflex_ctrl_ptr, str_delete_len);
			...
			#if 0
					 uint8 num = 0xff;
					wchar	  ascii_code[2] = {0};
						MMI_MEMCPY(ascii_code,2,add_actual_str.str_ptr,add_actual_str.str_len,1);
					  //  ascii_code = add_actual_str.str_ptr;
					// GUIEDIT_AddListItem(edit_ctrl_ptr, &ascii_code, 1);

					  
					  if(ascii_code[0] >=	'0' && ascii_code[0] <= '9')
					  {
						  num = 0x0030 + ascii_code[0] - '0';
					  }
					  ...
					  else if(ascii_code[0] == 'p' || ascii_code[0] == 'P')
					  {
						  num = 0xE; ///0xD;
					  }
					  else if(ascii_code[0] == 'w' || ascii_code[0] == 'W' )
					  {
						  num =0xE; /// 0xF;
					  }
					  else if(ascii_code[0] == '+')
					  ...
					  else if(ascii_code[0] == 'c' || ascii_code[0] == 'C')
					  {
						  num =0xE; ///0xB;
					  }
					if((ascii_code[0] == 42 ) )
					{
						if((add_str_ptr->commit_len == 0) &&( add_str_ptr->underline_len == 1))
						{
			#ifdef HERO_ENGINE_TTS_SUPPORT
							hero_tts_dial_broadcast(num);
			#endif
						}
					}
					else
					{
						if((add_str_ptr->commit_len == 1) &&( add_str_ptr->underline_len == 0))
						{
			#ifdef HERO_ENGINE_TTS_SUPPORT
							hero_tts_dial_broadcast(num);
			#endif
					   ...
					   if(ascii_code[0] == 42)
					   	{
					   		if((add_str_ptr->commit_len == 0) &&( add_str_ptr->underline_len == 1))
					   		{
								MMIAPISET_PlayDialRing(ascii_code[0]);
					   		}
						}
					   else
					   	{
        ...
        if (original_commit_len + original_underline_len > add_actual_str.str_len)
        {
            ...
            // Add for commit len > add_actual_str, need to press few more back key to return
            if (0 < original_commit_len)
                {
...
)
{
    ...
    else
    {
        ...
        else if (baseedit_ctrl_ptr->str_max_len == baseedit_ctrl_ptr->str_len) //for fix bugid:1049626
        {           
           baseedit_ctrl_ptr->is_line_num_cached = FALSE;
        }else if(highlight_len > 0 && add_str_len > 0)  //for bug 1099391
        {
            baseedit_ctrl_ptr->is_line_num_cached = FALSE;
        }
    }
