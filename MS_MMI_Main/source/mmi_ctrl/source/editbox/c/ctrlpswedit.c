...
)
{
	...
	baseedit_ctrl_param_ptr->is_inline_editor = pswedit_init_ptr->is_inline_editor;
...
)
{
   ...
   baseedit_ctrl_ptr->is_line_num_cached = FALSE;
    ...
    {
        VTLBASEEDIT_SetAlign (baseedit_ctrl_ptr, ALIGN_RIGHT);
    }
...
)
{
    ...
    case MSG_CTL_GET_FOCUS:
        ...
        {
            CTRLBASEEDIT_SetAlign(pswedit_ctrl_ptr, ALIGN_RVMIDDLE);
        }
...
)
{
    ...
    if(PNULL != pswedit_ctrl_ptr->password_ptr)
	{
		...
		//set string
		if(FALSE == baseedit_ctrl_ptr->is_disp_password)
		{
			for (i = 0; i < str_len; i++)
			{
...
// 				|| (GUIEDIT_STYLE_MULTI_DYN_UP == baseedit_ctrl_ptr->style))
		{
			for (i = 0; i < str_len; i++)
			{
				password_str_ptr[i] = baseedit_ctrl_ptr->str_ptr[i];
			}
			password_str_ptr[i] = 0; 
			}
		}  
