...
/*****************************************************************************/
//  Description : get editor style
//  Global resource dependence :
//  Author: arvin
//  Note:
/*****************************************************************************/
LOCAL void GetBaseFlexStyle (
	CTRLBASEEDIT_OBJ_T  *baseedit_ctrl_ptr,     //in
	GUIEDIT_STYLE_E *style        //in/out:
);
...
/*****************************************************************************/
//  Description : set edit themeline
//  Global resource dependence :
//  Author: Jassmine
//  Note: default don't display
/*****************************************************************************/
PUBLIC void CTRLBASEFLEX_SetThemeLine(
    MMI_CTRL_ID_T   ctrl_id,    //in:
    GUI_COLOR_T     line_focused_color,  //in:
	GUI_COLOR_T     line_unfocused_color  //in:
)
{
    CTRLBASEFLEX_OBJ_T  *baseflex_ctrl_ptr = PNULL;
    CTRLBASEEDIT_OBJ_T  *baseedit_ctrl_ptr = PNULL;
    ...
    {
        //set dividing line
        baseedit_ctrl_ptr->common_theme.themeline_focused_color= line_focused_color;
		baseedit_ctrl_ptr->common_theme.themeline_unfocused_color= line_unfocused_color;
		baseedit_ctrl_ptr->common_theme.is_themeline_set = TRUE;
    }
}

/*****************************************************************************/
//  Description : set edit theme color
//  Global resource dependence :
//  Author: Jassmine
//  Note: default don't display
/*****************************************************************************/
PUBLIC void CTRLBASEFLEX_SetThemeColor (
    MMI_CTRL_ID_T   ctrl_id,    //in:
    GUI_COLOR_T     theme_color  //in:
)
{
    CTRLBASEFLEX_OBJ_T  *baseflex_ctrl_ptr = PNULL;
    CTRLBASEEDIT_OBJ_T  *baseedit_ctrl_ptr = PNULL;
    ...
    {
        //set dividing line
        baseedit_ctrl_ptr->common_theme.theme_color= theme_color;
    }
}
...
/*****************************************************************************/
//  Description : set Seperate line
//  Global resource dependence :
//  Author: Howie
//  Note:
/*****************************************************************************/
PUBLIC void CTRLBASEFLEX_SetLine (
    MMI_CTRL_ID_T      ctrl_id,
    GUIEDIT_SEPERATE_LINE_T seperate_line,
    MMI_IMAGE_ID_T image_id
)
{
    CTRLBASEFLEX_OBJ_T  *baseflex_ctrl_ptr = PNULL;
    ...
    if (PNULL != baseflex_ctrl_ptr)
    {
        ...
        if (seperate_line != baseedit_ctrl_ptr->seperate_line)
        {
            //set style
            baseedit_ctrl_ptr->seperate_line = seperate_line;
        }
    }
	baseedit_ctrl_ptr->image_id = image_id;
}
                                ...
                                )
{
        ...
        (cursor_line_index <= bottom_line_index))
    {
        ...
        {
			cursor_rect = baseedit_ctrl_ptr->highlight.rect;
        }
       	else if (baseedit_ctrl_ptr->cursor.is_rect_valid)
        {
...
static IEDT_HANDLE_T s_iedthandle_array[MAX_IDET_HANDLE]={0};
/*****************************************************************************/
...
PUBLIC int RegisterEditorEventListener(EDTEVENTFUNC func)
{
	int i=0;
	int ret = -1;

	for(i = 0; i < MAX_IDET_HANDLE; i++)
	{
		if(s_iedthandle_array[i].is_used == FALSE)
		{
			s_iedthandle_array[i].is_used=TRUE;
			s_iedthandle_array[i].func = func;
			break;
		}
	}
	if(i < MAX_IDET_HANDLE)
	{
		ret = i;
	}
	else
	{
		SCI_TRACE_LOW("RegisterEditorEventListener FAIL");
	}
	return ret;
}
...
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN UnregisterEditorEventListener( int index)
{
	int i=0;
	BOOLEAN ret = FALSE;
	
	if(index < MAX_IDET_HANDLE)
	{
		s_iedthandle_array[i].is_used=FALSE;
		ret = TRUE;
	}
	else
	{
		SCI_TRACE_LOW("UnregisterEditorEventListener FAIL");
	}
	return ret;
}
...
//  Note:
/*****************************************************************************/
PUBLIC MMI_RESULT_E handleEditorMsg( CTRLBASEFLEX_OBJ_T* baseflex_ctrl_ptr, MMI_MESSAGE_ID_E msg_id)
{
	int i=0;
	int j= 0; 
	MMI_RESULT_E ret = MMI_RESULT_FALSE;
	CTRLBASEEDIT_OBJ_T  baseedit_ctrl = baseflex_ctrl_ptr->baseedit_ctrl;
	uint16 cur_pos=0;
	uint16 cursor_line_index;
	uint16 top_line_index;
	uint16 bottom_line_index;
	switch (msg_id)
	{
	case MSG_APP_LEFT:
	//case MSG_KEYREPEAT_LEFT:
		{
			cur_pos = baseedit_ctrl.cursor.cur_pos;
			cursor_line_index = BASEEDIT_GetCursorLineIndex (&baseedit_ctrl, cur_pos);
			
			/*if(cur_pos == baseedit_ctrl.line_info_ptr[cursor_line_index].start_pos)*/
			if(cur_pos == 0)
			{
				msg_id = MSG_NOTIFY_IEDITOR_KEY_LEFT;
				ret = MMI_RESULT_TRUE;
			}
		}
		break;
	...
	//case MSG_KEYREPEAT_UP:
		{
			cursor_line_index = BASEEDIT_GetCursorLineIndex (&baseedit_ctrl, baseedit_ctrl.cursor.cur_pos);
		//	top_line_index = BASEEDIT_GetTopLineIndex(&baseedit_ctrl);
		//	bottom_line_index = BASEEDIT_GetBottomLineIndex(&baseedit_ctrl);
			if(cursor_line_index == 0)
			{
				msg_id = MSG_NOTIFY_IEDITOR_KEY_UP;
				ret = MMI_RESULT_TRUE;
			}
		}
		break;
	...
	//case MSG_KEYREPEAT_DOWN:
		{
			...
			top_line_index = BASEEDIT_GetTopLineIndex(&baseedit_ctrl);
			bottom_line_index = BASEEDIT_GetBottomLineIndex(&baseedit_ctrl);
			if(cursor_line_index == bottom_line_index)
			{
				msg_id = MSG_NOTIFY_IEDITOR_KEY_DOWN;
				ret = MMI_RESULT_TRUE;
			}
		}
		break;
	...
	//case MSG_KEYREPEAT_RIGHT:
		{
			...
			cur_pos = baseedit_ctrl.cursor.cur_pos;
			cursor_line_index = BASEEDIT_GetCursorLineIndex (&baseedit_ctrl, cur_pos);
			line_str_len = BASEEDIT_GetLineStrLen (&baseedit_ctrl, cursor_line_index);
		//	if(cur_pos == (baseedit_ctrl.line_info_ptr[cursor_line_index].start_pos+line_str_len))
			if(cur_pos == baseedit_ctrl.str_len)
			{
				msg_id = MSG_NOTIFY_IEDITOR_KEY_RIGHT;
				ret = MMI_RESULT_TRUE;
			}
...
//	case MSG_KEYREPEAT_CANCEL:
		{
			...
			if(cur_pos == 0 )
			{
				msg_id = MSG_NOTIFY_IEDITOR_KEY_BACKSPACE;
				ret = MMI_RESULT_TRUE;
			}
		}
		break;
	case MSG_NOTIFY_REACH_END:
		{
				ret = MMI_RESULT_TRUE;
		}
		break;
	case MSG_NOTIFY_IIEDT_REDUCE_LINE:
		{
				ret = MMI_RESULT_TRUE;
		}
		break;		
	...
	for(i = 0; i < MAX_IDET_HANDLE; i++)
	{
		if(s_iedthandle_array[i].is_used)
		{
			s_iedthandle_array[i].func(baseedit_ctrl.base_ctrl.handle, msg_id);
...
)
{
	...
	case MSG_NOTIFY_REACH_END:
		{
			...
			{
				result = handleEditorMsg(baseflex_ctrl_ptr, msg_id);
			}
		}
		break;
	case MSG_NOTIFY_IIEDT_REDUCE_LINE:
    ...
    case MSG_KEYREPEAT_RIGHT:
		{
			...
			if(baseedit_ctrl_ptr->is_inline_editor)
			{
				...
				{
					HandleDirectionKey (baseflex_ctrl_ptr, msg_id, param);	
				}
				else
				{
    ...
    case MSG_APP_CANCEL:
		{
			MMI_RESULT_E is_inline_deal = MMI_RESULT_FALSE;

			if(baseedit_ctrl_ptr->is_inline_editor)
			{
				is_inline_deal = handleEditorMsg(baseflex_ctrl_ptr, msg_id);
			}
			...
			if(is_inline_deal == MMI_RESULT_TRUE)
			{
    ...
    case MSG_NOTIFY_IM_GET_CURSOR_RECT:
        GetHighlightRect (baseedit_ctrl_ptr, (GUI_RECT_T*)param);
        break;

    case MSG_NOTIFY_IM_GET_EDITOR_STYLE:
        GetBaseFlexStyle (baseedit_ctrl_ptr, (GUI_RECT_T*)param);
        break;
...
)
{
    ...
    else if(baseedit_ctrl_ptr->is_line_num_cached)
   {
         total_line_num = (baseedit_ctrl_ptr->cache_total_line_num > 0 ?baseedit_ctrl_ptr->cache_total_line_num:1);
    }
    else
    {
        ...
        {
            baseedit_ctrl_ptr->is_line_num_cached = TRUE;
            baseedit_ctrl_ptr->cache_total_line_num = total_line_num;
        }	
...
)
{
    ...
    if (MMI_RESULT_TRUE == result)
    {
        ...
        // Should notify im app to sync cursor position so as to enable 
        // spml modify the keymap table config accordingly if needed in time.
        VTLBASEEDIT_OnCursorChanged(baseedit_ctrl_ptr);
    }
...
)
{
    ...
    if ((baseflex_ctrl_ptr->is_display_prg) &&
		...
		(GUIEDIT_STYLE_MULTI_DYN_DOWN == baseedit_ctrl_ptr->style) || // for [MS]
		(GUIEDIT_STYLE_MULTI_DYN_UP == baseedit_ctrl_ptr->style) ||
		(GUIEDIT_STYLE_SINGLE_DYN_DOWN== baseedit_ctrl_ptr->style) ||
		(GUIEDIT_STYLE_MULTI_UP== baseedit_ctrl_ptr->style) )) // for [MS]
    {
        //get edit box rect
		...
		{
			baseedit_ctrl_ptr->base_ctrl.rect.top = 0;
			baseedit_ctrl_ptr->base_ctrl.rect.top = baseedit_ctrl_ptr->base_ctrl.rect.bottom - 30 - (total_line_num * 28); 				
		}
		// end [MS]
...
)
{
    ...
    //init data
    init_data.lcd_id = baseflex_ctrl_ptr->baseedit_ctrl.edit_layer_info.lcd_id;//base_ctrl_ptr->lcd_dev_info.lcd_id;
...
{
	GUIEDIT_STYLE_E edit_style = GUIEDIT_STYLE_NULL;

    if (baseedit_ctrl_ptr)
    {
		edit_style = baseedit_ctrl_ptr->style;
    }
