...
PUBLIC void GUIEDIT_SetPhoneNumImg(
                              MMI_CTRL_ID_T   ctrl_id,      //in
                              BOOLEAN  use_img,     //in
                              uint16 img_width,
                              uint16 img_height
                              )
{
    ...
    if (PNULL != phonenumedit_ctrl_ptr)
    {
        baseedit_ctrl_ptr->use_phnum_img = use_img;
        baseedit_ctrl_ptr->phnum_img_width = img_width;
        baseedit_ctrl_ptr->phnum_img_height = img_height;
        phonenumedit_ctrl_ptr->edit_font.is_valid = TRUE;
        phonenumedit_ctrl_ptr->edit_font.small_font = SONG_FONT_64;
        phonenumedit_ctrl_ptr->edit_font.big_font = SONG_FONT_64;
        phonenumedit_ctrl_ptr->edit_font.mid_font = SONG_FONT_64;
        baseedit_ctrl_ptr->common_theme.font.font = SONG_FONT_64;

        //modify line number in one page
      //  baseedit_ctrl_ptr->page_line_num = GetPageLineNum(baseflex_ctrl_ptr);
...
PUBLIC void GUIEDIT_SetCharSpace(
                                 MMI_CTRL_ID_T  ctrl_id,    //in
                                 uint8    char_space  //in
                                 )
{
    ...
    {
        baseedit_ctrl_ptr->common_theme.char_space = char_space;
    }
}
PUBLIC void GUIEDIT_SetLineSpace(
                                 MMI_CTRL_ID_T  ctrl_id,    //in
                                 uint8    line_space  //in
                                 )
{
    ...
    {
        baseedit_ctrl_ptr->common_theme.line_space = line_space;
    }
}
...
)
{
    ...
    baseedit_ctrl_param_ptr->is_inline_editor= phonenumedit_init_ptr->is_inline_editor;
...
)
{
    ...
    else
    {
		...
		{
			total_line_num = GUI_CalculatePhImgLinesByPixelNum(
										display_width,
										str_ptr,
										str_len,
										baseedit_ctrl_ptr->phnum_img_width,
...
)
{
    ...
    int16 edit_width = base_ctrl_ptr->rect.right - base_ctrl_ptr->rect.left + 1 - 2 * baseedit_ctrl_ptr->common_theme.char_space;
		...
		if(baseedit_ctrl_ptr->use_phnum_img)
		{
			if (edit_width > GUI_GetPhImgWidthEx(phonenumedit_ctrl_ptr->edit_font.big_font, baseedit_ctrl_ptr->str_ptr, baseedit_ctrl_ptr->str_len, baseedit_ctrl_ptr->common_theme.char_space))
			{
				return	phonenumedit_ctrl_ptr->edit_font.big_font;
			}
			else if (edit_width > GUI_GetPhImgWidthEx(phonenumedit_ctrl_ptr->edit_font.mid_font, baseedit_ctrl_ptr->str_ptr, baseedit_ctrl_ptr->str_len, baseedit_ctrl_ptr->common_theme.char_space) + 2)
			{
