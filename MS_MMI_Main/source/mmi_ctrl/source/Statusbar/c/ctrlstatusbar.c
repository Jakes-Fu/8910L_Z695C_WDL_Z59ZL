...
)
{
    ...
    win_id = (MMI_WIN_ID_T) MMK_GetWinHandleByCtrl (statusbar_ctrl_ptr->base_ctrl.handle);
        ...
        // 重新调整区域，去除边界值
    rect.left = statusbar_ctrl_ptr->base_ctrl.rect.left + 2;
        rect.top = statusbar_ctrl_ptr->base_ctrl.rect.top;
    rect.right = statusbar_ctrl_ptr->base_ctrl.rect.right - 2;
...
)
{
    ...
    rect.top = statusbar_ctrl_ptr->base_ctrl.rect.top + 2;
#else
    rect.top = statusbar_ctrl_ptr->base_ctrl.rect.top + 1;
#endif
    rect.right = statusbar_ctrl_ptr->base_ctrl.rect.right - 2;
    rect.bottom = statusbar_ctrl_ptr->base_ctrl.rect.bottom - 1;
    total_width = rect.right - rect.left + 1;
...
)
{
...
#endif
    {
        ...
        if (statusbar_ctrl_ptr->is_display_bg)
        {
            ...
            SCI_TRACE_LOW("DrawStatusBarBg is_draw_bg=%d", is_draw_bg);
        }
#if 0 //ndef UI_MULTILAYER_SUPPORT
        else
        {
            int16           image_width = bg_rect.right - bg_rect.left + 1;
            int16           image_height = bg_rect.bottom - bg_rect.top + 1;
            ...
            SCI_TRACE_LOW("DrawStatusBarBg img source =%d", statusbar_ctrl_ptr->style_info.theme_image_data.img_source);

            if(0 < image_width && 0 < image_height)
            {
               image_rect = GUI_CreateRect(0, 0, image_width - 1, image_height - 1);
               if (STATUSBAR_IMAGE_SOURCE_ID == statusbar_ctrl_ptr->style_info.theme_image_data.img_source
                            &&MMITHEME_CheckImageID(statusbar_ctrl_ptr->style_info.theme_image_data.res.id) )
               {
                    result=GUIRES_DisplayImg(PNULL,
                        &bg_rect,
                          &image_rect,
                        win_id,
                        statusbar_ctrl_ptr->style_info.theme_image_data.res.id,
                        &lcd_dev_info);
                    SCI_TRACE_LOW("DrawStatusBarBg source id GUIRES_DisplayImg =%d", result);
               }
               else if (STATUSBAR_IMAGE_SOURCE_FILE == statusbar_ctrl_ptr->style_info.theme_image_data.img_source)
               {
                   if(MMIAPIIDLE_IsExistWallpaperFile(&is_gif))   
                   { 
                        if ((!is_gif) &&(MMIAPIIDLE_GetWallpaperBitmap(&data_info.bitmap_data)))
                        {
                             if(PNULL != statusbar_ctrl_ptr->style_info.theme_image_data.data_ptr)
                             {
                                 result=GUIIMG_DisplayBmp(FALSE,
                                                        &bg_rect,
                                                        &image_rect,
                                                        statusbar_ctrl_ptr->style_info.theme_image_data.data_ptr,
                                                        &lcd_dev_info);
                                 SCI_TRACE_LOW("DrawStatusBarBg source file GUIRES_DisplayImg =%d", result);
                             }
                        }

                        if(data_info.bitmap_data.bit_ptr != PNULL)
                        {
                            SCI_FREE(data_info.bitmap_data.bit_ptr);
                            data_info.bitmap_data.bit_ptr = PNULL;
                        }
                   }
               }
            }
...
)
{
    ...
    for (item_index = 0; item_index < item_total_num; item_index++)
    {
        ...
        if (CTRLSTBDATA_IsItemVisible (item_index) && !CTRLSTBDATA_IsItemDisable (item_index)) // 可见的有效的
        {
...
#if 0
/*****************************************************************************/
// 	Description : SetThemeImageData
//	Global resource dependence : 
//  Author: Roy.Tsai
//	Note:
/*****************************************************************************/
PUBLIC void CTRLSTATUSBAR_SetThemeImageData(
                                MMI_HANDLE_T win_handle,//in
                                STATUSBAR_DATA_IMAGE_T image_data
                                )
{
    ...
    {
        //set icon
        ctrl_ptr->style_info.theme_image_data.img_source = image_data.img_source;
        ctrl_ptr->style_info.theme_image_data.res.id = image_data.res.id;
        ctrl_ptr->style_info.theme_image_data.file.filename_len = image_data.file.filename_len;
        ctrl_ptr->style_info.theme_image_data.file.filename_ptr = image_data.file.filename_ptr;
        ctrl_ptr->style_info.theme_image_data.data_ptr = image_data.data_ptr;
    }	
}
#endif
/*****************************************************************************/
...
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CTRLSTATUSBAR_GetBgIsDisplay (
    MMI_HANDLE_T win_handle
)
{
    BOOLEAN is_display_bg = TRUE;

    CTRLSTATUSBAR_OBJ_T *ctrl_ptr = GetStatusbarPtr(MMK_GetWinStatusBarHandle(win_handle));

    if (PNULL != ctrl_ptr)
    {
        is_display_bg = ctrl_ptr->is_display_bg;
    }

    return is_display_bg;
}
