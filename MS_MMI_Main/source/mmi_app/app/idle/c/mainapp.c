/****************************************************************************
** File Name:      manapp.c                                                *
** Author:                                                                 *
** Date:           03/11/2003                                              *
** Copyright:      2003 Spreadtrum, Incorporated. All Rights Reserved.       *
** Description:    This file is used to describe the message               *
****************************************************************************
**                         Important Edit History                          *
** ------------------------------------------------------------------------*
** DATE           NAME             DESCRIPTION                             *
** 11/2003       Louis.wei         Create
**
****************************************************************************/
#define _MAIN_APP_C_


/**---------------------------------------------------------------------------*
 **                         Dependencies                                      *
 **---------------------------------------------------------------------------*/
#include "mmi_app_idle_trc.h"
#ifdef WIN32
#include "std_header.h"
#endif
#include "mmk_app.h"
#include "mmk_kbd.h"
#include "guilcd.h"
#include "window_parse.h"
#include "mmiidle_export.h"
#include "mmi_image.h"
#include "guicommon.h"
#include "mmi_text.h"
#include "tb_dal.h"
#include "mmidisplay_data.h"
#include "mmi_menutable.h"
#include "guimenu.h"
//#include "mmiset.h"
#include "mmiset_export.h"
#include "mmiset_call_export.h"
#include "mmiset_display.h"
#include "mmisms_export.h"
#include "mmicl_export.h"
//#include "mmieng.h"
#include "mmieng_export.h"
#include "mmistk_export.h"
#include "mmicc_export.h"
#include "mmiphone_export.h"
#include "mmienvset_export.h"
#include "mmi_appmsg.h"
#include "mmiidle_subwintab.h"
#include "guimsgbox.h"
#include "guilabel.h"
#include "mmk_timer.h"
#include "mmi_default.h"
#include "mmiudisk_export.h"
#include "mmimms_export.h"
#include "mmiparse_export.h"
#include "mmimsd_export.h"
#include "mmi_textfun.h"
#include "mmikl_export.h"
#include "mmisd_export.h"
#include "mmiacc.h"
#include "guiblock.h"
#include "mmifm_export.h"
#include "mmitv_out.h"
#include "guibutton.h"
#ifndef WIN32
#include "gpio_prod_api.h"
#endif
#include "mmicalendar_export.h"
//#include "mmieng.h"
#include "mmieng_export.h"
#include "mmipb_export.h"
#include "mmipub.h"
#include "mmiss_export.h"
//#include "mmiebook.h"
#include "mmiebook_export.h"
#include "guistatusbar.h"
#include "mmialarm_id.h"
#include "mmisms_id.h"
#include "mms_image.h"
#include "mmiset_id.h"
#include "mmicc_id.h"
#include "mmiota_image.h"
#include "mmibt_export.h"
//#include "mmiset_nv.h"
#include "mmiset_text.h"
#include "mmimp3_export.h"
#include "mn_type.h"
#include "mn_api.h"
#include "mmipb_id.h"
#include "mmimp3_id.h"
#include "mmiacc_id.h"
#ifdef JAVA_SUPPORT
#include "mmijava_export.h"
#endif
#include "block_mem_def.h"
#include "block_mem.h"
#include "mmicountedtime_export.h"
#include "mmi_wallpaper_export.h"
#include "mmifmm_export.h"
#include "mmidc_export.h"
#include "mmischedule_export.h"

#include "mmi_mainmenu_export.h"
#ifdef MMI_SPRITE_ENABLE
#include "mmiss_sprite.h"
#endif
#ifdef KURO_SUPPORT
#include "mmikur.h"
#endif
#include "mmk_msg.h"
#include "guiwin.h"
#ifdef MMI_MOCOR_SAMPLE
#include "sample_export.h"
#endif
#include "ui_layer.h"
#include "guires.h"
#include "mmimtv_export.h"
#include "mmiatv_export.h"
#include "guistring.h"
#include "mmipdp_export.h"
#ifdef BROWSER_SUPPORT
#include "mmibrowser_export.h"
#endif
#include "mmidcd_export.h"
#include "guitab.h"
#ifdef DRM_SUPPORT
#include "mmidrm_export.h"
#endif
#ifdef FLASH_SUPPORT
#include "mmiflash_export.h"
#endif
#ifdef QQ_SUPPORT
#include "mmiqq_export.h"
#endif
#if (defined MMIWIDGET_SUPPORT) || (defined MMI_GRID_IDLE_SUPPORT) || (defined QBTHEME_SUPPORT)
#include "mmiwidget.h"
#include "mmiwidget_export.h"
#endif
#if defined QBTHEME_SUPPORT
#include "mmiqbtheme_export.h"
#endif
#if (defined QBTHEME_SUPPORT)
#include "mmiqbtheme_idleexport.h"
#endif

#ifdef PUSH_EMAIL_SUPPORT
#include "mmimail_export.h"
#endif

#ifdef WEATHER_SUPPORT
#include "mmiweather.h"
#endif

#ifdef MMIEMAIL_SUPPORT
#include "mmiemail_export.h"
#endif
#ifdef MMI_VIDEOWALLPAPER_SUPPORT
#include "mmiset_videowallpaper.h"
#endif

#include "mmk_gsensor.h"
#include "mmiset_func.h"
#ifdef PDA_UI_DROPDOWN_WIN
#include "mmidropdownwin_export.h"
#endif
#ifdef WIFI_SUPPORT
#include "mmiwifi_export.h"
#endif
#include "mmibt_text.h"

#include "mmisms_read.h"

#ifdef LIVE_WALLPAPER_FRAMEWORK_SUPPORT
#include "mmilivewallpaper_export.h"
#endif
#ifdef MMI_TASK_MANAGER
#include "mmi_applet_manager.h"
#endif

#include "mmiidle_cstyle.h"
#include "mmiidle_statusbar.h"
#include "mmicom_statusarea.h"
#ifdef MMI_IDLE_NO_SIM_SOS_SUPPORT
#include "mmicc_export.h"
#endif

#ifdef LIVE_WALLPAPER_FRAMEWORK_SUPPORT
#include "mmi_utility.h"
#endif

#ifdef SXH_APPLE_SUPPORT
#include "apple_export.h"
#endif

#include "mmi_autotest.h"
#ifdef VIDEO_PLAYER_SUPPORT
#include "mmivp_export.h"
#endif

#ifdef MMI_DEVICELOCK_SUPPORT
#include "mmi_devicelock_export.h"
#endif
#include "mmicom_statusarea.h"
#ifdef NPS_SUPPORT
#include "nps_export.h"
#endif

#ifdef MMIWNS_SUPPORT
#include "mmiwns_export.h"
#endif

#ifdef BUSMONITOR_SUPPORT
#ifndef WIN32
#include"busmonitor.h"
#endif
#endif

#if defined( TTS_SUPPORT)||defined(HERO_ENGINE_TTS_SUPPORT)
#include"mmitts_export.h"
#endif

/**---------------------------------------------------------------------------*
 **                         Macro Declaration                                 *
 **---------------------------------------------------------------------------*/


// MAIN screen icon
#define MMI_IDLE_VIBRA_DEFAULT_TIME 1500  //idle下长按#切换情景模式时振动时长
//the number display rect
//#define MMI_MAINAPP_DIALNUMBER_RECT     {MMI_MAINAPP_DIALNUMBER_RECT_LEFT, MMI_MAINAPP_DIALNUMBER_RECT_TOP, MMI_MAINAPP_DIALNUMBER_RECT_RIGHT, MMI_MAINAPP_DIALNUMBER_RECT_BOTTOM}
//#define MMI_IDLE_CALE_MAX_LEN   136  //idle日程名称最大长度

//#define MMI_IDLE_SUSP_POINTS_WIDTH  10//"..."显示所占的宽度


/**---------------------------------------------------------------------------*
 **                         Constant Variables                                *
 **---------------------------------------------------------------------------*/
// main application modual
MMI_APPLICATION_T   g_main_app;
const MMI_WIN_ID_T  g_idle_win = MAIN_IDLE_WIN_ID;       // idle窗口的id
LOCAL uint8         g_animal_time_id = 0;    //动画时钟

#ifndef FLASH_SUPPORT
LOCAL BOOLEAN               s_idlewin_is_open; //idle窗口是否打开
#endif
extern void  LCD_CheckLcdESD (void);

//LOCAL uint8 s_sim_error_time_id = 0;    // no sim or sim errror窗口定时器，用来显示正在充电

#if defined(MMI_INDIAN_SOS_SUPPORT) && defined(MMI_PDA_SUPPORT)
LOCAL uint8 s_sos_idle_key_pressed_count = 0;
LOCAL uint8 s_sos_idle_count_timer_id = 0;
#endif

/**---------------------------------------------------------------------------*
 **                         Functions                                         *
 **---------------------------------------------------------------------------*/
/*****************************************************************************/
//  Description : start Hard icon for tp
//  Global resource dependence :
//  Author: xiaoqing.lu
//  Note:
/*****************************************************************************/
LOCAL void StartHdShortCutBarIcon(void);

/*****************************************************************************/
//  Description : the process message function of the show window message
//  Global resource dependence :
//  Author:
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E IdleWin_HandleMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param);

/*****************************************************************************/
//  Description : handle idle message
//  Global resource dependence :
//  Author: Jassmine
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleIdleMsg(
    MMI_WIN_ID_T         win_id,
    MMI_MESSAGE_ID_E     msg_id,
    DPARAM               param
);
#ifndef TTS_SUPPORT_NONE
#ifndef WIN32
PUBLIC BOOLEAN  Hourly_chime_To_Speech(void);
#endif
#endif

#if defined(MMI_INDIAN_SOS_SUPPORT) && defined(MMI_PDA_SUPPORT)
/*****************************************************************************/
//  Description : handle the logic of sos mode entry 
//  Global resource dependence : 
//  Author: 
//  Note: 
/*****************************************************************************/
LOCAL void HandleSOSEntryType(uint8 timer_id, uint32 param);
#endif
#ifdef QQ_SUPPORT
/*****************************************************************************/
//  Description :InitIdleQQState
//  Global resource dependence : none
//  Author:
//  Note:
/*****************************************************************************/
LOCAL  void InitIdleQQState(void);
#endif
#ifdef MMI_IDLE_NO_SIM_SOS_SUPPORT
/*****************************************************************************/
//  Description : to handle  add blacklist open setting query window
//  Global resource dependence :
//  Author:dave.ruan
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleFlyModeCloseQueryWinMsg(
    MMI_WIN_ID_T        win_id,    //IN:
    MMI_MESSAGE_ID_E    msg_id,    //IN:
    DPARAM                param    //IN:
);
/*****************************************************************************/
//  Description :飞行模式设置的窗口处理函数
//  Global resource dependence :
//  Author:chunjie.liu
//  Note:
/*****************************************************************************/
LOCAL void SetFlyMode(void);
#endif
/**---------------------------------------------------------------------------*
 **                         difination                                        *
 **---------------------------------------------------------------------------*/

/*****************************************************************************/
//  Description : display the main and sub idle soft icon
//  Global resource dependence :
//  Author: Great.Tian
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPIIDLE_IdleWinIsOpen(void)
{
#ifndef FLASH_SUPPORT
    return (s_idlewin_is_open);
#else
    return MMIAPIFlash_FlashIsVisiable();
#endif
}

/*****************************************************************************/
//  Description : start the idle timer
//  Global resource dependence :
//  Author:
//  Note:
/*****************************************************************************/
LOCAL void StartAnimTimer(void)
{
    MMISET_WALLPAPER_SET_T      wallpaper_info = {0};

    if( (MMK_IsFocusWin(MAIN_IDLE_WIN_ID))  && (MMIDEFAULT_GetFlipStatus()) )
    {
        //动画时钟
        if(0 == g_animal_time_id)
        {
            //get wallpaper info
            MMIAPISET_GetWallpaperSetInfo(&wallpaper_info);
            if(MMISET_WALLPAPER_FIXED_ANIM == wallpaper_info.wallpaper_type)
            {
                g_animal_time_id = MMK_CreateWinTimer(MAIN_IDLE_WIN_ID, PHONE_ANIM_TIME_INTERVAL, FALSE);
            }
        }
    }
}

/*****************************************************************************/
//  Description : stop the idle timer
//  Global resource dependence :
//  Author:
//  Note:
/*****************************************************************************/
LOCAL void StopAnimTimer(void)
{
    //动画时间
    if (0 < g_animal_time_id )
    {
        MMK_StopTimer( g_animal_time_id );
        g_animal_time_id = 0;
    }
}

#if defined(MMI_INDIAN_SOS_SUPPORT) && defined(MMI_PDA_SUPPORT)
/*****************************************************************************/
//  Description : handle the logic of sos mode entry 
//  Global resource dependence : 
//  Author: 
//  Note: 
/*****************************************************************************/
LOCAL void HandleSOSEntryType(uint8 timer_id, uint32 param)
{
	BOOLEAN is_reset_now = (BOOLEAN)param;

	SCI_TRACE_LOW("[SOS] HandleSOSEntryType IN: s_sos_idle_count_timer_id = %d, s_sos_idle_key_pressed_count = %d", s_sos_idle_count_timer_id,s_sos_idle_key_pressed_count);
	if(!is_reset_now)
	{
		if(s_sos_idle_count_timer_id)
		{
			if(MMISET_SOS_ENTRY_MAGIC_TICK == (++s_sos_idle_key_pressed_count))
			{
				s_sos_idle_key_pressed_count = 0;
				MMK_StopTimer(s_sos_idle_count_timer_id);  
				s_sos_idle_count_timer_id = 0;

				MMIAPISET_StartSOSProcedures();
			}
		}
		else
		{
			s_sos_idle_key_pressed_count++;
			s_sos_idle_count_timer_id = MMK_CreateTimerCallback(MMISET_SOS_ENTRY_TIME_LIMIT, HandleSOSEntryType, TRUE, FALSE);
		}
	}
	else//if(is_reset_now)
	{
		s_sos_idle_key_pressed_count = 0;
		if(s_sos_idle_count_timer_id)
		{
			MMK_StopTimer(s_sos_idle_count_timer_id);  
			s_sos_idle_count_timer_id = 0;
		}
	}
	SCI_TRACE_LOW("[SOS] HandleSOSEntryType OUT: s_sos_idle_count_timer_id = %d, s_sos_idle_key_pressed_count = %d", s_sos_idle_count_timer_id,s_sos_idle_key_pressed_count);
}
#endif

/*****************************************************************************/
//  Description : update idle
//  Global resource dependence :
//  Author:
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIIDLE_MAIN_UpdateIdle(void)
{
    if (MAIN_IDLE_WIN_ID == MMK_GetFocusWinId())
    {
        MMIIDLE_SendMsgToIdleWin(MSG_IDLE_UPDATE_MP3, PNULL);
    }
}

/*****************************************************************************/
//  Description : 全键盘下，某些界面按下数字/字母共享键，转为对应的数字消息
//  Global resource dependence :
//  Author: tao.xue
//  Note:
/*****************************************************************************/
PUBLIC uint16 MMIAPIIDLE_GetNumMsgId(uint16 orign_msg_id)
{
    uint16 corresponding_msg_id = orign_msg_id;
    switch (orign_msg_id)
    {
    case MSG_KEYUP_R:
        corresponding_msg_id = MSG_KEYUP_1;
        break;

    case MSG_KEYUP_T:
        corresponding_msg_id = MSG_KEYUP_2;
        break;

    case MSG_KEYUP_Y:
        corresponding_msg_id = MSG_KEYUP_3;
        break;

    case MSG_KEYUP_U:
        corresponding_msg_id = MSG_KEYUP_STAR;
        break;

    case MSG_KEYUP_F:
        corresponding_msg_id = MSG_KEYUP_4;
        break;

    case MSG_KEYUP_G:
        corresponding_msg_id = MSG_KEYUP_5;
        break;

    case MSG_KEYUP_H:
        corresponding_msg_id = MSG_KEYUP_6;
        break;

    case MSG_KEYUP_J:
        corresponding_msg_id = MSG_KEYUP_HASH;
        break;

    case MSG_KEYUP_V:
        corresponding_msg_id = MSG_KEYUP_7;
        break;

    case MSG_KEYUP_B:
        corresponding_msg_id = MSG_KEYUP_8;
        break;

    case MSG_KEYUP_N:
        corresponding_msg_id = MSG_KEYUP_9;
        break;

    case MSG_KEYUP_M:
        corresponding_msg_id = MSG_KEYUP_0;
        break;

    case MSG_KEYDOWN_R:
        corresponding_msg_id = MSG_KEYDOWN_1;
        break;

    case MSG_KEYDOWN_T:
        corresponding_msg_id = MSG_KEYDOWN_2;
        break;

    case MSG_KEYDOWN_Y:
        corresponding_msg_id = MSG_KEYDOWN_3;
        break;

    case MSG_KEYDOWN_U:
        corresponding_msg_id = MSG_KEYDOWN_STAR;
        break;

    case MSG_KEYDOWN_F:
        corresponding_msg_id = MSG_KEYDOWN_4;
        break;

    case MSG_KEYDOWN_G:
        corresponding_msg_id = MSG_KEYDOWN_5;
        break;

    case MSG_KEYDOWN_H:
        corresponding_msg_id = MSG_KEYDOWN_6;
        break;

    case MSG_KEYDOWN_J:
        corresponding_msg_id = MSG_KEYDOWN_HASH;
        break;

    case MSG_KEYDOWN_V:
        corresponding_msg_id = MSG_KEYDOWN_7;
        break;

    case MSG_KEYDOWN_B:
        corresponding_msg_id = MSG_KEYDOWN_8;
        break;

    case MSG_KEYDOWN_N:
        corresponding_msg_id = MSG_KEYDOWN_9;
        break;

    case MSG_KEYDOWN_M:
        corresponding_msg_id = MSG_KEYDOWN_0;
        break;

    case MSG_KEYLONG_R:
        corresponding_msg_id = MSG_KEYLONG_1;
        break;

    case MSG_KEYLONG_T:
        corresponding_msg_id = MSG_KEYLONG_2;
        break;

    case MSG_KEYLONG_Y:
        corresponding_msg_id = MSG_KEYLONG_3;
        break;

    case MSG_KEYLONG_U:
        corresponding_msg_id = MSG_KEYLONG_STAR;
        break;

    case MSG_KEYLONG_F:
        corresponding_msg_id = MSG_KEYLONG_4;
        break;

    case MSG_KEYLONG_G:
        corresponding_msg_id = MSG_KEYLONG_5;
        break;

    case MSG_KEYLONG_H:
        corresponding_msg_id = MSG_KEYLONG_6;
        break;

    case MSG_KEYLONG_J:
        corresponding_msg_id = MSG_KEYLONG_HASH;
        break;

    case MSG_KEYLONG_V:
        corresponding_msg_id = MSG_KEYLONG_7;
        break;

    case MSG_KEYLONG_B:
        corresponding_msg_id = MSG_KEYLONG_8;
        break;

    case MSG_KEYLONG_N:
        corresponding_msg_id = MSG_KEYLONG_9;
        break;

    case MSG_KEYLONG_M:
        corresponding_msg_id = MSG_KEYLONG_0;
        break;

    default:
        break;
    }

    return corresponding_msg_id;
}

/*****************************************************************************/
//  Description : the process message function of the main application
//  Global resource dependence :
//  Author: Great.Tian
//  Note: 2003-12-09
/*****************************************************************************/
MMI_RESULT_E MAIN_HandleSysMsg(PWND app_ptr, uint16 msg_id, DPARAM param)
{
    //MMI_RESULT_E    recode  = MMI_RESULT_FALSE;

    switch(msg_id)
    {
    case MISC_MSG_RTC_SEC:
        if (MMIDEFAULT_IsBacklightOn()
                && MAIN_IDLE_WIN_ID == MMK_GetFocusWinId())
        {
            MMIIDLE_SendMsgToIdleWin( MSG_IDLE_UPDATE_TIME_SECOND, PNULL);
            SCI_TRACE_LOW("MAINAPP MAIN_HandleSysMsg MISC_MSG_RTC_SEC time == %d", SCI_GetTickCount());
        }
        break;

    case MISC_MSG_RTC_MIN:
        SCI_TRACE_LOW("MAINAPP MAIN_HandleSysMsg MISC_MSG_RTC_HOUR time == %d", SCI_GetTickCount());
        if (MMIDEFAULT_IsBacklightOn())
        {

            if (MAIN_IDLE_WIN_ID == MMK_GetFocusWinId())
            {
                MMIIDLE_SendMsgToIdleWin( MSG_IDLE_UPDATE_DATETIME, PNULL);
            }

#ifdef MMI_DEVICELOCK_SUPPORT
            if (MMI_DEVICELOCK_WIN_ID == MMK_GetFocusWinId())
            {
                MMIAPIDEVICELOCK_SendHourOrMinuteInd();
            }
#endif

            if (MMIAPIKL_IsPhoneLocked())
            {
                MMIAPIKL_SendHourOrMinuteInd();
            }
#ifndef SUBLCD_SIZE_NONE
            //刷新小屏的显示，时间分
            MMISUB_UpdateDisplaySubLcd();
#endif
        }
		#ifndef TTS_SUPPORT_NONE
		#ifndef WIN32
	if (STARTUP_NORMAL == MMIAPIPHONE_GetStartUpCondition())
		{
			Hourly_chime_To_Speech();
		}
	#endif
#endif
        // 更新状态栏的时间
        MMIAPICOM_StatusAreaSetDispTime();
        // dcd相关
        MMIAPIDCD_HandleSysMsg(app_ptr, msg_id, param);

#if defined(MMI_MSD_SUPPORT)
        MMIAPIMSD_MinuteCheck();
#endif
#ifdef MMIPB_SYNC_WITH_O365
		MMIPB_SYNC_Check_HeartBeat();
#endif
#ifdef HERO_ENGINE_SUPPORT
		{
			#if !defined(WIN32)
			extern void hero_im_heart_beat();
			extern void hero_auto_clean_spam_msg(void);
			hero_im_heart_beat();
			hero_auto_clean_spam_msg();
			#endif
		}
//modify 20181120 begin
#if defined(HERO_APP_SMART_OPEN) || (defined(HERO_ENGINE_FAMILY_SUPPORT) && defined(HERO_ENGINE_FAMILY_CONTROL_SUPPORT))
		{
			extern void hr_notify_reminder();
			hr_notify_reminder();
		}
#endif
//modify 20181120 end
#if (defined(HERO_ENGINE_FAMILY_SUPPORT) && defined(HERO_ENGINE_FAMILY_CONTROL_SUPPORT))
		{
			extern void hero_family_try_connect_tcp_channel();
			hero_family_try_connect_tcp_channel();
		}
#endif

#endif
#if defined(HERO_ENGINE_TTS_SUPPORT) && !defined(WIN32)
		{
			extern void hero_tts_auto_time_broadcast(void);
			if (STARTUP_NORMAL == MMIAPIPHONE_GetStartUpCondition())
			{
				hero_tts_auto_time_broadcast();
				 
			}
		}
#endif
        break;

    case MISC_MSG_RTC_HOUR:
        SCI_TRACE_LOW("MAINAPP MAIN_HandleSysMsg MISC_MSG_RTC_HOUR time == %d", SCI_GetTickCount());

        if (MAIN_IDLE_WIN_ID == MMK_GetFocusWinId())
        {
            MMIIDLE_SendMsgToIdleWin(MSG_IDLE_UPDATE_DATETIME, PNULL);
        }

        if (MMIAPIKL_IsPhoneLocked())
        {
            MMIAPIKL_SendHourOrMinuteInd();
        }
#ifndef SUBLCD_SIZE_NONE
        //刷新小屏的显示，时间小时
        MMISUB_UpdateDisplaySubLcd();
#endif
#if defined(MMI_MSD_SUPPORT)
        MMIAPIMSD_HourCheck();
#endif
        break;

    default:
        break;
    }

    return MMI_RESULT_TRUE;		//  (recode);
}

/*****************************************************************************/
//  Description : busmonitor
//  Global resource dependence :
//  Author:
//  Note:
/*****************************************************************************/
#ifdef BUSMONITOR_SUPPORT
#ifndef WIN32
extern uint32 Image$$EXEC_KERNEL_IMAGE$$Length;
LOCAL void BmCallback(void)
{
#ifdef DSP_ADDR0_BUSMONITOR_ALWAYS_ON
   ////1.DSP area
   SCI_PASSERT(0,("Memory overlap monitor for DSP area"));/*assert verified*/
#else
   ////1.DSP area
   //SCI_PASSERT(0,("Memory overlap monitor for DSP area"));/*assert verified*/
   ////2.Iram area
   //SCI_PASSERT(0,("Memory overlap monitor for Iram area"));/*assert verified*/
   ////3.kernel area
   SCI_PASSERT(0,("Memory overlap monitor for Kernel area"));/*assert verified*/
#endif
}
#endif
#endif


LOCAL uint8  s_lcd_chk_timer_id    = 0;

LOCAL void LCD_ESD_CHECKER_FUNC( uint8  timer_id, uint32 param)
{
	if(MMIAPIENGESDTestMode_IsSet() == SCI_TRUE && s_lcd_chk_timer_id == timer_id)
	{
		LCD_CheckLcdESD();
		SCI_TraceLow("lcd: esd Timer Timeout [LCD_CHECK_esd]"); 
	}
}

LOCAL void LCD_ESD_CHECKER_START(void)
{
	if(MMIAPIENGESDTestMode_IsSet() == SCI_TRUE)
	{
		if (!MMK_IsTimerActive(s_lcd_chk_timer_id))
			s_lcd_chk_timer_id = MMK_CreateTimerCallback(3 * 1000, LCD_ESD_CHECKER_FUNC, 0, TRUE);
		SCI_TraceLow("lcd: Start LCD_CHECK_esd Timer "); 
	}
}

LOCAL void LCD_ESD_CHECKER_STOP(void)
{
	if(MMIAPIENGESDTestMode_IsSet() == SCI_TRUE)
	{
		if (MMK_IsTimerActive(s_lcd_chk_timer_id))
			MMK_StopTimer(s_lcd_chk_timer_id);
		s_lcd_chk_timer_id = 0;
		SCI_TraceLow("lcd: Stop LCD_CHECK_esd Timer "); 
	}
}
#if defined(TTS_SUPPORT)||defined(HERO_ENGINE_TTS_SUPPORT)
#ifdef PHONE_NUMBER_VOICE
	#ifndef WIN32
/*****************************************************************************/
//  Description : ??μ? ±¨ê±
//  Global resource dependence :  Hourly chime function
//  Author:
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN  Hourly_chime_To_Speech(void)
{
    wchar temp_wchar[30+1] = {0};
    wchar  play_text[10 + 13] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
   uint8  startTime = 0;
   uint8 endTime = 0;
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};
	uint8 time_str[4] = {0};//in/out
	wchar  timewstr[4]        = {0};
	uint16  time_len                 = {0}; 	
	uint8 min_str[4] = {0};//in/out
	wchar  minwstr[4]        = {0};
	uint16  min_len                 = {0}; 
	wchar  pointwstr[4]        = {0};
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	MMI_STRING_T min_string = {0};
	
	MMI_STRING_T ampmnight_string = {0};
	wchar  ampmnight_str[6] 	   = {0};
	 MMISET_TIME_DISPLAY_TYPE_E time_type = MMIAPISET_GetTimeDisplayType();
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();
#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}

	memset(time_str,0,sizeof(uint8)*4);
	memset(timewstr,0,sizeof(wchar)*4);
	memset(min_str,0,sizeof(uint8)*4);
	memset(minwstr,0,sizeof(wchar)*4);
	memset(pointwstr,0,sizeof(wchar)*4);
	memset(minstringwstr,0,sizeof(wchar)*4);
	memset(ampmnight_str,0,sizeof(wchar)*6);
	
#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif
	{
		return TRUE;			
	}


	if(TM_GetSysTime(&time)!= ERR_TM_NONE)
	{
		return TRUE;		
	}

	if(Get_Pointtime_readstatus() == 0 ||  time.min != 0)
	{
		return TRUE;	
	}
	if(Get_Pointtime_mode() == 1)
	{
		if(time.hour > 18 || time.hour < 8)
		{
			return TRUE;	
		}
	}
	else if(Get_Pointtime_mode() == 2)
	{
		if(time.hour > 23 || time.hour < 18)
		{
			return TRUE;	
		}
	}
	else if(Get_Pointtime_mode() == 3)
	{//全天模式
		
	}
	else if(Get_Pointtime_mode() == 4)
	{
		startTime = Get_StartTimeParam();
		endTime = Get_endTimeParam();
		if(endTime>=startTime)
		{
			if((time.hour < startTime) || (time.hour > endTime))
			{
				return TRUE ;
			}
		}
		else
		{	//(endTime < startTime)
			if((time.hour < startTime) && (time.hour > endTime))
			{
				return TRUE;
			}
		}
	}


    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_MENU);
    SCI_MEMSET(play_text, 0, sizeof(play_text));
   SCI_MEMSET(temp_wchar, 0, sizeof(wchar)*31);
      SCI_MEMSET(ampmnight_str, 0, sizeof(ampmnight_str)*6);
	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_ZDT_CUR_TIME,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }
	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_ZDT_TIME_POINT,&point_str);
	MMIAPICOM_Wstrncpy(pointwstr, point_str.wstr_ptr, point_str.wstr_len);
	

	
    TM_GetSysTime(&time);
	 if (MMISET_TIME_12HOURS == time_type)
	 {
		if(time.hour > 6 && time.hour < 13)
		{
			MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_TIME_AM,&ampmnight_string);
			MMIAPICOM_Wstrncpy(ampmnight_str, ampmnight_string.wstr_ptr, ampmnight_string.wstr_len);
		}
		#if 1
		else if(time.hour > 12 && time.hour < 20)
		{
			MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_TIME_PM,&ampmnight_string);
			MMIAPICOM_Wstrncpy(ampmnight_str, ampmnight_string.wstr_ptr, ampmnight_string.wstr_len);
		}
		else if((time.hour > 19 && time.hour < 24 ) ||(time.hour >= 0 && time.hour <= 6))
		{
			MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_TIME_NIGHT,&ampmnight_string);
			MMIAPICOM_Wstrncpy(ampmnight_str, ampmnight_string.wstr_ptr, ampmnight_string.wstr_len);

		}
		#endif
		//if((time.hour > 21 && time.hour < 24)||(time.hour >= 0 && time.hour <= 6))
		//{
		//	return;
		//}
		if ((MMISET_TIME_PM_HOUR < time.hour))
		{
			time.hour = time.hour - MMISET_TIME_PM_HOUR;
		}
		 if(time.hour == 0 &&  time.min  == 0)
		 {
			sprintf((char*) time_str, "%d", time.hour);
				time_len = strlen((char *)time_str);
			MMI_STRNTOWSTR( timewstr, 4, time_str, 4, time_len );
			MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);
			MMIAPICOM_Wstrcat(play_text, timewstr);
			MMIAPICOM_Wstrcat(play_text, pointwstr);
		 }
		else
		{
			if(time.hour >9)
			{
			sprintf((char*) time_str, "%02d", time.hour);
		}
		else
		{
			sprintf((char*) time_str, "%d", time.hour);
		}
		
		time_len = strlen((char *)time_str);
		MMI_STRNTOWSTR( timewstr, 4, time_str, 4, time_len );
		TM_GetSysTime(&time);
		sprintf((char*) min_str, "%02d", time.min);
		min_len = strlen((char *)min_str);
		MMI_STRNTOWSTR( minwstr, 4, min_str, 4, min_len );
		
		
		MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);
		MMIAPICOM_Wstrcat(play_text, ampmnight_str);
		// MMIAPICOM_Wstrncpy(play_text, timewstr, play_len);
		MMIAPICOM_Wstrcat(play_text, timewstr);
		MMIAPICOM_Wstrcat(play_text, pointwstr);
		//MMIAPICOM_Wstrcat(play_text, minwstr);
		//MMIAPICOM_Wstrcat(play_text, minstringwstr);

		
	 }
		
	 }
	 else
	 {
		  if(time.hour == 0 &&  time.min  == 0)
		 {
			sprintf((char*) time_str, "%d", time.hour);
			time_len = strlen((char *)time_str);
			MMI_STRNTOWSTR( timewstr, 4, time_str, 4, time_len );
			MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);
			MMIAPICOM_Wstrcat(play_text, timewstr);
			MMIAPICOM_Wstrcat(play_text, pointwstr);
		 }
		  else
		  {
		 if(time.hour < 10)
		{
			sprintf((char*) time_str, "%d", time.hour);
		}
		else
		{
			sprintf((char*) time_str, "%02d", time.hour);
		}
		//sprintf((char*) time_str, "%02d", time.hour);
		time_len = strlen((char *)time_str);
		MMI_STRNTOWSTR( timewstr, 4, time_str, 4, time_len );
		TM_GetSysTime(&time);
		sprintf((char*) min_str, "%02d", time.min);
		min_len = strlen((char *)min_str);
		MMI_STRNTOWSTR( minwstr, 4, min_str, 4, min_len );
		
		
		MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);
		// MMIAPICOM_Wstrncpy(play_text, timewstr, play_len);
		MMIAPICOM_Wstrcat(play_text, timewstr);
		MMIAPICOM_Wstrcat(play_text, pointwstr);
		//MMIAPICOM_Wstrcat(play_text, minwstr);
		//MMIAPICOM_Wstrcat(play_text, minstringwstr);

	 }
	 }
	 

	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }

    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol =  ZDT_GetVol_Num() ;///MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_MENU ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}

PUBLIC BOOLEAN  Hourtime_To_Speech(void)
{
    wchar temp_wchar[30+1] = {0};
    wchar  play_text[10 + 13] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
   
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};
	uint8 time_str[4] = {0};//in/out
	wchar  timewstr[4]        = {0};
	uint16  time_len                 = {0}; 	
	uint8 min_str[4] = {0};//in/out
	wchar  minwstr[4]        = {0};
	uint16  min_len                 = {0}; 
	wchar  pointwstr[4]        = {0};
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	MMI_STRING_T min_string = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();
	MMI_STRING_T ampmnight_string = {0};
	wchar  ampmnight_str[6] 	   = {0};
	 MMISET_TIME_DISPLAY_TYPE_E time_type = MMIAPISET_GetTimeDisplayType();
#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}
	memset(time_str,0,sizeof(uint8)*4);
	memset(timewstr,0,sizeof(wchar)*4);
	memset(min_str,0,sizeof(uint8)*4);
	memset(minwstr,0,sizeof(wchar)*4);
	memset(pointwstr,0,sizeof(wchar)*4);
	memset(minstringwstr,0,sizeof(wchar)*4);
	memset(ampmnight_str,0,sizeof(wchar)*6);
	
	if(ZDT_GetOnekeytimeSwitch_type()== 1)
	{
		return TRUE;
	}
	
	#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
		//return TRUE;			
	}

    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_MENU);
    SCI_MEMSET(play_text, 0, sizeof(play_text));
 SCI_MEMSET(temp_wchar, 0, sizeof(wchar)*31);
	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_ZDT_CUR_TIME,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }
	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_HOUR_TIME_POINT,&point_str);
	MMIAPICOM_Wstrncpy(pointwstr, point_str.wstr_ptr, point_str.wstr_len);
	
	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_ZDT_TIME_MIN,&min_string);
	MMIAPICOM_Wstrncpy(minstringwstr, min_string.wstr_ptr, min_string.wstr_len);

	
    TM_GetSysTime(&time);
	//if((time.hour > 21 && time.hour < 24)||(time.hour >= 0 && time.hour <= 6))
	//{
	//	return;
	//}
	 if (MMISET_TIME_12HOURS == time_type)
	 {
		if(time.hour > 6 && time.hour < 13)
		{
			MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_TIME_AM,&ampmnight_string);
			MMIAPICOM_Wstrncpy(ampmnight_str, ampmnight_string.wstr_ptr, ampmnight_string.wstr_len);
		}
		#if 1
		else if(time.hour > 12 && time.hour < 20)
		{
			MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_TIME_PM,&ampmnight_string);
			MMIAPICOM_Wstrncpy(ampmnight_str, ampmnight_string.wstr_ptr, ampmnight_string.wstr_len);
		}
		else if((time.hour > 19 && time.hour < 24 ) ||(time.hour >= 0 && time.hour <= 6))
		{
			MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_TIME_NIGHT,&ampmnight_string);
			MMIAPICOM_Wstrncpy(ampmnight_str, ampmnight_string.wstr_ptr, ampmnight_string.wstr_len);

		}
		#endif
		//if((time.hour > 21 && time.hour < 24)||(time.hour >= 0 && time.hour <= 6))
		//{
		//	return;
		//}
		if ((MMISET_TIME_PM_HOUR < time.hour))
		{
			time.hour = time.hour - MMISET_TIME_PM_HOUR;
		}
		  if(time.hour == 0 &&  time.min  == 0)
		 {
			sprintf((char*) time_str, "%d", time.hour);
				time_len = strlen((char *)time_str);
			MMI_STRNTOWSTR( timewstr, 4, time_str, 4, time_len );
			MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);
			MMIAPICOM_Wstrcat(play_text, timewstr);
			MMIAPICOM_Wstrcat(play_text, pointwstr);
		 }
		  else
		  {
				if(time.hour >9)
				{
			sprintf((char*) time_str, "%02d", time.hour);
		}
		else
		{
			sprintf((char*) time_str, "%d", time.hour);
		}
		//sprintf((char*) time_str, "%02d", time.hour);
			if( time.min  == 0)
			{
				time_len = strlen((char *)time_str);
				MMI_STRNTOWSTR( timewstr, 4, time_str, 4, time_len );
				MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);
				MMIAPICOM_Wstrcat(play_text, timewstr);
				MMIAPICOM_Wstrcat(play_text, pointwstr);
			}
			else
			{
				time_len = strlen((char *)time_str);
				MMI_STRNTOWSTR( timewstr, 4, time_str, 4, time_len );
				TM_GetSysTime(&time);
				sprintf((char*) min_str, "%02d", time.min);
				min_len = strlen((char *)min_str);
				MMI_STRNTOWSTR( minwstr, 4, min_str, 4, min_len );
				#if 0
				TM_GetSysTime(&time);
				sprintf((char*) min_str, "%02d", time.min);
				min_len = strlen((char *)min_str);
				MMI_STRNTOWSTR( minwstr, 4, min_str, 4, min_len );
				#endif
			
				
		MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);
		MMIAPICOM_Wstrcat(play_text, ampmnight_str);

		MMIAPICOM_Wstrcat(play_text, timewstr);
				MMIAPICOM_Wstrcat(play_text, pointwstr);
				MMIAPICOM_Wstrcat(play_text, minwstr);
				MMIAPICOM_Wstrcat(play_text, minstringwstr);

				}
		
	 }
		
	 }
	 else
	{
		  if(time.hour == 0 &&  time.min  == 0)
		 {
			sprintf((char*) time_str, "%d", time.hour);
				time_len = strlen((char *)time_str);
			MMI_STRNTOWSTR( timewstr, 4, time_str, 4, time_len );
			MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);
			MMIAPICOM_Wstrcat(play_text, timewstr);
			MMIAPICOM_Wstrcat(play_text, pointwstr);
		 }
		  else
		  {
				  if(time.hour >9)
				{
					sprintf((char*) time_str, "%02d", time.hour);
				}
				else
				{
					sprintf((char*) time_str, "%d", time.hour);
				}

			if( time.min  == 0)
			{
				time_len = strlen((char *)time_str);
				MMI_STRNTOWSTR( timewstr, 4, time_str, 4, time_len );
				MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);
				MMIAPICOM_Wstrcat(play_text, timewstr);
				MMIAPICOM_Wstrcat(play_text, pointwstr);
			}
			else
			{
				    time_len = strlen((char *)time_str);
				    MMI_STRNTOWSTR( timewstr, 4, time_str, 4, time_len );
			
				TM_GetSysTime(&time);
				sprintf((char*) min_str, "%02d", time.min);
				min_len = strlen((char *)min_str);
				MMI_STRNTOWSTR( minwstr, 4, min_str, 4, min_len );

				
			    MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);
			    MMIAPICOM_Wstrcat(play_text, timewstr);
				MMIAPICOM_Wstrcat(play_text, pointwstr);
				MMIAPICOM_Wstrcat(play_text, minwstr);
				MMIAPICOM_Wstrcat(play_text, minstringwstr);
				}
	}
	}
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }


    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol = ZDT_GetVol_Num();//MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_SMS ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}



/*****************************************************************************/
//  Description : ??μ? ±¨ê±
//  Global resource dependence :  Hourly chime function
//  Author:
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN  Charge_Start_To_Speech(void)
{
    wchar temp_wchar[10+1] = {0};
    wchar  play_text[10 + 13] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
   
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};
	uint8 time_str[4] = {0};//in/out
	wchar  timewstr[4]        = {0};
	uint16  time_len                 = {0}; 	
	uint8 min_str[4] = {0};//in/out
	wchar  minwstr[4]        = {0};
	uint16  min_len                 = {0}; 
	wchar  pointwstr[4]        = {0};
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();

	MMI_STRING_T min_string = {0};
	#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}
if (MMIAPICC_IsInState(CC_IN_CALL_STATE))
{
	return TRUE;	
}

#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
		return TRUE;			
	}
	if( 1 == ZDT_GetChargeSwitch_type())
	{
		return TRUE;		

	}


    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_SMS);
    SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_CHARGE_START,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }

	
    MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);


	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }


    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol = ZDT_GetVol_Num();//MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_SMS ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}



/*****************************************************************************/
//  Description : 3?μ?ò??ú
//  Global resource dependence :  Hourly chime function
//  Author:
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN  ChrWarning_To_Speech(void)
{
    wchar temp_wchar[10+1] = {0};
    wchar  play_text[10 + 13] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
   
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};
	uint8 time_str[4] = {0};//in/out
	wchar  timewstr[4]        = {0};
	uint16  time_len                 = {0}; 	
	uint8 min_str[4] = {0};//in/out
	wchar  minwstr[4]        = {0};
	uint16  min_len                 = {0}; 
	wchar  pointwstr[4]        = {0};
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();

	MMI_STRING_T min_string = {0};
	#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}

#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
		return TRUE;			
	}
if( 1 == ZDT_GetChargeSwitch_type())
	{
		return TRUE;		

	}
if (MMIAPICC_IsInState(CC_IN_CALL_STATE))
{
	return TRUE;	
}
    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_MENU);
    SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_LOWPOWER_RING,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }

	
    MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);


	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }



    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol = ZDT_GetVol_Num();//MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_SMS ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}

/*****************************************************************************/
//  Description : 3?μ??áê?
//  Global resource dependence :  Hourly chime function
//  Author:
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN  ChrDisconnect_To_Speech(void)
{
    wchar temp_wchar[10+1] = {0};
    wchar  play_text[10 + 13] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
   
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};
	uint8 time_str[4] = {0};//in/out
	wchar  timewstr[4]        = {0};
	uint16  time_len                 = {0}; 	
	uint8 min_str[4] = {0};//in/out
	wchar  minwstr[4]        = {0};
	uint16  min_len                 = {0}; 
	wchar  pointwstr[4]        = {0};
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();

	MMI_STRING_T min_string = {0};
	#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}
	if (MMIAPICC_IsInState(CC_IN_CALL_STATE))
	{
		return TRUE;	
	}
#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
		return TRUE;			
	}

if( 1 == ZDT_GetChargeSwitch_type())
	{
		return TRUE;		

	}
    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_SMS);
    SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_CHRDISCONNECT,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }

	
    MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);


	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }



    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol = ZDT_GetVol_Num();///MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_SMS ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}




/*****************************************************************************/
//  Description : 3?μ?ò??ú
//  Global resource dependence :  Hourly chime function
//  Author:
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN  Charge_Finish_To_Speech(void)
{
    wchar temp_wchar[10+1] = {0};
    wchar  play_text[10 + 13] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
   
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};
	uint8 time_str[4] = {0};//in/out
	wchar  timewstr[4]        = {0};
	uint16  time_len                 = {0}; 	
	uint8 min_str[4] = {0};//in/out
	wchar  minwstr[4]        = {0};
	uint16  min_len                 = {0}; 
	wchar  pointwstr[4]        = {0};
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();

	MMI_STRING_T min_string = {0};
	#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}

#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
	//	return TRUE;			
	}
if (MMIAPICC_IsInState(CC_IN_CALL_STATE))
{
	return TRUE;	
}

    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_MENU);
    SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_CHARGE_FINISH,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }

	
    MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);


	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }



    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol = ZDT_GetVol_Num();//MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_SMS ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}

PUBLIC BOOLEAN  Counted_OneTime_To_Speech(void)
{
    wchar temp_wchar[10+1] = {0};
    wchar  play_text[10 + 20] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
   
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};
	uint8 time_str[4] = {','};//in/out
	wchar  timewstr[4]        = {0};
	uint16  time_len                 = {0}; 	
	uint8 min_str[4] = {0};//in/out
	wchar  minwstr[4]        = {0};
	uint16  min_len                 = {0}; 
	wchar  pointwstr[4]        = {0};
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();

	MMI_STRING_T min_string = {0};
	#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}

#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
	//	return TRUE;			
	}


    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_MENU);
    SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_COUNTED_ONE_TIME,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }

	
    MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);
	time_len = strlen((char *)time_str);
	MMI_STRNTOWSTR( timewstr, 4, time_str, 4, time_len );

	MMIAPICOM_Wstrcat(play_text, timewstr);
	MMIAPICOM_Wstrcat(play_text, name_str_ptr);
	MMIAPICOM_Wstrcat(play_text, timewstr);
	MMIAPICOM_Wstrcat(play_text, name_str_ptr);
	MMIAPICOM_Wstrcat(play_text, timewstr);
	MMIAPICOM_Wstrcat(play_text, name_str_ptr);


	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }



    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol = ZDT_GetVol_Num();//MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_MENU ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}

PUBLIC BOOLEAN  Counted_OneTime_Start_Speech(void)
{
    wchar temp_wchar[10+1] = {0};
    wchar  play_text[10 + 13] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
   
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};
	uint8 time_str[4] = {0};//in/out
	wchar  timewstr[4]        = {0};
	uint16  time_len                 = {0}; 	
	uint8 min_str[4] = {0};//in/out
	wchar  minwstr[4]        = {0};
	uint16  min_len                 = {0}; 
	wchar  pointwstr[4]        = {0};
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();

	MMI_STRING_T min_string = {0};
	#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}

#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
	//	return TRUE;			
	}

	
    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_MENU);
    SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_COUNTED_ONE_TIME_START,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }

	
    MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);


	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }



    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol =ZDT_GetVol_Num();// MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_SMS ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}


PUBLIC BOOLEAN  Counted_HalfTime_To_Speech(void)
{
    wchar temp_wchar[10+1] = {0};
    wchar  play_text[10 + 20] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
   
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};
	uint8 time_str[4] = {','};//in/out
	wchar  timewstr[4]        = {0};
	uint16  time_len                 = {0}; 	
	uint8 min_str[4] = {0};//in/out
	wchar  minwstr[4]        = {0};
	uint16  min_len                 = {0}; 
	wchar  pointwstr[2]        = {','};
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();

	MMI_STRING_T min_string = {0};
	#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}

#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
	//	return TRUE;			
	}

	
    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_MENU);
    SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_COUNTED_HALF_HOUR,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }

	
    MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);
	time_len = strlen((char *)time_str);
	MMI_STRNTOWSTR( timewstr, 4, time_str, 4, time_len );

	MMIAPICOM_Wstrcat(play_text, timewstr);
	MMIAPICOM_Wstrcat(play_text, name_str_ptr);
	MMIAPICOM_Wstrcat(play_text, timewstr);
	MMIAPICOM_Wstrcat(play_text, name_str_ptr);
	MMIAPICOM_Wstrcat(play_text, timewstr);
	MMIAPICOM_Wstrcat(play_text, name_str_ptr);

	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }



    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol = ZDT_GetVol_Num();//MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_MENU ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}

PUBLIC BOOLEAN  Counted_HalfTime_Start_Speech(void)
{
    wchar temp_wchar[10+1] = {0};
    wchar  play_text[10 + 13] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
   
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};
	uint8 time_str[4] = {0};//in/out
	wchar  timewstr[4]        = {0};
	uint16  time_len                 = {0}; 	
	uint8 min_str[4] = {0};//in/out
	wchar  minwstr[4]        = {0};
	uint16  min_len                 = {0}; 
	wchar  pointwstr[4]        = {0};
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();

	MMI_STRING_T min_string = {0};
	#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}

#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
		//return TRUE;			
	}
    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_MENU);
    SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_COUNTED_HALF_HOUR_START,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }

	
    MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);


	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }



    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol = ZDT_GetVol_Num();//MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_SMS ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}


PUBLIC BOOLEAN  Counted_Time_Stop_Speech(void)
{
    wchar temp_wchar[10+1] = {0};
    wchar  play_text[10 + 13] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
   
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};
	uint8 time_str[4] = {0};//in/out
	wchar  timewstr[4]        = {0};
	uint16  time_len                 = {0}; 	
	uint8 min_str[4] = {0};//in/out
	wchar  minwstr[4]        = {0};
	uint16  min_len                 = {0}; 
	wchar  pointwstr[4]        = {0};
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();

	MMI_STRING_T min_string = {0};
	#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}

#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
		//return TRUE;			
	}

	
    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_MENU);
    SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_COUNTED_TIME_STOP,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }

	
    MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);


	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }



    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol = ZDT_GetVol_Num();//MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_SMS ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}


PUBLIC BOOLEAN  DateInfo_To_Speech(void)

{
	wchar temp_wchar[10+10] = {0};
	wchar  play_text[100] = {0};
	uint16 play_len = 0;
	wchar* name_str_ptr = PNULL;

	MMI_STRING_T current_name_str = {0};
	MMITTS_PLAY_PARAM_T play_param = {0};
	SCI_DATE_T				  sys_date = {0};
	SCI_TIME_T				  time				  = {0};
	wchar  yearstringwstr[8]		= {0};
	MMI_STRING_T year_string = {0};
	wchar  monstringwstr[8] 	   = {0};
	MMI_STRING_T mon_string = {0};
	wchar  datestringwstr[8]		  = {0};
	MMI_STRING_T date_string = {0};
	uint8 year_str[8] = {0};//in/out
	wchar  yearwstr[8]		  = {0};
	uint16	year_len	= 0;	
	uint8 mon_str[4] = {0};//in/out
	wchar  monwstr[8]		 = {0};
	uint16	mon_len 		= 0; 
	uint8 date_str[4] = {0};//in/out
	wchar  datewstr[8]		 = {0};
	uint16	date_len	= 0; 
	uint8 week_str[4] = {0};//in/out
	wchar  weekwstr[8]		 = {0};
	uint16	week_len	= 0; 

	wchar  lunarstringwstr[8]		  = {0};
	MMI_STRING_T lunar_string = {0};
	MMI_STRING_T  LunarDate_str ={0};
	wchar		lunar_name[30 + 1]={0};

	wchar  pointwstr[4]        = {0};	
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	MMI_STRING_T min_string = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();
	MMI_STRING_T		str_week	= {0};

	MMI_TEXT_ID_T       week_table[] = {
									         STXT_SUNDAY,//       TXT_SHORT_IDLE_SUNDAY,
								        STXT_MONDAY,//       TXT_SHORT_IDLE_MONDAY,
								        STXT_TUESDAY,//      TXT_SHORT_IDLE_TUESDAY,
								        STXT_WEDNESDAY,//       TXT_SHORT_IDLE_WEDNESDAY,
								        STXT_THURSDAY,//       TXT_SHORT_IDLE_THURSDAY,
								        STXT_FRIDAY,//       TXT_SHORT_IDLE_FRIDAY,
								        STXT_SATURDAY,//       TXT_SHORT_IDLE_SATURDAY
                                    };

	
#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}
	SCI_MEMSET(temp_wchar, 0, sizeof(temp_wchar));
	
#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
	//	return TRUE;			
	}
	//Trace_Log_Buf_Print("Hourly_chime_To_Speech");
	MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_SMS);
	SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_ZDT_CUR_DATE,&current_name_str);
	{
		name_str_ptr = temp_wchar;
	   MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
		play_len = MMIAPICOM_Wstrlen(name_str_ptr);
	}
	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_DATE_YEAR,&year_string);
	MMIAPICOM_Wstrncpy(yearstringwstr, year_string.wstr_ptr, year_string.wstr_len);
	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_DATE_MONTH,&mon_string);
	MMIAPICOM_Wstrncpy(monstringwstr, mon_string.wstr_ptr, mon_string.wstr_len);
	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_DATE_DAY,&date_string);
	MMIAPICOM_Wstrncpy(datestringwstr, date_string.wstr_ptr, date_string.wstr_len);
	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_LUNAR,&lunar_string);
	MMIAPICOM_Wstrncpy(lunarstringwstr, lunar_string.wstr_ptr, lunar_string.wstr_len);
	
	TM_GetSysTime(&time);
	TM_GetSysDate(&sys_date);
	sprintf((char*) year_str, "%04d", sys_date.year);
	year_len = strlen((char *)year_str);
	MMI_STRNTOWSTR( yearwstr, 8, year_str, 8, year_len );

	sprintf((char*) mon_str, "%02d", sys_date.mon);
	mon_len = strlen((char *)mon_str);
	MMI_STRNTOWSTR( monwstr, 4, mon_str, 4, mon_len );
	sprintf((char*) date_str, "%02d", sys_date.mday);
	date_len = strlen((char *)date_str);
	MMI_STRNTOWSTR( datewstr, 4, date_str, 4, date_len );
	
	MMI_GetLabelTextByLang(week_table[sys_date.wday],&str_week);

	MMIAPICOM_Wstrncpy(weekwstr, str_week.wstr_ptr, str_week.wstr_len);


	LunarDate_str.wstr_ptr = lunar_name; 
	ZDT_LunarDate(&LunarDate_str, 30);

	
	
	MMIAPICOM_Wstrncpy(play_text, name_str_ptr, MMIAPICOM_Wstrlen(name_str_ptr));
	MMIAPICOM_Wstrcat(play_text, yearwstr);
	MMIAPICOM_Wstrcat(play_text, yearstringwstr);
	MMIAPICOM_Wstrcat(play_text, monwstr);
	MMIAPICOM_Wstrcat(play_text, monstringwstr);
	MMIAPICOM_Wstrcat(play_text, datewstr);
	MMIAPICOM_Wstrcat(play_text, datestringwstr);
	MMIAPICOM_Wstrcat(play_text, weekwstr);
	MMIAPICOM_Wstrcat(play_text, lunarstringwstr);
	MMIAPICOM_Wstrcat(play_text, LunarDate_str.wstr_ptr);
	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }
	

	///Trace_Log_Buf_Print("Hourly_chime_To_Speech play_len=%d",play_len);
    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol = ZDT_GetVol_Num();//MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_SMS ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
   play_param.win_handle = MMIKL_CLOCK_DISP_WIN_ID;	
	play_param.digit_mode = TTS_DIGIT_VALUE ;//TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	//play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

}
#define MMILUNAR_NAME_MAX_LEN   30

PUBLIC BOOLEAN  LunarDateInfo_To_Speech(void)

{
	wchar temp_wchar[10+1] = {0};
	wchar  play_text[100] = {0};
	uint16 play_len = 0;
	wchar* name_str_ptr = PNULL;

	MMI_STRING_T current_name_str = {0};
	MMITTS_PLAY_PARAM_T play_param = {0};
	SCI_DATE_T				   sys_date = {0};
	SCI_TIME_T				   time 			   = {0};

	MMI_STRING_T  LunarDate_str ={0};
	wchar		lunar_name[30 + 1]={0};
	wchar  pointwstr[4] 	   = {0};

	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4] 	   = {0};
	MMI_STRING_T min_string = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();

#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}


#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
		//return TRUE;			
	}
	///Trace_Log_Buf_Print("Hourly_chime_To_Speech");
	MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_SMS);
	SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_ZDT_CUR_Lunardate,&current_name_str);
	{
		name_str_ptr = temp_wchar;
	   MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
		play_len = MMIAPICOM_Wstrlen(name_str_ptr);
	}

	LunarDate_str.wstr_ptr = lunar_name; 
	ZDT_LunarDate(&LunarDate_str, MMILUNAR_NAME_MAX_LEN);


	MMIAPICOM_Wstrncpy(play_text, name_str_ptr, MMIAPICOM_Wstrlen(name_str_ptr));
	MMIAPICOM_Wstrcat(play_text, LunarDate_str.wstr_ptr);


	play_len = MMIAPICOM_Wstrlen(play_text)*2;
	if(PNULL == play_text || 0 == play_len)
	{
	//SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
		return;
	}


	//Trace_Log_Buf_Print("Hourly_chime_To_Speech play_len=%d",play_len);
	play_param.txt_addr = (uint8 *)play_text;
	play_param.txt_len_in_byte = play_len;
	play_param.is_ucs2 = TRUE;
	play_param.play_times = 1;
	play_param.tts_vol = ZDT_GetVol_Num();// MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
	play_param.ring_type = TTS_RING_TYPE_SMS ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
    play_param.win_handle = MMIKL_CLOCK_DISP_WIN_ID;
	play_param.digit_mode = TTS_DIGIT_VALUE ;//TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
//	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE;	
	return MMIAPITTS_PlayText(&play_param); 



}


PUBLIC BOOLEAN  Weekday_To_Speech(void)

{
	wchar temp_wchar[10+1] = {0};
	wchar  play_text[100] = {0};
	uint16 play_len = 0;
	wchar* name_str_ptr = PNULL;

	MMI_STRING_T current_name_str = {0};
	MMITTS_PLAY_PARAM_T play_param = {0};
	SCI_DATE_T				  sys_date = {0};
	SCI_TIME_T				  time				  = {0};
	
	uint8 week_str[4] = {0};//in/out
	wchar  weekwstr[8]		 = {0};
	uint16	week_len	= 0; 


	wchar  pointwstr[4]        = {0};	
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	MMI_STRING_T min_string = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();
	MMI_STRING_T		str_week	= {0};

	MMI_TEXT_ID_T       week_table[] = {
									          STXT_SUNDAY,//       TXT_SHORT_IDLE_SUNDAY,
									        STXT_MONDAY,//       TXT_SHORT_IDLE_MONDAY,
									        STXT_TUESDAY,//      TXT_SHORT_IDLE_TUESDAY,
									        STXT_WEDNESDAY,//       TXT_SHORT_IDLE_WEDNESDAY,
									        STXT_THURSDAY,//       TXT_SHORT_IDLE_THURSDAY,
									        STXT_FRIDAY,//       TXT_SHORT_IDLE_FRIDAY,
									        STXT_SATURDAY,//       TXT_SHORT_IDLE_SATURDAY
                                    };

	
#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}

	
#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
	//	return TRUE;			
	}
	//Trace_Log_Buf_Print("Hourly_chime_To_Speech");
	MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_SMS);
	SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)qtn_outlook_today,&current_name_str);
	{
		name_str_ptr = temp_wchar;
	   MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
		play_len = MMIAPICOM_Wstrlen(name_str_ptr);
	}

	TM_GetSysDate(&sys_date);


	MMI_GetLabelTextByLang(week_table[sys_date.wday],&str_week);

	MMIAPICOM_Wstrncpy(weekwstr, str_week.wstr_ptr, str_week.wstr_len);

	
	MMIAPICOM_Wstrncpy(play_text, name_str_ptr, MMIAPICOM_Wstrlen(name_str_ptr));


	MMIAPICOM_Wstrcat(play_text, weekwstr);
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }
	

	//Trace_Log_Buf_Print("Hourly_chime_To_Speech play_len=%d",play_len);
    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol = ZDT_GetVol_Num();//MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_SMS ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
   play_param.win_handle = MMIKL_CLOCK_DISP_WIN_ID;	
	play_param.digit_mode = TTS_DIGIT_VALUE ;//TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	//play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

}


PUBLIC BOOLEAN  Bat_cap_To_Speech(void)

{
	wchar temp_wchar[10+1] = {0};
	wchar  play_text[100] = {0};
	uint16 play_len = 0;
	wchar* name_str_ptr = PNULL;
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();

	MMI_STRING_T current_name_str = {0};
	MMITTS_PLAY_PARAM_T play_param = {0};
	wchar  pointwstr[4] 	   = {0};
    MMI_STRING_T        bat_cap     = {0};
    uint32               bat_level   =  0;
    uint8                num_str[16] = {0};
    CHGMNG_STATE_INFO_T* p_chgmng_info = CHGMNG_GetModuleState();
    
#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}
	bat_level = p_chgmng_info->bat_remain_cap;

#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
		//return TRUE;			
	}
if (MMIAPICC_IsInState(CC_IN_CALL_STATE))
{
	return TRUE;	
}
	///Trace_Log_Buf_Print("Hourly_chime_To_Speech");
	MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_SMS);
	SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_BTRY_CAP_LABEL_TEXT,&current_name_str);
	{
		name_str_ptr = temp_wchar;
	   MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
		play_len = MMIAPICOM_Wstrlen(name_str_ptr);
	}

    SCI_MEMSET(num_str,0,sizeof(char)*16);
    sprintf((char*)num_str, "%d%s", (int)bat_level, "%");
    bat_cap.wstr_ptr = SCI_ALLOCA(sizeof(wchar)*5);
    if(PNULL == bat_cap.wstr_ptr)
    {
        //SCI_TRACE_LOW:"SetPowerSaveParam SCI_ALLOCA error!"
        ///SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMISET_PHONEWIN_2603_112_2_18_2_51_35_262,(uint8*)"");
        return ;
    }    
    MMIAPICOM_StrToWstr(num_str, bat_cap.wstr_ptr);
    bat_cap.wstr_len = MMIAPICOM_Wstrlen(bat_cap.wstr_ptr);


	MMIAPICOM_Wstrncpy(play_text, name_str_ptr, MMIAPICOM_Wstrlen(name_str_ptr));
	MMIAPICOM_Wstrcat(play_text, bat_cap.wstr_ptr);


	play_len = MMIAPICOM_Wstrlen(play_text)*2;
	if(PNULL == play_text || 0 == play_len)
	{
	//SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
		return;
	}


	///Trace_Log_Buf_Print("Hourly_chime_To_Speech play_len=%d",play_len);
	play_param.txt_addr = (uint8 *)play_text;
	play_param.txt_len_in_byte = play_len;
	play_param.is_ucs2 = TRUE;
	play_param.play_times = 1;
	#ifdef HERO_ENGINE_TTS_SUPPORT
	play_param.tts_vol = TTS_GetTTSVolume();
	#else
	play_param.tts_vol = ZDT_GetVol_Num();//MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
	#endif
	play_param.ring_type = TTS_RING_TYPE_SMS ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
    play_param.win_handle = MMIKL_CLOCK_DISP_WIN_ID;
	play_param.digit_mode = TTS_DIGIT_VALUE ;//TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
//	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE;	
	return MMIAPITTS_PlayText(&play_param); 



}





//static uint8			s_countedtime_timer_id	= 0;
//static uint8			s_countedhalftime_timer_id	= 0;

uint8 s_countedtime_time_id = 0;//μ1??ê±μ?timer


PUBLIC void MMIAPICounted_HandleTimer(void)
{

	MMIPUB_OpenAlertWinByTextId(PNULL,TXT_COUNTED_ONE_TIME,TXT_NULL,IMAGE_PUBWIN_WARNING,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);

	Counted_OneTime_To_Speech();
}


PUBLIC BOOLEAN CountedTime(void)
{

	if (0 != s_countedtime_time_id)
	{
		MMK_StopTimer(s_countedtime_time_id);//stop timer
		s_countedtime_time_id = 0;
	}

	s_countedtime_time_id = MMK_CreateTimerCallback(3600*1000,  MMIAPICounted_HandleTimer, NULL, TRUE);


}

PUBLIC void MMIAPIHalfCounted_HandleTimer(void)
{

	MMIPUB_OpenAlertWinByTextId(PNULL,TXT_COUNTED_HALF_HOUR,TXT_NULL,IMAGE_PUBWIN_WARNING,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);

	Counted_HalfTime_To_Speech();
}


PUBLIC BOOLEAN CountedHalfTime(void)
{

	if (0 != s_countedtime_time_id)
	{
		MMK_StopTimer(s_countedtime_time_id);//stop timer
		s_countedtime_time_id = 0;
	}

	s_countedtime_time_id = MMK_CreateTimerCallback(1800*1000,  MMIAPIHalfCounted_HandleTimer, NULL, TRUE);


}



PUBLIC BOOLEAN  Lock_To_Speech(void)
{
    wchar temp_wchar[10+1] = {0};
    wchar  play_text[10 + 13] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};

	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();
#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}

	if( 1 == ZDT_GetKLSwitch_type())
	{
		return TRUE;	
	}
	
#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
		return TRUE;			
	}
if (MMIAPICC_IsInState(CC_IN_CALL_STATE))
{
	return TRUE;	
}
    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_MENU);
    SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_LOCK_ALERT,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }

    MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);
 
	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }


	///Trace_Log_Buf_Print("Hourly_chime_To_Speech play_len=%d",play_len);
    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol = TTS_GetTTSVolume();//MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_MENU ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_VALUE;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}

PUBLIC BOOLEAN  UnLock_To_Speech(void)
{
    wchar temp_wchar[10+1] = {0};
    wchar  play_text[10 + 13] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};

	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();
#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}

	if( 1 == ZDT_GetKLSwitch_type())
	{
		return TRUE;	
	}
	if (MMIAPICC_IsInState(CC_IN_CALL_STATE))
{
	return TRUE;	
}
#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
		return TRUE;			
	}

    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_MENU);
    SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_UNLOCK_ALERT,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }

    MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);
 
	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }


	///Trace_Log_Buf_Print("Hourly_chime_To_Speech play_len=%d",play_len);
    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol = ZDT_GetVol_Num();//MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_MENU ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_VALUE;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}

PUBLIC BOOLEAN  KLSOS_Start_Speech(void)
{
    wchar temp_wchar[10+1] = {0};
    wchar  play_text[10 + 13] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
   
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};
	uint8 time_str[4] = {0};//in/out
	wchar  timewstr[4]        = {0};
	uint16  time_len                 = {0}; 	
	uint8 min_str[4] = {0};//in/out
	wchar  minwstr[4]        = {0};
	uint16  min_len                 = {0}; 
	wchar  pointwstr[4]        = {0};
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();

	MMI_STRING_T min_string = {0};
	#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}

#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
		return TRUE;			
	}
if (MMIAPICC_IsInState(CC_IN_CALL_STATE))
{
	return TRUE;	
}
	
    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_MENU);
    SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_KL_SOS,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }

	
    MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);


	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }



    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol =ZDT_GetVol_Num();// MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_SMS ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}


PUBLIC BOOLEAN  NewSMS_Start_Speech(void)
{
    wchar temp_wchar[10+1] = {0};
    wchar  play_text[10 + 13] = {0};
    uint16 play_len = 0;
    wchar* name_str_ptr = PNULL;
   
    MMI_STRING_T current_name_str = {0};
    MMITTS_PLAY_PARAM_T play_param = {0};
    SCI_TIME_T                  time                = {0};
	uint8 time_str[4] = {0};//in/out
	wchar  timewstr[4]        = {0};
	uint16  time_len                 = {0}; 	
	uint8 min_str[4] = {0};//in/out
	wchar  minwstr[4]        = {0};
	uint16  min_len                 = {0}; 
	wchar  pointwstr[4]        = {0};
	MMI_STRING_T point_str = {0};
	wchar  minstringwstr[4]        = {0};
	uint8 active_mode = MMIAPIENVSET_GetActiveModeId();

	MMI_STRING_T min_string = {0};
	#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
    MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
    MMIAPISET_GetLanguageType(&lang_type);
#endif

	if(lang_type !=MMISET_LANGUAGE_SIMP_CHINESE )
	{
		return;
	}

#ifdef ZDT_PRODUCT_VIBRATE    	
	if((active_mode == MMIENVSET_SILENT_MODE)|| (active_mode == MMIENVSET_MEETING_MODE))
#else
	if((active_mode == MMIENVSET_SILENT_MODE))
#endif

	{
		return TRUE;			
	}

	if( 1 == ZDT_GetNewSmsSwitch_type())
	{
		return;
	}
	if (MMIAPICC_IsInState(CC_IN_CALL_STATE))
	{
		return TRUE;	
	}
    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_MENU);
    SCI_MEMSET(play_text, 0, sizeof(play_text));

	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_NEW_SMS,&current_name_str);
    {
        name_str_ptr = temp_wchar;
       MMIAPICOM_Wstrncpy(name_str_ptr, current_name_str.wstr_ptr, current_name_str.wstr_len);
        play_len = MMIAPICOM_Wstrlen(name_str_ptr);
    }

	
    MMIAPICOM_Wstrncpy(play_text, name_str_ptr, play_len);


	
    play_len = MMIAPICOM_Wstrlen(play_text)*2;
    if(PNULL == play_text || 0 == play_len)
    {
        //SCI_TRACE_LOW:"[MMITTS]:TTSAPI_PlayTextForControl ERROR!!!!!!!!!!!!"
        return;
    }



    play_param.txt_addr = (uint8 *)play_text;
    play_param.txt_len_in_byte = play_len;
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 1;
    play_param.tts_vol =ZDT_GetVol_Num();// MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_READMENU);
    play_param.ring_type = TTS_RING_TYPE_SMS ;//TTS_RING_TYPE_MENU;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
	play_param.digit_mode = TTS_DIGIT_AUTO;
	play_param.punc_mode = TTS_PUNC_OFF;	
	play_param.volume_type = MMISRVAUD_VOLUME_TYPE_NONE; 	
    return MMIAPITTS_PlayText(&play_param);	

	
}
#endif
#endif
#endif  //TTS_SUPPORT_YOUNGTONE

LOCAL SCI_TIMER_PTR g_poweron_timer=SCI_NULL;
#define POWERON_TIME   3600*1*1000*3  //60*1000
BOOLEAN poweron_time_isto_record  =FALSE ;
PUBLIC void MMI_Record_Temp_timer(void)
{
	//Trace_Log_Print("MMI_Record_Temp_timer");
	poweron_time_isto_record = TRUE ;
}

void StartRecordPoweronTime(void)
{
	g_poweron_timer = MMK_CreateTimerCallback(POWERON_TIME,MMI_Record_Temp_timer,PNULL,TRUE);
}


#ifdef SOS_KEYLONGWEB

LOCAL uint8         g_sos_time_id = 0;    //
extern BOOLEAN  sos_is_connected ;
extern uint8    sos_times;
extern BOOLEAN  sos_temp;
PUBLIC BOOLEAN SOS_GetNUM_IsOK(void);


PUBLIC void SOS_EmergencyCallWin(void);
PUBLIC void SOS_EmergencyCallWin3(void);
PUBLIC void SOS_EmergencyCallWin2(void);


LOCAL void StartSosTimer(void);
LOCAL void StopSosTimer(void) ;


#endif

BOOLEAN  s_is_torch_open = FALSE;
PUBLIC void SwitchTorchLight(void)
{
       s_is_torch_open = !s_is_torch_open;
#ifndef WIN32
        GPIO_SetTorch(s_is_torch_open);
#endif
 MMIAPICOM_StatusAreaSetTorchState(s_is_torch_open);
}

BOOLEAN star_to_lock = FALSE ;
/*****************************************************************************/
//  Description : the process message function of the show window message
//  Global resource dependence :
//  Author:
//  Note:
/*****************************************************************************/
#ifdef HERO_ENGINE_TTS_SUPPORT
extern void hero_tts_akey_time_broadcast(void);
extern void hero_tts_akey_power_level_broadcast(void);
extern void hero_tts_akey_dateweeklunar_broadcast(void);
#endif
LOCAL MMI_RESULT_E IdleWin_HandleMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
    MMI_RESULT_E            recode              = MMI_RESULT_TRUE;
    static uint8            s_vibra_timer_id    = 0;
    uint8 stk_timer_id = 0;
    char appname[4] = {'S','T','K','\0'};
    GUI_LCD_DEV_INFO        lcd_dev_info        = {GUI_LCD_0, GUI_BLOCK_0};
    //    MMISET_IDLE_DISPLAY_T   *idle_mode          = PNULL;
    static BOOLEAN          s_is_first_power_on = TRUE;
    //#if  (defined MMI_GRID_IDLE_SUPPORT) || (defined MMIWIDGET_SUPPORT) || (defined MMIQBTHEME_SUPPORT)
    //#else
    //    static BOOLEAN          s_is_timer_on       = FALSE;
    //#endif
    //#if  (defined MMI_GRID_IDLE_SUPPORT) || (defined MMIWIDGET_SUPPORT) || MMIQBTHEME_SUPPORT
    //#else
    //static BOOLEAN          s_is_timer_on       = FALSE;
    //#endif
    LOCAL BOOLEAN           s_is_down_red_key   = FALSE;  //ê?·?°′á?red key
      LOCAL BOOLEAN           s_is_down_web_key   = FALSE;  //是否按了red key
         LOCAL BOOLEAN           s_is_down_ok_key   = FALSE;  //是否按了red key
      LOCAL BOOLEAN           s_is_down_0_key   = FALSE;  //ê?·?°′á?red key
       LOCAL BOOLEAN           s_is_down_star_key   = FALSE;  //是否按了red key
    static BOOLEAN          s_is_need_set_time  = TRUE;
#if defined(HERO_ENGINE_TTS_KEY_0_TIME)
     // LOCAL BOOLEAN           s_is_down_0_key  = FALSE;  
#endif
#if defined(HERO_ENGINE_TTS_KEY_8_TIME)
    static BOOLEAN          s_is_down_eight_key  = FALSE;
#endif
#ifdef HERO_ENGINE_TTS_KEY_7_TIME
	static BOOLEAN          s_is_down_7_key = FALSE;
#endif
#ifdef HERO_ENGINE_TTS_KEY_9_TIME
	static BOOLEAN			s_is_down_9_key = FALSE;
#endif
#ifdef HERO_ENGINE_TTS_KEY_STAR_TIME
	static BOOLEAN			s_is_down_star_key = FALSE;
#endif

    MMISET_WALLPAPER_SET_T  wallpaper_info = {0};


    lcd_dev_info.lcd_id     = GUI_MAIN_LCD_ID;
    lcd_dev_info.block_id   = GUI_BLOCK_MAIN;
    //   idle_mode = MMIAPISET_GetIdleDisplayMode();
    stk_timer_id = MMISTK_GetIdleTimerID();

    //IDLE????￡?è??ü?ì1¤3ì￡???è?Dè??12ó?μ?êy×?°′?ü??DD???￠×a???￡
    if ((MSG_KEYDOWN_Q <= msg_id && msg_id <= MSG_KEYDOWN_EXCLAMATION)
            || (MSG_KEYUP_Q <= msg_id && msg_id <= MSG_KEYUP_EXCLAMATION)
            || (MSG_KEYLONG_Q <= msg_id && msg_id <= MSG_KEYLONG_EXCLAMATION))
    {
        msg_id = MMIAPIIDLE_GetNumMsgId(msg_id);
    }

    //SCI_TRACE_LOW:"[idle] IdleWin_HandleMsg msg_id 0x%x"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT, MAINAPP_585_112_2_18_2_23_34_3, (uint8 *)"d", msg_id);
    switch(msg_id)
    {
    #if defined(MMI_INDIAN_SOS_SUPPORT) && defined(MMI_PDA_SUPPORT)
	case MSG_KEYUP_POWER:
		HandleSOSEntryType(0, FALSE);
		break;
#endif
    case MSG_GSENSOR:
    {
#ifdef VIDEO_PLAYER_SUPPORT
#ifndef MMI_VIDEOPLAYER_MINI_FUNCTION
        if((uint32 )param == MMK_GSENSOR_STATUS_LEFT_SIDE || (uint32 )param == MMK_GSENSOR_STATUS_RIGHT_SIDE\
                || (uint32 )param == MMK_GSENSOR_STATUS_UP_SIDE || (uint32 )param == MMK_GSENSOR_STATUS_DOWN_SIDE)
        {
            if(MMIAPIVP_IsOpenPlayerWin())
            {
                MMIAPIVP_SetVPWinSupportAllAngle();
            }
        }
#endif
#endif
        switch( (uint32 )param)
        {
            //[6800H]Dè?ó?μá??a?aG-Sensor ??×??Dμ???ò?ê×￡?2??D?°ò?ê×.......
        case MMK_GSENSOR_STATUS_LEFT:
        case MMK_GSENSOR_STATUS_RIGHT:
            if(MMISET_IsSensorOpen(MMISET_SENSOR_TYPE_NEXT_SONG))
            {
#ifdef KURO_SUPPORT
                if(MMIAPIKUR_IsPlayerPlaying())
                {
                    MMIAPIKUR_GoToPreOrNextKur(FALSE);
                }
#ifdef MMI_AUDIO_PLAYER_SUPPORT
                else if(MMIAPIMP3_IsPlayerPlaying())
                {
                    MMIAPIMP3_GoToPreOrNextMp3(FALSE);

                    MMIAPIMP3_SwitchAnimCtrlInfo(FALSE, FALSE);
                }
#endif
#else
#ifdef MMI_AUDIO_PLAYER_SUPPORT
                    if(MMIAPIMP3_IsPlayerPlaying())
                    {
                        MMIAPIMP3_GoToPreOrNextMp3(FALSE);
                        
                        MMIAPIMP3_SwitchAnimCtrlInfo(FALSE, FALSE);
                    }                    
#endif                   
#endif

            }

            if(MMISET_IsSensorOpen(MMISET_SENSOR_TYPE_REPLACE_WALLPAPER))
            {
                if(MMK_IsFocusWin(win_id)
#ifdef  MMIWIDGET_SUPPORT
                        && !MMIAPIWIDGET_IsTpMovingPage()
#endif
                        && (!MMISET_IsSensorOpen(MMISET_SENSOR_TYPE_NEXT_SONG)
					     ||(!MMIAPIMP3_IsPlayerActive()
#ifdef KURO_SUPPORT
					&& !MMIAPIKUR_IsPlayerActive()
#endif
)
                           )
                  )
                {
                    MMIAPIIDLE_Change_Wallpaper();
#ifdef LIVE_WALLPAPER_FRAMEWORK_SUPPORT
                    if(MMK_IsFocusWin(win_id) && MMIAPIIDLE_IsLiveWallpaper())
                    {
                        MMIAPILIVEWALLPAPER_Play(win_id);//NEWMS00178618
                    }
#endif
#ifdef MMI_VIDEOWALLPAPER_SUPPORT
                    if (MMK_IsFocusWin(win_id) && MMIAPISETVWP_IsOpen())
                    {
                        MMIAPISETVWP_StopVideoWallpaper();
#ifdef  VIDEOTHUMBNAIL_SUPPORT
                        MMIAPISETVWP_DisplayThumbnail();
#endif
                        MMIAPISETVWP_SwitchToNext();
                    }
#endif
                }
            }
            break;

#ifdef MMI_RES_ORIENT_BOTH
        case MMK_GSENSOR_STATUS_LEFT_SIDE:
            if ( MMISET_IsSensorOpen( MMISET_SENSOR_TYPE_LANDSCAPE_270 ) )//cr227614 maryxiao
            {
                MMIAPIATV_HandleSensorMSG();
                MMK_SetScreenAngle( LCD_ANGLE_270 );
            }
            break;
        case MMK_GSENSOR_STATUS_RIGHT_SIDE:
            if ( MMISET_IsSensorOpen( MMISET_SENSOR_TYPE_LANDSCAPE_90 ) )
            {
                MMIAPIATV_HandleSensorMSG();
                MMK_SetScreenAngle( LCD_ANGLE_90 );
            }
            break;
        case MMK_GSENSOR_STATUS_UP_SIDE:
            if ( MMISET_IsSensorOpen( MMISET_SENSOR_TYPE_LANDSCAPE_270 ) || MMISET_IsSensorOpen( MMISET_SENSOR_TYPE_LANDSCAPE_180) || MMISET_IsSensorOpen( MMISET_SENSOR_TYPE_LANDSCAPE_90))
            {
                MMIAPIATV_HandleSensorMSG();
                MMK_SetScreenAngle( LCD_ANGLE_0 );
            }
            break;
        case MMK_GSENSOR_STATUS_DOWN_SIDE:
            if ( MMISET_IsSensorOpen( MMISET_SENSOR_TYPE_LANDSCAPE_180 ) )
            {
                MMIAPIATV_HandleSensorMSG();
                MMK_SetScreenAngle( LCD_ANGLE_180 );
            }
            break;
#endif
        default:
            break;
        }
    }
    break;

#ifdef SXH_APPLE_SUPPORT
    //#ifdef __APPLE_ADDSMS__
    case APPLE_BASE2_MSG_ID:
    {
        AppleIDLEMsgInd(param); /*lint !e718 !e746*/
    }
    break;
    //#endif
#endif
    case MSG_OPEN_WINDOW:
        MMITHEME_InitMenuItemCache();
#ifdef DPHONE_SUPPORT
#ifndef WIN32
        if (GPIO_GetHookState())
        {
            MMIDEFAULT_SetHookStates(TRUE);
        }
        GPIO_EnableHookInt(TRUE);
#endif
#endif

	if(MMIENG_GetFirstTimeIsrecord() == 0)
		{
			StartRecordPoweronTime();
		}

	
#ifdef MMI_DEBUGLAYER_SUPPORT
        MMIAPICOM_CreateDebugLayer();
#endif

        //#ifdef MMI_RES_ORIENT_BOTH
        //MMK_OpenGsensor( win_id );
        MMISET_UpdateCommonGSensor();
        //#endif

#ifdef MMI_SPRITE_ENABLE
        MMIAPIML_CreateLayerTest();
#endif
        s_is_down_red_key = FALSE;
	s_is_down_0_key= FALSE;
	s_is_down_web_key = FALSE;
	s_is_down_ok_key = FALSE;
	s_is_down_star_key = FALSE;
#if defined(HERO_ENGINE_TTS_KEY_0_TIME)
        s_is_down_0_key = FALSE;
#endif
#if defined(HERO_ENGINE_TTS_KEY_8_TIME)
    s_is_down_eight_key  = FALSE;
#endif 
#ifdef HERO_ENGINE_TTS_KEY_7_TIME
		s_is_down_7_key = FALSE;
#endif
#ifdef HERO_ENGINE_TTS_KEY_9_TIME
		s_is_down_9_key = FALSE;
#endif
#ifdef HERO_ENGINE_TTS_KEY_STAR_TIME
		s_is_down_star_key = FALSE;
#endif
#ifdef MMI_SPRITE_ENABLE
        MMIIDLE_InitSprite();
#endif
#ifdef NPS_SUPPORT
        MMIAPINPS_StartNPSTimer();
#endif
        MMIDEFAULT_StartLcdBackLightTimer();
        MMIDEFAULT_StartAutoKeylockTimer();
        StartHdShortCutBarIcon();
        //3?μ?ê±?ó???ˉ
#ifndef FLASH_SUPPORT
        s_idlewin_is_open = TRUE;
#endif
        //StartIdleTimer();

        //?ˉ?-ê±?ó???ˉ
        //g_animal_time_id = 0 ;

        MMIAPISTK_CnfIdleScrAva();
        MMIAPISTK_CnfSimUserAct();

#ifdef QBTHEME_SUPPORT
        if(STYLE_IS_SUPPORT_QBTHEME)
        {
            MMIAPIQBTHEME_StyleStart();
        }
#endif
        //creat wallpaper ctrl
        MMIAPIIDLE_CreatWallpaperCtrl();
        // StatusBarInit();
        GUIWIN_CreateStbDyn(win_id); // ?ˉì?′′?¨×′ì?à?

        GUIWIN_SetStbState(win_id, GUISTATUSBAR_STATE_EVENT, TRUE);
#ifndef MMI_IDLE_MINI_SUPPORT
        GUIWIN_SetStbBgIsDesplay(win_id, FALSE);//Dè?ó×′ì?à??ó°?í?oúé?±3?°￡?idleí3ò?′|àíá?
#else
        GUIWIN_SetStbBgIsDesplay(win_id, FALSE);//Dè?ó×′ì?à??ó°?í?oúé?±3?°￡?idleí3ò?′|àíá?
#endif
#if 0
#ifndef MMI_ONLY_IDLE_DISP_STATUSBAR
        GUIWIN_SetStbBgImageId(win_id, IMAGE_IDLE_STATUS_BAR);
#endif
#endif
        GUIWIN_SetStbState(win_id, GUISTATUSBAR_STATE_USE_LAYER, TRUE);
        MAIN_SetStbTimeVisible(FALSE);

        //MMIDLE_RestartSsTimer(win_id);
#if 0
#if defined MMI_COMMON_IDLE_SUPPORT || defined MMI_MSTYLE_SUPPORT
        MMIIDLE_CreateSoftkey();
        MMIIDLE_CreateDynamicCtrl(win_id);
#endif
#endif

#ifdef DCD_SUPPORT
        MMIAPIDCD_HandleIdleWinMessage(win_id, msg_id, param);
#endif

#ifdef MMIWIDGET_SUPPORT
        if(MMISET_IDLE_STYLE_WIDGET == MMIAPISET_GetIdleStyle())
        {
            MMIAPIWIDGET_CreateIdleWidget();
        }
#endif

#ifdef MMI_GRID_IDLE_SUPPORT
        if(MMISET_IDLE_STYLE_GRID == MMIAPISET_GetIdleStyle())
        {
            MMIAPIGRID_CreateIdle();
        }
#endif

#if defined QBTHEME_SUPPORT
        if(MMISET_IDLE_QBTHEME == MMIAPISET_GetIdleStyle())
        {
            //call
            // MMIAPIQBTHEME_StyleStart(); //°×?á￡?ìá?°
            MMIAPIQBTHEME_CreateIdle();
        }
#endif


#ifdef MMI_PDA_SUPPORT
        MMIIDLE_CreateIStyleMenu( win_id );
#endif

#ifdef LIVE_WALLPAPER_FRAMEWORK_SUPPORT
        if(MMIAPIIDLE_IsLiveWallpaper())
        {
            MMI_LivewallpaperResFoldInit();
            MMIAPILIVEWALLPAPER_Init();
        }
#endif

        HandleIdleMsg(win_id, msg_id, param);

#ifdef JAVA_SUPPORT_IA
        MMIAPIJAVA_JBlendiaAjmsInit();
#endif

#ifdef MMI_LDN_SUPPORT
        MMIAPIPHONE_CheckLdnReadyInd();
#endif
        //?à?¨μ??é????￡???PSμ????ˉ?óoó?áIlde Win Openμ?ê±oò
#if defined(MMI_MULTI_SIM_SYS_TRI)||defined(MMI_MULTI_SIM_SYS_QUAD)
        MMIAPIPHONE_PowerOnPsEx();
#endif

        /* BEGIN: Modified by George.Liu 01526, 2010/8/2   PN:MergeToMw1029forCustom */
#ifdef WIFI_SUPPORT
        MMIAPIWIFI_Init();
#endif
        /* END:   Modified by George.Liu 01526, 2010/8/2   PN:MergeToMw1029forCustom */

#ifdef MMIEMAIL_SUPPORT
        //  MMIAPIEMAIL_SyncEmail();
#endif

#ifdef MMI_STATUSBAR_SCROLLMSG_SUPPORT
        MMIMAIN_PowerOnAddStbScrollItem();
#endif

#if defined(MMI_UPDATE_TIME_SUPPORT)
        if (MMIAPISET_IsNeedResetDateTime())
        {
#ifndef WIN32
#if defined(MMI_UPDATE_TIME_SUPPORT)
            MMIAPISET_RestoreDateTimeFactorySetting();
#endif
#endif
        }
#endif


#ifdef 	ZDT_PRODUCT_Z685S
	if(MMIAPIPHONE_IsSimAvailable(MN_DUAL_SYS_1)|| MMIAPIPHONE_IsSimAvailable(MN_DUAL_SYS_2))
	{
	}
	else
	 {
#if defined(MMI_UPDATE_TIME_SUPPORT)
			MMIAPISET_RestoreDateTimeFactorySetting();
#endif
	 
	 }
	

#endif

        LCD_ESD_CHECKER_START();
        break;
#ifdef DCD_SUPPORT
    case MSG_NOTIFY_TAB_NEED_ITEM_CONTENT:
    case MSG_NOTIFY_TAB_SWITCH:
    case MSG_DCD_TAB_UPDATE:
        MMIAPIDCD_HandleIdleWinMessage(win_id, msg_id, param);
        break;
#endif
    case MSG_FULL_PAINT:
#ifdef MMI_SPRITE_ENABLE
        MMIAPIML_DrawLayerTest();
#endif
        if(s_is_first_power_on)
        {
            s_is_first_power_on = FALSE;
            if(MMIAPIENG_GetChipTestMode() == MMIENG_CHIP_TEST_SETTING_ON)
            {
                MMIENGAPI_StartChipTest();
            }
        }
#ifndef SUBLCD_SIZE_NONE
        //?￠D?D??áμ???ê?￡?oí?÷?áí?2?,?÷òaê?2êD?
        MMISUB_UpdateDisplaySubLcd();
#endif
        HandleIdleMsg(win_id, msg_id, param);

        //MMIAPIMMS_IdleWinDownloadMMS();

#ifdef MMI_SPRITE_ENABLE
        MMIIDLE_ShowSprite();
#endif

#ifdef DPHONE_SUPPORT

#else
        // óD?é?üé???μ?′°?úê?°?í?′°?ú￡??üμ??￠D??áμ???′?′°?úμ??￠D?￡??ùò??ó
        // s_is_need_set_timeà′?D??ê?·?òaμˉè??úéè??′°?ú
        if (s_is_need_set_time && MMIAPISET_IsNeedResetDateTime()) //?D??ê?·?Dèòaéè??ê±??oíè??ú
        {
            s_is_need_set_time = FALSE;
            MMIAPIPHONE_SetEventDelaied(MMI_SET_DATA_DELAY);

        }
#endif
        //??μ?Idleoó?D??ê?·?òa?12?×?USB
        if (!MMIAPIKL_IsPhoneLocked())
        {
            MMK_PostMsg(win_id, MSG_IDLE_USB, PNULL, PNULL); //?D??ê?·?óD?óê±μ?usb
        }

#ifdef BUSMONITOR_SUPPORT
#ifndef WIN32
        //busmonitor for debug
        {
	        struct bm_setting cfg;
#ifdef DSP_ADDR0_BUSMONITOR_ALWAYS_ON
	        ////1.DSP area
	        cfg.addr_min = 0x0;//0x20080;//monitor start address
	        cfg.addr_max = 0x19f;//0x9ffff;//monitor end address
#else
	        ////1.DSP area
	        //cfg.addr_min = 0x20080;//monitor start address
	        //cfg.addr_max = 0x9ffff;//monitor end address
	        ////2.Iram area
	        //cfg.addr_min = 0xFFFF0000;//monitor start address
	        //cfg.addr_max = 0xFFFF001F;//monitor end address
	        ////3.kernel area
	        cfg.addr_min = 0x200000;//monitor start address
	        cfg.addr_max = cfg.addr_min+(uint32)&Image$$EXEC_KERNEL_IMAGE$$Length;//monitor end address
#endif
	        cfg.data_min = 0;
	        cfg.data_max = 0xFFFFFFFF;
	        BusMonitor_Set(0, 1, &cfg, BmCallback);
        }
#endif
#endif
        break;

    case MSG_IDLE_USB:
        MMIAPIPHONE_ResumeAllDelaiedEvent();
        //        if (MMIAPIUdisk_IsUSBDelay())
        //        {
        //            MMIAPIUdisk_OpenUsbOperWindow();
        //            MMIAPIUdisk_SetUSBDelay(FALSE);
        //         }
        break;
#ifndef UI_MULTILAYER_SUPPORT
    case MSG_CTL_ANIM_GIF_UPDATE:
#endif
    case MSG_CTL_ANIM_DISPLAY_IND:
    {
        GUIANIM_DISPLAY_IND_T   display_ind = *(GUIANIM_DISPLAY_IND_T *)param;
        //close waiting win and prompt set wallpaper
        if(MMK_GetCtrlHandleByWin(win_id, MMIIDLE_WALLPAPER_ANIM_CTRL_ID) == display_ind.ctrl_handle)
        {
            MMIAPIFMM_CloseWaitSetWallpaper(display_ind.result);
        }

        if(display_ind.result == 0)
        {
            //GUIANIM_UpdateDisplay(display_ind.ctrl_handle);

            //MMK_SendMsg(display_ind.ctrl_handle,  MSG_CTL_PAINT, 0 );
        }

		
#if defined( MMI_ETWS_SUPPORT)&&defined(MMI_SMSCB_SUPPORT)         
        if( TRUE == MMIAPISMSCB_GetIsNeedRedisplay() )
        {
            MMIAPISMSCB_ShowNewETWSPrompt();
        }
		else
#endif
        if (MMK_IsFocusWin(win_id))
        {
            MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);
        }
    }
    break;

    case MSG_KEYDOWN_HEADSET_DETECT:
    case MSG_KEYUP_HEADSET_DETECT:
        recode = MMI_RESULT_FALSE;
        break;

    case MSG_IDLE_UPDATE_MP3:
    case MSG_IDLE_UPDATE_NETWORK:
        // ?ú?ú2?è?oí?ú?ú°?3?
        if(MMISET_IDLE_STYLE_COMMON == MMIAPISET_GetIdleStyle()
                || MMISET_IDLE_STYLE_SMART == MMIAPISET_GetIdleStyle())
        {
            if (MMIDEFAULT_IsBacklightOn() && MMK_IsFocusWin(win_id))
            {
                MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);
            }
        }
        else
        {
            GUIWIN_UpdateStb();
        }
        HandleIdleMsg( win_id,  msg_id,  param);
        break;
        //?￠D?D?í?±ê
    case MSG_IDLE_UPDATE_ICON:
        if ( (MMIDEFAULT_IsBacklightOn()) && (MAIN_IDLE_WIN_ID == MMK_GetFocusWinId()) )
        {
            MMK_SendMsg( IDLEWIN_STATUSBAR_CTRL_ID, MSG_CTL_PAINT, PNULL);
        }
        break;
        //?e?ˉ???ˉ′|àí
    case MSG_IDLE_VIBRA:
        MMIAPISET_SetVibrator(TRUE, MMISET_RING_TYPE_OTHER);
        if (s_vibra_timer_id != 0)
        {
            MMK_StopTimer(s_vibra_timer_id);
            s_vibra_timer_id = 0;
        }
        s_vibra_timer_id = MMK_CreateWinTimer(MAIN_IDLE_WIN_ID, MMI_IDLE_VIBRA_DEFAULT_TIME, FALSE);
        break;


    case MSG_APP_OK:
#ifdef MMI_MOCOR_SAMPLE
        MMIAPISAMPLE_CreateHelloSprdWin();
        break;
#endif


    case MSG_CTL_OK:
#if defined (DUALMODE_QUICK_SWITCH) && defined(MMI_DUALMODE_ENABLE)//ó?à′1¤3§2aê?ó?￡?ò??ü?D??TD/GSM?￡ê?
    {
        static uint32 network_switch_times = 0;

        SCI_TRACE_LOW("MAINAPP IdleWin_HandleMsg network == %d", network_switch_times);

        if (network_switch_times % 2)//?è?DTD￡?è?oóGSM
        {
            MMIAPISET_SetNetworkType(MN_GMMREG_RAT_GPRS);
        }
        else
        {
            MMIAPISET_SetNetworkType(MN_GMMREG_RAT_3G);
        }

        network_switch_times++;
    }
#else
#if defined CMCC_UI_STYLE && defined MMI_SPRITE_ENABLE
    if (!MMIAPIML_HandleLayerTest(win_id, msg_id, param))
#endif
    {
    		{
			star_to_lock = TRUE ;
		}
        HandleIdleMsg(win_id, msg_id, param);
    }
#endif
    break;

    case MSG_APP_MENU:
        HandleIdleMsg(win_id, msg_id, param);
        break;

		case MSG_KEYLONG_WEB:
#ifdef ZDT_WDL_Z35ZL_SUPPORT
		SwitchTorchLight();
		MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);
		if(s_is_torch_open)
		{
		 	 MMIPUB_OpenAlertWaitingWin(TXT_TORCH_OPEN);
		}
		else
		{
			MMIPUB_OpenAlertWaitingWin(TXT_TORCH_CLOSE);
		}

#else
	#ifdef SOS_KEYLONGWEB		
		{
		 BOOLEAN is_input_emergency_call = FALSE;
		is_input_emergency_call = MMIAPIGET_SosCall_Status();
		if (!MMISMS_IsOrderOk())
		{
			MMIPUB_OpenAlertWarningWin(TXT_SMS_INITIALIZING);
			break;
		}
		if (!is_input_emergency_call)
		{
			
			//StartSosTimer();
			
			sos_is_connected = FALSE;
			sos_times = 0;
			sos_temp = TRUE;
			if(SOS_GetNUM_IsOK()) 
			{
				MMIAPI_PlaySOSAlarm();
			///Sos_Send_SMS();
			  //  ZdtSendSOSMessage();
			 	 StartSosTimer();
				//SOS_EmergencyCallWin();
			}
			else
			{
				sos_temp = FALSE ;
			}
		}
		else
		{
			//MMIPUB_OpenAlertWinByTextId(PNULL,TXT_SOS_IS_CLOSE,TXT_NULL,IMAGE_PUBWIN_FAIL,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
			MMIPUB_OpenAlertWarningWin(TXT_SOS_IS_CLOSE);
			
		}
	//SOS_EmergencyCallWin();
	}
#endif 
#endif
		break;

#if 1//def SOS_KEYLONGWEB
 

    case MSG_KEYDOWN_WEB:
        s_is_down_web_key = TRUE;
       /// HandleIdleMsg(win_id, msg_id, param);
        recode = MMI_RESULT_FALSE;
        break;

	case MSG_KEYUP_WEB:
		//msg_id = MSG_APP_WEB;
	 
	   if(s_is_down_web_key)
	   	{
			 HandleIdleMsg(win_id, msg_id, param);
	    }
	   break;

#else
    case MSG_APP_WEB:
#endif

    case MSG_APP_RIGHT:
    case MSG_APP_LEFT:
    case MSG_APP_UP:
    case MSG_APP_DOWN:
  //  case MSG_APP_WEB:
#ifdef TOUCH_PANEL_SUPPORT //IGNORE9527
    case MSG_TP_PRESS_DOWN:
    case MSG_TP_PRESS_UP:
    case MSG_TP_PRESS_MOVE:
    case MSG_TP_PRESS_LONG:
#endif //TOUCH_PANEL_SUPPORT //IGNORE9527
    case MSG_KEYREPEAT_UP:
    case MSG_KEYREPEAT_DOWN:
    case MSG_KEYREPEAT_LEFT:
    case MSG_KEYREPEAT_RIGHT:

#ifdef MMI_SPRITE_ENABLE
        if (!MMIAPIML_HandleLayerTest(win_id, msg_id, param))
#endif
        {
            HandleIdleMsg(win_id, msg_id, param);
        }
        break;

    case MSG_APP_CANCEL:
#if defined MMI_GRID_IDLE_SUPPORT || defined QBTHEME_SUPPORT
        if (MMISET_IDLE_STYLE_GRID == MMIAPISET_GetIdleStyle()
                || MMISET_IDLE_QBTHEME == MMIAPISET_GetIdleStyle()
           )
        {
            HandleIdleMsg(win_id, msg_id, param);
            break;
        }
#endif
        /* fall through */
        //lint -fallthrough
    case MSG_CTL_CANCEL:    //óòèí?ü
#ifdef CMCC_UI_STYLE
        //MOò??üé?í?±ê???úweb?ü
        //MMISET_EnterMonternetWin();
        MMIAPIMENU_CreatMainMenu(); //??è??÷2?μ￥
#elif defined(BAR_PHONE_WIDGET)
        if(MMISET_IDLE_STYLE_GRID != MMIAPISET_GetIdleStyle())
        {
            MMK_SendMsg(win_id, MSG_LOSE_FOCUS, PNULL);
            MMISET_SetIdleStyle( MMISET_IDLE_STYLE_GRID );
            MMK_SendMsg(win_id, MSG_GET_FOCUS, PNULL);
            MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);
        }
#else
#ifdef MMI_PDA_SUPPORT
        if (MMISET_IDLE_ISTYLE != MMIAPISET_GetIdleStyle())
#endif
        {
#ifdef MMI_IDLE_NO_SIM_SOS_SUPPORT

            if (0 == MMIAPIPHONE_GetSimAvailableNum(PNULL, 0))

            {
                uint16  sim_sys[MMI_DUAL_SYS_MAX] = {0};
                uint32 sim_num = 0;
                const char *s_emergency_call = CC_CALL_EMERGENCY_CALL;

                if(MMIAPISET_GetFlyMode())
                {
                    MMIPUB_OpenQueryWinByTextIdEx(VIRTUAL_WIN_ID, TXT_IDLE_FLYMODE_QUERY, IMAGE_PUBWIN_WARNING, PNULL, HandleFlyModeCloseQueryWinMsg, PNULL);
                }
                else
                {
                    sim_num = MMIAPIPHONE_GetPSReadyNum(sim_sys, MMI_DUAL_SYS_MAX);
                    MMIAPICC_MakeCall((MN_DUAL_SYS_E)sim_sys[0],
                                      (uint8 *)s_emergency_call,
                                      strlen(s_emergency_call),
                                      PNULL,
                                      PNULL,
                                      CC_CALL_SIM_MAX, CC_CALL_NORMAL_CALL,
                                      PNULL);
                }
            }
            else
#endif
            {
                //HandleIdleMsg(win_id, msg_id, param);
                MMIAPISET_OpenFuncKeyWin(win_id, MMISET_FUNCKEY_RIGHT);
            }

        }
#endif
        break;

#ifdef DPHONE_SUPPORT //added by taiping.lai 2011/01/04
    case MSG_APP_SMS:
        MMIAPISMS_OpenMainMenuWin();
        break;
#endif

#ifndef MMI_MSTYLE_SUPPORT
        //ò??üé?í?
    case MSG_APP_AT:
#ifdef BROWSER_SUPPORT
    {
        MMIBROWSER_ENTRY_PARAM entry_param = {0};/*lint !e64*/

        entry_param.type = MMIBROWSER_ACCESS_MAINMENU;
        entry_param.dual_sys = MN_DUAL_SYS_MAX;
        entry_param.url_ptr = PNULL;
        entry_param.user_agent_ptr = PNULL;

        MMIAPIBROWSER_Entry(&entry_param);
    }
#endif
    break;

    //?ì?üò?2|μ??°
    case MSG_APP_GREEN:
    case MSG_APP_CALL2:
    case MSG_APP_CALL3:
    case MSG_APP_CALL4:
    {
        uint8           exist_sim_count = 0;
        MN_DUAL_SYS_E   dual_sys = MN_DUAL_SYS_1;

        for (dual_sys = MN_DUAL_SYS_1; dual_sys < MMI_DUAL_SYS_MAX; dual_sys++)
        {
            if (SIM_STATUS_NO_SIM != MMIAPIPHONE_GetSimStatus(dual_sys))
            {
                exist_sim_count++;
                break;
            }
        }
#ifndef ZDT_PRODUCT_Z685S
        if (0 == exist_sim_count)
        {
            MMIAPICC_MakeCall(MN_DUAL_SYS_MAX, (uint8 *)"112", 3, PNULL, PNULL, CC_CALL_SIM_MAX, CC_CALL_NORMAL_CALL, PNULL);

            break;
        }
#endif
        MMIAPICL_OpenCallsListWindow(MMICL_CALL_DIALED);
    }
    break;

    case MSG_KEYDOWN_TV:
#ifdef CMMB_SUPPORT
        MMIAPIMTV_OpenMTVPlayer();
#else
        MMIAPIATV_OpenATV();
#endif
        break;




#ifdef TAKESCREENPHOTO_SUPPORT
    case MSG_KEYLONG_CAMREA:
#ifdef  CAMERA_SUPPORT
        if(MMIDEFAULT_GetFlipStatus())
        {
            MMIAPIDC_OpenPhotoWin();
        }
        else
#endif
        {
            MMIDEFAULT_TurnOnBackLight();
            //MMIAPIMP3_HandleCameraKey();
        }
        break;
#else
    case MSG_APP_CAMERA:
#ifdef  CAMERA_SUPPORT
        if(MMIDEFAULT_GetFlipStatus())
        {
            MMIAPIDC_OpenPhotoWin();
        }
        else
#endif
        {
            MMIDEFAULT_TurnOnBackLight();
            //MMIAPIMP3_HandleCameraKey();
        }
        break;

#ifdef  CAMERA_SUPPORT
    case MSG_KEYLONG_CAMREA:
        MMIAPIDC_OpenPhotoWin();
        break;
#endif
#endif

#endif //#ifndef MMI_MSTYLE_SUPPORT

        //ê§è￥?1μ?
    case MSG_BACKLIGHT_TURN_OFF:
    case MSG_LOSE_FOCUS:
        if ( !MMK_IsFocusWin( MAIN_MAINMENU_WIN_ID ))
        {
            MMITHEME_DestroyMenuItemCache();
        }

        SCI_TRACE_LOW("MAINAPP IdleWin_HandleMsg MSG_LOSE_FOCUS start time == %d", SCI_GetTickCount());
        HandleIdleMsg(win_id, msg_id, param);
        SCI_TRACE_LOW("MAINAPP IdleWin_HandleMsg MSG_LOSE_FOCUS end time == %d", SCI_GetTickCount());

        MAIN_SetStbTimeVisible(FALSE);
        s_is_down_red_key = FALSE;
       s_is_down_0_key = FALSE;
	s_is_down_web_key = FALSE;
	s_is_down_ok_key = FALSE;
	s_is_down_star_key= FALSE;
#if defined(HERO_ENGINE_TTS_KEY_8_TIME)
        s_is_down_eight_key  = FALSE;
#endif 
#if defined(HERO_ENGINE_TTS_KEY_0_TIME)
        s_is_down_0_key = FALSE;
#endif
#ifdef HERO_ENGINE_TTS_KEY_7_TIME
		s_is_down_7_key = FALSE;
#endif
#ifdef HERO_ENGINE_TTS_KEY_9_TIME
		s_is_down_9_key = FALSE;
#endif
#ifdef HERO_ENGINE_TTS_KEY_STAR_TIME
		s_is_down_star_key = FALSE;
#endif
        MAIN_SetAllStbAppCallbackValid(FALSE);
        //        MMIAPIIDLESS_StopTimer();
        StopAnimTimer();
#if defined (MMI_COMMON_IDLE_SUPPORT) || defined (MMI_MSTYLE_SUPPORT)
#ifdef MMI_AUDIO_PLAYER_SUPPORT
        MMIIDLECOM_StopIdleMp3Timer();
#endif
#endif
        break;

        //?????1μ?
    case MSG_BACKLIGHT_TURN_ON:
    case MSG_GET_FOCUS:
#ifdef UPB_IDLE_SUPPORT
        UPB_DestoryAnim();
#endif
        MMITHEME_InitMenuItemCache();

#ifdef MMI_PDA_SUPPORT
        MMIIDLE_CreateIStyleMenu( win_id );
#endif
        SCI_TRACE_LOW("MAINAPP IdleWin_HandleMsg MSG_GET_FOCUS start time == %d", SCI_GetTickCount());
        HandleIdleMsg(win_id, msg_id, param);
        SCI_TRACE_LOW("MAINAPP IdleWin_HandleMsg MSG_GET_FOCUS end time == %d", SCI_GetTickCount());

        GUIWIN_SetStbState(win_id, GUISTATUSBAR_STATE_EVENT, TRUE);
#ifndef MMI_IDLE_MINI_SUPPORT
        GUIWIN_SetStbBgIsDesplay(win_id, FALSE);//Dè?ó×′ì?à??ó°?í?oúé?±3?°￡?idleí3ò?′|àíá?
#else
        GUIWIN_SetStbBgIsDesplay(win_id, FALSE);//Dè?ó×′ì?à??ó°?í?oúé?±3?°￡?idleí3ò?′|àíá?
#endif
#if 0//UNISOC_MMI_Delete
#ifndef MMI_ONLY_IDLE_DISP_STATUSBAR
        GUIWIN_SetStbBgImageId(win_id, IMAGE_IDLE_STATUS_BAR);
#endif
#endif
        GUIWIN_SetStbState(win_id, GUISTATUSBAR_STATE_USE_LAYER, TRUE);
        MAIN_SetStbTimeVisible(FALSE);

        MMIAPIDCD_HandleIdleWinMessage(win_id, msg_id, param);

#ifdef MMIMAINMENU_SUPPORT_SET_BGD
        MMIAPIIDLE_SetInitWallpaper(TRUE, win_id, MMIIDLE_WALLPAPER_ANIM_CTRL_ID);
#else
        //get wallpaper info
        MMIAPISET_GetWallpaperSetInfo(&wallpaper_info);
        switch (wallpaper_info.wallpaper_type)
        {
        case MMISET_WALLPAPER_ALBUM:
        case MMISET_WALLPAPER_DOWNLOAD:
#ifdef DRM_SUPPORT
            if (MMIAPIIDLE_IsExistWallpaperFile(PNULL))
            {
                MMIMULTIM_AS_WALLPAPER_INFO wallpaper_file_ptr = {0};
                MMIAPIFMM_GetWallPaperInfo(&wallpaper_file_ptr);
                if (MMIAPIFMM_IsDrmFileBySuffix(wallpaper_file_ptr.full_path_name,
                                                wallpaper_file_ptr.full_path_len))
                {
                    //?ì2éê?·?1y?úá?￡?è?1?1y?úá?￡?éè??3é??è?μ?í???
                    if (MMIAPIDRM_IsRightsExpired(SFS_INVALID_HANDLE,
                                                  wallpaper_file_ptr.full_path_name, DRM_PERMISSION_DISPLAY))
                    {
                        MMIAPISET_ResetDefaultWallpaper();
                    }
                }
            }
#endif
            //judge wallpaper
            if (MMIAPIIDLE_CheckWallPaperValid())
            {
                //judge wallpaper is overwrite etc. update
                if (!MMIAPISET_UpdateWallpaper())
                {
                    if (MMITHEME_IsDynaAllocWallpaperBuf())
                    {
                        //update wallpaper
                        MMIAPIIDLE_SetWallpaper(FALSE);
                    }
                }
            }
            else
            {
                //reset wallpaper
                MMIAPISET_ResetDefaultWallpaper();
            }
            break;

        default:
            break;
        }
#endif

        MAIN_SetAllStbAppCallbackValid(TRUE);
        MMIAPISMS_CheckSMSState(FALSE);
#if defined(SEARCH_SUPPORT)&&defined(MMI_ISTYLE_SUPPORT)
        if (MMIAPIMENU_IsInIsyleSearch())
        {
            if (MMIAPISEARCH_IsNeedSearchAgain())
            {
                MMIAPISEARCH_ResetAllIsNeedRestart();
                MMK_PostMsg(win_id, MSG_NOTIFY_EDITBOX_UPDATE_STRNUM, PNULL, 0);
            }
        }
        else
#endif
        {
            MMIAPISET_SetIsNeedValidatePrivacy(TRUE);
        }

        //MMIDLE_RestartSsTimer(win_id);

#ifndef MMI_MSTYLE_SUPPORT
        if (MMIAPIKL_IsPhoneLocked())
        {
            MMIAPIKL_LockPhone();
        }
#endif //#ifndef MMI_MSTYLE_SUPPORT


        if (MMIDEFAULT_GetFlipStatus())
        {
            // è?1?Flip′|óúOpen×′ì?2?StartTimer
            StartAnimTimer();
        }
        else
        {
            StopAnimTimer();
        }
        MMIAPISTK_CnfIdleScrAva();
        MMIAPISTK_CnfSimUserAct();
#ifdef DPHONE_SUPPORT
        if(msg_id != MSG_BACKLIGHT_TURN_ON)
            MMIDEFAULT_SetHandFreeStatesAndLed(FALSE);    //Modified by yanqi.2010-Apr-07 for ?±?ó2|o??óí¨oó°′RedKeyμ?????·μ??′y?ú????
#endif

#if defined MMI_AUTOTEST_SUPPORT && defined ENG_SUPPORT
        MMI_TraceAutoTest( (const uint8 *)"TEST MODULE:returned to idle", strlen("TEST MODULE:returned to idle") );
#endif

        MMIAPISET_OpenVodafoneWinIfNeeded();

        break;

    case MSG_APP_RED:
#ifdef HERO_ENGINE_TTS_SUPPORT
	{
		extern void MMIAPITTS_StopPlayText();
		MMIAPITTS_StopPlayText();
	}
#endif	
    case MSG_KEYDOWN_HANG:
#ifdef HERO_ENGINE_TTS_SUPPORT
	{
		extern void MMIAPITTS_StopPlayText();
		MMIAPITTS_StopPlayText();
	}
#endif	
        s_is_down_red_key = TRUE;
        HandleIdleMsg(win_id, msg_id, param);
        recode = MMI_RESULT_FALSE;
        break;

    case MSG_KEYLONG_RED:
        recode = HandleIdleMsg(win_id, msg_id, param);
        break;

    case MSG_KEYUP_RED:
    case MSG_KEYUP_HANG:
#ifdef MMI_MSTYLE_SUPPORT
        {
            if ((s_is_down_red_key) &&
                (MMIAPISET_GetRedKeyLock()))
            {
                //close all light and enter deep sleep
#ifdef LOCK_SCREEN_SPECIAL_EFFECT_SUPPORT
                MMISEPC_CloseAllLight();
#else
                MMIDEFAULT_CloseAllLight();
#endif
            }
            else
            {
                recode = HandleIdleMsg(win_id, msg_id, param);
            }
        }
#else //#ifdef MMI_MSTYLE_SUPPORT
{
	BOOLEAN is_back_light = MMIDEFAULT_IsBacklightOn();
        if ((s_is_down_red_key) &&
                (MMIAPISET_GetRedKeyLock())&& (MMISET_KEYLOCK_CLOSE != MMIAPISET_GetAutoBoardKey()))
        {
            //close all light and enter deep sleep
#ifdef LOCK_SCREEN_SPECIAL_EFFECT_SUPPORT
            MMISEPC_CloseAllLight();
#else
            MMIDEFAULT_CloseAllLight();
#endif
	///	MMIAPIKL_LockPhone();
        }
	else if((s_is_down_red_key) &&
                (MMIAPISET_GetRedKeyLock())&& (MMISET_KEYLOCK_CLOSE == MMIAPISET_GetAutoBoardKey()))
	{
			if(is_back_light == TRUE)
			{
					MMIDEFAULT_TurnOffBackLight();
				   MMIDEFAULT_StopKeybadBackLightTimer();
				 MMIDEFAULT_SetKeybadBackLight(FALSE);
			}
			else
			{
				
				 MMIDEFAULT_TurnOnBackLight();
				
			}

	}
        else
        {
#ifdef MMI_PDA_SUPPORT
            if(s_is_down_red_key)
            {
                HandleIdleMsg(win_id, msg_id, param);
            }
#endif
            //MMIDLE_RestartSsTimer(win_id);
            recode = MMI_RESULT_FALSE;
        }
}
#endif //#ifdef MMI_MSTYLE_SUPPORT

        break;

#ifdef FLIP_PHONE_SUPPORT
        //1???
    case MSG_KEYDOWN_FLIP:
        StopAnimTimer();
#if defined (MMI_COMMON_IDLE_SUPPORT) || defined (MMI_MSTYLE_SUPPORT)
#ifdef MMI_AUDIO_PLAYER_SUPPORT
        MMIIDLECOM_StopIdleMp3Timer();
#endif
#endif
        recode = MMI_RESULT_FALSE;
        break;

        //?a??
    case MSG_KEYUP_FLIP:
        // ???ˉTimer￡??íê??aá??a??FlipDown×′ì???2????ˉTImer
        StartAnimTimer();
#if defined (MMI_COMMON_IDLE_SUPPORT) || defined (MMI_MSTYLE_SUPPORT)
#ifdef MMI_AUDIO_PLAYER_SUPPORT
        MMIIDLECOM_StartIdleMp3Timer();
#endif
#endif
        recode = MMI_RESULT_FALSE;
        break;
#endif

        //rtc μ??D?????￠
    case MSG_IDLE_UPDATE_DATETIME:
    case MSG_IDLE_UPDATE_TIME_SECOND:
        HandleIdleMsg(win_id, msg_id, param);
        break;

        //ê±?óê??t
    case MSG_TIMER:
        //?e?ˉê±?ó
        if (s_vibra_timer_id == *((uint8 *) param))
        {
            //?e?ˉíê3é
            //SCI_TRACE_LOW:"IdleWin_HandleMsg:received timer msg!"
            SCI_TRACE_ID(TRACE_TOOL_CONVERT, MAINAPP_1396_112_2_18_2_23_35_6, (uint8 *)"");
            MMIAPISET_SetVibrator(FALSE, MMISET_RING_TYPE_OTHER);
            s_vibra_timer_id = 0;
        }
        //?ˉ?-ê±?ó
        else if (g_animal_time_id == *(uint8 *)param )
        {
            //SCI_TRACE_LOW:"IdleWin_HandleMsg: handle the anim wallpater timer!"
            SCI_TRACE_ID(TRACE_TOOL_CONVERT, MAINAPP_1403_112_2_18_2_23_35_7, (uint8 *)"");

            //?ˉ?-?￠D?
            if( (MMIDEFAULT_IsBacklightOn()) && (MAIN_IDLE_WIN_ID == MMK_GetFocusWinId()) )
            {
                MMK_PostMsg(win_id, MSG_FULL_PAINT, PNULL, 0);
            }

            StopAnimTimer();
            StartAnimTimer();
        }
        else if(stk_timer_id == *(uint8 *)param)
        {
             MMK_StopTimer(stk_timer_id);
             MMISTK_SetIdleTimerID(0);
             MMIIDLE_RemovePushMsg(MMI_IDLE_PUSH_MSG_STK, appname, PNULL);
             MMK_PostMsg(win_id, MSG_FULL_PAINT, PNULL, 0);
        }
#ifdef SOS_KEYLONGWEB
	else if (g_sos_time_id == *(uint8*)param )
	{
		  //SOS_EmergencyCallWin();
		  sos_is_connected = FALSE;
		  sos_times = 0;
		  sos_temp = TRUE;
		 if(SOS_GetNUM_IsOK()) 
		 {
			///MMIAPI_PlaySOSAlarm();
			ZdtSendSOSMessage();
			//SOS_EmergencyCallWin();
		 }
		 else
		 {
			sos_temp = FALSE ;
		 }
		 StopSosTimer();
	}
#endif
		
        //Mp3 name
#if defined (MMI_COMMON_IDLE_SUPPORT) || defined (MMI_MSTYLE_SUPPORT)
#ifdef MMI_AUDIO_PLAYER_SUPPORT
        else if (MMIIDLECOM_GetMp3NameTimerId() == *(uint8 *)param )
        {
            if (MAIN_IDLE_WIN_ID == MMK_GetFocusWinId())
            {
                MMIIDLECOM_StopIdleMp3Timer();
                MMIIDLECOM_UpdateMp3NameStartIndex();
                MMIIDLECOM_StartIdleMp3Timer();
                MMK_PostMsg(win_id, MSG_IDLE_UPDATE_MP3, PNULL, 0);
            }
        }
#endif
#endif
        //Screen saver
        //         else if (MMIAPIIDLESS_IsScreenSaverTimerId(param))
        //         {
        //             MMIAPIIDLESS_StopTimer();
        //             if (MAIN_IDLE_WIN_ID == MMK_GetFocusWinId())
        //             {
        // //#if !defined MMI_PDA_SUPPORT
        // //                if((!MMIAPIMP3_IsPlayerActive() && !MMIAPIKUR_IsPlayerActive()&& !MMIAPIFM_IsFmActive()))
        // //#endif
        //                 {
        //                     if(MMIAPIIDLESS_IsScreenSaverMp4Type())
        //                     {
        //                         MMIAPISET_StopAllRing(FALSE);
        //                     }
        //                     MMIAPIIDLESS_OpenScreenSaver();
        //                 }
        //             }
        //         }
#if defined MMI_COMMON_IDLE_SUPPORT || defined MMI_MSTYLE_SUPPORT
#ifdef MMI_IDLE_STKTEXT_SUPPORT
        else if (MMIIDLECOM_GetStkTextInfo()->idle_display_timer == *(uint8 *)param)
        {
            if (MAIN_IDLE_WIN_ID == MMK_GetFocusWinId())
            {
                MMK_PostMsg(win_id, MSG_STK_SET_UP_IDLE_MODE, PNULL, 0);
            }
        }
#endif
#endif
        else
        {
            //??′|àíμ?ê±??
            HandleIdleMsg(win_id, msg_id, param);
            recode = MMI_RESULT_FALSE;
        }
        break;

#if 0
    case MSG_KEYDOWN_UPSIDE:
#ifdef KURO_SUPPORT
        if(MMIAPIKUR_IsPlayerPlaying())
        {
            MMIAPIKUR_SetKurVolume(TRUE);
        }
#endif

#ifdef FM_SUPPORT
        if(FM_PLAYING == MMIAPIFM_GetStatus())
        {
            MMIAPIFM_AdjustVolume(1);
        }
#endif
        break;

    case MSG_KEYDOWN_DOWNSIDE:
        //?úmp3±3?°2￥·?￡?2?1ü?a??oD??￡??ì2à?üμ÷?úmp3ò?á?

#ifdef KURO_SUPPORT
        if(MMIAPIKUR_IsPlayerPlaying())
        {
            MMIAPIKUR_SetKurVolume(FALSE);
        }
#endif

#ifdef FM_SUPPORT
        if(FM_PLAYING == MMIAPIFM_GetStatus())
        {
            MMIAPIFM_AdjustVolume(-1);
        }
#endif

        break;
#endif

#if defined(HERO_ENGINE_TTS_KEY_0_TIME)
    case MSG_KEYDOWN_0:
        s_is_down_0_key = TRUE;	
        break;
    case MSG_KEYUP_0:
        if (s_is_down_0_key)
        {
            msg_id = MSG_KEYDOWN_0;
            s_is_down_0_key = FALSE;
        }
		MMIAPIIDLE_OpenDialWin(msg_id, TRUE);
        break;
	case MSG_KEYLONG_0:
		s_is_down_0_key = FALSE;
		hero_tts_akey_time_broadcast();
		break;
#endif

#if defined(HERO_ENGINE_TTS_KEY_7_TIME)
	case MSG_KEYDOWN_7:
		s_is_down_7_key = TRUE;
		break;
    case MSG_KEYUP_7:
        if (s_is_down_7_key)
        {
            msg_id = MSG_KEYDOWN_7;
            s_is_down_7_key = FALSE;
        }
		MMIAPIIDLE_OpenDialWin(msg_id, TRUE);
        break;
	case MSG_KEYLONG_7:
		s_is_down_7_key = FALSE;
		hero_tts_akey_time_broadcast();
		break;
#endif
#if defined(HERO_ENGINE_TTS_KEY_8_TIME)
    case MSG_KEYDOWN_8:
        s_is_down_eight_key  = TRUE;
        break;
    case MSG_KEYUP_8:
        if (s_is_down_eight_key)
        {
            msg_id = MSG_KEYDOWN_8;
            s_is_down_eight_key = FALSE;
        }
		MMIAPIIDLE_OpenDialWin(msg_id, TRUE);
        break;
	case MSG_KEYLONG_8:
		s_is_down_eight_key = FALSE;
		hero_tts_akey_time_broadcast();
		break;
#endif 
#if defined(HERO_ENGINE_TTS_KEY_9_TIME)
	case MSG_KEYDOWN_9:
		s_is_down_9_key = TRUE;
		break;
	#ifndef IN8_PROJ_C765_HYD01_Y01	
    case MSG_KEYUP_9:
        if (s_is_down_9_key)
        {
            msg_id = MSG_KEYDOWN_9;
            s_is_down_9_key = FALSE;
        }
		MMIAPIIDLE_OpenDialWin(msg_id, TRUE);
        break;
	#endif	
	case MSG_KEYLONG_9:
		s_is_down_9_key = FALSE;
		hero_tts_akey_power_level_broadcast();
		break;
#endif
#ifdef HERO_ENGINE_TTS_KEY_STAR_TIME
	case MSG_KEYDOWN_STAR:
		s_is_down_star_key = TRUE;
		break;
    case MSG_KEYUP_STAR:
        if (s_is_down_star_key)
        {
            msg_id = MSG_KEYDOWN_STAR;
            s_is_down_star_key = FALSE;
        }
		MMIAPIIDLE_OpenDialWin(msg_id, TRUE);
        break;
	case MSG_KEYLONG_STAR:
	    s_is_down_star_key = FALSE;
		hero_tts_akey_time_broadcast();
		break;
#endif

#ifndef MMI_MSTYLE_SUPPORT
        //??ê?è?o???′°?ú
    case MSG_KEYUP_HASH:
        msg_id = MSG_KEYDOWN_HASH;
        //lint -fallthrough

    case MSG_KEYDOWN_1:
    case MSG_KEYDOWN_2:
    case MSG_KEYDOWN_3:
    case MSG_KEYDOWN_4:
    case MSG_KEYDOWN_5:
    case MSG_KEYDOWN_6:
#if !defined(HERO_ENGINE_TTS_KEY_7_TIME)	
    case MSG_KEYDOWN_7:
#endif
#if !defined(HERO_ENGINE_TTS_KEY_8_TIME) 
    case MSG_KEYDOWN_8:
#endif
#if !defined(HERO_ENGINE_TTS_KEY_9_TIME) 
    case MSG_KEYDOWN_9:
#endif
        MMIAPIIDLE_OpenDialWin(msg_id, TRUE);
        break;
#if !defined(HERO_ENGINE_TTS_KEY_0_TIME) 		
    case MSG_KEYUP_0:
        MMIAPIIDLE_OpenDialWin(MSG_KEYDOWN_0, TRUE);
        break;
    case MSG_KEYLONG_0:
#if 0
def  BROWSER_SUPPORT
        MMIIDLE_StartBrowser();
#endif
#if 0
ndef TTS_SUPPORT_NONE
#ifdef PHONE_NUMBER_VOICE
	MMIAPIST_OpenTTSWin();
#endif
#endif
#ifndef TTS_SUPPORT_NONE
#ifdef PHONE_NUMBER_VOICE
	#ifndef WIN32
	Hourtime_To_Speech();
	#endif
#endif
#endif

        break;
#endif
    case MSG_KEYLONG_HASH:
        //MMIDLE_RestartSsTimer(win_id);

        if (MMIAPIENG_GetIQModeStatus())
        {
            MMIPUB_OpenAlertWarningWin(TXT_IQ_DATA_PROCESSING);
            break;
        }
        else if (!MMIAPITVOUT_GetStartingStatus())
        {
            //cr244608 2011-5-23
            uint8 active_mode = MMIAPIENVSET_GetActiveModeId();
            switch (active_mode)
            {
            case MMIENVSET_STANDARD_MODE:  // ??í¨?·?3
#ifdef ZDT_PRODUCT_VIBRATE            
            case MMIENVSET_INSIDE_MODE:      // 3μ?ú?·?3
            case MMIENVSET_NOISY_MODE:       // ?§ía?·?3
#endif            
            case MMIENVSET_USER_MODE1:
            case MMIENVSET_USER_MODE2:
            case MMIENVSET_USER_MODE3:
#ifndef DPHONE_SUPPORT
            case MMIENVSET_USER_MODE4:
            case MMIENVSET_USER_MODE5:
#endif
                active_mode = MMIENVSET_SILENT_MODE;
	  #ifdef ZDT_PRODUCT_VIBRATE 
	    MMIIDLE_SendMsgToIdleWin(MSG_IDLE_VIBRA, PNULL);
#endif

		 MMIPUB_OpenAlertWinByTextId(NULL,TXT_ENVSET_SIL_MODE,TXT_NULL,IMAGE_PUBWIN_SUCCESS,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);//add by James li
                break;

            case MMIENVSET_SILENT_MODE:      // °2?2?·?3
#ifdef ZDT_PRODUCT_VIBRATE            
                active_mode = MMIENVSET_MEETING_MODE;
 		MMIIDLE_SendMsgToIdleWin(MSG_IDLE_VIBRA, PNULL);
                //MMIIDLE_SendMsgToIdleWin(MSG_IDLE_VIBRA, PNULL);//delete by James li
                MMIPUB_OpenAlertWinByTextId(NULL,TXT_ENVSET_VIB_MODE,TXT_NULL,IMAGE_PUBWIN_SUCCESS,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);//add by James li
#else
                active_mode = MMIENVSET_STANDARD_MODE;
		  MMIPUB_OpenAlertWinByTextId(NULL,TXT_ENVSET_STA_MODE,TXT_NULL,IMAGE_PUBWIN_SUCCESS,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);//add by James li		
#endif
			break;
#ifdef ZDT_PRODUCT_VIBRATE                
            case MMIENVSET_MEETING_MODE:     // ?áòé?·?3
                active_mode = MMIENVSET_STANDARD_MODE;
		    MMIIDLE_SendMsgToIdleWin(MSG_IDLE_VIBRA, PNULL);	
		  MMIPUB_OpenAlertWinByTextId(NULL,TXT_ENVSET_STA_MODE,TXT_NULL,IMAGE_PUBWIN_SUCCESS,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);//add by James li		
                break;
#endif

#if !defined DPHONE_SUPPORT && defined MMIENVSET_EARPHONE_BLUETOOTH_SUPPORT
            case MMIENVSET_EARPHONE_MODE:
            case MMIENVSET_BLUETOOTH_MODE:
                break;
#endif
            default:
                //SCI_TRACE_LOW:"IdleWin_HandleMsg MSG_KEYLONG_HASH!"
                SCI_TRACE_ID(TRACE_TOOL_CONVERT, MAINAPP_1553_112_2_18_2_23_36_8, (uint8 *)"");
                break;
            }
            if(active_mode != MMIAPIENVSET_GetActiveModeId())
            {
                MMIAPIENVSET_SetActiveModeForIdle(active_mode);
                MMIENVSET_SetPreActiveMode(active_mode);
            }
            MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);
        }
        else
        {
            MMIAPITVOUT_Close(); //close tv out, MSG_KEYLONG_HASH message is handled here, not in MMK_HandlePublicKey() function.
        }
        break;


#if 0
def MMI_CSP_SUPPORT
    case MSG_KEYLONG_STAR:
        if(MMIAPIPHONE_IsCPHSSupport())
        {
            MMIAPISET_SetLineInfo();
        }
#if 0
        //need to handle multi sim
        if(MMISET_IsAlsValid(MN_DUAL_SYS_1))
        {
            if(MMISET_LINE_IS_LOCK == MMISET_GetLineLockStatus(MN_DUAL_SYS_1))
            {
                //prompt not allowed
                MMIPUB_OpenAlertWarningWin(TXT_LINE_LOCK);
            }
            else
            {
                MMISET_LINE_TYPE_E line = (0 == MMIAPISET_GetCallLineInfo(MN_DUAL_SYS_1)) ? MMISET_LINE_2 : MMISET_LINE_1;
                //set line
                MMIAPISET_SetSelectLine(MN_DUAL_SYS_1, line);
            }
        }
#endif
        break;
#endif
#if 1
 case MSG_KEYLONG_STAR:
		 {
#ifdef ZDT_WDL_Z35ZL_SUPPORT
		#ifdef SOS_KEYLONGWEB		
		{
		 BOOLEAN is_input_emergency_call = FALSE;
		is_input_emergency_call = MMIAPIGET_SosCall_Status();
		if (!MMISMS_IsOrderOk())
		{
			MMIPUB_OpenAlertWarningWin(TXT_SMS_INITIALIZING);
			break;
		}
		if (!is_input_emergency_call)
		{
			
			//StartSosTimer();
			
			sos_is_connected = FALSE;
			sos_times = 0;
			sos_temp = TRUE;
			if(SOS_GetNUM_IsOK()) 
			{
				MMIAPI_PlaySOSAlarm();
			///Sos_Send_SMS();
			  //  ZdtSendSOSMessage();
			 	 StartSosTimer();
				//SOS_EmergencyCallWin();
			}
			else
			{
				sos_temp = FALSE ;
			}
		}
		else
		{
			//MMIPUB_OpenAlertWinByTextId(PNULL,TXT_SOS_IS_CLOSE,TXT_NULL,IMAGE_PUBWIN_FAIL,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
			MMIPUB_OpenAlertWarningWin(TXT_SOS_IS_CLOSE);
			
		}
	//SOS_EmergencyCallWin();
	}
#endif 


#else
#ifndef TTS_SUPPORT_NONE
#ifdef PHONE_NUMBER_VOICE
	#ifndef WIN32
	Bat_cap_To_Speech();
	#endif
#endif
#endif
#endif
		 }
		 break;
#endif

	case MSG_KEYDOWN_STAR:
	  s_is_down_star_key = TRUE;
	 /// HandleIdleMsg(win_id, msg_id, param);
	  recode = MMI_RESULT_FALSE;
	  break;
      case MSG_KEYUP_STAR:
	  	if(s_is_down_star_key)
	   {
       	 MMIAPIIDLE_OpenDialWin(msg_id, TRUE);
	  }
        break;
#endif //#ifndef MMI_MSTYLE_SUPPORT


    case MSG_CLOSE_WINDOW:
        StopAnimTimer();

#ifndef FLASH_SUPPORT
        s_idlewin_is_open = FALSE;
#endif
#ifdef JAVA_SUPPORT_IA
        MMIAPIJAVA_JBlendiaAjmsFinalize();
#endif

        MMK_CloseGsensor( win_id );
        break;
        
#if defined TOUCH_PANEL_SUPPORT && defined MMI_SPRITE_ENABLE
    case MSG_TP_PRESS_MOVE:
        if (MMIIDLE_PressMoveSprite(MMK_GET_TP_X(param), MMK_GET_TP_Y(param)))
        {
            MMIIDLE_UpdatePressedSprite();
        }
        break;
#endif

    case MSG_PRE_FULL_PAINT:
        HandleIdleMsg(win_id, msg_id, param);
        break;

    case MSG_LCD_SWITCH:
#ifdef MMIWIDGET_WEATHER_ANIM
    case MSG_WEATHER_NEED_ANIM:
    case MSG_WEATHER_FORBID_ANIM:
#endif
        HandleIdleMsg(win_id, msg_id, param);
        MMIAPIDCD_HandleIdleWinMessage(win_id, msg_id, param);
        break;

#ifndef MMI_MSTYLE_SUPPORT
        //???÷?ú?ú?ü￡?o?3?×??üò?′?2|3?μ??°
    case MSG_APP_HEADSET_BUTTON:
        MMK_NeedKbdClkNotifyOnce();
        break;

    case MSG_KEY_DBLCLK:
        MMIAPICL_MakeCallByRecordIndex(MMICL_CALL_DIALED, 0);
        break;
#endif //#ifndef MMI_MSTYLE_SUPPORT

    default:
#if defined(MMI_PDA_SUPPORT) && defined(MMI_ISTYLE_SUPPORT)
        if ( MMISET_IDLE_ISTYLE == MMIAPISET_GetIdleStyle() )
        {
            recode = MMIIDLE_IStyleHandleMsg(win_id, msg_id, param);
        }
        else
#endif
        {
            HandleIdleMsg(win_id, msg_id, param);
            recode = MMI_RESULT_FALSE;
        }
        break;
    }

#ifdef MMI_VIDEOWALLPAPER_SUPPORT
    if ( MMI_RESULT_TRUE == MMIAPISETVWP_HandleVideoWallPaper(win_id, msg_id, param) )
    {
        recode = MMI_RESULT_TRUE;
    }
#endif

#ifdef LIVE_WALLPAPER_FRAMEWORK_SUPPORT
    MMIAPILIVEWALLPAPER_HandleEvent(win_id, msg_id, param);
#endif

    return (recode);
}

#ifdef LIVE_WALLPAPER_FRAMEWORK_SUPPORT
/*****************************************************************************/
//  Description : MMIAPIIDLE_GetMainAppFgInfo
//  Global resource dependence :
//  Author:arvin.wu
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIIDLE_GetMainAppFgInfo(LW_FG_INFO *lw_fg_info_ptr)
{
    MMISET_IDLE_STYLE_E idle_style = MMISET_IDLE_STYLE_COMMON;


    if(PNULL == lw_fg_info_ptr)
    {
        return;
    }
    idle_style = MMIAPISET_GetIdleStyle();

    switch(idle_style)
    {
#ifdef MMI_GRID_IDLE_SUPPORT
    case MMISET_IDLE_STYLE_GRID:
        MMIAPIGRID_GetMainAppFgInfo(lw_fg_info_ptr);//??μ?GRID·??????ˉì?±ú??μ?D??￠
        break;
#endif
#if defined QBTHEME_SUPPORT
    case MMISET_IDLE_QBTHEME:
        MMIAPIQBTHEME_GetMainAppFgInfo(lw_fg_info_ptr);//??μ?GRID·??????ˉì?±ú??μ?D??￠
        break;
#endif
#if defined MMI_ISTYLE_SUPPORT
    case MMISET_IDLE_ISTYLE:
        lw_fg_info_ptr->disp_dev_info.lcd_id = 0;
        lw_fg_info_ptr->disp_dev_info.block_id = 0;
        lw_fg_info_ptr->lw_fg_pos_info.rc = MMITHEME_GetFullScreenRect();
        lw_fg_info_ptr->lw_fg_pos_info.x = 0;
        lw_fg_info_ptr->lw_fg_pos_info.y = 0;
        //MMIISTYLE_GetMainAppFgInfo();//??μ?iphone·??????ˉì?±ú??μ?D??￠
        break;
#endif
    default:
        lw_fg_info_ptr->disp_dev_info.lcd_id = 0;
        lw_fg_info_ptr->disp_dev_info.block_id = 0;
        lw_fg_info_ptr->lw_fg_pos_info.rc = MMITHEME_GetFullScreenRect();
        lw_fg_info_ptr->lw_fg_pos_info.x = 0;
        lw_fg_info_ptr->lw_fg_pos_info.y = 0;
        break;
    }
}
#endif
/*****************************************************************************/
//  Description : init the main application
//  Global resource dependence :
//  Author: Great.Tian
//  Note: 2003-12-09
/*****************************************************************************/
void MMIAPIIDLE_init(void)
{
    MMISET_CALL_RING_TYPE_E     call_ring_type = MMISET_CALL_RING;
    uint32 i = 0;
#ifndef FLASH_SUPPORT
    s_idlewin_is_open = FALSE;
#endif

    MMIIDLE_InitWinInfo();

    //set s_idlewin_info.is_shake
    //get call ring type
    call_ring_type = (MMISET_CALL_RING_TYPE_E)MMIAPIENVSET_GetActiveModeOptValue(MN_DUAL_SYS_1, CALL_RING_TYPE);

#ifdef MMI_GPRS_SUPPORT
    MMIAPIPDP_HandleIdleEvent();
#endif
    //    MMIAPIPDP_SubscribeEvent(MMI_MODULE_COMMON,
    //                                                 HandlePdpEvent,
    //                                                 MMIPDP_NOTIFY_EVENT_MAX,
    //                                                  s_pdp_event);
    //SCI_TRACE_LOW:"MMAPIIDLE_init:call_ring_type is %d!"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT, MAINAPP_1710_112_2_18_2_23_36_9, (uint8 *)"d", call_ring_type);
    switch (call_ring_type)
    {
    case MMISET_CALL_RING:
        MMIIDLE_GetIdleWinInfo()->is_shake = FALSE;
        MMIIDLE_GetIdleWinInfo()->is_silent = FALSE;
        break;

    case MMISET_CALL_VIBRA:
    case MMISET_CALL_VIBRA_BEFORE_RING:
    case MMISET_CALL_VIBRA_AND_RING:
        MMIIDLE_GetIdleWinInfo()->is_shake = TRUE;
        MMIIDLE_GetIdleWinInfo()->is_silent = FALSE;
        break;

    case MMISET_CALL_SILENT:
        MMIIDLE_GetIdleWinInfo()->is_shake = FALSE;
        MMIIDLE_GetIdleWinInfo()->is_silent = TRUE;
        break;

    default:
        //SCI_PASSERT(0,("MMIAPIIDLE_init: call_ring_type is error!"));
        //SCI_TRACE_LOW:"MMIAPIIDLE_init: call_ring_type is error!"
        SCI_TRACE_ID(TRACE_TOOL_CONVERT, MAINAPP_1732_112_2_18_2_23_36_10, (uint8 *)"");
        break;
    }

    //set battery
    MMIIDLE_GetIdleWinInfo()->batlevel = MMIAPIPHONE_GetBatCapacity();

    //set call forward status
    SCI_MEMSET(MMIIDLE_GetIdleWinInfo()->is_forward, FALSE, (sizeof(BOOLEAN)*MMI_DUAL_SYS_MAX));

    //init line and read nv
    for (i = MN_DUAL_SYS_1; i < MMI_DUAL_SYS_MAX; i++)
    {
        MMIIDLE_GetIdleWinInfo()->line_state[i] = MMIAPISET_GetCallLineInfo(i);
    }
    // set the main applicaiton function and the type
    g_main_app.ProcessMsg     = MAIN_HandleSysMsg;
    // 3?ê??ˉ×′ì?à????t
    GUIWIN_InitStbData(MMICOM_WIN_ICON_MAX);
    MMIAPICOM_StatusAreaInit(); // 3?ê??ˉ×′ì?à?êy?Y

#ifdef MONOCHROME_LCD_SUPPORT  // dphoneòa1?±?í??÷??ê?1|?ü
    IMG_Turnoff_Transparent(TRUE);
    IMG_EnableCustomerTransparentColor(TRUE, 0xF81F);
    IMG_Turnoff_Transparent(FALSE);
#endif
}


#ifdef MAINLCD_SIZE_128X64
/*******************************************************************
*      Description: Get idle win ID
*	Global resource dependence
*     Author:  alex
*     Note:
*/
PUBLIC MMI_HANDLE_T MAIN_GetIdleWinId(void)
{
    return MAIN_IDLE_WIN_ID;
}
#endif

/*****************************************************************************/
//  Description : open idle win
//  Global resource dependence :
//  Author: bruce.chi
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIIDLE_OpenIdleWin(void)
{
    if( !MMK_IsOpenWin( MAIN_IDLE_WIN_ID ) )
    {
        MMI_WINDOW_CREATE_T win_create = {0};

        win_create.applet_handle = MMK_GetFirstAppletHandle();
        win_create.win_id = MAIN_IDLE_WIN_ID;
#ifdef FLASH_SUPPORT
        win_create.func = MMIAPIFlashWin_HandleMsg;
#else
        win_create.func = IdleWin_HandleMsg;
#endif
        win_create.win_priority = WIN_LOWEST_LEVEL;
        win_create.softicon_state = 0x127E;

        win_create.window_style = WS_NO_DEFAULT_STYLE | WS_HAS_ANIM_BG | WS_DISABLE_COMMON_BG; //idle′°?úò?anim???t?a±3?°, ?è?-animoó?-′°?ú
	if(!((MMIENG_GetFirstTimeIsrecord() == 0)||(MMIENG_GetFirstTimeIsrecord() == 1)) )
		{
			OpenPhoneClearTimeWin();
		}
        //MMK_OpenWindow( s_idle_win_handle );
        MMK_CreateWindow( &win_create );
#ifdef LOW_MEMORY_SUPPORT
        MMK_SetWinMoveStyle(MAIN_IDLE_WIN_ID, MOVE_FORBIDDEN);
#endif
    }
}


/*****************************************************************************/
//  Description : start the timer of no sim or sim error etc window
//  Global resource dependence :
//  Author: Jassmine.meng
//  Note:
/*****************************************************************************/
//TIMER_OPERATE_RESULT MAIN_StopIdleTimer(void)
//{
//    TIMER_OPERATE_RESULT recode = TIMER_INVALIDATE;
//
//    if (0 != s_sim_error_time_id)
//    {
//        if (!MMIIDLE_GetIdleWinInfo()->is_charge)
//        {
//            MMK_StopTimer(s_sim_error_time_id);
//            s_sim_error_time_id = 0;
//
//            recode = TIMER_STOP_SUCCESS;
//        }
//        else
//        {
//            recode = TIMER_STOP_FAILURE;
//        }
//    }
//    else
//    {
////        SCI_TRACE_LOW("MAIN_StopIdleTimer: the sim error timer has stoped!");
//        recode = TIMER_HAVE_STOPED;
//    }
//
//    return (recode);
//}

/*****************************************************************************/
//  Description : μ?áá?ò1?±?±31aê±ó|ó?Dèòa×?μ?′|àí
//  Global resource dependence :
//  Author: Great.Tian
//  Note:
/*****************************************************************************/
void MMIAPIIDLE_HandleLCDBacklight(
    BOOLEAN is_light // [IN] TRUE: μ?áá±31a; FALSE: 1?±?±31a
)
{
    if (is_light)
    {
        // ???ˉ?à1?μ?Timer￡?è?1?óD±?òa￡?Dèòa?D??Timer?ù′|μ?′°?úê?·?′|óúFocus
        // ?ú?aà????ˉTimerê±￡???ê1ó?MMK_CreateWinTimeroˉêy

        // ???ˉIdle′°?ú?Dμ?Timer
        //    StartIdleTimer();
#if defined (MMI_COMMON_IDLE_SUPPORT) || defined (MMI_MSTYLE_SUPPORT)
#ifdef MMI_AUDIO_PLAYER_SUPPORT
        MMIIDLECOM_StartIdleMp3Timer();
#endif
#endif
        StartAnimTimer();
#if defined(MMI_DUALMODE_ENABLE) && !defined (WIN32)
        //áá?áê±Enable
        MNPHONE_SetTdMeasurementState(MNPHONE_TD_MEAS_ENABLE);
#endif
        //áá?áê±PS?ì?ùDY??ê±?????a12??
        MMIIDLE_SetPsFastSleepTimer(MMIIDLE_LCD_ON_PS_FAST_SLEEP_TIME);

	LCD_ESD_CHECKER_START();
    }
    else
    {
        // 1?±??à1?μ?Timer

        // 1?±?Idle′°?ú?Dμ?Timer
        //         MustStopIdleTimer();
#if defined (MMI_COMMON_IDLE_SUPPORT) || defined (MMI_MSTYLE_SUPPORT)
#ifdef MMI_AUDIO_PLAYER_SUPPORT
        MMIIDLECOM_StopIdleMp3Timer();
#endif
#endif
        StopAnimTimer();
#ifndef SUBLCD_SIZE_NONE
        //1?±?D??áμ?Timer
        MMISUB_StopSubLcdTimer();
#endif
        //1?±?sim error????μ?Timer
        //MAIN_StopIdleTimer();
#if defined(MMI_DUALMODE_ENABLE) && !defined (WIN32)
        //°μ?áê±Disable
        MNPHONE_SetTdMeasurementState(MNPHONE_TD_MEAS_DISABLE);
#endif
        //°μ?áê±PS?ì?ùDY??ê±?????a3??
        MMIIDLE_SetPsFastSleepTimer(MMIIDLE_LCD_OFF_PS_FAST_SLEEP_TIME);

        LCD_ESD_CHECKER_STOP();
    }
#ifdef BROWSER_SUPPORT
    MMIAPIBROWSER_CheckTimerDisable();
#endif
}

/*****************************************************************************/
//  Description : start Hard icon for tp
//  Global resource dependence :
//  Author: xiaoqing.lu
//  Note:
/*****************************************************************************/
LOCAL void StartHdShortCutBarIcon(void)
{
#ifndef TOUCH_PANEL_HWICON_SUPPORT_NONE
    MMIAPIIDLE_StartHdShortCutBarIcon();
#endif
}
/*****************************************************************************/
//  Description : handle idle message
//  Global resource dependence :
//  Author: Jassmine
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleIdleMsg(
    MMI_WIN_ID_T         win_id,
    MMI_MESSAGE_ID_E     msg_id,
    DPARAM               param
)
{
    MMISET_IDLE_STYLE_E     idle_style  = MMISET_IDLE_STYLE_COMMON;
    MMI_RESULT_E res = MMI_RESULT_TRUE;

    //get idle style
    idle_style = MMIAPISET_GetIdleStyle();
    switch (idle_style)
    {
#if defined MMI_COMMON_IDLE_SUPPORT
    case MMISET_IDLE_STYLE_COMMON:
        res = MMIIDLE_CommonHandleMsg(win_id, msg_id, param);
        break;
#endif

#if defined MMI_MSTYLE_SUPPORT
    case MMISET_IDLE_MSTYLE:
        res = MMIIDLE_MStyleHandleMsg(win_id, msg_id, param);
        break;
#endif

#if defined (MMI_SMART_IDLE_SUPPORT_SMALL_ICON)||defined (MMI_SMART_IDLE_SUPPORT_LARGE_ICON)
    case MMISET_IDLE_STYLE_SMART:
        res = MMIIDLE_SmartHandleMsg(win_id, msg_id, param);
        break;
#endif

#ifdef MMIWIDGET_SUPPORT
    case MMISET_IDLE_STYLE_WIDGET:
        res = MMIAPIWIDGET_HandleMsg(win_id, msg_id, param);
        break;
#endif

#ifdef MMI_GRID_IDLE_SUPPORT
    case MMISET_IDLE_STYLE_GRID:

        res = MMIAPIGRID_HandleMsg(win_id, msg_id, param);
        break;
#endif

#ifdef QBTHEME_SUPPORT
    case MMISET_IDLE_QBTHEME:
        MMIAPIQBIDLE_HandleMsg(win_id, msg_id, param);
        break;
#endif

#if (defined MMI_PDA_SUPPORT) && defined(MMI_ISTYLE_SUPPORT)
    case MMISET_IDLE_ISTYLE:
        res = MMIIDLE_IStyleHandleMsg( win_id, msg_id, param );
        break;
#endif

    default:
        res = MMI_RESULT_FALSE;
        //SCI_TRACE_LOW:"HandleIdleMsg:idle_style %d is error!"
        SCI_TRACE_ID(TRACE_TOOL_CONVERT, MAINAPP_1948_112_2_18_2_23_36_11, (uint8 *)"d", idle_style);
        break;
    }

    return res;
}

#ifdef CALENDAR_SUPPORT
/*****************************************************************************/
//  Description : open schedule window
//  Global resource dependence :
//  Author: Jassmine
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIIDLE_OpenScheduleWin(
    MMI_WIN_ID_T    win_id
)
{
    MMIAPISET_ValidatePrivacyAppEntry(MMISET_PROTECT_CALENDAR, MMIAPICALEND_OpenMainWin);
}
#endif
//#ifdef MMI_KEY_LOCK_EFFECT_SUPPORT
/*****************************************************************************/
// Description :
// Global resource dependence :
// Author: renyi.hu
// Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPIIDLE_IsChargeConnected(void)
{
    if(MMIIDLE_GetIdleWinInfo()->is_charge)
    {
        return TRUE;
    }
    else
    {
        return FALSE;
    }
}

/*****************************************************************************/
//  Description : handle idle plmn message.
//  Global resource dependence :
//  Author: Michael.shi
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIIDLE_OpenSelNetWorkWin(MN_DUAL_SYS_E dual_sys )
{
#ifdef MMI_DUALMODE_ENABLE
    MMIAPISET_OpenSelNetWorkWin(dual_sys);
#else
    if (MMIAPIPHONE_IsSimAvailable(dual_sys))
    {
        MMIAPISET_OpenSelNetWorkWin(dual_sys);
    }
    else
    {
        MMIAPIPHONE_AlertSimNorOKStatusII(dual_sys);
    }
#endif
}

//#endif


#ifdef MMI_IDLE_NO_SIM_SOS_SUPPORT
/*****************************************************************************/
//  Description : to handle  add blacklist open setting query window
//  Global resource dependence :
//  Author:dave.ruan
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleFlyModeCloseQueryWinMsg(
    MMI_WIN_ID_T        win_id,    //IN:
    MMI_MESSAGE_ID_E    msg_id,    //IN:
    DPARAM                param    //IN:
)
{
    MMI_RESULT_E  recode = MMI_RESULT_TRUE;
    MMIPUB_INFO_T   *win_info_ptr = PNULL;
    uint16  sim_sys[MMI_DUAL_SYS_MAX] = {0};
    uint32 sim_num = 0;
    const char *s_emergency_call = CC_CALL_EMERGENCY_CALL;

    switch (msg_id)
    {
    case MSG_OPEN_WINDOW:
        MMIPUB_SetWinSoftkey(win_id, TXT_IDLE_FLYMODE_DISABLE, STXT_CANCEL, FALSE);
        break;
    case MSG_CTL_OK:
    case MSG_APP_OK:
    case MSG_APP_WEB:
        SetFlyMode();
        /*sim_num = MMIAPIPHONE_GetPSReadyNum(sim_sys, MMI_DUAL_SYS_MAX);
        MMIAPICC_MakeCall((MN_DUAL_SYS_E)sim_sys[0],
                                           (uint8 *)s_emergency_call,
                                           strlen(s_emergency_call),
                                           PNULL,
                                           PNULL,
                                           CC_CALL_SIM_MAX,CC_CALL_NORMAL_CALL,
                                           PNULL);*/
        //1?±??ˉ?ê′°?ú
        MMK_CloseWin(win_id);
        break;

    case MSG_CTL_CANCEL:
    case MSG_APP_CANCEL:
        //1?±??ˉ?ê′°?ú
        MMK_CloseWin(win_id);
        break;

    default:
        recode = MMIPUB_HandleQueryWinMsg(win_id, msg_id, param);
        break;
    }

    return (recode);
}

/*****************************************************************************/
//  Description :·éDD?￡ê?éè??μ?′°?ú′|àíoˉêy
//  Global resource dependence :
//  Author:chunjie.liu
//  Note:
/*****************************************************************************/
LOCAL void SetFlyMode(void)
{
    BOOLEAN    is_fly_mode_on  = MMIAPISET_GetFlyMode();
    uint16     i               = 0;
    BOOLEAN is_ps_deactive = FALSE;
    if(is_fly_mode_on)
    {
        is_ps_deactive = TRUE;
        for(i = MN_DUAL_SYS_1; i < MMI_DUAL_SYS_MAX; i++)
        {
            if(!MMIAPIPHONE_GetIsPsDeactiveCnf((MN_DUAL_SYS_E)i))
            {
                is_ps_deactive = FALSE;
                break;
            }
        }
        if (is_ps_deactive)
        {
            is_fly_mode_on = !is_fly_mode_on;

            MMIAPISET_SetFlyMode(is_fly_mode_on);



            // ?¤??D-òé??
            MMIAPIPHONE_StartUpPsAccordingToSetting(FALSE, TRUE);
            MMIAPIMMS_RestartAutodownMMS();
        }
        else
        {
            MMIPUB_OpenAlertWarningWin( TXT_SYS_WAITING_AND_TRY_LATER);

            //MMK_CloseWin(win_id);
            return;
        }
    }
    else
    {
        SCI_TRACE_LOW("mmicc_app: SetFlyMode: is_fly_mode_on = %d", is_fly_mode_on);
    }
}
#endif
/*Edit by script, ignore 5 case. Thu Apr 26 19:00:55 2012*/ //IGNORE9527

#ifdef SOS_KEYLONGWEB

LOCAL void StartSosTimer(void)
{
    if( MMK_IsFocusWin(MAIN_IDLE_WIN_ID) )
    {
        if(0 == g_sos_time_id)
        {
           g_sos_time_id= MMK_CreateWinTimer(MAIN_IDLE_WIN_ID, 2000, FALSE);
           
        }
    }
}

/*****************************************************************************/
//  Description : stop the idle timer 
//  Global resource dependence : 
//  Author: 
//  Note: 
/*****************************************************************************/
LOCAL void StopSosTimer(void)
{
 
    if (0 < g_sos_time_id )
    {
        MMK_StopTimer( g_sos_time_id );
        g_sos_time_id=0;
    }
}

PUBLIC void SOS_EmergencyCallWin(void)

{
     uint8 *Sos_str = PNULL;
	 uint8 * temp_str = PNULL;
	 uint16 len = 0;
     BOOLEAN is_input_emergency_call = FALSE;
     MMI_STRING_T str_info = {0};
	 MN_DUAL_SYS_E dual_sys = MN_DUAL_SYS_1;
	 CC_CALL_SIM_TYPE_E sim_type = CC_SIM1_CALL;
	 uint8 len_temp = 0;
	 uint16 sim_ok = MN_DUAL_SYS_1;

	if(MMIAPIPHONE_IsSimAvailable(MN_DUAL_SYS_1))
	 {
		dual_sys = MN_DUAL_SYS_1;
	 }
	 else if(MMIAPIPHONE_IsSimAvailable(MN_DUAL_SYS_2))
	 {
		dual_sys = MN_DUAL_SYS_2;
	 }
	else
	{
		MMIAPIPHONE_AlertSimNorOKStatus();
		return;
	}
	 
	 if(!sos_temp)
	 {
		return;
	 }

     is_input_emergency_call = MMIAPIGET_SosCall_Status();
	 Sos_str = (uint8*)SCI_ALLOC_APP(MMISET_SOS_MAX_LEN);
	 temp_str = MMIAPIGET_SosCall_Num1();
	len_temp  =strlen(temp_str);
	
	//if(len_temp ==0 )
	//{
	//	temp_str = MMIAPIGET_SosCall_Num2();
	//	len_temp  =strlen(temp_str);
	//	if(len_temp ==0 )
	//	{
	//		temp_str = MMIAPIGET_SosCall_Num3();
	//	}
	//}
	SCI_MEMSET(Sos_str,0,MMISET_SOS_MAX_LEN);
	SCI_MEMCPY(Sos_str,temp_str,MMISET_SOS_MAX_LEN);
	 len = strlen(Sos_str);
	 Trace_Log_Print("SOS_EmergencyCallWin sos_temp=%d",sos_temp);
     if (!is_input_emergency_call && (len != 0))
     {
		sos_times++ ;
		 SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin  00000");
		//sos_temp = TRUE;
  	     sim_type = MMIAPICC_GetCallSimType(dual_sys);
  	     MMIAPICC_MakeCall(
                    dual_sys,
                    Sos_str,
                    (uint8)(len),
                    PNULL,
                    PNULL,
                   sim_type,
                    CC_CALL_NORMAL_CALL,
                    PNULL
                    );

              //  MMK_CloseWin(win_id);
     }
	 else
	 {
	  Trace_Log_Print("SOS_EmergencyCallWin sos_temp0000=%d",sos_times);
	   SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin  1111");
	 	if(sos_times == 0)
	 	{
			sos_times = 1;
			  SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin  22222");
		}
		else if(sos_times == 3)
		{
			sos_times = 4;
			  SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin  3333");
		}
		else if(sos_times == 6)
		{
			sos_times = 7;
			  SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin  4444");
		}	
		SOS_EmergencyCallWin2();
		// MMIPUB_OpenAlertWinByTextId(PNULL,TXT_NULL_SOS_CALL_NUM,TXT_NULL,IMAGE_PUBWIN_FAIL,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
	 }
	 SCI_FREE(Sos_str);
}


PUBLIC void SOS_EmergencyCallWin2(void)

{
     uint8 *Sos_str = PNULL;
	 uint8 * temp_str = PNULL;
	 uint16 len = 0;
     BOOLEAN is_input_emergency_call = FALSE;
     MMI_STRING_T str_info = {0};
	 MN_DUAL_SYS_E dual_sys = MN_DUAL_SYS_1;
	 CC_CALL_SIM_TYPE_E sim_type = CC_SIM1_CALL;
	 uint8 len_temp = 0;

   if(MMIAPIPHONE_IsSimAvailable(MN_DUAL_SYS_1))
	 {
		dual_sys = MN_DUAL_SYS_1;
	 }
	 else if(MMIAPIPHONE_IsSimAvailable(MN_DUAL_SYS_2))
	 {
		dual_sys = MN_DUAL_SYS_2;
	 }
	else
	{
		MMIAPIPHONE_AlertSimNorOKStatus();
		return;
	}


     is_input_emergency_call = MMIAPIGET_SosCall_Status();
	 Sos_str = (uint8*)SCI_ALLOC_APP(MMISET_SOS_MAX_LEN);
	 temp_str = MMIAPIGET_SosCall_Num2();
	len_temp  =strlen(temp_str);
	//if(len_temp ==0 )
	//{
	//	temp_str = MMIAPIGET_SosCall_Num1();
	//	len_temp  =strlen(temp_str);
	//	if(len_temp ==0 )
	//	{
	//		temp_str = MMIAPIGET_SosCall_Num3();
	//	}

	//}
		SCI_MEMSET(Sos_str,0,MMISET_SOS_MAX_LEN);
	SCI_MEMCPY(Sos_str,temp_str,MMISET_SOS_MAX_LEN);
	 len = strlen(Sos_str);
	  Trace_Log_Print("SOS_EmergencyCallWin2 sos_temp=%d",sos_temp);
     if (!is_input_emergency_call && (len != 0))
     {

		sos_times++ ;
		 SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin2  0000");
		//sos_temp = TRUE;
  	     sim_type = MMIAPICC_GetCallSimType(dual_sys);
  	     MMIAPICC_MakeCall(
                    dual_sys,
                    Sos_str,
                    (uint8)(len),
                    PNULL,
                    PNULL,
                   sim_type,
                    CC_CALL_NORMAL_CALL,
                    PNULL
                    );

              //  MMK_CloseWin(win_id);
     }
	 else
	 {	
	  Trace_Log_Print("SOS_EmergencyCallWin2 sos_temp0000=%d",sos_times);
	  SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin2  1111");
	 	 if(sos_times == 1)
	 	{
			sos_times = 2;
			 SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin2  2222");
		}
		else if(sos_times == 4)
		{
			sos_times = 5;
			 SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin2  33333");
		}
		else if(sos_times == 7)
		{
			sos_times = 8;
			 SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin2  4444");
		}
		SOS_EmergencyCallWin3();
		// MMIPUB_OpenAlertWinByTextId(PNULL,TXT_NULL_SOS_CALL_NUM,TXT_NULL,IMAGE_PUBWIN_FAIL,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
	 }
	 SCI_FREE(Sos_str);
}


PUBLIC void SOS_EmergencyCallWin3(void)

{
     uint8 *Sos_str = PNULL;
	 uint8 * temp_str = PNULL;
	 uint16 len = 0;
     BOOLEAN is_input_emergency_call = FALSE;
     MMI_STRING_T str_info = {0};
	 MN_DUAL_SYS_E dual_sys = MN_DUAL_SYS_1;
	 CC_CALL_SIM_TYPE_E sim_type = CC_SIM1_CALL;
	 uint8 len_temp = 0;

    if(MMIAPIPHONE_IsSimAvailable(MN_DUAL_SYS_1))
	  {
		 dual_sys = MN_DUAL_SYS_1;
	  }
	  else if(MMIAPIPHONE_IsSimAvailable(MN_DUAL_SYS_2))
	  {
		 dual_sys = MN_DUAL_SYS_2;
	  }
	 else
	 {
		 MMIAPIPHONE_AlertSimNorOKStatus();
		 return;
	 }

     is_input_emergency_call = MMIAPIGET_SosCall_Status();
	 Sos_str = (uint8*)SCI_ALLOC_APP(MMISET_SOS_MAX_LEN);
	 temp_str = MMIAPIGET_SosCall_Num3();
	len_temp  =strlen(temp_str);
	//if(len_temp ==0 )
	//{
	//	temp_str = MMIAPIGET_SosCall_Num1();
	//	len_temp  =strlen(temp_str);
	//	if(len_temp ==0 )
	//	{
	//		temp_str = MMIAPIGET_SosCall_Num2();
	//	}
	//}
		SCI_MEMSET(Sos_str,0,MMISET_SOS_MAX_LEN);
	SCI_MEMCPY(Sos_str,temp_str,MMISET_SOS_MAX_LEN);
	 len = strlen(Sos_str);
	  Trace_Log_Print("SOS_EmergencyCallWin3 sos_temp=%d",sos_temp);
	 
     if (!is_input_emergency_call && (len != 0))
     {

		sos_times++ ;
		  SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin3  0000");
		//sos_temp = TRUE;
  	     sim_type = MMIAPICC_GetCallSimType(dual_sys);
  	     MMIAPICC_MakeCall(
                    dual_sys,
                    Sos_str,
                    (uint8)(len),
                    PNULL,
                    PNULL,
                   sim_type,
                    CC_CALL_NORMAL_CALL,
                    PNULL
                    );

              //  MMK_CloseWin(win_id);
     }
	 else
	 {

	   Trace_Log_Print("SOS_EmergencyCallWin3 sos_temp0000=%d",sos_times);
	     SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin3  1111");
	 	 if(sos_times == 2)
 	 	{
 			sos_times = 3;
			  SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin3  22222");
			SOS_EmergencyCallWin();
 		}
 		else if(sos_times == 5)
 		{
 			sos_times = 6;
			  SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin3  3333");
			SOS_EmergencyCallWin();
 		}
 		else if(sos_times == 8)
 		{
 		  SCI_TRACE_MID(" zdthero000	SOS_EmergencyCallWin3  44444");
 			sos_times = 9;
 		}
 		if(sos_times > 8 && sos_temp)
		{
			SCI_TRACE_MID(" zdthero000	  APP_MN_CALL_DISCONNECTED_INDasdasdasdas");
			sos_temp = FALSE;
		}
		// MMIPUB_OpenAlertWinByTextId(PNULL,TXT_NULL_SOS_CALL_NUM,TXT_NULL,IMAGE_PUBWIN_FAIL,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
	 }
	 SCI_FREE(Sos_str);
}

#if 0
PUBLIC void SOS_EmergencyCallWin(void)

{
     uint8 *Sos_str = PNULL;
	 uint8 * temp_str = PNULL;
	 uint16 len = 0;
     BOOLEAN is_input_emergency_call = FALSE;
     MMI_STRING_T str_info = {0};
	 MN_DUAL_SYS_E dual_sys = MN_DUAL_SYS_1;
	 CC_CALL_SIM_TYPE_E sim_type = CC_SIM1_CALL;
	 uint8 len_temp = 0;
	 if(!sos_temp)
	 {
		return;
	 }
	if(MMIAPIPHONE_IsSimOk(MN_DUAL_SYS_1))
	{
		dual_sys = MN_DUAL_SYS_1;
	}
	else if(MMIAPIPHONE_IsSimOk(MN_DUAL_SYS_2))
	{
		dual_sys = MN_DUAL_SYS_2;
	}
     is_input_emergency_call = MMIAPIGET_SosCall_Status();
	 Sos_str = (uint8*)SCI_ALLOC_APP(MMISET_SOS_MAX_LEN);
	 temp_str = MMIAPIGET_SosCall_Num1();
	len_temp  =strlen(temp_str);
	
	//if(len_temp ==0 )
	//{
	//	temp_str = MMIAPIGET_SosCall_Num2();
	//	len_temp  =strlen(temp_str);
	//	if(len_temp ==0 )
	//	{
	//		temp_str = MMIAPIGET_SosCall_Num3();
	//	}
	//}
	SCI_MEMSET(Sos_str,0,MMISET_SOS_MAX_LEN);
	SCI_MEMCPY(Sos_str,temp_str,MMISET_SOS_MAX_LEN);
	 len = strlen(Sos_str);
	 
     if (!is_input_emergency_call && (len != 0))
     {
		sos_times++ ;
		//sos_temp = TRUE;
  	     sim_type = MMIAPICC_GetCallSimType(dual_sys);
  	     MMIAPICC_MakeCall(
                    dual_sys,
                    Sos_str,
                    (uint8)(len),
                    PNULL,
                    PNULL,
                   sim_type,
                    CC_CALL_NORMAL_CALL,
                    PNULL
                    );

              //  MMK_CloseWin(win_id);
     }
	 else
	 {
	 //	sos_times = 1;
	//	SOS_EmergencyCallWin2();
		// MMIPUB_OpenAlertWinByTextId(PNULL,TXT_NULL_SOS_CALL_NUM,TXT_NULL,IMAGE_PUBWIN_FAIL,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
	 }
	 SCI_FREE(Sos_str);
}


PUBLIC void SOS_EmergencyCallWin2(void)

{
     uint8 *Sos_str = PNULL;
	 uint8 * temp_str = PNULL;
	 uint16 len = 0;
     BOOLEAN is_input_emergency_call = FALSE;
     MMI_STRING_T str_info = {0};
	 MN_DUAL_SYS_E dual_sys = MN_DUAL_SYS_1;
	 CC_CALL_SIM_TYPE_E sim_type = CC_SIM1_CALL;
	 uint8 len_temp = 0;

     is_input_emergency_call = MMIAPIGET_SosCall_Status();
	 Sos_str = (uint8*)SCI_ALLOC_APP(MMISET_SOS_MAX_LEN);
	 temp_str = MMIAPIGET_SosCall_Num2();
	len_temp  =strlen(temp_str);
	//if(len_temp ==0 )
	//{
	//	temp_str = MMIAPIGET_SosCall_Num1();
	//	len_temp  =strlen(temp_str);
	//	if(len_temp ==0 )
	//	{
	//		temp_str = MMIAPIGET_SosCall_Num3();
	//	}

	//}
		SCI_MEMSET(Sos_str,0,MMISET_SOS_MAX_LEN);
	SCI_MEMCPY(Sos_str,temp_str,MMISET_SOS_MAX_LEN);
	 len = strlen(Sos_str);
	 
     if (!is_input_emergency_call && (len != 0))
     {

		sos_times++ ;
		sos_temp = TRUE;
  	     sim_type = MMIAPICC_GetCallSimType(dual_sys);
  	     MMIAPICC_MakeCall(
                    dual_sys,
                    Sos_str,
                    (uint8)(len),
                    PNULL,
                    PNULL,
                   sim_type,
                    CC_CALL_NORMAL_CALL,
                    PNULL
                    );

              //  MMK_CloseWin(win_id);
     }
	 else
	 {	
	 	sos_times = 2;
		SOS_EmergencyCallWin3();
		// MMIPUB_OpenAlertWinByTextId(PNULL,TXT_NULL_SOS_CALL_NUM,TXT_NULL,IMAGE_PUBWIN_FAIL,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
	 }
	 SCI_FREE(Sos_str);
}


PUBLIC void SOS_EmergencyCallWin3(void)

{
     uint8 *Sos_str = PNULL;
	 uint8 * temp_str = PNULL;
	 uint16 len = 0;
     BOOLEAN is_input_emergency_call = FALSE;
     MMI_STRING_T str_info = {0};
	 MN_DUAL_SYS_E dual_sys = MN_DUAL_SYS_1;
	 CC_CALL_SIM_TYPE_E sim_type = CC_SIM1_CALL;
	 uint8 len_temp = 0;

     is_input_emergency_call = MMIAPIGET_SosCall_Status();
	 Sos_str = (uint8*)SCI_ALLOC_APP(MMISET_SOS_MAX_LEN);
	 temp_str = MMIAPIGET_SosCall_Num3();
	len_temp  =strlen(temp_str);
	//if(len_temp ==0 )
	//{
	//	temp_str = MMIAPIGET_SosCall_Num1();
	//	len_temp  =strlen(temp_str);
	//	if(len_temp ==0 )
	//	{
	//		temp_str = MMIAPIGET_SosCall_Num2();
	//	}
	//
	//}
		SCI_MEMSET(Sos_str,0,MMISET_SOS_MAX_LEN);
	SCI_MEMCPY(Sos_str,temp_str,MMISET_SOS_MAX_LEN);
	 len = strlen(Sos_str);
	 
     if (!is_input_emergency_call && (len != 0))
     {

		sos_times++ ;
		sos_temp = TRUE;
  	     sim_type = MMIAPICC_GetCallSimType(dual_sys);
  	     MMIAPICC_MakeCall(
                    dual_sys,
                    Sos_str,
                    (uint8)(len),
                    PNULL,
                    PNULL,
                   sim_type,
                    CC_CALL_NORMAL_CALL,
                    PNULL
                    );

              //  MMK_CloseWin(win_id);
     }
	 else
	 {
	 	sos_times++ ;
		// MMIPUB_OpenAlertWinByTextId(PNULL,TXT_NULL_SOS_CALL_NUM,TXT_NULL,IMAGE_PUBWIN_FAIL,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
	 }
	 SCI_FREE(Sos_str);
}
#endif
PUBLIC BOOLEAN SOS_GetNUM_IsOK(void)
{
	uint8* sos_num1 = {0};
	uint8 len1 = 0;
	uint8* sos_num2 = {0};
	uint8 len2 = 0;
	uint8* sos_num3 = {0};
	uint8 len3 = 0;
	
	sos_num1 =	MMIAPIGET_SosCall_Num1();
	len1 = strlen(sos_num1);
	//SCI_MEMSET(sos_num2,0,14);

	sos_num2 = MMIAPIGET_SosCall_Num2();
	len2  = strlen(sos_num2);
	//SCI_MEMSET(sos_num3,0,14);
	sos_num3 = MMIAPIGET_SosCall_Num3();
	len3  = strlen(sos_num3);
			

	if(len1 == 0  && len2 == 0 && len3 == 0)
	{
		//MMIPUB_OpenAlertWinByTextId(PNULL,TXT_NULL_SOS_CALL_NUM,TXT_NULL,IMAGE_PUBWIN_FAIL,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
		MMIPUB_OpenAlertWarningWin(TXT_NULL_SOS_CALL_NUM);
			
		return FALSE;
	}
	return TRUE;


}

#endif



