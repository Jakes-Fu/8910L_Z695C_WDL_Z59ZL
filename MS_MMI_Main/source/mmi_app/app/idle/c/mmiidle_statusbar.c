...
LOCAL BOOLEAN DisplayMsgInboxFunc(void)
{
    ...
    if(IMAGE_IDLE_TITLE_ICON_EB == image_id)
    {
        ...
        && !(IMAGE_STATUS_MESSAGE == image_id && MMISMS_FOLDER_SECURITY == MMIAPIMMS_GetDownloadingMMSFolderType())
        )
    {
       // 打开SMS列表窗口；
...
#if defined(TORCH_SUPPORT)
    case MMI_WIN_ICON_TORCH: //Torch
        item_data.icon_info.icon_array[0] = IMAGE_IDLE_TITLE_ICON_FLASHLIGHT;
        item_data.app_handle_func = PNULL;
        GUIWIN_AppendStbItemData(&item_data);
        if(TRUE == MMIIDLE_GetTorchStatus())
        {
            GUIWIN_SetStbItemVisible(MMI_WIN_ICON_TORCH, TRUE);
        }
        else
        {
            GUIWIN_SetStbItemVisible(MMI_WIN_ICON_TORCH, FALSE);
        }
        break; 
#endif
...
#else			
			IMAGE_STATUS_BATTERY_NORMAL_1,
                                                                    IMAGE_STATUS_BATTERY_NORMAL_3,
                                                                    IMAGE_STATUS_BATTERY_NORMAL_5,
                                                                    IMAGE_STATUS_BATTERY_NORMAL_7,
                                                                    IMAGE_STATUS_BATTERY_NORMAL_8,
			IMAGE_STATUS_BATTERY_NORMAL_FULL
#endif
			};
...
#if defined CMCC_UI_STYLE || defined NET_ICON_UI_STYLE
    for(i = MMI_WIN_ICON_SIGNAL; i < MMI_DUAL_SYS_MAX; i++)
    {
                                ...
                                )
{
    ...
    else if (MMIAPIPHONE_GetSimExistedStatus(dual_sys))
    {
        ...
        isRaticonable = FALSE;//MMIAPICOM_IsExistInRatIconTable(&cur_plmn, &isGmust, &isLTEmust);
...
//  Note:
/*****************************************************************************/
PUBLIC void MAIN_SetTorchState(BOOLEAN is_torch_on)
{
    MMI_IMAGE_ID_T      image_id    =   IMAGE_IDLE_TITLE_ICON_FLASHLIGHT;
    GUIWIN_SetStbItemIconId(MMI_WIN_ICON_TORCH, image_id);
    GUIWIN_SetStbItemIconCurFrame(MMI_WIN_ICON_TORCH, 0);
    ...
    GUIWIN_SetStbItemRightAlign(MMI_WIN_ICON_TORCH, FALSE);
...
#if 0
    if(back_info.app_id == HERO_APP_XXX)
    {
        switch(back_info.icon_state)
        {
    #if 0//def HERO_APP_WHATSAPP
        case HEROAPP_STATE_ONLINE:
            img_id = IMG_HERO_APP_ONLINE;
            break;
        case HEROAPP_STATE_OFFLINE:
            img_id = IMG_HERO_APP_OFFLINE;
            break;
	#endif
        default:
            break;
