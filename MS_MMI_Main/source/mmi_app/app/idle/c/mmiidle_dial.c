...
#if defined(ZDT_PHILIPS_DIAL_SEARCH_STYLE)// 恢复为40 长度是必要的.DTMF可能会输入身份证等号码或者帐号
...
#define MMI_IDLE_DIAL_EDIT_NUM_COLOR		   0xffff //GUI_RGB2RGB565(43,43,43)//0x4d60
#else
...
#else
#if defined(ZDT_PHILIPS_DIAL_SEARCH_STYLE)
#define MMIIDLE_DIAL_EDIT_FORM_HEIGHT 280///92 //280//45
#else
#define MMIIDLE_DIAL_EDIT_FORM_HEIGHT 280 //280//45
#endif
#endif
...
LOCAL uint16     s_key_up_press_inc;
...
LOCAL BOOLEAN IsResetpnut(MMI_STRING_T *str_ptr);
#ifdef ZDT_PHILIPS_DIAL_SEARCH_STYLE

static BOOLEAN  smart_dial_open = FALSE;
LOCAL void ChangeEditListBig(void);
LOCAL void ChangeEditListSmart(void);
#endif
...
//  Note: 
/*****************************************************************************/
PUBLIC    MMI_RESULT_E HandleSIMSelectWinMsg(
                                        MMI_WIN_ID_T     win_id, 
...
///显示使用SIM卡拨出的选项                                                
WINDOW_TABLE( MMIDIAL_SELECT_SIM_WIN_TAB ) = 
{
    WIN_FUNC((uint32)HandleSIMSelectWinMsg),    
    WIN_ID(MMIIDLE_DIAL_SELECT_SIM_WIN_ID),
...
LOCAL void HandleDialVoiceMailNumberMsg(
                       	        MMI_WIN_ID_T    win_id,
                                MMI_CTRL_ID_T   ctrl_id,
...
#endif
/*****************************************************************************/
// 	Description : 获取匹配中当前项的号码信息
//	Global resource dependence : 
//	Note: 
/*****************************************************************************/
LOCAL BOOLEAN GetCurrentNumber(MMI_WIN_ID_T win_id);
...
LOCAL BOOLEAN EntryListUpdateListContentItem2(
											 MMI_WIN_ID_T win_id,
											 MMI_CTRL_ID_T ctrl_id, 
											 void *item_ptr);
LOCAL MMI_RESULT_E  HandleAddOrEditVoiceMailNumberMsg(
                                             MMI_WIN_ID_T win_id, 
                                             MMI_MESSAGE_ID_E msg_id, 
                                             DPARAM param 
                                             );
...
WINDOW_TABLE( MMIIDLE_DIAL_MENU_WIN_TAB_1 ) = 
{
    WIN_FUNC( (uint32)HandleDialMenuWinMsg),
    WIN_ID(MMIIDLE_DIAL_MENU_WIN_ID),
    WIN_STYLE( WS_HAS_TRANSPARENT),
    CREATE_POPMENU_CTRL(MENU_ADD_PB_OPT,MMIIDLE_DIAL_POPMENU_CTRL_ID),
#ifdef MMI_DIAL_MINI_SUPPORT
    WIN_TITLE(STXT_OPTION),
#endif  
    WIN_SOFTKEY(TXT_COMMON_OK,TXT_NULL,STXT_RETURN),
...
#if 0
def HERO_ENGINE_TTS_SUPPORT
			{
				extern void hero_tts_printf(const char *format,...);
				hero_tts_printf("[inkleak.zhao] MMIAPIIDLE_EditDialWin msg_id = %d", msg_id);
				if((msg_id >= MSG_APP_1) && ( msg_id <= MSG_APP_HASH))
				{
					hero_tts_dial_broadcast(msg_id);
				}
			}
#endif
        if (is_dial_key)
        {
            ...
            {
                if (((msg_id >= MSG_APP_1) && ( msg_id <= MSG_APP_HASH)) || (msg_id == MSG_KEYUP_STAR))
                {
	...
	if(smart_dial_open)
	{
		...
		list_init.both_rect.v_rect.top = client_rect.v_rect.top + MMIIDLE_DIAL_EDIT_FORM_HEIGHT + MMIIDLE_DIAL_LINE_WIDTH - 95;
...
#endif		
	{
		...
		list_init.type = GUILIST_TEXTLIST_E;
            ...
            //init list
            {
                ...
                //GUILIST_TYPE_E	 listbox_type = GUILIST_TEXTLIST_E;

                GUILIST_SetMaxItem(list_ctrl.ctrl_id, 1, TRUE );
		  #if 0
				GUILIST_ChangeDisplayType(
					MMIIDLE_DIAL_LISTBOX_CTRL_ID,
					listbox_type,
					FALSE
					);
		   #endif
...
#endif
/*****************************************************************************/
//	Description : handle voice mail function for long press 1.
//	Global resource dependence : 
//	Note:
/*****************************************************************************/
PUBLIC void MMIAPIIDLE_HandleAddNewVoiceMailNumberFunction(void)
{
		...
		(SIM_STATUS_OK == MMIAPIPHONE_GetSimStatus(MN_DUAL_SYS_2)))
	{
		...
		{
			  s_sim_sel_dual_sys =  MMIAPICC_GetCallingSIM();
			  GetVoiceMailAddress(s_sim_sel_dual_sys, MN_MULTI_SUBSCRIBER_ID_1,  MN_VOICE_MAIL);	
		}
		else
		{
			 MMK_CreateWin((uint32 *)MMIDIAL_SELECT_SIM_WIN_TAB,PNULL);	
		}
	}
	else if ((SIM_STATUS_OK == MMIAPIPHONE_GetSimStatus(MN_DUAL_SYS_1)) &&
		(SIM_STATUS_OK != MMIAPIPHONE_GetSimStatus(MN_DUAL_SYS_2)))
	{
		s_sim_sel_dual_sys =  0;
		GetVoiceMailAddress(s_sim_sel_dual_sys, MN_MULTI_SUBSCRIBER_ID_1,  MN_VOICE_MAIL);
	}
	else if ((SIM_STATUS_OK == MMIAPIPHONE_GetSimStatus(MN_DUAL_SYS_2)) &&
		(SIM_STATUS_OK != MMIAPIPHONE_GetSimStatus(MN_DUAL_SYS_1)))
	{
		s_sim_sel_dual_sys =  1;
		GetVoiceMailAddress(s_sim_sel_dual_sys, MN_MULTI_SUBSCRIBER_ID_1,  MN_VOICE_MAIL);		
	}
	else
	{
		// no sim availble
		MMIPUB_OpenAlertWarningWin(TXT_INSERT_SIM_AND_INFO);
	}
}

PUBLIC void MMIAPIIDLE_HandleVoiceMailFunction(void)
{
	// 检查是否有插入SIM卡
	if (MMIAPIPHONE_GetSimExistedNum(PNULL, 0) == 0)
	{
		MMIPUB_OpenAlertWarningWin(STR_NOTE_NO_SIM_EXT01);
		return;
	}
	
	MMISET_SetVmOperType(MMISET_VMN_OPER_TYPE_IDLE_DIAL);
	...
	GetVoiceMailAddress(s_sim_sel_dual_sys, MN_MULTI_SUBSCRIBER_ID_1,  MN_VOICE_MAIL);
#else
	MMIAPIIDLE_HandleAddNewVoiceMailNumberFunction();
#endif
}

PUBLIC void MMIAPIIDLE_HandleVoiceMailFromNdt(MN_DUAL_SYS_E dual_sys)
{
    uint8 vm1tele_len = 0;
    wchar vm1tele_str[CC_MAX_TELE_NUM_LEN]  = {0};
    uint8 num_str[CC_MAX_TELE_NUM_LEN] = {0};
    MMI_STRING_T    title_str = {0};
    CC_CALL_SIM_TYPE_E call_sim_type = CC_SIM1_CALL;

    s_sim_sel_dual_sys = dual_sys;
    if (SIM_STATUS_OK != MMIAPIPHONE_GetSimStatus(dual_sys))
        return;
        
    if (MN_DUAL_SYS_2 == s_sim_sel_dual_sys)
    {
        call_sim_type = CC_SIM2_CALL;
    }
    vm1tele_len=MMISET_GetIVoiceMailNumberToNV(dual_sys,vm1tele_str);
    SCI_TRACE_LOW("ljg*MMIAPIIDLE_HandleVoiceMailFromNdt,dual_sys=%d,vm1tele_len=%d",dual_sys,vm1tele_len);

    if ((vm1tele_len > 0) && (vm1tele_str[0] != 'N'))
    {
        SCI_TRACE_LOW("ljg*MMIAPIIDLE_HandleVoiceMailFromNdt,vm1tele_str=%s", vm1tele_str);
        MMI_WSTRNTOSTR((uint8 *)num_str,vm1tele_len,vm1tele_str,vm1tele_len,vm1tele_len);
        MMIAPICC_MakeCall(dual_sys,num_str,strlen(num_str),PNULL,PNULL,call_sim_type,CC_CALL_NORMAL_CALL,PNULL);
    }
    else
    {
        MMIRES_GetText(STR_QUERY_VOICEMAIL_NUM_EXT01, PNULL, &title_str);
        MMIPUB_OpenDialogInputFieldWinByTextPtr(&title_str, PNULL, PNULL, MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN_VMAIL, HandleAddOrEditVoiceMailNumberMsg);
    }
}

/*****************************************************************************/
//	Description : handle speed dial function for long press 2-9.
//	Global resource dependence : 
//	Note:
/*****************************************************************************/
PUBLIC void MMIAPIIDLE_HandleSpeedDialFunction(MMI_MESSAGE_ID_E msg_id)
{
    ...
    uint8 sms_num[42] = {0};
...
#endif
        {
            ...
            key_num = msg_id - MSG_KEYLONG_1 + 1;
            SCI_MEMSET(&speedkey_entry, 0, sizeof( CC_SPEEDKEY_ENTRY_T ));
            MMINV_READ((MMI_CC_SPEEDKEY_1 + key_num - 1), &speedkey_entry, ret_val);
            ...
            if((MMIAPICC_SpeedDialEnableRead()))
            {
                if(MN_RETURN_SUCCESS == ret_val )
                {
                    if(speedkey_entry.is_sms){
                        ...
                        MMIAPISMS_WriteNewMessage(MN_DUAL_SYS_MAX,PNULL,sms_num,tele_len);
                    }else{
...
)
{
	...
	SCI_TRACE_LOW("[MMIIDLE] func[%s] line[%d] msg_id %x ", __FUNCTION__, __LINE__, msg_id);		
	switch (msg_id)
	{
	case MSG_OPEN_WINDOW:  
#if defined(TOUCH_PANEL_SUPPORT)
...
#ifdef TWO_CALL_KEY_SUPPORT
		GUIEDIT_SetSoftkey(edit_ctrl_id,0,0,TXT_CALL,TXT_CALL,NULL); 
		GUIEDIT_SetSoftkey(edit_ctrl_id,1,0,STXT_OPTION,STXT_OPTION,NULL); 
#else
		...
		{
			GUIEDIT_SetSoftkeyEx(edit_ctrl_id,0,0,STXT_OPTION,STXT_OPTION,STXT_SOFTKEY_MENU_MK,STXT_SOFTKEY_MENU_MK,EDIT_BUTTON_TEXT_ID,NULL);
		}
		else
		{
			GUIEDIT_SetSoftkeyEx(edit_ctrl_id,0,0,STXT_OPTION,STXT_OPTION,NULL,STXT_SOFTKEY_MENU_MK,EDIT_BUTTON_TEXT_ID,NULL);
		}
	
		GUIEDIT_SetSoftkey(edit_ctrl_id,1,0,STXT_SOFTKEY_CALL_MK,STXT_SOFTKEY_CALL_MK,NULL); 
#endif
#ifdef DPHONE_SUPPORT //added by taiping.lai 2010/12/28
		GUIEDIT_SetSoftkey(edit_ctrl_id,2,1,STXT_RETURN,TXT_DELETE,NULL); 
#else
       #if defined(ZDT_PHILIPS_DIAL_SEARCH_STYLE)
           //GUIEDIT_SetSoftkey(edit_ctrl_id,2,1,STXT_RETURN,TXT_DELETE,SetDialWinRightSoftkey);// ZDT_PHILIPS_DIAL_SEARCH_STYLE
           GUIEDIT_SetSoftkey(edit_ctrl_id,2,1,STXT_RETURN,TXT_DELETE,NULL);    //edit by song 20210409 去掉拨号时第一个按*以后右软键显示为"锁屏"
       //GUIEDIT_SetSoftkey(edit_ctrl_id,2,1,STXT_RETURN,TXT_COMMON_CLEAR,NULL); // ZDT_PHILIPS_DIAL_SEARCH_STYLE
           //GUIEDIT_SetSoftkeyEx(edit_ctrl_id,2,1,STXT_RETURN,TXT_DELETE,IMAGE_COMMON_BACK,IMAGE_COMMON_BACKSPACE,EDIT_BUTTON_TEXT_ID,SetDialWinRightSoftkey);
       #else
	//	GUIEDIT_SetSoftkey(edit_ctrl_id,2,1,STXT_RETURN,TXT_DELETE,SetDialWinRightSoftkey);
	GUIEDIT_SetSoftkey(edit_ctrl_id,2,1,STXT_RETURN,TXT_COMMON_CLEAR,NULL); 
		//GUIEDIT_SetSoftkeyEx(edit_ctrl_id,2,1,STXT_RETURN,TXT_DELETE,IMAGE_COMMON_BACK,IMAGE_COMMON_BACKSPACE,EDIT_BUTTON_TEXT_ID,SetDialWinRightSoftkey);
       #endif
#if 0        
		MMI_GetLabelTextByLang(STR_CL_HDR_CALL_EXT01, &title_str );
	  ...
	  if(smart_dial_open)
	  {
		 GUIEDIT_SetSoftkeyEx(edit_ctrl_id,1,0,STXT_BIG,STXT_BIG,IMAGE_NULL,IMAGE_NULL,EDIT_BUTTON_TEXT_ID,PNULL);
	  }
	  else
	  {
		GUIEDIT_SetSoftkeyEx(edit_ctrl_id,1,0,STXT_SMART,STXT_SMART,IMAGE_NULL,IMAGE_NULL,EDIT_BUTTON_TEXT_ID,PNULL);
	  }
#endif
#endif
	...
	case MSG_GET_FOCUS:
		...
		if (!s_is_switch_to_editor)
		{
			...
			MMK_SetActiveCtrl(edit_ctrl_id,TRUE);
			MMK_SendMsg(edit_ctrl_id, MSG_CTL_PAINT, PNULL);
			if (TRUE != *(BOOLEAN*)param)
			{
				// MMK_SendMsg(edit_ctrl_id, msg_id, &is_send);
			}	
			s_is_switch_to_editor=TRUE;
		}
#endif
	...
	case MSG_NOTIFY_EDITBOX_UPDATE_STRNUM:
	{
		...
		{
                if ((('#' == edit_str.wstr_ptr[edit_str.wstr_len - 1]) || ('*' == edit_str.wstr_ptr[edit_str.wstr_len - 1]))    //xiongkai    暗码以* 号结束也进行比较
		...
		else if (is_need_check && IsResetpnut(&edit_str))
		{ 
		...
		else if (MMIPHONE_IsSIMUnlockingInput(&edit_str))
		{
			MMIPHONE_HandleSIMUnlockingInputingStringEx(win_id, &edit_str, MN_DUAL_SYS_1);
		}
#endif
	...
	case MSG_KEYUP_UP:
		...
		s_key_up_press_inc=CTRLLIST_GetCurItemIndex(list_ctrl_id);
		// GetCurrentNumber(win_id);
		// MMK_SetActiveCtrl(edit_ctrl_id,TRUE);
		break;
	...
	case MSG_KEYUP_DOWN:
		...
		if (s_is_switch_to_editor)
		{
			if(CTRLLIST_GetTotalItemNum(list_ctrl_id)>0)
			{
				GUI_BG_T edit_bg = {GUI_BG_COLOR,GUI_SHAPE_RECT,0,MMI_BLACK_COLOR,FALSE};
				edit_bg.color=MMI_BACKGROUND_COLOR;
				GUIEDIT_SetBg(edit_ctrl_id, &edit_bg);
				list_ctrl_id = MMK_GetCtrlHandleByWin(win_id,MMIIDLE_DIAL_LISTBOX_CTRL_ID);  
				MMK_SetActiveCtrl(list_ctrl_id,TRUE);
				CTRLLIST_SetCycScroll(list_ctrl_id,FALSE);
				GUILIST_SetCurItemIndex(list_ctrl_id,0);
				MMK_SendMsg(edit_ctrl_id, MSG_CTL_PAINT, PNULL);
				// CTRLLIST_SetCurItemIndex(list_ctrl_id,0);
				s_is_switch_to_editor=FALSE;
			}
		}

		if (s_num_search_info.pb_match_item_num<1) 
		{
			if (GUILIST_GetTotalItemNum(list_ctrl_id)==2)
			{
	...
	case MSG_KEYUP_RIGHT:
		// wangfeizhou modify because Call UI spec changed	
		// MMK_CreateWin((uint32 *)MMIDIAL_SELECT_SIM_WIN_TAB,PNULL);
		break;
#endif // #if 0
	case MSG_CTL_MIDSK:
		// Mocor20b MIDDLE key与green key分开不同行为处理
		#if defined(ZDT_PHILIPS_DIAL_SEARCH_STYLE) //WEB KEY 拨号查找功能 OK键可以把完整号码加载到编辑框
              //  HandleDialWebMsg(win_id, edit_ctrl_id);
			if(smart_dial_open == TRUE)
			{
				smart_dial_open = FALSE;
				ChangeEditListBig();
				
			}
			else if (smart_dial_open == FALSE)
			{
				smart_dial_open = TRUE;
				ChangeEditListSmart();
			}
		...
		if (s_dial_down_flag)
		{
			HandleDialGreenUpMsg(win_id,edit_ctrl_id,MN_DUAL_SYS_MAX);
	...
	case MSG_KEYUP_CALL3:
	case MSG_KEYUP_CALL4:
		HandleDialGreenUpMsg(win_id,edit_ctrl_id,MMIAPICC_CheckDualSysType( msg_id, MN_DUAL_SYS_MAX));
		break;
	...
	case MSG_KEYLONG_9:
		GUIEDIT_GetString(edit_ctrl_id, &edit_str);
#if defined(MMI_INDIAN_SOS_SUPPORT) && !defined(MMI_PDA_SUPPORT)
		if ((TRUE == is_indiasos_supported) && (1 == edit_str.wstr_len) && (MMISET_SOS_ENTRY_CODE == msg_id) && MMIAPISET_GetSOSSwitchState())
		{
...
#endif
		if ((0 == edit_str.wstr_len)||(1 == edit_str.wstr_len))MMIAPIIDLE_HandleSpeedDialFunction(msg_id);
		break;
   ...
   case MSG_CTL_PENOK:
	...
	case MSG_APP_DOWN:
		#ifdef ZDT_PRODUCT_Z685S
		{
			...
			{
			        #if 0
					defined(ZDT_PHILIPS_DIAL_SEARCH_STYLE)
                                 //NULL
                                #else
				if(!GUILIST_GetListState( list_ctrl_id, GUILIST_STATE_ACTIVE ))
				{
					GUILIST_SetListState( list_ctrl_id, GUILIST_STATE_ACTIVE, TRUE );
	...
	case MSG_CTL_LIST_NEED_ITEM_CONTENT:
		EntryListUpdateListContentItem2(win_id, list_ctrl_id,param);		
		break;   
			...
			if( 0 < edit_str.wstr_len && s_num_search_info.cl_match_item_num <= effect_str->item_index)
			{
				...
				if (0 < index_str.wstr_len)
				{
					...
					for (index = 0; index < index_str.wstr_len; index++)
					{
						...
						effect_str->region_ptr[index].font_color = MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_DEFAULT) /*MMI_DARK_GREEN_COLOR*/;
					}
				}
			}
		}
                           ...
                           )
{
    uint8           temp_str[MMIIDLE_DIAL_MAX_LEN + 1]  = {0};
    MMI_STRING_T    edit_str = {0};
    MMI_CTRL_ID_T edit_ctrl_id = MMK_GetCtrlHandleByWin(ctrl_id, MMIIDLE_DIAL_EDIT_CTRL_ID);
    MMI_CTRL_ID_T list_ctrl_id = MMK_GetCtrlHandleByWin(ctrl_id,MMIIDLE_DIAL_LISTBOX_CTRL_ID);
...
PUBLIC void MMIAPIDIAL_MENU_ENTERMESSAGE(uint32  win_id )
{	
                                        ...
                                        )
{
    ...
    MMI_CTRL_ID_T edit_ctrl_id = MMK_GetCtrlHandleByWin((MMI_CTRL_ID_T)add_data, MMIIDLE_DIAL_EDIT_CTRL_ID);
    MMI_CTRL_ID_T list_ctrl_id = MMK_GetCtrlHandleByWin((MMI_CTRL_ID_T)add_data,MMIIDLE_DIAL_LISTBOX_CTRL_ID);
    MMI_MENU_ID_T       menu_id = 0;
    MMIPB_BCD_NUMBER_T  pb_num = {0};
    MMI_PARTY_NUMBER_T  party_num = {0};/*lint !e64*/
...
#else			
        //set dial menu gray
        SetDialMenuGray(win_id,edit_ctrl_id,list_ctrl_id);
#endif 
        MMK_SetAtvCtrl(win_id,ctrl_id);
        break;

   case MSG_CTL_OK:   //fix bug 491970
    case MSG_CTL_MIDSK:
#ifdef TOUCH_PANEL_SUPPORT //IGNORE9527
    case MSG_CTL_PENOK:
#endif //TOUCH_PANEL_SUPPORT //IGNORE9527
        //get edit string
#ifndef ZDT_PHILIPS_DIAL_SEARCH_STYLE
        GUIEDIT_GetString(edit_ctrl_id,&edit_str);
#endif
#if 0
        //get uint8 string
        if(!MMK_IsActiveCtrl(list_ctrl_id)&&(0 < edit_str.wstr_len) && (MMIIDLE_DIAL_MAX_LEN >= edit_str.wstr_len))
        {
        ...
        }else
        {
		    ...
		    if (!s_is_switch_to_editor)
                {
			if (s_num_search_info.pb_match_item_num>0) 
			{
				 GetCurrentNumber((MMI_CTRL_ID_T)add_data);
			}
                   GUIEDIT_GetString(edit_ctrl_id,&edit_str);
...
#else
#ifdef ZDT_PHILIPS_DIAL_SEARCH_STYLE
		cur_list_index = GUILIST_GetCurItemIndex(MMK_GetCtrlHandleByWin(MMIIDLE_DIAL_WIN_ID, MMIIDLE_DIAL_LISTBOX_CTRL_ID));
		if((cur_list_index >= 0) && (0 != (s_num_search_info.cl_match_item_num + s_num_search_info.pb_match_item_num)) && GUILIST_GetListState( MMIIDLE_DIAL_LISTBOX_CTRL_ID, GUILIST_STATE_ACTIVE ))
		{
				if(s_num_search_info.cl_match_item_num > cur_list_index)
				{
					MMIAPICL_GetSearchEntryNumber(&num_str, cur_list_index);
				...
				// MMK_PostMsg(MMK_GetCtrlHandleByWin(MMIIDLE_DIAL_WIN_TAB, MMIIDLE_DIAL_EDIT_CTRL_ID), MSG_CTL_PAINT, PNULL, 0);
				phone_num_len = num_str.wstr_len;
				MMI_WSTRNTOSTR((uint8 *)phone_num_str,MMIIDLE_DIAL_MAX_LEN,
					num_str.wstr_ptr,num_str.wstr_len,
					phone_num_len);
	...
	//s_is_switch_to_editor=TRUE;
        ...
        case ID_DIAL_SEND_SMS:
#if 0
			if( SCI_FALSE == MMIAPISMS_OpenConfirmation((void *)win_id,MMISMS_ENTER_BY_DIAL_MENU))
			{
        ...
        case ID_DIAL_SEND_MMS:
            //if (edit_str.wstr_len > 0 && PNULL != edit_str.wstr_ptr)	
                MMIAPIMMS_AnswerMMS(MN_DUAL_SYS_MAX,PNULL,&edit_str);
            //else
                //MMIAPIMMS_AnswerMMS(MN_DUAL_SYS_MAX,PNULL,PNULL);	
            break;
...
/*****************************************************************************/
//  Description : handle dial voice mail number
//  Global resource dependence : 
//  Note: 
/*****************************************************************************/
                                ...
                                )
{
	...
	if (!s_is_switch_to_editor)
        {
         if (! GetCurrentNumber(win_id)) return;
        }
#endif 
    //get edit string
    ...
    else
    {
		...
		//MMISET_GetIVoiceMailNumberToNV()
                           ...
                           )
{
    ...
    MMI_STRING_T    edit_str = {0};
    MMI_HANDLE_T edit_ctrl_handle = MMK_GetCtrlHandleByWin(win_id,edit_ctrl_id);//Dialer options menu
    uint32 sim_ok_num = 0;
    ...
    if (0 == edit_str.wstr_len)
    {
        ...
        GUIMENU_SetItemGrayed(MMIIDLE_DIAL_POPMENU_CTRL_ID,MENU_PHONENUM_DIALUP_OPERATE,ID_DIAL_CALL_AUDIO,TRUE);
        //Dialer options menu
...
//Dialer options menu
    ...
    else
    {
       ...
       MMI_MENU_GROUP_ID_T  group_id = MENU_DIAL_OPT;
        ...
        if (GUILIST_GetListState(list_ctrl_id,GUILIST_STATE_ACTIVE))
        {
            GUIMENU_SetItemVisible(MMIIDLE_DIAL_POPMENU_CTRL_ID,MENU_PHONENUM_DIALUP_OPERATE,ID_DIAL_ADD_PB_OPT,FALSE);
        }
	  #endif
    }
	...
	uint16 cur_list_index = 0; 
	  ...
	  if((cur_list_index >= 0) && (0 != (s_num_search_info.cl_match_item_num + s_num_search_info.pb_match_item_num)) && GUILIST_GetListState( MMIIDLE_DIAL_LISTBOX_CTRL_ID, GUILIST_STATE_ACTIVE ))
	  {
			  ...
			  else
			  {
			   ...
			   GUIEDIT_SetString(ctrl_id, num_str.wstr_ptr, num_str.wstr_len);
			  // MMK_PostMsg(MMK_GetCtrlHandleByWin(MMIIDLE_DIAL_WIN_TAB, MMIIDLE_DIAL_EDIT_CTRL_ID), MSG_CTL_PAINT, PNULL, 0);
			  list_phone_num_len = num_str.wstr_len;
			  MMI_WSTRNTOSTR((uint8 *)temp_str,MMIIDLE_DIAL_MAX_LEN,
				  num_str.wstr_ptr,num_str.wstr_len,
...
LOCAL void ChangeEditListSmart(void)
{
    ...
    GUI_BOTH_RECT_T client_rect = MMITHEME_GetWinClientBothRect(MMIIDLE_DIAL_WIN_ID);
...
#else
	GUI_BG_T		edit_bg = {GUI_BG_IMG,GUI_SHAPE_RECT,IMAGE_COMMON_BG,MMI_WHITE_COLOR,FALSE};
	GUI_BORDER_T	edit_border = {1,MMI_BACKGROUND_COLOR,GUI_BORDER_NONE};
	GUI_COLOR_T 	edit_font_color = MMI_BLACK_COLOR ; ///MMI_IDLE_DIAL_EDIT_NUM_COLOR;
#endif
	...
	{
	edit_rect.left = 0;
	edit_rect.top = client_rect.v_rect.top;
	edit_rect.right = client_rect.v_rect.right;
	edit_rect.bottom = edit_rect.top + MMIIDLE_DIAL_EDIT_FORM_HEIGHT - MMIIDLE_DIAL_LINE_WIDTH - 95;
	...
	edit_rect.top = client_rect.v_rect.top;
	edit_rect.right = client_rect.v_rect.right;
	edit_rect.bottom = edit_rect.top + MMIIDLE_DIAL_EDIT_FORM_HEIGHT - MMIIDLE_DIAL_LINE_WIDTH;
	}
	GUIEDIT_SetRect(MMK_GetCtrlHandleByWin(MMIIDLE_DIAL_WIN_ID, edit_ctrl_id),&edit_rect);
	if(smart_dial_open)
	{
		list_rect.left = 0;
		list_rect.right = client_rect.v_rect.right;
		list_rect.top = client_rect.v_rect.top + MMIIDLE_DIAL_EDIT_FORM_HEIGHT + MMIIDLE_DIAL_LINE_WIDTH - 95;
		list_rect.bottom = client_rect.v_rect.bottom;
	...
	else
	{
		list_rect.left = 0;
		list_rect.right = client_rect.v_rect.right;
		list_rect.top = client_rect.v_rect.top + MMIIDLE_DIAL_EDIT_FORM_HEIGHT + MMIIDLE_DIAL_LINE_WIDTH;
		list_rect.bottom = client_rect.v_rect.bottom;

	}
	GUILIST_SetRect(MMK_GetCtrlHandleByWin(MMIIDLE_DIAL_WIN_ID,MMIIDLE_DIAL_LISTBOX_CTRL_ID),&list_rect);
	{
			...
			GUIEDIT_SetFontColor(edit_ctrl_id, edit_font_color);
			{
				...
				GUIEDIT_SetFontEx(MMK_GetCtrlHandleByWin(MMIIDLE_DIAL_WIN_ID, edit_ctrl_id), &editfont);
				GUIEDIT_SetDialEditAdaptive(MMK_GetCtrlHandleByWin(MMIIDLE_DIAL_WIN_ID, edit_ctrl_id), TRUE, TRUE);
#ifdef ZDT_EDIT_NUM_IMG_SUPPORT
			if(smart_dial_open)
			//if(0)
			{
				GUIEDIT_SetLineSpace(MMK_GetCtrlHandleByWin(MMIIDLE_DIAL_WIN_ID, edit_ctrl_id),0);
			...
			GUIEDIT_SetCharSpace(MMK_GetCtrlHandleByWin(MMIIDLE_DIAL_WIN_ID, edit_ctrl_id),0);
			GUIEDIT_SetPhoneNumImg(MMK_GetCtrlHandleByWin(MMIIDLE_DIAL_WIN_ID, edit_ctrl_id), TRUE,MMITHEME_PHONE_IMG_WIDTH,MMITHEME_PHONE_IMG_HEIGHT);
#endif
	...
	GUIBUTTON_SetVisible(MMIIDLE_DIAL_EDIT_BOX_LINE_CTRL_ID,TRUE,TRUE);

	MMK_SendMsg(MMIIDLE_DIAL_WIN_ID, MSG_FULL_PAINT, PNULL);
...
LOCAL void ChangeEditListBig(void)
{
	...
	if(smart_dial_open)
	{
		...
		list_rect.right = client_rect.v_rect.right;
		list_rect.top = client_rect.v_rect.top + MMIIDLE_DIAL_EDIT_FORM_HEIGHT + MMIIDLE_DIAL_LINE_WIDTH - 95;
		list_rect.bottom = client_rect.v_rect.bottom - MMI_FULLSCREEN_SOFTKEY_BAR_HEIGHT;
	...
	GUIBUTTON_SetVisible(MMIIDLE_DIAL_EDIT_BOX_LINE_CTRL_ID,FALSE,TRUE);
...
LOCAL void InitDialEdit(MMI_WIN_ID_T win_id)
{
    ...
    edit_init.both_rect.v_rect.bottom = edit_init.both_rect.v_rect.top + MMIIDLE_DIAL_EDIT_FORM_HEIGHT - MMIIDLE_DIAL_LINE_WIDTH - 95;
    }
 else
#endif
 {
    edit_init.both_rect.v_rect.left = 0;
    edit_init.both_rect.v_rect.top =  0;///client_rect.v_rect.top;
    edit_init.both_rect.v_rect.right = client_rect.v_rect.right;
    ...
    #else
    edit_init.both_rect.h_rect.top = 0;//client_rect.h_rect.top;
    #endif
    ...
    {
        GUIEDIT_SetAlign(edit_ctrl_id, ALIGN_RVMIDDLE);
    }
    else
    {
    ...
    //set border & font color & bg
    {
		...
		edit_bg.color=MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_DEFAULT);
#else
	GUI_BG_T		edit_bg = {GUI_BG_COLOR,GUI_SHAPE_RECT,IMAGE_NULL,MMI_IDLE_DIAL_EDIT_NUM_COLOR,FALSE};
	GUI_BORDER_T	edit_border = {1,MMI_BACKGROUND_COLOR,GUI_BORDER_NONE};
	GUI_COLOR_T 	edit_font_color = MMI_BLACK_COLOR;
#endif
#endif
        ...
        GUIEDIT_SetFontColor(edit_ctrl_id, edit_font_color);
        {
           ...
           #if 0
		   defined(ZDT_PHILIPS_DIAL_SEARCH_STYLE)
           editfont.big_font = SONG_FONT_64; // ZDT_PHILIPS_DIAL_SEARCH_STYLE  36 --60
           editfont.mid_font = SONG_FONT_42; // ZDT_PHILIPS_DIAL_SEARCH_STYLE
           editfont.small_font = SONG_FONT_42;// ZDT_PHILIPS_DIAL_SEARCH_STYLE
           #else
...
#endif
			{
				...
				GUIEDIT_SetLineSpace(MMK_GetCtrlHandleByWin(MMIIDLE_DIAL_WIN_ID, edit_ctrl_id),0);
#else	
				GUIEDIT_SetLineSpace(MMK_GetCtrlHandleByWin(MMIIDLE_DIAL_WIN_ID, edit_ctrl_id),2);
	
#endif
			}
			else
			{
			#ifdef ZDT_PHILIPS_DIAL_SEARCH_STYLE //xiongkai change 5 to 3    //5   拨号数字超过12个后，按下方向键向下翻动，顶部刷新不良。
				GUIEDIT_SetLineSpace(MMK_GetCtrlHandleByWin(MMIIDLE_DIAL_WIN_ID, edit_ctrl_id),0);  //3
			#else
				GUIEDIT_SetLineSpace(MMK_GetCtrlHandleByWin(MMIIDLE_DIAL_WIN_ID, edit_ctrl_id),3);  			
			#endif	
			}
	
			GUIEDIT_SetCharSpace(MMK_GetCtrlHandleByWin(win_id, edit_ctrl_id),0);   //8
			GUIEDIT_SetPhoneNumImg(MMK_GetCtrlHandleByWin(win_id, edit_ctrl_id), TRUE,MMITHEME_PHONE_IMG_WIDTH,MMITHEME_PHONE_IMG_HEIGHT);
#endif
        }
    }
...
)
{
	...
	item_num += s_num_search_info.pb_match_item_num;
	
	//搜索匹配号码(通话记录)
	s_num_search_info.cl_match_item_num = MMIAPICL_SearchList(str_ptr);
    item_num += s_num_search_info.cl_match_item_num;
	...
	{   
		...
		item_t.item_style = GUIITEM_STYLE_ONE_LINE_TEXT; // GUIITEM_STYLE_ONE_LINE_TEXT_MS
	...
	else
	{
		...
		GUILIST_SetListState(list_ctrl_id, GUILIST_STATE_EFFECT_STR, TRUE);
                ...
                item_t.item_style = GUIITEM_STYLE_TWO_LINE_ANIM_TWO_TEXT_AND_TEXT_3_SCROLL_NEW;
                #else
		item_t.item_style = GUIITEM_STYLE_TWO_LINE_ANIM_TWO_TEXT_AND_TEXT_3_SCROLL;
                #endif
#else
...
PUBLIC void MMIAPIDIAL_IDLE_ENTERMESSAGE(uint32  win_id )
{	
	MMI_CTRL_ID_T list_ctrl_id = MMK_GetCtrlHandleByWin(win_id,MMIIDLE_DIAL_LISTBOX_CTRL_ID);
 	GUILIST_SetCurItemIndex( list_ctrl_id,1  );
	GetCurrentNumber(win_id);
}
...
LOCAL BOOLEAN GetCurrentNumber(MMI_WIN_ID_T win_id)
{
	CTRLLIST_ITEM_T *get_item_ptr;
	uint16 index = 0;
	uint32 group = 0;
	uint32 User_data = 0;
	uint16 display_content_index = 3; 
	MMIPB_BCD_NUMBER_T bcd_num = {0};
	GUILIST_ITEM_DATA_T item_data = {0};
	MMI_STRING_T  edit_str = {0};
	MMI_STRING_T name_str = {0};
	...
	if (s_num_search_info.pb_match_item_num<1) 
	{
		...
		if (GUILIST_GetTotalItemNum(list_ctrl_id)==2)
		{
			...
			{
			case 0:			
				MMK_CreateWin((uint32*) MMIIDLE_DIAL_MENU_WIN_TAB_1, (ADD_DATA) win_id);
				break;
#if 0
				//+bug132483
				//号码有效才可以添加到contacts中
...
#endif
			case 1:		
				if( SCI_FALSE == MMIAPISMS_OpenConfirmation((void *)win_id,MMISMS_ENTER_BY_DIAL_IDLE))
				{
					//g_enter_sms_back_play_by_pb_id = win_id;
					return FALSE;
				}
				...
				{
					MMIAPISMS_WriteNewMessage(MN_DUAL_SYS_MAX, PNULL,phone_num_str, (uint8)phone_num_len);
					MMK_CloseWin(win_id);
				}
				break;
			}
	...
	index = GUILIST_GetCurItemIndex(list_ctrl_id);
	if (index>=s_num_search_info.pb_match_item_num)index=index-1;
	get_item_ptr = CTRLLIST_GetItem(list_ctrl_id, index);
	//GUILIST_GetItemData(list_ctrl_id, index, &User_data);
	
	//User_data=get_item_ptr->data_ptr->item_content[GUIITEM_CONTENT_MAX_NUM-1].custom_font_color;
	//if (/*User_data>=0 &&*/ User_data<GUIITEM_CONTENT_MAX_NUM)
	//{
		//display_content_index=User_data;
		num_str=get_item_ptr->data_ptr->item_content[display_content_index].item_data.text_buffer;
       ...
       // if(1 > edit_str.wstr_len)
        {
            ...
            if(index==1)
                item_data.item_content[display_content_index].item_data.text_id= STXT_NUMBER_SEND_SMS;
            else
                item_data.item_content[display_content_index].item_data.text_id= STR_LIST_SAVE_ADDRESS;
            ...
            {
                index++;
                CTRLLIST_RemoveItem(list_ctrl_id,index);
            }
            GUILIST_SetListState( list_ctrl_id, GUILIST_STATE_ACTIVE, FALSE);
            return;
        }
       // else
       // {
            //新UI要求不 显示"加入电话簿"by wangfeizhou modify
...
*//*
       ...
       */ {      
            ...
            uint32 group = 0;
			MMI_STRING_T name_tpye= {0};
			MMI_STRING_T abbr_name= {0};
			GUIANIM_FILE_INFO_T anim_path= {0}; 
			wchar abbr_wchar[MMIPB_MAX_STRING_LEN] = {0};
			wchar anim_wchar[MMIPB_MAX_STRING_LEN] = {0};
			wchar type_wchar[MMIPB_MAX_STRING_LEN] = {0};
			uint16 entry_id;
			uint16 storage_id;
			uint8 total = 0;
			uint8 t_num;
			...
			abbr_name.wstr_ptr = abbr_wchar;
			anim_path.full_path_wstr_ptr = anim_wchar;
			name_tpye.wstr_ptr=type_wchar;
			...
			MMIAPIPB_GetSearchEntryWithAvatar(s_num_search_info.handle,&name_str,MMIPB_MAX_STRING_LEN,&bcd_num,&name_tpye,&group,&abbr_name,&anim_path,&entry_id,&storage_id, &total, pb_index);
			MMI_GetLabelTextByLang(STXT_NUMBER_TYPE_MOBILE_EXT01, &type_str);
			t_num=((uint8)(total&0xf0)) >> 4;
			t_ind=(uint8)(total&0x0f);
			if (1==t_num||1==t_ind)
				{
                  type_id=1;
	            }
            ...
            //icon
            if (anim_path.full_path_wstr_len<=0)
                {  //缩写显示处理部分
				//item_t.right_item_style = GUIITEM_STYLE_TWO_LINE_INITIALTEXT_TEXT_AND_TEXT_WITH_RIGHT_FOCUS_MS;
                ...
                if(0 != name_str.wstr_len && PNULL != name_str.wstr_ptr)
				{
                    ...
                    item_data.item_content[1].item_data.text_buffer=name_str; //abbr_name;
				}
                else
				{
                    item_data.item_content[1].item_data_type = GUIITEM_DATA_TEXT_ID;
                    item_data.item_content[1].item_data.text_id = STR_CONTA_NO_NAME_EXT01;
				} 
               if (type_id==1) 
               	{   //单行带缩写显示
				    if( abbr_name.wstr_len>0)
                        {
                            ...
                            item_data.item_content[2].item_data.text_buffer =abbr_name;
                        }
	              //number
				    item_data.item_content[4].font_color_id = MMITHEME_COLOR_9;
				    item_data.item_content[4].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
	              //GUILIST_SetItemUserData(list_ctrl_id, index, &user_data);
				    item_data.item_content[GUIITEM_CONTENT_MAX_NUM-1].custom_font_color=4;
			 	    item_data.item_content[4].item_data.text_buffer= num_str;
               	}
			   else
			   	{
			   	    //双行带缩写显示
				    ...
				    if( abbr_name.wstr_len>0)
					    {
						   ...
						   item_data.item_content[3].item_data.text_buffer =abbr_name;
                        }
				   //number
				   item_data.item_content[2].font_color_id = MMITHEME_COLOR_9;
				   item_data.item_content[2].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
				   //GUILIST_SetItemUserData(list_ctrl_id, index, &user_data);
				   
				   temp_ptr = (char*)":";
				   temp_len = SCI_STRLEN((char*)temp_ptr);
				   MMIAPICOM_StrcatFromStrToUCS2(name_tpye.wstr_ptr, &name_tpye.wstr_len, (uint8*)temp_ptr, temp_len);
				   MMIAPICOM_StrcatFromSTRINGToUCS2(name_tpye.wstr_ptr, &name_tpye.wstr_len, &num_str);
				   item_data.item_content[2].item_data.text_buffer= name_tpye;
				   item_data.item_content[4].font_color_id = MMITHEME_COLOR_9;
				   item_data.item_content[4].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
				   //GUILIST_SetItemUserData(list_ctrl_id, index, &user_data);
				   item_data.item_content[GUIITEM_CONTENT_MAX_NUM-1].custom_font_color=4;
...
LOCAL void AppendMatchingInfo(MMI_WIN_ID_T win_id, void * param)
{
    ...
    if(0 == (s_num_search_info.cl_match_item_num + s_num_search_info.pb_match_item_num))
    {
        ...
        {
            //显示"加入电话簿"
            item_data.item_content[display_content_index].item_data.text_id=TXT_NO_MATCH_CONTRACT ;// TXT_ADD_TO_CONTRACT;
        }
    }
    else
    {
...
if(PNULL == item_ptr)
	...
	name_str.wstr_ptr = name_wchar;
	name_tpye.wstr_ptr=type_wchar;
	MMIAPIPB_GetSearchEntryWithAvatar(s_num_search_info.handle,&name_str,MMIPB_MAX_STRING_LEN,PNULL,&name_tpye,PNULL,PNULL,PNULL,&entry_id,&storage_id,&total,item_index);
                                    ...
                                    )
{
    ...
    LOCAL wchar temp_wstr[MMIIDLE_DIAL_MAX_LEN + 1] = {0};
    LOCAL wchar temp_wstr_len = 0;
	BOOLEAN bl_speeddial_enable = FALSE;
    MMI_WIN_ID_T info_ptr = (MMI_WIN_ID_T )MMK_GetWinAddDataPtr(win_id);
   ...
   case MSG_LCD_SWITCH:
    ...
    case MSG_KEYLONG_9:
        ...
        if ((0 == edit_str.wstr_len) ||
            (1 == edit_str.wstr_len))
        {
            if(NVERR_NONE == EFS_NvitemRead(TELE_NV_ENABLE_SPEED_DIAL, sizeof(uint32), &bl_speeddial_enable)
            {
#if defined(MMI_SPEEDDIAL_SUPPORT) 
                if(bl_speeddial_enable)MMIAPICC_SpeedDial(MMI_DUAL_SYS_MAX, (uint16)(msg_id - MSG_KEYLONG_1 + 1));
#endif
            }
        }
        break;
...
LOCAL BOOLEAN IsSIMADNInput(MMI_STRING_T *str_ptr)
{
    ...
    if ('#' == str_ptr->wstr_ptr[str_ptr->wstr_len - 1])
    {
        ...
        if (is_sim_adn)
        {
            ...
            {
                MMIPUB_OpenAlertWarningWin(STR_CONTACTS_NOT_READY_EXT01);
...
typedef enum
{
    CC_SIM_SEL_MENU_NODE_ROOT,
    CC_SIM_SEL_MENU_VODAFONE,
    CC_SIM_SEL_MENU_CHINAUNICOM
};
...
PUBLIC void CreateSimSelectionMenu(
                                MMI_WIN_ID_T win_id,
                                MMI_CTRL_ID_T ctrl_id)
{
    uint8 index = 0;
                                             ...
                                             )
{
        ...
        {
        case MSG_OPEN_WINDOW:
            s_cur_sim_sel = s_sim_sel_dual_sys;
...
//  Note: 
/*****************************************************************************/
MMI_RESULT_E HandleSIMSelectWinMsg(
    MMI_WIN_ID_T     win_id, 
    MMI_MESSAGE_ID_E    msg_id, 
    DPARAM              param
    )
{
    MMI_RESULT_E        recode = MMI_RESULT_TRUE;
    MMI_CTRL_ID_T menu_ctrl_id = MMIDIAL_SIM_SEL_MENU_CTRL_ID;
    uint8 node_id = 0;
...
PUBLIC BOOLEAN MMI_IsIdleDial(void)
{
