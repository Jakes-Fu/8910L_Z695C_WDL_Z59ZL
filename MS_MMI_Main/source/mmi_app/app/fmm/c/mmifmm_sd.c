...
**                         GLOBAL DEFINITION                                *
**--------------------------------------------------------------------------*/
LOCAL uint8 format_timer_id = 0;
uint32 format_percent = 0;
...
/*****************************************************************************/
//  Description : HandleFmmFormatQueryWin
//  Global resource dependence :
//  Author: boda.lin
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleFmmFormatQueryWin (MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param);
...
/*****************************************************************************/
//  Description : to handle the message of format progress window
//  Global resource dependence :
//  Author: wen.dong
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleFormatProcessWinMsg(
                                       MMI_WIN_ID_T     win_id,
                                       MMI_MESSAGE_ID_E msg_id,
                                       DPARAM           param
                                       );
...
PUBLIC void MMIFMM_FmmMemoryFormat (uint32 device, MMI_TEXT_ID_T query_text_id)
{
    ...
    /*MMIPUB_OpenQueryWinByTextIdEx (MMK_GetFirstAppletHandle(),
                                   ...
                                   HandleFmmQueryWin,
                                   (uint32) device);*/
    MMIPUB_OpenAlertWinByTextIdEx(MMK_GetFirstAppletHandle(), 
        PNULL,
        query_text_id,
        PNULL,
        IMAGE_NULL,
...
/*****************************************************************************/
// Description :  SD card or special format after open file explore
// Global resource dependence :
// Author:  boda.lin
// RETRUN:
// Note:
/*****************************************************************************/
PUBLIC void MMIFMM_FmmFormatOpenExplore (uint32 device, MMI_TEXT_ID_T query_text_id)
{
...
LOCAL MMI_RESULT_E HandleFmmQueryWin (MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
    ...
    case MSG_APP_OK:
		...
		MMIPUB_OpenProgressWinByTextId(STR_FORMAT_WAIT_EXT01, &alert_win_id, PNULL, MMIPUB_SOFTKEY_CUSTOMER, HandleFormatProcessWinMsg);
		MMIPUB_SetProgressTotalSize(&alert_win_id, 100);
		{
			MMI_STRING_T text_str_ptr = {0};
			text_str_ptr.wstr_ptr = L" ";
			text_str_ptr.wstr_len = 1;
//			MMIPUB_SetProgressContentByTextPtr(&alert_win_id,&text_str_ptr,TRUE);
		}
		format_timer_id = MMK_CreateWinTimer(win_id, 200, TRUE);
		//End
	    break;
    ...
    case MSG_UDISK_FORMAT_CNF:
...
#endif
        if(format_timer_id != 0)
        {
            MMK_StopTimer(format_timer_id);
            format_timer_id = 0;
        }
        format_percent = 0;
        //Bug 141190 
        ...
        break;
    case MSG_TIMER:
        if (format_timer_id == *((uint8 *) param) && format_percent < 100)
        {
            MMIPUB_UpdateProgressBarEx(&alert_win_id,format_percent++,TRUE);
        }
        break;
//End
    default:
...
LOCAL MMI_RESULT_E HandleFmmFormatQueryWin (MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
    ...
    case MSG_TIMER:
        ...
        {
            MMIPUB_UpdateProgressBarEx(&alert_win_id,format_percent++,TRUE);
	     MMK_SendMsg(alert_win_id, MSG_FULL_PAINT, PNULL);
        }
        break;

   //boda, open file explore
...
)
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        temp_len = MIN((uint16) MMIAPICOM_Wstrlen (get_name),MMIFILE_NEW_DEVICE_NAME_MAX_LEN);
...
PUBLIC BOOLEAN MMIAPIFMM_CheckMemoryCard (MMIFILE_DEVICE_E file_dev)
{
    ...
    else if (file_dev >= MMI_DEVICE_NUM)
    {
        ...
        //alert_text_id = TXT_NO_SD_CARD_ALERT;
                                       ...
                                       )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        MMK_PostMsg(MMIFMM_FORMAT_WIN_ID,MSG_FMM_FORMAT,PNULL,0);
