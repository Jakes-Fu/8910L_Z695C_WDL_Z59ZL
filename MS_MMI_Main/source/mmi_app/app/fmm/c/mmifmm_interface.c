...
#endif
extern BOOLEAN  is_finish_prompt;

BOOLEAN s_select_file_enter = FALSE;
...
LOCAL MMI_RESULT_E MMIFMM_HandleOpenFileWinEx (
    MMI_WIN_ID_T         win_id,
    MMI_MESSAGE_ID_E     msg_id,
    DPARAM               param
);
...
#else
    WIN_FUNC ( (uint32) MMIFMM_HandleOpenFileWinEx),
#endif
...
#ifndef MMI_FMM_LIST_SUPPORT    
    WIN_SOFTKEY (STXT_SOFTKEY_MENU_MK, STXT_SOFTKEY_OPEN_MK, TXT_COMMON_COMMON_BACK),
#else
...
//    WIN_PRIO( WIN_ONE_LEVEL ),
    ...
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_MIDDLE, MMIFMM_OPEN_FILE_BYLAYER_TITLE_LABEL_CTRL_ID),
#endif
...
)
{
    ...
    static BOOLEAN  open_file_win_created = FALSE;
    ...
    SCI_TRACE_LOW("[mmifmm_interface.c] [HandleOpenFileByLayerWaitWin] msg id= %x,", msg_id);
    switch (msg_id)
    {
    case MSG_OPEN_WINDOW:
        open_file_win_created = FALSE;
    ...
    case MSG_CLOSE_WINDOW:
        ...
        if (!MMK_IsOpenWin (MMIFMM_OPEN_FILE_WIN_ID) && lwait_win_data->s_need_list && !open_file_win_created)
        {
            if (PNULL!=lwait_win_data->w_handle)
            {
                if (NULL != lwait_win_data->w_handle->s_file_array)
...
LOCAL MMI_RESULT_E SubOpenFileWinCloseCtrlFunc (MMI_WIN_ID_T win_id)
{
    ...
    if (PNULL != win_data_p)
    {
        ...
        if (NULL != win_data_p->s_sel_file_name_ptr)
        {
            ...
            win_data_p->s_sel_file_name_ptr = NULL;
        }
...
)
{
    ...
    MMI_CTRL_ID_T   label_ctrl_id = MMIFMM_OPEN_FILE_BYLAYER_TITLE_LABEL_CTRL_ID;
#endif
...
#ifdef MMI_FMM_LIST_SUPPORT
        MMK_SendMsg (win_id, MSG_FMM_OPENFILEWIN_RELOAD, NULL);
#endif
        break;
#ifdef MMI_PDA_SUPPORT
    ...
    case MSG_CTL_TAB_SWITCH:
        {
            ...
            {
                //格式错误，提示是否格式化
                MMIFMM_FmmMemoryFormat ((cur_tab_index+1), STR_MMC_FORMAT);
                break;
            }
#endif
    ...
    case MSG_FULL_PAINT:
        {
            ...
            if(path_depth > 1)
            {
                ...
                GUILABEL_SetFont(label_ctrl_id, CAF_FONT_20, MMI_DARK_WHITE_COLOR);
    ...
    case MSG_CTL_LIST_NEED_ITEM_DATA:
        {
			...
			SCI_TRACE_LOW("[mmifmm_interface.c][HandleOpenFileByLayerWin] w_handle = 0x%x" , win_data_p->w_handle);
			SCI_TRACE_LOW("[mmifmm_interface.c][HandleOpenFileByLayerWin] s_file_array = 0x%x" , win_data_p->w_handle->s_file_array);
...
#ifdef MMI_FMM_TITLE_LABEL_SUPPORT
                if (win_data_p->w_handle->s_cur_path_len > 1)
                {
                    uint32 i = 0;
                    MMI_STRING_T  label_str = {0};
                    for (i = win_data_p->w_handle->s_cur_path_len - 1; MMIFILE_SLASH != win_data_p->w_handle->s_cur_path[i] && i > 0; i--)
                    {
                        NULL;
                    }
                    label_str.wstr_ptr = &win_data_p->w_handle->s_cur_path[i + 1];
                    label_str.wstr_len = win_data_p->w_handle->s_cur_path_len - i - 1;
                    GUILABEL_SetText(MMIFMM_OPEN_FILE_BYLAYER_TITLE_LABEL_CTRL_ID,&label_str,TRUE);
    ...
    case MSG_APP_WEB:
        ...
        if (FILEARRAY_TYPE_FOLDER == f_array_data.type)
        {
#ifdef MMI_FMM_TITLE_LABEL_SUPPORT
            {
                uint32 i = 0;
                MMI_STRING_T    label_str = {0};
                for (i = f_array_data.name_len - 1; MMIFILE_SLASH != f_array_data.filename[i] && i > 0; i--)
                {
                    NULL;
                }
                label_str.wstr_ptr = &f_array_data.filename[i + 1];
                label_str.wstr_len = f_array_data.name_len - i - 1;
                GUILABEL_SetText(MMIFMM_OPEN_FILE_BYLAYER_TITLE_LABEL_CTRL_ID,&label_str,TRUE);
                MMK_PostMsg (win_id, MSG_FULL_PAINT, PNULL, 0);
            }
#endif
...
//#ifndef MMI_FMM_LIST_SUPPORT   //// ZDT_PRODUCT_Z685S
        if (PNULL != win_data_p->w_handle->s_handle_func)
        {
            win_data_p->w_handle->s_handle_func (win_id, msg_id, param);
        }
...
)
{
    ...
    else
    {
        ...
        if (FILEARRAY_TYPE_FOLDER == f_array_data.type)
        {
            ...
            {
                item_data_ptr->softkey_id[0] = STXT_SOFTKEY_SELECT_MK;
                item_data_ptr->softkey_id[1] = TXT_COMM_OPEN;

            }

            {
                item_data_ptr->item_content[0].item_data.image_id = IMAGE_FMM_FOLDER;
#ifdef MMI_PDA_SUPPORT
...
PUBLIC MMIFMM_OPENWIN_RET_E MMIFMM_OpenFileWinByLayerEx (MMIFMM_OPEN_LAYER_WIN_INFO_T *open_layer_info)
{
    ...
    SCI_TRACE_LOW("[mmifmm_interface.c][MMIFMM_OpenFileWinByLayerEx] w_handle = 0x%x" , w_handle);
    ...
    w_handle->s_leftsoft_id = STXT_SOFTKEY_SELECT_MK;
#ifdef MMI_PDA_SUPPORT
...
#else
        w_handle->s_middlesoft_id = STXT_SOFTKEY_MARK_MK;
#endif
    }
    else
    {
...
LOCAL MMI_RESULT_E FileWinCloseCtrlFunc (MMI_WIN_ID_T win_id)
{
    ...
    if (PNULL != win_data_p)
    {
        if (PNULL != win_data_p->w_handle)
        {
            ...
            {
                SCI_TRACE_LOW("[mmifmm_interface.c][FileWinCloseCtrlFunc] w_handle = 0x%x" , win_data_p->w_handle);
                SCI_FREE (win_data_p->w_handle);
                win_data_p->w_handle = PNULL;
            }
            else if(s_select_file_enter == TRUE)
            {
                SCI_TRACE_LOW("[mmifmm_interface.c][FileWinCloseCtrlFunc] s_select_file_enter w_handle = 0x%x" , win_data_p->w_handle);
                SCI_FREE (win_data_p->w_handle);
                win_data_p->w_handle = PNULL;
                s_select_file_enter = FALSE;
            }
        }
...
#else
        MMK_CreateWin((uint32*) MMIFMM_OPENFILE_BYLAYER_WIN_TAB,(ADD_DATA) subwin_param);
#endif
...
)
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        if(PNULL != win_data_p&& PNULL!=win_data_p->w_handle)
        {
			...
			{
				MMIFMM_FmmMemoryFormat(cur_device, STR_MMC_FORMAT);
			}
#endif
...
)
{
    ...
    wchar                 text[64] = {0};
    wchar    cur_path[MMIFMM_DEFAULT_PATH_LEN] = {0};
    FILEARRAY_DATA_T  f_array_data = {0};
    ...
    MMI_IMAGE_ID_T    list_image[] =
   {
    ...
    SCI_TRACE_LOW("[mmifmm_interface.c] [MMIFMM_HandleOpenFileWinEx] win_id=0x%x, msg_id=0x%x,", win_id, msg_id);
    switch(msg_id)
    {
    case MSG_OPEN_WINDOW:
        ...
        ///*Set Move/Copy Header info*/
        if (MMK_IsOpenWin (MMIFMM_MOVE_FILE_WIN_ID))
        {
            GUIWIN_SeSoftkeytButtonTextId(win_id, STXT_SOFTKEY_MOVE_MK, LEFT_BUTTON, TRUE);
            MMI_GetLabelTextByLang(STR_FILES_HDR_MOVE_TO_EXT01, &title_str);
        ...
        if (MMK_IsOpenWin (MMIFMM_COPY_SELECT_FILE_WIN_ID))
        {
            GUIWIN_SeSoftkeytButtonTextId(win_id, TXT_COMMON_COPY, LEFT_BUTTON, TRUE);
            MMI_GetLabelTextByLang(STR_FILES_HDR_COPY_TO_EXT01, &title_str);
            if(MMIAPICOM_StatusAreaSetViewHeaderinfo(title_str .wstr_ptr, title_str .wstr_len))
            {
                GUIWIN_UpdateStb();
            }
        ...
        {
            GUIWIN_SeSoftkeytButtonTextId(win_id, STXT_SOFTKEY_OK_MK, LEFT_BUTTON, TRUE);
        }
        //End
#endif
        ...
        for (; fmm_dev > MMI_DEVICE_SYSTEM; fmm_dev--)
        {
            ...
            if (MMIAPIFMM_GetDeviceTypeStatus (fmm_dev))
            {
                ...
                win_data_p->s_layerwin_device_tab[cur_list_index] = fmm_dev;
                cur_list_index++;
            }
        }
        break;
    ...
    case MSG_LOSE_FOCUS:
        break;
    case MSG_CTL_OK:
        if (MMK_IsOpenWin (MMIFMM_MOVE_FILE_WIN_ID) || MMK_IsOpenWin (MMIFMM_COPY_SELECT_FILE_WIN_ID)
#ifdef BROWSER_SUPPORT
			...
			)
        {
            ...
            MMIAPICOM_Wstrcpy (cur_path, MMIAPIFMM_GetDevicePath (win_data_p->s_layerwin_device_tab[cur_list_index]));
            MMIAPICOM_Wstrcat (cur_path, L":");
            cur_path_len = (uint16) MMIAPICOM_Wstrlen (cur_path);
            MMIAPICOM_Wstrncpy (win_data_p->w_handle->s_cur_path, cur_path, cur_path_len);
            win_data_p->w_handle->s_cur_path_len = cur_path_len;

            MMI_WSTRNCPY (f_array_data.filename, FMM_SEARCH_FILENAME_MAXLEN,
                          win_data_p->w_handle->s_cur_path,
                          win_data_p->w_handle->s_cur_path_len,
                          win_data_p->w_handle->s_cur_path_len);
            f_array_data.name_len = win_data_p->w_handle->s_cur_path_len;

            //select current path (D:\ or E:\)
            if (NULL != win_data_p->w_handle->s_callback && NULL != win_data_p->w_handle->s_ret_array)
            {
                MMIAPIFILEARRAY_Add (win_data_p->w_handle->s_ret_array, &f_array_data);
                win_data_p->w_handle->s_is_ret_succ = TRUE;
    ...
    case MSG_CTL_MIDSK:
        ...
        InitSearchParam (win_data_p->w_handle,cur_path,cur_path_len,&win_data_p->w_handle->s_filter);
        EnterOpenFileByLayerWin (win_data_p->w_handle, cur_list_index);
        OpenSearchFileWaitWin (win_data_p->w_handle);
        break;
    default:
        result = MMI_RESULT_FALSE;
