...
#define   P_APP_FILE_STACK_SIZE              6144    //6K
#define   P_APP_FILE_QUEUE_NUM               8
#define   P_APP_FILE_TASK_PRIORITY         PRI_APP_FILE_TASK //  (30)
...
LOCAL volatile BOOLEAN s_is_vaild_delete_handle = FALSE; //记录APP_DELETE_HANDLE_T 是否有效
...
LOCAL SCI_SEMAPHORE_PTR              s_del_semphore_p = PNULL;
/**--------------------------------------------------------------------------*
 **                         EXTERNAL DECLARE                                 *
 **--------------------------------------------------------------------------*/
...
LOCAL void MMI_SendSignal_2APPCOPY_CheckStoped (
    uint16              sig_id,
...
PUBLIC void MMIFMM_SetDeleteHandleState(
                                  BOOLEAN is_enable//IN:TRUE:enable file copy; FALSE:disable file copy.
                                  )
{
    SCI_GetSemaphore(s_del_semphore_p, SCI_WAIT_FOREVER);
    s_is_vaild_delete_handle = is_enable;
    SCI_TRACE_LOW("[MMIFMM]MMIFMM_SetDeleteHandleState  is_enable %d", is_enable);
    SCI_PutSemaphore(s_del_semphore_p);
}


PUBLIC BOOLEAN MMIFMM_GetDeleteHandleState(void)
{
    BOOLEAN is_enable = FALSE;
    SCI_GetSemaphore(s_del_semphore_p, SCI_WAIT_FOREVER);
    is_enable = s_is_vaild_delete_handle;
    SCI_PutSemaphore(s_del_semphore_p);
    return is_enable;
}
...
void APPFILE_CreateTask (void)
{
    ...
    if (s_del_semphore_p == PNULL)
    {
        s_del_semphore_p = SCI_CreateSemaphore("s_del_semphore_p", 1);
    }
...
PUBLIC void MMIFILE_ImpoveFileTaskPriority (void)
{
    ...
    if ((0 == g_app_file_task_id) || SCI_SUCCESS != SCI_GetThreadInfo (
                P_APP_COPY,
                PNULL,
                PNULL,
                &state,
    ...
    {
        uint32 old_priority_ptr;
       if(SCI_ERROR ==  SCI_ChangeThreadPriority (P_APP_COPY, priorityCur, &old_priority_ptr))
       {
       	SCI_TRACE_LOW("MMIFILE_ImpoveFileTaskPriority:SCI_ChangeThreadPriorityfail  ");
...
PUBLIC void APPFILE_HandleSearchFileCnf (MMIFMM_SRV_HANDLE handle, BOOLEAN search_result)
{
    ...
    SCI_TRACE_LOW("APPFILE_HandleSearchFileCnf search_handle = 0x%x, SearchFileTime: curr time: %d\n",handle, SCI_GetTickCount());
    if (search_result && !s_is_stopfindfile)
    {
        MMIAPIFMM_SearchFileCallBack(handle);
...
)
{
    ...
    while (1) /*lint !e716*/
    {
        MMI_RECEIVE_SIGNAL ( (xSignalHeader*) psig, P_APP_COPY);/*lint !e63*/
        //psig = s_cur_sig;
        if(NULL != psig)
        {
...
APP_DELETE_HANDLE_T *MMIFILE_DeleteOpen (wchar *del_file_name, uint16 del_name_len)
{
    ...
    MMIFMM_SetDeleteHandleState(TRUE);
...
PUBLIC MMIFILE_ERROR_E MMIFILE_DeleteClose (APP_DELETE_HANDLE_T *del_handle)
{
    ...
    {
        MMIFMM_SetDeleteHandleState(FALSE);
...
)
{
    ...
    case APP_DEL_ONE_FILE_IND:
        ...
        if (PNULL == app_handle_ptr || MMIFMM_GetDeleteHandleState() == FALSE)
        {
            //parameter error
            sfs_ret = SFS_ERROR_INVALID_PARAM;
        }
        ...
        MMI_SendSignal_2APP(APP_DEL_ONE_FILE, sfs_ret);
        break;
    case APP_DEL_ONE_FOLDER_IND:
        //delete next folder
        app_handle_ptr = (APP_DELETE_HANDLE_T *) psig->data_ptr;/*lint !e613*/
        
        if (PNULL != app_handle_ptr && MMIFMM_GetDeleteHandleState() == TRUE)
        {
			//coverity 10346
    ...
    case APP_DEL_END_IND:
        ...
        if (PNULL != app_handle_ptr && MMIFMM_GetDeleteHandleState() == TRUE) /*lint !e613*/
        {
...
//  uint16               find_name_len = 0;
    ...
    case APP_COPY_FIND_FILE:
        ...
        if (0 == copy_handle_ptr->find_handle)
        {
            ...
            if (SFS_ERROR_NONE == file_ret)
            {
...

                {
					...
					if(drm_file_type != MMIFMM_DCF_FILE)
					{
						...
						MMI_SendSignal_2APPCOPY_CheckStoped(APP_COPY_FIND_FILE, copy_handle_ptr);
						break;     
					}
                }
                ...
                MMI_SendSignal_2APPCOPY_CheckStoped (APP_COPY_FILE, copy_handle_ptr);
            ...
            else if (SFS_ERROR_INVALID_PARAM == file_ret || SFS_ERROR_NOT_EXIST == file_ret)
            {
                ...
                MMI_SendSignal_2APPCOPY_CheckStoped (APP_COPY_FIND_FOLDER, copy_handle_ptr);
            }
            else
            {
    ...
    case APP_COPY_FILE:
        ...
        MMI_SendSignal_2APP(APP_COPY_ONE_FILE, SFS_NO_ERROR);
        break;
...
void MMI_SendSignal_2APPDel (uint16      sig_id, APP_DELETE_HANDLE_T *handle_ptr)
{
    ...
    SCI_GetSemaphore(s_del_semphore_p, SCI_WAIT_FOREVER);
    if (s_is_vaild_delete_handle == TRUE)
    {
    ...
    {
        SCI_TRACE_LOW("[MMIFILE Del Error %s:%d]s_is_vaild_delete_handle is FALSE\n",__FUNCTION__,__LINE__);
    }
    SCI_PutSemaphore(s_del_semphore_p);
}
...
)
{
    ...
    SCI_GetSemaphore(s_copy_semphore_p, SCI_WAIT_FOREVER);
    if (s_is_stopcopy)//TRUE:enable file copy; FALSE:disable file copy.
    {
        MMI_CREATE_SIGNAL (sig_ptr,
                       sig_id,
                       sizeof (APP_DELETE_SIG_T),
    ...
    {
        SCI_TRACE_LOW("[%s:%d]user has been stoped copy, no need go on\n",__FUNCTION__,__LINE__);
    }
    SCI_PutSemaphore(s_copy_semphore_p);
}
    ...
    else
    {
        ...
        if (MMIAPICOM_Wstrcmp (handle_ptr->dest_full_path, handle_ptr->src_full_path))
        {
            ...
            if (SFS_INVALID_HANDLE == file_handle)
            {
                ...
                //需要考虑执行APP_COPY_INIT过程中可能被MMI打断并发送了STOP
                //MMI_SendSignal_2APPCOPY (APP_COPY_FILE, handle_ptr);
                MMI_SendSignal_2APPCOPY_CheckStoped(APP_COPY_FILE, handle_ptr);
            }
            else
            {
...
LOCAL SFS_ERROR_E FileCopy (APP_COPY_HANDLE_T *handle_ptr)
{
    ...
    if ((handle_ptr->current_full_path_len > MMIFMM_FILENAME_LEN) ||(handle_ptr->dest_full_path_len > MMIFMM_FILENAME_LEN))
    {
        SCI_TRACE_LOW ("[MMIFILE] FileCopy error, File Path > 255, curfile len is %d, dirfile len is %d",handle_ptr->current_full_path_len,handle_ptr->dest_full_path_len);
        return SFS_ERROR_INVALID_PARAM;
    }
