                                ...
                                )
{
    ...
    SCI_TRACE_LOW("MMIVIRTUALARRAY_Create, v_header_ptr = %x, thread_id=%x", v_header_ptr, SCI_IdentifyThread());
    ...
    if (NULL == v_header_ptr->buffer_ptr)
    {
        ...
        SCI_TraceLow("MMIVIRTUALARRAY_Create  malloc failed!!!!");
        return 0;
    }
...
//	SCI_MUTEX_PTR mutex = SCI_NULL;
    ...
    SCI_TRACE_LOW("MMIVIRTUALARRAY_Destroy, *virtual_array_ptr = %x ,thread_id:%x",*virtual_array_ptr, SCI_IdentifyThread());
    //判断virtual_array_ptr是否是有效的VIRTUALARRAY_HEADER_T类型指针
    v_header_ptr = (VIRTUALARRAY_HEADER_T *)(*virtual_array_ptr);
    if(mutex != PNULL || mutex != SCI_NULL)
    {
       if( SCI_SUCCESS !=SCI_GetMutex(mutex, SCI_WAIT_FOREVER))
       {
           SCI_TRACE_LOW("MMIVIRTUALARRAY MMIVIRTUALARRAY_Destroy SCI_GetMutex failed !");
           return;
       }
    }
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
        SCI_TRACE_LOW("MMIVIRTUALARRAY MMIVIRTUALARRAY_Destroy  v_header_ptr->identifier failed !");
        SCI_PutMutex(mutex);
        return;
    }
 ...
 //   }
    ...
    //清空virtual_array_ptr
    //释放头信息
    v_header_ptr->identifier =0xBBBB;
    SCI_FREE(v_header_ptr);
									...
									)
{
    ...
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
        SCI_TRACE_LOW("MMIVIRTUALARRAY_GetArrayCurSize   identifier error !");
        return 0;
    }
	...
	if(SFS_INVALID_HANDLE != file_handle)
	{
        ...
        else if(v_header_ptr->data_size != 0)
        {
                        ...
                        )
{
    ...
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
        SCI_TRACE_LOW("MMIVIRTUALARRAY_GetArraySize   identifier error !");
        return 0;
    }
                        ...
                        )
{
    ...
    if(mutex != PNULL || mutex != SCI_NULL)
    {
        if(SCI_SUCCESS !=  SCI_GetMutex(mutex, SCI_WAIT_FOREVER))
        {
            SCI_TRACE_LOW("MMIVIRTUALARRAY MMIVIRTUALARRAY_Add SCI_GetMutex failed !");
            return FALSE;
        }
    }
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
        ...
        SCI_TRACE_LOW("MMIVIRTUALARRAY_Add   identifier error !");
        return FALSE;
    }
    ...
    SCI_TRACE_LOW("max_num=%d..num_inbuf=%d..first_id=%d..num_del=%d..total_size=%d..is_update=%d ",v_header_ptr->max_num_in_buffer,v_header_ptr->num_inbuf,v_header_ptr->first_id_inbuf,v_header_ptr->num_del_files,v_header_ptr->total_size,v_header_ptr->need_update);
...
//			result = TRUE;
// 		}
        SCI_TRACE_LOW(" MMIVIRTUALARRAY_Add...need_update=%d ",v_header_ptr->need_update );
                        ...
                        )
{
    ...
    if(mutex != PNULL || mutex != SCI_NULL)
    {
        if(SCI_SUCCESS !=  SCI_GetMutex(mutex, SCI_WAIT_FOREVER))
        {
             SCI_TRACE_LOW("MMIVIRTUALARRAY MMIVIRTUALARRAY_Read SCI_GetMutex failed !");
             return FALSE;
        }
    }
    ...
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
        ...
        SCI_TRACE_LOW("MMIVIRTUALARRAY_Read   identifier error !");
        return FALSE;
    }
                        ...
                        )
{
    ...
    if(mutex != PNULL || mutex != SCI_NULL)
    {
        if(SCI_SUCCESS !=  SCI_GetMutex(mutex, SCI_WAIT_FOREVER))
        {
            SCI_TRACE_LOW("MMIVIRTUALARRAY MMIVIRTUALARRAY_Modify SCI_GetMutex failed !");
            return FALSE;
        }
    }
    ...
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
        ...
        SCI_TRACE_LOW("MMIVIRTUALARRAY_Modify   identifier error !");
        return FALSE;
    }
                        ...
                        )
{
    ...
    if(mutex != PNULL || mutex != SCI_NULL)
    {
        if(SCI_SUCCESS !=  SCI_GetMutex(mutex, SCI_WAIT_FOREVER))
        {
            SCI_TRACE_LOW("MMIVIRTUALARRAY MMIVIRTUALARRAY_Delete SCI_GetMutex failed !");
            return FALSE;
        }
    }
    ...
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
        ...
        SCI_TRACE_LOW("MMIVIRTUALARRAY_Delete   identifier error !");
        return FALSE;
    }
...
#define MMI_FMM_SORT_EXTEND

#ifdef MMI_FMM_SORT_EXTEND
extern FILEARRAY_SORT_E g_sort_type;
typedef struct
{
    union
    {
...
//  Note:		20100902
/*****************************************************************************/
LOCAL int CompareTimeAsec(
                           const void       *data_1_ptr,
                           const void       *data_2_ptr
                           )
{
    SORT_ITEM    *file_data_1 = NULL;
    SORT_ITEM    *file_data_2 = NULL;
    ...
    file_data_1 = (SORT_ITEM*)data_1_ptr;
    file_data_2 = (SORT_ITEM*)data_2_ptr;


    if(file_data_1->key.dt.date.year != file_data_2->key.dt.date.year)
    {
        return file_data_1->key.dt.date.year - file_data_2->key.dt.date.year ;
    }
    else if( file_data_1->key.dt.date.mon !=  file_data_2->key.dt.date.mon)
    {
    return file_data_1->key.dt.date.mon -  file_data_2->key.dt.date.mon ;
    }
    else if( file_data_1->key.dt.date.mday !=  file_data_2->key.dt.date.mday)
    {
        return file_data_1->key.dt.date.mday -  file_data_2->key.dt.date.mday ;
    }
    else if( file_data_1->key.dt.time.hour !=  file_data_2->key.dt.time.hour)
    {
        return file_data_1->key.dt.time.hour -  file_data_2->key.dt.time.hour ;
    }
    else if( file_data_1->key.dt.time.min !=  file_data_2->key.dt.time.min)
    {
        return file_data_1->key.dt.time.min -  file_data_2->key.dt.time.min ;
    }
    else if( file_data_1->key.dt.time.sec !=  file_data_2->key.dt.time.sec)
    {
        return file_data_1->key.dt.time.sec -  file_data_2->key.dt.time.sec ;
    }
    else
    {
        return -1;
...
{
	return (0 - CompareTimeAsec(data_1_ptr, data_2_ptr));
}
                           ...
                           )
{
    ...
    file_data_2 = (SORT_ITEM*)data_2_ptr;
   	...
   	return file_data_1->key.size - file_data_2->key.size ;
}
...
LOCAL uint32 GetContinueCount(SORT_ITEM * item_buf, uint32 start_index, uint32 max_count)
{
    uint32 count = 1;

    while (count < max_count-1)
    {
        if ((item_buf[start_index].origin_index + 1)== item_buf[start_index+1].origin_index)
        {            
            count++;
            start_index++; 
        }
...
LOCAL BOOLEAN  SortFileExtend(VIRTUALARRAY virtual_array, FILEARRAY_SORT_E sort_type, 
	uint16 *file_name, uint16 *file_name_len)
{
    uint32 count = 0;
    uint32 index = 0;
    int32 offset = 0;
    ...
    uint32 read_index = 0;
    uint32 item_count = 0;
    int read_item_count = 0;
    SORT_ITEM *pItem = NULL;
    BOOLEAN result = TRUE;
    SFS_HANDLE sfs_handle = NULL;
    uint32  to_read_size = 0;    
    uint32  to_write_size = 0;
    SFS_HANDLE sfs_handle_result = NULL;
    FILEARRAY_DATA_T  *file_data = NULL;
    MMIVIRTUALARRAY_COMPARE compare_func = NULL;
    VIRTUALARRAY_HEADER_T   *v_header_ptr = NULL;
    uint32 temp = 0;

    v_header_ptr = (VIRTUALARRAY_HEADER_T *)(virtual_array);
    pItem = SCI_ALLOCA(sizeof(SORT_ITEM) * v_header_ptr->total_size);
    if (pItem == NULL)
    {
        SCI_TRACE_LOW("Error %s %d\n", __FUNCTION__, __LINE__);
        return FALSE;
    }
    SCI_MEMSET(pItem, 0, sizeof(SORT_ITEM) * v_header_ptr->total_size);
    ...
    if (SFS_SetFilePointer(sfs_handle, 0, SFS_SEEK_BEGIN) != SFS_ERROR_NONE)
    {
        SCI_TRACE_LOW("Error %s %d\n", __FUNCTION__, __LINE__);
        SCI_FREE(pItem);
    ...
    read_index = 0;
    if (v_header_ptr->total_size - read_index > v_header_ptr->max_num_in_buffer)
    {
        to_read_size = v_header_ptr->max_num_in_buffer * v_header_ptr->data_size;
    }
    else
    {
        to_read_size = (v_header_ptr->total_size - read_index) * v_header_ptr->data_size;
    }
    if (SFS_ReadFile(sfs_handle, v_header_ptr->buffer_ptr, to_read_size, &read_size, NULL) != SFS_ERROR_NONE)
    {
        SCI_TRACE_LOW("Error %s %d\n", __FUNCTION__, __LINE__);
        SCI_FREE(pItem);
        SFS_CloseFile(sfs_handle);
	...
	while ((read_size > 0) && (item_count < v_header_ptr->total_size))
    {
        read_item_count = read_size / sizeof(FILEARRAY_DATA_T);
        file_data = (FILEARRAY_DATA_T *)v_header_ptr->buffer_ptr;

        index = 0;
        while (index < read_item_count)
        {
            switch(sort_type)
            {
                case FILEARRAY_SORT_TIME_ASCEND:
                    compare_func = CompareTimeAsec;
                    pItem[item_count].key.dt.date = file_data[index].modify_date;
                    pItem[item_count].key.dt.time = file_data[index].modify_time;
                    break;
                ...
                case FILEARRAY_SORT_TIME_CREATE_ASCEND:
                    compare_func = CompareTimeAsec;
                    pItem[item_count].key.dt.date= file_data[index].create_date;
                    pItem[item_count].key.dt.time = file_data[index].create_time;
                    break;
                ...
                case FILEARRAY_SORT_SIZE_ASCEND:
                    ...
                    pItem[item_count].key.size= file_data[index].size;
                    break;
            ...
            pItem[item_count].origin_index = item_count;
            item_count++;
            index++;
        }

        read_index += v_header_ptr->max_num_in_buffer;
        if (v_header_ptr->total_size - read_index > v_header_ptr->max_num_in_buffer)
        {
            to_read_size = v_header_ptr->max_num_in_buffer * v_header_ptr->data_size;
        }
    ...
    SCI_TRACE_LOW("[%s %d]Step 1 elapse: %d\n", __FUNCTION__, __LINE__, SCI_GetTickCount()-temp);

    SCI_TRACE_LOW("[%s %d] item_count=%d, total_size=%d\n", __FUNCTION__, __LINE__, item_count, v_header_ptr->total_size);

    temp = SCI_GetTickCount();
    SCI_TRACE_LOW("[%s %d]\n", __FUNCTION__, __LINE__, SCI_GetTickCount());

    qsort(pItem, item_count, sizeof(SORT_ITEM), compare_func);

    SCI_TRACE_LOW("[%s %d]Step 2 elapse: %d\n", __FUNCTION__, __LINE__, SCI_GetTickCount()-temp);

    //create random file
    if (SFS_ERROR_NONE != CreateRandFile(file_name, file_name_len))
    {
        SCI_TRACE_LOW("Error %s %d\n", __FUNCTION__, __LINE__);
        SCI_FREE(pItem);
        SFS_CloseFile(sfs_handle);
    ...
    sfs_handle_result = SFS_CreateFile(file_name, SFS_MODE_WRITE|SFS_MODE_OPEN_EXISTING, NULL, NULL);
    if (NULL == sfs_handle_result)
    {
        SCI_TRACE_LOW("Error %s %d\n", __FUNCTION__, __LINE__);
        SCI_FREE(pItem);
        SFS_CloseFile(sfs_handle);
    ...
    {
        offset = pItem[index].origin_index * v_header_ptr->data_size;
        if (SFS_SetFilePointer(sfs_handle, offset, SFS_SEEK_BEGIN) != SFS_ERROR_NONE)
        {
            result = FALSE;
            SCI_TRACE_LOW("Error %s %d\n", __FUNCTION__, __LINE__);
            break;
        ...
        if (item_count - index > v_header_ptr->max_num_in_buffer)
        {
            count = GetContinueCount(pItem, index, v_header_ptr->max_num_in_buffer);
        }
        else
        {
            count = GetContinueCount(pItem, index, item_count - index);
        }

        read_size = 0;
        write_size = 0;
        if (read_index + count > v_header_ptr->max_num_in_buffer)
        {
            count = v_header_ptr->max_num_in_buffer - read_index;
        }
        to_read_size = count * v_header_ptr->data_size;

        offset = v_header_ptr->data_size * read_index;
        if (SFS_ERROR_NONE == SFS_ReadFile(sfs_handle, (void*)(pTemp + offset), to_read_size, &read_size, NULL))
        {
            to_write_size += read_size;
        }
        read_index += count;

        if (read_index == v_header_ptr->max_num_in_buffer)
        {    
            //uint write_time = SCI_GetTickCount();

            SFS_WriteFile(sfs_handle_result, v_header_ptr->buffer_ptr, to_write_size, &write_size, NULL);
            //SCI_TRACE_LOW("[%s %d]Write elapse: %d\n", __FUNCTION__, __LINE__, SCI_GetTickCount()-write_time);

            read_index = 0;
            to_write_size = 0;
            if (write_size == 0)
            {
                SCI_TRACE_LOW("Error %s %d  write_size=%d\n", __FUNCTION__, __LINE__,  write_size);
            }
        }

        if (read_size == 0)
        {
            SCI_TRACE_LOW("Error %s %d read_size=%d\n", __FUNCTION__, __LINE__, read_size);
        }
    ...
    SCI_TRACE_LOW("[%s %d]index=%d, Step 3 elapse: %d\n", __FUNCTION__, __LINE__, index, SCI_GetTickCount()-temp);

    SFS_CloseFile(sfs_handle);
    SFS_CloseFile(sfs_handle_result);
    SCI_FREE(pItem);
...
//    uint16      sort_file[VIRTUALARRAY_FILENAME_MAX_LEN+1] = {0};
    ...
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
        ...
        SCI_TRACE_LOW("MMIVIRTUALARRAY_Sort   identifier error !");
        return FALSE;
    }
    ...
    if(mutex != PNULL || mutex != SCI_NULL)
    {
        if(SCI_SUCCESS !=  SCI_GetMutex(mutex, SCI_WAIT_FOREVER))
        {
            SCI_TRACE_LOW("MMIVIRTUALARRAY MMIVIRTUALARRAY_Sort SCI_GetMutex failed !");
            return FALSE;
        }
    }
...
#ifdef MMI_FMM_SORT_EXTEND
    if ((FILEARRAY_SORT_TIME_ASCEND == g_sort_type)
        || (FILEARRAY_SORT_TIME_DESCEND == g_sort_type)
        || (FILEARRAY_SORT_TIME_CREATE_ASCEND == g_sort_type)
        || (FILEARRAY_SORT_TIME_CREATE_DESCEND == g_sort_type)
        || (FILEARRAY_SORT_SIZE_ASCEND == g_sort_type)
        || (FILEARRAY_SORT_SIZE_DESCEND == g_sort_type))
    {
        sort_result = SortFileExtend(v_header_ptr, g_sort_type, sort_file_ptr, &sort_file_len);
    }
    else
#endif
    {
                            ...
                            )
{
    ...
    SCI_TRACE_LOW("ReadFileToBuffer...total_size=%d..data_size=%d..first_id=%d.. max_num=%d...first_is=%d..num=%d",v_header_ptr->total_size,v_header_ptr->data_size,first_id,v_header_ptr->max_num_in_buffer,v_header_ptr->first_id_inbuf,v_header_ptr->num_inbuf);
    sfs_handle = SFS_CreateFile(v_header_ptr->asso_file_name, SFS_MODE_READ|SFS_MODE_OPEN_EXISTING, NULL, NULL);/*lint !e655*/
    if(NULL != sfs_handle)
    {
    ...
    if(result)
    {
     ...
     SCI_TRACE_LOW("ReadFileToBuffer..",v_header_ptr->num_inbuf,v_header_ptr->first_id_inbuf);
    }
                            ...
                            )
{
    ...
    SCI_TRACE_LOW("ReadDataFromBuffer MEM CHECK 1...first_id_inbuf=%d..data_size=%d",v_header_ptr->first_id_inbuf,v_header_ptr->data_size);
    MMK_CheckAllocatedMemInfo( _D_FILE_NAME, _D_FILE_LINE );
...
LOCAL  SWDG_OBJECT *s_file_watchdog_ptr = PNULL;
PUBLIC void MMIAPIFMM_FmmSetWatchDogPtr(void *SwdgPtr)
{
    s_file_watchdog_ptr = (SWDG_OBJECT *)SwdgPtr;
}
#endif
/*****************************************************************************/
                                   ...
                                   )
{
    ...
    {
/*bug_686361 若文件很多，文件结构复杂时，此函数耗时很多，引起了task timeout，所以需要在此做喂狗动作*/
#ifndef WIN32
        SWDG_OBJECT     *watchdog_ptr = s_file_watchdog_ptr;  //监测mmi task出现死循环
        /* Fresh task tick */	
                                     ...
                                     )
{
    ...
    if(mutex != PNULL || mutex != SCI_NULL)
    {
        if(SCI_SUCCESS !=  SCI_GetMutex(mutex, SCI_WAIT_FOREVER))
        {
            SCI_TRACE_LOW("MMIVIRTUALARRAY MMIVIRTUALARRAY_LoadArrayFromFile SCI_GetMutex failed !");
            return FALSE;
        }
    }
    ...
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
        ...
        SCI_TRACE_LOW("MMIVIRTUALARRAY_LoadArrayFromFile   identifier error !");
        return FALSE;
    }
                                     ...
                                     )
{
    ...
    if(mutex != PNULL || mutex != SCI_NULL)
    {
        if(SCI_SUCCESS !=  SCI_GetMutex(mutex, SCI_WAIT_FOREVER))
        {
           SCI_TRACE_LOW("MMIVIRTUALARRAY MMIVIRTUALARRAY_SaveArrayToFile SCI_GetMutex failed !");
           return FALSE;
        }
    }	
    ...
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
        ...
        SCI_TRACE_LOW("MMIVIRTUALARRAY_SaveArrayToFile   identifier error !");
        return FALSE;
    }
                                     ...
                                     )
{
    ...
    if(mutex != PNULL || mutex != SCI_NULL)
    {
        if(SCI_SUCCESS !=  SCI_GetMutex(mutex, SCI_WAIT_FOREVER))
        {
            SCI_TRACE_LOW("MMIVIRTUALARRAY MMIVIRTUALARRAY_SaveArrayToFileEx SCI_GetMutex failed !");
            return FALSE;
        }
    }
    ...
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
        ...
        SCI_TRACE_LOW("MMIVIRTUALARRAY_SaveArrayToFileEx   identifier error !");
        return FALSE;
    }
                                     ...
                                     )
{
    ...
    if(mutex != PNULL || mutex != SCI_NULL)
    {
        if(SCI_SUCCESS !=  SCI_GetMutex(mutex, SCI_WAIT_FOREVER))
        {
            SCI_TRACE_LOW("MMIVIRTUALARRAY MMIVIRTUALARRAY_SetFolderStartFileAndEndFile SCI_GetMutex failed !");
            return FALSE;
        }
    }
    ...
    if(VIRTUALARRAY_IDENTIFIER != file_header_ptr->identifier)
    {
        ...
        SCI_TRACE_LOW("MMIVIRTUALARRAY_SetFolderStartFileAndEndFile   identifier error !");
        return FALSE;
    }
                                     ...
                                     )
{
    ...
    if(base_virtual_array != PNULL && folder_virtual_array != PNULL)
    {
        ...
        {    
             SCI_PutMutex(mutex);
             SCI_TRACE_LOW("MMIVIRTUALARRAY_AddFolderArray   identifier error !");
             return ;
         }
                                     ...
                                     )
{
    ...
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
        SCI_TRACE_LOW("SetBufferSize   identifier error !");
        return FALSE;
    }
    ...
    if(v_header_ptr->data_size != 0)v_header_ptr->max_num_in_buffer = size_in_KB * 1024 / v_header_ptr->data_size;    
...
//#if 0
    ...
    if(mutex != PNULL || mutex != SCI_NULL)
    {
        SCI_TRACE_LOW("MMIVIRTUALARRAY MMIVIRTUALARRAY_SetBufferSize SCI_GetMutex failed !");
        ret = SCI_GetMutex(mutex, SCI_NO_WAIT);
    }
    
    if(v_header_ptr != PNULL && VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
       if(mutex != PNULL || mutex != SCI_NULL)
       {
          SCI_PutMutex(mutex);
                                                     ...
                                                     )
{
    ...
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
       SCI_TRACE_LOW("MMIVIRTUALARRAY_GetAllDataSize   identifier error !");
        return 0;
    }
...
PUBLIC BOOLEAN MMIVIRTUALARRAY_UpdateBufferToFile(
                                                     VIRTUALARRAY virtual_array  //in
                                                     )
{
    VIRTUALARRAY_HEADER_T   *v_header_ptr = NULL;
    ...
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
        ...
        SCI_TRACE_LOW("MMIVIRTUALARRAY_UpdateBufferToFile   identifier error !");
        return FALSE;
    }
    
    ret = UpdateBufferToFile(v_header_ptr);
    if (ret)
    {
        SCI_FREE(v_header_ptr->buffer_ptr);
        v_header_ptr->buffer_ptr = NULL;
    }
...
PUBLIC BOOLEAN MMIVIRTUALARRAY_ReadFileToBuffer(
                                                     VIRTUALARRAY virtual_array  //in
                                                     )
{
    VIRTUALARRAY_HEADER_T   *v_header_ptr = NULL;
    ...
    if(VIRTUALARRAY_IDENTIFIER != v_header_ptr->identifier)
    {
        ...
        SCI_TRACE_LOW("MMIVIRTUALARRAY_ReadFileToBuffer   identifier error !");
        return FALSE;
    }
    
    if (v_header_ptr->buffer_ptr == NULL)
    {
        v_header_ptr->buffer_ptr = (void *)SCI_MallocApp(v_header_ptr->max_num_in_buffer*v_header_ptr->data_size,_D_FILE_NAME,_D_FILE_LINE);
    }
    
    if (v_header_ptr->buffer_ptr)
    {
        ret = ReadFileToBuffer(v_header_ptr, 0);
    }
