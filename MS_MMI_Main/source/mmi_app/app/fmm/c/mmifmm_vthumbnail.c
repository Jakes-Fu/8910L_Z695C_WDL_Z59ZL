...
#include "mmi_image.h"
#include "mmipicview_id.h"//zhuyongjing add for gallery video thumbnail
...
#define THUMBNAIL_VIDEOWIDTH            240//120
#define THUMBNAIL_VIDEOHEIGHT           240//90
...
LOCAL  BOOLEAN  FmmVideoThumbnailCallback (DPLAYER_IMAGE_DATA_T *image_data, uint32 total_time);
/*****************************************************************************/
...
/*****************************************************************************/
//  Description :VIDEO文件夹支持缩略图功能中 通过视频全路径获取对应txt 全路径
//  Global resource dependence :
//  Author: zhuyongjing
//  Note:
/*****************************************************************************/
PUBLIC  BOOLEAN  MMIFMM_FmmVideoTotaltimeGetPathname (wchar* totaltime, uint16 *length, wchar* videopath)
{
    BOOLEAN result = TRUE;
    int32 filename_len = 0;
    ...
    if (totaltime == PNULL || videopath == PNULL || 0 == MMIAPICOM_Wstrlen (videopath))
    {
    ...
    if (MMIAPICOM_Wstrlen (videopath) + THUMBNAIL_FOLDER_NAME_LEN + 4 + 1 <= MMIFMM_FILE_FILE_NAME_MAX_LEN) //  ""\"+.bmp" + "SysTumbNailRes"
    {
        ...
        MMIAPICOM_Wstrncpy (list_file_name + list_name_len,  L".txt", 4);
        MMIAPICOM_Wstrncpy (totaltime,  videopath, index + 1);
        MMIAPICOM_Wstrncpy (totaltime + index + 1,  THUMBNAIL_FOLDER_NAME, THUMBNAIL_FOLDER_NAME_LEN);
        SCI_MEM16SET (totaltime + THUMBNAIL_FOLDER_NAME_LEN + index + 1, MMIFILE_SLASH, 2);
        MMIAPICOM_Wstrcpy (totaltime + THUMBNAIL_FOLDER_NAME_LEN + index + 1 + 1, list_file_name);
        *length = MMIAPICOM_Wstrlen (totaltime);
    }
    else
    {
        MMIAPICOM_Wstrncpy (totaltime, videopath, MMIAPICOM_Wstrlen (videopath));
        *length = MMIAPICOM_Wstrlen (totaltime);
        MMIAPICOM_Wstrncpy (totaltime + *length - 4, L".txt", 4);
    }
    return result;
}
/*****************************************************************************/
//  Description :get total time from totaltime file in thumbnail folder
//  Global resource dependence :
//  Author:zhuyongjing
//  Note:
/*****************************************************************************/
PUBLIC  BOOLEAN  MMIFMM_FmmGetVideoTotaltimeByPathname (uint32 *media_lenght, wchar* totaltime_path)
{
    ...
    *media_lenght = 0;
    if (totaltime_path == PNULL || 0 == MMIAPICOM_Wstrlen (totaltime_path))
    {
        SCI_TRACE_LOW("[VP]  MMIFMM_FmmGetVideoTotaltimeByPathname error parameter");
        return FALSE;
    }
    if(!MMIAPIFMM_IsFileExist(totaltime_path,MMIAPICOM_Wstrlen(totaltime_path))){
        SCI_TRACE_LOW("[VP]  MMIFMM_FmmGetVideoTotaltimeByPathname dont exist");
        return FALSE;
    }
    file_handle = MMIAPIFMM_CreateFile(totaltime_path, SFS_MODE_READ|SFS_MODE_OPEN_EXISTING, NULL, NULL);
    ...
    if(SFS_INVALID_HANDLE != file_handle)
    {
        ...
        if(SFS_ERROR_NONE == file_ret && (read_size > 0))
        {
            result = TRUE;
            *media_lenght = totaltime;
        }
    ...
    for (index = 0; index < item_num; index++)
    {
        ...
        if (MMIAPIFMM_IsFileExist (thumbnail_name, thumbnail_name_len))
        {
            ...
            if (item_t_ptr->item_data_ptr->item_content[0].item_data_type ==  GUIITEM_DATA_IMAGE_ID)
            {
                ...
                if (!GUILIST_GetListState(MMIFMM_UDISK_LISTBOX_CTRL_ID,GUILIST_STATE_ENABLE_MARK))
                {
                ...
                else if (GUILIST_GetListState(MMIFMM_UDISK_LISTBOX_CTRL_ID,GUILIST_STATE_ENABLE_MARK))
                {
...
LOCAL  BOOLEAN  FmmVideoThumbnailCallback (DPLAYER_IMAGE_DATA_T *image_data, uint32 total_time)
{
    ...
    wchar  totaltime_file_name [MMIFMM_FULL_FILENAME_LEN+1] = {0};
    ...
    //zhuyongjing add begin
        MMIFMM_FmmVideoTotaltimeGetPathname (totaltime_file_name, &length1, video_fullpathname);
        file_handle = MMIAPIFMM_CreateFile(totaltime_file_name, SFS_MODE_WRITE|SFS_MODE_CREATE_ALWAYS, NULL, NULL);
        if(SFS_INVALID_HANDLE == file_handle)
	{
	    SCI_TRACE_LOW("FmmVideoThumbnailCallback file_handle = NULL POINTER");
            return FALSE;
	}

        if(SFS_ERROR_NONE != MMIAPIFMM_WriteFile(file_handle, &total_time, sizeof(uint32), &write_len, NULL))
        {
            SCI_TRACE_LOW("FmmVideoThumbnailCallback MMIAPIFMM_WriteFile fail");
            MMIAPIFMM_CloseFile(file_handle);
            return FALSE;
        }
        MMIAPIFMM_CloseFile(file_handle);
        if (result && MMIAPICOM_Wstrlen (video_fullpathname) + THUMBNAIL_FOLDER_NAME_LEN + 4 + 1 > MMIFMM_FILE_FILE_NAME_MAX_LEN) //  ""\"+.bmp" + "SysTumbNailRes"
        {
            MMIAPIFMM_SetAttr (totaltime_file_name, length1, TRUE, FALSE, FALSE, FALSE);
        }
...
//zhuyongjing add for gallery video thumbnail
    }else if(MMK_IsFocusWin (MMIPICVIEW_LIST_WIN_ID)){
        MMIPICVIEW_VideoThumbnailOpenCallback(thumbnail_file_name,video_fullpathname,total_time);
    }else if(MMK_IsFocusWin (MMIPICVIEW_PREVIEW_WIN_ID)){
        MMIPICVIEW_PreviewVideoThumbnailOpenCallback(thumbnail_file_name,video_fullpathname);
    }else if(MMK_IsFocusWin (MMIVP_DEFAULT_LOCALLIST_WIN_ID)){
        MMIVPThumbnailUpdateIcon();
    //add end
	}
...
PUBLIC void MMIFMM_VideoThumbnailAdd (wchar  *video_fullpath_name)
{
    ...
    {
		// Avoid add same file
        if(!MMIAPIVP_CheckThumbnailVideoAdded(video_fullpath_name))
        {
...
PUBLIC void  MMIFMM_VideoThumbnailDeleteRelatedFile(wchar *file_name_ptr)
{
    ...
    if (result && MMIFILE_IsFileExist(thumbnail_name, thumbnail_name_len))
    {
        ...
        SCI_TRACE_LOW("FmmVideoThumbnailDeleteRelatedFile thumbnail NULL POINTER");
    }

    SCI_TRACE_LOW("FmmVideoThumbnailDeleteRelatedFile thumbnail Leave result = %d", result);
    //zhuyongjing add 
    result = MMIFMM_FmmVideoTotaltimeGetPathname(totaltime_name, &totaltime_name_len, file_name_ptr);
    if (result && MMIFILE_IsFileExist(totaltime_name, totaltime_name_len))
    {
        //DELETE
        MMIFILE_DeleteFile(totaltime_name, PNULL);//pclint
        SCI_TRACE_LOW("FmmVideoThumbnailDeleteRelatedFile totaltime NULL POINTER");
    }
    // zhuyongjing add end
    SCI_TRACE_LOW("FmmVideoThumbnailDeleteRelatedFile totaltime Leave result = %d", result);
}
