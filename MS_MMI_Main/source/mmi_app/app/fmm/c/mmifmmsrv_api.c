...
PUBLIC void MMIAPIFMM_SearchFileNotify(MMIFMM_SRV_HANDLE handle)
{
    MMIFMM_SearchFileNotify(handle);
}
...
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIFMM_SearchFileInPathsAndSort(
                           wchar         **path_array,      //in, 搜索目录名，如果是NULL，则搜索所有存储器
                            ...
                            )
{
    ...
    search_file_info.path_array = path_array;
    search_file_info.path_array_len = path_array_len;
    search_file_info.file_array = file_array;
    search_file_info.filter_ptr = filter_ptr;
    search_file_info.is_recurse = is_recurse;
    search_file_info.sort_type  = sort_type;
    ...
    search_file_info.msg_id     = msg_id;
    search_file_info.func_type = func_type; 
    search_file_info.search_type = SearchFileInPathsAndSort;
    search_file_info.search_cbk = DefaultSearchFileCallBack;
...
//  Note:
/*****************************************************************************/
void MMIAPIFMM_SearchFileInPaths(
                           wchar         **path_array,      //in, 搜索目录名，如果是NULL，则搜索所有存储器
                            uint16              path_array_len,
                            MMIFMM_FILTER_T     *filter_ptr,    //筛选参数
                            BOOLEAN             is_recurse,     //搜索是否递归
                            ...
                            )
{
    ...
    search_file_info.msg_id     = msg_id;
    search_file_info.func_type = func_type;
    search_file_info.search_type = SearchFileInPaths;
    search_file_info.search_cbk = DefaultSearchFileCallBack;
