...
PUBLIC void MMIAPIFMM_HandleWallpaperWaitMsg (void)
{
    ...
    MMIAPISET_SetWallpaperSetInfo (wallpaper_info);
        {//dong.chunguang add for set wallpaper successfully tip
...
)
{
                               ...
                               &wallpaper_modify_time))
    {
        ...
        //prompt waiting
        //MMIAPIFMM_OpenWaitSetWallpaper();//zhuyongjing delete
...
LOCAL void SetPictureToWallpaperCallBack (MMIPICCROP_CB_T* callback)
{
    if (PNULL != callback)
    {
		...
		SCI_TRACE_LOW("yangyu SetPictureToWallpaperCallBack errtype: %d",callback->err_type);
        if (MMK_IsOpenWin (MMIPIC_CROP_MAIN_WIN_ID))
        {
            MMK_CloseParentWin (MMIPIC_CROP_MAIN_WIN_ID);
        }
...
extern BOOLEAN s_mmipreview_is_error_type;
#endif
/*****************************************************************************/
...
//#endif
    ...
    {
#ifdef PIC_VIEWER_SUPPORT
    	SCI_TRACE_LOW("MMIAPIFMM_SetPictureToWallpaperEx error_type %d ",s_mmipreview_is_error_type);
...
PUBLIC BOOLEAN MMIAPIFMM_GetMoreWallpaperNVByStyle(MMIFMM_AS_WALLPAPER_INFO* wallpaper_info_ptr)
{
    ...
    SCI_TRACE_LOW("MMIAPIFMM_GetMoreWallpaperNVByStyle: idle_style = %d,idle_substyle = %d,wallpaper_type = %d",idle_style,idle_substyle);
