...
/*****************************************************************************/
//  Description : whether file is in default backup folder
//  Global resource dependence :
//  Author: WEN.DONG
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIFMM_IsDefaultBackupFolder (
    const wchar      *file_name,
    uint16      name_len
)
{
    BOOLEAN     result = FALSE;
    uint16     offset = 0;
    wchar       backup_name [MMIMULTIM_DIR_BACKUP_LEN + MMIFMM_PATH_NAME_OFFSET+1] = {0};
    uint16      backup_len = 0;
    ...
    backup_len = MMIAPICOM_Wstrlen (file_name);
	offset = MMIFMM_PATH_NAME_OFFSET;
	if(backup_len - offset == MMIAPICOM_Wstrlen(MMIMULTIM_DIR_BACKUP))
	{
		MMIAPICOM_Wstrncpy(backup_name,file_name,backup_len);
	}
	else
	{
		return FALSE;
    ...
    if ( (MMIAPICOM_Wstrlen (MMIMULTIM_DIR_BACKUP) == (backup_len - offset)))
    {
        if (0 == MMIAPICOM_CompareTwoWstrExt (&backup_name[offset], MMIAPICOM_Wstrlen (MMIMULTIM_DIR_BACKUP),
                                              MMIMULTIM_DIR_BACKUP, MMIAPICOM_Wstrlen (MMIMULTIM_DIR_BACKUP), FALSE))
        {
            result = TRUE;
        }
    }
...
/*****************************************************************************/
//  Description : whether file is in default received folder
//  Global resource dependence :
//  Author: WEN.DONG
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIFMM_IsDefaultReceivedFolder (
    const wchar      *file_name,
    uint16      name_len
)
{
    BOOLEAN     result = FALSE;
    uint16     offset = 0;
    wchar       received_name [MMIMULTIM_DIR_RECEIVED_LEN + MMIFMM_PATH_NAME_OFFSET+1] = {0};
    ...
    received_len = MMIAPICOM_Wstrlen (file_name);
	offset = MMIFMM_PATH_NAME_OFFSET;
	if(received_len - offset == MMIAPICOM_Wstrlen(MMIMULTIM_DIR_RECEIVED))
	{
		MMIAPICOM_Wstrncpy(received_name,file_name,received_len);
	}
	else
	{
		return FALSE;
    ...
    if ( (MMIAPICOM_Wstrlen (MMIMULTIM_DIR_RECEIVED) == (received_len - offset)))
    {
        if (0 == MMIAPICOM_CompareTwoWstrExt (&received_name[offset], MMIAPICOM_Wstrlen (MMIMULTIM_DIR_RECEIVED),
                                              MMIMULTIM_DIR_RECEIVED, MMIAPICOM_Wstrlen (MMIMULTIM_DIR_RECEIVED), FALSE))
        {
            result = TRUE;
        }
    }
...
/*****************************************************************************/
//  Description : whether file is in default recordings folder
//  Global resource dependence :
//  Author: WEN.DONG
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIFMM_IsDefaultRecordingsFolder (
    const wchar      *file_name,
    uint16      name_len
)
{
    BOOLEAN     result = FALSE;
    uint16     offset = 0;
    wchar       recording_name [MMIMULTIM_DIR_RECORDING_LEN + MMIFMM_PATH_NAME_OFFSET+1] = {0};
    uint16      recording_len = 0;
    ...
    recording_len = MMIAPICOM_Wstrlen (file_name);
	offset = MMIFMM_PATH_NAME_OFFSET;
	if(recording_len - offset == MMIAPICOM_Wstrlen(MMIMULTIM_DIR_RECORDING))
	{
		MMIAPICOM_Wstrncpy(recording_name,file_name,recording_len);
	}
	else
	{
		return FALSE;
    ...
    if ( (MMIAPICOM_Wstrlen (MMIMULTIM_DIR_RECORDING) == (recording_len - offset)))
    {
        if (0 == MMIAPICOM_CompareTwoWstrExt (&recording_name[offset], MMIAPICOM_Wstrlen (MMIMULTIM_DIR_RECORDING),
                                              MMIMULTIM_DIR_RECORDING, MMIAPICOM_Wstrlen (MMIMULTIM_DIR_RECORDING), FALSE))
        {
            result = TRUE;
        }
    }
