...
//  Note:
    ...
    )
    {
        ...
        if ( (PNULL == device_ptr) || (0 == device_len) || (PNULL == full_path_ptr) || (PNULL == full_path_len_ptr) || (0 ==*full_path_len_ptr))
        {
...
//  Note:
    ...
    )
    {
        ...
        if (size < kb_size)
        {
            sprintf (temp_str_ptr, "%ld B", (int32) size);
        }
        /* KB */
        else if (size < mb_size)
        {
            if (is_rounding)
            {
                sprintf (temp_str_ptr, "%ld KB", (int32) (size / kb_size));/*lint !e737 */
            }
            else
            {
                sprintf (temp_str_ptr, "%ld.%ld KB", (int32) (size / kb_size), (int32) (size % kb_size) / (kb_size / 10));/*lint !e737 !e573*/
            }
        }
        /* MB */
        else if (size < gb_size)
        {
            if (is_rounding)
            {
                sprintf (temp_str_ptr, "%ld MB", (int32) (size / mb_size));/*lint !e737 */
            }
            else
            {
                sprintf (temp_str_ptr, "%ld.%ld MB", (int32) (size / mb_size), (int32) (size % mb_size) / (mb_size / 10));/*lint !e737 !e573*/
            }
        }
        /* GB */
        else
        {
            if (is_rounding)
            {
                sprintf (temp_str_ptr, "%ld GB", (int32) (size / gb_size));/*lint !e737 */
            }
            else
            {
                sprintf (temp_str_ptr, "%ld.%ld GB", (int32) (size / gb_size), (int32) (size % gb_size) / (gb_size / 10));/*lint !e737 !e573*/
            }
        }
...
//  Note:
    ...
    )
    {
        ...
        if (0 < full_path_len)
        {
            ...
            SCI_TRACE_LOW("MMIAPIFMM_GetFileInfo: sfs_handle=0x%x", sfs_handle);
            ...
            if (SFS_INVALID_HANDLE != append_file_handle)
            {
                ...
                if (SFS_ERROR_NONE == file_ret)
                {
                    if (append_postion < file_size)
                    {
                        ...
                        if (append_postion > 0)
                        {
                            file_ret = SFS_SetFilePointer (append_file_handle, append_postion, SFS_SEEK_BEGIN);/*lint !e747*/
                        }
                        if(SFS_ERROR_NONE == file_ret)
                        {
...
//  Note:
    ...
    PUBLIC MMI_TEXT_ID_T  MMIAPIFMM_GetDeviceName (MMIFILE_DEVICE_E device_type)
    {
        ...
        case MMI_DEVICE_SDCARD:
			...
			text_id = STR_ILIST_MEMORY_CARD_EXT01;
#else
...
//  Note:
    ...
    PUBLIC MMI_TEXT_ID_T  MMIAPIFMM_GetDeviceFreeSpaceText (MMIFILE_DEVICE_E device_type)
    {
        ...
        {
        case MMI_DEVICE_UDISK:
            text_id = STR_BT_PHONE_SPACE;
            break;
        case MMI_DEVICE_SDCARD:
            text_id = STR_BT_MMC_SPACE;
            break;
        case MMI_DEVICE_SDCARD_1:
...
//  Note: change byte to kb for caculate
    ...
    PUBLIC int32 MMIAPIFMM_GetFreeSpaceInPercent (const wchar *device_ptr)
    {
        ...
        if(SFS_ERROR_NONE != SFS_GetDeviceUsedSpace (device_ptr, &used_high, &used_low))
        {
	        return -1;
        }
        if(SFS_ERROR_NONE != SFS_GetDeviceFreeSpace (device_ptr, &free_high, &free_low))
        {
			return -1;
        }
...
static uint32  temp_log_buf_len = 0;
						  ...
						  )
	{
		...
		{
			Trace_Log_Print("UserFile_WriteQQ Return No SD");
			return FALSE;
		}	
		 ...
		 if(MMIFILE_IsFileExist(full_path,full_path_len))
		 {
			Trace_Log_Print("UserFile_WriteQQ Return Exsit");
			return FALSE;
		 }
...
void Trace_Log_Buf_PrintEx(char level, char channel,const char *pszFmt, ...)
{
    //#if MMI_WAP_FILE_TEST
    #if 0
    if(temp_log_buf_msg != PNULL)
