...
extern BOOLEAN is_enter_record;
#if defined(ZDT_VIDEO_KING_SUPPORT)
extern BOOLEAN is_enter_media;
#endif
/**-------------------------------------------------------------------------*
**                         GLOBAL DEFINITION                                *
**--------------------------------------------------------------------------*/
...
PUBLIC BOOLEAN InitRecorderFolder (
    const wchar   *device_ptr,
    uint16  device_len,
    const wchar   *dir_ptr,
    uint16  dir_len
)
{
    ...
    if (MMIAPIFMM_CombineFullPath (device_ptr, device_len,
                                   ...
                                   full_path_name, &full_path_len))
    {
    	  result = MMIAPIFMM_IsFolderExist (full_path_name, full_path_len);
        if (!result)
        {
            result = MMIAPIFMM_CreateDir (full_path_name, full_path_len);
        }
	  else{
	  	SCI_TRACE_LOW("mmifmm path of RecorderFolder was exist");
		return result;
	  }
    }
	SCI_TRACE_LOW("mmifmm InitRecorderFolder create folder fail");
	return result;
}

/*****************************************************************************/
//  Description : init photo thumbnail folder
//  Global resource dependence : none
//  Author:zhuyongjing
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIFMM_InitThumbnailFolder (
    const wchar   *device_ptr,
    uint16  device_len,
    const wchar   *dir_ptr,
    uint16  dir_len
)
{
                                   ...
                                   full_path_name, &full_path_len))
    {
        ...
        MMIAPIFMM_SetAttr (full_path_name, full_path_len, TRUE, FALSE, FALSE, FALSE);
    }
...
PUBLIC void MMIAPIFMM_InitFolders (void)
{
    ...
    do
    {
        if (MMIAPIFMM_GetDeviceTypeStatus (fmm_dev))
        {
            ...
            //zhuyongjing add for pic dir accoding to ms require 20150811
#ifdef MMI_SHOW_MSA
                InitMultimFolder (MMIAPIFMM_GetDevicePath (fmm_dev), MMIAPIFMM_GetDevicePathLen (fmm_dev),
                              MMIMULTIM_DIR_PIC_CAMERA_ROLL, MMIAPICOM_Wstrlen (MMIMULTIM_DIR_PIC_CAMERA_ROLL));           
                InitMultimFolder (MMIAPIFMM_GetDevicePath (fmm_dev), MMIAPIFMM_GetDevicePathLen (fmm_dev),
                            MMIMULTIM_DIR_PIC_ONEDRIVE_DOWNLOAD, MMIAPICOM_Wstrlen (MMIMULTIM_DIR_PIC_ONEDRIVE_DOWNLOAD));

               MMIAPIFMM_InitThumbnailFolder(MMIAPIFMM_GetDevicePath (fmm_dev), MMIAPIFMM_GetDevicePathLen (fmm_dev),
                            MMIMULTIM_DIR_PIC_ONEDRIVE_THUMBNAILS, MMIAPICOM_Wstrlen (MMIMULTIM_DIR_PIC_ONEDRIVE_THUMBNAILS));
#endif
             
                InitMultimFolder (MMIAPIFMM_GetDevicePath (fmm_dev), MMIAPIFMM_GetDevicePathLen (fmm_dev),
                            MMIMULTIM_DIR_PIC_MMS_BROWSER_SAVE_PICS, MMIAPICOM_Wstrlen (MMIMULTIM_DIR_PIC_MMS_BROWSER_SAVE_PICS));
            //add end
            InitMultimFolder (MMIAPIFMM_GetDevicePath (fmm_dev), MMIAPIFMM_GetDevicePathLen (fmm_dev),
                            MMIMULTIM_DIR_BACKUP, MMIAPICOM_Wstrlen (MMIMULTIM_DIR_BACKUP));
            InitMultimFolder (MMIAPIFMM_GetDevicePath (fmm_dev), MMIAPIFMM_GetDevicePathLen (fmm_dev),
                            MMIMULTIM_DIR_RECORDING, MMIAPICOM_Wstrlen (MMIMULTIM_DIR_RECORDING));
            InitMultimFolder (MMIAPIFMM_GetDevicePath (fmm_dev), MMIAPIFMM_GetDevicePathLen (fmm_dev),
                            MMIMULTIM_DIR_RECEIVED_RECORDING, MMIAPICOM_Wstrlen (MMIMULTIM_DIR_RECEIVED_RECORDING));
            InitMultimFolder (MMIAPIFMM_GetDevicePath (fmm_dev), MMIAPIFMM_GetDevicePathLen (fmm_dev),
                            MMIMULTIM_DIR_RADIO_RECORDING, MMIAPICOM_Wstrlen (MMIMULTIM_DIR_RADIO_RECORDING));
            InitMultimFolder (MMIAPIFMM_GetDevicePath (fmm_dev), MMIAPIFMM_GetDevicePathLen (fmm_dev),
                            MMIMULTIM_DIR_RECEIVED, MMIAPICOM_Wstrlen (MMIMULTIM_DIR_RECEIVED));
            InitMultimFolder (MMIAPIFMM_GetDevicePath (fmm_dev), MMIAPIFMM_GetDevicePathLen (fmm_dev),
                            MMIMULTIM_DIR_MUSIC_FILE, MMIAPICOM_Wstrlen (MMIMULTIM_DIR_MUSIC_FILE));
...
//format SD card when LIST UI 
    if(MMIAPISD_IsSDNeedFormat(MMI_DEVICE_SDCARD))
    {  
          MMI_TEXT_ID_T alert_text_id = STR_MMC_FORMAT;
		  
	   MMIFMM_FmmFormatOpenExplore(MMI_DEVICE_SDCARD,alert_text_id);		
    }
    else
    {
   	    MMIFMM_OpenExplorerWin (&openwin_info);
...
/*****************************************************************************/
//  Description : open record fmm win
//  Global resource dependence : none
//  Author: dong.chunguang
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIFMM_OpenExplorerRecord (
    const wchar * path_ptr,
    uint16   path_len,
    const wchar * filename,
    uint16   file_len,
...
)
{
    ...
    openwin_info.sk_text_id.leftsoft_id = STXT_OPTION;
	openwin_info.sk_text_id.middlesoft_id = TXT_COMM_OPEN;
    openwin_info.sk_text_id.rightsoft_id = STXT_RETURN;
#else
    ...
    //MMIFMM_OpenExplorerWin (&openwin_info);
	MMIFMM_OpenExplorerRecordWin(&openwin_info);
}
#if defined(ZDT_VIDEO_KING_SUPPORT)
PUBLIC void MMIAPIFMM_OpenExplorerMedia (
    const wchar * path_ptr,
    uint16   path_len,
    const wchar * filename,
    uint16   file_len,
...
)
{
	...
	openwin_info.sk_text_id.middlesoft_id = TXT_NULL;
    openwin_info.sk_text_id.rightsoft_id = TXT_NULL;
	MMIFMM_OpenExplorerMediaWin(&openwin_info);
}
#endif
/*****************************************************************************/
//  Description : open any fmm win
...
)
{
	...
	{
		//BUG 1172798 Start[Germany version requirement: file manager can not play mp3 file]
		if(MMIAPISET_isRegionDE())
		{
			if(FileManager_IsMp3File(full_file_name,filename_len))
			{
				MMIPUB_OpenAlertWarningWin(STR_FILE_NOT_SUPPORT);
				return;
			}
...
)
{
    ...
    if (MMIFMM_PATH_DEPTH <= MMIFMM_GetPathDepth (full_path_ptr, (uint16) current_path_len))
    {
        ...
        //MMIPUB_OpenAlertFailWin(STR_FILE_PATH_MAXIMUM);
...
/*****************************************************************************/
//  Description : send Email by Outlook
//  Global resource dependence :
//  Author: wen.dong
//  Note:
//      Example 3 for Share:
//      Gallery/Voice/...
//      file://c:/1.jpg
//      url://http://www.microsoft.com/onedrive/2.jpg
/*****************************************************************************/
#ifdef MMIEMAIL_SUPPORT
PUBLIC void MMIFMM_FmmSendEmailByOutlook (
    MMIFMM_DATA_INFO_T    *list_data_ptr,
    MMIFMM_PATH_INFO_T    *current_path_ptr,
    uint16                 index
)
{
    wchar*  outlook_file_name = L"outlookfmmshare.txt";
    ...
    if(0 != des_file_handle)
    {
        ...
        MMIFMM_CombineFullFileName (list_data_ptr, current_path_ptr, index, full_file_name, MMIFILE_FULL_PATH_MAX_LEN);

        //strcat(path, "file://");
        MMIAPICOM_WstrReplace(&full_file_name,sizeof(wchar)*(MMIFILE_FULL_PATH_MAX_LEN + 1), slash, new_split);
        ...
        {
            MMIAPICOM_Wstrcat(content, L"Voice\n");
        }
        MMIAPICOM_Wstrcat(content,L"file://");
        MMIAPICOM_Wstrcat(content,full_file_name);
        ...
        //content_len = MMIAPICOM_Wstrlen(content);
...
#ifdef DRM_SUPPORT
                || (MMIAPIDRM_IsDRMFile (SFS_INVALID_HANDLE, power_on_off_anim_info.more_anim_info.name_wstr)
                    && MMIFMM_FILE_TYPE_PICTURE== MMIAPIDRM_GetMediaFileType (SFS_INVALID_HANDLE, power_on_off_anim_info.more_anim_info.name_wstr))
#endif
        )			
        {
...
{
    if (MMK_IsOpenWin (MMIFMM_COPY_SELECT_FILE_WIN_ID) && !MMK_IsOpenWin (MMIFMM_OPEN_FILE_WIN_ID))
    {
...
#if 0 //WXY no this api used temply
#ifdef VIDEO_PLAYER_SUPPORT
    if (MMK_IsOpenWin (MMIVP_WAIT_WIN_ID))
    {
        MMK_SendMsg (MMIVP_WAIT_WIN_ID, MSG_VP_SEARCH_INTERRUPTED, PNULL);
    }
#endif
#endif  
    ...
    if (SFS_INVALID_HANDLE != file_handle && (SFS_ERROR_NONE == result))
    {
        ...
        do
        {
            ...
            if ( (full_path_len + filename_len + 1) <= MMIFMM_FULL_FILENAME_LEN && (filename_len > 0))
            {
                ...
                data_ptr->data[index]->filename_ptr = SCI_ALLOCA ( (sizeof (wchar) * data_ptr->data[index]->filename_len + 1));
                ...
                SCI_MEMSET (data_ptr->data[index]->filename_ptr, 0x00, (sizeof (wchar) *data_ptr->data[index]->filename_len + 1));
                ...
                else
                {
                    ...
                    * (full_file_path_ptr + full_path_len) = MMIFILE_SLASH;
                    ...
                    //add for record list
                    #if defined(ZDT_VIDEO_KING_SUPPORT)
                    if(is_enter_media == TRUE){
	                    if (!MMIFMM_IsFileNeedDisplay (MMIFMM_GetMediaFileType(), full_file_path_ptr, full_file_path_len))
	                    {
	                        SCI_FREE (data_ptr->data[index]->filename_ptr);
	                        data_ptr->data[index]->filename_ptr = PNULL;
	                        continue;
                    ...
                    if(is_enter_record == TRUE){
	                    if (!MMIFMM_IsFileNeedDisplay (MMIFMM_GetRecordFileType(), full_file_path_ptr, full_file_path_len))
	                    {
	                        SCI_FREE (data_ptr->data[index]->filename_ptr);
	                        data_ptr->data[index]->filename_ptr = PNULL;
	                        continue;
                    ...
                    else if (!MMIFMM_IsFileNeedDisplay (MMIFMM_GetDisplayFileType(), full_file_path_ptr, full_file_path_len))
                    {
                        SCI_FREE (data_ptr->data[index]->filename_ptr);
                        data_ptr->data[index]->filename_ptr = PNULL;
                        continue;
...
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPIFMM_SetMP3ToRing (
    MMIFMM_SET_RING_TYPE_E   ring_type,
    DPARAM                      param,
    uint16                      *full_name_ptr,
    uint16                      full_name_len
...
BOOLEAN FileManager_IsMp3File(wchar *filename,uint16 filename_len)
{
    MMISRVAUD_CONTENT_INFO_T    mp3_file_info = {0};

    if(NULL == filename ||
       0 == filename_len    ||
       MMIFILE_FILE_NAME_MAX_LEN < filename_len)
    {
        SCI_TRACE_LOW("error:filename is PNULL or filename len > MMIFILE_FILE_NAME_MAX_LEN");
        return FALSE;
    }
    SCI_TRACE_LOW("filename=%s,filename_len=%d",filename,filename_len);
    if(MMISRVAUD_RING_FMT_MP3 == MMIAPICOM_GetMusicType(filename,filename_len)) //TODO:according mp3 file content
    {
        SCI_TRACE_LOW("suffix is mp3");
        return TRUE;
    }

    if(TRUE == MMISRVAUD_GetFileContentInfo(filename,filename_len,&mp3_file_info))
    {
        if(MMISRVAUD_RING_FMT_MP3 == MMISRVAUD_GetAudioTypeAccordingCodec(mp3_file_info.codec))
        {
            SCI_TRACE_LOW("codec is mp3");
            return TRUE;
        }
    }
    else
    {
        SCI_TRACE_LOW("MMISRVAUD_GetFileContentInfo is fail");
    }
