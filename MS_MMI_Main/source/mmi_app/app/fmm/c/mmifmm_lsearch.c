...
LOCAL void SearchPlayMovieFile (uint16 item_index)
{
    ...
    if (0 == file_info.size)
    {
        MMIPUB_OpenAlertWinByTextId (PNULL, STR_FILE_EMPTY, TXT_NULL, IMAGE_PUBWIN_WARNING, PNULL, PNULL, MMIPUB_SOFTKEY_ONE, PNULL);
        return;
    }
...
PUBLIC void MMIAPIFMM_OpenMatchedItem (uint32 user_data)
{
    ...
    else
    {
        ...
        if (MMIFMM_FILE_TYPE_DRM == file_type)
        {
            ...
            if (0 == drm_permission)
            {
                MMIPUB_OpenAlertWinByTextId (PNULL, STR_FILE_NOT_SUPPORT, TXT_NULL, IMAGE_PUBWIN_WARNING, PNULL, PNULL, MMIPUB_SOFTKEY_ONE, PNULL);
                return;
            }
