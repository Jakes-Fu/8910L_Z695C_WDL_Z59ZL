...
BOOLEAN  is_finish_prompt = FALSE; //TRUE;
LOCAL uint8 copy_timer_id = 0;
uint32 copy_files_num = 0;
LOCAL BOOLEAN is_last_file = FALSE;
LOCAL BOOLEAN can_cancel_copy = TRUE;
/*---------------------------------------------------------------------------*/
...
LOCAL MMI_RESULT_E HandleConfirmDialogFun(
    MMI_WIN_ID_T     win_id,
    MMI_MESSAGE_ID_E msg_id,
    DPARAM           param
    );
...
)
{
    ...
    MMI_WIN_ID_T        confirm_wid = MMIFMM_CONFORM_DIALOG_WIN;
    MMI_STRING_T         prompt_str = {0};
    wchar    text[MMIFMM_PATHNAME_LEN + MMIFMM_PATHNAME_LEN] = {0};
    LOCAL wchar   target_folder_name[MMIFMM_PATHNAME_LEN + 1] = {0};
#ifdef MMI_FMM_MOVE_SUPPORT
    ...
    case MSG_FMM_SELECT_MARK_FILE://标记状态复制
        //查找标记的文件
        for (i = copy_win_info->s_copy_del_path_data_ptr->cur_mark_num; 
                i < (uint16) (copy_win_info->list_data_ptr->file_num + copy_win_info->list_data_ptr->folder_num); i++)
        {
...
//#endif
            if (copy_win_info->list_data_ptr->path_is_valid > 0)
            {
                ...
                MMIAPICOM_Wstrncpy (copy_win_info->s_copy_del_path_data_ptr->user_scr_path, 
                                    copy_win_info->list_data_ptr->data[i]->filename_ptr,
                                    copy_win_info->list_data_ptr->data[i]->filename_len);

                //去掉后面的文件名，只保留路径
                for (j = copy_win_info->list_data_ptr->data[i]->filename_len; 
				j > 0 && copy_win_info->s_copy_del_path_data_ptr->user_scr_path[j-1] != MMIFILE_SLASH; j--)
                {
                    copy_win_info->s_copy_del_path_data_ptr->user_scr_path[j-1] = 0x0000;
                }

                MMIAPICOM_Wstrncpy (copy_win_info->s_copy_del_path_data_ptr->cur_pathname, 
                                          copy_win_info->list_data_ptr->data[i]->filename_ptr,
                                          copy_win_info->list_data_ptr->data[i]->filename_len);
            }
            else
            {
                MMIFMM_CombineToFileName (copy_win_info->current_path_ptr->pathname, 
                                          copy_win_info->current_path_ptr->pathname_len, 
                                          copy_win_info->list_data_ptr->data[i]->filename_ptr, 
                                          copy_win_info->list_data_ptr->data[i]->filename_len,
    ...
    case MSG_FMM_COPY_CHOOSE_PATH_RETURN://选择路径返回
        //MMIFMM_OpenComWaitWin (HandleWaitCopyWinMsg);
        {
            ...
            uint32 ifa, total_size;
            FILEARRAY file_array = NULL;
            MMIFMM_FILTER_T filter = {"*"};
            MMIFILE_DEVICE_E dest_dev;
            uint32 free_space_high = 0, free_space_low = 0;

            dest_dev = MMIAPIFMM_GetDeviceTypeByPath (copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname, MMIFILE_DEVICE_LEN);
            total_size = 0;

            if (copy_win_info->list_data_ptr->mark_num > 0)
            {
                total_num = GUILIST_GetSelectedItemNum(MMIFMM_UDISK_LISTBOX_CTRL_ID);
                for (i = 0; i < (uint16) (copy_win_info->list_data_ptr->file_num + copy_win_info->list_data_ptr->folder_num); i++)
                {
                    if (MMIFMM_FILE_STATE_UNCHECKED == copy_win_info->list_data_ptr->data[i]->state)
                    {
                        total_size += copy_win_info->list_data_ptr->data[i]->file_size;
                    }
                }
            }
            else
            {
                ...
                MMIFMM_SearchFileInPathNoSort(copy_win_info->s_copy_del_path_data_ptr->cur_pathname, 
                ...
                {
                    FILEARRAY_DATA_T _fdata;
                    MMIAPIFILEARRAY_Read(file_array, i, &_fdata);
                    total_size += _fdata.size;
                }
            ...
            MMIAPIFMM_GetDeviceFreeSpace (
                (uint16 *) MMIAPIFMM_GetDevicePath (dest_dev),
                MMIAPIFMM_GetDevicePathLen (dest_dev),
                &free_space_high,
                &free_space_low
            );

            SCI_TRACE_LOW("[%s %d] need size : %d, [dev:%d] free space : %d\n", __FUNCTION__, __LINE__, total_size,
				dest_dev, free_space_low);

            if(free_space_high == 0 && free_space_low < total_size)
            {
                MMI_STRING_T    src_str = {0};

                prompt_str.wstr_ptr = text;
                SCI_MEMSET (target_folder_name, 0, sizeof (target_folder_name));
                MMI_GetLabelTextByLang(STR_FILES_NOT_ENOUGH_MEMORY_EXT01, &src_str);
                MMI_GetLabelTextByLang(MMIAPIFMM_GetDeviceMemStatusText(dest_dev), &device_str);
                MMIAPICOM_Wstrncpy(&target_folder_name,device_str.wstr_ptr,device_str.wstr_len);
                MMIAPICOM_CustomGenerateReplaceStr(&src_str, L"%U", 
					target_folder_name, MMIFMM_PATHNAME_LEN + MMIFMM_PATHNAME_LEN, &prompt_str);
                ...
                MMIPUB_OpenConfirmationDialogByTextPtr ( PNULL,
                    ...
                    HandleConfirmDialogFun
                    );
            }
            else
            {
                SCI_MEMSET (target_folder_name, 0, sizeof (target_folder_name));
                if (0 == MMIAPICOM_Wstrcmp(copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname,L"D:"))
                {
                    MMI_GetLabelTextByLang(MMIAPIFMM_GetDeviceMemStatusText(MMI_DEVICE_UDISK), &device_str);
                    MMIAPICOM_Wstrncat(&target_folder_name,device_str.wstr_ptr,device_str.wstr_len);
                }
                else if (0 == MMIAPICOM_Wstrcmp(copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname,L"E:"))
                {
                    MMI_GetLabelTextByLang(MMIAPIFMM_GetDeviceMemStatusText(MMI_DEVICE_SDCARD), &device_str);
                    MMIAPICOM_Wstrncat(&target_folder_name,device_str.wstr_ptr,device_str.wstr_len);
                }
                else
                {
                    for (i = copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname_len - 1; 
    				MMIFILE_SLASH != copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname[i] && i > 0; i--)
                    {
                        NULL;
                    }
                    MMIAPICOM_Wstrncat(&target_folder_name,
    					&copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname[i + 1],
    					copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname_len- i - 1);
                }

                prompt_text = (copy_win_info->s_is_copy_from_move) ? STR_FILES_WAIT_MOVING_EXT01 : STR_FILES_WAIT_COPYING_EXT01;
                MMI_GetLabelTextByLang(prompt_text, &temp_str);
                prompt_str.wstr_ptr = text;
                MMIAPICOM_CustomGenerateReplaceStr(&temp_str, L"%U", target_folder_name, MMIFMM_PATHNAME_LEN + MMIFMM_PATHNAME_LEN, &prompt_str);
                MMIPUB_OpenProgressWinByTextPtr(&prompt_str, &wait_win_id, PNULL, MMIPUB_SOFTKEY_CUSTOMER, HandleProcessWinMsg);
                ...
                copy_timer_id = MMK_CreateWinTimer(win_id, 200, TRUE);
                APPFILE_CreateTask();
...
#endif
                {
                    for (k = copy_win_info->s_copy_del_path_data_ptr->cur_mark_num; 
                            k < (uint16) (copy_win_info->list_data_ptr->file_num + copy_win_info->list_data_ptr->folder_num); k++)
                    {
                        if (MMIFMM_FILE_STATE_UNCHECKED != copy_win_info->list_data_ptr->data[k]->state)
                        {
                            break;
                        }
                    }
                    if (k >= (uint16) (copy_win_info->list_data_ptr->file_num + copy_win_info->list_data_ptr->folder_num))
                    {
                        is_last_file = TRUE;
                    }
                }
                is_finish_prompt = TRUE;
            }
        }
    ...
    case MSG_FMM_FILE_EXIST:
        ...
        MMIPUB_CloseWaitWin (MMIFMM_FILE_WAITING_WIN_ID);
        prompt_str.wstr_ptr = text;
        prompt_text = (0 == copy_win_info->list_data_ptr->folder_num) ? STR_FILE_EXISTING_EXT01 : STR_FOLDER_EXISTING_EXT01;
        {
            //char num_str[10+1] = {0};
            ...
            //sprintf((char *)num_str,"%d",100);
            //MMIAPICOM_StrToWstr(num_str, num_wstr);
            //MMIAPICOM_Wstrcat(num_wstr,L"%");
            MMI_GetLabelTextByLang(prompt_text, &src_str);
            MMIAPICOM_CustomGenerateReplaceStr(&src_str, L"%U", target_folder_name,MMIFMM_PATHNAME_LEN + MMIFMM_PATHNAME_LEN,&prompt_str);
        }
        //MMIAPICOM_CustomGenerateNumReplaceStrByTextId(prompt_text, L"U",100,64,&prompt_str);
        MMIPUB_OpenConfirmationDialogByTextPtr ( PNULL,
            &prompt_str,
            PNULL,
            IMAGE_NULL,
...
/*#ifdef MMI_FMM_MOVE_SUPPORT
...
#endif
        {
            ...
            MMI_GetLabelTextByLang (STR_FILE_EXIST_QUERY, &prompt_str2);
    ...
    case MSG_FMM_OPERATE_ONE_FILE_RETURN:
        ...
        else
        {
            ...
            if (SFS_ERROR_NO_SPACE == file_ret)
            {
                ...
                //wchar num_wstr[10+1] = {0};
                MMI_STRING_T    src_str = {0};
                MMI_STRING_T	device_name_str = {0};
                wchar   device_str[MMIFMM_PATHNAME_LEN + 1] = {0};
                wchar   device_name[MMIFMM_PATHNAME_LEN + 1] = {0};
                //MMI_STRING_T   banner_str = {0};
                is_finish_prompt = FALSE;
                ...
                //MMIAPICOM_Wstrcat(num_wstr,L"%");
                MMI_GetLabelTextByLang(STR_FILES_NOT_ENOUGH_MEMORY_EXT01, &src_str);
                MMIAPICOM_Wstrncat(&device_str,&copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname[0],1);
                if (0 == MMIAPICOM_Wstrcmp(device_str,L"D"))
                {
                    MMI_GetLabelTextByLang(MMIAPIFMM_GetDeviceMemStatusText(MMI_DEVICE_UDISK), &device_name_str);
                    MMIAPICOM_Wstrncat(&device_name,device_name_str.wstr_ptr,device_name_str.wstr_len);
                }
                else if (0 == MMIAPICOM_Wstrcmp(device_str,L"E"))
                {
                    MMI_GetLabelTextByLang(MMIAPIFMM_GetDeviceMemStatusText(MMI_DEVICE_SDCARD), &device_name_str);
                    MMIAPICOM_Wstrncat(&device_name,device_name_str.wstr_ptr,device_name_str.wstr_len);
                }
                MMIAPICOM_CustomGenerateReplaceStr(&src_str, L"%U", device_name,MMIFMM_PATHNAME_LEN + MMIFMM_PATHNAME_LEN,&prompt_str);
                //MMIAPICOM_CustomGenerateNumReplaceStrByTextId(STR_FILES_NOT_ENOUGH_MEMORY_EXT01, L"U",100,64,&prompt_str);
                MMIPUB_OpenConfirmationDialogByTextPtr ( PNULL,
                    &prompt_str,
                    PNULL,
                    IMAGE_NULL,
            ...
            /* when user cancel moving/copying,then delete the incomplete files or folders
             * which has been moved/copied to target path.
             */
            if (SFS_ERROR_ABORT == file_ret)
            {
                int32     i = 0;
                wchar     file_name_arr[MMIFMM_FULL_FILENAME_LEN + 20] ={0};
                for (i = MMIAPICOM_Wstrlen(copy_win_info->s_copy_del_path_data_ptr->cur_pathname) - 1; MMIFILE_SLASH != copy_win_info->s_copy_del_path_data_ptr->cur_pathname[i] && i > 0; i--)
                {
                    NULL;
                }

                MMIAPICOM_Wstrncat(&file_name_arr,&copy_win_info->s_copy_del_path_data_ptr->cur_pathname[i],MMIAPICOM_Wstrlen(copy_win_info->s_copy_del_path_data_ptr->cur_pathname) - 1);
                MMIAPICOM_Wstrcat(copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname,&file_name_arr);

                if (MMIAPIFMM_IsFolderExist(copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname,MMIAPICOM_Wstrlen(copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname)) 
                    || MMIAPIFMM_IsFileExist(copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname,MMIAPICOM_Wstrlen(copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname)))
                {
                    MMIFILE_DeleteOpen (copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname,MMIAPICOM_Wstrlen (copy_win_info->s_copy_del_path_data_ptr->target_path_info.pathname));
                }
            }
            //End
            MMK_CloseWin (win_id);
            ...
            MMK_PostMsg (MMIFMM_UDISK_LISTBOX_CTRL_ID, MSG_FMM_COPY_END_CNF, PNULL, 0);
        }
...
/*
        ...
        {
            selected_num = (copy_win_info->list_data_ptr->file_num + copy_win_info->list_data_ptr->folder_num);
...
*/
    ...
    case MSG_CLOSE_WINDOW:
       ...
       MMIPUB_CloseProgressWin(&wait_win_id);
       {
           ...
           prompt_str1.wstr_ptr = (wchar *) SCI_ALLOC_APP ((MMIFMM_FILENAME_LEN + 1) * sizeof (wchar));
           SCI_MEMSET(prompt_str1.wstr_ptr, 0x00, ((MMIFMM_FILENAME_LEN + 1)*sizeof (wchar)));
           prompt_str1.wstr_len = MMIFMM_FILENAME_LEN;
           MMIAPICOM_GetFileName(copy_win_info->s_copy_del_path_data_ptr->cur_pathname,
                                  MMIFMM_FILENAME_LEN,
                                  prompt_str1.wstr_ptr,
                                  &prompt_str1.wstr_len);

           temp_str.wstr_ptr = SCI_ALLOC_APP((6 /* MMICOM_SUFFIX_MAX_LEN */) * sizeof(wchar));
           MMIAPICOM_GetNameAndSuffix(prompt_str1.wstr_ptr, prompt_str1.wstr_len, PNULL, &temp_str);
           temp_str.wstr_len = MMIAPICOM_Wstrlen(temp_str.wstr_ptr);
           file_type = MMIAPIFMM_GetFileType(temp_str.wstr_ptr, temp_str.wstr_len);

           banner_info.banner_type = MMI_BANNER_STR_TEXT;
           banner_info.start_point.x = 0;
           banner_info.start_point.y = 0;
#if 0
           if (TRUE == copy_win_info->s_is_copy_from_move)
           {
               prompt_text = (0 == copy_win_info->list_data_ptr->folder_num) ? STR_NOTE_FILE_MOVED_EXT01 : STR_NOTE_FOLDER_MOVED_EXT01;
           }
           else
           {
               prompt_text = (0 == copy_win_info->list_data_ptr->folder_num) ? STR_NOTE_FILE_COPIED_EXT01 : STR_NOTE_FOLDER_COPIED_EXT01;
           }
#endif
           if (TRUE == copy_win_info->s_is_copy_from_move)
           {
               prompt_text = (MMIFMM_UNKNOW_TYPE != file_type) ? STR_NOTE_FILE_MOVED_EXT01 : STR_NOTE_FOLDER_MOVED_EXT01;
           }
           else
           {
               prompt_text = (MMIFMM_UNKNOW_TYPE != file_type) ? STR_NOTE_FILE_COPIED_EXT01 : STR_NOTE_FOLDER_COPIED_EXT01;
           }
           ...
           //MMIAPICOM_Wstrcat(num_wstr,L"%");
           MMI_GetLabelTextByLang(prompt_text, &src_str);
           MMIAPICOM_CustomGenerateReplaceStr(&src_str, L"%U",target_folder_name,MMIFMM_PATHNAME_LEN + MMIFMM_PATHNAME_LEN,&banner_str);
           //MMIAPICOM_CustomGenerateNumReplaceStrByTextId(prompt_text, L"U",100,64,&banner_str);
    ...
    case MSG_TIMER:
        if (copy_timer_id == *((uint8 *) param))
        {
            if (copy_files_num > 0 && is_last_file)
            {
                GUIWIN_SetSoftkeyBtnGray(wait_win_id, RIGHT_BUTTON, TRUE);
                is_last_file = FALSE;
                can_cancel_copy = FALSE;
            }
             MMIPUB_UpdateProgressBarEx(&wait_win_id,copy_files_num,TRUE);
        }
        break;
    default:
                                       ...
                                       )
{
    ...
    case MSG_CTL_CANCEL:
    case MSG_APP_CANCEL:
        if (PNULL != win_data_p && !win_data_p->s_user_stop_copy && can_cancel_copy)
        {
            win_data_p->s_user_stop_copy = TRUE;
            is_finish_prompt = FALSE;
            MMK_CloseWin (win_id);
            MMK_PostMsg (MMIFMM_COPY_SELECT_FILE_WIN_ID, MSG_FMM_ABORT, PNULL, 0);
            {
                ...
                banner_info.start_point.x = 0;
                banner_info.start_point.y = 0;
                if (TRUE == win_data_p->s_is_copy_from_move)
                {
                    prompt_text = (0 == win_data_p->list_data_ptr->folder_num) ? STR_FILE_MOVE_CANCELLED_EXT01 : STR_FOLDER_MOVE_CANCELLED_EXT01;
                }
                else
                {
                    prompt_text = (0 == win_data_p->list_data_ptr->folder_num) ? STR_FILE_COPY_CANCELLED_EXT01 : STR_FOLDER_COPY_CANCELLED_EXT01;
                }
                banner_info.text1_id = prompt_text;
                MMIAPICOM_OpenBannerWin(MMIFMM_MAIN_WIN_ID, banner_info);
            }
        }
        break;
                                       ...
                                       )
{
    ...
    {
        case MSG_OPEN_WINDOW:
            GUIWIN_SeSoftkeytButtonTextId(win_id, PNULL, RIGHT_BUTTON, TRUE);
            GUIWIN_SeSoftkeytButtonTextId(win_id, PNULL, LEFT_BUTTON, TRUE);
            break;
        case MSG_APP_WEB:
        case MSG_CTL_MIDSK:
            is_finish_prompt = FALSE;
            MMK_SendMsg (MMIFMM_COPY_SELECT_FILE_WIN_ID, MSG_PROMPTWIN_CANCEL, PNULL);
            MMK_CloseWin(win_id);
            break;
        case MSG_APP_OK:
        case MSG_APP_CANCEL:
...
PUBLIC void MMIAPIFMM_CopyProgressCBK(uint16 result)
{
