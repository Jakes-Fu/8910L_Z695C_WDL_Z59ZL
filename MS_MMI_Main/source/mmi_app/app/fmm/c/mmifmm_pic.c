...
//  Note:
/*****************************************************************************/
LOCAL void SetPreviewTitleOsdEx(
    uint32            cur_index,  //in:from 1 start
    uint32            total_num,  //in:
    MMI_CTRL_ID_T     ctrl_id,
    MMI_WIN_ID_T      win_id
...
//  Note:
/*****************************************************************************/
LOCAL void SetPreviewSoftkeyEx(
    uint32         cur_index,
    MMI_WIN_ID_T   win_id  //in:
);
...
/*****************************************************************************/
//  Description : GenerateTwoNumReplaceStrByTextId
//  Global resource dependence :
//  Author: wen.dong
//  Note:
//  %0N/%1N
/*****************************************************************************/
LOCAL void GenerateTwoNumReplaceStrByTextId(
...
)
{
    ...
    case MSG_APP_MENU:
        ...
        MMIFMM_OpenMeunPubWin (win_id, MENU_FMM_PIC_PREVIEW_OPT, MMIFMM_PIC_PREVIEW_OPT_MENU_WIN_ID, MMIFMM_PIC_PREVIEW_OPT_MENU_CTRL_ID);
        break;
...
)
{
    ...
    SetPreviewTitleOsdEx(cur_index, total_num, ctrl_id, win_id);

    //set softkey
    //SetPreviewSoftkey (win_id);
    SetPreviewSoftkeyEx(cur_index,win_id);
...
)
{
    ...
    title_height = GUIFONT_GetHeight(SONG_FONT_28);
#ifdef  PIC_PREVIEW_U200_SUPPORT
    ...
    title_rect.v_rect.bottom = 24;//title_rect.v_rect.top + title_height + 15 - 1;
    title_rect.h_rect.bottom = 24;//title_rect.h_rect.top + title_height + 10 - 1;
#else
    title_rect.v_rect.bottom = 24;
    title_rect.h_rect.bottom = 24;
#endif
    if(PNULL != ctrl_id)
    {
        ...
        string_style.font = SONG_FONT_24;
      ...
      //  title_rect.h_rect.left   = MMIPICVIEW_TITLE_MARGIN;
       ...
       // title_rect.h_rect.right  = title_rect.h_rect.left + min_width - 1;
        ...
        str_len = (uint16)strlen((char *)str);
        MMI_STRNTOWSTR(wstr, 15,
                       str, 15,
                       str_len);*/
        title_str.wstr_ptr = text;
        GenerateTwoNumReplaceStrByTextId(STR_NUMBER_SLASH_NUMBER,cur_index,total_num,&title_str);

        //set icon huyongjing add for action bar 20150722
         ...
         if (MMIPICVIEW_GetPicFileInfo(cur_index-1, &file_info))
		  {
            ...
            if(ONEDRIVE_THUMBNAIL== GetFileSource(file_info.filename,file_info.name_len)){
                ...
                item_info.image_info.image_width = 24;
                item_info.image_info.image_height = 24;
                ...
                item_info.image_info.image_height = 24;
                     ...
                     && FALSE == MMISRV_CAMERAROLL_Is_Pic_On_OneDrive(file_info.filename,file_info.name_len)
    ...
    #else   
        item_info.item_state = GUISETLIST_ITEM_STATE_TEXT;		
    #endif  // CAMERAROLL_SUPPORT

         //set title
        item_info.str_info.wstr_ptr = title_str.wstr_ptr;
        item_info.str_info.wstr_len = title_str.wstr_len;
        GUISETLIST_AddItem(ctrl_id, &item_info);

        //set font
        font.color = MMI_DEFAULT_TEXT_COLOR;
        font.font  = SONG_FONT_24;//MMIPICPREVIEW_TITLE_FONT;
        GUISETLIST_SetFont(ctrl_id, &font);
#ifndef MMI_GUI_STYLE_MINISCREEN
        //no bg
        bg.bg_type = GUI_BG_HALF_TRANS;//GUI_BG_NONE;zhuyongjing change for action bar 20150722
...
)
{
    ...
    GUIWIN_SeSoftkeytButtonTextId(owner_draw_ptr->win_handle, TXT_COMMON_COMMON_BACK, RIGHT_BUTTON, FALSE);
    GUIWIN_SeSoftkeytButtonTextId(owner_draw_ptr->win_handle, STXT_SOFTKEY_MENU_MK, LEFT_BUTTON, FALSE);
    GUIWIN_SeSoftkeytButtonTextId(owner_draw_ptr->win_handle, PNULL, MIDDLE_BUTTON, FALSE);

 	color = MMI_GRAY_WHITE_COLOR;
	font = SONG_FONT_35;
	GUIWIN_SetSoftKeyfont(owner_draw_ptr->win_handle, &color,&font);
...
)
{
    ...
    MMIAPICOM_CustomGenerateReplaceStr(&temp_str, L"%1N", num_wstr, 64,result_str);
}
