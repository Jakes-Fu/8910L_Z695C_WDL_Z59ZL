...
#endif
BOOLEAN is_enter_record = FALSE;
#if defined(ZDT_VIDEO_KING_SUPPORT)
BOOLEAN is_enter_media = FALSE;
#endif
extern uint8 delay_time_id;
/*
 * multi_select_mode:
 *     non_multi_select:  0
 *     nothing selected:  1
 *    all item selected:  2
 *   some item selected:  3
*/
uint32 multi_select_mode = 0;
uint32 file_mark_num = 0;
uint32 actual_file_mark_num = 0;
uint32 folder_mark_num = 0;
BOOLEAN is_quick_delete = FALSE;
/*---------------------------------------------------------------------------*/
...
#ifdef MMI_FMM_LIST_SUPPORT
LOCAL MMI_RESULT_E HandleFmmListWinMsg (
    MMI_WIN_ID_T         win_id,
    MMI_MESSAGE_ID_E     msg_id,
    DPARAM               param
);
...
/*****************************************************************************/
//  Description : HandleFmmMainDetailMenuMsg
//  Global resource dependence :
//  Author: WEN.DONG
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleFmmMainDetailMenuMsg (
    MMI_WIN_ID_T         win_id,
    MMI_MESSAGE_ID_E     msg_id,
    DPARAM               param
);
...
/*****************************************************************************/
//  Description : HandleMainWaitingSpinnerWinMsg
//  Global resource dependence :
//  Author:wen.dong
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleMainWaitingSpinnerWinMsg(
    MMI_WIN_ID_T     win_id,
    MMI_MESSAGE_ID_E msg_id,
    DPARAM           param
);
...
/*****************************************************************************/
//  Description : MMIFMM_IsChinaVersion
//  Author: wen.dong
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIFMM_IsChinaVersion(void);

/*****************************************************************************/
// Description : GetBackupFolderSize
// Global resource dependence :
// Author: wen.dong
// Note:
/*****************************************************************************/
PUBLIC void GetBackupFolderSize(MMIFMM_DETAIL_DATA_T* detail_data);
...
/*****************************************************************************/
//  Description : Cancel discards all changes and exits multi selection mode.
//  Global resource dependence :
//  Author: wen.dong
//  Note:
/*****************************************************************************/
LOCAL void HandleMenuCancelMultiSelect (
    MMIFMM_DATA_INFO_T    *list_data_ptr,
    MMI_CTRL_ID_T         ctrl_id
);
...
#if defined(ZDT_VIDEO_KING_SUPPORT)
LOCAL MMI_RESULT_E HandleMediaListWinMsg (
									MMI_WIN_ID_T         win_id,
									MMI_MESSAGE_ID_E     msg_id,
									DPARAM               param
									);
#endif
/*****************************************************************************/
// Description : record item share opt
// Global resource dependence :
// Author: dong.chunguang
// Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleShareItemsRecordlistOptWinMsg (
									MMI_WIN_ID_T         win_id,
									MMI_MESSAGE_ID_E     msg_id,
									DPARAM               param
									);
/*****************************************************************************/
// Description : record list opt
// Global resource dependence :
// Author: dong.chunguang
// Note:
/*****************************************************************************/
...
/*****************************************************************************/
// Description : ResetPathList for restore
// Global resource dependence :
// Author: dong.chunguang
// Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E ResetPathListForRestore(MMI_WIN_ID_T win_id);
...
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleFmmRestoreConfirmWin(
                                               MMI_WIN_ID_T     win_id,
                                               MMI_MESSAGE_ID_E msg_id,
                                               DPARAM           param
                                               );
...
/*****************************************************************************/
//  Description : 添加列表recordlist的item
//  Global resource dependence :
//  Author: dong.chunguang
//  Note:
/*****************************************************************************/
LOCAL void FmmAppendRecordListItem (//当前索引
									MMI_WIN_ID_T         win_id,
									MMI_CTRL_ID_T        ctrl_id,
									MMI_TEXT_ID_T        left_softkey_id,
									MMIFMM_DATA_INFO_T   *list_data_ptr,
...
/*****************************************************************************/
//  Description : 添加列表recrod list的item
//  Global resource dependence :
//  Author: dong.chunguang
//  Note:  保存文件的类型
/*****************************************************************************/
LOCAL void AppendTwoLineAnimTextAndTextRecordListItem (
								    MMI_WIN_ID_T            win_id,
								    MMI_CTRL_ID_T           ctrl_id,
								    MMI_TEXT_ID_T           left_softkey_id,
								    MMIFMM_FILE_TYPE_E      file_type,
...
/*****************************************************************************/
// Description : GetFileTypeBySuffix
// Global resource dependence :
// Author: wen.dong
// Note:
/*****************************************************************************/
PUBLIC void GetFileTypeBySuffix(wchar* filename,uint16 filename_len,wchar* type);

WINDOW_TABLE (MMIFMM_MAIN_WIN_TAB) =
{
    //CLEAR_LCD,
    //WIN_PRIO( WIN_ONE_LEVEL ),
#ifndef MMI_FMM_LIST_SUPPORT
    WIN_FUNC ( (uint32) HandleFmmMainWinMsg),
#else
    WIN_FUNC ( (uint32) HandleFmmListWinMsg),
#endif
    WIN_ID (MMIFMM_MAIN_WIN_ID),
      WIN_TITLE(TXT_EXPLORER),
...
#else
    CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMIFMM_LIST_CTRL_ID),
#endif

#endif
    WIN_SOFTKEY (STXT_OPTION, STXT_SOFTKEY_OPEN_MK, TXT_COMMON_COMMON_BACK),
    END_WIN
};
//dong.chunguang add
WINDOW_TABLE (MMIFMM_RECORD_WIN_TAB) =
{
    //CLEAR_LCD,
    //WIN_PRIO( WIN_ONE_LEVEL ),
    WIN_FUNC ( (uint32) HandleRecordListWinMsg),
    WIN_ID (MMIFMM_RECORD_LIST_WIN_ID),
        WIN_TITLE(STR_MENU_SINGLE_GAL_VOICE_MEMO),
    WIN_STATUSBAR,
    WIN_STYLE(WS_HAS_TRANSPARENT|WS_DISABLE_COMMON_BG),
    CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMIFMM_RECORD_LIST_CTRL_ID),
    WIN_SOFTKEY (STXT_OPTION, TXT_COMM_OPEN, STXT_RETURN),
    END_WIN
};
#if defined(ZDT_VIDEO_KING_SUPPORT)
WINDOW_TABLE (MMIFMM_MEDIA_WIN_TAB) =
{
    WIN_FUNC ( (uint32) HandleMediaListWinMsg),
    WIN_ID (MMIFMM_MEDIA_LIST_WIN_ID),
     WIN_TITLE(TXT_MEDIA_VIDEO_KING),
    WIN_STATUSBAR,
    WIN_STYLE(WS_HAS_TRANSPARENT|WS_DISABLE_COMMON_BG),
    CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMIFMM_MEDIA_LIST_CTRL_ID),
    WIN_SOFTKEY (TXT_COMM_OPEN, TXT_NULL,  STXT_RETURN),
    END_WIN
};
#endif
WINDOW_TABLE( MMIFMM_RECORD_LIST_OPT_WIN_TAB ) =
{
    WIN_FUNC( (uint32)HandleRecordlistOptWinMsg),
    WIN_ID(MMIFMM_RECORD_OPT_MENU_WIN_ID),
    CREATE_POPMENU_CTRL(MENU_FMM_RECORD_LIST_OPT,MMIFMM_RECORD_OPT_MENU_CTRL_ID),
	WIN_STYLE( WS_HAS_TRANSPARENT|WS_DISABLE_COMMON_BG),
    WIN_SOFTKEY(TXT_NULL,STXT_SOFTKEY_SELECT_MK,STXT_RETURN),
    END_WIN
};
WINDOW_TABLE( MMIFMM_SHARE_ITEMS_RECORD_LIST_OPT_WIN_TAB ) =
{
    WIN_FUNC( (uint32)HandleShareItemsRecordlistOptWinMsg),
    WIN_ID(MMIFMM_SHARE_ITEMS_RECORD_LIST_OPT_WIN_ID),
    CREATE_POPMENU_CTRL(MENU_FMM_RECORD_SEND_ITEMS,MMIFMM_RECORD_SHARE_OPT_MENU_CTRL_ID),
	WIN_STYLE( WS_HAS_TRANSPARENT|WS_DISABLE_COMMON_BG),
    WIN_SOFTKEY(TXT_NULL,STXT_SOFTKEY_SELECT_MK,STXT_RETURN),
    END_WIN
};
...
#endif
#ifdef MMI_FMM_TITLE_LABEL_SUPPORT
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_MIDDLE, MMIFMM_UDISK_LABEL_CTRL_ID),
#endif
    END_WIN
};
...
WINDOW_TABLE (MMFMM_MAIN_DETAIL_MENU_WIN_TAB) =
{
    WIN_FUNC((uint32) HandleFmmMainDetailMenuMsg),
    WIN_ID(MMFMM_MAIN_DETAIL_MENU_WIN_ID),
    WIN_STYLE(WS_HAS_TRANSPARENT),
    CREATE_POPMENU_CTRL(MENU_FMM_MAIN_DETAIL_OPT,MMIFMM_MAIN_DETAIL_MENU_CTRL_ID),
    WIN_SOFTKEY(TXT_COMMON_OK,TXT_NULL,STXT_RETURN),
    END_WIN
};

WINDOW_TABLE(MMIFMM_MAIN_WAIT_SPINNER_WIN_TAB) =
{
    WIN_FUNC((uint32)HandleMainWaitingSpinnerWinMsg),
    WIN_ID(MMIFMM_MAIN_WAIT_SPINNER_WIN_ID),
...
#else
    MMK_CreateWin ( (uint32 *) MMIFMM_MAIN_WIN_TAB, (ADD_DATA) main_win_p);
#endif
    //MMIPUB_OpenAlertWinByTextId (PNULL, TXT_COMMON_WAITING, TXT_NULL, IMAGE_PUBWIN_WAIT, PNULL, PNULL, MMIPUB_SOFTKEY_NONE, PNULL);
    MMK_CreateWin((uint32 *)MMIFMM_MAIN_WAIT_SPINNER_WIN_TAB, PNULL);
...
)
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        for (; fmm_dev < MMI_DEVICE_NUM; fmm_dev++)
        {
               ...
               )
            {
                ...
                if (get_label_res)
                {
                    ...
                    str_text.wstr_len = MIN(MMIAPICOM_Wstrlen (buffer_name),MMIFILE_NEW_DEVICE_NAME_MAX_LEN);
                }
                else
                {
        ...
        {
            //MMIFMM_FmmMemoryFormat(main_d_ptr->s_file_path_info_ptr->s_cur_dev, STR_MMC_FORMAT);
            MMI_TEXT_ID_T alert_text_id = STR_MMC_FORMAT;
...
)
{
    ...
    MMI_CTRL_ID_T              label_ctrl_id = MMIFMM_UDISK_LABEL_CTRL_ID;
    ...
    case MSG_OPEN_WINDOW:
        ...
        MMI_GetLabelTextByLang(MMIAPIFMM_GetDeviceMemStatusText(view_win_d->main_d_ptr->s_file_path_info_ptr->s_cur_dev), &title_str);
		
	 //	GUIWIN_SetTitleText(win_id, title_str .wstr_ptr, MMIAPICOM_Wstrlen(title_str .wstr_ptr), FALSE);	
...
#else
        cur_tab = GUILIST_GetCurItemIndex(MMIFMM_LIST_CTRL_ID);
#endif
        ...
        if ((cur_tab + 1) > MMI_DEVICE_SYSTEM)
        {
#ifndef MMI_FMM_TITLE_BUTTON_SUPPORT
...
//        GUITAB_SetItemWidth(MMIFMM_TAB_CTRL_ID, img_width);
    ...
    case MSG_GET_FOCUS:
        ...
        if (0 != multi_select_mode)
        {
            MMI_GetLabelTextByLang(STR_LIST_DELETE, &title_str);
            if (2 == multi_select_mode || 3 == multi_select_mode)
            {
                title_str .wstr_ptr = text;
                MMIAPICOM_CustomGenerateNumReplaceStrByTextId(STR_GAL_HDR_DELETE_NUM_EXT01, L"%N",file_mark_num + folder_mark_num,32,&title_str);
            }
        }
        else
        {
    ...
    case MSG_CTL_TAB_SWITCH:
        ...
        if (!MMIAPIFMM_GetDeviceStatus (MMIAPIFMM_GetDevicePath (view_win_d->main_d_ptr->s_file_path_info_ptr->s_cur_dev), view_win_d->main_d_ptr->s_file_path_info_ptr->s_device_table[cur_tab]))
        {
            ...
            if (MMIAPISD_IsSDNeedFormat (view_win_d->main_d_ptr->s_file_path_info_ptr->s_cur_dev))
            {
                ...
                MMIFMM_FmmMemoryFormat (view_win_d->main_d_ptr->s_file_path_info_ptr->s_cur_dev, STR_MMC_FORMAT);
                break;
            }
            else
#endif
        ...
        case MSG_FULL_PAINT:
#ifdef MMI_FMM_TITLE_LABEL_SUPPORT
            if(view_win_d->main_d_ptr->s_fmm_list_data_ptr->path_depth > 1)
            {
                 GUI_BG_T      bg_ptr = {0};
                 GUI_LCD_DEV_INFO    lcd_dev_info = {GUI_MAIN_LCD_ID, GUI_BLOCK_MAIN};
                 GUI_RECT_T  rect = MMITHEME_GetFullScreenRect();
                 bg_ptr.bg_type = GUI_BG_COLOR;
                 bg_ptr.color = MMI_WINDOW_BACKGROUND_COLOR;     //// //// ZDT_PRODUCT_Z685S
                 list_rect = MMITHEME_GetWinClientBothRect (win_id);
                 list_rect.h_rect.bottom = list_rect.h_rect.top + MMI_FULLSCREEN_SOFTKEY_BAR_HEIGHT - 3;
                 list_rect.v_rect.bottom = list_rect.v_rect.top + MMI_FULLSCREEN_SOFTKEY_BAR_HEIGHT - 3;
                 GUIAPICTRL_SetBothRect (label_ctrl_id, &list_rect);
                 GUILABEL_SetVisible(label_ctrl_id,TRUE,TRUE);
                 GUILABEL_SetBg(label_ctrl_id,&bg_ptr);
                 //GUILABEL_SetBackgroundColor(label_ctrl_id, MMI_BLACK_COLOR);
                 GUILABEL_SetFont(label_ctrl_id, CAF_FONT_20, MMI_BLACK_COLOR);

                 rect.top = list_rect.h_rect.top + MMI_FULLSCREEN_SOFTKEY_BAR_HEIGHT - 3;
                 rect.bottom = list_rect.v_rect.top + MMI_FULLSCREEN_SOFTKEY_BAR_HEIGHT;
                 LCD_FillRect(&lcd_dev_info, rect, MMI_DARK_WHITE_COLOR);

                 list_rect = MMITHEME_GetWinClientBothRect (win_id);
                 list_rect.h_rect.top = list_rect.h_rect.top + MMI_FULLSCREEN_SOFTKEY_BAR_HEIGHT;
                 list_rect.v_rect.top = list_rect.v_rect.top + MMI_FULLSCREEN_SOFTKEY_BAR_HEIGHT;
                 GUIAPICTRL_SetBothRect (list_ctrl_id, &list_rect);
            }
            else
            {
                 GUILABEL_SetVisible(label_ctrl_id,FALSE,TRUE);
                 list_rect = MMITHEME_GetWinClientBothRect (win_id);
                 GUIAPICTRL_SetBothRect (list_ctrl_id, &list_rect);
            }
#endif
    ...
    case MSG_APP_CANCEL:
    case MSG_CTL_CANCEL:
        if (GUILIST_GetItemState((GUILIST_ITEM_T*) GUILIST_GetItemPtrByIndex (list_ctrl_id, GUILIST_GetCurItemIndex(list_ctrl_id)), GUIITEM_STATE_LEFT_FOCUS))
        {
            MMK_SendMsg(win_id, MSG_APP_RIGHT, PNULL);
            break;
        }
#ifdef MMI_FMM_LIST_SUPPORT
        if(1 == view_win_d->main_d_ptr->s_fmm_list_data_ptr->path_depth && !GUILIST_GetListState(list_ctrl_id,GUILIST_STATE_ENABLE_MARK))
       {
           MMK_CloseWin(win_id);
       }
      else
#endif
       //if (view_win_d->main_d_ptr->s_fmm_list_data_ptr->mark_num > 0)
       if (1 == multi_select_mode)
       {
           HandleMenuCancelMultiSelect(view_win_d->main_d_ptr->s_fmm_list_data_ptr, list_ctrl_id);
           multi_select_mode = 0;
       ...
       else if (2 == multi_select_mode || 3 == multi_select_mode)
       {
           //is_quick_delete = TRUE;
           MMIFMM_DeleteSelectFile (view_win_d->main_d_ptr->s_fmm_list_data_ptr, list_ctrl_id, view_win_d->main_d_ptr->s_fmm_current_path_ptr);
       }
       else
       {
    ...
    case MSG_CTL_MIDSK:
    case MSG_APP_WEB:
//dong.chunguang add foor restore backup files
		if(GUILIST_GetTotalItemNum(list_ctrl_id) > 0)
		{
			...
			char*           	temp_ptr = PNULL;
			uint16          	temp_len = 0;
			uint16 i = 0;//backup in file must be the first
			char  name_str[MMIFILE_FULL_PATH_MAX_LEN + 1] = {0};
			BOOLEAN         is_backup_files = FALSE;
			MMIFMM_FILE_TYPE_E      file_type = MMIFMM_FILE_TYPE_NORMAL;

			view_win_d->main_d_ptr->s_fmm_list_data_ptr->data[index]->type;
		    is_backup_files = MMIFMM_IsDefaultBackupFolder(view_win_d->main_d_ptr->s_fmm_list_data_ptr->pathname,
                                                   view_win_d->main_d_ptr->s_fmm_list_data_ptr->pathname_len);
			if(is_backup_files)
			{
				MMI_WIN_PRIORITY_E      win_priority_dialog = WIN_ONE_LEVEL;
				//MMI_WIN_ID_T     win_id_confirm = MMIIKB_RESTORE_CONFIRM_WIN_ID;
				MMIPUB_OpenConfirmationDialogByTextId(PNULL,STR_NOTE_RESTORE_EXT01, PNULL, IMAGE_NULL, PNULL,&win_priority_dialog, MMIPUB_SOFTKEY_DIALOG_OK,HandleFmmRestoreConfirmWin);
				return TRUE;
			}
		}
        ...
        else if (MMIFMM_FILE_STATE_UNCHECKED == view_win_d->main_d_ptr->s_fmm_list_data_ptr->checked)
        {
#if defined (MAINLCD_SIZE_128X160)
    ...
    case MSG_FMM_DELETE_END_CNF:
      ...
      //  index = view_win_d->s_fmm_path_ptr[view_win_d->main_d_ptr->s_file_path_info_ptr->s_cur_dev - base_dev].reserved_pos;/*lint !e656 */
        ...
        else
        {
            ...
            GUIWIN_SeSoftkeytButtonTextId(win_id, TXT_COMMON_COMMON_BACK, RIGHT_BUTTON, TRUE);
            GUIWIN_SeSoftkeytButtonTextId(win_id, PNULL, LEFT_BUTTON, TRUE);
    ...
    case MSG_FMM_UPDATE_LIST_DATA:
        ...
        //MMIPUB_CloseAlertWin();
        if (MMK_IsOpenWin(MMIFMM_MAIN_WAIT_SPINNER_WIN_ID))
        {
            MMK_CloseWin (MMIFMM_MAIN_WAIT_SPINNER_WIN_ID);
        }
        break;
...
)
{
    ...
    uint16             replace_pos = 0;
    GUILIST_ITEM_DATA_T  item_data = {0};
    GUILIST_ITEM_T            item = {0};
    GUI_COLOR_T         font_color = MMI_WHITE_COLOR;
    MMI_STRING_T memory = {0};
    MMI_STRING_T         title_str = {0};
    wchar  text[64]={0};
    MMI_TEXT_ID_T      list_name[] =
    {
        STR_ILIST_MEMORY_CARD_EXT01,
        STR_CAM_ILIST_MEMORY_PHN_MEMORY,
    };

    MMI_IMAGE_ID_T    list_image[] =
   {
       IMAGE_FMM_MEMORY_CARD,
       IMAGE_FMM_PHONE_MEMORY,
   };
    ...
    case MSG_OPEN_WINDOW:
        ...
        main_d_ptr->s_file_path_info_ptr->sk_text_id.leftsoft_id = STXT_OPTION;
        main_d_ptr->s_file_path_info_ptr->sk_text_id.middlesoft_id = STXT_SOFTKEY_OPEN_MK;
        main_d_ptr->s_file_path_info_ptr->sk_text_id.rightsoft_id = TXT_COMMON_COMMON_BACK;

        MMK_SetAtvCtrl(win_id,MMIFMM_LIST_CTRL_ID);
        GUILIST_SetMaxItem(MMIFMM_LIST_CTRL_ID,MMI_DEVICE_NUM,FALSE);
        GUILIST_SetListState(MMIFMM_LIST_CTRL_ID, GUILIST_STATE_SPLIT_LINE, FALSE);
        GUILIST_SetItemState(&item, GUIITEM_STATE_COSTMER_FONT_COLOR, TRUE);
        GUILIST_SetItemContentCustomFontColor(MMIFMM_LIST_CTRL_ID, 2, &font_color, 255*4/10,&font_color,255*4/10);

        base_index = fmm_dev;
        for (; fmm_dev > MMI_DEVICE_SYSTEM; fmm_dev--)
        {
            SCI_MEMSET (text, 0, sizeof (text));
            memory.wstr_ptr = text;
            if (MMIAPIFMM_GetDeviceTypeStatus (fmm_dev))
            {
                MMIUDISK_GetMemoryDetail(fmm_dev,&memory);
                item.item_style = GUIITEM_STYLE_TWO_LINE_ICON_TEXT_AND_TEXT_MS;
                item.item_data_ptr = &item_data;
                item_data.item_content[0].item_data_type = GUIITEM_DATA_IMAGE_ID;
                item_data.item_content[0].item_data.image_id = list_image[base_index - fmm_dev];
                item_data.item_content[1].item_data_type = GUIITEM_DATA_TEXT_ID;
                item_data.item_content[1].item_data.text_id = list_name[base_index - fmm_dev];
                item_data.item_content[2].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
                item_data.item_content[2].item_data.text_buffer.wstr_ptr = memory.wstr_ptr;
                item_data.item_content[2].item_data.text_buffer.wstr_len= memory.wstr_len;
                GUILIST_AppendItem(MMIFMM_LIST_CTRL_ID,&item);

                main_d_ptr->s_file_path_info_ptr->s_device_table[cur_list_index] = fmm_dev;
                cur_list_index++;
            }
        }
        if (MMK_IsOpenWin(MMIFMM_MAIN_WAIT_SPINNER_WIN_ID))
	...
	if(main_d_ptr->s_fmm_current_path_ptr->pathname_len != 0 && main_d_ptr->s_fmm_current_path_ptr->is_goto_cur_path)
        {
        	MMK_CreateWin((uint32*) MMIFMM_UDISK_CHILD_WIN_TAB,(ADD_DATA) main_d_ptr);
	}
        //update current path
        if (main_d_ptr->s_file_path_info_ptr->user_filename_valid)
        {
        ...
        break;
	case MSG_FULL_PAINT:
        MMI_GetLabelTextByLang(STR_MAINMENU_FILES, &title_str);
        if(MMIAPICOM_StatusAreaSetViewHeaderinfo(title_str .wstr_ptr, title_str .wstr_len))
        {
            GUIWIN_UpdateStb();
        }
    ...
    case MSG_GET_FOCUS:
        ...
        for (; fmm_dev > MMI_DEVICE_SYSTEM; fmm_dev--)
        {
            ...
            if (MMIAPIFMM_GetDeviceTypeStatus (fmm_dev))
            {
                ...
                item_data.item_content[2].item_data.text_buffer.wstr_ptr = memory.wstr_ptr;
                item_data.item_content[2].item_data.text_buffer.wstr_len= memory.wstr_len;
                GUILIST_ReplaceItem (MMIFMM_LIST_CTRL_ID, &item, replace_pos++);
            }
        }
        break;
    case MSG_LOSE_FOCUS:
        break;
    case MSG_CTL_OK:
        MMK_CreateWin((uint32 *)MMFMM_MAIN_DETAIL_MENU_WIN_TAB, (ADD_DATA) main_d_ptr);
        break;
    case MSG_CTL_CANCEL:
		MMK_CloseWin(win_id);
        break;
    case MSG_CLOSE_WINDOW:
        ...
        {
            MMK_CloseWin (MMIFMM_UDISK_CHILD_WIN_ID);
        }
        MMIFMM_FreeFileInfo (main_d_ptr->s_fmm_list_data_ptr, TRUE);
        MainWinCloseCtrlFunc (win_id);
        break;
    case MSG_CTL_MIDSK:
        ...
        cur_list_index = GUILIST_GetCurItemIndex(MMIFMM_LIST_CTRL_ID);
        main_d_ptr->s_file_path_info_ptr->s_cur_dev =  main_d_ptr->s_file_path_info_ptr->s_device_table[cur_list_index];

        SCI_MEMSET (main_d_ptr->s_fmm_current_path_ptr, 0, sizeof (MMIFMM_PATH_INFO_T));
        main_d_ptr->s_fmm_current_path_ptr->pathname_len = FmmInitDevicePath (main_d_ptr->s_file_path_info_ptr->s_cur_dev, main_d_ptr->s_fmm_current_path_ptr->pathname);
        main_d_ptr->s_fmm_current_path_ptr->path_depth = 1;
        MMIAPICOM_Wstrncpy (main_d_ptr->s_fmm_list_data_ptr->pathname, main_d_ptr->s_fmm_current_path_ptr->pathname, main_d_ptr->s_fmm_current_path_ptr->pathname_len);
        main_d_ptr->s_fmm_list_data_ptr->pathname_len = main_d_ptr->s_fmm_current_path_ptr->pathname_len;
        main_d_ptr->s_fmm_list_data_ptr->path_depth = main_d_ptr->s_fmm_current_path_ptr->path_depth;
        ...
        break;
	//dong.chunguang add for restore successful reset
	case MSG_TIMER:
...
)
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        MMK_SetAtvCtrl(win_id, MMIFMM_MAIN_DETAIL_MENU_CTRL_ID);
        break;
    //case MSG_CTL_OK:
    case MSG_APP_OK:
    case MSG_CTL_MIDSK:
        GUIMENU_GetId(MMIFMM_MAIN_DETAIL_MENU_CTRL_ID,&group_id,&menu_id);
        switch (menu_id)
        {
            case ID_FMM_DETAIL:
				MMIFMM_FmmMemoryDetail((uint32) main_d_ptr->s_file_path_info_ptr->s_device_table[GUILIST_GetCurItemIndex(MMIFMM_LIST_CTRL_ID)]);
                MMK_CloseWin(win_id);
                break;
            case ID_FMM_UDISK_FORMAT:
                {
                    MMI_TEXT_ID_T query_text_id = TXT_NULL;
                    if (MMI_DEVICE_UDISK == main_d_ptr->s_file_path_info_ptr->s_device_table[GUILIST_GetCurItemIndex(MMIFMM_LIST_CTRL_ID)])
                    {
                        query_text_id = STR_FORMAT_PHONE_MEMORY;
                    }
                    else if (MMI_DEVICE_SDCARD == main_d_ptr->s_file_path_info_ptr->s_device_table[GUILIST_GetCurItemIndex(MMIFMM_LIST_CTRL_ID)])
                    {
                        query_text_id = STR_FORMAT_QUERY_EXT01;
                    }
                    MMIFMM_FmmMemoryFormat((uint32) main_d_ptr->s_file_path_info_ptr->s_device_table[GUILIST_GetCurItemIndex(MMIFMM_LIST_CTRL_ID)],query_text_id);
                }
                MMK_CloseWin(win_id);
                break;
        }
...
LOCAL void UpdateFileList (MMI_WIN_ID_T win_id, BOOLEAN need_fullpiant)
{
            ...
            || (main_d_ptr->s_file_path_info_ptr->patch_info.pathname_len) > MMIFMM_PATHNAME_LEN)
    {
        if (MMIFMM_PATH_DEPTH < main_d_ptr->s_file_path_info_ptr->patch_info.path_depth)
        {
            alert_text_id = STR_FILE_PATH_MAXIMUM;
        }
        else
        {
...
//  Note:
/*****************************************************************************/
LOCAL void FmmHandleRecordListAppendData (
    MMI_WIN_ID_T         win_id,
    DPARAM param,
    MMI_HANDLE_T ctrl_handle,
    MMIFMM_LIST_ITEM_TYPE_E type
)
{
    ...
    if (PNULL != view_win_d)
    {
        ...
        {
            AppendTwoLineAnimTextAndTextRecordListItem (win_id,
                                                  list_handle,
                                                  view_win_d->main_d_ptr->s_file_path_info_ptr->sk_text_id.leftsoft_id,
                                                  view_win_d->main_d_ptr->s_fmm_list_data_ptr->data[index]->type,
...
#if defined(ZDT_VIDEO_KING_SUPPORT)
LOCAL void FmmHandleMediaListAppendData (
    MMI_WIN_ID_T         win_id,
    DPARAM param,
    MMI_HANDLE_T ctrl_handle,
    MMIFMM_LIST_ITEM_TYPE_E type
...
)
{
    ...
    if (PNULL == view_win_d)
    {
        SCI_TRACE_LOW ("MMIFMM AppendTwoLineAnimTextAndTextRecordListItem PNULL == view_win_d!");
        return;
    }
    ...
    {
        item_t.item_style = GUIITEM_STYLE_TWO_LINE_BIGICON_TEXT_AND_TEXT_2_WITH_LEFT_FOCUS_MS;
        ...
        GUILIST_SetQuickActionIcon(ctrl_id,IMAGE_COMMON_LIST_DISMISS,IMAGE_NULL);
    }
    ...
    else
    {
        ...
        //GUIWIN_SeSoftkeytButtonTextId(win_id, TXT_COMM_OPEN, MIDDLE_BUTTON, TRUE);
        //GUIWIN_SeSoftkeytButtonTextId(win_id, TXT_COMMON_COMMON_BACK, RIGHT_BUTTON, TRUE);

        item_data.lfocus_softkey_id[0] = STXT_OPTION;
        item_data.lfocus_softkey_id[1] = STR_LIST_DELETE;
        item_data.lfocus_softkey_id[2] = TXT_COMMON_COMMON_BACK;
        ...
        if (MMIFMM_FILE_TYPE_MUSIC == file_type)
        {
             ...
             item_data.item_content[0].item_data.text_buffer.wstr_len = path_len - i;
             item_data.item_content[0].item_data.text_buffer.wstr_ptr = &filename[i];
        }
        ...
        if (MMIFMM_FILE_TYPE_MUSIC == file_type)
        {
            ...
            item_data.item_content[1].item_data.text_buffer.wstr_ptr = record_time_wstr;
            item_data.item_content[1].item_data.text_buffer.wstr_len = (uint16) MMIAPICOM_Wstrlen (record_time_wstr);
        }
        ...
        if (MMIFMM_FILE_TYPE_FOLDER != file_type)
        {
            ...
            sprintf ( (char*) temp_wstr, "%.2ld:%.2ld:%.2ld", ( (mp3_file_info.total_time) / (60*60)), ( ( (mp3_file_info.total_time) / 60) % 60), ( (mp3_file_info.total_time) % 60));
            MMIAPICOM_StrToWstr (temp_wstr, time_wstr);

            item_data.item_content[2].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
            item_data.item_content[2].item_data.text_buffer.wstr_ptr = time_wstr;
            item_data.item_content[2].item_data.text_buffer.wstr_len = (uint16) MMIAPICOM_Wstrlen (time_wstr);
        }

	    CTRLLIST_SetTextFont(ctrl_id, MMI_DEFAULT_NORMAL_FONT, MMI_BLACK_COLOR);
...
)
{
    ...
    for (i = 0; i < max_num; i++)
    {
        AppendTwoLineAnimTextAndTextRecordListItem (win_id, list_handle,
                                              left_softkey_id,
...
//  Note:
/*****************************************************************************/
PUBLIC uint32 MMIFMM_GetRecordFileType (void)
{
    MMIFMM_VIEW_WIN_DATA_T   *view_win_d = (MMIFMM_VIEW_WIN_DATA_T*) MMK_GetWinUserData (MMIFMM_RECORD_LIST_WIN_ID);
...
#if defined(ZDT_VIDEO_KING_SUPPORT)
PUBLIC uint32 MMIFMM_GetMediaFileType (void)
{
    MMIFMM_VIEW_WIN_DATA_T   *view_win_d = (MMIFMM_VIEW_WIN_DATA_T*) MMK_GetWinUserData (MMIFMM_MEDIA_LIST_WIN_ID);
    if (PNULL == view_win_d)
    {
        return 0;
    }
...
//  Note:
/*****************************************************************************/
LOCAL void UpdateRecordListData (
    MMI_WIN_ID_T           win_id,
    MMI_CTRL_ID_T          ctrl_id,
    MMIFMM_READ_TYPE_E     type,
    MMI_TEXT_ID_T          sfk_left_id,
...
)
{
	...
	*/
    ...
    if (is_empty)
    {
	  ...
	  if(!InitRecorderFolder(MMIAPIFMM_GetDevicePath (MMI_DEVICE_SDCARD), MMIAPIFMM_GetDevicePathLen (MMI_DEVICE_SDCARD),
                            MMIMULTIM_DIR_RECORDING, MMIAPICOM_Wstrlen (MMIMULTIM_DIR_RECORDING))){
		max_num = 0;
	  }
	  SCI_TRACE_LOW("dcg get input text %s ,max_num:%d",str_welcome, max_num);
    }
    else
    {
	...
	FmmAppendRecordListItem(win_id, ctrl_id,  sfk_left_id, view_win_d->main_d_ptr->s_fmm_list_data_ptr, view_win_d->main_d_ptr->s_fmm_current_path_ptr);
    ...
    {
        empty_info.text_id = STR_RECORDINGS_FOLDER_EMPTY_EXT01;
...
#if defined(ZDT_VIDEO_KING_SUPPORT)
LOCAL void UpdateMediaListData (
    MMI_WIN_ID_T           win_id,
    MMI_CTRL_ID_T          ctrl_id,
    MMIFMM_READ_TYPE_E     type,
    MMI_TEXT_ID_T          sfk_left_id,
...
)
{
    ...
    {
        empty_info.text_id = STR_FILES_FOLDER_EMPTY_EXT01;
        GUILIST_SetEmptyInfo(ctrl_id, &empty_info);
#ifdef MID_SOFTKEY_USE_IMG_SUPPORT
        GUIWIN_SetSoftkeyTextId(win_id, STXT_OPTION, TXT_NULL, STXT_RETURN, FALSE);
		GUIWIN_SeSoftkeytButtonIconId(win_id, IMAGE_NULL, 1, TRUE);
#else
		GUIWIN_SetSoftkeyTextId(win_id, PNULL, STXT_SOFTKEY_OPEN_MK, STXT_RETURN, FALSE);   
...
#endif
/*****************************************************************************/
// Description : open record ctrl func
// Global resource dependence :
// Author:  dong.chunguang
// Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E RecordListViewWinOpenCtrlFunc (MMI_WIN_ID_T win_id)
{
    ...
    if (PNULL != main_win_d)
    {
        ...
        view_win_d->list_ctrl_id = MMIFMM_RECORD_LIST_CTRL_ID;
...
/*****************************************************************************/
// Description : handle record list
// Global resource dependence :
// Author:  dong.chunguang
// Note:
/*****************************************************************************/
...
)
{
    ...
    uint32                      cur_tab = 0;
    uint16                      img_width = 0;
    MMI_CTRL_ID_T               list_ctrl_id = MMIFMM_RECORD_LIST_CTRL_ID;
    MMIFILE_DEVICE_E            base_dev  = MMI_DEVICE_UDISK;
    MMI_STRING_T    			title_str   =  {0};
    GUI_BOTH_RECT_T             list_rect = {0};
    MMIFMM_VIEW_WIN_DATA_T* 	view_win_d = (MMIFMM_VIEW_WIN_DATA_T*) MMK_GetWinUserData (win_id);/*lint !e64 */
    ...
    {
    case MSG_OPEN_WINDOW:
		is_enter_record = TRUE;
        if (MMI_RESULT_FALSE == RecordListViewWinOpenCtrlFunc (win_id))
        {
            MMK_CloseWin (win_id);
            break;
        }
        ...
        MMI_GetLabelTextByLang(STR_MENU_SINGLE_GAL_VOICE_MEMO, &title_str);
        if(MMIAPICOM_StatusAreaSetViewHeaderinfo(title_str .wstr_ptr, title_str .wstr_len))
        {
            GUIWIN_UpdateStb();
        }
		MMK_SetAtvCtrl (win_id, list_ctrl_id);
#ifdef MMI_FMM_LIST_SUPPORT
        UpdateRecordListData (win_id, list_ctrl_id, MMIFMM_READ_TYPE_FILE_DIR, STXT_OPTION, TRUE);
#endif
        break;
    case MSG_CTL_LIST_NEED_ITEM_DATA:
        FmmHandleRecordListAppendData (win_id, param, list_ctrl_id, MMIFMM_NEED_LIST_ITEM_DATA_TYPE);
        break;

    case MSG_CTL_LIST_NEED_ITEM_CONTENT:
        FmmHandleRecordListAppendData (win_id, param, list_ctrl_id, MMIFMM_NEED_LIST_ITEM_CONTENT_TYPE);
        break;
    ...
    case MSG_APP_WEB:
		...
		cur_item = GUILIST_GetCurItemIndex(list_ctrl_id);
		if(GUILIST_GetItemState((GUILIST_ITEM_T*) GUILIST_GetItemPtrByIndex (list_ctrl_id,cur_item), GUIITEM_STATE_LEFT_FOCUS))
		{
           is_quick_delete = TRUE;
           MMIFMM_DeleteSelectFile (view_win_d->main_d_ptr->s_fmm_list_data_ptr, list_ctrl_id, view_win_d->main_d_ptr->s_fmm_current_path_ptr);
		}else
    ...
    case MSG_FMM_SEARCH_CNF:
        ...
        UpdateRecordListData (win_id, list_ctrl_id, MMIFMM_READ_TYPE_FILE_DIR, STXT_OPTION, FALSE);
    ...
    case MSG_FMM_UPDATE_LIST_DATA:
        ...
        UpdateRecordListData (win_id, list_ctrl_id, MMIFMM_READ_TYPE_FILE_DIR,
                        view_win_d->main_d_ptr->s_file_path_info_ptr->sk_text_id.leftsoft_id, TRUE);
    ...
    case MSG_CLOSE_WINDOW:
        {
            is_enter_record = FALSE;
            if (PNULL != view_win_d->main_d_ptr)
            {
                if (PNULL != view_win_d->main_d_ptr->s_fmm_list_data_ptr)
                {
                    MMIFMM_FreeFileInfo (view_win_d->main_d_ptr->s_fmm_list_data_ptr, TRUE);
                }

                if (PNULL != view_win_d->main_d_ptr->s_file_path_info_ptr)
                {
                    SCI_FREE(view_win_d->main_d_ptr->s_file_path_info_ptr);
                    view_win_d->main_d_ptr->s_file_path_info_ptr = PNULL;
                }
                if (PNULL != view_win_d->main_d_ptr->s_fmm_list_data_ptr)
                {
                    SCI_FREE(view_win_d->main_d_ptr->s_fmm_list_data_ptr);
                    view_win_d->main_d_ptr->s_fmm_list_data_ptr = PNULL;
                }
                if (PNULL != view_win_d->main_d_ptr->s_fmm_current_path_ptr)
                {
                    SCI_FREE(view_win_d->main_d_ptr->s_fmm_current_path_ptr);
                    view_win_d->main_d_ptr->s_fmm_current_path_ptr = PNULL;
                }
                
                SCI_FREE(view_win_d->main_d_ptr);
                view_win_d->main_d_ptr = PNULL;
            }
            ListViewWinCloseCtrlFunc (win_id);
            result = MMI_RESULT_FALSE;
        }
...
#if defined(ZDT_VIDEO_KING_SUPPORT)
LOCAL MMI_RESULT_E MediaListViewWinOpenCtrlFunc (MMI_WIN_ID_T win_id)
{
    MMIFMM_MAIN_WIN_DATA_T *main_win_d = (MMIFMM_MAIN_WIN_DATA_T*) MMK_GetWinAddDataPtr (win_id);
    MMIFMM_VIEW_WIN_DATA_T *view_win_d = PNULL;
    if (PNULL != main_win_d)
    {
        ...
        view_win_d->s_list_file_name_ptr = SCI_ALLOC_APPZ (sizeof (wchar) * (MMIFMM_FILENAME_LEN + 1));
        view_win_d->list_ctrl_id = MMIFMM_MEDIA_LIST_CTRL_ID;
        view_win_d->com_ctrl_id = 0;
        view_win_d->tab_ctrl_id = MMIFMM_TAB_CTRL_ID;
    }
    MMK_SetWinUserData (win_id, (void *) view_win_d);
...
LOCAL BOOLEAN MMIFMM_GetPathName (
    wchar *fullpath_ptr,
...
)
{
    int32   i = 0;
    int32   j = 0;
    if(fullpath_ptr == PNULL || full_path_len == 0 || path_ptr == NULL)
    {
        return FALSE;
    }
    for (i = 0; i < full_path_len; i++)
    {
        if (MMIFILE_SLASH == fullpath_ptr[i])
        {
            j = i+1;
        }
    }
    ...
    *p_path_len = full_path_len-j;
    MMIAPICOM_Wstrncpy (path_ptr, &fullpath_ptr[j], full_path_len-j);
    return TRUE;
}
LOCAL MMI_RESULT_E HandleMediaListWinMsg (
    MMI_WIN_ID_T         win_id,
...
)
{
    ...
    uint32                      cur_tab = 0;
    uint16                      img_width = 0;
    MMI_CTRL_ID_T               list_ctrl_id = MMIFMM_MEDIA_LIST_CTRL_ID;
    MMIFILE_DEVICE_E            base_dev  = MMI_DEVICE_UDISK;
    MMI_STRING_T    			title_str   =  {0};
    GUI_BOTH_RECT_T             list_rect = {0};
    MMIFMM_VIEW_WIN_DATA_T* 	view_win_d = (MMIFMM_VIEW_WIN_DATA_T*) MMK_GetWinUserData (win_id);/*lint !e64 */
    ...
    case MSG_OPEN_WINDOW:
		is_enter_media = TRUE;
        if (MMI_RESULT_FALSE == MediaListViewWinOpenCtrlFunc (win_id))
        {
            MMK_CloseWin (win_id);
            break;
        }
        ...
        {
            wchar tmp_name[MMIFMM_PATHNAME_LEN + 1] = {0};
            uint16 tmp_len = 0;
            if(MMIFMM_GetPathName(view_win_d->main_d_ptr->s_file_path_info_ptr->patch_info.pathname,view_win_d->main_d_ptr->s_file_path_info_ptr->patch_info.pathname_len,tmp_name,&tmp_len))
            {
                title_str.wstr_ptr = tmp_name;
                title_str.wstr_len = tmp_len;
            }
            else
            {
                MMI_GetLabelTextByLang(TXT_MEDIA_VIDEO_KING, &title_str);
            }
        }
        if(MMIAPICOM_StatusAreaSetViewHeaderinfo(title_str .wstr_ptr, title_str .wstr_len))
        {
        ...
        GUIWIN_SetTitleText(win_id, title_str.wstr_ptr, title_str.wstr_len, TRUE);
        MMK_SetAtvCtrl (win_id, list_ctrl_id);
#ifdef MMI_FMM_LIST_SUPPORT
        UpdateMediaListData (win_id, list_ctrl_id, MMIFMM_READ_TYPE_FILE_DIR, TXT_NULL, TRUE);
#endif
        break;
    case MSG_CTL_LIST_NEED_ITEM_DATA:
        FmmHandleMediaListAppendData (win_id, param, list_ctrl_id, MMIFMM_NEED_LIST_ITEM_DATA_TYPE);
        break;
    case MSG_CTL_LIST_NEED_ITEM_CONTENT:
        FmmHandleMediaListAppendData (win_id, param, list_ctrl_id, MMIFMM_NEED_LIST_ITEM_CONTENT_TYPE);
        break;
    case MSG_GET_FOCUS:
        {
            wchar tmp_name[MMIFMM_PATHNAME_LEN + 1] = {0};
    ...
    case MSG_FMM_SEARCH_CNF:
        view_win_d->main_d_ptr->s_fmm_list_data_ptr->path_is_valid = TRUE;
        UpdateMediaListData (win_id, list_ctrl_id, MMIFMM_READ_TYPE_FILE_DIR, TXT_NULL, FALSE);
        view_win_d->main_d_ptr->s_fmm_list_data_ptr->state = MMIFMM_FMI_STATE_LOOKUP_FILE;
        MMK_UpdateScreen();
        MMK_CloseWin (MMIFMM_FIND_FILE_WIN_ID);
        MMIPUB_CloseAlertWin();
    ...
    case MSG_FMM_MOVE_END_CNF:
        ...
        break;
    case MSG_FMM_UPDATE_LIST_DATA:
        UpdateMediaListData (win_id, list_ctrl_id, MMIFMM_READ_TYPE_FILE_DIR,
                        view_win_d->main_d_ptr->s_file_path_info_ptr->sk_text_id.leftsoft_id, TRUE);
        if (!view_win_d->main_d_ptr->s_file_path_info_ptr->user_filename_valid)
        {
            index = (uint16) view_win_d->s_fmm_path_ptr[view_win_d->main_d_ptr->s_file_path_info_ptr->s_cur_dev - base_dev].reserved_pos;/*lint !e656 */
									...
									)
{
    uint32              cur_index = 0;
    MMI_RESULT_E        result = MMI_RESULT_TRUE;
    MMI_CTRL_ID_T       ctrl_id = MMIFMM_RECORD_OPT_MENU_CTRL_ID;
    //MMI_CTRL_ID_T       sel_anim_ctrl_id = MMIPICVIEW_SEL_PICTURE_CTRL_ID;
    MMI_CTRL_ID_T       list_ctrl_id = MMIFMM_RECORD_LIST_CTRL_ID;
    MMI_MENU_ID_T       menu_id = 0;
    MMI_MENU_GROUP_ID_T group_id = 0;
	MMI_STRING_T  		string_title = {0};
	MMI_WIN_ID_T		list_win_id = MMIFMM_RECORD_LIST_WIN_ID;
	MMIFMM_VIEW_WIN_DATA_T* 	view_win_d = (MMIFMM_VIEW_WIN_DATA_T*) MMK_GetWinUserData (list_win_id);

    switch (msg_id)
    {
    case MSG_OPEN_WINDOW:
	    //GUIMENU_SetPopMenuCentered(ctrl_id, TRUE);
		//MMI_GetLabelTextByLang(TXT_FMM_SEND_SHARE,&string_title);
		//GUIMENU_SetMenuTitle(&string_title,ctrl_id);
#ifndef BLUETOOTH_SUPPORT
	  //if(TRUE == MMIAPISET_IsDisableShareViaOption())
        {
            CTRLMENU_SetItemVisible(ctrl_id,MENU_FMM_RECORD_LIST_OPT,ID_FMM_RECORD_SHARE,FALSE);
        }
#endif
        if(view_win_d->main_d_ptr->s_fmm_list_data_ptr->file_num == 1)
        {
            CTRLMENU_SetItemVisible(ctrl_id,MENU_FMM_RECORD_LIST_OPT,ID_FMM_RECORD_DELETE_ALL,FALSE);
        }
    ...
    case MSG_CTL_PENOK:
	    ...
	    switch (menu_id)
	    {
		case ID_FMM_RECORD_SHARE:
			MMK_CreateWin((uint32 *)MMIFMM_SHARE_ITEMS_RECORD_LIST_OPT_WIN_TAB,PNULL);
			break;
		case ID_FMM_RECORD_RENAME:
			MMIFMM_FmmRenameFile (GUILIST_GetCurItemIndex (list_ctrl_id), list_win_id, MSG_FMM_RENAME_END_CNF, view_win_d->main_d_ptr->s_fmm_list_data_ptr, view_win_d->s_list_file_name_ptr);
			MMK_CloseWin(win_id);
			break;
		case ID_FMM_RECORD_DELETE:
			MMIFMM_DeleteAllFiles(view_win_d->main_d_ptr->s_fmm_list_data_ptr, list_ctrl_id, view_win_d->main_d_ptr->s_fmm_current_path_ptr, TRUE);
			MMK_CloseWin(win_id);
			break;
		case ID_FMM_RECORD_DELETE_ALL:
			MMIFMM_DeleteAllFiles(view_win_d->main_d_ptr->s_fmm_list_data_ptr, list_ctrl_id, view_win_d->main_d_ptr->s_fmm_current_path_ptr, FALSE);
			MMK_CloseWin(win_id);
			break;
	    }
		break;
									...
									)
{
    uint32              cur_index = 0;
    MMI_RESULT_E        result = MMI_RESULT_TRUE;
    MMI_CTRL_ID_T       ctrl_id = MMIFMM_RECORD_SHARE_OPT_MENU_CTRL_ID;
    //MMI_CTRL_ID_T       sel_anim_ctrl_id = MMIPICVIEW_SEL_PICTURE_CTRL_ID;
    MMI_CTRL_ID_T       list_ctrl_id = MMIFMM_RECORD_LIST_CTRL_ID;
    MMI_MENU_ID_T       menu_id = 0;
    MMI_MENU_GROUP_ID_T group_id = 0;
	...
	MMI_WIN_ID_T		list_win_id = MMIFMM_RECORD_LIST_WIN_ID;
	MMIFMM_VIEW_WIN_DATA_T* 	view_win_d = (MMIFMM_VIEW_WIN_DATA_T*) MMK_GetWinUserData (list_win_id);
	//LOCAL MMIFILE_FILE_INFO_T      *file_info_ptr = PNULL;
    switch (msg_id)
    {
    case MSG_OPEN_WINDOW:
	    ...
	    GUIMENU_SetPopMenuCentered(ctrl_id, TRUE);
		MMI_GetLabelTextByLang(STR_LIST_SHARE,&string_title);
		GUIMENU_SetMenuTitle(&string_title,ctrl_id);
#ifdef MMI_ORIG_UNISOC_SUPPORT
        if(MMIFMM_IsChinaVersion())
#else
        if (TRUE)
#endif
        {
       #ifdef     MMIEMAIL_SUPPORT 
            CTRLMENU_SetItemVisible(ctrl_id,MENU_FMM_RECORD_SEND_ITEMS,ID_FMM_RECORD_SEND_ITEMS_BY_EMAIL,FALSE);
	#endif
        }
        MMK_SetAtvCtrl(win_id,ctrl_id);
        //CTRLANIM_SetVisible(sel_anim_ctrl_id, FALSE, FALSE);
    ...
    case MSG_CTL_PENOK:
	    ...
	    switch (menu_id)
	    {
	    case ID_FMM_RECORD_SEND_ITEMS_BY_MMS:
#if defined (MMS_SUPPORT)
			//MMIRECORD_SendFileViaMMS(view_win_d->main_d_ptr->s_fmm_current_path_ptr->pathname,view_win_d->main_d_ptr->s_fmm_current_path_ptr->pathname_len);
            MMIFMM_FmmSendMMS (view_win_d->main_d_ptr->s_fmm_list_data_ptr, view_win_d->main_d_ptr->s_fmm_current_path_ptr, GUILIST_GetCurItemIndex (MMIFMM_RECORD_LIST_CTRL_ID));
#endif
			MMK_CloseWin(win_id);
			MMK_CloseWin(MMIFMM_RECORD_OPT_MENU_WIN_ID);
			break;
#if defined (BLUETOOTH_SUPPORT)
	    case ID_FMM_RECORD_SEND_ITEMS_BY_BT:
			//MMIRECORD_SendFileViaBT(view_win_d->main_d_ptr->s_fmm_current_path_ptr->pathname,view_win_d->main_d_ptr->s_fmm_current_path_ptr->pathname_len);
#ifdef FM_SUPPORT
                    if(MMIAPIFM_IsFmActive())
                    {
                        SCI_TRACE_LOW("mmifmm_mainwin.c:HandleShareItemsRecordlistOptWinMsg FM is avtiving!!");
                        MMIPUB_OpenAlertWarningWin(TXT_EXIT_BACKGROUND_FM);
                    }
                    else
                    {
                        MMIFMM_FmmBtSendFile (view_win_d->main_d_ptr->s_fmm_list_data_ptr, view_win_d->main_d_ptr->s_fmm_current_path_ptr, GUILIST_GetCurItemIndex (MMIFMM_RECORD_LIST_CTRL_ID));
                    }
#else
                    MMIFMM_FmmBtSendFile (view_win_d->main_d_ptr->s_fmm_list_data_ptr, view_win_d->main_d_ptr->s_fmm_current_path_ptr, GUILIST_GetCurItemIndex (MMIFMM_RECORD_LIST_CTRL_ID));
#endif
...
#ifdef MMIEMAIL_SUPPORT			
		case ID_FMM_RECORD_SEND_ITEMS_BY_EMAIL:
			//need email api
/*#if defined MMI_VCARD_SUPPORT && defined MMIEMAIL_SUPPORT
            if(MMIAPIFMM_GetFileInfoFormFullPath(view_win_d->main_d_ptr->s_fmm_current_path_ptr->pathname, view_win_d->main_d_ptr->s_fmm_current_path_ptr->pathname_len, file_info_ptr))  //should check the size
            {
                MMIAPIEMAIL_EditEMAILFromFile(file_info_ptr);
            }
#endif*/
			MMIFMM_FmmSendEmailByOutlook(view_win_d->main_d_ptr->s_fmm_list_data_ptr, view_win_d->main_d_ptr->s_fmm_current_path_ptr, GUILIST_GetCurItemIndex (MMIFMM_RECORD_LIST_CTRL_ID));
			MMK_CloseWin(win_id);
			MMK_CloseWin(MMIFMM_RECORD_OPT_MENU_WIN_ID);
		 	break;
#endif
...
#if defined(ZDT_VIDEO_KING_SUPPORT)
PUBLIC BOOLEAN CreateExplorerMediaWin (MMIFMM_MAIN_WIN_DATA_T *main_win_p)
{
#ifndef MMI_FMM_LIST_SUPPORT
    MMK_CreateParentWin ( (uint32 *) MMIFMM_MAIN_WIN_TAB, (ADD_DATA) main_win_p);
#else
    SCI_TRACE_LOW("CreateExplorerRecordWin-%d", MMK_IsOpenWin(MMIFMM_MEDIA_LIST_WIN_ID));
    if(MMK_IsOpenWin(MMIFMM_MEDIA_LIST_WIN_ID))
    {
        MMK_CloseWin(MMIFMM_MEDIA_LIST_WIN_ID);
    }
    MMK_CreateWin ( (uint32 *) MMIFMM_MEDIA_WIN_TAB, (ADD_DATA) main_win_p);
#endif
    return TRUE;
}
#endif
/*****************************************************************************/
//  Description : create record exploer windows
//  Global resource dependence : none
//  Author: dong.chunguang
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN CreateExplorerRecordWin (MMIFMM_MAIN_WIN_DATA_T *main_win_p)
{
	//DPARAM param = SCI_ALLOC_APPZ(sizeof(uint8)*(MMIFILE_FILE_NAME_MAX_LEN + 1));	
    //ATC_ExtCmd(param);
#ifndef MMI_FMM_LIST_SUPPORT
    MMK_CreateParentWin ( (uint32 *) MMIFMM_MAIN_WIN_TAB, (ADD_DATA) main_win_p);
#else
    SCI_TRACE_LOW("CreateExplorerRecordWin-%d", MMK_IsOpenWin(MMIFMM_RECORD_LIST_WIN_ID));
    if(MMK_IsOpenWin(MMIFMM_RECORD_LIST_WIN_ID))
    {
        MMK_CloseWin(MMIFMM_RECORD_LIST_WIN_ID);
    }
    MMK_CreateWin ( (uint32 *) MMIFMM_RECORD_WIN_TAB, (ADD_DATA) main_win_p);
#endif
    //MMIPUB_OpenAlertWinByTextId (PNULL, TXT_COMMON_WAITING, TXT_NULL, IMAGE_PUBWIN_WAIT, PNULL, PNULL, MMIPUB_SOFTKEY_NONE, PNULL);
...
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIFMM_OpenExplorerRecordWin (MMIFMM_OPENWIN_INFO_T * openwin_info)
{
    BOOLEAN result = FALSE;
    ...
    else
    {
        ...
        if (MMI_DEVICE_NUM > fs_dev)
        {
            ...
            if (PNULL == main_win_p->s_file_path_info_ptr)
            {
                ...
                main_win_p->s_fmm_current_path_ptr = PNULL;
                main_win_p = PNULL;
                SCI_TRACE_LOW ("OpenFmmMainExplorer:PNULL == main_win_p->s_file_path_info_ptr!");
                return FALSE;
            }
               ...
               )
            {
                ...
                main_win_p->s_fmm_current_path_ptr->is_goto_cur_path = openwin_info->is_goto_cur_path;
            ...
            if (FmmCheckMemoryCardState())
            {
                ...
                {
                    CreateExplorerRecordWin(main_win_p);
                }
            }
            else
            {
...
#if defined(ZDT_VIDEO_KING_SUPPORT)
PUBLIC BOOLEAN MMIFMM_OpenExplorerMediaWin (MMIFMM_OPENWIN_INFO_T * openwin_info)
{
    BOOLEAN result = FALSE;
    if (PNULL == openwin_info)
    {
    ...
    else
    {
        ...
        if (MMI_DEVICE_NUM > fs_dev)
        {
            ...
            if (FmmCheckMemoryCardState())
            {
                ...
                {
                    MMIAPISET_ValidatePrivacy (MMISET_PROTECT_MYDOC, FMM_ValidatePrivacyFmmDoc, (ADD_DATA) main_win_p, sizeof (MMIFMM_MAIN_WIN_DATA_T));
                    SCI_FREE(main_win_p->s_file_path_info_ptr);
                    SCI_FREE(main_win_p->s_fmm_list_data_ptr);
                    SCI_FREE(main_win_p->s_fmm_current_path_ptr);
                    SCI_FREE(main_win_p);
                    main_win_p->s_file_path_info_ptr = PNULL;
                    main_win_p->s_fmm_list_data_ptr = PNULL;
                    main_win_p->s_fmm_current_path_ptr = PNULL;
                    main_win_p = PNULL;
                }
                ...
                {
                    CreateExplorerMediaWin(main_win_p);
                }
            }
            else
            {
...
)
{
    ...
    else
    {
		...
		GUIWIN_SeSoftkeytButtonIconId(win_id, IMAGE_NULL, 1, TRUE);
#else
		GUIWIN_SetSoftkeyTextId(win_id, PNULL, STXT_SOFTKEY_OPEN_MK, TXT_COMMON_COMMON_BACK, FALSE);   
    ...
    if (PNULL != view_win_d && PNULL != list_param_ptr)
    {
        ...
        if (list_param_ptr->had_maked)
        {
            if (MMIFMM_FILE_STATE_UNCHECKED == view_win_d->main_d_ptr->s_fmm_list_data_ptr->data[list_param_ptr->item_idx]->state)
            {
                ...
                if (MMIFMM_FILE_TYPE_FOLDER == view_win_d->main_d_ptr->s_fmm_list_data_ptr->data[list_param_ptr->item_idx]->type)
                {
                    folder_mark_num++;
                }
                else
                ...
                head_title_str .wstr_ptr = text;
                MMIAPICOM_CustomGenerateNumReplaceStrByTextId(STR_GAL_HDR_DELETE_NUM_EXT01, L"%N",file_mark_num + folder_mark_num,32,&head_title_str);
                if(MMIAPICOM_StatusAreaSetViewHeaderinfo(head_title_str .wstr_ptr, head_title_str .wstr_len))
                {
                    GUIWIN_UpdateStb();
                }
        ...
        else
        {
            if (MMIFMM_FILE_STATE_CHECKED == view_win_d->main_d_ptr->s_fmm_list_data_ptr->data[list_param_ptr->item_idx]->state)
            {
                ...
                head_title_str .wstr_ptr = text;
                MMIAPICOM_CustomGenerateNumReplaceStrByTextId(STR_GAL_HDR_DELETE_NUM_EXT01, L"%N",file_mark_num + folder_mark_num,32,&head_title_str);
                if (0 == (file_mark_num + folder_mark_num))
                {
                    MMI_GetLabelTextByLang(STR_LIST_DELETE, &head_title_str);
                }
                if(MMIAPICOM_StatusAreaSetViewHeaderinfo(head_title_str .wstr_ptr, head_title_str .wstr_len))
                {
                    GUIWIN_UpdateStb();
        ...
        else if (view_win_d->main_d_ptr->s_fmm_list_data_ptr->mark_num == (view_win_d->main_d_ptr->s_fmm_list_data_ptr->file_num + view_win_d->main_d_ptr->s_fmm_list_data_ptr->folder_num))
        {
            multi_select_mode = 2;
        }
        else
...
#endif
#ifdef DRM_SUPPORT
    if ((MMIFMM_TXT_ALL | MMIFMM_DRM_ALL) == MMIFMM_GetDisplayFileType())
#else
   if (MMIFMM_TXT_ALL == MMIFMM_GetDisplayFileType())
#endif
    {
        MMK_CloseParentWin (MMIFMM_MAIN_WIN_ID);
        return;
    ...
    else
    {
        ...
        if (view_win_d->main_d_ptr->s_fmm_current_path_ptr->path_depth > 1)
        {
...
#ifdef MMI_FMM_TITLE_LABEL_SUPPORT
            {
                 ...
                 for (i = view_win_d->main_d_ptr->s_fmm_current_path_ptr->pathname_len - 1; MMIFILE_SLASH != view_win_d->main_d_ptr->s_fmm_current_path_ptr->pathname[i] && i > 0; i--)
                 {
                     NULL;
                 }
                 label_str.wstr_ptr = &view_win_d->main_d_ptr->s_fmm_current_path_ptr->pathname[i + 1];
                 label_str.wstr_len = (view_win_d->main_d_ptr->s_fmm_current_path_ptr->pathname_len) - i - 1;
                 GUILABEL_SetText(MMIFMM_UDISK_LABEL_CTRL_ID,&label_str,TRUE);
            }
#endif
            // 更新数据
            is_flush = TRUE;
        ...
        else if (is_two_line && MMIFMM_FILE_TYPE_DRM == file_type)
        {
            ...
            if (SFS_INVALID_HANDLE != file_handle)
            {
                ...
                else if (MMIFMM_FILE_TYPE_MOVIE == MMIAPIDRM_GetMediaFileType (file_handle, PNULL))
                {
                    ...
                    {
                        icon_id = IMAGE_DRM_VIDEO_UNLOCK_ICON;
                    }
                    else
                    {
                        icon_id = IMAGE_DRM_VIDEO_LOCK_ICON;
                    }
                }
                ...
                {
                    icon_id = MMIFMM_GetIconIdByFileName (view_win_d->s_list_file_name_ptr, full_name_len);//IMAGE_DRM_PIC_UNKNOW_ICON;
                }
            }
            else
            {
...
)
{
    ...
    //MMISRVAUD_CONTENT_INFO_T        movie_file_info = {0};
    BOOLEAN         is_backup_files = FALSE;
    BOOLEAN         is_recording_files = FALSE;
    BOOLEAN         is_received_files = FALSE;
    ...
    /*Quick Action*/
    if (!GUILIST_GetListState(ctrl_id,GUILIST_STATE_ENABLE_MARK) && MMIFMM_FILE_TYPE_FOLDER != file_type)
    {
        item_t.item_style = GUIITEM_STYLE_TWO_LINE_BIGICON_TEXT_AND_TEXT_2_WITH_LEFT_FOCUS_MS;
        item_t.left_item_style = GUIITEM_STYLE_2TEXT_AND_TEXT_WITH_LEFT_FOCUS_MS;
        //GUILIST_SetShowLRTriangleIcon(ctrl_id, TRUE, FALSE);
        //GUILIST_SetHandleLeftRightEx(ctrl_id, TRUE, FALSE);
        //GUILIST_SetQuickActionIcon(ctrl_id,IMAGE_COMMON_LIST_DISMISS,IMAGE_NULL);
        //GUILIST_SetItemShowLRTriangleIconByIndex(ctrl_id, item_index, TRUE, FALSE); // 画高亮 item 左边会有一个小圆
        GUILIST_SetItemHandleLeftRightByIndex(ctrl_id,item_index, TRUE, FALSE);
        GUILIST_SetItemQuickIconByIndex(ctrl_id, item_index, IMAGE_COMMON_LIST_DISMISS, IMAGE_NULL);
    }
    else if (GUILIST_GetListState(ctrl_id,GUILIST_STATE_ENABLE_MARK))
    {
        item_t.item_style = GUIITEM_STYLE_MULTI_CHOICE_PICKER_TWO_LINE_BIGICON_TEXT_AND_TEXT;
    }
    ...
    is_recording_files = MMIFMM_IsDefaultRecordingsFolder(view_win_d->main_d_ptr->s_fmm_list_data_ptr->pathname,
                                                          view_win_d->main_d_ptr->s_fmm_list_data_ptr->pathname_len);

    is_received_files = MMIFMM_IsDefaultReceivedFolder(view_win_d->main_d_ptr->s_fmm_list_data_ptr->pathname,
                                                       view_win_d->main_d_ptr->s_fmm_list_data_ptr->pathname_len);

    is_two_line = TRUE;

    if (MMIFMM_FILE_TYPE_FOLDER == file_type && !is_backup_files)
    {
        //folder one text
        if (!GUILIST_GetListState(ctrl_id,GUILIST_STATE_ENABLE_MARK))
        {
            item_t.item_style = GUIITEM_STYLE_ONE_LINE_ICON_TEXT_MS;
            //item_t.left_item_style = GUIITEM_STYLE_ONE_LINE_BIGICON_TEXT_WITH_LEFT_FOCUS_MS;
        }
        else if (GUILIST_GetListState(ctrl_id,GUILIST_STATE_ENABLE_MARK))
        {
            item_t.item_style = GUIITEM_STYLE_ONE_LINE_CHECK_ICON_MS;
        }
    }
	else if(is_two_line && (MMIFMM_FILE_TYPE_PICTURE == file_type
#ifdef  VIDEOTHUMBNAIL_SUPPORT  
	...
	{ //双行情形：pic缩略图，movie缩略图，DRM
        if (!GUILIST_GetListState(ctrl_id,GUILIST_STATE_ENABLE_MARK))
        {
            item_t.item_style = GUIITEM_STYLE_TWO_LINE_BIGICON_TEXT_AND_2TEXT_MS;
            item_t.left_item_style = GUIITEM_STYLE_TWO_LINE_BIGICON_TEXT_AND_TEXT_2_WITH_LEFT_FOCUS_MS;
        }
        else if (GUILIST_GetListState(ctrl_id,GUILIST_STATE_ENABLE_MARK))
        {
            item_t.item_style = GUIITEM_STYLE_MULTI_CHOICE_PICKER_TWO_LINE_BIGICON_TEXT_AND_TEXT;
        ...
        GUILIST_SetItemContentCustomFontColor(ctrl_id, 2, &font_color, 255*4/10,&font_color,255*4/10);
        GUILIST_SetItemContentCustomFontColor(ctrl_id, 3, &font_color, 255*4/10,&font_color,255*4/10);
	}
    else
    {
        //单行显示
    ...
    if (MMIFMM_FILE_TYPE_MUSIC == file_type || is_backup_files || is_recording_files || (is_received_files  && MMIFMM_FILE_TYPE_PICTURE != file_type && MMIFMM_FILE_TYPE_MOVIE != file_type))
    {
         item_t.item_style = GUIITEM_STYLE_TWO_LINE_TEXT_AND_2TEXT_MS;
    ...
    else
    {
        ...
        if (MMIFMM_FILE_TYPE_JAVA == file_type)
        {
			...
			item_data.softkey_id[1] = IMAGE_COMMON_SOFTKEY_LOCALINSTALL_ICON;
#else
			item_data.softkey_id[1] = STR_SOFTK_INSTALL;
#endif
        }
        ...
        {
            MMIFMM_FmmVideoThumbnailGetPathname (thumbnail_name, &thumbnail_name_len, view_win_d->s_list_file_name_ptr);
            MMIFMM_FmmVideoTotaltimeGetPathname(totaltime_name,&total_name_len,view_win_d->s_list_file_name_ptr);

            if (!MMIAPIFMM_IsFileExist (thumbnail_name, thumbnail_name_len) || !MMIAPIFMM_IsFileExist (totaltime_name, total_name_len))
            {
// 				MMIFILE_HANDLE file_handle = MMIAPIFMM_CreateFile(thumbnail_name, SFS_MODE_WRITE|SFS_MODE_OPEN_ALWAYS, NULL, NULL);
// 				if(SFS_INVALID_HANDLE != file_handle)
// 				{
...
// 				}
        ...
        {
            DPLAYER_MEDIA_INFO_T media_info;
            item_data.item_content[0].item_data_type     = GUIITEM_DATA_IMAGE_ID;
            item_data.item_content[0].item_data.image_id = MMIAPIFMM_GetIconIdByFileType (file_type, MMI_DEVICE_UDISK);
            
            if(MMIAPIVP_GetInfoFromFullPathName(view_win_d->s_list_file_name_ptr, MMIAPICOM_Wstrlen (view_win_d->s_list_file_name_ptr), &media_info))
            {
...
//#ifdef  VIDEOTHUMBNAIL_SUPPORT
                else if (MMIFMM_FILE_TYPE_MOVIE == MMIAPIDRM_GetMediaFileType (file_handle, PNULL))
                {
                    ...
                    if (MMIAPIDRM_IsRightsValid (file_handle, PNULL, TRUE, DRM_PERMISSION_PLAY))
                    {
                        icon_id = IMAGE_DRM_VIDEO_UNLOCK_ICON;//IMAGE_DRM_MOVIE_UNLOCK_ICON;
                    }
                    else
                    {
                        icon_id = IMAGE_DRM_VIDEO_LOCK_ICON;//IMAGE_DRM_MOVIE_LOCK_ICON;
                    }
                }
...
//            item_data.item_content[0].item_data.image_id = IMAGE_FMM_FOLDER_2L_ICON;
//        }
        else if (MMIFMM_FILE_TYPE_MUSIC != file_type || !is_backup_files)
        {
            item_data.item_content[0].item_data_type     = GUIITEM_DATA_IMAGE_ID;
#ifdef DRM_SUPPORT
        ...
        //add for new style of list item
        //remove extension
        if (MMIFMM_FILE_TYPE_MUSIC == file_type)
        {
             item_data.item_content[0].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
        ...
        if (is_backup_files || is_recording_files)
        {
             item_data.item_content[0].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
             item_data.item_content[0].item_data.text_buffer.wstr_len = list_name_len;
             item_data.item_content[0].item_data.text_buffer.wstr_ptr = list_file_name;
        }

        if (is_received_files && MMIFMM_FILE_TYPE_PICTURE != file_type && MMIFMM_FILE_TYPE_MOVIE != file_type)
        {
            MMIAPIFMM_SplitFullPathExt(filename,
                filename_len,
                PNULL,&path_len,
            ...
            item_data.item_content[0].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
            item_data.item_content[0].item_data.text_buffer.wstr_len = path_len;
            item_data.item_content[0].item_data.text_buffer.wstr_ptr = &filename[0];
        }
        //End
        ...
        {
             item_data.item_content[1].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
             item_data.item_content[1].item_data.text_buffer.wstr_len = suffix_len - 1;
             item_data.item_content[1].item_data.text_buffer.wstr_ptr = &filename[list_name_len - suffix_len + 1];
        }

        if (is_backup_files)
        {
             item_data.softkey_id[1] = STXT_SOFTKEY_RESTORE_MK;
             item_data.item_content[1].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
             item_data.item_content[1].item_data.text_buffer.wstr_len = 11;
             item_data.item_content[1].item_data.text_buffer.wstr_ptr = L"Backup file";
        }
        ...
        if (is_received_files && MMIFMM_FILE_TYPE_PICTURE != file_type && MMIFMM_FILE_TYPE_MOVIE != file_type)
        {
             ...
             //sprintf(detail_arr,"%s", "PDF file");
             //MMIAPICOM_StrToWstr((const uint8 *)detail_arr, wchar_detail_arr);
             GetFileTypeBySuffix(filename,filename_len,wchar_detail_arr);
             item_data.item_content[1].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
             item_data.item_content[1].item_data.text_buffer.wstr_len = (uint16) MMIAPICOM_Wstrlen (wchar_detail_arr);
             item_data.item_content[1].item_data.text_buffer.wstr_ptr = wchar_detail_arr;
        }
        //End

        if (MMIFMM_FILE_TYPE_FOLDER != file_type || is_backup_files || is_received_files)
        {
            /* text 2 */
            //MMIFMM_GetFileInfo(view_win_d->s_list_file_name_ptr, full_name_len, &time, &size );
            ...
            if (is_backup_files || is_received_files)
            {
                GetBackupFolderSize(&detail_data);
            }

            //item_data.item_content[2].item_data.text_buffer.is_ucs2 = FALSE;
            MMIAPIFMM_GetFileSizeString (detail_data.file_size,
                                         temp_wstr,
                                         MMIFMM_SIZE_STR_LEN,
                                         TRUE);
            ...
            if (MMIFMM_FILE_TYPE_PICTURE == file_type)
           {
                ...
                item_data.item_content[2].item_data.text_buffer.wstr_ptr = wchar_detail_arr;
                item_data.item_content[2].item_data.text_buffer.wstr_len = (uint16) MMIAPICOM_Wstrlen (wchar_detail_arr);

                //set picture size
                item_data.item_content[3].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
                item_data.item_content[3].item_data.text_buffer.wstr_ptr = temp_wstr;
                item_data.item_content[3].item_data.text_buffer.wstr_len = (uint16) MMIAPICOM_Wstrlen (temp_wstr);
           }
...
)
{
    ...
    list_data_ptr->mark_num = 0;
    is_received_files = MMIFMM_IsDefaultReceivedFolder(list_data_ptr->pathname,list_data_ptr->pathname_len);
    is_backup_files = MMIFMM_IsDefaultBackupFolder(list_data_ptr->pathname,list_data_ptr->pathname_len);
    is_recording_files = MMIFMM_IsDefaultRecordingsFolder(list_data_ptr->pathname,list_data_ptr->pathname_len);

    for (i = 0; i < (list_data_ptr->file_num + list_data_ptr->folder_num); i++)
    {
        list_data_ptr->data[i]->state = MMIFMM_FILE_STATE_UNCHECKED;
        GUILIST_SetSelectedItem (ctrl_id, i, FALSE);
        if (MMIFMM_FILE_TYPE_FOLDER != list_data_ptr->data[i]->type)
        {
            //GUILIST_SetItemShowLRTriangleIconByIndex(ctrl_id, i, TRUE, FALSE);
            GUILIST_SetItemHandleLeftRightByIndex(ctrl_id,i, TRUE, FALSE);
            GUILIST_SetItemQuickIconByIndex(ctrl_id, i, IMAGE_COMMON_LIST_DISMISS, IMAGE_NULL);
        }

#if defined (MAINLCD_SIZE_128X160)
        if (MMIFMM_FILE_TYPE_PICTURE == list_data_ptr->data[i]->type)
        {
            GUILIST_SetItemStyle(ctrl_id, GUIITEM_STYLE_TWO_LINE_ANIM_TEXT_AND_TEXT, i);
        }
        else if (MMIFMM_FILE_TYPE_MUSIC == list_data_ptr->data[i]->type || MMIFMM_FILE_TYPE_MOVIE == list_data_ptr->data[i]->type)
        {
            GUILIST_SetItemStyle(ctrl_id, GUIITEM_STYLE_ONE_LINE_ICON_TWO_TEXT, i);
        }
        else
        {
            GUILIST_SetItemStyle(ctrl_id, GUIITEM_STYLE_ONE_LINE_ICON_TWO_TEXT, i);
        ...
        //Received folder
        if (MMIFMM_FILE_TYPE_PICTURE != list_data_ptr->data[i]->type
            && MMIFMM_FILE_TYPE_MOVIE != list_data_ptr->data[i]->type
            && is_received_files)
        {
            GUILIST_SetItemStyle(ctrl_id, GUIITEM_STYLE_ONE_LINE_ICON_TEXT, i);
        }

        //Backup folder/Voice Recording folder
        if (is_backup_files || is_recording_files)
        {
            GUILIST_SetItemStyle(ctrl_id, GUIITEM_STYLE_ONE_LINE_ICON_TEXT, i);
        }
        ...
        if (MMIFMM_FILE_TYPE_PICTURE == list_data_ptr->data[i]->type
            || MMIFMM_FILE_TYPE_MOVIE == list_data_ptr->data[i]->type)
        {
            GUILIST_SetItemStyle(ctrl_id, GUIITEM_STYLE_TWO_LINE_BIGICON_TEXT_AND_2TEXT_MS, i);
        }
        else if (MMIFMM_FILE_TYPE_MUSIC == list_data_ptr->data[i]->type)
        {
            GUILIST_SetItemStyle(ctrl_id, GUIITEM_STYLE_TWO_LINE_TEXT_AND_2TEXT_MS, i);
        }
        else
        {
            GUILIST_SetItemStyle(ctrl_id, GUIITEM_STYLE_ONE_LINE_ICON_TEXT_MS, i);
        }
...
)
{
    ...
    MMI_WIN_ID_T     conform_win_id  = MMIFMM_CONFORM_DIALOG_WIN;
    MMIFMM_VIEW_WIN_DATA_T   *view_win_d = (MMIFMM_VIEW_WIN_DATA_T*) MMK_GetWinUserData (win_id);
    ...
    else
    {
        ...
        {   // 保存当前的路径
            ...
            {
                MMI_STRING_T    label_str = {0};
                label_str.wstr_ptr = view_win_d->main_d_ptr->s_fmm_list_data_ptr->data[index]->filename_ptr;
                label_str.wstr_len = view_win_d->main_d_ptr->s_fmm_list_data_ptr->data[index]->filename_len;
                GUILABEL_SetText(MMIFMM_UDISK_LABEL_CTRL_ID,&label_str,TRUE);
                MMK_PostMsg (win_id, MSG_FULL_PAINT, PNULL, 0);
            }
#endif
...
)
{
    ...
    if (list_data_ptr->mark_num >= MMIFMM_FILE_NUM)
    {
       FILEARRAY file_array = NULL;
       MMIFMM_FILTER_T filter = {"*"};

       file_array = MMIAPIFILEARRAY_Create();
       MMIFMM_SearchFileInPathNoSort(list_data_ptr->pathname, MMIAPICOM_Wstrlen(list_data_ptr->pathname),
           &filter, TRUE, FUNC_FIND_FILE, file_array, 0, 0);
       actual_file_mark_num = MMIVIRTUALARRAY_GetArraySize(file_array);
...
)
{
    ...
    else
    {
        ...
        else if (0 == multi_select_mode)//non_multi_select mode
        {
...
#endif
            {
                ...
                case MMIFMM_FILE_TYPE_PICTURE:
                    ...
                    MMIFMM_OpenMeunPubWin (win_id, MENU_FMM_FILES_LIST_OPT, MMIFMM_FILES_OPT_MENU_WIN_ID, MMIFMM_FILES_OPT_MENU_CTRL_ID);
                    break;

                case MMIFMM_FILE_TYPE_MUSIC:
					//dong.chunguang add
					if(is_enter_record == TRUE)
					{
						//MMIFMM_OpenMeunPubWin (win_id, MENU_FMM_RECORD_LIST_OPT, MMIFMM_RECORD_OPT_MENU_WIN_ID, MMIFMM_RECORD_OPT_MENU_CTRL_ID);
						MMK_CreateWin((uint32 *)MMIFMM_RECORD_LIST_OPT_WIN_TAB,PNULL);
						break;
					}
					//end
                ...
                case MMIFMM_FILE_TYPE_FOLDER:
                    ...
                    MMIFMM_OpenMeunPubWin (win_id, MENU_FMM_FOLDER_LIST_OPT, MMIFMM_FOLDER_OPT_MENU_WIN_ID, MMIFMM_FOLDER_OPT_MENU_CTRL_ID);
                    break;
                ...
                case MMIFMM_FILE_TYPE_VCARD:
                    ...
                    MMIFMM_OpenMeunPubWin (win_id, MENU_FMM_VCARD_FILES_LIST_OPT, MMIFMM_VCARD_FILES_OPT_MENU_WIN_ID, MMIFMM_VCARD_FILES_OPT_MENU_CTRL_ID);
                    break;
#endif
#ifdef MMI_VCALENDAR_SUPPORT
                case MMIFMM_FILE_TYPE_VCALENDAR:
                    ...
                    MMIFMM_OpenMeunPubWin (win_id, MENU_FMM_VCALENDAR_FILES_LIST_OPT, MMIFMM_VCALENDAR_FILES_OPT_MENU_WIN_ID, MMIFMM_VCALENDAR_FILES_OPT_MENU_CTRL_ID);
                    break;
#endif
#ifdef EBOOK_SUPPORT
                case MMIFMM_FILE_TYPE_EBOOK:
        ...
        if(1 == multi_select_mode)
        {
            MMIFMM_OpenMeunPubWin (win_id, MENU_FMM_NOTHING_SELECT_OPT, MMIFMM_MULTI_SELECT_OPT_MENU_WIN_ID, MMIFMM_MULTI_SELECT_OPT_MENU_CTRL_ID);
        }
        else if (3 == multi_select_mode)
        {
            MMIFMM_OpenMeunPubWin (win_id, MENU_FMM_SOME_SELECT_OPT, MMIFMM_MULTI_SELECT_OPT_MENU_WIN_ID, MMIFMM_MULTI_SELECT_OPT_MENU_CTRL_ID);
        }
        else if (2 == multi_select_mode)
        {
            MMIFMM_OpenMeunPubWin (win_id, MENU_FMM_ALL_SELECT_OPT, MMIFMM_MULTI_SELECT_OPT_MENU_WIN_ID, MMIFMM_MULTI_SELECT_OPT_MENU_CTRL_ID);
        }
        //End
    }
}
...
#ifdef MMI_ORIG_UNISOC_SUPPORT
        if (MMK_IsOpenWin(MMIFMM_PICTURE_PREVIEW_WIN_ID) && MMIFMM_IsChinaVersion())
#else
        if (TRUE)
#endif
        {
   #ifdef     MMIEMAIL_SUPPORT
            CTRLMENU_SetItemVisible(param->ctrl_id,MENU_FMM_SEND_OPT,ID_FMM_EMAIL,FALSE);
   #endif
        }
        if (MMK_IsOpenWin(MMIFMM_PICTURE_PREVIEW_WIN_ID))
        {
            MMIFMM_VIEW_WIN_DATA_T* view_win_ptr = (MMIFMM_VIEW_WIN_DATA_T*) MMK_GetWinUserData (MMIFMM_UDISK_CHILD_WIN_ID);
            wchar    full_file_name [MMIFILE_FULL_PATH_MAX_LEN + 1] = {0};
            wchar    suffix_wstr[MMIFMM_FULL_FILENAME_LEN+1]        = {0};
            uint16   suffix_len = 0;

            MMIFMM_CombineFullFileName (view_win_ptr->main_d_ptr->s_fmm_list_data_ptr,
                view_win_ptr->main_d_ptr->s_fmm_current_path_ptr,
                GUILIST_GetCurItemIndex (MMIFMM_UDISK_LISTBOX_CTRL_ID),
                full_file_name,
                MMIFILE_FULL_PATH_MAX_LEN);

            MMIAPIFMM_SplitFullPathExt(full_file_name,(uint16) MMIAPICOM_Wstrlen (full_file_name),
                PNULL,PNULL,
                PNULL,PNULL,
                PNULL,&suffix_len);

            MMIAPICOM_Wstrncpy(suffix_wstr,&full_file_name[(uint16) MMIAPICOM_Wstrlen (full_file_name) - suffix_len + 1],suffix_len - 1);
            if (0 == MMIAPICOM_Wstrcmp(&suffix_wstr,L"gif"))
            {
                CTRLMENU_SetItemVisible(param->ctrl_id,MENU_FMM_PIC_PREVIEW_OPT,ID_FMM_WALLPAPER,FALSE);
            }
        }
    }
...
LOCAL void HandlePubMenuCloseHandler (MMIPUB_MENU_NOTIFY_T *param)
{
    ...
    {
        MMIAPICOM_FreeGrayed (param->win_id);
	 //add by qin.he for bug id 1025951
	 if (param->win_id == MMIFMM_FOLDER_OPT_MENU_WIN_ID)
	 {
		file_mark_num = 0;
        actual_file_mark_num = 0;
		folder_mark_num = 0;
...
LOCAL BOOLEAN HandleMenuOption (MMIPUB_MENU_NOTIFY_T *param)
{
    ...
    case ID_FMM_BLUETOOTH:
        ...
        if(MMIAPIFM_IsFmActive()) 
        {
            SCI_TRACE_LOW("mmifmm_mainwin.c:HandleMenuOption FM is avtiving!!");
            MMIPUB_OpenAlertWarningWin(TXT_EXIT_BACKGROUND_FM);
        }
        else
        {
...
//fixed by liyan.zhu for bugzilla 2760 2012-05-22 fmm search support
    ...
    case  ID_FMM_CANCEL_MARK:
        ...
        HandleMenuCancelMultiSelect(view_win_d->main_d_ptr->s_fmm_list_data_ptr, sel_ctrl_id);
        break;
...
//#endif
    ...
    case ID_FMM_SMS:
        ...
        break;
#endif
#if defined(MMI_VCALENDAR_SUPPORT) && defined(MMIEMAIL_SUPPORT)
    case  ID_FMM_EMAIL:
        MMIFMM_FmmSendEmailByOutlook(view_win_d->main_d_ptr->s_fmm_list_data_ptr, view_win_d->main_d_ptr->s_fmm_current_path_ptr, GUILIST_GetCurItemIndex (sel_ctrl_id));
        break;
#endif
    ...
    case ID_FMM_EDIT:
        {
             ...
             filename_len = (uint16) MMIFMM_CombineFullFileName (view_win_d->main_d_ptr->s_fmm_list_data_ptr,
                                                                 view_win_d->main_d_ptr->s_fmm_current_path_ptr,
                                                                 GUILIST_GetCurItemIndex (sel_ctrl_id),
                                                                 full_name,
                                                                 MMIFMM_FULL_FILENAME_LEN);
             MMIAPIPIC_OpenPicEditorMainWin(full_name,filename_len);
        }
        break;
#endif
    //End
    default:
        ...
        {
            MMI_TEXT_ID_T alert_text_id = STR_FORMAT_QUERY_EXT01;
...
)
{
    ...
    {
        MMK_SendMsg(MMIFMM_UDISK_CHILD_WIN_ID, MSG_APP_RIGHT, PNULL);
    }

    //current_index = GUILIST_GetCurItemIndex (ctrl_id);
    list_num = CTRLLIST_GetTotalItemNum(ctrl_id);
	...
	{
#if defined (MAINLCD_SIZE_128X160)
        if (is_backup_files || is_recording_files || (is_received_files && MMIFMM_FILE_TYPE_PICTURE != list_data_ptr->data[index]->type && MMIFMM_FILE_TYPE_MOVIE != list_data_ptr->data[index]->type))
        {
            GUILIST_SetItemStyle(MMIFMM_UDISK_LISTBOX_CTRL_ID, GUIITEM_STYLE_ONE_LINE_CHECK_ICON, index);
        }
        else if (MMIFMM_FILE_TYPE_FOLDER == list_data_ptr->data[index]->type && !is_backup_files)
        {
            GUILIST_SetItemStyle(MMIFMM_UDISK_LISTBOX_CTRL_ID, GUIITEM_STYLE_ONE_LINE_CHECK_ICON, index);
        }
        else if (MMIFMM_FILE_TYPE_MUSIC== list_data_ptr->data[index]->type || MMIFMM_FILE_TYPE_MOVIE == list_data_ptr->data[index]->type)
        {
            GUILIST_SetItemStyle(MMIFMM_UDISK_LISTBOX_CTRL_ID, GUIITEM_STYLE_ONE_LINE_CHECK_ICON, index);
        }
        else if (MMIFMM_FILE_TYPE_PICTURE == list_data_ptr->data[index]->type)
        {
            GUILIST_SetItemStyle(MMIFMM_UDISK_LISTBOX_CTRL_ID, GUIITEM_STYLE_MULTI_CHOICE_PICKER_TWO_LINE_BIGICON_TEXT_AND_TEXT, index);
        }
        else
        {
            GUILIST_SetItemStyle(MMIFMM_UDISK_LISTBOX_CTRL_ID, GUIITEM_STYLE_ONE_LINE_CHECK_ICON, index);	
        ...
        if (is_backup_files || is_recording_files || (is_received_files && MMIFMM_FILE_TYPE_PICTURE != list_data_ptr->data[index]->type && MMIFMM_FILE_TYPE_MOVIE != list_data_ptr->data[index]->type))
        {
            GUILIST_SetItemStyle(MMIFMM_UDISK_LISTBOX_CTRL_ID, GUIITEM_STYLE_MULTI_CHOICE_PICKER_TWO_LINE_TEXT_AND_TEXT_2, index);
        }
        else if (MMIFMM_FILE_TYPE_FOLDER == list_data_ptr->data[index]->type && !is_backup_files)
        {
            GUILIST_SetItemStyle(MMIFMM_UDISK_LISTBOX_CTRL_ID, GUIITEM_STYLE_ONE_LINE_CHECK_ICON_MS, index);
        }
        else if (MMIFMM_FILE_TYPE_MUSIC== list_data_ptr->data[index]->type)
        {
            GUILIST_SetItemStyle(MMIFMM_UDISK_LISTBOX_CTRL_ID, GUIITEM_STYLE_MULTI_CHOICE_PICKER_TWO_LINE_TEXT_AND_TEXT_2, index);
        }
        else if (MMIFMM_FILE_TYPE_PICTURE == list_data_ptr->data[index]->type || MMIFMM_FILE_TYPE_MOVIE == list_data_ptr->data[index]->type)
        {
            GUILIST_SetItemStyle(MMIFMM_UDISK_LISTBOX_CTRL_ID, GUIITEM_STYLE_MULTI_CHOICE_PICKER_TWO_LINE_BIGICON_TEXT_AND_TEXT, index);
        }
        else
        ...
        if (MMIFMM_FILE_TYPE_FOLDER != list_data_ptr->data[index]->type)
        {
            GUILIST_SetItemHandleLeftRightByIndex(MMIFMM_UDISK_LISTBOX_CTRL_ID,index, FALSE, FALSE);
            //GUILIST_SetItemShowLRTriangleIconByIndex(MMIFMM_UDISK_LISTBOX_CTRL_ID, index, FALSE, FALSE);
        }
        GUILIST_SetSelectedItem (ctrl_id, index, FALSE);
	}
	GUILIST_SetIsCheckUsedForMultiLine(MMIFMM_UDISK_LISTBOX_CTRL_ID, TRUE);
	//GUILIST_SetHandleLeftRightEx(MMIFMM_UDISK_LISTBOX_CTRL_ID, FALSE, FALSE);
	//GUILIST_SetShowLRTriangleIcon(MMIFMM_UDISK_LISTBOX_CTRL_ID, FALSE, FALSE);

	CTRLLIST_ChangeCheckListDefSoftkeyID(MMIFMM_UDISK_LISTBOX_CTRL_ID,STXT_SOFTKEY_UNMARK_MK,STXT_SOFTKEY_MARK_MK,STXT_SOFTKEY_OPEN_MK);
	CTRLLIST_SetNeedCustomRightSK(MMIFMM_UDISK_LISTBOX_CTRL_ID,TRUE);
	CTRLLIST_ChangeCheckListRightSoftkey(MMIFMM_UDISK_LISTBOX_CTRL_ID, STR_LIST_DELETE, IMAGE_COMMON_DELETE);
	CTRLLIST_SetCheckIconLeftFlag(ctrl_id,TRUE);
...
#else
        cur_tab_index =  GUILIST_GetCurItemIndex(MMIFMM_LIST_CTRL_ID);
#endif
...
LOCAL void  Importvcard_Callback (MMIPB_HANDLE_T handle, MMIPB_ERROR_E error)
{
    ...
    {
        MMIPUB_OpenAlertWinByTextId (PNULL, STR_NO_SPACE_EXT01, TXT_NULL, IMAGE_PUBWIN_FAIL, PNULL, PNULL, MMIPUB_SOFTKEY_ONE, PNULL);
    }
                                               ...
                                               )
{
    ...
    case MSG_APP_WEB:
    //case MSG_APP_OK:
		if (MMI_RESULT_FALSE == ResetPathListForRestore(MMIFMM_UDISK_CHILD_WIN_ID))
		{
			break;
		}
		data_ptr = (FILEARRAY_DATA_T*) SCI_ALLOC_APP (sizeof (FILEARRAY_DATA_T));
		...
		file_name_len = MMIFMM_CombineFullFileName (view_win_d->main_d_ptr->s_fmm_list_data_ptr, view_win_d->main_d_ptr->s_fmm_current_path_ptr,i, file_name_ptr, MMIFMM_FULL_FILENAME_LEN + 1);
		MMIAPICOM_WstrToStr(file_name_ptr, (uint8*)name_str);
		data_ptr->name_len = file_name_len;
		MMIAPICOM_Wstrncpy (data_ptr->filename, file_name_ptr, data_ptr->name_len);
		data_ptr->size = view_win_d->main_d_ptr->s_fmm_list_data_ptr->data[i]->file_size;
		data_ptr->type = FILEARRAY_TYPE_FILE;
		MMIIKB_RestoreItemsByFile(data_ptr);
		MMK_CloseWin(win_id);
		//MMK_CloseWin(MMIFMM_UDISK_CHILD_WIN_ID);
        break;
...
LOCAL MMI_RESULT_E ResetPathListForRestore(MMI_WIN_ID_T win_id)
{
    ...
    else
    {
        ...
        //is_flush = FALSE;
        //MMK_PostMsg (win_id, MSG_FMM_UPDATE_LIST_DATA, (DPARAM) &is_flush, sizeof (BOOLEAN));
        UpdateListData (win_id, view_win_d->list_ctrl_id, MMIFMM_READ_TYPE_FILE_DIR,
                        view_win_d->main_d_ptr->s_file_path_info_ptr->sk_text_id.leftsoft_id, TRUE);
        ...
        //GUILIST_SetCurItemIndex (view_win_d->list_ctrl_id, index);
        //GUILIST_SetTopItemIndex (view_win_d->list_ctrl_id, index);
...
PUBLIC void GetBackupFolderSize(MMIFMM_DETAIL_DATA_T* detail_data)
{
    ...
    file_array = MMIAPIFILEARRAY_Create();
    MMIAPIFMM_SearchFileInPath(detail_data->filename,
...
PUBLIC void GetFileTypeBySuffix(wchar* filename,uint16 filename_len,wchar* type)
{
    ...
    uint16        suffix_len = 0;
    uint16          path_len = 0;
    wchar    suffix_str[MMIFMM_FULL_FILENAME_LEN + 1] = {0};
    uint8    detail_arr[MMIFMM_FULL_FILENAME_LEN] = {0};
    ...
    MMIAPICOM_Wstrncat(&suffix_str,&filename[path_len + 1],suffix_len - 1);
    //MMIAPICOM_WstrToStr((wchar*)suffix_str, (char*)str_welcome);
    //SCI_TRACE_LOW("GetFileTypeBySuffix	Suffix: %s",str_welcome);
    SCI_MEMSET (detail_arr, 0, sizeof (detail_arr));

    //folder
    if (0 == suffix_len)
    {
        sprintf(detail_arr,"%s", "Folder");
    }
    //file
    else if (0 == MMIAPICOM_Wstrcmp(&suffix_str,L"vcf"))
    {
        sprintf(detail_arr,"%s", "Vcard file");
    }
    else if (0 == MMIAPICOM_Wstrcmp(&suffix_str,L"vcs"))
    {
        sprintf(detail_arr,"%s", "Vcalendar file");
    }
    else if (0 == MMIAPICOM_Wstrcmp(&suffix_str,L"txt"))
    {
        sprintf(detail_arr,"%s", "Text file");
    }
    else if (0 == MMIAPICOM_Wstrcmp(&suffix_str,L"pdf"))
    {
        sprintf(detail_arr,"%s", "PDF file");
    }
    else if (0 == MMIAPICOM_Wstrcmp(&suffix_str,L"apk"))
    {
        sprintf(detail_arr,"%s", "Java file");
    }
    else
    {
        sprintf(detail_arr,"%s", "Other file");
    }

    MMIAPICOM_StrToWstr((const uint8 *)detail_arr, type);
}
