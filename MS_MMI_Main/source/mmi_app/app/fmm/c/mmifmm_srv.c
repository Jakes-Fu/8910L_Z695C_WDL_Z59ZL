...
LOCAL void  SearchFile (MMIFMM_SRV_HANDLE handle)
{
    ...
    if (PNULL != search_handle)
    {
        ...
        else
        {
            ...
            APPFILE_CreateTask();
            SCI_TRACE_LOW("[qin.he %s:%d] search file send signal to file task. curr time: %d\n",__FUNCTION__,__LINE__,SCI_GetTickCount());   
...
)
{
    ...
    SCI_TRACE_LOW("MMIFMM_SearchFileInTask, enter search_handle = 0x%x, SearchFileTime:  curr time: %d\n",search_handle,SCI_GetTickCount());
    ...
    if (PNULL != search_handle && PNULL !=search_handle->s_file_array_search)
    {
        ...
        if (search_handle->s_is_recurse)
        {
            ...
            MMK_CheckAllocatedMemInfo( _D_FILE_NAME, _D_FILE_LINE );
            DoSearchInFolderRecurse (search_handle, search_handle->s_cur_path, search_handle->s_cur_path_len, folder_array, search_handle->s_file_array_search);
            if (search_handle->path_array_len > 0 && search_handle->path_array != NULL)
            {
                for (i = 0; i< search_handle->path_array_len; i++)
                {
                    if (search_handle->path_array[i] != NULL && search_handle->path_array[i][0] == FALSE)
                    {
                        SCI_MEMSET (search_handle->s_cur_path, 0x00, FMM_SEARCH_FILENAME_MAXLEN+1);
                        search_handle->s_cur_path_len = FMM_SEARCH_FILENAME_MAXLEN;
                     
                        MMI_WSTRNCPY (search_handle->s_cur_path, FMM_SEARCH_FILENAME_MAXLEN,
                                     &(search_handle->path_array[i][2]), search_handle->path_array[i][1], search_handle->path_array[i][1]);/*lint !e670 */
                        search_handle->s_cur_path_len = search_handle->path_array[i][1];
                        DoSearchInFolder (search_handle, search_handle->s_cur_path, search_handle->s_cur_path_len, folder_array, search_handle->s_file_array_search);
                    }
                }
            }
...
PUBLIC void MMIFMM_SearchFileCallBack (MMIFMM_SRV_HANDLE handle)
{
    ...
    SCI_TRACE_LOW("MMIFMM_SearchFileCallBack, enter search_handle=0x%x, sort start ,SearchFileTime: curr time: %d\n",search_handle,SCI_GetTickCount());
    ...
    if (PNULL != search_handle && !search_handle->s_is_sent_msg)
    {
        ...
        {
        //zhuyongjing add for search sort 20151225
#ifdef PIC_VIEWER_SUPPORT
            if((search_handle->s_cbk_msg_id == MSG_PICVIEWER_LOAD_FINISH) && !MMIPICVIEW_IsClose())
            {
                MMIAPIFILEARRAY_Sort(search_handle->s_file_array_search,FILEARRAY_SORT_TIME_CREATE_DESCEND);
				SCI_TRACE_LOW("MMIFMM_SearchFileCallBack, zyj create time sort end,SearchFileTime: curr time: %d\n", SCI_GetTickCount());
...
PUBLIC void MMIFMM_SearchFileNotify (MMIFMM_SRV_HANDLE handle)
{
     MMIFMM_SEARCH_SRV_HANDLE_T* search_handle = (MMIFMM_SEARCH_SRV_HANDLE_T*)handle;
    if (PNULL != search_handle)
    {
...
)
{
    ...
    if (search_handle->path_array_len > 0 && search_handle->path_array != NULL)
    {
        for (i = 0; i< search_handle->path_array_len; i++)
        {
            if (search_handle->path_array[i] != NULL && search_handle->path_array[i][0] == TRUE)
            {
                SCI_MEMSET (&file_data, 0x00, sizeof (file_data));
                file_data.type = FILEARRAY_TYPE_FOLDER;
                ...
                MMI_WSTRNCPY (file_data.filename, FMM_SEARCH_FILENAME_MAXLEN,
                             &(search_handle->path_array[i][2]), search_handle->path_array[i][1], search_handle->path_array[i][1]);/*lint !e670 */
                file_data.name_len = search_handle->path_array[i][1];
                MMIAPIFILEARRAY_Add (Recurse_folder_array, &file_data);
            }
        }
    }
    //file_data.type = FILEARRAY_TYPE_FOLDER;
    else if (NULL != folder_ptr && 0 < folder_namelen)
    {
        //搜索指定目录
        SCI_MEMSET (&file_data, 0x00, sizeof (file_data));
...
)
{
    ...
    if (MMIAPIFILEARRAY_Read (Recurse_folder_array, MMIAPIFILEARRAY_GetArraySize (Recurse_folder_array) - 1, &file_data))
    {
        //在最后一个目录下按筛选字查找
        MMIAPIFILEARRAY_Delete (Recurse_folder_array, MMIAPIFILEARRAY_GetArraySize (Recurse_folder_array) - 1);
...
)
{
            ...
            && FMM_SEARCH_FILENAME_MAXLEN >= find_path_len + 1 + file_name_len)
    {
        ...
        if((FUNC_FIND_FILE == search_handle->s_func_type) &&  (find_data->attr & SFS_ATTR_DIR))
            return;
...

{
    ...
    else if (SearchFileInPaths == search_file_info->search_type || SearchFileInPathsAndSort == search_file_info->search_type)
    {
        InitSearchParam (search_handle,
         ...
         search_handle->path_array = search_file_info->path_array;
         search_handle->path_array_len = search_file_info->path_array_len;
    }
    else
    {
        InitSearchParam (search_handle,
                               ...
                               )
{
    ...
    if (search_handle->path_array_len > 0 && search_handle->path_array != NULL)
    {
        for (i = 0; i< search_handle->path_array_len; i++)
        {
            if (search_handle->path_array[i] != NULL)
            {
                SCI_TRACE_LOW("[qin.he %s:%d] free memory path_array %p \n",__FUNCTION__,__LINE__,search_handle->path_array[i]);
                SCI_FREE (search_handle->path_array[i]);
            }
        }
        SCI_TRACE_LOW("[qin.he %s:%d] free memory path_array %p \n",__FUNCTION__,__LINE__,search_handle->path_array);
        SCI_FREE (search_handle->path_array);
    }
