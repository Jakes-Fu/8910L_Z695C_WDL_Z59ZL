...
LOCAL void DisplayKeyPadUnLockTips(MMI_TEXT_ID_T  tips)
{
    uint16                      str_width = 0;
    GUI_RECT_T                  tips_rect = {0};
    MMI_STRING_T                tips_string = {0};
  ...
  //  if (UILAYER_IsLayerActive(layer_handle_ptr))
    {
        ...
        //set string style
        str_style.font       = SONG_FONT_36 ;//MMI_DEFAULT_NORMAL_FONT;
...
#if 0
ndef EFFECT_RIPPLE_SUPPORT //144786 
    ...
    case MSG_KEYUP_OK:
        if ((MMIDEFAULT_IsBacklightOn()))
        {
		 ...
		 DisplayKeyPadUnLockTips(TXT_KEYLOCK_KEYPAD_UNLOCK_TIP_2);
	...
	case MSG_KEYUP_HASH:	
	//case MSG_KEYUP_STAR:		
        if(s_is_redkey_closelight)
        {
#if 1//add by James li
            DisplayKeyPadUnLockTips(TXT_KEYLOCK_KEYPAD_UNLOCK_TIP);
#elif defined(UI_MULTILAYER_SUPPORT)
