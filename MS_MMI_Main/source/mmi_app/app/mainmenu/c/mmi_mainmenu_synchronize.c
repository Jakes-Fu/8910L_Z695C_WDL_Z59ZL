...
#define   P_APP_WAKEUPSD_STACK_SIZE              6144    //6K
#define   P_APP_WAKEUPSD_QUEUE_NUM               1
#define   P_APP_WAKEUPSD_TASK_PRIORITY         PRI_EFS //  (28)
...
//task id
LOCAL BLOCK_ID g_wakeupsd_task_id = SCI_INVALID_BLOCK_ID;
...
/*****************************************************************************/
// Description : Set Item Suite Id
// Global resource dependence :
// Author:
// Note:
/*****************************************************************************/
LOCAL BOOLEAN SetItemSuiteId(
    GUIMENU_FILE_DEVICE_E store_idx,
    GUIMENU_MAINMENU_DYNAMIC_T *dy_data_ptr,
    uint8* icon_ptr,
    GUIMAINMENU_DYNAMIC_INFO_T *dynamic_item_ptr,
...
/*****************************************************************************/
// Description: Is Find Item Menu Id
// Global resource dependence:
// Author:
// Note:
/*****************************************************************************/
LOCAL BOOLEAN IsFindItemMenuId(
    GUIMAINMENU_DYNAMIC_INFO_T *dynamic_item_ptr,
    GUIMAINMENU_DYNAMIC_INFO_T *dynamic_all_item_ptr,
    uint32 dynamic_menu_num
);
    ...
    {   
        ...
        for (item_idx = 0; item_idx < s_dy_data_array[store_idx].dy_menu_num; item_idx++)
        {
            ...
            if(is_same_name
               ...
               && (data_ptr[item_idx].suite_id == dynamic_item_ptr->suite_id)
#endif
              )
            {
...
#if 0//[coverity]total_item_num is between 1 and 100
...
)
{
    ...
    if (dy_data_ptr->dy_menu_num > 0)
    {
        ...
        do
        {
            //[coverity]continue_in_do_while_false: loop the continue statement has the same effect as a break statement
            if (PNULL == s_icon_data_array_ptr[(uint16)store_index]) break;//continue;
            ...
            if (file_size > DATA_FILE_SIZE_LIMIT)  break;//continue;

            file_ptr = SCI_ALLOCA(file_size);

            if (PNULL == file_ptr)  break;//continue;
...
)
{
    ...
    dy_data_ptr->data_ptr[item_idx].suite_id = suite_id;
...
)
{
    ...
    for (i = 0; i < dynamic_menu_num; i++)
    {
        if(dynamic_item_ptr->menu_id == dynamic_all_item_ptr[i].menu_id)
        {
            result = TRUE;
            break;
        }
...
// Note:
/*****************************************************************************/
PUBLIC uint32 MMIMENU_DyGenMenuId(void)
{
    return GenMenuId();
}
												...
												)
{
    ...
    is_all_read = ReadDevMenuData(&icon_ptr,
                                  &all_dy_data,
                                  store_idx);

    if(!is_all_read)//add for coverity
    {
        SCI_TRACE_LOW("[MMIMENU_DyAppendItem] ReadDevMenuData return FALSE!");
    }
    ...
    else
    {
        ...
        is_preinstall = *(BOOLEAN*)(dynamic_item_ptr->dynamic_menu_info.param1ptr);
        if(!is_preinstall)
#endif
        {
            MMIMAINMENU_AddMenuId(dynamic_item_ptr->menu_id);
        }
        MMIMAINMENU_SetInstallAppMenuId(dynamic_item_ptr->menu_id);
    }
...
)
{
    ...
    {        
        ...
        //如果删除的是主菜单最后一个应用，需要修改高亮位置
        if(dynamic_item_ptr->menu_id == MMIMAINMENU_GetMenuIdByIndex(MMIMAINMENU_GetItemCount() - 1))
        {
            MMIMAINMENU_SetCurItemIndex(0);
        }

        MMIMAINMENU_DelMenuId(dynamic_item_ptr->menu_id);
...
// Note:成功返回TRUE，失败返回FALSE
/*****************************************************************************/
PUBLIC BOOLEAN MMIMENU_DyDeleteItemByMenuId(
    uint32 menu_id
)
{
    GUIMAINMENU_DYNAMIC_INFO_T dynamic_item_info = {0};
    GUIMAINMENU_DYNAMIC_INFO_T dynamic_item = {0};
    GUIMENU_MAINMENU_DYNAMIC_T dy_data_info[GUIMENU_MAINMENU_STORE_STYLE_NUM] = {0};
    uint8* icon_ptr[GUIMENU_MAINMENU_STORE_STYLE_NUM] = {PNULL};
    GUIMENU_FILE_DEVICE_E store_idx = 0; 
    BOOLEAN is_got_menu_id = FALSE;
    ...
    dynamic_item_info.menu_id = menu_id;
    ...
    {        
        ...
        is_got_menu_id = IsItemStoredIn(&dynamic_item_info,
                                        dy_data_info[store_idx].data_ptr,
                                        dy_data_info[store_idx].dy_menu_num);
        ...
        //先删除文件系统中存储的文件
        DeleteItemData(store_idx, &dy_data_info[data_index], icon_ptr[store_idx], &dynamic_item_info);

        //如果删除的是主菜单最后一个应用，需要修改高亮位置
        if(dynamic_item_info.menu_id == MMIMAINMENU_GetMenuIdByIndex(MMIMAINMENU_GetItemCount() - 1))
        {
            MMIMAINMENU_SetCurItemIndex(0);
        }

        MMIMAINMENU_DelMenuId(dynamic_item_info.menu_id);
...
/*****************************************************************************/
// Description : Set Item Param
// Global resource dependence :
// Author:
// Note:成功返回TRUE，失败返回FALSE
/*****************************************************************************/
PUBLIC BOOLEAN MMIMENU_DySetItemParam(
    uint32 menu_id,
    void* param1ptr,
    uint32 parm1_size,
    void* param2ptr,
...
)
{
    ...
    uint8* icon_ptr[GUIMENU_MAINMENU_STORE_STYLE_NUM] = {PNULL};
    GUIMENU_FILE_DEVICE_E store_idx = 0; 
    BOOLEAN is_find_menu_id = FALSE;
    uint16 data_index = 0;
    wchar parm1_name[DYNAMIC_MAINMENU_DATA_FILE_LEN + 1] = {0};
    wchar parm2_name[DYNAMIC_MAINMENU_DATA_FILE_LEN + 1] = {0};
    BOOLEAN is_param_write = FALSE;

    if(PNULL == param1ptr && PNULL == param2ptr)
    {
        return FALSE;
    }
    ...
    GetFilename_Param(dynamic_item_info.menu_id, parm1_name, parm2_name);
    ...
    {        
        ...
        is_find_menu_id = IsFindItemMenuId(&dynamic_item_info,
                                           dy_data_info[store_idx].data_ptr,
                                           dy_data_info[store_idx].dy_menu_num);
        ...
        if(PNULL != param1ptr)
        {
            dynamic_item_info.dynamic_menu_info.param1ptr = param1ptr;
            dynamic_item_info.dynamic_menu_info.parm1_size = parm1_size;
            
            DeleteDevFile(parm1_name, store_idx);
            is_param_write = WriteDataToFile(parm1_name,
                                             dynamic_item_info.dynamic_menu_info.param1ptr,
                                             dynamic_item_info.dynamic_menu_info.parm1_size,
                                             store_idx);
        ...
        if(PNULL != param2ptr)
        {
            dynamic_item_info.dynamic_menu_info.param2ptr = param2ptr;
            dynamic_item_info.dynamic_menu_info.parm2_size = parm2_size;
            
            DeleteDevFile(parm2_name, store_idx);
            is_param_write = WriteDataToFile(parm2_name,
                                             dynamic_item_info.dynamic_menu_info.param2ptr,
                                             dynamic_item_info.dynamic_menu_info.parm2_size,
                                             store_idx);
        }
...
// Note:成功返回TRUE，失败返回FALSE
/*****************************************************************************/
PUBLIC BOOLEAN MMIMENU_DySetItemSuiteId(
    uint32 menu_id,
    int32 suite_id
)
{
    ...
    {        
        ...
        is_suite_id_set = SetItemSuiteId(store_idx, &dy_data_info[data_index], icon_ptr[store_idx], &dynamic_item_info, suite_id);
...
// Note:
/*****************************************************************************/
LOCAL void DrawDefaultImage(MMI_WIN_ID_T win_handle, GUI_LCD_DEV_INFO *lcd_dev_ptr, GUI_RECT_T icon_rect)
{
	uint16     img_width = 0;
	uint16     img_height = 0;
#if 0	
	GUIRES_GetImgWidthHeight(&img_width, &img_height, IMAGE_MY_APPLICATION, win_handle );
	icon_rect = GUI_GetCenterRect( icon_rect, img_width, img_height );		
	GUIRES_DisplayImg(PNULL,
		&icon_rect,
		PNULL,
		win_handle,
		IMAGE_MY_APPLICATION, 
		lcd_dev_ptr);  
#endif    
}
...
{     
    ...
    switch (item_ptr->icon.type)
    {
    	...
    	case CTRL_ICON_NONE:
    	default:
            DrawDefaultImage(win_handle,lcd_dev_ptr,dis_rect);
    		break;
    }
}
...
)
{
	...
	MMIFILE_HANDLE file_handle = 0;
       ...
       {
          SCI_TRACE_LOW("[%s:%d] card log is on.\n",__FUNCTION__,__LINE__);
          SCI_ThreadExit();
          return;
       }

	GetFilename_Data(data_file_name);
	has_file_path = GetPathByFilename(GUIMENU_DEVICE_SDCARD_1,
									  data_file_name,
									  DYNAMIC_MAINMENU_DATA_FILE_LEN,
									  file_path,
									  &file_path_len);
...
PUBLIC void MMIMENU_DyWakeUpSDCard(void)
{
    if (MMIAPIENG_GetIsCardLogOn())
    {
        SCI_TRACE_LOW("[%s:%d] card log is on.\n",__FUNCTION__,__LINE__);
    ...
    if (SCI_IsThreadExist(g_wakeupsd_task_id) != SCI_SUCCESS || g_wakeupsd_task_id == SCI_INVALID_BLOCK_ID)
    {
        g_wakeupsd_task_id = SCI_CreateAppThread (
                             "T_P_APP_WAKEUPSD_TASK",
                             "Q_P_APP_WAKEUPSD_TASK",
                             APPWakeUpSD_Task,
                             0,
                             0,
                             P_APP_WAKEUPSD_STACK_SIZE,
                             P_APP_WAKEUPSD_QUEUE_NUM,
                             P_APP_WAKEUPSD_TASK_PRIORITY,
                             SCI_PREEMPT,
                             SCI_AUTO_START); 
    }
}
