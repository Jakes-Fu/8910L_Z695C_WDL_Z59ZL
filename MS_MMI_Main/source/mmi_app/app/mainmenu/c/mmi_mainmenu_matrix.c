...
#endif
#define COLOR_THEME_RECT_WIDTH    35 //Colortheme状态下颜色矩形的高度/宽度
#define COLOR_THEME_RECT_SPACE    6//Colortheme状态下颜色矩形之间的间隔
#define COLOR_THEME_TITLE_HEIGHT  24//Colortheme状态下title显示高度
/*---------------------------------------------------------------------------*/
/*                         TYPE AND CONSTANT                                 */
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
...
typedef struct
{
    ...
    GUI_RECT_T                  color_theme_rect[MMITHEME_ACCENT_COLOR_MAX];
    GUI_RECT_T                  color_theme_title_rect;
} MMIMAINMENU_MATRIX_DATA_T;

typedef struct NDT_MENU_TREE_T{
    uint32 Total_App;
...
LOCAL GUI_LCD_DEV_INFO s_softkey_layer_handle = {0, UILAYER_NULL_HANDLE};
#endif
uint8  s_cur_color_theme_index = 0;
...
LOCAL uint16 s_pre_cur_index = 0;
LOCAL int16 s_pre_offset_y = 0;

LOCAL uint32 s_mainmenu_menu_id_array[MMITHEME_MENUMAIN_TOTAL_ITEM_MAX_NUM] = {0};
LOCAL uint32 s_mainmenu_item_count = 0;

LOCAL MMIMAINMENU_STATE_T s_mainmenu_state = 0;
LOCAL uint8 s_install_progress = 0;

LOCAL uint16 s_install_item_index = 0;
LOCAL uint16 s_pre_install_item_index = 0;

LOCAL uint32 s_install_app_menu_id = 0;
LOCAL int32 s_update_item_suite_id = -1;
...
LOCAL const MMI_TEXT_ID_T s_default_softkey_text_id[] = {TXT_COMMON_OK, TXT_NULL, TXT_COMMON_COMMON_BACK};
LOCAL const MMI_TEXT_ID_T s_default_softkey2_text_id[] = {TXT_NULL, TXT_NULL, STXT_FINISH};
LOCAL const MMI_TEXT_ID_T s_default_softkey3_text_id[] = {TXT_COMMON_OK, TXT_NULL, TXT_COMMON_COMMON_BACK};
#endif
/*---------------------------------------------------------------------------*/
...
/*****************************************************************************/
//  Description : Matrix Menu Cancel Move
//  Global resource dependence:
//  Author:
//  Note:
/*****************************************************************************/
LOCAL void MatrixMenuCancelMove(
        MMIMAINMENU_MATRIX_DATA_T *menu_ctrl_ptr
        );

/*****************************************************************************/
//  Description : Matrix Menu Redraw
//  Global resource dependence :
//  Author: 
//  Note: 
/*****************************************************************************/
LOCAL void MatrixMenuRedraw(
        MMIMAINMENU_MATRIX_DATA_T *menu_ctrl_ptr
        );
#if 0
/*****************************************************************************/
//  Description : Matrix Menu draw colortheme rect
//  Global resource dependence :
//  Author: 
//  Note: 
/*****************************************************************************/
LOCAL void MatrixMenuDrawColorTheme(MMIMAINMENU_MATRIX_DATA_T *menu_ctrl_ptr);
#endif
#ifdef TOUCH_PANEL_SUPPORT
#ifdef UI_MULTILAYER_SUPPORT
/*****************************************************************************/
...
//  Note:
/*****************************************************************************/
LOCAL void MatrixMenuCreateSoftkeyLayer(
        MMIMAINMENU_MATRIX_DATA_T   *menu_ctrl_ptr  //in
        );
...
/*****************************************************************************/
//  Description : Draw Dummy Softkey
//  Global resource dependence :
//  Author:
//  Note:
/*****************************************************************************/
LOCAL void MatrixMenuDrawDummySoftkey(
...
/*****************************************************************************/
//  Description : MatrixMenu Move Item
//  Global resource dependence :
//  Author:
//  Note:
/*****************************************************************************/
LOCAL void MatrixMenuMoveItem(
        MMIMAINMENU_MATRIX_DATA_T    *menu_ctrl_ptr,
        const uint16  next_item_index
        );
...
/*****************************************************************************/
//  Description : set menu softkey param--color
//  Global resource dependence :
//  Author: Jassmine
//  Note:
/*****************************************************************************/
LOCAL void MatrixMenuSetSoftkeyColor(
        MMIMAINMENU_MATRIX_DATA_T   *menu_ctrl_ptr
        );
#ifdef TOUCH_PANEL_SUPPORT
/*****************************************************************************/
...
/*****************************************************************************/
//  Description : Release Softkey Layer
//  Global resource dependence :
//  Author:
//  Note:
/*****************************************************************************/
LOCAL void MatrixMenuReleaseSoftkeyLayer(
        MMIMAINMENU_MATRIX_DATA_T   *menu_ctrl_ptr  //in
        );
...
//  Note:
/*****************************************************************************/
LOCAL void MatrixMenuAppendSoftkeyLayer(
        void
        );
...
LOCAL void MatrixMenuDrawItem(
        ...
        uint16                 item_index,
        BOOLEAN                is_compound,
        BOOLEAN                is_draw_item_bg
        );
#endif

/*****************************************************************************/
//  Description : Adjust Menu Id Array
//  Global resource dependence :
//  Author: 
//  RETRUN: 
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN MatrixMenuAdjustMenuIdArray(
        MMIMAINMENU_MATRIX_DATA_T *menu_ctrl_ptr
        );
...
{
    s_cur_color_theme_index = cur_color_theme_index;
}
/**--------------------------------------------------------------------------*
 **                         FUNCTION DEFINITION                              *
 **--------------------------------------------------------------------------*/
        ...
        )
{
    ...
    uint16 item_width  = MMITHEME_TITLE_ICONMENU_ITEM_WIDTH_V;
        ...
        )
{
    ...
    if (!menu_ctrl_ptr->is_move_state && !is_handled)
    {
        ...
        {
			// Bug 1404715, SIM不存在/STK不支持时，不处理mainmenu中的OK/MIDSK事件
			if (item.menu_id != ID_COMMUNICATION_STK)
			{
        ...
        )
{
    ...
    MatrixMenuReleaseSoftkeyLayer(menu_ctrl_ptr);
        ...
        )
{
    ...
    item_list_ptr = MMIMAINMENU_GetMatrixItemList(MMIMAINMENU_GetState(MMIMAINMENU_INSTALL_STATE));
    ...
    for (i = 0; i < item_list_ptr->item_count; i++)
    {
        ...
        if (ID_GOMENU_CHANGE_STYLE == ((MMIMAINMENU_ITEM_INFO_T *)node_ptr->data)->menu_id )
        {
...
#endif
            {
                ...
                item_info_ptr->suite_id = ((MMIMAINMENU_ITEM_INFO_T *)node_ptr->data)->suite_id;
        ...
        )
{
    ...
    sk_height = MMITHEME_GetSoftkeyHeight();
    GUILCD_GetLogicWidthHeight(GUI_MAIN_LCD_ID,&lcd_width,&lcd_height);      
    sk_rect.right = lcd_width - 1;
    sk_rect.bottom = lcd_height - 1;
    sk_rect.top = sk_rect.bottom - sk_height + 1;

    s_softkey_layer_handle.lcd_id = GUI_MAIN_LCD_ID;
    s_softkey_layer_handle.block_id = UILAYER_NULL_HANDLE;

    // 创建层
    create_info.lcd_id = GUI_MAIN_LCD_ID;
    create_info.owner_handle = menu_ctrl_ptr->win_handle;
    create_info.offset_x = sk_rect.left;
    create_info.offset_y = sk_rect.top;
    create_info.width = (sk_rect.right + 1 - sk_rect.left);
    create_info.height = (sk_rect.bottom + 1 - sk_rect.top);
    create_info.is_bg_layer = FALSE;
    create_info.is_static_layer = FALSE;
    UILAYER_CreateLayer(&create_info,  &s_softkey_layer_handle);

    // 设置color key
    UILAYER_SetLayerColorKey(&s_softkey_layer_handle, TRUE, UILAYER_TRANSPARENT_COLOR);

    // 使用color清除层
    UILAYER_Clear(&s_softkey_layer_handle);
        ...
        )
{
    ...
    if (UILAYER_IsMultiLayerEnable())
    {
        UILAYER_RELEASELAYER(&s_softkey_layer_handle);   /*lint !e506 !e774*/
    }
        ...
        )
{
#ifdef UI_MULTILAYER_SUPPORT
    uint32 i = 0;
    GUI_LCD_DEV_INFO lcd_dev_info[4] = {0};
    ...
    lcd_dev_info[3] = s_softkey_layer_handle;
        ...
        )
{
    ...
    if (0 != s_softkey_layer_handle.block_id)
    {
        UILAYER_RemoveBltLayer(&s_softkey_layer_handle);
    }

    lcd_dev_info.block_id = s_softkey_layer_handle.block_id;

    if (0 != s_softkey_layer_handle.block_id && UILAYER_IsLayerActive(&lcd_dev_info))
    {
        append_layer.lcd_dev_info = lcd_dev_info;
        append_layer.layer_level = UILAYER_LEVEL_NORMAL;
        UILAYER_AppendBltLayer(&append_layer);
        ...
        )
{
    ...
    //item_rect.top += (MENUTEXT_HEIGHT - 1);
        ...
        )
{
    ...
    MMITHEME_GetIconMenuInfo( menu_ctrl_ptr->win_handle, &menu_ctrl_ptr->icon_data, menu_ctrl_ptr->is_title_style, menu_ctrl_ptr->is_move_mode);
    ...
    MatrixMenuAdjustMenuIdArray(menu_ctrl_ptr);//如果用户修改了记录动态菜单信息的文件，需要调整s_mainmenu_menu_id_array

    menu_ctrl_ptr->item_total_num = item_num;

    if(menu_ctrl_ptr->cur_item_index >= menu_ctrl_ptr->item_total_num)
    {
    ...
    MatrixMenuCreateSoftkeyLayer(menu_ctrl_ptr);
#endif
        ...
        )
{
    ...
    GUIWIN_SetSoftkeyStyle(menu_ctrl_ptr->win_handle,
            ...
            theme_color,
            IMAGE_NULL);
    if(menu_ctrl_ptr->is_color_theme_mode)
    {
        GUIWIN_SeSoftkeytButtonTextId(menu_ctrl_ptr->win_handle,STXT_SOFTKEY_SELECT_MK, 1, TRUE);
        GUIWIN_SeSoftkeytButtonTextId(menu_ctrl_ptr->win_handle, TXT_NULL, 0, TRUE);
    }
    else
    {
        GUIWIN_SeSoftkeytButtonTextId(menu_ctrl_ptr->win_handle,TXT_NULL, 1, TRUE);
        GUIWIN_SeSoftkeytButtonTextId(menu_ctrl_ptr->win_handle, STXT_SOFTKEY_MENU_MK, 0, TRUE);
    }
        ...
        )
{
    ...
    GUIWIN_SetSoftkeyStyle(menu_ctrl_ptr->win_handle,
        ...
        GUIWIN_SeSoftkeytButtonTextId(menu_ctrl_ptr->win_handle,TXT_COMMON_OK, 0, TRUE);
        GUIWIN_SeSoftkeytButtonIconId(menu_ctrl_ptr->win_handle, IMAGE_NULL, 1, TRUE);
        GUIWIN_SeSoftkeytButtonTextId(menu_ctrl_ptr->win_handle,TXT_COMMON_COMMON_BACK, 2, TRUE);
        ...
        )
{
    ...
    if (PNULL != menu_ctrl_ptr && menu_ctrl_ptr->main_menu_theme.is_has_button)
    {
        ...
        button_text[1] = TXT_COMMON_COMMON_BACK;
...
{
    MMI_WIN_ID_T win_id = MAIN_MAINMENU_WIN_ID;
    uint16 width = 0;
    uint16 height = 0;
    GUI_COLOR_T sk_bg_color = 0;
    GUI_RECT_T sk_bg_rect = {0};
    GUI_RECT_T lsk_rect = {0};
    GUI_RECT_T rsk_rect = {0};
...
return;
		...
		MMI_GetLabelTextByLang(TXT_COMMON_OK, &left_str);
		MMI_GetLabelTextByLang(STXT_RETURN, &right_str);
		...
		str_style.font = SONG_FONT_34;//menu_ctrl_ptr->main_menu_theme.title_font;
		str_style.font_color = MMI_BLACK_COLOR ;
		
		GUISTR_DrawTextToLCDInRect(lcd_dev_info_ptr ,//s_menu_item_layer,
						 (const GUI_RECT_T*)&lsk_rect,
						 (const GUI_RECT_T*)&lsk_rect,
						 (const MMI_STRING_T*)&left_str, 
						 &str_style, 
						 str_state,
		...
		GUISTR_DrawTextToLCDInRect(lcd_dev_info_ptr ,//s_menu_item_layer,
						 (const GUI_RECT_T*)&rsk_rect,
						 (const GUI_RECT_T*)&rsk_rect,
						 (const MMI_STRING_T*)&right_str, 
						 &str_style, 
						 str_state,
						 GUISTR_TEXT_DIR_AUTO); 
    ...
    sk_bg_rect = MMITHEME_GetFullScreenRect();
    sk_bg_rect.top = sk_bg_rect.bottom - MMITHEME_GetSoftkeyHeight() + 1;
    //sk_bg_color = MMITHEME_GetAccentColor(s_cur_color_theme_index, MMITHEME_OFFSET_COLOR_DEFAULT);
    sk_bg_color = MMI_BLACK_COLOR;
    LCD_FillArgbRect(lcd_dev_info_ptr, sk_bg_rect, sk_bg_color, MMIMENU_SK_BG_ALPHA);

    lsk_rect = MMITHEME_GetSoftkeyRect(LEFT_BUTTON);
    GUIRES_GetImgWidthHeight(&width, &height, IMAGE_PRIMO_COMMON_MENU, win_id);
    lsk_rect.right = lsk_rect.left + width - 1;
    lsk_rect.top = lsk_rect.top + ((lsk_rect.bottom - lsk_rect.top + 1 - height)/2);
    lsk_rect.bottom = lsk_rect.top + height - 1;
    ...
    GUIRES_DisplayImg(PNULL,
            ...
            &image_rect,
            win_id,
            IMAGE_PRIMO_COMMON_MENU,
            lcd_dev_info_ptr);           

    rsk_rect = MMITHEME_GetSoftkeyRect(RIGHT_BUTTON);      
    GUIRES_GetImgWidthHeight(&width, &height, IMAGE_PRIMO_COMMON_BACK, win_id);
    rsk_rect.left = rsk_rect.right - width + 1;
    rsk_rect.top = rsk_rect.top + ((rsk_rect.bottom - rsk_rect.top + 1 - height)/2);
    rsk_rect.bottom = rsk_rect.top + height - 1;
    image_rect.right = width - 1;
    image_rect.bottom = height - 1;
    GUIRES_DisplayImg(PNULL,
            &rsk_rect,
...
#ifdef UI_MULTILAYER_SUPPORT
        if (UILAYER_NULL_HANDLE != s_softkey_layer_handle.block_id)
        {
            IGUICTRL_T *softkey_ptr = (IGUICTRL_T*)MMK_GetCtrlPtrByWin(menu_ctrl_ptr->win_handle, MMITHEME_GetSoftkeyCtrlId());
            GUICTRL_SetLcdDevInfo(softkey_ptr, &s_softkey_layer_handle);
        }
#endif
    }
...
//    MatrixMenuTransmitTpMsg(menu_ctrl_ptr, MSG_CTL_PAINT, PNULL);
    ...
    if (UILAYER_IsLayerActive(&s_item_layer_handle))
    {
        ...
        {
#if defined MAINMENU_SELECT_LAYER_ON_TOP || defined(MMI_3DUI_SUPPORT)
            MatrixMenuSetBltIconMenuDispLayer(menu_ctrl_ptr, s_item_layer_handle.block_id, menu_ctrl_ptr->highlight_layer_handle.block_id);
#else
  ...
  MatrixMenuDrawDummySoftkey(&lcd_dev_info);
    return;
}
        ...
        )
{
    ...
    uint16 item_width = 0;
    uint16 item_heigth = 0;
    BOOLEAN is_install_item = FALSE;
    ...
    if(menu_ctrl_ptr->is_move_mode)
    {
        item_width = MMITHEME_MENU_ITEM_WIDTH_MOVE;
        item_heigth = MMITHEME_MENU_ITEM_HEIGHT_MOVE;
    }
    else
    {
        item_width = MMITHEME_MENU_ITEM_WIDTH;
        item_heigth = MMITHEME_MENU_ITEM_HEIGHT;
    }
    disp_rect = GUI_GetCenterRect(disp_rect, item_width, item_heigth);
...
#ifdef DYNAMIC_MAINMENU_SUPPORT
    //安装应用图标，其背景高度根据安装进度计算得出
    if((MMIMAINMENU_GetState(MMIMAINMENU_INSTALL_STATE) || MMIMAINMENU_GetState(MMIMAINMENU_PRE_INSTALL_STATE) || MMIMAINMENU_GetState(MMIMAINMENU_UPDATE_STATE)) &&
            (item_index == s_install_item_index))
    {
        is_install_item = TRUE;
    }
#endif
    ...
    if (is_compound)
    {
        item_layer = MMITHEME_DrawMainmenuItem(menu_ctrl_ptr->win_handle, &menuitem_info, PNULL, is_install_item);
    ...
    else
    {
        ...
        item_layer = MMITHEME_DrawMainmenuItem(menu_ctrl_ptr->win_handle, &menuitem_info, &param, is_install_item);
    }
#endif    
}
        ...
        for (row = 0; row < menu_ctrl_ptr->icon_data.page_col_num; row++)
        {
            ...
            if (item_index < menu_ctrl_ptr->item_total_num)
            {
                ...
                if ((int32)menu_ctrl_ptr->menu_list_info.item_count > (int32)item_index && GUI_IntersectRect(&cross_rect, disp_rect, menu_rect))
                {
                    ...
                    if (is_need_draw)
                    {
                        MatrixMenuDrawItem(menu_ctrl_ptr, &lcd_dev_info, item_index, FALSE, FALSE);
                    }
                }
            }
        }
...
// BUG 506394, Need to increase one in message icon when receive a new MMS, if the user select manual download. 
...
#if defined (MAINLCD_SIZE_128X160)//boda,resize the font size
    if( count_num <10)
    {
...
#endif //#if defined (MAINLCD_SIZE_128X160)//boda
    text_style.align=ALIGN_HVMIDDLE;

    GUIRES_GetImgWidthHeight(&img_width, &img_height, IMAGE_MAINMENU_NUMICON,win_handle);

    disp_rect.top += 3;
    disp_rect.right -= 3;
    disp_rect.left   = disp_rect.right - img_width ; //MENUICON_WIDTH/2;
    disp_rect.bottom = disp_rect.top + img_height ; //MENUICON_WIDTH/2;
        ...
        )
{
    ...
    SCI_TRACE_LOW("MAIN MENU :: MatrixMenuDrawItemIcon 0");
    ...
    IMG_EnableTransparentColor(TRUE);
    SCI_TRACE_LOW("MAIN MENU :: MatrixMenuDrawItemIcon 1");
    ...
    for (line_index = s_start_line_index; line_index <= s_end_line_index; line_index++)
    {
        ...
        for (row = 0; row < menu_ctrl_ptr->icon_data.page_col_num; row++)
        {
            ...
            {
                // 计算显示区域
                SCI_TRACE_LOW("MAIN MENU :: MatrixMenuDrawItemIcon -- idx = %d", item_index);
                ...
                MatrixMenuDrawItem(menu_ctrl_ptr, lcd_dev_info_ptr, item_index, FALSE, FALSE);
#else
                ...
                // draw icon
                disp_rect = MatrixMenuGetItemRect(menu_ctrl_ptr, item_index);
                ...
                icon_rect = MatrixMenuGetIconDispRect(disp_rect, cross_rect, MENUICON_WIDTH, MENUICON_HEIGHT);
                MatrixMenuGetItemInfo(menu_ctrl_ptr, item_index, &menuitem_info);
                SCI_TRACE_LOW("MAIN MENU :: MatrixMenuDrawItemIcon -- idx = %d draw", item_index);

                MMIMAINMENU_DrawItemIcon(menu_ctrl_ptr->win_handle, &menuitem_info, lcd_dev_info_ptr, &icon_rect, &disp_rect);

                // draw icon number
                count_num = GetMenuIconNumber(menuitem_info.menu_id);
                DrawMenuNumberIcon(menu_ctrl_ptr->win_handle, count_num, 
                disp_rect, *lcd_dev_info_ptr);


                // draw icon text
                if (MMIMAINMENU_MATRIX_TITLE_E != MMIAPISET_GetMainmenuStyle())
                {
                    text_rect.left = (disp_rect.left + disp_rect.right - MMITHEME_MENU_ITEM_WIDTH)>>1;
                    text_rect.top =  disp_rect.bottom;
                    text_rect.right = (disp_rect.left + disp_rect.right + MMITHEME_MENU_ITEM_WIDTH)>>1;
                    text_rect.bottom = disp_rect.bottom + 2 * MMITHEME_MAINMENU_TEXT_FONT;
                }
...
//START: add for BUG 1150142: Shortcut menu depends on the mainmenu for different regions
LOCAL BOOLEAN MMIMAINMENU_ShortcutIsValid(uint32* applist,  uint32 count, MMISET_SHORTCUT_TYPE_E  shortcut)
{
    MMI_MENU_ID_T menuid = ID_MAINMENU_NULL;
    uint32 i=0;
    
    switch(shortcut)
    {
        case MMISET_SHORTCUT_PB:
            menuid = ID_COMMUNICATION_PB;
            break;
    
        case MMISET_SHORTCUT_CL:
            menuid = ID_COMMUNICATION_CL;
            break;
    
        case MMISET_SHORTCUT_MESSAGE:
            menuid = ID_COMMUNICATION_MESSAGE;
            break;
    
    #ifdef MMI_AUDIO_PLAYER_SUPPORT
        case MMISET_SHORTCUT_MP3:
            menuid = ID_ENTERTAIMENT_MP3;
            break;
    #endif
        ...
        case MMISET_SHORTCUT_CAMERA:
            menuid = ID_ENTERTAIMENT_CAMERA;
            break;
    #endif
    
        case MMISET_SHORTCUT_DOCUMENT:
            menuid = ID_ENTERTAIMENT_FAVORITE;
            break;
...
PUBLIC uint8 MMIMAINMENU_GetALLShortCut(uint8  *shortcut_menu, uint8 max)
{
    ...
    uint32 i = 0;
    uint8 count=0;
    NDT_MENU_TREE_T config_menu_tree = {0};
    NVITEM_ERROR_E nv_reslut = NVERR_NOT_EXIST;
    uint32 config_item_count = 0;
    uint32 appid_list = 0;
    MMISET_SHORTCUT_TYPE_E shortcut;
    
    nv_len = NVITEM_GetLength(OTHER_NV_MENU_TREE_LIST);
    if(nv_len == sizeof(config_menu_tree))
    {
        nv_reslut = EFS_NvitemRead(OTHER_NV_MENU_TREE_LIST, nv_len, (uint8*)(&config_menu_tree));
    }
    
    if(NVERR_NONE == nv_reslut) 
    {
        config_item_count = config_menu_tree.Total_App;
    }
    //enum none
    shortcut_menu[count]= 0;
    count++;
    
    for(i = 1; i < max; i++)
    {
    	if(MMIMAINMENU_ShortcutIsValid(config_menu_tree.APPID_LIST,config_item_count,i))
    	{
    		shortcut_menu[count]= i;
    		count++;
    	}
    }	
    return count;
}
//END: add for BUG 1150142: Shortcut menu depends on the mainmenu for different regions
...
PUBLIC void MMIMAINMENU_InitMenuIdArray(void)
{
    ...
    MMINV_READ(MMINV_MAINMENU_MENU_ID_ARRAY, s_mainmenu_menu_id_array, return_value);
    SCI_TRACE_LOW("MMIMAINMENU_InitMenuIdArray return_value=%d", return_value);

    if(MN_RETURN_SUCCESS != return_value)
    {
        uint32 config_menu_id_array[MMITHEME_MENUMAIN_TOTAL_ITEM_MAX_NUM] = {0};
        uint32 config_item_count = 0;
        uint32 j = 0, st_item_cnt = 0, dy_item_cnt = 0;
        BOOLEAN is_find = FALSE;
        uint16 nv_len = 0;
        NVITEM_ERROR_E nv_reslut = NVERR_NOT_EXIST;
        NDT_MENU_TREE_T config_menu_tree = {0};

        s_mainmenu_item_count = 0;
        ...
        if(NVERR_NONE == nv_reslut) 
        {
            ...
            {
                config_menu_id_array[i] = config_menu_tree.APPID_LIST[i];
            }
            ...
            for (i = 0; i < config_item_count; i++)
            {
                ...
                for(j = 0; j < st_item_cnt; j++)
                {
                    if(config_menu_id_array[i] == s_mainmenu_item_data[j].menu_id)
                    {
                        is_find = TRUE;

                        s_mainmenu_menu_id_array[s_mainmenu_item_count] = s_mainmenu_item_data[j].menu_id;

                        s_mainmenu_item_count++;
...
#ifdef DYNAMIC_MAINMENU_SUPPORT
                //动态
                if(!is_find && MMIMENU_DyGetAllItemData())
                {
                    dy_item_cnt = MMIMENU_DyGetItemCount();

                    for(j = 0; j < dy_item_cnt; j++)
                    {
                        BOOLEAN is_get_info = FALSE;
                        GUIMAINMENU_DY_ITEM_T item_info = {0};

                        is_get_info = MMIMENU_DyListGetDispInfoEx(j, &item_info);

                        if (is_get_info && PNULL != item_info.ori_data_ptr)
                        {
                            if(config_menu_id_array[i] == item_info.ori_data_ptr->menu_id)
                            {
                                is_find = TRUE;

                                s_mainmenu_menu_id_array[s_mainmenu_item_count] = item_info.ori_data_ptr->menu_id;
                ...
                if(!is_find)
                {
                    SCI_TRACE_LOW("MMIMAINMENU_InitMenuIdArray could not find i = %d", i);
                }
            }

            if(!is_find)
            {
                SCI_TRACE_LOW("MMIMAINMENU_InitMenuIdArray load menu from defalut table");
                st_item_cnt = sizeof(s_mainmenu_item_data) / sizeof(MMI_MAINMENU_ITEM_T);  
                for(i = 0; i < st_item_cnt; i++)
                {
                    s_mainmenu_menu_id_array[s_mainmenu_item_count] = s_mainmenu_item_data[i].menu_id;
                    s_mainmenu_item_count++;
                }
            }
        }
        ...
        MMINV_WRITE(MMINV_MAINMENU_MENU_ID_ARRAY, s_mainmenu_menu_id_array);
    }
    else
    {
        for(i = 0; i < MMITHEME_MENUMAIN_TOTAL_ITEM_MAX_NUM; i++)
        {
            if(0 == s_mainmenu_menu_id_array[i])
            {
                break;
            }
        }
        s_mainmenu_item_count = i;
    }
    SCI_TRACE_LOW("MMIMAINMENU_InitMenuIdArray s_mainmenu_item_count=%d", s_mainmenu_item_count);	
}
...
{
    BOOLEAN is_append_preinstall_item = FALSE;
    MN_RETURN_RESULT_E return_value = MN_RETURN_FAILURE;

    MMINV_READ(MMIMV_MAINMENU_IS_APPEND_PREINSTALL_ITEM, &is_append_preinstall_item, return_value);
    if(MN_RETURN_SUCCESS != return_value)
    {
        is_append_preinstall_item = TRUE;
    }

    if(is_append_preinstall_item)
    {
        MMIJAVA_GeneratePreinstallMainmenu(DATA_JAVA_APPS);
        is_append_preinstall_item = FALSE;
        MMINV_WRITE(MMIMV_MAINMENU_IS_APPEND_PREINSTALL_ITEM, &is_append_preinstall_item);
    }   
}
#endif
...
{
    return s_mainmenu_menu_id_array[index];
}
        ...
        )
{
    ...
    uint32 cur_item_menu_id = 0;

    cur_item_index = menu_ctrl_ptr->cur_item_index;
    cur_item_menu_id = s_mainmenu_menu_id_array[cur_item_index];

    s_mainmenu_menu_id_array[cur_item_index] = s_mainmenu_menu_id_array[next_item_index];
    s_mainmenu_menu_id_array[next_item_index] = cur_item_menu_id;

#ifdef DYNAMIC_MAINMENU_SUPPORT
    if(MMIMAINMENU_GetState(MMIMAINMENU_INSTALL_STATE) || MMIMAINMENU_GetState(MMIMAINMENU_PRE_INSTALL_STATE) || MMIMAINMENU_GetState(MMIMAINMENU_UPDATE_STATE))
    {
        if(cur_item_index == s_install_item_index)
        {
            s_install_item_index = next_item_index;
        }
        else if(next_item_index == s_install_item_index)
        {
            s_install_item_index = cur_item_index;
        }
    }
#endif
}
        ...
        )
{
    ...
    if(menu_ctrl_ptr->is_move_mode)
    {
        MatrixMenuMoveItem(menu_ctrl_ptr, next_item_index);
    }
        ...
        )
{
    ...
    if (MMIAPI3D_IsIconMenuAnimOpen())
    {
        ...
        //set buffer size
        buf_size = (uint32)(img_width * img_height) * sizeof(uint16);
        ...
        )
{
...
#ifdef UI_MULTILAYER_SUPPORT
    //显示menu item高亮条
    if (UILAYER_IsLayerActive(&s_item_layer_handle) && UILAYER_IsLayerActive(&(menu_ctrl_ptr->highlight_layer_handle)))
    {
...
#ifdef DYNAMIC_MAINMENU_SUPPORT
    if((MMIMAINMENU_GetState(MMIMAINMENU_INSTALL_STATE) || MMIMAINMENU_GetState(MMIMAINMENU_PRE_INSTALL_STATE) || MMIMAINMENU_GetState(MMIMAINMENU_UPDATE_STATE)) &&
            (menu_ctrl_ptr->cur_item_index == s_install_item_index))
    {
        focus_rect_height = focus_rect.bottom - focus_rect.top + 1;
        focus_rect_height = (focus_rect_height*s_install_progress)/100;

        focus_rect.top = focus_rect.bottom - focus_rect_height + 1;
    }
#endif
...
#ifndef MMI_PDA_SUPPORT
    {
        ...
        GUIRES_GetImgWidthHeight(&img_width, &img_height, menuitem_info.active_icon.data.icon_id, menu_ctrl_ptr->win_handle);

        cross_rect.top = cross_rect.top + (cross_rect.bottom - cross_rect.top - img_height)/2;
        cross_rect.left = cross_rect.left + (cross_rect.right - cross_rect.left - img_width)/2;
	layer_rect.top = 0;
	layer_rect.left = 0;
	layer_rect.right = MMI_MAINSCREEN_WIDTH-1;
	layer_rect.bottom = MMI_MAINSCREEN_HEIGHT-1;
        GUIRES_DisplayImg(PNULL,
                          &layer_rect,
                &layer_rect,
                menu_ctrl_ptr->win_handle,
                menuitem_info.active_icon.data.icon_id,//IMAGE_MAINMENU_FOCUSFRAME,//menuitem_info.active_icon.data.icon_id,
                (const GUI_LCD_DEV_INFO *)&lcd_dev_info);
    }
#if defined UI_MULTILAYER_SUPPORT && !defined(MMI_3DUI_SUPPORT)
    UILAYER_ClearRect(&s_item_layer_handle, MatrixMenuGetTotalItemRect(menu_ctrl_ptr, menu_ctrl_ptr->cur_item_index));
    MatrixMenuDrawItem(menu_ctrl_ptr, &lcd_dev_info, menu_ctrl_ptr->cur_item_index, TRUE, FALSE);
#endif
...
#if 1
    if(!(menu_ctrl_ptr->is_color_theme_mode || menu_ctrl_ptr->is_move_mode))
    {
        MMI_STRING_T        item_str = {0};
        CTRLMAINMENU_ITEM_T menuitem_info = {0};
        ...
        )
{
    ...
    MatrixMenuDrawItem(menu_ctrl_ptr, &lcd_dev_info, index, FALSE, FALSE);
    return;
}
#endif
        ...
        )
{
    ...
    MatrixMenuModifyRect(menu_ctrl_ptr, FALSE);
#ifdef UI_MULTILAYER_SUPPORT
    if(UILAYER_IsLayerActive(&s_item_layer_handle))
    {
...
{ 
    ...
    MMI_STRING_T   item_str = {0};
    GUISTR_STYLE_T  str_style = {0};
    GUISTR_STATE_T  str_state = GUISTR_STATE_ALIGN | GUISTR_STATE_SINGLE_LINE | GUISTR_STATE_ELLIPSIS|GUISTR_STATE_ALIGN_BY_DIR;

    MMI_IMAGE_ID_T  current_highlight_image = IMAGE_COLORTHEME_FOCUSFRAME;
#ifdef UI_MULTILAYER_SUPPORT
    if (UILAYER_IsLayerActive(&s_item_layer_handle))
    {
        lcd_dev_info.block_id = s_item_layer_handle.block_id;
...
#endif
    //依次画每个color
    for(i = 0;i < MMITHEME_ACCENT_COLOR_MAX; i++)
    {
        color_rect = menu_ctrl_ptr->color_theme_rect[i];
        color_theme = MMITHEME_GetAccentColor(i,MMITHEME_OFFSET_COLOR_DEFAULT);
        LCD_FillRect(&lcd_dev_info, color_rect, color_theme);
    }
    disp_rect = menu_ctrl_ptr->color_theme_rect[s_cur_color_theme_index];
    GUIRES_GetImgWidthHeight(&width, &height, current_highlight_image, menu_ctrl_ptr->win_handle);
    image_rect.right = width - 1;
    image_rect.bottom = height - 1;
    GUIRES_DisplayImg(PNULL,
            &disp_rect,
    ...
    //update status bar
    MMI_GetLabelTextByLang(STR_MAIN_HDR_THEME_EXT01, &item_str);
    if(MMIAPICOM_StatusAreaSetViewHeaderinfo(item_str.wstr_ptr, item_str.wstr_len))
    {
        GUIWIN_UpdateStb();
    }
        ...
        )
{
    ...
    switch (msg_id)
    {
        ...
        case MSG_CTL_CLOSE:
			SCI_TRACE_LOW("MatrixMenuHandle MSG_CTL_CLOSE\n");
            MMITheme_StopControlText();
            break;
        ...
        case MSG_CTRLMAINMENU_GET_CUR_INDEX:
            *((uint16*)param) = menu_ctrl_ptr->cur_item_index;
            break;

        case MSG_CTRLMAINMENU_GET_OFFSET_Y:
            *((uint16*)param) = menu_ctrl_ptr->offset_y;
            break;

        case MSG_CTRLMAINMENU_IS_MOVE_MODE:
            *((BOOLEAN*)param) = menu_ctrl_ptr->is_move_mode;
            break;

        case MSG_CTRLMAINMENU_SET_MOVE_MODE:
            menu_ctrl_ptr->is_move_mode = *((BOOLEAN*)param);
            break;

        case MSG_CTRLMAINMENU_IS_UNINSTALLABLE:
            {
                CTRLMAINMENU_ITEM_T cur_item_info = {0};

                MatrixMenuGetItemInfo(menu_ctrl_ptr, menu_ctrl_ptr->cur_item_index, &cur_item_info);

                *((BOOLEAN*)param) = cur_item_info.is_uninstallable;
            }
            break;

        case MSG_CTRLMAINMENU_SET_COLOR_THEME_MODE:
            menu_ctrl_ptr->is_color_theme_mode = *((BOOLEAN*)param);
            //s_cur_color_theme_index = MMITHEME_GetAccentId();
            break;
        case MSG_CTRLMAINMENU_IS_COLOR_THEME_MODE:
            *((BOOLEAN*)param) = menu_ctrl_ptr->is_color_theme_mode;
            break;

        case MSG_CTRLMAINMENU_REDRAW:
            if(MMK_IsFocusWin(MAIN_MAINMENU_WIN_ID))
            {
                MatrixMenuRedraw((MMIMAINMENU_MATRIX_DATA_T *)menu_ctrl_ptr);
            }
            break;

        case MSG_CTRLMAINMENU_LOSE_FOCUS_REDRAW:
            MatrixMenuDraw((MMIMAINMENU_MATRIX_DATA_T *)menu_ctrl_ptr);
#if 0
            if(menu_ctrl_ptr->is_color_theme_mode)
            {
                MatrixMenuDrawColorTheme((MMIMAINMENU_MATRIX_DATA_T *)menu_ctrl_ptr);
            }
#endif
            break;

        case MSG_CTRLMAINMENU_SET_RECT:
            MatrixMenuModifyRect(menu_ctrl_ptr, TRUE);
            break;
        ...
        )
{
    ...
    menu_ctrl_ptr->is_move_mode = FALSE;

    menu_ctrl_ptr->cur_item_index = s_pre_cur_index;
    menu_ctrl_ptr->offset_y = s_pre_offset_y;
#ifdef DYNAMIC_MAINMENU_SUPPORT
    s_install_item_index = s_pre_install_item_index;
#endif

    MMINV_READ(MMINV_MAINMENU_MENU_ID_ARRAY, s_mainmenu_menu_id_array, return_value);
    if(MN_RETURN_SUCCESS != return_value)
    {
        SCI_TRACE_LOW("MatrixMenuHandle restore index err");
    }
...
{
    uint32 i = 0;
    MMIMAINMENU_MATRIX_DATA_T *menu_ctrl_ptr = (MMIMAINMENU_MATRIX_DATA_T *)GUIMAINMENU_GetPM(MAINMENUWIN_MENU_CTRL_ID);

    for(i = 0; i < s_mainmenu_item_count; i++)
    {
        if(s_mainmenu_menu_id_array[i] == menu_id)
        {
            break;
        }
    }

    for(; i + 1 < s_mainmenu_item_count; i++)
    {
        s_mainmenu_menu_id_array[i] = s_mainmenu_menu_id_array[i + 1];
    }   
    s_mainmenu_menu_id_array[s_mainmenu_item_count - 1] = 0;

    if(PNULL != menu_ctrl_ptr)
    {
        //move模式下用户有可能取消move操作，此时不能写NV
        if(!menu_ctrl_ptr->is_move_mode)
        {   
            MMINV_WRITE(MMINV_MAINMENU_MENU_ID_ARRAY, s_mainmenu_menu_id_array);
        }
        ...
        )
{
    ...
    if(s_mainmenu_item_count < MMITHEME_MENUMAIN_TOTAL_ITEM_MAX_NUM)
    {
        s_mainmenu_menu_id_array[s_mainmenu_item_count] = menu_id;
        MMINV_WRITE(MMINV_MAINMENU_MENU_ID_ARRAY, s_mainmenu_menu_id_array);
        s_mainmenu_item_count++;
        result = TRUE;
    }
        ...
        )
{
    ...
    item_list_ptr = &(menu_ctrl_ptr->menu_list_info);

    if(PNULL != item_list_ptr)//lint !e774
    {
        //s_mainmenu_menu_id_array多余的项删除
        for(i = 0; i < s_mainmenu_item_count; i++)
        {
            is_find = FALSE;
            menu_id = s_mainmenu_menu_id_array[i];

            node_ptr = item_list_ptr->list_head_ptr;

            for (j = 0; j < item_list_ptr->item_count; j++)
            {
                if (PNULL == node_ptr)
                {
                    break;
                ...
                if(menu_id == ((MMIMAINMENU_ITEM_INFO_T *)node_ptr->data)->menu_id)
                {
                    is_find = TRUE;
                    break;
                }
            ...
            if(!is_find)
            {
                MMIMAINMENU_DelMenuId(menu_id);
            }
        }

        //s_mainmenu_menu_id_array少的项加上
        node_ptr = item_list_ptr->list_head_ptr;
        ...
        for (i = 0; i < item_list_ptr->item_count; i++)
        {
            ...
            menu_id = ((MMIMAINMENU_ITEM_INFO_T *)node_ptr->data)->menu_id;

            for(j = 0; j < s_mainmenu_item_count; j++)
            {
                if(menu_id == s_mainmenu_menu_id_array[j])
                {
                    is_find = TRUE;
                    break;
                }
...
PUBLIC BOOLEAN MMIMAINMENU_SetPreState(void)
{
    ...
    if(PNULL != menu_ctrl_ptr)
    {
        s_pre_cur_index = menu_ctrl_ptr->cur_item_index;
        s_pre_offset_y = menu_ctrl_ptr->offset_y;
#ifdef DYNAMIC_MAINMENU_SUPPORT
        s_pre_install_item_index = s_install_item_index;
#endif
        result = TRUE;
    }
    ...
    if (PNULL != menu_ctrl_ptr)
    {
        ...
        {       
            ...
            for (i = 0; i < item_list_ptr->item_count; i++)
            {
                ...
                if(((MMIMAINMENU_ITEM_INFO_T *)node_ptr->data)->menu_id == s_mainmenu_menu_id_array[cur_item_index])
                {
                    is_find = TRUE;
                    if(PNULL != suite_id_ptr)
                    {
                        *suite_id_ptr = ((MMIMAINMENU_ITEM_INFO_T *)node_ptr->data)->suite_id;
                    }
                    if(PNULL != menu_id_ptr)
                    {
                        *menu_id_ptr = ((MMIMAINMENU_ITEM_INFO_T *)node_ptr->data)->menu_id;
                    }
                    break;
                }
...
PUBLIC void MMIMAINMENU_DeleteCurrentPreloadItem(void)
{
    ...
    MMIMAINMENU_GetCurItemInfo(&suite_id, &menu_id);
        ...
        )
{
    ...
    if (PNULL != menu_ctrl_ptr)
    {
        ...
        {       
            ...
            for (i = 0; i < item_list_ptr->item_count; i++)
            {
                ...
                if(suite_id == ((MMIMAINMENU_ITEM_INFO_T *)node_ptr->data)->suite_id)
                {
                    is_find = TRUE;
                    menu_id = ((MMIMAINMENU_ITEM_INFO_T *)node_ptr->data)->menu_id;
                    break;
    ...
    if(is_find)
    {
        ...
        for(i = 0; i < s_mainmenu_item_count; i++)
        {
            if(menu_id == s_mainmenu_menu_id_array[i])
            {
                is_find = TRUE;
...
{
    BOOLEAN result = FALSE;
    MMIMAINMENU_MATRIX_DATA_T *menu_ctrl_ptr = PNULL;

    if(is_true)
    {
        s_mainmenu_state |= state;
    }
    else
    {
        s_mainmenu_state &= ~state;
    ...
    switch(state)
    {
        case MMIMAINMENU_INSTALL_STATE:
            menu_ctrl_ptr = (MMIMAINMENU_MATRIX_DATA_T *)GUIMAINMENU_GetPM(MAINMENUWIN_MENU_CTRL_ID);
            if(PNULL != menu_ctrl_ptr)
            {
                MatrixMenuReloadItem(menu_ctrl_ptr);
#ifdef UI_MULTILAYER_SUPPORT
                ...
                if(is_true)
                {
                    s_install_item_index = menu_ctrl_ptr->item_total_num - 1;
                }
                else
                {
                    s_install_item_index = 0;
                    s_install_progress = 0;
                }
                result = TRUE;
            }
            break;

        case MMIMAINMENU_PRE_INSTALL_STATE:
            if(is_true)
            {
                menu_ctrl_ptr = (MMIMAINMENU_MATRIX_DATA_T *)GUIMAINMENU_GetPM(MAINMENUWIN_MENU_CTRL_ID);
                if(PNULL != menu_ctrl_ptr)
                {
                    s_install_item_index = menu_ctrl_ptr->cur_item_index;
                    result = TRUE;
                }
            }
            else
        ...
        case MMIMAINMENU_UPDATE_STATE:
            if(is_true)
            {
                s_update_item_suite_id = *((int32*)param);
                result = MMIMAINMENU_GetItemInfoBySuiteId(s_update_item_suite_id, PNULL, &s_install_item_index);
                //设置UPDATE状态时如果mainmenu找不到图标，就当成INSTALL状态处理
                if(!result)
                {
            ...
            {
                s_update_item_suite_id = -1;
                s_install_item_index = 0;
                s_install_progress = 0;
                if(MMIMAINMENU_GetState(MMIMAINMENU_INSTALL_STATE))
                {
    ...
    GUIMAINMENU_Redraw(MAINMENUWIN_MENU_CTRL_ID);
...
{
    return (BOOLEAN)((s_mainmenu_state & state) == state);
}
        ...
        )
{
    ...
    {
        s_install_progress = install_progress;
        GUIMAINMENU_Redraw(MAINMENUWIN_MENU_CTRL_ID);
        result = TRUE;
    }
...
PUBLIC BOOLEAN MMIMAINMENU_ReturnMainmenuByJava(void)
{
    ...
    result = MMIMAINMENU_SetCurItemIndex(s_install_item_index);
...
PUBLIC BOOLEAN MMIMAINMENU_IsPreInstallItem(void)
{
    ...
    if (MMIMENU_DyGetAllItemData())
    {
        ...
        for (i = 0; i < dy_item_cnt; i++)
        {
            ...
            if (is_get_info && PNULL != item_info.ori_data_ptr)
            {
                if(s_mainmenu_menu_id_array[s_install_item_index] == item_info.ori_data_ptr->menu_id)
                {
...
{
    return s_cur_color_theme_index;
}
...
{
    return GUIMAINMENU_Redraw(MAINMENUWIN_MENU_CTRL_ID);
}
...
{
#ifdef UI_MULTILAYER_SUPPORT
    return UILAYER_IsLayerActive(&s_item_layer_handle);
#else
    return FALSE;
#endif
}
