...
static uint32 s_mms_max_body_size = 0;   
static uint16 s_mms_max_number = 0;
static uint8 s_mms_auto_receive = 0;
...
PUBLIC MMIMMS_NV_SETTING_T MMIMMS_ResetMMSFactorySetting(void)
{
    ...
    BOOLEAN	mms_delivery_rpt_send_flag = FALSE;
    BOOLEAN	mms_is_advertisingallowable = FALSE;
    ...
    ret_value = MMIMMS_GetNDTNvSetting(MESSAGES_NV_MMS_DELIVERY_REPORT,&mms_delivery_rpt_send_flag);
    if (MN_RETURN_SUCCESS != ret_value)
    {    
        mms_delivery_rpt_send_flag = FALSE;
    }

    ret_value = MMIMMS_GetNDTNvSetting(MESSAGES_NV_MADVERTISE_ON,&mms_is_advertisingallowable);
    if (MN_RETURN_SUCCESS != ret_value)
    {    
        mms_is_advertisingallowable = TRUE;
    }
    
    nv_setting.is_need_delivery_rpt_send = mms_delivery_rpt_send_flag;
    nv_setting.is_need_delivery_rpt_recv = mms_delivery_rpt_send_flag; 
    nv_setting.is_need_read_rpt_send = FALSE;
    nv_setting.is_anonymous = FALSE;
    nv_setting.is_play_sound = TRUE;
    nv_setting.is_advertisingallowable = mms_is_advertisingallowable;
...
// modified by feng.xiao
    ...
    {
        nv_setting.retrieve_mode_roam = MMIMMS_SETTING_RETRIEVE_MODE_DENY;
    }
    else
    {
    ...
    SCI_TRACE_LOW("MMIMMS: s_mms_auto_receive = %d", s_mms_auto_receive);
    switch(s_mms_auto_receive)
    {
    case 0:
        nv_setting.retrieve_mode = MMIMMS_SETTING_RETRIEVE_MODE_IMMEDIATE;
        break;
    case 1:
        nv_setting.retrieve_mode = MMIMMS_SETTING_RETRIEVE_MODE_DEFER;
        break;
    case 2:
        nv_setting.retrieve_mode = MMIMMS_SETTING_RETRIEVE_MODE_DENY;
        break;
    default:
        break;
    }
...
/*****************************************************************************/
//     Description : init NDT nv setting,fox max number, max body size
//    Global resource dependence : 
//  Author: rong.gu
//    Note:
/*****************************************************************************/
PUBLIC  void MMIMMS_InitNDTSetting(void)
{
    _NDT_MMIMMS_CONFIG_T mms_ms_config = {0};
    MN_RETURN_RESULT_E ret_value = MN_RETURN_FAILURE;

    ret_value = MMIMMS_GetNDTNvSetting(MESSAGES_NV_MMS_CONFIG,&mms_ms_config);	
    if (MN_RETURN_SUCCESS != ret_value)
    {    
        s_mms_max_number = MMIMMS_MAX_MMS_NUM;
        s_mms_max_body_size = MMIMMS_MAX_ONE_MMS_SIZE;
        s_mms_auto_receive = FALSE;
    }
    else
    {
        s_mms_max_number = ((MMIMMS_MAX_MMS_NUM > mms_ms_config.max_number) ? mms_ms_config.max_number : MMIMMS_MAX_MMS_NUM);
        s_mms_auto_receive = mms_ms_config.mms_auto_receive;
        SCI_TRACE_LOW("[MMIMMS]MMIMMS_InitNDTSetting: %d, %d, %d", s_mms_max_number, s_mms_auto_receive, mms_ms_config.max_body_size);
        switch(mms_ms_config.max_body_size)
        {
            case MMS_100K:
                s_mms_max_body_size = 100 * 1024;
                break;
        
            case MMS_300K:
                s_mms_max_body_size = 300 * 1024;
                break;
        
            case MMS_600K:
                s_mms_max_body_size = 600 * 1024;
                break;
...
//    Note:
/*****************************************************************************/
PUBLIC  uint32 MMIMMS_GetMaxSize(void)
{
     SCI_PASSERT((s_mms_max_body_size != 0),( "MAX SIZE NOT 0"));
     SCI_TRACE_LOW("[MMIMMS]MMIMMS_GetMaxSize: s_mms_max_body_size=%d", s_mms_max_body_size);
     return s_mms_max_body_size;
}
...
//    Note:
/*****************************************************************************/
PUBLIC  uint16 MMIMMS_GetMaxNumber(void)
{
     SCI_PASSERT((s_mms_max_number != 0),( "MAX NUMBER NOT 0"));
     SCI_TRACE_LOW("[MMIMMS]MMIMMS_GetMaxNumber: s_mms_max_number=%d", s_mms_max_number);
     return s_mms_max_number;
}
...
PUBLIC MMIFILE_DEVICE_E MMIMMS_Setting_GetPreferDisk(void)
{
    MMIMMS_NV_SETTING_T nv_setting = {0};
    MN_RETURN_RESULT_E    nv_result = MN_RETURN_FAILURE;
    MMIFILE_DEVICE_E user_setting_dev_type = MMI_DEVICE_SYSTEM;

    //SCI_TRACE_LOW:"MMIMMS_Setting_GetPreferDisk"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMIMMS_SETTING_514_112_2_18_2_38_56_415,(uint8*)"");
    
    MMINV_READ(MMINV_MMS_SETTING, &nv_setting, nv_result);
    //connect the BYD prefferred storage in settings page. storage the pic attachments according to the user settings.
    user_setting_dev_type  = MMISet_GetPreferDisk();
    
    if(MN_RETURN_SUCCESS != nv_result)
    {
        nv_setting = MMIMMS_ResetMMSFactorySetting();
    }
    if(user_setting_dev_type != nv_setting.device_type)
    {
        nv_setting.device_type = user_setting_dev_type;
        MMINV_WRITE(MMINV_MMS_SETTING, &nv_setting);
    }
...
PUBLIC uint8 MMIMMS_Setting_GetNetSettingIndex(MN_DUAL_SYS_E dual_sys)
{
    ...
    SCI_TRACE_LOW("[szp:]bite:%d", sizeof(unsigned long));
    return index;
}
