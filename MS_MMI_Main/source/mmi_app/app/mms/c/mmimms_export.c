...
MMIMMS_EDIT_DOCUMENT_T * editdoc_multi_ptr = PNULL;
/**---------------------------------------------------------------------------*
 **                         TYPE AND CONSTANT    							  *
 **---------------------------------------------------------------------------*/
typedef struct 
...
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN MMIMMS_IsOnlyContainTextContent(MMIMMS_EDIT_DOCUMENT_T *editdoc_ptr);
    ...
    //if(string_to_ptr && string_to_ptr->wstr_ptr && 0 != string_to_ptr->wstr_len)
    {
        ...
        else
        {
            ...
            if(MMIAPISET_IsOpenPrivacyProtect(MMISET_PROTECT_SMS))
            {
                ...
                if(string_to_ptr &&string_to_ptr->wstr_ptr)
                {
...
//  uint32  unsettle_num                    = 0;
    ...
    {        
        ...
        else 
        {
			...
			else if (MMIMMS_IsOnlyContainTextContent(editdoc_ptr))
			{
				MMSMMI_OpenMMSAlertWin(TXT_MMS_ONLY_TEXT, TXT_NULL, IMAGE_PUBWIN_WARNING);  
				MMIMMS_DestroyEditDocument(editdoc_ptr);
				return FALSE;
			}
            ...
            { 
                ...
                {
                	MMK_SendMsg(MMISMS_SHOW_CHAT_SMS_WIN_ID, MSG_MMS_OPEN_ATTA_LIST_VIEW, PNULL);
					MMIMMS_DestroyEditDocument(editdoc_ptr);
    ...
    else
    {
        ...
        if(editdoc_ptr)
        {
            ...
            if(PNULL != file_info_ptr)
            {
                ...
                {
                   SCI_TRACE_LOW("[MMIMMS] func[%s] line[%d] MMIMMS_AddFile fail",__FUNCTION__,__LINE__);
                }
            }
                                      ...
                                      )
{
    ...
    else if (MMIMMS_MMS_LIMIT_SIZE - MMIMMS_SIZE_LEFT < file_info_ptr->file_size)
    {
        ...
        if (MMIFMM_FILE_TYPE_PICTURE == file_type 
            ...
            && file_info_ptr->file_size > reduce_pic_size
        )
        {
            MMK_CloseWin(win_id);
            editdoc_ptr = MMIMMS_CreateEditDocument();
...
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIMMS_EditMMSFromMultiFile(
                                      MMIFILE_FILE_INFO_T   *file_info_ptr,
                                      MMIFMM_FILE_TYPE_E   file_type,
                                      uint16 send_num,
                                      uint16 total_num
...
#endif
        if(send_num == total_num)
        {
            editdoc_multi_ptr = editdoc_ptr = MMIMMS_CreateEditDocument();
        }
        else
        {
            editdoc_ptr = editdoc_multi_ptr ;
        }
        if(editdoc_ptr)
        {
            if(send_num == 1)
                             ...
                             )
{
    ...
    {
        SCI_TRACE_LOW("[%s:%d]find mms order ptr faile for record_id: %d\n",__FUNCTION__,__LINE__,record_id);
        return FALSE;
    }
...
LOCAL BOOLEAN MMIMMS_IsOnlyContainTextContent(MMIMMS_EDIT_DOCUMENT_T *editdoc_ptr)
{
    ...
    for(pSlide = editbody_ptr->slide_root; pSlide; pSlide = pSlide->next)
    {
        ...
        {
			return_val = FALSE;
			SCI_TRACE_LOW("MMIMMS_IsOnlyContainTextContent: only contain text content = %d", return_val);
            return return_val;
        }
