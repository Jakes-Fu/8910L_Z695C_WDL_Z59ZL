...
#endif
#include "mmisms_text.h"
#define  MMIMMS_STRING_ALPHA  (255*0.6)
/**---------------------------------------------------------------------------*
 **                         Macro Declaration    							  *
 **---------------------------------------------------------------------------*/
...
typedef enum
{
	...
	MMISMS_PDA_MMS_SETTINGS_ADV_OPT,
...
{
    WIN_TITLE(STR_SMSSE_LIST_MULTIMEDIA),
...
WINDOW_TABLE( MMIMMS_RECEVINGSETTING_WIN_TAB ) = 
{
    ...
    WIN_TITLE(STR_HDR_MMS_RECEPTION),    
    WIN_SOFTKEY(PNULL, STXT_SOFTKEY_SELECT_MK, TXT_COMMON_COMMON_BACK),
...
WINDOW_TABLE( MMIMMS_SEND_SETTINGS_WIN_TAB ) = 
{
    ...
    WIN_TITLE(STXT_HEADER_VALIDITY),
#ifdef MMI_PDA_SUPPORT    
...
WINDOW_TABLE(MMIMMS_SETTING_RETRIEVE_MODE_WIN_TBL)=  
{
    ...
    WIN_TITLE(TXT_MMS_SET_RETRIEVE_LOCAL),
                                 ...
                                 )
{
	...
	item_data.item_content[0].item_data.text_id = text_1;
	   ...
	   if(text_str.wstr_ptr != PNULL && text_str.wstr_len != 0 && 
		   GUIITEM_STYLE_TWO_LINE_TEXT_AND_TEXT_MS != item_t.item_style)
	   {
		   item_data.item_content[2].item_data_type    = GUIITEM_DATA_TEXT_BUFFER;
		   item_data.item_content[2].item_data.text_buffer.wstr_ptr = temp_wstr;
		   item_data.item_content[2].item_data.text_buffer.wstr_len = temp_len;
	   ...
	   if (GUIITEM_STYLE_TWO_LINE_TEXT_AND_TEXT_ANIM == item_t.item_style || GUIITEM_STYLE_ONE_LINE_TEXT_ICON == item_t.item_style)
	   { 
		   item_data.item_content[1].item_data_type    = GUIITEM_DATA_IMAGE_ID;
		   item_data.item_content[1].item_data.image_id = image_id;
	   }
	   else if (GUIITEM_STYLE_TWO_LINE_TEXT_AND_TEXT_MS == item_t.item_style && 
		   text_str.wstr_ptr != PNULL && text_str.wstr_len != 0)
	   {
			GUILIST_SetItemState(&item_t, GUIITEM_STATE_COSTMER_FONT_COLOR, TRUE);
			//设置第1个content的字体颜色、透明度,包括高亮状态下和正常状态下
			GUILIST_SetItemContentCustomFontColor(ctrl_id, 1, &color, MMIMMS_STRING_ALPHA, &color,  MMIMMS_STRING_ALPHA);//设置第1个content的字体颜色,包括高亮状态下和正常状态下
...
LOCAL void  InitPdaMMSRecieveSettingsListCtrl(void)
{
    ...
    MMI_STRING_T  text_str = {0};
    uint32 item_index =0;
	MMI_TEXT_ID_T  mode_text[MMS_SETTING_DROPPAGE_NUM2] = {STR_MSG_MMS_RECEIV_IMME_EXT01,STR_MSG_MMS_RECEIV_MANUAL_EXT01,STR_MSG_MMS_RECEIV_DENY_EXT01};
 	MMIMMS_SETTING_RETRIEVE_MODE_E mode = 0;
    uint16 cur_select = GUILIST_GetCurItemIndex(ctrl_id);
    ...
    MMI_GetLabelTextByLang(mode_text[mode], &text_str);
    ...
    AppendPdaMMSSettingsItem2Line(ctrl_id,
                                  ...
                                  IMAGE_COMMON_RIGHT_ARROW,
                                  MMISMS_PDA_MMS_RETRIEVE_MODE_LOCAL,
                                  FALSE,GUIITEM_STYLE_TWO_LINE_TEXT_AND_TEXT_MS);
    item_index++;
    SCI_MEMSET(&text_str, 0x00, sizeof(text_str));
	...
	GUILIST_SetToggleState(ctrl_id,item_index,flag);
    item_index++;
    ...
    SCI_MEMSET(&text_str, 0x00, sizeof(text_str)); 
    flag = MMIMMS_Setting_GetDeliveryReport();  //modify to delivery report as MS ndt setting
    if(flag)
    {
        image_id = IMAGE_CHECK_SELECTED_ICON;
    }
...
LOCAL void  InitPdaMMSSettingsListCtrl(BOOLEAN is_need_replace)
{
    ...
    {
#ifndef MMI_MULTI_SIM_SYS_SINGLE
        STR_SIM1_MMS_APN_EXT01,
        STR_SIM2_MMS_APN_EXT01,
#if defined(MMI_MULTI_SIM_SYS_TRI)||defined(MMI_MULTI_SIM_SYS_QUAD)
        TXT_MMS_USER3,
#if defined(MMI_MULTI_SIM_SYS_QUAD)
        TXT_MMS_USER4,
    ...
    {
        if( ((is_need_replace && 0 == focus_index|| 1 == focus_index) ||!is_need_replace) && MMIAPIPHONE_GetSimExistedStatus(i))
        {
	...
	if (!is_need_replace)
	{
		...
		AppendPdaMMSSettingsItem2Line(            
			ctrl_id,
			STR_MMS_SETT_RECEIVE_ADS,
			text_str,
			PNULL,  
			(uint32)MMISMS_PDA_MMS_SETTINGS_ADV_OPT,
			FALSE,
			GUIITEM_STYLE_TOGGLE_ONE_LINE_TEXT);	
                                           ...
                                           )
{
	...
	MMI_STRING_T str_sim1_apn = {0};
	MMI_STRING_T str_sim2_apn = {0};

	MMI_GetLabelTextByLang(STR_HDR_SIM1_MMS_APN_EX01,&str_sim1_apn);
	MMI_GetLabelTextByLang(STR_HDR_SIM2_MMS_APN_EX01,&str_sim2_apn);

    SCI_TRACE_LOW("[MMIMMS] HandleMMSSettingWinMsg: msg_id=0x%x", msg_id);
    switch(msg_id)
    {
...
//    case MSG_CTL_OK:
            ...
            switch((MMISMS_PDA_MMS_SETTINGS_TYPE_E)user_data)
            {
                ...
                case MMISMS_PDA_MMS_SIM4_SC:
					...
					if (!MMIAPIPHONE_GetSimExistedStatus(MN_DUAL_SYS_1))
					{
		            	...
		            	stb_data_text.wstr_len = str_sim2_apn.wstr_len;
						SCI_MEMCPY(stb_data_text.wstr_ptr, str_sim2_apn.wstr_ptr,
					...
					else
					{
						...
						{
							GUISTBDATA_GetItemText(0,&stb_data_text);
							stb_data_text.wstr_len = str_sim1_apn.wstr_len;
							SCI_MEMCPY(stb_data_text.wstr_ptr, str_sim1_apn.wstr_ptr,
							sizeof(wchar)*MIN(stb_data_text.wstr_len, MMISTATUSBAR_ITEM_TEXT_LEN));
							GUISTBDATA_SetItemText(0,&stb_data_text);
						} 
		         		else if( 1 == index )
	...
	case MSG_CTRL_TOGGLE_CHANGED:
		...
		{
		case MMISMS_PDA_MMS_SETTINGS_ADV_OPT:
            {
                BOOLEAN  is_flag = !MMIMMS_Setting_GetAdvertising();
                MMIMMS_Setting_SetAdvertising(is_flag); 
...
#ifdef MMI_PDA_SUPPORT
                MMIAPICONNECTION_CreateLinklistWin(MMICONNECTION_ACCOUNT_MMS, list_index,
                    (MN_DUAL_SYS_E)i, MmsConnectionCallback);
#else
                MMIAPICONNECTION_CreateLinklistWin(MMICONNECTION_ACCOUNT_MMS, list_index,
                    (MN_DUAL_SYS_E)i, MmsConnectionCallback);
#endif
...
LOCAL void MmsConnectionCallback(MN_DUAL_SYS_E dual_sys, uint32 selected_index)
{
    ...
    MMSMMI_OpenMMSAlertWin(TXT_DONE_COMMAND, TXT_NULL, IMAGE_PUBWIN_SUCCESS);
                                           ...
                                           )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        MMIAPISET_AppendListItemByTextIdExt(TXT_COMMON_ENABLE_ON, STXT_SOFTKEY_OK_MK, TXT_NULL, STXT_EXIT, list_ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO );//open
        MMIAPISET_AppendListItemByTextIdExt(STR_POC_ILIST_OFF, STXT_SOFTKEY_OK_MK, TXT_NULL, STXT_EXIT, list_ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO );//close
                                           ...
                                           )
{
	...
	GUISTBDATA_TEXT_T stb_data_text = {0};
	MMI_STRING_T str_reception = {0};
	MMI_STRING_T str_local = {0};
	MMI_STRING_T str_roaming = {0};
	MMI_GetLabelTextByLang(STR_HDR_MMS_RECEPTION,&str_reception);
	MMI_GetLabelTextByLang(TXT_MMS_SET_RETRIEVE_LOCAL,&str_local);
	MMI_GetLabelTextByLang(TXT_MMS_SET_RETRIEVE_ROAM,&str_roaming);
...
//    case MSG_CTL_OK:
    ...
    case MSG_CTL_MIDSK:
         {
            ...
            {           
                ...
                case MMISMS_PDA_MMS_RETRIEVE_MODE_ROAM:
					...
					{
		                GUISTBDATA_GetItemText(0,&stb_data_text);
                        stb_data_text.wstr_len = str_local.wstr_len;
                        SCI_MEMCPY(stb_data_text.wstr_ptr, str_local.wstr_ptr,
                        sizeof(wchar)*MIN(stb_data_text.wstr_len, MMISTATUSBAR_ITEM_TEXT_LEN));
						GUISTBDATA_SetItemText(0,&stb_data_text);
					} 
		         	else if( 1 == index )
					{
		        		GUISTBDATA_GetItemText(0,&stb_data_text);
		            	stb_data_text.wstr_len = str_roaming.wstr_len;
			            SCI_MEMCPY(stb_data_text.wstr_ptr, str_roaming.wstr_ptr,
		             	sizeof(wchar)*MIN(stb_data_text.wstr_len, MMISTATUSBAR_ITEM_TEXT_LEN));
						GUISTBDATA_SetItemText(0,&stb_data_text);
					}
                        //MMK_CreatePubListWin((uint32 *)MMIMMS_SETTING_RETRIEVE_MODE_WIN_TBL, (ADD_DATA)user_data);
						MMK_CreateWin((uint32 *)MMIMMS_SETTING_RETRIEVE_MODE_WIN_TBL, (ADD_DATA)user_data);
	...
	case MSG_CTRL_TOGGLE_CHANGED:
        ...
        switch((MMISMS_PDA_MMS_RECIEVE_TYPE_E)user_data)
        {
            ...
            case MMISMS_PDA_MMS_SEND_REPORT:
            {
                BOOLEAN  is_flag = !MMIMMS_Setting_GetDeliveryReport();
                MMIMMS_Setting_SetDeliveryReport(is_flag);
                MMIMMS_Setting_SetRecvDeliveryReport(is_flag);          
                GUILIST_SetToggleState(ctrl_id,index,is_flag);
            }
            break;
     ...
     case MSG_FULL_PAINT:
        ...
        stb_data_text.wstr_len = str_reception.wstr_len;
        SCI_MEMCPY(stb_data_text.wstr_ptr, str_reception.wstr_ptr,
        sizeof(wchar)*MIN(stb_data_text.wstr_len, MMISTATUSBAR_ITEM_TEXT_LEN));	
        GUISTBDATA_SetItemText(0,&stb_data_text);
        GUILIST_SetListState(ctrl_id, GUILIST_STATE_SPLIT_LINE, FALSE);
        break;
                                               ...
                                               )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        GUILABEL_SetTextById(MMS_SETTING_PERIOD_TITLE_LABEL_CTRL_ID, STXT_HEADER_VALIDITY,FALSE);
                                            ...
                                            )
{
    ...
    case MSG_OPEN_WINDOW:       
        ...
        {
            GUIWIN_SetTitleTextId(win_id,STXT_HEADER_VALIDITY,FALSE);
...
LOCAL void SetRecvDropDownList( void )
{
    ...
    MMI_TEXT_ID_T  aoto_text[MMS_SETTING_DROPPAGE_NUM1]                 = {STR_POC_ILIST_OFF, TXT_COMMON_ENABLE_ON};
    MMI_TEXT_ID_T  adv_text[MMS_SETTING_DROPPAGE_NUM1]                  = {STR_POC_ILIST_OFF, TXT_COMMON_ENABLE_ON};
    MMI_TEXT_ID_T  sendrpt_text[MMS_SETTING_DROPPAGE_NUM1]              = {STR_POC_ILIST_OFF, TXT_COMMON_ENABLE_ON};
...
LOCAL void SetRecvDropDownList( void )
{
    ...
    MMI_STRING_T rpt_dropdownlist[MMS_SETTING_DROPPAGE_NUM1]      = {0};
#ifdef DATA_ROAMING_SUPPORT
    MMI_TEXT_ID_T  mode_text[MMS_SETTING_DROPPAGE_NUM2] = {STR_MSG_MMS_RECEIV_IMME_EXT01,
                                                               STR_MSG_MMS_RECEIV_MANUAL_EXT01,
                                                               STR_MSG_MMS_RECEIV_DENY_EXT01};
#else
    MMI_TEXT_ID_T  aoto_text[MMS_SETTING_DROPPAGE_NUM1]                 = {STR_POC_ILIST_OFF, TXT_COMMON_ENABLE_ON};
#endif
    MMI_TEXT_ID_T  adv_text[MMS_SETTING_DROPPAGE_NUM1]                  = {STR_POC_ILIST_OFF, TXT_COMMON_ENABLE_ON};
...
LOCAL void SetSendDropDownList( void )
{
    ...
    MMI_TEXT_ID_T  priority_text[MMS_SETTING_DROPPAGE_NUM2]              = {TXT_COMMON_HIGH, TXT_COMMON_MID,TXT_COMM_LOW};
    MMI_TEXT_ID_T  sendrpt_text[MMS_SETTING_DROPPAGE_NUM1]               = {STR_POC_ILIST_OFF, TXT_COMMON_ENABLE_ON};
    MMI_TEXT_ID_T  readrpt_text[MMS_SETTING_DROPPAGE_NUM1]               = {STR_POC_ILIST_OFF, TXT_COMMON_ENABLE_ON};
    MMI_TEXT_ID_T  anonymity_text[MMS_SETTING_DROPPAGE_NUM1]             = {STR_POC_ILIST_OFF, TXT_COMMON_ENABLE_ON};
    ...
    case MSG_OPEN_WINDOW:
        {
			...
			GUILIST_SetListState(MMIMMS_SETTING_RETRIEVE_MODE_CTRL_ID, GUILIST_STATE_OWN_SOFTKEY, TRUE);
