        ...
        case ID_MMS_MENU_EXTRACT_ADDTOPB_EXIST_ITEM_ID:
            {
                ...
                // save to phonebook 
                if( '\0' != mms_list_info_ptr->sender[0])
                {
                    MMIAPICOM_GenPartyNumber(mms_list_info_ptr->sender,(uint16)SCI_STRLEN((char *)mms_list_info_ptr->sender), &party_num);
                }
        ...
        {  
          ...
          //  list_item_ptr = GUILIST_GetItemPtrByIndex(menu_ctrl_id, select_index);
            ...
            if (MMIMMS_NO_ERROR != mms_op_res)
            {
                ...
                else if (MMIMMS_ERROR_NO_SDCARD == mms_op_res)
                {
                    MMSMMI_OpenMMSAlertWin(STR_NOT_ENOUGH_MEMORY_JT, TXT_NULL, IMAGE_PUBWIN_WARNING);
                }
...
LOCAL void MMSDetailExpiryString(MMI_STRING_T *str_dst, uint32 expiry_time)
{
    ...
    MMI_GetLabelTextByLang(STXT_HEADER_VALIDITY, &temp_string);
...
#ifdef DRM_SUPPORT  
			if (PNULL != music_name_ptr && (MMIFMM_DM_FILE == MMIMMS_GetDrmFileTypeFromName(music_name_ptr) || MMIFMM_DCF_FILE == MMIMMS_GetDrmFileTypeFromName(music_name_ptr)))
			{
				item.img_type = GUIRICHTEXT_IMAGE_RES;
...
PUBLIC void MMIMMS_OutboxSendMMS(uint32 record_id, MMIMMS_EDIT_DOCUMENT_T * edit_doc_ptr)
{
    ...
    else if (1 == sim_num)
    {
        ...
        //MMSMMI_CreateSendingWaitingWin(send_dual_sim, record_id);  //not show sending win
    }
    else
    {
