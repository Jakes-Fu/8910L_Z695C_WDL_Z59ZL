    ...
    case MMS_PUSH_TYPE_NOTIFICATION_IND:
        {
            ...
            if(MMIMMS_Setting_GetAdvertising() || (msg && 0x81 != msg->message_class_value)) //MMS广告判断 
            {
                ...
                SCI_TRACE_LOW("[MMIMMS] HandleMMSPushCallback: file:%s, line:%d", _D_FILE_NAME, _D_FILE_LINE);
#if 0
                if(MMI_DEVICE_NUM <= MMIAPIFMM_GetFirstValidSD())
                {
                    received_para.push_status = MMIMMS_PUSH_NEW_NO_SDCARD;
...
#endif
                if(!MMIMMS_IsEnoughSpace(MMIMMS_RESERVE_HEAD_SIZE))//save mms push space
                {
                    SCI_TRACE_LOW("[MMIMMS] HandleMMSPushCallback: not Enough Space");
                ...
                else
                {
                    ...
                    SCI_TRACE_LOW("[MMIMMS] HandleMMSPushCallback: DATA_ROAMING_SUPPORT, line:%d, retrieve_mode=%d", _D_FILE_LINE, retrieve_mode);
                    if (MMIMMS_SETTING_RETRIEVE_MODE_DENY == retrieve_mode)
                    {
                        MMIMMS_ResponceMMS(s_push_dual_sys, msg, MMS_STATUS_VALUE_REJECTED);
                    }
#if 0
                    else if(MMIMMS_IsMaxNum() || MMIMMS_IsCurrentChatTotalSizeOverrun(msg->msgsize))
                    {
                        retrieve_mode = MMIMMS_SETTING_RETRIEVE_MODE_DEFER;
                    }
#endif
                    else
#endif
                    ...
                    #ifdef DATA_ROAMING_SUPPORT
                        SCI_TRACE_LOW("[MMIMMS] HandleMMSPushCallback: DATA_ROAMING_SUPPORT, line:%d, retrieve_mode=%d", _D_FILE_LINE, retrieve_mode);
                        if(MMIMMS_IsMaxNum() || MMIMMS_IsCurrentChatTotalSizeOverrun(msg->msgsize))
                        {
                            retrieve_mode = MMIMMS_SETTING_RETRIEVE_MODE_DEFER;
                        }
                        #if 0
                    ...
                    if(MMIPHONE_ROAMING_STATUS_NO_ROMING != MMIAPIPHONE_GetRoamingStatus(s_push_dual_sys))
                    {
...
#endif
                        SCI_TRACE_LOW("[MMIMMS] MMIMMS_HandleMMSPushCallback: retrieve_mode=%d", retrieve_mode);
                        if(MMIMMS_IsMaxNum() || MMIMMS_IsCurrentChatTotalSizeOverrun(msg->msgsize))
                        {
                            retrieve_mode = MMIMMS_SETTING_RETRIEVE_MANUAL;
                        }
                        ...
                        {  
                            SCI_TRACE_LOW("[MMIMMS] MMIMMS_HandleMMSPushCallback: retrieve_mode=%d, FirstValidSD=%d, mo_mt_type=%d", retrieve_mode, MMIAPIFMM_GetFirstValidSD(), mo_mt_type);
                ...
                SCI_TRACE_LOW("[MMIMMS] HandleMMSPushCallback: push_status=%d", received_para.push_status);
#ifdef DATA_ROAMING_SUPPORT
                retrieve_mode = MMIMMS_Setting_GetCurRetrieveMode(s_push_dual_sys);
                if (MMIMMS_SETTING_RETRIEVE_MODE_DENY != retrieve_mode)
                {
