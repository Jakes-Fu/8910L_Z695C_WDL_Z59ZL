                                            ...
                                            uint32 *all_space_ptr)
{
    ...
    *total_size_ptr = MMIMMS_GetTotalSize();
    ...
    max_size = sizeof(MMIMMS_LIST_FILE_HEAD_T)+max_mms_number*sizeof(MMISMS_ORDER_INFO_T);
    max_size += max_mms_number * MMIMMS_MMS_LIMIT_SIZE;
                                 ...
                                 )
{
	...
	SCI_TRACE_LOW("MMIMMS: HandleHttpMsg Enter...");
...
PUBLIC uint32 MMIMMS_GetMMSLimitSize(void)
{
    ...
    uint32 mms_max_size = MMIMMS_GetMaxSize();
    ...
    if (NULL == mms_limit_size || mms_limit_size > mms_max_size)
#endif
    {
#ifdef CSC_XML_SUPPORT
        if (10*1024 < s_mmimms_mms_limit && s_mmimms_mms_limit < mms_max_size)
        {
