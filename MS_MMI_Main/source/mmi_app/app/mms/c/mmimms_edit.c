...
LOCAL wchar *s_mms_text_content_ptr = PNULL;

/**---------------------------------------------------------------------------*
 **                         Constant Variables                                *
 **---------------------------------------------------------------------------*/
#define DELETE_ALL_ACCESSORY_FLAG 1111
/**--------------------------------------------------------------------------*
 **                         STATIC DEFINITION                                *
 **--------------------------------------------------------------------------*/
...
/*****************************************************************************/
//  Description : get the file icon for the given type
//  Global resource dependence : none
//  Author: arvin
//  Note:
/*****************************************************************************/
LOCAL MMI_IMAGE_ID_T GetFileTypeIcon(MMIFMM_FILE_TYPE_E      file_type);
/**--------------------------------------------------------------------------*
 **                         FUNCTION DEFINITION                              *
 **--------------------------------------------------------------------------*/
/*****************************************************************************/
                              ...
                              )
{
    ...
    SCI_TRACE_LOW("[MMIMMS] IsAvailableType:type=%d, file_type=%d", type, file_type);
    switch(file_type)
    {
...
//*****************************************************************************/
//  Description : del multimedia file in given slide
//  Global resource dependence : none
//  Author: arvin
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIMMS_DelMultimFileInSlide(
                    MMIFMM_FILE_TYPE_E file_type,//in(MMIFMM_FILE_TYPE_PICTURE/MOVIE/MUSIC)				
					uint32 slide_index,//in
                    ...
                    )
{
	...
	//valid slide
	slide_count = MMIMMS_GetSlideCount(editdoc_ptr);
	...
	for(slide_count = 0; slide; slide = slide->next)
	{
		if(slide_index == slide_count++)
		{
			//hit the target slide
			if(MMIFMM_FILE_TYPE_PICTURE == file_type ||MMIFMM_FILE_TYPE_MOVIE == file_type )
			{
				content = slide->content_root;
				while (content)
				{
				    if (IsAvailableType((uint16)content->type, file_type))
				    {
				        result = TRUE;
				        break;
				    }
				...
				//delete the slide current content
			...
			else if(MMIFMM_FILE_TYPE_MUSIC == file_type)
			{
			    ...
			    {
			        vfname_ptr = (uint8 *)slide->audiofile;
			    ...
			    MMS_DeleteAudio(slide, editdoc_ptr->editbody_ptr);
			    result = TRUE;
			}

			if (0 == slide->text_flag && 0 == slide->image_flag && 0 == slide->video_flag && 0 == slide->audiosize)
			{
				MMIMMS_DelSlide(editdoc_ptr);

                MMIMMS_HeadSlide(editdoc_ptr); //删完之后定位到第一页

                if(PNULL == editdoc_ptr->editbody_ptr->slide_root)
                {
                    MMIMMS_SlideInit();
                }
                else
        ...
        MMS_NextSlide(editdoc_ptr->editbody_ptr);
	}
	//[ARVIN_ADD_TRACE]file_type + slide_index + result
...
LOCAL MMI_IMAGE_ID_T GetFileTypeIcon(MMIFMM_FILE_TYPE_E      file_type)
{
    ...
    SCI_TRACE_LOW("GetFileTypeIcon:file_type=%d", file_type);

    switch(file_type)
    {
    //where's the calenda type definition????
    //MMS_TYPE_TEXT:
    //MMS_TYPE_WBMP:
    //MMS_TYPE_GIF:
    //MMS_TYPE_JPEG:
    ...
    case MMS_TYPE_MIDI:
    case MMS_TYPE_MP3:
		icon_id = IMAGE_MESSAGING_MUSIC_ICON;
	break;

    case MMS_TYPE_AVI://BUG 494339
    case MMIFMM_FILE_TYPE_MOVIE:
    case MMIFMM_FILE_TYPE_MTV:
		icon_id = IMAGE_MESSAGING_PLAY_ICON;//temp solution
	break;

    case MMS_TYPE_VCF:
		icon_id = IMAGE_MESSAGING_VCF_ICON;
	break;

#ifdef MMI_VCALENDAR_SUPPORT
    //case MMIFMM_FILE_TYPE_VCALENDAR:
	//icon_id = IMAGE_MESSAGING_CALENDAR_ICON;
	//break;
#endif
    default:
		icon_id = IMAGE_MESSAGING_ATTACHMENT_ICON;
	break;
    }
...
/*****************************************************************************/
//  Description : get accessory_ptr ptr by index
//  Global resource dependence : none
//  Author: rong.gu
//  Note:
/*****************************************************************************/
PUBLIC MMS_ACCESSORY_PARAM_T* MMIMMS_GetAccessoryPtrByIndex(
                                              MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr,
                                              uint16 accessory_index
                                              )
{
    MMS_ACCESSORY_PARAM_T* accessory_ptr = PNULL;
    P_MMS_EDIT_BODY_T     editbody_ptr = PNULL;
    uint16 accessory_number = 0;
    if(PNULL == editdoc_ptr || PNULL == editdoc_ptr->editbody_ptr)
    {
        return accessory_ptr;
    }
    editbody_ptr = editdoc_ptr->editbody_ptr;
    
    accessory_ptr = editbody_ptr->accessory_ptr;

    for(; accessory_ptr; accessory_ptr = accessory_ptr->next, ++accessory_number)
    {
        if(accessory_index == accessory_number)
        {
            break; 
        }            
    }
...
/*****************************************************************************/
//  Description : get slide ptr by index
//  Global resource dependence : none
//  Author: rong.gu
//  Note:
/*****************************************************************************/
PUBLIC P_MMS_SLIDE_T MMIMMS_GetSlidePtrByIndex(
                                              MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr,
                                              uint16 slide_index
                                              )
{
...
/*****************************************************************************/
//  Description : get multi-media file info of the given slide
//  Global resource dependence : none
//  Author: arvin
//  Note:
/*****************************************************************************/
PUBLIC uint32 MMIMMS_GetMultimediaFileInfoInSlide(
                                              MMIFMM_FILE_TYPE_E file_type,//in(MMIFMM_FILE_TYPE_PICTURE/MOVIE/MUSIC)
                                              uint32 *type,//out ---the real type of the file(jpg/bmp/mp3/midi...) 
                                              uint8 *fname,//out
                                              uint32 fname_max_len,//in
                                              P_MMS_SLIDE_T slide_ptr)//in
{
    uint32          file_name_len=0;
    uint32          file_size =0;
    ...
    switch(file_type)
    {
        ...
        case MMIFMM_FILE_TYPE_MOVIE:
        {
            P_MMS_CONTENT_T content = slide_ptr->content_root;
            for(;content;content = content->next)
            {
                if(IsAvailableType((uint16)content->type, file_type))
                {
        ...
        {
            file_name_len = SCI_STRLEN(slide_ptr->audiofile);
            file_name_len=MIN(fname_max_len,file_name_len);
            SCI_MEMCPY(fname,slide_ptr->audiofile,file_name_len);
            file_size=slide_ptr->audiosize;
            //*type = MMSTypeConvertToAudioType(slide_ptr->audio_type);			    
            *type = slide_ptr->audio_type;    //no need convert
        }
        break;
...
/*****************************************************************************/
//  Description : get the text content memory
//  Global resource dependence : none
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC void MMIMMS_GetTextContentMem(uint16 len)
{
	s_mms_text_content_ptr = (wchar *)SCI_ALLOC_APPZ(len * sizeof(wchar) + 1);

	if (PNULL == s_mms_text_content_ptr)
	{
		SCI_TRACE_LOW("MMIMMS: MMIMMS_GetTextContent s_mms_text_content_ptr MALLOC FAIL!");
	}	
}
/*****************************************************************************/
//  Description : get the text content memory
...
//  Note:
/*****************************************************************************/
PUBLIC void MMIMMS_FreeTextContentMem()
{
	if (PNULL != s_mms_text_content_ptr)
	{
		SCI_FREE(s_mms_text_content_ptr);
	}	
}

/*****************************************************************************/
//  Description : get the text content count
//  Global resource dependence : none
//  Author: rong.gu
//  Note:
/*****************************************************************************/
PUBLIC uint16 MMIMMS_GetTextContentCount(
                                           MMIMMS_EDIT_DOCUMENT_T   *editdoc_ptr
                                           )
{
    uint16         total_textcontent_count = 0;
    P_MMS_SLIDE_T pSlide = NULL;
    P_MMS_CONTENT_T    content = PNULL;
    P_MMS_EDIT_BODY_T editbody_ptr = PNULL;
    ...
    //Tranverse all slides to get the first text content
    //maybe we should expand to get all text contents and combine them later...
    for(pSlide = editbody_ptr->slide_root; pSlide; pSlide = pSlide->next)
    {
        if(pSlide->text_flag)
        {
            //for bug 519343
            content = pSlide->content_root;
            for(;content;content = content->next)
            {
                if(content->type == MMS_TYPE_TEXT)
                {
                    total_textcontent_count++;
                }
            }
        }
    }

    return total_textcontent_count;
}

/*****************************************************************************/
//  Description : For there's no image included in the mms so that we should get
//                             first non-image Attachment if exists
//  Global resource dependence : none
//  Author: arvin.zhang
//  Note: return TRUE if exists and package the detail info(type icon and file
//              name), or else return FALSE
/*****************************************************************************/
PUBLIC BOOLEAN MMIMMS_GetFirstNonImageFileInfoIfExists(uint32                   *type, 
                                           uint32 fname_max_len,
                                           uint8               *pFilename, 
                                           MMI_IMAGE_ID_T *icon_id_ptr,
                                           MMIMMS_EDIT_DOCUMENT_T   *editdoc_ptr
                                           )
{
    uint32          file_name_len=0;
    ...
    //Traverse all slides begining from slide_root to find the first non-image file
    for(pSlide = editbody_ptr->slide_root; pSlide; pSlide = pSlide->next)
    {
        //set the audio file top priority
        if (pSlide->audiofile[0] != '\0')
        {
            file_name_len = SCI_STRLEN(pSlide->audiofile);
            file_name_len=MIN(fname_max_len,file_name_len);
            SCI_MEMCPY(pFilename, pSlide->audiofile, file_name_len);
            *type = pSlide->audio_type;
            *icon_id_ptr = GetFileTypeIcon(pSlide->audio_type);
            return_val = TRUE;
            SCI_TRACE_LOW("MMIMMS_GetFirstNonImageFileInfoIfExists1:return_val=%d", return_val); 
            return return_val;
        }
        //If content_root of the current slide is null, we can run into the next slide directly
        //without thinking about content_root->next for it should be null as well.
        content = pSlide->content_root;
        for(; content; content = content->next)
        {
               if(MMS_TYPE_TEXT != content->type)
               {
                   file_name_len = SCI_STRLEN(content->filename);
                   file_name_len=MIN(fname_max_len,file_name_len);
                   SCI_MEMCPY(pFilename, content->filename, file_name_len);
                   *type = content->type;
                   *icon_id_ptr = GetFileTypeIcon(content->type);
                   return_val = TRUE;
                   SCI_TRACE_LOW("MMIMMS_GetFirstNonImageFileInfoIfExists2:return_val=%d", return_val); 
                   return return_val;
               }
        }
    }

    for(accessory_ptr = editbody_ptr->accessory_ptr; accessory_ptr; accessory_ptr = accessory_ptr->next)
    {
        if(accessory_ptr->type != MMS_TYPE_TEXT)
        {
            file_name_len = SCI_STRLEN(accessory_ptr->filename);
            file_name_len=MIN(fname_max_len,file_name_len);
            SCI_MEMCPY(pFilename, accessory_ptr->filename, file_name_len);
            *type = accessory_ptr->type;
            *icon_id_ptr = GetFileTypeIcon(accessory_ptr->type);
            return_val = TRUE;
            break;
        }
    }
    SCI_TRACE_LOW("MMIMMS_GetFirstNonImageFileInfoIfExists3:return_val=%d", return_val); 
    return return_val;
}
...
{
    uint16 iSlideCount = 0;

    if(PNULL == editdoc_ptr)
    {
        return 0;
    }
    iSlideCount = MMS_GetSlideCount(editdoc_ptr->editbody_ptr);
    SCI_TRACE_LOW("MMIMMS_GetSlideCount: iSlideCount=%d", iSlideCount);
...
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIMMS_GetVideoFromSlide(uint32                   *type, 
                                           uint32                   *multim_buf_len,
                                           void                     **multim_buf_ptr,
                                           P_MMS_SLIDE_T   slide_ptr
                                           )
{
    BOOLEAN         return_val = FALSE;
    P_MMS_CONTENT_T     content = NULL;
    ...
    {
        content = slide_ptr->content_root;
        for(;content;content = content->next)
        {
            if(IsAvailableType((uint16)content->type, MMIFMM_FILE_TYPE_MOVIE))
            {
...
/*****************************************************************************/
//  Description : del attachment based on accessory index
//  Global resource dependence : none
//  Author: arvin
//  Note:accessory_index == 0xFFFF FFFF stands for delete all accessories
/*****************************************************************************/
PUBLIC BOOLEAN MMIMMS_DelSelectedAccessory(
                                           uint32 accessory_index,
                                           MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr
                                           )
{
    BOOLEAN result = FALSE;
    uint32 index = 0;
    MMS_ACCESSORY_PARAM_T* accessory_ptr = PNULL;
    if(PNULL == editdoc_ptr || PNULL == editdoc_ptr->editbody_ptr || PNULL == editdoc_ptr->editbody_ptr->accessory_ptr)
    {
        return result;
    }
    
    accessory_ptr = editdoc_ptr->editbody_ptr->accessory_ptr;
    for(; accessory_ptr; accessory_ptr = accessory_ptr->next, ++index)
    {
        if(((uint32)DELETE_ALL_ACCESSORY_FLAG == accessory_index) || (index == accessory_index))
        {
            result = MMS_DeleteAttachment((char *)accessory_ptr->filename, editdoc_ptr->editbody_ptr);
            if( index == accessory_index)  //DELETE_ALL_ACCESSORY_FLAG need delete all accessory
            {
                break;
            }
    ...
    //[ARVIN_ADD_TRACE]accessory_index + result

    return FALSE;
}

/*****************************************************************************/
//  Description : get multi-media file  of the given slide
//  Global resource dependence : none
//  Author: arvin
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIMMS_GetMultimediaBufferInSlide(
                                                 MMIFMM_FILE_TYPE_E file_type,//in(MMIFMM_FILE_TYPE_PICTURE/MOVIE/MUSIC)
                                                 uint32 *multim_buf_len,
                                                 void **multim_buf_ptr,
                                                 P_MMS_SLIDE_T slide_ptr)//in
{
    ...
    {
        *multim_buf_ptr = PNULL;
        sysVFGetConstFileData(filename,(int8 **)multim_buf_ptr,(int32 *)multim_buf_len);
        if(*multim_buf_ptr)
        {
            return_val = TRUE;
        }
...
/*****************************************************************************/
//  Description : get the text content len
//  Global resource dependence : none
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC uint16 MMIMMS_GetTextContentLength(MMIMMS_EDIT_DOCUMENT_T *editdoc_ptr)
{
    uint32          str_len = 0;
    uint8*          str_ptr = PNULL;
        ...
        if(pSlide->text_flag)
        {
            ...
            if(content)
            {
                if(sysVFGetConstFileData(content->filename,(int8 **)&str_ptr,(int32 *)&str_len))
                {
                    string_ptr.wstr_ptr = (wchar*)(str_ptr+sizeof(uint32));
                    string_ptr.wstr_len = MMIAPICOM_Wstrlen(string_ptr.wstr_ptr);
                    content_len += string_ptr.wstr_len;
                    
                    if (PNULL != pSlide->next)
                    {
                        content_len += 2;
                    }
                }     
...
/*****************************************************************************/
//  Description : get the text content(now we only get the first text)
//  Global resource dependence : none
//  Author: arvin
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIMMS_GetTextContent(
                                     MMIMMS_EDIT_DOCUMENT_T   *editdoc_ptr,
                                     MMI_STRING_T             *string_ptr
                                     )
{
...
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIMMS_GetAllSlideTextContent(
                                             MMIMMS_EDIT_DOCUMENT_T   *editdoc_ptr,
                                             MMI_STRING_T             *string_ptr
                                             )
{
    ...
    content_len = MMIMMS_GetTextContentLength(editdoc_ptr);
    
    SCI_TRACE_LOW("MMIMMS: get text content content_len = %d", content_len);
    
    MMIMMS_GetTextContentMem(content_len);
    ...
    head_ptr = s_mms_text_content_ptr;
    editbody_ptr = editdoc_ptr->editbody_ptr;
        ...
        if(pSlide->text_flag)
        {
            ...
            if(content)
            {
                if(sysVFGetConstFileData(content->filename,(int8 **)&str_ptr,(int32 *)&str_len))
                {
                    ...
                    SCI_MEMCPY(s_mms_text_content_ptr, string_ptr->wstr_ptr, (string_ptr->wstr_len * 2));
                    s_mms_text_content_ptr += string_ptr->wstr_len;
                    
                    if (PNULL != pSlide->next)
                    {
                        //每个slide间以换行分隔
                        *s_mms_text_content_ptr = 13;
                        s_mms_text_content_ptr++;
                        *s_mms_text_content_ptr = 10;
                        s_mms_text_content_ptr++;
                    }
    ...
    s_mms_text_content_ptr = head_ptr;
    string_ptr->wstr_ptr = head_ptr;
...
/*****************************************************************************/
//  Description : Get First Image Attachment if exists
//  Global resource dependence : none
//  Author: arvin.zhang
//  Note: return TRUE if exists and package the detail info, or else return FALSE
/*****************************************************************************/
PUBLIC BOOLEAN MMIMMS_GetFirstImageFileIfExists(uint32                   *type, 
                                                uint32                   *multim_buf_len,
                                                void                     **multim_buf_ptr,
                                                MMIMMS_EDIT_DOCUMENT_T   *editdoc_ptr
                                                )
{
    ...
    //Traverse all slides begining from slide_root to find the image file
    for(pSlide = editbody_ptr->slide_root; pSlide; pSlide = pSlide->next)
    {
        //If image_flag is true, we'll jump into and traverse to looking for its picture
        if (pSlide->image_flag)
        {
            content = pSlide->content_root;
            for(;content; content = content->next)
            {
    ...
    {
        MMS_ACCESSORY_PARAM_T* accessory_ptr = PNULL;
        SCI_TRACE_LOW("MMIMMS_GetFirstImageFileIfExists: (FALSE == return_val)");  
        for(accessory_ptr = editbody_ptr->accessory_ptr; accessory_ptr; accessory_ptr = accessory_ptr->next)
        {
            if(accessory_ptr->type == MMS_TYPE_GIF ||
                accessory_ptr->type == MMS_TYPE_JPEG ||
                accessory_ptr->type == MMS_TYPE_WBMP ||
                accessory_ptr->type == MMS_TYPE_BMP1 ||
                accessory_ptr->type == MMS_TYPE_PNG )
            {
                *multim_buf_ptr = PNULL;
                sysVFGetConstFileData(accessory_ptr->filename,(int8 **)multim_buf_ptr,(int32 *)multim_buf_len);
                SCI_TRACE_LOW("MMIMMS_GetFirstImageFileIfExists: (FALSE == return_val): *multim_buf_ptr=0x%x, type=%d", *multim_buf_ptr, accessory_ptr->type);  
                if(*multim_buf_ptr)
                {
                    *type = accessory_ptr->type;
                    return_val = TRUE;
    ...
    SCI_TRACE_LOW("MMIMMS_GetFirstImageFileIfExists:return_val=%d, *type=%d", return_val, *type);  
    return return_val;
}
#else // ifdef MMS_SUPPORT mms dummy

LOCAL void DelSlideContent(MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr, uint16 del_type) {}
PUBLIC MMIMMS_ERROR_E MMIMMS_AddSlide(MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr, uint32  slide_direct) {return MMIMMS_NO_ERROR;}
PUBLIC MMIMMS_ERROR_E MMIMMS_AddText(MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr, const MMI_STRING_T* str_info_ptr) {return MMIMMS_NO_ERROR;}
PUBLIC void MMIMMS_DelSlide(MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr){}
PUBLIC void MMIMMS_DelImage(MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr){}
PUBLIC void MMIMMS_DelMusic(MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr){}
PUBLIC void MMIMMS_DelVideo(MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr){}
PUBLIC BOOLEAN MMIMMS_DelAttachment(uint8 * attachment_name_ptr, MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr) {return FALSE;}
PUBLIC uint16 MMIMMS_GetSlideIndex(MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr) {return 0;}
PUBLIC uint16 MMIMMS_GetSlideCount(MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr) {return 0;}
PUBLIC uint16 MMIMMS_GetAllContentCount(MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr) {return 0;}
PUBLIC BOOLEAN MMIMMS_GetContentByName(uint8 *filename, uint8 **data, uint32 *data_len) {return FALSE;}
PUBLIC BOOLEAN MMIMMS_GetCurrentSlideAudio(MMISRVAUD_RING_FMT_E *type, uint32 *multim_buf_len, void **multim_buf_ptr, MMIMMS_EDIT_DOCUMENT_T *editdoc_ptr) {return FALSE;}
PUBLIC BOOLEAN MMIMMS_GetCurrentSlideImage(uint32 *type, uint32 *multim_buf_len, void **multim_buf_ptr, MMIMMS_EDIT_DOCUMENT_T *editdoc_ptr) {return FALSE;}
PUBLIC BOOLEAN MMIMMS_GetCurrentSlideVideo(uint32 *type, uint32 *multim_buf_len, void **multim_buf_ptr, MMIMMS_EDIT_DOCUMENT_T *editdoc_ptr) {return FALSE;}
PUBLIC BOOLEAN MMIMMS_GetVideoFromSlide(uint32 *type, uint32 *multim_buf_len, void **multim_buf_ptr, P_MMS_SLIDE_T slide_ptr) {return FALSE;}
PUBLIC BOOLEAN MMIMMS_GetCurrentSlideText(MMIMMS_EDIT_DOCUMENT_T *editdoc_ptr, MMI_STRING_T *string_ptr) {return FALSE;}
PUBLIC BOOLEAN MMIMMS_AddFile(MMIMMS_EDIT_DOCUMENT_T *edit_doc_ptr, MMIFILE_FILE_INFO_T *file_info_ptr, MMIFMM_FILE_TYPE_E file_type,
                              BOOLEAN is_replace_multifile, uint32 replace_multifile_size)  {return FALSE;}
PUBLIC BOOLEAN MMIMMS_DelMultimFileInSlide(MMIFMM_FILE_TYPE_E file_type, uint32 slide_index, MMIMMS_EDIT_DOCUMENT_T *editdoc_ptr, BOOLEAN *is_need_reset) {return FALSE;}
PUBLIC BOOLEAN MMIMMS_DelSelectedAccessory(uint32 accessory_index, MMIMMS_EDIT_DOCUMENT_T *editdoc_ptr) {return FALSE;}
PUBLIC BOOLEAN MMIMMS_GetMultimediaBufferInSlide(MMIFMM_FILE_TYPE_E file_type, uint32 *multim_buf_len, void **multim_buf_ptr, P_MMS_SLIDE_T slide_ptr) {return FALSE;}
PUBLIC uint16 MMIMMS_GetTextContentLength(MMIMMS_EDIT_DOCUMENT_T *editdoc_ptr) {return 0;}
PUBLIC BOOLEAN MMIMMS_GetTextContent(MMIMMS_EDIT_DOCUMENT_T *editdoc_ptr, MMI_STRING_T *string_ptr) {return FALSE;}
PUBLIC MMIMMS_EDIT_DOCUMENT_T * EditMMSReadReport(MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr) {return PNULL;}
PUBLIC void MMIMMS_DelText(MMIMMS_EDIT_DOCUMENT_T * editdoc_ptr){}
PUBLIC MMIMMS_EDIT_DOCUMENT_T *MMIMMS_CreateEditDocument(void) {return PNULL;}
PUBLIC void MMIMMS_DestroyEditDocument(MMIMMS_EDIT_DOCUMENT_T *editdoc_ptr){}
PUBLIC BOOLEAN MMIMMS_AddAndReplaceAttachment(MMIMMS_EDIT_DOCUMENT_T *edit_doc_ptr, MMIFILE_FILE_INFO_T *file_info_ptr, uint8 *replaced_name_ptr,
                                              uint8 *file_data_ptr, uint32 file_data_len, MMIFMM_FILE_TYPE_E file_type, MMI_STRING_T *vacard_str) {return FALSE;}
PUBLIC BOOLEAN MMIMMS_GetAttachmentNumber(P_MMS_EDIT_BODY_T pDocument, uint32 *attachment_num_ptr) {return FALSE;}
PUBLIC BOOLEAN MMIMMS_GetAttachmentSize(char *pFilename, P_MMS_EDIT_BODY_T pDocument, uint32 *file_size_ptr) {return FALSE;}
PUBLIC BOOLEAN MMIMMS_GetAllSlideTextContent(MMIMMS_EDIT_DOCUMENT_T *editdoc_ptr, MMI_STRING_T *string_ptr) {return FALSE;}
PUBLIC BOOLEAN MMIMMS_GetFirstImageFileIfExists(uint32 *type, uint32 *multim_buf_len, void **multim_buf_ptr, MMIMMS_EDIT_DOCUMENT_T *editdoc_ptr) {return FALSE;}
#endif