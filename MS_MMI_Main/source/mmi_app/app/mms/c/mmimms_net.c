...
#ifdef DATACOUNTER_SUPPORT
    DATACOUNTER_DATATYPE_E dataType = DATACOUNTER_SENT_DATATYPE;
#endif
    ...
    case  MMS_STATUS_SEND_RETURN:               
...
#ifdef DATACOUNTER_SUPPORT
                dataType = DATACOUNTER_SENT_DATATYPE;
                MMIAPIDataCounter_mmsAlertNotifaction(s_activemms_info.dual_sys, dataType);
#endif
                //End
                break;       
                ...
                )
            {
                if (s_send_retry_pdp_time < MMS_SEND_RETRY_MAX_TIMES)
                {
                    ...
                    if  (MMS_NETLINK_FAILED == status_value ||
                        (MMS_OPR_CANCEL == status_value && MMIMMS_STATUS_PDP_DEACTIVE_IND == s_activemms_info.status))
                    {
    ...
    case  MMS_STATUS_RECV_RETURN:
            ...
            case MMS_RECV_SUCCESS:
                ...
                if(MMIMMS_UpdateRecvItem(recv_end_info.record_id))
                {
                    ...
                    //Begin zhang.lu7@byd.com add dataCounter threshold tips
#ifdef DATACOUNTER_SUPPORT
                    dataType = DATACOUNTER_RECV_DATATYPE;
                    MMIAPIDataCounter_mmsAlertNotifaction(s_activemms_info.dual_sys, dataType);
#endif
                    //End
                }
                else
                {
                    ...
                    if(MMI_DEVICE_NUM <= MMIAPIFMM_GetFirstValidSD())
                    {
                        recv_end_info.recv_error = MMIMMS_ERROR_NO_SDCARD;
                    }
                    else
                    {
        ...
        // added by feng.xiao
            ...
            {
                #ifdef MMI_GPRS_SWITCH_SUPPORT
                BOOLEAN gprs_is_off = MMIAPICONNECTION_isGPRSSwitchedOff();
                #endif
                ...
                if (s_recv_retry_time < MMS_RECV_RETRY_MAX_TIMES 
                #ifdef MMI_GPRS_SWITCH_SUPPORT
                 && !gprs_is_off
                #endif                    
                    )//重试的次数小于5次则开启timer重复下载当前彩信
                {                    
                    ...
                    g_mms_recv_timer = MMK_CreateTimerCallback(MMS_NET_TIME_OUT, MMIAPIMMS_HandleNetTimer, NULL, FALSE);
                    s_recv_retry_time ++;
                    if ((MMS_OPR_CANCEL == status_value && MMIMMS_STATUS_PDP_DEACTIVE_IND == s_activemms_info.status) || (MMS_NETLINK_FAILED == status_value))
                    {
                        g_is_pdp_active_timer = TRUE;
                        s_is_send_retry_timer = FALSE;
                    }
...
LOCAL void ResetActiveMMSInfo(void)
{
    ...
    SCI_TRACE_LOW("[MMIMMS]ResetActiveMMSInfo");
}
...
PUBLIC void MMIMMS_ResetActiveMMSInfo(void)
{
    SCI_TRACE_LOW("[wwww] MMIMMS_IsPrepareSendMMS: MMIMMS_ResetActiveMMSInfo");
    ResetActiveMMSInfo();
}

PUBLIC void MMIMMS_GetActiveMMSInfo(MMIMMS_PROGRESS_T* pActivemmsInfo)
{
    SCI_MEMCPY(pActivemmsInfo, &s_activemms_info, sizeof(MMIMMS_PROGRESS_T));
}

PUBLIC BOOLEAN MMIMMS_IsPrepareSendMMS(int32 record_id)
{
    SCI_TRACE_LOW("[wwww] MMIMMS_IsPrepareSendMMS: record_id=%d,%d,status=%d,type=%d", record_id, s_activemms_info.record_id, s_activemms_info.status, s_activemms_info.type);
    if(record_id != s_activemms_info.record_id)
    {
        SCI_TRACE_LOW("[wwww] MMIMMS_IsPrepareSendMMS:FALSE1");
        return FALSE;
    }
    else if((MMIMMS_STATUS_NONE < s_activemms_info.status) && (s_activemms_info.status < MMIMMS_STATUS_DATA_TRANSFERING) && (MMIMMS_SENDRECV_SENDING == s_activemms_info.type))
    {
        SCI_TRACE_LOW("[wwww] MMIMMS_IsPrepareSendMMS:TRUE");
        return TRUE;
    }
    else
    {
        SCI_TRACE_LOW("[wwww] MMIMMS_IsPrepareSendMMS:FALSE2");
        return FALSE;
    }
}
...
PUBLIC void MMIMMS_ActiveRecvSendModule(void)
{
    ...
    if(MMIMMS_STATUS_NONE == s_activemms_info.status && !s_is_send_retry_timer)//当彩信发送pdp激活失败进入timer的时候不允许其他先操作
    {
        ...
        SCI_TRACE_LOW("[MMIMMS] MMIMMS_ActiveRecvSendModule manual_list=0x%x, auto_list=0x%x", manual_list, auto_list);
        if(manual_list)
        {

#ifdef MMI_SMS_CHAT_SUPPORT
            mms_display_info = MMIAPIMMS_GetListItemByID(manual_list->record_id);
            if (mms_display_info != NULL && MMIMMS_SENDRECV_RECVING == manual_list->send_type)
            {
                SCI_TRACE_LOW("[MMIMMS]:MMIMMS_ActiveRecvSendModule will be receving file size: %d\n", mms_display_info->file_size);
                if (MMIMMS_IsCurrentChatTotalSizeOverrun(mms_display_info->file_size))
                {
                    SCI_TRACE_LOW("[MMIMMS]:MMIMMS_ActiveRecvSendModule pdp deactive for size overrun");
                    s_switch_simcard_pdp_active = FALSE;
                    MMIMMS_DeleteManualListItem(manual_list->record_id);
                    MMK_PostMsg(VIRTUAL_WIN_ID,MSG_MMS_ACTIVE_RECV_SEND,PNULL,0);
                    MMIMMS_PDPDeactive(); 
                    return;
                }
            ...
            SCI_TRACE_LOW("[MMIMMS]:MMIMMS_ActiveRecvSendModule: type=%d", s_activemms_info.type);
            if(    MMIMMS_SENDRECV_SENDING == s_activemms_info.type
                                        ...
                                        )
{
    ...
    else
    {
        ...
        if(MMIMMS_NO_ERROR == return_val || MMIMMS_ERROR_NO_SPACE == return_val || MMIMMS_ERROR_NO_SDCARD == return_val)
        {
...
{
    SCI_TRACE_LOW("[mms ak s_activemms_info.record_id is %d, record_id %d]%s        %d\n",s_activemms_info.record_id, record_id, __FUNCTION__,__LINE__);
...
PUBLIC BOOLEAN MMIMMS_CancelSendRecv(BOOLEAN is_manual)
{
    ...
    SCI_TRACE_LOW("MMIMMS_CancelSendRecv: status=%d", s_activemms_info.status);
...
//  Note:
/*****************************************************************************/
PUBLIC MMIMMS_SENDRECV_TYPE_E MMIMMS_GetSendRecvType(void)
{
    return s_activemms_info.type;
}
...
PUBLIC BOOLEAN MMIMMS_HandleNetTimer(uint8 timer_id, uint32 param)
{
    if (g_mms_recv_timer == timer_id)
    {
        ...
        else//下载失败的timer
        {
            ...
            {
                if(0 != MMIPDP_FindLinkIdByApphandler(MMI_MODULE_MMS)){
                    MMK_PostMsg(VIRTUAL_WIN_ID,MSG_MMS_ACTIVE_RECV_SEND,PNULL,0);
                }else{
...
PUBLIC BOOLEAN MMIMMS_IsPdpPermitUsing(MN_DUAL_SYS_E dual_sys)
{
    ...
    if (PLMN_NORMAL_GSM_GPRS_BOTH == net_status || PLMN_NORMAL_GSM_ONLY == net_status)
    {
	 ...
	 if(0 == MNPHONE_GetAttState(dual_sys)) 
	 {
			  is_permit_used = FALSE;
	 }
	SCI_TRACE_LOW("MMIMMS func[%s] line[%d]dual_sys %d %d ",__FUNCTION__,__LINE__,dual_sys,is_permit_used);
...
LOCAL void HandlePDPActiveCnf(MMIPDP_CNF_INFO_T *msg_ptr)
{
    ...
    {
	  SCI_TRACE_LOW("[MMIMMS] func[%s] line[%d] This shouldn't happen ",__FUNCTION__,__LINE__);
	  return ;
    }
    ...
    //else if((MMIMMS_SENDRECV_RECVING == s_activemms_info.type) && (MMI_DEVICE_NUM > MMIAPIFMM_GetFirstValidSD()))
    else if((MMIMMS_SENDRECV_RECVING == s_activemms_info.type))
    {
        fname = (uint8 *)SCI_ALLOCAZ(MMIMMS_FILE_NAME_LEN + 1);
