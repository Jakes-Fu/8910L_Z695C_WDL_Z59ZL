                             ...
                             )
{
    ...
    SCI_TRACE_LOW("[MMIMMS] GetMMSListPath: result_val=%d", result_val);
                                    ...
                                    )
{
    ...
    SCI_TRACE_LOW("[MMIMMS] GetMMSFilePathForRead: result_val=%d", result_val);
    return result_val;  
}
                                             ...
                                             )
{
...
#endif
    return_val = MMIAPIFMM_GetSysFileFullPathForWriteEx(
        FALSE,
        FALSE,
        MMIMMS_DEFAULT_DIR,
        MMIMMS_DEFAULT_DIR_LEN,
    ...
    SCI_TRACE_LOW("[MMIMMS] MMIMMS_GetMMSFilePathForWrite: return_val=%d", return_val);
    return return_val;
}
...
/*****************************************************************************/
//  Description : 获取当前会话中彩信已占用的总大小
//  Global resource dependence : none
//  Author: weipeng.wang
//  Note:
/*****************************************************************************/
PUBLIC int32 MMIMMS_GetCurrentSizeByRecordId(int32 record_id)
{
    uint32 i = 0;
    uint32 cur_size = 0;
    MMISMS_ORDER_INFO_T* cursor_ptr = s_mms_list.list_head_ptr;
    ...
    //累加每条彩信的大小
    while((PNULL != cursor_ptr)&& (i < MMIMMS_GetMaxNumber()))
    {
        if(record_id == cursor_ptr->record_id)
        {
            cur_size = cursor_ptr->file_size;
            break;
        }
        cursor_ptr = cursor_ptr->next;
        i++;
    }
    SCI_TRACE_LOW("[MMIMMS] MMIMMS_GetCurrentSizeByRecordId: cur_size=%d, record_id=%d", cur_size, record_id);
    return cur_size;
}
...
//  Note:
/*****************************************************************************/
PUBLIC uint32 MMIMMS_GetCurrentChatTotalSize(void)
{
    uint32 total_size = 0;
    ...
    uint32 j = 0;
    uint32 id_indx = 0;
    BOOLEAN is_manual_id = FALSE;
    MMIMMS_AUTOLIST_T *manual_list = PNULL;
    int32 manual_record_id[20] = {0};
    manual_list = MMIMMS_GetManualListFirstItem();
    //cur_order_id = MMISMS_GetCurOperationOrderId();
    //累加每条彩信的大小
    while((PNULL != manual_list) && (i < s_mms_list.total_num))
    {
        manual_record_id[id_indx] = manual_list->record_id;
        id_indx++;
        manual_list = manual_list->next;
    }
    while((PNULL != cursor_ptr) && (i < MMIMMS_GetMaxNumber()))
    {
        //if(0 == MMIAPICOM_CompareTwoUint8String(cur_order_id->number.sender, MMISMS_NUMBER_MAX_LEN, cursor_ptr->sender, MMISMS_NUMBER_MAX_LEN))
       if(MMISMS_MT_NOT_DOWNLOAD != cursor_ptr->mo_mt_type)
       {
           if (0 == id_indx)//no manual record id
           {
               total_size += cursor_ptr->file_size;
           }else{
               for(j=0; j<id_indx; j++){
                   if(cursor_ptr->record_id == manual_record_id[j]){
                       is_manual_id = TRUE;
                       SCI_TRACE_LOW("[MMIMMS] MMIMMS_GetCurrentChatTotalSize: is_manual_id=%d, mo_mt_type=%d\n", is_manual_id, cursor_ptr->mo_mt_type);
               break;
                   }
               }
               if((!is_manual_id) || (cursor_ptr->mo_mt_type == MMISMS_MO_SEND_FAIL)){
                   total_size += cursor_ptr->file_size;
               }
           }
       }
       ...
       {
            SCI_TRACE_LOW("[MMIMMS] MMIMMS_GetCurrentChatTotalSize: mo_mt_type is MMISMS_MT_NOT_DOWNLOAD for recordid: %d\n", cursor_ptr->record_id);
       }
        cursor_ptr = cursor_ptr->next;
        i++;
        is_manual_id = FALSE;
    ...
    SCI_TRACE_LOW("[MMIMMS] MMIMMS_GetCurrentChatTotalSize: total_size=%d, id_indx: %d\n", total_size,id_indx);
    return total_size;
}

/*****************************************************************************/
//  Description : 当前回话彩信大小是否超出600K
//  Global resource dependence : none
//  Author: weipeng.wang
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIMMS_IsCurrentChatTotalSizeOverrun(uint32 cur_size)
{
    uint32 total_size = 0;
    BOOLEAN result = FALSE;

    total_size = MMIMMS_GetCurrentChatTotalSize();
    //total_size = MMIMMS_GetTotalSize();
    if(cur_size + total_size >= 510 * 1024)
    {
        result = TRUE; 
    }
    SCI_TRACE_LOW("[MMIMMS] MMIMMS_IsCurrentChatTotalSizeOverrun: cur_size=%d, total_size=%d, result=%d", cur_size, total_size, result);
    return result;
}
...
//  MMIMMS_AUTOLIST_T* cursor_ptr = PNULL;
    ...
    //while((PNULL != list_ptr) && (MMI_DEVICE_NUM > MMIAPIFMM_GetFirstValidSD()))
                               ...
                               )
{
    ...
    SCI_TRACE_LOW("[MMIMMS]:MMIMMS_AddAutoList  record_id=%d\n",record_id);
    if(record_id <= 0)
    {
        return ;
    }
...
PUBLIC void MMIMMS_DeleteManualListItem(int32 record_id)
{
    ...
    MMIMMS_AUTOLIST_T   *list_ptr = PNULL;
    SCI_TRACE_LOW("[MMIMMS]:MMIMMS_DeleteManualListItem record_id=%d\n",record_id);
    if(record_id && cursor_ptr)
    {
        if(record_id == cursor_ptr->record_id)
        {
...
PUBLIC BOOLEAN MMIMMS_IsMaxNum(void)
{
    ...
    {
        SCI_TRACE_LOW("[MMIMMS] MMIMMS_IsMaxNum reach max number");
        return TRUE;
    }
}
...
PUBLIC void MMIMMS_ReadMMSListFile(void)
{
    ...
    if(GetMMSListPath(MMIMMS_LIST_NAME,MMIMMS_LIST_NAME_LEN,list_path, &list_path_len))
    {
        ...
        SCI_TRACE_LOW("[MMIMMS]:MMIMMS_ReadMMSListFile file_handle=0x%x", file_handle); 
        ...
        if(SFS_INVALID_HANDLE != file_handle)
        {
                ...
                ))
            {
                ...
                SCI_TRACE_LOW("[MMIMMS]:MMIMMS_ReadMMSListFile cmp=%d, max_mms_number=%d, %d, file_size=%d, %d", 
					MMIAPICOM_Stricmp((uint8 *)list_head.version, (uint8 *)MMIMMS_LIST_VER), 
					max_mms_number,
					list_head.total_num,
					file_size,
					(sizeof(MMIMMS_LIST_FILE_HEAD_T)+max_mms_number*sizeof(MMISMS_ORDER_INFO_T)));
                //检验版本是否正确，数据是否完整
                if(0 == MMIAPICOM_Stricmp((uint8 *)list_head.version, (uint8 *)MMIMMS_LIST_VER)
                    /*&& max_mms_number >= list_head.total_num
                    &&file_size ==(sizeof(MMIMMS_LIST_FILE_HEAD_T)+max_mms_number*sizeof(MMISMS_ORDER_INFO_T))*/)
                {
        ...
        else//两种情况，一种是系统无法分析handle，一种是不存在list的ini文件
        {
            ...
            SCI_TRACE_LOW("[MMIMMS]:MMIMMS_ReadMMSListFile line:%d, file_handle_wr=0x%x", _D_FILE_LINE, file_handle_wr); 
            ...
            {                
                ...
                MMIAPIFMM_SetFileSize(file_handle_wr, (sizeof(MMIMMS_LIST_FILE_HEAD_T)+max_mms_number*sizeof(MMISMS_ORDER_INFO_T)));//设置大小
...
//  uint16                  subject_len = 0;
    ...
    if(0 != record_id)
    {
        ...
        file_len = SCI_STRLEN((char *)fname);
        MMI_STRNTOWSTR(w_fname,MMIMMS_FILE_NAME_LEN,(uint8 *)fname,MMIMMS_FILE_NAME_LEN,file_len);
        SCI_TRACE_LOW("[MMIMMS]:MMIMMS_ReadMMS device=%d", MMIAPIFMM_GetFirstValidSD());
        if(GetMMSFilePathForRead(w_fname,file_len,mms_file_path, &mms_file_path_len))
        {
        ...
        if(file_size > 0)// && file_size <= MMIMMS_MMS_LIMIT_SIZE + MMIMMS_ENCODE_LEFT_SIZE)
        {
            ...
            if(PNULL != buffer_ptr)
            {
                ...
                {
                 	_snprintf((char *)fname,(MMIMMS_FILE_NAME_LEN-1),"%ld_read.mms",record_id);
                    vf_handle = sysVFopen((int8*)fname, "wb");
                    if(vf_handle)
                    {
                        vf_write_size = sysVFwrite((int8*)buffer_ptr,file_size, 1, vf_handle);
                        SCI_TRACE_LOW("[MMIMMS %s:%d]vf_write_size = %d\n", __FUNCTION__,__LINE__,vf_write_size);
                        //Free tmp buffer
                        MMIMMS_FREE(buffer_ptr);
                        buffer_ptr = PNULL; 
                        
                        //add for debug
                        sysVFseek(vf_handle,0,2);
                        SCI_TRACE_LOW("[MMIMMS %s:%d]vf file size  = %d\n", __FUNCTION__,__LINE__,(int32)sysVFtell(vf_handle));
                        sysVFseek(vf_handle,0,0);
...
//  int32               vfilebuf_len = 0;
    ...
    uint32              file_size = 0;
    char	            phoneNumber[MMIMMS_FILE_NAME_LEN+1]	= {0};    //bug969753
            ...
            if(mmsdoc)
            {
                ...
                {                    
                    ...
                    else
                    {
                           ...
                           MMIMMS_EDIT_DOCUMENT_T * edit_doc = MMIMMS_ReadMMS(record_id);
                           MMI_STRING_T          string_ptr = {0};          
                           if(edit_doc != PNULL)
                           {
                               MMIMMS_GetTextContent(edit_doc,&string_ptr);
                                        SCI_MEMSET(cursor_ptr->display_content.content,0,((MMIMESSAGE_DIAPLAY_LEN+1)*sizeof(wchar)));                                    
                                        SCI_MEMCPY(cursor_ptr->display_content.content,string_ptr.wstr_ptr,MIN(string_ptr.wstr_len,MMIMESSAGE_DIAPLAY_LEN) * 2);      
                                             cursor_ptr->display_content.content_len = MMIAPICOM_Wstrlen(cursor_ptr->display_content.content);
                           }         
                           MMIMMS_DestroyEditDocument(edit_doc);
                    }
...
//  MMISMS_ORDER_INFO_T     list_item = {0};
    ...
    SCI_TRACE_LOW("[wangwp] line[%d] file[%s] func[%s]", __LINE__, __FILE__, __FUNCTION__);
    if(GetMMSListPath(MMIMMS_LIST_NAME,MMIMMS_LIST_NAME_LEN,list_path, &list_path_len))
    {
        file_handle = MMIAPIFMM_CreateFile(list_path, SFS_MODE_WRITE|SFS_MODE_CREATE_ALWAYS, NULL, NULL);/*lint !e655*/
        SCI_TRACE_LOW("[wangwp] line[%d] file[%s] func[%s] file_handle=%d, SFS_INVALID_HANDLE=%d", __LINE__, __FILE__, __FUNCTION__, file_handle, SFS_INVALID_HANDLE);
        ...
        if(SFS_INVALID_HANDLE != file_handle)
        {
            ...
            MMIAPIFMM_SetFileSize(file_handle, (sizeof(MMIMMS_LIST_FILE_HEAD_T)+max_mms_number*sizeof(MMISMS_ORDER_INFO_T)));//设置大小s
            return_val = TRUE;
                             ...
                             )
{
	...
	if( msg_ptr->subject ){   //coverity 11358
                                     ...
                                     )
{
    ...
    if(MMIMMS_IntegrateMMS(editdoc_ptr))        
    {
       ...
       // if((is_record_exist || !is_maxnum) && is_enough_space && (device < MMI_DEVICE_NUM))
        if((is_record_exist || !is_maxnum) && is_enough_space )
        {
            if(PNULL != editdoc_ptr->edit_subject.wstr_ptr && 0 != editdoc_ptr->edit_subject.wstr_len)
            {
            ...
            {
                list_item.display_content.content_len = MIN(MMIMESSAGE_DIAPLAY_LEN, MMIAPICOM_Wstrlen(editdoc_ptr->wstr_ptr));
                if(list_item.display_content.content_len > 0)
                {
                    MMI_WSTRNCPY(list_item.display_content.content, MMIMESSAGE_DIAPLAY_LEN, editdoc_ptr->wstr_ptr, list_item.display_content.content_len, list_item.display_content.content_len);
                }
            }
...
//                                 else
            ...
            SCI_TRACE_LOW("[MMIMMS] MMIMMS_SaveMMS: file_size=%d", list_item.file_size);
            ...
            if(MMIMMS_SaveMMSListFile())
            {
                ...
                else
                {
                    ...
                    {
                        return_val = MMIMMS_ERROR_NO_SDCARD;
                    }
                    else
                    {
        ...
        else
        {
            ...
            {
                //MMIMMS_EditMMSAlertWin(MMIMMS_ERROR_NO_SPACE);
                return_val = MMIMMS_ERROR_NO_SPACE;
            }
        }
    }
                                            ...
                                            )
{
    ...
    if(buffer_ptr)
    {
        ...
        SCI_TRACE_LOW("[MMIMMS]:MMIMMS_SaveMMSContent device_type = %d", device_type);
...
PUBLIC BOOLEAN MMIMMS_IsInQueue(int32 record_id)
{
    ...
    SCI_TRACE_LOW("MMIMMS: MMIMMS_IsInQueue: return_val=%d", return_val);
    return return_val;
}

PUBLIC BOOLEAN MMIMMS_IsNeedDelete(int32 record_id)
{
    BOOLEAN return_val = FALSE;
    MMIMMS_AUTOLIST_T* cursor_ptr = PNULL;
    ...
    while(PNULL != cursor_ptr)
    {
        if(cursor_ptr->record_id == record_id)
        {
            if (cursor_ptr->send_type == MMIMMS_SENDRECV_RECVING)
                return_val = TRUE;
            break;
        }
        cursor_ptr = cursor_ptr->next;
    }
    SCI_TRACE_LOW("MMIMMS: MMIMMS_IsNeedDelete: return_val=%d", return_val);
    return return_val;
} 
