...
WINDOW_TABLE(MMIENVSET_CHANGE_ENV_NAME_WIN_TAB) = 
{
    ...
    CHILD_SOFTKEY_CTRL(STXT_SOFTKEY_SAVE_MK, TXT_NULL, STXT_RETURN,MMICOMMON_SOFTKEY_CTRL_ID,MMIENVSET_CHANGE_NAME_FORM_CTRL_ID),
#endif
#endif
...
#endif
//dial tone
#if 0//defined(MMI_DIALPANEL_DTMF_SUPPORT)//delete by James li
...
#endif
#if 0//def MMI_READMENU_ENABLE//delete by James li  
...
LOCAL uint8 GetEnvAddIndex(void)
{
    ...
    for(i=MMIENVSET_USER_MODE1; i<MMIENVSET_TOTAL_MODE; i++)
    {
        ...
        SCI_TRACE_BUF("MMIENV GetEnvAddIndex",mode_setting.name_str_t.wstr,MMIENVSET_ENVNAME_MAX_LEN);

	  if( FALSE == mode_setting.is_valid )
         {
               MMIENVSET_InitUserIndexItem(i);
               break;
        }
    }
                                           ...
                                           )
{
	...
	case MSG_KEYUP_UP:
	case MSG_KEYUP_DOWN:
		if (GUIMENU_GetCurNodeId(ctrl_id) == MMIEVN_OPTMENU_NODE_DEL && 
			MMIENVSET_GetEnvRealIndex(cur_select_index) < MMIENVSET_USER_MODE1)
		{
			// Bug 1382147, 内置情景模式不能删除，屏蔽左软键
    ...
    case MSG_OPEN_WINDOW:
        {
            ...
            GUIWIN_SetSoftkeyTextId(win_id,  STXT_SOFTKEY_SELECT_MK, TXT_NULL, STXT_FINISH, FALSE);
                                              ...
                                              )
{
    ...
    case MSG_OPEN_WINDOW:
		...
		//set left softkey
        GUIEDIT_SetStyle(ctrl_id, GUIEDIT_STYLE_SINGLE_DYN_DOWN);
...
LOCAL void SetRingVolumeCB(MMICOM_PANEL_CALLBACK_T *para_ptr) 
{
    ...
    SCI_TRACE_LOW("SetRingVolumeCB s_ring_type=%d, cur_value=%d", s_ring_type, para_ptr->cur_value);
                                          ...
                                          )
{
    ...
    SCI_TRACE_LOW("HandleAllRingVolumeWindow msg_id=0x%x", msg_id);
    switch(msg_id)
    {
    case MSG_OPEN_WINDOW:
#if !defined (MMI_PDA_SUPPORT)
...
def MMI_READMENU_ENABLE
        ...
        {
            //GUIFORM_SetChildDisplay(MMIENVSET_OTHER_RING_SET_FORM_CTRL_ID,MMIENVSET_KEYPAD_RING_SET_FORM_CTRL_ID,GUIFORM_CHILD_DISP_GRAY);//delete by James li
#ifdef TOUCH_PANEL_SUPPORT
...
#endif		
			// Bug 1382051, 静音模式与会议模式不能设置其它铃声，故屏蔽左softkey
...
PUBLIC BOOLEAN MMI_IsRingType(void)
{
	if (MMK_IsOpenWin(MMIENVSET_CALL_RING_TYPE_WIN_ID))
	{
		return TRUE;
	}
	else if (MMK_IsOpenWin(MMIENVSET_MSG_RING_TYPE_WIN_ID))
	{
		return TRUE;
	}    
	else if (MMK_IsOpenWin(MMIENVSET_ALARM_RING_TYPE_WIN_ID))
	{
		return TRUE;
	}     
	else
