...
#else
LOCAL uint8        s_silent_pre_mode = MMIENVSET_STANDARD_MODE;/*lint -esym(551, s_silent_pre_mode) */
#endif
...
#if defined(MMI_MULTI_SIM_SYS_DUAL) || defined(MMI_MULTI_SIM_SYS_TRI) || defined(MMI_MULTI_SIM_SYS_QUAD)
            ,{
                MMISET_CALL_RING_FIXED, /*lint !e64*/
                MMISET_RING_DEFAULT_ID              
            }
#if defined(MMI_MULTI_SIM_SYS_TRI) || defined(MMI_MULTI_SIM_SYS_QUAD)
            ,{
...
const MMIENVSET_SETTING_T mmienvset_default_setting_ex[MMIENVSET_TOTAL_MODE]=
{
    // 普通环境
    {
#if !defined(MMI_MULTI_SIM_SYS_SINGLE)
...
#endif
    {
        //dong.chunguang modify for volume error
        //set active mode as cur mode,not default,default is standard mode
        //else if active mode is silent,the cur mode is default
        //the volume will display error
        s_mmienvset_cur_mode = s_mmienvset_active_mode;//MMIENVSET_DEFUALT_MODE;
    }
...
//#endif            
 ...
 /*****************************************************************************/
//  Description : reinit user nv item
//  Global resource dependence : none
//  Author: panvs.zhong
//  Note:
/*****************************************************************************/
PUBLIC void MMIENVSET_InitUserIndexItem(
                                    uint16 item_index // IN
                                    )
{
    SET_NV_ITEM_E               nv_item_id   = MMIENVSET_SETTING_BEGIN;  // nvo???
    MMIENVSET_SETTING_T         mode_setting = {0};

    nv_item_id = nv_item_id + item_index;    
    SCI_MEMSET(&mode_setting,0x00,sizeof(MMIENVSET_SETTING_T));
    SCI_MEMCPY(&mode_setting,&mmienvset_default_setting[item_index],sizeof(MMIENVSET_SETTING_T));
    MMINV_WRITE(nv_item_id, &mode_setting);
    SCI_TRACE_LOW(" [%s] [%d] i[%d] ", __FUNCTION__,__LINE__,item_index);
}
                              ...
                              )
{
    ...
    case MENUREAD_VOL:
		  ...
		  return_value =ZDT_GetVol_Num(); //mode_setting_ptr->is_menu_read;
#else	
...
#ifdef PHONE_NUMBER_VOICE
		return_value =ZDT_GetMenuSwitch_type(); //mode_setting_ptr->is_menu_read;
#else		
                                  ...
                                  )
{
    ...
    SCI_TRACE_LOW("MMIENVSET_SetCurModeOptValue mode_id=%d, set_opt=%d, set_value=%d", mode_id, set_opt, set_value);
                                     ...
                                     )
{
    ...
    SCI_TRACE_LOW("MMIENVSET_SetActiveModeOptValue mode_id=%d, set_opt=%d, set_value=%d", mode_id, set_opt, set_value);
    SetEnvSetOptValue(mode_id,set_opt,set_value, dual_sys);
...
//        break;
    ...
    case MMISET_RING_TYPE_ALARM:
        ...
        break;
        // Added by sunhongzhe to fix Bug 533182.
      //  return MMISET_ALARM_DEFAULT_VOL;
#ifdef MMI_AZAN_SUPPORT
    case MMISET_RING_TYPE_AZAN:
#endif
    case MMISET_RING_TYPE_SCH: // Added for calendar ringtone.
                      ...
                      
{
    ...
    case MMISET_RING_TYPE_POWER_OFF:
    #endif
        ring_id = MMISET_POWER_DEFAULT_ID;//MMISET_RING_DEFAULT_ID;//dong.chunguang modify to default
        break;
                           ...
                           )
{
    //添加列表项
    MMIAPISET_AppendListItemByTextId(TXT_RING_SILENT,TXT_COMMON_OK,ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);//静音
    MMIAPISET_AppendListItemByTextId(TXT_COMMON_RING_RING1,STXT_SOFTKEY_OK_MK,ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);//铃声1
#ifndef MMI_LOW_MEMORY_RING
    MMIAPISET_AppendListItemByTextId(TXT_COMMON_RING_RING2,STXT_SOFTKEY_OK_MK,ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);//铃声2
#endif
...
void MMIENVSET_ResetEnvSetSetting(void)
{
    ...
    {
    	if(g_ringtone_max != MMISET_RING_FIXED_DEFAUT_NUM)//special country
    	{
			mode_setting_ptr = &mmienvset_default_setting_ex[i];
		}else
...
{   
    ...
    if (MMISET_MSG_RING_MORE_RING == mode_setting_ptr->msg_ring_info[dual_sys].msg_ring_type)
    {
        ...
        if(!ReadMoreRing(dual_sys,mode_id,&(msg_ring_info.more_ring_info),MMIENVSET_MORE_RING_MSG,filename))
        {
            ...
            msg_ring_info.fix_ring_index = MMISET_MSG_DEFAULT_ID;
        }
    }
...
LOCAL MMIENVSET_SETTING_T* GetModeValue(uint16 cur_mode)
{
    ...
    SCI_TRACE_LOW("GetModeValue key_ring_type = %d, key_ring_vol=%d", s_mmienvset_setting_info.last_setting.key_ring_type, s_mmienvset_setting_info.last_setting.key_ring_vol);
    ...
    if(MMIENVSET_SETTING_END >= nv_item_id)
    {
        ...
        {   
			...
			if(g_ringtone_max != MMISET_RING_FIXED_DEFAUT_NUM)//special country
			{
            SCI_MEMCPY(&s_mmienvset_setting_info.last_setting,&mmienvset_default_setting_ex[cur_mode],sizeof(MMIENVSET_SETTING_T));  
			}
			else
...
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPI_UpdateCurrentSetingInfo(uint16 mode_id,MMIENVSET_SETTING_T* mode_setting_ptr)
{
		 UpdateCurrentSetingInfo(mode_id,mode_setting_ptr);
}
/*****************************************************************************/
...
PUBLIC void MMIAPIENVSET_ResetActModeOtherRingSet(void)
{
    ...
    {
    	if(g_ringtone_max != MMISET_RING_FIXED_DEFAUT_NUM)//sepcial country
		{
	        MMIENVSET_SetActiveModeOptValue(KEY_RING_TYPE, mmienvset_default_setting_ex[mode_id].key_ring_type, 0);
	        MMIENVSET_SetActiveModeOptValue(TKEY_RING_TYPE, mmienvset_default_setting_ex[mode_id].tkey_ring_type, 0);
	        MMIENVSET_SetActiveModeOptValue(VOLTAGE_WARN, mmienvset_default_setting_ex[mode_id].low_voltage_warn, 0);
	        MMIENVSET_SetActiveModeOptValue(POWER_RING_SWITCH, mmienvset_default_setting_ex[mode_id].power_ring_swithch, 0);    
	        MMIENVSET_SetActiveModeOptValue(ENVSET_DIAL_TP_VIBRATE_SWITCH, mmienvset_default_setting_ex[mode_id].is_dial_tp_vibrator_on, 0);
#if defined(MMI_DIALPANEL_DTMF_SUPPORT)
	        MMIENVSET_SetActiveModeOptValue(ENVSET_DIAL_TONE_SWITCH, mmienvset_default_setting_ex[mode_id].is_dial_tone_off, 0);
#endif
	        MMIENVSET_SetActiveModeOptValue(ENVSET_TP_KEY_LOCK_SWITCH, mmienvset_default_setting_ex[mode_id].is_tp_key_lock_on, 0);
		}
		else
		{
...
/*****************************************************************************/
//  Description : MMIAPIENVSET_SetBothSIMFixMsgRingId
//  Global resource dependence :
//  Author:dong.chunguang
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIENVSET_SetBothSIMFixMsgRingId(uint16 ring_id)
{
    //IAPIENVSET_SetFixedMsgRingId(MN_DUAL_SYS_1,ring_id,MMIENVSET_GetCurModeId());
    //IAPIENVSET_SetFixedMsgRingId(MN_DUAL_SYS_2,ring_id,MMIENVSET_GetCurModeId());
    MMIENVSET_SETTING_T *mode_setting_ptr = PNULL;
    uint8 mode_id = MMIENVSET_GetCurModeId();
    ...
    if((0 < ring_id) && (MMISET_MAX_RING_ID >= ring_id)) 
    {
        mode_setting_ptr->msg_ring_info[MN_DUAL_SYS_1].msg_ring_type = MMISET_MSG_RING_FIXED;
        mode_setting_ptr->msg_ring_info[MN_DUAL_SYS_1].fix_ring_index = ring_id; 
#if defined(MMI_MULTI_SIM_SYS_DUAL)
        mode_setting_ptr->msg_ring_info[MN_DUAL_SYS_2].msg_ring_type = MMISET_MSG_RING_FIXED;
        mode_setting_ptr->msg_ring_info[MN_DUAL_SYS_2].fix_ring_index = ring_id;
#endif
    }
    else
    {
        mode_setting_ptr->msg_ring_info[MN_DUAL_SYS_1].msg_ring_type = MMISET_MSG_RING_FIXED;
        mode_setting_ptr->msg_ring_info[MN_DUAL_SYS_1].fix_ring_index = 1;
#if defined(MMI_MULTI_SIM_SYS_DUAL)
        mode_setting_ptr->msg_ring_info[MN_DUAL_SYS_2].msg_ring_type = MMISET_MSG_RING_FIXED;
        mode_setting_ptr->msg_ring_info[MN_DUAL_SYS_2].fix_ring_index = 1;
#endif
        //SCI_TRACE_LOW:"MMIAPIENVSET_SetFixedMsgRingId fail ring_id=%d"
        SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMIENVSET_4702_112_2_18_2_19_3_46,(uint8*)"d",ring_id);
    }
...
/*****************************************************************************/
//  Description : MMIAPIENVSET_SetBothSIMFixedCallRingId
//  Global resource dependence :
//  Author:dong.chunguang
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIENVSET_SetBothSIMFixedCallRingId(uint16 ring_id)
{
	//IAPIENVSET_SetFixedCallRingId(MN_DUAL_SYS_1,ring_id,MMIENVSET_GetCurModeId());
	//IAPIENVSET_SetFixedCallRingId(MN_DUAL_SYS_2,ring_id,MMIENVSET_GetCurModeId());
    MMIENVSET_SETTING_T *mode_setting_ptr = PNULL;
	uint8 mode_id = MMIENVSET_GetCurModeId();
    ...
    SCI_MEMSET(mode_setting_ptr,0,sizeof(MMIENVSET_SETTING_T));
    MMIAPIENVSET_GetModeValue(mode_id,mode_setting_ptr);
    mode_setting_ptr->call_ring_info[MN_DUAL_SYS_1].call_ring_type = MMISET_CALL_RING_FIXED;
    mode_setting_ptr->call_ring_info[MN_DUAL_SYS_1].fix_ring_index = ring_id;
#if defined(MMI_MULTI_SIM_SYS_DUAL)
    mode_setting_ptr->call_ring_info[MN_DUAL_SYS_2].call_ring_type = MMISET_CALL_RING_FIXED;
    mode_setting_ptr->call_ring_info[MN_DUAL_SYS_2].fix_ring_index = ring_id;
#endif
    //SCI_TRACE_LOW:"MMIAPIENVSET_SetFixedCallRingId nv_item_id = %d"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMIENVSET_4671_112_2_18_2_19_3_45,(uint8*)"d",mode_id);
    MMINV_WRITE((MMIENVSET_SETTING_BEGIN+mode_id),mode_setting_ptr);
...
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN WriteBothSIMMoreRing(
                           uint8 mode_id,
                           MMISET_CALL_MORE_RING_T *ring_info,
                           MMIENVSET_MORE_RING_INDEX_E index,
                           wchar                   *filename
...
{
	BOOLEAN                 result               = FALSE;
    WriteMoreRing(MN_DUAL_SYS_1,mode_id,ring_info,index,filename);
    if(WriteMoreRing(MN_DUAL_SYS_2,mode_id,ring_info,index,filename)){
		result = TRUE;
	}
	return result;
}
/*****************************************************************************/
//  Description : Both SIM more msg info
//  Global resource dependence :
//  Author:dong.chunguang
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIENVSET_SetBothSIMMoreMsgRingInfo(MMISET_CALL_MORE_RING_T *ring_info)
{
    //IAPIENVSET_SetMoreMsgRingInfo(MN_DUAL_SYS_1,ring_info,MMIENVSET_GetCurModeId());
	//IAPIENVSET_SetMoreMsgRingInfo(MN_DUAL_SYS_2,ring_info,MMIENVSET_GetCurModeId());
	MMIENVSET_SETTING_T *mode_setting_ptr = PNULL;   
	wchar                   filename[MMIFILE_FULL_PATH_MAX_LEN+2] = {0};
	int8 mode_id = MMIENVSET_GetCurModeId();
 
	CombineMoreRingFullPath(filename);

	if(WriteBothSIMMoreRing(mode_id,ring_info,MMIENVSET_MORE_RING_MSG,filename))
	{
	    mode_setting_ptr = SCI_ALLOC_APP(sizeof(MMIENVSET_SETTING_T));
	    if(mode_setting_ptr == PNULL)
	    {
	    ...
	    SCI_MEMSET(mode_setting_ptr,0,sizeof(MMIENVSET_SETTING_T));
	    MMIAPIENVSET_GetModeValue(mode_id,mode_setting_ptr);
	    mode_setting_ptr->msg_ring_info[MN_DUAL_SYS_1].msg_ring_type = MMISET_MSG_RING_MORE_RING;        
	    mode_setting_ptr->msg_ring_info[MN_DUAL_SYS_1].fix_ring_index = MMISET_MSG_RING_DEFAULT_ID;
#if defined(MMI_MULTI_SIM_SYS_DUAL)
	    mode_setting_ptr->msg_ring_info[MN_DUAL_SYS_2].msg_ring_type = MMISET_MSG_RING_MORE_RING;        
	    mode_setting_ptr->msg_ring_info[MN_DUAL_SYS_2].fix_ring_index = MMISET_MSG_RING_DEFAULT_ID;
#endif
	    //SCI_TRACE_LOW:"MMIAPIENVSET_SetMoreMsgRingInfo nv_item_id = %d"
	    SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMIENVSET_4640_112_2_18_2_19_3_44,(uint8*)"d",mode_id);
	    MMINV_WRITE((MMIENVSET_SETTING_BEGIN+mode_id),mode_setting_ptr);
...
/*****************************************************************************/
//  Description : Both SIM more ring info
//  Global resource dependence :
//  Author:dong.chunguang
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIENVSET_SetBothSIMMoreCallRingInfo(MMISET_CALL_MORE_RING_T *ring_info)
{
    MMIENVSET_SETTING_T *mode_setting_ptr = PNULL;   
    wchar                   filename[MMIFILE_FULL_PATH_MAX_LEN+2] = {0};
	int8 mode_id = MMIENVSET_GetCurModeId();
    ...
    if(WriteBothSIMMoreRing(mode_id,ring_info,MMIENVSET_MORE_RING_CALL,filename))
    {
        mode_setting_ptr = SCI_ALLOC_APP(sizeof(MMIENVSET_SETTING_T));
        if(mode_setting_ptr == PNULL)
        {
        ...
        SCI_MEMSET(mode_setting_ptr,0,sizeof(MMIENVSET_SETTING_T));   
        MMIAPIENVSET_GetModeValue(mode_id,mode_setting_ptr);   
        mode_setting_ptr->call_ring_info[MN_DUAL_SYS_1].call_ring_type = MMISET_CALL_RING_MORE_RING;  
        mode_setting_ptr->call_ring_info[MN_DUAL_SYS_1].fix_ring_index = MMISET_MSG_RING_DEFAULT_ID;
#if defined(MMI_MULTI_SIM_SYS_DUAL)
        mode_setting_ptr->call_ring_info[MN_DUAL_SYS_2].call_ring_type = MMISET_CALL_RING_MORE_RING;  
        mode_setting_ptr->call_ring_info[MN_DUAL_SYS_2].fix_ring_index = MMISET_MSG_RING_DEFAULT_ID;
#endif
        //SCI_TRACE_LOW:"MMIAPIENVSET_SetMoreCallRingInfo nv_item_id = %d"
        SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMIENVSET_4601_112_2_18_2_19_3_43,(uint8*)"d",mode_id);
...
PUBLIC void MMIAPIENVSET_ReSetAllVolmue(uint32 volume)
{
    ...
    MMIENVSET_SETTING_T     *setting_ptr    =   PNULL;
    uint16 cur_mode = MMIAPIENVSET_GetActiveModeId();
    
    SCI_TRACE_LOW("[baokun] MMIAPIENVSET_ReSetAllVolmue volume %d cur_mode %d", volume, cur_mode);
    if(MMIENVSET_SILENT_MODE != cur_mode 
#ifdef ZDT_PRODUCT_VIBRATE        
        && MMIENVSET_MEETING_MODE != cur_mode
#endif        
        )
    {
	    //first set all fixed ring volume
	    setting_ptr = GetModeValue(cur_mode);
	    if(PNULL != setting_ptr)
	    {    
	         setting_ptr->call_ring_vol = volume;
	         setting_ptr->msg_ring_vol = volume;
	         setting_ptr->comm_vol =volume;
	         setting_ptr->alarm_ring_vol= volume;
	         setting_ptr->power_ring_vol=volume;
	         MMIAPIENVSET_SetModeValue(cur_mode, setting_ptr);
	    }
	    //set mp3 player volume
	    MMIAPIMP3_SetVolume(volume);
	    //set fm volume
#ifdef FM_SUPPORT
		MMIFM_AdjustVolumeTP(volume);
#endif
#ifdef VIDEO_PLAYER_SUPPORT
	    //set video volume
		MMIAPIVP_FSMUpdateCurrentVolNoHandle(volume);
#endif
    }
}
