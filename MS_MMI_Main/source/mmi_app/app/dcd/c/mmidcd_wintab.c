...
WINDOW_TABLE(MMIDCD_LINK_LIST_WIN_TAB) = 
{
    ...
    WIN_TITLE( TXT_DCD_STXT_LINK_LIST),
    WIN_SOFTKEY(STXT_OPTION, TXT_NULL, STXT_RETURN),
                              ...
                              )
{
    ...
    if(0 < totalnum)
    {
        ...
        if(PNULL != item_ptr)
        {
            DcdInsertNodeByLabel(menuctrl_id, STXT_SOFTKEY_MARK_MK,            DCD_ITEMMENU_SUB_MARK,        parent_node_id, index++);
...
LOCAL void  DCD_OpenMainItemMenu(void)
{
    ...
    GUIMENU_SetDynamicMenuSoftkey(ctrl_id, STXT_SOFTKEY_SELECT_MK, TXT_NULL, STXT_RETURN);
    ...
    if(0 < s_dcd_gui_struct.current_item_num)
    {
        ...
        //mark
        DcdInsertNodeByLabel(ctrl_id, STXT_SOFTKEY_MARK_MK, DCD_ITEMMENU_MARK, 0, one_level_index++);
    }
    ...
    two_level_index = 0;
    DcdInsertNodeByLabel(ctrl_id, STR_CB_ADD_CHANNEL, DCD_ITEMMENU_ITEM_ADD_CHANNEL, DCD_ITEMMENU_CHANNEL_MANAGEMENT, two_level_index++);
    
    DcdInsertNodeByLabel(ctrl_id, STR_CB_DELETE_CHANNEL, DCD_ITEMMENU_ITEM_DELETE_CHANNEL, DCD_ITEMMENU_CHANNEL_MANAGEMENT, two_level_index++);
...
LOCAL void CreateLocalItemDyMenu(const MMI_CTRL_ID_T menuctrl_id)
{
    ...
    uint16 subindex = 0;
    uint16 selectnum = 0;
    GUIMENU_SetDynamicMenuSoftkey(menuctrl_id, STXT_SOFTKEY_SELECT_MK, TXT_NULL, STXT_RETURN);
    ...
    DcdInsertNodeByLabel(menuctrl_id, STXT_SOFTKEY_MARK_MK, DCD_ITEMMENU_MARK, 0,    rootindex++);
...
{    
    ...
    else
    {
        ...
        case DCD_ERROR_FLY_MODE:
            MMI_GetLabelTextByLang(STR_TURN_OFF_FLIGHT_MODE, &prompt_str);
            break;
                                 ...
                                 )
{
    ...
    {
        // 保存...
        DcdInsertNodeByLabel(menuctrl_id, STXT_SOFTKEY_SAVE_MK, DCD_ITEMMENU_SAVE, 0,  rootindex++);
    }
...
PUBLIC BOOLEAN HERO_MMIDCD_IsOpened(void)
{
    return MMK_IsOpenWin(MMIDCD_MAIN_WIN_ID);
}
#endif
/**--------------------------------------------------------------------------*
 **                         Compiler Flag                                    *
