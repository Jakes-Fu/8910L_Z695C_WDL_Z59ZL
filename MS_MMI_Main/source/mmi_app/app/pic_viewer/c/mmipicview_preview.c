...
//zhuyongjing define for upload/donwnload status
enum
{
    MMIPICVIEW_UPLOADING_STATUS,               
    MMIPICVIEW_UPLOADED_SUCCESS_STATUS, 
    MMIPICVIEW_UPLOADED_FAILED_STATUS, 
    MMIPICVIEW_ONEDRIVE_ONLY
};
//add end

extern BOOLEAN               s_is_pic_edit_complete;//lijinghua add for pic edit save complete
#ifndef MMI_PIC_CROP_SUPPORT
BOOLEAN s_mmipreview_is_error_type = FALSE;
#endif
/**--------------------------------------------------------------------------*
 **                         STATIC DEFINITION                                *
 **--------------------------------------------------------------------------*/
...
//zhuyongjing add 
LOCAL void ModifyPreviewTitleWithStatus(
    uint32            cur_index,  //in:from 1 start
    uint32            total_num,  //in:
    MMI_CTRL_ID_T     ctrl_id,
    uint32 status
);
LOCAL void HandleDeleteSinglePicCnf(
...
PUBLIC void MMIPICVIEW_PreviewVideoThumbnailOpenCallback(const wchar *filename,const wchar *video_filename)
{
    uint16 index = MMIPICVIEW_GetCurIndex(MMIPICVIEW_LIST_CTRL_ID);
    FILEARRAY_DATA_T file_info={0};
    if(MMIPICVIEW_GetPicFileInfo(index, &file_info)){
        if(0 == MMIAPICOM_CompareTwoWstr(file_info.filename, file_info.name_len, video_filename, MMIAPICOM_Wstrlen(video_filename))){
            SetAnimParam(FALSE, TRUE,index , MMIPICVIEW_PREVIEW_ANIM_CTRL_ID);
        }
    }
}
#endif
...
WINDOW_TABLE(MMIPICVIEW_PREVIEW_WIN_TAB) =
{
    ...
    WIN_SOFTKEY(STXT_OPTION, TXT_NULL, TXT_COMMON_COMMON_BACK),//zhuyongjing add choose for gallary ui
    END_WIN
};
...
//  Note:
/*****************************************************************************/
#ifdef MMI_SETTING_WALLPAER_WITH_PREVIEW
LOCAL void SetWallpaperWithPreivew(
        uint32  item_index, //from 0
        MMI_CTRL_ID_T  ctrl_id
        )
{
...
)
{
...
#ifndef MMI_PIC_CROP_SUPPORT
    GUIANIM_DISPLAY_IND_T *anim_ind_ptr= PNULL;
    MMI_STRING_T        note_info   = {0};
#endif

    SCI_TRACE_LOW("[MMIPICVIEW] HandlePicPreviewWinMsg msg_id=%x",msg_id);
    switch (msg_id)
    {
    case MSG_OPEN_WINDOW:
...
#endif
#ifndef MMI_PIC_CROP_SUPPORT
        s_mmipreview_is_error_type = FALSE;
#endif
       ...
       // SetDisplayPreviewOsd(TRUE);//zhuyongjing delete for action bar 20150722
	...
	GUISETLIST_SetCtrlState(setlist_ctrl_id,GUISETLIST_STATE_OPEN_POPWIN,FALSE);
#endif
        break;
...
#endif
        CTRLANIM_SetDefaultIcon(MMIPICVIEW_PREVIEW_ANIM_CTRL_ID, IMAGE_COMMON_PIC_ICON, IMAGE_COMMON_PIC_ERROR);
        break;
	//end
    case MSG_GET_FOCUS:
        //set display preview osd
        SetDisplayPreviewOsd(TRUE);
        //zhuyongjing add for edit back when save 
        GUISETLIST_SetVisible(setlist_ctrl_id, FALSE, FALSE);
        GUIWIN_SetSoftkeyVisible(win_id, FALSE);
        ModifyPreviewTitle((item_index + 1), pic_num, setlist_ctrl_id);   
        //SetPreviewOsd((item_index + 1), pic_num, setlist_ctrl_id, win_id);
        //SetAnimParam(TRUE, TRUE, item_index, ctrl_id);
        //add end
#ifdef PIC_EDITOR_SUPPORT
        s_is_pic_edit_complete=PIC_EDIT_GetSavedCompletestatus();

        if(s_is_pic_edit_complete)
        {
            if(PIC_EDIT_GetSavedCompletestatus())
            {
                ...
                banner_info.text1_id = STXT_STORED;
                MMIAPICOM_OpenBannerWin(win_id, banner_info);
                PIC_EDIT_SetSavedCompletestatus(FALSE);
            }
            s_is_pic_edit_complete=FALSE;
            PIC_EDIT_SetSavedCompletestatus(FALSE);
        }
#endif
    ...
    case MSG_TIMER:
        ...
        break;
        //zhuyongjing delete for action bar 20150722
    case MSG_CTL_ANIM_UPDATE_END:
        //update title
       // GUISETLIST_SetVisible(setlist_ctrl_id, TRUE, TRUE);
        //update softkey
        if(!GUIWIN_GetSoftkeyVisible(win_id)){
#ifdef HW_LCD_TEST
            GUIWIN_SetSoftkeyVisible(win_id, FALSE);
            GUISETLIST_SetVisible(setlist_ctrl_id, FALSE, TRUE);
#else
        ...
        break;
    case MSG_NOTIFY_ANIM_GIF_UPDATE:
#ifdef HW_LCD_TEST
        GUIWIN_SetSoftkeyVisible(win_id, FALSE);
        GUISETLIST_SetVisible(setlist_ctrl_id, FALSE, TRUE);
#else
...
#endif
        break;
	//lijinghua add it for pic edit
	case MSG_NOTIFY_ANIM_DISPLAY_IND:   
#ifndef MMI_PIC_CROP_SUPPORT
	anim_ind_ptr = (GUIANIM_DISPLAY_IND_T*)param;
	SCI_TRACE_LOW("HandlePicPreviewWinMsg--MSG_CTL_ANIM_UPDATE_END %d",anim_ind_ptr->result);
	if(anim_ind_ptr->result != GUIANIM_RESULT_SUCC)
		s_mmipreview_is_error_type = TRUE;
	else
		s_mmipreview_is_error_type = FALSE;
#endif
#ifdef HW_LCD_TEST
    ...
    case MSG_APP_MENU:    
        if(MMIPICVIEW_GetEnterType()==MMIPICVIEW_PHOTOS_FROM_WALLPAPER)
        {
            ...
            if(MMIPICVIEW_GetPicFileInfo(item_index,&file_info_ptr)){
    ...
    case MSG_APP_WEB:
    SCI_TRACE_LOW("HandlePicPreviewWinMsg--MSG_APP_WEB  s_mmipreview_is_error_type=   %d",s_mmipreview_is_error_type);
#ifdef MMI_SETTING_WALLPAER_WITH_PREVIEW
        if(MMIPICVIEW_GetEnterType()==MMIPICVIEW_PHOTOS_FROM_WALLPAPER)
        {
             if(s_mmipreview_is_error_type)
            {
                    MMI_GetLabelTextByLang(STR_FILE_NOT_SUPPORT,&note_info);
                    MMIPUB_OpenAlertWinByTextPtr(PNULL,&note_info,PNULL,IMAGE_PUBWIN_WARNING,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);

            }
            else
            {
                    SetWallpaperWithPreivew(item_index, ctrl_id);
                    break;
            }
        }
#endif
    ...
    case MSG_APP_LEFT:
        ...
        if (PreviousPic(&anim_result, item_index, PNULL, pic_num, ctrl_id, list_ctrl_id))
        {
           ...
           // SetDisplayPreviewOsd(FALSE);//zhuyongjing delete for action bar 20150722
    ...
    case MSG_APP_RIGHT:
        ...
        break;
		//zhuyongjing add for camera roll
#ifdef CAMERAROLL_SUPPORT
        case MSG_CAMERA_ROLL_DOWNLOAD_PHOTO_CNF:
        {
            CAMERA_ROLL_SIG_T * data =(CAMERA_ROLL_SIG_T *)param;	
            SCI_TRACE_LOW("MMIPICVIEW HandlePicPreviewWinMsg receive MSG_CAMERA_ROLL_DOWNLOAD_PHOTO_CNF done=%d",data->camera_roll_msg_data.bDone);
            if(data->camera_roll_msg_data.bDone){
                DisplaybannerInfo(win_id,STR_GAL_NOTE_DOWN_SUC_EXT02);
                ModifyPreviewTitle((item_index + 1), pic_num, setlist_ctrl_id);   
            }else{
                DisplaybannerInfo(win_id,STR_ONEDRIVE_DOWNLOAD_FAILED_EXT01);
            }
        }
        break;
        case MSG_CAMERA_ROLL_UPLOAD_PHOTO_NOTIFY :
            {
                uint  index_uploading= *(uint16*)param;
                SCI_TRACE_LOW("MMIPICVIEW HandlePicPreviewWinMsg receive MSG_CAMERA_ROLL_UPLOAD_PHOTO_NOTIFY index_uploading=%d,item_index=%d",index_uploading,item_index);

                if(item_index == index_uploading){
                    ModifyPreviewTitleWithStatus(item_index+1,pic_num,setlist_ctrl_id,MMIPICVIEW_UPLOADING_STATUS);
                }
            }
            break;
	case MSG_CAMERA_ROLL_UPLOAD_PHOTO_CNF:
		{
                    ...
                    char    temp1_code[100]={0};

                    MMIAPICOM_WstrToStr(data->camera_roll_msg_data.filename, (uint8 *)temp_code);
                    SCI_TRACE_LOW("[msa]:MMIPICVIEW HandlePicPreviewWinMsg MSG_CAMERA_ROLL_UPLOAD_PHOTO_CNF zyj %d,filename=%s" , data->camera_roll_msg_data.bDone,temp_code);
                    
                    //a.draw banner
                    if(MMIPICVIEW_GetPicFileInfo(item_index,&file_info)){
                        MMIAPICOM_WstrToStr(file_info.filename, (uint8 *)temp1_code);
                        SCI_TRACE_LOW("[msa]:MMIPICVIEW HandlePicPreviewWinMsg native file=%s",temp1_code);
                        if(0 == MMIAPICOM_Wstrcmp(file_info.filename, data->camera_roll_msg_data.filename)){
                            if(CAMERA_ROLL_MANUAL_SYNC== MMISRV_CAMERAROLL_Get_Sync_State()){  
                                if(data->camera_roll_msg_data.bDone){
                                    DisplaybannerInfo(win_id,STR_ONEDRIVE_CONF_UPLOADED_EXT01);
                                }else{
                            ...
                            //b. draw cloud  in setlist statusbar
                            if(data->camera_roll_msg_data.bDone){
                                ModifyPreviewTitleWithStatus(item_index+1,pic_num,setlist_ctrl_id,MMIPICVIEW_UPLOADED_SUCCESS_STATUS);
                            }else{
                                ModifyPreviewTitleWithStatus(item_index+1,pic_num,setlist_ctrl_id,MMIPICVIEW_UPLOADED_FAILED_STATUS);
                            }
                        }
                    }
    ...
    case MSG_CAMERA_ROLL_ERROR_NOTIFY:
    {
        ...
        ONE_DRIVE_ERROR_E err_trype = data->camera_roll_msg_data.error_type;
            SCI_TRACE_LOW("[msa]:MMIPICVIEW HandlePicPreviewWinMsg MSG_CAMERA_ROLL_ERROR_NOTIFY zyj err_trype=%d",err_trype );
#if defined(WIN32)
            break;
#endif
        if(MMISRV_CAMERAROLL_Get_Auto_upload_status()==ONE_DRIVE_AUTO_UPLOAD_NO_NETWORK
            ...
            if(MMIPICVIEW_GetPicFileInfo(item_index, &file_info)){     
                if(CAMERA_ROLL_MANUAL_SYNC== MMISRV_CAMERAROLL_Get_Sync_State()
                    ...
                    SCI_TRACE_LOW("[msa]:MMIPICVIEW HandlePicPreviewWinMsg MSG_CAMERA_ROLL_ERROR_NOTIFY  err" );
                }
                //b. draw cloud  in setlist statusbar
            }
        }
    ...
    case MSG_CAMERA_ROLL_SHARE_LINK_CNF:
    {
        ...
        SCI_TRACE_LOW("[msa]:MMIPICVIEW HandlePicPreviewWinMsg MSG_CAMERA_ROLL_SHARE_LINK_CNF " );
        if(data->camera_roll_msg_data.bDone){
    ...
    case MSG_PICVIEWER_DELETE_CNF:
            HandleDeleteSinglePicCnf(*((BOOLEAN*)param),item_index,list_ctrl_id,wait_win_id,win_id);
        break;
		//add end
    case MSG_APP_CANCEL:
        //zhuyongjing change
      ...
      //  }
        MMK_CloseWin(win_id);
        if(MMIAPIDC_IsOpened()&&MMK_IsOpenWin(MMIPICVIEW_LIST_WIN_ID)){
...
#ifndef HW_LCD_TEST
        GUIWIN_UpdateSoftkey(win_id);////zhuyongjing add for action bar 20150722
#endif
        break;
...
)
{
    ...
    SetPreviewSoftkey(cur_index,win_id);
#else
    //set title
...
)
{
    ...
    FILEARRAY_DATA_T    file_info = {0};//zhuyongjing add for action bar 20150722 
...
#endif
#endif
    title_rect.v_rect.top   	= 0;
    title_rect.h_rect.top       = 0;
#ifndef MMI_GUI_STYLE_MINISCREEN
    title_rect.v_rect.bottom = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;//title_rect.v_rect.top + title_height + 15 - 1;
    title_rect.h_rect.bottom = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;//title_rect.h_rect.top + title_height + 10 - 1;
#else
    ...
    if(PNULL != ctrl_id)
    {
      ...
      //  title_rect.h_rect.left   = MMIPICVIEW_TITLE_MARGIN;
       ...
       // title_rect.h_rect.right  = title_rect.h_rect.left + min_width - 1;
        ...
        //set string
     /* sprintf((char *)str, "%ld/%ld", cur_index, total_num); 
        str_len = (uint16)strlen((char *)str);
        MMI_STRNTOWSTR(wstr, MMIPICVIEW_TITLE_STR_MAX_LEN,
                       str, MMIPICVIEW_TITLE_STR_MAX_LEN,
                       str_len);*/
        title_string.wstr_ptr = wstr;
        MMIAPIPICVIEW_ReplaceTextIdByTwoNum(STR_NUMBER_SLASH_NUMBER,cur_index,total_num,&title_string);   
	 #if defined(CAMERAROLL_SUPPORT)
        upload_status = MMISRV_CAMERAROLL_Get_Auto_upload_status();
        SCI_TRACE_LOW("MMIPICVIEW SetPreviewTitleOsd upload_status=%d index=%d",upload_status,cur_index-1);
        //set icon zhuyongjing add for action bar 20150722
        //cur_index:show num, item index=cur_index-1
         if (MMIPICVIEW_GetPicFileInfo(cur_index-1, &file_info)){
            ...
            if(ONEDRIVE_THUMBNAIL== GetFileSource(file_info.filename,file_info.name_len)){
                item_info.image_info.image_data.image_id =  IMAGE_GALLERY_ONEDRIVE_SYNCED_ICON_BLUE;
                item_info.image_info.image_data_type = GUISETLIST_DATA_IMAGE_ID;
                item_info.image_info.image_width = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
                item_info.image_info.image_height = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
                SCI_TRACE_LOW("[MMIPIC]:SetPreviewTitleOsd CAMERA_ROLL_SOURCE_ONEDRIVE_ONLY !!!!");
            }else if(TRUE == MMISRV_CAMERAROLL_Is_Pic_On_OneDrive(file_info.filename,file_info.name_len)){
                item_info.image_info.image_data.image_id =  IMAGE_GALLERY_ONEDRIVE_UPLOADED_ICON;
                item_info.image_info.image_data_type = GUISETLIST_DATA_IMAGE_ID;
                item_info.image_info.image_width = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
                item_info.image_info.image_height = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
                 SCI_TRACE_LOW("[MMIPIC]:SetPreviewTitleOsd IMAGE_GALLERY_ONEDRIVE_UPLOADED_ICON !!!!");
                ...
                &&TRUE == MMISRV_CAMERAROLL_Is_Pic_Auto_Uploading(file_info.filename,file_info.name_len)
                &&(ONE_DRIVE_AUTO_UPLOAD_NO_NETWORK != upload_status&&ONE_DRIVE_AUTO_UPLOAD_NONE != upload_status)))){
                item_info.image_info.image_data.image_id =  IMAGE_ONEDRIVE_UPLOADING_ICON;
                item_info.image_info.image_data_type = GUISETLIST_DATA_IMAGE_ID;
                item_info.image_info.image_width = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
                item_info.image_info.image_height = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
                SCI_TRACE_LOW("[MMIPIC]:SetPreviewTitleOsd IMAGE_ONEDRIVE_UPLOADING_ICON !!!!");
            }else if(MMISRV_CAMERAROLL_Get_Connect_State()
                &&MMIAPIMSA_IsAccountExist(MSA_APP_TYPE_CAMERA_ROLL)
                &&CAMERA_ROLL_AUTO_SYNC== MMISRV_CAMERAROLL_Get_Sync_State() 
                &&!MMISRV_CAMERAROLL_Is_Pic_On_OneDrive(file_info.filename,file_info.name_len)
                &&MMISRV_CAMERAROLL_Is_Pic_Auto_Uploading(file_info.filename,file_info.name_len)
                &&upload_status==ONE_DRIVE_AUTO_UPLOAD_NO_NETWORK){
                item_info.image_info.image_data.image_id =  IMAGE_GALLERY_ONEDRIVE_FAILED_UPLOAD_ICON;
                item_info.image_info.image_data_type = GUISETLIST_DATA_IMAGE_ID;
                item_info.image_info.image_width = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
                item_info.image_info.image_height = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
                SCI_TRACE_LOW("[MMIPIC]:SetPreviewTitleOsd IMAGE_GALLERY_ONEDRIVE_FAILED_UPLOAD_ICON !!!!");
        }else{
                item_info.item_state = GUISETLIST_ITEM_STATE_TEXT;
                SCI_TRACE_LOW("[MMIPIC]:SetPreviewTitleOsd none icon!");
      ...
      //  item_info.str_info.wstr_len = str_len;
        item_info.str_info = title_string;
        GUISETLIST_AddItem(ctrl_id, &item_info);

        //set font
        font.color = MMI_DEFAULT_TEXT_COLOR;
        font.font  = MMI_DEFAULT_SMALL_FONT;//MMI_DEFAULT_NORMAL_FONT;//MMIPICPREVIEW_TITLE_FONT;
        GUISETLIST_SetFont(ctrl_id, &font);
#ifndef MMI_GUI_STYLE_MINISCREEN
        //no bg
        bg.bg_type = GUI_BG_HALF_TRANS;//GUI_BG_COLOR;//GUI_BG_NONE;zhuyongjing change for action bar 20150722
        bg.color = MMI_BLACK_COLOR;
        bg.shape = GUI_SHAPE_RECT;//zhuyongjing add for setlist rect
		//change end
        ...
        GUISETLIST_SetVisible(ctrl_id, TRUE, TRUE);//zhuyongjing add for action bar 20150722
    }
#ifndef MMI_GUI_STYLE_MINISCREEN
#ifdef  PIC_PREVIEW_U200_SUPPORT
...
)
{
 ...
 #if defined(CAMERAROLL_SUPPORT)  
    upload_status = MMISRV_CAMERAROLL_Get_Auto_upload_status();
    SCI_TRACE_LOW("MMIPICVIEW ModifyPreviewTitle upload_status=%d",upload_status);

    //set icon zhuyongjing add action bar 20150722
     if (MMIPICVIEW_GetPicFileInfo(cur_index-1, &file_info)){
        item_info.item_state = GUISETLIST_ITEM_STATE_TEXT|GUISETLIST_ITEM_STATE_IMAGE;
        if(ONEDRIVE_THUMBNAIL== GetFileSource(file_info.filename,file_info.name_len)){
            item_info.image_info.image_data.image_id =  IMAGE_GALLERY_ONEDRIVE_SYNCED_ICON_BLUE;
...
/*****************************************************************************/
//  Description : modify preview title according to upload/download cnf
//  Global resource dependence :
//  Author: zhuyongjing 
//  Note:
/*****************************************************************************/
...
)
{
    ...
    title_string.wstr_ptr = wstr;
    MMIAPIPICVIEW_ReplaceTextIdByTwoNum(STR_NUMBER_SLASH_NUMBER,cur_index,total_num,&title_string);          
    SCI_TRACE_LOW("[MMIPIC]:ModifyPreviewTitleWithStatus status=%d !!!!",status);

     if (MMIPICVIEW_GetPicFileInfo(cur_index-1, &file_info)){
        item_info.item_state = GUISETLIST_ITEM_STATE_TEXT|GUISETLIST_ITEM_STATE_IMAGE;
        if(status == MMIPICVIEW_ONEDRIVE_ONLY){
            item_info.image_info.image_data.image_id =  IMAGE_GALLERY_ONEDRIVE_SYNCED_ICON_BLUE;
            item_info.image_info.image_data_type = GUISETLIST_DATA_IMAGE_ID;
            item_info.image_info.image_width = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
            item_info.image_info.image_height = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
        }else if(status == MMIPICVIEW_UPLOADED_SUCCESS_STATUS){
            item_info.image_info.image_data.image_id =  IMAGE_GALLERY_ONEDRIVE_UPLOADED_ICON;
            item_info.image_info.image_data_type = GUISETLIST_DATA_IMAGE_ID;
            item_info.image_info.image_width = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
            item_info.image_info.image_height = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
        }else if(status ==MMIPICVIEW_UPLOADING_STATUS){
            item_info.image_info.image_data.image_id =  IMAGE_ONEDRIVE_UPLOADING_ICON;
            item_info.image_info.image_data_type = GUISETLIST_DATA_IMAGE_ID;
            item_info.image_info.image_width = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
            item_info.image_info.image_height = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
        }else if(status == MMIPICVIEW_UPLOADED_FAILED_STATUS){
            item_info.image_info.image_data.image_id =  IMAGE_GALLERY_ONEDRIVE_FAILED_UPLOAD_ICON;
            item_info.image_info.image_data_type = GUISETLIST_DATA_IMAGE_ID;
            item_info.image_info.image_width = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
            item_info.image_info.image_height = MMIPICVIEW_PREVIEW_TITLE_HEIGHT;
...
/*****************************************************************************/
//  Description : handle delete single picture 
//  Global resource dependence : none
//  Author: zhuyongjing
//  Note:
/*****************************************************************************/
                              ...
                              )
{
    ...
    {
        //update list
        MMIPICVIEW_RemoveItem(list_ctrl_id,index);
        if(MMIPICVIEW_GetPicFileInfo(index,&file_info)){
#ifdef  VIDEOTHUMBNAIL_SUPPORT
        ...
        if (MMIAPIPICVIEW_GetAfterSearchFousStatus())
        {
        ...
        SCI_TRACE_LOW("[pic_view]HandleDeleteSinglePicCnf pic_num=%d",pic_num);
                ...
                else
                {
                    ...
                    if(index == pic_num){
                        cur_index = index - 1;
                    }else if(index < pic_num){
                        cur_index = index;
                    }
                    MMIPICVIEW_SetCurIndex(MMIPICVIEW_LIST_CTRL_ID,cur_index);
                   // MMIPICVIEW_PostUpdateMsg(cur_index);

                    //SetPreviewOsd((cur_index + 1), pic_num, setlist_ctrl_id, win_id);
                    ModifyPreviewTitle((cur_index + 1), pic_num, setlist_ctrl_id);
                    SetPreviewSoftkey((cur_index+1),win_id);
                    //MMITHEME_GetAnimTheme(&tmpTheme);
                    //CTRLANIM_SetDefaultIcon(MMIPICVIEW_PREVIEW_ANIM_CTRL_ID, 0, tmpTheme.error_bg.img_id);
                    CTRLANIM_SetDefaultIcon(MMIPICVIEW_PREVIEW_ANIM_CTRL_ID, 0, IMAGE_COMMON_PIC_ERROR);//对比zuk ，删除图片后，不需要显示default pic
                    SetAnimParam(TRUE, FALSE, cur_index, MMIPICVIEW_PREVIEW_ANIM_CTRL_ID);
                    //close wait win
                    DisplaybannerInfo(win_id,STR_GAL_CONF_ITEM_REMOVED); 
...
)
{
    ...
    //set common bg type
    //zhuyongjing change for gallery ui 20150722
...
//    GUIWIN_SetSoftkeyCommonBg(win_id, &common_bg);
    GUIWIN_SetSoftkeyBgAlpha(win_id,65);   //lijinghua modify it
    GUIWIN_SetSoftkeyStyle(win_id,MMITHEME_GetCurThemeFontColor(MMI_THEME_SOFTKEY_FONT),1,MMI_BLACK_COLOR,0);//GUI_SOFTKEY_BG_COLOR_ONLY

//zhuyongjing add for video preview
    if (MMIPICVIEW_GetPicFileInfo(cur_index-1, &file_info)){
        if(MMIPICVIEW_IsVideoSuffix(&file_info) == TRUE){
            GUIWIN_SeSoftkeytButtonIconId(win_id,
                                  IMAGE_PICVIEW_VIDEO_PLAY,
                                  MIDDLE_BUTTON,
                                  TRUE);
        ...
        if(MMIPICVIEW_IsVideoSuffix(&file_info)){
            ...
            if (!MMIAPIFMM_IsFileExist (thumbnail_name, thumbnail_name_len)){
                ...
                return (anim_result);
                 ...
                 //set file name
                anim_info.full_path_wstr_ptr = thumbnail_name;
                anim_info.full_path_wstr_len = thumbnail_name_len;
            }
            display_info.is_syn_decode  = FALSE;
            display_info.is_auto_zoom_in = TRUE;
...
)
{
            ...
            (!GUI_IsRectEmpty(owner_draw_ptr->display_rect)))
    {
...
#ifdef HW_LCD_TEST
        //set softkey visible
        GUIWIN_SetSoftkeyVisible(owner_draw_ptr->win_handle, FALSE);
#else
        //set softkey visible
        ...
        {
            //start display timer
           // StartDisplayTimer(owner_draw_ptr->win_handle);//zhuyongjing delete
        }
    }
}
...
)
{
    ...
    if (s_mmipicview_display_timer_id == timer_id)
    {
       ...
       // GUIWIN_SetSoftkeyVisible(win_id, FALSE);//zhuyongjing delete 

        //set not display preview osd
       //SetDisplayPreviewOsd(FALSE);//zhuyongjing delete 
#else
...
#ifdef MMI_SETTING_WALLPAER_WITH_PREVIEW
            if(MMIPICVIEW_GetEnterType()==MMIPICVIEW_PHOTOS_FROM_WALLPAPER){
                GUIWIN_SeSoftkeytButtonTextId(MMIPICVIEW_PREVIEW_WIN_ID, TXT_NULL, LEFT_BUTTON, TRUE);
                GUIWIN_SeSoftkeytButtonTextId(MMIPICVIEW_PREVIEW_WIN_ID, TXT_COMMON_COMMON_BACK, RIGHT_BUTTON, FALSE);
                GUIWIN_SeSoftkeytButtonTextId(MMIPICVIEW_PREVIEW_WIN_ID, STXT_SOFTKEY_SELECT_MK, MIDDLE_BUTTON, FALSE);
            }else
#endif
	{
            if(MMIPICVIEW_IsVideoSuffix(&file_info) == TRUE){
                GUIWIN_SeSoftkeytButtonIconId(MMIPICVIEW_PREVIEW_WIN_ID,
                                      IMAGE_PICVIEW_VIDEO_PLAY,
                                      MIDDLE_BUTTON,
                                      TRUE);
            }else{
                GUIWIN_SeSoftkeytButtonIconId(MMIPICVIEW_PREVIEW_WIN_ID,
                                      IMAGE_GALLERY_ZOOM_IN,
                                      MIDDLE_BUTTON,
                                      TRUE);
            }
	}
        ...
        //add pic
        anim_result = SetAnimParam(FALSE, TRUE, cur_index, ctrl_id);
...
#else
        SetAnimParam(FALSE, FALSE, item_index, MMIPICVIEW_PREVIEW_ANIM_CTRL_ID);
#endif
    }
}
#if defined(PICVIEW_F_VIEW_ZOOM)
                                      ...
                                      )
{
    ...
    if(MMIPICVIEW_GetPicFileInfo(item_index,&file_info))
    {
        if(MMIPICVIEW_PICOPE_POINT == ope_type)
        {
            ...
            {
                result = TRUE;
                MMI_GetLabelTextByLang(STR_FILE_EMPTY,&note_info); //NEWMS00181048 
                MMIPUB_OpenAlertWinByTextPtr(PNULL,&note_info,PNULL,IMAGE_PUBWIN_WARNING,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
            }
            else
            {
        ...
        else if (MMIPICVIEW_PICOPE_GLID == ope_type && ctrl_id == GetMiddleAnimCtrlId())
        {
            ...
            {
                result = TRUE;
                SetLabelDisplayByTextId(MMIPICVIEW_PREVIEW_LABEL_CTRL_ID, STR_FILE_EMPTY); //NEWMS00181048 
            }
            else
            {
                if (!MMIAPIFMM_GetPictureWidthHeight(file_info.filename,file_info.name_len,file_info.size,&pic_width,&pic_height))
                {
	                result = TRUE;
	                SetLabelDisplayByTextId(MMIPICVIEW_PREVIEW_LABEL_CTRL_ID, STR_FILE_NOT_SUPPORT);
				}
            }
        }
#endif        
...
#endif
        SCI_TRACE_LOW("[MMIPIC]:MMIPICVIEW_RefreshAnimDisplay 1");
        ModifyPreviewTitle((item_index + 1), pic_num, setlist_ctrl_id);
}
#if defined (TOUCH_GESTURE_ENABLE) && defined(PICVIEW_F_VIEW_ZOOM)
/*****************************************************************************/
