...
#endif
LOCAL uint8 s_mmipicview_zoom_level=0;         //lijinghua add it for pic zoom
/*---------------------------------------------------------------------------*/
/*                         TYPE AND CONSTANT                                 */
/*---------------------------------------------------------------------------*/
...
LOCAL uint8 GetZoomRatioMax(MMI_CTRL_ID_T ctrl_id);//zhuyongjing add for get zoom ratio

//lijinghua add for pic zoom						 
PUBLIC BOOLEAN MMIPICVIEW_UpdateZoomBar(MMI_WIN_ID_T	  win_id,uint8 zoom_level);
/**--------------------------------------------------------------------------*
 **                         CONSTANT VARIABLES                               *
 **--------------------------------------------------------------------------*/
...
WINDOW_TABLE( MMIPICVIEW_ZOOM_WIN_TAB ) = 
{
    ...
    WIN_HIDE_STATUS,      //lijinghua add it for pic zoom
    ...
    WIN_SOFTKEY(TXT_NULL,TXT_NULL,TXT_NULL),  //lijinghua change it
    END_WIN
};
                                       ...
                                       )
{
    ...
    SCI_TRACE_LOW("MMIPICVIEW HandlePicZoomWinMsg msg_id=%d",msg_id);
    switch (msg_id)
    {
    case MSG_OPEN_WINDOW:
        ...
        GUIWIN_SetSoftkeyVisible(win_id,TRUE); //lijinghua modify it
	...
	GUIWIN_SeSoftkeytButtonIconId(win_id,IMAGE_GALLERY_ZOOM_OUT,0,FALSE);
	GUIWIN_SeSoftkeytButtonIconId(win_id,IMAGE_GALLERY_ZOOM_IN,1,FALSE);
	...
	case MSG_FULL_PAINT:
//		MMIPICVIEW_UpdateZoomBar(win_id,s_mmipicview_zoom_level);
		break;
	//lijinghua end
    case MSG_TIMER:
        //result = HandleZoomTimer(*(uint8*)param,win_id,ctrl_id,osd_ctrl_id);   //lijinghua delete it
        break;

    case MSG_APP_OK:
    case MSG_KEYREPEAT_OK:
        if(s_mmipicview_zoom_level == 1){
            MMK_CloseWin(win_id);
            break;
        }
        if (HandleDisplayZoomOsd(win_id,ctrl_id,osd_ctrl_id))
        {
            GUIANIM_Zoom(ctrl_id,FALSE,TRUE);
             //zhuyongjing add for max zoomin Msk gray
            if(s_mmipicview_zoom_level == GetZoomRatioMax(ctrl_id)){
                GUIWIN_SeSoftkeytButtonIconId(win_id,IMAGE_GALLERY_ZOOM_IN,MIDDLE_BUTTON,TRUE);
                //GUIWIN_SetSoftkeyBtnGray(win_id,MIDDLE_BUTTON,FALSE);
            }
            //zhuyongjing add end

            //lijinghua add it for pic zoom
            if(s_mmipicview_zoom_level>0)
            {
            	s_mmipicview_zoom_level--;
            }
	...
	//lijinghua add it for pic zoom
            //if(s_mmipicview_zoom_level<4)//zhuyongjing change max ---max not a fix value
            if(s_mmipicview_zoom_level < GetZoomRatioMax(ctrl_id))
            {
            	s_mmipicview_zoom_level++;
            }
         //zhuyongjing add for max zoomin Msk gray
            if (GUIANIM_GetZoomWidthRange(ctrl_id, PNULL, &width_max, PNULL, &height_max))
            {
                ...
                { 
                    GUIWIN_SeSoftkeytButtonIconId(win_id,IMAGE_GALLERY_ZOOM_IN_DIM,MIDDLE_BUTTON,TRUE);
    ...
    case MSG_APP_RIGHT:
        if (HandleDisplayZoomOsd(win_id,ctrl_id,osd_ctrl_id))
        {
			...
			MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);   //lijinghua add for pic zoom
        }
        break;
    ...
    case MSG_APP_CANCEL:
        if (HandleDisplayZoomOsd(win_id,ctrl_id,osd_ctrl_id))
        {
            s_mmipicview_zoom_level=0;   //lijinghua add for pic zoom
            MMK_CloseWin(win_id);
        }
        break;
                             ...
                             )
{
    ...
    if (GUIANIM_GetZoomWidthRange(ctrl_id,&width_min,&width_max,PNULL,PNULL))
    {
        ...
        {
            is_gray = FALSE;
            img_id  = IMAGE_GALLERY_ZOOM_OUT;  //lijinghua modify 
        }
        else
        {
            is_gray = TRUE;
            img_id  = IMAGE_GALLERY_ZOOM_OUT;

            ProcessPicState(MMIPICVIEW_PIC_MIN_ZOOM_OUT,      //lijinghua modify 
                            win_id,
                            ctrl_id
                            );
        }
        ...
        {
            is_gray = FALSE;
            img_id  = IMAGE_GALLERY_ZOOM_IN;     //lijinghua modify 
        }
        else
        {
            is_gray = TRUE;
    ...
    //set right softkey
    GUIWIN_SeSoftkeytButtonTextId(win_id,
        TXT_COMMON_COMMON_BACK,                  //lijinghua modify 
        2,
        TRUE);
}
                             ...
                             )
{
    ...
    {
       // s_mmipicview_osd_timer_id = MMK_CreateWinTimer(win_id,MMIPICVIEW_OSD_TIME,FALSE); //lijinghua delete it
    }
    else
    {
                                   ...
                                   )
{
    ...
    if (!result)
    {
        ...
        //display zoom osd
     //   DisplayZoomOsd(win_id,ctrl_id,osd_ctrl_id);//zhuyongjing delete
     ...
     case MMIPICVIEW_PIC_DECODING:
        {
            ...
            else
            {
                ...
                if (s_mmipicview_sleep_counter < MMIPICVIEW_VIEW_ZOOM_SLEEP_COUNTER_MAX)
                {
                    ...
                    //zhuyongjing add for special large pic
                }else{
                    s_mmipicview_sleep_counter = 0;
                    MMK_CloseWin(win_id);
                }
                    //add end
            }  
...
PUBLIC BOOLEAN MMIPICVIEW_UpdateZoomBar(MMI_WIN_ID_T	 win_id,uint8 zoom_level)
{
		...
		GUI_RECT_T rect_plus = {0};
		GUI_RECT_T rect_minus = {0};
		GUI_RECT_T rect_indicator = {0};
		...
		rect_bg.right = 6;
		rect_bg.bottom = 282;
		LCD_FillArgbRect(&lcd_dev_info, rect_bg, MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_DEFAULT), (uint8)GUIILIST_MASK_BG_ALPHA);
	    ...
	    dis_point.x = rect_plus.left;
	    dis_point.y = rect_plus.top;
		...
		rect_minus.top = 252;
		rect_minus.right = 36;
		rect_minus.bottom = 282;

	    dis_point.x = rect_minus.left;
	    dis_point.y = rect_minus.top;
	    GUIRES_DisplayImg(&dis_point,
        PNULL,
        PNULL,
        win_id,
		...
		// Indicator
		GUIRES_GetImgWidthHeight(&left_icon_width,&left_icon_height, 
		0,
		win_id);
		rect_indicator.left = 0;
		rect_indicator.right = 6;
		rect_indicator.top = 282-9-zoom_level*((282-9)/GetZoomRatioMax(MMIPICVIEW_ZOOM_ANIM_CTRL_ID));
	    rect_indicator.bottom = rect_indicator.top+left_icon_height;

	    dis_point.x = rect_indicator.left;
	    dis_point.y = rect_indicator.top;
		GUIRES_DisplayImg(&dis_point,
		PNULL,
		PNULL,
		win_id,
...
//zhuyongjing add for get zoom ratio for diff pic
LOCAL uint8 GetZoomRatioMax(MMI_CTRL_ID_T ctrl_id){
    uint16          height_max = 0;
    uint16          width_max = 0;
    uint8 max= 0;
  ...
  /*  anim_ctrl_ptr = GUIANIM_GetPtr(ctrl_id);
    ...
    {
        SCI_TRACE_LOW("MMIPICVIEW GetZoomRatioMax MMIPICVIEW_GetPicFileInfo  error");
        return 1;
    }
    if(MMIAPIFMM_GetPictureWidthHeight(file_info.filename,file_info.name_len,file_info.size,&pic_width,&pic_height)){

        if(GUIANIM_GetZoomWidthRange(ctrl_id, PNULL, &width_max, PNULL, &height_max)){
            uint16 targetwidth = 0;
            if(pic_width >= lcd_width){
                targetwidth= lcd_width;
            }else{
                targetwidth = pic_width;
            }
            while(targetwidth <= width_max){
                uint16 pre_targetwidth = targetwidth;
                targetwidth = targetwidth* 5/4;//anim_ctrl_ptr->theme.zoom_in_ratio.num1/anim_ctrl_ptr->theme.zoom_in_ratio.num2;
                max++;
                if(pre_targetwidth == targetwidth){//just for 1*1 icon
                    targetwidth++;
                }
            }
