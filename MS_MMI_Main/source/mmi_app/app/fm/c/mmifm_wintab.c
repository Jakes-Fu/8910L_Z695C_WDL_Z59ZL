...
LOCAL BOOLEAN is_first_open = FALSE;
...
extern uint16 s_fm_freq_range_low;
extern uint16 s_fm_freq_range_high;
...
//#endif
    ...
    #ifdef FM_RDS_SUPPORT    
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_LEFT,MMIFM_RDS_PS_CTRL_ID),
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_LEFT,MMIFM_RDS_RT_CTRL_ID),
    #endif
#if !defined(MMI_FM_MINI_SUPPORT) && !defined( MMI_MINI_QVGA_UI_SUPPORT)
...
WINDOW_TABLE( MMI_FM_SET_TIMER_INFO_WIN_TAB ) = 
{
    ...
    CHILD_LABEL_CTRL(GUILABEL_ALIGN_LEFT,TRUE,MMIFM_EDIT_TIME_ENDURE_LABEL_CTRL_ID,MMIFM_EDIT_TIME_ENDURE_FORM_CTRL_ID),
#ifdef MMI_PDA_SUPPORT
    ...
    CHILD_LABEL_CTRL(GUILABEL_ALIGN_LEFT,TRUE,MMIFM_EDIT_FREQUENCY_LABEL_CTRL_ID,MMIFM_EDIT_FREQUENCY_FORM_CTRL_ID),
#ifdef MMI_PDA_SUPPORT
...
PUBLIC void MMIFM_OpenMainWin(void)
{
    ...
    if(MMITHEME_IsTVOpen())
    {
        ...
        if( MMIAPIBT_IsBtOn() ) 
        {
            MMIPUB_OpenAlertWarningWin(TXT_EXIT_BT_IS_OPEN);
            return;
        }
#endif
...
/*****************************************************************************/
//  Description : open play FM main window without play fm
//  Global resource dependence : 
//  Author:
//  Note: 
/*****************************************************************************/
PUBLIC void MMIFM_OpenMainWinNoPlay(void)
{
    if(MMIAPICC_IsInState(CC_IN_CALL_STATE))
    {
        MMIPUB_OpenAlertWarningWin(TXT_CALL_USING);
...
PUBLIC void SetPSText(wchar* ps){
    int32     str_len = 0;
    MMI_STRING_T      * string_ptr  = PNULL;
    MMI_STRING_T       string  = {0};
    if(PNULL == ps){
         SCI_TRACE_LOW("[MMIFM] Set RDS text ps is null");
         GUILABEL_SetText(MMIFM_RDS_PS_CTRL_ID,&string,TRUE);
    }

    if(PNULL != ps){
        string_ptr= (MMI_STRING_T *)ps;       
        str_len = MMIAPICOM_Wstrlen(string_ptr);
        string.wstr_len = str_len;
        string.wstr_ptr = string_ptr; 
    

        MMK_PostMsg(MMIFM_RDS_PS_CTRL_ID, MSG_CTL_GET_ACTIVE, PNULL, PNULL);
        GUILABEL_SetFont(MMIFM_RDS_PS_CTRL_ID, SONG_FONT_15, MMIFM_FONT_CLOLOR);
        GUILABEL_SetText(MMIFM_RDS_PS_CTRL_ID,&string,TRUE);    
    }    

}

PUBLIC void SetRTText(wchar  * rt){
    int32     str_len = 0;
    MMI_STRING_T      * string_ptr  = PNULL;
    MMI_STRING_T       string  = {0};

    if(PNULL == rt){     
         SCI_TRACE_LOW("[MMIFM] Set RDS text rt is null");
         GUILABEL_SetText(MMIFM_RDS_RT_CTRL_ID,&string,TRUE);
    }
    
    if(PNULL != rt){
        string_ptr= (MMI_STRING_T *)rt;       
        str_len = MMIAPICOM_Wstrlen(string_ptr);
        SCI_TRACE_LOW("[MMIFM] Set RDS text rt len: %d", str_len);
        string.wstr_len = str_len;
        string.wstr_ptr = string_ptr;    

        MMK_PostMsg(MMIFM_RDS_RT_CTRL_ID, MSG_CTL_GET_ACTIVE, PNULL, PNULL);
        GUILABEL_SetFont(MMIFM_RDS_RT_CTRL_ID, SONG_FONT_15, MMIFM_FONT_CLOLOR);
        GUILABEL_SetText(MMIFM_RDS_RT_CTRL_ID,&string,TRUE);    
    }

}

PUBLIC void SetRDSText(wchar* ps,wchar  * rt){
    SetPSText(ps);
    SetRTText(rt);
}
                                   ...
                                   )
{
    ...
    switch (msg_id)
    {
        case MSG_OPEN_WINDOW:
        {
	     if(FM_SUCCESS != FM_Open())
	     {
                SCI_TRACE_LOW("[MMIFM] open fail");
            }
...
//	   GUIEDIT_SetHandleRedKey(FALSE, MMIFM_EDITBOX_CTRL_ID);
   ...
   //case MSG_APP_MENU:
       ...
       if(MMIFM_SAVE_FREQUENCY == opt_type)
       {
           ...
           if(MMIFM_INVALID_CHANNEL != MMIFM_GetChannelIndexByFreq(channel.freqency))
           {
               MMIPUB_OpenAlertFailWin(TXT_FM_FREQUENCY_ALREADY_EXIST);           
           }
           else if(!MMIFM_SetOneChannel(GetCurrentSelChannel(), &channel))
           {
               //save error
           ...
           else
           {
              ...
              // MMIPUB_OpenAlertSuccessWin(TXT_FM_SAVE_CHANNEL);                
               ...
               MMIFM_SetCurrrentChannel(GetCurrentSelChannel());
               MMK_CloseWin(MMI_FM_CHANNEL_LIST_WIN_ID);
       ...
       else if(MMIFM_MANAUL_SEARCH == opt_type)
       {
         ...
         {
           if(MMIFM_PlayFreq(MMIAPIFM_ConvertStringToFreq(&str_t)))
           {
             SetButtonBgWhenResume();
           }
		   /*
...
//#endif
		...
		MMK_SetAtvCtrl(win_id, MMIFM_DETAILS_NAME_FORM_CTRL_ID);
		break;
	case MSG_APP_OK:
	case MSG_CTL_OK: 
		...
		freq = MMIAPIFM_ConvertStringToFreq(&fm_fre_info);
...
LOCAL void DrawChannelDetails(MMI_WIN_ID_T win_id,const MMIFM_CHANNEL_T *channel_ptr)
{
	...
	// Channel name
	GUIEDIT_SetStyle(MMIFM_DETAILS_NAME_EDIT_LABEL_CTRL_ID, GUIEDIT_STYLE_MULTI_DYN_DOWN);
	GUIEDIT_SetFontColor(MMIFM_DETAILS_NAME_EDIT_LABEL_CTRL_ID, MMI_BLACK_COLOR);
	CTRLBASEEDIT_SetLabelId(win_id, MMIFM_DETAILS_NAME_EDIT_LABEL_CTRL_ID, TXT_FM_CHANNEL_ANME_COLON, FALSE);
	...
	// Channe frequency
	GUIEDIT_SetStyle(MMIFM_DETAILS_FREQ_EDIT_LABEL_CTRL_ID, GUIEDIT_STYLE_MULTI_DYN_DOWN);
	GUIEDIT_SetFontColor(MMIFM_DETAILS_FREQ_EDIT_LABEL_CTRL_ID, MMI_BLACK_COLOR);
	CTRLBASEEDIT_SetLabelId(win_id, MMIFM_DETAILS_FREQ_EDIT_LABEL_CTRL_ID, TXT_FM_FREQEUENCY_COLON, FALSE);
    ...
    if(PNULL != channel_ptr && MMIFM_IsChannelValid(channel_ptr))
    {
		...
		MMIAPIFM_ConvertFreqToString(channel_ptr->freqency, (char*) num_buf);
...
PUBLIC void MMIFM_GetChannelDispName(const MMIFM_CHANNEL_T *channel_ptr, uint16 channel_index, wchar *disp_name_ptr, uint16 max_name_len)
{
	...
	else
	{
		...
		if(str_t.wstr_len == 0)
		{
			...
			MMIAPIFM_ConvertFreqToString(channel_ptr->freqency, (char*) freq_str);
...
LOCAL void CreateMainOptMenu(uint32 ctr_id)
{
    ...
    {  
        ...
        #ifdef FM_RDS_SUPPORT
        if((MMIFM_RDS_ENABLE == s_fm_main_opt_item[i][MMIFM_POP_MENU_NODE_ID] )
...
#endif
  #ifndef ZDT_PRODUCT_Z685S
#if (defined(MMI_FM_NEED_HEADSET) && defined(FM_S_ANT_SUPPORT) ) || (!defined(MMI_FM_NEED_HEADSET))
                                           ...
                                           )
{
    ...
    case MSG_CLOSE_WINDOW:
		...
		// 工模FM test 可能调用auto search, 在这里调用callback
		// 后续是否为工模调用的则由这个callback判断，FM应用不做处理
		MMIAPIENG_AutoSearchCallback();
        break;
    default:
        result = MMIPUB_HandleWaitWinMsg(win_id, msg_id, param);;
        break;
...
/*****************************************************************************/
//  Discription: draw mini fm radio's main pannel
//  Global resource dependence: none 
//  Author: 
//  Note:
//
...
LOCAL void DrawFrqqencyText(MMI_WIN_ID_T win_id)
{
	...
	if (PREVIEW_START_FM == start_way)
	{
		MMIAPIFM_ConvertFreqToString(cur_info_ptr->export_freq, (char*) freq_number);
	}
	else
	{
		MMIAPIFM_ConvertFreqToString(cur_info_ptr->cur_freq, (char*) freq_number);
	}
...
PUBLIC void MMIFM_HandleHeadsetAction(MMI_MESSAGE_ID_E msg_id)
{
    ...
    cur_info_ptr = MMIFM_GetFMCurrentInfo();
	SCI_TRACE_LOW("[MMIFM] MMIFM_HandleHeadsetAction status %d", cur_info_ptr->status);
    if(FM_IsInit())
    {
#ifdef FM_S_ANT_SUPPORT
        //耳机无论插拔，根据状态进行长短天线和对应音频的切换
        MMIFM_CheckAndSwitchFmAnt(FALSE, msg_id);
#else
...
LOCAL void DisplayFMFrePic(GUIANIM_OWNER_DRAW_T    *draw_ptr)
{
    ...
    if(PNULL != cur_info_ptr)
    {
        MMIFM_START_WAY_E           start_way = NORMAL_START_FM;
        #ifdef FM_RDS_SUPPORT
        GUIRES_GetImgWidthHeight(&icon_width, NULL, IMAGE_FM_ADJ_BUT_RDS, FM_MAIN_WIN_ID);
        #else
        GUIRES_GetImgWidthHeight(&icon_width, NULL, IMAGE_FM_ADJ_BUT, FM_MAIN_WIN_ID);
        #endif
        if(draw_ptr != NULL)
    ...
    dis_point.y = adj_but_left_y;
     #ifdef FM_RDS_SUPPORT
     GUIRES_DisplayImg(&dis_point, PNULL, PNULL, FM_MAIN_WIN_ID, IMAGE_FM_ADJ_BUT_RDS, &lcd_dev_info);
     #else
     GUIRES_DisplayImg(&dis_point, PNULL, PNULL, FM_MAIN_WIN_ID, IMAGE_FM_ADJ_BUT, &lcd_dev_info);
     #endif
    #endif
...
//#endif*/
    ...
    switch(msg_id)
    {
        ...
        case MSG_CLOSE_WINDOW:
        {               
            if(MMIAPIFMM_IsFileExist(MMIFM_GetRecordFilePath(),MMIAPICOM_Wstrlen(MMIFM_GetRecordFilePath())))
            {
...
#ifdef MMI_FM_TIMER_RECORD_SUPPORT
            SCI_TRACE_LOW("MMIFM s_is_need_set_bgplay %d", s_is_need_set_bgplay);

            if(FM_timer_record == record_type)
            {                 
                if(s_is_need_set_bgplay
                    && (FM_PLAYING == MMIFM_GetStatus())
                    && MMIFM_IsBGPlay())
                {   
                    SCI_TRACE_LOW("MMIFM background play");

                    MMIFM_Exit(TRUE);
                }
                else  if(FM_STOP == MMIFM_GetStatus())
                { 
                      SCI_TRACE_LOW("MMIFM stopped after timer record");
                      MMIFM_Exit(FALSE);
                }
                else if(FM_SUSPENDED == MMIFM_GetStatus())
                {
                      SCI_TRACE_LOW("MMIFM SUSPENDED after timer record");
                      MMIFM_Stop();
                      #ifndef WIN32
        ...
        {   
            ...
            if(FM_PLAYING != MMIFM_GetStatus())
            {
                  SCI_TRACE_LOW("MMIFM stop FM when stop record");
                  FM_PlayStop();
                  MMISRVAUD_Stop(MMIFM_GetPlayHandle());
            }
            if(STARTUP_NORMAL == MMIAPIPHONE_GetStartUpCondition())
            ...
            if (MMIAPICOM_FileNameIsValid(&name_str))
            {
                    ...
                    || !MMIAPIFMM_IsFileExist(buffer_ptr,full_path_len))
                {
                    ...
                    if(is_rename_success)
                    {
                     ...
                     //   MMIPUB_OpenAlertSuccessWin(TXT_SUCCESS);
#endif
                        SCI_TRACE_LOW("MMIFM: Save record File success");
...
#ifndef MMI_PDA_SUPPORT
			GUIWIN_SetSoftkeyTextId(win_id, TXT_COMMON_OK, TXT_NULL, TXT_DELETE, TRUE);
#endif
			break;
		case MMIFM_EDITWIN_BUTTON0_CTRL_ID:
		case MMIFM_EDITWIN_BUTTON1_CTRL_ID:
...
LOCAL BOOLEAN  SaveTimerRecordFre(void)
{
   ...
   // int16           i_endure_time = 0;
    ...
    record_info.record_freq = MMIAPIFM_ConvertStringToFreq(&str);
...
LOCAL BOOLEAN  SaveTimerRecordInfo(void)
{
    ...
    if(MMIFM_INVALID_CHANNEL ==MMIFM_GetChannelIndexByFreq(MMIAPIFM_ConvertStringToFreq(&str)))
    {
       one_channel.freqency= MMIAPIFM_ConvertStringToFreq(&str);
       next_available_id = MMIFM_GetNextAvailableChannelID();
       if(-1 != next_available_id)
       {
           MMIFM_SetOneChannel(next_available_id, &one_channel);
       }
    }
        ...
        if(HEADSET_BUTTON_SHORT_PRESS == action_type)
        {
...
#endif
          {
            ...
            else if(MMIFM_GetStatus() == FM_SUSPENDED)
            {
              if(MMIFM_GetSuspendByFM())
              {
                cur_info_ptr->cur_freq = MIN(cur_info_ptr->cur_freq, s_fm_freq_range_high);
                cur_info_ptr->cur_freq = MAX(cur_info_ptr->cur_freq, s_fm_freq_range_low);
                MMIFM_EnableMute();
...
LOCAL void  SetButtonsAndLabelRect(MMI_WIN_ID_T win_id)
{
...
#ifdef FM_RDS_SUPPORT
    uint16 rds_height = GUI_ConvertGUIFontToSize(SONG_FONT_15);
#endif
    ...
    both_rect.v_rect.bottom = both_rect.v_rect.top + rds_height;
    ...
    both_rect.h_rect.bottom=both_rect.h_rect.top + rds_height;
    both_rect.h_rect.left= MMIFM_H_LEFT_BUTTON_START_X;
    both_rect.h_rect.right=anim_width;
    GUIAPICTRL_SetBothRect(MMIFM_RDS_PS_CTRL_ID,&both_rect);

    
    both_rect.v_rect.top    =both_rect.v_rect.bottom + 1;
    both_rect.v_rect.bottom = both_rect.v_rect.top + rds_height;
    
    both_rect.h_rect.top = both_rect.h_rect.bottom + 1;
    both_rect.h_rect.bottom=both_rect.h_rect.top + rds_height;
    GUIAPICTRL_SetBothRect(MMIFM_RDS_RT_CTRL_ID,&both_rect);
#endif
    ...
    #ifdef FM_RDS_SUPPORT
    both_rect.h_rect.top = win_client_both_rect.h_rect.top + anim_height +   rds_height + rds_height;
    #else
...
LOCAL void  SetButtonsAndLabelRect(MMI_WIN_ID_T win_id)
{
    ...
    both_rect.v_rect.bottom = both_rect.v_rect.top + rds_height;
    //both_rect.v_rect.left   =MMIFM_V_LEFT_BUTTON_START_X;
    //both_rect.v_rect.right  =left_width;
