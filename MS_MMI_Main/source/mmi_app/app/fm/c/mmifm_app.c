...
LOCAL uint16	fm_tune_step = 0;

#ifdef FM_RDS_SUPPORT
#define RDS_TIMER_PERIOD  1700
#define MAX_RDS_RT_LENGTH 64
#define MAX_RDS_PS_LENGTH 8

LOCAL uint8 s_timer_read_rds = 0;
LOCAL char ps_last[MAX_RDS_PS_LENGTH +1] = {0};
LOCAL char rt_last[MAX_RDS_RT_LENGTH +1] = {0};
#endif

/**--------------------------------------------------------------------------*
 **                         PUBLIC VARIABLE DEFINITION                       *
 **--------------------------------------------------------------------------*/
PUBLIC uint16	s_fm_freq_range_low = 8750;
PUBLIC uint16	s_fm_freq_range_high = 10800;
...
LOCAL uint32 MMIFM_ReadFmTuneStep(void);
...
/*****************************************************************************/
//  Description : Update Idle EarPhone State when switch audio device
//  Global resource dependence : 
//  Author:  
//  Note:
/*****************************************************************************/
LOCAL void MMIFM_UpdateIdleEarPhoneState(BOOLEAN enable);
...
//  Note:
/*****************************************************************************/
PUBLIC  void  MMIFM_ReadRDS(BOOLEAN  force){

    int32 str_len = 0;
    /*
        rds_data_ptr->ps_data.PS[2] 8 bytes as ps data,
        use rds_data_ptr-> rt_data.textdata[2] 64 bytes as rt data
     */

    wchar psW[MAX_RDS_PS_LENGTH +1] = {0};
    wchar rtW[MAX_RDS_RT_LENGTH+1] = {0};
    
    char ps[MAX_RDS_PS_LENGTH +1] = {0};
    char rt[MAX_RDS_RT_LENGTH +1] = {0};
    //char blank[MAX_RDS_PS_LENGTH] = {"        "}; 
    //char rtBlank[MAX_RDS_RT_LENGTH] = {"                                                                "}; 


    #ifdef  WIN32  
    LOCAL char looper ;
    char   *wStrRDS_RT = PNULL;
    char   *wStrRDS_PS = PNULL;

    char    rds_ps_data1[MAX_RDS_PS_LENGTH + 1] = {"RDS PS 2"};
   // char    rds_ps_data1[MAX_RDS_PS_LENGTH + 1] = {"        "};
    char    rds_ps_data2[MAX_RDS_PS_LENGTH + 1] = {"RDS PS 2"};
    char    rds_data1[MAX_RDS_RT_LENGTH + 1] = {"11111222223333344444555556666677777888889999900000aaaaabbbbbcccc"};

    //char    rds_data2[MAX_RDS_RT_LENGTH + 1] = {"11111222223333344444555556666677777888889999900000aaaaabbbbbcccc"};
    char    rds_data2[MAX_RDS_RT_LENGTH + 1] = {"aaaaabbbbbcccc111112222233333dddddeeeeefffffggg"};
    
    if(0 == looper%2){
        wStrRDS_RT = rds_data1;   
        wStrRDS_PS = rds_ps_data1;    

    } else {
        wStrRDS_RT = rds_data2;    
        wStrRDS_PS = rds_ps_data2;    
    }
    
    looper ++;

    SCI_MEMCPY(ps,wStrRDS_PS,MAX_RDS_PS_LENGTH);
    SCI_MEMCPY(rt,wStrRDS_RT,MAX_RDS_RT_LENGTH);
    
    #else
    //#if 0
    struct fm_rds_data *rds_data_ptr = SCI_NULL;

    rds_data_ptr =  
        (struct  fm_rds_data *)SCI_ALLOC_APP(sizeof(struct fm_rds_data));

    if(SCI_NULL == rds_data_ptr){
        SCI_TRACE_LOW("[MMIFM] MMIFM_ReadRDS  ALLOC rds_data_ptr fail  !");
        return;
    }
    
    if(TRUE == FM_RDS_Get_Data(rds_data_ptr)){
        SCI_MEMCPY(ps,rds_data_ptr->ps_data.PS[2],MAX_RDS_PS_LENGTH);
        SCI_MEMCPY(rt,rds_data_ptr->rt_data.textdata[3],MAX_RDS_RT_LENGTH);
        SCI_TRACE_LOW("[MMIFM] MMIFM_ReadRDS PS %s, rt %s",rds_data_ptr->ps_data.PS[2],rds_data_ptr->rt_data.textdata[3]);
        SCI_FREE(rds_data_ptr);
    }else {
        SCI_TRACE_LOW("[MMIFM] MMIFM_ReadRDS FM_RDS_Get_Data false ");
        SetRDSText(PNULL,PNULL);
        SCI_FREE(rds_data_ptr);
        return;
    }
    #endif

    // force to show rds text when open FM
    if(force){
        SCI_MEMCPY(ps_last,ps,MAX_RDS_PS_LENGTH);
        SCI_MEMCPY(rt_last,rt,MAX_RDS_RT_LENGTH);
        MMIAPICOM_StrToWstr(ps, psW); 
        MMIAPICOM_StrToWstr(rt, rtW);
        SetRDSText(psW,rtW);
        return;
    }

    // not show RDS when it is same as last time    
    if((0 == MMIAPICOM_StrCmp(ps,MAX_RDS_PS_LENGTH,ps_last,MAX_RDS_PS_LENGTH))
         &&(0 == MMIAPICOM_StrCmp(rt,MAX_RDS_RT_LENGTH,rt_last,MAX_RDS_RT_LENGTH)))
    {
        return;
    }

    // not show RDS when ps is blank
    //if(0 != MMIAPICOM_StrCmp(blank,MAX_RDS_PS_LENGTH, ps, MAX_RDS_PS_LENGTH))
    {    
        SCI_MEMCPY(ps_last,ps,MAX_RDS_PS_LENGTH);
        MMIAPICOM_StrToWstr(ps, psW); 
        SetPSText(psW);
    ...
    //if(0 != MMIAPICOM_StrCmp(rtBlank,MAX_RDS_RT_LENGTH, rt, MAX_RDS_RT_LENGTH))
    {    
        SCI_MEMCPY(rt_last,rt,MAX_RDS_RT_LENGTH);
        MMIAPICOM_StrToWstr(rt, rtW);
        SetRTText(rtW);
...
/*****************************************************************************/
//  Description : pause read rds
//  Global resource dependence : 
//  Author:
//  Note: 
/*****************************************************************************/
PUBLIC void MMIFM_RdsStop(void)
{
    SCI_TRACE_LOW("[MMIFM] MMIFM_RdsStop");
    MMIFM_StopRDSTimer();
    SCI_MEMSET(ps_last,0,MAX_RDS_PS_LENGTH+1);
    SCI_MEMSET(rt_last,0,MAX_RDS_RT_LENGTH+1);

    #ifndef WIN32
    if(FM_SUCCESS !=FM_RDS_Close()){
        SCI_TRACE_LOW("[MMIFM] MMIFM_RdsStop failed");
    }
    #endif   
}
...
/*****************************************************************************/
//  Description : resume read rds
//  Global resource dependence : 
//  Author:
//  Note: 
/*****************************************************************************/
PUBLIC void MMIFM_RdsOpen(void)
{
    SCI_TRACE_LOW("[MMIFM] MMIFM_RdsOpen");
    MMIFM_StartRDSTimer();
    #ifndef WIN32
    if(FM_SUCCESS != FM_RDS_Open()){
         SCI_TRACE_LOW("[MMIFM] RDS OPEN ERROR!");
    }
    #endif
...
{
    SCI_TRACE_LOW("[MMIFM] MMIFM_SwitchRDS  , current status is %d ",s_fm_cur_info.rds_open);
...
//  Description : 选择声音设备
//  Global resource dependence : 
//  Author: weipeng.wang 2018/10/26
//  Parameter: 无
//  Note: porting from 8910 2019/11/26
/*****************************************************************************/
PUBLIC BOOLEAN MMIFM_SetAudioDevice(MMIFM_AUDIO_DEVICE_MODE_E   sound_mode)
{
    BOOLEAN result = TRUE;
    MMISRVAUD_ROUTE_T support_route = MMISRVAUD_ROUTE_EARPHONE|MMISRVAUD_ROUTE_SPEAKER;

    SCI_TRACE_LOW("[FM_S_ANT] MMIFM_SetAudioDevice status %d sound_mode %d", s_fm_cur_info.status, s_fm_cur_info.sound_mode);
    switch (sound_mode)
    {
...
/*****************************************************************************/
//  Description : 耳机无论插拔，根据状态进行长短天线和对应音频的切换
//  Global resource dependence : 
//  Author: weipeng.wang 2018/10/26
//  Note: porting from 8910 2019/11/26
/*****************************************************************************/
PUBLIC void MMIFM_CheckAndSwitchFmAnt(BOOLEAN isForce, MMI_MESSAGE_ID_E msg_id)
{
    BOOLEAN headset_on = FALSE;
    MMIFM_AUDIO_DEVICE_MODE_E sound_mode = FM_HEADSET_MODE;
    // set FM short ant or long ant
    headset_on = MMIFM_CheckGPIOStatusAndSetFmLNA();
    
    SCI_TRACE_LOW("[FM_S_ANT] SwitchFmAnt headset_on = %d, isForce = %d, msg_id=%d", headset_on, isForce, msg_id);
   
    if(headset_on)
    {
        if((FM_HEADSET_MODE == MMIFM_GetFMCurrentDeviceMode()) || (msg_id == MSG_KEYDOWN_HEADSET_DETECT))
        {
            sound_mode = FM_HEADSET_MODE;
        }
        else
        {
            sound_mode = FM_SPEAKER_MODE;
        }
    }
    else
    {
    ...
    // Set soundmode, engmode use FM_HEADSET_MODE
    if (isForce)
    {
        MMIFM_SetAudioDevice(sound_mode);
    }
    SCI_TRACE_LOW("[FM_S_ANT] s_fm_cur_info.sound_mode = %d", s_fm_cur_info.sound_mode);
}
#endif
...
PUBLIC void MMIFM_Init(void)
{
    ...
    if(FALSE == read_ret)
    {
        ...
        if (fm_tune_step == 0)
		{
        	s_fm_cur_info.cur_freq  = MMIFM_FREQ_INITIAL_VAL_1;
        }
		else
		{
    ...
    if(fm_tune_step == 0)
    {
        s_fm_freq_range_low = 8750;
        s_fm_freq_range_high = 10800;
	}
}
...
LOCAL uint32 MMIFM_ReadFmTuneStep(void)
{
	...
	uint16 nv_len = 0;
	nv_len = NVITEM_GetLength(APP_NV_FM_TUNNING_STEP);	
	nv_reslut = EFS_NvitemRead(APP_NV_FM_TUNNING_STEP, nv_len, (uint8*)(&fm_tune_step));
	if (NVERR_NONE == nv_reslut)
	{
		SCI_TRACE_LOW("tt---nv:MMIFM_ReadFmTuneStep fm_tune_step=%d  ",fm_tune_step);
		return SCI_SUCCESS;
	}
	SCI_TRACE_LOW("tt---nv:MMIFM_ReadFmTuneStep error ");
	return SCI_ERROR;
}

PUBLIC uint16 MMIFM_GetFmTuneStep(void)
{
    return fm_tune_step;
}

/*****************************************************************************/
//  Description : check FM headset status
//  Global resource dependence : 
//  Author:weipeng.wang
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMIFM_CheckGPIOStatusAndSetFmLNA(void)
{
    BOOLEAN  status = GPIO_CheckHeadsetStatus();
    if(TRUE == status)
    {
       GPIO_SetFmLNA(0);
    }
    ...
    SCI_TRACE_LOW("[MMIFM] MMIFM_CheckGPIOStatusAndSetFmLNA status=%d", status);
    return status;
}
...
LOCAL BOOLEAN PlayCallBack(MMISRV_HANDLE_T handle, MMISRVMGR_NOTIFY_PARAM_T *param)
{
	...
	{
		SCI_TRACE_LOW("[MMIFM]PlayCallBack  event %d",param->event);
		switch(param->event)
		{
		...
		case MMISRVMGR_NOTIFY_RESUME:
			...
			{
#ifndef FM_S_ANT_SUPPORT            
				if(MMIFM_CheckHeadsetStatusWithoutPrompt())
#endif  
				{
					if(!MMIFM_GetSuspendByFM() && !MMK_IsOpenWin(FM_AUTO_SEARCH_WIN_ID))
					{
						s_fm_cur_info.cur_freq = MIN(s_fm_cur_info.cur_freq, s_fm_freq_range_high);
						s_fm_cur_info.cur_freq = MAX(s_fm_cur_info.cur_freq, s_fm_freq_range_low);
						MMISRVAUD_SetVolume(handle, 0);
		...
		case MMISRVMGR_NOTIFY_EXT:
			...
			SCI_TRACE_LOW("[MMIFM]PlayCallBack: report_ptr=%d",report_ptr->report);
...
{
    SCI_TRACE_LOW("MMIFM_FreePlaySrv: s_fm_play_handle=0x%x",s_fm_play_handle);
...
PUBLIC BOOLEAN MMIFM_ReqPlaySrv(void)
{
    ...
    else
    {
        ...
        audio_srv.info.type = MMISRVAUD_TYPE_FM;
...
PUBLIC BOOLEAN MMIFM_ReqRecSrv(MMISRVAUD_RECORD_FMT_E fmt, wchar *file_name)
{
    ...
    else
    {
        ...
        if(FM_HEADSET_MODE == s_fm_cur_info.sound_mode)
        {
...
{
    MMISRVAUD_SetVolume(MMIFM_GetPlayHandle(), (uint32)MMISET_VOL_ONE);
...
PUBLIC BOOLEAN MMIFM_AdjustVolumeTP(int32 step)
{
    ...
    MMIAPISET_SetMultimVolume(vol);
    SCI_TRACE_LOW("[MMIFM] MMIFM_AdjustVolume status %d",s_fm_cur_info.status);
...
PUBLIC BOOLEAN MMIFM_Play(BOOLEAN is_fm_first_open)
{
...
#endif
	{
		...
		FM_Open();
#ifdef FM_S_ANT_SUPPORT
		MMIFM_CheckAndSwitchFmAnt(FALSE, 0);
#endif
...
#ifdef FM_RDS_SUPPORT
					if(FM_RDS_STATUS_OPEN == MMIFM_GetRdsStatus() && (MMIFM_GetStatus() == FM_PLAYING))
					{
						MMIFM_RdsOpen();
					}     
#endif
...
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMIFM_EngTestPlay(const BOOLEAN is_auto)
{
	uint32  init_result = FM_SUCCESS;
	...
	if(FM_SUCCESS == init_result)
	{
		...
		if(MMIFM_ReqPlaySrv())
		{
			...
			// Bug 1374580, 进入工模FM test后，判断是手动搜台或自动搜台
			if (is_auto)
			{
				// 自动搜台, 由第一个搜到的电台播放
			...
			else
			{
				...
				if (fm_tune_step == 0)
				{
					//MMIAPIFM_Play(MMIFM_GetPlayHandle(),9900); //与工程模式中的默认频率相同
					init_result = FM_AutoSeek(MMIFM_FREQ_INITIAL_VAL, FM_SCAN_UP, FM_SCAN_ONE, &channel_info);
					SCI_TRACE_LOW("[MMIFM] MMIAPIFM_EngTestPlay FM_AutoSeek result %d, freq %d", init_result,channel_info.freq_count);
					MMIAPIFM_Play(MMIFM_GetPlayHandle(),channel_info.auto_freq_tab_ptr[0]);
				}
				else
				{
					//MMIAPIFM_Play(MMIFM_GetPlayHandle(),990); //与工程模式中的默认频率相同
					init_result = FM_AutoSeek(MMIFM_FREQ_INITIAL_VAL/10, FM_SCAN_UP, FM_SCAN_ONE, &channel_info);
					SCI_TRACE_LOW("[MMIFM] MMIAPIFM_EngTestPlay FM_AutoSeek result %d, freq %d", init_result,channel_info.freq_count);
					MMIAPIFM_Play(MMIFM_GetPlayHandle(),channel_info.auto_freq_tab_ptr[0]);
				}
			}
...
{
    BOOLEAN ret = FALSE;
    if(fre >= s_fm_freq_range_low && fre <= s_fm_freq_range_high)
    {
        ret = TRUE;
    }
    else
...
PUBLIC BOOLEAN MMIFM_AdjustFrequencyByFre(uint16 fre)
{
    ...
    if(s_fm_cur_info.cur_freq > s_fm_freq_range_high)
    {
       s_fm_cur_info.cur_freq = s_fm_freq_range_high;
    }
    else if(s_fm_cur_info.cur_freq < s_fm_freq_range_low)
    {
       s_fm_cur_info.cur_freq = s_fm_freq_range_low;
    }
...
#endif
    {
		...
		if(MMIFM_GetNextValidChannelByFreq(s_fm_cur_info.cur_freq,step,&new_channel_index))
...
#endif
            {
                ...
                {
                    if(channel.freqency > s_fm_freq_range_high || channel.freqency < s_fm_freq_range_low)
                    {
                        //invalid channel
...
//  SCI_TRACE_LOW:"[MMIFM] MMIFM_SwitchAudioDevice  status %d sound_mode "
    ...
    switch (s_fm_cur_info.sound_mode)
    {
        case FM_SPEAKER_MODE:
        {
            s_fm_cur_info.sound_mode = FM_HEADSET_MODE;
            support_route = MMISRVAUD_ROUTE_EARPHONE|MMISRVAUD_ROUTE_SPEAKER;
            MMIFM_UpdateIdleEarPhoneState(TRUE);
        }
        break;
        ...
        case FM_HEADSET_MODE:
        {
            s_fm_cur_info.sound_mode = FM_SPEAKER_MODE;
            support_route = MMISRVAUD_ROUTE_SPEAKER;
            MMIFM_UpdateIdleEarPhoneState(FALSE);
        }
        break;
...
PUBLIC uint16 MMIFM_SearchValidFreq(int32 direct)
{
	...
	if(direct >= 0)
	{
	   //search next
		dir = FM_SCAN_UP;
		if(s_start_freq >= s_fm_freq_range_high)
		{
			//current freqency is highest. freqency. must start from lowest
			s_start_freq = s_fm_freq_range_low - 10;
		}
	}
	else if(direct < 0)
	{
		//search previous 
		dir = FM_SCAN_DOWN;
		if(s_start_freq <= s_fm_freq_range_low)
		{
			 //current freqency is lowest, search from highest
			s_start_freq = s_fm_freq_range_high + 10;
		}
	}
	s_start_freq = MIN(s_fm_freq_range_high + 10, s_start_freq);
	s_start_freq = MAX(s_fm_freq_range_low - 10, s_start_freq);
	SCI_TRACE_LOW("[MMIFM] MMIFM_SearchValidFreq: s_start_freq=%d", s_start_freq);
	...
	if(FM_SUCCESS == ret)// FM_SCAN_ON
	{
		...
		if((channel_info.auto_freq_tab_ptr[0] <= s_fm_freq_range_high) && (channel_info.auto_freq_tab_ptr[0] >= s_fm_freq_range_low))
		{
			s_fm_cur_info.cur_freq = channel_info.auto_freq_tab_ptr[0];
			SaveCurrentFMInfo(&s_fm_cur_info);
		}
		SCI_TRACE_LOW("[MMIFM] MMIFM_SearchValidFreq: ch_index=%d, cur_freq=%d", ch_index, channel_info.auto_freq_tab_ptr[0]);
...
PUBLIC uint16 MMIFM_AuotSearchFreq(uint16 cur_search_channel)
{
	...
	static uint16			backup_channel = 0;
	static MMIFM_CHANNEL_T	one_channel = {0,MMIFM_FREQ_RANGE_LOW};
	...
	if (MMIFM_CheckHeadsetStatusWithoutPrompt())
	{
		...
		if (FM_SUCCESS == init_result)
		{
			SCI_TRACE_LOW("one_channel.freqency =%d\n",one_channel.freqency );
			...
			{
				one_channel.freqency = MMIFM_FREQ_RANGE_LOW - 10;
				backup_channel=0;
			}
			
			//if (cur_search_channel < MMIFM_MAX_CHANNEL_NUM)
			{
				if (one_channel.freqency > (MMIFM_FREQ_RANGE_HIGH + 10) || one_channel.freqency < (MMIFM_FREQ_RANGE_LOW - 10))
				{
				...
				{
					ret = FM_AutoSeek(one_channel.freqency, FM_SCAN_UP, FM_SCAN_ALL, &all_channel);
					...
					if (all_channel.auto_freq_tab_ptr[0])
					{
						//channel_info->freqency = one_channel.freqency;
						one_channel.freqency -= 10;
						MMIFM_SetOneChannel(backup_channel, &one_channel);
						one_channel.freqency += 10;
						backup_channel++;
					}
					one_channel.freqency += 10;
...
PUBLIC BOOLEAN MMIFM_GetOneChannel(uint16 channel_index, MMIFM_CHANNEL_T *channel_ptr)
{
    ...
    if(PNULL != channel_ptr)
    {
        ...
        {
            nv_result = MNNV_ReadItem(MMI_GetTrueNVItemId(item_id), sizeof(MMIFM_CHANNEL_T), (void *)(channel_ptr));/*lint !e64*/
        }

        if(channel_ptr->freqency > s_fm_freq_range_high || channel_ptr->freqency < s_fm_freq_range_low)
        {
...
LOCAL BOOLEAN  SaveCurrentFMInfo(const MMIFM_CUR_INFO_T *cur_info_ptr)
{
    ...
    {
        nv_result = MNNV_WriteItem(MMI_GetTrueNVItemId(MMIFM_NV_CUR_INFO), sizeof(MMIFM_CUR_INFO_T), (void *)(cur_info_ptr)); /*lint !e64*/
    }
    ret = (nv_result == MN_RETURN_SUCCESS ? TRUE : FALSE);
    return ret;
}
...
LOCAL BOOLEAN  ReadCurrentFMInfo(MMIFM_CUR_INFO_T *cur_info_ptr)
{
    ...
    if(PNULL != cur_info_ptr)
    {
        nv_result = MNNV_ReadItem(MMI_GetTrueNVItemId(MMIFM_NV_CUR_INFO), sizeof(MMIFM_CUR_INFO_T), (void *)(cur_info_ptr)); /*lint !e64*/
    }//MNNV_ReadItem
    ret = ((nv_result == MN_RETURN_SUCCESS) ? TRUE : FALSE);
    return ret;
}
...
PUBLIC BOOLEAN  MMIFM_IsChannelValid(const MMIFM_CHANNEL_T *channel_ptr)
{
    ...
    if(PNULL != channel_ptr)
    {
        if(channel_ptr->freqency >= s_fm_freq_range_low && channel_ptr->freqency <= s_fm_freq_range_high)
        {
            ret = TRUE;
        }
        else
...
/*****************************************************************************/
//  Description : get next available channel id
//  Global resource dependence : 
//  Author:
//  return 
//  Note: 
/*****************************************************************************/
PUBLIC uint16  MMIFM_GetNextAvailableChannelID(void)
{
    uint16  i  =0;
    ...
    for(i = 0; i < MMIFM_MAX_CHANNEL_NUM; i++)
    {
       ...
       {
            SCI_TRACE_LOW("[MMIFM] MMIFM_GetNextAvailableChannelID ret id %d",i);
            return i;           
       }
    }
    return -1;
...
LOCAL BOOLEAN WriteChannelToNv(uint16 channel_index, const MMIFM_CHANNEL_T *channel_ptr)
{
    ...
    {
        nv_result = MNNV_WriteItem(MMI_GetTrueNVItemId(item_id), sizeof(MMIFM_CHANNEL_T), (void *)(channel_ptr));/*lint !e64*/
    }
    ret = ((MN_RETURN_SUCCESS == nv_result) ? TRUE : FALSE);
    return ret;
}
...
#endif
    {   
        if(s_fm_freq_range_low <= input_freq && input_freq <= s_fm_freq_range_high)
        {
            if(!FM_IsInit())
            {
                init_result = FM_Init();
...
{    
    ...
    {
        MMIFM_EnableMute();
        if(s_fm_freq_range_low <= freq && freq <= s_fm_freq_range_high)
        {
            if(!FM_IsInit())
            {
                init_result = FM_Init();
...
LOCAL  void ShowStorageChangeAlert(MMIFILE_DEVICE_E    file_dev )
{
    SCI_TRACE_LOW("[MMIFMRECORD] StorageChangeAlert dev  %d",file_dev );
    if(MMI_DEVICE_UDISK == file_dev)
    {
        MMIPUB_OpenAlertWarningWin(TXT_SD_FULL_TO_UDISK);
    }
    else if(MMI_DEVICE_SDCARD == file_dev)
    {
        MMIPUB_OpenAlertWarningWin(TXT_UDISK_FULL_TO_SD);         
    }
}
/*****************************************************************************/
...
// #endif
    ...
    {   
        ...
        {
            ShowStorageChangeAlert(file_dev);
...
PUBLIC MMIFMRECORD_RESULT_E MMIFM_RecordStopRecord(void)
{
    ...
    //SCI_TRACE_LOW:"[MMIRECORD] enter funciton MMIFM_RecordStopRecord()"
...
{	
    ...
    { 
        nv_result = MNNV_ReadItem(MMI_GetTrueNVItemId(MMINV_FM_TIMER_RECORD_INFO), sizeof(MMIFM_TIMER_RECORD_INFO_T),(void *)(record_info_ptr));/*lint !e64*/
        if (MN_RETURN_SUCCESS != nv_result)
        {
            record_info_ptr->endure_time = 1;
            record_info_ptr->record_freq = 875;
            nv_result = MNNV_WriteItem(MMI_GetTrueNVItemId(MMINV_FM_TIMER_RECORD_INFO), sizeof(MMIFM_TIMER_RECORD_INFO_T),(void *)(record_info_ptr));/*lint !e64*/
            ret = (nv_result == MN_RETURN_SUCCESS ? TRUE : FALSE);
        }
    }
#endif
...
#endif
    {
        if(MMIFMRECORD_STATE_NONE == record_status)
        {
            ...
            else
            {
                if(!MMK_IsOpenWin(FM_MAIN_WIN_ID))
                {
                    MMIFM_OpenMainWinNoPlay();
                    SCI_TRACE_LOW("MMIFM MMIFM_StartFMTimerRecord open FM MAIN WIN");
                }
                MMIFM_StopAutoSearch();
    	        MMIFM_PlayFreq(record_info.record_freq);
                if(FALSE == MMIAPIFM_IsFmActive())
                {
                    SCI_TRACE_LOW("MMIFM MMIFM_StartFMTimerRecord FM is stoped");
...
PUBLIC void MMIFM_Reset(void)
{
        ...
        MNNV_WriteItem(MMI_GetTrueNVItemId(MMINV_FM_TIMER_RECORD_INFO), sizeof(MMIFM_TIMER_RECORD_INFO_T),&timer_record_info);/*lint !e64*/
#endif
#ifdef MMIFM_SUPPORT_FM_RECORD
...
{    
    ...
    if(!MMIFM_CheckGPIOStatusAndSetFmLNA())
    {
#ifndef FM_S_ANT_SUPPORT
        //not headset, can not play
        MMIPUB_OpenAlertWinByTextId(PNULL,TXT_FM_INPUT_HEADSET,TXT_NULL,/*IMAGE_PUBWIN_WARNING*/IMAGE_NULL,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,HandleHeadsetAlertWin);
#endif
        result = FALSE;
    }
    else
    {
        /*add by King.Wang 从耳机天线出*/
        return TRUE;
    }
#ifdef FM_S_ANT_SUPPORT
        /*add by King.Wang 从内置天线出*/
    return TRUE;
#endif
#endif
    return result;
