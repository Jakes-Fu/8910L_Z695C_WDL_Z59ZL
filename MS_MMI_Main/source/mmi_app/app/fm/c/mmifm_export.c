...
{
	MMIFM_EngTestStop();
	// Bug 1370182, 离开工模FM测试时，将FM应用状态设置为STOP
...
/*****************************************************************************/
//  Description : 判断fm是否playing,供外部使用
//  Global resource dependence : 
//  Author:juan.wu
//  Note: 
/*****************************************************************************/
...
PUBLIC void MMIAPIFM_ConvertFreqToString(const uint16 freq, char* freq_buf)
{
	...
	{
		/*
		FM通过此接口获取到的频率，会根据不同步长值会不一样。
		如果50khz 8750~10800
		如果100khz 875~1050

		所以需要调用接口，MMIFM_GetFmTuneStep（）来判断下。
		此接口返回值（0,1） 0 对应 8750~10800  1 对应875~1050
		*/
