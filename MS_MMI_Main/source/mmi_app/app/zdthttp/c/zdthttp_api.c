...
LOCAL void MMIAPICOM_BcdICCIDToStr
(
    uint8*    pBcd,    // BCD码的头指针
...
LOCAL uint32 GetCurrentTimeStamp(void)
{
    SCI_TM_T sys_time = {0};
    uint32   second = 0;
    TM_GetTime(&sys_time);
...
LOCAL uint8* GetCurrentTimeStampString(void)
{
    int index = 0;
    uint8 *time;
...
LOCAL uint8* WATCHCOM_GetDeviceModel()
{
    #ifdef ZTE_WATCH
    return ZDT_SFR_TYPE;
    #else
...
LOCAL uint8* WATCHCOM_GetSoftwareVersion()
{
    #ifdef ZTE_WATCH
