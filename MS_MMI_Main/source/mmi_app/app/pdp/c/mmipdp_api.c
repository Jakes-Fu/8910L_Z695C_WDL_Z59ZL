            ...
            {   
                    ...
                    MN_GPRS_ERR_RETRYING == ((APP_MN_GPRS_EXT_T*)msg_ptr->msg_body)->result)
                {
...

                    {
                        ...
                        {
                            SetLinkReqChapForPapOrChap(fsm_ptr->link_id, TRUE);
                            MMIPDP_PsActivePdp(fsm_ptr->link_id);
                            break;
                        }
                        else
                        {
                            if (MMIPDP_REACTIVE_MAX_COUNT > g_mmipdp_manager_instance.link_arr[fsm_ptr->link_id-1].reactive_count)
                            {
                    ...
                    else if(ReActive(fsm_ptr->link_id))
                    {
                        break;
                    }
                }
