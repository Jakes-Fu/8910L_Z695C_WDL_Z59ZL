...
//  Note: 
/*****************************************************************************/
extern uint8  gprs_switch_settings_fix_nv;
PUBLIC void MMIAPICONNECTION_ResetFactorySetting(BOOLEAN is_clean_user_data)
{
    ...
    MMIAPICONNECTION_SetGPRSSwitchStatus(gprs_switch_settings_fix_nv?(MMICONNECTION_SETTING_GPRS_SWITCH_ON):(MMICONNECTION_SETTING_GPRS_SWITCH_OFF));
#endif
...
PUBLIC MMICONNECTION_SETTING_DATA_ROAMING_E MMAPIICONNECTION_Setting_GetDataRoamingStatus(void)
{
    ...
    {
        is_support_domestic_roaming = MMICONNECTION_IsSupportDomesticRoaming();
        if(TRUE == is_support_domestic_roaming)
        {
            data_roaming_status = MMICONNECTION_SETTING_DATA_ROAMING_DOMESTIC_ENABLE;
        }
        else if(0xFF == is_support_domestic_roaming)
        {
...
/*****************************************************************************/
// 	Description : set data domestic roaming default setting status
//	Global resource dependence : none
//  Author: cheney.wu
//	Note:
/*****************************************************************************/
PUBLIC void MMAPIICONNECTION_Setting_SetDataRoamingDefaultStatus(void)
{
    MMICONNECTION_SETTING_DATA_ROAMING_E data_roaming_status = MMICONNECTION_SETTING_DATA_ROAMING_DISABLE;
    MN_RETURN_RESULT_E return_value = MN_RETURN_FAILURE;
...
//	Note:
/*****************************************************************************/
PUBLIC void MMIAPICONNECTION_Setting_SetDataRoamingStatus(MMICONNECTION_SETTING_DATA_ROAMING_E data_roaming_status)
{

    if (data_roaming_status < MMICONNECTION_SETTING_DATA_ROAMING_MAX)
    {
        MMINV_WRITE(MMINV_CONNECTION_DATA_ROAMING, &data_roaming_status);
    }
}
