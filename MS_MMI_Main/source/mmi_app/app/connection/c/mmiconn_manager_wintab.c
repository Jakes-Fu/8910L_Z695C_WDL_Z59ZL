...
#ifdef MMI_GPRS_SUPPORT
/**************************************************************************************************************/
//  Description : 此接口是判断能否进入网络帐户及当前几张卡可以进入,判断条件为:
//  若飞行模式，无卡，卡关闭，puk 锁,pin锁卡reject，卡不支持都不会进入网络帐户
//  Param in: ok_idx: 若当前ok_num 是1，则ok_idx里面存储哪张卡是ok的，其他情况为无效值
//                  ok_num:当前多少张卡为ok 的
//  Note: ok_idx只有在ok_num 为1的时候是有效的其他情况无效。
/**************************************************************************************************************/
                                            ...
                                            )
{
    ...
    case MSG_APP_WEB:
        ...
        MMIAPICOM_WstrToStr(string_info.wstr_ptr, (uint8 *)tempbuf_byte);//只允许输入ASCII，可以用这个函数
