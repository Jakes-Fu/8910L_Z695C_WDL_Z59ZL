...
#else
#define MMICONNECTION_MAX_SETTING_NUM			    15             //设置的最大数量15,  change 8->15
#endif
#define MMICONNECTION_MAX_LINKMODENAME_LEN		    32			//连接方式设定名的最大长度2008.1.10 modify
...
#define MMICONNECTION_MAX_APN_LEN				        32        //接入点的最大长度102
...
#endif
#define MMICONNECTION_MAX_DNS_LEN                               39               //备选dns字串的长度
...
{
    uint8 mvno_type;//0~5
    char mvno_value[32]; //MVNO VALUE  最大
    uint16 wstr_len;
}MMICONNECTION_MVNOINFO_T;
typedef struct
{
    MMICONNECTION_MVNOINFO_T mvno_info[MMI_DUAL_SYS_MAX];
}MMICONNECTION_MVNOINDEX_T;
...
typedef enum _MMICONNECTION_SETTING_DATA_ROAMING_E
{
    ...
    MMICONNECTION_SETTING_DATA_ROAMING_DOMESTIC_ENABLE,
