...
#include "IN_message.h" //modify 20220807
...
BOOLEAN MMIHEROAPP_checkStorage(uint8 grade);
#else
...
PUBLIC BOOLEAN MMIHEROAPP_AppEntry(HERO_APP_APPENTRY_E index)
{
	...
	sprintf(gHeroAppEntryTempBuf, "-s 0 -h %d -i 920009 -n tosbrw.too", hr_get_smainm_title_buf(TXT_HERO_APP_BROWSER));
	...
	sprintf(gHeroAppEntryTempBuf, "-s 0 -h %d -i 920008 -n tchbrw.too", hr_get_smainm_title_buf(TXT_HERO_APP_BROWSER));
	recode = MMIHEROAPP_HrAppRun((void*)gHeroAppEntryTempBuf, index); 
	}
	#endif
    	break;
	...
	sprintf(gHeroAppEntryTempBuf, "-s 0 -h %d -i 920037 -n tvnews.too", hr_get_smainm_title_buf(TXT_HERO_APP_NEWS));
	recode = MMIHEROAPP_HrAppRun((void*)gHeroAppEntryTempBuf, index); 
	}
    	break;
#endif
	...
	sprintf(gHeroAppEntryTempBuf, "-s 0 -h %d -i 920057 -n tsmart.too", hr_get_smainm_title_buf(TXT_HERO_APP_SMART));
	recode = MMIHEROAPP_HrAppRun((void*)gHeroAppEntryTempBuf, index); 
#else
#if !defined(HERO_ENGINE_GPRS_TIP_SUPPORT)
	if(MMIHEROAPP_checkStorage(0))
	...
	case HR_APPID_FAMILY:
		...
		//recode = MMIHEROAPP_HrAppRun((void*)"-s 0 -t Family -i 920300 -n tfamily.too -p 101", index);
	       sprintf(gHeroAppEntryTempBuf, "-s 0 -h %d -i 920300 -n tfamily.too -p 101", hr_get_smainm_title_buf(TXT_HERO_APP_FAMILY));
	       recode = MMIHEROAPP_HrAppRun((void*)gHeroAppEntryTempBuf, index); 		
	#else
#if !defined(HERO_ENGINE_GPRS_TIP_SUPPORT)
		if(MMIHEROAPP_checkStorage(0))
	...
	case HERO_APP_TREADER:
		...
		//recode = MMIHEROAPP_HrAppRun((void*)"-s 0 -t Reader -i 920029 -n treader.too", index);		
	       sprintf(gHeroAppEntryTempBuf, "-s 0 -h %d -i 920029 -n treader.too", hr_get_smainm_title_buf(TXT_HERO_APP_READER));
	       recode = MMIHEROAPP_HrAppRun((void*)gHeroAppEntryTempBuf, index); 	
		}
		break;
#endif
	...
	case HERO_APP_TWEATHER:
		...
		//recode = MMIHEROAPP_HrAppRun((void*)"-s 0 -t Weather -i 920044 -n tweath.too -p 1", index);			
	       sprintf(gHeroAppEntryTempBuf, "-s 0 -h %d -i 920044 -n tweath.too  -p 1", hr_get_smainm_title_buf(TXT_HERO_APP_WEATHER));
	       recode = MMIHEROAPP_HrAppRun((void*)gHeroAppEntryTempBuf, index); 	
		}
		break;
#endif
	...
	case HERO_APP_TIME:
		...
		//recode = MMIHEROAPP_HrAppRun((void*)"-s 0 -t smainm -i 400002 -n smainm.too -p 1", index);		
	       sprintf(gHeroAppEntryTempBuf, "-s 0 -h %d -i 400002 -n smainm.too -p 1", hr_get_smainm_title_buf(TXT_HERO_APP_TIME));
	       recode = MMIHEROAPP_HrAppRun((void*)gHeroAppEntryTempBuf, index); 	
		}
		break;
#endif
	...
	case HERO_APP_HMGS:
		...
		sprintf(gHeroAppEntryTempBuf, "-s 0 -h %d -i 930008 -n tcall5.too", hr_get_smainm_title_buf(TXT_HERO_APP_HMGS));
		recode = MMIHEROAPP_HrAppRun((void*)gHeroAppEntryTempBuf, index); 
#else
		if(MMIHEROAPP_checkStorage(0))
		{
	...
	case HERO_APP_FM:
		...
		//recode = MMIHEROAPP_HrAppRun((void*)"-s 0 -t tfm -i 920032 -n tfm.too -p 1", index);		
	       sprintf(gHeroAppEntryTempBuf, "-s 0 -h %d -i 920032 -n tfm.too", hr_get_smainm_title_buf(TXT_HERO_APP_FM));
	       recode = MMIHEROAPP_HrAppRun((void*)gHeroAppEntryTempBuf, index); 	
		}
		break;
#endif
...
#endif
{
    ...
    {
    		MMIPUB_OpenAlertWinByTextId(NULL,STR_SIM_NOT_SIM_EXT01,TXT_NULL,IMAGE_PUBWIN_WARNING,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,NULL);	
		return FALSE;
    }
#if 0
...
#ifdef DUAL_TCARD_SUPPORT
	||(MMIAPIUDISK_GetDeviceStatus(MMIAPIFMM_GetDevicePath(MMI_DEVICE_SDCARD_1), MMIAPIFMM_GetDevicePathLen(MMI_DEVICE_SDCARD_1)) != FALSE)
#endif
   )
	{		
...
typedef struct tagHeroAsynHostNameModuleSigSt {
	SIGNAL_VARS
	void *parm1; void *parm2;
	void *parm3; void *parm4;
} HeroAsynHostNameModuleSigSt;

MMI_APPLICATION_T gHeroEngineAppModlue;

void hero_aysn_host_ui_modle_callback(void *parm1, void *parm2, void *parm3, void *parm4);

static MMI_RESULT_E handle_hero_engine_module_pp_msg(PWND app_ptr, uint16 msg_id, DPARAM param)
{
    MMI_RESULT_E ret = MMI_RESULT_TRUE;
	if(msg_id >= HERO_ENGINE_SIGNAL_BEG && msg_id <= HERO_ENGINE_SIGNAL_END)
	{
		HeroAsynHostNameModuleSigSt *pp_getdata = (HeroAsynHostNameModuleSigSt*)(param);
		if(pp_getdata != NULL)
		{
			switch(msg_id)
			{
				case HERO_SIGNAL_ASYN_HOSTNAME:
					hero_aysn_host_ui_modle_callback(pp_getdata->parm1, pp_getdata->parm2, pp_getdata->parm3, pp_getdata->parm4);
					break;
			}
		}
	}
...
{
	HeroAsynHostNameModuleSigSt *psig = PNULL;

	MMI_CREATE_SIGNAL((xSignalHeaderRec*)psig, msgId, sizeof(HeroAsynHostNameModuleSigSt), SCI_IdentifyThread());

	psig->parm1 = parm1; psig->parm2 = parm2; psig->parm3 = parm3; psig->parm4 = parm4;
...
{
	gHeroEngineAppModlue.ProcessMsg = handle_hero_engine_module_pp_msg;
