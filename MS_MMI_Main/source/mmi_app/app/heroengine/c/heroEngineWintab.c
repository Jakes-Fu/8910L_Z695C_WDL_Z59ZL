...
#if defined(HERO_ENGINE_GPRS_TIP_SUPPORT)
LOCAL MMI_RESULT_E HandleHeroShowGPRSTipWinMsg( 
											MMI_WIN_ID_T win_id, 
											MMI_MESSAGE_ID_E msg_id, 
											DPARAM param
											);
...
#if defined(HERO_ENGINE_GPRS_TIP_SUPPORT)
WINDOW_TABLE(MMIHEROAPP_SHOWGPRSTIP_TEXT_WIN_TAB) = 
{
    WIN_FUNC((uint32)HandleHeroShowGPRSTipWinMsg),
    WIN_ID(HERO_APP_SHOWGPRSTIP_TEXT_WIN_ID),
    #if !defined(PLATFORM_UIX8910)	
    WIN_TITLE(TXT_HERO_SHOW_GPRS_TIP),
    #endif
    CREATE_TEXT_CTRL(HERO_APP_SHOWGPRSTIP_TEXT_CTRL_ID),
    //WIN_SOFTKEY(STXT_OK, TXT_NULL, STXT_RETURN),
    WIN_SOFTKEY(STXT_SOFTKEY_OK_MK, TXT_NULL, STXT_RETURN),
        END_WIN
};
...
int32 MMIHEROAPP_createADynWin(MMI_HERO_APP_ADD_PARAMETERS_T *addData, MMI_STRING_T *textInfo)
{
	...
	if(win_handle != 0)
	{
		...
		MMIHR_Trace(("hero. MMIHEROAPP_createADynWin() winType=%d", winType));
		switch(winType)
		{
		case HERO_APP_WINTYPE_TEXT:
			{
		...
		case HERO_APP_WINTYPE_EDITOR:
			{
				...
				case HR_EDIT_ANY:
                    GUIWIN_CreateTitleDyna(addData->winId, TXT_NULL);
                    MMIHEROAPP_CreatSoftkeyCtrl(addData->winId, STXT_SOFTKEY_OK_MK, TXT_NULL, STXT_RETURN);
...
LOCAL void HeroShowFactoryInfo(void)
{
    ...
    //templen=hr_get_local_version(temp);
    hr_setting_get_handsetID_ex(temp, sizeof(temp), NULL, NULL, NULL);

    memset(temp_data, 0, sizeof(temp_data));
#ifdef __HERO_CFG_FEATURE_HANDSETID_EX__
	sprintf((char*)temp_data, "FactoryId:%s\nHandsetId:%s\nExt HandsetId:%s\nimsi:%s\nimei:%s\nRAM:%d\nbuildtime:%s", usrinfo.manufactory, usrinfo.type, temp, usrinfo.IMSI, usrinfo.IMEI, SCI_GetSystemSpaceMaxAvalidSpace(),VERSION_GetInfo(BUILD_TIME));
#else
        sprintf((char*)temp_data, "FactoryId:%s\nHandsetId:%s\nimsi:%s\nimei:%s\nbuildtime:%s", usrinfo.manufactory, usrinfo.type, usrinfo.IMSI, usrinfo.IMEI, VERSION_GetInfo(BUILD_TIME));
#endif
											...
											)
{
    ...
    switch (msg_id) 
    {
        case MSG_OPEN_WINDOW:
	 case MSG_GET_FOCUS:
		    MMIAPICOM_StatusAreaSetViewHeaderinfoByTextId(TXT_HERO_SHOW_GPRS_TIP);
            break;
        ...
        case MSG_FULL_PAINT:
        {
            GUIRES_DisplayImg(PNULL, &rect, &rect, win_id, IMAGE_COMMON_BG, &lcd_dev_info);
            #if !defined(PLATFORM_UIX8910)	
            GUIWIN_SetTitleTextId(win_id, TXT_HERO_SHOW_GPRS_TIP, FALSE);
	     #endif		
         
            MMI_GetLabelTextByLang(TXT_HERO_SHOW_GPRS_TIP, &text);
            GUITEXT_SetString(HERO_APP_SHOWGPRSTIP_TEXT_CTRL_ID, text.wstr_ptr,text.wstr_len, TRUE);
            MMK_SetAtvCtrl(win_id, HERO_APP_SHOWGPRSTIP_TEXT_CTRL_ID);
	...
	case MSG_APP_WEB:
        case MSG_NOTIFY_MIDSK:
			MMK_CloseWin(win_id);
            MMIHEROAPP_AppEntry(g_hero_appid);
            break;
...
PUBLIC BOOLEAN MMIHERO_ShowGPRSTipWin(void)
{
    return MMK_CreateWin((uint32*)MMIHEROAPP_SHOWGPRSTIP_TEXT_WIN_TAB,PNULL);
}
#endif											
//modify 20181114 begin
