...
void hero_tts_SettingsDefault(hero_tts_setting_struct* pTtsSetting)
{
...
#if defined(HERO_ENGINE_TTS_AKEY_TIME_OFF)
	pTtsSetting->cur_akey_time_read_status = 0;
#else
	pTtsSetting->cur_akey_time_read_status = 1;
#endif	

#if defined(HERO_ENGINE_TTS_AUTO_TIME_DEFAULT_OFF)
	pTtsSetting->cur_auto_time_read_status = 0;
	pTtsSetting->cur_auto_time_quantum_mode = 0;
#else
...
#if  1//defined(HERO_ENGINE_TTS_INCOMING_READ_ONE_TIMES)
	pTtsSetting->cur_incoming_read_status = INCOMING_BROADCAST_1; // 1.播报一次；2.播报五次；3.重复播报；4.关闭播报
#else
    ...
    #if 1//defined(HERO_ENGINE_TTS_LOW_POWER_SUPPORT)
	pTtsSetting->cur_low_power_read_status = 1;
    #else
	pTtsSetting->cur_low_power_read_status = 0;
    #endif
    #if defined(FLIP_PHONE_SUPPORT)
	pTtsSetting->cur_lock_screen_read_status = 0;
    #else	
	pTtsSetting->cur_lock_screen_read_status = 1;
    #endif	
    #if defined(HERO_ENGINE_TTS_AKEY_POWER_SUPPORT)
	pTtsSetting->cur_akey_power_level_read_status = 1;
    #else
	pTtsSetting->cur_akey_power_level_read_status = 0;
    #endif
...
int32 hero_app_get_tts_menu_switch_status(uint8* input, int32 input_len, uint8** output, int32* output_len)
{
...
#if 1 /*start modify by matthew for fix svn #2964 problem:解决一开机快速进入语音王菜单重启问题; */
	{
		static uint8 flag=1; 
...
#if  defined(__TIHO_TTS_REMOVE_TIME_DESCRIPTION__)
	//gHeroTtsMenuStatus.speakTimeSwitch.params1 = 99;
    key_time = 99;
#elif defined(HERO_ENGINE_TTS_KEY_7_TIME)
	key_time = HR_KEY_7;
#elif defined(HERO_ENGINE_TTS_KEY_8_TIME)
	key_time = HR_KEY_8;
#elif defined(HERO_ENGINE_TTS_KEY_9_TIME)
	key_time = HR_KEY_9;
	...
	gHeroTtsMenuStatus.masterSwitch.status = 1;
#endif
...
#if defined(HERO_ENGINE_TTS_AKEY_TIME_OFF)
	gHeroTtsMenuStatus.speakTimeSwitch.isShow = 0;
	gHeroTtsMenuStatus.speakTimeSwitch.status = 0;
#else
...
#else
	gHeroTtsMenuStatus.lowPowerSwitch.isShow = 0;
	gHeroTtsMenuStatus.lowPowerSwitch.status = 0;
#endif
#if defined(FLIP_PHONE_SUPPORT)
	gHeroTtsMenuStatus.broadcastLockSwitch.isShow = 0;
	gHeroTtsMenuStatus.broadcastLockSwitch.status = 0;
#else
...
#else
	gHeroTtsMenuStatus.akeyPowerSwitch.isShow = 0;
	gHeroTtsMenuStatus.akeyPowerSwitch.status = 0;
	gHeroTtsMenuStatus.akeyPowerSwitch.params1 = HR_KEY_STAR;
#endif
...
void hero_tts_SettingsReset(void)
{
	hero_tts_setting_struct stSetting = {0};
	hero_tts_SettingsDefault(&g_herotts_setting);//stSetting
	hero_tts_RestoreSettings(&g_herotts_setting);//stSetting
	hero_tts_apps_init();
}
...
#elif defined(HERO_ENGINE_TTS_KEY_9_TIME)
	g_hero_dialog_content_id = TXT_HERO_TTS_SET_9_AKEY_TIME_HELP;
#elif defined(HERO_ENGINE_TTS_KEY_0_TIME)
...
{
	g_hero_dialog_content_id = TXT_HERO_TTS_SET_STAR_AKEY_POWER_LEVEL_HELP;
...
void hero_tts_auto_time_Info(void)
{
	
		g_hero_dialog_content_id = TXT_HERO_TTS_SET_AUTO_TIME_HELP_INFO;
		g_hero_dialog_title_id = TXT_HERO_TTS_SET_AUTO_TIME_INFO;
		MMK_CreateWin((uint32*)HERO_DIALOG_WIN_TAB, PNULL);
}
        ...
        case MSG_OPEN_WINDOW:
            {
                ...
                MMI_STRING_T  title = {0};
                int i = 0;
                int str_list[6] = {TXT_HERO_TTS_SET_AUTO_TIME_MODE_DAY, TXT_HERO_TTS_SET_AUTO_TIME_MODE_NIGHT,
				                   TXT_HERO_TTS_SET_AUTO_TIME_MODE_WHOLE_DAY, TXT_HERO_TTS_SET_AUTO_TIME_MODE_CUSTOM,TXT_HERO_TTS_SET_AUTO_TIME_MODE_NIGHT};
                int j, n;
        ...
        case MSG_KEYDOWN_MIDDLE:
            {
				...
				else  if(cur_selection == 4)
				{
					hero_tts_auto_time_Info();
				}
				else
				{
