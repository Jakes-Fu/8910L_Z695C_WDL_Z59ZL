 ...
 **--------------------------------------------------------------------------*/
//clark add 20220817
static BOOLEAN g_hero_tts_incall =FALSE;
//clark add 20220817
#ifdef __cplusplus
    extern   "C"
    {
...
#if 1//def MMI_IDLE_SHORTCUT_SUPPORT	
...
{
#ifdef HERO_ENGINE_TTS_ALL_SUPPORT
...
PUBLIC BOOLEAN TTSAPI_PlayText(MMITTS_PLAY_PARAM_T *play_param_ptr)
{
	...
	{
		g_hero_tts_incall = TRUE;//clark add 20220817
	...
	if(TTS_RING_TYPE_NEW_SMS == play_param_ptr->ring_type ){
...
BOOLEAN hero_tts_lock_broadcast_is_open(void)
{
	BOOLEAN ret = FALSE;
	if(hero_tts_need_broadcast(TTS_RING_TYPE_LOCK_SCREEN) && hero_tts_switch_PreCheck(TTS_RING_TYPE_LOCK_SCREEN)) {
		ret = TRUE;
	}
	return ret;
}
...
static void hero_tts_date_to_12_hour_only_time_string(wchar* output, SCI_DATE_T*	pdate, SCI_TIME_T  * ptime, BOOLEAN isMin)
{
	uint8 temp_buff[32] = {0};
	uint8 temp_buff_w[64] = {0};
	uint8 myHour = 0;
	MMISET_TIME_DISPLAY_TYPE_E  time_display_type   = MMIAPISET_GetTimeDisplayType();  
	uint8 u8AmPm = 0;//0 上午 1 下午
	
	hero_tts_Wstrcpy(output, (const wchar*)"\xB0\x73\x28\x57\x2F\x66\x17\x53\xAC\x4E\xF6\x65\xF4\x95\x2C\x00\x2C\x00\x00\x00\x00\x00");
	...
	{
		if ((MMISET_TIME_PM_HOUR > ptime->hour))//AM
		{
			// 上午
			MMIAPICOM_Wstrcat(output, "\x0a\x4e\x48\x53\x00\x00");
			hero_tts_Wstrcat(output, (const wchar*)"\x2c\x00\x00\x00");
		}
		else//PM
		{
			// 下午
			MMIAPICOM_Wstrcat(output, "\x0b\x4e\x48\x53\x00\x00");
			hero_tts_Wstrcat(output, (const wchar*)"\x2c\x00\x00\x00");
		}
	}
	...
	if((ptime->hour > 12)&&(time_display_type == MMISET_TIME_12HOURS))
	{
...
void hero_tts_auto_time_broadcast(void)
{
		...
		&& TM_GetSysTime(&time) == ERR_TM_NONE)
	{
		do
		{
		 ...
		 #if defined(HERO_ENGINE_TTS_TIME_BROADCAST_NO_WEEK_LUNAR)
	       hero_tts_date_to_12_hour_only_time_string(text_buff_w, &date, &time, 1);
	#else
...
PUBLIC void  hero_tts_akeytime_To_Speech(void)
{
    wchar temp_wchar[10+10] = {0};
    wchar  play_text[10 + 30] = {0};
    uint16 play_len = 0;
 ...
 //   MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_MENU);
...
#if defined(ZDT_PRODUCT_W2_DZ)||defined(BKIX_ZHT_G16)
 	MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_CUR_TIME,&current_name_str);
 #else
	...
	hero_tts_send_text(play_text, MMIAPICOM_Wstrlen(play_text), TTS_RING_TYPE_AKEY_TIME);
...
static MMI_TEXT_ID_T hero_tts_get_text_id(HERO_TTS_TEXT_ID_E text_id_type)
{
	...
	switch(text_id_type)
	{
	    ...
	    case HERO_TTS_TEXT_ID_POWER_LEVEL_TEXT:
	    	...
	    	break;
	#if 1 //defined(MMI_POP_ONLY_CHARGE_NOTICE)
...
//clark add 20220817
BOOLEAN g_hero_tts_incall;
BOOLEAN hero_tts_incall_playing(void)
{
	return g_hero_tts_incall;
}
void hero_tts_incall_playing_stop(BOOLEAN parameters)
{
	g_hero_tts_incall = parameters;
}
//clark add 20220817
/**--------------------------------------------------------------------------*
 **                         Compiler Flag                                    *
