                          ...
                          )
{
	...
	if(lineno>=uni_show_txt_ptr->total_line_num)
	{
		return FALSE;
	}
    
    item_length = uni_show_txt_ptr->line_offset_tab_ptr[lineno + 1] - uni_show_txt_ptr->line_offset_tab_ptr[lineno];
                    ...
                    )//返回失败说明有结束符
{
    ...
    uint16          data_str_len = MMIEBOOK_SEG_BUF_UNIT_LEN;
   ...
   if((orig_file_offset == 0) && ((GUI_CHAR_CODE_UCS2_BE == s_ebook_file_type) || (GUI_CHAR_CODE_UCS2_LE == s_ebook_file_type)))
    ...
    //读取一段txt原始内容
    data_str_len = MMIEBOOK_FFS_fread(data_str_ptr, read_len, 1,orig_file_handle );
                    ...
                    )//返回失败说明有结束符
{
    ...
    result = LayoutSegmentLineInfo(//段做layout断行
         ...
         cur_show_txt_ptr->this_seg_offset,
         g_ebook_show_file_brief_ptr->file_size,
         FALSE,
         FALSE
         );
     ...
     first_delimeter_offset = FindFirstParagraphSperator(
        ...
        SCI_MEMSET(data_str_ptr, 0 , data_str_len + 1);
         ...
         offset = GUI_WstrToOtherCode(
             ...
             data_str_len, 
             cur_show_txt_ptr->ucs2_str_ptr, 
             cur_show_txt_ptr->ucs2_str_len//该行的结束位置
             );      
    ...
    if(0 != s_ebook_show_info.pre_show_txt_ptr->this_seg_offset)
    {
        ...
        if(first_delimeter_offset != 0)
        {
            ...
            offset = GUI_WstrToOtherCode(
                ...
                data_str_len, 
                s_ebook_show_info.pre_show_txt_ptr->ucs2_str_ptr, 
                MIN(first_delimeter_offset, MMIEBOOK_SEG_BUF_UNIT_LEN)//该行的结束位置
                );      
            //上一次只用了first_delimeter_offset的长度
            SCI_MEMSET(data_str_ptr, 0, first_delimeter_offset+1);
...
LOCAL BOOLEAN ReMakeSegmentAfterScreenSwitch( void )
{
    ...
    offset = GUI_WstrToOtherCode(
        ...
        MIN(cur_line_offset, MMIEBOOK_SEG_BUF_UNIT_LEN)//该行的结束位置
        );      
                           ...
                           )
{
    ...
    else
    {
        ...
        offset = MMIEBOOK_ConvertWstrOffsetToOtherCodeOffset(
            s_ebook_show_info.cur_show_txt_ptr->ucs2_str_ptr,
            s_ebook_show_info.cur_show_txt_ptr->line_offset_tab_ptr[line]
            );
...
LOCAL void InitEbookShowRectInfo( MMI_WIN_ID_T win_id )
{
	...
	if(MMIEBOOK_GetIsFullScreenOff())
	{
        ...
        s_show_rect_info.txt_show_bottom = client_rect.bottom - MMIEBOOK_COUNT_BAR_HEIGHT;
	}
	else
	{
    client_rect = MMITHEME_GetFullScreenRect();
    s_show_rect_info.txt_show_bottom = client_rect.bottom ;
	}

    s_show_rect_info.txt_show_left = 0;
    s_show_rect_info.txt_show_top = 0;
    s_show_rect_info.txt_show_right = client_rect.right;
...
PUBLIC BOOLEAN MMIEBOOK_AddBookmark( void )
{
    ...
    offset = GUI_WstrToOtherCode(
        ...
        ebook_seg_ptr->line_offset_tab_ptr[s_ebook_show_info.cur_line]
        );
		...
		else
		{
            ...
            if(msg==MMIEBOOK_MSG_TURNTO_OFFSET)
            {
                ...
                offset = GUI_WstrToOtherCode(
                    ...
                    data_str_len, 
                    s_ebook_show_info.cur_show_txt_ptr->ucs2_str_ptr, 
                    MIN(s_ebook_show_info.cur_show_txt_ptr->line_offset_tab_ptr[1], MMIEBOOK_SEG_BUF_UNIT_LEN)//该行的结束位置
                    ); 
...
LOCAL void MakeBookmarkNameTab(void)
{
    ...
    {       
        ...
        ucs2_str_len = ReadOneLineUCS2(
                                    ...
                                    s_ebook_show_info.bookmark_info.offset_tab[i],
