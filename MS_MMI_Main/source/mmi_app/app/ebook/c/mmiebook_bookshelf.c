...
LOCAL BOOLEAN InitFileBriefFromLatest( uint16 cur_selection )
{
    ...
    {
        //判断该文件是否还存在？
        if (MMIEBOOK_IsFileExist(s_ebook_latest_list_ptr->item_brief[cur_selection].full_name_wstr,s_ebook_latest_list_ptr->item_brief[cur_selection].full_name_len,s_ebook_latest_list_ptr->item_brief[cur_selection].file_size))
        {
