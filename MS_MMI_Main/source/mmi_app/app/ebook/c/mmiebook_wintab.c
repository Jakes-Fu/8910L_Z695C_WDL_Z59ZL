...
LOCAL BOOLEAN  WriteEbookBookMarkInfo(BOOLEAN is_open_alert_win)
{
        ...
        &&PNULL!= ebook_show_file_brief_ptr)
    {
        offset = MMIEBOOK_ConvertWstrOffsetToOtherCodeOffset(
            ...
            show_info->cur_show_txt_ptr->line_offset_tab_ptr[show_info->cur_line]
            );
...
//    SCI_ASSERT(PNULL != g_ebook_show_file_detail_ptr);/*assert verified*/
    ...
    //case MSG_APP_MENU:
        ...
        if(input_str.wstr_len > 0)
        {
            ...
            {
                MMIEBOOK_SetPercent(page_input);
            }
     ...
     )
{
     ...
     MMIAPISET_AppendListItemByTextId( TXT_CODING_TYPE_GB, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO );
     MMIAPISET_AppendListItemByTextId( TXT_CODING_TYPE_BIG5, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO );
...
PUBLIC void MMIEBOOK_OpenExplorerWinforEbook( BOOLEAN is_has_read_ebook,BOOLEAN is_need_privacy_protection ) 
{
    ...
    openwin_info.privacy_protect = !is_need_privacy_protection;
     openwin_info.is_goto_cur_path = TRUE;//goto current child folder
...
{  
     ...
     s_ebook_fontsize_id = MMIEBOOK_FONT_BIG ;//MMIEBOOK_FONT_NORMAL;
     SetEbookFontSize(s_ebook_fontsize_id);
                            ...
                            )
{
    ...
    else
    {
        ...
        if(MMIAPIDRM_IsDRMFile(SFS_INVALID_HANDLE, full_path_ptr))
        {
            ...
            file_handle = MMIAPIFMM_CreateFile(full_path_ptr, 
...
PUBLIC MMIEBOOK_FONT_SIZE_E MMIAPIEBOOK_GetFontSize( void )
{
    ...
    if (MN_RETURN_SUCCESS != return_value)
    {
         font_size =MMIEBOOK_FONT_BIG ;// MMIEBOOK_FONT_NORMAL;
