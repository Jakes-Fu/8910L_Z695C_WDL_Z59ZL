...
typedef struct
{
    ...
    int16 x3;                   /* (x3,y3):起始位置；(x4,y4):结束位置*/
    int16 y3;                   
    int16 x4;
    int16 y4;
...
typedef struct
{
    ...
    uint16 *buff_bg;
    //uint16 *buff_move; //UNISOC_MMI_Delete
    uint16 *buff_old;
    uint16 *buff_new;
...
LOCAL void EffectPlaySlide(MMI_POPUP_MENU_EFFECT_PARAM_T *param)
{
    ...
    uint16 *temp_dst_ptr3  = PNULL;
    uint16 *temp_src_ptr3  = PNULL;

    uint16 buf_width3 = 0;
    uint16 buf_height3 = 0;

    int16 step_x3 = 0;
    int16 step_y3 = 0;

    int16  dst_start_y3 = 0;
    int16  src_start_y3 = 0;

    int16  copy_height3 = 0;
    
    int16  dst_start_x3 = 0;
    int16  src_start_x3 = 0;
    uint16 copy_width3 = 0;
    ...
    buf_width3 = param->buff_wid3;
    buf_height3 = param->buff_height3;

    move_x = param->x1;
    move_y = param->y1;

    buf_width = param->buff_wid;
    buf_height = param->buff_height;
    ...
    if(PNULL != param->buff_bg)
    {
        step_y3 = (int16)(param->y4 - param->y3)/frame_num;
        step_x3 = (int16)(param->x4 - param->x3)/frame_num;
    }
    ...
    for ( idx = 1; idx < frame_num; idx++ )
    {
        ...
        if(NULL != param->buff_bg)
        {
            ...
            if(move_x3 < 0)
            {
                if((move_x3 + buf_width3) <= 0)
                {
                    break;
                }
                dst_start_x3 = 0;
                src_start_x3 = -move_x3;
                copy_width3 = buf_width3 - src_start_x3;
            }
            else
            {
                if(move_x3 >= lcd_width)
                {
                    break;
                }
                dst_start_x3 = move_x3;
                src_start_x3 = 0;
                if((dst_start_x3 + buf_width3) > lcd_width)
                {
                    copy_width3 = lcd_width - dst_start_x3;
                }
                else
                {
                    copy_width3 = buf_width3;
                }
            }
            
            if(move_y3 < 0)
            {
                if((move_y3 + buf_height3) <= 0)
                {
                    break;
                }
                dst_start_y3 = 0;
                src_start_y3 = -move_y3;
                copy_height3 = buf_height3 - src_start_y3;
            }
            else
            {
                if(move_y3 >= lcd_height)
                {
                    break;
                }
                src_start_y3 = 0;
                dst_start_y3 = move_y3;
                if((dst_start_y3 + buf_height3) > lcd_height)
                {
                    copy_height3 = lcd_height - dst_start_y3;
                }
                else
                {
                    copy_height3 = buf_height3;
                }
            }
        ...
        if(PNULL != param->buff_bg)
        {
            SCI_MEMCPY( temp_lcd_buff, param->buff_bg, SCR_SIZE );
        }
        else
        {
        ...
        if(PNULL != param->buff_bg)
        {
            temp_dst_ptr3 = temp_lcd_buff  + dst_start_y3 * lcd_width + dst_start_x3;
            temp_src_ptr3 = param->buff_old + src_start_y3 * param->buf_mem_width + src_start_x3;

            for( i = 0; i < copy_height3; i++ )
            {
                SCI_MEMCPY( temp_dst_ptr3, temp_src_ptr3, copy_width3 * sizeof(uint16));

                temp_dst_ptr3 += lcd_width;
                temp_src_ptr3 += param->buf_mem_width;
            } 
        }
                                ...
                                )
{
    ...
    effect_param.buff_new = my_param->buff_new + my_param->rect.top* lcd_width + my_param->rect.left;
    if(PNULL != my_param->buff_bg)
    {
        effect_param.buff_old = my_param->buff_old + my_param->pop_rect.top* lcd_width + my_param->pop_rect.left;
        effect_param.buff_bg = my_param->buff_bg;
        effect_param.buff_wid3 = my_param->pop_rect.right - my_param->pop_rect.left + 1;
        effect_param.buff_height3 = my_param->pop_rect.bottom - my_param->pop_rect.top + 1;
    }
    else
    {
        effect_param.buff_old = my_param->buff_old;
        effect_param.buff_bg = PNULL;
    }
    ...
    if(param->type == MMI_POPUP_MENU_EFFECT_SLIDE_IN)
    {
        ...
        case MMI_POPUP_MENU_EFFECT_DIR_LEFT:
            ...
            if(PNULL != my_param->buff_bg)
            {
                effect_param.x3 = my_param->pop_rect.left;
                effect_param.x4 = 0 - effect_param.buff_wid3;
                effect_param.y3 = my_param->pop_rect.top;
                effect_param.y4 = my_param->pop_rect.top;
            }
            break;
    ...
    else
    {
        ...
        case MMI_POPUP_MENU_EFFECT_DIR_RIGHT:
            ...
            if(PNULL != my_param->buff_bg)
            {
                effect_param.x3 = 0 - effect_param.buff_wid3;
                effect_param.x4 = my_param->pop_rect.left;
                effect_param.y3 = my_param->pop_rect.top;
                effect_param.y4 = my_param->pop_rect.top;
            }
                                     ...
                                     )
{
        ...
        && MMI_IsPermit3D((BLOCK_MEM_ID_E)BLOCK_MEM_POOL_MMI_3D_DESKTOP))
    {
        ...
        if((0 != level) && (NULL != last_rect_ptr))
        {

            effect_param.pop_rect = *last_rect_ptr;
            effect_param.buff_bg = MMIDEFAULT_GetPopMenuBgBuf();

            if(PNULL != effect_param.buff_bg && 0 < effect_param.pop_rect.bottom
                && effect_param.pop_rect.bottom < lcd_rect.bottom)
            {
                //因为取的背景是打开Option menu前应用界面，需要将应用
                //Softkey区域替换成Option menu的Softkey区域。
                lcd_width  = lcd_rect.right - lcd_rect.left + 1;
                softkey_height = lcd_rect.bottom - effect_param.pop_rect.bottom + 1;

                temp_dst_ptr = effect_param.buff_bg  + effect_param.pop_rect.bottom* lcd_width;
                temp_src_ptr = buff_old  + effect_param.pop_rect.bottom* lcd_width;

                SCI_MEMCPY(temp_dst_ptr, temp_src_ptr, softkey_height* lcd_width *sizeof(uint16));
            }
        ...
        effect_param.buff_old = buff_old;
        effect_param.buff_new = buff_new;
