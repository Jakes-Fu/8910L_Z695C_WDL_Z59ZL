...
#else
#define MMITHEME_EDIT_LINE_SPACE       8  // 2 // xiongkai   //xiongkai     短信编辑界面编辑内容的时候汉字和字母J显示一行J会显示不全  2
#define MMITHEME_EDIT_CHAR_SPACE        0//2//should be 0 for MS SPEC arvin
#endif
//end 20101202
...
#define MMITHEME_EDI_DATE_YEAR_QUERY_MIN    1900
                           ...
                           )
{
    ...
    theme_ptr->border.color     = MMITHEME_GetAccentColor(MMITHEME_GetAccentId(), MMITHEME_OFFSET_COLOR_DEFAULT);
...
//@for light function modify
    ...
    theme_ptr->highlight_str_color = MMI_BLACK_COLOR;
    //theme_ptr->highlight_bg_color  = GUI_RGB2RGB565(0,0,0);/*lint !e572*/
	theme_ptr->highlight_bg_color  = GUI_RGB2RGB565(100,100,100);

    //set softkey,默认显示设置的softkey,输入字符后右softkey显示为删除
    theme_ptr->softkey.button[2].str_len = 1;
    theme_ptr->softkey.button[2].text_2  = STXT_SOFTKEY_DELETE_MK;//TXT_COMMON_CLEAR;//BUG496725

    theme_ptr->lcd_dev.lcd_id   = GUI_MAIN_LCD_ID;
    theme_ptr->lcd_dev.block_id = GUI_BLOCK_MAIN;
    theme_ptr->theme_color = MMI_DARK_BLUE_COLOR; // for [MS]
    theme_ptr->non_focused_color = MMI_WHITE_COLOR;
}
    ...
    )
{
    ...
    if (PNULL != digital_theme_ptr)
    {
        ...
        digital_theme_ptr->im.allow_im         = GUIIM_TYPE_DIGITAL;
    ...
    )
{
    ...
    if (PNULL != password_theme_ptr)
    {
        ...
        //set im
        //it seems that the param is_forbid_symbol is only
        // vaild on PDA version to influnce ui layout config.
#if defined GUIF_EDIT_PDASTYLE && defined MMI_IM_PDA_SUPPORT
        password_theme_ptr->im.is_forbid_symbol = TRUE;
#else
...
#else
        list_theme_ptr->separator_style = GUIEDIT_SEPARATOR_BLANK;	// [MS]
#endif
...
//  Note:
/*****************************************************************************/
PUBLIC void MMITHEME_GetMsDateEditTheme(
    MMITHEME_EDIT_DATE_T    *date_theme_ptr
    )
{
    GUIIM_INPUT_TYPE_T  init_input_type = GUIIM_INPUT_TYPE_NONE;
    ...
    if (PNULL != date_theme_ptr)
    {
        ...
        date_theme_ptr->date_style      = GUIEDIT_DATE_STYLE_DMY;
        date_theme_ptr->separator_style = GUIEDIT_SEPARATOR_LINE;
        date_theme_ptr->align           = ALIGN_LEFT;
        ...
        date_theme_ptr->text_id  = TXT_SET_DATE;
    }
}
...
//  Note:
/*****************************************************************************/
PUBLIC void MMITHEME_GetMsTimeEditTheme(
    MMITHEME_EDIT_TIME_T    *time_theme_ptr
    )
{
    GUIIM_INPUT_TYPE_T  init_input_type = GUIIM_INPUT_TYPE_NONE;
    ...
    if (PNULL != time_theme_ptr)
    {
        ...
        time_theme_ptr->separator_style = GUIEDIT_SEPARATOR_COLON;
        time_theme_ptr->display_style   = GUIEDIT_TIME_DISPLAY_STYLE_HM;
        time_theme_ptr->align           = ALIGN_LEFT;
        ...
        time_theme_ptr->text_id  = TXT_SET_TIME;
    }
}
...
/*****************************************************************************/
//  Description : get text display theme,include border,bg etc.
//  Global resource dependence : 
//  Author: Jassmine
//  Note:
/*****************************************************************************/
PUBLIC void MMITHEME_GetTextDisplayTheme(
                                       MMITHEME_EDIT_TEXT_DISPLAY_T  *theme_ptr  //in/out:
                                       )
{
    //set border
#ifdef MAINLCD_SIZE_128X64 //added by taiping.lai 2011/01/03
    theme_ptr->border.type      = GUI_BORDER_NONE;
#else
    theme_ptr->border.type      = GUI_BORDER_NONE;//GUI_BORDER_ROUNDED;
#endif
    theme_ptr->border.width     = 2;
    theme_ptr->border.color     = MMITHEME_GetAccentColor(MMITHEME_GetAccentId(), MMITHEME_OFFSET_COLOR_DEFAULT);
