...
/*****************************************************************************/
//  Description : Get Menu Number Width
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL uint16 GetMenuNumberWidth(
                                uint32       count_num
                                );
...
/*****************************************************************************/
//  Description : Create Item Layer According to Mode
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL GUI_LCD_DEV_INFO CreateItemLayerByMode(
                                       BOOLEAN is_move_mode
                                       );
#endif

/*****************************************************************************/
//  Description : Display Menu Icon In Rect
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL void DisplayMenuIconInRect(
                                 ...
                                 )
{
    ...
    theme_ptr->bg_alpha         = MMIMENU_BG_ALPHA;
                                    ...
                                    )
{
	...
	uint16 item_margin       = MMITHEME_TITLE_ICONMENU_ITEM_MARGIN_V;      //menu item的间距
	...
	if(is_move_mode)
	{
	    item_height       = MMITHEME_TITLE_ICONMENU_ITEM_HEIGHT_MOVE;
		item_width        = MMITHEME_TITLE_ICONMENU_ITEM_WIDTH_MOVE;
	    left_margin       = MMITHEME_TITLE_ICONMENU_LEFT_MARGIN_MOVE;
	    right_margin      = MMITHEME_TITLE_ICONMENU_RIGHT_MARGIN_MOVE;
		item_margin       = MMITHEME_TITLE_ICONMENU_ITEM_MARGIN_MOVE;
	}
    ...
    if (PNULL != icon_menu_info_ptr)
    {
		...
		icon_menu_info_ptr->left_margin      = left_margin;
		icon_menu_info_ptr->right_margin     = right_margin;
		icon_menu_info_ptr->item_margin      = item_margin;
                                        ...
                                        )
{
    ...
    {
        result = GetTitleIconMenuInfo(win_handle, icon_menu_info_ptr, is_move_mode);
    }
...
{ 
    ...
    text_width = GUISTR_GetStringWidth(&text_style, &text_str, text_state);
                                       ...
                                       )
{
    ...
    if ( is_move_mode )
    {
        create_info.width = MMITHEME_MENU_ITEM_WIDTH_MOVE;
        create_info.height = MMITHEME_MENU_ITEM_HEIGHT_MOVE;
    }
    else
    {
        create_info.width  = MMITHEME_MENU_ITEM_WIDTH;
                           ...
                           )
{
    ...
    //显示区域大则在显示区域居中显示
    icon_rect = GUI_GetCenterRect(disp_rect, img_width, img_height);

    //图片尺寸大则只显示图片中间显示区域内的部分
    image_rect.right = img_width - 1;
    image_rect.bottom = img_height - 1;
    disp_width = disp_rect.right - disp_rect.left + 1;
    disp_height = disp_rect.bottom - disp_rect.top + 1;
    image_rect = GUI_GetCenterRect(image_rect, disp_width, disp_height);
                             ...
                             )
{
    ...
    {
        s_menu_item_layer = CreateItemLayerByMode( is_move_mode );
    }
#endif
                                                  ...
                                                  )
{
    ...
    uint8*           buf_ptr     = PNULL;
    uint32           buf_size    = 0;
    BOOLEAN          is_move_mode = FALSE;
    BOOLEAN          is_color_theme_mode = FALSE;

    is_move_mode = GUIMAINMENU_IsMoveMode(MAINMENUWIN_MENU_CTRL_ID);
    is_color_theme_mode = GUIMAINMENU_IsColorThemeMode(MAINMENUWIN_MENU_CTRL_ID);

    InitMenuItemCache(is_move_mode);

    UILAYER_GetLayerInfo( &s_menu_item_layer, &layer_info );
    buf_ptr  = UILAYER_GetLayerBufferPtr( &s_menu_item_layer );
    buf_size = (uint32)(layer_info.layer_width) * (layer_info.layer_height) * sizeof(uint32);
        ...
        if ( PNULL != menu_item_ptr )
        {
            ...
            if (0 != menu_id )
            {
                ...
                if ( is_in_cache
                    ...
                    && !is_color_theme_mode)//color theme 模式因为item背景色变化，需要重画
                {
                    if ( !is_to_layer )
                    {
                        SCI_MEMCPY( buf_ptr, cache_ptr, cache_size );
...
#endif
                {
                    ...
                    case MMITHEME_MAINMENU_DATA_TYPE_DYNAMIC:
                        ...
                        if(CTRL_TEXT_ID == menu_item_ptr->text.type)
                        {
                            MMI_GetLabelTextByLang(STR_JAVA_APPLICATION_EX01, &item_str);	
                        }
                        else
                        {
                            item_str.wstr_len = menu_item_ptr->text.data.str_info.wstr_len;
                    ...
                    num_full_rect.left   = icon_rect.right - MENUICON_WIDTH/2 - RIGHT_MARGIN*2;
                    num_full_rect.right  = icon_rect.right;
                    num_full_rect.bottom = icon_rect.top + MENUICON_WIDTH/2 + 4;
                #else
...
{
#if 0
    MMI_IMAGE_ID_T  highlight_image[] = {IMAGE_HIGHLIGHT_VIOLET, IMAGE_HIGHLIGHT_RED, IMAGE_HIGHLIGHT_YELLOW,
        IMAGE_HIGHLIGHT_GREEN, IMAGE_HIGHLIGHT_COPPER, IMAGE_HIGHLIGHT_BLUE};
    
    if(accent_id < MMITHEME_ACCENT_COLOR_MAX)
    {
        return highlight_image[accent_id];
    }
    else
    {
        return highlight_image[MMITHEME_ACCENT_COLOR_DEFAULT];
    }
#endif
    return 0;
}
