...
/*---------------------------------------------------------------------------*/
LOCAL BOOLEAN MMK_IsWinIncludeTitleRect(MMI_HANDLE_T win_handle);
...
//  Author: Jassmine
//  Note:
// 判断是否有状态栏等，请用MMK_IsWinIncludeStatusbar,MMK_IsWinIncludeSoftkey,MMK_IsWinIncludeTitleRect
/*****************************************************************************/
...
//  Note:
/*****************************************************************************/
PUBLIC GUI_BOTH_RECT_T MMITHEME_GetWinPswEditBothRect(
                                                    MMI_HANDLE_T    win_handle
                                                    )
{
    uint16          lcd_width = 0;
    uint16          lcd_height = 0;
    GUI_BOTH_RECT_T pswedit_rect = {0};

    //is include status bar
    if (MMK_IsWinIncludeStatusbar(win_handle))
    {
        pswedit_rect.v_rect.top = pswedit_rect.h_rect.top = MMI_STATUSBAR_HEIGHT;
    }
    ...
    if ( lcd_width > lcd_height )
    {
        pswedit_rect.h_rect.right  = (int16)(lcd_width - 1);
        pswedit_rect.h_rect.bottom = (int16)(pswedit_rect.h_rect.top + MMI_TITLE_THEME_HEIGHT_H - 1);
        
        pswedit_rect.v_rect.right  = (int16)(lcd_height - 1);
        pswedit_rect.v_rect.bottom = (int16)(pswedit_rect.v_rect.top + MMI_TITLE_THEME_HEIGHT - 1); 
    }
    else
    {
        pswedit_rect.v_rect.right  = (int16)(lcd_width - 7);
	pswedit_rect.v_rect.left  = (int16)(pswedit_rect.v_rect.left + 7);
        pswedit_rect.v_rect.bottom = (int16)(pswedit_rect.v_rect.top + MMI_PSWEDIT_THEME_HEIGHT_TOP +MMI_PSWEDIT_THEME_HEIGHT - 1);
	pswedit_rect.v_rect.top = (int16)(pswedit_rect.v_rect.top + MMI_PSWEDIT_THEME_HEIGHT_TOP + 1);
...
//  Note:
/*****************************************************************************/
PUBLIC GUI_BOTH_RECT_T MMITHEME_GetWinNoHeaderAndDividerBothRect(
                                                    MMI_HANDLE_T    win_handle
                                                    )
{
    uint16          lcd_width = 0;
    uint16          lcd_height = 0;
    GUI_BOTH_RECT_T headeranddivider_rect = {0};

    //is include status bar
    if (MMK_IsWinIncludeStatusbar(win_handle))
    {
        headeranddivider_rect.v_rect.top = headeranddivider_rect.h_rect.top = MMI_STATUSBAR_HEIGHT;
    }
    ...
    if ( lcd_width > lcd_height )
    {
        headeranddivider_rect.h_rect.right  = (int16)(lcd_width - 1);
        headeranddivider_rect.h_rect.bottom = (int16)(headeranddivider_rect.h_rect.top + MMI_NOHEADERANDDIVIDER_THEME_HEIGHT_H - 1);
        
        headeranddivider_rect.v_rect.right  = (int16)(lcd_height - 1);
        headeranddivider_rect.v_rect.bottom = (int16)(headeranddivider_rect.v_rect.top + MMI_NOHEADERANDDIVIDER_THEME_HEIGHT - 1); 
    }
    else
    {
        headeranddivider_rect.v_rect.right  = (int16)(lcd_width - 1);
        headeranddivider_rect.v_rect.bottom = (int16)(headeranddivider_rect.v_rect.top + MMI_NOHEADERANDDIVIDER_THEME_HEIGHT - 1);
        
        headeranddivider_rect.h_rect.right  = (int16)(lcd_height - 1);
        headeranddivider_rect.h_rect.bottom = (int16)(headeranddivider_rect.h_rect.top + MMI_NOHEADERANDDIVIDER_THEME_HEIGHT_H- 1);
    }
    return headeranddivider_rect;
}
/*****************************************************************************/
//  Description : get win default title rect
//  Global resource dependence : 
...
//  Note:
/*****************************************************************************/
PUBLIC GUI_BOTH_RECT_T MMITHEME_GetWinOnlyDividerBothRect(
                                                    MMI_HANDLE_T    win_handle
                                                    )
{
    uint16          lcd_width = 0;
    ...
    {
        headeranddivider_rect.h_rect.right  = (int16)(lcd_width - 1);
        headeranddivider_rect.h_rect.bottom = (int16)(headeranddivider_rect.h_rect.top + MMI_ONLYDIVIDER_THEME_HEIGHT_H - 1);
        
        headeranddivider_rect.v_rect.right  = (int16)(lcd_height - 1);
        headeranddivider_rect.v_rect.bottom = (int16)(headeranddivider_rect.v_rect.top + MMI_ONLYDIVIDER_THEME_HEIGHT - 1); 
    }
    else
    {
        headeranddivider_rect.v_rect.right  = (int16)(lcd_width - 1);
...
//  Note:
/*****************************************************************************/
PUBLIC GUI_BOTH_RECT_T MMITHEME_GetWinHeaderAndDividerBothRect(
                                                    MMI_HANDLE_T    win_handle
                                                    )
{
    uint16          lcd_width = 0;
    ...
    {
        headeranddivider_rect.h_rect.right  = (int16)(lcd_width - 1);
        headeranddivider_rect.h_rect.bottom = (int16)(headeranddivider_rect.h_rect.top + MMI_HEADERANDDIVIDER_THEME_HEIGHT_H - 1);
        
        headeranddivider_rect.v_rect.right  = (int16)(lcd_height - 1);
        headeranddivider_rect.v_rect.bottom = (int16)(headeranddivider_rect.v_rect.top + MMI_HEADERANDDIVIDER_THEME_HEIGHT - 1); 
    }
    else
    {
        headeranddivider_rect.v_rect.right  = (int16)(lcd_width - 1);
                                          ...
                                          )
{
    ...
    {
    case LEFT_BUTTON:
        softkey_rect.left   = MMI_SOFTKEY_SPACE_H - 1;//6
        softkey_rect.top = (int16)(lcd_height - MMI_FULLSCREEN_SOFTKEY_BAR_HEIGHT + MMI_SOFTKEY_SPACE_H - 1);//320-38+6
        softkey_rect.right = MMI_SOFTKEY_SPACE_H + MMI_SOFTKEY_BUTTON_WIDTH -1;//6+64
        softkey_rect.bottom = (int16)(lcd_height - MMI_SOFTKEY_SPACE_H -1);//320-6
        break;

    case MIDDLE_BUTTON:
        softkey_rect.left   = MMI_SOFTKEY_SPACE_H*2 + MMI_SOFTKEY_BUTTON_WIDTH - 1;//6*2+64
        softkey_rect.top    = (int16)(lcd_height - MMI_FULLSCREEN_SOFTKEY_BAR_HEIGHT + MMI_SOFTKEY_SPACE_H- 1);//320-38+6
        softkey_rect.right  = (int16)(lcd_width - MMI_SOFTKEY_BUTTON_WIDTH - MMI_SOFTKEY_SPACE_H*2 - 1);//240-64-6*2
        softkey_rect.bottom = (int16)(lcd_height - MMI_SOFTKEY_SPACE_H -1);//320-6
        break;

    case RIGHT_BUTTON:
        softkey_rect.left   = (int16)(lcd_width - MMI_SOFTKEY_BUTTON_WIDTH - MMI_SOFTKEY_SPACE_H -1);//240-64-6
        softkey_rect.top    = (int16)(lcd_height - MMI_FULLSCREEN_SOFTKEY_BAR_HEIGHT + MMI_SOFTKEY_SPACE_H - 1);//320-38+6
        softkey_rect.right  = (int16)(lcd_width  - MMI_SOFTKEY_SPACE_H - 1);//240-6
        softkey_rect.bottom = (int16)(lcd_height - MMI_SOFTKEY_SPACE_H -1);//320-6
        break;
