...
{    
    ...
    if (is_drm_file && is_net_sys_time && !is_valid && !MMIAPIKL_IsPhoneLocked())
    {
        switch (method_type)
        {
        case DRM_LEVEL_SD:
                                                               ...
                                                               )
{
    ...
    if (is_drm_file && is_net_sys_time && !is_valid && is_alert_win && !MMIAPIKL_IsPhoneLocked())
    {
        switch (method_type)
        {
        case DRM_LEVEL_SD:
...
)
{
    ...
    switch (setting_type)
    {
        ...
        case DRMFILE_SETTING_SCREENSAVER:
            ...
            is_ret = MMIDRM_CanDrmFileSetToWSPC(file_path_ptr, file_path_len, 
                MMIFMM_FILE_TYPE_MOVIE, DRM_PERMISSION_PLAY)|| MMIDRM_CanDrmFileSetToWSPC(file_path_ptr, file_path_len, 
                MMIFMM_FILE_TYPE_PICTURE, DRM_PERMISSION_DISPLAY);			
            break;
    ...
    SCI_TRACE_LOW("[MMIDRM] MMIAPIDRM_CanDrmFileSetToWSPC: is_ret=%d, setting_type=%d", is_ret, setting_type);
    return is_ret;
}
