                                     ...
                                     )
{
    ...
    GUI_RECT_T      display_rect=MMITHEME_GetFullScreenRectEx(MMIPIC_EDIT_MAIN_WIN_ID);
...
PUBLIC void MMIPIC_DisplayImage(void)
{
	...
	bg_display.rect = display_rect;
	bg_display.win_handle = MMK_ConvertIdToHandle(MMIPIC_EDIT_ROTATE_WIN_ID);
...
//  Note: 
/*****************************************************************************/
PUBLIC void MMIPIC_CopyOrgImgDataToCropBufferData(uint8 **destbuff,uint32 *dest_size)
{

    *destbuff =(uint8 *)SCI_ALLOC_APP(s_edit_buffer_size);
        MMI_MEMCPY(*destbuff, s_edit_buffer_size,
        s_pic_editor_info.output_buf_ptr, s_edit_buffer_size, s_edit_buffer_size);

 *dest_size = s_edit_buffer_size;
}
#if 0
// #ifndef PIC_EDIT_U200_SUPPORT
// /*****************************************************************************/
...
//lijinghua add it for pic crop
PUBLIC void MMIPICCROP_OnlySaveOrgImgToUndoBuf(void)
{
	MMIPICCROP_INFO_T* crop_info_ptr = MMIPICCROP_GetCropInfoPtr();
    MMI_MEMCPY(s_pic_editor_info.undo_buf_ptr, s_edit_buffer_size,
        crop_info_ptr->display_bitmap_ptr->bit_ptr, s_edit_buffer_size, s_edit_buffer_size);

    s_pic_editor_info.undo_pic_width=crop_info_ptr->display_bitmap_ptr->img_width;
    s_pic_editor_info.undo_pic_heigh=crop_info_ptr->display_bitmap_ptr->img_height;
    
    s_pic_editor_info.undo_pic_x_start=crop_info_ptr->display_rect.left;
    s_pic_editor_info.undo_pic_y_start=crop_info_ptr->display_rect.top;
}
   //zhuyongjing add for back from crop,then goto others such as effect, bright...
PUBLIC void MMIPICCROP_CopyOutputImgDataToCropData(void){
    MMIPICCROP_INFO_T* crop_info_ptr = MMIPICCROP_GetCropInfoPtr();
    if(crop_info_ptr->crop_bitmap_size<=s_edit_buffer_size){
        SCI_MEMSET(crop_info_ptr->crop_bitmap_ptr->bit_ptr,0,crop_info_ptr->crop_bitmap_size);

        MMI_MEMCPY(crop_info_ptr->crop_bitmap_ptr->bit_ptr,crop_info_ptr->crop_bitmap_size,
                s_pic_editor_info.display_bitmap_ptr->bit_ptr,crop_info_ptr->crop_bitmap_size,
                crop_info_ptr->crop_bitmap_size);
    }else{
        SCI_MEMSET(crop_info_ptr->crop_bitmap_ptr->bit_ptr,0,s_edit_buffer_size);
        MMI_MEMCPY(crop_info_ptr->crop_bitmap_ptr->bit_ptr,crop_info_ptr->crop_bitmap_size,
                s_pic_editor_info.display_bitmap_ptr->bit_ptr,s_edit_buffer_size,
                s_edit_buffer_size);
    }
    crop_info_ptr->crop_bitmap_ptr->img_width = s_pic_editor_info.display_bitmap_ptr->img_width;
    crop_info_ptr->crop_bitmap_ptr->img_height = s_pic_editor_info.display_bitmap_ptr->img_height;

}
   //add end
PUBLIC void MMIPICCROP_CopyOutputImgDataToOrgImgData(void)
{
    MMIPICCROP_INFO_T* crop_info_ptr = MMIPICCROP_GetCropInfoPtr();

    s_pic_editor_info.display_bitmap_ptr->img_width = crop_info_ptr->crop_bitmap_ptr->img_width ;
    s_pic_editor_info.display_bitmap_ptr->img_height = crop_info_ptr->crop_bitmap_ptr->img_height;

    //copy output image data to display image data 
    //zhuyongjing changed for clear different buffer
 //   MMI_MEMCPY(s_pic_editor_info.display_bitmap_ptr->bit_ptr, s_edit_buffer_size,
    //   crop_info_ptr->crop_bitmap_ptr->bit_ptr, s_edit_buffer_size, s_edit_buffer_size);

    if(crop_info_ptr->crop_bitmap_size<=s_edit_buffer_size){
        SCI_MEMSET(s_pic_editor_info.display_bitmap_ptr->bit_ptr,0,s_edit_buffer_size);
        MMI_MEMCPY(s_pic_editor_info.display_bitmap_ptr->bit_ptr,s_edit_buffer_size,
                crop_info_ptr->crop_bitmap_ptr->bit_ptr,crop_info_ptr->crop_bitmap_size,
                crop_info_ptr->crop_bitmap_size);
    }else{
        SCI_MEMSET(s_pic_editor_info.display_bitmap_ptr->bit_ptr,0,s_edit_buffer_size);
        MMI_MEMCPY(s_pic_editor_info.display_bitmap_ptr->bit_ptr,s_edit_buffer_size,
                crop_info_ptr->crop_bitmap_ptr->bit_ptr,s_edit_buffer_size,
                s_edit_buffer_size);
    }
  //change end  
}
PUBLIC void MMIPICCROP_CopyOutputRectToOrgRect(void)
{
	MMIPICCROP_INFO_T* crop_info_ptr = MMIPICCROP_GetCropInfoPtr();
	...
	crop_img_width=crop_info_ptr->crop_bitmap_ptr->img_width;
	crop_img_height=crop_info_ptr->crop_bitmap_ptr->img_height;
	
	if(crop_img_width<=lcd_width && crop_img_height<=lcd_height)
	{
            crop_rect.left=(lcd_width-crop_img_width)/2;
	    crop_rect.right=crop_rect.left+crop_img_width;
	    crop_rect.top=(lcd_height-crop_img_height)/2;
	    crop_rect.bottom=crop_rect.top+crop_img_height;
	}else if(crop_img_width > lcd_width && crop_img_height<=lcd_height){
            img_height=crop_img_height*lcd_width/crop_img_width;
            crop_rect.left=0;
	    crop_rect.right=lcd_width -1;
	    crop_rect.top=(lcd_height-img_height)/2 -1;
	    crop_rect.bottom=crop_rect.top+img_height;
        }else if(crop_img_width <= lcd_width && crop_img_height>lcd_height){
	    img_width = crop_img_width*lcd_height/crop_img_height;
            crop_rect.left= (lcd_width-img_width)/2;
	    crop_rect.right=crop_rect.left+img_width;
	    crop_rect.top=0;
	    crop_rect.bottom=lcd_height -1;
	}else if(crop_img_width > lcd_width && crop_img_height>lcd_height){
            if(crop_img_width/crop_img_height >= 240/320){
                img_width = lcd_width;
                img_height = crop_img_height*lcd_width/crop_img_width;
                crop_rect.left=0;
	        crop_rect.right=lcd_width -1;
   ...
   // MMI_MEMCPY(&s_pic_editor_info.display_rect,sizeof(s_pic_editor_info.display_rect),\
    //    &crop_rect,sizeof(crop_rect),\
    //     sizeof(GUI_RECT_T));
    s_pic_editor_info.display_rect.left = crop_rect.left;
   s_pic_editor_info.display_rect.right= crop_rect.right;
   s_pic_editor_info.display_rect.top = crop_rect.top;
   s_pic_editor_info.display_rect.bottom= crop_rect.bottom;
}
//lijinghua end
/*****************************************************************************/
//  Description : copy output image data to original image data
...
// #endif
    ...
    if(s_pic_editor_info.display_bitmap_ptr == PNULL || s_pic_editor_info.display_bitmap_ptr->bit_ptr == PNULL)
    {
...
/*****************************************************************************/
//  Description : get crop image file info
//  Global resource dependence : 
//  Author: lijinghua
//  Note: 
/*****************************************************************************/

PUBLIC void  MMIPICCROP_GetFileInfo(PIC_EDIT_FILE_INFO_T * param_to_save_as_win)
{
    MMIPICCROP_INFO_T* crop_info_ptr = MMIPICCROP_GetCropInfoPtr();
    if (PNULL == param_to_save_as_win)
    {
    ...
    param_to_save_as_win->full_path_wstr_ptr=crop_info_ptr->random_name_ptr;
    param_to_save_as_win->full_path_wstr_len=crop_info_ptr->random_name_len;
}

/*****************************************************************************/
//  Description : save as new file for temp crop
//  Global resource dependence : 
//  Author: zhuyongjing
//  Note: 
/*****************************************************************************/
PUBLIC MMI_PIC_ERROR_TYPE_E  MMIPIC_SaveAsForCrop(
                           ...
                           )
{
    ...
    {
        ret_value = MMI_PIC_ERR_EXIT;
        SCI_TRACE_LOW("[MMIPIC] MMIPIC_SaveAsForCrop full_path_ptr is PNULL !!!");
        return ret_value;
    }
    ...
    {
        ret_value = MMI_PIC_ERR_EXIT;
        SCI_TRACE_LOW("[MMIPIC] MMIPIC_SaveAsForCrop full_path_len is 0 !!!");
        return ret_value;
    }
    ...
    if(result)
    {   
        is_write_success = MMIAPIFMM_WriteFilesDataSyn(full_path_ptr, full_path_len,\
...
/*****************************************************************************/
PUBLIC BOOLEAN  MMIPIC_AppendRotatePreviewImageData(uint16 cur_index)//lijinghua modify
{
    ...
    uint16  margin_pixel = 0;   //lijinghua modify
    uint16  width_min = 0;
    int16   offset_h = 0;
    int16   offset_v = 0;
	//lijinghua modify
    const int8    each_line_num = 1; //picture numbers for each line
    uint16  lcd_width = 0;
    uint16  lcd_height = 0;
    //lijinghua end
    GUI_RECT_T    display_rect = MMIPIC_GetClientWinRect(MMIPIC_EDIT_ROTATE_WIN_ID);
    GUILCD_GetLogicWidthHeight(GUI_MAIN_LCD_ID,&lcd_width,&lcd_height);              //lijinghua add
    ...
    //{
        ...
        if(MMI_ROTATION_DEGREE_90==i || MMI_ROTATION_DEGREE_270==i)
        {
        	s_icon_image[i].icon_bitmap_ptr->bit_ptr=SCI_ALLOCA(buf_size);
        }else
        {
			s_icon_image[i].icon_bitmap_ptr->bit_ptr=SCI_ALLOCA(s_edit_buffer_size);
		}
		...
		{
			SCI_MEMSET(s_icon_image[i].icon_bitmap_ptr->bit_ptr, 0, s_edit_buffer_size);
		}
    ...
    //{
        ...
        if(MMI_ROTATION_DEGREE_90==j || MMI_ROTATION_DEGREE_270==j)
        {
        ...
        //offset_v = (rotate_height + v_space) * (j / each_line_num) + (rotate_height - output_param.actual_out_height) / 2;
		    s_icon_image[j].display_rect.left =(lcd_width-s_icon_image[j].icon_bitmap_ptr->img_width)/2; //h_space + offset_h;
			s_icon_image[j].display_rect.top =display_rect.top + (int16)(client_height - s_icon_image[j].icon_bitmap_ptr->img_height) / 2;//display_rect.top + v_space + offset_v;		 
...
//lijinghua add for vertical mirror
PUBLIC BOOLEAN  MMIPIC_AppendVerticalMirrorPreviewImageData(uint16 cur_index)
{
    uint32  pic_height=0;
    uint32  pic_width=0; 
    PEPROCESS_INPUT_T     input_param={0};
    ...
    //alloc bitmap struct memory
    s_icon_image[cur_index].icon_bitmap_ptr = SCI_ALLOCA(sizeof(GUIIMG_BITMAP_T));
    if (PNULL == s_icon_image[cur_index].icon_bitmap_ptr)
    {
        SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMIPIC_EDIT_1859_112_2_18_2_45_45_33,(uint8*)"");
        ret_value = FALSE;
        return ret_value;
    }
    SCI_MEMSET(s_icon_image[cur_index].icon_bitmap_ptr, 0, sizeof(GUIIMG_BITMAP_T));
    MMI_MEMCPY(s_icon_image[cur_index].icon_bitmap_ptr, sizeof(GUIIMG_BITMAP_T),
        s_pic_editor_info.display_bitmap_ptr, sizeof(GUIIMG_BITMAP_T), sizeof(GUIIMG_BITMAP_T));

    //alloc bitmap data memory
	s_icon_image[cur_index].icon_bitmap_ptr->bit_ptr=SCI_ALLOCA(s_edit_buffer_size);
    if (PNULL == s_icon_image[cur_index].icon_bitmap_ptr->bit_ptr)
    {
        SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMIPIC_EDIT_1871_112_2_18_2_45_45_34,(uint8*)"");
        ret_value = FALSE;
        return ret_value;
	...
	SCI_MEMSET(s_icon_image[cur_index].icon_bitmap_ptr->bit_ptr, 0, s_edit_buffer_size);
    ...
    input_param.out_buf_ptr=s_icon_image[cur_index].icon_bitmap_ptr->bit_ptr;
	input_param.out_buf_size=s_edit_buffer_size;
	...
	process_param.type = PEPROCESS_ROTATION;
	process_param.process_param.rotation.degree = PEPROCESS_ROTATION_DEGREE_180;
	...
	s_icon_image[cur_index].icon_bitmap_ptr->img_width=output_param.actual_out_width;
	s_icon_image[cur_index].icon_bitmap_ptr->img_height=output_param.actual_out_height;

    s_icon_image[cur_index].display_rect.left =(lcd_width-s_icon_image[cur_index].icon_bitmap_ptr->img_width)/2;
	s_icon_image[cur_index].display_rect.top =display_rect.top + (int16)(client_height - s_icon_image[cur_index].icon_bitmap_ptr->img_height) / 2;		 
	s_icon_image[cur_index].display_rect.right =s_icon_image[cur_index].display_rect.left + s_icon_image[cur_index].icon_bitmap_ptr->img_width - 1;
	s_icon_image[cur_index].display_rect.bottom =s_icon_image[cur_index].display_rect.top + s_icon_image[cur_index].icon_bitmap_ptr->img_height - 1;
    return ret_value;
}
//lijinghua end
/*****************************************************************************/
...
PUBLIC void   MMIPIC_FreeRotatedPreviewImage(void)
{
    ...
    for(i=MMI_ROTATION_DEGREE_90;i<=MMI_ROTATION_DEGREE_VERTICAL_MIRROR;i++)//lijinghua modify
    {
    	  if(PNULL!=s_icon_image[i].icon_bitmap_ptr)
        {
          if(PNULL!=s_icon_image[i].icon_bitmap_ptr->bit_ptr)
...
{
    GUI_RECT_T        display_rect = MMITHEME_GetFullScreenRectEx(MMIPIC_EDIT_ROTATE_WIN_ID);   //lijinghua modify
    GUI_POINT_T     dis_point = {0};
    uint32  i=0;
	//lijinghua modify it
    GUI_BG_T            bg          = {0};
    ...
    GUI_FillRect(&s_pic_editor_info.lcd_dev,s_icon_image[cur_index].display_rect,MMIPIC_ICON_BG_COLOR);
    
    dis_point.x = s_icon_image[cur_index].display_rect.left;
    dis_point.y = s_icon_image[cur_index].display_rect.top;
    GUIIMG_DisplayBmp(FALSE,
        &s_icon_image[cur_index].display_rect,
        &dis_point,
        s_icon_image[cur_index].icon_bitmap_ptr,
        &s_pic_editor_info.lcd_dev);
...
//lijinghua add for vertical mirror
PUBLIC BOOLEAN MMIPIC_GetVerticaledMirrorImageData(uint16 cur_image_index)
{
    PEPROCESS_INPUT_T         input_param={0};
    PEPROCESS_OUTPUT_T       output_param={0};
    PEPROCESS_PARAMETER_T  process_param={0};
    ...
    GUILCD_GetLogicWidthHeight(GUI_MAIN_LCD_ID,&lcd_width,&lcd_height);
    SCI_FREE(s_icon_image[cur_image_index].icon_bitmap_ptr->bit_ptr);
    s_icon_image[cur_image_index].icon_bitmap_ptr->bit_ptr=PNULL;
    //set input param   
    input_param.src_ptr=s_pic_editor_info.display_bitmap_ptr->bit_ptr;
    input_param.out_buf_ptr=s_pic_editor_info.output_buf_ptr;
    input_param.out_buf_size=s_edit_buffer_size;
