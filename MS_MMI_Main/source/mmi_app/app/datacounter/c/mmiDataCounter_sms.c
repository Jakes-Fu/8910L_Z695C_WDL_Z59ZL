/****************************************************************************
** File Name:      mmidatacounter_sms.c                            
** Author:                                                                  
** Date:            2015/08/24
...
LOCAL MN_DUAL_SYS_E m_sms_simId = MMI_SmartDS_SIM_ID_1;
...
LOCAL MMI_RESULT_E HandleDataCounterSmsWinMsg(
                                         MMI_WIN_ID_T     win_id, 
                                         MMI_MESSAGE_ID_E msg_id, 
                                         DPARAM           param);

LOCAL MMI_RESULT_E HandleClearSmsCounterWinMsg(
                                           MMI_WIN_ID_T     win_id, 
                                           MMI_MESSAGE_ID_E msg_id, 
                                           DPARAM           param
                                           );

LOCAL void SetSMSLabelRect(void);
LOCAL void DisplayDataCounterSmsInfo(void);
PUBLIC void MMIAPIDataCounter_smsAlertNotifaction(MN_DUAL_SYS_E dual_sys, DATACOUNTER_DATATYPE_E dataType);

LOCAL MMI_RESULT_E HandleSmsSetWinMsg(
									MMI_WIN_ID_T win_id, 
									MMI_MESSAGE_ID_E msg_id, 
									DPARAM param
									);
LOCAL uint32 ReadAlertValue(MN_DUAL_SYS_E dual_sys, uint8 dataType);
LOCAL void MMIDataCounter_SmsItems();
LOCAL void MMIAPIDataCounter_readRecordSmsNV(MMI_DATACOUNTER_RECORD_DATA_NV_E* record_data_nv);

WINDOW_TABLE( DATA_COUNTER_WIN_SMS_TAB ) = 
{
    WIN_STATUSBAR,
    WIN_ID(MMIACC_DATA_COUNTER_WIN_SMS_ID),
    WIN_FUNC((uint32) HandleDataCounterSmsWinMsg),
    CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMI_DATACOUNTER_SIM_SMS_ALERT_CTRL_ID),
    //All
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_LEFT, MMI_DATACOUNTER_SMS_LABEL_ALL),
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_RIGHT, MMI_DATACOUNTER_SMS_LABEL_ALL_TEXT),
    //Sent
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_LEFT, MMI_DATACOUNTER_SMS_LABEL_SENT),
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_RIGHT, MMI_DATACOUNTER_SMS_LABEL_SENT_TEXT),
    //Received
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_LEFT, MMI_DATACOUNTER_SMS_LABEL_RECEIVED),
    CREATE_LABEL_CTRL(GUILABEL_ALIGN_RIGHT, MMI_DATACOUNTER_SMS_LABEL_RECEIVED_TEXT),
...
WINDOW_TABLE(MMITEST_CLEAR_SMS_WIN_TAB) = 
{
    WIN_FUNC( (uint32)HandleClearSmsCounterWinMsg),
    WIN_ID(MMIACC_DATA_COUNTER_WIN_SMS_MENU_ID),
    WIN_STYLE( WS_HAS_TRANSPARENT),
    WIN_HIDE_STATUS,
    CREATE_POPMENU_CTRL(MENU_DATACOUNTER_CLEAR_SMS_TABLE_ID,MMI_DATACOUNTER_CLEAR_SMS_COUNTER_MENU_CTRL_ID),
    WIN_SOFTKEY(TXT_NULL,STXT_SOFTKEY_SELECT_MK,STXT_RETURN),

    END_WIN
};

WINDOW_TABLE(MMITEST_SET_SMS_WIN_TAB ) = 
{
    WIN_FUNC((uint32)HandleSmsSetWinMsg),
    WIN_ID(MMIACC_DATA_COUNTER_WIN_SET_SMS_MENU_ID),
    //CREATE_POPMENU_CTRL(MENU_DATACOUNTER_SMS_ALERT_SET_TABLE_ID,MMI_DATACOUNTER_SMS_SET_CTRL_ID),
    WIN_STYLE( WS_HAS_TRANSPARENT|WS_DISABLE_COMMON_BG),
    WIN_SOFTKEY(TXT_NULL, STXT_SOFTKEY_SELECT_MK, STXT_RETURN),
    END_WIN
};

WINDOW_TABLE( MMITEST_CLEAR_SMS_CONFIRM_WIN_TAB) =
{
    WIN_ID(MMIACC_DATA_COUNTER_WIN_CLEAR_SMS_ID),
    WIN_STATUSBAR,
    WIN_STYLE(WS_DISABLE_RETURN_WIN),
    END_WIN
};

LOCAL MMI_RESULT_E HandleClearSmsQuery(
                                             MMI_WIN_ID_T win_id,
                                             MMI_MESSAGE_ID_E msg_id,
                                             DPARAM param
                                             )
...
LOCAL MMI_RESULT_E HandleOpenSmsQuery(
                                             MMI_WIN_ID_T win_id,
                                             MMI_MESSAGE_ID_E msg_id,
                                             DPARAM param
                                             )
{
	...
	case MSG_APP_WEB:
	case MSG_NOTIFY_MIDSK:
               MMK_CreateWin((uint32*)DATA_COUNTER_WIN_SMS_TAB, PNULL);
	        MMK_CloseWin(win_id);
		break;
       case MSG_APP_CANCEL:
		if (win_id == MMK_GetFocusWinId())
...
LOCAL void Clear_SmsConfirmationDialogWindow(void)
{
    MMI_WIN_ID_T     win_id_query = MMIACC_DATA_COUNTER_WIN_CLEAR_SMS_ID;
    MMIPUB_HANDLE_FUNC     handle_func = HandleClearSmsQuery;
    MMIPUB_OpenConfirmationDialogByTextId(PNULL,STR_COUN_CLEAR_SMS_EXT01, PNULL, PNULL, &win_id_query,PNULL, MMIPUB_SOFTKEY_DIALOG_OK_CLOSE,handle_func);
}
...
LOCAL void OpenSetAlertSelectMenu(MMI_WIN_ID_T win_id,MMI_CTRL_ID_T ctrl_id)
{
    ...
    GUIMENU_InsertNode(0,MMI_DATACOUNTER_SMS_ALERT_SET_OFF_MENU,0,&node_item,ctrl_id);
    pop_info.node_id =MMI_DATACOUNTER_SMS_ALERT_SET_OFF_MENU;
    GUIMENU_SetPopItemStatus(select_index == 0,&pop_info);
        
    MMI_GetLabelTextByLang(STR_DATA_ALERT_SETTINGS_ALL, &kstring);
    node_item.item_text_ptr = &kstring; 
    GUIMENU_InsertNode(1,MMI_DATACOUNTER_SMS_ALERT_SET_ALL_MENU,0,&node_item,ctrl_id);
    pop_info.node_id =MMI_DATACOUNTER_SMS_ALERT_SET_ALL_MENU;
    GUIMENU_SetPopItemStatus(select_index == 1,&pop_info);

    MMI_GetLabelTextByLang(STR_DATA_ALERT_SETTINGS_SENT, &kstring);
    node_item.item_text_ptr = &kstring; 
    GUIMENU_InsertNode(2,MMI_DATACOUNTER_SMS_ALERT_SET_SENT_MENU,0,&node_item,ctrl_id);
    pop_info.node_id =MMI_DATACOUNTER_SMS_ALERT_SET_SENT_MENU;
    GUIMENU_SetPopItemStatus(select_index == 2,&pop_info);

    MMI_GetLabelTextByLang(STR_DATA_ALERT_SETTINGS_RECV, &kstring);
    node_item.item_text_ptr = &kstring; 
    GUIMENU_InsertNode(3,MMI_DATACOUNTER_SMS_ALERT_SET_RECEIVED_MENU,0,&node_item,ctrl_id);
    pop_info.node_id =MMI_DATACOUNTER_SMS_ALERT_SET_RECEIVED_MENU;
...
{
    MMI_RESULT_E recode = MMI_RESULT_TRUE;
    MMI_CTRL_ID_T               ctrl_id = MMI_DATACOUNTER_SMS_SET_CTRL_ID;
    MMI_MENU_GROUP_ID_T group_id = 0;
    uint8 menu_index = 0;
    uint32 item_id = 0;   
    MMI_STRING_T    statusBar_str = {0};
    MMI_DATACOUNTER_DATA_E dataType = {0};
    dataType.simID = m_sms_simId;
    switch(msg_id)
    {
       case MSG_OPEN_WINDOW:
	   ...
	   case MSG_CTL_MIDSK:
			...
			switch(item_id)
			{
				case MMI_DATACOUNTER_SMS_ALERT_SET_OFF_MENU:
					//save
					menu_index = 0;
					break;
				case MMI_DATACOUNTER_SMS_ALERT_SET_ALL_MENU:
					//save
                    menu_index = 1;
                    dataType.dataType = 2;
                    MMIAPIDATACOUNTER_OpenSmsAlertWin(&dataType);
					break;
				case MMI_DATACOUNTER_SMS_ALERT_SET_SENT_MENU:
					menu_index = 2;
					dataType.dataType = 0;
					MMIAPIDATACOUNTER_OpenSmsAlertWin(&dataType);
					break;
				case MMI_DATACOUNTER_SMS_ALERT_SET_RECEIVED_MENU:
					menu_index = 3;
					dataType.dataType = 1;
					MMIAPIDATACOUNTER_OpenSmsAlertWin(&dataType);
					break;
			...
			if ( MMI_SmartDS_SIM_ID_1 == m_sms_simId )
			{
				MMINV_WRITE(MMI_DATACOUNTER_SIM1_SMS_SAVE_SELECT_ITEM, &menu_index);
			}
			else if( MMI_SmartDS_SIM_ID_2 == m_sms_simId )
			{
                           MMINV_WRITE(MMI_DATACOUNTER_SIM2_SMS_SAVE_SELECT_ITEM, &menu_index);
			}
...
{
    MMI_RESULT_E        recode = MMI_RESULT_TRUE;
    MMI_CTRL_ID_T               ctrl_id = MMI_DATACOUNTER_CLEAR_SMS_COUNTER_MENU_CTRL_ID;
    ...
    case MSG_CTL_OK:
        ...
        break;
    case MSG_CTL_MIDSK:
	 Clear_SmsConfirmationDialogWindow();
	 MMK_CloseWin(win_id);
		break;
    case MSG_CTL_CANCEL:
        MMK_CloseWin(win_id);
...
LOCAL void readRevData(MMI_DATACOUNTER_DATA_SET_TYPE* setType, MN_DUAL_SYS_E simCard)
{
    ...
    {
        nvItem = MMI_DATACOUNTER_SIM1_SMS_RECEIVED_LIMIT;
    }
    else if( MMI_SmartDS_SIM_ID_2 == simCard  )
    {
        nvItem = MMI_DATACOUNTER_SIM2_SMS_RECEIVED_LIMIT;
    }
...
LOCAL void readSentData(MMI_DATACOUNTER_DATA_SET_TYPE* setType, MN_DUAL_SYS_E simCard)
{
    ...
    if ( MMI_SmartDS_SIM_ID_1 == simCard )
    {
        nvItem = MMI_DATACOUNTER_SIM1_SMS_SENT_LIMIT;
    }
    else if( MMI_SmartDS_SIM_ID_2 == simCard  )
    {
        nvItem = MMI_DATACOUNTER_SIM2_SMS_SENT_LIMIT;
    }
...
LOCAL void readALLData(MMI_DATACOUNTER_DATA_SET_TYPE* setType, MN_DUAL_SYS_E simCard)
{
    ...
    if ( MMI_SmartDS_SIM_ID_1 == simCard )
    {
        nvItem = MMI_DATACOUNTER_SIM1_SMS_ALL_LIMIT;
    }
    else if( MMI_SmartDS_SIM_ID_2 == simCard  )
    {
        nvItem = MMI_DATACOUNTER_SIM2_SMS_ALL_LIMIT;
    }
...
LOCAL void readSetSMSStorageDataType(uint8* select_index)
{
    MN_RETURN_RESULT_E  return_value = MN_RETURN_FAILURE;
    uint32 nvItem = 0;
    ...
    if ( MMI_SmartDS_SIM_ID_1 == m_sms_simId )
    {
        nvItem = MMI_DATACOUNTER_SIM1_SMS_SAVE_SELECT_ITEM;
    }
    else if( MMI_SmartDS_SIM_ID_2 == m_sms_simId )
    {
        nvItem = MMI_DATACOUNTER_SIM2_SMS_SAVE_SELECT_ITEM;
    }
...
{
    uint32 i = 0; 
    MMI_CTRL_ID_T               ctrl_id = MMI_DATACOUNTER_SIM_SMS_ALERT_CTRL_ID;
    ...
    switch(select_index)
    {
        ...
        case 1:
                ...
                //get all
                readALLData(&setType, m_sms_simId);
                data_str.wstr_ptr = setType.limit;
                data_str.wstr_len = MMIAPICOM_Wstrlen(setType.limit);
            break;
        case 2:
                text_id = STR_DATA_ALERT_SETTINGS_SENT;
                //get sent
                readSentData(&setType, m_sms_simId);
                data_str.wstr_ptr = setType.limit;
                data_str.wstr_len = MMIAPICOM_Wstrlen(setType.limit);
            break;
        case 3:
                ...
                //get rev
                readRevData(&setType, m_sms_simId);
                data_str.wstr_ptr = setType.limit;
                data_str.wstr_len = MMIAPICOM_Wstrlen(setType.limit);
             break;
        default:
...
LOCAL void SetSMSLabelRect(void)
{
    ...
    switch( select_index )
    {
        ...
        case 2:
            ...
            alertValue = ReadAlertValue(m_sms_simId, 0);
            break;
        case 3:
            ...
            alertValue = ReadAlertValue(m_sms_simId, 1);
            break;
        default:
            break;
    }
    ...
    GUILABEL_SetRect(MMI_DATACOUNTER_SMS_LABEL_ALL, &rect, FALSE);
    GUILABEL_SetFont(MMI_DATACOUNTER_SMS_LABEL_ALL, font_size, font_color);
    GUILABEL_SetFont(MMI_DATACOUNTER_SMS_LABEL_ALL_TEXT, font_size, font_color);
     if ( select_index == 1 && (alertActualValue >= alertValue) )
    {
        GUILABEL_SetFont(MMI_DATACOUNTER_SMS_LABEL_ALL, font_size, font_color_ex);
        GUILABEL_SetFont(MMI_DATACOUNTER_SMS_LABEL_ALL_TEXT, font_size, font_color_ex);
    }
    ...
    rect.bottom = DATACOUNTER_EDIT_Y1 + DATACOUNTER_EDIT_HIGHT - 1;
    GUILABEL_SetRect(MMI_DATACOUNTER_SMS_LABEL_ALL_TEXT, &rect, FALSE);
    ...
    GUILABEL_SetRect(MMI_DATACOUNTER_SMS_LABEL_SENT, &rect, FALSE);
    GUILABEL_SetFont(MMI_DATACOUNTER_SMS_LABEL_SENT, font_size, font_color);
    GUILABEL_SetFont(MMI_DATACOUNTER_SMS_LABEL_SENT_TEXT, font_size, font_color);
     if ( select_index == 2 && (alertActualValue >= alertValue) )
    {
       GUILABEL_SetFont(MMI_DATACOUNTER_SMS_LABEL_SENT, font_size, font_color_ex);
        GUILABEL_SetFont(MMI_DATACOUNTER_SMS_LABEL_SENT_TEXT, font_size, font_color_ex);
    }
    ...
    rect.right = DATACOUNTER_EDIT_X3;
    rect.bottom = DATACOUNTER_EDIT_Y2 + DATACOUNTER_EDIT_HIGHT - 1;
    GUILABEL_SetRect(MMI_DATACOUNTER_SMS_LABEL_SENT_TEXT, &rect, FALSE);
    ...
    GUILABEL_SetRect(MMI_DATACOUNTER_SMS_LABEL_RECEIVED, &rect, FALSE);
    GUILABEL_SetFont(MMI_DATACOUNTER_SMS_LABEL_RECEIVED, font_size, font_color);
    GUILABEL_SetFont(MMI_DATACOUNTER_SMS_LABEL_RECEIVED_TEXT, font_size, font_color);
    if ( select_index == 3 && (alertActualValue >= alertValue) )
    {
        GUILABEL_SetFont(MMI_DATACOUNTER_SMS_LABEL_RECEIVED, font_size, font_color_ex);
        GUILABEL_SetFont(MMI_DATACOUNTER_SMS_LABEL_RECEIVED_TEXT, font_size, font_color_ex);
    }
    ...
    rect.right = DATACOUNTER_EDIT_X3;
    rect.bottom = DATACOUNTER_EDIT_Y3 + DATACOUNTER_EDIT_HIGHT - 1;
    GUILABEL_SetRect(MMI_DATACOUNTER_SMS_LABEL_RECEIVED_TEXT, &rect, FALSE);
...
LOCAL void DisplayDataCounterSmsInfo(void)
{
    ...
    if (PNULL != record_data_nv)
    {
        ...
        GUILABEL_SetText(MMI_DATACOUNTER_SMS_LABEL_ALL, &text, FALSE);
        str.wstr_ptr =record_data_nv->buffer_ALL;
        str.wstr_len = MMIAPICOM_Wstrlen(str.wstr_ptr);
        GUILABEL_SetText(MMI_DATACOUNTER_SMS_LABEL_ALL_TEXT, &str, FALSE);
        
        //Sent
        MMIRES_GetText(STR_DATA_ALERT_SETTINGS_SENT, MMK_GetFirstAppletHandle(), &text);
        GUILABEL_SetText(MMI_DATACOUNTER_SMS_LABEL_SENT, &text, FALSE);
        str.wstr_ptr = record_data_nv->buffer_send_outgoing;
        str.wstr_len = MMIAPICOM_Wstrlen(str.wstr_ptr);
        GUILABEL_SetText(MMI_DATACOUNTER_SMS_LABEL_SENT_TEXT, &str, FALSE);
        
        //Received
        MMIRES_GetText(STR_DATA_ALERT_SETTINGS_RECV, MMK_GetFirstAppletHandle(), &text);
        GUILABEL_SetText(MMI_DATACOUNTER_SMS_LABEL_RECEIVED, &text, FALSE);
        str.wstr_ptr = record_data_nv->buffer_recv_incoming;
        str.wstr_len = MMIAPICOM_Wstrlen(str.wstr_ptr);
        GUILABEL_SetText(MMI_DATACOUNTER_SMS_LABEL_RECEIVED_TEXT, &str, FALSE);
									...
									)
{
	...
	MN_DUAL_SYS_E simID = m_sms_simId;
	MMI_STRING_T    statusBar_str = {0};
    MMI_STRING_T text1 = {0};
    MMI_STRING_T text2 = {0};
	readSetSMSStorageDataType(&select_index);
    switch(msg_id)
    {
       case MSG_OPEN_WINDOW:
		MMK_SetAtvCtrl(win_id, MMI_DATACOUNTER_SIM_SMS_ALERT_CTRL_ID);
	   	break;

       case MSG_FULL_PAINT:
                SetSMSLabelRect();
	   	   MMIDataCounter_SmsItems();
		   DisplayDataCounterSmsInfo();
		   MMI_GetLabelTextByLang(STR_SIM_MANAGER_SMS, &statusBar_str);
	   ...
	   case MSG_APP_OK:
          ...
          case MSG_KEYUP_RIGHT:
            ...
            break;
	   case MSG_CTL_MIDSK:
		MMK_CreateWin((uint32*)MMITEST_SET_SMS_WIN_TAB,PNULL);
		//MMIAPIDataCounter_smsAlertNotifaction(m_simId, DATACOUNTER_SENT_DATATYPE);
	   	break;

       case MSG_CTL_OK:	
		MMK_CreateWin((uint32*)MMITEST_CLEAR_SMS_WIN_TAB,PNULL);
	   	break;
       case MSG_APP_CANCEL:
        case MSG_CTL_CANCEL:
	     MMK_CloseWin(win_id);
...
PUBLIC void ResetSmsShowJustOne(MN_DUAL_SYS_E dual_sys, uint8 dataType, BOOLEAN showjustOne)
{
    ...
    {
        item_id = MMI_DATACOUNTER_SIM1_SMS_SHOW_WIN_NV;
    }
    else if( MN_DUAL_SYS_2 == dual_sys )
    {
        item_id = MMI_DATACOUNTER_SIM2_SMS_SHOW_WIN_NV;
    }
...
LOCAL void ReadSmsShowJustOne(MN_DUAL_SYS_E dual_sys, DATACOUNTER_SHOW_WIN_JUST_ONE_E* showJustone)
{
    uint32 item_id = 0;
    MN_RETURN_RESULT_E  return_value = MN_RETURN_FAILURE;
...
LOCAL uint32 ReadActualValue(MN_DUAL_SYS_E dual_sys, uint8 dataType)
{
    ...
    if (PNULL != record_data_nv)
    {
        ...
        MMIDATACounter_GetSmsRecord(dual_sys, isPeroidSet, nDate, nMonth, nYear, record_data_nv);
#endif
...
PUBLIC void MMIAPIDataCounter_smsAlertNotifaction(MN_DUAL_SYS_E dual_sys, DATACOUNTER_DATATYPE_E dataType)
{
    ...
    SCI_TRACE_LOW("MMIAPIDataCounter_smsAlertNotifaction dataType = %d", dataType);
    if (MN_DUAL_SYS_1 == dual_sys)
    {
        item_id = MMI_DATACOUNTER_SIM1_SMS_RECORD;
    }
    ...
    if (FALSE == CheckSameSimCard(dual_sys, item_id))
    {
        ...
        SCI_TRACE_LOW("MMIAPIDataCounter_smsAlertNotifaction CheckSameSimCard %d", bAlertNotifaction);
    }
    ...
    if (bAlertNotifaction)
    {
        ...
        SCI_TRACE_LOW("MMIAPIDataCounter_smsAlertNotifaction bAlertStatus = %d", bAlertStatus);
        switch(alertIndex)
        {
            case 0:
                bAlertStatus = FALSE;
            ...
            case 3:
                bAlertStatus = TRUE;
                alertStatus = 1;//received
                break;
            default:
                break;
        }
        dataActualSize = ReadActualValue(dual_sys, dataType);            
        SCI_TRACE_LOW("MMIAPIDataCounter_smsAlertNotifaction bAlertStatus = %d, dataActualSize = %d", bAlertStatus, dataActualSize);
        ...
        {
            ReadSmsShowJustOne(dual_sys, &showJustone);
            SCI_TRACE_LOW("MMIAPIDataCounter_smsAlertNotifaction showJustone.dataType = %d", showJustone.dataType);
            SCI_TRACE_LOW("MMIAPIDataCounter_smsAlertNotifaction showJustone.showJustOne = %d", showJustone.showJustOne);
            //警介提示对话框如果弹过就不再给用户提示
            ...
            //设置的警介状态与当前数据类型不匹配，不提醒
                ...
                SCI_TRACE_LOW("MMIAPIDataCounter_smsAlertNotifaction bAlertNotifaction 111  = %d", bAlertNotifaction);
                if ( bAlertNotifaction )
                { 
                    if (alertStatus == 3)
                    {
                    ...
                    SCI_TRACE_LOW("MMIAPIDataCounter_smsAlertNotifaction dataActualSize = %d", dataActualSize);
                    SCI_TRACE_LOW("MMIAPIDataCounter_smsAlertNotifaction dataAlertSize = %d", dataAlertSize);
                    if ( dataActualSize >= dataAlertSize )
                    {
                        ...
                        {
                            case 3:
                                MMI_GetLabelTextByLang(STR_COUN_LIMIT_ALL_SMS_EXT01, &data_type_str);
                                break;
                                
                            case 0:
                                MMI_GetLabelTextByLang(STR_COUN_LIMIT_SENT_SMS_EXT01, &data_type_str);
                                break;
                                
                            case 1:
                                MMI_GetLabelTextByLang(STR_COUN_LIMIT_RECEIV_SMS_EXT01, &data_type_str);
                                break;
                            default:
                                break;
                        }
                        ...
                        MMIAPICOM_Int2Str(dataActualSize, str_buff, sizeof(str_buff));
                        data_size_str.wstr_ptr = buff_temp;
                        MMIAPICOM_StrToWstr(str_buff, buff_temp);
                        data_size_str.wstr_len = MMIAPICOM_Wstrlen(data_size_str.wstr_ptr);
                       
                        MMIAPICOM_Wstrncat(text2.wstr_ptr, data_size_str.wstr_ptr, data_size_str.wstr_len);
                        
                        ResetSmsShowJustOne(dual_sys, alertStatus, TRUE);
    ...
    SCI_TRACE_LOW("MMIAPIDataCounter_smsAlertNotifaction bAlertNotifaction 2222 = %d", bAlertNotifaction);
    if (bAlertNotifaction)
    {
        
        m_sms_simId = dual_sys;
        MMIAPIDataCounter_ThresholdTips(dual_sys, &text1, &text2, HandleOpenSmsQuery);
    }
    
    if (MMK_IsOpenWin(MMIACC_DATA_COUNTER_WIN_SMS_ID) && (MMIACC_DATA_COUNTER_WIN_SMS_ID == MMK_GetFocusWinId()))
    {
        MMK_SendMsg(MMIACC_DATA_COUNTER_WIN_SMS_ID, MSG_FULL_PAINT, PNULL);
    }
...
/*****************************************************************************/
// 	Description : MMIAPIDATACOUNTER_OpenSmsWin
//         
...
{
    m_sms_simId = *((MN_DUAL_SYS_E*)param);
    MMK_CreateWin((uint32*)DATA_COUNTER_WIN_SMS_TAB,(ADD_DATA)param);
...
/*****************************************************************************/
/*
PUBLIC void MMIAPIDATACOUNTER_OpenSmsWin(uint32 simcard)
{
