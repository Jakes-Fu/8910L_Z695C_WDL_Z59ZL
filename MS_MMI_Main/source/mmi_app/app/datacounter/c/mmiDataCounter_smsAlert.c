...
** ------------------------------------------------------------------------*
****************************************************************************/
#define _MMIDATACOUNTER_SMS_ALERT_C_  
...
LOCAL MMI_RESULT_E HandleDataCounterSmsAlertWinMsg(
                                         MMI_WIN_ID_T     win_id, 
                                         MMI_MESSAGE_ID_E msg_id, 
                                         DPARAM           param);

WINDOW_TABLE( DATA_COUNTER_WIN_SMS_ALERT_TAB ) = 
{
	WIN_STATUSBAR,
	WIN_ID(MMIACC_DATA_COUNTER_WIN_SMS_LIMIT_ID),
	WIN_FUNC((uint32) HandleDataCounterSmsAlertWinMsg),
	
	CREATE_FORM_CTRL(GUIFORM_LAYOUT_ORDER, MMI_DATACOUNTER_SMS_FORM_CTRL_ID),
	CHILD_LABEL_CTRL(GUILABEL_ALIGN_LEFT, FALSE, MMI_DATACOUNTER_SMS_ALERT_LABEL_TEXT, MMI_DATACOUNTER_SMS_FORM_CTRL_ID),
	CHILD_EDIT_DIGITAL_CTRL(TRUE, DATACOUNTER_SMS_MMS_INPUT_MAX_LENGTH, MMI_DATACOUNTER_SMS_ALERT_LABEL_EDIT, MMI_DATACOUNTER_SMS_FORM_CTRL_ID),
	WIN_SOFTKEY(TXT_NULL, TXT_COMMON_OK, STXT_RETURN),         
	END_WIN
};

LOCAL void ReadSMSNv(wchar* limit_num)
{
      MN_RETURN_RESULT_E  return_value = MN_RETURN_FAILURE;
	uint32 nvItem = 0;
	MMI_DATACOUNTER_DATA_SET_TYPE setType = {0};
	wchar* temp = L"200";
    MN_DUAL_SYS_E sms_simid = MMIAPIDataCounter_GetSMSSimID();
    
      if (MMI_SmartDS_SIM_ID_1 == sms_simid)
     {
     ...
     else if( MMI_SmartDS_SIM_ID_2 == sms_simid )
     {
	    switch( m_dataType.dataType )
        {
            case DATACOUNTER_SENT_TYPE:
									...
									)
{
    ...
    switch(msg_id)
    {
       case MSG_OPEN_WINDOW:	
            ...
            GUIFORM_SetBg(MMI_DATACOUNTER_SMS_FORM_CTRL_ID, &bg);
            CTRLFORM_SetActiveChildTheme(MMI_DATACOUNTER_SMS_FORM_CTRL_ID, &bg, &border, &FONT_ALL);
            GUIFORM_SetChildTheme(MMI_DATACOUNTER_SMS_FORM_CTRL_ID, &bg, &border, &FONT_ALL);
            //set limit label
            //GUITEXT_SetFont(MMI_DATACOUNTER_DATA_ALERT_LABEL_TEXT, &font, &color);
            bg_rect.bottom = bg_rect.top + 70 - 6;			
            CTRLFORM_SetRect(MMI_DATACOUNTER_SMS_FORM_CTRL_ID , &bg_rect);	
            GUIFORM_SetSpace(MMK_GetCtrlHandleByWin(win_id, MMI_DATACOUNTER_SMS_FORM_CTRL_ID), &hor_space, &ver_space);
            ctrl_ptr = MMK_GetCtrlPtr(MMI_DATACOUNTER_SMS_FORM_CTRL_ID);
            IGUICTRL_SetProgress(ctrl_ptr, FALSE);

            GUILABEL_SetTextById(MMI_DATACOUNTER_SMS_ALERT_LABEL_TEXT,STR_COUN_LIST_LIMIT_EXT02,FALSE);
            //set limit edit
            GUIEDIT_SetStyle(MMI_DATACOUNTER_SMS_ALERT_LABEL_EDIT, GUIEDIT_STYLE_SINGLE);
	     CTRLBASEEDIT_SetIsDisplayInverse(MMI_DATACOUNTER_SMS_ALERT_LABEL_EDIT, TRUE);
            ...
            GUIEDIT_SetString(MMI_DATACOUNTER_SMS_ALERT_LABEL_EDIT, inputInfo, MMIAPICOM_Wstrlen(inputInfo));
            osd_item_width.type     = GUIFORM_CHILD_WIDTH_FIXED;
            osd_item_width.add_data = lcd_width -50 ;
            GUIFORM_SetChildWidth(MMI_DATACOUNTER_SMS_FORM_CTRL_ID, MMI_DATACOUNTER_SMS_ALERT_LABEL_EDIT,   &osd_item_width);
            MMK_SetAtvCtrl(win_id, MMI_DATACOUNTER_SMS_ALERT_LABEL_EDIT);
            GUIEDIT_SetSoftkey(MMI_DATACOUNTER_SMS_ALERT_LABEL_EDIT, 1, 1, TXT_NULL, TXT_COMMON_OK, PNULL);
            GUIEDIT_SetSoftkeyEx(MMI_DATACOUNTER_SMS_ALERT_LABEL_EDIT, 2, 1, STXT_RETURN,TXT_DELETE, PNULL, PNULL, EDIT_BUTTON_TEXT_ID, PNULL);
	   	break;
	...
	case MSG_CTL_MIDSK:
            //save set nv value
            GUIEDIT_GetString(MMI_DATACOUNTER_SMS_ALERT_LABEL_EDIT, &limit_str);
            if ( limit_str.wstr_ptr != PNULL && limit_str.wstr_len > 0 )
            {
                ...
                MMINV_WRITE(nvItem, &setType);
                ResetSmsShowJustOne(sms_simid, m_dataType.dataType, FALSE);
                MMK_CloseWin(win_id);
            } 
	   	break;
...
PUBLIC void MMIAPIDATACOUNTER_OpenSmsAlertWin(void* param)
{
    ...
    MMIAPIDataCounter_SetSMSSimID(m_dataType.simID);
    MMK_CreateWin((uint32*)DATA_COUNTER_WIN_SMS_ALERT_TAB,(ADD_DATA)param);
