...
LOCAL MMI_RESULT_E  HandleCloseFDNServiceWindow(
                                         MMI_WIN_ID_T    win_id, 
...
LOCAL MMI_RESULT_E HandleFDNServiceCnf(DPARAM param)
{
    ...
    if (is_fdn_enable)
    {
        ...
        if (MMIFDN_ENABLE_OPER == s_fdn_context[dual_sys].oper_type)
        {
           ...
           MMIPUB_OpenAlertWinByTextId(PNULL,TXT_SUCCESS,TXT_NULL,IMAGE_PUBWIN_SUCCESS,PNULL,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,HandleCloseFDNServiceWindow);
