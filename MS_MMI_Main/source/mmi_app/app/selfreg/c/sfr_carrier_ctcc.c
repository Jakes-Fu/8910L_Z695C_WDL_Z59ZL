...
/////////////////////////////////////////////////////////////////////////////////////////////
extern uint8 g_sfr_cdma_sim_sys;
...
extern int g_http_err_code;
static char *ctcc_get_url(void *ctx, int *len)
{
    if (CTCC_CHECK(ctx))
    {
        *len = -1;
        if(g_http_err_code == 15)
        {
            return "http://42.99.2.15/";
        }
    ...
    if (CTCC_CHECK(ctx))
    {
	 ...
	 int json_base64_len;
        ...
        if (sfr_add_json_item(DATASIM_KEY, sfr_dm_get_data_sim()))
            sfr_add_json_char(JSON_SPLIT);
        #if 0 // deleted no need item by zdtone bao 
        // - ROM
	 ...
	 if (_ctx->package_buff != NULL)
	 {
            mbedtls_base64_encode(_ctx->package_buff, _base64_max, &json_base64_len, p_json, p_json_len);
	 }
        sfr_json_buff_term();
	 *len = json_base64_len;
