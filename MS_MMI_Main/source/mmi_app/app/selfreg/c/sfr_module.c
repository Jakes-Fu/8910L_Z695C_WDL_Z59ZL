...
LOCAL MMI_RESULT_E SlfReg_HandleModuleMsg(PWND app_ptr, uint16 msg_id, DPARAM param)
{
    ...
    {
        SCI_TraceLow("[Self Reg] SlfReg_HandleModuleMsg msg_id = %d [HTTPC_SIG_BEG = %d]", msg_id, HTTPC_SIG_BEG);
...
PUBLIC void MMISFR_InitModule(void)
{
    ...
    SCI_TraceLow("[Self Reg] MMISFR_InitModule");
    ...
    g_sfr_app.ProcessMsg = SlfReg_HandleModuleMsg;
