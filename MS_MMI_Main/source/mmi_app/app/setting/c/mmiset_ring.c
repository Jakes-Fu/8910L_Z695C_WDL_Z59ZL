...
#if defined(MMI_MP3_FIXED_MUSIC)
...
BOOLEAN          g_cc_ring_play_num = FALSE;

BOOLEAN			g_is_play_menu = FALSE;

BOOLEAN			g_is_time_menu = FALSE;
#ifdef PHONE_QUIT_TTS

uint8 s_num_char = 0;


PUBLIC SCI_TIMER_PTR s_play_tts_ring_timer = PNULL;  
#endif
PUBLIC void  MMIAPISET_PlayPhoneRing(uint8 n_char,MMISET_RING_CALLBACK_PFUNC callback_func);
...
LOCAL  uint32 s_ct_task_id = 0;                     //ct task id
...
LOCAL BOOLEAN   s_mmiset_vibra_onetime = FALSE;
...
extern BOOLEAN g_is_special_resource;
/**--------------------------------------------------------------------------*
**                         GLOBAL DEFINITION                                *
**--------------------------------------------------------------------------*/
...
#ifdef VIRTUAL_VIBRATE_FUNC // ZSP_VIBRATE
PUBLIC void MMIAPISET_SetVibStartPara(void)
{
    ARMVB_SetZspVibratePara_Direct(TRUE);
    ARMVB_SetCompositionofRingandVibFlag(TRUE);
}

PUBLIC void MMIAPISET_SetVibStopPara(void)
{
    ARMVB_SetZspVibratePara_Direct(FALSE);
    ARMVB_SetCompositionofRingandVibFlag(FALSE);
}

PUBLIC void MMIAPISET_SetVibrateOneTime(void)
{
    s_mmiset_vibra_onetime = TRUE;
}

PUBLIC void MMIAPISET_StartZspVibrator(void)
{
    ARMVB_SetZspVibratePara_Direct(TRUE);
    if (TRUE == s_mmiset_vibra_onetime)
    {
        AUDIO_ZSP_MusicVibrateStart(1200);
        s_mmiset_vibra_onetime = FALSE;
    }
    else
    {
        AUDIO_ZSP_MusicVibrateStartAllTime();
    }
}

PUBLIC void MMIAPISET_StopZspVibrator(void)
{
    ARMVB_SetZspVibratePara_Direct(FALSE);
    ARMVB_SetCompositionofRingandVibFlag(FALSE);
    AUDIO_ZSP_MusicVibrateStop();
}
#endif
...
//  Note: 
/*****************************************************************************/
PUBLIC void VibrateCallbackEx(void)
{
                                  ...
                                  )
{
    if (is_open)//打开振动
    {
        ...
        {
            s_vibrate_handle = MMISRVAUD_StartVibrate(pri, duration, 0, 1, VibrateCallbackEx);
        }
    ...
    if(MMIAPISET_IsPermitPlayRing(ring_type_temp)) 
    {
        ...
        if (MMISET_RING_TYPE_CALL_IN_CALL == ring_type)
        {
            ...
            if( 505 == cur_plmn_ptr.mcc )// The MCC of Australia is 505 // "AUS"
            {
                ...
                PlayGenericTone(AUD_GENERIC_WARNING_TONE, MMISET_CALL_IN_CALL_PLAY_RING_TIME_AUS, 425);
            }
            else
            {
                PlayGenericTone(AUD_GENERIC_WARNING_TONE, MMISET_CALL_IN_CALL_PLAY_RING_TIME, 2000);
            }
        }
        else if (MMISET_RING_TYPE_MSG_IN_CALL == ring_type)
        {
            PlayGenericTone(AUD_GENERIC_WARNING_TONE, MMISET_DEFAULT_DSP_PLAY_RING_TIME, 2000);
        }
        ...
        {
            PlayGenericTone(AUD_GENERIC_DIAL_TONE, AUD_PLAY_FOREVER, 2000);
        }
        else if (MMISET_RING_TYPE_HANGUP_REMIND == ring_type)
        {
            PlayGenericTone(AUD_GENERIC_WARNING_TONE, AUD_PLAY_FOREVER, 2000);
        }
	#endif 
#ifdef MMI_ETWS_SUPPORT
...
#ifdef PHONE_NUMBER_VOICE
			if(MMISET_RING_TYPE_PHONENUM == ring_type)
			{
			  ///Trace_Log_Buf_Print("MMIAPISET_PlayDialRing ring_tyaaape%d",ring_type);
				ring_id = group_id;
				//if(callback_func != PNULL)
				//{
				#ifdef HERO_ENGINE_TTS_SUPPORT
				ring_vol =	TTS_GetTTSVolume();
				#else
				#ifndef TTS_SUPPORT_NONE
					ring_vol =	ZDT_GetVol_Num();//MMIAPIENVSET_GetActiveModeRingVol( MMISET_RING_TYPE_CALL);
				#else
...
#endif 
#ifdef MMI_MP3_FIXED_MUSIC
			 if(MMISET_RING_TYPE_FIXED_MUSIC == ring_type)
			 {
				 ring_id = group_id;
				 //if(callback_func != PNULL)
				 //{
					ring_vol = MMIAPISET_GetMultimVolume();
				 //}
			 }
#endif 

#ifdef SOS_KEYLONGWEB
			if(MMISET_RING_TYPE_SOSALARM == ring_type)
			{
				ring_id = group_id;
				ring_vol = 8 ;
			}
                         ...
                         )
{
    ...
    SCI_TRACE_LOW("[MMISET]:PlayFixedRing, ring_type = 0x%x ring_id= %d", ring_type, ring_id);
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMISET_RING_689_112_2_18_2_51_39_275,(uint8*)"d",ring_type);

    if (MMISET_RING_TYPE_HANDHOLD == ring_type)
    {
        GetRingInfo(ring_id,MMISET_RING_TYPE_CALL,&ring_data);
    }
    else
    {
    ...
    if (PNULL != ring_data.data_ptr)
    {
        ...
        if (/*MMISET_RING_FIXED_DEFAUT_NUM*/g_ringtone_max < ring_id)
        {
        ...
        if(MMISET_RING_TYPE_HANDHOLD == ring_type)
        {
            audio_srv.all_support_route = MMISRVAUD_ROUTE_HANDHOLD;
        }
                    ...
                    )
{
    ...
    //add ring_type==MMISET_RING_TYPE_KEY for keypad ring when silent
    if (( (ring_type==MMISET_RING_TYPE_TKEY)&&0== ring_id&&(MMISET_VOL_ZERO != ring_vol))
        || (ring_type!=MMISET_RING_TYPE_TKEY&&((MMISET_VOL_ZERO != ring_vol)&&(0 != ring_id))))
    {
        //播放Mp3
        if (MMISET_MAX_RING_ID == ring_id)
        {
            ...
            case MMISET_RING_TYPE_CALL:
                ...
                if (!MMIAPIFMM_PlayMusicFile(g_is_playing_ring_type,FALSE,call_ring_info.more_ring_info.name_wstr,call_ring_info.more_ring_info.name_wstr_len,play_times,0,ring_vol,callback_func))
                {
                    ...
                    PlayFixedRing(g_ringtone_default_id,ring_type,play_times,ring_vol,callback_func);
                    MMIAPIENVSET_SetFixedCallRingId(dual_sys,g_ringtone_default_id,mode_id);
                }
                break;
            ...
            case MMISET_RING_TYPE_MSG:
                ...
                if (!MMIAPIFMM_PlayMusicFile(g_is_playing_ring_type,FALSE,msg_ring_info.more_ring_info.name_wstr,msg_ring_info.more_ring_info.name_wstr_len,play_times,0,ring_vol,callback_func))
                {
                    ...
                    PlayFixedRing(g_alert_default_id,ring_type,play_times,ring_vol,callback_func);
                    MMIAPIENVSET_SetFixedMsgRingId(dual_sys,g_alert_default_id,mode_id);
                }
                break;
                
            case MMISET_RING_TYPE_ALARM:
            case MMISET_RING_TYPE_SCH:
...
/*****************************************************************************/
//  Description : new thread to stop Ring. because Fm init is needed long time.
//  Global resource dependence : 
//  Author: jsong
//  Note:   
/*****************************************************************************/
LOCAL void CT_TaskHandle(uint32 argc, void * argv)
{
    SCI_DEBUG("");
    MMIAPISET_StopAppRing();
    SCI_ThreadExit();
                                   ...
                                   )
{
    ...
    uint8 mode_id = MMIAPIENVSET_GetPlayRingModeId();
    BOOLEAN cb_alert_flag = MMIAPISMSCB_GetCBMsgType();
    if((MMIENVSET_SILENT_MODE != mode_id) && cb_alert_flag)
    {
        ring_vol = MMISET_VOL_SIX;
    }
#endif
...
/*****************************************************************************/
//  Description : to append a group of items to listbox by num.
//  Global resource dependence : 
//  Author:
//  Note: 
/*****************************************************************************/
PUBLIC void MMIAPISET_AppendFixedItemsOnly(
                                           MMI_TEXT_ID_T        prefix_txt_id,
                                           uint32               item_num,      
                                           MMI_TEXT_ID_T        left_softkey_id,
                                           MMI_TEXT_ID_T        middle_softkey_id,
                                           ...
                                           )
{
    ...
    wchar *text_ring[MMISET_RING_FIXED_DEFAUT_NUM] =
        {RING_NAMES};
    wchar *text_alert[MMISET_MSG_FIXED_RING_MAX_ITEM] =
        {ALERT_NAMES};
    wchar *text_ring_ex[MMISET_RING_FIXED_DEFAUT_NUM_EX] =
        {RING_NAMES_EX};
    wchar *text_alert_ex[MMISET_MSG_FIXED_RING_MAX_ITEM_EX] =
        {ALERT_NAMES_EX};
    wchar **str_arr_ptr = text_ring;

    MMISET_RING_TITLE_INFO_T orthodox_text_ring[] = {ORTHODOX_RING_NAMES};

    if (item_num > 999)
    {
        return;
    }
	if(item_num==MMISET_MSG_FIXED_RING_MAX_ITEM_EX)
	{
		str_arr_ptr = text_alert_ex;
	}
	else if (item_num == MMISET_MSG_FIXED_RING_MAX_ITEM)
	{
		str_arr_ptr = text_alert;
	}
	else if(item_num==MMISET_RING_FIXED_DEFAUT_NUM_EX)
	{
		str_arr_ptr = text_ring_ex;
	}
	else
	{
		str_arr_ptr = text_ring;
	}

    for (i = 0; i < item_num; i++)
    {   
        if(i < MMISET_RING_FIXED_DEFAUT_NUM)
        {
            MMIAPISET_AppendListItemByTextBuffer(
                str_arr_ptr[i],
                MMIAPICOM_Wstrlen(str_arr_ptr[i]),
                0,
                left_softkey_id, 
                middle_softkey_id, 
                right_softkey_id, 
        ...
        else
        {
            ...
            {
                MMIAPISET_AppendListItemByTextBuffer(
                    orthodox_text_ring[i-MMISET_RING_FIXED_DEFAUT_NUM].title_id,
                    MMIAPICOM_Wstrlen(orthodox_text_ring[i-MMISET_RING_FIXED_DEFAUT_NUM].title_id),
                    0,
                    left_softkey_id, 
                    middle_softkey_id, 
                    right_softkey_id, 
...
PUBLIC uint16 MMIAPISET_GetAlertToneName(uint16 idx, wchar *ring_name, uint32 len)
{
    uint16 id = idx;
    wchar *text_wstr[MMISET_MSG_FIXED_RING_MAX_ITEM] = {ALERT_NAMES};
	wchar *text_wstr_ex[MMISET_MSG_FIXED_RING_MAX_ITEM_EX] = {ALERT_NAMES_EX};
    uint16 ring_len = 0;
    ...
    if(id > /*MMISET_MSG_FIXED_RING_MAX_ITEM*/g_alert_max || 0 == id )
    {
        id = 1;
    }
	if(g_alert_max == MMISET_MSG_FIXED_RING_MAX_ITEM){
	    ring_len = MMIAPICOM_Wstrlen(text_wstr[id-1]);//return the name from 0
	    MMIAPICOM_Wstrncpy(ring_name, text_wstr[id-1],ring_len);
	}else{
	    ring_len = MMIAPICOM_Wstrlen(text_wstr_ex[id-1]);//return the name from 0
	    MMIAPICOM_Wstrncpy(ring_name, text_wstr_ex[id-1],ring_len);
	}
    return ring_len;
}

//[alarm ring tone UI and function]
PUBLIC uint16 MMIAPISET_GetClockRingName(uint16 idx, wchar *ring_name, uint32 len)
{
    uint16 id = idx;
    wchar  *text_wstr[MMIALM_RING_FIXED_DEFAULT_NUM] = {CLOCK_NAMES};
    wchar  *text_wstr_ex[MMIALM_RING_FIXED_DEFAULT_NUM_MAT] = {CLOCK_NAMES_MAT};
    ...
    if(id > /*MMIALM_RING_FIXED_DEFAULT_NUM*/g_clock_ring_max_num || id == 0)
    {
        id = 1;
    }

    wstr_ptr = g_clock_ring_max_num == MMIALM_RING_FIXED_DEFAULT_NUM ?
            text_wstr : text_wstr_ex;
    ring_len = MMIAPICOM_Wstrlen(wstr_ptr[id-1]);//return the name from 0
    MMIAPICOM_Wstrncpy(ring_name, wstr_ptr[id-1],ring_len);
    return ring_len;
}
//[end]
                                           ...
                                           MMI_WIN_ID_T win_id,MMI_IMAGE_ID_T img_id)
{
	...
	//uint16				adjust_vol_level_left  =0;
	//uint16				adjust_vol_level_top  =0;
...
PUBLIC MMISET_CALL_RING_T MMIAPISET_GetCallRingInfo(MN_DUAL_SYS_E dual_sys,uint8 mode_id)
{
    ...
    if (MMISET_CALL_RING_MORE_RING == call_ring_info.call_ring_type)
    {
        if(!MMIAPIFMM_IsFileExist( call_ring_info.more_ring_info.name_wstr,call_ring_info.more_ring_info.name_wstr_len))
        {
            ...
            call_ring_info.fix_ring_index = g_ringtone_default_id;
...
#endif
    {
        //except choose from music
        /*if(cur_item == 0)
        {
            return;
        }*/
        ring_id = cur_item + 1;//except choose from music
    }
...
{
   // SCI_ASSERT(PNULL != ring_info_ptr);
   ...
   if(PNULL == ring_info_ptr)
   {
    ...
    case MMISET_RING_TYPE_CALL:        
        ...
        // *ring_info_ptr = g_midi_table[ring_id-1];

        SCI_TRACE_LOW("PlayFixedRing ->GetRingInfo id=%d", (ring_id + MMI_CALL_RING_START_ID - 1));
        MMI_GetRingInfo(
            (ring_id + MMI_CALL_RING_START_ID - 1),
            ring_info_ptr);   
        break;
    case MMISET_RING_TYPE_MSG:
    // case MMISET_RING_TYPE_SCH:
    /*
        if (ring_id <= MMISET_DEFAULT_SMS_NUM)
        {            
            MMI_GetRingInfo(
        ...
        {            
            MMI_GetRingInfo(
                (ring_id - MMISET_DEFAULT_SMS_NUM + MMI_MMS_RING_START_ID - 1),
                ring_info_ptr);
        }
    */
        MMI_GetRingInfo(
    ...
    case MMISET_RING_TYPE_DC_REC_STOP:
        MMI_GetRingInfo(
            (ring_id + MMI_DC_REC_STOP_ID - 1),
            ring_info_ptr);
        ring_info_ptr->type = (MMISRVAUD_RING_FMT_E)MMISRVAUD_RING_FMT_WAVE;/*lint !e64*/
        break;
    case MMISET_RING_TYPE_DC_REC_START:
        MMI_GetRingInfo(
            (ring_id + MMI_DC_REC_START_ID - 1),
            ring_info_ptr);
        ring_info_ptr->type = (MMISRVAUD_RING_FMT_E)MMISRVAUD_RING_FMT_WAVE;/*lint !e64*/
        break;
    ...
    #if defined(MMI_MP3_FIXED_MUSIC)
	   case MMISET_RING_TYPE_FIXED_MUSIC:
			MMI_GetRingInfo(
				MMI_MP3_FIXED_MUSIC_RING_START_ID,
				ring_info_ptr);
		#if 1//defined(COMMON_3232_240320_WX)
				ring_info_ptr->type = (MMISRVAUD_RING_FMT_E)MMISRVAUD_RING_FMT_MP3;
		#endif
		  break;
#endif   
#ifdef SOS_KEYLONGWEB
		case MMISET_RING_TYPE_SOSALARM:
		{
			if(sos_temp)
			{
				MMI_GetRingInfo(
					(ring_id + MMI_SOS_ALARM_START_ID - 1),
					ring_info_ptr);
			}
			ring_info_ptr->type = (MMISRVAUD_RING_FMT_E)MMISRVAUD_RING_FMT_MP3;
		}
...
#ifdef PHONE_NUMBER_VOICE
	case MMISET_RING_TYPE_PHONENUM:
				if(g_is_play_menu == TRUE)
			{
				//MMI_GetRingInfo(
				//	(ring_id + MMI_MENU_RING_START_ID - 1),
				//	ring_info_ptr);
			}
			else
				{
				MMI_GetRingInfo(
				(ring_id + MMI_PHONENUM_RING_START_ID - 1),
				ring_info_ptr);
			}
    ...
    case MMISET_RING_TYPE_OTHER:
        ...
        {
            MMI_GetRingInfo(
                (ring_id + MMI_OPER_RING_START_ID - MMISET_RING_FIXED_DEFAUT_NUM- 1),
                ring_info_ptr); 
        }
        break;
                                           ...
                                           )
{
    ...
    {   
        ...
        {
        case 0:
            MMIAPISET_AppendListItemByTextId( TXT_COMMON_RING_RING1, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO );
            break;
        case 1:
            MMIAPISET_AppendListItemByTextId( TXT_COMMON_RING_RING2, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO );
            break;
        case 2:
                                           ...
                                           )
{
    ...
    if(MMIAPISET_IsOrthodoxVariant())
    {
        MMIAPISET_AppendFixedItemsOnly(
            TXT_RING,
            /*MMISET_RING_FIXED_DEFAUT_NUM*/g_ringtone_max,
            TXT_COMMON_OK,
            TXT_NULL,
            STXT_RETURN,
            ctrl_id,
    ...
    else
    {
        ...
        if ( MMISET_CALL_RING_FIXED == ring_info.call_ring_type)
        {
            cur_selection = ring_info.fix_ring_index;
    ...
    ring_id = (uint8)(cur_selection + 1);
	//ring_id = (uint8)(cur_selection);//dong .chunguang addexcept choose from music
    //get ring volume
    ring_vol = MMIAPIENVSET_GetCurModeRingVol(MMISET_RING_TYPE_CALL);
	...
	//dong.chunguang add if more music don't play
    /*if ( MMISET_CALL_RING_MORE_RING == ring_info.call_ring_type||cur_selection ==0)
    {
		return;
	}*/
	//end
                           ...
                           )
{
    ...
    if (is_dsp_play_ring)
    {
        ...
        MMISRVAUD_PlayTone(MMISRVAUD_TYPE_TONE_GENERIC, tone_id, duration, frequency);
#else
...
PUBLIC MMISRV_HANDLE_T MMISRVAUD_StartVibrate(uint16 pri, uint16 duation, uint16 interval, uint16 times, MMISRVAUD_VIBRATE_CB callback)
{
    ...
    {
        SCI_TRACE_LOW("MMISRVAUD_StartVibrate  because CC in connected state, so send vibrate start to zsp and return 0");
        return 0;
    }
    handle = MMISRVAUD_GetHandleByName("VIBRATE SRV");
    if(handle > 0)
    {
        if(pri >= MMISRVAUD_GetAudPriorityInfo(handle))
        {
            MMISRVMGR_Free(handle);
        }
        else
...
//    req.is_auto_free = TRUE;
    ...
    audio_srv.interval=  0 ;  //  MMI_3SECONDS   interval  
    ...
    audio_srv.volume = MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_CALL);
	if(audio_srv.volume ==0)
	{
...
LOCAL MMISRV_HANDLE_T MMISRVAUD_StartVibrateWithAlarmVolume(uint16 pri, uint16 duation, uint16 interval, uint16 times, MMISRVAUD_VIBRATE_CB callback)
{
    MMISRV_HANDLE_T handle = 0;
    BOOLEAN res = FALSE;
    MMISRVMGR_SERVICE_REQ_T req = {0};
...
//	audio_srv.is_standing = TRUE;
    SCI_TRACE_LOW("MMISRVAUD_StartVibrateWithAlarmVolume  ..ring_buf.fmt=%d..data_len=%d ", audio_srv.info.ring_buf.fmt,audio_srv.info.ring_buf.data_len);
    handle = MMISRVMGR_Request(STR_SRV_AUD_NAME, &req, (void *)&audio_srv);
    if(handle > 0)
    {
        ARMVB_SetZspVibratePara_Direct(TRUE);
...
PUBLIC void MMIAPISET_SetVibratorWithAlarmVolume(
                                  BOOLEAN                 is_open,
                                  MMISET_ALL_RING_TYPE_E  ring_type,//in: the type of all vibrator
                                  BOOLEAN is_period, //是否间断不停的振动
                                  uint32 duration   //振动间隔
                                  )
{
    if (is_open)//打开振动
    {
        ...
        if(is_period)
        {
            s_vibrate_handle = MMISRVAUD_StartVibrateWithAlarmVolume(pri, duration, 1000, 0xFFFF, VibrateCallback);
        }
        else
        {
            s_vibrate_handle = MMISRVAUD_StartVibrateWithAlarmVolume(pri, duration, 0, 1, VibrateCallbackEx);
        }
        s_is_vibrate = is_open;
        s_is_playing_vibrate_type = ring_type; 
    }
                                   ...
                                   )
{
    ...
    case MMISET_CALL_RING:
        MMIAPICOM_StatusAreaSetSlientState(FALSE);
        MMIAPICOM_StatusAreaSetShakeState(FALSE);
        break;
    ...
    case MMISET_CALL_VIBRA_AND_RING:
        MMIAPICOM_StatusAreaSetSlientState(FALSE);
        MMIAPICOM_StatusAreaSetShakeState(TRUE);
        break;
        
    case MMISET_CALL_SILENT:
        MMIAPICOM_StatusAreaSetShakeState(FALSE);
        MMIAPICOM_StatusAreaSetSlientState(TRUE);
        break;
...
PUBLIC void MMISET_AppendNVCallRingCalssItem(MN_DUAL_SYS_E dual_sys, MMI_CTRL_ID_T  ctrl_id)
{
    ...
    //append item
    MMIAPISET_AppendListItemByTextId( TXT_RING_FIXED, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO );
    MMIAPISET_AppendListItemByTextId( TXT_COMMON_MORE_RING, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO );
...
#ifdef PHONE_NUMBER_VOICE
uint8          g_phone_ring_num[CC_MAX_TELE_NUM_LEN + 2]= {0};
uint16          g_phone_ring_num_len = 0;
uint16          g_phone_ring_cur_num = 0;
BOOLEAN          g_phone_ring_for_call = FALSE;
MN_DUAL_SYS_E g_phone_ring_cur_sys = MN_DUAL_SYS_1;

PUBLIC void MMISET_PhoneRingPlayResult(MMISRVAUD_REPORT_RESULT_E result, DPARAM param)
{
  ///  Trace_Log_Print("mmiset_ring:MMISET_PhoneRingPlayResult result=%d,param=%d,num = %d,len = %d", result, param,g_phone_ring_cur_num,g_phone_ring_num_len);

    if (MMISRVAUD_REPORT_RESULT_NOT_SUPPORT == result)
    {
        MMIPUB_OpenAlertWinByTextId(PNULL,TXT_COMMON_NO_SUPPORT,TXT_NULL,IMAGE_PUBWIN_FAIL,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
    }
    else if ((MMISRVAUD_REPORT_RESULT_ERROR == result))
    {
        MMIPUB_OpenAlertWinByTextId(PNULL,TXT_COMMON_NO_SUPPORT,TXT_NULL,IMAGE_PUBWIN_FAIL,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
    }
    else if(MMISRVAUD_REPORT_RESULT_SUCESS == result)
    {
        MMK_PostMsg(VIRTUAL_WIN_ID, MSG_START_PHONE_RING, PNULL, 0);
    }
...
PUBLIC void  MMIAPISET_PlayPhoneRing(uint8 n_char,MMISET_RING_CALLBACK_PFUNC callback_func)
{
	   ...
	   g_is_play_menu = FALSE;
        ...
        else if(n_char == 'p' || n_char == 'P')
        {
            num = 0xD;
        }
        else if(n_char == 'w' || n_char == 'W' )
        {
            num = 0xF;
        }
        else if(n_char == '+')
        ...
        {
            num++;
            g_phone_ring_for_call = TRUE;
			///Trace_Log_Buf_Print("MMIAPISET_PlayRing00011");
			if (!MMIAPICC_IsInState(CC_IN_CALL_STATE) || !MMIAPIPHONE_GetPowerAnimMpeg4Flag())
			{
				MMIAPISET_SetCurRingType(MMISET_RING_TYPE_PHONENUM);
			}
		     if(MMIAPIMP3_IsPlayerActive())	  
		{
			return;
		}
           MMIAPISET_PlayRing(g_phone_ring_cur_sys, FALSE,num, 1, MMISET_RING_TYPE_PHONENUM, callback_func);
       }

}

PUBLIC BOOLEAN  MMIAPISET_PlayDialRing(uint8 n_char)
{
 // if(0 == MMIAPIENVSET_GetActiveModeOptValue(MN_DUAL_SYS_1, DIAL_RING_TYPE))
  uint8 active_mode = MMIAPIENVSET_GetActiveModeId();
   // if(0 == MMIAPIENVSET_GetActiveModeOptValue(MN_DUAL_SYS_1, DIAL_RING_TYPE))
	...
	if(MMIAPIALM_IsAlarmActive())
	{
		return TRUE;
	}
	
#ifndef TTS_SUPPORT_NONE
	 if((ZDT_GetDialSwitch_type() == 1)||( 1 == ZDT_GetTTSSwitch_type()))
	 {
		return TRUE;
	 }
	#endif 
		...
		{	// 屏蔽默认按键音
			...
			#if defined(HERO_ENGINE_TTS_SWTICH_SUPPORT)
			extern BOOLEAN hero_tts_master_switch_is_open(void);
			#endif
			if(hero_tts_dial_broadcast_is_open() == FALSE)
			{
				return TRUE;
			}
                     #if defined(HERO_ENGINE_TTS_SWTICH_SUPPORT)
	...
	///if(0 == MMIAPISET_GetDIALVOICE_type())
   {
    // Trace_Log_Buf_Print("MMIAPISET_PlayDialRing bn_char =%d",n_char);
		...
		s_num_char = n_char;
	 	...
	 	Play_TTS_Ring_TmrStart();   ////ZDT_PRODUCT_Z605
	 	#else
        MMIAPISET_PlayPhoneRing(n_char,PNULL);
       #endif
        return TRUE;
    }
    return FALSE;
...
PUBLIC void  MMIAPISET_PlayMenuRing(uint8 num,MMISET_RING_CALLBACK_PFUNC callback_func)
{
      // uint8 num = 0xff;
       //Trace_Log_Print("mmiset_ring:MMISet_PlayPhoneRing n_char=0X%02x", n_char);
...
PUBLIC BOOLEAN  MMIPlayMenuRing(uint8 num)
{
 // if(0 == MMIAPIENVSET_GetActiveModeOptValue(MN_DUAL_SYS_1, DIAL_RING_TYPE))
  uint8 active_mode = MMIAPIENVSET_GetActiveModeId();
   // if(0 == MMIAPIENVSET_GetActiveModeOptValue(MN_DUAL_SYS_1, DIAL_RING_TYPE))
...
#ifdef HERO_ENGINE_TTS_SUPPORT
	{	// 屏蔽默认按键音
		extern BOOLEAN hero_tts_menu_broadcast_is_open(void);
		#if defined(HERO_ENGINE_TTS_ALL_SUPPORT)
		extern BOOLEAN hero_tts_master_switch_is_open(void);
		#endif
		if(hero_tts_menu_broadcast_is_open() == FALSE)
		{
			return TRUE;
		}
		#if defined(HERO_ENGINE_TTS_ALL_SUPPORT)
    ...
    {    
        MMIAPISET_PlayMenuRing(num,PNULL);
        return TRUE;
    }
    return FALSE;
}
...
PUBLIC BOOLEAN  MMISet_PlayNextPhoneRing(void)
{       
        BOOLEAN result = TRUE;
        uint8  group_id;
        
    ///   Trace_Log_Print("mmiset_ring:MMISet_PlayNextPhoneRing num=%d,len = %d", g_phone_ring_cur_num,g_phone_ring_num_len);
	if(g_phone_ring_cur_num < g_phone_ring_num_len)
	{
	 	g_cc_ring_play_num = TRUE;
		MMIAPISET_PlayPhoneRing(g_phone_ring_num[g_phone_ring_cur_num++],MMISET_PhoneRingPlayResult);
	}
	else if(g_phone_ring_num_len != 0 && g_phone_ring_cur_num == g_phone_ring_num_len)
	{
		g_cc_ring_play_num = FALSE;
		g_phone_ring_cur_num = 0;
#if 0
		MMISet_PlayPhoneRing('c',MMISET_PhoneRingPlayResult);
#else
		MMIAPISET_PlayRing(g_phone_ring_cur_sys,FALSE,1,AUD_PLAY_FOREVER,MMISET_RING_TYPE_CALL,PNULL);
#endif
	}
	else
	{
		g_cc_ring_play_num = FALSE;
	    g_phone_ring_cur_num = 0;
	    g_phone_ring_num_len = 0;
	    result = FALSE;
	}
	return result;
}

PUBLIC void  MMISet_PlayPhoneRingALL(MN_DUAL_SYS_E dual_sys,uint8 * tel_num,uint8 tel_num_len)
{
   /// Trace_Log_Print("mmiset_ring:MMISet_PlayPhoneRingALL num_len = %d", tel_num_len);
#if 0	
    if(address->num_len != 0)
    {
        SCI_MEMSET(g_phone_ring_num,0,CC_MAX_TELE_NUM_LEN + 2);
        g_phone_ring_num_len = MMIAPICOM_GenDispNumber( address->number_type,
                                                                                        address->num_len,
                                                                                        (uint8*)address->party_num,
                                                                                        g_phone_ring_num,
                                                                                        CC_MAX_TELE_NUM_LEN + 2);
        g_phone_ring_cur_num = 0;
        g_phone_ring_cur_sys = dual_sys;

        MMISet_PlayNextPhoneRing();
    }
    else
    {
   ...
   {
	   SCI_MEMSET(g_phone_ring_num,0,CC_MAX_TELE_NUM_LEN + 2);
	   strcpy(g_phone_ring_num,tel_num);
	   g_phone_ring_num_len = strlen(g_phone_ring_num);
	   g_phone_ring_cur_num = 0;
	   g_phone_ring_cur_sys = dual_sys;
...
PUBLIC void  MMISet_StopPhoneRingALL(MN_DUAL_SYS_E dual_sys)
{
    MMIAPISET_StopRing(MMISET_RING_TYPE_PHONENUM);
    g_phone_ring_cur_num = 0;
    g_phone_ring_num_len = 0;
    g_phone_ring_cur_sys = dual_sys;
}
...
PUBLIC void MMISET_PreTimePlayResult(MMISRVAUD_REPORT_RESULT_E result, DPARAM param)
{
	//if(FALSE == g_is_time_menu)
	//{
	//	return;
	//}
...
PUBLIC void MMISETAPM_PreTimePlayResult(MMISRVAUD_REPORT_RESULT_E result, DPARAM param)
{
    ...
    {
    	MMIAPISET_PlayRing(g_phone_ring_cur_sys, FALSE,21, 1, MMISET_RING_TYPE_PHONENUM, MMISET_PreTimePlayResult);
    }
    
}


PUBLIC void MMISETBJTIME_PreTimePlayResult(MMISRVAUD_REPORT_RESULT_E result, DPARAM param)
{
    ...
    {
    	MMIAPISET_PlayRing(g_phone_ring_cur_sys, FALSE,20, 1, MMISET_RING_TYPE_PHONENUM, MMISETAPM_PreTimePlayResult);
    }
    
}

PUBLIC void MMISETNOW_PreTimePlayResult(MMISRVAUD_REPORT_RESULT_E result, DPARAM param)
{
	if(FALSE == g_is_time_menu)
	{
		return;
    ...
    {
    	MMIAPISET_PlayRing(g_phone_ring_cur_sys, FALSE,19, 1, MMISET_RING_TYPE_PHONENUM, MMISETBJTIME_PreTimePlayResult);
    }
    
}


PUBLIC void MMISETBH_PreTimePlayResult(MMISRVAUD_REPORT_RESULT_E result, DPARAM param)
{
	if(FALSE == g_is_time_menu)
	{
		return;
    ...
    {
    	MMIAPISET_PlayRing(g_phone_ring_cur_sys, FALSE,18, 1, MMISET_RING_TYPE_PHONENUM, MMISETNOW_PreTimePlayResult);
    }
    
}

PUBLIC void  MMI_PlayPreTimeRing(MMISET_RING_CALLBACK_PFUNC callback_func)
{
 
       
     //   g_phone_ring_for_call = TRUE;
       MMIAPISET_PlayRing(g_phone_ring_cur_sys, FALSE,17, 1, MMISET_RING_TYPE_PHONENUM, callback_func);

}

PUBLIC BOOLEAN  MMIAPISET_PlayPreTimeRing(void)
{
   ...
   // if(0 == MMIAPIENVSET_GetActiveModeOptValue(MN_DUAL_SYS_1, DIAL_RING_TYPE))
	...
	//if(0 == MMIAPISET_GetHourCode_type())
	{
	  g_is_play_menu = FALSE;
	//	g_is_time_menu = TRUE ;

 	//MMI_PlayPreTimeRing(MMISET_PreTimePlayResult);
		MMIAPISET_PlayRing(g_phone_ring_cur_sys, FALSE,17, 1, MMISET_RING_TYPE_PHONENUM, MMISET_PreTimePlayResult);
...
#endif
#if defined(MMI_MP3_FIXED_MUSIC)
PUBLIC void MMIMP3_PlayFixedMusic(void)
{
        //停掉所有铃声和振动
   //   if (!MMIAUDIO_GetIsPlaying())

    // if(!MMIAPISET_IsPlayingRingType(MMISET_RING_TYPE_OTHER))	  
        {
#ifndef MMI_VIBRATE_DISABLE //Modify By LSG 2010-01-19        
        //	StopAllVibrator();
#endif			
        //	MMIAPISET_StopAllRing(TRUE);
       	//MMIAUDIO_PauseBgPlay(MMIBGPLAY_TYPE_ALL,MMIBGPLAY_MODULE_SELFROM_MYDOC);

		if (!MMIAPICC_IsInState(CC_IN_CALL_STATE) || !MMIAPIPHONE_GetPowerAnimMpeg4Flag())
		{
			MMIAPISET_SetCurRingType(MMISET_RING_TYPE_FIXED_MUSIC);
		}
		
	   MMIAPISET_PlayRing(MN_DUAL_SYS_1, FALSE,1, 1, MMISET_RING_TYPE_FIXED_MUSIC, MMIMP3_FixedMusicCallBack);

		
		//PlayFixedRing(1,MMISET_RING_TYPE_FIXED_MUSIC,  0xffff,MMIAPISET_GetMultimVolume(), MMIMP3_FixedMusicCallBack);
		//PlayFixedRing(1,MMISET_RING_TYPE_FIXED_MUSIC,  0xffff,MMIAPISET_GetMultimVolume(), MMIMP3_FixedMusicCallBack);
	//	MMIAPISET_PlayRing(MN_DUAL_SYS_1,FALSE,0,0xffff,MMISET_RING_TYPE_FIXED_MUSIC,PNULL);
        }
}
#endif
...
PUBLIC BOOLEAN  MMIAPI_PlaySOSAlarm(void)
{
   // if(0 == MMIAPIENVSET_GetActiveModeOptValue(MN_DUAL_SYS_1, DIAL_RING_TYPE))
	if(0 == MMIAPIGET_SosCall_Status())
   {
     	MMIAPISET_PlayRing(MN_DUAL_SYS_1,FALSE,1,1,MMISET_RING_TYPE_SOSALARM,PNULL);
        return TRUE;
    }
    return FALSE;
}
