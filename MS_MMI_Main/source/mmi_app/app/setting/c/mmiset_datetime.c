...
//tangting add
//#define MMISET_DATE_TIME_UNISOC_SUPPORT
...
//  Note: 
/*****************************************************************************/
LOCAL void setTimeZoneParam(void)
{
    GUILIST_ITEM_T      item_t      =   {0};/*lint !e64*/
    GUILIST_ITEM_DATA_T item_data   =   {0};/*lint !e64*/
    MMI_STRING_T string_to={0};
    MMI_STRING_T string_city={0};
    MMI_STRING_T string_dst = {0};
    wchar blank_wchar[] = {0x20,0x00};

    GUIFORM_SetType(MMISET_SET_DATE_TIME_CHILD_FORM_CTRL_ID,GUIFORM_TYPE_TP);
    GUILIST_SetMaxItem(MMISET_SET_TIME_ZONE_CTRL_ID, 1, FALSE);
    ...
    item_data.item_content[0].item_data.text_id = STR_PRMPT_TIME_ZONE;

#if defined WORLD_CLOCK_SUPPORT
    MMI_GetLabelTextByLang(MMIAPIACC_GetLocalTimeZoneTextID(), &string_to );
#else
    //MMI_GetLabelTextByLang(MMIAPIACC_GetLocalCityStringID(), &string_city );
    //string_dst.wstr_ptr = SCI_ALLOCAZ((string_to.wstr_len + string_city.wstr_len + 2)*sizeof(wchar));
    //MMIAPICOM_Wstrcat(string_dst.wstr_ptr, string_to.wstr_ptr);
    // MMIAPICOM_Wstrncat(string_dst.wstr_ptr, blank_wchar,1);
    // MMIAPICOM_Wstrcat(string_dst.wstr_ptr, string_city.wstr_ptr);
    //string_dst.wstr_len=MMIAPICOM_Wstrlen(string_dst.wstr_ptr);
#endif

    item_data.item_content[1].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
    // item_data.item_content[1].item_data.text_buffer.wstr_len = string_dst.wstr_len;
    //item_data.item_content[1].item_data.text_buffer.wstr_ptr = string_dst.wstr_ptr;
    item_data.item_content[1].item_data.text_buffer = string_to;
	//modify by nhl for 494439
    item_data.item_content[1].is_opacity = TRUE;
//     GUILIST_SetItemContentOpacityFontColor(&item_data,1,255*0.6,255*0.6);
	//modify end
    GUILIST_AppendItem( MMISET_SET_TIME_ZONE_CTRL_ID, &item_t );
    GUILIST_SetListState(MMISET_SET_TIME_ZONE_CTRL_ID, GUILIST_STATE_SPLIT_LINE, FALSE);

    //SCI_FREE(string_dst.wstr_ptr);
	//string_dst.wstr_ptr = PNULL;
...
LOCAL void UpdateStbDispTime(BOOLEAN is_enter_from_setting)
{
    ...
    if(MMIAPICOM_StatusAreaSetDispTimeinfo(time_wchar_buf,len))
    {
        GUIWIN_UpdateStb();
    }
}
...
LOCAL void SaveTimeDatesetting(void)
{
    ...
    //设置时间格式
    #ifndef MMISET_DATE_TIME_UNISOC_SUPPORT
...
LOCAL void SetAutLabelParam(void)
{
...
#else    
    {
        ...
        MMI_GetLabelTextByLang(STR_ILIST_DT_AUTO_UPDATE_EXT01, &aut_displayname);
        GUILABEL_SetText(MMISET_AUTO_UPDATE_TIME_LABEL_CTRL_ID, &aut_displayname, FALSE);
        //text 1
        ...
        {
            MMI_GetLabelTextByLang(STR_POC_ILIST_OFF, &text_str);
        }
                                               ...
                                               )
{
    ...
    case MSG_APP_WEB:
        ...
        switch (menu_id)
        {
            case ID_SET_AUTO_UPDATE_TIME:/* Start Auto update time */
    #if !defined MMI_GUI_STYLE_TYPICAL
    ...
    #endif
                break;
            case ID_SET_UPDATE_SYSTEM_TIME:/* Start Manual update time */
				SCI_TRACE_LOW("[mmiset_datetime.c] ID_SET_UPDATE_SYSTEM_TIME Start Manual update time");
...
LOCAL void MMISET_OpenAutoUpdateTimeQueryWin(BOOLEAN is_auto_update_time)
{
    ...
    {
        MMIAPISET_SetIsAutoUpdateTime(TRUE);
                                            ...
                                            )
{
    ...
    uint32                   create_value = (uint32)MMK_GetWinUserData(win_id);
    switch(msg_id)
    {
    case MSG_OPEN_WINDOW:
...
//#endif
    ...
    case MSG_CLOSE_WINDOW:
        ...
        //add for bug516094
     case MSG_KEYUP_RED:
           SCI_TRACE_LOW("HandleSetTimeDateWindow  create_value=%d",create_value);
            if(1 == create_value)
            {
                MMK_CloseWin(win_id);
            }
...
{
    //niuhuiling modify Bug 490712 now there is only in first power on or in settings can set time so delete other operator
...
//  Description :
//  Global resource dependence : 
//  Author:zhenxing modify by nhl for interface MMISET_SET_DATE_TIME_LIST_WIN_TAB
//  Note: 
/*****************************************************************************/
PUBLIC void MMISET_CreatePowerOnDateTimeWin(void)
{
    MMI_HANDLE_T      win_handle = PNULL;
    uint32         value  = 0;
//     win_handle = MMK_CreateWin((uint32*)MMISET_SET_DATE_TIME_LIST_WIN_TAB, PNULL);
    if(PNULL != win_handle)
      {
          value = 1;
      }
     MMK_SetWinUserData(win_handle, (ADD_DATA)value);    
}
