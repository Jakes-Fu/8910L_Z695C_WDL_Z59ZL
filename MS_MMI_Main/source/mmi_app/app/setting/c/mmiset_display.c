...
#define MMISET_AM_PM_LEN_MAX                20 //多国语言对应的Unicode长度不确定，最大长度暂设为20
//#define HW_LCD_TEST
...
static MMISET_FONT_SIZE_T      g_font_size_info[3]   =   {0};
static uint8                        s_cur_font_size = 0;
static uint32                       s_real_contrast     =   0;	//real lcd contrast
static MMISET_IDLE_DISPLAY_T       s_idle_display_info  =   {0};	//idle display info

LOCAL MMISET_IDLE_STYLE_E       s_mmiset_idle_style     =   MMISET_IDLE_STYLE_COMMON;//MMISET_IDLE_STYLE_SMART;
...
//2015-7-9 tangting add
LOCAL void InitFontSizeInfo(void);
...
{
    uint8                           mainlcd_contrast        =   MMISET_EDEFAULT_CONTRAST;//MMISET_CONTRAST_FIVE; //change by James li 
#ifdef TTS_SUPPORT
    ...
    if (MN_RETURN_SUCCESS != return_value)
    {
        is_tts_on = FALSE;//TRUE//change by James li
...
PUBLIC void MMISET_ResetIdleDisplayMode(void)
{
	...
	s_idle_display_info.visible_flag.item_flag.is_show_mp3 = TRUE;
#endif
#ifdef FM_SUPPORT
	s_idle_display_info.visible_flag.item_flag.is_show_fm = TRUE;
#endif
...
PUBLIC void RestoreTimeFormatType(void)
{
    uint32 timeformat_type;
    NVITEM_ERROR_E nv_reslut = NVERR_NONE; 
    nv_reslut = EFS_NvitemRead(SETTINGS_NV_TIME_FORMAT_TYPE, (uint16)sizeof(timeformat_type),&timeformat_type);
	
    if (NVERR_NONE == nv_reslut)
    {
        if(timeformat_type==1)
        {
            s_time_display_type = MMISET_TIME_24HOURS;
        }
        else
...
LOCAL void InitIdleStyle(void)
{
    ...
    s_mmiset_idle_style = MMISET_IDLE_STYLE_COMMON;//MMISET_IDLE_MSTYLE;
    s_mmiset_idle_substyle = MMISET_IDLE_SUBSTYLE_DEFAULT;
...
LOCAL void InitBacklightInfo(void)
{
    ...
    g_backlight_info.backlight_time = backlight_info.backlight_time;
}
//2015-7-9 tangting add 
/*****************************************************************************/
// 	Description : init the information about font size set
//	Global resource dependence : none//	Note:
/*****************************************************************************/
LOCAL void InitFontSizeInfo(void)
{
    MMISET_FONT_SIZE_T      font_info[3]  =   {0};
    g_font_size_info[MMISET_FONT_SIZE_CONTACTS].font_size_index = 0;
    g_font_size_info[MMISET_FONT_SIZE_MENU].font_size_index = 0;
    g_font_size_info[MMISET_FONT_SIZE_MESSAGING].font_size_index = 0;        
}
...
LOCAL void InitFontSize(void)
{
    ...
    MMINV_READ(MMINV_SET_FONT_SIZE,&font_size,return_value);
    if (MN_RETURN_SUCCESS != return_value)
    {
        //set initial value
        font_size = 0;
        
        MMINV_WRITE(MMINV_SET_FONT_SIZE,&font_size);
    }
    s_cur_font_size = font_size;
}
/*****************************************************************************/
// 	Description : init the information about wallpaper set
//	Global resource dependence : none
    ...
    if(PNULL == wallpaper_ptr)
   {
    ...
    if (wallpaper_ptr->wallpaper_type >= MMISET_WALLPAPER_MAX || !return_value )
    {
...
#endif
        {
            ...
            {
                wallpaper_ptr->wallpaper_id = MMISET_ORTHODOX_WALLPAPER_DEFAULT_ID;
            }
        }
...
//2015-7-9 tangting add
PUBLIC void MMIAPISET_GetFontSizeInfo(
                                        MMISET_FONT_SIZE_TYPE_E  fontsize_type,
                                        MMISET_FONT_SIZE_T *fontsize_ptr
                                        )
{
    if(PNULL == fontsize_ptr)
   {
   	//SCI_TRACE_LOW:"mmisetting_assert MMIAPISET_GetBackLightInfo"
   	SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMISET_DISPLAY_742_112_2_18_2_50_52_139,(uint8*)"");
	return;
    ...
    //get the information about back light info
    *fontsize_ptr = g_font_size_info[fontsize_type];

}

PUBLIC void MMIAPISET_SetFontSizeInfo(
                                        MMISET_FONT_SIZE_TYPE_E  fontsize_type,
                                        MMISET_FONT_SIZE_T fontsize_ptr
                                        )
{
    //to do
    //get the information about back light info
    g_font_size_info[fontsize_type] = fontsize_ptr;

}

PUBLIC uint8 MMIAPISET_GetCurrentFontSize(void)
{
    return s_cur_font_size;
}
/*****************************************************************************/
                                                 ...
                                                 )
{
    ...
    MMIAPISET_AppendListItemByTextId( TXT_COMM_12_HOURS, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO );
    MMIAPISET_AppendListItemByTextId( TXT_COMM_24_HOURS, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO );
...
MMISET_ANIMATE_SET_T        animate_info    = {0};
  ...
  SCI_MEMSET(&animate_info, 0, sizeof(MMISET_ANIMATE_SET_T));
    ...
    //add items to listbox
    MMIAPISET_AppendListItemByTextId(TXT_SET_ANIMATION_1,STXT_SOFTKEY_OK_MK,ctrl_id);
    MMIAPISET_AppendListItemByTextId(TXT_SET_ANIMATION_2,STXT_SOFTKEY_OK_MK,ctrl_id);
    MMIAPISET_AppendListItemByTextId(TXT_SET_ANIMATION_3,STXT_SOFTKEY_OK_MK,ctrl_id);
...
PUBLIC BOOLEAN MMIAPISET_GetTTSInfo( void )
{
    ...
    if (MN_RETURN_SUCCESS != return_value)
    {
        is_tts_open =FALSE;//TRUE//change by James li
                                                 ...
                                                 )
{
    ...
    MMIAPISET_AppendListItemByTextIdExt(TXT_EXPORT_LOC_DEFAULT,STXT_SOFTKEY_OK_MK, STXT_PREVIEW, STXT_RETURN, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO  );
    ...
    //MMIAPISET_AppendListItemByTextId(TXT_SET_WALLPAPER_MORE, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO); 
                                                 ...
                                                 )
{
    ...
    MMIAPISET_AppendListItemByTextIdExt(TXT_SET_WALLPAPER_MORE,TXT_COMMON_OK, PNULL, STXT_RETURN, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
    //展开anim的list
    //MMISET_AppendTwoLineAnimListItem(ctrl_id,STXT_SOFTKEY_OK_MK, STXT_PREVIEW, STXT_RETURN);
                                            ...
                                            )
{
    ...
    MMIAPISET_AppendListItemByTextId( TXT_COMM_10SECONDS, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO );
...
//2015-7-9 tangting modify blacklight func
/*****************************************************************************/
// 	Description :
//	Global resource dependence : 
//  Author: Jassmine
/*****************************************************************************/
PUBLIC void MMIAPISET_Intensity(
                                    uint16 intensity
                                    )
{
    s_cur_contrast = intensity;
    MMIAPISET_UpdateLCDContrast(GUI_MAIN_LCD_ID,s_cur_contrast);
}
/*****************************************************************************/
...
PUBLIC uint8 MMIAPISET_GetFirstDayOfWeek( void )
{
    ...
    // Added by sunhongzhe to determin 1st day of week with sim mcc.
    sim_num = MMIAPIPHONE_GetSimExistedNum(&sim_sys, 1);
    if (sim_num == 0)
    {
        return first_day_of_week;
    }

    MMIAPIPHONE_GetSimHPLMN(&curr_plmn,  sim_sys);
    switch (curr_plmn.mcc)
    {
    case 631:// Angola
    case 722:// Argentina
    case 283:// Armenia
    ...
    case 630:// Dem. Rep. of Congo
    case 238:// Denmark
    case 638:// Djibouti
    case 366:// Dominica
    case 370:// Dominican Republic
    case 740:// Ecuador
    case 706:// El Salvador
    case 248:// Estonia
    case 636:// Ethiopia
    case 542:// Fiji
    case 244:// Finland
    ...
    case 250:// Russia
    case 635:// Rwanda
    case 626:// S?o Tomé and Príncipe
    case 608:// Senegal
    case 220:// Serbia
    case 633:// Seychelles
    case 619:// Sierra Leone
    case 525:// Singapore
    case 231:// Slovakia
    case 293:// Slovenia
    ...
    case 641:// Uganda
    case 255:// Ukraine
    case 234:// United Kingdom
    case 748:// Uruguay
    case 434:// Uzbekistan
    case 734:// Venezuela
    //case 452:// Vietnam  modifed by bug1079024
    case 645:// Zambia
    case 648:// Zimbabwe
    case 242:// Norway
        first_day_of_week = 1; // Monday.
        break;
    case 412: // Afghanistan
    case 432: // Iran
    ...
    SCI_TRACE_LOW("MMIAPISET_GetFirstDayOfWeek: curr_plmn.mcc = %d, first_day_of_week = %d", curr_plmn.mcc, first_day_of_week);
    return first_day_of_week;
}
                                               ...
                                               )
{
    ...
    MMIAPISET_AppendListItemByTextId(TXT_COMM_FADE_INOUT, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO );
...
#ifdef MMI_MSTYLE_SUPPORT
    MMISET_SetIdleStyle(MMISET_IDLE_MSTYLE);
#endif
...
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPISET_FormatDateStrByDateDisplyType(
                                            uint16 year, //in
                                            uint8 month, //in
                                            uint8 date, //in
                                            char symbol, //in
                                            uint8* date_str,//in/out
                                            uint16 date_str_max_len,//in
                                            MMISET_DATE_DISPLAY_TYPE_E date_display_type//in
                                            )
{
    BOOLEAN result = FALSE;
    char sym_str[2] = {0};
...
LOCAL void GeMonthStringFromIndex(uint8 month_index, uint8* str_buff)
{
                                                        ...
                                                        )
{
    ...
    date_display_type = MMIAPISET_GetDateDisplayType();
    SCI_MEMSET(date_str,0,date_str_max_len);
    GeMonthStringFromIndex(month-1, str_month);
    ...
    case MMISET_DATE_YMD:
        if (is_display_year)
        {
            sprintf((char*) date_str, "%04d%s%s%s%02d", year, sym_str, str_month, sym_str, date);
        }
        else
        {
            sprintf((char*) date_str, "%s%s%02d", str_month, sym_str, date);
        }
        result = TRUE;
        break;
    case MMISET_DATE_MDY:
        if (is_display_year)
        {
            sprintf((char*) date_str, "%s%s%02d%s%04d", str_month, sym_str, date, sym_str, year);
        }
        else
        {
            sprintf((char*) date_str, "%s%s%02d", str_month, sym_str, date);
    ...
    case MMISET_DATE_DMY:
        if (is_display_year)
        {
            sprintf((char*) date_str, "%02d%s%s%s%04d", date, sym_str, str_month, sym_str, year);
        }
        else
        {
            sprintf((char*) date_str, "%02d%s%s", date, sym_str, str_month);
        }
        result = TRUE;
        break;
    default:
...
PUBLIC BOOLEAN MMIAPISET_FormatDateStrForSMS(
                                            ...
                                            )
 {
    ...
    if(!(1 <= month && 12 >= month))
    {     
        SCI_TRACE_LOW("Waring, MMIAPISET_FormatDateStrForSMS: month = %d",month);
        //return result;
    }
    if(!(1 <= date && 31 >= date))
    {
        SCI_TRACE_LOW("Waring, MMIAPISET_FormatDateStrForSMS: date = %d",date);
        //return result;
    }
    if (PNULL ==date_str|| date_str_max_len < MMISET_DATE_STR_LEN)
    {
        SCI_TRACE_LOW("Waring, MMIAPISET_FormatDateStrForSMS: date = %d,min len = % d",date_str_max_len,MMISET_DATE_STR_LEN);  
        return result;
    }
    
    SCI_MEMSET(date_str,0,date_str_max_len);
    //显示的格式默认为MMISET_DATE_DMY
    if (is_display_year)
    {
        sprintf((char*) date_str, "%02d%s%02d%s%04d", date, sym_str, month, sym_str, year);
    }
					  ...
					  )
{
    ...
    MMI_STRING_T am_text = {0};
    MMI_STRING_T pm_text = {0};
    int i = 0;
    uint8 am_temp[MMISET_AM_PM_LEN_MAX] = {0} ;
    uint8 pm_temp[MMISET_AM_PM_LEN_MAX] = {0} ;
    ...
    case MMISET_TIME_12HOURS:
        {
            ...
            {
                MMI_GetLabelTextByLang(STR_CL_TIME_AM, &am_text); //get AM multilanguage

                for( i = 0;i<am_text.wstr_len;i++)  //convert wchar to uint8
                {
                    am_temp[i]=(uint8)am_text.wstr_ptr[i];
                }

                if ((MMISET_TIME_1_HOUR> hour))//00:00-1:00
                {
                    sprintf((char*)timestr,"%02d:%02d%s",(hour + MMISET_TIME_PM_HOUR),min,am_temp);
                }
                else
                {
                    sprintf((char*)timestr,"%02d:%02d%s",hour,min,am_temp);
                }
            }
            else//PM
            {
                MMI_GetLabelTextByLang(STR_CL_TIME_PM, &pm_text); //get PM multilanguage
                for( i = 0;i<pm_text.wstr_len;i++) //convert wchar to uint8
                {
                    pm_temp[i]=(uint8)pm_text.wstr_ptr[i];
                }

                if ((MMISET_TIME_PM_HOUR <= hour) && (MMISET_TIME_13_HOUR > hour))//12:00-13:00
                {
                    sprintf((char*)timestr, "%02d:%02d%s", hour, min, pm_temp);
                }
                else
                {
                    sprintf((char*)timestr, "%02d:%02d%s", (hour - MMISET_TIME_PM_HOUR), min, pm_temp);
                }
            }
            result = TRUE;
        }
