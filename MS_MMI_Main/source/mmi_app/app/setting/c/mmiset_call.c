...
#define TXT_NOT_PROVISION STR_STATUS_NOT_PROVIDED_EXT01
#define TXT_ERROR STR_OPERATION_FAIL_EXT01
/**--------------------------------------------------------------------------*
**                         STATIC DEFINITION                                *
**--------------------------------------------------------------------------*/
...
extern uint32  s_call_divert_conditions;
extern SET_ID_MENU_E s_call_barr_menu_id;

//add by ljg for voice mail
#define MMISET_VOICEMAIL_LINE1  1
#define MMISET_VOICEMAIL_LINE2  2
#define MMISET_VOICEMAIL_FAX     3
#define MMISET_VOICEMAIL_DATA   4
BOOLEAN	 s_is_vmn_from_sim1 = FALSE;
BOOLEAN	 s_is_vmn_from_sim2 = FALSE;
BOOLEAN	 s_is_vmn_first_querysim1 = TRUE;
BOOLEAN	 s_is_vmn_first_querysim2 = TRUE;
MMI_PARTY_NUMBER_T g_sim_vmnum_info;
MMI_PARTY_NUMBER_T g_sim2_vmnum_info;
MMI_PARTY_NUMBER_T g_temp_sim_vmnum_info = {0};
MMI_ALPHA_ID_T g_temp_sim_alpha_info = {0};

MMI_ALPHA_ID_T g_sim_alpha_info;
MMI_ALPHA_ID_T g_sim2_alpha_info;
extern BOOLEAN s_is_invalid_cnf_msg;
extern BOOLEAN s_is_invalid_cnf2_msg;


LOCAL MMISET_VMN_OPER_TYPE_E g_vmn_oper_from_type = 0;
//end
LOCAL MN_SS_REGISTER_PARAM_U g_register_param = {0};/*lint !e64*/// parameter's union of register ss data operation
LOCAL SIMAT_DO_SS_STRING_T g_ss_string = {0};
LOCAL BOOLEAN g_send_ss =FALSE;
BOOLEAN  g_is_ss_handling= FALSE;
#ifdef MMI_CSP_SUPPORT
...
LOCAL void GenInterrogateServiceReport(MMI_STRING_T *name_str);
...
LOCAL void GenInterrogateStatusReport(
                                      uint8 list_num,
                                      MN_SS_INTERROGATE_INFO_T    *info_list_ptr,
                                      MMI_STRING_T *status_str,
                                      MMI_STRING_T *status_str2);
...
//    g_mmiset_app.component_type = CT_APPLICATION;
	...
	MMIAPISET_GetForwardConditions();
	MMIAPISET_GetSendCallLd();
	MMIAPISET_GetForwardSimCards();
...
{
    MN_CALL_CLIR_E clir_type = MN_CALL_CLIR_NETWORK_DECIDE;/*lint !e64*/
    MN_RETURN_RESULT_E return_value = MN_RETURN_FAILURE;

    SCI_ASSERT(dual_sys < MMI_DUAL_SYS_MAX);/* assert to do*/
    if(MN_DUAL_SYS_1 == dual_sys){
        MMINV_READ(MMINV_SET_CLIR_TYPE, &clir_type, return_value);
    }else{
        MMINV_READ(MMINV_SET_CLIR_TYPE_SIM2, &clir_type, return_value);	
    }
    if (MN_RETURN_SUCCESS != return_value)
    {
    	if(MN_DUAL_SYS_1 == dual_sys){
            MMINV_WRITE(MMINV_SET_CLIR_TYPE, &clir_type);
        }else{
            MMINV_WRITE(MMINV_SET_CLIR_TYPE_SIM2, &clir_type);
        }
    }
    return clir_type;
}
...
PUBLIC void MMISET_StartClirReq(MN_DUAL_SYS_E dual_sys, MN_CALL_CLIR_E clir_type)
{
    ...
    g_mmiset_ss_status.ss_code = MN_SS_CLIR;
    g_mmiset_ss_status.basic_service.service_type = MN_SS_ALL_SERVICE;
    g_mmiset_ss_status.dual_sys = dual_sys;

    if (clir_type == MN_CALL_CLIR_SUPPRESSION)
    {
        //activate ss
        SCI_TRACE_LOW("MMISET_StartClirReq:active g_mmiset_ss_status.ss_code =%d ",g_mmiset_ss_status.ss_code);
    ...
    else if (clir_type == MN_CALL_CLIR_INVOCATION)
    {
        //deactivate ss
        SCI_TRACE_LOW("MMISET_StartClirReq: DEACTIVATESS g_mmiset_ss_status.ss_code =%d ",g_mmiset_ss_status.ss_code);
...
//    Note:
/*****************************************************************************/
PUBLIC void MMISET_SetVmOperType(MMISET_VMN_OPER_TYPE_E type)
{
    g_vmn_oper_from_type = type;
}
...
//    Note:
/*****************************************************************************/
PUBLIC MMISET_VMN_OPER_TYPE_E MMISET_GetVmOperType(void)
{
    return g_vmn_oper_from_type;
}

PUBLIC void MMISET_SetIsVMFromSIM(MN_DUAL_SYS_E dual_sys, BOOLEAN is_fromsim)
{
    if (MN_DUAL_SYS_1 == dual_sys)
    {
        s_is_vmn_from_sim1 = is_fromsim;
    }
    else if (MN_DUAL_SYS_2 == dual_sys)
    {
        s_is_vmn_from_sim2 = is_fromsim;
    }
    else
    {
        SCI_TRACE_LOW("[szp] MMISET_SetIsVMFromSIM Error sim number!!! dual_sys: %d",dual_sys);
    }

    return;
}

PUBLIC BOOLEAN MMISET_GetIsVMFromSIM(MN_DUAL_SYS_E dual_sys)
{
    if (MN_DUAL_SYS_1 == dual_sys)
    {
        return s_is_vmn_from_sim1;
    }
    else if (MN_DUAL_SYS_2 == dual_sys)
    {
        return s_is_vmn_from_sim2;
    }
    else
    {
        SCI_TRACE_LOW("[szp]MMISET_GetIsVMFromSIM Error sim number!!! dual_sys: %d",dual_sys);
        return FALSE;
    }
}

PUBLIC void MMISET_SetIsVMFirstQueryFromSIM(MN_DUAL_SYS_E dual_sys, BOOLEAN is_first_query)
{
    if (MN_DUAL_SYS_1 == dual_sys)
    {
        s_is_vmn_first_querysim1 = is_first_query;
    }
    else if (MN_DUAL_SYS_2 == dual_sys)
    {
        s_is_vmn_first_querysim2 = is_first_query;
    }
    else
    {
        SCI_TRACE_LOW("[szp] MMISET_SetIsVMFirstQueryFromSIM Error sim number!!! dual_sys: %d",dual_sys);
    }

    return;
}

PUBLIC BOOLEAN MMISET_GetIsVMFirstQueryFromSIM(MN_DUAL_SYS_E dual_sys)
{
    if (MN_DUAL_SYS_1 == dual_sys)
    {
        return s_is_vmn_first_querysim1;
    }
    else if (MN_DUAL_SYS_2 == dual_sys)
    {
        return s_is_vmn_first_querysim2;
    }
    else
    {
        SCI_TRACE_LOW("[szp]MMISET_GetIsVMFromSIM Error sim number!!! dual_sys: %d",dual_sys);
...
//    Note:
/*****************************************************************************/
PUBLIC void MMISET_HandleSetVoiMessAddrCnf( 
                                           DPARAM param
                                           )
{
    ...
    SCI_TRACE_LOW("ljg*MMISET_HandleSetVoiMessAddrCnf:set_vm_addr_info.cause=%d",set_vm_addr_info.cause);

    MMK_CloseWin(MAIN_WAIT_WIN_ID);

    if((MN_DUAL_SYS_1 == set_vm_addr_info.dual_sys)&&(s_is_invalid_cnf_msg)){
        s_is_invalid_cnf_msg = FALSE;
        return;
    }else if((MN_DUAL_SYS_2 == set_vm_addr_info.dual_sys)&&(s_is_invalid_cnf2_msg)){
        s_is_invalid_cnf2_msg = FALSE;
        return;
    }

    if (MN_SMS_OPERATE_SUCCESS == set_vm_addr_info.cause )
    {
        if (MN_DUAL_SYS_1 == set_vm_addr_info.dual_sys )
        {
            g_sim_vmnum_info.num_len = g_temp_sim_vmnum_info.num_len;
            SCI_MEMSET(g_sim_vmnum_info.bcd_num, 0x00, sizeof(g_sim_vmnum_info.bcd_num)); 
            SCI_MEMCPY(g_sim_vmnum_info.bcd_num, g_temp_sim_vmnum_info.bcd_num, g_sim_vmnum_info.num_len);
            g_sim_vmnum_info.number_type = g_temp_sim_vmnum_info.number_type;
            
            SCI_MEMSET(g_sim_alpha_info.alpha_id, 0xFF, PHONEBOOK_MAX_DN_ALPHA_IDENTIFY_LEN); 
            SCI_MEMCPY(g_sim_alpha_info.alpha_id, g_temp_sim_alpha_info.alpha_id, 
                MIN(PHONEBOOK_MAX_DN_ALPHA_IDENTIFY_LEN, g_temp_sim_alpha_info.alpha_id_len));
            g_sim_alpha_info.alpha_id_len = g_temp_sim_alpha_info.alpha_id_len;
            SCI_TRACE_LOW("ljg*MMISET_HandleSetVoiMessAddrCnf:set temp1 DONE");
        }
        else
        {
            g_sim2_vmnum_info.num_len = g_temp_sim_vmnum_info.num_len;
            SCI_MEMSET(g_sim2_vmnum_info.bcd_num, 0x00, sizeof(g_sim2_vmnum_info.bcd_num)); 
            SCI_MEMCPY(g_sim2_vmnum_info.bcd_num, g_temp_sim_vmnum_info.bcd_num, g_sim2_vmnum_info.num_len);
            g_sim2_vmnum_info.number_type = g_temp_sim_vmnum_info.number_type;
            
            SCI_MEMSET(g_sim2_alpha_info.alpha_id, 0xFF, PHONEBOOK_MAX_DN_ALPHA_IDENTIFY_LEN); 
            SCI_MEMCPY(g_sim2_alpha_info.alpha_id, g_temp_sim_alpha_info.alpha_id, 
                MIN(PHONEBOOK_MAX_DN_ALPHA_IDENTIFY_LEN, g_temp_sim_alpha_info.alpha_id_len)); 
            g_sim2_alpha_info.alpha_id_len = g_temp_sim_alpha_info.alpha_id_len;
            SCI_TRACE_LOW("ljg*MMISET_HandleSetVoiMessAddrCnf:set temp2 DONE");
        } 
        MMIPUB_OpenAlertSuccessWin(TXT_SUCCESS);     
    }
    else
    {
        MMIPUB_OpenAlertFailWin(STR_OPERATION_FAIL_EXT01);
    }     
}
...
//    Note:
/*****************************************************************************/
PUBLIC void MMISET_HandleGetVoiMessAddrCnf( 
                                           DPARAM param)
{
    uint8 vm1tele_len = 0;
    uint8 vm2tele_len = 0;
    wchar vm1tele_str[32]  = {0};
    wchar vm2tele_str[32]  = {0};
    uint8 num_str[32] = {0};
    uint8 mail_len = 0;
    uint8 i = 0;
    uint8 voice_mail[MMISMS_PBNUM_MAX_LEN + 2] = {0};
    MN_MULTI_SUBSCRIBER_ID_TYPE_E	record_id = 0;
    BOOLEAN is_first_query_vmn = FALSE;
    BOOLEAN is_direct_convert = TRUE;

    APP_MN_GET_VMW_ADDR_CNF_T		get_vm_addr_info = *(APP_MN_GET_VMW_ADDR_CNF_T *)param;
    SCI_TRACE_LOW("ljg*MMISET_HandleGetVoiMessAddrCnf:get_vm_addr_info.dual_sys=%d", get_vm_addr_info.dual_sys);
    ...
    is_first_query_vmn = MMISET_GetIsVMFirstQueryFromSIM(get_vm_addr_info.dual_sys);
    if ((MN_DUAL_SYS_1 == get_vm_addr_info.dual_sys) && (s_is_invalid_cnf_msg))
    {
        s_is_invalid_cnf_msg = FALSE;
        return;
    }
    else if ((MN_DUAL_SYS_2 == get_vm_addr_info.dual_sys) && (s_is_invalid_cnf2_msg))
    {
        s_is_invalid_cnf2_msg = FALSE;
        return;
    }
    
    SCI_TRACE_LOW("ljg*MMISET_HandleGetVoiMessAddrCnf, g_vmn_oper_from_type = %d, get_vm_addr_info.cause = %d", g_vmn_oper_from_type, get_vm_addr_info.cause);
    SCI_TRACE_LOW("MMISET_HandleGetVoiMessAddrCnf g_sim_vmnum_info number_type = %d, num_len = %d", g_sim_vmnum_info.number_type, g_sim_vmnum_info.num_len);

    if (MN_SMS_OPERATE_SUCCESS == get_vm_addr_info.cause)
    {
        SCI_TRACE_LOW("ljg*MMISET_HandleGetVoiMessAddrCnf:SUCCESS::get_vm_addr_info.line_id=%d",get_vm_addr_info.line_id);
        record_id =get_vm_addr_info.line_id;
        if(MN_DUAL_SYS_1==get_vm_addr_info.dual_sys)
        {
            s_is_vmn_from_sim1 = TRUE;
        }
        else
        {
            s_is_vmn_from_sim2 = TRUE;	
        }
        SCI_TRACE_LOW("ljg*MMISET_HandleGetVoiMessAddrCnf:mailbox_number_len=%d,dual_sys=%d",get_vm_addr_info.vmw_addr_t.mailbox_number_len,get_vm_addr_info.dual_sys);
        for(i = 0;i<get_vm_addr_info.vmw_addr_t.mailbox_number_len;i++)
        {
            SCI_TRACE_LOW("ljg:mailbox_number[%d]=%d",i,get_vm_addr_info.vmw_addr_t.mailbox_number[i]);
            if(get_vm_addr_info.vmw_addr_t.mailbox_number[i] != 0xFF)
            {
                is_direct_convert = FALSE;
            }
        }
        switch( record_id ) 
        {
        case MN_MULTI_SUBSCRIBER_ID_1:
            if (MMISET_VMN_OPER_TYPE_CALL_SETTING == MMISET_GetVmOperType())
            {
                if (MN_DUAL_SYS_1==get_vm_addr_info.dual_sys )
                {
                    g_sim_vmnum_info.num_len = get_vm_addr_info.vmw_addr_t.mailbox_number_len;
                    SCI_MEMSET( g_sim_vmnum_info.bcd_num, 0x00, sizeof(g_sim_vmnum_info.bcd_num)); 
                    SCI_MEMCPY( g_sim_vmnum_info.bcd_num,get_vm_addr_info.vmw_addr_t.mailbox_number, g_sim_vmnum_info.num_len);
                    g_sim_vmnum_info.number_type = get_vm_addr_info.vmw_addr_t.ton;
                    
                    SCI_MEMSET( g_sim_alpha_info.alpha_id, 0xFF, PHONEBOOK_MAX_DN_ALPHA_IDENTIFY_LEN); 
                    SCI_MEMCPY( g_sim_alpha_info.alpha_id, get_vm_addr_info.vmw_addr_t.alpha_id, 
                        MIN(PHONEBOOK_MAX_DN_ALPHA_IDENTIFY_LEN,get_vm_addr_info.vmw_addr_t.alpha_id_len)); 
                    g_sim_alpha_info.alpha_id_len = get_vm_addr_info.vmw_addr_t.alpha_id_len;
                    
                }
                else
                {
                    g_sim2_vmnum_info.num_len = get_vm_addr_info.vmw_addr_t.mailbox_number_len;
                    SCI_MEMSET( g_sim2_vmnum_info.bcd_num, 0x00, sizeof(g_sim2_vmnum_info.bcd_num)); 
                    SCI_MEMCPY( g_sim2_vmnum_info.bcd_num,get_vm_addr_info.vmw_addr_t.mailbox_number, g_sim2_vmnum_info.num_len);
                    g_sim2_vmnum_info.number_type = get_vm_addr_info.vmw_addr_t.ton;
                    
                    SCI_MEMSET( g_sim2_alpha_info.alpha_id, 0xFF, PHONEBOOK_MAX_DN_ALPHA_IDENTIFY_LEN); 
                    SCI_MEMCPY( g_sim2_alpha_info.alpha_id, get_vm_addr_info.vmw_addr_t.alpha_id, 
                        MIN(PHONEBOOK_MAX_DN_ALPHA_IDENTIFY_LEN,get_vm_addr_info.vmw_addr_t.alpha_id_len)); 
                    g_sim2_alpha_info.alpha_id_len = get_vm_addr_info.vmw_addr_t.alpha_id_len;
                }
                
                if(is_direct_convert)
                {
                    mail_len = 0;
                }
                else
                {
                    mail_len = get_vm_addr_info.vmw_addr_t.mailbox_number_len;
                }

                MMISET_GetVoicemailFromMbdn(get_vm_addr_info.dual_sys, mail_len);
            }
            else
            {
                if (MN_DUAL_SYS_1==get_vm_addr_info.dual_sys)
                {
                    ...
                    SCI_MEMCPY( g_sim_vmnum_info.bcd_num,get_vm_addr_info.vmw_addr_t.mailbox_number, g_sim_vmnum_info.num_len);
                    g_sim_vmnum_info.number_type = get_vm_addr_info.vmw_addr_t.ton;
                    vm1tele_len = g_sim_vmnum_info.num_len;
                    ...
                    SCI_TRACE_LOW("MMISET_HandleGetVoiMessAddrCnf, voice_mail = %s, mail_len = %d", voice_mail, mail_len);

                    if (TRUE == is_first_query_vmn)
                    {
                        MMISET_SetIsVMFirstQueryFromSIM(get_vm_addr_info.dual_sys, FALSE);
                    }
                    else
                    {
                        if (mail_len>0)
                        {
                            if(MMISET_VMN_OPER_TYPE_IDLE_DIAL==MMISET_GetVmOperType())
                                MMIAPICC_MakeCall(MN_DUAL_SYS_1,voice_mail,strlen(voice_mail),PNULL,PNULL,CC_SIM1_CALL,CC_CALL_NORMAL_CALL,PNULL);
                        }
                        else
                        {
                            MMISET_GetVoicemailFromMbdnCallIdle(MN_DUAL_SYS_1);
                        }
                    }
                }
                else
                {
                    ...
                    SCI_MEMCPY( g_sim2_vmnum_info.bcd_num,get_vm_addr_info.vmw_addr_t.mailbox_number, g_sim2_vmnum_info.num_len);
                    g_sim2_vmnum_info.number_type = get_vm_addr_info.vmw_addr_t.ton;
                    vm2tele_len = g_sim2_vmnum_info.num_len;
                    ...
                    else
                    {
                        ...
                        {
                            if(MMISET_VMN_OPER_TYPE_IDLE_DIAL==MMISET_GetVmOperType())
                                MMIAPICC_MakeCall(MN_DUAL_SYS_2,voice_mail,strlen(voice_mail),PNULL,PNULL,CC_SIM2_CALL,CC_CALL_NORMAL_CALL,PNULL);
                        }
                        else
                        {
                            MMISET_GetVoicemailFromMbdnCallIdle(MN_DUAL_SYS_2);
                        }
                    }
                }
            }    
    ...
    {
        SCI_TRACE_LOW("ljg*MMISET_HandleGetVoiMessAddrCnf:failue");
        if(MN_DUAL_SYS_1==get_vm_addr_info.dual_sys)
        {
            s_is_vmn_from_sim1 = FALSE;
        }
        else
        {
            s_is_vmn_from_sim2 = FALSE;	
        }
        if (TRUE == is_first_query_vmn)
        {
            MMISET_SetIsVMFirstQueryFromSIM(get_vm_addr_info.dual_sys, FALSE);
        ...
        else
        {
            ...
            case MMISET_VMN_OPER_TYPE_CALL_SETTING:
                MMISET_GetVoicemailFromNdtAndMultivar(get_vm_addr_info.dual_sys);	
                break;
            case MMISET_VMN_OPER_TYPE_IDLE_DIAL:
                MMIAPIIDLE_HandleVoiceMailFromNdt(get_vm_addr_info.dual_sys);
                break;
            default:
                break;
            }
                               ...
                               )
{
    ...
    case APP_MN_REGISTER_SS_CNF:
        {
	        ...
	        MNSS_SENDSS_RESULT_E result_e = MNSS_SUCCESS;
            //@gang.tong add 2005.11.24
            //如果stk等待这个消息，那么stk处理完后就返回TRUE
            ...
            {
                g_send_ss  = FALSE;
                result_e = MNSS_SendSSEx(1, MN_SS_REGISTERSS, g_mmiset_ss_status.ss_code, g_mmiset_ss_status.basic_service, g_register_param, g_ss_string);
                if(MNSS_FAILURE == result_e || MNSS_FDN_FAILED == result_e)
		{
			SCI_TraceLow("HandlePsMsg  MNSS_SendSSEx fail ");		
		}
		g_is_ss_handling = TRUE;		
            }
            result = HandleOperateSsCnf(
    ...
    case APP_MN_GET_VMW_ADDR_CNF:
        SCI_TRACE_LOW("ljg*HandlePsMsg,APP_MN_GET_VMW_ADDR_CNF"); 
        MMISET_HandleGetVoiMessAddrCnf(param);
        break;
    case APP_MN_SET_VMW_ADDR_CNF:
        SCI_TRACE_LOW("ljg*HandlePsMsg,APP_MN_SET_VMW_ADDR_CNF"); 
         MMISET_HandleSetVoiMessAddrCnf(param);
         break;
    default:
...
LOCAL MMI_RESULT_E  HandleCloseRegisterPasswordWindow(
                                         MMI_WIN_ID_T    win_id, 
                                             ...
                                             )
{
    ...
    case MN_SS_OPERATION_SUCC:
        MMIPUB_OpenAlertWinByTextId(PNULL,TXT_SUCCESS,TXT_NULL,IMAGE_PUBWIN_SUCCESS,PNULL,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,HandleCloseRegisterPasswordWindow);
        //MMIPUB_OpenAlertSuccessWin(TXT_SUCCESS);
        break;
    ...
    case MN_SS_OPERATION_ERROR:
        ...
        case MN_SS_ERROR_NEGATIVE_PW_CHECK:
            //提示密码错误
            MMIPUB_OpenAlertWinByTextId(PNULL,TXT_KL_PSW_WRONG_PSW,TXT_NULL,IMAGE_PUBWIN_SUCCESS,PNULL,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,HandleCloseRegisterPasswordWindow);
            //MMIPUB_OpenAlertWarningWin(TXT_KL_PSW_WRONG_PSW);
            break;
            
        case MN_SS_ERROR_NUMBER_OF_PW_ATTEMPTS_VIOLATION:
            //提示密码无效
            MMIPUB_OpenAlertWinByTextId(PNULL,TXT_PASSWORD_INVALID,TXT_NULL,IMAGE_PUBWIN_SUCCESS,PNULL,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,HandleCloseRegisterPasswordWindow);
            //MMIPUB_OpenAlertWarningWin(TXT_PASSWORD_INVALID);
            break;
            
        default:
			MMIPUB_OpenAlertWinByTextId(PNULL,TXT_ERROR,TXT_NULL,IMAGE_PUBWIN_SUCCESS,PNULL,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,HandleCloseRegisterPasswordWindow);
            //MMIPUB_OpenAlertWarningWin(TXT_ERROR);
            break;
        }
        break;
        ...
        case MN_SS_OPERATION_REJECT:
			MMIPUB_OpenAlertWinByTextId(PNULL,TXT_REJECT,TXT_NULL,IMAGE_PUBWIN_SUCCESS,PNULL,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,HandleCloseRegisterPasswordWindow);
...
LOCAL void UpdateCfuStatuaAfterOperate(MN_DUAL_SYS_E dual_sys, MMISET_SS_OPERATE_TYPE_E  operate_type)
{
    ...
    if(operate_type == MMISET_SS_REGISTER || operate_type == MMISET_SS_ACTIVATE)
    {
        if ( (g_mmiset_ss_status.ss_code == MN_SS_CFU) || (g_mmiset_ss_status.ss_code == MN_SS_ALL_FORWARD))
        {
            ...
            MMIAPICOM_StatusAreaSetForwardState(dual_sys, TRUE);
        }			    		
    ...
    else if (operate_type == MMISET_SS_ERASE || operate_type == MMISET_SS_DEACTIVATE )
    {
            ...
            ||  (g_mmiset_ss_status.ss_code == MN_SS_ALL_FORWARD))
        {
            ...
            MMIAPICOM_StatusAreaSetForwardState(dual_sys, MMISET_GetIsCfuActived(dual_sys, MMISET_SS_MAX));
        }				
        //调用接口，查询出CSP的状态
        UpdateCustomerServiceProfile(dual_sys);	
    }
...
/*****************************************************************************/
// 	Description : open activate  tips for success or failure  in Banner include call forwarding, call waiting and call barring.
//	Global resource dependence : 
//	Note:
/*****************************************************************************/
LOCAL void MMIAPISET_OperateRegisterSsTips(MN_DUAL_SYS_E  dual_sys,
                                           MMI_TEXT_ID_T text_id,
                                           MMISET_SS_OPERATE_TYPE_E  operate_type,
                                           MN_SS_OPERATE_RESULT_E  result_type)
{
    MMI_WIN_ID_T win_id = MMISET_SET_SS_BANNER_TIPS_WIN_ID;
    ...
    if(dual_sys >= MMI_DUAL_SYS_MAX)return;
    if (TXT_NULL == text_id)return;
    SCI_TRACE_LOW("enter ljg*MMIAPISET_OperateRegisterSsTips::g_mmiset_ss_status.ss_code=%d",g_mmiset_ss_status.ss_code);	
    if ((MN_SS_ALL_FORWARD == g_mmiset_ss_status.ss_code)||
        ...
        (MN_SS_CFNRC == g_mmiset_ss_status.ss_code))
    {
        if ((MN_SS_OPERATION_SUCC == result_type) && 
            ((MMISET_SS_REGISTER == operate_type) || (MMISET_SS_ACTIVATE == operate_type)))
        {
            if (MN_DUAL_SYS_1 == dual_sys)
            {
                banner_info.text1_id = STR_CALL_FORWARDING_ON;
            }
            ...
            if (MN_SS_CFU == g_mmiset_ss_status.ss_code)
            {
                if (0 !=MMIAPISET_GetForwardConditions())
                {
                    MMIAPISET_SetForwardConditions(0);
                    MMIAPISET_SetForwardSimCards(dual_sys);		
                }
            }
            else if ((MN_SS_CFB == g_mmiset_ss_status.ss_code) && (1 !=MMIAPISET_GetForwardConditions()))
            {
                MMIAPISET_SetForwardConditions(1);
                MMIAPISET_SetForwardSimCards(dual_sys);	
            }
            else if ((MN_SS_CFNRY == g_mmiset_ss_status.ss_code) && (2 !=MMIAPISET_GetForwardConditions()))
            {
                MMIAPISET_SetForwardConditions(2);
                MMIAPISET_SetForwardSimCards(dual_sys);	
            }
            else if ((MN_SS_CFNRC == g_mmiset_ss_status.ss_code) && (3 !=MMIAPISET_GetForwardConditions()))
            {
                MMIAPISET_SetForwardConditions(3);
                MMIAPISET_SetForwardSimCards(dual_sys);	
            }
        }
        else if ((MN_SS_OPERATION_SUCC == result_type) && 
                 ((MMISET_SS_ERASE == operate_type) || (MMISET_SS_DEACTIVATE == operate_type)))
        {
            if (MN_DUAL_SYS_1 == dual_sys)
            {
                banner_info.text1_id = STR_CALL_FORWARDING_OFF;
            }
#if defined(MMI_MULTI_SIM_SYS_DUAL)
            else
            {
...
#endif
            if (MN_SS_CFU == g_mmiset_ss_status.ss_code || (MN_SS_ALL_FORWARD == g_mmiset_ss_status.ss_code))
            {
                if (4 !=MMIAPISET_GetForwardConditions())
                {
                    MMIAPISET_SetForwardConditions(4);
                    MMIAPISET_SetForwardSimCards(dual_sys);		
                }
            }
            else if ((MN_SS_CFB == g_mmiset_ss_status.ss_code  && (1 ==MMIAPISET_GetForwardConditions()))
                     || (MN_SS_CFNRY == g_mmiset_ss_status.ss_code  && (2 ==MMIAPISET_GetForwardConditions()))
                     || (MN_SS_CFNRC == g_mmiset_ss_status.ss_code  && (3 ==MMIAPISET_GetForwardConditions())))
            {
                MMIAPISET_SetForwardConditions(4);
                MMIAPISET_SetForwardSimCards(dual_sys);	
            }
        }
        else if ((MN_SS_OPERATION_ERROR == result_type) && 
                 ((MMISET_SS_REGISTER == operate_type) || (MMISET_SS_ACTIVATE == operate_type)))
        {
            if (MN_DUAL_SYS_1 == dual_sys)
            {
                banner_info.text1_id = STR_CALL_FORWARDING_FAIL;
            }
#if defined(MMI_MULTI_SIM_SYS_DUAL)
            else
            {
                 ...
                 ((MMISET_SS_ERASE == operate_type) || (MMISET_SS_DEACTIVATE == operate_type)))
        {
            if (MN_DUAL_SYS_1 == dual_sys)
            {
                banner_info.text1_id = STR_CALL_FORWARDING_CANCEL_FAIL;
            }
#if defined(MMI_MULTI_SIM_SYS_DUAL)
            else
            {
    ...
    else if (MN_SS_CW == g_mmiset_ss_status.ss_code)
    {
        if ((MN_SS_OPERATION_SUCC == result_type) && 
            ((MMISET_SS_REGISTER == operate_type) || (MMISET_SS_ACTIVATE == operate_type)))
        {
            banner_info.text1_id = STR_CALL_WAITING_ACTIVATED_EXT01;
        }
        else if ((MN_SS_OPERATION_SUCC == result_type) &&
                 ((MMISET_SS_ERASE == operate_type) || (MMISET_SS_DEACTIVATE == operate_type)))
        {
            banner_info.text1_id = STR_CALL_WAITING_OFF_EXT01;
        }
        else if ((MN_SS_OPERATION_ERROR == result_type) &&
                 ((MMISET_SS_REGISTER == operate_type) || (MMISET_SS_ACTIVATE == operate_type)))
        {
            banner_info.text1_id = STR_CALL_WAITING_FAIL;
    	}
    	else if ((MN_SS_OPERATION_ERROR == result_type)&&
    	         ((MMISET_SS_ERASE == operate_type) || (MMISET_SS_DEACTIVATE == operate_type)))
        {
            banner_info.text1_id = STR_CALL_WAITING_CANCEL_FAIL;
    	}    
    }
    else if ((MN_SS_ALL_BAR_SERVICE == g_mmiset_ss_status.ss_code)||
             (MN_SS_BAR_OUTGOING == g_mmiset_ss_status.ss_code)||
             (MN_SS_BAOC == g_mmiset_ss_status.ss_code)||
             (MN_SS_BOIC == g_mmiset_ss_status.ss_code)||
             (MN_SS_BOIC_EXHC == g_mmiset_ss_status.ss_code)||
             (MN_SS_BAR_INCOMING == g_mmiset_ss_status.ss_code)||
             (MN_SS_BAIC == g_mmiset_ss_status.ss_code)||
             (MN_SS_BICROAM == g_mmiset_ss_status.ss_code))
    {
        if ((MN_SS_OPERATION_SUCC == result_type) && 
            ((MMISET_SS_REGISTER == operate_type) || (MMISET_SS_ACTIVATE == operate_type)))
        {
            banner_info.text1_id = STR_CALL_BARRING_ACTIVATED_EXT01;
        }
        else if ((MN_SS_OPERATION_SUCC == result_type) &&
                 ((MMISET_SS_ERASE == operate_type) || (MMISET_SS_DEACTIVATE == operate_type)))
        {
            banner_info.text1_id = STR_CALL_BARRING_OFF_EXT01;
        }
        else if ((MN_SS_OPERATION_ERROR == result_type) && 
                 ((MMISET_SS_REGISTER == operate_type) || (MMISET_SS_ACTIVATE == operate_type)))
        {
            banner_info.text1_id = STR_CALL_BARRING_FAIL;
        }
        else if ((MN_SS_OPERATION_ERROR == result_type) &&
                 ((MMISET_SS_ERASE == operate_type) || (MMISET_SS_DEACTIVATE == operate_type)))
        {
            banner_info.text1_id = STR_CALL_BARRING_CANCEL_FAIL;
        }
    }
    else if (((MN_SS_CLIP == g_mmiset_ss_status.ss_code) || (MN_SS_CLIR == g_mmiset_ss_status.ss_code))
             && (MN_SS_OPERATION_SUCC == result_type))
    {
        if ((MMISET_SS_REGISTER == operate_type) || (MMISET_SS_ACTIVATE == operate_type))
        {
            banner_info.text1_id = TXT_COMM_OPEN;
            if (1 == MMIAPISET_IsVodafoneVersion() && (MN_SS_CLIR == g_mmiset_ss_status.ss_code))
            { 
                //MMISET_SetClirType(dual_sys, MN_CALL_CLIR_SUPPRESSION);
            }
        }
        else if ((MMISET_SS_ERASE == operate_type) || (MMISET_SS_DEACTIVATE == operate_type))
        {
            banner_info.text1_id = STXT_EXIT	;
            if (1 == MMIAPISET_IsVodafoneVersion() && (MN_SS_CLIR == g_mmiset_ss_status.ss_code))
            { 
                //MMISET_SetClirType(dual_sys, MN_CALL_CLIR_INVOCATION);
            }
        }
    }
    else
    ...
    if (!MMISET_IsOpenDualSysCfnrc())
    {
        ...
        case MN_SS_OPERATION_SUCC:
            ...
            case MMISET_SS_ACTIVATE:
                ...
                //ljg add for Banner tips
				MMIAPISET_OperateRegisterSsTips(dual_sys,STR_STATUS_ACTIVATED_EXT01,operate_type,result_type);
				break;
            ...
            case MMISET_SS_DEACTIVATE:
				...
				//ljg add for Banner tips
				MMIAPISET_OperateRegisterSsTips(dual_sys,STR_STATUS_NOT_ACTIVATED_EXT01,operate_type,result_type);
            ...
            default:
                //MMIPUB_OpenAlertSuccessWin(TXT_SUCCESS);
                //ljg add for Banner tips
                MMIAPISET_OperateRegisterSsTips(dual_sys,TXT_SUCCESS,operate_type,result_type);
                break;
            }
            break;
        ...
        case MN_SS_OPERATION_ERROR:
            ...
            case MN_SS_ERROR_NEGATIVE_PW_CHECK:
                ...
                //MMIPUB_OpenAlertWarningWin(TXT_KL_PSW_WRONG_PSW);
                //ljg add for Banner tips
				MMIAPISET_OperateRegisterSsTips(dual_sys,TXT_KL_PSW_WRONG_PSW,operate_type,result_type);
                break;
            ...
            case MN_SS_ERROR_NUMBER_OF_PW_ATTEMPTS_VIOLATION:
                ...
                //MMIPUB_OpenAlertWarningWin(TXT_PASSWORD_INVALID);
				//ljg add for Banner tips
				MMIAPISET_OperateRegisterSsTips(dual_sys,TXT_PASSWORD_INVALID,operate_type,result_type);
                break;
            ...
            default:
                ...
                case MMISET_SS_ACTIVATE:
                    ...
                    //ljg add for Banner tips
                    MMIAPISET_OperateRegisterSsTips(dual_sys,TXT_ERROR,operate_type,result_type);
                    break;
        ...
        case MN_SS_OPERATION_REJECT:
            ...
            //MMIPUB_OpenAlertWarningWin(TXT_REJECT);
			//ljg add for Banner tips
			MMIAPISET_OperateRegisterSsTips(dual_sys,TXT_REJECT,operate_type,result_type);
            break;
                                          ...
                                          )
{
    ...
    if( g_is_for_cfu_icon ) 
    {
        ...
        case MN_SS_OPERATION_SUCC:
	      ...
	      s_dualsys_cfu_query_info[interrogate_ss_ind.dual_sys] = TRUE;
            break;
            
        case MN_SS_OPERATION_ERROR:			
            SCI_TraceLow("HandleInterrogateSsCnf interrogate_ss_ind.err_code=%d",interrogate_ss_ind.err_code);
	    if(interrogate_ss_ind.err_code == MN_SS_ERROR_ERROR_SS_STATUS)
		   s_dualsys_cfu_query_info[interrogate_ss_ind.dual_sys] = TRUE;
                                               ...
                                               )
{
    ...
    case MSG_OPEN_WINDOW:
        {
            MMI_GetLabelTextByLang(STR_SETT_STATUS_EXT01, &title_str);
...
LOCAL void MMIAPISET_CreateCheckStatusWin(
                                uint8 list_num,
                                MN_SS_INTERROGATE_INFO_T    *info_list_ptr
                                )
{
    MMI_WIN_ID_T        win_id = MMISET_SS_ALL_CHECK_STATUS_WIN_TAB;
    //MMI_STRING_T        content_str = {0};
    MMI_STRING_T        temp_str = {0};
    MMI_STRING_T        status_str = {0};
    MMI_STRING_T        status_str2 = {0};
    wchar               content_text[200] = {0};
	 wchar              status_str_temp[200] = {0};
    uint16 uint16_str_len = 0;

	status_str.wstr_ptr = status_str_temp;
    GenInterrogateStatusReport(list_num, info_list_ptr, &status_str, &status_str2);
    MMIAPICOM_StrcatFromSTRINGToUCS2(content_text, &uint16_str_len, &status_str);
    MMIPUB_OpenTextDialogByTextPtr(PNULL, &status_str, &status_str2, PNULL, &win_id, PNULL, MMIPUB_SOFTKEY_DIALOG_OK, HandleCheckStatusWindow);
}
                                ...
                                )
{
	...
	*/
    ...
    else 
    {
		...
		MMIAPISET_CreateCheckStatusWin(list_num,info_list_ptr);
    }
    else
    {
        //SCI_TRACE_LOW:"GenInterrogateReport:list_num = %d"
        ...
        MMK_CreateWin((uint32*)MMISET_SS_ALL_REPORT_WIN_TAB,PNULL);
        //generate interrogate ss service name
        GenInterrogateServiceReport(&temp_str);*/
        //SCI_TRACE_LOW:"GenInterrogateReport:list_num = %d"
        SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMISET_CALL_1826_112_2_18_2_50_7_21,(uint8*)"d", list_num);
        //MMK_CloseWin(MMISET_SS_ALL_REPORT_WIN_ID);
        //MMK_CreateWin((uint32*)MMISET_SS_ALL_REPORT_WIN_TAB,PNULL);
...
LOCAL void GenInterrogateServiceReport(MMI_STRING_T *name_str)
{
    ...
    case MN_SS_COLR://隐藏实际接通号码
        ...
        break;
    case MN_SS_CNAP:
        MMI_GetLabelTextByLang(TXT_SET_CNAP_SUCC_ID,&name_str);
                                      ...
                                      MMI_STRING_T *status_str2)
{
    ...
    if (MN_SS_ALL_TELESERVICES == info_list_ptr[0].basic_service.service_code)
    {
        if(info_list_ptr[0].ss_status_present)
        {
            GenInterrogateDetailReport(&info_list_ptr[0], status_str, status_str2);
        }
        else
        {
    ...
    for (i=0; i<list_num; i++)
    {
        if ((info_list_ptr[i].basic_service.service_type == MN_SS_TELE_SERVICE)&&
            ...
            (info_list_ptr[i].basic_service.service_code == MN_SS_ALL_SPEECH_TRANS_SERVICES)||
            (info_list_ptr[i].basic_service.service_code == MN_SS_TELEPHONE)||
            (info_list_ptr[i].basic_service.service_code == MN_SS_ALL_SHORT_MSG_SERVICE)))
        {
            if (info_list_ptr[i].ss_status_present)
            {
                GenInterrogateDetailReport(&info_list_ptr[i], status_str, status_str2);
            }
            else
            {
...
{
    wchar           divert_num_wstr[MMISET_PHONE_NUMBER_MAX_LEN + 2] = {0};
    wchar           divert_time[MMISET_NO_ANSWER_TIME_MAX_LEN + 1] = {0};
    wchar text_temp[200] = {0};
        ...
        || MN_SS_COLR == g_mmiset_ss_status.ss_code)
    {
        ...
        {
            MMI_GetLabelTextByLang(STR_STATUS_PROVIDED_EXT01/*TXT_PROVISION*/,status_str);
        }
        else
        {
            MMI_GetLabelTextByLang(TXT_NOT_PROVISION,status_str);
        }
    }else if(MN_SS_CLIR == g_mmiset_ss_status.ss_code){
        if(info_list_ptr->clir_info_present){
            switch(info_list_ptr->clir_info){
                case MN_SS_CLIR_PERMANENT:
                case MN_SS_CLIR_TMP_DEFAULT_RESTRICTED:
                     MMI_GetLabelTextByLang(STR_CALLER_ID_HIDE,status_str);
                     break;
                case MN_SS_CLIR_TMP_DEFAULT_ALLOWED:	
                     MMI_GetLabelTextByLang(STR_CALLER_ID_SENT,status_str);
                     break;
                default:
                     MMI_GetLabelTextByLang(TXT_NOT_PROVISION,status_str);
                     break;
        ...
        )   //for call forword
    {
        ...
        {
        case MN_SS_NOT_PROVITION:
            MMI_GetLabelTextByLang(STR_STATUS_NOT_PROVIDED_EXT01,status_str);
            break;
            
        case MN_SS_NOT_ACTIVE:
            //SCI_TRACE_LOW("GenInterrogateDetailReport: info_list_ptr->ss_status is MN_SS_NOT_ACTIVE");
            MMI_GetLabelTextByLang(STR_STATUS_NOT_ACTIVATED_EXT01, status_str);
            break;
        ...
        case MN_SS_PROVITION:
            ...
            {              
                GenInterrogateTelenumReport(info_list_ptr,divert_num_wstr);
            };
        
            if(info_list_ptr->no_reply_time_present)
            {
                GenInterrogateTimeReport(info_list_ptr,divert_time);
            };
            break;
        ...
        case MN_SS_ACITVE_QUIESCENT:
           // MMI_GetLabelTextByLang(TXT_ACTIVE_OPERATIVE,status_str);
            ...
            SCI_TRACE_LOW("info_list_ptr->no_reply_time_present = %d",info_list_ptr->no_reply_time_present);
            if(info_list_ptr->no_reply_time_present)
            {
                GenInterrogateTimeReport(info_list_ptr,divert_time);
            };
            ...
            switch(g_mmiset_ss_status.ss_code){
                case MN_SS_CFNRC:
                    MMI_GetLabelTextByLang(STR_FWD_IF_OUT_OF_REACH, &old_string);
                    break;
                case MN_SS_CFB:
                    MMI_GetLabelTextByLang(STR_FWD_IF_BUSY, &old_string);	
                    break;
                case MN_SS_ALL_FORWARD:
                case MN_SS_CFU:
		  case MN_SS_ALL_COND_FORWARD:
                    MMI_GetLabelTextByLang(STR_FWD_ALL_CALLS, &old_string);
                    break;
                case MN_SS_CFNRY:	
                    MMI_GetLabelTextByLang(STR_FWD_IF_NO_REPLY, &old_string);
                    break;
            }
            MMIAPICOM_Wstrncpy(text_temp, old_string.wstr_ptr,old_string.wstr_len);
            if(MN_SS_CFNRY == g_mmiset_ss_status.ss_code){
                MMIAPICOM_WstrReplace(text_temp, sizeof(text_temp), L"%0U", divert_num_wstr);
                MMIAPICOM_WstrReplace(text_temp, sizeof(text_temp), L"%1N", divert_time);
            }else{
                MMIAPICOM_WstrReplace(text_temp, sizeof(text_temp), L"%U", divert_num_wstr);
            }
            status_str->wstr_len = (uint16)MMIAPICOM_Wstrlen(text_temp);
            MMIAPICOM_Wstrncpy(status_str->wstr_ptr,text_temp,MMIAPICOM_Wstrlen(text_temp));
            break;
        }    
    }
    else
    {
        ...
        {
        case MN_SS_NOT_PROVITION:
            MMI_GetLabelTextByLang(STR_STATUS_PROVIDED_EXT01/*TXT_PROVISION_NOT_ACTIVED*/,status_str);
            MMI_GetLabelTextByLang(STR_STATUS_NOT_ACTIVATED_EXT01, status_str2);
            break;
        ...
        case MN_SS_ACTIVE_OPERATIVE:
        case MN_SS_ACITVE_QUIESCENT:
            MMI_GetLabelTextByLang(STR_STATUS_ACTIVATED_EXT01,status_str);
        	if(info_list_ptr->forward_num_present)
            {              
                GenInterrogateTelenumReport(info_list_ptr,divert_num_wstr);
            };
                                       ...
                                       wchar *telenum_wstr)
{
    ...
    MMIAPICOM_Wstrncpy(telenum_wstr,tele_num_wstr,MMIAPICOM_Wstrlen(tele_num_wstr));
    SCI_TRACE_LOW("enter ljg*GenInterrogateTelenumReport::tele_num=%s",tele_num);	
                                    ...
                                    wchar *times_wstr)
{
   ...
   // MMI_GetLabelTextByLang(TXT_SECOND,&second_str);
    ...
    time_str_len = (uint8)strlen((char*)time_str);
     SCI_TRACE_LOW("enter ljg*GenInterrogateTimeReport  no_answer.wstr_len =%d time_str=%s ",no_answer.wstr_len,time_str);
...
//     }
    ...
    MMIAPICOM_Wstrncpy(times_wstr,time,MMIAPICOM_Wstrlen(time));
...
PUBLIC void MMIAPISET_SetIsQueryCfu( BOOLEAN is_query,MN_DUAL_SYS_E dual_sys)
{
    ...
    SCI_TraceLow(" MMIAPISET_SetIsQueryCfu  is_query=%d",is_query);
...
{
    SCI_TraceLow("MMIAPISET_GetIsQueryCfu: s_has_query_cfu = %d",s_has_query_cfu);
                                                 ...
                                                 )
{
    ...
    if (MMISET_IsSSProcessing())
    {
       SCI_TRACE_LOW("enter MMIAPISET_InterrogateCfuStatusFromPs::MMISET_IsSSProcessing return");
        return;
    }
    ...
    if (!MMIAPISET_GetIsQueryCfu())
    {
        ...
        {
         SCI_TRACE_LOW("enter MMIAPISET_InterrogateCfuStatusFromPs::s_dualsys_cfu_query_info return");
            return;
        }
       ...
       // s_dualsys_cfu_query_info[dual_sys] = TRUE;
        ...
        SCI_TRACE_LOW("enter ljg*MMIAPISET_InterrogateCfuStatusFromPs==MMIAPIENG_GetCallForwardQuery=%d,err_code=%d",MMIAPIENG_GetCallForwardQuery(),err_code);
        if (!MMIAPIENG_GetCallForwardQuery())//(ERR_MNDATAMAG_NO_ERR == err_code)
        {
            BOOLEAN is_cf_state = FALSE;
            MMISET_SetIsCfuActived(dual_sys, MMISET_SS_AUDIO, TRUE);
            SCI_TRACE_LOW("enter ljg*MMIAPISET_InterrogateCfuStatusFromPs::line_index=%d",line_index);
            if (0 == line_index)
            {
            ...
            SCI_TRACE_LOW("enter ljg*MMIAPISET_InterrogateCfuStatusFromPs::is_cf_state=%d",is_cf_state);
            if(is_cf_state)MMIAPICOM_StatusAreaSetForwardState(dual_sys, TRUE);
            //MMIAPIPHONE_InterrogateCfuForIdle(GetCfuSimId());
        }
        else
        {
...
#ifdef MMI_CSP_SUPPORT        
    if(MMIAPIPHONE_IsCPHSSupport()
	 &&MMISET_IsCphsValid(dual_sys) 
        && MMISET_IsCspValid(dual_sys, 0)
        )
    {
    ...
    if(s_cphs_info[dual_sys].is_cphs_valid)
    {
        ...
        //handle cf icon in statusbar
        {
            ...
            if (ERR_MNDATAMAG_NO_ERR == err_code)
            {
                ...
                {
                    MMIAPICOM_StatusAreaSetForwardState(dual_sys, call_forward_flag.cff_line_2);
                }
                else
                {
                    MMIAPICOM_StatusAreaSetForwardState(dual_sys, call_forward_flag.cff_line_1);
                }
             }        
             else
            {
                                       ...
                                       )
{
	...
	else if(!MMIAPIPHONE_IsCPHSSupport())
	{
		return FALSE;
	}
    else
                                       ...
                                       )
{
    ...
    if(g_is_ss_handling == TRUE)
    {
        SCI_TraceLow("MMISET_CloseAllCallForwarding  g_is_ss_handling = %d", g_is_ss_handling);
        MMIPUB_OpenAlertWarningWin( TXT_SYS_WAITING_AND_TRY_LATER);
        return ;
    }
...
LOCAL MMI_RESULT_E HandleCloseAllCallForwardingConnectWindow(
                                              MMI_WIN_ID_T       win_id, 	// 窗口的ID
                                              MMI_MESSAGE_ID_E 	    msg_id, 	// 窗口的内部消息ID
                                              DPARAM 		        param		// 相应消息的参数
                                              )
                                              ...
                                              )
{
    ...
    //add items to listbox
    MMIAPISET_AppendListItemByTextId( TXT_SET_TIME_30S, COMMON_TXT_NULL, ctrl_id, GUIITEM_STYLE_RADIO_ONE_LINE_TEXT );
    MMIAPISET_AppendListItemByTextId( TXT_SET_TIME_25S, COMMON_TXT_NULL, ctrl_id, GUIITEM_STYLE_RADIO_ONE_LINE_TEXT );
    MMIAPISET_AppendListItemByTextId( TXT_SET_TIME_20S, COMMON_TXT_NULL, ctrl_id, GUIITEM_STYLE_RADIO_ONE_LINE_TEXT );
    MMIAPISET_AppendListItemByTextId( TXT_SET_TIME_15S, COMMON_TXT_NULL, ctrl_id, GUIITEM_STYLE_RADIO_ONE_LINE_TEXT );
    MMIAPISET_AppendListItemByTextId( TXT_SET_TIME_10S, COMMON_TXT_NULL, ctrl_id, GUIITEM_STYLE_RADIO_ONE_LINE_TEXT );//cr273646
    MMIAPISET_AppendListItemByTextId( TXT_SET_TIME_5S, COMMON_TXT_NULL, ctrl_id, GUIITEM_STYLE_RADIO_ONE_LINE_TEXT );
...
//#endif
    ...
    //set ss param,include ss_code and sevice_type,service_code
    SetSsParam(dual_sys,FALSE);
	switch(s_call_divert_conditions){
	case 0:
		called_fuc_id = ID_SET_DIVERTS_ALL_CALLS;
		break;
	case 1:
		called_fuc_id = ID_SET_DIVERTS_WHEN_BUSY;
		break;
	case 2:
		called_fuc_id = ID_SET_DIVERTS_NO_ANSWER;
		break;
	case 3:
		called_fuc_id = ID_SET_DIVERTS_NOT_REACHABLE;
		break;
	case 4:
		called_fuc_id = ID_SET_DIVERTS_ALL_CONDITIONAL;
		break;	
	default:
		called_fuc_id = ID_SET_DIVERTS_ALL_CALLS;
		break;
...
PUBLIC void MMISET_CloseAllCallForwarding(MN_DUAL_SYS_E dual_sys)
{
    ...
    MN_SS_REGISTER_PARAM_U register_param = {0};/*lint !e64*/// parameter's union of register ss data operation
    SIMAT_DO_SS_STRING_T ss_string = {0};
    MNSS_SENDSS_RESULT_E ss_err_code1 = MNSS_SUCCESS;
	MNSS_SENDSS_RESULT_E ss_err_code2 = MNSS_SUCCESS;
    ...
    if (MMISET_IsSSProcessing())//if(g_is_ss_handling == TRUE)
    {
        SCI_TraceLow("MMISET_CloseAllCallForwarding  g_is_ss_handling = %d", g_is_ss_handling);
        MMIPUB_OpenAlertWarningWin( TXT_SYS_WAITING_AND_TRY_LATER);
        return ;
    ...
    SCI_TRACE_LOW("MMISET_CloseAllCallForwarding dual_sys = %d", dual_sys);
    g_mmiset_ss_status.ss_code = MN_SS_ALL_FORWARD;
    g_mmiset_ss_status.basic_service.service_type = MN_SS_TELE_SERVICE;
    g_mmiset_ss_status.basic_service.service_code = 0;
    g_mmiset_ss_status.dual_sys = dual_sys;

    ss_err_code1 = MNSS_SendSSEx(dual_sys, MN_SS_ERASESS, g_mmiset_ss_status.ss_code, g_mmiset_ss_status.basic_service, register_param, ss_string);
    SCI_TRACE_LOW("MMISET_CloseAllCallForwarding MNSS_SendSSEx ss_err_code1 = %d", ss_err_code1);
    if (MNSS_SUCCESS == ss_err_code1 || MNSS_INFORM_STK == ss_err_code1)
    {
        g_is_ss_handling= TRUE;
...
PUBLIC void MMISET_DisplayBannerInfo(MMI_WIN_ID_T win_id, MMI_TEXT_ID_T text_id)
{
    MMICOM_BANNER_INFO_T banner_info = {0};
    banner_info.banner_type = MMI_BANNER_TEXT;
...
PUBLIC void MMIAPISET_CheckCWStatus(MMI_TEXT_ID_T text_id)
{
    MMI_WIN_ID_T        win_id = MMISET_SS_ALL_CHECK_STATUS_WIN_TAB;
    MMI_STRING_T        status_str = {0};

    MMI_GetLabelTextByLang(text_id, &status_str);
    MMIPUB_OpenTextDialogByTextPtr(PNULL, &status_str, PNULL, PNULL, &win_id, PNULL, MMIPUB_SOFTKEY_DIALOG_OK, HandleCheckStatusWindow);
}
PUBLIC BOOLEAN MMISET_GetCallWaitStatus(MN_DUAL_SYS_E dual_sys)
{
    BOOLEAN cw_status = FALSE;
    MN_RETURN_RESULT_E return_value = MN_RETURN_FAILURE;
    ...
    if(MN_DUAL_SYS_1 == dual_sys){
            MMINV_READ(MMINV_SET_CALL_WAIT_STATUS_SIM1,&cw_status,return_value);
    }else{
            MMINV_READ(MMINV_SET_CALL_WAIT_STATUS_SIM2,&cw_status,return_value);
    }

    if (MN_RETURN_SUCCESS != return_value)
    {
        cw_status = FALSE;
        MMISET_SetCallWaitStatus(dual_sys,cw_status);
    }

    return (cw_status); 
}

PUBLIC void MMISET_SetCallWaitStatus(MN_DUAL_SYS_E dual_sys,BOOLEAN cw_status)
{
    uint32 ret;
    ...
    if(MN_DUAL_SYS_1 == dual_sys){
            MMINV_WRITE(MMINV_SET_CALL_WAIT_STATUS_SIM1,&cw_status);	

    }else{
            MMINV_WRITE(MMINV_SET_CALL_WAIT_STATUS_SIM2,&cw_status);	
    }
    SCI_TraceLow("MMISET_SetCallWaitStatus: dual_sys= %d ,cw_status=%d",dual_sys,cw_status);
}
#endif
/*****************************************************************************/
...
#if 0 
	called_fuc_id= ID_SET_BAR_CANCEL_ALL;
                                ...
                                )
{
    ...
    case MN_SS_CFNRC:
        ...
        else
        {
            ...
            if(ss_result_ptr->sia_is_present && ss_oper != MN_SS_DEACTIVATESS)
            {
                return FALSE;
            }
        }
...
PUBLIC BOOLEAN MMIAPISET_GetIsAutoRedail(void)
{
    ...
    MN_RETURN_RESULT_E      return_value = MN_RETURN_FAILURE;
#ifdef CALL_AUTO_REDIAL//for queen bug 1060474
    ...
    if (MN_RETURN_SUCCESS != return_value)
    {
		...
		nv_reslut = EFS_NvitemRead(TELE_NV_CALL_CFG,(uint16)sizeof(config_value),(uint8*)&config_value);
		...
		{
			is_auto_redial = config_value;
		}
		MMIAPISET_SetIsAutoRedial(is_auto_redial);
    }
#endif
...
PUBLIC void MMIAPISET_SetSendCallLd(
                                      BOOLEAN  is_send_callid
                                      )
{
    MMINV_WRITE(MMINV_SET_CALL_SENDCALLID,&is_send_callid);
}

PUBLIC BOOLEAN MMIAPISET_GetSendCallLd(void)
{
    BOOLEAN                 is_send_callid = FALSE;
    MN_RETURN_RESULT_E      return_value = MN_RETURN_FAILURE;
    
    MMINV_READ(MMINV_SET_CALL_SENDCALLID,&is_send_callid,return_value);
    if (MN_RETURN_SUCCESS != return_value)
    {	
        is_send_callid = FALSE;
        MMINV_WRITE(MMINV_SET_CALL_SENDCALLID,&is_send_callid);
    }
    return (is_send_callid);
}


PUBLIC void MMIAPISET_SetForwardConditions(
                                      uint8 call_forward_conditions
                                      )
{
    MMINV_WRITE(MMINV_SET_CALL_FORWARD_CONDITIONS,&call_forward_conditions);
}

PUBLIC uint8 MMIAPISET_GetForwardConditions(void)
{
    uint8 call_forward_conditions = 4;
    MN_RETURN_RESULT_E return_value = MN_RETURN_FAILURE;
    
    MMINV_READ(MMINV_SET_CALL_FORWARD_CONDITIONS,&call_forward_conditions,return_value);
    if (MN_RETURN_SUCCESS != return_value)
    {
        call_forward_conditions = 4;
        MMIAPISET_SetForwardConditions(call_forward_conditions);
    }
    
    return (call_forward_conditions); 

}

PUBLIC void MMIAPISET_SetForwardSimCards(
                                      uint8 call_forward_simcard
                                      )
{
    MMINV_WRITE(MMINV_SET_CALL_FORWARD_SIMCARDS,&call_forward_simcard);
}

PUBLIC uint8 MMIAPISET_GetForwardSimCards(void)
{
    uint8 call_forward_simcard = 0;
    MN_RETURN_RESULT_E return_value = MN_RETURN_FAILURE;
    
    MMINV_READ(MMINV_SET_CALL_FORWARD_SIMCARDS,&call_forward_simcard,return_value);
    if (MN_RETURN_SUCCESS != return_value)
    {
        call_forward_simcard = 0;
        MMIAPISET_SetForwardSimCards(call_forward_simcard);
    }
    
    return (call_forward_simcard); 

}
//add by ljg
PUBLIC void MMISET_SetVoiceMailNumberToNV(MN_DUAL_SYS_E dual_sys, wchar *voicemail_number,uint8 len)
{
	if(PNULL == voicemail_number)return;
	if(dual_sys >= MMI_DUAL_SYS_MAX)
    {
        return ;
    }
    SCI_TRACE_LOW("[szp]MMISET_SetIVoiceMailNumberToNV,voicemail_number=%s", voicemail_number);
	if(MN_DUAL_SYS_1 == dual_sys){
		MMINV_WRITE(MMINV_SET_CALL_VOICEMAIL_SIMONE, voicemail_number);		
	}else{
		MMINV_WRITE(MMINV_SET_CALL_VOICEMAIL_SIMTWO, voicemail_number);	
	}
}

//add by ljg  
PUBLIC uint8 MMISET_GetIVoiceMailNumberToNV(MN_DUAL_SYS_E dual_sys,wchar *vm_number)
{
    MN_RETURN_RESULT_E return_value = MN_RETURN_FAILURE;
    wchar voicemail_temp[32] = {0};
    uint8 voice_mail[32]= {0};
    uint8 len = 0;
    char str[256] = {0};
    MMI_VOICEMAIL_ITEM_T voicemail_number[VOICEMAIL_MAX_COUNT] = {0};
    ...
    if(MN_DUAL_SYS_1 == dual_sys){
            MMINV_READ(MMINV_SET_CALL_VOICEMAIL_SIMONE,voicemail_temp,return_value);
    }else{
            MMINV_READ(MMINV_SET_CALL_VOICEMAIL_SIMTWO,voicemail_temp,return_value);
    }
    SCI_TRACE_LOW("[szp]MMISET_GetIVoiceMailNumberToNV,vm1tele_str=%s", voicemail_temp);
    if (MN_RETURN_SUCCESS != return_value)
    {
        if(MMI_MULTIVAR_ReadVoiceMail(voicemail_number)){
                MMI_STRNTOWSTR(voicemail_temp, VOICEMAIL_STRING_LEN,(uint8 *)voicemail_number[0].viocemail, VOICEMAIL_STRING_LEN, VOICEMAIL_STRING_LEN);
                len = MMIAPICOM_Wstrlen(voicemail_temp);
                MMIAPICOM_Wstrncpy(vm_number,voicemail_temp,len);
                return len;
        }else{
            return 0;
        }
...
PUBLIC uint8 MMISET_GetSimVoiceMailNumber(MN_DUAL_SYS_E dual_sys,wchar *vm_number)
{
    wchar voicemail_temp[32] = {0};
    uint8 voice_mail[32]= {0};
    uint8 len = 0;
    ...
    if (MN_DUAL_SYS_1 == dual_sys)
    {
        mail_len = MMIAPICOM_GenDispNumber(g_sim_vmnum_info.number_type, g_sim_vmnum_info.num_len, 
                                      g_sim_vmnum_info.bcd_num, voice_mail, MMISMS_PBNUM_MAX_LEN + 2);
    }
    else if (MN_DUAL_SYS_2 == dual_sys)
    {
        mail_len = MMIAPICOM_GenDispNumber(g_sim2_vmnum_info.number_type, g_sim2_vmnum_info.num_len, 
                                      g_sim2_vmnum_info.bcd_num, voice_mail, MMISMS_PBNUM_MAX_LEN + 2);
    }

    if (0 < mail_len)
    {
        MMI_STRNTOWSTR(voicemail_temp, VOICEMAIL_STRING_LEN, voice_mail, VOICEMAIL_STRING_LEN, VOICEMAIL_STRING_LEN);
        len = MMIAPICOM_Wstrlen(voicemail_temp);
        MMIAPICOM_Wstrncpy(vm_number,voicemail_temp,len);
    }
...
BOOLEAN MMIAPISET_GetIsConnectPrompt(void)
{
    ...
    if (MN_RETURN_SUCCESS != return_value)
    {
        is_connect_prompt = FALSE;//TRUE//change by James li
        MMINV_WRITE(MMINV_SET_CONNECT_PROMPT_STATUS,&is_connect_prompt);
    }
...
LOCAL void HandleAllQueryCF(MN_DUAL_SYS_E dual_sys, uint8 list_num, APP_MN_INTERROGATE_CNF_T *ss_info_ptr)
{
    ...
    memcpy(&s_cf_info_ptr[(uint32)((uint32)s_query_cf_num - 1) * sizeof(APP_MN_INTERROGATE_CNF_T)], ss_info_ptr, sizeof(APP_MN_INTERROGATE_CNF_T));
...
PUBLIC BOOLEAN MMISET_IsSSProcessing(void)
{
    ...
    SCI_TRACE_LOW("MMISET_IsSSProcessing g_is_ss_handling = %d", g_is_ss_handling);

    if (g_is_background_search_send || g_is_ss_user_query || g_is_for_cfu_icon || is_network_connecting || g_is_ss_handling)
    {
        return TRUE;
    }
...
PUBLIC BOOLEAN MMIAPISET_GetAutoHandFreeSetting(void)
{
    BOOLEAN is_autohandfree=FALSE;
    MN_RETURN_RESULT_E      return_value = MN_RETURN_FAILURE;

    MMINV_READ(MMINV_SET_AUTO_HANDFREE,&is_autohandfree,return_value);
    if (return_value == MN_RETURN_FAILURE)
    {
        is_autohandfree = FALSE;
        MMINV_WRITE(MMINV_SET_AUTO_HANDFREE,&is_autohandfree);
    }
    
    return is_autohandfree;
}
...
//	Note: 
/*****************************************************************************/
PUBLIC void MMIAPISET_SetAutoHandFreeSetting(BOOLEAN is_autohandfree)
{
    MMINV_WRITE(MMINV_SET_AUTO_HANDFREE,&is_autohandfree);
    return;
}
...
//The status is about: Don't rev push message with security code. 
//FALSE:   rev push message with security code. 
//TRUE:    Don't rev push message with security code. 
PUBLIC BOOLEAN MMIAPISET_GetOTASecStatus(MN_DUAL_SYS_E dual_sys)
{
    uint8       sec_value = 0;
    BOOLEAN enabled=FALSE;
    MN_RETURN_RESULT_E      return_value = MN_RETURN_FAILURE;

    MMINV_READ(MMINV_SET_OTA_SECURITY_ENABLE,&sec_value,return_value);
    if (return_value == MN_RETURN_FAILURE)
    {
        enabled = FALSE;
        sec_value = 0;
        MMINV_WRITE(MMINV_SET_OTA_SECURITY_ENABLE,&sec_value);
    }
    switch (dual_sys)
    {
        case MN_DUAL_SYS_1:
            enabled = 0x01 & sec_value;
            break;
        case MN_DUAL_SYS_2:
            enabled = 0x02 & sec_value;
            break;
        default:
            enabled = FALSE;
            break;
...
PUBLIC void MMIAPISET_SetOTASecStatus(MN_DUAL_SYS_E dual_sys,BOOLEAN enabled)
{
    uint8   sec_value = 0;
    MN_RETURN_RESULT_E      return_value = MN_RETURN_FAILURE;
    ...
    if(enabled)
        {
            sec_value = sec_value|(0x01<<dual_sys) ;
        }
    else
        {
        sec_value = sec_value &(0xFE-dual_sys);     //dual_sys_1 = 0, 0xFE;dual_sys_2 =1, 0xFD
    }
...
PUBLIC BOOLEAN MMISET_CloseDualsysCFNRC(MN_DUAL_SYS_E dual_sys)
{
    MN_SS_REGISTER_PARAM_U register_param = {0};/*lint !e64*/// parameter's union of register ss data operation
    SIMAT_DO_SS_STRING_T ss_string = {0};
    MNSS_SENDSS_RESULT_E ss_err_code = MNSS_SUCCESS;
