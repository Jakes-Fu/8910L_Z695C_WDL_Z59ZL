...
#define TXT_KL_PSW_EMERGENCY_CALL STXT_EMERGENCY_CALL_EXT01
...
BOOLEAN is_need_choose_sim1 = FALSE;
BOOLEAN is_need_choose_sim2 = FALSE;
uint8 s_keyguard_cur_id = 0;
...
LOCAL MMIPB_BCD_NUMBER_T *s_new_whitelist_num_ptr = PNULL;     
#endif

//#ifdef MMI_DEVICELOCK_SUPPORT
typedef struct  
{
    wchar     need_check_str[MMI_DEVICELOCKSCREEN_PWD_MAX_LEN];
    uint16 need_check_str_len;
    BOOLEAN     is_init_set;
}MMISET_SECURITY_CODE_CHECK_T;
//#endif
...
#endif
#if defined(MMI_WHITELIST_SUPPORT)
    MMISMS_PDA_SECURITY_SETTINGS_WHITELIST, 	
#endif   

#if defined(OMA_CP_SEC_SUPPORT)
    MMISET_PDA_OTA_SEC_ENABLE,
#endif
...
#endif
#if defined(MMI_WHITELIST_SUPPORT)
extern MMISET_WHITELIST_T *g_whitelist_ptr;
#endif
...
#if defined(MMI_WHITELIST_SUPPORT)
LOCAL MMI_RESULT_E  HandleWhitelistMenuWindow(
                                               MMI_WIN_ID_T    win_id, 
                                               MMI_MESSAGE_ID_E   msg_id, 
                                               DPARAM             param
                                               );

LOCAL MMI_RESULT_E HandleSetWhiteListWindow(
                                            MMI_WIN_ID_T        win_id,     //IN: 窗口的ID
                                            MMI_MESSAGE_ID_E     msg_id,     //IN: 窗口的内部消息ID
                                            DPARAM                 param        //IN: 相应消息的参数
                                            );

LOCAL MMI_RESULT_E  HandleWhitelistTypeWindow(
                                           MMI_WIN_ID_T    win_id, 
                                           MMI_MESSAGE_ID_E   msg_id, 
                                           DPARAM             param
                                           );

LOCAL void UpdateWhiteList(
                           MMI_CTRL_ID_T    ctrl_id        //IN:
                           );

LOCAL void SetWhitelistInfo(
                            uint16 index,
                            MMIPB_BCD_NUMBER_T *sender_ptr
                            );

LOCAL void AppendWhitelistItem(
                                MMI_CTRL_ID_T             ctrl_id,         //要设置的列表控件ID号 (输入参数)
                                uint16                    pos,
                                BOOLEAN                   is_update
                                );

LOCAL MMI_RESULT_E  HandleWhitelistOptWinMsg(
                                          MMI_WIN_ID_T        win_id, 
                                          MMI_MESSAGE_ID_E    msg_id, 
                                          DPARAM            param
                                          );

LOCAL MMI_RESULT_E  HandleEditWhitelistWindow(
                                           MMI_WIN_ID_T    win_id, 
                                           MMI_MESSAGE_ID_E   msg_id, 
                                           DPARAM             param
                                           );

LOCAL void HandleWhitelistMenuMsg(
                                 MMI_MENU_ID_T menu_id,    //IN: 被选中的菜单项
                                 MMI_CTRL_ID_T ctrl_id
                                 );


LOCAL BOOLEAN SaveWhitelistInfo(
                                uint16 index,
                                BOOLEAN is_new
                               );

LOCAL MMI_RESULT_E HandleWhitelistDelQueryWinMsg(
                                            MMI_WIN_ID_T        win_id,    //IN:
                                            MMI_MESSAGE_ID_E    msg_id,    //IN:
                                            DPARAM                param    //IN:
                                            );

LOCAL MMI_RESULT_E HandleWhitelistDelAllQueryWinMsg(
                                            MMI_WIN_ID_T        win_id,    //IN:
                                            MMI_MESSAGE_ID_E    msg_id,    //IN:
                                            DPARAM                param    //IN:
                                            );

LOCAL MMI_RESULT_E  HandleNewWhitelistWindow(
                                           MMI_WIN_ID_T    win_id, 
                                           MMI_MESSAGE_ID_E   msg_id, 
                                           DPARAM             param
                                           );

LOCAL MN_RETURN_RESULT_E DeleteWhitelistItem(
                                             uint16 pos
                                             );

LOCAL MN_RETURN_RESULT_E DeleteAllWhitelistItem(void);

LOCAL BOOLEAN SetWhitelistDetailFromPB(MMIPB_HANDLE_T handle);


LOCAL BOOLEAN SetWhitelistFromPB(MMIPB_HANDLE_T handle);

LOCAL void AddNumToWhitelist(
                                MMI_STRING_T mmi_pb_name,
                                MMIPB_BCD_NUMBER_T pb_bcd      
                                );


LOCAL void InitWhitelistInfo(void);

/*****************************************************************************/
//  Description : to handle add to whitelist query window    
//  Global resource dependence : 
//  Author:jixin.xu
//  Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleAddToWhiteListQueryWinMsg(
                                            MMI_WIN_ID_T        win_id,    //IN:
                                            MMI_MESSAGE_ID_E    msg_id,    //IN:
                                            DPARAM                param    //IN:
                                            );

LOCAL MMI_RESULT_E HandleWhitelistAddQueryWinMsg(
                                            MMI_WIN_ID_T        win_id,    //IN:
                                            MMI_MESSAGE_ID_E    msg_id,    //IN:
                                            DPARAM                param    //IN:
                                            );
...
//	Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E  HandleNewSecuritySetWindow(
                                            MMI_WIN_ID_T    win_id, 
                                            MMI_MESSAGE_ID_E   msg_id, 
                                            DPARAM             param
                                            );
...
/*****************************************************************************/
// 	Description : to handle KEYGUARD setting list
//	Global resource dependence : 
//  Author:niuhuiling
//	Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E  HandleKeyguardSetWindow(
                                            MMI_WIN_ID_T    win_id, 
                                            MMI_MESSAGE_ID_E   msg_id, 
                                            DPARAM             param
                                            );
...
/*****************************************************************************/
// 	Description : to handle KEYGUARD list setting list
//	Global resource dependence : 
//  Author:niuhuiling
//	Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E  HandleKeyguardListSetWindow(
                                            MMI_WIN_ID_T    win_id, 
                                            MMI_MESSAGE_ID_E   msg_id, 
                                            DPARAM             param
                                            );
...
//	Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E	HandleCallbarringSetWindow(
											MMI_WIN_ID_T	win_id, 
											MMI_MESSAGE_ID_E   msg_id, 
											DPARAM			   param
											);
...
//  Note: 
/*****************************************************************************/
LOCAL void  InitSIMLockUnlockSettingsListCtrl(MMI_CTRL_ID_T ctrl_id);
...
//  Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E  HandleSIMLockUnlockSettingMenuWindow
(
	MMI_WIN_ID_T		win_id, 
	MMI_MESSAGE_ID_E	msg_id, 
	DPARAM				param
...
/*****************************************************************************/
//  Description :open reset sim lock Unlock menu
//  Global resource dependence : 
//  Author:
//  Note: 
/*****************************************************************************/
LOCAL void OpenSIMLockUnlockWin(void);
...
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMIDEVICELOCKSCREEN_ReadPwd(MMISET_LOCKSCREEN_SECURITY_CODE_T* devicelock_code);
/*****************************************************************************/
//  Description : 
//  Global resource dependence :
//  Author: 
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMIDEVICELOCKSCREEN_SetPwd(const wchar *pwd_ptr, uint16 pwd_len);
...
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMIDEVICELOCKSCREEN_SetPwdEnable(BOOLEAN is_enable);

/*****************************************************************************/
//  Description : to handle phone lock input password
//  Global resource dependence :
//  Author:dong.chunguang
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E  HandlePhoneLockWindow(
                                         MMI_WIN_ID_T    win_id,
                                         MMI_MESSAGE_ID_E   msg_id,
                                         DPARAM             param
                                         );

LOCAL MMISET_LOCKSCREEN_SECURITY_CODE_T s_devicelock_code = {0};
//nhl modify 494866
LOCAL void MMIPHONE_OpenWrongLockAlertWindow(void);


LOCAL MMI_RESULT_E HandleWrongLockAlertWindow(
									MMI_WIN_ID_T win_id,
									MMI_MESSAGE_ID_E msg_id,
									DPARAM param
									);
//nhl modify 494866 end
...
// the window for input old power on password when power on
WINDOW_TABLE( MMIPHONE_PHONE_LOCK_PWD_WIN_TAB ) =
{
    WIN_FUNC( (uint32)HandlePhoneLockWindow ),
    WIN_ID( MMISET_SET_PHONE_LOCK_PWD_WIN_ID ),
    WIN_TITLE( TXT_INPUT_PHONE_PASSWORD ),
    CREATE_EDIT_PASSWORD_CTRL(MMISET_POWER_ON_PWD_MAX_LEN,MMISET_SET_PHONE_LOCK_PWD_CTRL_ID),
    WIN_SOFTKEY(TXT_COMMON_OK, TXT_NULL, STXT_RETURN),
    END_WIN
};
...
#if defined(MMI_WHITELIST_SUPPORT)
// the window for set whitelist
WINDOW_TABLE( MMISET_SET_SECURITY_WHITELIST_WIN_TAB ) = 
{
    //CLEAR_LCD,
    //WIN_PRIO( WIN_ONE_LEVEL ),
    WIN_FUNC( (uint32)HandleWhitelistMenuWindow ),	  
    WIN_ID( MMISET_SET_SECURITY_WHITELIST_WIN_ID ),
    WIN_TITLE( TXT_SET_WHITELIST ),
    CREATE_MENU_CTRL(MENU_WHITELIST_SET_OPT, MMISET_SET_WHITELIST_CTRL_ID),
    WIN_SOFTKEY(TXT_COMMON_OK, TXT_NULL, STXT_RETURN ),
    END_WIN
};

	//the window for whitelist win
WINDOW_TABLE( MMISET_SET_WHITELIST_LIST_WIN_TAB ) = 
{
    WIN_TITLE( TXT_SET_WHITELIST_LIST ),
    WIN_FUNC((uint32) HandleSetWhiteListWindow ),    
    WIN_ID( MMISET_SET_WHITELIST_LIST_WIN_ID ),
    WIN_SOFTKEY(STXT_OPTION, TXT_NULL, STXT_RETURN),
    
    CREATE_LISTBOX_CTRL( GUILIST_TEXTLIST_E, MMISET_SET_WHITELIST_LIST_CTRL_ID),
    END_WIN
};

WINDOW_TABLE( MMISET_SET_WHITELIST_TYPE_WIN_TAB ) = 
{
    //CLEAR_LCD,
    //WIN_PRIO( WIN_ONE_LEVEL ),
    WIN_FUNC( (uint32)HandleWhitelistTypeWindow ),    
    WIN_ID( MMISET_SET_WHITELIST_TYPE_WIN_ID ),
    WIN_TITLE(TXT_SET_WHITELIST_SETTING),
#ifndef MMI_PDA_SUPPORT
        WIN_SOFTKEY(STXT_SELECT, TXT_NULL, STXT_FINISH),
#endif
    CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMISET_SET_WHITELIST_TYPE_FROM_CTRL_ID),

    END_WIN
};


//the window for whitelist opt win
WINDOW_TABLE( MMISET_SET_WHITELIST_OPT_WIN_TAB ) = 
{
    WIN_FUNC( (uint32)HandleWhitelistOptWinMsg),
    WIN_ID(MMISET_SET_WHITELIST_OPT_WIN_ID),
    WIN_STYLE( WS_HAS_TRANSPARENT),
    CREATE_POPMENU_CTRL(MENU_WHITELIST_OPT,MMISET_SET_WHITELIST_LIST_OPT_CTRL_ID),
    WIN_SOFTKEY(TXT_COMMON_OK,TXT_NULL,STXT_RETURN),
    END_WIN
};

//the window for new whitelist 
WINDOW_TABLE( MMISET_SET_WHITELIST_NEW_WIN_TAB ) = 
{
    //CLEAR_LCD,
    //WIN_PRIO( WIN_ONE_LEVEL ),
    WIN_FUNC( (uint32)HandleNewWhitelistWindow ),    
    WIN_ID( MMISET_SET_WHITELIST_NEW_WIN_ID ),
    WIN_TITLE(TXT_SET_NEW_WHITELIST),
    #ifdef MMI_PDA_SUPPORT
    ...
    WIN_SOFTKEY(TXT_COMMON_OK, TXT_NULL, STXT_RETURN),
    #endif
    CREATE_FORM_CTRL(GUIFORM_LAYOUT_ORDER,MMISET_SET_WHITELIST_NEW_FROM_CTRL_ID),

    CHILD_FORM_CTRL(TRUE,GUIFORM_LAYOUT_ORDER,MMISET_SET_WHITELIST_SET_NAME_FORM_CTRL_ID,MMISET_SET_WHITELIST_NEW_FROM_CTRL_ID),
    CHILD_LABEL_CTRL(GUILABEL_ALIGN_LEFT,FALSE,MMISET_SET_WHITELIST_NEW_NAME_LABEL_CTRL_ID,MMISET_SET_WHITELIST_SET_NAME_FORM_CTRL_ID),
    CHILD_EDIT_TEXT_CTRL(TRUE,MMIPB_NAME_MAX_LEN,MMISET_SET_WHITELIST_NEW_NAME_CTRL_ID,MMISET_SET_WHITELIST_SET_NAME_FORM_CTRL_ID),

    CHILD_FORM_CTRL(TRUE,GUIFORM_LAYOUT_ORDER,MMISET_SET_WHITELIST_SET_NUM_FORM_CTRL_ID,MMISET_SET_WHITELIST_NEW_FROM_CTRL_ID),
    CHILD_LABEL_CTRL(GUILABEL_ALIGN_LEFT,FALSE,MMISET_SET_WHITELIST_NEW_NUM_LABEL_CTRL_ID,MMISET_SET_WHITELIST_SET_NUM_FORM_CTRL_ID),
    CHILD_EDIT_PHONENUM_CTRL(TRUE,MMISET_PHONE_NUM_MAX_LEN,MMISET_SET_WHITELIST_NEW_NUM_CTRL_ID,MMISET_SET_WHITELIST_SET_NUM_FORM_CTRL_ID),
    END_WIN
};


//the window for new whitelist 
WINDOW_TABLE( MMISET_SET_WHITELIST_EDIT_WIN_TAB ) = 
{
    //CLEAR_LCD,
    //WIN_PRIO( WIN_ONE_LEVEL ),
    WIN_FUNC( (uint32)HandleEditWhitelistWindow ),    
    WIN_ID( MMISET_SET_WHITELIST_EDIT_WIN_ID ),
    WIN_TITLE(TXT_SET_EDIT_WHITELIST),
#ifdef MMI_PDA_SUPPORT
    WIN_STYLE(WS_HAS_BUTTON_SOFTKEY),
#endif
    WIN_SOFTKEY(TXT_COMMON_OK, TXT_NULL, STXT_RETURN),
...
#endif /* MMI_GUI_STYLE_TYPICAL */
#endif 
//nhl add for security setting new

WINDOW_TABLE( MMISET_SET_NEW_SECURITY_WIN_TAB ) = 
{
    //CLEAR_LCD,
    //WIN_PRIO( WIN_ONE_LEVEL ),
	WIN_FUNC( (uint32)HandleNewSecuritySetWindow ),
    WIN_ID( MMISET_SET_NEW_SECURITY_WIN_ID ),
...
WINDOW_TABLE( MMISET_SET_KEYGUARD_WIN_TAB ) = 
{
    //CLEAR_LCD,
    //WIN_PRIO( WIN_ONE_LEVEL ),
	WIN_FUNC( (uint32)HandleKeyguardSetWindow ),
    WIN_ID( MMISET_SET_KEYGUARD_WIN_ID),
    WIN_SOFTKEY(NULL, STXT_SELECT, TXT_COMMON_COMMON_BACK),
    CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMISET_KEYGUARD_LIST_CTRL_ID),
    END_WIN
};

WINDOW_TABLE( MMISET_SET_CALLBARRING_WIN_TAB ) = 
{
    //CLEAR_LCD,
    //WIN_PRIO( WIN_ONE_LEVEL ),
	WIN_FUNC( (uint32)HandleCallbarringSetWindow ),
    WIN_ID( MMISET_SET_CALLBARRING_WIN_ID),
    WIN_SOFTKEY(NULL, TXT_COMMON_CHOOSE, TXT_COMMON_COMMON_BACK),
    CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMISET_CALLBARRING_LIST_CTRL_ID),
    END_WIN
};


WINDOW_TABLE( MMISET_SET_KEYGUARD_LIST_WIN_TAB ) = 
{
    //CLEAR_LCD,
    //WIN_PRIO( WIN_ONE_LEVEL ),
	WIN_FUNC( (uint32)HandleKeyguardListSetWindow ),
    WIN_ID( MMISET_SET_KEYGUARD_LIST_WIN_ID),
    WIN_SOFTKEY(NULL, STXT_SELECT, TXT_COMMON_COMMON_BACK),
    CREATE_LISTBOX_CTRL(GUILIST_RADIOLIST_E, MMISET_KEYGUARD_LIST_VALUE_CTRL_ID),
    END_WIN
};

//ADD END by nhl for security end
//nhl add keyguard nv begin 

/*****************************************************************************/
//  Description :MMIAPISET_SetKeyguardSwitch
//  Global resource dependence : 
//  Author:huiling.niu
//  Note:设置锁屏模式开关
/*****************************************************************************/
PUBLIC void MMIAPISET_SetKeyguardSwitch(BOOLEAN is_switch)
{
    MMINV_WRITE(MMINV_KEYGUARD_SETTING_SWITCH, &is_switch);
}
/*****************************************************************************/
//  Description :MMIAPISET_GetKeyguardSwitch
//  Global resource dependence : 
//  Author:huiling.niu
//  Note: 当前锁屏是否开启
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPISET_GetKeyguardSwitch(void)
{
    MN_RETURN_RESULT_E          return_value    =   MN_RETURN_FAILURE;
    BOOLEAN                     is_switch          =   FALSE;
    
    MMINV_READ(MMINV_KEYGUARD_SETTING_SWITCH, &is_switch, return_value);
...
#if defined(MMI_SIM_LOCK_TYPE2_SUPPORT) && !defined(MMI_SIM_LOCK_SUPPORT)
#ifdef MMI_SIM_LOCK_TYPE2_UNLOCK_SUPPORT
WINDOW_TABLE( MMISET_SIM_LOCK_UNLOCK_SETTING_WIN_TAB ) =
{
    //CLEAR_LCD,
    //WIN_PRIO( WIN_ONE_LEVEL ),
    WIN_FUNC( (uint32)HandleSIMLockUnlockSettingMenuWindow ),    
    WIN_ID( MMISET_SET_SECURITY_SIM_LOCK_WIN_ID ),
    WIN_TITLE(TXT_SET_SIM_LOCK),
#ifndef MMI_PDA_SUPPORT
    WIN_SOFTKEY(STXT_SELECT, TXT_NULL, STXT_RETURN),
#endif
    CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMISET_SECURITY_SIMLOCK_UNLOCK_MENU_FORM_CTRL_ID),
    END_WIN
};
...
#if defined(MMI_WHITELIST_SUPPORT)
/*****************************************************************************/
// 	Description : whitelist select contact callback
//	Global resource dependence : 
//  Author:
//	Note: 
/*****************************************************************************/
LOCAL void AddWhiteListSelectContactCallback(MMIPB_HANDLE_T handle)
{
    MMK_SendMsg(MMISET_SET_WHITELIST_NEW_WIN_ID, MMI_SELECT_CONTACT_CNF, (DPARAM)handle);      
}

/*****************************************************************************/
// 	Description : edit whitelist select contact callback
//	Global resource dependence : 
//  Author:
//	Note: 
/*****************************************************************************/
LOCAL void EditWhiteListSelectContactCallback(MMIPB_HANDLE_T handle)
{
    MMK_SendMsg(MMISET_SET_WHITELIST_EDIT_WIN_ID, MMI_SELECT_CONTACT_CNF, (DPARAM)handle);      
}

/*****************************************************************************/
// 	Description : set whitelist select contact callback
//	Global resource dependence : 
//  Author:
//	Note: 
/*****************************************************************************/
LOCAL void SetWhiteListSelectContactCallback(MMIPB_HANDLE_T handle)
{
    MMK_SendMsg(MMISET_SET_WHITELIST_LIST_WIN_ID, MMI_SELECT_CONTACT_CNF, (DPARAM)handle);      
}
#endif
...
LOCAL void SetPrivacyProtectionList(void)
{
    ...
    {
	    MMI_GetLabelTextByLang(STR_POC_ILIST_OFF, &label_name);
	    GUILABEL_SetText(MMISET_SECURITY_PRIVACY_EDIT_LABEL_CTRL_ID, &label_name, FALSE);
    }
}
/*****************************************************************************/
...
/*****************************************************************************/
// 	Description : Get plmn for Poland
//	Global resource dependence : 
//	Note: 
/*****************************************************************************/
LOCAL BOOLEAN GetCurCarrierPlmn(void)
{
         MN_PLMN_T plmn1 = {0};
         MN_PLMN_T plmn2 = {0};

#ifdef MMI_MULTI_SIM_SYS_SINGLE
        plmn1 = MNSIM_GetHplmnEx(MN_DUAL_SYS_1);
        if((260 == plmn1.mcc)&&(06 == plmn1.mnc))
        {
            return TRUE;
        }
#else
        plmn1 = MNSIM_GetHplmnEx(MN_DUAL_SYS_1);
        plmn2 = MNSIM_GetHplmnEx(MN_DUAL_SYS_2);

        SCI_TRACE_LOW("enter ljg*GetCurCarrierPlmn::mcc1=%d,mnc1=%d,mcc2=%d,mnc2=%d", plmn1.mcc, plmn1.mnc, plmn2.mcc, plmn2.mnc);

        if(((260 == plmn1.mcc)&&(06 == plmn1.mnc))||((260 == plmn2.mcc)&&(06 == plmn2.mnc)))
        {
            return TRUE;
        }
#endif
...
//add by nhl for new windows security settings
LOCAL MMI_RESULT_E  HandleNewSecuritySetWindow(
                                            MMI_WIN_ID_T    win_id, 
                                            MMI_MESSAGE_ID_E   msg_id, 
                                            DPARAM             param
    ...
    switch(msg_id)
    {
     case MSG_OPEN_WINDOW:
	    ...
	    if(GetCurCarrierPlmn())
            ...
            GUILIST_SetMaxItem(ctrl_id,item_cnt, FALSE );
            GUILIST_SetListState(ctrl_id, GUILIST_STATE_SPLIT_LINE, FALSE);
	    item_t.item_style    = GUIITEM_STYLE_ONE_LINE_TEXT_MS;
        item_t.item_data_ptr = &item_data;
	    item_data.item_content[0].item_data_type     = GUIITEM_DATA_TEXT_ID;            
        item_data.item_content[0].item_data.text_id = STR_SETT_LIST_KEYGUARD_EXT01;
	    ...
	    GUILIST_AppendItem( ctrl_id, &item_t );  
    	 ...
    	 if(!GetCurCarrierPlmn()){	
                ...
                item_t.item_data_ptr = &item_data;
                item_data.item_content[0].item_data_type     = GUIITEM_DATA_TEXT_ID;            
                item_data.item_content[0].item_data.text_id = TXT_SET_CALL_BAR;
                //item_data.softkey_id[0] = TXT_COMMON_OK;
                //item_data.softkey_id[1] = TXT_COMMON_CHOOSE;
                //item_data.softkey_id[2] = IMAGE_COMMON_BACK;
                GUILIST_AppendItem( ctrl_id, &item_t );
	 ...
	 //add for bug 486822
// 	 case MSG_FULL_PAINT:
//         MMI_GetLabelTextByLang(STR_SETT_HDR_SECURITY_EXT01, &temp_str);
...
// 	 	break;
  ...
  //  case MSG_APP_WEB:
    case MSG_CTL_MIDSK:
	 ...
	 {
	 case 0:
	     MMK_CreateWin((uint32*)MMISET_SET_KEYGUARD_WIN_TAB, PNULL);
	     break;
	 case 1:
	 	//add by ljg	
	     MMK_CreateWin((uint32*)MMISET_SET_CALLBARRING_WIN_TAB, (ADD_DATA)cur_item);
...
//        
/*****************************************************************************/
PUBLIC void MMISET_DisplaybannerInfo(MMI_WIN_ID_T win_id, MMI_TEXT_ID_T text_id, MMI_STRING_T *text_str)
{
...
//        
/*****************************************************************************/
PUBLIC void MMISET_SendSecurityCodeChangeMsg(void)
{
	BOOLEAN need_set_list = TRUE;
	if(MMK_IsOpenWin(MMISET_SET_KEYGUARD_WIN_ID))
	{
		MMK_SendMsg(MMISET_SET_KEYGUARD_WIN_ID, MSG_FULL_PAINT, &need_set_list);
	}
}
...
//  Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E  HandleCheckNewSecurityCodeDialogWindow
(
 MMI_WIN_ID_T     win_id, 
 MMI_MESSAGE_ID_E msg_id, 
 DPARAM           param 
 )
{
	...
	MMISET_SECURITY_CODE_CHECK_T* need_check = PNULL;
	...
	need_check= (MMISET_SECURITY_CODE_CHECK_T *)add_data_ptr->user_data;
	if(PNULL==need_check)
	{
		recode = MMI_RESULT_FALSE;
		return recode;
	...
	{
	case MSG_OPEN_WINDOW:
		GUIEDIT_SetMaxLen(input_field_ctrl_id,MMI_DEVICELOCKSCREEN_PWD_MAX_LEN);
        ...
        GUIEDIT_ConfigImCustomKey(input_field_ctrl_id, input_mode, GUIIM_CUSTOM_KEY_STAR, dot_str, 1);
        GUIEDIT_SetImTag(input_field_ctrl_id, GUIIM_TAG_DOT_NUM);
        MMIPUB_SetDialogBgLayerAlpha(win_id, 127);
		break;
	case MSG_FULL_PAINT:
		GUIEDIT_GetString(input_field_ctrl_id,&string_info);
	...
	case MSG_APP_WEB:
		{
			...
			if(str.wstr_len < MMI_DEVICELOCKSCREEN_PWD_MIN_LEN)
			{
				MMISET_DisplaybannerInfo(win_id, STR_PASSWORD_CHARACTERS_EXT01, PNULL);
				GUIEDIT_ClearAllStr(input_field_ctrl_id);
			}
			else
			{
				if( need_check->need_check_str_len == str.wstr_len
					&& 0 == MMIAPICOM_Wstrncmp(need_check->need_check_str, (uint8 *)str.wstr_ptr, str.wstr_len))
				{
					MMIDEVICELOCKSCREEN_SetPwd(str.wstr_ptr, str.wstr_len);
					if(need_check->is_init_set)
					{
						MMISET_DisplaybannerInfo(win_id, STXT_SECURITY_CODE_SET_EXT01, PNULL);
						MMIDEVICELOCKSCREEN_SetPwdEnable(TRUE);
						MMISET_SendSecurityCodeChangeMsg();
					}
					else
					{
						MMISET_DisplaybannerInfo(win_id, STXT_SECURITY_CODE_CHANGED, PNULL);
					}
					
				}
				else
				{
					MMISET_DisplaybannerInfo(win_id, STR_CERTMAN_PW_MISMATCH, PNULL);
				}
	...
	default:
		...
		GUIEDIT_SetIm(input_field_ctrl_id,input_mode,input_mode);
		break;
	}
	return (recode);
}
...
//  Note: 
/*****************************************************************************/
LOCAL void MMISET_OpenCheckNewSecurityCodeDialogWin(MMI_STRING_T *input_str, BOOLEAN is_init_set)
{
	MMI_STRING_T title = {0};
	MMI_WIN_ID_T win_id = MMISET_CHECK_NEW_SECURITY_CODE_WIN_ID;
	MMISET_SECURITY_CODE_CHECK_T* check_ptr = PNULL;
	if(PNULL == input_str || PNULL == input_str->wstr_ptr)
	{
		return;
	}
	check_ptr =  (MMISET_SECURITY_CODE_CHECK_T*)SCI_ALLOCA(sizeof(MMISET_SECURITY_CODE_CHECK_T));
	if(PNULL == check_ptr)
	{
		return;
	}
	SCI_MEMSET(check_ptr, 0x00, sizeof(MMISET_SECURITY_CODE_CHECK_T));
	check_ptr->is_init_set = is_init_set;
	check_ptr->need_check_str_len = MIN(input_str->wstr_len,MMI_DEVICELOCKSCREEN_PWD_MAX_LEN);
	MMIAPICOM_Wstrncpy(check_ptr->need_check_str, input_str->wstr_ptr, check_ptr->need_check_str_len);
	MMI_GetLabelTextByLang(STR_SETT_INPUT_CHECK_SECUR_CODE_EXT01, &title );
	MMIPUB_OpenDialogPasswordWinByTextPtrEx(
		...
		HandleCheckNewSecurityCodeDialogWindow,
		(uint32)check_ptr
		);
}
...
//  Note: 
/*****************************************************************************/
LOCAL void MMISET_OpenNewSecurityCodeDialogWin(BOOLEAN is_init_set)
{
	MMI_STRING_T title = {0};
	MMI_WIN_ID_T win_id = PNULL/*MMISET_NEW_SECURITY_CODE_WIN_ID*/;	
	BOOLEAN* is_init_set_ptr =  (BOOLEAN*)SCI_ALLOCA(sizeof(BOOLEAN));
	SCI_MEMSET(is_init_set_ptr, 0x00, sizeof(BOOLEAN));
	*is_init_set_ptr = is_init_set;
	MMI_GetLabelTextByLang(STR_SETT_INPUT_NEW_SECUR_CODE_EXT01, &title );
...
//  Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E  HandleCheckCurSecurityCodeDialogWindow
(
 MMI_WIN_ID_T     win_id, 
 MMI_MESSAGE_ID_E msg_id, 
 DPARAM           param 
 )
{
	...
	is_set_new= (BOOLEAN *)add_data_ptr->user_data;
	if(PNULL==is_set_new)
	{
		recode = MMI_RESULT_FALSE;
		return recode;
	...
	case MSG_APP_WEB:
		{
			...
			else
			{
				...
				{
					if( device_lock_pwd.pwd_len == str.wstr_len
						&& 0 == MMIAPICOM_WstrStrncmp(str.wstr_ptr, (uint8 *)device_lock_pwd.pwd, str.wstr_len))
					{
						if(*is_set_new)
						{
							MMISET_OpenNewSecurityCodeDialogWin(FALSE);
						}
						else
						{
							if(device_lock_pwd.is_enable)
							{
								MMIDEVICELOCKSCREEN_SetPwdEnable(FALSE);
							}
							else
							{
								MMIDEVICELOCKSCREEN_SetPwdEnable(TRUE);
...
{
	MN_RETURN_RESULT_E   return_value = MN_RETURN_FAILURE;
	uint8 pwd_num[MMI_DEVICELOCKSCREEN_PWD_MAX_LEN] =   {0};
	
	SCI_MEMSET(devicelock_code, 0 , sizeof(MMISET_LOCKSCREEN_SECURITY_CODE_T));
...
PUBLIC BOOLEAN MMIDEVICELOCKSCREEN_SetPwd(const wchar *pwd_ptr, uint16 pwd_len)
{
	...
	MMI_WSTRNTOSTR((uint8 *)s_devicelock_code.pwd,MMI_DEVICELOCKSCREEN_PWD_MAX_LEN,pwd_ptr,pwd_len,pwd_len);
	MMINV_WRITE(MMINV_DEVICELOCKSCREEN_PWD,&s_devicelock_code);
	return TRUE;
 }
...
//  Note: 
/*****************************************************************************/
LOCAL void MMISET_OpenCheckCurSecurityCodeDialogWin(BOOLEAN set_new)
{
	MMI_STRING_T title = {0};
	MMI_WIN_ID_T win_id = MMISET_CHECK_CURRENT_SECURITY_CODE_WIN_ID;	
	BOOLEAN* is_set_new_ptr =  (BOOLEAN*)SCI_ALLOCA(sizeof(BOOLEAN));
	SCI_MEMSET(is_set_new_ptr, 0x00, sizeof(BOOLEAN));
	*is_set_new_ptr = set_new;
	MMI_GetLabelTextByLang(STR_SETT_INPUT_CURRENT_SECUR_CODE_EXT01, &title );
	MMIPUB_OpenDialogPasswordWinByTextPtrEx(
		MMK_GetFirstAppletHandle(),
		&title , 
		&win_id,
		PNULL,
		MMIPUB_SOFTKEY_DIALOG_OK_CLOSE,
		HandleCheckCurSecurityCodeDialogWindow,
		(uint32)is_set_new_ptr
		);
}
//#endif /*MMI_DEVICELOCK_SUPPORT*/

/*****************************************************************************/
// 	Description : to add keyguard  windows
//	Global resource dependence : 
//  Author:huiling.niu
//	Note: 
/*****************************************************************************/
LOCAL void  SetKeyguardSetWinList(MMI_WIN_ID_T win_id, MMI_CTRL_ID_T list_ctrl_id )
{
	...
	uint16   cur_item = 0;
	int i=0;
	MMI_TEXT_ID_T               str_id[] = {STXT_KEYGUARD_AUTO_ON_EXT01,STXT_KEYGUARD_AUTO_OFF_EXT01};
	//#ifdef MMI_DEVICELOCK_SUPPORT
	MMISET_LOCKSCREEN_SECURITY_CODE_T device_lock_pwd = {0};
	//#endif /*MMI_DEVICELOCK_SUPPORT*/
	...
	item_t.item_data_ptr = &item_data;
	item_data.item_content[0].item_data_type     = GUIITEM_DATA_TEXT_ID;            
	item_data.item_content[0].item_data.text_id = STXT_KEYGUARD_AUTO_ACT;
	item_data.item_content[1].item_data_type    = GUIITEM_DATA_TEXT_ID;
	//nhl add for keyguard nv
	if(MMIAPISET_GetKeyguardSwitch())
	{
		item_data.item_content[1].item_data.text_id = str_id[0];
	}
	else
	{
		item_data.item_content[1].item_data.text_id = str_id[1];
	}
	//nhl add for 493381
...
// 	GUILIST_SetItemContentOpacityFontColor(&item_data,1,255*0.6,255*0.6);
	...
	item_t.item_data_ptr = &item_data;
	item_data.item_content[0].item_data_type     = GUIITEM_DATA_TEXT_ID;            
	item_data.item_content[0].item_data.text_id = STR_SECURITY_CODE_EXT01;
	item_data.softkey_id[0] = TXT_NULL;
...
//#ifdef MMI_DEVICELOCK_SUPPORT
	if(MMIDEVICELOCKSCREEN_ReadPwd(&device_lock_pwd))
	{
		GUILIST_SetToggleState(list_ctrl_id, cur_item, device_lock_pwd.is_enable);
		if(device_lock_pwd.is_enable)
		{
			cur_item++;
			item_t.item_style    = GUIITEM_STYLE_ONE_LINE_TEXT_MS;
			item_t.item_data_ptr = &item_data;
			item_data.item_content[0].item_data_type     = GUIITEM_DATA_TEXT_ID;            
			item_data.item_content[0].item_data.text_id = STR_SETT_LIST_CHANGE_SECUR_CODE_EXT01;
			item_data.softkey_id[0] = TXT_NULL;
			item_data.softkey_id[1] = STXT_SELECT;
			item_data.softkey_id[2] = TXT_COMMON_COMMON_BACK;
			GUILIST_AppendItem( list_ctrl_id, &item_t );
...
//#endif /*MMI_DEVICELOCK_SUPPORT*/
	{
		GUILIST_SetToggleState(list_ctrl_id, cur_item, FALSE);
	}
	//for bug486941
	if(s_keyguard_cur_id>cur_item)
		s_keyguard_cur_id = 0;
	GUILIST_SetCurItemIndex(list_ctrl_id, s_keyguard_cur_id);
	//end
	MMK_SetAtvCtrl(win_id,list_ctrl_id);
}

//bug745575 begin
LOCAL BOOL initKeyguardEnableStatus()
{
    MMISET_LOCKSCREEN_SECURITY_CODE_T device_lock_pwd = {0};
    BOOLEAN ret = FALSE;	
    ...
    if(MMIDEVICELOCKSCREEN_ReadPwd(&device_lock_pwd))
    {
        ret = MMIDEVICELOCKSCREEN_ReadPwd(&s_devicelock_code);
        if (device_lock_pwd.is_enable)
        {
            s_devicelock_code.is_enable = TRUE;
        }
        return TRUE;
    }
    return FALSE;
...
static MMI_WIN_ID_T s_hero_keyguard_win_id = 0;
#endif
 ...
 )
{
	...
	MMI_MENU_ID_T               menu_id         =   0;
	MMI_RESULT_E                recode          =   MMI_RESULT_TRUE;
	MMI_CTRL_ID_T               ctrl_id = MMISET_KEYGUARD_LIST_CTRL_ID;
	uint16   cur_item = 0;
	int i=0;
	MMI_STRING_T    temp_str = {0};
	//#ifdef MMI_DEVICELOCK_SUPPORT
	...
	case MSG_OPEN_WINDOW:
            ...
            if (!isInited)
            {
                isInited = initKeyguardEnableStatus();
            }
#ifdef PHONE_NUMBER_VOICE
		s_hero_keyguard_win_id = win_id;
#endif

            //bug745575 end
            SetKeyguardSetWinList(win_id, ctrl_id);
            //GUILIST_SetSelectedItem(ctrl_id, cur_item, TRUE);
            //GUILIST_SetCurItemIndex(ctrl_id, cur_item);
            break;
	case MSG_FULL_PAINT:
		SetKeyguardSetWinList(win_id, ctrl_id);
// 		MMI_GetLabelTextByLang(STR_SETT_HDR_KEYGUARD_EXT01, &temp_str);
// 		if(MMIAPICOM_StatusAreaSetViewHeaderinfo(temp_str.wstr_ptr, temp_str.wstr_len))
// 		{
// 		GUIWIN_UpdateStb();
// 		}
		if(PNULL !=param && TRUE == *(BOOLEAN*)param)
		{
			...
			if(MMIDEVICELOCKSCREEN_ReadPwd(&device_lock_pwd))
			{
				GUILIST_SetToggleState(ctrl_id, 1, device_lock_pwd.is_enable);
			}
			else
			{
	...
	case MSG_CTL_MIDSK:
		cur_item = GUILIST_GetCurItemIndex(ctrl_id);
		s_keyguard_cur_id = cur_item;//for bug486941
		switch (cur_item)
		{
		case 0:
			MMK_CreateWin((uint32*)MMISET_SET_KEYGUARD_LIST_WIN_TAB, PNULL);
			break;
//#ifdef MMI_DEVICELOCK_SUPPORT
		case 1:
			{
				if(MMIAPISET_IsPowerOnSecruityCodeEnable()/*MMIDEVICELOCKSCREEN_ReadPwd(&device_lock_pwd)*/)
				{
					MMISET_OpenCheckCurSecurityCodeDialogWin(FALSE);
				}
				else
				{
					MMISET_OpenNewSecurityCodeDialogWin(TRUE);
				}
			}
			break;
		case 2:
			{
				...
				if(MMIDEVICELOCKSCREEN_ReadPwd(&device_lock_pwd))
				{
					MMISET_OpenCheckCurSecurityCodeDialogWin(TRUE);
				}
				else
				{
					MMISET_OpenNewSecurityCodeDialogWin(TRUE);
...
PUBLIC MMI_TEXT_ID_T MMIAPIPHONE_GetCarrierName(MN_DUAL_SYS_E dual_sys){
/*
	if(( PLMN_CMCC == MMIAPIPHONE_GetNetworkName(dual_sys))||
		( PLMN_460_02 == MMIAPIPHONE_GetNetworkName(dual_sys))||
		( PLMN_460_07 == MMIAPIPHONE_GetNetworkName(dual_sys))
		){
		return TXT_MAINMENU_CHINAMOBILE;

	}if(PLMN_UNICOM == MMIAPIPHONE_GetNetworkName(dual_sys)){ 
		return TXT_NET_CHINA_UNICOM;
	}else{//should be no sim
		return STR_NSM_IND_NO_SIM;//STR_ILIST_MEMORY_CARD_EXT01;
	}
*/
    return STR_NSM_IND_NO_SIM;//STR_ILIST_MEMORY_CARD_EXT01;
}
...
/*****************************************************************************/
// 	Description : update callbarring stauts
//	Global resource dependence : 
//	Note: 
/*****************************************************************************/

LOCAL void UpdateCallbarringStatusWindows(uint8  timer_id, uint32 param)
{
		GUILIST_ITEM_T      item_t = {0};
    	GUILIST_ITEM_DATA_T item_data = {0};
		uint16   cur_item = 0;
		...
		MMI_CTRL_ID_T               ctrl_id = MMISET_CALLBARRING_LIST_CTRL_ID;
		MMI_WIN_ID_T win_id =MMISET_SET_CALLBARRING_WIN_ID;
...
// 		MMIAPICOM_StatusAreaSetViewHeaderinfo(title_str.wstr_ptr, title_str.wstr_len);
...
#endif
		if ((MMIAPIPHONE_IsSimAvailable(MN_DUAL_SYS_1))&&(IsCampOnOperator(MN_DUAL_SYS_1)))
		{
			is_need_choose_sim1 = TRUE;
			MMIPHONE_GetNetworkNameString(MN_DUAL_SYS_1,&network_str);
		...
		{
			item_data.item_content[1].item_data_type    = GUIITEM_DATA_TEXT_ID;
			item_data.item_content[1].item_data.text_id = TXT_KL_PSW_EMERGENCY_CALL;
		}
    ...
    if(!MMIAPIPHONE_IsSimAvailable(MN_DUAL_SYS_1)){	
		...
		if ((MMIAPIPHONE_IsSimAvailable(MN_DUAL_SYS_2))&&(IsCampOnOperator(MN_DUAL_SYS_2)))
		{
			is_need_choose_sim2= TRUE;
			SCI_MEMSET(&network_str, 0x00, sizeof(network_str));
			network_str.wstr_ptr = (wchar *)opn_spn_name;
			MMIPHONE_GetNetworkNameString(MN_DUAL_SYS_2,&network_str);
			item_data.item_content[1].item_data_type    = GUIITEM_DATA_TEXT_BUFFER;
			item_data.item_content[1].item_data.text_buffer.wstr_ptr = network_str.wstr_ptr;
			item_data.item_content[1].item_data.text_buffer.wstr_len = network_str.wstr_len;
		}
...
/*****************************************************************************/
// 	Description : to add carring  windows
//	Global resource dependence : 
//	Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E  HandleCallbarringSetWindow(
...
// 		MMIAPICOM_StatusAreaSetViewHeaderinfo(title_str.wstr_ptr, title_str.wstr_len);
        ...
        item_data.item_content[0].item_data.text_id = STR_LIST_SIM_EXT01;
		item_data.item_content[1].item_data_type    = GUIITEM_DATA_TEXT_ID;
		item_data.item_content[1].item_data.text_id = STR_SEARCH_NETWORK_EXT01;
		item_data.softkey_id[0] = COMMON_TXT_NULL;
        item_data.softkey_id[1] = STXT_SELECT;
        item_data.softkey_id[2] = TXT_COMMON_COMMON_BACK;
	    GUILIST_AppendItem( ctrl_id, &item_t);  
...
// 		MMIAPICOM_StatusAreaSetViewHeaderinfo(title_str.wstr_ptr, title_str.wstr_len);
    ...
    if(!MMIAPIPHONE_IsSimAvailable(MN_DUAL_SYS_2)){	
        ...
        GUILIST_SetSelectedItem(ctrl_id, cur_item, TRUE);
        GUILIST_SetCurItemIndex(ctrl_id, cur_item);
		MMK_CreateTimerCallback(2000, UpdateCallbarringStatusWindows, NULL, FALSE);
    	break;
// 	case MSG_FULL_PAINT:
// 		MMIRES_GetText(STR_SIM_HDR_SELECTION_EXT01, PNULL, &title_str);
// 		MMIAPICOM_StatusAreaSetViewHeaderinfo(title_str.wstr_ptr, title_str.wstr_len);	
// 		break;
	case MSG_CTL_MIDSK:
	 ...
	 {
	 case 0:
	 	 if(!is_need_choose_sim1){
		 	is_need_choose_sim1 = FALSE;
			break;
		 }
		 s_cur_dual_sys = 0; 
	     MMK_CreateWin((uint32*)MMISET_SET_CALL_BAR_WIN_TAB, (ADD_DATA)cur_item);
	     break;
#ifdef MMI_MULTI_SIM_SYS_DUAL		 
	 case 1:
	 	 if(!is_need_choose_sim2){
		 	is_need_choose_sim2 = FALSE;
			break;
		 }
		 s_cur_dual_sys = 1;
	     MMK_CreateWin((uint32*)MMISET_SET_CALL_BAR_WIN_TAB, (ADD_DATA)cur_item);
...
/*****************************************************************************/
// 	Description : to add keyguard list  windows
//	Global resource dependence : 
//  Author:huiling.niu
//	Note: 
/*****************************************************************************/
                                            ...
                                            )
{
    ...
    MMI_CTRL_ID_T               ctrl_id = MMISET_KEYGUARD_LIST_VALUE_CTRL_ID;
    GUILIST_ITEM_T      item_t = {0};
    GUILIST_ITEM_DATA_T item_data = {0};
    uint16   cur_item = 0;
    int i=0;
...
// 		MMI_GetLabelTextByLang(STR_KEYG_HDR_AUTOMATIC, &temp_str);
// 		if(MMIAPICOM_StatusAreaSetViewHeaderinfo(temp_str.wstr_ptr, temp_str.wstr_len))
// 		{
// 		GUIWIN_UpdateStb();
// 		}
    ...
    case MSG_CTL_MIDSK:
	 ...
	 {
	 case 0:
	     MMIAPISET_SetKeyguardSwitch(TRUE);
		 MMK_CloseWin(win_id);
	     break;
	 case 1:
	    MMIAPISET_SetKeyguardSwitch(FALSE);
		MMK_CloseWin(win_id);
	     break;
	 default:
            break;
...
#endif
#if defined(MMI_WHITELIST_SUPPORT)
        case MMISET_WHITELIST:
            if(MMISET_InitWhitelistParam())
            {
                MMISET_GetWhiteList();
                MMK_CreateWin((uint32*)MMISET_SET_SECURITY_WHITELIST_WIN_TAB,PNULL);
            }
            break; 
#endif
...
#if 0
ndef MMI_KEY_LOCK_BACKLIGHT_IN_1_SUPPORT
    if((is_need_replace && index == focus_index) ||!is_need_replace)
    {
    switch (MMIAPISET_GetAutoBoardKey())
    {
...
ndef MMI_PDA_SUPPORT
    ...
    index++;
        ...
        if(!is_need_replace )
        {
            ...
            ctrl_id,
            TXT_SET_WHITELIST,
            (uint32)MMISMS_PDA_SECURITY_SETTINGS_WHITELIST);
        }    
    index++;	
#endif
    ...
    if((is_need_replace && index == focus_index) || !is_need_replace)
        {
        if (MMIAPISET_GetOTASecStatus(MN_DUAL_SYS_1))
        {
            image_id = IMAGE_CHECK_SELECTED_ICON;
        }
        else
        ...
        }
        {
            AppendPdaSecuritySettingsItem2Line(            
            ctrl_id,
            TXT_SET_OTA_SEC_ENABLE,
            text_str,
            image_id,
            (uint32)MMISET_PDA_OTA_SEC_ENABLE,
            is_need_replace);
        }
        }
#endif            
...
#ifndef MMI_PIN_SETTING_OFF //for monkey test
#ifndef MMI_MULTI_SIM_SYS_SINGLE             
...
#endif
#if defined(MMI_WHITELIST_SUPPORT)
            case MMISMS_PDA_SECURITY_SETTINGS_WHITELIST:
            if(MMISET_InitWhitelistParam())
            {
                MMISET_GetWhiteList();
                MMK_CreateWin((uint32*)MMISET_SET_SECURITY_WHITELIST_WIN_TAB,PNULL);
...
#if defined(OMA_CP_SEC_SUPPORT)
            case MMISET_PDA_OTA_SEC_ENABLE:
            {
                BOOLEAN flag = !MMIAPISET_GetOTASecStatus(0);
                MMIAPISET_SetOTASecStatus(0,flag);
                MMK_PostMsg(win_id, MSG_SET_BLACKLIST_LIST_UPDATE, PNULL, NULL);
                MMK_PostMsg(win_id, MSG_FULL_PAINT, PNULL, NULL);
                break;
            }
                                         ...
                                         )
{
    ...
    case MSG_FULL_PAINT:
        ...
        {
            MMI_GetLabelTextByLang(STR_POC_ILIST_OFF, &label_string);
            GUILABEL_SetText(MMISET_PIN1_EDIT_LABEL_CTRL_ID, &label_string, FALSE); 
        }
        break;
...
#endif
    MMIAPISET_AppendListItemByTextId(STR_CAM_ILIST_MEMORY_PHN_MEMORY,STXT_SELECT,ctrl_id,GUIITEM_STYLE_ONE_LINE_CHECK);
...
//    MN_RETURN_RESULT_E      return_value = MN_RETURN_FAILURE;
    ...
    //read power on password from
    InitPhonePwd(&phone_code);
       sprintf((char*)pwd_num, "%s" , MMISET_PRIVACY_PROTECT_BH_PWD);
...
#ifdef MMI_SIM_LOCK_TYPE2_UNLOCK_SUPPORT
/*****************************************************************************/
//  Description :open unlock sim lock menu
//  Global resource dependence : 
//  Author:
//  Note: 
/*****************************************************************************/
PUBLIC void MMIAPISET_OpenSIMLockNetUnlockWin(void)
{
    MMK_CreateWin((uint32*)MMISET_SIM_LOCK_UNLOCK_SETTING_WIN_TAB, PNULL);
}
                                                                                                       ...
                                                                                                       )
{
   ...
   MMI_RESULT_E recode = MMI_RESULT_TRUE;
   MMI_CTRL_ID_T ctrl_id = MMISET_SECURITY_SIMLOCK_UNLOCK_MENU_FORM_CTRL_ID;
   
   switch(msg_id)
   {
    case MSG_OPEN_WINDOW:
        InitSIMLockUnlockSettingsListCtrl(ctrl_id);
        MMK_SetAtvCtrl(win_id, ctrl_id);        
        break;
...
/*****************************************************************************/
//              A   B   C   D   F   G   H   I
//sim1 /存在    1   0   1   1   0   1   1   0 
//     /上锁    1   0   1   0   0   1   0   0
//sim2 /存在    1   1   1   1   1   0   0   0
//     /上锁    1   1   0   0   0   0   0   0
/*****************************************************************************/

/*****************************************************************************/
//  Description : 执行simlock解锁功能
//  Global resource dependence : 
//  Author: 
//  Note: 
/*****************************************************************************/
PUBLIC void MMISET_Unlocking_SimLock(void)
{
    //设置为解锁
    MMIAPISET_SetNetLockSetting(TRUE);
	...
	{
		//SIM1 存在且上锁
		if (MMIAPIPHONE_GetSIM1PowerOnStatus())
		{
			MNPHONE_PowerOffSimEx(MN_DUAL_SYS_1);
			MMIAPIPHONE_SetSIM1PowerOnStatus(FALSE);
		}
		//激活SIM1
		MMIAPIPHONE_PowerOnSIM(MN_DUAL_SYS_1);
	}
#ifdef MMI_MULTI_SIM_SYS_DUAL 
	...
	{
		//SIM2 存在且上锁
		if (MMIAPIPHONE_GetSIM2PowerOnStatus())
		{
			MNPHONE_PowerOffSimEx(MN_DUAL_SYS_2);
			MMIAPIPHONE_SetSIM2PowerOnStatus(FALSE);
		}
		//激活SIM2
		MMIAPIPHONE_PowerOnSIM(MN_DUAL_SYS_2);
	}
#endif
}

/*****************************************************************************/
//  Description : handle simlock unlock win msg
//  Global resource dependence : 
//  Author:
//  Note: 
/*****************************************************************************/
PUBLIC MMI_RESULT_E MMISET_HandleSIMLockUnlockWinMsg(
                                            MMI_WIN_ID_T     win_id,
                                            MMI_MESSAGE_ID_E msg_id,
                                            DPARAM           param
                                            )
    ...
    case MSG_APP_WEB:
        {
            ...
            MMK_CloseWin(MMISET_SET_SECURITY_SIM_LOCK_WIN_ID);
        }
        break;
...
LOCAL void OpenSIMLockUnlockWin(void)
{
    ...
    {
		//输入解锁密码后解锁
        MMIPHONE_OpenSIMLockInputingWin();
        //询问是否解锁后直接解锁
        //MMIPUB_OpenQueryWinByTextId(TXT_SET_SIM_LOCK_UNLOCKING, IMAGE_PUBWIN_QUERY, PNULL, MMISET_HandleSIMLockUnlockWinMsg);
    }
    else
    {
        ////////////////////
        //for test, 再次上锁用(需重启)
        //MMIAPISET_SetNetLockSetting(FALSE);
        ////////////////////

        MMIPUB_OpenAlertWinByTextId(PNULL, TXT_SET_SIM_LOCK_UNLOCKIED, TXT_NULL, 
...
LOCAL int GetSIMLockInfoStr(uint8** sim_lock_info_buf, uint32* buf_len)
{
    //MN_SIM_LOCK_CUSTOMIZE_DATA_T sim_lock_customize_data = {0};
    //MN_SIM_LOCK_STATUS_T *sim_lock_status= MMIAPIPHONE_GetSIMUnlockConfigDataPtr();
    //MN_SIM_LOCK_USER_DATA_T sim_lock_user_data = {0};
    PHONE_SIM_ID_T *sim_lock_datas = MMIAPIPHONE_GetSIMLockTable();
    uint8 numlocks = MMIAPIPHONE_GetSIMLockNum();
    uint32 offset = 0;
    uint32 i = 0;
    ...
    else 
    {
        ...
        //"PLMNi: 460 1  2  (CR)" 4+1+1+1 + 3+1  +3+3 +1 =18
        temp_buf_len += 18 *numlocks + SIM_LOCKED_NULL_STR_LEN + 2;
        ...
        *buf_len = temp_buf_len;
        ...
        ///////NETWORK_LOCKED/////////////
        {
            ...
            for(i = 0; i < numlocks; i++)
            {
                ...
                // MCC
                MMIAPICOM_Int2Str(sim_lock_datas[i].mcc, temp_buf, 3);
                MMI_MEMCPY(*sim_lock_info_buf + offset, temp_buf_len, temp_buf, 3, 3);
                SCI_MEMSET(temp_buf, 0x00, TEMP_BUF_LEN);
                offset += 3;
                (*sim_lock_info_buf)[offset] = BL_CHAR;
                offset += 1;
                ...
                //MNC
                MMIAPICOM_Int2Str(sim_lock_datas[i].mnc, temp_buf, 3);
                temp_str_len = SCI_STRLEN((char*)temp_buf);
                MMI_MEMCPY(*sim_lock_info_buf + offset, temp_buf_len, temp_buf, temp_str_len, temp_str_len);
                SCI_MEMSET(temp_buf, 0x00, TEMP_BUF_LEN);
                offset += temp_str_len;
                ...
                offset += 1;
                //digital num
                MMIAPICOM_Int2Str(sim_lock_datas[i].mnc_digit_num, temp_buf, 3);
                temp_str_len = SCI_STRLEN((char*)temp_buf);
                MMI_MEMCPY(*sim_lock_info_buf + offset, temp_buf_len, temp_buf, temp_str_len, temp_str_len);
                SCI_MEMSET(temp_buf, 0x00, TEMP_BUF_LEN);
                offset += temp_str_len;
...
/*****************************************************************************/
// 	Description : Get SIM LOCK Net Unlock flag
//	Global resource dependence : 
//  Author:
//	Note: TRUE:Unlocked
//        FALSE:Locked
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPISET_GetNetLockSetting(void)
{
    BOOLEAN is_net_unlocked=FALSE;
    MN_RETURN_RESULT_E      return_value = MN_RETURN_FAILURE;

    MMINV_READ(MMINV_SET_NET_UNLOCKED,&is_net_unlocked,return_value);
    if (return_value == MN_RETURN_FAILURE)
    {
        is_net_unlocked = FALSE;
        MMINV_WRITE(MMINV_SET_NET_UNLOCKED,&is_net_unlocked);
    }
    
    return is_net_unlocked;
}

/*****************************************************************************/
// 	Description : Set SIM LOCK Net Unlock flag
//	Global resource dependence : 
//  Author:
//	Note: 
/*****************************************************************************/
PUBLIC void MMIAPISET_SetNetLockSetting(BOOLEAN is_net_unlocked)
{
    SCI_TRACE_LOW("MMIAPISET_SetNetLockSetting, is_net_unlocked = %d",is_net_unlocked);
    MMINV_WRITE(MMINV_SET_NET_UNLOCKED,&is_net_unlocked);
    return;
}
#endif
...
PUBLIC void MMIAPISET_EnterNewSecuritySettingWin(void)
{
    MMK_CreateWin((uint32*)MMISET_SET_NEW_SECURITY_WIN_TAB,PNULL);
}
//  Author: xiangjie
//  Note:
/*****************************************************************************/
...
LOCAL void AppendAutoKeylockTimeListItem(MMI_CTRL_ID_T      ctrl_id)
{
    ...
    MMIAPISET_AppendListItemByTextId( STR_POC_ILIST_OFF, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO );
...
#if  defined(MMI_WHITELIST_SUPPORT)
/*****************************************************************************/
//  Description :InitPdaWhiteLIstSettingsListCtrl
//  Global resource dependence : 
//  Author: rong.gu 
//  Note: 
/*****************************************************************************/
LOCAL void  InitPdaWhiteLIstSettingsListCtrl(void)
{
    MMI_IMAGE_ID_T      image_id = IMAGE_NULL;
    BOOLEAN flag = FALSE;
    MMI_CTRL_ID_T       ctrl_id = MMISET_SET_WHITELIST_TYPE_FROM_CTRL_ID;
    MMI_STRING_T  text_str = {0};

    GUILIST_SetMaxItem(ctrl_id, 3, FALSE );   
    
    flag =  MMIAPISET_GetIsWhitelistAudioCall();
    if(flag)
    {
        image_id = IMAGE_CHECK_SELECTED_ICON;
    }
    ...
    AppendPdaSettingsItem2Line(            
    ctrl_id,
    TXT_SET_WHITELIST_TYPE_AUDIOCALL,
    text_str,
    image_id,
    (uint32)0);

#ifdef VT_SUPPORT
    flag =  MMIAPISET_GetIsWhitelistVedioCall();
    if(flag)
    {
        image_id = IMAGE_CHECK_SELECTED_ICON;
    }
    ...
    AppendPdaSettingsItem2Line(            
    ctrl_id,
    TXT_SET_WHITELIST_TYPE_VEDIOCALL,
    text_str,
    image_id,
    (uint32)0);
#endif /*VT_SUPPORT*/
    ...
    flag = MMIAPISET_GetIsWhitelistMessage();
    if(flag)
    {
        image_id = IMAGE_CHECK_SELECTED_ICON;
    }
    ...
    AppendPdaSettingsItem2Line(            
    ctrl_id,
    TXT_SET_WHITELIST_TYPE_MESSAGE,
    text_str,
    image_id,
    (uint32)0);
}
...
#ifdef MMI_POWER_ON_PWD_SOS_SUPPORT
#if defined(MMI_SIM_LOCK_TYPE2_SUPPORT) && !defined(MMI_SIM_LOCK_SUPPORT)
            MMIAPIPHONE_SetIsInPwdWin(TRUE);
#endif
...
#endif
        {
            if (PHONE_CODE_LEN_IS_IN_RANGE(old_pwd_value.wstr_len))
            {
                ...
                if (MMISET_IsCorrectPowerOnPwd(old_pwd_value.wstr_ptr, old_pwd_value.wstr_len))
                {
                    ...
                    {
#if defined(MMI_SIM_LOCK_TYPE2_SUPPORT) && !defined(MMI_SIM_LOCK_SUPPORT)
                        MMIAPIPHONE_SetIsInPwdWin(FALSE);
#endif
                             ...
                             )
{
    ...
    //    {
     ...
     //   else
      //  {
            CreateLanguageSelectFirstPowerOn(); //因为安全需要，否测首次开机不会有phone lock
...
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPISET_OpenMsgBoxCustom(uint32 win_tab, BOOLEAN is_special_func,APP_MN_SIM_PIN_FUNC_CNF_T *sim_pin_cnf)
{
    if (is_special_func)
    {
        //@zhaohui modified for fdn
        ...
        else
        {
            ...
            if(MNSIM_PIN_CHANGE == sim_pin_cnf->pin_function_type)
            {
                ...
                {
                    text1_id = STXT_PIN_CHANGED;
                }
                else if(MNSIM_PIN2 == sim_pin_cnf->pin_num)
                {
                    text1_id = STXT_PIN2_CHANGED;
                }
                else
                {
                }
                MMISET_DisplaybannerInfo(PNULL, text1_id, PNULL);
            }
            else if(MNSIM_PIN_UNBLOCK == sim_pin_cnf->pin_function_type)
            {
                if(MNSIM_PIN1 == sim_pin_cnf->pin_num)
                {
            ...
            {
                MMISET_DisplaybannerInfo(PNULL, TXT_SUCCESS, PNULL);
            }
            #endif
...
BOOLEAN is_startup_lock = FALSE;
BOOLEAN GetStartupLockStatus(void)
{
    return is_startup_lock;
}
/*****************************************************************************/
//  Description :power on password win callback
//  Global resource dependence : 
//  Author: Jassmine
//  Note:
/*****************************************************************************/
LOCAL void HandlePowerOnPwdCallBack(void *param1, void *param2)
{
#ifdef MMI_POWER_ON_PWD_SOS_SUPPORT
...
#endif
      is_startup_lock = FALSE;
      CreateLanguageSelectFirstPowerOn(); //因为安全需要，否测首次开机不会有phone lock
      MMIAPIPHONE_NormalInitAfterPwd();
	  SCI_TRACE_LOW("[phone lock] HandlePowerOnPwdCallBack begin");
}
#endif
...
PUBLIC void MMIAPISET_OpenPowerOnPwdWin(void)
{
	...
	IDLE_LOCK_DATA idle_lock_data = {0};
     ...
     MMIPHONE_PowerOnPs(MN_DUAL_SYS_1); //开机密码界面，未启动SIM卡，启动卡1即可
#endif
    idle_lock_data.e_phone_status = MMI_IDLE_PHONE_START_UP;
    idle_lock_data.app_entry_func_ptr = HandlePowerOnPwdCallBack;
    idle_lock_data.para1 = PNULL;
    idle_lock_data.para2 = PNULL;
    is_startup_lock = TRUE;	 
	MMIIDLE_UlockPhoneBySecurityCode(&idle_lock_data);
#else
...
#if 0  // not suppot capacity ps massage yet
   ...
   switch(msg_id)
   {
    ...
    case MSG_APP_WEB:
        ...
        case ID_SET_BLACKLIST_LIST:
            //[block list in call setting]
            MMK_CreateWin((uint32 *)MMISET_SET_BLACKLIST_LIST_WIN_TAB,PNULL);   //xiongkai
            //[end]
            break;

        case ID_SET_BLACKLIST_SETTING:
            //[block list in call setting]
            MMK_CreateWin((uint32 *)MMISET_SET_BLACKLIST_TYPE_WIN_TAB,PNULL);   //xiongkai
            //[end]
            break;
        ...
        case MSG_CTL_PENOK:
#endif //TOUCH_PANEL_SUPPORT //IGNORE9527
          {
            ...
            if(0 == index)
            {
            		...
            		{
				 MMIPUB_OpenAlertWarningWin(TXT_CLOSE_WHITELIST_AUDIOCALL);
		                MMK_CloseWin(win_id);
		                break;
			}
		#endif			
            ...
            else
            {
            		...
            		{
				 MMIPUB_OpenAlertWarningWin(TXT_CLOSE_WHITELIST_MESSAGE);
		                MMK_CloseWin(win_id);
		                break;
			}
		#endif
                                            ...
                                            )
{
   ...
   case MSG_CTL_LIST_LONGOK:  
    ...
    case MSG_APP_MENU:
        //[block list in call setting]
        MMK_CreateWin((uint32 *)MMISET_SET_BLACKLIST_OPT_WIN_TAB,PNULL);  //xiongkai
        //[end]
        break;
    ...
    case MSG_CTL_PENOK:
        ...
        {
            //[block list in call setting]
            MMK_CreateWin((uint32 *)MMISET_SET_BLACKLIST_EDIT_WIN_TAB,PNULL);   //xiongkai
            //[end]
        }
        break;
                                 ...
                                 )
{
    ...
    case ID_SET_BLACKLIST_NEW:   
            ...
            #if  defined(MMI_WHITELIST_SUPPORT)
        	if(MMIAPISET_GetIsWhitelistAudioCall() ||MMIAPISET_GetIsWhitelistMessage())
        		{
			 MMIPUB_OpenAlertWarningWin(TXT_CLOSE_WHITELIST_AUDIOCALL);
	                  break;
		}
        ...
        {
            //[block list in call setting]
            MMK_CreateWin((uint32 *)MMISET_SET_BLACKLIST_NEW_WIN_TAB, (ADD_DATA)ctrl_id);   //xiongkai
            //[end]
        }
        break;
                                ...
                                )
{
    ...
    {
        //[after add number to block
        // list,call log will delete logs]
        //return AddNumToBlacklist(mmi_pb_name, pb_bcd);
        if(AddNumToBlacklist(mmi_pb_name,pb_bcd))
        {
          ////  MMIAPICL_DeleteLogsAfterAddBlockListB(pb_bcd);  ///fangsm
        }
        //[end]
    }
}
...
PUBLIC void MMIAPISET_SetSmartDualSIMPinEnable(MN_DUAL_SYS_E dual_sys, BOOLEAN enable)
{
    if(enable)
    {
        EnablePin1(dual_sys);
...
PUBLIC void MMIAPISET_ChangeSmartDualSIMPin(MN_DUAL_SYS_E dual_sys)
{
    ModifyPin1(dual_sys);
}
#endif

//lulifei add the pin enable and change the pin
PUBLIC void MMIAPISET_SetSIMPinEnable( BOOLEAN enable)
{
    if(enable)
    {
        EnablePin1(MN_DUAL_SYS_1);
    }
    else
    {
        DisablePin1(MN_DUAL_SYS_1);
    }
}
PUBLIC void MMIAPISET_ChangeSIMPin()
{
    ModifyPin1(MN_DUAL_SYS_1);
}
                                         ...
                                         )
{
    ...
    uint16 emergency_call_len = 0;
#endif
    MMI_CTRL_ID_T ctrl_id = MMISET_SET_PHONE_LOCK_PWD_CTRL_ID;
    GUI_RECT_T          bg_rect         =   {0, 0, 0, 0};
    GUI_LCD_DEV_INFO    lcd_dev_info    =   {GUI_LCD_0, GUI_BLOCK_0};
    ...
    case MSG_OPEN_WINDOW:
        ...
        if ((MMISET_PWD_NORMAL == g_mmiset_pwd_type)||(MMISET_PWD_OTHER == g_mmiset_pwd_type))
        {
            GUIEDIT_SetSoftkey(ctrl_id,0,MMISET_POWER_ON_PWD_MIN_LEN,TXT_NULL,TXT_COMMON_OK,PNULL);
            GUIEDIT_SetSoftkey(ctrl_id,2,1,TXT_NULL,TXT_COMMON_CLEAR,PNULL);
        }
        else
        {
            //set left softkey
		...
		MMIPUB_SetWinTitleTextId(win_id, TXT_INPUT_PHONE_PASSWORD, TRUE);
        MMK_SetAtvCtrl(win_id, ctrl_id);
#ifndef SUBLCD_SIZE_NONE
        //刷新小屏的显示,输入开机密码
        MMISUB_IsPermitUpdate(TRUE);
        ...
        GUIEDIT_GetString(ctrl_id,&old_pwd_value);
        {
            if (PHONE_CODE_LEN_IS_IN_RANGE(old_pwd_value.wstr_len))
            {
                if (MMISET_IsCorrectPowerOnPwd(old_pwd_value.wstr_ptr, old_pwd_value.wstr_len))
                {
					...
					{
						// 输入phone lock密码后，如果有设置开机飞行模式，则弹出询问提示
                ...
                else
                {
					//nhl modify 494866
					//MMIPUB_OpenAlertWarningWin(TXT_KL_PSW_WRONG_PSW);
					MMIPHONE_OpenWrongLockAlertWindow();
					//nhl modify end
                    GUIEDIT_ClearAllStr(ctrl_id);
                }
            }
...
LOCAL void MMIPHONE_OpenWrongLockAlertWindow(void)
{
  MMIPUB_OpenConfirmationDialogByTextId(PNULL,TXT_KL_PSW_WRONG_PSW,PNULL,
                                                          IMAGE_NULL,PNULL,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,HandleWrongLockAlertWindow);
}
...
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPISET_OpenPhoneLockPwdWin(void)
{
    MMK_CreatePubEditWin((uint32*)MMIPHONE_PHONE_LOCK_PWD_WIN_TAB,PNULL);
}
...
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPISET_IsPowerOnSecruityCodeEnable(void)
{
	MMISET_LOCKSCREEN_SECURITY_CODE_T    device_lock = {0};
	BOOLEAN   return_value = FALSE;
	return_value = MMIDEVICELOCKSCREEN_ReadPwd(&device_lock);
	if( TRUE == return_value)
	{
		return (device_lock.is_enable);
	}
	else
	{
		return FALSE;
...
PUBLIC BOOLEAN MMIAPISET_IsSecruityWinOpen()
{
    return MMK_IsOpenWin(MMISET_SET_SECURITY_WIN_ID)
                ||MMK_IsOpenWin(MMISET_SET_PHONE_LOCK_PWD_WIN_ID);
}

#ifdef PHONE_NUMBER_VOICE
PUBLIC unsigned int hero_Keyguard_GetWinId(void)
{
	return s_hero_keyguard_win_id;
}
#endif
...
//  Note: 
/*****************************************************************************/
PUBLIC void MMISET_OpenWhitelistEditWin(MMIPB_BCD_NUMBER_T *orginal_addr_ptr)
{
    if (PNULL == orginal_addr_ptr)
    {
        return;
    ...
    s_new_whitelist_num_ptr = SCI_ALLOCA(sizeof(MMIPB_BCD_NUMBER_T));
    if(PNULL == s_new_whitelist_num_ptr)
    {
        return;
    }
    
    SCI_MEMSET(
               s_new_whitelist_num_ptr,
               0x00,
               sizeof(MMIPB_BCD_NUMBER_T)
               );
    ...
    MMK_CreateWin((uint32 *)MMISET_SET_WHITELIST_EDIT_WIN_TAB,(ADD_DATA)s_new_whitelist_num_ptr);
}
...
LOCAL void InitWhitelistInfo(void)
{
    ...
    MMI_GetLabelTextByLang(TXT_NAME, &str_name);
	GUILABEL_SetText(MMISET_SET_WHITELIST_NEW_NAME_LABEL_CTRL_ID, &str_name, FALSE);

    MMI_GetLabelTextByLang(TXT_COMMON_NUMBER, &str_num);
	GUILABEL_SetText(MMISET_SET_WHITELIST_NEW_NUM_LABEL_CTRL_ID, &str_num, FALSE);

    GUIEDIT_SetString(MMISET_SET_WHITELIST_NEW_NAME_CTRL_ID, name, name_len);
    GUIEDIT_SetString(MMISET_SET_WHITELIST_NEW_NUM_CTRL_ID, num, num_len);
...
/*****************************************************************************/
//  Description : to handle security setting whitelist menu
//  Global resource dependence : 
//  Author:fengming.huang
//  Note: 
/*****************************************************************************/
                                               ...
                                               )
{
   ...
   MMI_MENU_ID_T                menu_id    =   0;
   MMI_RESULT_E                recode      =   MMI_RESULT_TRUE;
   MMI_CTRL_ID_T               ctrl_id     =   MMISET_SET_WHITELIST_CTRL_ID;
   ...
   switch(msg_id)
   {
    ...
    case MSG_APP_WEB:
       ...
       switch (menu_id)
       {
       case ID_SET_WHITELIST_LIST:
           MMK_CreateWin((uint32 *)MMISET_SET_WHITELIST_LIST_WIN_TAB,PNULL);
           break;
           
       case ID_SET_WHITELIST_SETTING:
           MMK_CreateWin((uint32 *)MMISET_SET_WHITELIST_TYPE_WIN_TAB,PNULL);
           break;
...
{
    MMI_RESULT_E recode = MMI_RESULT_TRUE;
    MMI_CTRL_ID_T ctrl_id = MMISET_SET_WHITELIST_LIST_CTRL_ID;
    uint16 index = 0;
    
    switch (msg_id)
    {
    case MSG_OPEN_WINDOW:
        UpdateWhiteList(ctrl_id);
        GUILIST_SetCurItemIndex(ctrl_id, 0);
        MMK_SetAtvCtrl(win_id, ctrl_id);
#ifdef MMI_PDA_SUPPORT
        GUIWIN_SetTitleButtonState(win_id, GUITITLE_BUTTON_RIGHT, TITLE_BUTTON_NORMAL, FALSE);
    ...
    case MSG_FULL_PAINT:
           //MMIAPICOM_StatusAreaSetViewHeaderinfoByTextId(qtn_msg_hdr_white_number_bc);    ///mmiidle_statusbar.c
           break;

    case MSG_SET_WHITELIST_LIST_UPDATE:
        UpdateWhiteList(ctrl_id);
        index = *(uint16*)param;
        GUILIST_SetCurItemIndex(ctrl_id, index);
        MMK_SetAtvCtrl(win_id, ctrl_id);
   ...
   #ifdef MMI_PDA_SUPPORT 
   case MSG_CTL_LIST_LONGOK:  
        MMK_CreateWin((uint32 *)MMISET_SET_WHITELIST_ITEM_OPT_WIN_TAB,PNULL);        
        break;
   #endif  
    ...
    case MSG_APP_OK:
    case MSG_APP_MENU:
        MMK_CreateWin((uint32 *)MMISET_SET_WHITELIST_OPT_WIN_TAB,PNULL);
        break;
    ...
    case MSG_CTL_PENOK:
#endif //TOUCH_PANEL_SUPPORT //IGNORE9527
        if(0 == g_whitelist_ptr->num)
        {
            break;
        }
        else
        {
            MMK_CreateWin((uint32 *)MMISET_SET_WHITELIST_EDIT_WIN_TAB,PNULL);
        }
        break;

    case MMI_SELECT_CONTACT_CNF:
        SetWhitelistFromPB((MMIPB_HANDLE_T)param);
        UpdateWhiteList(ctrl_id);
        GUILIST_SetCurItemIndex(ctrl_id, 0); 
        MMK_PostMsg(win_id,MSG_FULL_PAINT,PNULL,0);
        break;
                           ...
                           )
{
    ...
    GUILIST_SetMaxItem( ctrl_id, MMISET_WHITELIST_MAX_NUM, FALSE );//max item 10
    
    for ( i = g_whitelist_ptr->num; i > 0; i-- )
    {
        AppendWhitelistItem(
                            ctrl_id,
                            i - 1,
                            ...
                            )
{
    ...
    if(PNULL == sender_ptr)
    {
        GUIEDIT_SetString(MMISET_SET_WHITELIST_NEW_NAME_CTRL_ID, g_whitelist_ptr->whitelist[index].name, g_whitelist_ptr->whitelist[index].name_len);
        GUIEDIT_SetString(MMISET_SET_WHITELIST_NEW_NUM_CTRL_ID, g_whitelist_ptr->whitelist[index].num, g_whitelist_ptr->whitelist[index].num_len);
    }
    else
    {
        if(PNULL != s_new_whitelist_num_ptr)
        {
            ...
            name_str.wstr_len = 0;
            name_str.wstr_ptr = str_ptr;
            find_result = MMIAPIPB_GetNameByNumber(s_new_whitelist_num_ptr, &name_str, MMIPB_MAX_STRING_LEN, FALSE);
            if(find_result)
            {
                GUIEDIT_SetString(MMISET_SET_WHITELIST_NEW_NAME_CTRL_ID,name_str.wstr_ptr,name_str.wstr_len);                   
            }
        }
...
//                                             );
        ...
        GUIEDIT_SetString(MMISET_SET_WHITELIST_NEW_NUM_CTRL_ID, num, tele_len);
    }
                                ...
                                )
{
    ...
    //item_data.item_content[list_index].item_data.image_id = IMAGE_SET_WHITELIST;

    list_index++;

    //name
    name_len = MIN(g_whitelist_ptr->whitelist[pos].name_len,MMIPB_NAME_MAX_LEN);
    item_data.item_content[list_index].item_data_type = GUIITEM_DATA_TEXT_BUFFER;    
    item_data.item_content[list_index].item_data.text_buffer.wstr_len = name_len;
    
    item_data.item_content[list_index].item_data.text_buffer.wstr_ptr = g_whitelist_ptr->whitelist[pos].name;

    list_index++;

    //number
    str_num.wstr_len = MIN(g_whitelist_ptr->whitelist[pos].num_len,MMISET_PHONE_NUM_MAX_LEN);

    item_data.item_content[list_index].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
    item_data.item_content[list_index].item_data.text_buffer.wstr_len = str_num.wstr_len;

    item_data.item_content[list_index].item_data.text_buffer.wstr_ptr = g_whitelist_ptr->whitelist[pos].num;
                                           ...
                                           )
{
    ...
    {
        case MSG_OPEN_WINDOW:
            InitPdaWhiteLIstSettingsListCtrl();
            MMK_SetAtvCtrl(win_id, MMISET_SET_WHITELIST_TYPE_FROM_CTRL_ID);
            break;
        ...
        case MSG_CTL_PENOK:
          ...
          {
          #if 0
            if (g_whitelist_ptr->num == 0)
            {
                MMIPUB_OpenAlertWarningWin(TXT_ENTER_WHITELIST_NUM_FIRST);
                MMK_CloseWin(win_id);
                break;
            }
		#endif
            ...
            if(0 == index)
            {
				 ...
				 MMIPUB_OpenAlertWarningWin(TXT_CLOSE_BLACKLIST_AUDIOCALL);
		                MMK_CloseWin(win_id);
		                break;
			}
			#endif		
                   MMISET_SetIsWhitelistAudioCall((BOOLEAN)!MMIAPISET_GetIsWhitelistAudioCall());
            }
            #ifdef VT_SUPPORT
            else if(1 == index)
            {
                    MMISET_SetIsWhitelistVedioCall((BOOLEAN)!MMIAPISET_GetIsWhitelistVedioCall());
            }
            #endif
            else
            {
				 ...
				 MMIPUB_OpenAlertWarningWin(TXT_CLOSE_BLACKLIST_MESSAGE);
		                MMK_CloseWin(win_id);
		                break;
			}
		#endif
                 MMISET_SetIsWhitelistMessage((BOOLEAN)!MMIAPISET_GetIsWhitelistMessage());
            }
            MMK_PostMsg(win_id, MSG_SET_WHITELIST_LIST_UPDATE, PNULL, 0);
            MMK_PostMsg(win_id, MSG_FULL_PAINT, PNULL, NULL);
         }
         break;
                                 ...
                                 )
{
    ...
    switch (menu_id)
    {
    case ID_SET_WHITELIST_NEW:                 
        if(g_whitelist_ptr->num == MMISET_WHITELIST_MAX_NUM)
        {
            MMIPUB_OpenAlertWarningWin(TXT_SET_WHITELIST_FULL);            
        }
        /*else  if(!MMIAPISET_GetIsWhitelistAudioCall()
#ifdef VT_SUPPORT
                &&!MMIAPISET_GetIsWhitelistVedioCall()
#endif            
                &&!MMIAPISET_GetIsWhitelistMessage()
                )
        {
            user_data = ID_SET_WHITELIST_NEW;
            MMIPUB_OpenQueryWinByTextIdEx(VIRTUAL_WIN_ID,TXT_OPEN_WHITELIST_QUERY,IMAGE_PUBWIN_QUERY,PNULL,HandleWhitelistAddQueryWinMsg,(uint32)user_data);
        }*/
        else
        {
            MMK_CreateWin((uint32 *)MMISET_SET_WHITELIST_NEW_WIN_TAB, (ADD_DATA)ctrl_id);
        }
        break;
        
    case ID_SET_WHITELIST_EDIT:                     
        MMK_CreateWin((uint32 *)MMISET_SET_WHITELIST_EDIT_WIN_TAB,PNULL);
        break;
        
    case ID_SET_WHITELIST_DEL: 

		{
			  MMI_STRING_T title_str = {0};
			  MMIRES_GetText(TXT_DELETE_QUERY, PNULL, &title_str);
			  //MMIPUB_OpenConfirmationDialogByTextPtr(PNULL, &title_str, PNULL,
                        //IMAGE_NULL, PNULL, PNULL, MMIPUB_SOFTKEY_OKCANCEL, HandleWhitelistDelQueryWinMsg);

		     //   MMIPUB_OpenAlertWinByTextId(PNULL, &title_str ,TXT_NULL,PNULL,PNULL,PNULL,MMIPUB_SOFTKEY_OKCANCEL,HandleWhitelistDelQueryWinMsg);			    
		}
        MMIPUB_OpenQueryWinByTextId(TXT_DELETE_QUERY,IMAGE_PUBWIN_QUERY,PNULL,HandleWhitelistDelQueryWinMsg);
        break;
        
    case ID_SET_WHITELIST_CONTACT:            
        if(g_whitelist_ptr->num == MMISET_WHITELIST_MAX_NUM)
        {
            MMIPUB_OpenAlertWarningWin(TXT_SET_WHITELIST_FULL);            
        }
        ...
        {
            user_data = ID_SET_WHITELIST_CONTACT;
            MMIPUB_OpenQueryWinByTextIdEx(VIRTUAL_WIN_ID,TXT_OPEN_BLACKLIST_QUERY,IMAGE_PUBWIN_QUERY,PNULL,HandleWhitelistAddQueryWinMsg,(uint32)user_data);
        }*/
        else
        {
            /*MMIAPIPB_OpenNumberList(
            ...
            MMIAPIPB_OpenMainWin(
                    ...
                    MMISET_WHITELIST_MAX_NUM - g_whitelist_ptr->num,//如果是多选时，此项限制了多选的最大个数
                    PNULL,
                    SetWhiteListSelectContactCallback
                    );	
        }
        break;
        
    case ID_SET_WHITELIST_CL:
       // SetFromWhiteListFlag();
      //  MMIAPICL_OpenCLWindow();
        break;

    case ID_SET_WHITELIST_DELALL:  
		{
			  MMI_STRING_T title_str = {0};
			  MMIRES_GetText(TXT_DELETE_ALL_QUERY, PNULL, &title_str);
			  //MMIPUB_OpenConfirmationDialogByTextPtr(PNULL, &title_str, PNULL,
                        //IMAGE_NULL, PNULL, PNULL, MMIPUB_SOFTKEY_OKCANCEL, HandleWhitelistDelAllQueryWinMsg);
			//  MMIPUB_OpenAlertWinByTextId(PNULL, &title_str ,TXT_NULL,PNULL,PNULL,PNULL,MMIPUB_SOFTKEY_OKCANCEL,HandleWhitelistDelAllQueryWinMsg);
	  MMIPUB_OpenQueryWinByTextId(TXT_DELETE_ALL_QUERY,IMAGE_PUBWIN_QUERY,PNULL,HandleWhitelistDelAllQueryWinMsg);
		}
      //  MMIPUB_OpenQueryWinByTextId(TXT_DELETE_ALL_QUERY,IMAGE_PUBWIN_QUERY,PNULL,HandleWhitelistDelAllQueryWinMsg);
        break;
                                            ...
                                            )
{
    ...
    case MSG_APP_OK:  
    case MSG_APP_WEB:
        index = GUILIST_GetCurItemIndex(MMISET_SET_WHITELIST_LIST_CTRL_ID);
        real_index = g_whitelist_ptr->num - index - 1;
        DeleteWhitelistItem(real_index);
#ifndef MMI_PDA_SUPPORT
        //MMIPUB_OpenAlertSuccessWin(TXT_COMPLETE);
#endif
        if (g_whitelist_ptr->num == 0)
        ...
        MMK_PostMsg(MMISET_SET_WHITELIST_LIST_WIN_ID, MSG_SET_WHITELIST_LIST_UPDATE, (DPARAM)&index, sizeof(uint16));
        //关闭询问窗口
                                           ...
                                           )
{
    ...
    case MSG_OPEN_WINDOW:
        #ifndef MMI_PDA_SUPPORT
        GUIFORM_SetStyle(MMISET_SET_WHITELIST_SET_NAME_FORM_CTRL_ID,GUIFORM_STYLE_UNIT);
        GUIFORM_SetStyle(MMISET_SET_WHITELIST_SET_NUM_FORM_CTRL_ID,GUIFORM_STYLE_UNIT);
        #else
        GUIFORM_SetType(MMISET_SET_WHITELIST_NEW_FROM_CTRL_ID,GUIFORM_TYPE_TP);
        #endif
        InitWhitelistInfo();
        #ifndef MMI_PDA_SUPPORT
        GUIFORM_SetType(MMISET_SET_WHITELIST_NEW_FROM_CTRL_ID,GUIFORM_TYPE_TP);        
        GUIWIN_SetSoftkeyTextId(win_id,  STXT_SAVE, STXT_MAIN_PB, STXT_RETURN, FALSE);
        #endif
        MMK_SetAtvCtrl(win_id, MMISET_SET_WHITELIST_NEW_NAME_CTRL_ID);
        break;
#ifndef MMI_PDA_SUPPORT    
    case MSG_CTL_OK:
    case MSG_APP_OK:
        ...
        {
            if(SaveWhitelistInfo(g_whitelist_ptr->num,TRUE))
            {
                //MMIPUB_OpenAlertSuccessWin(TXT_COMPLETE);            
                MMK_PostMsg(MMISET_SET_WHITELIST_LIST_WIN_ID, MSG_SET_WHITELIST_LIST_UPDATE, (DPARAM)&g_whitelist_ptr->num, sizeof(uint16));
                MMK_CloseWin(win_id);
            }
        }
        break;
    ...
    case MSG_CTL_PENOK:
       ...
       /* MMIAPIPB_OpenNumberList(
            ...
            MMIAPIPB_OpenMainWin(
                    ...
                    1,//如果是多选时，此项限制了多选的最大个数
                    PNULL,
                    AddWhiteListSelectContactCallback
                    );	
        break;
#else
     case MSG_APP_WEB:
    ...
    case MMI_SELECT_CONTACT_CNF:
        ...
        if (MMIAPIPB_GetSelectCount((MMIPB_HANDLE_T)param) > 0)
        {
            SetWhitelistDetailFromPB((MMIPB_HANDLE_T)param);
        }
		break;
        
    case MSG_CTL_CANCEL:
        SCI_MEMSET( &edit_string, 0, sizeof( MMI_STRING_T ) );
        GUIEDIT_GetString(MMISET_SET_WHITELIST_NEW_NUM_CTRL_ID, &edit_string);
        ...
        else
        {
      	  {
				  MMI_STRING_T title_str = {0};
				  MMIRES_GetText(TXT_EXIT_WITHOUT_SAVE, PNULL, &title_str);
				  //MMIPUB_OpenConfirmationDialogByTextPtr(PNULL, &title_str, PNULL,
	                        //IMAGE_NULL, PNULL, PNULL, MMIPUB_SOFTKEY_OKCANCEL, PNULL);
	                        MMIPUB_OpenAlertWinByTextId(PNULL, &title_str ,TXT_NULL,PNULL,PNULL,PNULL,MMIPUB_SOFTKEY_OKCANCEL,PNULL);
			}
            //MMIPUB_OpenQueryWinByTextId(TXT_EXIT_WITHOUT_SAVE,IMAGE_PUBWIN_QUERY,PNULL,PNULL);
        }
        break;
                                           ...
                                           )
{
    ...
    {
        index = GUILIST_GetCurItemIndex(MMISET_SET_WHITELIST_LIST_CTRL_ID);
        pos = g_whitelist_ptr->num - index - 1;
    }
    else
    {
        is_new = TRUE;
        pos = g_whitelist_ptr->num;
    }
    ...
    case MSG_OPEN_WINDOW:
        GUIFORM_SetStyle(MMISET_SET_WHITELIST_SET_NAME_FORM_CTRL_ID,GUIFORM_STYLE_UNIT);
        GUIFORM_SetStyle(MMISET_SET_WHITELIST_SET_NUM_FORM_CTRL_ID,GUIFORM_STYLE_UNIT);        
        SetWhitelistInfo(pos,sender_ptr);
        #ifndef MMI_PDA_SUPPORT
        GUIFORM_SetType(MMISET_SET_WHITELIST_NEW_FROM_CTRL_ID,GUIFORM_TYPE_TP);        
        GUIWIN_SetSoftkeyTextId(win_id,  STXT_SAVE, STXT_MAIN_PB, STXT_RETURN, FALSE);
    ...
    case MSG_APP_OK:
	//case MSG_APP_MENU:
        if(SaveWhitelistInfo(pos,is_new))
        {
#ifdef MMI_GUI_STYLE_TYPICAL
            MMIPUB_OpenAlertSuccessWin(TXT_COMPLETE);            
#endif
    ...
    case MSG_CTL_PENOK:
       ...
       /* MMIAPIPB_OpenNumberList(
            ...
            MMIAPIPB_OpenMainWin(
                    ...
                    1,//如果是多选时，此项限制了多选的最大个数
                    PNULL,
                    EditWhiteListSelectContactCallback
                    );	
        break;
#endif //TOUCH_PANEL_SUPPORT //IGNORE9527
   ...
   case MSG_PROMPTWIN_CANCEL: 
    ...
    case MSG_CLOSE_WINDOW:
        if(!MMK_IsOpenWin(MMISET_SET_WHITELIST_LIST_WIN_ID))
        {
            ...
            if(PNULL != s_new_whitelist_num_ptr)
            {
                SCI_FREE(s_new_whitelist_num_ptr);
                s_new_whitelist_num_ptr = PNULL;
            }
        }
        break;
                               ...
                               )
{
    ...
    GUIEDIT_GetString(MMISET_SET_WHITELIST_NEW_NAME_CTRL_ID,&str_name);
    GUIEDIT_GetString(MMISET_SET_WHITELIST_NEW_NUM_CTRL_ID,&str_num);

    if(0 == str_num.wstr_len)
    {
        MMIPUB_OpenAlertWarningWin(TXT_SET_WHITELIST_INPUT_NUM);
        result = FALSE;
    }
    else if(!MMIAPICOM_IsValidNumberStringEx(&str_num))
    {
    ...
    else
    {
        ...
        for(j=0; j<g_whitelist_ptr->num;j++)
        {
            uint32 cmp_count =0;
            cmp_count = (uint32)MAX(g_whitelist_ptr->whitelist[j].num_len,str_num.wstr_len);
            if(!MMIAPICOM_Wstrncmp((const wchar *)g_whitelist_ptr->whitelist[j].num, str_num.wstr_ptr, cmp_count)
            &&( j != index))//for edit blacklist and modfy nothing
            {
                MMIPUB_OpenAlertWarningWin(TXT_SET_WHITELIST_NUM_EXIST);
                return FALSE;
            }
        }
        ...
        if(MMISET_InitBlacklistParam())
        {
            ...
            name_len = MIN(str_name.wstr_len,MMIPB_NAME_MAX_LEN);
            g_whitelist_ptr->whitelist[index].name_len = name_len;
            MMI_WSTRNCPY(
                         g_whitelist_ptr->whitelist[index].name,
                         MMIPB_NAME_MAX_LEN,
                         str_name.wstr_ptr,
                         name_len,
                         name_len);
            ...
            num_len = MIN(str_num.wstr_len,MMISET_PHONE_NUM_MAX_LEN);
            g_whitelist_ptr->whitelist[index].num_len = num_len;
            MMI_WSTRNCPY(
                         g_whitelist_ptr->whitelist[index].num,
                         MMISET_PHONE_NUM_MAX_LEN,
                         str_num.wstr_ptr,
                         num_len,
                         num_len);        
            ...
            MMISET_SetWhiteList(g_whitelist_ptr);
            result = TRUE;
        }
    }
                                             ...
                                             )
{
    ...
    if(pos > (g_whitelist_ptr->num - 1))
    {
        return MN_RETURN_FAILURE;
    }
    
    //将删除位置以后的所有项前移
    for (i=pos+1; i<g_whitelist_ptr->num; i++)
    {
        //g_blacklist_ptr->blacklist[i-1] = g_blacklist_ptr->blacklist[i];
        SCI_MEMCPY(
                   &g_whitelist_ptr->whitelist[i-1],
                   &g_whitelist_ptr->whitelist[i],
                   sizeof(MMISET_WHITELIST_ITEM_T));
    }
...
LOCAL BOOLEAN SetWhitelistDetailFromPB(MMIPB_HANDLE_T handle)
{
    ...
    if(0 == name_str.wstr_len)//name is empty, clean the privous name string
    {
        GUIEDIT_ClearAllStr(MMISET_SET_WHITELIST_NEW_NAME_CTRL_ID);
    }
    else
    {
        GUIEDIT_SetString(MMISET_SET_WHITELIST_NEW_NAME_CTRL_ID, name_str.wstr_ptr, name_str.wstr_len);
...
LOCAL BOOLEAN SetWhitelistFromPB(MMIPB_HANDLE_T handle)
{
    ...
    if ((MMISET_WHITELIST_MAX_NUM - g_whitelist_ptr->num) < count)
    {
        MMIPUB_OpenAlertWarningWin(TXT_SET_WHITELIST_BEYOND_MAX_NUM);
        return FALSE;
    }
    ...
    for (i=0; i<MIN(MMISET_WHITELIST_MAX_NUM, count); i++)
    {
        uint16 j= 0;
        BOOLEAN is_duplicate = FALSE;
        uint16 total_num = 0;
        ...
        SCI_MEMSET(name_str.wstr_ptr,0x00,(MMIPB_NAME_MAX_LEN+1)*sizeof(wchar));   
        MMIAPIPB_GetSelectName(handle,&name_str,MMIPB_NAME_MAX_LEN,i);       
...
//                                             );
        ...
        {
            uint32 cmp_count =0;
            cmp_count = (uint32)MAX(g_whitelist_ptr->whitelist[j].num_len,tele_len);
            if(!MMIAPICOM_WstrStrncmp((const wchar *)g_whitelist_ptr->whitelist[j].num, tele_num, cmp_count))
            {
                is_duplicate =TRUE;
                MMIPUB_OpenAlertWarningWin(TXT_SET_WHITELIST_NUM_EXIST);
                break;
        ...
        if(PNULL == g_blacklist_ptr)
        {
            is_need_free = TRUE;//if g_blacklist_ptr is pnull, need to be free
        }
        if(MMISET_InitBlacklistParam())
        {
            MMISET_GetBlackList();
            ...
            {
                uint32 count = 0;
                count = (uint32)MAX(g_blacklist_ptr->blacklist[j].num_len,tele_len);
                if (!MMIAPICOM_WstrStrncmp(g_blacklist_ptr->blacklist[j].num,tele_num,count))
                {
                    is_duplicate = TRUE;
                    MMIPUB_OpenAlertWarningWin(TXT_SET_WHITELIST_NUM_EXIST);                    
                    break;
        ...
        if(!is_duplicate)
        {
            total_num = g_whitelist_ptr->num;
            g_whitelist_ptr->whitelist[total_num].name_len = name_str.wstr_len;
            
            MMI_WSTRNCPY(
                         g_whitelist_ptr->whitelist[total_num].name,
                         MMIPB_NAME_MAX_LEN,
                         name_str.wstr_ptr,
                         MMIPB_NAME_MAX_LEN,
                         name_str.wstr_len
            ...
            g_whitelist_ptr->whitelist[total_num ].num_len = tele_len;
            MMI_STRNTOWSTR(
                           g_whitelist_ptr->whitelist[total_num].num, 
                           MMISET_PHONE_NUM_MAX_LEN + 2, 
                           tele_num, 
                           MMISET_PHONE_NUM_MAX_LEN + 2, 
                           tele_len
                            ...
                            )
{
    ...
    MMISET_WHITELIST_ERROR_TYPE_E add_whitelist_result = MMISET_WHITELIST_ERROR_MAX;
       
    add_whitelist_result = MMIAPISET_AddWhitelistFromExt(&mmi_pb_name,&pb_bcd);      
    
    if(MMISET_WHITELIST_ERROR_NONE == add_whitelist_result)
    {
#ifdef MMI_GUI_STYLE_TYPICAL
        MMIPUB_OpenAlertSuccessWin(TXT_COMPLETE);
#else
    ...
    else if(MMISET_WHITELIST_ERROR_BEYOND_MAX_NUM == add_whitelist_result)
    {
        MMIPUB_OpenAlertWarningWin(TXT_SET_WHITELIST_FULL);            
    }
    else if(MMISET_WHITELIST_ERROR_SAME_NUM == add_whitelist_result)
    {
        MMIPUB_OpenAlertWarningWin(TXT_SET_WHITELIST_NUM_EXIST);
    }
    else
                                            ...
                                            )
{
    MMI_RESULT_E  recode = MMI_RESULT_TRUE;
    MMIPUB_INFO_T *win_info_ptr = PNULL;
    MMISET_WHITELIST_NAME_NUM_ITEM_T *white_name_num_ptr = PNULL;
    win_info_ptr = MMIPUB_GetWinAddDataPtr(win_id);

    if (PNULL != win_info_ptr)
    {
        white_name_num_ptr  = (MMISET_WHITELIST_NAME_NUM_ITEM_T *)win_info_ptr->user_data;
    }
    ...
    case MSG_CTL_MIDSK:
    case MSG_APP_WEB:        
        MMIAPISET_SetAllWhitelistSetting(TRUE);
        if (PNULL != white_name_num_ptr)
        {
            AddNumToWhitelist(white_name_num_ptr->mmi_pb_name, white_name_num_ptr->pb_bcd);
        }
        MMK_CloseWin(win_id);
        break;
    ...
    case MSG_CLOSE_WINDOW:
        if (PNULL != white_name_num_ptr)
        {
            if (white_name_num_ptr->mmi_pb_name.wstr_ptr)
            {
                SCI_FREE(white_name_num_ptr->mmi_pb_name.wstr_ptr);
            }

            SCI_FREE(white_name_num_ptr);
        }
        recode = MMIPUB_HandleQueryWinMsg(win_id, msg_id, param);
        break;
                                            ...
                                            )
{
    ...
    case MSG_APP_WEB:
        ...
        if ((win_info_ptr != PNULL) && ((uint32 )(win_info_ptr->user_data) != NULL))
        {
            if( ID_SET_WHITELIST_NEW == win_info_ptr->user_data)
            {
                MMK_CreateWin((uint32 *)MMISET_SET_WHITELIST_NEW_WIN_TAB, PNULL);
            }
            else if(ID_SET_WHITELIST_CONTACT== win_info_ptr->user_data)
            {
               /* MMIAPIPB_OpenNumberList(
                        MMIPB_LIST_FOR_SMS,
                        MMIPB_LIST_CHECK,
...
{
    MMI_RESULT_E recode = MMI_RESULT_TRUE;
    MMI_CTRL_ID_T ctrl_id = MMISET_SET_WHITELIST_LIST_OPT_CTRL_ID;
    MMI_MENU_GROUP_ID_T group_id = 0;
    MMI_MENU_ID_T menu_id = 0;
    ...
    case MSG_OPEN_WINDOW:
        if(g_whitelist_ptr->num == 0)
        {
            MMIAPICOM_EnableGrayed(win_id, MENU_WHITELIST_OPT,ID_SET_WHITELIST_EDIT,TRUE);
            MMIAPICOM_EnableGrayed(win_id, MENU_WHITELIST_OPT,ID_SET_WHITELIST_DEL,TRUE);
            MMIAPICOM_EnableGrayed(win_id, MENU_WHITELIST_OPT,ID_SET_WHITELIST_DELALL,TRUE);
        }
        MMK_SetAtvCtrl(win_id, ctrl_id);
        GUIWIN_SetSoftkeyTextId(win_id,  TXT_COMMON_OK, TXT_NULL, STXT_RETURN, FALSE);
        break;
    ...
    case MSG_APP_OK:
        GUIMENU_GetId(ctrl_id, &group_id, &menu_id);
        HandleWhitelistMenuMsg(menu_id,ctrl_id);
        MMK_CloseWin(win_id);
        break;
...
PUBLIC BOOLEAN MMIAPISET_OpenblacklistWin(void)
{
...
PUBLIC BOOLEAN MMIAPISET_OpenWhitelistWin(void)
{
