...
#define MMISTK_DISPLAYTEXT_DELAY_DURATION			9000//5000
#define MMISTK_GETINPUT_DURATION					320000//120000
...
#define MMISTK_DTMF_DURATION                     4000
#define MMISTK_DTMF_PAUSE_DURATION               3000
#define MMISTK_CHANNEL_MAX  (8)
#define MMISTK_RECONNECT_SOCKET_TIMES_MAX  (40)
#define MMISTK_CNF_RECEIVEDATA_MAX_LEN       200
...
LOCAL BOOLEAN s_is_stk_have_msg = FALSE;
...
LOCAL uint8 s_mmistk_refresh_delay_timer_id = 0;
...
LOCAL MN_DUAL_SYS_E    s_stk_dual_sys = MN_DUAL_SYS_MAX;
...
uint16 s_mmistk_menu_chosen_itemid = 0;

uint16 s_mmistk_selectitem = 0;
...
LOCAL BOOLEAN s_mmistk_is_bear_desc = FALSE;
...
#endif
LOCAL uint32 s_test_dtmf_setTime = 0;
/**--------------------------------------------------------------------------*
	                         TYPE AND CONSTANT
 **--------------------------------------------------------------------------*/
...
uint8 item_timer_mainmenu=0;
uint8 item_timer_menu=0;
uint8 item_timer=0;
wchar* title_content_mainmenu=PNULL;
wchar* title_content_menu=PNULL;
wchar* title_content=PNULL;
uint8 title_len=0;
uint8 title_len_menu=0;
uint8 title_len_mainmenu=0;
uint8 count=0;
...
#endif
uint8 g_TopStkMenu_Select_OfMmiStk_Win = 0; /////1:selecttopmenu
...
/********************************************************************************
 ...
 DATE:			2015.11.20
********************************************************************************/
LOCAL BOOLEAN IdletextCmdShowString(MMI_STRING_T * mmistk_idle_mode_str_ptr);

/********************************************************************************
 NAME:			IdletextCmdDeleteString
 DESCRIPTION:	
 PARAM IN:		None
 PARAM OUT:		
 AUTHOR:		Yongli Tong
 DATE:			2015.11.20
********************************************************************************/
LOCAL void IdletextCmdDeleteString(void);
...
//        
/*****************************************************************************/
LOCAL void CreateDynaMenuItemData(MMI_CTRL_ID_T ctrl_id, uint16 index, STK_MENU_INFO_T *menu_info_ptr);
...
//    Note: 
/*****************************************************************************/
LOCAL void SetStkWinStatusBar(MN_DUAL_SYS_E dual_sys, MMI_WIN_ID_T win_id);
...
LOCAL BOOLEAN IdletextCmdShowString(MMI_STRING_T * mmistk_idle_mode_str_ptr)
{
    ...
    push_msg_info.e_push_msg_type = MMI_IDLE_PUSH_MSG_STK;
#if 0
    push_msg_info.app_entry_func_ptr = PNULL;
    push_msg_info.app_left_focus_action_func_ptr = PNULL;
    push_msg_info.app_right_focus_action_func_ptr = PNULL;
#else
    push_msg_info.app_entry_e = IDLE_APP_ENTRY_NULL;
    push_msg_info.app_left_focus_action_e = IDLE_APP_ENTRY_NULL;
    push_msg_info.app_right_focus_action_e = IDLE_APP_ENTRY_NULL;
#endif
    if((mmistk_idle_mode_str_ptr ->wstr_len) > MMIIDLE_LIST_MAX_STRLEN)
    {
        mmistk_idle_mode_str_ptr ->wstr_len = MMIIDLE_LIST_MAX_STRLEN;
    }
    MMIAPICOM_Wstrncpy(push_msg_info.contents, mmistk_idle_mode_str_ptr ->wstr_ptr, mmistk_idle_mode_str_ptr ->wstr_len);
    strncpy(push_msg_info.appname, appname, 4);

     return MMIIDLE_AppendPushMsg(&push_msg_info);
}
...
void MMISTK_OnSelectItemCmd(void* arg)
{
    ...
    SCI_TRACE_LOW("MMISTK_OnSelectItemCmd:cll %d, %d", s_is_mmistk_setup_call, MMIAPICC_IsInState(CC_CALL_CONNECTED_STATE));
	if (MMIAPICC_IsInState(CC_CALL_CONNECTED_STATE) || s_is_mmistk_setup_call)
	{
	    SCI_TRACE_LOW("MMISTK_OnSelectItemCmd:The ExistActiveOrSetupCall is 1, return");
    ...
    if (!param_in->is_icon_id || !param_in->icon_id.is_self_explanatory)
    {
        if (param_in->is_alpha_id /*&& s_mmistk_menutitle_need_refresh*/) 
        {
                ...
                (uint8)0x82 == *(param_in->alpha_id.id_ptr)) 
            {
                ...
                {
                    MMI_STRING_T opt_title_str;		
                    MMI_GetLabelTextByLang(STXT_OPTION, &opt_title_str);//Modify for Bug1020557
    ...
    if (!menuwin_is_open) 
    {
        ...
        CreatSoftkeyCtrl(MMISTK_MENU_WIN_ID, TXT_NULL, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK);
#endif
   ...
   // s_title_from_selected_item_context->length = s_mmistk_cur_menu_info_ptr[arg_in_ptr->dual_sys]->menu_title_len;
  //  SCI_MEMCPY(s_title_from_selected_item_context->stringData,&s_mmistk_cur_menu_info_ptr[arg_in_ptr->dual_sys]->menu_title,sizeof(s_title_from_selected_item_context->stringData));

#if 0
    if (CFW_SSCheckTestSim(arg_in_ptr->dual_sys))
	{
		//set title
    ...
    GUIWIN_SetTitleText(MMISTK_MENU_WIN_ID, menu_name.wstr_ptr, menu_name.wstr_len, FALSE);
    //SetStkWinStatusBar(arg_in_ptr->dual_sys, MMISTK_MENU_WIN_ID);
	...
	else //no item identifier in selectitem proactive command
	{
		uint16 select_index1 = 0;
	    SCI_TRACE_LOW("[MMI_STK] GUILIST_SetTopItemIndex no item identifier, s_mmistk_menu_chosen_itemid %d", s_mmistk_menu_chosen_itemid);

		select_index1 = s_mmistk_menu_chosen_itemid;
		if(select_index1 >= 1)
			select_index1--;
		GUILIST_SetCurItemIndex(MMISTK_MENU_CTRL_ID, select_index1);
		s_mmistk_menu_chosen_itemid = 0;
	}
    if (menuwin_is_open) 
    {
												...
												DPARAM param)
{
    ...
    SCI_TRACE_LOW("[MMI_STK] enter SelectItemOptionWinHandleMsg, msg_id = 0x%04X",msg_id);
    switch (msg_id) 
    {
...
// no use
    ...
    MMK_CreateWindow( &win_create );
#ifndef MMI_PDA_SUPPORT
    CreatSoftkeyCtrl(MMISTK_SELECT_ITEM_OPTION_WIN_ID, TXT_NULL, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK);
#endif
...
// no use
    ...
    //SetStkWinStatusBar(dual_sys, MMISTK_SELECT_ITEM_OPTION_WIN_ID);
...
LOCAL void OpenTopMenuOptionWin(MN_DUAL_SYS_E dual_sys)
{
    ...
    //SetStkWinStatusBar(dual_sys, MMISTK_TOP_MENU_OPTION_WIN_ID);
...
LOCAL void CreateDisplayTextVision(MN_DUAL_SYS_E dual_sys)
{
...
#ifndef MMI_PDA_SUPPORT
        CreatSoftkeyCtrl(MMISTK_DISPLAYTEXT_WIN_ID, TXT_NULL, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK);
#else
    ...
    //SetStkWinStatusBar(dual_sys, MMISTK_DISPLAYTEXT_WIN_ID);
...
void MMISTK_OnDisplayTextCmd(void* arg)
{
    ...
    if(FALSE == MMICDMA_CheckCard_OK(arg_in_ptr->dual_sys))
    {
        return;
    }
#endif
...
//        && !(param_in->icon_id.icon_id & 1)
	...
	//SCI_TRACE_LOW("[MMI_STK] MMISTK_OnDisplayTextCmd param_in->user_clear %d", param_in->user_clear);
    ...
    if (param_in->is_ir) 
    {
        if (s_mmistk_displaytext_cnf)
        {
        	SCI_TRACE_LOW("Display-text s_mmistk_displaytext_is_icon_exist %d", s_mmistk_displaytext_is_icon_exist);
...
//      MMK_CloseWin(MMISTK_DISPLAYTEXT_WIN_ID);
    ...
    //for 505445,case 27.22.4.1.4/2 fail  follow orange version
    if (0 != s_mmistk_displaytext_timer_duration)
    {
       s_mmistk_displaytext_timer_id = MMK_CreateWinTimer(MMISTK_DISPLAYTEXT_WIN_ID,
                                                               ...
                                                               FALSE);
       //SCI_TRACE_LOW("[MMI_STK] MMISTK_OnDisplayTextCmd timerid %d", s_mmistk_displaytext_timer_id);
    }
...
LOCAL void CreateGetInkeyVision(MN_DUAL_SYS_E dual_sys)
{
    ...
    if (!win_exist)
    {
        if (s_mmistk_inkey_type != SIMAT_YES_NO)
        {
			...
			{
				CreatSoftkeyCtrl(MMISTK_GETINKEY_WIN_ID, TXT_HELP, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK);
			} 
			else
			{
				CreatSoftkeyCtrl(MMISTK_GETINKEY_WIN_ID, TXT_NULL, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK);
			}
#else
            if (s_mmistk_is_inkey_help_info)
            {
                CreatSoftkeyCtrl(MMISTK_GETINKEY_WIN_ID, STXT_SOFTKEY_OK_MK, TXT_NULL, TXT_HELP);
            }
            else
            {
    ...
    else
    {
        ...
        {
#ifndef MMI_PDA_SUPPORT
            GUIWIN_SetSoftkeyTextId(MMISTK_GETINKEY_WIN_ID, TXT_NULL, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK,FALSE);
#else
            if (s_mmistk_is_inkey_help_info)
            {
                GUIWIN_SetSoftkeyTextId(MMISTK_GETINKEY_WIN_ID, STXT_SOFTKEY_OK_MK, TXT_NULL, TXT_HELP, FALSE);
            }
            else
            {
    ...
    if (is_icon_can_display)
    {
     ...
     GUIWIN_SetTitleText(MMISTK_GETINKEY_WIN_ID, 
                                 ...
                                 FALSE);
    //MMIAPICOM_StatusAreaSetViewHeaderinfo(s_title_from_selected_item_context->stringData, s_title_from_selected_item_context->length);
    //MMIAPICOM_StatusAreaSetViewHeaderinfo(s_mmistk_displaydata.wstr_ptr, s_mmistk_displaydata.wstr_len);
...
#ifndef MMI_PDA_SUPPORT
        GUIEDIT_SetSoftkeyEx(MMISTK_EDITBOX_CTRL_ID, 2, 0, TXT_COMMON_COMMON_BACK, TXT_COMMON_COMMON_BACK, PNULL, PNULL, EDIT_BUTTON_TEXT_ID, PNULL);
#endif
...
void MMISTK_OnGetInkeyCmd(void* arg)
{
	...
	SCI_TRACE_LOW("[MMI_STK] enter MMISTK_OnGetInkeyCmd text_str.len %d", param_in->text_str.len);
	/*if(param_in->text_str.len == 0)
	{
        param_out.mp_info = SIMAT_MP_NO_SPECIFIC_CAUSE;
        param_out.result = SIMAT_TR_REQUIRE_V_MISSING;
...
LOCAL void CreateGetInputVision(MN_DUAL_SYS_E dual_sys)
{
    ...
    if (!win_exist) 
    {
        ...
        {
            CreatSoftkeyCtrl(MMISTK_GETINPUT_WIN_ID, TXT_NULL, TXT_HELP, TXT_COMMON_COMMON_BACK);
        } 
        else
        {
            CreatSoftkeyCtrl(MMISTK_GETINPUT_WIN_ID, TXT_NULL, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK);
        }
#else
        if (s_mmistk_is_input_help_info)
        {
            CreatSoftkeyCtrl(MMISTK_GETINPUT_WIN_ID, STXT_SOFTKEY_OK_MK, TXT_NULL, TXT_HELP);
        }
        else
        {
    ...
    else
    {
        ...
        if (s_mmistk_is_input_help_info)
        {
            GUIWIN_SetSoftkeyTextId(MMISTK_GETINPUT_WIN_ID, STXT_SOFTKEY_OK_MK, TXT_NULL, TXT_HELP, FALSE);
        }
        else
        {
...
#endif
    {
         ...
         GUIWIN_SetTitleText(MMISTK_GETINPUT_WIN_ID, 
                                     ...
                                     s_mmistk_displaydata.wstr_len,
                                     FALSE);
         //GUIWIN_SetTitleBackground(MMISTK_GETINPUT_WIN_ID, 0,  MMI_WHITE_COLOR);
         //GUIWIN_SetTitleFontColor(MMISTK_GETINPUT_WIN_ID, MMI_BLACK_COLOR);
	  ...
	  if (CFW_SSCheckTestSim(dual_sys))
         {
		CreateSTKBufferMemory(&title_content,MMISTK_STRING_MAX_LEN*4);
	        SCI_MEMSET(title_content, 0x00, MMISTK_STRING_MAX_LEN*4);
                title_len=s_mmistk_displaydata.wstr_len;
                MMIAPICOM_Wstrcpy(title_content,s_mmistk_displaydata.wstr_ptr);
         }
#endif
    }
    ...
    if (!s_mmistk_input_is_echo) 
    {
        GUIEDIT_SetStyle(MMISTK_EDITBOX_CTRL_ID, GUIEDIT_STYLE_MULTI_DYN_DOWN);
        ...
        else
        {
            ...
            GUIEDIT_SetIm(MMISTK_EDITBOX_CTRL_ID, allow_im, init_im);
        }
    }
...
void MMISTK_OnGetInputCmd(void* arg)
{
   ...
   /* if(param_in->text_str.len == 0) //ut3 exception mandaroy Tlvnoneexist
   	{
        param_out.result = SIMAT_TR_REQUIRE_V_MISSING;
        param_out.mp_info = SIMAT_MP_NO_SPECIFIC_CAUSE;
        s_mmistk_getinput_cnf = FALSE;
...
LOCAL void CreateSendSmsVision(MN_DUAL_SYS_E dual_sys)
{
...
#ifndef MMI_PDA_SUPPORT
        CreatSoftkeyCtrl(MMISTK_SENDSM_WIN_ID, TXT_NULL, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK);
#endif
    ...
    //SetStkWinStatusBar(dual_sys, MMISTK_SENDSM_WIN_ID);
...
void MMISTK_OnSendSmsCmd(void* arg)
{
    ...
    if (!MMIAPIPHONE_IsSimAvailable(arg_in_ptr->dual_sys))
    {
        param_out.mp_info = SIMAT_MP_NO_SPECIFIC_CAUSE;
        param_out.result = SIMAT_TR_ME_UNABLE;
    	SCI_TRACE_LOW("[MMI_STK]SIMAT_TR_ME_UNABLE Cnf SendSMS");
    ...
    if ((MMIAPICC_IsInState(CC_MT_CONNECTING_STATE)) || MMIAPICC_IsInState(CC_MO_CONNECTING_STATE))
    {
        param_out.mp_info = SIMAT_MP_BUSY_ON_CALL;
        param_out.result = SIMAT_TR_ME_UNABLE;
    	SCI_TRACE_LOW("[MMI_STK]SIMAT_TR_ME_UNABLE second Cnf SendSMS");
        SIMAT_CnfSendSmsEx((uint8)arg_in_ptr->dual_sys, &param_out);
    ...
    if (!MMIAPIPHONE_IsCSAvail(arg_in_ptr->dual_sys))
    {
        param_out.result = SIMAT_TR_ME_UNABLE;
        param_out.mp_info = SIMAT_MP_NO_SPECIFIC_CAUSE;
    	SCI_TRACE_LOW("[MMI_STK]SIMAT_TR_ME_UNABLE third Cnf SendSMS");
    ...
    SCI_TRACE_LOW("[MMI_STK]SendSMS mn_result %d", mn_result);
...
LOCAL void CreateSetupCallVision(MN_DUAL_SYS_E dual_sys)
{
...
#ifndef MMI_PDA_SUPPORT
        CreatSoftkeyCtrl(MMISTK_SETUP_CALL_WIN_ID, TXT_NULL, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK);
#else
    ...
    //SetStkWinStatusBar(dual_sys, MMISTK_SETUP_CALL_WIN_ID);
...
void MMISTK_OnSetupCallCmd(void* arg)
{
   ...
   tel_len = MMIAPICOM_GenDispNumber(param_in->call_parameter.address.number_type,
    ...
    if (!MMIAPIPHONE_IsCSAvail(arg_in_ptr->dual_sys)&& !MMIAPIPHONE_IsPSAvail(arg_in_ptr->dual_sys) && !MMIAPICC_IsEmergencyNum(s_mmistk_setup_call_str, s_mmistk_setup_call_str_len, arg_in_ptr->dual_sys, FALSE))
    {
        call_cnf.is_command_changed = FALSE;
        call_cnf.result = SIMAT_TR_ME_UNABLE;
        call_cnf.mp_info = SIMAT_MP_NO_SPECIFIC_CAUSE;
		SCI_TRACE_LOW("Mmistk_win: setupcall SIMAT_TR_ME_UNABLE");
...
LOCAL void CreatePlayToneVision(MN_DUAL_SYS_E dual_sys)
{
...
#ifndef MMI_PDA_SUPPORT
        CreatSoftkeyCtrl(MMISTK_PLAYTONE_WIN_ID, TXT_NULL, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK);
#else
    ...
    //SetStkWinStatusBar(dual_sys, MMISTK_PLAYTONE_WIN_ID);
...
void MMISTK_OnPlayToneCmd(void* arg)
{
        ...
        && 0 != param_in->duration)
    {
	 ...
	 if ( 16 == param_in->tone ) //general beep 
	 	duration += 200;
    }	
   /* if (param_in->tone == 0)//bug 1057459
    {
        SIMAT_PLAY_TONE_CNF_T playtone_cnf = {0};
		
		SCI_TRACE_LOW("[MMI_STK] leave MMISTK_OnPlayToneCmd, as mmi_tone index is 0");
		playtone_cnf.result = SIMAT_TR_SUCCESS;
        playtone_cnf.mp_info = SIMAT_MP_NO_SPECIFIC_CAUSE;
        SIMAT_CnfPlayToneEx(arg_in_ptr->dual_sys, &playtone_cnf);
        s_mmistk_playtone_cnf = 0;
...
//- '06' = TIA/EIA/IS-2000;
            if (MN_GMMREG_RAT_GPRS == gmmreg_rat)
            {
                ...
                access_cnf.access_type = SIMAT_ACCESS_TECHNOLOGY_TYPE_GSM;
            }
            else if (MN_GMMREG_RAT_3G == gmmreg_rat || MN_GMMREG_RAT_TD == gmmreg_rat)
            {
                access_cnf.result = SIMAT_TR_SUCCESS;
                access_cnf.mp_info = SIMAT_MP_NO_SPECIFIC_CAUSE;
                access_cnf.access_type = SIMAT_ACCESS_TECHNOLOGY_TYPE_UTRAN;
            }
            else
    #endif
            {
...
void MMISTK_OnSendUSSDCmd(void* arg)
{
    ...
    if (!MMIAPIPHONE_IsSimAvailable(arg_in_ptr->dual_sys))
    {
		...
		SCI_TRACE_LOW("[MMI_STK] send ussd failure SIMAT_TR_ME_UNABLE");
    ...
    if ((MMIAPICC_IsInState(CC_MT_CONNECTING_STATE)) || MMIAPICC_IsInState(CC_MO_CONNECTING_STATE))
    {
        ...
        ussd_cnf.result = SIMAT_TR_ME_UNABLE;
		SCI_TRACE_LOW("[MMI_STK] send ussd failure SIMAT_MP_BUSY_ON_CALL");
    ...
    MMISTK_SetSTKSim(arg_in_ptr->dual_sys);
	SCI_TRACE_LOW("[MMI_STK] send ussd s_mmistk_sendss_waiting_respond %d, s_mmistk_sendss_waiting_respond %d", s_mmistk_sendss_waiting_respond, s_mmistk_sendss_waiting_respond);
    ...
    if (!MMIAPIPHONE_IsCSAvail(arg_in_ptr->dual_sys))
    {
        ussd_cnf.result = SIMAT_TR_ME_UNABLE;
        ussd_cnf.mp_info = SIMAT_MP_NO_SPECIFIC_CAUSE;
		SCI_TRACE_LOW("[MMI_STK] send ussd failure IsCSinAvail");
...
// no use
        ...
        //for bug512246
//        MMIPUB_OpenAlertWinByTextId(PNULL,TXT_SIM_UPDATE_COMPLETE,TXT_NULL,IMAGE_PUBWIN_SUCCESS,PNULL,&win_priority,MMIPUB_SOFTKEY_ONE,PNULL);
    }
    else
    {
#if 0
...
/********************************************************************************
 ...
 DATE:			2004.08.31
********************************************************************************/
APP_MN_SIMAT_REFRESH_IND_T g_refresh_back;
    ...
    switch(param_in->refresh_type)
    {
        ...
        case SIMAT_RF_RESET:
	 	case SIMAT_RF_3G_SESSION_RESET:
            if(((SIMAT_RF_INITIAL == param_in->refresh_type) || (SIMAT_RF_RESET == param_in->refresh_type))
            && s_is_stk_have_msg)
            {
                IdletextCmdDeleteString();
                s_is_stk_have_msg = FALSE;
            }
                ...
                && MMIAPISMS_IsOrderOk())
            {
                ...
                if (ERR_MNDATAMAG_NO_ERR == err_code)
                {
                    ...
                    MMIPUB_OpenWaitWin(1,&wait_text,PNULL,PNULL,MMISTK_REFRESH_WAITING_WIN_ID,IMAGE_NULL,
                                                    ANIM_PUBWIN_WAIT,WIN_ONE_LEVEL,MMIPUB_SOFTKEY_NONE,HandleRefreshWaitingWindow);


                    MMK_SetWinDisplayStyleState(MMISTK_REFRESH_WAITING_WIN_ID, WS_DISABLE_RETURN_WIN, TRUE);

                        if ((SIMAT_RF_RESET == param_in->refresh_type) 
                            && MMIAPIPHONE_GetIsPsReady(dual_sys))
                        {
            ...
            else
            {
				...
				if(0x01 == MMIPB_GetPBStatus()) //  MMIPB_STATUS_SIM_NOT_READY = 0x1, //SIM卡没有初始化完成
				{
				    SCI_TRACE_LOW("Not allow do STK Refresh refresh back move");
				    if(SIMAT_RF_INITIAL == param_in->refresh_type)
				    {
				        SCI_TRACE_LOW("delay handle this");
				        SCI_Memcpy(&g_refresh_back, arg, sizeof(g_refresh_back));
				        if (0 != s_mmistk_refresh_delay_timer_id)
				            MMK_StopTimer(s_mmistk_refresh_delay_timer_id);
				        s_mmistk_refresh_delay_timer_id= MMK_CreateTimerCallback(3000,  MMISTK_HandleSTKTimer, NULL, FALSE);
				    }
				    else
				    {
				        prefresh.result = SIMAT_TR_BACK_MOVE;
				        prefresh.mp_info = SIMAT_MP_NO_SPECIFIC_CAUSE;
				...
				{
					SCI_TRACE_LOW("Not allow do STK Refresh type = %d");
...
/********************************************************************************
 ...
 DATE:			2015.10.21
********************************************************************************/
PUBLIC uint8 MMISTK_GetIdleTimerID(void)
{
    return g_iSTKTimerId;
}

/********************************************************************************
 NAME:			MMISTK_SetIdleTimerID
 DESCRIPTION:	
 PARAM IN:		
 PARAM OUT:		None
 AUTHOR:		Yongli Tong
 DATE:			2015.10.21
********************************************************************************/
PUBLIC void MMISTK_SetIdleTimerID(uint8 iSTKTimerId)
{
    g_iSTKTimerId = iSTKTimerId;
}
...
void MMISTK_OnSetupIdletextCmd(void* arg)
{
    ...
    if (0 != param_in->text_string.len || (0 == param_in->text_string.len &&!param_in->is_icon_id))
    {
        //if (!param_in->is_icon_id || !param_in->icon_id.is_self_explanatory)
        {
            ...
            //MMIAPIIDLE_MAIN_SetIdleModeText(arg_in_ptr->dual_sys, &mmistk_idle_mode_mmistr);
#endif
            if(s_is_stk_have_msg)
            {
                IdletextCmdDeleteString();
            }
            IdletextCmdShowString(&mmistk_idle_mode_mmistr);
            s_is_stk_have_msg = TRUE;
            /*push_msg_info.e_push_msg_type = MMI_IDLE_PUSH_MSG_STK;
            push_msg_info.app_entry_func_ptr = PNULL;
            push_msg_info.app_left_focus_action_func_ptr = PNULL;
            push_msg_info.app_right_focus_action_func_ptr = PNULL;
            if(mmistk_idle_mode_mmistr.wstr_len > MMIIDLE_LIST_MAX_STRLEN)
            {
               mmistk_idle_mode_mmistr.wstr_len = MMIIDLE_LIST_MAX_STRLEN;
            }
            MMIAPICOM_Wstrncpy(push_msg_info.contents, mmistk_idle_mode_mmistr.wstr_ptr, mmistk_idle_mode_mmistr.wstr_len);
            strncpy(push_msg_info.appname, appname, 4);

            MMIIDLE_AppendPushMsg(&push_msg_info);*/
            /*if(0 == g_iSTKTimerId)
            {
                g_iSTKTimerId = MMK_CreateWinTimer(MAIN_IDLE_WIN_ID, IDLE_STK_TIME, FALSE);
             }
             else
             {
                 MMK_StartWinTimer(MAIN_IDLE_WIN_ID, g_iSTKTimerId,IDLE_STK_TIME, FALSE);
             }*/
        }
...
void MMISTK_OnCcToMmiCmd(void* arg)
{
    ...
    else
    {
        if (0 != param_in->alpha_id.len)//alpha id present
        {
            ...
            if (SIMAT_SIMC_ALLOWED_AND_MODIFIED == result)//call modified,alpha id存在，不能修改号码
            {
...
/********************************************************************************
 ...
 DATE:			2004.08.17
********************************************************************************/
uint8 stk_temp_index =0;;
uint16 stk_top_index =0;
MMI_RESULT_E MMISTK_MainMenuWinHandleMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
	...
	GUILIST_SetCurItemIndex(MMISTK_MAINMENU_CTRL_ID,stk_temp_index);
	GUILIST_SetTopItemIndex(MMISTK_MAINMENU_CTRL_ID,stk_top_index);
	GUILIST_SetSelectedItem(MMISTK_MAINMENU_CTRL_ID,stk_temp_index,1);
	MMK_SendMsg(MMISTK_MAINMENU_WIN_ID, MSG_FULL_PAINT, PNULL);
#ifdef GCF_TA_SUPPORT
	if (CFW_SSCheckTestSim(dual_sys))
        {
	     item_timer_mainmenu = MMK_CreateWinTimer(
                                                        MMISTK_MAINMENU_WIN_ID, 
                                                        1000,
                                                        TRUE
                                                        );
	}
...
//    case MSG_APP_HASH:
    ...
    case MSG_CTL_LIST_NEED_ITEM_DATA:
        {
            ...
            CreateDynaMenuItemData(MMISTK_MAINMENU_CTRL_ID, index, s_mmistk_toplevel_menu_info[dual_sys]);
        }
        break;
 ...
 case MSG_TIMER:
      ...
      if (CFW_SSCheckTestSim(dual_sys))
	 {
		 if(title_len_mainmenu>20)
		 {
			 if(count<title_len_mainmenu)
			 {
				MMIAPICOM_StatusAreaSetViewHeaderinfo(&title_content_mainmenu[count],title_len_mainmenu-count);
		        count++;
			 }
			 else
			 {
			 	count=0;
				MMIAPICOM_StatusAreaSetViewHeaderinfo(title_content_mainmenu,title_len_mainmenu);
			 }
		 }
	 }
#endif
    ...
    case MSG_CTL_MIDSK:
        if (s_mmistk_indmenusel_cnf) 
        {
            ...
            if (s_mmistk_toplevel_menu_info[dual_sys]->menu_item_amount > 0)
            {
   			...
   			stk_temp_index = menuitem_id;
			stk_top_index =     GUILIST_GetTopItemIndex(MMISTK_MAINMENU_CTRL_ID);
            if (menuitem_id < s_mmistk_toplevel_menu_info[dual_sys]->menu_item_amount)
            {
                menu_select.menu_id = s_mmistk_toplevel_menu_info[dual_sys]->menu_item[menuitem_id].itemID;
                s_mmistk_indmenusel_cnf = FALSE;
                ...
                //SCI_TraceLow("STK s_mmistk_menu_chosen_itemid=%d, id %d ",s_mmistk_menu_chosen_itemid, menu_select.menu_id );
    ...
    case MSG_GET_FOCUS:
        ...
        MMIAPICOM_StatusAreaSetViewHeaderinfo(s_mmistk_toplevel_menu_info[dual_sys]->menu_title, s_mmistk_toplevel_menu_info[dual_sys]->menu_title_len);
		break;
    ...
    case MSG_CLOSE_WINDOW:
		...
		if (CFW_SSCheckTestSim(dual_sys))
   	{
   	        MMK_StopTimer(item_timer_mainmenu);
					DestroySTKStrMemory(&title_content_mainmenu);
    ...
    case MSG_OPEN_WINDOW:
     ...
     /*   if (0 != s_mmistk_selectitem_timer_id)
        ...
        MMK_SetAtvCtrl(MMISTK_MENU_WIN_ID, MMISTK_MENU_CTRL_ID);
			...
			if (CFW_SSCheckTestSim(dual_sys))
        {
		 if (0 != item_timer_menu )
        {
            MMK_StopTimer(item_timer_menu );
            item_timer_menu  = 0;
        }
        item_timer_menu = MMK_CreateWinTimer(	
                                                        MMISTK_MENU_WIN_ID, 
                                                        1000,
                                                        TRUE
                                                        );
...
//    case MSG_APP_HASH:
    ...
    case MSG_CTL_LIST_NEED_ITEM_DATA:
        {
            ...
            index = need_item_data_ptr->item_index;
            CreateDynaMenuItemData(MMISTK_MENU_CTRL_ID, index, s_mmistk_cur_menu_info_ptr[dual_sys]);
        }
        break;
    ...
    case MSG_TIMER:
        ...
        else
        {
         ...
         if (CFW_SSCheckTestSim(dual_sys))
	 {
		if(count<title_len_menu)
		{ 
			MMIAPICOM_StatusAreaSetViewHeaderinfo(&title_content_menu[count],title_len_menu-count);
			count++;
		}
		else
		{
			count=0; 
			MMIAPICOM_StatusAreaSetViewHeaderinfo(title_content_menu,title_len_menu);
		}
    ...
    case MSG_CTL_CANCEL:
        ...
        if (s_mmistk_selectitem_cnf) 
        {
            ...
            menu_select.item_id = s_mmistk_menu_chosen_itemid;
            SIMAT_CnfSelItemEx(dual_sys, &menu_select);
           SCI_TRACE_LOW("mmistk_win: cancel g_TopStkMenu_Select_OfMmiStk_Win =%d",g_TopStkMenu_Select_OfMmiStk_Win);
            if(g_TopStkMenu_Select_OfMmiStk_Win == 0)
            {
                s_mmistk_menu_chosen_itemid = 0;
                MMIAPISTK_EnterToplevelMenu(dual_sys);  //must for cmcc stk goto topmenu
            }
        }
        if (0 != s_mmistk_selectitem_timer_id)
        {
...
//                menu_select.result = SIMAT_TR_ICON_FAIL_DISPLAY;
            ...
            if (menuitem_id < s_mmistk_cur_menu_info_ptr[dual_sys]->menu_item_amount)
            {
              ...
              SCI_TRACE_LOW("[MMI_STK] itemID is %d", s_mmistk_cur_menu_info_ptr[dual_sys]->menu_item[menuitem_id].itemID);
              //if menu item is "pre page" or "next page",then press "right soft key" will return to topmenu
              if( ( s_mmistk_cur_menu_info_ptr[dual_sys]->menu_item[menuitem_id].itemID==0xFE)||
                   ( s_mmistk_cur_menu_info_ptr[dual_sys]->menu_item[menuitem_id].itemID==0xFF))
                   g_TopStkMenu_Select_OfMmiStk_Win = 0;
              else
                   g_TopStkMenu_Select_OfMmiStk_Win = 1;


                s_mmistk_menu_chosen_itemid = menu_select.item_id; //save cur itemid
                SCI_TRACE_LOW("mmistk_win: SIMAT_CnfSelItemEx s_mmistk_menu_chosen_itemid %d", s_mmistk_menu_chosen_itemid);
    ...
    case MSG_CLOSE_WINDOW:
        ...
        {
            uint16 index = 0;
            if(s_mmistk_cur_menu_info_ptr[dual_sys])
            {
   	...
   	{
   	MMK_StopTimer(item_timer_menu);
			DestroySTKStrMemory(&title_content_menu);
    ...
    case MSG_CTL_CANCEL:
        ...
        if (s_mmistk_displaytext_cnf) 
        {
            ...
            if (!s_mmistk_displaytext_is_ir)
            {
              ...
              if (s_mmistk_cur_menu_info_ptr[dual_sys]->menu_item_amount > 0)
              {
		         s_mmistk_menu_chosen_itemid = s_mmistk_cur_menu_info_ptr[dual_sys]->menu_item[GUILIST_GetCurItemIndex(MMISTK_MENU_CTRL_ID)].itemID;

		         SCI_TRACE_LOW("mmistk_win: disp move  itemid %d", s_mmistk_menu_chosen_itemid);
		         s_mmistk_selectitemtr_result = SIMAT_TR_BACK_MOVE;
               }
#endif
		         SIMAT_CnfDisplayTextEx(dual_sys, &disp_cnf);
		         s_mmistk_close_window = MMISTK_CLOSE_DISPLAYTEXT_WIN;
				 s_mmistk_displaytext_is_ir = TRUE;//add for the displaytext's exit when first boot
	    ...
	    if (s_mmistk_cur_menu_info_ptr[dual_sys]->menu_item_amount > 0)
            {
              s_mmistk_menu_chosen_itemid = s_mmistk_cur_menu_info_ptr[dual_sys]->menu_item[GUILIST_GetCurItemIndex(MMISTK_MENU_CTRL_ID)].itemID;
               ...
               SCI_TRACE_LOW("mmistk_win: disp ok  itemid %d", s_mmistk_menu_chosen_itemid);
               s_mmistk_selectitemtr_result = SIMAT_TR_BACK_MOVE;
             }
#endif
                SIMAT_CnfDisplayTextEx(dual_sys, &disp_cnf);
    ...
    case MSG_TIMER:
        if (*(uint8*)param == s_mmistk_displaytext_timer_id) 
        {
            ...
            if (s_mmistk_displaytext_cnf) 
            {
                ...
                s_mmistk_displaytext_cnf = FALSE;
                s_mmistk_text_is_displaying[dual_sys] = FALSE;
				SCI_TRACE_LOW("[MMI_STK] displaytext_cnf.result %d", disp_cnf.result);
                if (!s_mmistk_displaytext_is_ir)
                {
                    SIMAT_CnfDisplayTextEx(dual_sys, &disp_cnf);
					MMK_CloseWin(win_id);
...
//                    inkey_cnf.result = SIMAT_TR_ICON_FAIL_DISPLAY;
            ...
            else 
            {
				...
				if(g_TopStkMenu_Select_OfMmiStk_Win == 0)
				{
					SCI_TRACE_LOW("stkk 112\n");
					s_mmistk_getinkey_cnf = FALSE;
					s_mmistk_key_is_getting[dual_sys] = FALSE;
					SIMAT_CnfGetInkeyEx(dual_sys, &inkey_cnf);
					s_mmistk_close_window = MMISTK_CLOSE_GETINKEY_WIN;
					
					MMIAPISTK_EnterToplevelMenu(dual_sys);
				}
            }
			SCI_TRACE_LOW("stkk 12\n");
			if(g_TopStkMenu_Select_OfMmiStk_Win != 0)
			{
				SCI_TRACE_LOW("stkk 1212\n");
				s_mmistk_getinkey_cnf = FALSE;
				s_mmistk_key_is_getting[dual_sys] = FALSE;
				SIMAT_CnfGetInkeyEx(dual_sys, &inkey_cnf);
				s_mmistk_close_window = MMISTK_CLOSE_GETINKEY_WIN;
    ...
    case MSG_CTL_PENOK:   
        ...
        {
            if (s_mmistk_is_inkey_help_info
                && (MSG_CTL_OK == msg_id || MSG_APP_WEB == msg_id || MSG_SK_SKB_ICON_PRESSED == msg_id))
            {
                MMK_StopTimer(s_mmistk_getinkey_timer_id);
                s_mmistk_getinkey_timer_id = 0;
...
//                    inkey_cnf.result = SIMAT_TR_ICON_FAIL_DISPLAY;
                ...
                else
                {
                    ...
                    MMI_WIN_ID_T	alert_win_id = MMISTK_MSGBOX_WIN_ID;
...
//                        input_cnf.result = SIMAT_TR_ICON_FAIL_DISPLAY;
    ...
    case MSG_TIMER:
	 ...
	 if(count<title_len)
	 {
		MMIAPICOM_StatusAreaSetViewHeaderinfo(&title_content[count],title_len-count);
                count++;
	 }
	 else
	 {
	 	count=0;
		MMIAPICOM_StatusAreaSetViewHeaderinfo(title_content,title_len);
	 }
	 }
#endif
        break;
    ...
    case MSG_CLOSE_WINDOW:
	 ...
	 if (CFW_SSCheckTestSim(dual_sys))
   	{
   	        MMK_StopTimer(item_timer);
		DestroySTKStrMemory(&title_content);
   	}
#endif
        break;
...
LOCAL MMI_RESULT_E SendSmsWinHandleMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
    ...
    case MSG_CLOSE_WINDOW:
        if (s_mmistk_sendsms_cnf[dual_sys]) 
        {
			...
			SCI_TRACE_LOW("[MMI_STK]SIMAT_TR_TERMINATED_BY_USER Cnf SendSMS");
...
LOCAL MMI_RESULT_E PlayToneWinHandleMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
    ...
    case MSG_CTL_OK:
        ...
        s_mmistk_close_window = MMISTK_CLOSE_PLAYTONE_WIN;
		MMK_CloseWin(MMISTK_PLAYTONE_WIN_ID); //add 2019/5/20
...
LOCAL void CreateSendSSVision(MN_DUAL_SYS_E dual_sys)
{
...
#ifndef MMI_PDA_SUPPORT
        CreatSoftkeyCtrl(MMISTK_SENDSS_WIN_ID, TXT_NULL, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK);
#endif
    ...
    //SetStkWinStatusBar(dual_sys, MMISTK_SENDSS_WIN_ID);
...
LOCAL void StartSetupCallOperation(MN_DUAL_SYS_E dual_sys)
{
    ...
    case SIMAT_STC_FREE_REDIAL:
		...
		if(!s_is_mmistk_setup_call)//BUG1063247
	    {
            setup_call_cnf.is_command_changed = FALSE;
            setup_call_cnf.result = SIMAT_TR_ME_UNABLE;
            setup_call_cnf.mp_info = SIMAT_MP_BUSY_ON_CALL;
            SIMAT_CnfSetupCallEx((uint8)dual_sys, &setup_call_cnf);
            s_mmistk_close_window = MMISTK_CLOSE_SETUP_CALL_WIN;
            SCI_TRACE_LOW("[MMI_STK] SetupCall WinHandleMsg because s_is_mmistk_setup_call is FALSE");
            return;
	    }
		SCI_TRACE_LOW("[MMI_STK] StartSetupCallOperation TO MMISTK_SetupCall");
    ...
    case SIMAT_STC_HOLD_REDIAL:
        if (ExistActiveOrSetupCall() && s_is_mmistk_setup_call)
        {
            cc_result = MMIAPICC_HoldAndAnswerCW();//Hold call
...
LOCAL MMI_RESULT_E SetupCallWinHandleMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
    ...
    case MSG_LOSE_FOCUS:
		#if 0
		SCI_TRACE_LOW("SetupCallWinHandleMsg MSG_LOSE_FOCUS 0x%x, s_is_mmistk_setup_call %d", MSG_LOSE_FOCUS, s_is_mmistk_setup_call);
    ...
    if (is_oper_cnf)
    {
        if (!is_call_active && (MMISTK_SETUP_CALL_NO_OPER != s_mmistk_call_oper_type))
        {
            ...
            if (CC_RESULT_SUCCESS == cc_result)
            {
                ...
                //deleted by Tong Yongli. according to Coverity CID 10042
...
//                    setup_call_cnf.result = SIMAT_TR_ICON_FAIL_DISPLAY;
    ...
    else
    {
        if (!MMIAPISTK_GetSetupCallDuration())
        {
			...
			else if(CC_RELEASE_OPER == disc_cause)
			{
			   	setup_call_cnf.result = SIMAT_TR_USER_DENY_CALL;
				setup_call_cnf.mp_info = SIMAT_MP_NO_SPECIFIC_CAUSE;
			}
			...
			SCI_TRACE_LOW("MMistk_win:  s_mmistk_setup_call_str_len  %d", s_mmistk_setup_call_str_len);
			if(s_mmistk_setup_call_str_len >= 40)
			{
                setup_call_cnf.result = SIMAT_TR_DATA_NOT_UNDERSTOOD;
			}
            SIMAT_CnfSetupCallEx(dual_sys, &setup_call_cnf);
            s_mmistk_close_window = MMISTK_CLOSE_SETUP_CALL_WIN;
        }
...
LOCAL MMI_RESULT_E SendSSWinHandleMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
    ...
    case MSG_CTL_MIDSK:
        // do nothing
        SCI_TRACE_LOW("SendSSWinMsg Ok ");
      //  MMK_CloseWin(win_id);
        break;
	case MSG_APP_CANCEL:
    case MSG_CTL_CANCEL:
		{
			SCI_TRACE_LOW("SendSSWinMsg Cancel dual_sys %d, s_mmistk_sendussd_waiting_respond %d",dual_sys, s_mmistk_sendussd_waiting_respond);

			if(s_mmistk_sendussd_waiting_respond)
			{
				s_mmistk_sendussd_waiting_respond = FALSE;

				ussd_cnf.result = SIMAT_TR_USSD_SS_TERMINATED_BY_USER; 
				ussd_cnf.mp_info = SIMAT_MP_NO_SPECIFIC_CAUSE;
				SIMAT_CnfSendUssdEx(dual_sys, &ussd_cnf);
			}
    	}
		MMK_CloseWin(win_id);
		break;
...
BOOLEAN MMISTK_OnSendUSSDCnf(void *arg)
{
    ...
    if(!s_mmistk_sendussd_waiting_respond)
    {
		...
		SCI_TRACE_LOW("[MMI_STK] MMISTK_OnSendUSSDCnf TRSENDUSSD SIMAT_TR_TERMINATED_BY_USER");
        return FALSE;
    }
...
LOCAL void OnSetupMenuInit(MN_DUAL_SYS_E dual_sys, SIMAT_SETUP_MENU_T* param_in, STK_MENU_INFO_T* toplevel_menu_info)
{
    ...
    if (param_in->alpha_id.len > 0)
    {
        ...
        toplevel_menu_info->menu_title_len = menu_title.wstr_len;
        		title_len_mainmenu=menu_title.wstr_len;
...
LOCAL void CreateDynaMenuItemData(MMI_CTRL_ID_T ctrl_id, uint16 index, STK_MENU_INFO_T *menu_info_ptr)
{
    ...
    {
        item_data.softkey_id[0] = (MMI_TEXT_ID_T)STXT_SOFTKEY_MENU_MK;
        item_data.softkey_id[1] = (MMI_TEXT_ID_T)TXT_NULL;
        item_data.softkey_id[2] = (MMI_TEXT_ID_T)TXT_COMMON_COMMON_BACK;
    }
    else
    {
        item_data.softkey_id[0] = (MMI_TEXT_ID_T)TXT_NULL;
        item_data.softkey_id[1] = (MMI_TEXT_ID_T)STXT_SOFTKEY_OK_MK;
        item_data.softkey_id[2] = (MMI_TEXT_ID_T)TXT_COMMON_COMMON_BACK;
    }
...
#ifndef MMI_PDA_SUPPORT
//        CreatSoftkeyCtrl(MMISTK_MAINMENU_WIN_ID, TXT_COMMON_OK, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK);
		//for bug498998
          CreatSoftkeyCtrl(MMISTK_MAINMENU_WIN_ID, TXT_NULL, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK);
#endif
...
#endif
    {
        ...
        //MMIAPICOM_StatusAreaSetViewHeaderinfo(menu_name.wstr_ptr, menu_name.wstr_len);
#ifdef GCF_TA_SUPPORT
	  if (CFW_SSCheckTestSim(dual_sys))
         {
		CreateSTKBufferMemory(&title_content_mainmenu,MMISTK_STRING_MAX_LEN*4);
 	        SCI_MEMSET(title_content_mainmenu, 0x00, MMISTK_STRING_MAX_LEN*4);
                MMIAPICOM_Wstrcpy(title_content_mainmenu,menu_name.wstr_ptr);
         }
#endif
    }
...
// no use
    ...
    if (!MMK_IsOpenWin(MMISTK_LAUNCH_BROWSER_WIN_ID)) 
    {
        ...
        CreatSoftkeyCtrl(MMISTK_LAUNCH_BROWSER_WIN_ID, TXT_NULL, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK);
...
// no use
    ...
    //SetStkWinStatusBar(launch_browser_ind->dual_sys, MMISTK_LAUNCH_BROWSER_WIN_ID);
...
LOCAL void StartLaunchBrowserOperation(MN_DUAL_SYS_E dual_sys,SIMAT_LAUNCH_BROWSER_T* launch_para_in)
{
    ...
    if(0 < launch_para_in->url.url_len)
    {
        ...
        default:
            //param_out.result = SIMAT_TR_ME_UNABLE;
            param_out.result = SIMAT_TR_SUCCESS; //2019-5-23
            param_out.mp_info = SIMAT_LB_NO_SPECIFIC_CAUSE;
...
LOCAL void MMISTK_CnfOpenChannel(BOOLEAN is_success, SIMAT_GENERAL_PC_RESULT_E result)
{
    ...
    param_out.is_bear_desc = s_mmistk_is_bear_desc;

    if(s_mmistk_is_bear_desc)
    {
...
// no use
    ...
    if (!MMK_IsOpenWin(MMISTK_OPEN_CHANNEL_WIN_ID)) 
    {
        ...
        CreatSoftkeyCtrl(MMISTK_OPEN_CHANNEL_WIN_ID, TXT_NULL, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK);
        ...
        //set title
        GUIWIN_CreateTitleDyna(MMISTK_OPEN_CHANNEL_WIN_ID, TXT_NULL);
        //SetStkWinStatusBar(open_channel_ind->dual_sys, MMISTK_OPEN_CHANNEL_WIN_ID);
#if 0
// no use
        result = TRUE;
#endif
...
PUBLIC void MMISTK_OnOpenChannelCmd(void* arg)
{
    ...
    if (!MMIAPIPHONE_IsSimAvailable(arg_in_ptr->dual_sys))
    {
        if (param_in->bear_desc.bear_len > 0)
        {
    ...
    {
    	SCI_TRACE_LOW("Mmistk: Open channel s_mmistk_is_bear_desc %d",s_mmistk_is_bear_desc);

        if(s_mmistk_is_bear_desc == FALSE)
        {
        	MMISTK_CnfOpenChannel(FALSE, 0x03);
		}
		else
		{
			MMISTK_CnfOpenChannel(FALSE, SIMAT_TR_BEYOND_ME_CAP);
...
PUBLIC void MMISTK_OnCloseChannelCmd(void* arg)
{
	...
	SCI_TRACE_LOW("MNSTK: closechannel id:%d", param_out.channel_id);
...
LOCAL void HandleSTkRefreshDelayTimer(void)
{
    if (0 != s_mmistk_refresh_delay_timer_id)
    {
        MMK_StopTimer(s_mmistk_refresh_delay_timer_id);
        s_mmistk_refresh_delay_timer_id = 0;
        MMISTK_OnRefreshCmd(&g_refresh_back);
    }
}
...
PUBLIC void MMISTK_OnSendDataCmd(void* arg)
{
    ...
    MMISTK_SocketSendData(s_mmistk_sock_id, (char *)param_in->channel_data.data, param_in->channel_data.data_len);
    ...
    {
        param_out.channel_data_len = 0xFF; //socket buffer - param_in->channel_data.data_len > 200时填0xFF
    }
    else
    {
...
PUBLIC void MMISTK_OnReceiveDataCmd(void* arg)
{
    ...
    {
    	SCI_TRACE_LOW("Mmistk:receive data result: 0x%x", SIMAT_TR_BEYOND_ME_CAP);
...
/*****************************************************************************/
// 	Description : MMISTK_SetStkRefresh
//	Global resource dependence : 
//   Author: wancan.you
//	Note:
/*****************************************************************************/
PUBLIC void MMISTK_SetStkRefresh(BOOLEAN refresh)
{
    s_is_stk_refresh = refresh;
}
...
PUBLIC void MMISTK_HandleSTKTimer(uint8 timer_id, uint32 param )
{
    ...
    else if (timer_id == s_mmistk_refresh_delay_timer_id)
    {
        HandleSTkRefreshDelayTimer();
    }
}
...
LOCAL void HandleSTKSendDtmfStr(MN_DUAL_SYS_E dual_sys)
{
    ...
    if (PNULL != s_mmistk_dtmf_string)
    {
        ...
        {
            //SCI_TRACE_LOW:"[MMI_STK]HandleSTKSendDtmfStr, the dtmf length is %d, the dtmf value is %d"
            ...
            else
            {
                ...
                {
                    s_mmistk_dtmf_timer_id = MMK_CreateTimerCallback(MMISTK_DTMF_PAUSE_DURATION,  MMISTK_HandleSTKTimer, NULL, FALSE);
                }
                else
                {
                ...
                //s_mmistk_dtmf_timer_id = MMK_CreateTimerCallback(s_test_dtmf_setTime,  MMISTK_HandleSTKTimer, NULL, FALSE);
...
//    anim_init_data.both_rect.v_rect.right = anim_init_data.both_rect.v_rect.left + icon_bitmap->img_width; //此处应该使用icon的宽度
...
//    anim_init_data.both_rect.h_rect.right = anim_init_data.both_rect.h_rect.left + icon_bitmap->img_width; //此处应该使用icon的宽度
	    ...
	    if (CFW_SSCheckTestSim(dual_sys))
	    {
			if (PNULL != s_mmistk_cur_menu_info_ptr[dual_sys])
			{
				GUIWIN_SetTitleText(win_id, 
				                ...
				                s_mmistk_cur_menu_info_ptr[dual_sys]->menu_title_len,
				                FALSE);
				MMIAPICOM_StatusAreaSetViewHeaderinfo(s_mmistk_cur_menu_info_ptr[dual_sys]->menu_title, s_mmistk_cur_menu_info_ptr[dual_sys]->menu_title_len);
		                CreateSTKBufferMemory(&title_content_menu,MMISTK_STRING_MAX_LEN*4);
				SCI_MEMSET(title_content_menu,0x00, MMISTK_STRING_MAX_LEN*4)
                                title_len_menu=s_mmistk_cur_menu_info_ptr[dual_sys]->menu_title_len;
                                MMIAPICOM_Wstrcpy(title_content_menu,s_mmistk_cur_menu_info_ptr[dual_sys]->menu_title);
 	}
	     }
#endif
		if(s_mmistk_selectitem == 0)
		{
...
PUBLIC void MMITEST_DTMFSetTime(uint32 iSetNum)
{
    s_test_dtmf_setTime = iSetNum;
}   
