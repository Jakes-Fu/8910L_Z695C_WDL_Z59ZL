...
void MMIAPISTK_EnterToplevelMenu(MN_DUAL_SYS_E dual_sys)
{
        ...
        || SIM_STATUS_PUK_BLOCKED == MMIAPIPHONE_GetSimStatus(dual_sys))
    {
		...
		SCI_TRACE_LOW("mmistk_app: stkwin  entertoplevel menu pinorpuk blocked");
    ...
    {
    	SCI_TRACE_LOW("mmistk_app: stkwin  sms is notorderok");
...
/*****************************************************************************/
// 	Description : MMIAPISTK_SetStkRefresh
//	Global resource dependence : 
//   Author: wancna.you
//	Note:
/*****************************************************************************/
PUBLIC void MMIAPISTK_SetStkRefresh(BOOLEAN refresh)
{
    MMISTK_SetStkRefresh(refresh);
}
...
{
	SCI_TRACE_LOW("Mmistk_app: API_SetupCall is_oper_cnf %d, is_call_active %d, disc_cause %d", is_oper_cnf, is_call_active, disc_cause);
