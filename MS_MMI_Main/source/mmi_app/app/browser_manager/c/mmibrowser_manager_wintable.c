...
LOCAL MMI_RESULT_E HandleBgWinMsg(
                                   MMI_WIN_ID_T win_id, 
                                   MMI_MESSAGE_ID_E msg_id,
                                   DPARAM param
                                   );
...
WINDOW_TABLE(MMISET_BG_WIN_TAB) =
{
    WIN_FUNC((uint32)HandleBgWinMsg),
    WIN_TITLE(TXT_NULL),
    WIN_ID(MMI_BG_WIN_ID),
    END_WIN
};
...
LOCAL void AppendSetBrowserListItem(MMI_CTRL_ID_T ctrl_id)
{
    ...
    // MMIAPISET_AppendListItemByTextIdExt(TXT_COMMON_ASK_EVERYTIME, STXT_SOFTKEY_OK_MK, TXT_NULL, STXT_RETURN, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO );
    
    for (i = 0; i < MMIBROWSER_TYPE_MAX; i++)
    {
        MMI_GetLabelTextByLang(s_browser_type_text_id[i], &str);
        MMIAPISET_AppendListItemByTextBuffer(str.wstr_ptr, str.wstr_len, i, STXT_SOFTKEY_OK_MK, TXT_NULL, STXT_RETURN, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
    }
    
    default_browser_type = MMIBM_GetDefaultBrowser();
    GUILIST_SetSelectedItem(ctrl_id, default_browser_type, TRUE);//set selected item
    GUILIST_SetCurItemIndex(ctrl_id,default_browser_type);//set current item
}
...
PUBLIC void MMIBM_OpenSimSelWin(uint8* phone_number_ptr)
{
    MMK_CreateWin((uint32*)MMISET_BG_WIN_TAB, phone_number_ptr);
}

/*****************************************************************************/
//     Description :create bg win
//    Global resource dependence : 
//  Author:
//    Note: 
/*****************************************************************************/
                                   ...
                                   )
{
    ...
    case MSG_OPEN_WINDOW:
		{
			uint8*phone_number_ptr = (uint8*)MMK_GetWinAddDataPtr(win_id);
...
PUBLIC void MMIAPIBRW_HandleNewSMSInd(APP_MN_SMS_IND_T *param)
{
    ...
    MMI_WIN_ID_T           sms_win_id= MMIBM_POPSMS_WIN_ID;
    ...
    if(!MMIAPIBROWSER_IsActive() && !MMK_IsFocusWin(sms_win_id))
    {
        return;
    }
    ...
    {
        MMK_CloseWin(sms_win_id);
    }
    
    //init display string;
    prompt_str.wstr_len = 0;
    prompt_str.wstr_ptr = SCI_ALLOC_APPZ(160*sizeof(wchar));     //#define     MMISMS_DEFAULT_CHAR_LEN     153   // the length of default split SMS
    if (prompt_str.wstr_ptr == PNULL)
    {
        return;
    }
    ...
    MMISMS_ConvertValidDataToDispStr(prompt_str.wstr_ptr,
                                     (uint16*)&(prompt_str.wstr_len),
                                     sms_user_data_t.user_valid_data_t.user_valid_data_arr,
                                     MIN(sms_user_data_t.user_valid_data_t.length,160),
                                     alpha,
                                     language,
                                     is_lock_shift,
                                     is_single_shift
    ...
    //show msg content
    MMIPUB_OpenAlertWinByTextPtr(PNULL,&prompt_str,PNULL,IMAGE_PUBWIN_WARNING, &sms_win_id,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,PNULL);
