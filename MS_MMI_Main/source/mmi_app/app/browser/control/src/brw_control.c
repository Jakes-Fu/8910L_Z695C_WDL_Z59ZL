...
PUBLIC BOOLEAN  BRW_SetPublicParam(BRW_PUBLIC_PROFILE_T* public_profile,BRW_PROFILE_TYPE_E profile_type)
{
    ...
    public_profile_ptr->dual_sys = public_profile->dual_sys;
    ...
    if(temp_profile_type & BRW_COOKIE_PROFILE)
    {
		...
		BRWDebug_Printf("[cookie]BRW_SetPublicParam,is_cookie_enable=%d",public_profile->is_cookie_enable);
    }
...
#if defined(WIFI_SUPPORT) || defined(MMIDORADO_GOOGLE_TRANSCODE_SUPPORT) || defined(MMIDORADO_WRE_TRANSCODE_SUPPORT) 
    BRW_PUBLIC_PROFILE_T    *public_profile_ptr = BrwGetPublicProfile();
#endif
...
/* BEGIN: Modified by George.Liu 01526, 2010/8/5   PN:MergeToMw1029forCustom */
    if (MNSS_CheckTestSim(MN_DUAL_SYS_1) &&
           (MN_GMMREG_RAT_4G ==  MNPHONE_GetCurrentRAT(MN_DUAL_SYS_1)))
    {
        SetNetLinked(TRUE);
    }
...
PUBLIC BOOLEAN BrwStartNewInstance(const LAYOUT_REQ_PARAM_T* param_ptr,const uint32 control_handler)
{
    ...
    {
        BRWDebug_Printf("[BRW]W BrwStartNewInstance,param_ptr=0x%x!");
        return FALSE;
    }  
...
ParserAgain:
        ...
        else
        {
			...
			BRWDebug_Printf("[cookie]ProcPageContent,is_cookie_enable=%d",public_profile_ptr->is_cookie_enable);
    ...
    if(media_obj_ptr)
    {
            ...
            && (wap_req_ptr->layout_handle != BRW_INVALID_LAYOUT_HANDLE))
        {
                ...
                && (media_obj_ptr == wap_req_ptr->media_obj_info.wait_process_obj_list_ptr->object))
            {
                ...
                {
                    wap_req_ptr->media_obj_info.wait_process_obj_list_ptr = wap_req_ptr->media_obj_info.meida_list.next->next;
                }
                else
                {
                                 ...
                                 const uint32              media_param)
{
    ...
    SCI_TRACE_LOW("[BRW]BrwAddNewMediaReq:media_obj_ptr = 0x%x, media_obj_ptr->url_ptr =0x%x",media_obj_ptr, media_obj_ptr->url_ptr );
...
PUBLIC BOOLEAN ActiveNetwork(BRW_INSTANCE instance)
{
    ...
    // NOTE: check sim everytime if project support set prefer sim during visit website, remove by Bug 1363417
    /*if(Brw_ShouldResetSIM())
        {
            BrwResetSIM();
            public_profile_ptr = BrwGetPublicProfile();
        }*/
...
#endif
    {
        ...
        else
        {
            ...
            SCI_TraceLow("ActiveNetwork ip_type = %d, auth_type = %d", public_profile_ptr->ip_type, public_profile_ptr->auth_type);
#endif
            result =  MMIAPIPDP_Active(&active_info);
        }
    }
