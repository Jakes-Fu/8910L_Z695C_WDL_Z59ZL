...
//#define DAPS_HTTP_MEM_DEBUG  // for JS_cookie memory debug
...
{    
    ...
    {       
    	...
    	if( MMIBROWSER_TYPE_OPERA_MINI== MMIBM_GetDefaultBrowser())
	{
		block_buff_size = MMIDORADO_MEMORY_SIZE_OPERA_IN_USE;  //return memory size.
	}
	else
#endif  
	{
    ...
    if( MMIBROWSER_TYPE_OPERA_MINI== MMIBM_GetDefaultBrowser())
	{
		g_browser_mem_handle = CFL_CreateMspaceEx(MMIDORADO_MEMORY_GRANULARITY, 
                                                  MMIDORADO_MEMORY_SIZE_OPERA_IN_USE/MMIDORADO_MEMORY_GRANULARITY);
	}
	else
#endif
	{ 
 ...
 *  return:void
 */
PUBLIC void   BRWCookieMem_FreeEx(void* mem_ptr)
{
    if (mem_ptr != PNULL)
    {
#ifndef DAPS_HTTP_MEM_DEBUG    //if HTTP_MEM_DEBUG is defined, DAPS_HTTP_MEM_DEBUG must be defined; if HTTP_MEM_DEBUG is not defined, DAPS_HTTP_MEM_DEBUG must not be defined;
    SCI_FREE(mem_ptr);
#else   
    CFLMEM_Free(mem_ptr);
#endif
