...
{
#if (defined IM_CHINESE_T_SUPPORT) ||(defined IM_TRAD_CHINESE_SUPPORT) || defined(MULTI_VARIANT_SUPPORT)
...
{
#if ((defined IM_RUSSIAN_SUPPORT) && (!defined WIN32)) || defined(MULTI_VARIANT_SUPPORT)
...
{
#if ((defined IM_THAI_SUPPORT) && (!defined WIN32)) || defined(MULTI_VARIANT_SUPPORT)
...
PUBLIC uint16 BrwFontGetStringWidth(uint16 *unicode_ptr,uint32 len, FONT_TYPE_STATE_T font_type_state)
{
    ...
    if(len >0xF0000000)         //check input len is -1 or not
    {
        len = BRW_WStrlen(unicode_ptr);
    }
    if(len==0||len >0xF0000000) 
    {
        return 0;
    }
                                     ...
                                     )
{
    ...
    if(len >0xF0000000)         //check input len <0 (-1) or not
    {
        len =BRW_WStrlen(unicode_ptr);
    }
    if(len== 0 ||len >0xF0000000) 
...
LOCAL void SetupWmlListControl(MMI_CTRL_ID_T ctrl_id, void *select_ptr)
{
    ...
    for(index=0; index < item_num; index++) {
        ...
        if(WmlSelectIsMultiple(select_ptr)) {
            ...
            item_data.softkey_id[0] = (MMI_TEXT_ID_T)STXT_SOFTKEY_SELECT_MK;
            item_data.softkey_id[1] = (MMI_TEXT_ID_T)TXT_NULL;//STXT_SOFTKEY_OK_MK;
            item_data.softkey_id[2] = (MMI_TEXT_ID_T)TXT_COMMON_COMMON_BACK;
        } else {
            item_t.item_style    = GUIITEM_STYLE_ONE_LINE_RADIO;
            item_data.softkey_id[0] = (MMI_TEXT_ID_T)STXT_SOFTKEY_OK_MK;
...
PUBLIC void WmlLayoutCreateSelectWindow(WML_LAYOUT_SELECT_T *select_ptr, LAYOUT_CTRL_T  *layout_ctrl_ptr)
{
    ...
    WINDOW_TABLE(SELECT_WIN_TABLE) = {
            ...
            WIN_TITLE(STXT_SOFTKEY_SELECT_MK),
#ifdef MMI_PDA_SUPPORT
    //WIN_STYLE(WS_HAS_BUTTON_SOFTKEY),
