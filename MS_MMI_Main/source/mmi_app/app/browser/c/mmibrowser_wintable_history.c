...
//	Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E BrwHandleHistoryDeleteOneQueryWinMsg(
                                                        MMI_WIN_ID_T win_id, 
                                                        MMI_MESSAGE_ID_E msg_id, 
                                                        DPARAM param
                );
...
//			GUIWIN_SetSoftkeyTextId(win_id, TXT_NULL, TXT_NULL, text3_id, FALSE);
    ...
    case MSG_NOTIFY_TITLE_MENU_ICON:
        if (GUILIST_GetTotalItemNum(list_ctrl_id) > 0)
        {
            MMIBROWSER_OpenQueryWinByTextId(STR_BROWSER_DELETE_ALL,IMAGE_PUBWIN_QUERY,PNULL,BrwHandleHistoryDeleteAllQueryWinMsg);
        }
        break;
#endif
        ...
        case ID_POPUP_MENU_HISTORY_DELETE:
            {
                ...
                MMI_GetLabelTextByLang(TXT_DELETE_QUERY, &dst_temp1);
                dst_temp2.wstr_len = visit_info_ptr->name_len;
                dst_temp2.wstr_ptr = visit_info_ptr->name_ptr;
                MMIBROWSER_OpenQueryWinByTextPtr(&dst_temp1,&dst_temp2,IMAGE_PUBWIN_QUERY,PNULL,BrwHandleHistoryDeleteOneQueryWinMsg);
                MMK_CloseWin(win_id);
            }
            break;
...
#endif //TOUCH_PANEL_SUPPORT //IGNORE9527
    case MSG_CTL_MIDSK:
        BrwDelOneHistory(MMIBROWSER_HISTORY_LIST_CTRL_ID);
#ifdef MMI_GUI_STYLE_TYPICAL
        MMIPUB_OpenAlertSuccessWin(TXT_DELETED);
#endif /* MMI_GUI_STYLE_TYPICAL */
        MMK_CloseWin(win_id);
...
#else
                MMI_GetLabelTextByLang(STXT_EXIT, &name_str);
                MMI_GetLabelTextByLang(STXT_OPTION, &url_str);
#endif
#ifdef MMI_PDA_SUPPORT
                MMIBROWSER_AppendTwoLineTextListItem(list_ctrl_id,STXT_OPTION,name_str,url_str,IMAGE_BROWSER_PAGE,i,FALSE);
#else
                MMIBROWSER_AppendTwoLineTextListItem(list_ctrl_id,STXT_SOFTKEY_MENU_MK,name_str,url_str,IMAGE_BROWSER_HISTORY_LIST_ICON,i,FALSE);
#endif /* MMI_PDA_SUPPORT */                
            }                   
        }
