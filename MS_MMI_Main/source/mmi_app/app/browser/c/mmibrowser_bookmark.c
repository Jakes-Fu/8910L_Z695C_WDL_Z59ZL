...
#ifdef MULIT_VARIANT_SUPPORT
LOCAL uint16	s_ndt_count = 0;
LOCAL void AdjustBookmarkArray(uint16 new_ndt_count);
LOCAL void ReadBookMarkFromNDT(void);
#endif
...
LOCAL void SetDefaultBookmark(void)
{
    ...
    //#ifdef IM_SIMP_CHINESE_SUPPORT//just for chinese for temp 
        ...
        if (is_res_data_exist && PNULL != res_info.res_item_ptr)
        {
            res_info.res_num = MIN(res_info.res_num, MMIBROWSER_BOOKMARK_NDT_MAXNUM);
            ...
            s_bookmark_info_ptr->ndt_item_count = i;
...
#ifdef MULIT_VARIANT_SUPPORT
    if (!MMIBROWSER_CanAddNewBookmark((uint16)s_bookmark_info_ptr->item_count))
    {
    	return FALSE;
    }
#endif	
...
/*****************************************************************************/
//  Description : move cutomize bookmark to the offset new_ndt_count
//  Global resource dependence : 
//  Author: sam.Hua
/*****************************************************************************/
LOCAL void AdjustBookmarkArray(uint16 new_ndt_count)
{
	uint16 i=0;
	uint16 custom_count = 0;

	if (s_bookmark_info_ptr == PNULL)
		return;
	
	custom_count = s_bookmark_info_ptr->item_count-s_bookmark_info_ptr->ndt_item_count;

	SCI_TRACE_LOW("ReadBookMarkFromNDT AdjustBookmarkArray  new_ndt_count=%d",new_ndt_count);
	//calc new item count 
	s_bookmark_info_ptr->item_count = custom_count+new_ndt_count;
	if (s_bookmark_info_ptr->item_count >MMIBRW_BOOKMARK_MAX_ITEM)
	{
		s_bookmark_info_ptr->item_count = MMIBRW_BOOKMARK_MAX_ITEM;
		custom_count = MMIBRW_BOOKMARK_MAX_ITEM - new_ndt_count;
	}
	if (custom_count ==0)	//if no custom bookmark,return;
		return;
	//Copy custom bookmark to the new position,
	if (new_ndt_count>s_bookmark_info_ptr->ndt_item_count)  
	{
		//Increase,copy with backward sequence
		//the new last position should be item_info[new_ndt_count+custom_count-1]
		//The copied should from s_bookmark_info_ptr->ndt_item_count+custom_count-1;
		for (i=0;i<custom_count;i++)
		{
			SCI_MEMCPY(&s_bookmark_info_ptr->item_info[new_ndt_count+custom_count-1-i],
				&s_bookmark_info_ptr->item_info[s_bookmark_info_ptr->ndt_item_count+custom_count-1-i],
				sizeof(MMIBRW_BOOKMARK_ITEM_T));
		}
	}
	else		
	{
		//Reduce, copy with forward sequence
		//the new first position should be item_info[new_ndt_count].
		//The copied should from s_bookmark_info_ptr->ndt_item_count;
		for (i = 0;i<custom_count;i++)
		{
			SCI_MEMCPY(&s_bookmark_info_ptr->item_info[i+new_ndt_count],
				&s_bookmark_info_ptr->item_info[i+s_bookmark_info_ptr->ndt_item_count],
				sizeof(MMIBRW_BOOKMARK_ITEM_T));
		}

	}
	//set new ndt_count
	s_bookmark_info_ptr->ndt_item_count = new_ndt_count;
	BookmarkOverwriteInfo();
...
/*****************************************************************************/
//  Description : Read ndt bookmark count and save them to s_bookmark_info_ptr
//  Global resource dependence : 
//  Author: sam.Hua
//  Note:  
//  if ndt_item_count == res_num, just replace
//  if ndt_item_count !=res_num //move custom,then replace ndt
/*****************************************************************************/
LOCAL void ReadBookMarkFromNDT(void)
{
	...
	{
		int32 i,url_len;
		MMIBRW_BOOKMARK_ITEM_T *cur_item=PNULL;

		//test code,
/*		if (s_bookmark_info_ptr->item_count == 13)
		{
			bookmark.res_num = 5;  //adjust bookmark resource.
		}
*/		
		if (bookmark.res_num>MMIBROWSER_BOOKMARK_NDT_MAXNUM)
			bookmark.res_num = MMIBROWSER_BOOKMARK_NDT_MAXNUM;

		if (s_bookmark_info_ptr->ndt_item_count  != bookmark.res_num)
		{
			AdjustBookmarkArray(bookmark.res_num);
		}
		s_bookmark_info_ptr->ndt_item_count  = bookmark.res_num;
		SCI_TRACE_LOW("ReadBookMarkFromNDT!  s_ndt_count=%d",s_bookmark_info_ptr->item_count);

		//replace the NDT bookmark
		for(i = 0; i< bookmark.res_num ; i++) 
		{	
			cur_item = &s_bookmark_info_ptr->item_info[i];
			cur_item->title_len = MIN(bookmark.res_item_ptr[i].name_len,
				MMIBRW_BOOKMARK_MAX_TITLE_LENGTH);
			SCI_MEMCPY(cur_item->title_arr,bookmark.res_item_ptr[i].name,
				sizeof(uint16)*cur_item->title_len);
			
			url_len = MIN(bookmark.res_item_ptr[i].url_len,MMIBRW_BOOKMARK_MAX_URL_LENGTH+1);				
			SCI_MEMCPY(cur_item->url_ptr,bookmark.res_item_ptr[i].url,
				sizeof(char)*url_len);
		}
	}
	else
	{
		SCI_TRACE_LOW("ReadBookMarkFromNDT: failed !");
		//remove old NDT bookmark
		bookmark.res_num = 0;
		AdjustBookmarkArray(bookmark.res_num);
	}
	MMIRES_UnloadBookmarkInfo(&bookmark);	
}		

/*****************************************************************************/
//  Description : Get ndt count
//  Global resource dependence : 
//  Author: sam.Hua
//  Note: 
/*****************************************************************************/
PUBLIC uint16 MMIBROWSER_GetNDTBookmarkCount(void)
{
	if(s_bookmark_info_ptr==PNULL)
	{
		return 0;
	}
	return (uint16)s_bookmark_info_ptr->ndt_item_count ;
}

//Max num of 
PUBLIC BOOLEAN MMIBROWSER_CanAddNewBookmark(uint16 cur_total_count)
{
	if (s_bookmark_info_ptr == PNULL)
	{
		return FALSE;
	}
	//MMIBRW_BOOKMARK_MAX_ITEM -MMIBROWSER_BOOKMARK_NDT_MAXNUM
	if (cur_total_count <s_bookmark_info_ptr->ndt_item_count) 
		return FALSE;
	if( (cur_total_count -s_bookmark_info_ptr->ndt_item_count)<(MMIBRW_BOOKMARK_MAX_ITEM -MMIBROWSER_BOOKMARK_NDT_MAXNUM))
	{
		return TRUE;
	}
	return FALSE;
