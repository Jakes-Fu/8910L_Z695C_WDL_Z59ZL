...
/*****************************************************************************/
//  Description : handle query clear msg
//  Global resource dependence : 
//  Author: Jiaoyouw.wu
//  Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E BrwHandleQueryClearWinMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param);
...
WINDOW_TABLE( MMIBROWSER_SETTING_HOMEPAGE_TYPE_OPT_WIN_TAB ) = 
{
    ...
    WIN_TITLE(STR_BROWSER_HOMEPAGE_TYPE),
...
{
    WIN_TITLE(STR_BROWSER_CUSTOMIZED),
                                   ...
                                   )
{
    ...
    SCI_TRACE_LOW("[BRW] BrwHandleSettingWinMsg msg_id=0x%x", msg_id);
    switch(msg_id) 
    {
...
LOCAL void BrwSetSettingWindow(MMI_WIN_ID_T win_id)
{
    ...
    GUILABEL_SetTextById(MMIBROWSER_SETTING_HOME_TYPE_LABEL_CTRL_ID,STR_BROWSER_HOMEPAGE_TYPE,FALSE);
    GUILABEL_SetTextById(MMIBROWSER_SETTING_HOME_LABEL_CTRL_ID,STR_BROWSER_CUSTOMIZED,FALSE);
    GUILABEL_SetTextById(MMIBROWSER_SETTING_IMAGE_LABEL_CTRL_ID,STR_BROWSER_DOWNLOAD_PICTURE,FALSE);
#ifdef MMIDORADO_GOOGLE_TRANSCODE_SUPPORT
...
#endif
    GUILABEL_SetTextById(MMIBROWSER_SETTING_CLEAR_LABEL_CTRL_ID,STR_BROWSER_CLEAR,FALSE);

    //image download and google change
    MMI_GetLabelTextByLang(STXT_SOFTKEY_OPEN_MK, &str_item[0]);
...
LOCAL void BrwUpdateSimNetAccountLabelText(MMI_WIN_ID_T win_id)
{
    ...
    {      
        ...
        select_index = MMIDORADO_GetNetSettingIndex(i);
...
PUBLIC void BrwResetSIM(void)
{
    ...
    SCI_TraceLow("BrwResetSIM active_sim =%d",active_sim);

    Brw_ResetLinkSettings(MMIBROWSER_GetInstance());
    MMIBROWSER_SetSim(active_sim);
    public_profile = MMIBROWSER_SetPublicProfile();
    public_profile.dual_sys = active_sim;
    BRW_SetPublicParam(&public_profile, 
        BRW_CACHE_PROFILE|BRW_COOKIE_PROFILE|BRW_IMG_DL_PROFILE|BRW_BGSOUND_DL_PROFILE|BRW_GWT_PROFILE);
...
PUBLIC BOOLEAN Brw_ShouldResetSIM(void)
{
    MN_DUAL_SYS_E   current_sim = MMIBROWSER_GetSim();
    MN_DUAL_SYS_E   active_sim=MN_DUAL_SYS_1;
    BOOLEAN ret= FALSE;
    if(MMIAPIPHONE_GetDataServiceSIM(&active_sim))
        {
            if(active_sim != current_sim)
                {
                    ret = TRUE;
                }
    }
    SCI_TraceLow("Brw_ShouldResetSIM ret =%d",ret);
    return ret;
}
...
LOCAL BOOLEAN BrwExecuteSetting(BOOLEAN is_update_font)
{
    ...
    if (BrwIsSimChange())
    {
        ...
        Brw_ResetLinkSettings(MMIBROWSER_GetInstance());
        // Bug 1370968 if prefered sim not exist, switch to another sim.
        if (!MMIAPIPHONE_GetSimExistedStatusEx(dual_sys))
        {
            MMIAPIPHONE_GetDataServiceSIM(&dual_sys);
        }
...
LOCAL void BrwSetNetIndexSettingCallback(MN_DUAL_SYS_E dual_sys, uint32 index)
{
    if (dual_sys < MMI_DUAL_SYS_MAX)
    {
        ...
        SCI_TRACE_LOW("[BRW] BrwSetNetIndexSettingCallback");

        if ((dual_sys == sim_type) &&(index != MMIDORADO_GetNetSettingIndex(dual_sys)))
        {
            MMIBROWSER_SetSettingChange(TRUE);
        }
...
{
    MMI_RESULT_E recode = MMI_RESULT_TRUE;
    SCI_TRACE_LOW("[BRW] BrwHandleSettingListWinMsg msg_id=0x%x, s_is_has_reset=%d", msg_id, s_is_has_reset);
    switch (msg_id)
    {
    case MSG_OPEN_WINDOW:
        MMIBROWSER_SetSettingChange(FALSE);
...
LOCAL void BrwCreateHomePageTypePopMenu(MMI_WIN_ID_T win_id, MMI_CTRL_ID_T ctrl_id)
{
    MMI_TEXT_ID_T       text_index[] = {STR_BROWSER_TYPE_DEFAULT, STR_BROWSER_TYPE_CUSTOMIZED};
...
LOCAL MMI_RESULT_E BrwHandleSettingClearOptWinMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
    ...
    case MSG_OPEN_WINDOW:
        {    
            ...
            GUILIST_ChangeCheckListDefSoftkeyID(win_ctrl_id, STXT_SOFTKEY_UNMARK_MK, STXT_SOFTKEY_MARK_MK, TXT_NULL);            

            item_data.softkey_id[0] = TXT_NULL;
            item_data.softkey_id[1] = STXT_SOFTKEY_MARK_MK;
            ...
            if (GUILIST_GetSelectedItemNum(win_ctrl_id) > 0)
            {
                for (index = 0; index < node_num; index++)
                {
                    GUILIST_SetItemSoftkey(win_ctrl_id, index, TXT_DELETE, STXT_SOFTKEY_MARK_MK, TXT_COMMON_COMMON_BACK);
                }
            }
    ...
    case MSG_CTL_MIDSK:
        ...
        {
            MMI_CTRL_ID_T       ctrl_id = 0;
            BOOLEAN has_item_selected = FALSE;
            uint16 index = 0;
            uint16 node_num = 0;
            ...
            if (PNULL != param)
            {
                ...
                else if (ctrl_id == win_ctrl_id)
                {
                    ...
                    node_num = GUILIST_GetTotalItemNum(win_ctrl_id);
                    for (index = 0; index < node_num; index++)
                    {
                        GUILIST_SetItemSoftkey(win_ctrl_id, index, has_item_selected ? TXT_DELETE: TXT_NULL,
                                STXT_SOFTKEY_MARK_MK, TXT_COMMON_COMMON_BACK);
                    }
    ...
    case MSG_APP_OK:
        {
            ...
            if (checked_count > 0)
            {
                MMIBROWSER_OpenQueryWinByTextId(TXT_DELETE_QUERY, IMAGE_PUBWIN_QUERY, PNULL, BrwHandleQueryClearWinMsg);
                break;
            }
            /*else//NEWMS00200883
            {
...
//  Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E BrwHandleQueryClearWinMsg(
                    MMI_WIN_ID_T win_id, 
                    MMI_MESSAGE_ID_E msg_id, 
                    DPARAM param
                    )
{
    MMI_RESULT_E        result = MMI_RESULT_TRUE;
    SCI_TRACE_LOW("BrwHandleQueryClearWin msg_id=%d", msg_id);
    ...
    case MSG_APP_OK:
        {
            ...
            checked_count = GUILIST_GetSelectedItemIndex(MMIBROWSER_SETTING_CLEAR_OPT_CTRL_ID, check_info_ptr, MMIBROWSER_CLEAR_TYPE_MAX);
            ...
            if (checked_count > 0)
            {
                ...
                {
                    GUILIST_GetItemData(MMIBROWSER_SETTING_CLEAR_OPT_CTRL_ID, check_info_ptr[i], &select_clear_type);
#ifndef MMI_BROWSER_MINI_SUPPORT
                ...
                MMIBROWSER_CloseQuerytWin(PNULL);
                MMK_CloseWin(MMIBROWSER_SETTING_CLEAR_OPT_WIN_ID);
...
LOCAL void BrwUpdateSettingList(MMI_CTRL_ID_T ctrl_id)
{
...
#endif
    MMI_TEXT_ID_T       lang_text_id[] = {STR_BROWSER_TYPE_DEFAULT, STR_BROWSER_TYPE_CUSTOMIZED};
    ...
    for (sim_index = MN_DUAL_SYS_1; sim_index < MMI_DUAL_SYS_MAX; sim_index++)
    {
        ...
        sim_available = MMIAPIPHONE_IsSimAvailable(sim_index);
        ...
        if (sim_available)
        {
            ...
            select_index = MMIDORADO_GetNetSettingIndex(sim_index);
        ...
        if (!sim_available)
        {
            CTRLLIST_SetItemGray(ctrl_id, list_index, TRUE);
            CTRLLIST_SetItemInvalid(ctrl_id, list_index, TRUE);
        }
    ...
    MMIRES_GetText(STR_BROWSER_HOMEPAGE_TYPE, PNULL, &string_1);
    ...
    MMIRES_GetText(STR_BROWSER_CUSTOMIZED, PNULL, &string_1);
    MMIAPICOM_StrToWstr(MMIBROWSER_GetHomepageSetting(), temp_arr);
    ...
    /* Image */
    MMIBROWSER_AppendSplitListItem(ctrl_id, STR_BROWSER_SETTINGS);
...
#else
    MMIRES_GetText(STR_BROWSER_DOWNLOAD_PICTURE, PNULL, &string_1);
#endif
    ...
    MMIRES_GetText(STR_BROWSER_CLEAR, PNULL, &string_1);
...
LOCAL void BrwHandleSetttingHomageType(void)
{
    MMK_CreateWin((uint32 *)MMIBROWSER_SETTING_HOMEPAGE_TYPE_OPT_WIN_TAB, PNULL);
}
...
LOCAL void BrwHandleSetttingHomageUrl(void)
{
    MMK_CreateWin((uint32 *)MMIBROWSER_SETTING_HOMEPAGE_WIN_TAB, PNULL);
}
...
LOCAL void BrwHandleSetttingFont(void)
{
    MMK_CreateWin((uint32 *)MMIBROWSER_SETTING_FONT_OPT_WIN_TAB, PNULL);
}
...
LOCAL void BrwHandleSetttingClear(void)
{
    MMK_CreateWin((uint32 *)MMIBROWSER_SETTING_CLEAR_OPT_WIN_TAB, PNULL);
}
...
LOCAL void BrwHandleSetttingReset(void)
{
    MMIBROWSER_OpenQueryWinByTextId(TXT_BROWSER_SETTING_RE_DEFAULT,IMAGE_PUBWIN_QUERY,PNULL,BrwHandleSetttingResetAlertWinMsg);
}
...
//    MMIBROWSER_CERT_MODE_E mode = (uint32)MMK_GetWinAddDataPtr(win_id);/*lint !e64*/
    ...
    {
    case MSG_OPEN_WINDOW:
        MMIBRW_AppendCertListItem( win_id, MMIBRW_LISTBOX_CERTIFICATE_CTRL_ID, cer_num, STXT_SOFTKEY_OK_MK/*TXT_WWW_NO_HISTORY*/, 0, STXT_RETURN);
