...
LOCAL uint8 s_exit_timer_id=0;

MMI_MULTI_VAR_URL_T  g_default_homepage;	//default homepage
/**--------------------------------------------------------------------------*
**                         TYPE AND CONSTANT                                *
**--------------------------------------------------------------------------*/
...
PUBLIC void MMIBROWSER_Exit(void)
{
    ...
    if (MMIBROWSER_GetKernelInit())
    {
        ...
        Brw_HttpExit();
        // add timer with BRW_VisitHisExit for performance issue
        SCI_TRACE_LOW("[BRW] MMK_CreateTimerCallback 1000 BRW_VisitHisExit");
        s_exit_timer_id = MMK_CreateTimerCallback(1000, BRW_VisitHisExit, PNULL, FALSE);
        //BRW_VisitHisExit();
        BRW_SnapshotExit();
#ifdef MMIDORADO_INPUT_HISTORY_SUPPORT
        BRW_InputHisExit();
...
{   
    ...
    {
        // Bug 1370968 behavior need same as T117, set major sim as browser sim even if sim disactived
        ...
        if (major_sim_type < MMISET_MAJOR_SIM_SEL_MANUAL_SEL && MMIAPIPHONE_GetSimExistedStatusEx(major_sim_type))
        {
            MMIBROWSER_SetSim(MN_DUAL_SYS_1+(major_sim_type-MMISET_MAJOR_SIM_SEL_SIM1));
        }
        else
        {
...
PUBLIC uint8* MMIBROWSER_GetDefaultHomepageUrl(void)
{
    ...
    {
        SCI_TRACE_LOW("[BRW] MMIBROWSER_GetDefaultHomepageUrl MMIBROWSER_GetIsUseWifi true");
        homepage_url_ptr = MMIBROWSER_GetHomepageSetting();
    }
    else
    {
        if(MMI_MULTIVAR_ReadHomePage(&g_default_homepage) && BRW_Strlen(g_default_homepage.url) != 0)
        {
            SCI_TRACE_LOW("[BRW] MMIBROWSER_GetDefaultHomepageUrl get from g_default_homepage.url");
            homepage_url_ptr = g_default_homepage.url;
        }
        else
        {
            ...
            {
                uint32  homepage_len = strlen(setting_item_ptr->homepage);
                SCI_TRACE_LOW("[BRW] MMIBROWSER_GetDefaultHomepageUrl homepage_len=%d", homepage_len);
                if(0 == homepage_len) //bug955240
                {
                    homepage_url_ptr = MMIBROWSER_GetHomepageSetting();
    ...
    SCI_TRACE_LOW("[BRW] MMIBROWSER_GetDefaultHomepageUrl homepage_url_ptr=%x", homepage_url_ptr);
...
LOCAL void BrwSnapshotOpenFileCallback(char *url_ptr, BOOLEAN is_succ)
{
    ...
    {
        MMIPUB_OpenAlertWarningWin(STR_BROWSER_SNAPSHOT_FAIL);
    }
                ...
                )
{
    ...
    if (wurl_pptr != PNULL)
    {
            SCI_TRACE_LOW("MMIBROWSER_FreeCurPageUrlInfo  wurl_pptr");

        SCI_FREE(*wurl_pptr);
    }

    SCI_TRACE_LOW("MMIBROWSER_FreeCurPageUrlInfo");
