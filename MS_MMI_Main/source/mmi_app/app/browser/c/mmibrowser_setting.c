...
LOCAL const uint8 s_browser_custom_default_homepage[] = "http://m.baidu.com/?from=1214a";
...
PUBLIC uint8 MMIDORADO_GetNetSettingIndex(MN_DUAL_SYS_E dual_sys)
{
    ...
    default_index = MMIAPICONNECTION_GetDefaultAPN(dual_sys);
    SCI_TRACE_LOW("[BRW] MMIDORADO_GetNetSettingIndex index=%d, default_index=%d", index, default_index);
    if(default_index != index)
    {
        MMIBROWSER_SetSettingChange(TRUE);
        s_nv_setting.sim_net_index[dual_sys] = default_index;
    }
...
PUBLIC void MMIBROWSER_SetNetIndexSetting(MN_DUAL_SYS_E dual_sys, uint8 index)
{
    ...
    SCI_TRACE_LOW("[BRW] MMIBROWSER_SetNetIndexSetting dual_sys=%d, index=%d", dual_sys, index);

    MMICONNECTION_SetApnDefaultId(dual_sys, index);
}
...
PUBLIC void MMIBROWSER_ResetNvSetting(void)
{
    ...
    default_homapage_len = MIN(MMIBROWSER_HOMEPAGE_MAX_URL_LEN, (SCI_STRLEN((char*)s_browser_custom_default_homepage)));
