...
LOCAL void SnapShotHandleMethodSet(void);
/*****************************************************************************/
...
#endif /* MMI_z_SUPPORT */
/*****************************************************************************/
// Description :MMIBRW_HandleSetPreferStorageWindow
// Global resource dependence : none
// Author:Jiaoyou.wu
// Note:    
/*****************************************************************************/
LOCAL MMI_RESULT_E MMIBRW_HandleSetPreferStorageWindow( MMI_WIN_ID_T  win_id,MMI_MESSAGE_ID_E   msg_id,DPARAM param);
/**--------------------------------------------------------------------------*
 **                         WINDOW DEFINITION                              *
 **--------------------------------------------------------------------------*/
WINDOW_TABLE(BRW_SNAPSHOT_LIST_WIN_TAB) = 
{
    WIN_TITLE(STR_BROWSER_OFFLINE_PAGE),
...
WINDOW_TABLE(BRW_SNAPSHOT_SAVE_FILE_WIN_TAB) = 
{
    ...
    CREATE_FORM_CTRL(GUIFORM_LAYOUT_ORDER,MMIBROWSER_SNAPSHOT_SAVE_FILE_FORM_CTRL_ID),
       CHILD_EDIT_TEXT_CTRL(TRUE, MMIBROWSER_SNAPSHOT_FILE_NAME_LEN, MMIBROWSER_SNAPSHOT_SAVE_F_EDIT_CTRL_ID, MMIBROWSER_SNAPSHOT_SAVE_FILE_FORM_CTRL_ID),
       CHILD_LIST_CTRL(TRUE, GUILIST_TEXTLIST_E, MMIBROWSER_SNAPSHOT_SAVE_F_DROPDOWN_CTRL_ID, MMIBROWSER_SNAPSHOT_SAVE_FILE_FORM_CTRL_ID),
       END_WIN
};
#if 0 
WINDOW_TABLE(BRW_SNAPSHOT_SAVE_FILE_WIN_TAB) = 
...
#ifdef MMI_PDA_SUPPORT
        CHILD_SOFTKEY_CTRL(STXT_SOFTKEY_OK_MK, TXT_NULL, STXT_RETURN,MMICOMMON_SOFTKEY_CTRL_ID,MMIBROWSER_SNAPSHOT_SAVE_FILE_FORM_CTRL_ID),
#endif
    END_WIN
};
#endif
...
WINDOW_TABLE(BRW_SNAPSHOT_SAVE_MEDIAFILE_WIN_TAB) = 
{
    ...
    WIN_SOFTKEY(STXT_SOFTKEY_OK_MK, TXT_BROWSER_CHANGE, STXT_RETURN),
...
WINDOW_TABLE( MMIBRW_PREFER_STORAGE_WIN_TAB ) =
{
    //CLEAR_LCD,
    //WIN_PRIO( WIN_ONE_LEVEL ),
    WIN_STATUSBAR,
    WIN_FUNC( (uint32)MMIBRW_HandleSetPreferStorageWindow ),
    WIN_ID( MMIBRW_SNS_PREFER_STORAGE_WIN_ID ),
    ///WIN_TITLE(TXT_SET_SYSTEM),
    WIN_SOFTKEY(NULL, STXT_SOFTKEY_SELECT_MK, TXT_COMMON_COMMON_BACK),
    CREATE_LISTBOX_CTRL(GUILIST_RADIOLIST_E, MMIBROWSER_SNAPSHOT_PREFER_STORE_CTRL_ID),
    END_WIN
};
                                           ...
                                           )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        GUIWIN_SetTitleButtonTextId(win_id, GUITITLE_BUTTON_RIGHT, STXT_SOFTKEY_MARK_MK, FALSE);
...
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E MMIBRW_HandleSetPreferStorageWindow(
        MMI_WIN_ID_T    win_id,
        MMI_MESSAGE_ID_E   msg_id,
        DPARAM             param
        )
{
    ...
    uint16  max_items = 0;
    uint16   cur_item = 0;
    MMI_CTRL_ID_T               ctrl_id = MMIBROWSER_SNAPSHOT_PREFER_STORE_CTRL_ID;
    ...
    switch(msg_id)
    {
        case MSG_OPEN_WINDOW:
            ...
            if(!MMIAPIFMM_GetDeviceTypeStatus(MMI_DEVICE_SDCARD))
            {
                ...
                MMIBROWSER_SetSaveDeviceType(MMI_DEVICE_UDISK);
            }
            else if(MMIBROWSER_GetSaveDeviceType()==MMI_DEVICE_SDCARD)
            {
        ...
        case MSG_CTL_MIDSK:
			...
			SCI_TRACE_LOW("mymark func[%s] line[%d] cur_item %d ",__FUNCTION__,__LINE__,cur_item);
            udisk_exit = MMIAPIFMM_GetDeviceTypeStatus (MMI_DEVICE_UDISK);
            sd_exit = MMIAPIFMM_GetDeviceTypeStatus (MMI_DEVICE_SDCARD);
            if (udisk_exit)
            {
                if(cur_item == 0)
                	{
                		SCI_TRACE_LOW("mymark func[%s] line[%d]",__FUNCTION__,__LINE__);
                    	MMIBROWSER_SetSaveDeviceType(MMI_DEVICE_UDISK);
                	}
                else if(sd_exit && cur_item == 1)
                	{
                	SCI_TRACE_LOW("mymark func[%s] line[%d]",__FUNCTION__,__LINE__);
                   		 MMIBROWSER_SetSaveDeviceType(MMI_DEVICE_SDCARD);
                	}
            }
            else if (sd_exit)
            {
...
typedef void (*SNSSETTING_ITEM_FUNC)(void);
/*****************************************************************************/
...
//  Note: 
/*****************************************************************************/
LOCAL uint32 SNS_DlGetListUserdata(MMI_CTRL_ID_T ctrl_id, uint16 index)
{
    uint32 user_data = 0;
...
{
	MMI_RESULT_E result = MMI_RESULT_TRUE;
	MMI_CTRL_ID_T          current_ctrl_id = MMIBROWSER_SNAPSHOT_SAVE_FILE_FORM_CTRL_ID;
	GUI_BG_T               bg_info = {0};
    switch(msg_id) 
    {
    case MSG_OPEN_WINDOW:
		...
		//GUIFORM_SetBg(current_ctrl_id,&bg_info);
		//GUILABEL_SetFont(MMIBROWSER_SNAPSHOT_SAVE_F_LABEL1_CTRL_ID, 20, MMI_WHITE_COLOR);
		//GUILABEL_SetTextById(MMIBROWSER_SNAPSHOT_SAVE_F_LABEL1_CTRL_ID, STR_BROWSER_NAME, FALSE);
              CTRLBASEEDIT_SetLabelId(win_id,MMIBROWSER_SNAPSHOT_SAVE_F_EDIT_CTRL_ID,STR_BROWSER_NAME,FALSE);
		GUIEDIT_SetStyle(MMIBROWSER_SNAPSHOT_SAVE_F_EDIT_CTRL_ID, GUIEDIT_STYLE_MULTI_DYN_DOWN);//设置显示风格
		//GUIEDIT_PermitBorder(MMIBROWSER_SNAPSHOT_SAVE_F_EDIT_CTRL_ID, FALSE);
		...
		MMIBROWSER_SetSaveDeviceType(MMIAPIFMM_GetFirstValidDevice());//init
    ...
    case MSG_APP_OK:
    {
        ...
        else if (0 == result_for_check)/* parasoft-suppress BD-RES-INVFREE "有判断检查" */
        {
            ...
            {
                MMI_GetLabelTextByLang(STR_BROWSER_SAVING, &wait_str); 
    ...
    case MSG_CTL_MIDSK:
			...
			uint32              index = 0;
			SNSSETTING_ITEM_FUNC   item_func = PNULL;

			index = GUILIST_GetCurItemIndex(active_ctrl);
			item_func = (SNSSETTING_ITEM_FUNC)SNS_DlGetListUserdata(active_ctrl, index);
                                           ...
                                           )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        //GUIWIN_SeSoftkeytButtonIconId(win_id, IMAGE_COMMON_MENU, LEFT_BUTTON, FALSE);
        GUIWIN_SeSoftkeytButtonTextId(win_id, TXT_COMMON_COMMON_BACK, RIGHT_BUTTON, FALSE);      
        break;
...
LOCAL void BrwSetSnapshotDetailWindow(MMI_CTRL_ID_T ctrl_id,BRW_SNAPSHOT_ITEM_INFO_T *item_info_ptr)
{
    ...
    SCI_MEMSET(&item_data, 0x00, sizeof(GUIRICHTEXT_ITEM_T));
    item_data.text_type = GUIRICHTEXT_TEXT_RES;
    item_data.text_data.res.id = STR_BROWSER_SIZE;
    GUIRICHTEXT_AddItem(ctrl_id, &item_data, &index);
    ...
    SCI_MEMSET(&item_data, 0x00, sizeof(GUIRICHTEXT_ITEM_T));
    item_data.text_type = GUIRICHTEXT_TEXT_RES;
    item_data.text_data.res.id = STR_BROWSER_TIME;
    GUIRICHTEXT_AddItem(ctrl_id, &item_data, &index);
...
LOCAL void SnapShotHandleMethodSet(void)
{
	MMK_CreateWin((uint32 *)MMIBRW_PREFER_STORAGE_WIN_TAB, PNULL);
}
...
LOCAL void BrwSetSnapshotSaveFileWindow(void)
{
    ...
    GUILABEL_SetTextById(MMIBROWSER_SNAPSHOT_SAVE_F_LABEL1_CTRL_ID, STR_BROWSER_NAME, FALSE);
    GUILABEL_SetTextById(MMIBROWSER_SNAPSHOT_SAVE_F_LABEL2_CTRL_ID, STR_SETT_LIST_STORAGE_EXT01, FALSE);
...
LOCAL void BrwSetSnapshotSaveFileWindow(void)
{
	...
	//new style to show all 
	{
		MMI_TEXT_ID_T method_text = PNULL;
		GUILIST_SetMaxItem(MMIBROWSER_SNAPSHOT_SAVE_F_DROPDOWN_CTRL_ID, 1, FALSE);
		GUILIST_SetSlideState(MMIBROWSER_SNAPSHOT_SAVE_F_DROPDOWN_CTRL_ID, FALSE); 
              CTRLLIST_PermitBorder(MMIBROWSER_SNAPSHOT_SAVE_F_DROPDOWN_CTRL_ID, FALSE);
		
		if(MMIBROWSER_GetSaveDeviceType()==MMI_DEVICE_SDCARD)
		{
			method_text  = STR_ILIST_MEMORY_CARD_EXT01;
		}
		else
		{
			method_text  = STR_ILIST_PHONE_EXT01;
		}
		AppendTwoTextOneImageItem(MMIBROWSER_SNAPSHOT_SAVE_F_DROPDOWN_CTRL_ID, STXT_STORE_MEDIUM, method_text, IMAGE_COMMON_RIGHT_ARROW, (uint32)SnapShotHandleMethodSet);
	}
                                       ...
                                       )
{
    ...
    SCI_TRACE_LOW("[BRW] BrwHandleDeleteSnapshotWinMsg: msg_id=0x%x", msg_id);
    switch(msg_id)
    {
...
//     uint16  i = 0;
    ...
    //get mark num
    mark_num = GUILIST_GetSelectedItemIndex(list_ctrl_id,PNULL,0);
    SCI_TRACE_LOW("[BRW] BrwDeleteSelectedSnapshot: mark_num=0x%x", mark_num);
    if (0 == mark_num)
    {
        //ger current index
...
/*****************************************************************************/
//	Description : the handle function of snapshot list delete Selected Snapshot
//	Global resource dependence : 
//	Author:weipeng.wang
//	Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E BrwHandleSnapshotDeleteOneQueryWinMsg(
                MMI_WIN_ID_T win_id, 
                MMI_MESSAGE_ID_E msg_id, 
                DPARAM param
                )
{
    ...
    SCI_TRACE_LOW("[wangwp] line[%d] file[%s] func[%s] msg_id=0x%x", __LINE__, __FILE__, __FUNCTION__, msg_id);
    switch(msg_id)
    {
    case MSG_APP_CANCEL:
    case MSG_CTL_CANCEL:
    ...
    case MSG_CTL_MIDSK:
        BrwDeleteSelectedSnapshot(MMIBROWSER_SNAPSHOT_LIST_CTRL_ID);
#ifdef MMI_GUI_STYLE_TYPICAL
...
//     uint16                      i = 0;
    ...
    if (0 == mark_num)
    {
        ...
        if (PNULL != item_info_ptr)
        {
...
#else
            MMI_GetLabelTextByLang(STR_BROWSER_DELETE_SNAPSHOT, &dst_temp1);
#endif
            ...
            MMIBROWSER_OpenQueryWinByTextPtr(&dst_temp1,&dst_temp2,IMAGE_PUBWIN_QUERY,PNULL,BrwHandleSnapshotDeleteOneQueryWinMsg);
        }
    ...
    {
        wchar   wstr_num[MMIBROWSER_NUM_TEMP_LEN]        = {0}; 
        wchar   wstr_old[]        = L"%N";  
        ...
        //Prepare text_wstr
        MMI_GetLabelTextByLang(STR_BROWSER_DELETE_BOOKMARKS, &dst_temp2);
        ...
        if (temp_wptr != PNULL)
        {
            ...
            SCI_MEMCPY(temp_wptr,dst_temp2.wstr_ptr,sizeof(wchar) *dst_temp2.wstr_len);
        }
        else//error,无法显示数量
        {
        ...
        MMIAPICOM_WstrReplace(temp_wptr, sizeof(wchar) *temp_wstr_len, wstr_old, wstr_num);
        dst_temp2.wstr_ptr = temp_wptr;
        dst_temp2.wstr_len  = temp_wstr_len;
        MMIBROWSER_OpenQueryWinByTextPtr(&dst_temp1,&dst_temp2,IMAGE_PUBWIN_QUERY,PNULL,BrwHandleSnapshotDeleteOneQueryWinMsg);
                                   ...
                                   )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        {
            uint16  file_name_len = MMIAPICOM_Wstrlen(item_info_ptr->file_name);    
            if(file_name_len > (MMIBROWSER_SNAPSHOT_PATH_LEN + 1 + MMIBROWSER_SNAPSHOT_EXT_NAME_LEN))
            {
    ...
    case MSG_CTL_OK:
    {
        ...
        else
        {
            ...
            {
                MMIPUB_OpenAlertWarningWin(STR_BROWSER_RENAME_FAIL);
            }
        }
        MMK_CloseWin(win_id);
    }
...
{
    MMI_STRING_T    wait_str = {0};
    MMI_GetLabelTextByLang(STR_BROWSER_OPENING, &wait_str); 
...
LOCAL void BrwOpenPageMemoPropertyWin(const BRW_SNAPSHOT_ITEM_INFO_T *item_info_ptr)
{
    ...
    /* Size: */
    MMIRES_GetText(STR_BROWSER_SIZE, PNULL, &str);
    MMIAPICOM_Wstrncpy(property_ptr, str.wstr_ptr, str.wstr_len);
    property_ptr += str.wstr_len;
    ...
    /* Date: */
    MMIRES_GetText(STR_BROWSER_TIME, PNULL, &str);
    MMIAPICOM_Wstrncpy(property_ptr, str.wstr_ptr, str.wstr_len);
    property_ptr += str.wstr_len;
    ...
    case MSG_OPEN_WINDOW:
        {
            ...
            MMIRES_GetText(STR_BROWSER_DETAILS, PNULL, &title_str);
