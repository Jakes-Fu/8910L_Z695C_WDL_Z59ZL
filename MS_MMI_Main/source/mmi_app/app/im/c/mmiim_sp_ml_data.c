                                             ...
                                             )
{
    ...
    //获取是否为PUA的值，如果之前没有得到键值并且是PUA对应的组合字符索引的unicode，则按照PUA再查询一遍
    tmp_key_char = MMIIM_SP_ML9KEY_GetPUAChar(key_char);
    if(!result && key_char != tmp_key_char)
    {
#if (!defined KEYPAD_TYPE_TYPICAL_KEYPAD ||defined  MMI_IM_PDA_SUPPORT)
        if (ml_data_ptr->is_26key)
        {
            result = MMIIM_SP_ML26KEY_GetKeyIndexByChar(key_table_ptr, tmp_key_char, key_index_ptr);
        }
        else
        {
            result = MMIIM_SP_ML9KEY_GetKeyIndexByChar(key_table_ptr, tmp_key_char, key_index_ptr);
        }
#else
            result = MMIIM_SP_ML9KEY_GetKeyIndexByChar(key_table_ptr, tmp_key_char, key_index_ptr);
#endif
...
)
{
    ...
    //language_type = (MMISET_LANGUAGE_TYPE_E)MMISET_GetCurrentLanguageType();
    ...
    //微软的需求，当显示语言为arabic\urdu\farsi时，在显示时统一将所有的数字0123456789
    //按照与native digit的offset 0x0630做个转换，这部分需要从输入法移到显示中统一处理
#if 0
    if((MMISET_LANGUAGE_URDU == language_type) || (MMISET_LANGUAGE_PERSIAN == language_type))
    {
        if((char_data >= '0') && (char_data <= '9'))
        {
            /*Arabic number1234 change to Arabic indic number--for MS*/
            char_data = char_data + 0x0630;
