...
)
{
    ...
    //判断是否为越南语
    if(GUIIM_LANG_VIETNAMESE == data_ptr->init_param.method_param_ptr->im_def_ptr->lang)
    {
...
)
{
    ...
    if ((PNULL != event_data_ptr) && (MSG_NOTIFY_EDITBOX_IMSYNC_CURSOR == event_data_ptr->sys_msg.msg_id))
    {
...
)
{
    ...
    case MMIIM_SP_MSG_KEY_PRESS:
        ...
        else if (cur_key < MMIIM_SP_KEY_INDEX_MAX)
        {
            ...
            if((MMIIM_SP_KEY_0 == msg.para.key) && (GUIIM_M_THAI_ABC == data_ptr->init_param.method_param_ptr->im_set.method))
            {
                // Customization on key 0 only for Thai
                SCI_TRACE_LOW("HandleSysMsg_Thai char_count is zero");                      
                char_count = 0;
			}
			else
            {
        ...
        if (!is_dealt)
        {
			if (MMIIM_SP_KEY_STAR == msg.para.key &&
				...
				!(GUIIM_LIMIT_SYMBOL & data_ptr->init_param.method_param_ptr->limit))
            {
				data_ptr->last_key = cur_key;
                data_ptr->last_key_index = 0;
                is_dealt = TRUE;
		...
		MMIIM_SwitchingIM(&(data_ptr->init_param)); // for [MS]
            ...
            else if((MMIIM_SP_KEY_0 == msg.para.key) && (GUIIM_M_THAI_ABC == data_ptr->init_param.method_param_ptr->im_set.method))
            {
                // Customization on key 0 only for Thai         
                MMIIM_SP_SymbCreateWinEx(data_ptr->init_param.im_handle, MMIIM_SP_SYMB_TYPE_THAI_ZERO);
                is_dealt = TRUE;
            }
    ...
    case MMIIM_SP_MSG_KEY_RELEASE:
        if (is_last_key)
        {
			// Add symbol limit check here to forbid open symbol list window
			if (MMIIM_SP_KEY_STAR == msg.para.key &&
				(PNULL == data_ptr->language_data.custom_kb_ptr) &&
                !(GUIIM_LIMIT_SYMBOL & data_ptr->init_param.method_param_ptr->limit)
                &&(GUIIM_LANG_VIETNAMESE != data_ptr->init_param.method_param_ptr->im_def_ptr->lang))
            {
    ...
    case MMIIM_SP_MSG_KEY_LONG_PRESS:
        ...
        if (MMIIM_SP_KEY_CANCEL == msg.para.key || MMIIM_SP_KEY_DEL == msg.para.key)
        {
            ...
            is_dealt = TRUE;
                ...
                ||(GUIIM_LANG_BENGALI_BD == data_ptr->init_param.method_param_ptr->im_def_ptr->lang)
                ||(GUIIM_LANG_VIETNAMESE == data_ptr->init_param.method_param_ptr->im_def_ptr->lang))  //越南语长按*键显示符号表
                &&(PNULL == data_ptr->language_data.custom_kb_ptr)
                )
                {
                    //need delete the current underline char
                    char_count = MMIIM_SP_ML_GetCount(&data_ptr->language_data, cur_key);
                    if(( GUIIM_LANG_BENGALI        == data_ptr->init_param.method_param_ptr->im_def_ptr->lang)
			||(GUIIM_LANG_BENGALI_BD  == data_ptr->init_param.method_param_ptr->im_def_ptr->lang)			
                     ||((GUIIM_LANG_VIETNAMESE == data_ptr->init_param.method_param_ptr->im_def_ptr->lang)&&(char_count > 0))
                        )
                    {
                        data_ptr->last_key = MMIIM_SP_KEY_INDEX_MAX;
                        data_ptr->last_key_index = 0;
        ...
        else if (cur_key < MMIIM_SP_KEY_INDEX_MAX && is_last_key)
        {
            ...
            if ((PNULL != data_ptr->init_param.method_param_ptr)
                ...
                && ((GUIIM_LANG_BENGALI       == data_ptr->init_param.method_param_ptr->im_def_ptr->lang) || (GUIIM_LANG_BENGALI_BD == data_ptr->init_param.method_param_ptr->im_def_ptr->lang))
...
)
{
    ...
    if (need_change_kb)
    {
        if (PNULL != data_ptr->init_param.f_get_context)
        {
			...
			if (im_context.pos > 0 && im_context.pos <= im_context.str_len)
			{
				prev_char = im_context.str_ptr[im_context.pos - 1];
			}
        }
        else
        {
...
)
{
    ...
    if (commit_data_ptr->commit_len > 0 || commit_data_ptr->underline_len > 0)
    {
       ...
       if(GUIIM_MODE_LEAD_UPPER == data_ptr->language_data.method_cfg_ptr->capital)
        ...
        if(MMIIM_SP_KEYMAP_COMBINE_START <= (commit_data_ptr->commit[0])
            && MMIIM_SP_KEYMAP_COMBINE_END >= (commit_data_ptr->commit[0]))
        {
            MMI_STRING_T keymap_info = {0};
    		keymap_info.wstr_ptr = MMIIM_SP_ML9KEY_GetCombindata(commit_data_ptr->commit[0]);
            keymap_info.wstr_len = MMIAPICOM_Wstrlen(keymap_info.wstr_ptr);
            MMIAPICOM_Wstrncpy(commit_data_ptr->commit, keymap_info.wstr_ptr, keymap_info.wstr_len);
            commit_data_ptr->commit_len = keymap_info.wstr_len;
        }
        if(MMIIM_SP_KEYMAP_COMBINE_START <= (commit_data_ptr->underline[0])
            && MMIIM_SP_KEYMAP_COMBINE_END >= (commit_data_ptr->underline[0]))
        {
            MMI_STRING_T keymap_info = {0};
    		keymap_info.wstr_ptr = MMIIM_SP_ML9KEY_GetCombindata(commit_data_ptr->underline[0]);
            keymap_info.wstr_len = MMIAPICOM_Wstrlen(keymap_info.wstr_ptr);
            MMIAPICOM_Wstrncpy(commit_data_ptr->underline, keymap_info.wstr_ptr, keymap_info.wstr_len);
            commit_data_ptr->underline_len = keymap_info.wstr_len;
        }
