...
#if !defined (WIN32) && defined(BROWSER_SUPPORT) && defined(OPERA_MINI_SUPPORT)
#include "opdevmain.h"
#endif
  ...
  if (g_audioplayer_mode !=0)
    {
        ...
        info_ptr = GetApInfoPtr();
	   g_audioplayer_mode = 0;	
    }
#endif
    ...
    {
#if !defined (WIN32) && defined(BROWSER_SUPPORT) && defined(OPERA_MINI_SUPPORT)
        OD_SPRD_SetOperaDisplay(FALSE); //Add this
#endif
        //关闭已经打开的所有窗口
...
#if defined(ZDT_MUSIC_KING_SUPPORT)    
PUBLIC void MMIAPIMP3_OpenMp3Player_byindex(uint8 index)
{
    BOOLEAN ret = FALSE;
    MMIAP_INFO_T *info_ptr = GetApInfoPtr();
    SCI_TRACE_LOW("[MMIAP] MMIAPIMP3_OpenMp3Player");
    if (g_audioplayer_mode!=index)
    {
        MMIAPISET_StopAllRing(TRUE);
        MMIAPIMP3_StopAudioPlayer();
        MMIAPIMP3_StopApplet(FALSE);
...
//  Note:
/*****************************************************************************/
PUBLIC MMIAP_INFO_T* MMIAP_GetApInfoPtr(void)
{
    return s_mmiap_info_ptr;
}
