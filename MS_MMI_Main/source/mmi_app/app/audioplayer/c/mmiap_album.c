...
LOCAL BOOLEAN album_get_flag = FALSE;
LOCAL BOOLEAN album_fresh_flag = TRUE;
/**--------------------------------------------------------------------------*
**                         FUNCTION DEFINITION                               *
**--------------------------------------------------------------------------*/
/*****************************************************************************/
...
PUBLIC void MMIAP_ResetAlbumFlag(void)
{
	album_get_flag = FALSE;
}
PUBLIC void MMIAP_NeedtoRefreshAlbum(BOOLEAN is_refresh)
{
	album_fresh_flag = is_refresh;
}
...
{
    static MMIMP3_LIST_FILE_BODY_T last_play_file_info = {0};
    MMIMP3_LIST_FILE_BODY_T cur_play_file_info = {0};
    
    cur_play_file_info.full_path_len = FMM_SEARCH_FILENAME_MAXLEN;
    if((album_get_flag==TRUE) && MMIMP3_GetCurPlayFileFullPath(cur_play_file_info.full_path_name ,&(cur_play_file_info.full_path_len)))

    {
        if((cur_play_file_info.full_path_len == last_play_file_info.full_path_len) &&
            ...
            (cur_play_file_info.full_path_len != 0) && !album_fresh_flag)
        {
            SCI_TraceLow("Mp3 MMIAP_GetAllAlbumInfo: not need to get album info, song not changed");
            return;
        }
