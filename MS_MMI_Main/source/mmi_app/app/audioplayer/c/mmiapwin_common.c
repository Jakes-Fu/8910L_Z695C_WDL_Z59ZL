...
PUBLIC void MMIAP_HandleMp3PlayPauseButton(void)
{
    ...
    case MMIMP3_PLAYER_AUDIO_ACTIVE:
    {
        ...
        case MMIMP3_AUDIO_PLAYING:
			...
			if ((MMIAPIMP3_IsPlayerActive()) && (0 == MMIMP3_GetCurPlayListTotalNum()))
			{
...
#endif
			{
	            if(MMISRVAUD_IsAudPlaying(MMIAPIMP3_GetBGPlayerHandle())){  //bug1118712
...
LOCAL void SetVolumeCallback(MMICOM_PANEL_CALLBACK_T *para_ptr)
{
    if(PNULL != para_ptr)
    {
		...
		if((TRUE == MMISRVAUD_CheckAudioVolumeExcessThreshold(MMIMP3_GetAPSrvHandle(), TRUE))
          && (TRUE == needalert))
		{
			///MMK_PostMsg (VIRTUAL_WIN_ID, MSG_AUDIO_EU_ALERT_NOTIFY, PNULL, 0);  //ZDT_PRODUCT_Z685S
		}
    }
}
...
#else
    GUILABEL_SetFont(MMIMP3_FILE_INFO_LABLE_CTRL_ID, MMIMP3_SUB_TEXT_FONT, MMI_BLACK_COLOR);//MMI_WHITE_COLOR//change by James li
#endif
}
...
)
{
    ...
    slide_y_offset = (progress_height - slide_height) / 2; // - 1;
#if defined(MMI_MP3_FIXED_MUSIC)
		if (0 == MMIMP3_GetCurPlayListTotalNum())
		{
			return;
		}
#endif
...
)
{
    ...
    GUISTR_STATE_T      state =         GUISTR_STATE_ALIGN | GUISTR_STATE_WORDBREAK | GUISTR_STATE_SINGLE_LINE | GUISTR_STATE_FROM_HEAD;
    MMI_CTRL_ID_T       ctrl_id = MMIMP3_TIME_INFO_OWNER_DRAW_CTRL_ID ;
    //Begin add by zhaijing for title and album
    MMI_STRING_T                title_string = {0};
    wchar                       file_name[FMM_SEARCH_FILENAME_MAXLEN + 1] = {0};
    GUI_RECT_T   title_rect             = MMIMP3_VER_NAME_FORM_RECT;
    GUI_RECT_T   title_rect_mini    = MMIMP3_VER_NAME_FORM_RECT_MINI;
    GUISTR_STYLE_T      title_style     = {0};
    MMI_STRING_T    tmp_str_t = {0};
    wchar           tmp_wstr[32]   = {0};
    uint32   tmp_str_width = 0;
    uint16   str_crop_num = 0;
	uint16   tmp_wstr_len = 0;
	   MMI_WIN_ID_T        win_id = MMIMP3_MAIN_PLAY_WIN_ID;
    //End
    MMIMP3_GetTimeRect(is_bg_player, &cur_time_rect, &total_time_rect);
		...
		if (0 == MMIMP3_GetCurPlayListTotalNum())
		{
			...
			title_style.font = MMIMP3_MAIN_TITLE_FONT;
			title_style.font_color = MMI_BLACK_COLOR;
			title_style.align = ALIGN_HVMIDDLE;

			MMI_GetLabelTextByLang( listen_music, &title_string );
		      GUISTR_DrawTextToLCDInRect(
                    (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
                    (const GUI_RECT_T      *)&title_rect,       //the fixed display area
                    (const GUI_RECT_T      *)&title_rect,       //用户要剪切的实际区域
                    (const MMI_STRING_T    *)&title_string,//title_string,
                    &title_style,
                    state,
    ...
    //if(MMK_IsFocusWin(win_id))
    {
        //if(is_seek_support)
        {
            if(dyn_info.total_time / 60 < 10)
            //Zhaijing begin add for time UI
            {
            ...
            //Zhaijing end
            else if(dyn_info.total_time / 60 < 60)
            {
            ...
            else if(dyn_info.total_time / 60 < 60*10)
            {
            ...
            text_style.font_color = MMI_BLACK_COLOR;//MMIMP3_TIME_COLOR;//change by James li

            GUIOWNDRAW_GetDisplayRect(ctrl_id, &time_bg_rect);

            //MMIMP3_SetDisplayTimeInfoFlag(TRUE);
            // display
            GUISTR_DrawTextToLCDInRect(
                (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
         ...
         //   MMIMP3_SetDisplayTimeInfoFlag(FALSE);
            //Begin add by zhaijing for title and album
            title_string.wstr_ptr=file_name;
            MMIMP3_DisplayTitleName(&title_string);
            ...
            //Title
            #if 0  ///ZDT_PRODUCT_Z605
            title_style.font = MMIMP3_MAIN_TITLE_FONT;
#ifdef ZDT_PRODUCT_BH            
            title_style.font_color = MMI_BLACK_COLOR;
#else            
            title_style.font_color = MMI_WHITE_COLOR;
#endif
            title_style.align = ALIGN_HVMIDDLE;
			tmp_wstr_len = title_string.wstr_len;
			tmp_str_width = MMIAPICOM_GetStrWidth(title_string.wstr_ptr, &tmp_wstr_len, 0, title_rect.right-title_rect.left, title_style.font);
            if(dyn_info.total_time < 60*60*10){
                if ((MMIAPICOM_GetStrNumForDisplayWidth(title_string.wstr_ptr, &title_string.wstr_len, 0, title_rect.right-title_rect.left, title_style.font) <  title_string.wstr_len) 
					|| tmp_str_width == title_rect.right-title_rect.left)  //bug1108351
                {
                    tmp_str_width = MMIAPICOM_GetStrWidth(tmp_str_t.wstr_ptr, &tmp_str_t.wstr_len, 1, title_rect.right-title_rect.left, title_style.font);
                    str_crop_num = MMIAPICOM_GetStrNumForDisplayWidth(title_string.wstr_ptr, &title_string.wstr_len, 1, 
                        title_rect.right-title_rect.left-tmp_str_width, title_style.font);
                    if (title_string.wstr_len > str_crop_num)
                    {
                        MMI_WSTRNCPY((title_string.wstr_ptr+str_crop_num),4,tmp_str_t.wstr_ptr,4,4);
                        title_string.wstr_len = str_crop_num+4;
                    }
                }
                ...
                if (MMIAPICOM_GetStrNumForDisplayWidth(title_string.wstr_ptr, &title_string.wstr_len, 0, title_rect_mini.right-title_rect_mini.left, title_style.font) <  title_string.wstr_len)
                {
                    tmp_str_width = MMIAPICOM_GetStrWidth(tmp_str_t.wstr_ptr, &tmp_str_t.wstr_len, 1, title_rect_mini.right-title_rect_mini.left, title_style.font);
                    str_crop_num = MMIAPICOM_GetStrNumForDisplayWidth(title_string.wstr_ptr, &title_string.wstr_len, 1, 
                        title_rect_mini.right-title_rect_mini.left-tmp_str_width, title_style.font);
                    if (title_string.wstr_len > str_crop_num)
                    {
                        MMI_WSTRNCPY((title_string.wstr_ptr+str_crop_num),4,tmp_str_t.wstr_ptr,4,4);
                        title_string.wstr_len = str_crop_num+4;
                ...
                GUISTR_DrawTextToLCDInRect(
                    (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
                    (const GUI_RECT_T      *)&title_rect_mini,       //the fixed display area
                    (const GUI_RECT_T      *)&title_rect_mini,       //用户要剪切的实际区域
                    (const MMI_STRING_T    *)&title_string,//title_string,
                    &title_style,
                    state,
                    GUISTR_TEXT_DIR_AUTO
...
)
{
	...
	//GUILABEL_SetTextScroll(MMIMP3_TITLE_LABLE_CTRL_ID, TRUE);
}
...
//uint16        music_full_path_len,
{
    ...
    if(MMK_IsOpenWin(win_id))
    {
        ...
        {
            if(TRUE == MMIAPIFMM_SetMP3ToRing(
                MMIMULTIM_SET_CALL_RING, (DPARAM)&dual_sys,
                selected_file.full_path_name, selected_file.full_path_len
               ))
            {
		   MMIPUB_OpenAlertSuccessWin (STR_NOTE_MUSIC_SET_RINGTONE);
            }
            else
            {
                MMIPUB_OpenAlertSuccessWin (STR_NOTE_MUSIC_RINGTONE_FAIL);
            }
        }
        else
        {
...
)
{
   ...
   LCD_FillRect(&dev_info, disp_rect, MMI_WHITE_COLOR);//add by James li
}
...
PUBLIC void MMIMP3_DisplayTitleName(MMI_STRING_T *title_string)
{
    MMIMP3_LIST_FILE_BODY_T     cur_play_info = {0};
    wchar                       file_name[FMM_SEARCH_FILENAME_MAXLEN + 1] = {0};
    uint16                      file_name_len = 0;
#if 0
def MMI_AUDIOPLAYER_ALBUM_SUPPORT    
    MMIMP3_ALBUM_INFO_T         *album_info_ptr = PNULL;
#endif
    cur_play_info.full_path_len = FMM_SEARCH_FILENAME_MAXLEN;
...
def MMI_AUDIOPLAYER_ALBUM_SUPPORT
    ...
    if (PNULL != album_info_ptr)
    {
        file_name_len = MMIAPICOM_Wstrlen(album_info_ptr->title);
        if(file_name_len != 0)
        {
            MMI_WSTRNCPY(title_string->wstr_ptr,FMM_SEARCH_FILENAME_MAXLEN + 1,album_info_ptr->title,FMM_SEARCH_FILENAME_MAXLEN + 1,file_name_len);
            title_string->wstr_len = file_name_len;
            return;
        }
    }
    ...
    if(MMIMP3_GetCurPlayFileFullPath(cur_play_info.full_path_name,&(cur_play_info.full_path_len)))
    {
        ...
        MMI_WSTRNCPY(title_string->wstr_ptr,FMM_SEARCH_FILENAME_MAXLEN + 1,file_name,FMM_SEARCH_FILENAME_MAXLEN + 1,file_name_len);
        title_string->wstr_len = file_name_len;
    }
}
