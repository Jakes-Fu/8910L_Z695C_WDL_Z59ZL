...
LOCAL MMIMP3_LIST_FILE_BODY_T last_play_file_info = {0};
/*---------------------------------------------------------------------------*/
...
//  Note:
/*****************************************************************************/
PUBLIC MMISRV_HANDLE_T  MMIMP3_GetAPSrvHandle(void)
{
	return MMISRVAP_GetAudHandle(GetAPSrvHandle());   
}
...
PUBLIC void MMIMP3_ResetLastPlayFileInfo(void)
{
	memset(&last_play_file_info,0,sizeof(last_play_file_info));
}
...
{
    static MMIMP3_PLAYING_DYN_INFO_T last_dyn_info = {0};
    BOOLEAN ret = FALSE;
    ...
    if (ret)
    {
        if((cur_play_file_info.full_path_len == last_play_file_info.full_path_len) &&
            (0 == MMIAPICOM_Wstrncmp(cur_play_file_info.full_path_name,last_play_file_info.full_path_name,cur_play_file_info.full_path_len)) &&
            (cur_play_file_info.full_path_len != 0))
        {
            SCI_TraceLow("MMIMP3_GetCurFileContent: the same song ,not need to get info again");
            
            //SetDynPlayingInfo(&last_dyn_info);
          ...
          )
        {
              ...
              )
            {
                ...
                last_play_file_info = cur_play_file_info;
...
)
{
    ...
    if((TRUE == MMISRVAUD_CheckAudioVolumeExcessThreshold(MMIMP3_GetAPSrvHandle(), TRUE)) && needalert)
	{       
...
PUBLIC void MMIMP3_FixedMusicCallBack(
                                     MMISRVAUD_REPORT_RESULT_E  result,
                                     DPARAM           param
                                     )
{
#ifdef MMIMP3_BUTTON_FOCUS_ENABLE
    MMIMP3_SetFocusButton(MMIMP3_BUTTON_PLAY_PAUSE);
#endif    
    ...
    { 
		...
		MMIAPIMP3_StopAudioPlayer();
		 ...
		 MMIMP3_DisplayMp3PlayWin(TRUE,MMIMP3_MAIN_PLAY_WIN_ID);
    }
...
PUBLIC BOOLEAN MMIAPIMP3_IsPlayingAudioState(void)
{
    BOOLEAN result = FALSE;
    MMIMP3_AUDIO_STATE_E audio_state = MMIMP3_GetAudioState();
    if(audio_state == MMIMP3_AUDIO_PLAYING)
    {
        result = TRUE;
    }
    return result;
