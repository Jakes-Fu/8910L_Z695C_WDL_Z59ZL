...
)
{
    ...
    while(1) /*lint !e716*/
    {
        MMI_RECEIVE_SIGNAL((xSignalHeader*)psig, g_app_mp3_del_task_id);/*lint !e63*/
        //psig = s_cur_sig;
...
LOCAL void DelFileInfo(MMIMP3_LIST_DEL_INFO_T* opr_info_ptr)
{
    ...
    wchar     full_path[MMIFILE_FULL_PATH_MAX_LEN + 1] = {0};
    uint16    full_path_len = MMIFILE_FULL_PATH_MAX_LEN;
    BOOLEAN to_del_flag = FALSE;
    uint16 file_index,cur_list_index;
            ...
            && PNULL != opr_info_ptr->select_index_ptr)
    {
        ...
        cur_list_index = s_cur_audioplayer_list_info.is_music_list ? (s_cur_audioplayer_list_info.cur_list_index + APLAYER_LIST_TYPE_MYMUSIC):s_cur_audioplayer_list_info.cur_list_index;
        
        for (i = opr_info_ptr->select_num -1;  i < opr_info_ptr->select_num; i--) /*lint !e685 !e568*/
        {

            //get file info to be deleted!
            memset(full_path,0,sizeof(full_path));
            if(MMIAPlayer_GetAudioFileFullPath(list_index,opr_info_ptr->select_index_ptr[i],full_path,&full_path_len))
            {
                if((opr_info_ptr->is_del_file)&&(cur_list_index != list_index)&&(APLAYER_LIST_SUCCESS == MMIAPlayer_GetAudioFileIndexInList(cur_list_index,full_path,full_path_len, &file_index)))
                    to_del_flag = TRUE;
            }
                    ...
                    || APLAYER_LIST_DELETE_FILE_ERROR == del_result)
            {
                ...
                if ((APLAYER_LIST_SUCCESS == del_result) && to_del_flag)
                {
                        MMIAPlayer_DeleteFileFromList(cur_list_index,file_index,FALSE);
                }
...
extern uint8 g_audioplayer_mode;
#endif
...
#if defined(ZDT_MUSIC_KING_SUPPORT)
    if (g_audioplayer_mode == 1)
    {
    MMIAPIFMM_CombineFullPath(MMIAPIFMM_GetDevicePath(tem_dev),
                              MMIAPIFMM_GetDevicePathLen(tem_dev),
                                  MMIMULTIM_DIR_MUSIC_KING_01,
                                  MMIAPICOM_Wstrlen(MMIMULTIM_DIR_MUSIC_KING_01),
                              PNULL,//MMIMULTIM_DIR_MUSIC,
                              NULL,///MMIAPICOM_Wstrlen(MMIMULTIM_DIR_MUSIC),
                                  full_dir_path,
                                  &full_dir_path_len);
    }
    else if (g_audioplayer_mode == 2)
    {
        MMIAPIFMM_CombineFullPath(MMIAPIFMM_GetDevicePath(tem_dev),
                                  MMIAPIFMM_GetDevicePathLen(tem_dev),
                                  MMIMULTIM_DIR_MUSIC_KING_02,
                                  MMIAPICOM_Wstrlen(MMIMULTIM_DIR_MUSIC_KING_02),
                              PNULL,
                              NULL,
                              full_dir_path,
                              &full_dir_path_len);
    }
    else if (g_audioplayer_mode == 3)
    {
        MMIAPIFMM_CombineFullPath(MMIAPIFMM_GetDevicePath(tem_dev),
                                  MMIAPIFMM_GetDevicePathLen(tem_dev),
                                  MMIMULTIM_DIR_MUSIC_KING_03,
                                  MMIAPICOM_Wstrlen(MMIMULTIM_DIR_MUSIC_KING_03),
                                  PNULL,
                                  NULL,
                                  full_dir_path,
                                  &full_dir_path_len);
    }
 else if (g_audioplayer_mode == 4)
    {
        MMIAPIFMM_CombineFullPath(MMIAPIFMM_GetDevicePath(tem_dev),
                                  MMIAPIFMM_GetDevicePathLen(tem_dev),
                                  MMIMULTIM_DIR_MUSIC_KING_04,
                                  MMIAPICOM_Wstrlen(MMIMULTIM_DIR_MUSIC_KING_04),
                                  PNULL,
                                  NULL,
                                  full_dir_path,
                                  &full_dir_path_len);
    }
   else if (g_audioplayer_mode == 5)
    {
        MMIAPIFMM_CombineFullPath(MMIAPIFMM_GetDevicePath(tem_dev),
                                  MMIAPIFMM_GetDevicePathLen(tem_dev),
                                  MMIMULTIM_DIR_MUSIC_KING_05,
                                  MMIAPICOM_Wstrlen(MMIMULTIM_DIR_MUSIC_KING_05),
                                  PNULL,
                                  NULL,
                                  full_dir_path,
                                  &full_dir_path_len);
                     ...
                     uint16 list_index)
{
    ...
    total_num = MMIAPlayer_GetListReallySize(list_index);
    for (i = 0; i < total_num; i++)
    {
