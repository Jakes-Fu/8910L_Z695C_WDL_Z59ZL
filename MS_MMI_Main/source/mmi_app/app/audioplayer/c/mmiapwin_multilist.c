...
APLAYER_LIST_ERROR_E g_list_state_by_copy = APLAYER_LIST_SUCCESS;
...
)
{
    ...
    case MSG_PROMPTWIN_OK:
        ...
        else
        {
            ...
            if(g_list_state_by_copy != APLAYER_LIST_NO_SAPCE &&  g_list_state_by_copy != APLAYER_LIST_HAS_EXIST) MMIMP3_DeleteList(TRUE,&cur_selection);
...
LOCAL void HandlePubMenuOKHandler(MMIPUB_MENU_NOTIFY_T *param)
{
    ...
    if(PNULL != param)
    {
        ...
        case ID_MP3_LIST_LIST_DEL:
            //打开询问是否删除窗口;
            MMIPUB_OpenQueryWinByTextIdEx(SPRD_MP3_APPLET_ID,
                                          TXT_DELETEQUERY, IMAGE_PUBWIN_QUERY, &query_win_id, PNULL,
                                          PNULL);
                    ...
                    uint16    cur_list_selection)
{
    ...
    if(menu_id >= ID_MP3_LIST_LIST_COPY && (menu_id <= ID_MP3_LIST_LIST_OPT_END))
    {
        ...
        g_list_state_by_copy = copy_result;
...
)
{
    ...
    case MSG_APP_WEB:
        if (PNULL != cur_selection_ptr)
        {
            ...
            {
                MMIPUB_OpenAlertWinByTextId(PNULL, STR_FILES_INVALID_CHARACTER_EXT01, TXT_NULL,
                                            IMAGE_PUBWIN_WARNING, &alert_win_id, PNULL, MMIPUB_SOFTKEY_ONE, PNULL);
            }
        }
        break;
...
)
{
    ...
    case MSG_OPEN_WINDOW:
	    ...
	    GUIEDIT_SetSoftkeyEx(ctrl_id, 0, 1, PNULL, TXT_COMMON_OK, PNULL, PNULL, EDIT_BUTTON_TEXT_ID, PNULL);
        MMK_SetAtvCtrl(win_id, ctrl_id);
        break;
...
)
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        GUILIST_SetOwnSofterKey(play_list_user_data_ptr->list_ctrl_id, FALSE);
#endif
        break;
...
LOCAL void InitPlayListCtrl(LIST_WIN_USER_DATA_T      *user_data_ptr)
{
    ...
    if (PNULL != user_data_ptr)
    {
        ...
        {
            GUILIST_SetTitleIndexType(user_data_ptr->list_ctrl_id, GUILIST_TITLE_INDEX_EXCLUDE_FIRST);
            GUILIST_SetTitleStyle(user_data_ptr->list_ctrl_id, GUILIST_TITLE_ONLY);
        ...
        MMI_CTRL_ID_T   ctrl_id)
{
    ...
    {
        // for bug 1296194, ID_MP3_LIST_DEL_REAL should set to be gray in this case
        if(is_music_list && (MMIMP3_MY_MUSIC_MENU_INDEX == cur_list_selection))
        {
            menu_group2[2].parent_id = 0;
        }
...
/*            MMIAPIMP3_StopAudioPlayer();*/
    ...
    case MSG_CLOSE_WINDOW:
		MMK_PostMsg (MMIMP3_PLAY_LIST_WIN_ID, MSG_FULL_PAINT, PNULL, 0);
        result = MMIPUB_HandleAlertWinMsg(win_id, msg_id, param);
        break;
    ...
    if(is_success && 0 < file_num && PNULL != user_data_ptr)
    {
        ...
        if(MMIMP3_AppendFilesToList(TRUE,user_data_ptr->cur_list_selection,file_array,&added_num))
        {
            if(file_num != added_num)
            {
             ...
             if (MMIMP3_IsCurPlayList(user_data_ptr->is_music_list,user_data_ptr->cur_list_selection))
            {
                MMIMP3_SetCurPlayInfo(user_data_ptr->is_music_list,user_data_ptr->cur_list_selection,user_data_ptr->cur_file_selection,MMIMP3_GetMp3PlayMode());
            }
