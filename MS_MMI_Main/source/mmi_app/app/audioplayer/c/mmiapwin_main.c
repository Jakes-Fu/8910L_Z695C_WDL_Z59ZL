...
#include "mmi_id.h"
static MMI_WIN_ID_T  s_hero_SongPlayMain_winid = 0;
#endif
...
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIMAINWIN_EnterPlayer(void)
{
    if(MMK_IsExistWin(MMK_GetAppletHandleByWin(MMK_ConvertIdToHandle(MMIMP3_MAIN_PLAY_WIN_ID)),MMIMP3_MAIN_PLAY_WIN_ID))
    {
        MMK_WinGrabFocus(MMIMP3_MAIN_PLAY_WIN_ID);
    }
    else
    {
        MMIAPMAINWIN_Enter(TRUE);
    }
}
...
#ifndef MMI_MP3_FIXED_MUSIC 	
    if(SFS_ERROR_NO_SPACE == MMIAPIFMM_GetSuitableFileDev(MMI_DEVICE_SDCARD, VIRTUALARRAY_MIN_DISKSPACE, &file_dev))
    {
...
/*****************************************************************************/
//  Description : Destrory the MP3 applet
//  Global resource dependence :
//  Author:weipeng.wang
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPMAINWIN_Destory(void)
{
    MMIAP_MAIN_WIN_INFO_T *win_ptr = GetMainWinInfoPtr();
    MMIAP_INFO_T *info_ptr = MMIAP_GetApInfoPtr();
    SCI_TRACE_LOW("[MMIAP]WINMAIN MMIAPMAINWIN_Destory 1 0x%x, 0x%x",  win_ptr, info_ptr);
    if((PNULL != win_ptr) || (PNULL != info_ptr))
    //if(MMK_IsExistWin(MMK_GetAppletHandleByWin(MMK_ConvertIdToHandle(MMIMP3_MAIN_PLAY_WIN_ID)),MMIMP3_MAIN_PLAY_WIN_ID))
    {
        //SCI_TRACE_LOW("[MMIAP]WINMAIN MMIAPMAINWIN_Destory 2");
        //DestroyMainWinInfo();
        MMIAPIMP3_StopApplet(FALSE);
    }
...
/*****************************************************************************/
//  Description :get main play win id
//  Global resource dependence :
//  Author:weipeng.wang
//  Note:
/*****************************************************************************/
PUBLIC MMI_HANDLE_T MMIAPMAINWIN_GetMainPlayWinHandle(void)
{
    return MMK_ConvertIdToHandle(MMIMP3_MAIN_PLAY_WIN_ID);
}
...
defined(MMI_MP3_FIXED_MUSIC)
		...
		if(MMIMP3_GetAudioState() != MMIMP3_AUDIO_PLAYING)
		{
			 MMIAPIMP3_StopAudioPlayer();
...
#ifdef MMI_AUDIOPLAYER_ALBUM_SUPPORT
        	MMIAP_ResetAlbumFlag();
#endif
        	MMIMP3_ResetLastPlayFileInfo();
#ifndef TOUCH_PANEL_SUPPORT
#ifdef MMI_AUDIOPLAYER_REVIEW_SUPPORT
            //if(!MMITHEME_IsMainScreenLandscape())
            {
...
//			s_mp3_is_web_key_down = TRUE;
	 ...
	 #ifdef HERO_ENGINE_TTS_SUPPORT
		s_hero_SongPlayMain_winid = 0;
	 #endif	
		/// MMK_CloseWin(win_id);
          ///  MMIMP3_SetIsStartFromFmm(FALSE);
#if defined(MMI_MP3_FIXED_MUSIC)
			if ((MMIAPIMP3_IsPlayerActive()) && (0 == MMIMP3_GetCurPlayListTotalNum()))
			{
				MMIAPISET_StopAllRing(TRUE);
...
)
{
    ...
    if (MMK_IsOpenWin(win_handle))
    {
        ...
        {
            MMISRVAP_ERROR_E result = req_param_ptr->srvap_result;
...
PUBLIC void MMIAPIMp3_HeadsetInd(MMI_MESSAGE_ID_E msg_id)
{
    ...
    {
        //bug 1299016若当前声道是耳机，则耳机拔出后需要暂停，其他情况比如当前连着蓝牙耳机
        //则不应该做暂停
        MMISRVAUD_ROUTE_T cur_mp3_audio_route = MMISRVAUD_ROUTE_NONE;
        cur_mp3_audio_route = MMISRVAUD_GetRoute(MMIAP_GetAPAudioSrvHandle());
        SCI_TRACE_LOW("MMIAPIMp3_HeadsetInd:cur_mp3_audio_route:%d\n",cur_mp3_audio_route);
        if(MMISRVAUD_ROUTE_EARPHONE == cur_mp3_audio_route)
        {
            if(MMIAPIMP3_IsPlayerPlaying())
            {
                SCI_TRACE_LOW("MMIAPIMp3_HeadsetInd:mp3 is playing\n");
                MMIAP_HandleMp3PlayPauseButton();
            }
        }
        else
        {
            SCI_TRACE_LOW("MMIAPIMp3_HeadsetInd:cur audio route is not earphone!\n");
        }
    }
}
...
#ifdef HERO_ENGINE_TTS_SUPPORT
PUBLIC unsigned int hero_GetSongPlayMainWinId(void)
{
	return s_hero_SongPlayMain_winid;
}
#endif
