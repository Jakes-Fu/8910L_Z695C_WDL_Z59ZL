...
#define MMICL_DURATION_TIME_STR_MAX_LEN 20
//[detail UI layout:contact item height]
#define DETAIL_VIEW_CONTACT_ITEM_HEIGHT MMI_LIST_DOUBLE_LINE_ITEM2_HEIGHT_MS
//[end]
/**--------------------------------------------------------------------------*
 **                         TYPE AND CONSTANT                                *
 **--------------------------------------------------------------------------*/
...
typedef struct
{
    uint8    number[MMIPB_BCD_NUMBER_MAX_LEN]; //the dialling number
    uint8    num_len;
    uint8    position[MMICL_RECORD_TOTAL_NUM]; //record position in nv
    uint8    total_num;
    MN_DUAL_SYS_E		dual_sys;
}MMICL_GROUP_LIST_ITEM_T;

typedef struct
{
    uint8 record_index;
    uint8 call_time_index;
}MMICL_GROUP_ITEM_DETAIL_T;
//[end]
...
//[add call log select sim menu]
typedef enum
{
    CL_SIM_SEL_MENU_NODE_ROOT,
    CL_SIM_SEL_MENU_VODAFONE,
    CL_SIM_SEL_MENU_CHINAUNICOM
};
//[end]
/**--------------------------------------------------------------------------*
 **                         EXTERNAL DECLARE                                 *
