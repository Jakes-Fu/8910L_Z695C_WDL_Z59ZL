                                 ...
                                 )
{
    ...
    if(((0 != same_num_pos) || (call_times > 1))/* && call_info.call_num.is_num_present*/)
    {
        ...
        else
        {
            ...
            #endif
            for (i=same_num_pos; i>insert_index; i--)
            {
                arraycall_info_ptr->call_bcdinfo[i] = arraycall_info_ptr->call_bcdinfo[i-1];
            }
        }
                                 ...
                                 )
{
    ...
    //bug id: 1307447, 1301782,1311747 ,1314442    
...
#endif
#endif
//[interface for data counter: get call
// duration time by period and sim card]
PUBLIC uint32 MMIAPICL_GetSimCallDurTimeByPeriod(
                                MN_DUAL_SYS_E dual_sys,
                                MMICL_CALL_TYPE_E call_type,
                                uint32 start_time,
                                uint32 stop_time,
                                ...
                                )
{
    ...
    MMICL_CALL_ARRAY_BCDINFO_T* p_arraycall_info = PNULL;
    ...
    SCI_MEMSET(p_arraycall_info, 0, sizeof(MMICL_CALL_ARRAY_BCDINFO_T));
    MMICL_GetRecordInfo(call_type, p_arraycall_info);

    for (i = 0; i < p_arraycall_info->record_num; i++)
    {
        //uint8 start_pos = 0, end_pos = 0;

        p_callinfo = &(p_arraycall_info->call_bcdinfo[i]);
        if (p_callinfo->dual_sys != dual_sys
                || start_time > p_callinfo->call_start_time[p_callinfo->call_times - 1]
                || stop_time < p_callinfo->call_start_time[0])
        {
            continue;
        }
        ...
        {
            if (start_time <= p_callinfo->call_start_time[j]
                    && p_callinfo->call_start_time[j] <= stop_time)
            {
                duration_time += p_callinfo->call_duration_time[j];
            }
        }
    }

    SCI_FREE(p_arraycall_info);
    SCI_TRACE_LOW("MMIAPICL_GetSimCallDurTimeByPeriod ----start_time: %d , stop_time: %d , duration_time: %d", start_time, stop_time, duration_time);
    MMICL_FormatTimeStrByTime(duration_time,p_time_str);
    return duration_time;
}
//[delete group log list]
uint8 MMICL_DeleteGroupAppointedCallInfo(
                                        MMICL_GROUP_LIST_ITEM_T *p_group_list_info,
                                        uint8     pos
                                        )
{
    int         i = 0, step = 0;
    uint8 j = 0, record_num = 0;
    MMICL_CALL_ARRAY_BCDINFO_T* arraycall_info = PNULL;
    uint8 * p_del_position = &(p_group_list_info->position);
    ...
    //delete ldn
    {
        for (i = 0; i < p_group_list_info->total_num; i++)
        {
            MMICL_CALL_INFO_T call_info = {0};
            MMICL_CALL_TYPE_E call_type = arraycall_info->call_bcdinfo[p_del_position[i]].call_type;

            call_info.dual_sys = arraycall_info->call_bcdinfo[p_del_position[i]].dual_sys;
            call_info.call_num.is_num_present = arraycall_info->call_bcdinfo[p_del_position[i]].is_num_present;
            call_info.call_num.number.number_plan = arraycall_info->call_bcdinfo[p_del_position[i]].number_plan;
            call_info.call_num.number.number_type = arraycall_info->call_bcdinfo[p_del_position[i]].number_type;
            call_info.call_num.number.num_len = MIN(arraycall_info->call_bcdinfo[p_del_position[i]].number_len, MN_MAX_ADDR_BCD_LEN);
            #ifdef MMI_CSP_SUPPORT
            call_info.line = arraycall_info->call_bcdinfo[p_del_position[i]].line;
            #endif
            SCI_MEMCPY(&call_info.call_num.number.party_num, arraycall_info->call_bcdinfo[p_del_position[i]].number, call_info.call_num.number.num_len);

            if (arraycall_info->call_bcdinfo[p_del_position[i]].is_sim_record && 0 < arraycall_info->sim_record_num)
            {
                arraycall_info->sim_record_num--;
            }
    ...
    for (i = 0; i < p_group_list_info->total_num; i++)
    {
        SCI_MEMSET(&(arraycall_info->call_bcdinfo[p_del_position[i]]),0,sizeof(MMICL_CALL_BCDINFO_T));
    }

    for (i = 0; i < p_group_list_info->total_num; i++, step++)
    {
        //后面的电话记录前移
        int end = (i < p_group_list_info->total_num - 1)
                ? (p_del_position[i+1] - 1) : (arraycall_info->record_num - 1);
        for (j = p_del_position[i]; j < end; j++)
        {
            arraycall_info->call_bcdinfo[j - step] = arraycall_info->call_bcdinfo[j+1];
        }
    }

    //set last n record to 0,n is delete numbers.
    SCI_MEMSET(&arraycall_info->call_bcdinfo[arraycall_info->record_num - step],
            0, sizeof(MMICL_CALL_BCDINFO_T) * step);

    arraycall_info->record_num -= step;
...
//[after add number to block ist,call log will delete logs]
BOOLEAN MMICL_DeleteCallInfoByNum(MMIPB_BCD_NUMBER_T pb_bcd)
{
    int         i = 0, step = 0;
    uint8 j = 0, record_num = 0;
    MMICL_CALL_ARRAY_BCDINFO_T* arraycall_info = PNULL;
    uint8 position[MMICL_RECORD_TOTAL_NUM] = {0};
    uint8 total_delete_num = 0;
    ...
    for (i = 0; i < arraycall_info->record_num; i++)
    {
        if(memcmp(arraycall_info->call_bcdinfo[i].number,pb_bcd.number,
                MMIPB_BCD_NUMBER_MAX_LEN) == 0)
        {
            position[total_delete_num] = i;
            total_delete_num++;
        }
    }
    ...
    //delete ldn
    {
        for (i = 0; i < total_delete_num; i++)
        {
            MMICL_CALL_INFO_T call_info = {0};
            MMICL_CALL_TYPE_E call_type = arraycall_info->call_bcdinfo[position[i]].call_type;

            call_info.dual_sys = arraycall_info->call_bcdinfo[position[i]].dual_sys;
            call_info.call_num.is_num_present = arraycall_info->call_bcdinfo[position[i]].is_num_present;
            call_info.call_num.number.number_plan = arraycall_info->call_bcdinfo[position[i]].number_plan;
            call_info.call_num.number.number_type = arraycall_info->call_bcdinfo[position[i]].number_type;
            call_info.call_num.number.num_len = MIN(arraycall_info->call_bcdinfo[position[i]].number_len, MN_MAX_ADDR_BCD_LEN);
            #ifdef MMI_CSP_SUPPORT
            call_info.line = arraycall_info->call_bcdinfo[position[i]].line;
            #endif
            SCI_MEMCPY(&call_info.call_num.number.party_num, arraycall_info->call_bcdinfo[position[i]].number, call_info.call_num.number.num_len);

            if (arraycall_info->call_bcdinfo[position[i]].is_sim_record && 0 < arraycall_info->sim_record_num)
            {
                arraycall_info->sim_record_num--;
            }
    ...
    for (i = 0; i < total_delete_num; i++)
    {
        SCI_MEMSET(&(arraycall_info->call_bcdinfo[position[i]]),0,sizeof(MMICL_CALL_BCDINFO_T));
    }

    for (i = 0; i < total_delete_num; i++, step++)
    {
        //后面的电话记录前移
        int end = (i < total_delete_num - 1)
                ? (position[i+1] - 1) : (arraycall_info->record_num - 1);
        for (j = position[i]; j < end; j++)
        {
            arraycall_info->call_bcdinfo[j - step] = arraycall_info->call_bcdinfo[j+1];
        }
    }
...
//[interface for call: get call
// duration time by sim card]
PUBLIC uint32 MMIAPICL_GetSimCallDurationTime(
                                                MN_DUAL_SYS_E dual_sys,
                                                MMICL_CALL_TYPE_E call_type
                                                )
{
