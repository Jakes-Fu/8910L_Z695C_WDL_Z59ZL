...
#define MAX_HERO_CALL_LOCATION_LEN   12
#endif
    ...
    case MSG_CTL_MIDSK:
            ...
            case ID_CL_DELETE_ALL_RECORDS:
                ...
                else
                {
                    ...
                    //询问是否删除!! 
                    MMIPUB_OpenQueryWinByTextId(TXT_DELETE_ALL_QUERY,IMAGE_PUBWIN_QUERY,&query_win_id,PNULL);
                }
                break;
...
LOCAL void SetLogListItem(void)
{
    ...
    //load log list
    GUILIST_SetMaxItem(list_handle, MMICL_RECORD_TOTAL_NUM, FALSE ); //  GUILIST_SetMaxItem(list_handle, MMICL_RECORD_MAX_NUM, FALSE );
    ...
    else
    {
        ...
        find_result = MMIAPICL_GetNameByClInfo(*callinfo, &namestrid, FALSE);
#if defined(HERO_ENGINE_SUPPORT)&&defined(HERO_ENGINE_HMGS_SUPPORT)
			{
				...
				HcCall_hist_populate_call_list(cityName, 64, tele_num);
				cityName_len = MMIAPICOM_Wstrlen((const wchar *) &cityName[0]);
				hccall_printf("[INKLEAK_LOG] CC_FindLocationInfor str_length = %d", cityName_len);
			}
#endif
        ...
        if ( !find_result || 0 == namestrid.wstr_len )
        {
            ...
            if((callinfo->call_times > 1)&&(isAddCalltimes))
            {
                ...
                {
                	extern void hccall_printf(const char *format,...);
                	hccall_printf("[INKLEAK_LOG] mmicl_wintab.c 222 max_len = %d, tempBuff_len = %d", max_len, cityName_len);
                	hccall_printf("[INKLEAK_LOG] mmicl_wintab.c 222 in_str = %d", MMIAPICOM_Wstrlen(in_str));
	                MMI_WSTRNCPY(in_str+MMIAPICOM_Wstrlen(in_str),max_len,(const wchar*)cityName,
	                            cityName_len,cityName_len);
                }
#endif
            }
            else
#endif
            {
                ...
                {
                	extern void hccall_printf(const char *format,...);
                	hccall_printf("[INKLEAK_LOG] mmicl_wintab.c 333 max_len = %d, tempBuff_len = %d", max_len, cityName_len);
                	hccall_printf("[INKLEAK_LOG] mmicl_wintab.c 333 in_str = %d", MMIAPICOM_Wstrlen(in_str));
	                MMI_WSTRNCPY(in_str+MMIAPICOM_Wstrlen(in_str),max_len,(const wchar*)cityName,
	                            cityName_len,cityName_len);
                }
#endif
        ...
        else//there is name
        {
            ...
            if((callinfo->call_times > 1) &&(isAddCalltimes))
            {
                ...
                {
                	extern void hccall_printf(const char *format,...);
                	hccall_printf("[INKLEAK_LOG] mmicl_wintab.c 444 max_len = %d, tempBuff_len = %d", max_len, cityName_len);
                	hccall_printf("[INKLEAK_LOG] mmicl_wintab.c 444 in_str = %d", MMIAPICOM_Wstrlen(in_str));
	                MMI_WSTRNCPY(in_str+MMIAPICOM_Wstrlen(in_str),max_len,(const wchar*)cityName,
	                            cityName_len,cityName_len);
                }
#endif
...
#endif            
            {
				...
				{
                	extern void hccall_printf(const char *format,...);
                	hccall_printf("[INKLEAK_LOG] mmicl_wintab.c 555 max_len = %d, tempBuff_len = %d", max_len, cityName_len);
                	hccall_printf("[INKLEAK_LOG] mmicl_wintab.c 555 in_str = %d", MMIAPICOM_Wstrlen(in_str));
	                MMI_WSTRNCPY(in_str+MMIAPICOM_Wstrlen(in_str),max_len,(const wchar*)cityName,
	                            cityName_len,cityName_len);
                }
#endif
                             ...
                             )
{
    ...
    MMI_STRING_T name_number_str = {0};
#if defined(HERO_ENGINE_SUPPORT)&&defined(HERO_ENGINE_HMGS_SUPPORT)
	wchar name_number_wchar[PHONE_NUM_MAX_LEN +MAX_CALL_TIME_LEN+ 1 + MAX_HERO_CALL_LOCATION_LEN]  ={0};
#else
    ...
    else
    {
        for ( i = 0; i < arraycall_info->record_num; i++ )
        {
            ...
            //Add Contact name/or call number content
        #if defined(HERO_ENGINE_SUPPORT)&&defined(HERO_ENGINE_HMGS_SUPPORT)
            if (GetNameOrNumberContent(name_number_wchar,PHONE_NUM_MAX_LEN+MAX_CALL_TIME_LEN+MAX_HERO_CALL_LOCATION_LEN,callinfo,TRUE,isUseNumberType))
		#else
                             ...
                             )
{
    ...
    MMI_STRING_T name_number_str = {0};
#if defined(HERO_ENGINE_SUPPORT)&&defined(HERO_ENGINE_HMGS_SUPPORT)
	wchar name_number_wchar[PHONE_NUM_MAX_LEN + 1+MAX_CALL_TIME_LEN+MAX_HERO_CALL_LOCATION_LEN]  ={0}; 
#else
...
LOCAL void StartPbapCalllogSync(MMI_WIN_ID_T win_id)
{
    ...
    if(IsBTCalllogWinId(win_id))
    {
        ...
        if(MMIAPIBT_HfuGetConnStatus(&dev_addr))
        {
    ...
    case MSG_APP_OK:
        ...
        case ID_CL_LOG_LIST_DELETE:
...
#endif    
            {
                ...
                MMIPUB_OpenQueryWinByTextIdEx(VIRTUAL_WIN_ID, TXT_DELETE_QUERY,IMAGE_PUBWIN_QUERY,&query_win_id,CC_HandleLogListDeleteQueryWinMsg, PNULL);
                MMK_CloseWin(win_id);
            }
            break;
       ...
       case ID_CL_DIAL_VIDEOCALL://@zhaohui add
        ...
        case ID_CL_DELETE_ALL:
            //询问是否删除!! 
            MMIPUB_OpenQueryWinByTextId(TXT_DELETE_ALL_QUERY,IMAGE_PUBWIN_QUERY,&query_win_id,CC_HandleLogListDeleteAllQueryWinMsg);
            MMK_CloseWin(win_id);
            break;
...
#ifdef IM_MYANMAR_SUPPORT    
    if(MMISET_LANGUAGE_MYANMAR == lang_type)
    {    
        item_data.text_set_align = TRUE;
        item_data.setting_align = ALIGN_LVMIDDLE;
    }
#endif
                                             ...
                                             )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        MMI_GetLabelTextByLang((MMI_TEXT_ID_T)TXT_COMMON_FLOW_STATISTICS, &title_body); //fix bug1259463
...
//fix by bingjie.chen
            ...
            {
            #if defined(HERO_ENGINE_SUPPORT)&&defined(HERO_ENGINE_HMGS_SUPPORT)
            	if (GetNameOrNumberContent(name_number_wchar,PHONE_NUM_MAX_LEN+MAX_CALL_TIME_LEN+MAX_HERO_CALL_LOCATION_LEN,callinfo,FALSE,TRUE))
            #else
                                            ...
                                            )
{
    ...
    case MSG_OPEN_WINDOW:
        //set edit single
        GUIEDIT_SetStyle(MMICL_EDIT_BEFORE_DIAL_EDIT_CTRL_ID,GUIEDIT_STYLE_MULTI);// BUG - 1290856,1290171 联系人详情页，Edit before dialing, 内容超过一行时，改为多行显示 
                             ...
                             )
{
    ...
    //Append item
        ...
        //Add Contact name/or call number content
    #if defined(HERO_ENGINE_SUPPORT)&&defined(HERO_ENGINE_HMGS_SUPPORT)
        if (GetNameOrNumberContent(name_number_wchar,PHONE_NUM_MAX_LEN+MAX_CALL_TIME_LEN+MAX_HERO_CALL_LOCATION_LEN,callinfo,TRUE,FALSE))
    #else
...
PUBLIC uint8 MMICL_GetCallNumByType(uint8 call_type, uint16 record_index)
{
    uint8 ret=0;
    MMICL_CALL_ARRAY_BCDINFO_T* arraycall_info = PNULL;
    arraycall_info = (MMICL_CALL_ARRAY_BCDINFO_T*)SCI_ALLOCAZ(sizeof(MMICL_CALL_ARRAY_BCDINFO_T));
    ...
    SCI_TRACE_LOW("MMICL_GetCallNumByType record_num=%d", arraycall_info->record_num);
     if ((arraycall_info->record_num != 0)&& arraycall_info->call_bcdinfo[record_index].is_num_present)
        ret=1;
    else 
        ret=0;
