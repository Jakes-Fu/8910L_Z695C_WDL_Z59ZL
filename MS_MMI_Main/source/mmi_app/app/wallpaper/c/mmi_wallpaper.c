                                 ...
                                 )
{
    ...
    anim_ctrl_rect.v_rect.top = MMITHEME_GetStatusBarHeight();
    anim_ctrl_rect.h_rect.top = MMITHEME_GetStatusBarHeight();
                                        ...
                                        )
{
...

    {
        ...
        {
            GUIAPICTRL_SetRect(MMIIDLE_WALLPAPER_ANIM_CTRL_ID, &both_rect.h_rect);
        }
        else
        {
    ...
    case MMISET_WALLPAPER_FIXED_PIC:
        ...
        //the unsigned value always >=0,mark for coverity scan
...
/*****************************************************************************/
//  Description : get wallpaper width and height
//  Global resource dependence : 
//  Author: Roy
//  Note: 
/*****************************************************************************/
PUBLIC void MMIAPIIDLE_GetWallpaperWidthHeight(
                                             MMIIDLE_WALLPAPER_WIDTH_HEIGHT_T     *width_height_ptr
                                             )
{
    GetWallpaperWidthHeight(width_height_ptr);
}
                                        ...
                                        )
{
    ...
    case MMISET_WALLPAPER_FIXED_PIC:
        ...
        //if (wallpaper_info.wallpaper_id >= MMISET_WALLPAPER_DEFAULT_ID)/*lint !e685 !e568*/
        {
            ...
            data_info.img_id = MMIAPISET_GetKLBG();        
        #else    
        	 data_info.img_id = IMAGE_WALLPAPER_1  ;// MMIAPISET_GetKLBG();
        #endif	 
           // data_info.img_id = MMIAPISET_GetWallpaperByIndex(wallpaper_info.wallpaper_id);
            anim_result = GUIANIM_SetParam(&control_info,&data_info,PNULL,&display_info);
            if ((GUIANIM_RESULT_SUCC == anim_result) ||
