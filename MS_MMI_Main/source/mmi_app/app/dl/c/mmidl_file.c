...
#define MMIDL_DEFAULT_OTHER_DIR_LEN  6//in the unit of uint8
...
const wchar g_mmidl_default_other_dir_str[] = { 'O', 't', 'h', 'e', 'r', 's', 0 };//other
                                            ...
                                            )
{
    ...
    // Note: download path replacer by default path, if customized path include sdcard but device didn't input sdcard.  
    if (MMI_DEVICE_UDISK == MMIAPIFMM_GetFirstValidDevice() && MMI_DEVICE_UDISK < MMIAPIFMM_GetDeviceTypeByPath(full_path_ptr , MMIFILE_DEVICE_LEN))
    {
        MMIDL_GetDefaultDownloadPath(type, full_path_ptr, full_path_len_ptr);
    }        
