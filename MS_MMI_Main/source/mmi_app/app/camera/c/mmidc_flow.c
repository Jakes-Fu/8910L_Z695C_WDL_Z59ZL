...
static MMIDC_IMAGE_INFO_T			*s_dc_info_ptr = PNULL;
static BOOLEAN					s_is_cancel_multishot = FALSE;
static BOOLEAN					s_is_snapshot_succss = TRUE;
static BOOLEAN					s_is_review_succss = TRUE;/*lint -esym(551,s_is_review_succss) */
                              ...
                              )
{
    ...
    s_dc_info_ptr->image_len = data_len;       
    s_dc_info_ptr->image_addr = (uint8*)data_buffer;
...
LOCAL uint32 CaptureCallBack(uint32 ret, uint16* data_buffer, uint32 data_len)
{
    ...
    do
    {
        ...
        s_photo_capture_info.photo_validate[s_photo_capture_info.captured_photo_num] = TRUE;
...
LOCAL void GetLastFrameCallback(DRECORDER_VIDEO_FRAME_T *p_lastframe)
{
	...
	// Bug 1351848, DV review时，将camera角度设置90度，得到全屏review的效果
    camera_angle = DCAMERA_ROTATION_90; 
    ...
    SCI_TRACE_LOW("[MMIDC] p_lastframe width = %d, height = %d",p_lastframe->width, p_lastframe->height);
    ...
    lcd_buffer_size = (uint32)(lcd_width * lcd_height) * sizeof(GUI_COLOR_T);
    ...
    if (PNULL != src_angle_ptr)
    {
            ...
            #endif
        #endif
#if 0 // don't show waiting bar in DV review
...
LOCAL BOOLEAN StartRecord(void)
{
	...
	// Mocor20B原生video摄像格式仅支持 AUDIO_TYPE_IMAADPCM
    ...
    video_info.width = size_val.size_width;
    video_info.height = size_val.size_height;
...
LOCAL void MMIDC_CopyLastFrame(void)
{
    ...
    buffer_size = (uint32)(screen_width * screen_height) * sizeof(GUI_COLOR_T);
...
#ifdef WIN32
        SCI_MEM16SET((uint16 *)s_lastframe_ptr, MMI_BLACK_COLOR, buffer_size);
#endif
    }
    else
    {
        SCI_MEM16SET((uint16 *)main_buffer_ptr, MMI_BLACK_COLOR, buffer_size);
    }
}
#endif
...
LOCAL BOOLEAN Capture(void)
{
    ...
    s_dc_current_flow_mode = DC_CAPTURE_MODE;
...
LOCAL BOOLEAN Review(void)
{
    ...
    {
        s_dc_current_flow_mode = DC_REVIEW_MODE;
...
#if 0 // Mocor20B auto save为开启，capture保存后不review
    MMIDC_ReviewPhotos();
#endif
    return TRUE;
}
...
{   
    s_dc_current_flow_mode = DV_REVIEW_MODE;
    StopRecord();
    StopDVPreview();
    CloseDV();
...
LOCAL BOOLEAN BackDCPreview(void)
{
    ...
    s_dc_current_flow_mode = DC_PREVIEW_MODE;
...
LOCAL BOOLEAN CancelRecord(void)
{
    ...
    s_dc_current_flow_mode = DV_PREVIEW_MODE;
    StopRecord();
    StopDVPreview();
    CloseDV();
...
LOCAL BOOLEAN Pause(void)
{
    ...
    s_dc_current_flow_mode = DV_PAUSE_MODE;
    ...
    MMIDC_DisplayZoomButton();
#endif
	// MMIDC_ClearOSDBlock(); // 先清除一次屏幕上的功能表，避免重迭的现象
    MMIDC_DisplaySoftKey(mode);
    MMIDC_CompleteDisplayOSD();
...
LOCAL BOOLEAN Resume(void)
{
    ...
    s_dc_current_flow_mode = DV_RECORD_MODE;

#if defined(MMIDC_DESKTOP_ICON_BAR) && defined(MMIDC_F_DV_ZOOM)
    MMIDC_DisplayZoomButton();
#endif
	MMIDC_ClearOSDBlock(); // Bug 1365995, 先清除一次屏幕上的功能表，避免重迭的现象
    mode = MMIDC_GetCurrentMode();
    MMIDC_DisplaySoftKey(mode);
    MMIDC_CompleteDisplayOSD();
...
//     SCI_MEMSET(&s_dc_info, 0, sizeof(s_dc_info));
	...
	{
		s_dc_info_ptr = (MMIDC_IMAGE_INFO_T*)SCI_ALLOC_APPZ(sizeof(MMIDC_IMAGE_INFO_T));
		if (PNULL == s_dc_info_ptr)
		{
			return;
		}
	...
	{
		if (PNULL == s_dc_info_ptr->file_list[index].file_name_ptr)
		{
			s_dc_info_ptr->file_list[index].file_name_ptr = (wchar*)SCI_ALLOC_APPZ((sizeof(wchar)) * (MMIFILE_FULL_PATH_MAX_LEN + 1));
			if (PNULL == s_dc_info_ptr->file_list[index].file_name_ptr)
			{
				return;
			}
...
PUBLIC void MMIDC_FreeDCImageInfoMemory(void)
{
	...
	if(PNULL != s_dc_info_ptr)
	{
		for (index = 0; index < MAX_DC_BURST_PHOTO_NUMBER; index ++)
		{
			if (PNULL != s_dc_info_ptr->file_list[index].file_name_ptr)
			{
				SCI_FREE(s_dc_info_ptr->file_list[index].file_name_ptr);
				s_dc_info_ptr->file_list[index].file_name_ptr = PNULL;
			}
		}
		
		SCI_FREE(s_dc_info_ptr);
		s_dc_info_ptr = PNULL;		
	}
}
/*****************************************************************************/
