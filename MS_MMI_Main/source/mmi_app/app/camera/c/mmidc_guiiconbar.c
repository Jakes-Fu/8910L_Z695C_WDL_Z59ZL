                              ...
                              )
{
    ...
    *out_rect_ptr = start_rect;    
    ...
    //bug1271625 获取首选的存储后，需要判断当前存储的状态，ex:sd卡拔除，则不应该显示sd卡的图标的
    if( FALSE == MMIAPIFMM_GetDeviceTypeStatus(dc_dev))
    {
        SCI_TRACE_LOW("DisplayIconStore:cur dc_dev:%d,is Not ok",dc_dev);
        dc_dev = MMI_DEVICE_UDISK;
    }
    else
    {
        SCI_TRACE_LOW("DisplayIconStore:cur dc_dev:%d,is ok",dc_dev);
    }
...
// Mocor20B默认存储路径为SDCARD，且不可修改，故屏蔽此快捷菜单
#if 0
    //display store img
    *second_tag_left_ptr = *second_tag_right_ptr + second_space;
