...
/*****************************************************************************/
// 	Description : get photo full path ,to provide for autotest_device
//	Global resource dependence : none
//  Author: chunjuanliang.rda
//	Note:this API is used,first capture photo,and enter autotest_device mode to test
/*****************************************************************************/
PUBLIC uint16 MMIDC_GetphotoFullPath(wchar* photofullpath)
{
     if(PNULL == s_save_data_ptr)
    {
        return 0;
    ...
    {
        if(photofullpath != PNULL)
        {
            MMIAPIFMM_CombineFullPathEx(photofullpath,255,(s_save_data_ptr->photo_info[0].file_info.file_dev),MMIDC_GetPhotoDefaultPathName(),(s_save_data_ptr->photo_info[0].ucs2_name));
            return MMIAPICOM_Wstrlen(photofullpath);
        }
        return 0;
    }
}
...
#else
    MMIAPIVP_PlayVideoFromVideoPlayer(VP_ENTRY_FROM_DV,MMIDC_SAVE_DATA_PTR->video_full_path_arr,MMIAPICOM_Wstrlen(MMIDC_SAVE_DATA_PTR->video_full_path_arr),PNULL);//bug1279598
#endif
#endif
}
