...
//	Note:
/*****************************************************************************/
LOCAL void Set1200X1600(void)
{
    SetPhotoSizeState(PHOTO_SIZE_1200X1600);
}
                                      ...
                                      )
{
    ...
    {   
        index = 0;
        for (i = max_photo_size; i >= PHOTO_SIZE_480X640; i--)
        {
...
#ifdef VIDEO_PLAYER_SUPPORT
    osd_menu->MenuSetItemCallback(2, MMIDC_ReviewVideo);
    osd_menu->MenuSetItemCallback(3, OpenMyVideo);    
    osd_menu->MenuSetDisplayNumber(4);
#else
    osd_menu->MenuSetItemCallback(2, OpenMyVideo);    
    osd_menu->MenuSetDisplayNumber(3);
#endif
...
PUBLIC BOOLEAN MMIDC_HandleAdjustTP(int16 x, int16 y)
{
    ...
    if(GUI_PointIsInRect(point, s_adjust_data.rect))
    {
        ...
        else
        {
            ...
            {
                step =  (icon_height < 5 ? space : (space / ((icon_height + 1) / 6) + 1));
            }
