...
typedef enum
{
	MMIAPIDC_OPENMODE_PHOTO = 0,
	//MMIAPIDC_OPENMODE_PHOTOGALLERY,
	MMIAPIDC_OPENMODE_CHIPTEST,
	MMIAPIDC_OPENMODE_VIDEO,
#if defined(MMIDC_F_U_CAMERA)
	MMIAPIDC_OPENMODE_UCAMERAPHOTO,
#endif    
	MMIAPIDC_OPENMODE_MAX
} MMIAPIDC_OPENMODE_E;
...
LOCAL MMIAPIDC_OPENMODE_E s_open_mode = MMIAPIDC_OPENMODE_PHOTO;

//flag for wait record ring play. if waiting it, flag is true.
LOCAL BOOLEAN bWaitRingPlay = FALSE;
/*---------------------------------------------------------------------------*/
...
/*****************************************************************************/
// 	Description : open close app confirmation dialog
//	Global resource dependence : none
//  Author: fulu.song
//	Note: 
/*****************************************************************************/
LOCAL void DisplayCloseAPPConfirmDialog(MMI_TEXT_ID_T text_id);
...
LOCAL void StartRecordTimer(void)
{
...
#ifndef WIN32
	// Bug 1350549, 避免跳秒现象，将调用 DRECORDER_GetRecordInfo 的频率调快
	time_out = MMI_200MSECONDS;
#else
    ...
    if(0 == s_record_timer)
    {
        s_record_timer = MMK_CreateTimer(time_out, TRUE);
    }
    else
    {
...
LOCAL BOOLEAN CheckDCRunningEnvironment(void)
{
	...
	//for 8910 low mem model
	//disable dc and gallar exist in same time for memory not enough reason
	MMIPICVIEW_Destroy();
...
#endif
#endif
	//when enter camera , check whether fm & music is running at first.
#if defined(FM_SUPPORT) && !defined(WIN32)
	if(MMIAPIFM_IsFmPlaying())
	{
		//FM active suspend or play background
		DisplayCloseAPPConfirmDialog(STR_COM_TURN_BGD_RADIO);
		return FALSE;
	}
	else 
#endif
	if(MMIAPIMP3_IsPlayerPlaying())
	{
		...
		DisplayCloseAPPConfirmDialog(STR_COM_TURN_BGD_MUSIC);
		return FALSE;
	}
	else
	{   
		//未播放状态也有可能存在内存中。此时不提示，直接调用关闭函数。
		//关闭FM
		MMIAPIFM_Exit();
		//关闭MP3
...
PUBLIC BOOLEAN MMIAPIDC_OpenPhotoWin(void)
{
    ...
    s_open_mode = MMIAPIDC_OPENMODE_PHOTO;
...
PUBLIC BOOLEAN MMIAPIDC_OpenUCameraPhotoWin(void)
{
    ...
    work_param.dv.max_video_size = WORK_VIDEO_SIZE_MAX;
    work_param.dv.disp_mode = DRECORDER_DISP_MODE_MAX;
    s_open_mode = MMIAPIDC_OPENMODE_UCAMERAPHOTO;
    ret = MMIAPIDCSRV_OpenMainWin(&work_param);
...
PUBLIC BOOLEAN MMIAPIDC_OpenForChipTest(void)
{
	...
	work_param.dv.max_video_size = WORK_VIDEO_SIZE_MAX;
	work_param.dv.disp_mode = DRECORDER_DISP_MODE_MAX;
	s_open_mode = MMIAPIDC_OPENMODE_CHIPTEST;
	ret = MMIAPIDCSRV_OpenMainWin(&work_param);
...
{	
    ...
    work_param.dv.max_video_size = WORK_VIDEO_SIZE_MAX;
    work_param.dv.disp_mode = DRECORDER_DISP_MODE_MAX;
    s_open_mode = MMIAPIDC_OPENMODE_VIDEO;
    ret = MMIAPIDCSRV_OpenMainWin(&work_param);
...
/*****************************************************************************/
// 	Description : start record video
//	Global resource dependence : none
//  Author: ryan.xu
//	Note:
/*****************************************************************************/
LOCAL void StartRecord(void)
{
...
#endif
        StartRecordTimer();
  //zhuyongjing add for gallary ui record video when no item 20150713
#ifdef PIC_VIEWER_SUPPORT
...
/*****************************************************************************/
// 	Description : send recorder message
//	Global resource dependence : none
//  Author: ryan.xu
//	Note:
/*****************************************************************************/
LOCAL void SendRecordMessage(MMISRVAUD_REPORT_RESULT_E result, DPARAM param)
{
    SCI_TRACE_LOW("lzk log. SendRecordMessage.");
#if 1
    //MMK_PostMsg(MMIDC_MAIN_WIN_ID, MSG_PLAY_RING_END, NULL, 0);
    MMK_SendMsg(MMIDC_MAIN_WIN_ID, MSG_PLAY_RING_END, PNULL);
    bWaitRingPlay = FALSE;
#else
    StartRecord();
#endif
...
LOCAL void Record(void)
{
    if (bWaitRingPlay)
    {
        //waiting ring. do nothing
        SCI_TRACE_LOW("lzk log. Record. bWaitRingPlay = TRUE");
        return;
    }

    if(SHUTTER_VOICE_DISABLE != MMIDC_GetVideoShutterVoice())
    {
        bWaitRingPlay = TRUE;
        SCI_TRACE_LOW("lzk log. PlayRing Start.");
        MMIAPISET_PlayRing((MN_DUAL_SYS_E)0, FALSE, 0, 1, MMISET_RING_TYPE_DC_REC_START, SendRecordMessage);
        SCI_TRACE_LOW("lzk log. PlayRing End.");
        //SCI_Sleep(800);
    }
    else
    {
...
#if defined(MMIDC_DESKTOP_PDA)
{
 ...
 #if 1//defined(MMIDC_DESKTOP_PDA)
...
LOCAL void ResumeCallBack(MMISRVAUD_REPORT_RESULT_E result, DPARAM param)
{
    SCI_TRACE_LOW("lzk log.ResumeCallBack");
...
LOCAL void PauseOrResume(void)
{
    ...
    if (s_is_dv_pause)
    {
        if (bWaitRingPlay)
        {
            SCI_TRACE_LOW("lzk log. bWaitRingPlay = true.");
            return;
        }  
        ...
        {
            bWaitRingPlay = TRUE;
            SCI_TRACE_LOW("lzk log. resume playring");
            MMIAPISET_PlayRing((MN_DUAL_SYS_E)0, FALSE, 0, 1, MMISET_RING_TYPE_DC_REC_START, ResumeCallBack);
            //SCI_Sleep(500);
        }
        else
        {
            bWaitRingPlay = FALSE;
            SCI_TRACE_LOW("lzk log. resume playnoring");
            ResumeCallBack(0, 0);
            s_is_dv_pause = FALSE;
        }
    }
    else
    {
        ...
        if(SHUTTER_VOICE_DISABLE != MMIDC_GetVideoShutterVoice()
            ...
            && (FALSE == MMIAPISMSCB_IsETWSAlert() )
#endif            
            )
        {
            MMIAPISET_PlayRing((MN_DUAL_SYS_E)0, FALSE, 0, 1, MMISET_RING_TYPE_DC_REC_STOP, PNULL);
        }
    }
...
PUBLIC void MMIDC_HandleLoseFocus(void)
{
	...
	////CMCC入库测试修改
	if (!MMIAPICC_IsInState(CC_MT_CONNECTING_STATE) && MMIAPIDC_IsRecording())
	{
		if(!s_is_dv_pause)
		{
			PauseOrResume();
                                      ...
                                      )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        //bug1246143 if torch is on,close it
        SCI_TRACE_LOW("[MMIDC]: close torch!");
#ifndef WIN32
        GPIO_SetTorch(FALSE);
#endif
        MMIAPICOM_StatusAreaSetTorchState(FALSE);
#endif
		// Bug 1379399, 避免当前开启的banner影响OSD菜单显示，进入camera后关闭banner
...
#ifdef MMI_ETWS_SUPPORT        
        if( MMIAPISMSCB_GetIsETWSDisplay())
        {
            break;
        }
#endif
        ...
        if(MMK_IsFocusWin(win_id))
        {
...

			{
...
#endif
                {
                    ...
                    if (MMIDC_GetIsAutoSave() == AUTO_SAVE_ON || s_is_pressed_save
#if defined(MMIDC_F_U_CAMERA)
                        || (MMIDC_IsUCameraEnable() && !MMIDC_IsPhotoData())
#endif
                        )
                    {
                        MMIDC_SetCurrentMode(DC_REVIEW_MODE);
    ...
    case MSG_PROMPTWIN_OK:
        if(MMIDC_GetCameraMode() == CAMERA_MODE_DC)
        {
			// Mocor20B auto save为开启，因此DC mode下POPWIN回传的MSG_PROMPTWIN_OK事件不需删除照片
            // MMIDC_DeletePhoto();
    ...
    case MSG_APP_OK:
        ...
        else
        {
            ...
            {
                KeyFunction(DC_LEFT_SOFT_KEY);
#if defined(MMIDC_F_WORK_MODE) //1272384
                if (MMIDC_IsWorkMode())
                {   
                    MMIDC_SaveWorkFile();
                }
...
LOCAL MMI_RESULT_E HandleCloseAPPConfirmMsg
(
	MMI_WIN_ID_T		win_id,
	MMI_MESSAGE_ID_E	msg_id,
	DPARAM				param
)
{
	...
	SCI_TRACE_LOW("[MMIDC] line[%d] HandleConfirmationDialogMsg: msg_id=0x%x", msg_id);
	switch (msg_id)
	{
	case MSG_OPEN_WINDOW:
		//pubwin_info_ptr = MMIPUB_GetWinAddDataPtr(win_id);
	...
	case MSG_CTL_MIDSK:
		...
		switch(s_open_mode)
		{
		case MMIAPIDC_OPENMODE_PHOTO:
			MMIAPIDC_OpenPhotoWin();
			break;
#if 0
		case MMIAPIDC_OPENMODE_PHOTOGALLERY: 
			MMIAPIDC_OpenPhotoWinForGallery();
			break;
#endif
		case MMIAPIDC_OPENMODE_VIDEO:
			MMIAPIDC_OpenVideoWin();
			break;
		case MMIAPIDC_OPENMODE_CHIPTEST:
			MMIAPIDC_OpenForChipTest();
			break;
#if defined(MMIDC_F_U_CAMERA)
		case MMIAPIDC_OPENMODE_UCAMERAPHOTO:
...
LOCAL void DisplayCloseAPPConfirmDialog(MMI_TEXT_ID_T text_id)
{
	SCI_TRACE_LOW("[MMIDC] line[%d] DisplayCloseAPPConfirmDialog");
...
PUBLIC void MMIAPIDC_Exit(void)
{
    ...
    if( TRUE == MMIPICVIEW_GetIsGoToCamera())
    {
        SCI_Sleep(500);
        MMK_CreateWin((uint32 *)MMIPICVIEW_LIST_WIN_TAB,PNULL); 
        MMIPICVIEW_SetIsGoToCamera(FALSE);
    }
...
LOCAL void HandleShortcutKey(MMI_MESSAGE_ID_E msg_id)
{
    ...
    case MSG_APP_6:
        ...
        break;
    case MSG_APP_7:
		// mocor20b只支持SD卡存储，故屏蔽快捷键7切换存储位置
...
LOCAL void CreateDCWaitWin(CAMERA_MODE_E mode)
{
    ...
    win_table.add_data_ptr = (void*)mode;
...
PUBLIC BOOLEAN MMIAPIDCSRV_OpenMainWin(MMIDCSRV_WORK_PARAM_T *work_param_ptr /*in*/)
{
	...
	SCI_TRACE_LOW("[MMIDCSRV] MMIAPIDCSRV_OpenMainWin ret=%d", ret);
