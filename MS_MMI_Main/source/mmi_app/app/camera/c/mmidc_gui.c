...
LOCAL void MenuFocusNextItem(void)
{
    ...
    if(callback != PNULL && !s_menu_data->_is_not_callback_focus)
    {	
		// bug1337166, 上下移动选单时，不直接操作闪光灯(按下ok键才操作闪光灯)
#ifdef MMIDC_F_FLASH
		if (callback != SetFlashOn && callback != SetFlashOff && callback != SetFlashAuto)
		{
			callback();
		}
#endif
    ...
    if(is_display_arrow)
    {
        ...
        if (is_display_arrow_top) /*lint !e774*/
        {
            ...
            else
            {
                ...
                {
                   MMIDC_DisplayBmp(68, 67, IMG_DC_MENU_DOWN);
                }
                else
                {
            ...
            else
            {
                ...
                if (CAMERA_MODE_DV == mode)
                {
                   MMIDC_DisplayBmp(68, 2, IMG_DC_MENU_UP);;
                }
                else
                {
