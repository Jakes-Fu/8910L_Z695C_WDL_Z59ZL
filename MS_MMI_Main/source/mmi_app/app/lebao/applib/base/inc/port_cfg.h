 ...
 *********************/
	...
	extern int helper_log_printf(const char* fmt, ...);	// D:\Others\_lebao_log.txt
#else
	#include "os_api.h"
...
#else
#define os_thread_sleep os_thread_sleep_lebao
#endif
#endif
...
#define  CFG_SPACE_MIN_SIZE			(32 * 1024)
#else
#define  CFG_SPACE_MIN_SIZE			(64 * 1024)
#endif
...
#define  CFG_MP3_CHUNK_SIZE			(8 * 1024)
...
#define helper_trace(_format,...)	lebao_helper_log_printf("[%s]<%d>"_format, __FUNCTION__, __LINE__, ##__VA_ARGS__)
extern void lebao_helper_log_printf(const char* fmt, ...);
