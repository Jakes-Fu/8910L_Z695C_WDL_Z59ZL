                                             ...
                                             )
{
    ...
    GUI_COLOR_T                   font_color = 0xffff; //MMI_WHITE_COLOR
    switch (msg_id)
    {
    case MSG_OPEN_WINDOW:
        //display incoming file info
		s_is_cancelled = FALSE;
	GUIWIN_SetSoftkeyTextId(win_id,STXT_SOFTKEY_ACCEPT_MK,NULL,STXT_SOFTKEY_CANCEL_MK,TRUE);


        //set text to content_str
        GetIncomingFileInfo(win_id, &win_param_ptr->transfer_file_info,&content_str);
        //display text
        text_backgrand.bg_type = GUI_BG_COLOR; //lulifei add for the text style
        text_backgrand.color = MMI_BLACK_COLOR;
        GUITEXT_SetBg(MMIBT_TEXT_BOX_NEW_FILE_DETAIL_CTRL_ID, &text_backgrand);
        GUITEXT_SetFont(MMIBT_TEXT_BOX_NEW_FILE_DETAIL_CTRL_ID, &font, &font_color);
...
LOCAL void GetIncomingFileInfo(MMI_WIN_ID_T win_id, MMIBT_TRANSFER_FILE_INFO_T* file_info, MMI_STRING_T *info_str)
{
    ...
    if(PNULL != info_str)
    {
        ...
        //new file: file name
        MMI_GetLabelTextByLang(STR_BT_FILE_NAME, &text_str);
        ...
        info_str->wstr_len += 2;
        //file size :xxxK
        MMI_GetLabelTextByLang(STR_BT_FILE_SIZE, &text_str);
...
#if 0
        //aceept or not
        MMI_GetLabelTextByLang(STXT_SOFTKEY_ACCEPT_MK, &text_str);
