...
PUBLIC void MMIAPIBT_SpecifyHeadset(MMIBT_HEADSET_TYPE headset_type)
{
    ...
    //lulifei modify for the coverity 20151003
         result = MMIAPIBT_GetActiveBTHeadset();
}
...
//	Note:
/*****************************************************************************/
PUBLIC void AppendPdaBtItem1ToggleLine(
                                 MMI_CTRL_ID_T  ctrl_id,    //控件ID
                                 MMI_TEXT_ID_T  text_id,     //主文本
                                uint16           pos,
                                MMI_TEXT_ID_T    left_text_id,
                                BOOLEAN          is_update
                                 )
...
//  Return: 
/*****************************************************************************/
PUBLIC void MMIBT_OpenQueryWin(
                               MMI_WIN_ID_T             win_id,
...
PUBLIC void BtPauseBGPlay(MMIBT_SUSPEND_TYPE_E type)
{
    ...
    //if(type >= MMIBT_OPP_SUSPEND && type < MMIBT_SUSPEND_MAX) lulifei modify for Bug 499512
    if(type < MMIBT_SUSPEND_MAX)
    {
                                    ...
                                    )
{
    ...
    {
        MMIPUB_OpenAlertWinByTextId(&tm_dur,txt_id,NULL,img_id,PNULL,PNULL,MMIPUB_SOFTKEY_DIALOG_DISMISS,PNULL);
    }
    if(MMK_IsOpenWin(win_id))
    {
        MMK_CloseWin(win_id);
...
{   
    ...
    SCI_TRACE_LOW("[MMIBT]MMIBT_GetPinRequestInfo: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x", 
        pin_request_info.addr.addr[0],
        pin_request_info.addr.addr[1],
        pin_request_info.addr.addr[2],
        pin_request_info.addr.addr[3],
        pin_request_info.addr.addr[4],
        pin_request_info.addr.addr[5]);
...
PUBLIC void MMIBT_UpdateFtpSharedFolder(BT_MSG_T* msg_body_ptr)
{
    ...
    SCI_MEMSET(setting_ptr, 0x00, sizeof(MMIBT_USER_SETTING_U));
...
//  Return:
/*****************************************************************************/
PUBLIC BOOLEAN MMIBT_GetPairedPBAPDevList(uint8  *paired_index_ptr)
{
    uint16     i =0;
    uint16     device_number =0;
    BT_STATUS  status = 0;
    ...
    SCI_TRACE_LOW("[MMIBT] MMIBT_GetPairedPBAPDevList Entry");    
    if (PNULL == paired_index_ptr)
    {
        //SCI_TRACE_LOW:"MMIBT_GetPairedDevList PNULL == paired_dev_ptr!"
        SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMIBT_WINTAB_7573_112_2_18_2_8_15_290,(uint8*)"");
    ...
    SCI_TRACE_LOW("[MMIBT] MMIBT_GetPairedPBAPDevList device_number = %d", device_number);

    for (i = 1; i <= device_number; i++)
    {
        SCI_MEMSET(&dev_info, 0x00, sizeof(dev_info));
        status = BT_GetPairedDeviceInfo(BT_SERVICE_PBAP_SERVER, i, &dev_info);
        if(BT_SUCCESS != status)
        {
            continue;
        }
        else if((BT_SERVICE_PBAP_SERVER) & BT_GetPairedDeviceService(&(dev_info.addr)))
        {
            *paired_index_ptr = i;
            paired_index_ptr ++;
        }
    ...
    SCI_TRACE_LOW("[MMIBT] MMIBT_GetPairedPBAPDevList paired_index_ptr = %d", *paired_index_ptr);
                                      ...
                                      BOOLEAN   is_visible   )
{
    ...
    if ( is_power_on )
    {
        ...
        if(MMIBT_OpenedHeadsetIsExist(PNULL)
            || MMIBT_IsExitedActiveA2dp())
        {
            bt_status = ON_AND_HEADSETCONNECT;
        }
    }
    else
    {
    ...
    MMIAPICOM_StatusAreaSetBlueToothState(bt_status);
}
/*****************************************************************************/
...
{   
    ...
    MMIAPICOM_StatusAreaSetBTStateWhenPoweron(bt_status);
}
/*****************************************************************************/
                                        ...
                                        )
{
     if(MMIBT_IsPhoneReady())
     {
        ...
        //if pair wait win open, then close
        if(MMK_IsOpenWin(MMIBT_PAIR_WAIT_WIN_ID))
        {
            MMK_CloseWin(MMIBT_PAIR_WAIT_WIN_ID);
        }
        if(MMK_IsOpenWin(MMIBT_NUMERIC_PASSKEY_WIN_ID))
        {
...
{ 
    ...
    else
    {
        ...
        else
        {
            ...
            MMIPUB_OpenAlertWinByTextId(PNULL,STR_BT_SEND_FAIL,TXT_NULL,IMAGE_PUBWIN_FAIL,PNULL,PNULL,MMIPUB_SOFTKEY_DIALOG_OK_CLOSE,PNULL);
...
/*****************************************************************************/
//  Discription: set bt call type WB or NB
//  Global resource dependence: None
//  Author: weipeng.wang 
//  Note : 
//  Return:
/*****************************************************************************/
LOCAL void MMIBT_SetCallType(BT_MSG_T *msg_body_ptr)
{
    BT_HFG_WB_FLAG btHfgWB = {0};
    SCI_TRACE_LOW("wangwp--MMIBT_SetCallType: Enter");

    if(PNULL != msg_body_ptr){
        btHfgWB.is_wb_call_flag = (BOOLEAN)(((BT_HFG_WB_FLAG *)msg_body_ptr->body_ptr)->is_wb_call_flag);
        btHfgWB.is_nrec_flag = (BOOLEAN)(((BT_HFG_WB_FLAG *)msg_body_ptr->body_ptr)->is_nrec_flag);
        //SCI_MEMCPY(&btHfgWB, ((BT_HFG_WB_FLAG *)msg_body_ptr->body_ptr), sizeof(BT_HFG_WB_FLAG));
    }
    SetCallTypeEventFlag(TRUE);
    ARMVB_SetBtWorkMode_Direct(&btHfgWB);//add by ljf
    SCI_TRACE_LOW("wangwp--MMIBT_SetCallType: is_wb_call_flag = %d, is_nrec_flag=%d, s_WBNBInd=%d", 
        btHfgWB.is_wb_call_flag, btHfgWB.is_nrec_flag, GetCallTypeEventFlag());
}

LOCAL void HandleStartReopenTimer(uint8  timer_id, uint32 param)
{
    SCI_TRACE_LOW("MMIBT: HandleStartReopenTimer: %d, %d", BT_IsEnabled(),s_bt_reopen_wait_indx);

    if (0 != s_reopen_bt_timer)
    {
        MMK_StopTimer(s_reopen_bt_timer);
        s_reopen_bt_timer = 0;
    }

    if (!BT_IsEnabled())
    {
        MMIBT_SendSignal(MMI_BT_REOPEN_ACTION_IND,&s_bt_reopen_wait_indx);
        s_bt_reopen_wait_indx = 0;//clear it
    }
    else
    {
        s_bt_reopen_wait_indx++;
        if(s_bt_reopen_wait_indx < 20)
        {
            s_reopen_bt_timer = MMK_CreateTimerCallback(100, HandleStartReopenTimer, PNULL, FALSE);
        }
        else
        {
            //After 20*100ms, the BT still cannot response, so we force to power off it.
            MMIBT_SendSignal(MMI_BT_REOPEN_ACTION_IND,&s_bt_reopen_wait_indx);
            s_bt_reopen_wait_indx = 0;//clear it
        }
    }
...
PUBLIC void MMIBT_CallBackFunc(DPARAM param)
{
	...
	case ID_STATUS_BT_OFF_RES:
       ...
       case ID_STATUS_HFAG_CALL_TYPE_WB_NB:
            SCI_TRACE_LOW("Bluetooth  ID_STATUS_HFAG_CALL_TYPE_WB_NB");
            msg_id = MMI_BT_CALL_TYPE_WB_NB_EVENT;
            MMIBT_SendSignal(msg_id,param);
        break;

      case ID_STATUS_BT_REOPEN_IND:
            //SCI_TRACE_LOW("Bluetooth  ID_STATUS_BT_REOPEN_IND");
            msg_id = MMI_BT_REOPEN_IND;
            MMIBT_SendSignal(msg_id,param);
        break;
...
//OPP Server Message begin (receive file)
	...
	case ID_STATUS_OPS_PUT_FINAL:
		...
		msg_id = MMI_BT_RECEIVE_FILE_RES;
        MMIBT_SendSignal(msg_id,param);	
		break;
...
//FTP Server Message end (receive file)			
    ...
    case ID_STATUS_PBAP_TP_DISCONNECTED:
        SCI_TRACE_LOW("[MMIBT_CallBackFunc] Bluetooth ID_STATUS_PBAP_TP_DISCONNECTED.\n");
        msg_id = MMI_BT_PBAP_TP_DISCONNECTED;
        MMIBT_SendSignal(msg_id,param);
        break;
        
    case ID_STATUS_PBAP_COMPLETE:
        SCI_TRACE_LOW("[MMIBT_CallBackFunc] Bluetooth ID_STATUS_PBAP_COMPLETE.\n");
        msg_id = MMI_BT_PBAP_COMPLETE;
        MMIBT_SendSignal(msg_id,param);
        break;
        
    case ID_STATUS_PBAP_ABORT:
        SCI_TRACE_LOW("[MMIBT_CallBackFunc] Bluetooth ID_STATUS_PBAP_ABORT.\n");
        msg_id = MMI_BT_PBAP_ABORT;
        MMIBT_SendSignal(msg_id,param);
        break;
        
    case ID_STATUS_PBAP_PARAMS_RX:
        SCI_TRACE_LOW("[MMIBT_CallBackFunc] Bluetooth ID_STATUS_PBAP_PARAMS_RX.\n");
        msg_id = MMI_BT_PBAP_PARAMS_RX;
        MMIBT_SendSignal(msg_id,param);
        break;
        
    case ID_STATUS_PBAP_DATA_IND:
        SCI_TRACE_LOW("[MMIBT_CallBackFunc] Bluetooth ID_STATUS_PBAP_DATA_IND.\n");
								  ...
								  )
{
...
#ifdef MMI_UNISOC_MANUALLY_CIT_SUPPORT
		if(MMK_IsOpenWin(MMIENG_UNISOCTEST_BT_WIN_ID))
		{
			MMK_PostMsg(MMIENG_UNISOCTEST_BT_WIN_ID, MMI_BT_ME_ON_CNF, (DPARAM)msg_body,sizeof(BT_MSG_T));
			return result;
		}
#endif
	...
	case MMI_BT_ME_OFF_CNF:
		...
		MMIBT_A2dpLinkDisconnect(NULL);
		break;

	case MMI_BT_CALL_TYPE_WB_NB_EVENT:
		SCI_TRACE_LOW("AppHandleBTMsg:MMI_BT_CALL_TYPE_WB_NB_EVENT");
		MMIBT_SetCallType(msg_body);
		break;

	case MMI_BT_REOPEN_IND:
		SCI_TRACE_LOW("AppHandleBTMsg:MMI_BT_REOPEN_IND 0x%x",((BT_NOTIFY_CONTROLLER_ADDR *)msg_body->body_ptr)->addr_value);
		if (BT_IsEnabled() && (0 == s_bt_reopen_wait_indx))
		{
                BT_MSG_T bt_app_msg;

                MMIBT_SetOpertor(MMIBT_OPC_NOT_CHANGE, MMIBT_OPC_POWER_ONOFF);
                //MMIBT_OpenBluetooth(FALSE, TRUE);//close BT
                MMIBT_OpenBluetooth_Ext(FALSE, TRUE);//close BT
                SCI_Sleep(300);//wait BT power off

                bt_app_msg.msg_id = 0;//we will NOT use this in case MMI_BT_REOPEN_ACTION_IND:
                bt_app_msg.status = 0;
                bt_app_msg.body_size = 0;
                bt_app_msg.body_ptr = NULL;
                MMIBT_SendSignal(MMI_BT_REOPEN_ACTION_IND,&bt_app_msg);
		}
		//s_reopen_bt_timer = MMK_CreateTimerCallback(100, HandleStartReopenTimer, PNULL, FALSE);
		break;

	case MMI_BT_REOPEN_ACTION_IND:
		SCI_Sleep(100);//wait BT power off
		SCI_TRACE_LOW("AppHandleBTMsg:MMI_BT_REOPEN_ACTION_IND");
		MMIBT_SetOpertor(MMIBT_OPC_MAIN, MMIBT_OPC_POWER_ONOFF);
		//MMIBT_OpenBluetooth(TRUE, TRUE);//open BT
		MMIBT_OpenBluetooth_Ext(TRUE, TRUE);//open BT
		break;
	...
	case MMI_BT_INQ_DEV_NAME:
		...
		if(MMK_IsOpenWin(MMIENG_UNISOCTEST_BT_WIN_ID))
		{
			MMK_PostMsg(MMIENG_UNISOCTEST_BT_WIN_ID, MMI_BT_INQ_DEV_NAME, (DPARAM)msg_body,sizeof(BT_MSG_T));
			break;
		}
#endif
	...
	case MMI_BT_INQ_COMP_CNF://搜索设备结束
		...
		if(MMK_IsOpenWin(MMIENG_UNISOCTEST_BT_WIN_ID))
		{
			MMK_PostMsg(MMIENG_UNISOCTEST_BT_WIN_ID, MMI_BT_INQ_COMP_CNF, PNULL,0);
			break;
		}
#endif
		MMIBT_GetInqCompCnf();
...
//receive file begin
    ...
    case MMI_BT_FTS_DISCONNECT_IND://手机和电脑连接(包含发送、接收)
      SCI_TRACE_LOW("MMI_BT_FTS_DISCONNECT_IND");
    ...
    case MMI_BT_OPS_DISCONNECT_IND: //手机接收文件
	  SCI_TRACE_LOW("MMI_BT_OPS_DISCONNECT_IND");
	  //SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMIBT_EXPORT_1383_112_2_18_2_7_50_103,(uint8*)"");
	  MMIBT_SetTransferStatus(MMIBT_TRANSFER_END);
	  MMIBT_UpdateMainMenu(TRUE);
	  break;
    case MMI_BT_OPC_DISCONNECT_IND://手机发送文件
        SCI_TRACE_LOW("MMI_BT_OPC_DISCONNECT_IND");
	...
	case MMI_BT_RECEIVE_FILE_RES:
		//SCI_TRACE_LOW:"MMI_BT_RECEIVE_FILE_RES"
		SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMIBT_EXPORT_1397_112_2_18_2_7_50_106,(uint8*)"");
		MMIBT_ReceiveFileResInd(msg_body);
		break;
#ifdef MMIBT_FTS_SUPPORT
...
//begin for a2dp message
	...
	case MMI_BT_A2DP_DISCONNECTED:
		...
		if(MMIAPICC_IsInState(CC_IN_CALL_STATE) && (!MMISRVAUD_IsAudPlaying(MMIAPICC_GetAudioHandle())))//1084655
		{
			SCI_TRACE_LOW("MMI_BT_A2DP_DISCONNECTED: Play the CC audio handle");
			MMISRVAUD_Play(MMIAPICC_GetAudioHandle(), 0);
		}
