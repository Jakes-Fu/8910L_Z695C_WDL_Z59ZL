...
LOCAL MMI_RESULT_E   HandleEditDeviceNameMsg(
 ...
 LOCAL MMI_RESULT_E HandleBTZeroNameWarningWinMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id,DPARAM param) ;
    ...
    case MSG_OPEN_WINDOW:
        {
            ...
            {
               text_id = STR_BT_PRMPT_NEW_NAME_EXT01;
            ...
            GUIWIN_SetTitleText(win_id, title.wstr_ptr, title.wstr_len, TRUE);
        ...
        if ( 0 < new_name.wstr_len  && new_name.wstr_len <= MMIBT_DEVICE_NAME_ALPHA_MAX_LEN)
        {
            ...
            {                
                if(edit_win_info->is_local_name)
                {
                 ...
                 MMIBT_AppendTwoLineTextIconSmallTextListItem(MMIBT_MAIN_LIST_CTRL_ID,STR_BT_SETT_RENAME_PHONE,IMAGE_COMMON_RIGHT_ARROW,new_name,BT_INDEX_NAME,TRUE);
#endif
                }
...
//#endif                
            ...
            else
            {
                ...
                MMIPUB_OpenConfirmationDialogByTextId(PNULL,TXT_ERROR,PNULL,
                                                          IMAGE_NULL,PNULL,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,HandleBTZeroNameWarningWinMsg);
            }
        }
#if 0
        else
        {
            ...
            MMIPUB_OpenConfirmationDialogByTextId(PNULL,STR_BT_NOTE_NAME_EMPTY_EXT01,PNULL,
                                                          IMAGE_NULL,PNULL,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,HandleBTZeroNameWarningWinMsg);
        }
        if(0 == new_name.wstr_len)
        {
        	MMIPUB_OpenConfirmationDialogByTextId(PNULL,STR_BT_NOTE_NAME_EMPTY_EXT01,PNULL,
                                                          IMAGE_NULL,PNULL,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,HandleBTZeroNameWarningWinMsg);
        }
    ...
    case MSG_CLOSE_WINDOW:
        ...
        break;
   //lulifei add for the name will be empty
   case MSG_FULL_PAINT:
	  {
...
LOCAL MMI_RESULT_E CreateEditWin(MMIBT_EDITWIN_PARAM_T *editwin_info)
{
    ...
    MMI_WIN_ID_T linkMenu_dialog_id = MMIBT_EDIT_DEVICE_NAME_WIN_ID;
    ...
    MMI_GetLabelTextByLang(STR_BT_PRMPT_NEW_NAME_EXT01, &temp_str);
    MMIPUB_OpenDialogInputFieldWinByTextPtr(&temp_str,&linkMenu_dialog_id,PNULL,MMIPUB_SOFTKEY_CUSTOMER,HandleEditDeviceNameMsg);
    MMK_SetWinUserData (linkMenu_dialog_id, (void *) param_ptr);
...
LOCAL MMI_RESULT_E HandleBTZeroNameWarningWinMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id,DPARAM param) {
    MMI_RESULT_E         result = MMI_RESULT_TRUE;
    uint16                   device_name[BT_DEVICE_NAME_SIZE] = {0};
    MMI_WIN_ID_T        win_edit_id = MMIBT_EDIT_DEVICE_NAME_WIN_ID;
	...
	break;
        case MSG_APP_WEB: {
	   if(MMK_IsOpenWin(MMIBT_EDIT_DEVICE_NAME_WIN_ID)){
            ctrl_id = MMIPUB_GetInputFieldDialogCtrlId(win_edit_id);
	  BT_GetLocalName(device_name);
             GUIEDIT_SetString(ctrl_id, device_name,MMIAPICOM_Wstrlen((wchar*)device_name) );
	   }
            MMK_CloseWin(win_id);
