                                                    ...
                                                    )
{
    ...
    //MMI_MENU_GROUP_ID_T     auto_group_id = MENU_BT_OTHER_DEVICE_DETAILS;
...
#endif
#if 0
         if(HEADSET_DISCONNECTED == win_param_ptr->func_type){
             auto_group_id = MENU_BT_DEACTIVE_DEVICE_DETAILS;
         }else if(HEADSET_CONNECTED == win_param_ptr->func_type){
            auto_group_id = MENU_BT_DEVICE_DETAILS;
         }else{
            auto_group_id = MENU_BT_OTHER_DEVICE_DETAILS;
	}
#endif
	...
	menu_id = ID_BT_AUTO_CONNECT_TOGGLE;
	pop_info.is_static = TRUE;
         pop_info.ctrl_id = ctrl_id;
         pop_info.menu_id = menu_id;
         pop_info.group_id = auto_group_id;
         GUIMENU_SetItemStateEx(&pop_info, GUIMENU_ITEM_STATE_TOGGLE, TRUE);
         if(be_authorize){
             GUIMENU_SetItemStateEx(&pop_info, GUIMENU_ITEM_STATE_TOGGLE_ON, TRUE);
         }else{
           GUIMENU_SetItemStateEx(&pop_info, GUIMENU_ITEM_STATE_TOGGLE_ON, FALSE);
         }
    ...
    case MSG_CTL_MIDSK:
        ...
        case ID_BT_DEVICE_DELETE:
            ...
            //if(BT_GetState())
            {
                MMI_GetLabelTextByLang(STR_BT_NOTE_REMOVE_PAIR_EXT01, &wait_text);
                MMIBT_OpenQueryWin(MMI_BT_QUERY_WIN_ID, &wait_text, PNULL, MMIBT_OPC_DEVICE_DELETE);
            }
            MMK_CloseWin(win_id);
            break;
	//lulifei add for UI menu spec begin
	case ID_BT_AUTO_CONNECT_TOGGLE:
	   BT_GetDeviceAuthorisation(&win_param_ptr->device_info.addr,&be_authorize);
	   SCI_TRACE_LOW("[settings] HandlePairedDeviceDetailsWinMsg:be_authorize %d ", be_authorize);
            if(be_authorize){
                BT_SetDeviceAuthorisation(&win_param_ptr->device_info.addr,FALSE);
            }else{
                BT_SetDeviceAuthorisation(&win_param_ptr->device_info.addr,TRUE);
         ...
         case ID_BT_SEATCH_DEVICE:
            ...
            break;
         //lulifei add for UI menu spec end
        case ID_BT_DEVICE_DELETE_ALL:
            ...
            //if(BT_GetState())
            {
                ...
                MMIBT_OpenQueryWin(MMI_BT_QUERY_WIN_ID, &wait_text, PNULL, MMIBT_OPC_DEVICE_DELETE_ALL);
            }
            break;
...
#endif
            {
                SCI_TRACE_LOW("[MMIBT] ID_BT_DEVICE_CONNECT with BT_DEVICE_CLASS_AUDIO");
                ...
                else
                {
                    ...
                    MMIBT_OpenQueryWin(MMI_BT_QUERY_WIN_ID, &wait_text, PNULL, MMIBT_OPC_OPEN_HEADSET);
                }
            }
#ifdef MMI_BT_PBAP_SUPPORT
            else if(dev_type & BT_DEVICE_CLASS_PHONE)
            {
                SCI_TRACE_LOW("[MMIBT] ID_BT_DEVICE_CONNECT with BT_DEVICE_CLASS_PHONE");
                MMIBT_SetOpertor(MMIBT_OPC_NOT_CHANGE, MMIBT_OPC_OPEN_HANDSET_GATEWAY);
                if(BT_GetState())
                {              
                    SCI_TRACE_LOW("[MMIBT] ID_BT_DEVICE_CONNECT send MMIBT_OPC_OPEN_HANDSET_GATEWAY");
                    DoOperation(MMIBT_OPC_OPEN_HANDSET_GATEWAY, TRUE);
                }
                else
                {
                    //blue tooth is power off
                    SCI_TRACE_LOW("[MMIBT] ID_BT_DEVICE_CONNECT query to MMIBT_OPC_OPEN_HANDSET_GATEWAY");
                    MMI_GetLabelTextByLang(TXT_TURN_ON_OR_NOT, &wait_text);
                    MMIBT_OpenQueryWin(MMI_BT_QUERY_WIN_ID, &wait_text, PNULL, MMIBT_OPC_OPEN_HANDSET_GATEWAY);
                }
            }
#endif
            MMK_CloseWin(win_id);
...
#endif
            {
                SCI_TRACE_LOW("[MMIBT] ID_BT_DEVICE_DISCONNECT with BT_DEVICE_CLASS_AUDIO");
            //headset connect
             ...
             if(BT_GetState())
             {
				 ...
				 MMI_GetLabelTextByLang(STR_SOFTKEY_BT_DISCONNECT_MK, &wait_text);
                 ...
                 MMIBT_OpenQueryWin(MMI_BT_QUERY_WIN_ID, &wait_text, &name_str, MMIBT_OPC_CLOSE_HEADSET);
             }
             else
             {
                 //blue tooth is power off
                 MMI_GetLabelTextByLang(STR_BT_TURN_ON, &wait_text);
                 MMIBT_OpenQueryWin(MMI_BT_QUERY_WIN_ID, &wait_text, PNULL, MMIBT_OPC_CLOSE_HEADSET);
             }
            }
#ifdef MMI_BT_PBAP_SUPPORT
            else if(dev_type & BT_DEVICE_CLASS_PHONE)
            {
                SCI_TRACE_LOW("[MMIBT] ID_BT_DEVICE_DISCONNECT with BT_DEVICE_CLASS_PHONE");
                ...
                if(BT_GetState())
                {
                    ...
                    name_str.wstr_ptr = bt_dev_info.name;
                    name_str.wstr_len = MMIAPICOM_Wstrlen(bt_dev_info.name);
                    MMIBT_OpenQueryWin(MMI_BT_QUERY_WIN_ID, &wait_text, &name_str, MMIBT_OPC_CLOSE_HANDSET_GATEWAY);
                }
                else
                {
                    //blue tooth is power off
                    MMI_GetLabelTextByLang(TXT_TURN_ON_OR_NOT, &wait_text);
                    MMIBT_OpenQueryWin(MMI_BT_QUERY_WIN_ID, &wait_text, PNULL, MMIBT_OPC_CLOSE_HANDSET_GATEWAY);
                }
            }
#endif
            MMK_CloseWin(win_id);
