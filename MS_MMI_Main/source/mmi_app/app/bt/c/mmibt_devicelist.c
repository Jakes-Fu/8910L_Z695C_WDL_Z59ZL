...
WINDOW_TABLE(MMIBT_DEVICE_LIST_WIN_TAB) =
{
    //CLEAR_LCD,
        ...
        WIN_SOFTKEY(STXT_SOFTKEY_OK_MK, STR_SOFTKEY_BT_DISCONNECT_MK, STXT_RETURN),
        END_WIN
                                              ...
                                              )
{
    ...
    SCI_TRACE_LOW("HandleDeviceListWinMsg msg_id =%d",msg_id);
    ...
    case MSG_CTL_OK:
        ...
        if(0 == cur_select_item)
        {
            ...
            else
            {
                ...
                MMIBT_OpenQueryWin(MMI_BT_QUERY_WIN_ID, &display_str, PNULL, MMIBT_OPC_SEARCH);
            }
        }
#ifdef MMI_PDA_SUPPORT
...
#ifdef MMI_BT_PBAP_SUPPORT
                //selected this device to do operation
                if(MMIBT_OPC_LIST_PAIRED_PBAP == win_param_ptr->root_opertion)
                {
                    BOOLEAN                is_connect = FALSE;
                    uint16                 device_type = 0;
                    MMI_MENU_GROUP_ID_T group_id = MENU_BT_OTHER_DEVICE_DETAILS;
                    ...
                    )
{
    ...
    {
        left_softkey_id = STXT_SOFTKEY_SELECT_MK;
    }
...
#else
    MMIAPISET_AppendListItemByTextId(TXT_BT_ADD_DEVICE, TXT_NULL, ctrl_id, GUIITEM_STYLE_ONE_LINE_TEXT );
#endif
    SCI_MEMSET(paired_device_index,0,sizeof(paired_device_index));
