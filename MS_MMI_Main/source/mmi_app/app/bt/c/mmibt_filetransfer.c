...
//
/*****************************************************************************/
LOCAL MMI_RESULT_E  HandleSharingFileProgressbarDialogWindow(
												  MMI_WIN_ID_T     win_id,
												  MMI_MESSAGE_ID_E msg_id,
												  DPARAM           param
												  );
...
WINDOW_TABLE(MMIBT_SENDING_FILE_WAIT_WIN_TAB) = 
{
    ...
    WIN_SOFTKEY(TXT_NULL, TXT_NULL, STXT_SOFTKEY_CANCEL_MK),
    END_WIN
};
                                             ...
                                             )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        GUIWIN_SetSoftkeyTextId(win_id,  (MMI_TEXT_ID_T)TXT_NULL, (MMI_TEXT_ID_T)TXT_NULL, STXT_SOFTKEY_CANCEL_MK, FALSE);
#ifdef PDA_UI_DROPDOWN_WIN
                                                ...
                                                uint32     total_file_size)
{
    MMIBT_FILE_TRANSFER_PARAM_T param_info = {0};
         ...
         SCI_TRACE_LOW("MMIBT_EnterFileTransferWin");
...
LOCAL  MMI_RESULT_E  OpenFileTransfersWin(MMIBT_FILE_TRANSFER_PARAM_T *param_ptr)
{
    ...
    //lulifei modify for the sharing UI
	...
	/*lulifei add for the sharing UI
    ...
    if(PNULL != win_param_ptr)
    {
        ...
        else
        {
            ...
            if(win_id == MMIBT_SENDING_FILE_WAIT_WIN_ID){
     	   	MMI_GetLabelTextByLang(STR_BT_WAIT_SEND,&query_text);
     	   }else{
     	         MMI_GetLabelTextByLang(STR_BT_WAIT_RECEIVE,&query_text);
   	   }
            MMIPUB_OpenProgressWinByTextPtr(&query_text, &win_id, PNULL, MMIPUB_SOFTKEY_DIALOG_CLOSE, HandleSharingFileProgressbarDialogWindow);
	       //MMIPUB_SetProgressTotalSize(&win_id, param_ptr->total_file_size);
	       MMIPUB_SetProgressTotalSize(&win_id,100);
           {
            MMI_STRING_T text_str_ptr = {0};
            text_str_ptr.wstr_ptr = L" ";
            text_str_ptr.wstr_len = 1;
            MMIPUB_SetProgressContentByTextPtr(&win_id,&text_str_ptr,TRUE);
            }
	        res = MMI_RESULT_TRUE;
	        //MMIBT_SetSendFileEndTimer();
    //}
    //SCI_TRACE_LOW("OpenFileTransfersWin malloc memery fail");
...
LOCAL void Create_TextLabel(MMI_WIN_ID_T win_id, MMIBT_FILE_SERVER_TYPE_E tran_type)
{
    ...
    if(font_height *loop_count + image_width < (both_rect.v_rect.bottom - both_rect.v_rect.top))
	{
		...
		line_space_v = (both_rect.v_rect.bottom - both_rect.v_rect.top - image_width -font_height *loop_count)/(loop_count + 2);
	}
    ...
    for(i = 0; i < loop_count; i++)
    {
        ...
        GUILABEL_SetFont(create.ctrl_id, MMI_DEFAULT_NORMAL_FONT, MMI_WHITE_COLOR);
    }
}
/*****************************************************************************/
//  Description : display transfer file information
...
LOCAL void Display_FileInfo(MMI_WIN_ID_T win_id, MMIBT_FILE_SERVER_TYPE_E tran_type)
{
    ...
    {
        //正在连接
        GUILABEL_SetTextById(s_labels_array[label_index][MMIBT_LABEL_ID],STR_BT_WAIT_CONNECTING_EXT01,FALSE);
        label_index ++;
	...
	else//arvin zhang added MMK_SetAtvCtrl() to solve NEWMS00210635 and modified the following code structure
	{
    	...
    	{
        	GUILABEL_SetTextById(s_labels_array[label_index][MMIBT_LABEL_ID],STR_BT_SENDING,FALSE);
		}
		else//set receive info
    	{
        	GUILABEL_SetTextById(s_labels_array[label_index][MMIBT_LABEL_ID],STR_BT_RECEIVING,FALSE);
		}
		//device name
        label_index ++;
        label_index = MIN(label_index, label_max_id -1);
...
PUBLIC void MMIBT_GetPacketSentInd(BT_MSG_T* msg_body_ptr)
{
	...
	MMIBT_TRANSFER_STATUS_TYPE_E transfer_status = MMIBT_TRANSFER_END;
       //lulifei modify for the sharing UI
    MMI_WIN_ID_T        wait_win_id = MMIBT_SENDING_FILE_WAIT_WIN_ID;
    uint8 translate_percent=0;
    ...
    if (MMK_IsFocusWin(MMIBT_SENDING_FILE_WAIT_WIN_ID)){
        if(MMIBT_GetTransferedTotalSize() != 0){
            translate_percent = (uint8)((uint64)transfer_info.transmitted_count*100/(uint64)MMIBT_GetTransferedTotalSize());
        }else{
                      translate_percent = 100;
        }
        MMIPUB_UpdateProgressBarEx(&wait_win_id,translate_percent,FALSE);//bug1106408
        MMK_SendMsg(wait_win_id, MSG_FULL_PAINT, PNULL);
#if 0
        {
	            MMI_STRING_T text_str_ptr = {0};
	            text_str_ptr.wstr_ptr = L" ";
...
PUBLIC void MMIBT_GetPacketReceivedInd(BT_MSG_T* msg_body_ptr)
{
    BT_FILE_TRANSFER_T      transfer_info = *((BT_FILE_TRANSFER_T *)msg_body_ptr->body_ptr);
    //lulifei modify for the sharing UI
    MMI_WIN_ID_T        wait_win_id = MMIBT_RECEIVING_FILE_WIN_ID;
	 uint8 translate_percent=0;
    //SCI_TRACE_LOW:"MMIBT_GetPacketReceivedInd: the received data is = %d"
    ...
    {
        /*lulifei remove for the sharing UI
...
PUBLIC void MMIBT_SendFileResInd(BT_MSG_T* msg_body_ptr)
{
    ...
    if (BT_SUCCESS == msg_body_ptr->status)
    {
        ...
        //send file success when the file translate overredraw send 100% progress bar agin
        if (MMK_IsFocusWin(MMIBT_SENDING_FILE_WAIT_WIN_ID))
        {
            /*DisplayFtpFileSizeInfo(transfer_info.transmitted_count,MMIBT_BT_SEND_FILE_TEXT_CTRL_ID, MMIBT_SENDING_FILE_WAIT_WIN_ID);
...
//  Return:
/*****************************************************************************/
PUBLIC void MMIBT_ReceiveFileResInd(BT_MSG_T* msg_body_ptr)
{
    ...
    if (BT_SUCCESS == msg_body_ptr->status)
    {
        ...
        MMIPUB_UpdateProgressBarEx(&wait_win_id,100,FALSE);//bug1106408
        MMK_SendMsg(wait_win_id, MSG_FULL_PAINT, PNULL);
#if 0
	   {
                MMI_STRING_T text_str_ptr = {0};
