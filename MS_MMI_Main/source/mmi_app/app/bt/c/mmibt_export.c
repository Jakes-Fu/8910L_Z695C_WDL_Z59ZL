...
{   	
    ...
    SCI_TRACE_LOW("[MMIBT]MMIBT_SendNextFileCallBack: TransferStatus=%d", MMIBT_GetTransferStatus());
...
{
    SCI_TRACE_LOW("wangwp--SetCallTypeEventFlag: Enter: is_WBNBInd=%d", is_WBNBInd);
    s_WBNBInd = is_WBNBInd;
}
...
{
    SCI_TRACE_LOW("wangwp--GetCallTypeEventFlag: Enter: s_WBNBInd=%d", s_WBNBInd);
    return s_WBNBInd;
}
...
{
    if(BT_GetState() && MMIBT_IsOnTransferStatus())
    {
        SCI_TRACE_LOW("MMIAPIBT_StopBTTransferFile");
