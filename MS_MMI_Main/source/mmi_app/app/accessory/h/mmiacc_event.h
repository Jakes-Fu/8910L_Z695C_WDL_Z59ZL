...
//[modify alarm max num from 3 to 5]
#define ALM_ALARM_NUM                       /*3*/5           // used for alarm list menu,不能大于MMIACC_ALARM_NUM
//[end]
...
#define MMIACC_ALARM_NUM                    10          // 最大闹钟数,修改该宏，注意修改NV.
    ...
    #define ALM_ALARM_NUM                   3       // used for alarm list menu,不能大于MMIACC_ALARM_NUM
#else
    #define ALM_ALARM_NUM                   6       // used for alarm list menu,不能大于MMIACC_ALARM_NUM
...
#define ALM_POWERONOFF_EDIT_ITEM_NUM    2
#define ALM_POWERONOFF_EDIT_ITEM_TIME   0
//[end]
...
#define MMIACC_EVENT_MAX_NUM        (MMIACC_ALARM_NUM + MMIACC_DELAY_ALARM_NUM + ALM_AUTOPOWER_NUM+ MMIACC_SCH_NUM + MMIACC_SCH_NUM + MMIACC_AZAN_NUM + FM_TIMER_RECORD_NUM+IM_SOGOU_NUM)
...
//modify alarm and calendar auto snooze times]
#define MMIALM_DELAY_MAX_NUM                /*2*/4           // 每个延时闹钟最大延时次数
//[end]
...
//[modify default alarm ring tone]
#define ALM_DEFAULT_ALARM_RINGTONG          /*1*/ 1       // default alarm ring tone
#define ALM_DEFAULT_ALARM_RINGTONG_MAT           1       //default alarm ring tone id for MAT
//[end]
...
#define ALM_MODE_WEEKDAY        6
#define ALM_MODE_WEEKEND        7
#define ALM_MODE_SPECIALDAY     8
#define ALM_MODE_MAX_TYPE       9
//[end]
#endif

//[alarm main button height]
#if defined (MAINLCD_SIZE_128X160)
#define  CHILD_FORM_BUTTON_HIGHT 32
#else
#define  CHILD_FORM_BUTTON_HIGHT 38
#endif
//[end]
/**--------------------------------------------------------------------------*
 **                         TYPE DEFINITION                                  *
...
typedef enum{
    ...
    // Added by sunhongzhe for calendar snooze function.
    EVENT_SCH_DELAY_0,
    EVENT_SCH_DELAY_MAX = EVENT_SCH_DELAY_0 + MMIACC_SCH_NUM-1,
    // End
#ifdef MMI_AZAN_SUPPORT
...
typedef enum
{
    ...
    MMISCH_REMIND_NONE, // Added for MS Custom UI.
...
typedef enum
{
    ALM_SNOOZE_TIME_2_MIN,
    ALM_SNOOZE_TIME_5_MIN,
    ALM_SNOOZE_TIME_10_MIN,
    ALM_SNOOZE_TIME_15_MIN,
    ALM_SNOOZE_TIME_20_MIN,
    ALM_SNOOZE_TIME_MAX
}ALM_SNOOZE_TYPE_E;
//[end]
...
typedef struct MMIACC_SMART_EVENT_FAST_TABLE_TAG
{
    ...
    //[alarm snooze time]
    ALM_SNOOZE_TYPE_E  snooze_type;        // default value is 10 min
    //[end]
...
{
    //text
    uint16                       w_length;                    // subject
    wchar                       w_content[MMIACC_EVENT_TEXT_MAX_LEN+1];

    //content text
    uint16                       w_text_length;          // note
    wchar                       w_text[MMIACC_EVENT_TEXT_MAX_LEN+1];

    // Added by sunhongzhe for MS Custom UI.
    uint16                       w_place_length;        // place
    wchar                       w_place[MMIACC_EVENT_TEXT_MAX_LEN+1];
    // End
...
typedef struct MMIACC_SCH_LIST_TAG
{
    ...
    uint16                  entry_id[MMIACC_SCH_NUM * 2 + MMIACC_ALARM_NUM];
}MMIACC_SCH_LIST_T;
