                                    ...
                                    )
{
    ...
    if (MMIALMSVC_INVLID_SVC_ID != svc_id)
    {
        MMI_LINK_NODE_T *node = SearchSvcInListById(s_alarm_service_list, svc_id);
        if (PNULL != node)
        {
            MMIALMSVC_EVENT_INFO_T *info = (MMIALMSVC_EVENT_INFO_T *)node->data;

            if (PNULL != info
                && info->is_valid
                && MMIALMSVC_FIRST_SMALL == CompareSvcDateTime(cur_time, info->event_time))
            {
                result = TRUE;
            }
        }
