...
LOCAL uint32 AH_GetYearDays( 
                               uint16   iAHYear 
                               )
{
    uint32    days = 354;

    if (AH_IsLeapYear(iAHYear))
    {
        days = 355;
    }
    return days;
...
LOCAL uint32 AH_GetMonthDays(uint16   iAHYear,  uint16  iAHMonth)
{
    uint32    days = 30;

    if (iAHMonth == 12 && AH_IsLeapYear(iAHYear))
    {
        days = 30;
    }
    else if (iAHMonth % 2 == 0)
    {
        days = 29;
    }
...
uint16 MMICALENDAR_SolarToThai( 
                             uint16     solar_year,
                             uint8      solar_month,
                             uint8      solar_day
                             )
{
    ...
    ret = solar_year + 543;
    //MMICALENDAR_SolarToLunar(solar_year, solar_month, solar_day, &iYear, &iMonth, &iDay);

    //if (iMonth > 4 || iMonth == 4 && iDay >= 8) ret++;

    return ret;
}

void MMICALENDAR_SolarToAH( 
                             uint16     solar_year,
                             uint8      solar_month,
                             uint8      solar_day,
                             uint16     *ah_year_ptr,
                             ...
                             )
{
    ...
    if(NULL == ah_year_ptr ||  NULL == ah_month_ptr || NULL == ah_day_ptr)
    {
        return;
    }  

    iSpanDays = CalcDaysBetweenTwoDate(AH_START_YEAR, 7, 19,
                                     solar_year, solar_month, solar_day);
    ...
    //计算月
    tmp = AH_GetMonthDays( iYear, iMonth );
    while( iSpanDays >= tmp )
    {
        iSpanDays -= tmp;
        tmp = AH_GetMonthDays( iYear, ++iMonth );
