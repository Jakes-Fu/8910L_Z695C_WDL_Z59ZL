...
//  Note:
/*****************************************************************************/
LOCAL void ST_DisplayCurTime_new(
                    MMI_WIN_ID_T            win_id,
                    const GUI_BOTH_RECT_T   *rect1_ptr,
                    const GUI_BOTH_RECT_T   *rect2_ptr,
                    const GUI_BOTH_RECT_T   *rect3_ptr,
                    const GUI_BOTH_RECT_T   *rect4_ptr,
...
//added by du.yuxuan
/*****************************************************************************/
//  Description :set both rect of running time.
//  Global resource dependence :
//  Author:
//  Note:
/*****************************************************************************/
LOCAL void GetSTTimeBothRect(
                                MMI_WIN_ID_T win_id,
                                GUI_BOTH_RECT_T *time_hour_ptr,
                                GUI_BOTH_RECT_T *time_min_ptr,
                                GUI_BOTH_RECT_T *time_sec_ptr,
                                GUI_BOTH_RECT_T *time_msec_ptr
                                );
//end
/*****************************************************************************/
...
//  Note:
/*****************************************************************************/
LOCAL void CustomST_Stop(MMI_WIN_ID_T win_id);
...
#else
    WIN_SOFTKEY(TXT_COMMON_START, TXT_RESET, TXT_COMMON_COMMON_BACK),
#endif    
    END_WIN
}; 
...
//    MN_RETURN_RESULT_E return_value = MN_RETURN_FAILURE;//nv read的返回值
    ...
    GUI_BOTH_RECT_T time_hour_rect      = {0};
    GUI_BOTH_RECT_T time_min_rect      = {0};
    GUI_BOTH_RECT_T time_sec_rect      = {0};
    GUI_BOTH_RECT_T time_msec_rect      = {0};
    GUI_BOTH_RECT_T time_rect       = {0};  //
    GUI_BOTH_RECT_T *time1_ptr      = {0}; //time1指针
    GUI_BOTH_RECT_T *time2_ptr      = {0}; //time2指针
    GUI_BOTH_RECT_T *time_hour_ptr      = {0};
    GUI_BOTH_RECT_T *time_min_ptr      = {0};
    GUI_BOTH_RECT_T *time_sec_ptr      = {0};
    GUI_BOTH_RECT_T *time_msec_ptr      = {0};
    GUI_BOTH_RECT_T *time_ptr       = {0};  //time总区域指针
    GUI_BOTH_RECT_T list_rect       = {0};  //

    GUISTR_STYLE_T  time_style      = {0};
    MMI_TEXT_ID_T text_id = STXT_STOPW_HDR;
#ifdef TOUCH_PANEL_SUPPORT
    ...
    GetSTTimeBothRect(win_id, &time_hour_rect, &time_min_rect, &time_sec_rect, &time_msec_rect);
    time1_ptr          = &time1_rect;
    time2_ptr          = &time2_rect;
    time_ptr           = &time_rect;

    time_hour_ptr = &time_hour_rect;
    time_min_ptr = &time_min_rect;
    time_sec_ptr = &time_sec_rect;
    time_msec_ptr = &time_msec_rect;
    ...
    switch(msg_id)
    {
        case MSG_OPEN_WINDOW:
...
#else              
                GUIWIN_SetSoftkeyTextId(win_id, IMAGE_NULL, TXT_COMMON_START, TXT_COMMON_COMMON_BACK, FALSE);
                //GUIWIN_SeSoftkeytButtonIconId(win_id, IMAGE_NULL, LEFT_BUTTON, TRUE);
                //GUIWIN_SeSoftkeytButtonTextId(win_id, STXT_SOFTKEY_START_MK, MIDDLE_BUTTON, TRUE);
          ...
          //  GUILIST_SetNeedPrgbar(MMIST_LIST_CRTL_ID, TRUE);
           // GUILIST_SetListScrollBarStyle(MMIST_LIST_CRTL_ID, GUIPRGBOX_STYLE_VERTICAL_SCROLL);
            ...
            if (max_count_number > 6)
            {
                GUILIST_SetNeedHiLightBar(MMIST_LIST_CRTL_ID, TRUE);
            }
            else
            {
                GUILIST_SetNeedHiLightBar(MMIST_LIST_CRTL_ID, FALSE);
            ...
            MMK_SetAtvCtrl(win_id, MMIST_LIST_CRTL_ID);
            break;
        ...
        case MSG_FULL_PAINT:
            ...
            //ST_DisplayCurTime_new(win_id,time_hour_ptr,time_min_ptr,time_sec_ptr,time_msec_ptr,s_st_current_time2);
        ...
        //case MSG_APP_OK:
            {
                ...
                { 
                    ...
                    GUIWIN_SetSoftkeyTextId(win_id, TXT_RESET, STR_SOFTKEY_RESUME_MK, STXT_RETURN, FALSE);
                    //GUIWIN_SeSoftkeytButtonIconId(win_id, CUSTOM_EXTRAS_RESET, LEFT_BUTTON, TRUE);
                    //GUIWIN_SeSoftkeytButtonTextId(win_id, STR_SOFTKEY_RESUME_MK, MIDDLE_BUTTON, TRUE);
                    //GUIWIN_SeSoftkeytButtonIconId(win_id, IMAGE_COMMON_BACK, RIGHT_BUTTON, TRUE);

                    MMIAPICOM_StatusAreaSetStopwatchRunningState(FALSE);
                }
                ...
                {   
                    ...
                    GUIWIN_SetSoftkeyTextId(win_id, TXT_ACC_COUNT, STXT_STOP, TXT_COMMON_COMMON_BACK, FALSE);
                    //GUIWIN_SeSoftkeytButtonIconId(win_id, CUSTOM_EXTRAS_STOPWATCH, LEFT_BUTTON, TRUE);
                    //GUIWIN_SeSoftkeytButtonTextId(win_id, STXT_SOFTKEY_STOP_MK, MIDDLE_BUTTON, TRUE);
                    //GUIWIN_SeSoftkeytButtonIconId(win_id, IMAGE_COMMON_BACK, RIGHT_BUTTON, TRUE);

                    MMIAPICOM_StatusAreaSetStopwatchRunningState(TRUE);
                }
        ...
        case MSG_APP_OK:
            {
                ...
                if(MMI_ST_RUNNING == s_stopwatich_status)//此时，按下的button2为"Count"
                {
                    ...
                    if(max_count_number <= ST_COUNT_RESTORE_NUM)//目前记录的count次数小于20时
                    {
                        ...
                        st_time_count[max_count_number - 1].count_time = s_st_current_time2;
                    ...
                    else //(max_count_number >ST_COUNT_RESTORE_NUM)
                    {
                       ...
                       st_time_count[ST_COUNT_RESTORE_NUM - 1].count_time = s_st_current_time2;
			...
			GUILIST_SetListScrollBarStyle(MMIST_LIST_CRTL_ID, GUIPRGBOX_STYLE_VERTICAL_SCROLL);
                    ...
                    {
                        GUILIST_SetNeedHiLightBar(MMIST_LIST_CRTL_ID, FALSE);
                        //GUILIST_SetNeedPrgbar(MMIST_LIST_CRTL_ID, FALSE);
                    }
                ...
                /*
                    ...
                    //GUIWIN_SetSoftkeyTextId(win_id, TXT_NULL, TXT_COMMON_START, TXT_NULL, STXT_RETURN, FALSE);
                    GUIWIN_SeSoftkeytButtonIconId(win_id, IMAGE_NULL, LEFT_BUTTON, TRUE);
                    GUIWIN_SeSoftkeytButtonTextId(win_id, STXT_SOFTKEY_START_MK, MIDDLE_BUTTON, TRUE);
                    GUIWIN_SeSoftkeytButtonIconId(win_id, IMAGE_COMMON_BACK, RIGHT_BUTTON, TRUE);
...
LOCAL void DisplayStopWatchBG(MMI_WIN_ID_T win_id)
{
    ...
    GUI_FillRect(&lcd_dev_info, image_rect, MMI_BLACK_COLOR);
    /*
...
//    MMI_IMAGE_ID_T      image_id     = 0;
    ...
    GUI_FillRect(&lcd_dev_info, rect_p, MMI_BLACK_COLOR);
    /*
    GUIRES_DisplayImg(PNULL,
        &rect_p,
        &rect_p,
                    ...
                    )
{
    ...
    if(!MMK_IsWindowLandscape(win_id))//纵向
    {
        ...
        {
            style_ptr->font = CAF_FONT_48;//GUI_ConvertSizeToGUIFont(ST_TIMER2_HEIGHT_V);
        }
    }
    else
    {  
    ...
    {
        style_ptr->font_color = MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_DEFAULT);
    }
    style_ptr->align = ALIGN_HVMIDDLE;//水平方向右对齐,垂直方向居中
...
{
    uint8                   time_hour_str[ST_TIME_STRING_LENGTH+1] = {0};
    uint8                   time_min_str[ST_TIME_STRING_LENGTH+1] = {0};
    uint8                   time_sec_str[ST_TIME_STRING_LENGTH+1] = {0};
    uint8                   time_msec_str[ST_TIME_STRING_LENGTH+1] = {0};
    wchar                   time_hour_temp[ST_TIME_STRING_LENGTH+1] = {0};
    wchar                   time_min_temp[ST_TIME_STRING_LENGTH+1] = {0};
    wchar                   time_sec_temp[ST_TIME_STRING_LENGTH+1] = {0};
    wchar                   time_msec_temp[ST_TIME_STRING_LENGTH+1] = {0};
    MMI_STRING_T            cur_hour     = {0};
    MMI_STRING_T            cur_min     = {0};
    MMI_STRING_T            cur_sec     = {0};
    MMI_STRING_T            cur_msec     = {0};
    GUI_LCD_DEV_INFO        lcd_dev_info = {GUI_MAIN_LCD_ID, GUI_BLOCK_MAIN};
    size_t                  str1_len      = 0;
    size_t                  str2_len      = 0;
    size_t                  str3_len      = 0;
    size_t                  str4_len      = 0;
    GUI_RECT_T              rect_hour         = {0};
    GUI_RECT_T              rect_min         = {0};
    GUI_RECT_T              rect_sec         = {0};
    GUI_RECT_T              rect_msec         = {0};
    GUISTR_STYLE_T  hour_style      = {0};
    GUISTR_STYLE_T  min_style      = {0};
    GUISTR_STYLE_T  sec_style      = {0};
    GUISTR_STYLE_T  msec_style      = {0};

    GUISTR_STATE_T  state           = GUISTR_STATE_ALIGN|GUISTR_STATE_WORDBREAK|GUISTR_STATE_SINGLE_LINE;

    MMI_ST_TIME_T   curtime     = {0};
    uint32          time_temp   = (uint32)(current_time / 10);
    uint32          time_sec    = 0;

    curtime.dsec    = (uint32)time_temp % 100;
    time_sec        = (uint32)time_temp / 100;//以sec为单位
    curtime.sec     = time_sec % 60;
    time_sec        = time_sec / 60; //以min为单位
    curtime.min     = time_sec % 60;
    time_sec        = time_sec / 60; //以hour为单位
    curtime.hour    = time_sec % 10;

    sprintf((char*)time_hour_str, "%d:", curtime.hour);
    sprintf((char*)time_min_str, "%02d:", curtime.min);
    sprintf((char*)time_sec_str, "%02d.", curtime.sec);
    sprintf((char*)time_msec_str, "%02d", curtime.dsec);

    rect_hour    = rect1_ptr->v_rect;
    rect_min    = rect2_ptr->v_rect;
    rect_sec    = rect3_ptr->v_rect;
    rect_msec    = rect4_ptr->v_rect;

    //ST_GetCurTime(time_hour_str, time_min_str, time_sec_str, time_msec_str, current_time);

    str1_len = strlen((char *)time_hour_str);
    str2_len = strlen((char *)time_min_str);
    str3_len = strlen((char *)time_sec_str);
    str4_len = strlen((char *)time_msec_str);

    MMI_STRNTOWSTR(time_hour_temp, ST_TIME_STRING_LENGTH, time_hour_str, ST_TIME_STRING_LENGTH, str1_len);
    cur_hour.wstr_ptr = time_hour_temp;
    cur_hour.wstr_len = str1_len;

    MMI_STRNTOWSTR(time_min_temp, ST_TIME_STRING_LENGTH, time_min_str, ST_TIME_STRING_LENGTH, str2_len);
    cur_min.wstr_ptr = time_min_temp;
    cur_min.wstr_len = str2_len;

    MMI_STRNTOWSTR(time_sec_temp, ST_TIME_STRING_LENGTH, time_sec_str, ST_TIME_STRING_LENGTH, str3_len);
    cur_sec.wstr_ptr = time_sec_temp;
    cur_sec.wstr_len = str3_len;

    MMI_STRNTOWSTR(time_msec_temp, ST_TIME_STRING_LENGTH, time_msec_str, ST_TIME_STRING_LENGTH, str4_len);
    cur_msec.wstr_ptr = time_msec_temp;
    cur_msec.wstr_len = str4_len;

    //获取当前主题的字体颜色
    hour_style.font = CAF_FONT_52;
    if(0 == curtime.hour)
    {
        hour_style.font_color = MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_NEG_2);
    }
    else
    {
        hour_style.font_color = MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_DEFAULT);
    }
    hour_style.align = ALIGN_HVMIDDLE;

    min_style.font = CAF_FONT_52;
    if(0 == curtime.hour && 0 == curtime.min)
    {
        min_style.font_color = MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_NEG_2);
    }
    else
    {
        min_style.font_color = MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_DEFAULT);
    }
    min_style.align = ALIGN_HVMIDDLE;

    sec_style.font = CAF_FONT_52;
    if(0 == curtime.hour && 0 == curtime.min && 0 == curtime.sec)
    {
        sec_style.font_color = MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_NEG_2);
    }
    else
    {
        sec_style.font_color = MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_DEFAULT);
    }
    sec_style.align = ALIGN_HVMIDDLE;

    msec_style.font = CAF_FONT_52;
    if(0 == curtime.hour && 0 == curtime.min && 0 == curtime.sec && 0 == curtime.dsec)
    {
        msec_style.font_color = MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_NEG_2);
    }
    else
    {
        msec_style.font_color = MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_DEFAULT);
    }
    msec_style.align = ALIGN_HVMIDDLE;

    // display
    GUISTR_DrawTextToLCDInRect(
                    (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
                    (const GUI_RECT_T       *)&rect_hour,       //the fixed display area
                    (const GUI_RECT_T       *)&rect_hour,       //用户要剪切的实际区域
                    (const MMI_STRING_T     *)&cur_hour,
                    &hour_style,
                    state,
                    GUISTR_TEXT_DIR_AUTO
                    );
    GUISTR_DrawTextToLCDInRect(
                    (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
                    (const GUI_RECT_T       *)&rect_min,       //the fixed display area
                    (const GUI_RECT_T       *)&rect_min,       //用户要剪切的实际区域
                    (const MMI_STRING_T     *)&cur_min,
                    &min_style,
                    state,
                    GUISTR_TEXT_DIR_AUTO
                    );
    GUISTR_DrawTextToLCDInRect(
                    (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
                    (const GUI_RECT_T       *)&rect_sec,       //the fixed display area
                    (const GUI_RECT_T       *)&rect_sec,       //用户要剪切的实际区域
                    (const MMI_STRING_T     *)&cur_sec,
                    &sec_style,
                    state,
                    GUISTR_TEXT_DIR_AUTO
                    );
    GUISTR_DrawTextToLCDInRect(
                    (const GUI_LCD_DEV_INFO *)&lcd_dev_info,
                    (const GUI_RECT_T       *)&rect_msec,       //the fixed display area
                    (const GUI_RECT_T       *)&rect_msec,       //用户要剪切的实际区域
                    (const MMI_STRING_T     *)&cur_msec,
                    &msec_style,
                    state,
                    GUISTR_TEXT_DIR_AUTO
                    );
...
{
    MMI_ST_TIME_T   curtime     = {0};
    uint32          time_temp   = (uint32)(current_time / 100);
...
/*
    ...
    { 
        sprintf((char*)str1_ptr, "%02d:%02d.%02d", curtime.min, curtime.sec,curtime.dsec);
    }*/
    //else 
    //显示 0...0:00:00.00
    //{
...
{ 
    ...
    if (MMISTOPWATCH_MAIN_WIN_ID == MMK_GetFocusWinId() || MMISTOPWATCH_MAIN_WIN_ID == MMK_GetFocusChildWinId() || \
         (MMICOMMON_BANNER_WIN_ID == MMK_GetFocusWinId() && MMK_IsOpenWin(MMISTOPWATCH_MAIN_WIN_ID)))
    {
...
//time2
    ...
    time2_ptr->v_rect.top       = both_rect.v_rect.top;//time1_ptr->v_rect.bottom + MMIST_TIME2_OFFSET_Y_V;
    time2_ptr->h_rect.top       = both_rect.h_rect.top;//time1_ptr->h_rect.bottom + MMIST_TIME2_OFFSET_Y_H;
    ...
    time2_ptr->v_rect.left      = both_rect.v_rect.left  + ST_TIME_MARGIN;
    time2_ptr->h_rect.left      = both_rect.h_rect.left +ST_BUTTON_OFFSET_X_H;
    time2_ptr->v_rect.right     = both_rect.v_rect.right - ST_TIME_MARGIN;
...
/*
    ...
    time_ptr->v_rect.top       = both_rect.v_rect.top;/*time1_ptr->v_rect.bottom + MMIST_TIME2_OFFSET_Y_V;*/
    time_ptr->h_rect.top       = both_rect.h_rect.top;/*time1_ptr->h_rect.bottom + MMIST_TIME2_OFFSET_Y_H;*/
    time_ptr->v_rect.bottom    = both_rect.v_rect.top + ST_TIMER2_HEIGHT_V/* - 1*/;
    time_ptr->h_rect.bottom    = both_rect.h_rect.top + ST_TIMER2_HEIGHT_H/* - 1*/;
    time_ptr->v_rect.left      = both_rect.v_rect.left;
    time_ptr->h_rect.left      = both_rect.h_rect.left;
    time_ptr->v_rect.right     = both_rect.v_rect.right;
    time_ptr->h_rect.right     = both_rect.h_rect.right;
...
#else
    list_ptr->v_rect.top        = time2_ptr->v_rect.bottom + 2;
    list_ptr->h_rect.top        = time2_ptr->h_rect.bottom + 2;
    list_ptr->v_rect.bottom     = both_rect.v_rect.bottom ;
    list_ptr->h_rect.bottom     = both_rect.h_rect.bottom;
    list_ptr->v_rect.left       = both_rect.v_rect.left;
    list_ptr->h_rect.left       = both_rect.h_rect.left;
                                ...
                                )
{
    ...
    //hour rect
    time_hour_ptr->v_rect.top       = both_rect.v_rect.top;
    time_hour_ptr->v_rect.bottom    = both_rect.v_rect.top + ST_TIMER2_HEIGHT_V;
    time_hour_ptr->v_rect.left      = both_rect.v_rect.left + ST_TIME_MARGIN;
    time_hour_ptr->v_rect.right     = time_hour_ptr->v_rect.left + ST_TIME_HOUR_WIDTH;

    //minute rect
    time_min_ptr->v_rect.top       = both_rect.v_rect.top;
    time_min_ptr->v_rect.bottom    = both_rect.v_rect.top + ST_TIMER2_HEIGHT_V;
    time_min_ptr->v_rect.left      = time_hour_ptr->v_rect.right + ST_TIME_SPACE;
    time_min_ptr->v_rect.right     = time_min_ptr->v_rect.left + ST_TIME_MIN_SEC_WIDTH;

    //second rect
    time_sec_ptr->v_rect.top       = both_rect.v_rect.top;
    time_sec_ptr->v_rect.bottom    = both_rect.v_rect.top + ST_TIMER2_HEIGHT_V ;
    time_sec_ptr->v_rect.left      = time_min_ptr->v_rect.right + ST_TIME_SPACE;
    time_sec_ptr->v_rect.right     = time_sec_ptr->v_rect.left + ST_TIME_MIN_SEC_WIDTH;

    //millisecond rect
    time_msec_ptr->v_rect.top       = both_rect.v_rect.top;
    time_msec_ptr->v_rect.bottom    = both_rect.v_rect.top + ST_TIMER2_HEIGHT_V ;
    time_msec_ptr->v_rect.left      = time_sec_ptr->v_rect.right + ST_TIME_SPACE;
    time_msec_ptr->v_rect.right     = both_rect.v_rect.right - ST_TIME_MARGIN;
}
//end
...
#else
    item_t.item_style = GUIITEM_STYLE_ONE_LINE_2TEXT_MS;
#endif 
...
//#endif
    ...
    sprintf((char*)(temp_wstr), "%02d", num);
    str_len1 = strlen((char *)temp_wstr);
    MMI_STRNTOWSTR(temp_wstr1, GUILIST_STRING_MAX_NUM, (uint8*)temp_wstr, GUILIST_STRING_MAX_NUM, str_len1);
    ...
    item_data.item_content[index].item_data.text_buffer.wstr_len = str_len1;
...
LOCAL void CustomST_Stop(MMI_WIN_ID_T win_id)
{
    ...
    GUIWIN_SetSoftkeyTextId(win_id, TXT_NULL, STXT_SOFTKEY_START_MK, TXT_COMMON_COMMON_BACK, FALSE);
    //GUIWIN_SeSoftkeytButtonIconId(win_id, IMAGE_NULL, LEFT_BUTTON, TRUE);
    //GUIWIN_SeSoftkeytButtonTextId(win_id, STXT_SOFTKEY_START_MK, MIDDLE_BUTTON, TRUE);
    //GUIWIN_SeSoftkeytButtonIconId(win_id, IMAGE_COMMON_BACK, RIGHT_BUTTON, TRUE);
