...
PUBLIC BOOLEAN MMIAPICALENDAR_ReplaceWildcardsInString
(
	MMI_STRING_T*	dst_ptr,		//out.
	MMI_STRING_T*	origin_str,		//in.
	wchar*			wildcards,		//in.
	MMI_STRING_T*	num_str			//in.
)
{
    return FALSE;
}
...
#endif
PUBLIC void ZDT_LunarDate(
                                      MMI_STRING_T   *lunar_name_ptr,
                    				uint16         max_name_len
                        )
{
	GUISTR_STYLE_T      text_style      = {0}; /*lint !e64*/
	...
	uint16              lun_text_len = 0;
	SCI_DATE_T					date = {0};
	if(PNULL == lunar_name_ptr || PNULL == lunar_name_ptr->wstr_ptr)
	{
	return ;
	}	
	SetCalendarTextStyle(&text_style,ALIGN_LVMIDDLE, SONG_FONT_20, MMI_WHITE_COLOR);
	TM_GetSysDate(&date);
	MMICALENDAR_SolarToLunar(date.year, date.mon, date.mday,&lun_year, &lun_month, &lun_day);
	FormatLunarInfo(lun_year, lun_month, lun_day, lun_text, &lun_text_len, NULL, NULL);
	LunarDate_str.wstr_len = MIN(max_name_len, LunarDate_str.wstr_len);
	MMI_WSTRNCPY( lunar_name_ptr->wstr_ptr, LunarDate_str.wstr_len, LunarDate_str.wstr_ptr, LunarDate_str.wstr_len, LunarDate_str.wstr_len );
	lunar_name_ptr->wstr_len = LunarDate_str.wstr_len;
	max_name_len = max_name_len -LunarDate_str.wstr_len;
	lun_text_len = 	MIN(max_name_len , lun_text_len);
	MMI_WSTRNCPY( lunar_name_ptr->wstr_ptr+LunarDate_str.wstr_len, lun_text_len, lun_text, lun_text_len, lun_text_len );
	lunar_name_ptr->wstr_len +=lun_text_len;
}
/*****************************************************************************/
//    Description : draw calendar main window
//    Global resource dependence : 
	...
	case MSG_CTL_MIDSK:
		...
		case CALENDAR_OPT_CLEAR_TODAY:
			...
			else
			{
                ...
                {
				    MMIPUB_OpenQueryWinByTextId(TXT_CALENDAR_CLEAR_TODAY_QUERY,IMAGE_PUBWIN_QUERY,PNULL,PNULL);
                }
			}
			break;
		...
		case CALENDAR_OPT_CLEAR_ALL:
            ...
            {
                MMIPUB_OpenQueryWinByTextId(TXT_CALENDAR_CLEAR_ALL_QUERY,IMAGE_PUBWIN_QUERY,PNULL,PNULL);
            }
			// MMK_CloseWin(win_id);
			break;	
									        ...
									        )
{
	...
	case MSG_OPEN_WINDOW:
		...
		// Bug 1343920, 设置日期查询范围至1980~2099
		// GUIEDIT_SetDateRange(ctrl_id,GUIEDIT_DATE_YEAR_RANGE_QUERY);
		GUIEDIT_SetDateRange(ctrl_id, GUIEDIT_DATE_YEAR_RANGE_SET);
...
)
{
	...
	case MSG_OPEN_WINDOW:
		...
		GUIWIN_SeSoftkeytButtonIconId(win_id, IMAGE_NULL, 1, FALSE);
#else
		// Bug 1375590, 删除时长短，不显示取消右软件 
		// GUIWIN_SetSoftkeyTextId(win_id,  (MMI_TEXT_ID_T)TXT_NULL, (MMI_TEXT_ID_T)TXT_NULL, (MMI_TEXT_ID_T)STXT_CANCEL, FALSE);
		GUIWIN_SetSoftkeyTextId(win_id,  (MMI_TEXT_ID_T)TXT_NULL, (MMI_TEXT_ID_T)TXT_NULL, (MMI_TEXT_ID_T)TXT_NULL, FALSE);
#endif
...
PUBLIC void MMIALM_SetFuncDelete(void)
{
	if(!MMIALM_DelFirstValidEvent())//(MMIALM_DelEvents(EVENT_SCH_0, EVENT_SCH_0+MMIACC_SCH_NUM-1))
	{
		if(MMIALM_IsExistValidEvent())
                                ...
                                )
{
    ...
    wchar solar_2[5]={0x967D,0x66C6,0x65E5,0x671F,0xff1a};
    wchar lunar[5]={0x519c,0x5386,0x65E5,0x671F,0xff1a};
    wchar lunar_2[5]={0x8FB2,0x66C6,0x65E5,0x671F,0xff1a};
	...
	if(PNULL != year_month_day.wstr_ptr)
	{
		...
		item_data.text_data.buf.len = 5;
		 ...
		 if(MMISET_LANGUAGE_TRAD_CHINESE==lang_type){
		 	item_data.text_data.buf.str_ptr = lunar_2;
		 }else{
	...
	#if  defined (IM_TRAD_CHINESE_SUPPORT)
		 if(MMISET_LANGUAGE_TRAD_CHINESE==lang_type){
		 	item_data.text_data.buf.str_ptr = solar_2;
		 }else{
                             ...
                             )
{
    ...
    int16 focus_y=0;
#endif
    MMISET_TIME_DISPLAY_TYPE_E  time_type = 0;
    ...
    if(!MMITHEME_IsMainScreenLandscape ())
    {
        ...
        if(time_type == MMISET_TIME_12HOURS)
        {
            ...
            time_right=CALENDAR_DAILY_TIME_RIGHT_V +20;
#else
    ...
    for(i=0;i<24;i++)
    {
        ...
        sch_rect.top=single_item_rect.top+2;
        ...
        if(GUI_IntersectRect(&cross_rect,single_item_rect,item_rect))
        {
            ...
            if(0!=result)
            {
                ...
                SetCalendarTextStyle(&text_style, ALIGN_LVMIDDLE, CALENDAR_DAILY_DATE_FONT, MMI_WHITE_COLOR);
                GUISTR_DrawTextToLCDInRect( 
        ...
        if(IsInDailyItemRange(time_rect.bottom)||IsInDailyItemRange(time_rect.top))
        {
            ...
            SetCalendarTextStyle(&text_style, ALIGN_HVMIDDLE, CALENDAR_DAILY_DATE_FONT, MMI_BLACK_COLOR);
            GUISTR_DrawTextToLCDInRect( 
                (const GUI_LCD_DEV_INFO *)daily_calendar->dev_info,
