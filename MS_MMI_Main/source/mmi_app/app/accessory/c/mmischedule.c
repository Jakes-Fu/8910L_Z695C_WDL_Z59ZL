...
LOCAL MMISCH_DETAIL_WIN_PARAM_T s_sch_event_detail = {0};
                                           ...
                                           )
{
    ...
    case MSG_SCH_LIST_UPDATE:
        ...
        SetTitleIndexType(current_ctrl_id,is_list_has_item);
        if(!is_list_has_item)
	    GUIWIN_ClearTitleSubTextInfo(win_id);
#endif
        break;
                                             ...
                                             )
{
	...
	case MSG_CTL_MIDSK:
        ...
        case ID_SCH_LIST_DEL:
            MMIPUB_OpenQueryWinByTextId(TXT_DELETE_QUERY, IMAGE_PUBWIN_QUERY, PNULL, HandleListDelQueryWin);
            MMK_CloseWin(win_id);
            break;
        ...
        case ID_SCH_LIST_DEL_ALL:
		    ...
		    {
		        MMIPUB_OpenQueryWinByTextId(TXT_DELETE_ALL_QUERY,IMAGE_PUBWIN_QUERY,PNULL,HandleListDelAllQueryWin);
		    }
            MMK_CloseWin(win_id);
            break;
#endif
...
{
	MMI_CTRL_ID_T	edit_ctrl_id = MMISCH_NAME_EDIT_CTRL_ID;
	...
	//GUILABEL_SetFont(MMISCH_NAME_TITLE_LABEL_CTRL_ID, MMI_LIST_ITEM_SMALL_FONT, MMI_GRAY_COLOR);
	...
	CTRLBASEEDIT_SetLabelId(win_id, edit_ctrl_id, STR_CALE_SUBJECT_EXT01, FALSE);
	GUIEDIT_SetDefaultStringById(edit_ctrl_id, STR_CALE_ADD_SUBJECT_EXT01);
	GUIEDIT_SetString(edit_ctrl_id, name_str.wstr_ptr, name_str.wstr_len );
#endif
}
...
#ifdef MMI_GUI_STYLE_MINISCREEN    
	GUIEDIT_SetDateStyle(MMISCH_DEADLINE_EDIT_CTRL_ID,&date_show_style,&date_display_style,FALSE);   
#else
...
)
{
	...
	SCI_DATE_T                  second_date     = {0};
		SCI_DATE_T                  cur_second_date     = {0};//next event week day aftering cur_date
...
//#ifdef MMI_PDA_SUPPORT
...
#endif
#ifndef MMI_PDA_SUPPORT
		SetSchEditWinNameParam(win_id, event);
	...
	case MSG_APP_OK:
		...
		// 设置默认snooze时间为15 mins
		event->event_fast_info.snooze_type = ALM_SNOOZE_TIME_15_MIN;
		...
		if(ALM_MODE_ONCE == event->event_fast_info.fre_mode || 
			(ALM_MODE_NO == event->event_fast_info.fre_mode))
		{
			// 判断时间是否过期
...
// 			uint16 sel_count = 0;
			...
			// 判断时间是否过期
			TM_GetSysDate(&cur_date); 
			cur_second_date.year = cur_date.year;
			cur_second_date.mon  = cur_date.mon;
			cur_second_date.mday = cur_date.mday;
					
			cur_second_date.wday = MMICALENDAR_GetWeekday(cur_second_date.year,cur_second_date.mon,cur_second_date.mday);
			if (MMIAPIALM_CompareDateAndTime(cur_second_date, cur_time, cur_second_date, second_time) <= CMP_FIRST_SMALL)
			{
				offset_day = MMIAPIALM_RoundWeek(cur_second_date.wday, event->event_fast_info.fre, CMP_FIRST_SMALL);
			}
			else
			{
				offset_day = MMIAPIALM_RoundWeek(cur_second_date.wday, event->event_fast_info.fre, CMP_SECOND_SMALL);
			}

			for (i=0; i <offset_day; i++)
			{
				if ((QUERY_YEAR_TOP == cur_second_date.year) && (SCH_DECEMBER == cur_second_date.mon) && (BIG_MONTH_DAY == cur_second_date.mday))
				{
					break;
				}
				MMIAPIALM_IncreaseDateByOne(&cur_second_date);
			}
			deadline.year = event->event_fast_info.end_year;
			deadline.mon= event->event_fast_info.end_mon;
			deadline.mday= event->event_fast_info.end_day;
			if (IsValidDeadline(deadline, cur_date))
			{
				if ((MMIAPIALM_CompareDateAndTime(cur_date, cur_time, second_date, second_time) == CMP_SECOND_SMALL) &&
					(CMP_FIRST_SMALL == CompareDate(deadline, cur_second_date)))
				{
					MMIPUB_OpenAlertWarningWin(TXT_DATE_IS_PAST);
				}
				else
				{
					if (IsValidDeadline(deadline, cur_second_date))
					{			
						MMK_PostMsg(win_param->win_id, MSG_SCH_EVENT_EDIT_FINISH, PNULL, 0);
						MMK_PostMsg(MMISCH_VIEW_WIN_ID, MSG_SCH_LIST_UPDATE, PNULL, 0); 
						MMK_PostMsg(MMISCH_VIEW_ALL_WIN_ID, MSG_SCH_LIST_UPDATE, PNULL, 0);
		...
		else
		{
			...
			second_date.mon  = event->event_fast_info.mon;
			second_date.mday = event->event_fast_info.day;			
			if(CMP_FIRST_SMALL == CompareDate(deadline, cur_date))
			{
				MMIPUB_OpenAlertWarningWin(TXT_SCH_INVALID_DEADLINE);
				break;
			}
			if(ALM_MODE_EVERYYEAR== event->event_fast_info.fre_mode)
			{
				SCI_DATE_T                  cur_next_event_date     = {0};
				if(((TRUE == MMIAPICOM_IsLeapYear(second_date.year)) && (2 == second_date.mon) && (29 == second_date.mday)) &&
					(FALSE == MMIAPICOM_IsLeapYear(cur_date.year)))
				{
					cur_next_event_date.year = cur_date.year + 1;
					cur_next_event_date.mon  = second_date.mon;
					cur_next_event_date.mday = second_date.mday - 1;//only 28 days in Feb
				}
				else
				{
					cur_next_event_date.year = cur_date.year + 1;
					cur_next_event_date.mon  = second_date.mon;
					cur_next_event_date.mday = second_date.mday;
				}

				if((CMP_SECOND_SMALL == CompareDate(second_date, deadline))||
					((CMP_SECOND_SMALL == CompareDate(cur_next_event_date, deadline)) && (MMIAPIALM_CompareDateAndTime(second_date, second_time, cur_date, cur_time) <= CMP_FIRST_SMALL)))
				{
					//no event after cur date
					MMIPUB_OpenAlertWarningWin(TXT_DATE_IS_PAST);
					break;                        
				}
			}
...
//     {
//     case MSG_OPEN_WINDOW:
//         MMIAPIALM_SetFMEditCtrlParam(win_id);
// #ifdef MMI_PDA_SUPPORT
...
//CR163242
	...
	case MSG_CTL_MIDSK:
        ...
        case ID_SCH_ALL_LIST_DEL:
            MMIPUB_OpenQueryWinByTextId(TXT_DELETE_QUERY, IMAGE_PUBWIN_QUERY, PNULL, HandleAllListDelQueryWin);
            MMK_CloseWin(win_id);
            break;
            
        case ID_SCH_ALL_LIST_DEL_ALL:
            MMIPUB_OpenQueryWinByTextId(TXT_DELETE_ALL_QUERY,IMAGE_PUBWIN_QUERY,PNULL,HandleAllListDelAllQueryWin);
            MMK_CloseWin(win_id);
            break;
#endif
#if defined (IM_SIMP_CHINESE_SUPPORT) || defined (IM_TRAD_CHINESE_SUPPORT) || defined(MMI_DISPLAY_SIMP_CHINESE_SUPPORT) || defined(MMI_DISPLAY_TRAD_CHINESE_SUPPORT)
...
PUBLIC void MMIAPISCH_OpenSchEventFromApp(uint16 id)
{
    CloseAllSchWindows();
    SCI_MEMSET(&s_sch_event_detail, 0, sizeof(MMISCH_DETAIL_WIN_PARAM_T));

    s_sch_event_detail.index = id;

    MMIAPIALM_FormInteEventInfo(id, &s_sch_event_detail.event);
    if(!MMIAPIALM_GetRingInfo(id, &s_sch_event_detail.event))
    {
        s_sch_event_detail.event.event_content_info.ring_type = ALM_FIX_RING;
        s_sch_event_detail.event.event_content_info.ring = 1;
    }

    MMK_CreateWin((uint32 *)SCH_VIEW_DETAIL_WIN_TAB, (ADD_DATA)&s_sch_event_detail);
    MMIAPISCH_DeleteUnreadEvent(id);
}

PUBLIC BOOLEAN MMI_IsRingSchfixring(void)
{
	if (MMK_IsOpenWin(MMISCH_FIXEDRINGLIST_WIN_ID))
	{
		return TRUE;
	}     
	else
