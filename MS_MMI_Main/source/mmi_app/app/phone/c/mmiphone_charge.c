...
extern void hero_tts_charge_opt_broadcast(MMI_TEXT_ID_T textId);
extern BOOLEAN hero_tts_low_power_broadcast_is_open(void);
uint8 s_tts_charge_start_ind_timer_id = 0;
#endif
/**--------------------------------------------------------------------------*
**                         MACRO DEFINITION                                 *
**--------------------------------------------------------------------------*/
...
#define ACHARGE_CONNECTED_TIMER_5SEC   (5*1000)
#define UPDATEANIM_TIMER_500SSEC     (500)
#define UPDATEANIM_BEGINLEVEL_TIMER_10SEC   (10*1000)
...
#define MMIPHONE_CHARGE_FULL_CAP        100
...
LOCAL uint8         s_charge_connected_timer = 0;
LOCAL uint8         s_charging_updateanim_timer=0;
LOCAL uint8         s_charging_updatebatlevel_timer = 0;
LOCAL uint8         s_bat_begin_level	= 0;  //用于存储充电动画起始值
LOCAL uint8         s_bat_level_anim   =  0;
...
/*---------------------------------------------------------------------------*/
#if  1 //defined(ZDT_CHARGE_FULL_INFO_AND_VOICE)//wuxx 充电满提示:充电已满// ZDT_CHARGE_FULL_INFO
BOOLEAN g_charge_complete_info_pop_once = FALSE;//弹出一次标识
BOOLEAN g_charge_connected = FALSE;//充电器是否连接
#endif
BOOLEAN g_charge_connected_TTS_play_once = FALSE;//充电器已连接//语音每次连接充电只报一次语音//wuxx 搜索g_charge_connected_TTS_play_once --26
...
/*****************************************************************************/
//  Description : deal with signal of HandleChargeOverTempatureInd
//  Global resource dependence : 
//  Author: Cheney.Wu3
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleChargeOverTempatureInd(void);
...
//MD: add for high temp power off
LOCAL void Is_HighTemp_PowerOff(void);
/*****************************************************************************/
...
//#endif
    if (is_charge)
    {
        ...
        MMK_CreateWin((uint32*)PHONE_STARTUP_CHARGE_WIN_TAB,PNULL);
                                     ...
                                     )
{
    ...
    case CHR_MSG_CHARGE_START_IND:
        ...
        s_charge_status_remain = CHR_MSG_CHARGE_START_IND;
        SCI_TRACE_LOW("[charge] MMIPHONE_ChargeHandlePsAndRefMsg s_charge_status_remain = %d", s_charge_status_remain);
        if(MMIAPIPHONE_IsNeedDelayEvent(MMI_CHARGE_PLUG_IN_DELAY))
        {
            MMIAPICOM_StatusAreaSetChargeState(TRUE);
    ...
    case CHR_MSG_CHARGE_FINISH_IND:
        ...
        s_charge_status_remain = CHR_MSG_CHARGE_FINISH_IND;
        SCI_TRACE_LOW("[charge] MMIPHONE_ChargeHandlePsAndRefMsg s_charge_status_remain = %d", s_charge_status_remain);
        // 该消息用于汇报此时充电已满。
        ...
        if (MMIAPIIDLE_IdleWinIsOpen())
        {
            ...
            banner_info.text1_id = STXT_BATTERY_FULL;
            banner_info.start_point = pin_alert_start_point;
            MMIAPICOM_OpenBannerWin(win_id, banner_info);
           #if 1 //defined(ZDT_CHARGE_FULL_INFO_AND_VOICE)//wuxx 充电满提示:充电已满// ZDT_CHARGE_FULL_INFO  //E506 电池弹出充电满在此
           // if ((MMIPHONE_CHARGE_FULL_CAP == p_chgmng_info->bat_remain_cap)&&(g_charge_complete_info_pop_once == FALSE)&&(g_charge_connected == TRUE))//(MMIIDLE_GetIdleWinInfo()->is_charge)
            if ((g_charge_complete_info_pop_once == FALSE)&&(g_charge_connected == TRUE))
            {
                   g_charge_complete_info_pop_once = TRUE;
                   time_out = 10000000;
                  //MMIPUB_OpenAlertWaitingWin(TXT_CHARGE_FINISH);// 弹出2 秒消失框
                  //MMIPUB_OpenAlertWarningWin(TXT_CHARGE_FINISH);  
                  MMIPUB_OpenAlertWinByTextId(&time_out,TXT_CHARGE_FINISH,TXT_NULL,IMAGE_PUBWIN_SUCCESS,PNULL,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,PNULL);
                ...
                #ifndef TTS_SUPPORT_NONE//test 1
                #ifdef PHONE_NUMBER_VOICE
                #ifndef WIN32
                 //播报语音--充电已满
                 Charge_Finish_To_Speech();
                #endif
                #endif
                #endif
	#ifdef HERO_ENGINE_TTS_SUPPORT			
		//extern void hero_tts_charge_opt_broadcast(MMI_TEXT_ID_T textId);
		hero_tts_charge_opt_broadcast(TXT_CHARGE_END);
	#endif	
    ...
    case CHR_MSG_CHARGE_FAULT_IND:
        ...
        s_charge_status_remain = CHR_MSG_CHARGE_FAULT_IND;
        SCI_TRACE_LOW("[charge] MMIPHONE_ChargeHandlePsAndRefMsg s_charge_status_remain = %d", s_charge_status_remain);
    ...
    case CHR_MSG_CHARGE_OVER_TEMPTURE_IND:
        ...
        MMIDEFAULT_SetBackLight(TRUE);
        // 在充电电压超过额电电压
        result = HandleChargeOverTempatureInd();
        break;
    ...
    case STARTUP_NORMAL:
        {
            ...
            #if 1 //defined(ZDT_CHARGE_FULL_INFO_AND_VOICE)//wuxx 充电满提示:充电已满// ZDT_CHARGE_FULL_INFO 
            if ((MMIPHONE_CHARGE_FULL_CAP == bat_cap)&&(g_charge_complete_info_pop_once == FALSE)&&(g_charge_connected == TRUE))//(MMIIDLE_GetIdleWinInfo()->is_charge)
            {
                   g_charge_complete_info_pop_once = TRUE;
                    time_out = 10000000;
                  //MMIPUB_OpenAlertWaitingWin(TXT_CHARGE_FINISH);// 弹出2 秒消失框
...
//                     HandleChrWarningInd();
        ...
        break;
		case STARTUP_CHARGE:
         s_bat_begin_level =  (uint8)MMIAPIPHONE_GetBatCapacity();//关机充电电量获取
		 break;
...
/*****************************************************************************/
#ifdef HERO_ENGINE_TTS_SUPPORT
void hero_tts_charge_start_ind_callback(void)
{
	if( s_tts_charge_start_ind_timer_id != 0 )
    {
        MMK_StopTimer(s_tts_charge_start_ind_timer_id);
        s_tts_charge_start_ind_timer_id = 0;
    }
	hero_tts_charge_opt_broadcast(TXT_CHARGE_START);
}
#endif
LOCAL MMI_RESULT_E HandleChargeStartInd(void)
{
...
#ifdef PHONE_NUMBER_VOICE
#ifndef WIN32
                if (g_charge_connected_TTS_play_once == FALSE)//充电器已连接//语音每次连接充电只报一次语音//wuxx 搜索g_charge_connected_TTS_play_once
                {
                    g_charge_connected_TTS_play_once =TRUE;//充电器已连接//语音每次连接充电只报一次语音//wuxx 搜索g_charge_connected_TTS_play_once
		    Charge_Start_To_Speech();
                }
#endif
#endif
#endif			
...
if (g_charge_connected_TTS_play_once == FALSE)//充电器已连接//语音每次连接充电只报一次语音//wuxx 搜索g_charge_connected_TTS_play_once
{
		...
		s_tts_charge_start_ind_timer_id = MMK_CreateTimerCallback(1000, hero_tts_charge_start_ind_callback, NULL, FALSE);
}
#endif
#ifdef PDA_UI_DROPDOWN_WIN
     ...
     //   MMIPUB_OpenAlertWinByTextId(PNULL,TXT_CHARGE_START,TXT_NULL,IMAGE_PUBWIN_SUCCESS,&alert_win_id,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
        if(MMK_IsOpenWin(win_id) && !MMIAPIDC_IsOpened()
#ifdef VIDEO_PLAYER_SUPPORT
            && !MMIAPIVP_IsVideoDisplaying()
#endif
            )    //for bug484938,bug480581, for
        {
           banner_info.banner_type = MMI_BANNER_TEXT;
#if  1 //defined(ZDT_CHARGE_FULL_INFO_AND_VOICE)//wuxx 充电满提示:充电已满// ZDT_CHARGE_FULL_INFO
              g_charge_connected = TRUE;
#endif
           if(MMIPHONE_CHARGE_FULL_CAP != p_chgmng_info->bat_remain_cap)  //for bug516759
            {
               banner_info.text1_id = STXT_CHARGING;
            }
           else
            {
               banner_info.text1_id = STXT_BATTERY_FULL;  //for bug516759
            }
           banner_info.start_point = pin_alert_start_point;
           MMIAPICOM_OpenBannerWin(win_id, banner_info);
        }       
...
LOCAL MMI_RESULT_E HandleChargeEndInd(BOOLEAN need_delay)
{
    ...
    case STARTUP_CHARGE:
	    //停止充电，并设置电池强度
        MMIAPICOM_StatusAreaSetChargeState(FALSE);
...
#ifdef PHONE_NUMBER_VOICE
#ifndef WIN32
		ChrDisconnect_To_Speech();
#endif
#endif
#endif

#ifdef HERO_ENGINE_TTS_SUPPORT
		hero_tts_charge_opt_broadcast(TXT_CHARGE_PLUGOUT);

#endif

#if  1// defined(ZDT_CHARGE_FULL_INFO_AND_VOICE)//wuxx 充电满提示:充电已满// ZDT_CHARGE_FULL_INFO // 拨出充电器置FALSE
    g_charge_complete_info_pop_once = FALSE;//弹出一次标识
    g_charge_connected = FALSE;
#endif
        g_charge_connected_TTS_play_once = FALSE;//充电器已连接//语音每次连接充电只报一次语音//wuxx 搜索g_charge_connected_TTS_play_once
        ...
        if(!need_delay)
        {
            ...
            CHGMNG_STATE_INFO_T *p_chgmng_info = CHGMNG_GetModuleState();
			if ((s_charge_status_remain != CHR_MSG_CHARGE_FINISH_IND) && 
				(MMIPHONE_CHARGE_FULL_CAP != p_chgmng_info->bat_remain_cap) &&
				!MMIAPIDC_IsOpened())
            {
                banner_info.banner_type = MMI_BANNER_TEXT;
		        banner_info.text1_id =  TXT_CHRDISCONNECT;//   STR_NOTE_BATTERY_NOT_FULL;//需要更换文字资源
		        banner_info.start_point = pin_alert_start_point;  
			    MMIAPICOM_OpenBannerWin(win_id, banner_info);
            }
			#endif
...
LOCAL MMI_RESULT_E HandleChrWarningInd(void)
{
    ...
    case STARTUP_NORMAL:
...
#ifdef PHONE_NUMBER_VOICE
#ifndef WIN32
		ChrWarning_To_Speech();
#endif
#endif
#endif		

#ifdef HERO_ENGINE_TTS_SUPPORT
		hero_tts_charge_opt_broadcast(TXT_CHARGE_WARNING);
        ...
        MMIPUB_OpenAlertWinByTextId(PNULL,STR_BATTERY_LOW,TXT_NULL,IMAGE_PUBWIN_WARNING,&alert_win_id,&win_priority,MMIPUB_SOFTKEY_ONE,HandleLowPowerRingWin);
...
//            MMK_CreateWin((uint32*)PHONE_STARTUP_CHARGE_OK_WIN_TAB,PNULL);
              MMK_CreateWin((uint32*)PHONE_STARTUP_CHARGE_OK_WIN_TAB,TRUE);
        }
...
LOCAL MMI_RESULT_E HandleChargeFaultInd(void)
{
    ...
    case STARTUP_CHARGE:
        ...
        {
//            MMK_CreateWin((uint32*)PHONE_STARTUP_CHARGE_OK_WIN_TAB,PNULL);
              MMK_CreateWin((uint32*)PHONE_STARTUP_CHARGE_OK_WIN_TAB,FALSE);
        }
...
#endif
#if 0// def HERO_ENGINE_TTS_SUPPORT
		hero_tts_charge_opt_broadcast(TXT_CHARGE_OVER_VOLTAGE);
#endif
        break;
...
LOCAL MMI_RESULT_E HandleChargeOverTempatureInd(void)
{
    ...
    case STARTUP_CHARGE:
        //提示超过额定电压
        if(g_charge_connected)
        {		
            MMIPUB_OpenAlertFailWin(STR_CHARGING_LOW_TEMPERATURE);
        }
...
#endif
/*****************************************************************************/
//  Description : set the high battery temp power off(0-9)
//  Global resource dependence : 
//  Author: md
//  Note:
/*****************************************************************************/
...
{
    CHGMNG_STATE_INFO_T* p_chgmng_info = CHGMNG_GetModuleState();
    if(p_chgmng_info->charging_temperature >= 1065){
        MMIPHONE_PowerOff();
    }
}
/*****************************************************************************/
//  Description : get capacity level(0-9)
//  Global resource dependence : 
//  Author: Jassmine
//  Note:
/*****************************************************************************/
PUBLIC uint8 MMIAPIPHONE_GetBatCapacity(void)
{
    ...
    //获得电量强度
#if 0
    bat_level = (uint8)(bat_level / PHONE_RX_BAT_LEVEL_SPAN); 
    if (PHONE_BAT_DEFAULT_LEVEL <= bat_level)
    {
        bat_level = PHONE_BAT_DEFAULT_LEVEL - 1;
    }
#else
    if (bat_level < 12)
    {
    ...
    else if((bat_level >= 12) && (bat_level < 21))
    {
        bat_level = 1;
    }
    else if((bat_level >= 21) && (bat_level < 58))
    {
        bat_level = 2;
    }
    else if((bat_level >= 58) && (bat_level < 74))
    {
        bat_level = 3;
    }
    else if((bat_level >= 74) && (bat_level < 88))
    {
        bat_level = 4;
    }
    else if(bat_level >= 88)
    {
        bat_level = 5;
    }
#endif
                         ...
                         )
{
    ...
    case MSG_OPEN_WINDOW:
        //发出警告音
            ...
            if(!hero_tts_low_power_broadcast_is_open())
            {
...
#else	
#ifndef TTS_SUPPORT_NONE
        if (MMIAPISET_GetIsOpenVoltageWarning()&&( 1 == ZDT_GetChargeSwitch_type()))
#else
	if (MMIAPISET_GetIsOpenVoltageWarning())
#endif
        {
...
/*#ifdef VIDEO_PLAYER_SUPPORT
        ...
        || ((MMI_CHARGE_END_DELAY == event ) && MMIAPIDC_IsPreviewing())
#endif
        )
    {
        bret = TRUE;
                                              ...
                                              )
{
	...
	{
		IMAGE_STARTUP_CHARGING_0,
	    IMAGE_STARTUP_CHARGING_1,
		IMAGE_STARTUP_CHARGING_2,
		IMAGE_STARTUP_CHARGING_3,
		IMAGE_STARTUP_CHARGING_4,
		IMAGE_STARTUP_CHARGING_5,
		IMAGE_STARTUP_CHARGING_6,
		IMAGE_STARTUP_CHARGING_7,
		IMAGE_STARTUP_CHARGING_8,
		//wk45 add
		IMAGE_STARTUP_CHARGING_9,
	};
    ...
    case MSG_OPEN_WINDOW:
        {
           ...
           GUI_BOTH_RECT_T anim_rect = MMITHEME_GetFullScreenBothRect();			
            ...
            data_info.img_id = IMAGE_STARTUP_CHARGER_CONNECTED;//充电首先显示电源连接图片
            ...
            if( s_charge_connected_timer != 0 )	//用于显示连接图片5s
            {
                MMK_StopTimer(s_charge_connected_timer);
                s_charge_connected_timer = 0;
            } 
            s_charge_connected_timer = MMK_CreateTimer(ACHARGE_CONNECTED_TIMER_5SEC, FALSE); 
        }
        break;
	case MSG_TIMER:
		if (s_charge_connected_timer == *((uint8 *) param))
		{
			MMK_StopTimer(s_charge_connected_timer);
            s_charge_connected_timer = 0;
			s_bat_level_anim =  (uint8)MMIAPIPHONE_GetBatCapacity();//第一次获取电量，即手机剩余电量值
			ctrl_info.is_ctrl_id = TRUE;
            ctrl_info.ctrl_id = MMI_CHARGE_ANIM_CTRL_ID;
            display_info.align_style = GUIANIM_ALIGN_HVMIDDLE;
			display_info.is_update = TRUE;
			data_info.img_id = image_id[s_bat_level_anim];
			GUIANIM_SetParam(&ctrl_info,&data_info,PNULL,&display_info); 
			MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);
			if(s_charging_updatetime_timer!=0) //对每一张充电图片播放0.5s
            {
                MMK_StopTimer(s_charging_updateanim_timer);
                s_charging_updateanim_timer=0;
            }
            s_charging_updateanim_timer=MMK_CreateTimer(UPDATEANIM_TIMER_500SSEC, TRUE); 
		}
		if (s_charging_updateanim_timer == *((uint8 *) param))
        {	
			s_bat_level_anim = s_bat_level_anim + 1;
			if(s_bat_level_anim > 9)//s_bat_level_anim > 9
			{
				s_bat_level_anim = s_bat_begin_level;
			}
			ctrl_info.is_ctrl_id = TRUE;
            ctrl_info.ctrl_id = MMI_CHARGE_ANIM_CTRL_ID;
            display_info.align_style = GUIANIM_ALIGN_HVMIDDLE;
 ...
 //       DisplayDayTimeEffert(win_id);
    ...
    case MSG_CLOSE_WINDOW:
        if (s_charging_updateanim_timer != 0)
                {
            MMK_StopTimer(s_charging_updateanim_timer);
            s_charging_updateanim_timer = 0;
                }
		if(s_charging_updatebatlevel_timer!=0) //每10s判断一下所剩电量，改变充电动画的起始值
		{
			MMK_StopTimer(s_charging_updatebatlevel_timer);
			s_charging_updatebatlevel_timer=0;
		}
        break;
    case MSG_LOSE_FOCUS:
        //[add show alarm expired win when start
        // up charging]
        if(s_charge_connected_timer != 0)
        {
            MMK_StopTimer(s_charge_connected_timer);
            s_charge_connected_timer = 0;
        }
	...
	case MSG_GET_FOCUS:
		...
		if(s_charging_updatebatlevel_timer!=0) //每10s判断一下所剩电量，改变充电动画的起始值
		{
            ...
            s_charging_updatebatlevel_timer=MMK_CreateTimer(UPDATEANIM_BEGINLEVEL_TIMER_10SEC, TRUE);
        break;
...
#endif
#endif
            is_finish = (BOOLEAN)MMK_GetWinAddDataPtr(win_id);
            ctrl_info.is_ctrl_id = TRUE;
            ctrl_info.ctrl_id = MMI_CHARGE_ANIM_CTRL_ID;
          //  data_info.img_id = IMAGE_CHARGE_FINISH;
            if(is_finish == TRUE)
            {
               data_info.img_id = IMAGE_STARTUP_CHARGING_FULL_2;
            }
			else
			{
			   data_info.img_id = IMAGE_STARTUP_BAD_CHARGER;	
			}			
            display_info.align_style = GUIANIM_ALIGN_HVMIDDLE;
            anim_init.both_rect = anim_rect;
            ...
            GUIANIM_SetParam(&ctrl_info,&data_info,PNULL,&display_info);
#if 0       
            //creat update-timer,[ms]spec中没有time
...
LOCAL void SetBtryCapacityFormParam(void)
{
    ...
    //GUILABEL_SetTextById(MMIPHONE_DISPLAY_BTRYCAP_LABEL_ID,STXT_BATTERY_FULL,FALSE);
    DisplayBtryCapacityStr();
