...
LOCAL ALL_NETWORK_INFO_NAME_T s_network_all_name={0};
LOCAL BOOLEAN s_network_nitz_update = FALSE; 
/**--------------------------------------------------------------------------*
**                         STATIC DEFINITION                                *
**--------------------------------------------------------------------------*/
/**--------------------------------------------------------------------------*
...
LOCAL void PHONE_InitAllNetworkNameInfo(void);
...
LOCAL void StoreNitzTimeAndTickTimeInfo(const APP_MN_NETWORK_INFO_IND_T *network_info_ind)
{
    ...
    {
       if (!s_tick_time.is_net_time_exist) 
        {
            return;
        }
        //need to update  TIME ZONE information except  ut_lt_zone,because we need to use it again
        s_tick_time.net_time_info.daylight_saving_time_flag= network_info_ind->time_and_timezone.daylight_saving_time_flag;
        s_tick_time.net_time_info.daylight_saving_time = network_info_ind->time_and_timezone.daylight_saving_time;
        s_tick_time.net_time_info.local_time_zone_flag = network_info_ind->time_and_timezone.local_time_zone_flag;
        s_tick_time.net_time_info.local_time_zone = network_info_ind->time_and_timezone.local_time_zone;
        s_tick_time.net_time_info.ut_lt_zone_flag = network_info_ind->time_and_timezone.ut_lt_zone_flag;
        SCI_TRACE_LOW("[MMIPHONE]: StoreNitzTimeAndTickTimeInfo");
    }
...
/*****************************************************************************/
// 	Description : 利用当前tick和NITZ时间get系统时间
//	Global resource dependence : 
//    Author: mary.xiao
//	Note:
/*****************************************************************************/

PUBLIC uint32 MMIAPIPHONE_GetSysTimeWithNitzAndTickTime(void)//maryxiao
{
    BOOLEAN result = FALSE;
    
    SCI_TRACE_LOW("[MMIPHONE]: MMIAPIPHONE_GetSysTimeWithNitzAndTickTime is_net_time_exist= %d, is_tick_time_exist = %d",
    ...
    if(s_tick_time.is_net_time_exist && s_tick_time.is_tick_time_exist)
    {
        ...
        if(MMIAPIAUT_SetSysTime(time_second))
        {
            ...
            if(s_tick_time.net_time_info.ut_lt_zone_flag)
            {
                ...
                SCI_TRACE_LOW("[mmiphone_nitz]:ut_lt_zone_flag   time_zone =%d", net_time_zone);
            }
            else if(s_tick_time.net_time_info.local_time_zone_flag)
            {
                net_time_zone = s_tick_time.net_time_info.local_time_zone;
                SCI_TRACE_LOW("[mmiphone_nitz]:local_time_zone_flag   time_zone =%d", net_time_zone);
            }
            else
            {
                //use default 0
            ...
            SCI_TraceLow("[szp]MMIAPIPHONE_SetSysTimeWithNitzAndTickTime curr_plmn.mcc = %d, real_time_zone = %d",curr_plmn.mcc, real_time_zone);
...
/*****************************************************************************/
// 	Description : 不支持nitz开关时sim 卡时区读取
//	Global resource dependence :
//    Author: niu.huiling
//	Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPIPHONE_SetSysTimeZoneWithoutNitz(void)
{
    ...
    if ((s_tick_time.is_net_time_exist == FALSE || s_tick_time.is_tick_time_exist == FALSE) && 0 < sim_available_num)
	{
        uint16 mcurrent_year = MMI_NET_TIME_YEAR_BASE + s_tick_time.net_time_info.ut_lt_zone.year;
        int8 real_time_zone = 0;
        int8 net_time_zone = 0;
        MN_PLMN_T curr_plmn = {0};
        MMIAPIPHONE_GetCurPLMN(&curr_plmn, MMIAPIPHONE_GetAttachingNitzSIM());
        //for cetecom test
        if (s_tick_time.net_time_info.daylight_saving_time_flag)
        {
            real_time_zone = net_time_zone -s_tick_time.net_time_info.daylight_saving_time * 4;
			SCI_TRACE_LOW("[NHL 1 mmiphone_nitz]:MMIAPIAUT_GetTimezoneByMcc  real_time_zone =%d", real_time_zone);
        }

        if (0 == real_time_zone)
        {
            MMIAPIAUT_GetTimezoneByMcc(curr_plmn.mcc,&real_time_zone);
            SCI_TRACE_LOW("[NHL 2  mmiphone_nitz]:MMIAPIAUT_GetTimezoneByMcc mcc = %d, net_time_zone =%d", curr_plmn.mcc, net_time_zone);
        }

        #ifdef WORLD_CLOCK_SUPPORT
        SCI_TraceLow("[szp]MMIAPIPHONE_SetSysTimeZoneWithoutNitz curr_plmn.mcc = %d, real_time_zone = %d",curr_plmn.mcc, real_time_zone);
        MMIAPIWCLK_SetTimeZone(curr_plmn.mcc, real_time_zone, mcurrent_year, s_tick_time.net_time_info.daylight_saving_time);
        #endif
...
#ifdef WORLD_CLOCK_SUPPORT
            SCI_TraceLow("[szp]ParseSysTimeFromNetworkInfo curr_plmn.mcc = %d, real_time_zone = %d",network_info_ind->cur_plmn.mcc, real_time_zone);
...
PUBLIC MMI_RESULT_E MMIPHONE_HandleNetworkTimeInfoInd(const APP_MN_NETWORK_INFO_IND_T *network_info_ind)
{
    ...
    if ((e_dual_sys & (1<<network_info_ind->dual_sys)) && MMIAPIPHONE_GetSimExistedStatus(network_info_ind->dual_sys))
    {
...
/*    if (network_info_ind->time_and_timezone.ut_lt_zone_flag)
    {
        ...
        StoreNitzTimeAndTickTimeInfo(network_info_ind);//网络提供了本地时区
#ifdef MMI_UPDATE_TIME_WHEN_STARTUP		
...
PUBLIC void  MMIAPIPHONE_StartGetNitzTime(void)
{
    ...
    if (MMIAPIAUT_GetIsNeedUpdateTimeByNet())
    {
        ...
        MMIAUT_SetSNTPUpdateFlag(TRUE);
        bIsFound = MMIAPIPHONE_GetDataServiceSIM(&sim_ok);
        if (TRUE == bIsFound)
        {
            SCI_TRACE_LOW("[mmiphone] MMIAPIPHONE_GetNitzTime");
...
/*****************************************************************************/
PUBLIC void  MMIAPIPHONE_GetNitzTime(MN_DUAL_SYS_E dual_sys)
{
    BOOLEAN result = FALSE;
    ...
    else
    {
        ...
        if (dual_sys < MMI_DUAL_SYS_MAX)
        {
            ...
            result = MMIAPIPHONE_SetSysTimeZoneWithoutNitz();
            if (TRUE == result)
            {
            ...
            {
                SCI_TRACE_LOW("[mmiphone]:GetNitzTime error!!!");
            }
        }
    }
    SCI_TRACE_LOW("[mmiphone]:GetNitzTime attaching_sim =%d MMI_DUAL_SYS_MAX %d", dual_sys, MMI_DUAL_SYS_MAX);
...
PUBLIC void MMIPHONE_InitNITZ(void)
{
    ...
    //从NV 读取network name   
    nv_len = NVITEM_GetLength(OTHER_NV_NETWORK_OPN_UPDATE);
    nv_reslut = EFS_NvitemRead(OTHER_NV_NETWORK_OPN_UPDATE,(uint16)sizeof(s_network_nitz_update),(uint8*)&s_network_nitz_update);    
	if (NVERR_NONE == nv_reslut)
	{
	    if (TRUE == s_network_nitz_update)
	    {
            MMINV_READ(MMINV_NETWORK_NAME,&s_network_name_from_nitz,return_value);
            SCI_TRACE_LOW("[mmiphone]:MMIPHONE_InitNITZ  return_value=%d", return_value);
            ...
            PHONE_InitAllNetworkNameInfo();
	    }
	}
	else
	{
        SCI_TRACE_LOW("[mmiphone]:Failed to get s_network_nitz_update, nv_reslut=%d", nv_reslut);
	}
...
{
    return &s_network_all_name;

}

int16 saved_timezone_fromnitz = 0;
int16 saved_daylight = 0;
PUBLIC int16 MMIAPIPHONE_GetTimeZoneFromNitz(void)
{
    return saved_timezone_fromnitz;
}
PUBLIC int16 MMIAPIPHONE_GetDaylightFromNitz(void)
{
    return saved_daylight;
}
...
PUBLIC void MMIAPIPHONE_GetSPNNameFromNitz(MN_DUAL_SYS_E dual_sys,MMI_STRING_T* spn_string)
{
    ...
    if (!network_name[dual_sys].is_spn_supported)
        {
            return ;
        }
    ...
    MMIPHONE_GetNetworkNameString(dual_sys,spn_string);
...
LOCAL void PHONE_InitAllNetworkNameInfo(void)
{
    ...
    //从NV 读取network name    
    MMINV_READ(MMINV_NETWORK_ALL_NAME,&s_network_all_name,return_value);
    SCI_TRACE_LOW("[mmiphone]:PHONE_InitAllNetworkNameInfo  return_value=%d", return_value);
    if(MN_RETURN_SUCCESS !=return_value)
    {
        SCI_MEMSET(&s_network_all_name,0,sizeof(ALL_NETWORK_INFO_NAME_T));
        MMINV_WRITE(MMINV_NETWORK_ALL_NAME,&s_network_all_name);
    }
...
PUBLIC MMI_RESULT_E MMIPHONE_StoreNetworkNameInfo(const APP_MN_NETWORK_INFO_IND_T *network_info_ind)
{
    ...
    if (TRUE == network_ntiz_update)
    {
        ...
        if (dual_sys == MN_DUAL_SYS_1)  //ONly for GCF test,Display Sim1
        {
            if(network_info_ind->full_name.network_name_exist)
            {
                s_network_all_name.fullname_len= network_info_ind->full_name.length;
                SCI_MEMCPY(s_network_all_name.full_name, network_info_ind->full_name.name, network_info_ind->full_name.length); 
                SCI_TRACE_LOW("[mmiphone]:StoreNetworkNameInfo length save full_name= %d ",network_info_ind->full_name.length);            
            }
            else
            {
                s_network_all_name.fullname_len= 0;
                SCI_MEMSET(s_network_all_name.full_name, 0, MAX_NETWORK_NAME_LEN); 
                SCI_TRACE_LOW("[mmiphone]:StoreNetworkNameInfo length save full_name= 0 ");     
            }
            if(network_info_ind->short_name.network_name_exist)
            {
                s_network_all_name.shortname_len = network_info_ind->short_name.length;
                SCI_MEMCPY(s_network_all_name.short_name, network_info_ind->short_name.name, network_info_ind->short_name.length); 
                SCI_TRACE_LOW("[mmiphone]:StoreNetworkNameInfo length save short_name= %d ",network_info_ind->short_name.length);          
            }
            else
            {
                s_network_all_name.shortname_len= 0;
                SCI_MEMSET(s_network_all_name.short_name, 0, MAX_NETWORK_NAME_LEN); 
                SCI_TRACE_LOW("[mmiphone]:StoreNetworkNameInfo length save short_name= 0 ");     
            }
            MMINV_WRITE(MMINV_NETWORK_ALL_NAME,&s_network_all_name);
    ...
    if(network_info_ind->time_and_timezone.ut_lt_zone_flag)
    {
        saved_timezone_fromnitz   = network_info_ind->time_and_timezone.ut_lt_zone.time_zone;
    }
    else if (network_info_ind->time_and_timezone.local_time_zone_flag)
    {
        saved_timezone_fromnitz = network_info_ind->time_and_timezone.local_time_zone;
    }

    if (network_info_ind->time_and_timezone.daylight_saving_time_flag)
    {
        saved_daylight = network_info_ind->time_and_timezone.daylight_saving_time;
        //saved_timezone_fromnitz = saved_timezone_fromnitz -4; //    //Don't minus daysaving in local timezone
    }
    else
    {
        saved_daylight = 0;
