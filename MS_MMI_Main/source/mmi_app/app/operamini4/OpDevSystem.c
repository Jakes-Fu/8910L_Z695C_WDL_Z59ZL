...
static OD_BOOL _OD_MncThreeDigits(const char *imsi);
...
OD_STATUS OD_ClipboardSetData(OD_Context *context, OD_CLIPBOARD_FORMAT format, void* data, int len)
{
    ...
    {
        clip_res = MMICLIPBRD_SetData(CLIPBRD_FORMAT_TEXT, data, len);
    }
...
OD_STATUS OD_ClipboardGetData(OD_Context *context, OD_CLIPBOARD_FORMAT format, void** data, int* len)
{
    ...
    if(format == OD_CLIPBOARD_FORMAT_TEXT_UNICODE)
    {
        clip_res = MMICLIPBRD_GetData(CLIPBRD_FORMAT_TEXT, data, len);
    }
    return _OD_TranslateClipboardError(clip_res);
#endif /*PLATFORM_SC6600L*/
}
...
OD_STATUS OD_ClipboardHasData(OD_Context *context, OD_CLIPBOARD_FORMAT format, OD_BOOL* hasData)
{
    ...
    if(format == OD_CLIPBOARD_FORMAT_TEXT_UNICODE)
    {
        *hasData = MMICLIPBRD_IsAvailable(CLIPBRD_FORMAT_TEXT);
...
OD_STATUS OD_GetSystemProperty(OD_Context *context, OD_SYSTEM_PROPERTY_TYPE property, int* value)
{
	...
	case OD_MEDIUM_FONT_DISABLED:
		{
#ifdef CSTAR_CHINESE_LIMITATION_SUPPORT
			*value = OD_TRUE;
#else
			*value = OD_FALSE;
#endif /* CSTAR_CHINESE_LIMITATION_SUPPORT */
			return OD_SUCCESS;
		}
    }
...
OD_STATUS OD_GetIMSI(OD_Context *context, OD_IMSI* od_imsi)
{
    ...
    // The standard IMSI string consists of 15 digits: MCC(3 digits) + MNC(2 digits) + MSIN(10 digits).
    // The IMSI may contain 16 digits and starts with '9' on SPRD, we should skip the leading '9' in such a case.
    if(16 == dst_size && '9' == od_imsi->data[0])
    {
        od_memmove(od_imsi->data, od_imsi->data + 1, dst_size);
    }
...
OD_STATUS OD_GetPlatformName(OD_Context *context, char *buf, int buf_size)
{
...
#ifdef OD_7701_SUPPORT
	const char *productName = PRODUCT_NAME_Read();
	const char *fwVersion = VERSION_Read();
#else
	const char *productName = NULL;
	const char *fwVersion = NULL;
	...
	if (productName && od_strlen(productName)
		&& od_strlen(buf) + od_strlen(productName) + 3 < (unsigned int)buf_size)
	{
		od_strcat(buf, " P:");
		od_strcat(buf, productName);
	}

	if (fwVersion && od_strlen(fwVersion)
		&& od_strlen(fwVersion) + od_strlen(fwVersion) + 4 < (unsigned int)buf_size)
	{
		od_strcat(buf, " FW:");
		od_strcat(buf, fwVersion);
	}
...
OD_STATUS OD_GetNativeString(OD_NativeStringID nativeStringID, char* buffer, int bufferSize)
{
    MMI_STRING_T nativeWStr = {0};
    MMI_TEXT_ID_T platformTextId = TXT_NULL;
    uint32 utf8Size;

    OD_ASSERT(bufferSize > 1);
    switch(nativeStringID)
    {
        case OD_NATIVE_STR_BOOKMARKS_FULL:
#ifdef OD_HAVE_NATIVE_STRING_BOOKMARKS_FULL
            platformTextId = TXT_BROWSER_BOOKMARK_FULL;
#else
            platformTextId = TXT_ERROR;
#endif
            break;
        default:
            OD_ASSERT(!"unknown nativeStringID in OD_GetNativeString");
            return OD_ERROR;
    }
    MMI_GetLabelTextByLang(platformTextId, &nativeWStr);
    utf8Size = GUI_WstrToUTF8((uint8*)buffer, bufferSize - 1, nativeWStr.wstr_ptr, nativeWStr.wstr_len);
    buffer[OD_MIN(utf8Size, bufferSize - 1)] = 0;
    return OD_SUCCESS;
}

OD_STATUS OD_GetMccMnc(OD_Context *context, OD_MccMnc *mccmnc)
{
	OD_IMSI imsi = {0};

	OD_RETURNIFNOCTX(context);
	OD_CHECK_ARGS_RETURN_STATUS(mccmnc);

	OD_RETURNIF(OD_GetIMSI(context, &imsi));

	mccmnc->data[0] = 0;
	od_strncpy(mccmnc->data, imsi.data, 3);
	mccmnc->data[3] = 0;
	od_strcat(mccmnc->data, "-");

	if (_OD_MncThreeDigits(imsi.data))
	{
		od_strncat(mccmnc->data, imsi.data + 3, 3);
		mccmnc->data[7] = 0;
	}
	else
	{
...
static OD_BOOL _OD_MncThreeDigits(const char *imsi)
{
	//Fixme: determine the MNC's length by the MCC.
	//refer to "http://en.wikipedia.org/wiki/Mobile_country_code"
	char threeDigitMccs[][4] = {
								"302", "310", "311", "312", "330", "334", "338", "342", "344",
								"346", "348", "356", "358", "360", "365", "366", "352",
								"405",
								"708", "722", "732"
								};
	int count = od_countof(threeDigitMccs);
	int i = 0;

	for (i = 0; i < count; i++)
	{
		if (od_strncmp(imsi, threeDigitMccs[i], 3) == 0)
		{
			return OD_TRUE;
	}
}
...
OD_BOOL OD_isRtspPlaybackSupported()
{
    return OD_TRUE;
}
