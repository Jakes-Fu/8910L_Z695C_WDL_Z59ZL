...
static OD_BOOL OD_SocketEventDispatcher(OD_Context* context, OD_SYS_Event* sysEvt)
{
    ...
    switch (evt->SignalCode)
    {
        ...
        case SOCKET_ASYNC_GETHOSTBYNAME_CNF:
        {
            ...
            OD_TRACE_LOW("net: OD_SocketEventDispatcher, DNS event, h_addrtype = %d, h_length = %d ",
                dns->hostent.h_addrtype, dns->hostent.h_length);

            if (socketEvt.result == OD_SUCCESS)
            {
                int i = 0;

                if(AF_INET == dns->hostent.h_addrtype)		
                {
                    OD_ASSERT(dns->hostent.h_length == 4);
                }
                else if(AF_INET6 == dns->hostent.h_addrtype  )
                {
                    OD_ASSERT(dns->hostent.h_length == 16);
                }
