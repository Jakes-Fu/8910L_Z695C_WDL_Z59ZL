...
extern  OD_BOOL oprea_search_delete ;
...
OD_STATUS OD_FileRename(OD_Context *context, const OD_FileName *filename, const OD_FileName *newname)
{
	...
	unsigned short* opera_file_search_e = L"E:\\OperaMini\\mini4\\search.ini";
	unsigned short* opera_file_search_d = L"D:\\OperaMini\\mini4\\search.ini";
    CHECKFILEHANDLE;
...
if((od_wcscmp(newname,opera_file_search_e) == 0 ) || (od_wcscmp(newname,opera_file_search_d) == 0 ))
{
	unsigned short* opera_file_tmp = L"E:\\OperaMini\\mini4\\tmp.ini";
	unsigned short* opera_file_tmp_d = L"D:\\OperaMini\\mini4\\tmp.ini";
	SFS_HANDLE hHandle = 0;
	unsigned long int size = 0;
	unsigned int upos = 0;
	
	if(od_wcscmp(newname,opera_file_search_e) == 0 )
	{
	 	hHandle = MMIAPIFMM_CreateFile(opera_file_tmp, SFS_MODE_READ|SFS_MODE_OPEN_EXISTING, NULL, NULL); 
	}
	else
	{
		hHandle = MMIAPIFMM_CreateFile(opera_file_tmp_d, SFS_MODE_READ|SFS_MODE_OPEN_EXISTING, NULL, NULL); 
	}
	
	if(hHandle == NULL)
	{
		OD_TRACE_LOW("OD_FileRename search hHandle error");
	}
	//SFS_GetFileSize(hHandle, (unsigned long int*)size);

	
	SFS_SetFilePointer(hHandle,0,SFS_SEEK_END);
	SFS_GetFilePointer(hHandle, SFS_SEEK_BEGIN, &upos);
		
	OD_TRACE_LOW("OD_FileRename search return error  tmp ini %d hHandle %d upos %d ",size,hHandle,upos);
	SFS_CloseFile(hHandle);
	if(upos == 4)
	{
		OD_TRACE_LOW("OD_FileRename OD_FILE_NO_ACCESS return error id [%d] win_id [%d]",context->data.app.win_id,OPERAMINI_MAIN_WIN_ID);	
		ret = OD_FILE_NO_ACCESS;	

		oprea_search_delete = TRUE;
		return ret;
	}
...
OD_STATUS OD_GetDriveFreeSpace(OD_WCHAR * driveName, OD_INT64 * freeSpace)
{
    ...
    if(!MMIFILE_GetDeviceFreeSpace((const OD_WCHAR *)driveName, od_wcslen(driveName), &freeSpaceHigh, &freeSpaceLow))
        return OD_ERROR;
...
OD_STATUS OD_XDelete(OD_Context *context, const OD_FileName *path)
{
	...
	*pos = L'\0';
	...
	return _OD_TranslateFsError(MMIFILE_DeleteTreeEx(dir, fileNameOnly, SCI_TRUE));
#else
