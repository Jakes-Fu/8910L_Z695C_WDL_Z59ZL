...
typedef enum _OD_SYSTEM_PROPERTY
{
	...
	/** MEDIUM FONT support present */
	OD_MEDIUM_FONT_DISABLED,
...
#define OD_NATIVE_STR_MAX_UTF8_LENGTH	100
typedef enum _OD_NativeStringID
{
	OD_NATIVE_STR_BOOKMARKS_FULL,
	OD_NATIVE_STR_NUM
} OD_NativeStringID;

/*
 * Get native string
 * The result string should be put in buffer, encoded in UTF-8, whose length(including tailing '\0') shouldn't
 * exceed bufferSize
 */
OD_MMI_TASK OD_STATUS OD_GetNativeString(OD_NativeStringID nativeStringID, char* buffer, int bufferSize);
...
/**
* Gets the MCC and the MNC for the application.
*
* @param[in] context the context.
* @param[in] mccmnc a pointer to the OD_MccMnc container.
*
* @return OD_SUCCESS if the operation succeeded, OD_NULL_POINTER if
*         context is NULL, or the appropriate error code from the
*         OD_Error enumeration.
...
/**
*  if rtsp playing back is supported by platform.
*
*  @return OD_TRUE if support, else return OD_FALSE
**/
