							   ...
							   )
{
    ...
    struct bm_setting bmcfg = {0};
    bmcfg.addr_min = bm_start_addr;
    bmcfg.addr_max = bm_end_addr;

    if (0xFFFFFFFF == bm_data)
    {
        bmcfg.data_min = 0;
        bmcfg.data_max = 0xFFFFFFFF;
    }
    else
    {
        bmcfg.data_min = bm_data;
        bmcfg.data_max = bm_data;
    }

    bm_monitor_set(bm_mode, &bmcfg, bm_callback);
#else
#if 0
	bm_monitor_set(bm_start_addr, bm_end_addr, bm_data, bm_mode, bm_callback);
#else
	struct bm_setting cfg = {0};

	cfg.addr_min = bm_start_addr;
	cfg.addr_max = bm_end_addr;
	cfg.data_min = (bm_data == 0xffffffff) ? 0 : bm_data;
	cfg.data_max = bm_data;

	bm_monitor_set(bm_mode, &cfg, bm_callback);
#endif
#endif
