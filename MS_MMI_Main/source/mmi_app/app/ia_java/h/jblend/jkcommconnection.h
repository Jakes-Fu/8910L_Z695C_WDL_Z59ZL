 ...
 * If the parameters that are neither baudrate nor extended parameters are passed in optional parameters in the URL, the designated values are set by this function. If the parameters are not passed, the default values specified in MIDP 2.0 are set. 
 *
 ...
 *   Return #JK_COMM_RESULT_FAILURE if it fails. <br>
 *
 * In the native system, receive the data to the bufffer designated by the <i>buffer</i> parameter and the <i>length</i> parameter from the serial connection designated by the <i>handle</i> parameter. 
 *
 * This function can be implemented either as an asynchronous function or as a synchronous function.
 * If the processing by this function is likely to block JBlend processing for an extended period,
 * it should be implemented as an asynchronous function.
 ...
 * If #JK_COMM_PARAM_RXBLOCKING has been set to OFF, make an event notification to JBlend immediately. If the buffer is empty, set 0 to <i>result</i>. Otherwise, size of available data, at most <i>length</i> bytes.
 *
