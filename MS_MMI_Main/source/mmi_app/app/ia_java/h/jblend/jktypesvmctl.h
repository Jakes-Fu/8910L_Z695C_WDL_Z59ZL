...
#define JK_VMEVENT_APPBACKGROUND    11
#define JK_VMEVENT_APPFOREGROUND    12
...
/**
 * The Java application has requested resumption.
 *
 * This is the JVM event for notifying the native system that the Java application has requested resumption.
 * This is passed when a Java application in the Paused state calls 
 * the <code>javax.microedition.midlet.MIDlet</code> class <code>resumeRequest</code> method
 * to request resumption.
 * 
 * When this event is passed, the AMS should determine whether or not to actually resume the application.
 * If the application is to be resumed, call AamsVmResumeForeMIDlet().
 *
 * This JVM event is passed only when the Java application is in the Paused state.
 * 
 * This JVM event is implemented in the JBlend JVM libraries that support MIDP 2.1.
 */
