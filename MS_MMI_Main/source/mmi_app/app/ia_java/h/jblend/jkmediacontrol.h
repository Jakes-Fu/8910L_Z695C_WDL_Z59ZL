...
typedef struct {
    ...
    /**
     * Gets the class name with full path corresponding to a custom
     * control.
     * @param count
     ...
     * @param pStr
     *   A pointer to the buffer for storing the custom control class name 
     *   (full path)
     * @param length
     *   The size (number of characters) of the buffer designated in <i>pStr</i>
     * @return
     *   Return the number of characters of the string stored in the buffer 
     *   designated in <i>pStr</i>. Return a negative value if the class name could 
     *   not be obtained. <br>
     *   Return the required buffer size if the buffer is too small to store 
     *   the class name.
     *
     *   Put in their respective buffers the type and class name of the custom
...
typedef struct {
     ...
     *   In the case of media types defined for MMAPI 1.1 Live-media Capture,
     ...
     *   #JKMEDIA_STATE_ERROR<br>
     *   Return #JKMEDIA_STATE_STARTED when the native system is buffering 
     *   the data coming from streaming media.
     */
     ...
     *
     * When MIDP2.0 or MMAPI is supported, it is necessary to implement 
     ...
     *
     *  @if MIDP20
     *   If only MIDP2.0 is supported, set #JK_NULL.
     *  @endif
     */
