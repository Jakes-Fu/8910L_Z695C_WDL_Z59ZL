...
long AmFiberSuspend( JKFiberHandle hFiber );
...
long AmFiberResume( JKFiberHandle hFiber );
 ...
 *  JK_FIBER_SLEEP_MODE_LIGHT "JK_FIBER_SLEEP_MODE_XXX" flags are 
 *  ignored. If the join would occur, then the function will return 
 ...
 *  The error code #JK_RESULT_FIBER_SYNC_TIMEOUT is returned, indicating a 
 *  synchronization failure, if the Synchronized Section object is not 
 *  unlocked before the <i>ulDuration</i> parameter expires.
 *
 *  Valid values for <i>ulFlags</i> include either of the 
 *  @ref JK_FIBER_SYNC_METHOD_SLEEP "JK_FIBER_SYNC_METHOD_XXX" flags and 
 *  either of the @ref JK_FIBER_SLEEP_MODE_LIGHT "JK_FIBER_SLEEP_MODE_XXX"
 ...
 *  JK_FIBER_SLEEP_MODE_LIGHT "JK_FIBER_SLEEP_MODE_XXX" flags are 
 *  ignored. If the synchronization would occur, then the function will 
 *  return #JK_RESULT_FIBER_BUSY. 
 * 
 *  Valid values for the <i>ulDuration</i> parameter may be anything. A 
 *  synchronization duration of <code>0x00000000</code> may not seem very 
 *  practical but does support a few limited use cases. A synchronization 
 ...
 *  hold a lock on the Synchronized Section object or if the section has 
 *  been @ref AmFiberSectionEnter "entered" more than once recursively.
 * 
 ...
 *  AmFiberEventSignal "signaled". The error code 
 *  #JK_RESULT_FIBER_SYNC_TIMEOUT is returned, indicating a 
 *  synchronization failure, if the Synchronized Event object has not 
 *  been signaled before the <i>ulDuration</i> parameter expires.
 *
