...
// #endif
    ...
    case VP_CALLBACK_END_IND:     
        ...
        i_type = fsm_ptr->has_play_end_reason;
        
        SCI_TraceLow("FsmPaused: VP_CALLBACK_END_IND play_type",fsm_ptr->play_type);             
        if (VP_PLAY_FROM_BUFFER == fsm_ptr->play_type)
        {
            //关闭dplayer,进入ready状态
            DPLAYER_Close(fsm_ptr->dplayer_handle);
...
PUBLIC BOOLEAN MMIAPIVP_FSMCompulsivePause(VP_HANDLE  vp_handle)
{
   ...
   VP_FSM_T * fsm_ptr = (VP_FSM_T *)vp_handle;
    ...
    SCI_TRACE_LOW("[VP] fsm_ptr->total_time = 0x%x, fsm_ptr->state = %d, fsm_ptr->play_type = %d.", fsm_ptr->total_time, fsm_ptr->state, fsm_ptr->play_type);
    if(fsm_ptr->is_to_resume)
    {
       fsm_ptr->is_to_resume = FALSE;
    }
    ...
    {        
        ...
        //dis_param.videoSupport = 0; // Bug-1305320 流媒体播放会进入该分支
    ...
    else
    {
        ...
        ret = MMIAPIVP_FSMPause(vp_handle);
    }   
...
PUBLIC void MMIAPIVP_FSMCompulsiveResume(VP_HANDLE  vp_handle)
{
   ...
   BOOLEAN is_need_fill = TRUE;
    ...
    SCI_TRACE_LOW("[VP] fsm_ptr->is_to_pause = %d, fsm_ptr->total_time = 0x%x, fsm_ptr->state = %d.", fsm_ptr->is_to_pause, fsm_ptr->total_time, fsm_ptr->state);
    if(fsm_ptr->is_to_pause)
    {
        fsm_ptr->is_to_pause = FALSE;  
        ...
        && VP_STATE_PLAY_PENDING <= fsm_ptr->state)
    {
        ...
        SCI_TRACE_LOW("[VP] fsm_ptr->support_type = %d.", fsm_ptr->support_type);
        if (VP_SUPPORTED_NEITHER == fsm_ptr->support_type
            || VP_SUPPORTED_AUDIO_ONLY == fsm_ptr->support_type)
        {
             is_need_fill = FALSE;
                ...
                )
{
    ...
    if (PNULL == video_buf || 0 == video_length || video_type == MMICOM_VIDEO_TYPE_MAX)
    {
        //VP_STRACE_LOW:"[VP] MMIAPIVP_GetSupportedStatus param is null"
        SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMIVP_FSM_6207_112_2_18_3_5_16_259,(uint8*)"");
        return VP_SUPPORTED_NEITHER;
    ...
    MMIAPIVP_GetMediaInfoFromBuf(video_buf, video_length, video_type, &media_info);
