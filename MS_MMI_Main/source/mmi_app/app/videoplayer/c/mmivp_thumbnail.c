...
LOCAL BOOLEAN Thumbnail_VideoDelete(void)
{
    ...
    if(videoqueue->videonum > THUMBNAIL_QUEUENUM)
    {
        SCI_TRACE_LOW("Thumbnail_VideoDelete,ERROR! video num is over max!!!!\n");
        return result;
    }
    for(indx=0;indx<(videoqueue->videonum - 1);indx++)
    {
        //bug1284144 thumbnail_name最大下标为6, indx为6时不能再访问indx +1了
