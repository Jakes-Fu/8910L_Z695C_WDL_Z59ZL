...
LOCAL uint32 s_mmivp_curr_media_length = 0;/*---------------------------------------------------------------------------*/
/*                         GLOBAL DEFINITION                                 */
/*---------------------------------------------------------------------------*/
typedef enum
{
...
LOCAL BOOLEAN MMIAPIVP_SetSoftkeyTextId(
...
LOCAL BOOLEAN MMIAPIVP_OpenConfirmation(void);
/*****************************************************************************/
//  Description : handle unsupport format msg
//  Global resource dependence : none
//  Author: qi.yanqing
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleVPUnsupportFormatWin(
        MMI_WIN_ID_T     win_id,
        MMI_MESSAGE_ID_E msg_id,
        DPARAM           param
        );
...
WINDOW_TABLE( MMIVP_DEFAULT_LOCALLIST_POPMENU_WIN_TAB ) = 
{
    ...
    WIN_SOFTKEY(TXT_COMMON_DATE,TXT_NULL,STXT_RETURN),
    END_WIN
};
...
#else
                    // Bug 1291385 - [TJ][Local][T117]进入Video打开mp4视频播放，提示语“Unsupported video image,pl...”显示不完整
                    DisplayVPWinStation(TXT_VP_VIDIO_NOT_SUPPORT_MINI,TRUE,FALSE);  //显示播放中
#endif
                }
                else 
                {
...
PUBLIC void MMIAPIVP_SaveCurrVideoLen(uint32 len)
{
    s_mmivp_curr_media_length = len;
}

PUBLIC uint32 MMIAPIVP_GetCurrVideoLen()
{
    return s_mmivp_curr_media_length;
}
/*****************************************************************************/
//  Description : 播放视频文件
//  Global resource dependence : 
                                  ...
                                  )
{
    ...
    {
        MMI_WIN_ID_T        query_win_id = MMIVPVIEW_QUERY_WIN_ID;
        void                    *argv[2]        = {0};
        DPLAYER_HANDLE              dplayer_handle;
        DPLAYER_MEDIA_INFO_T    p_info      = {0};
        DPLAYER_RETURN_E        dplayer_result      = DPLAYER_SUCCESS;
        //modify for Bug 511127
#ifdef  VIDEOTHUMBNAIL_SUPPORT        
        if (MMIAPIVP_IsThumbnailRunning())
        {
            MMIAPIVP_ThumbnailStop();
            SCI_SLEEP(20);
        }
#endif		
        //modify end
        argv[0] = full_path_name_ptr;
        argv[1] = (void *)((uint32)DPLAYER_FA_MOD_RB);
        DPLAYER_Init();
        dplayer_handle = DPLAYER_Open(DPLAYER_OPEN_FROM_FILE, 2, argv);
        dplayer_result = DPLAYER_GetMovieInfo(dplayer_handle,&p_info);
        MMIAPIVP_SaveCurrVideoLen(p_info.media_length);
        DPLAYER_Close(dplayer_handle);
        DPLAYER_Release();
        SCI_TRACE_LOW("[VP]video resource, audio_support:%d,  video_support:%d", p_info.audio_info.audio_support, p_info.video_info.video_support);
        if (DPLAYER_SUCCESS != dplayer_result)
        {
            MMIPUB_OpenConfirmationDialogByTextId(PNULL,TXT_VP_UNSUPPORT_FORMAT_ID,PNULL,PNULL,&query_win_id,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,HandleVPUnsupportFormatWin);
            return FALSE;
        }
    }
...
{  
	...
	if( SCI_FALSE == MMIAPIVP_OpenConfirmation())
	{
		return ;
	}
    ...
    {
    	  MMIPUB_OpenAlertWarningWin(STR_NOTE_NO_CARD);
     //   MMIPUB_OpenAlertWinByTextId (PNULL, , , IMAGE_PUBWIN_WARNING, PNULL, PNULL, MMIPUB_SOFTKEY_ONE, PNULL);
          return FALSE;;
    }
...
LOCAL void VP_ValidatePrivacyAppEntry(void)
{
	...
	g_videoplayer_mode = 0;
	#endif
    MMIAPIVP_PlayVideoFromVideoPlayer(VP_ENTRY_FROM_PLAYER,PNULL,0,PNULL);
}
#ifdef ZDT_VIDEO_KING_SUPPORT
PUBLIC void MMIAPIVP_Open_byindex(uint8 index)
{
	g_videoplayer_mode = index;
	 MMIAPIVP_PlayVideoFromVideoPlayer(VP_ENTRY_FROM_PLAYER,PNULL,0,PNULL);
}
#endif
...
LOCAL MMI_RESULT_E HandleVPConfirmationDialogMsg(
                                            MMI_WIN_ID_T win_id,
                                            MMI_MESSAGE_ID_E msg_id,
                                            DPARAM param
                                            )
...
//	Note:
/*****************************************************************************/
LOCAL void MMIAPIVP_OpenConfirmationDialog(MMI_TEXT_ID_T text_id)
{
...
LOCAL BOOLEAN MMIAPIVP_OpenConfirmation(void)
{
	...
	{
		MMIAPIVP_OpenConfirmationDialog(STR_COM_TURN_BGD_MUSIC);
...
////////////进入后全屏
    ...
    case MSG_VP_LOADING_END:  //加载结束
            ...
            else
            {
                ...
                {       
                    if(MMI_DEVICE_NUM <= MMIAPIFMM_GetValidateDevice(MMI_DEVICE_UDISK))
                    {
                        ...
                        MMIPUB_OpenAlertWinByTextId(PNULL,TXT_PLEASE_INSERT_SD,TXT_NULL,IMAGE_PUBWIN_WARNING,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
                        /************************Modify over*************************/
                    }
                    else
                    {
                ...
                {  
                    MMIPUB_OpenAlertWinByTextId(PNULL,TXT_VP_LOADING_ERROR,TXT_NULL,IMAGE_PUBWIN_FAIL,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
        ...
        {            
            ...
            {
                MMIPUB_OpenAlertWinByTextId(PNULL,TXT_VP_LOADING_ERROR,TXT_NULL,IMAGE_PUBWIN_WARNING,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);//@andy.he 11-06-07 cr245369
            }
...
{   
   ...
   MMI_IMAGE_ID_T  impressed_id  = IMAGE_NULL;
    ...
    if (GUIFORM_CHILD_DISP_HIDE != display_type
        ...
        && VP_PLAY_FROM_NONE != s_video_player_control_info.play_type))
    {
...
{   
    ...
    SCI_TraceLow("cur play_type:%d; history type:%d",s_video_player_control_info.play_type, history_info_ptr->type);

    if(s_video_player_control_info.play_type != history_info_ptr->type)//fix bug1310617
    {
        return FALSE;
    }
...
LOCAL void VideoPlayerCompulsivePause(MMI_WIN_ID_T       win_id)
{
    ...
    {
        if(MMIAPIVP_FSMCompulsivePause(s_video_player_control_info.vp_handle))
        {
...
LOCAL BOOLEAN NotifyFsmPlay(void)
{
    ...
    else
    {
        ...
        {
            // Bug 1291385 - [TJ][Local][T117]进入Video打开mp4视频播放，提示语“Unsupported video image,pl...”显示不完整
            DisplayVPWinStation(TXT_VP_VIDIO_NOT_SUPPORT_MINI,TRUE,FALSE);
        }
        else
        {
    ...
    if ((VP_PLAY_FROM_LOCALLIST == s_video_player_control_info.play_type)
        && (VP_ENTRY_FROM_PLAYER == s_video_player_control_info.vp_entry))//bug1279598
    {
...
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPIVP_FSMUpdateCurrentVolNoHandle(uint8 vol)
{
...
LOCAL void HandleShortCutMsg(MMI_MESSAGE_ID_E msg_id)
{
    ...
    case MSG_KEYDOWN_Y:
        ...
        DisplayOSD();
#else
#if 0//def MMI_STREAMING_PLAYER_SUPPORT//delete by James li
...
LOCAL void SetFormParam(void)
{
    ...
    screen_rect.top = screen_rect.top;// + MMI_STATUSBAR_HEIGHT;//change by James li
    if(MMITHEME_IsMainScreenLandscape())
    {          
        SetFormParamH();
    }
...
#ifndef MMI_PDA_SUPPORT
    height = (uint16)((uint16)(screen_rect.bottom - screen_rect.top + 1 - height_button) - height_title - VP_PROCESS_HEIGHT - MMITHEME_GetSoftkeyHeight() - 4*VP_TITLE_MARGIN);
#else
    //PDA has no softkey
...
PUBLIC BOOLEAN MMIAPIVP_IsVideoDisplaying()
{
    return (s_video_player_control_info.state == VP_STATE_PLAYING) || (s_video_player_control_info.state == VP_STATE_PAUSED);
}
/*****************************************************************************/
//  Description : 视频播放器主界面消息处理.
//  Global resource dependence : 
                                     ...
                                     )
{
	...
	//	LCD_SetOpMode(MAIN_LCD_ID, 1);
	///	LCD_SetOpMode(MAIN_LCD_ID, 0);		
    ...
    case MSG_FULL_PAINT:
        ...
        DisplayVPWinRandomModeIcon(TRUE);       
	 DisplayVPWinProgress(TRUE);//add by James li
        if (!MMK_IsFocusWin(win_id)
            && !s_video_player_control_info.is_display_win)
        {           
...
//        MMIAPIVP_FSMExit(s_video_player_control_info.vp_handle);        
    ...
    case MSG_GET_FOCUS: 
        ...
        SCI_TraceLow("handlevpwinmsg: MSG_GET_FOCUS s_is_complusive_pause=%d, state=%d,repeat mode=%d",
        s_is_complusive_pause,s_video_player_control_info.state,VP_Setting_GetRepeatPlayMode());
 
        if((s_is_complusive_pause) && (VP_STATE_READY == s_video_player_control_info.state)) 
        {
            if(VP_REPEAT_PLAY_MODE_ONE == VP_Setting_GetRepeatPlayMode())
                PlayOrPause();
            else
                PlayNextFile();
            s_is_complusive_pause = FALSE;
...
#if 0
def MMI_STREAMING_PLAYER_SUPPORT
    MMI_GetLabelTextByLang(TXT_VP_NET_VIEW, &kstring);
    node_item.item_text_ptr = &kstring;
    node_item.is_grayed = FALSE;
    GUIMENU_InsertNode(j, ID_VP_OPEN_NET, ID_VP_OPEN, &node_item, ctrl_id);
...
#if 0//def SCREENSAVER_SUPPORT//delete by James li
        MMI_GetLabelTextByLang(TXT_COMMON_SCREENSAVE, &kstring);
        node_item.item_text_ptr = &kstring;
        node_item.is_grayed = ((0==s_video_player_control_info.full_path_len)? TRUE:FALSE);
        GUIMENU_InsertNode(j, ID_VP_SCEENSAVE, ID_VP_SETTING_AS, &node_item, ctrl_id);
...
#if 0//def MMI_VIDEOWALLPAPER_SUPPORT//delete by James li
        MMI_GetLabelTextByLang(TXT_VIDEO_WALLPAPER, &kstring);
        node_item.item_text_ptr = &kstring;
        node_item.is_grayed = ((0==s_video_player_control_info.full_path_len)? TRUE:FALSE);
        GUIMENU_InsertNode(j, ID_VP_VIDEO_WALLPAPER, ID_VP_SETTING_AS, &node_item, ctrl_id);
...
#if 0//ndef MMI_STREAMING_PLAYER_SUPPORT//delete by James li
                                    ...
                                    DPARAM param)
{
    ...
    switch(msg_id)
    {
     case MSG_OPEN_WINDOW:
         ...
         if (0 == list_num)
         {
             MMIAPIVP_SetSoftkeyTextId(win_id, STXT_OPTION, TXT_NULL, STXT_RETURN, FALSE); 
         }
         else
         {
             MMIAPIVP_SetSoftkeyTextId(win_id, STXT_OPTION, TXT_COMMON_PLAY, STXT_RETURN, FALSE); 
         }
#ifdef MMI_PDA_SUPPORT
    ...
    { 
...
#endif
        {
            ...
            NotifyPlayerForPlayFile(history_info_ptr->local_movie_file_info_ptr->full_path_ptr,
                ...
                process_time,
                history_info_ptr->total_time,
                history_info_ptr->type);//bug1268032
        }
        return TRUE;        
    }
    else
...
#endif
#endif
#if 0//ndef MMI_VIDEO_PLAYER_MINI_LAYOUT//delete by James li   
   if(IsDisplayFullScreenReferInfoEnable())
    {
        if (LCD_ANGLE_90 == s_video_player_control_info.cur_angle
            || LCD_ANGLE_270 == s_video_player_control_info.cur_angle)
...
//            GUITAB_SetItemWidth(ctrl_id, img_width);
            ...
            {
                curr_index = VP_LocalList_GetCurrentIndex(VP_LOCALLIST_DEFAULT);
                GUILIST_SetCurItemIndex(VP_DEFAULT_LOCALLIST_CTRL_ID, curr_index == -1 ? 0 : curr_index);
            ...
            {
                curr_index = VP_LocalList_GetCurrentIndex(VP_LOCALLIST_USER);
                GUILIST_SetCurItemIndex(VP_USER_LOCALLIST_CTRL_ID, curr_index == -1 ? 0 : curr_index);
                                    ...
                                    DPARAM param)
{
    ...
    switch(msg_id) 
    {
     case MSG_OPEN_WINDOW: 
         ...
         {
            #ifdef MMI_VIDEO_PLAYER_MINI_SUPPORT //6530_MINI
                MMIAPIVP_SetSoftkeyTextId(win_id, TXT_NULL, TXT_NULL, STXT_RETURN, FALSE);
            #else
                MMIAPIVP_SetSoftkeyTextId(win_id, STXT_OPTION, TXT_NULL, STXT_RETURN, FALSE);
            #endif //MMI_VIDEO_PLAYER_MINI_SUPPORT //6530_MINI
         }                 
   ...
   case MSG_GET_FOCUS:
       ...
       {            
            ...
            {                
                #ifdef MMI_VIDEO_PLAYER_MINI_SUPPORT //6530_MINI
                    MMIAPIVP_SetSoftkeyTextId(win_id, TXT_COMMON_PLAY, TXT_NULL, STXT_RETURN, FALSE);
                #else
                    MMIAPIVP_SetSoftkeyTextId(win_id, STXT_OPTION, TXT_COMMON_PLAY, STXT_RETURN, FALSE);
                #endif          
    ...
    case MSG_APP_WEB:
        ...
        case ID_VP_DELETE:
            ...
            else //删除标记
            {
                ...
                if (PNULL != select_item_ptr)
                {
                    MMIPUB_OpenAlertWinByTextId(PNULL,TXT_COMMON_WAITING,TXT_NULL,IMAGE_PUBWIN_WAIT,PNULL,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
...
//  Note: 
/*****************************************************************************/
BOOLEAN MMIAPIVP_SetSoftkeyTextId(
                                       MMI_HANDLE_T    win_handle,    // 窗口句柄
                                       MMI_TEXT_ID_T   leftsoft_id,   // the left softkey id
                                       MMI_TEXT_ID_T   middlesoft_id, // the middle softkey id
                                       MMI_TEXT_ID_T   rightsoft_id,  // the right softkey id
...
LOCAL void DisplayVPWinSoftkey(BOOLEAN       is_refresh)
{
   ...
   MMI_IMAGE_ID_T  impressed_id  = IMAGE_NULL;
    ...
    else
    {
        ...
        MMIAPIVP_SetSoftkeyTextId(MMIVP_PLAYER_WIN_ID, leftsoft_id,middlesoft_id,rightsoft_id,is_refresh);
    }
}
...
LOCAL BOOLEAN IsAdjustBrightEnable(void)
{
    //bug1268270 enable adjust bright when state ready
    if (VP_STATE_STOPING != s_video_player_control_info.state)
    {
        return TRUE;
    }
    else
...
{
#if defined(MMI_MULTI_SIM_SYS_SINGLE) || defined(WIFI_SUPPORT)
...
LOCAL void InitVpPdaSettingsListCtrl(void)
{
    ...
    for (i = MN_DUAL_SYS_1; i < MMI_DUAL_SYS_MAX; i++)
    {
        switch((MN_DUAL_SYS_E)i)
        {
            case MN_DUAL_SYS_1:
                text_id = TXT_COMMON_SET_NET_SIM1;
                settings_net = MMIVP_PDA_SETTINGS_NET1;
                break;
#if defined(MMI_MULTI_SIM_SYS_DUAL) || defined(MMI_MULTI_SIM_SYS_TRI) || defined(MMI_MULTI_SIM_SYS_QUAD)
            case MN_DUAL_SYS_2:
                text_id = TXT_COMMON_SET_NET_SIM2;
                settings_net = MMIVP_PDA_SETTINGS_NET2;
                break;
#endif
#if defined(MMI_MULTI_SIM_SYS_TRI) || defined(MMI_MULTI_SIM_SYS_QUAD)
            case MN_DUAL_SYS_3:
                text_id = TXT_COMMON_SET_NET_SIM3;
                settings_net = MMIVP_PDA_SETTINGS_NET3;
                break;
#endif
#if defined(MMI_MULTI_SIM_SYS_QUAD)
            case MN_DUAL_SYS_4:
                text_id = TXT_COMMON_SET_NET_SIM4;
                settings_net = MMIVP_PDA_SETTINGS_NET4;
                break;
#endif
            default:
                break;
        ...
        {
            //bug1258830 sim card is disabled, flight mode,no sim,pukblocked,pinblocked,sim reject,sim not support,the network connect settings should be grayed
            if(TRUE == MMIAPIPHONE_IsSimCardOk((MN_DUAL_SYS_E)i))
            {
                BOOLEAN existValidLinkSetting = FALSE;
                uint8 linkSettingNum = MMIAPICONNECTION_GetLinkSettingNum((MN_DUAL_SYS_E)i);

                link_index = VP_Setting_GetNetIndex((MN_DUAL_SYS_E)i);
                while(1)
                {
                    // Bug-1310629
                    if ((0 == linkSettingNum) || (link_index > (linkSettingNum - 1)))
                    {
                        SCI_TRACE_LOW("No valid link setting. link_index = %d, linkSettingNum = %d.", link_index, linkSettingNum);
                        existValidLinkSetting = FALSE;
                        break;
                    }
                    link_setting_ptr = MMIAPICONNECTION_GetLinkSettingItemByIndex((MN_DUAL_SYS_E)i, link_index);
                    link_index++;
                    if(PNULL != link_setting_ptr)
                    {
                        if(TRUE == link_setting_ptr->visible)
                        {
                            SCI_TRACE_LOW("Has valid link setting. link_index = %d, linkSettingNum = %d.", link_index, linkSettingNum);
                            existValidLinkSetting = TRUE;
                            break;
                        }
                    }
                    else
                    {
                        SCI_TRACE_LOW("[VP] InitVpPdaSettingsListCtrl link_setting_ptr = PNULL");
                        break;
                    }
                }
                if (TRUE == existValidLinkSetting)
                {
        ...
        GUILIST_SetItemGray(ctrl_id, settings_net, is_gray);
    }
...
LOCAL void AppendRepeatListItem(void)
{
    ...
    MMIAPISET_AppendListItemByTextId(TXT_COMMON_SET_REPEAT_ALL, TXT_COMMON_OK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
    MMIAPISET_AppendListItemByTextId(TXT_VP_SINGLE, TXT_COMMON_OK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
                    ...
                    )
{
    ...
    MMIAPISET_AppendListItemByTextId(TXT_CLOSE,TXT_COMMON_OK,ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
    MMIAPISET_AppendListItemByTextId(TXT_COMMON_SET_REPEAT_ALL,TXT_COMMON_OK,ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
    MMIAPISET_AppendListItemByTextId(TXT_COMM_REPEAT_ONE,TXT_COMMON_OK,ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
...
PUBLIC BOOLEAN HERO_MMIVP_IsOpened(void)
{    
    return MMK_IsOpenWin(MMIVP_MAIN_PLAY_WIN_ID);
}
#endif
