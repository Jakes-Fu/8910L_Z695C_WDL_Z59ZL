...
//boda, QQVGA it sometimes think about the profile
...
#else
#define CC_MAIN_INFO_COMMON_LABEL_TOP_MARGIN	(88)		// 通话介面中LABEL控件与上边的margin
#define CC_MAIN_INFO_COMMON_LABEL_LR_MARGIN		(3)					// 通话介面中LABEL控件与左右边的margin
#define CC_MAIN_INFO_COMMON_LABEL_HEIGHT		(12)				// 通话介面中LABEL控件的高度
#define CC_MAIN_INFO_COMMON_SMALL_LABEL_HEIGHT 	(12)				// 通话介面中较小LABEL控件的高度
#define CC_MAIN_INFO_NAME_LABEL_LR_MARGIN		(11)				// 通话介面中姓名LABEL控件左右边的margin
#define CC_MAIN_INFO_COMMON_LABEL_UD_MARGIM		(2)					// 通话介面中各LABEL控件之间的垂直margin
#endif
...
LOCAL MMIRECORD_SRV_RESULT_E s_record_result = MMIRECORD_SRV_RESULT_SUCCESS;
#endif
/*---------------------------------------------------------------------------*/
...
/*****************************************************************************/
// 	Description : the process KBD for 128x160 LCD
//	Global resource dependence : 
//    Author:boda.lin
//	Note:2021/01/22
/*****************************************************************************/
LOCAL MMI_RESULT_E CC_HandleCCMiniWinMsg
(
	MMI_WIN_ID_T		win_id, 
	MMI_MESSAGE_ID_E	msg_id, 
	DPARAM				param
...
//	Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E CC_HandleCCAnimMiniWinMsg
(
	MMI_WIN_ID_T		win_id, 
	MMI_MESSAGE_ID_E	msg_id, 
	DPARAM				param
...
{
    WIN_STATUSBAR,
    WIN_FUNC((uint32)CC_HandleCCMiniWinMsg),    
    WIN_ID(MMICC_STATUS_WIN_ID),
    CREATE_EDIT_PHONENUM_CTRL(MMICC_PHONE_NUM_MAX_LEN,MMICC_MO_CALL_DTMF_CTRL_ID),
    WIN_SOFTKEY(STXT_SOFTKEY_MENU_MK, TXT_HAND_ON, TXT_NULL),
	END_WIN
};
#endif
...
{
    WIN_STATUSBAR,
    WIN_FUNC((uint32)CC_HandleCCAnimMiniWinMsg),    
    WIN_ID(MMICC_ANIMATION_WIN_ID),
    WIN_SOFTKEY(TXT_NULL, STXT_SOFTKEY_ANSWER_MK, TXT_RING_SILENT),
    END_WIN
};
#endif
...
#if defined (MMI_CALLCONTROL_MINI_SUPPORT)
#define BOTTOM_BUTTON_MARGIN    3
#define BOTTOM_BUTTON_WIDTH    19
#define BOTTOM_MID_WIDTH    82
#else
    ...
    )
{
   ...
   // MMI_CTRL_ID_T       ctrl_id = MMICC_COM_SELECT_SIM_CTRL_ID;
    ...
    {
        MMI_STRING_T    titlestr = {0};
        MMI_GetLabelTextByLang(TXT_SIM_SEL, &titlestr);
        GUIMENU_SetMenuTitle(&titlestr, menu_ctrl_id);
    }
#endif
...
)
{
...
#if defined (MAINLCD_SIZE_128X160)//boda
	...
	label_init.both_rect.v_rect.left = CC_MAIN_INFO_COMMON_LABEL_LR_MARGIN;
	label_init.both_rect.v_rect.right = MMI_MAINSCREEN_WIDTH - CC_MAIN_INFO_COMMON_LABEL_LR_MARGIN;
	label_init.both_rect.v_rect.top = CC_MAIN_INFO_COMMON_LABEL_TOP_MARGIN + (CC_MAIN_INFO_COMMON_LABEL_HEIGHT * 2);
	label_init.both_rect.v_rect.bottom =  label_init.both_rect.v_rect.top + CC_MAIN_INFO_COMMON_LABEL_HEIGHT;
#endif//#if defined (MAINLCD_SIZE_128X160)//boda
	PdaCallCreateLabelByTextPtr(win_id,
			MMICC_CONNECTED_STATE_LABEL_CTRL_ID, 
...
)
{
	...
	wchar display_wchar[MMIPB_NAME_MAX_LEN + MMIFINDMP_CITYNAME_LENGTH + 6]={0};
	MMI_STRING_T prompt_name_str = prompt_str_array[CC_DISPALY_INFO_NAME];
	MMI_STRING_T prompt_number_str = prompt_str_array[CC_DISPALY_INFO_NUMBER];
	char number[24] = {0};
#endif    
	if (PNULL == prompt_str_array)
	{
	...
	MMK_DestroyControl(MMICC_CONNECTED_NAME_LABEL_CTRL_ID);
	MMK_DestroyControl(MMICC_CONNECTED_NUMBER_LABEL_CTRL_ID);
#if defined(HERO_ENGINE_SUPPORT)&&defined(HERO_ENGINE_HMGS_SUPPORT)&&defined(HERO_ENGINE_HMGS_SUPPORT_ON_OTHER_LINE)
    MMK_DestroyControl(MMICC_CONNECTED_HMGS_LABEL_CTRL_ID);
#endif
	MMK_DestroyControl(MMICC_CONNECTED_STATE_LABEL_CTRL_ID);
#ifdef MMI_CALLCONTROL_MINI_SUPPORT
	MMK_DestroyControl(MMICC_CONNECTED_TIME_LABEL_CTRL_ID);
...
#if defined(HERO_ENGINE_SUPPORT)&&defined(HERO_ENGINE_HMGS_SUPPORT)
	{
		...
		display_str.wstr_ptr = display_wchar;
		GUI_WstrToUTF8(number, 24, prompt_number_str.wstr_ptr, prompt_number_str.wstr_len);
		hccall_printf("[INKLEAK_LOG] mmicc_wintab.c PdaDisplaySingleCallInfoForCommon 6205 number = %s", number);
		HcCall_hist_populate_call_list(cityName, 64, number);
		cityName_len = MMIAPICOM_Wstrlen((const wchar *) &cityName[0]);
		hccall_printf("[INKLEAK_LOG] mmicc_wintab.c PdaDisplaySingleCallInfoForCommon 6209 str_length = %d", cityName_len);
		MMI_WSTRNCPY(display_wchar,  MMIFINDMP_CITYNAME_LENGTH + 6, 
			(const wchar*)cityName, cityName_len, cityName_len);
		display_str.wstr_len = cityName_len;
	}
#endif
#if 0 // no use
#if defined(HERO_ENGINE_SUPPORT)&&defined(HERO_ENGINE_HMGS_SUPPORT)
	{
		...
		cityName_len = MMIAPICOM_Wstrlen((const wchar *) &cityName[0]);
		hccall_printf("[INKLEAK_LOG] mmicc_wintab.c PdaDisplaySingleCallInfoForCommon 6209 str_length = %d", cityName_len);
        #if defined(HERO_ENGINE_HMGS_SUPPORT_ON_OTHER_LINE)
		//MMI_WSTRNCPY(display_wchar, MMIPB_NAME_MAX_LEN + MMIFINDMP_CITYNAME_LENGTH + 6, 
			//prompt_name_str.wstr_ptr, prompt_name_str.wstr_len, prompt_name_str.wstr_len);
		MMI_WSTRNCPY(display_wchar,  MMIFINDMP_CITYNAME_LENGTH + 6, 
			(const wchar*)cityName, cityName_len, cityName_len);
		display_str.wstr_len = prompt_name_str.wstr_len + cityName_len;
	 #else
		MMI_WSTRNCPY(display_wchar, MMIPB_NAME_MAX_LEN + MMIFINDMP_CITYNAME_LENGTH + 6, 
			prompt_name_str.wstr_ptr, prompt_name_str.wstr_len, prompt_name_str.wstr_len);
		MMI_WSTRNCPY(display_wchar+prompt_name_str.wstr_len, MMIPB_NAME_MAX_LEN + MMIFINDMP_CITYNAME_LENGTH + 6, 
			(const wchar*)cityName, cityName_len, cityName_len);
		display_str.wstr_len = prompt_name_str.wstr_len + cityName_len;
	 #endif
	}
	...
	*/
	...
	label_init.both_rect.v_rect.left = CC_MAIN_INFO_COMMON_LABEL_LR_MARGIN;
	label_init.both_rect.v_rect.right = MMI_MAINSCREEN_WIDTH - CC_MAIN_INFO_COMMON_LABEL_LR_MARGIN;
	label_init.both_rect.v_rect.top = label_init.both_rect.v_rect.bottom;
	label_init.both_rect.v_rect.bottom =  label_init.both_rect.v_rect.top + CC_MAIN_INFO_COMMON_SMALL_LABEL_HEIGHT;
#else
	label_init.both_rect.v_rect.left = CC_MAIN_INFO_COMMON_LABEL_LR_MARGIN;
	label_init.both_rect.v_rect.right = MMI_MAINSCREEN_WIDTH - CC_MAIN_INFO_COMMON_LABEL_LR_MARGIN;
...
#if defined (MAINLCD_SIZE_128X160)
            anim_init.both_rect.v_rect.left = 32;
            anim_init.both_rect.v_rect.right = anim_init.both_rect.v_rect.left + 61; //image width / 2
            anim_init.both_rect.v_rect.top = MMITHEME_GetStatusBarHeight()+ 8;
            anim_init.both_rect.v_rect.bottom =  anim_init.both_rect.v_rect.top + 61;
#else
            anim_init.both_rect.v_rect.left = 59;
            anim_init.both_rect.v_rect.right = anim_init.both_rect.v_rect.left + 122;
            anim_init.both_rect.v_rect.top = MMITHEME_GetStatusBarHeight()+ 28 + 1;
            anim_init.both_rect.v_rect.bottom =  anim_init.both_rect.v_rect.top + 122;
#endif
...
LOCAL void CustomDisplayMultiCallPhotoForCommon( MMI_WIN_ID_T win_id, CC_PHOTO_TYPE_E photo_type)
{
    ...
    switch (win_id)
    {
        ...
        case MMICC_STATUS_WIN_ID:
            anim_init.both_rect.v_rect.left = 59;
            anim_init.both_rect.v_rect.right = anim_init.both_rect.v_rect.left + 122;
            anim_init.both_rect.v_rect.bottom =  MMI_MAINSCREEN_HEIGHT - MMITHEME_GetSoftkeyHeight() - 32 - 24 - 1;
            anim_init.both_rect.v_rect.top = anim_init.both_rect.v_rect.bottom - 122;
            anim_ctrl_id = MMICC_CONNECTED_PHOTO_ANIM_CTRL_ID;
            break;
                                   ...
                                   )
{
    ...
    if (CC_RELEASE_OPER != MMICC_GetCallOperatedByIndex(call_index)
        && 0 != call_time)
    {
        MMICC_SetIsPlayCallEndHintFlag(FALSE);////TRUE
    }
    else
    {
...
)
{
    ...
    case MSG_OPEN_WINDOW:
...
#endif            
        {
            //显示来电界面允许播放铃声，cr116652
            need_playring = FALSE;
		Trace_Log_Print("MSG_TTS_OVER  PlayNewCallRing cc_tts_over false ");
	     cc_tts_over = FALSE;
...
#endif//#if defined(MMI_GUI_STYLE_TYPICAL)||(!defined(MMI_GUI_STYLE_TYPICAL)&&!defined(TOUCH_PANEL_SUPPORT))
    ...
    case MSG_OPEN_WINDOW:
		...
		MMICC_GetCallNumStrByIndex(tel_num, &tel_num_len, MMICC_GetCurrentCallIndex());
              ...
              if (tel_num_len > 0)
	       {
			...
			{
                            need_playring = TRUE;
				GUIWIN_SetSoftkeyTextId(win_id, TXT_NULL, STXT_SOFTKEY_ANSWER_MK, TXT_RING_SILENT, TRUE);                        
			}
			else
			{
                             need_playring = FALSE;
			        GUIWIN_SetSoftkeyTextId(win_id, TXT_NULL, STXT_SOFTKEY_ANSWER_MK, TXT_NULL, TRUE);	
			}//if(MMIENVSET_SILENT_MODE != MMIENVSET_GetCurModeId())
	       }
       ...
       if(MMICC_IsEmergencyCall())
       {
	       if(MMIAPICC_IsHandFree())
	        {
	             GUIWIN_SetSoftkeyTextId(win_id, TXT_NULL, TXT_HAND_ON, TXT_NULL,TRUE);
	        }
		 else
		 {
			GUIWIN_SetSoftkeyTextId(win_id, TXT_NULL, TXT_HAND_FREE, TXT_NULL,TRUE);
		 }
       }
	 else if(MMICC_IsExistOutgoingCall())
       {
...
//        MMK_CloseWin(win_id);
    ...
    case MSG_CTL_MIDSK:
        if(MMICC_IsExistOutgoingCall())
        {
	      ...
	      if(MMIAPICC_IsHandFree())
            {
                GUIWIN_SeSoftkeytButtonTextId(win_id,TXT_HAND_ON , MIDDLE_BUTTON, TRUE);
            }
	     else
	     {
                GUIWIN_SeSoftkeytButtonTextId(win_id, TXT_HAND_FREE, MIDDLE_BUTTON, TRUE);
	     }
        }
...
#if defined(MMI_REJECT_CALL_BY_SMS_SUPPORT)
#ifdef CMCC_UI_STYLE
        if(reject_info.is_sms_reply_open &&)
        {
...
//#endif
            //if(avatar_type != AVATAR_TYPE_RECORDING)
            {
            display_info.bg.bg_type = GUI_BG_NONE;//GUI_BG_IMG;
            display_info.bg.img_id = 0; //IMAGE_CC_AVATAR_INITIALS;
            }
#if defined(TOUCH_PANEL_SUPPORT) && !defined(MMI_GUI_STYLE_TYPICAL)
            if(MMICC_ANIMATION_WIN_ID == win_id
                && MMIAPIKL_IsPhoneLocked())
...
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPICC_StopCCRecord(BOOLEAN is_prompt)
{

    SCI_TRACE_LOW("ccapp MMIAPICC_StopCCRecord s_record_result = %d", s_record_result);
...
PUBLIC BOOLEAN MMIAPICC_StopRecordByCallId(uint8 call_id)
{
    ...
    SCI_TRACE_LOW("ccapp MMIAPICC_StopRecordByCallId s_record_result = %d", s_record_result);
...
LOCAL void HandleRecordEndMsg(MMIRECORD_SRV_RESULT_E result)
{
	...
	MMIDEFAULT_TurnOnBackLight();
        ...
        SCI_TRACE_LOW("ccapp HandleRecordEndMsg, result = %d", result);
...
{    
	...
	MMIFILE_DEVICE_E record_dev = MMIAPIRECORD_GetRecordStorage();
       ...
       s_record_result = MMIRECORD_SRV_RESULT_SUCCESS;
...
LOCAL void CC_BeginRecord(uint8 timer_id, uint32 param)
{
    ...
    SCI_TRACE_LOW("ccapp CC_BeginRecord");
    
    if(MMIRECORD_SRV_RESULT_SUCCESS != error_code)
    {
         s_record_result = error_code;
        SCI_TRACE_LOW("ccapp CC_BeginRecord, s_record_result = %d", s_record_result);
        MMIAPICC_StopCCRecord(TRUE);
    }
    switch (error_code)
    {
						...
						case MMISRVAUD_REPORT_END:
							...
							if(start_record && !MMICC_IsCCRecording())
							{
								...
								error_code = StartCCRecord();
                                                        ...
                                                        SCI_TRACE_LOW("ccapp RecordTonePlayCallback, s_record_result = %d", s_record_result);
        ...
        if (!MMIAPIENG_GetIQModeStatus())
        {
		 ...
		 MMICC_OpenAlertSuccessWin(TXT_CC_STARTRECORD);	
            s_record_result = error_code;
             ...
             SCI_TRACE_LOW("ccapp HandleRecordOpt, s_record_result = %d", s_record_result);
            
            if(MMIRECORD_SRV_RESULT_SUCCESS != error_code)
            {
                MMIAPICC_StopCCRecord(FALSE);
            }
                              ...
                              )
{
    ...
    if (CC_BUTTON_HANDFREE != CC_INSTANCE.cc_win_focus_button_type )//phone locked by dm,  forbid hot buttons except handfree
    {
        return ;
    }
...
{     
	...
	if (MMICC_BUTTON_HANDFREE_CTRL_ID != ctrl_id && MMICC_BUTTON_ANSWER_CTRL_ID != ctrl_id )//window locked by dm,  forbid some hot button
    {
        return ;
    }
...
//boda, call out run here!!!!
#ifndef MMI_GUI_STYLE_TYPICAL
    if (MMICC_IsExistOutgoingCall())
    {
        return CC_HandleCCAnimMiniWinMsg(win_id, msg_id, param);
    }
#endif
...
#endif//#ifdef MMI_RECORD_SUPPORT            
        }
#endif//#ifdef MMI_AUTO_RECORD_SUPPORT
    ...
    case MSG_APP_RIGHT:
        {
          //boda, remove not use and receive the key event to avoid anim ctrl to action
        }
        break;
    ...
    case MSG_FULL_PAINT:
	 ...
	 else
	 {
	       if(MMIAPICC_IsHandFree())
	        {
	             GUIWIN_SetSoftkeyTextId(win_id, STXT_SOFTKEY_MENU_MK, TXT_HAND_ON, TXT_NULL,TRUE);
	        }
		 else
		 {
			GUIWIN_SetSoftkeyTextId(win_id, STXT_SOFTKEY_MENU_MK, TXT_HAND_FREE, TXT_NULL,TRUE);
		 }
	 }//if(MMICC_IsEmergencyCall)
...
#endif        
#endif//#ifndef MMI_GUI_STYLE_TYPICAL
        {
            MMI_STRING_T stb_str = {0};
            CustomUpdateHeaderinfo(stb_str);
        }
    ...
    case MSG_APP_CAMERA:
        ...
        break;
#endif
#endif//#ifndef PRINTSCREEN_SUPPORT 
...
LOCAL void InitBottomButtonsRects(MMI_WIN_ID_T win_id)
{
    ...
    button_right_rect.right = full_rect.right - BOTTOM_BUTTON_MARGIN;
    button_right_rect.left = button_right_rect.right-LRBUTTON_WIDTH;
    button_right_rect.top = full_rect.bottom - MMITHEME_GetSoftkeyHeight() + BOTTOM_BUTTON_MARGIN;
    button_right_rect.bottom = full_rect.bottom - BOTTOM_BUTTON_MARGIN;
...
LOCAL void CustomShowMultiCallList(MMI_WIN_ID_T win_id,MMI_CTRL_ID_T ctrl_id,BOOLEAN only_one)
{
    ...
    if(only_one)
    {
        ...
        list_init.both_rect.v_rect.bottom = list_init.both_rect.v_rect.top + 80;
    }
    else
    {
        list_init.both_rect.v_rect.left = client_rect.v_rect.left;
...
LOCAL BOOLEAN CustomGetMultiCallListAvatar(uint8 call_id,GUIITEM_CONTENT_T *item_content,MMI_CTRL_ID_T ctrl_id,uint16 index)
{
    ...
    if(CC_HaveRelatedRecordCall(call_id))
    {
        ...
        item_content->item_data.image_id = THEME_RECORDING_ANIM_SMALL_IMAGE[MMITHEME_GetAccentId()];
    }
...
// Bug 1367006, end
    ...
    else if(MMICC_GetCallStatusByIndex(call_index) == CC_HOLD_STATE)
    {
        if(!MMICC_IsExistIncommingCall())
        {
            CTRLLIST_SetItemShowLRTriangleIconByIndex(MMICC_MULTI_CALL_ANIMATION_LIST_ID,index,FALSE,TRUE);
            CTRLLIST_SetItemHandleLeftRightByIndex(MMICC_MULTI_CALL_ANIMATION_LIST_ID,index,FALSE,TRUE);
...
//#ifndef MMI_MULTI_SIM_SYS_SINGLE
    if(MMIAPICC_IsVolteNetworkStatus())
    {
	    ...
	    MN_DUAL_SYS_E dual_sys = MN_DUAL_SYS_1;
#if defined (MAINLCD_SIZE_128X160)
	    nt_ind_rect.left = 3;//boda, it sometimes over the "+8860919999999"
#else
	    nt_ind_rect.left = 6;
#endif
	    nt_ind_rect.right = nt_ind_rect.left + 20;
	    nt_ind_rect.top = d_rect->bottom +16;
	    nt_ind_rect.bottom = d_rect->bottom+36;
	    dual_sys = MMIAPICC_GetCallingSIM();	
	    if(MMIAPIPHONE_volte_icon_display_style(dual_sys) == 1)
	    {
...
#if defined(MAINLCD_SIZE_240X320)	
        sim_rect.top = d_rect->bottom - 16+34;
        sim_rect.bottom = d_rect->bottom+34;
        sim_rect.right = d_rect->right + 6+16+25;
        sim_rect.left = d_rect->right + 6+25;
#else
        sim_rect.top = d_rect->top;
        sim_rect.bottom = d_rect->bottom;
        sim_rect.right = d_rect->right;
        sim_rect.left = d_rect->left;
#endif
		}
...
#if defined(MAINLCD_SIZE_240X320)	
        sim_rect.top = d_rect->bottom - 16+26;
        sim_rect.bottom = d_rect->bottom+26;
        sim_rect.right = d_rect->right + 6+16+25;
        sim_rect.left = d_rect->right + 6+25;
#else
        sim_rect.top = d_rect->bottom;
        sim_rect.bottom = d_rect->bottom;
        sim_rect.right = d_rect->right;
        sim_rect.left = d_rect->right;
#endif
		}
	else
		{
...
LOCAL void CustomAnimOwnerCustomDrawSIMIcon(GUI_RECT_T *d_rect,GUIANIM_OWNER_DRAW_T *owner_draw_ptr)
{
	...
	if(MMICC_GetCallNum()==1||MMICC_GetCallNum()==0)
		{
        sim_rect.top = d_rect->bottom - 16+30;
		...
		{
        sim_rect.top = d_rect->bottom - 16+22;
        sim_rect.bottom = d_rect->bottom+22;
        sim_rect.right = d_rect->right + 6+16+25;
        sim_rect.left = d_rect->right + 6+25;
		}
	else
...
#if defined (MAINLCD_SIZE_128X160)//boda
    rect.top = owner_draw_ptr->display_rect.top;
...
#else
    rect.top = owner_draw_ptr->display_rect.top + 23 + 12;
    rect.bottom = owner_draw_ptr->display_rect.bottom - 23;
    rect.left = owner_draw_ptr->display_rect.left + 1;//23;
    rect.right = owner_draw_ptr->display_rect.right - 1;//23;
    text_style.region_ptr = &region_style;
...
LOCAL void CustomOpenRecordStopWin()
{
    ...
    SCI_TRACE_LOW("ccapp CustomOpenRecordStopWin s_record_result = %d", s_record_result);
    
    if( MMIRECORD_SRV_RESULT_ERROR != s_record_result )
    {
...
LOCAL MMI_RESULT_E CustomHandleRecordStopWinMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
    ...
    SCI_TRACE_LOW("ccapp CustomHandleRecordStopWinMsg msg_id = %d", msg_id);
