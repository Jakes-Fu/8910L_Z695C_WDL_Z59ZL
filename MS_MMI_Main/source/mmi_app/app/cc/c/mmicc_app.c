...
LOCAL uint8 hold_by_user[CC_MAX_CALL_NUM] = {0};//record by call id.
...
//add by ljg
static const char* const s_emergency_call_ui[]=
{
    "110", "119","999","120","122"
};

//lzk begin 2017.07.04
//bug 696460 MCC判断
static const char* const s_emergency_call_ui_AX[]=
{
...
LOCAL BOOLEAN s_is_need_connect_call_after_hold = FALSE;//without hold cnf msg but with disconnect ind msg
...
LOCAL uint8 s_india_ring_timer =0; //india ring tone timer
LOCAL uint8 s_network_ring_timer =0; //tch没有分配，对方已经振铃，播放本地回铃音的计时器
LOCAL uint8 s_bt_call_type_event_timer =0; //等待bt MMI_BT_CALL_TYPE_WB_NB_EVENT消息上来的timer
...
LOCAL uint8 emergency_num_len = 0;

LOCAL BOOLEAN s_is_call_audio_on = FALSE;  //是否上来网络音
...
typedef enum MMICC_INDIA_TONE_STATE_
{
    CC_RING_TONE_NONE,
    CC_RING_TONE_PLAY_400MS_START_1,
    CC_RING_TONE_PAUSE_200MS_START,
    CC_RING_TONE_PLAY_400MS_START_2,
    CC_RING_TONE_PAUSE_2600MS_START,
    CC_RING_TONE_STATE_MAX
} MMICC_INDIA_TONE_STATE;

LOCAL uint8 s_india_ringtone_state = 0;

#if defined(MMI_SIM_LOCK_TYPE2_SUPPORT) && !defined(MMI_SIM_LOCK_SUPPORT)
LOCAL BOOLEAN s_mmi_simlock_is_emergcall = FALSE;
#endif
...
LOCAL void mmicc_Changenumber(DPARAM param);
/*****************************************************************************/
...
LOCAL MMI_RESULT_E HandleSelectCurSIMWindow(
														  MMI_WIN_ID_T	  win_id, 
...
LOCAL void  CC_HandleCallingNameInd(DPARAM param);
...
LOCAL BOOLEAN CC_IsEmergencyOnlyStatus(MN_DUAL_SYS_E dual_sys);
...
LOCAL void HandleIndiaRingToneTimer(uint8  timer_id, uint32 param);
...
LOCAL void CC_TerminateSosCall(BOOLEAN flag)
{    //flag : flag to distinguish whether flow stopped normally.
...
LOCAL CC_RESULT_E CC_ProcessSosCall(MN_DUAL_SYS_E dual_sys)
{
    ...
    if (MMIAPISET_GetFlyMode() && (MMIAPICC_IsEmergencyNum(sos_call_info.tele_num, sos_call_info.tele_len, dual_sys, FALSE)))
    {
        MMIAPISET_SetFlyModeWholeFlow(FALSE);
        MMICC_EnterFlymodeEccWin((ADD_DATA)(&sos_call_info));
    }
    else
#endif
    if (MMICC_ProcessMakeCall(dual_sys, &sos_call_info))
    {       //0 : calling failed
...
PUBLIC CC_RESULT_E MMICC_MakeSosCall(void* param, MMISET_SOS_CALLBACK_PFUNC callback){
	...
	uint8 i;
	uint8 emergency_num[3] = "112"; // temporarily hardcode
	sos_num_list_tag[0].num_len = 3;

	SCI_TRACE_LOW("MMICC_MakeSosCall enter sim_num=%d", sim_num);
	MMIAPICOM_StrToWstr(emergency_num, sos_num_list_tag[0].num);
	...
	for (i = 0; i < MMISET_SOS_CONTACTS_MAX_NUM; i++)
	{
		if (sos_num_list_tag[i].num_len > 0)
		{
			if (sim_num <= 0)
			{
				...
				if (!MMIAPICC_IsEmergencyNum(sos_num, sos_num_list_tag[i].num_len, MN_DUAL_SYS_1, FALSE))
				{
					continue;
				}
			}
	...
	if (sim_num > 2)
	{		//unless exists only one sim card, apply MN_DUAL_SYS_1 by default.
...
PUBLIC void MMICC_HandleRemoteUserBusy();

LOCAL void CC_FreeMakeCallInfo(CC_MAKE_CALL_INFO * make_call_info_ptr);

LOCAL void CC_ResetCallingName(uint8 index);
LOCAL void CC_ClearCallingName(uint8 index);
LOCAL void CC_SetCallingName(uint8 index);
typedef enum
{
    CP_SIM_SEL_MENU_NODE_ROOT,
	CP_SIM_SEL_MENU_NOTE,
    CP_SIM_SEL_MENU_SIM1,
    CP_SIM_SEL_MENU_SIM2,
    CP_SIM_SEL_MENU_ALWAYS_ASK
};

WINDOW_TABLE( MMICC_SELECT_PRE_SIM_WIN_TAB ) = 
{
	WIN_FUNC( (uint32)HandleSelectCurSIMWindow ),	 
...
uint8 call_path_status[MMI_DUAL_SYS_MAX] = {0,};
PUBLIC void MMIAPICC_InitVolteStatus(void)
{
    int i=0;
    for(i=MN_DUAL_SYS_1;i< MMI_DUAL_SYS_MAX;i++)
    {
        if(MMIAPIPHONE_GetImsStatus(i))
            call_path_status[i] = 1;
        else
            call_path_status[i] = 0;
	SCI_TraceLow("MMIAPICC_InitVolteStatus call_path_status[%d] =%d",i,call_path_status[i]);	
    }
}
...
SCI_TraceLow("MMIAPICC_IsVolteNetworkStatus call_path_status[%d] =%d",dual_sys,call_path_status[dual_sys] );
    if(call_path_status[dual_sys] == 1)
        return TRUE;
    else	
        return FALSE;
}

LOCAL MMI_RESULT_E HandleCallPathInd(DPARAM param )
{
    APP_MN_CALL_PATH_IND_T call_path_ind = *((APP_MN_CALL_PATH_IND_T *)param);
	
    SCI_TraceLow("HandleCallPathInd sim=%d, nPath=%d",call_path_ind.dual_sys,call_path_ind.nPath);
    call_path_status[call_path_ind.dual_sys] = call_path_ind.nPath;
    MMICC_UpdateCallStatusDisplay(MMICC_CALL_PATH_IND);
    return MMI_RESULT_TRUE;
}
#endif
/*****************************************************************************/
// 	Description : Handle Net work Emg Ind(APP_MN_CALL_BARRED_IND)
//	Global resource dependence : none
//	Note:
/*****************************************************************************/
LOCAL void HandleCallBarredInd(DPARAM param)
{
    APP_MN_CALL_BARRED_IND_T *call_barred_ptr = (APP_MN_CALL_BARRED_IND_T *)param;
    SCI_TRACE_LOW("ljg*HandleCallBarredInd::barred_operation=%d",call_barred_ptr->barred_operation);
    if((MN_SS_BAR_OUTGOING == call_barred_ptr->barred_operation)||
        (MN_SS_BAOC == call_barred_ptr->barred_operation)||
        (MN_SS_BOIC == call_barred_ptr->barred_operation)||
        (MN_SS_BOIC_EXHC == call_barred_ptr->barred_operation)){
        //MMICC_OpenAlertWarningWin(TXT_CALL_BARRED); 
            GUI_POINT_T start_point = {0, 0};
            MMICOM_BANNER_INFO_T banner_info = {0};

            banner_info.banner_type = MMI_BANNER_TEXT;
            banner_info.text1_id = TXT_CALL_BARRED;
            banner_info.start_point = start_point;
            MMIAPICOM_OpenBannerWin(MMICL_CALL_BARRED_BANNER_WIN_ID,banner_info);
    }
}


LOCAL void CC_SetCallingNameTestInd(void)
{
    APP_MN_CALLING_NAME_IND_T  test_param={0};
    uint16 len = 0;
    uint8 test_str0[7]={0xD4,0xE2,0x94,0xEA,0x0c,0x36,0x8B};//"D4E294EA0C368B"; MN_SS_DEFAULT_DCS_VALUEs
    uint8 test_str1[8]={0x54,0x45,0x53,0x54,0x4E,0x41,0x4D,0x45};//"434553544e414d45";   MN_SS_8BIT_NO_CLASS_DCS
    uint8 test_str2[16]={0x00,0x54,0x00,0x45,0x00,0x53,0x00,0x54,0x00,0x4E,0x00,0x41,0x00,0x4D,0x00,0x45}; //  MN_SS_USC2_NO_CLASS_DCSs
    uint8* test_str_ptr = PNULL;
   test_param.dual_sys = 0;
    test_param.call_id = 0;
    test_param.calling_name =MN_SS_NAME_PRESENT_ALLOWED;

    if(test_id ==0)
        {
        test_param.name_set.data_code_scheme =MN_SS_DEFAULT_DCS_VALUE;
        len = 7;
        test_str_ptr = test_str0;
        test_id++;
        }
    else if(test_id ==1)
        {
        test_param.name_set.data_code_scheme =MN_SS_8BIT_NO_CLASS_DCS;
         test_str_ptr = test_str1;
        len = 8;
        test_id++;
        }
    else if (test_id ==2)
        {
        test_param.name_set.data_code_scheme =MN_SS_USC2_NO_CLASS_DCS;
         test_str_ptr = test_str2;
        len = 16;
        test_id = 0;
        }
    SCI_MEMCPY(test_param.name_set.name_str, test_str_ptr,len);
    test_param.name_set.name_str_len = len;
    CC_HandleCallingNameInd(&test_param);
}     
LOCAL uint8 is_ecc_call =0;
LOCAL uint8 is_ecc_call_alerting =0;
/*****************************************************************************/
...
PUBLIC MMI_RESULT_E MMICC_HandlePsMsg( PWND app_ptr, uint16 msg_id, DPARAM param)
{
    ...
    if (FilterMsgFromATCmdCall(msg_id))
    {
        return result;
    }
    ...
    case APP_MN_CALL_START_IND:
        ...
        s_is_call_audio_on = FALSE;
        break;

    case APP_MN_CALL_AUDIO_ON_IND:
        SCI_TRACE_LOW("CC_HandlePsMsg(): APP_MN_CALL_AUDIO_ON_IND network_ring=%d, india_ring=%d", s_network_ring_timer, s_india_ring_timer);
        if ((0 != s_network_ring_timer) || (0 != s_india_ring_timer))
        {        
            ...
            s_is_call_audio_on = TRUE;
        }
        if( is_ecc_call)
        {
                 is_ecc_call_alerting=1;
                 MMICC_UpdateStatusWin();
        }        
        break;
        
    case APP_MN_CALL_AUDIO_RESTART_IND:
        SCI_TRACE_LOW(" zgt CC_HandlePsMsg(): APP_MN_CALL_AUDIO_RESTART_IND start ");
        // restart net backring tone
        if(TRUE ==MMICC_IsVoiceCodecEnable())
        {
#ifdef MMI_RECORD_SUPPORT
			if (MMICC_IsCCRecording())
			{
				SCI_TRACE_LOW("ccapp.c: CC_HandlePsMsg(): pause CC recording");
				MMICC_PauseCCRecord();
			}
#endif
			...
			if (MMICC_IsCCRecording())
			{
				SCI_TRACE_LOW("ccapp.c: CC_HandlePsMsg(): resume CC recording");
				MMICC_ResumeCCRecord();
			}
#endif
            SCI_TRACE_LOW("  CC_HandlePsMsg(): APP_MN_CALL_AUDIO_RESTART_IND end");
        } 

        // restart local backring tone  bug 1150211 
        if (  (FALSE ==s_is_call_audio_on) && 
            (MO_CALL_STATE_DONE != s_is_downlnk_mute_mocalling_flag)&&
            ( MMIAPICC_IsInState(CC_MO_CONNECTING_STATE) ) )
        {
            StartLocalNetRing();       
            SCI_TRACE_LOW("  CC_HandlePsMsg(): APP_MN_CALL_AUDIO_RESTART_IND restart local backring tone");
        }
        else
        {
            SCI_TRACE_LOW("  CC_HandlePsMsg(): APP_MN_CALL_AUDIO_RESTART_IND is_audio_on:%u,mo_flag:%u,state:%u",s_is_call_audio_on,s_is_downlnk_mute_mocalling_flag,MMIAPICC_IsInState(CC_MO_CONNECTING_STATE) );
        }
        break;
    ...
    case APP_MN_ALERTING_IND:
        ...
        if(MMIBT_IsOnTransferStatus())
        {
            ...
            SCI_TRACE_LOW("ccapp CC_HandlePsMsg APP_MN_ALERTING_IND cancel file transfer");
        }
#endif        
        //收到振铃提示事件的处理
        CC_CallAlertingInd(param);
#ifdef WIN32        
        //CC_SetCallingNameTestInd(); //test code
#endif
        break;
...
//============== MET MEX end ===================     
...
#ifdef MMI_INDIAN_SOS_SUPPORT
	if (CC_IsSosCalling() 
		//&& (((MnCcSetupCompleteCnfS*)param)->call_id == CC_GetSosCallId())
		)
	{
		CC_TerminateSosCall(TRUE);
	}
    ...
    case APP_MN_CALL_DISCONNECTED_IND:
        ...
        {
            SCI_TRACE_LOW("ccapp MMICC_HandlePsMsg APP_MN_CALL_DISCONNECTED_IND PHONE_LANGUAGE_FIRST_POWER_WIN_ID");
            break;
        }
...
#if defined(MMI_SIM_LOCK_TYPE2_SUPPORT) && !defined(MMI_SIM_LOCK_SUPPORT)
		if (s_mmi_simlock_is_emergcall)
		{
			MMIAPIPHONE_PowerOffPs(s_call_context.dual_sys);
			s_mmi_simlock_is_emergcall = FALSE;
		}
#endif
#ifdef MMI_INDIAN_SOS_SUPPORT
		if (CC_IsSosCalling()
			//&& ((MnCcCallDisconnectdIndS*)param)->call_id == CC_GetSosCallId()
			)
		{
			MN_DUAL_SYS_E sos_dual_sys = CC_GetSosCallingSimIndex();
			uint8 num_ind = CC_GetSosCallNumIndex();
			
//			SCI_TRACE_LOW("SOS Calling, call id=%d", ((MnCcCallDisconnectdIndS*)param)->call_id);
			...
			else
			{
				...
				{
					CC_TerminateSosCall(TRUE);   //only loop 1 times.
					break;
...
//				SCI_TRACE_LOW("SOS Calling, call id=%d", ((MnCcCallDisconnectdIndS*)param)->call_id);
    ...
    case APP_MN_SETUP_IND:
        ...
        if(MMK_IsOpenWin(PHONE_LANGUAGE_FIRST_POWER_WIN_ID))
        {
            SCI_TRACE_LOW("ccapp MMICC_HandlePsMsg APP_MN_SETUP_IND PHONE_LANGUAGE_FIRST_POWER_WIN_ID");
            if(MN_RETURN_SUCCESS != MNCALL_DisconnectCallEx(((APP_MN_SETUP_IND_T*)param)->dual_sys, ((APP_MN_SETUP_IND_T*)param)->call_id, MN_CAUSE_NORMAL_CLEARING, PNULL))
            {
                SCI_TRACE_LOW("ccapp MMICC_HandlePsMsg APP_MN_SETUP_IND PHONE_LANGUAGE_FIRST_POWER_WIN_ID DisconnectCall fail");
            }
            break;
        }
        ...
        if (!HandleMTCallDisc(param))
        {
                ...
                && (CC_CALLING_STATE == MMICC_GetCurrentCallStatus()) )
            {
                ...
                MMICC_EnableVoiceCodec(FALSE);//bug1079514
                MMICC_DestroyVoiceCodec();
            }               
        ...
        break;
    case APP_MN_CALLING_NAME_IND:
        CC_HandleCallingNameInd(param);
        break;
    ...
    case APP_MN_CALL_BARRED_IND:
        HandleCallBarredInd(param);
        break;
#ifdef MMI_VOLTE_SUPPORT		
    case APP_MN_CALL_PATH_IND:
        //call path status
        HandleCallPathInd(param);
        HandleMoMuteAndLocalRingState(APP_MN_CALL_PATH_IND);
        break;
#endif		
    default:
        result = MMI_RESULT_FALSE;
                                     ...
                                     )
{
    ...
    if (CC_DIAL_METHOD_NULL != s_dial_method)
    {
        ...
        MMIPUB_OpenAlertWarningWin(STR_OPERATION_FAIL_EXT01);
        return CC_RESULT_ERROR;
    }
...
/*****************************************************************************/
//  Description : process emergency call under absent of sim and fly mode.
//  Global resource dependence : 
//  Note:2015.12.04
/*****************************************************************************/
LOCAL BOOLEAN OpenAlertWarningWinEmergency(void){

    MMISET_DUALSYS_TYPE_E e_dualsys_setting = MMIAPISET_GetMultiSysSetting();
    if((!(e_dualsys_setting & (0x01 << 0))&& MMIAPIPHONE_GetSimExistedStatus(0))
        && (!(e_dualsys_setting & (0x01 << 1))&& MMIAPIPHONE_GetSimExistedStatus(1)))
    {
            //MMIAPIPHONE_PowerOnPs(i);
            MMIPUB_OpenAlertWarningWin(STR_SIM_OFFLINE_EMERGENCY);
            return TRUE;
    }
    return FALSE;
}
                                 ...
                                 )
{
    ...
    ecc = MMIAPICC_EmergencyCallCheckType(tele_num, tele_len, dual_sys);
    SCI_TRACE_LOW("ccapp.c:MakeCallReqExt() ECC call_type = %d , dual_sys= %d", ecc.call_type, ecc.dual_sys);
    is_ecc_call_alerting = 0;
    if(ecc.call_type != NOT_ECC)
    {
        if(ecc.call_type == FAKE_ECC){
            call_type = MN_CALL_TYPE_FAKE_EMERGENCY;
            if(!MMICC_IsExistActiveAndHoldCalls())
                dual_sys = ecc.dual_sys;
        }else{
    ...
    if (MN_CALL_TYPE_EMERGENCY == call_type || MN_CALL_TYPE_FAKE_EMERGENCY == call_type)
    {
        if (CC_NO_NUM != MMICC_GetCallNum())
        {
            ...
            MMICC_EnableVoiceCodec(FALSE);//1089495
            MMICC_DestroyVoiceCodec();
...
#else
//replaced by ljg
    if (MN_RETURN_SUCCESS != MNCALL_StartCallWithEscvEx(dual_sys, call_type, 
        &(s_call_context.want_mo_call.address),
        sub_addr, 
        mmi_clir_type, 
        PNULL,
                            ...
                            )
{
    ...
    if (!b_search_result && MMIAPICC_RemoveIpNumber(tele_num, tele_len, origin_number_arr,CC_MAX_TELE_NUM_LEN))
    {
...
PUBLIC void MMIAPPICC_BtAcceptCall(void)
{
    ...
    {	
        MMK_SendMsg(MMICC_ANIMATION_WIN_ID,MSG_BT_ACCEPT_INCOMING_CALL,PNULL);	
    }
...
PUBLIC void MMIAPICC_BtCancelCall(void)
{
    ...
    if (CC_INCOMING_CALL_STATE == call_status)
    {
        MMK_SendMsg(MMICC_ANIMATION_WIN_ID,MSG_BT_REJECT_INCOMING_CALL,PNULL);
    }
    else if( CC_CALLING_STATE == call_status)
    {
        MMK_SendMsg(MMICC_ANIMATION_WIN_ID,MSG_BT_CANCEL_OUTGOING_CALL,PNULL);	
#if !defined(MMI_GUI_STYLE_TYPICAL)
        MMK_SendMsg(MMICC_STATUS_WIN_ID,MSG_BT_CANCEL_OUTGOING_CALL,PNULL);	
#endif
    }
    else if ( CC_CALL_STATE == call_status || CC_HOLD_STATE == call_status || CC_WAITING_STATE == call_status)//正在通话中，将电话挂断
    {
        ...
        {
            MMK_SendMsg(MMICC_STATUS_WIN_ID,MSG_BT_CANCEL_CALL,PNULL);
        }
...
LOCAL void CC_CallStartInd(DPARAM param)
{
    ...
    uint8				copy_len = 0;
    BOOLEAN construct_result = FALSE;
    ...
    if( sig_ptr->call_id >= CC_MAX_CALL_NUM || s_call_context.call_number > CC_MAX_CALL_NUM - 1)
    {
    ...
    construct_result = MMICC_ConstructVoiceCodec(FALSE);
#ifdef MMI_VOLTE_SUPPORT
    ...
    isEmergencyCall = MMIAPICC_IsEmergencyNum(tele_num, tele_len, s_call_context.want_mo_call.dual_sys, FALSE);
    SCI_TRACE_LOW("CC_CallStartInd construct_result=%d, isEmergencyCall=%d", construct_result, isEmergencyCall);
    if(construct_result && isEmergencyCall){
        CC_BOTTOM_BUTTONS_STATUS_INFO_T buttons_status_info = {0};
        MMICC_GetBottomButtonsStatus(&buttons_status_info);
        if(buttons_status_info.bottom_speaker_pressed)
        {
...
#ifdef BLUETOOTH_SUPPORT
PUBLIC void MMIAPICC_BtSetAudioTunnel(void)
{
    BT_HFG_WB_FLAG btHfgWB = {0};
    SCI_TRACE_LOW("wangwp--MMIAPICC_BtSetAudioTunnel--Enter");
    CC_OpenAudioTunnel(TRUE);
    SetCallTypeEventFlag(FALSE);
    MMK_StopTimer(s_bt_call_type_event_timer);
    s_bt_call_type_event_timer = 0;
}
#endif
/*****************************************************************************/
// 	Description : process when received signal APP_MN_SYNC_IND
...
LOCAL void CC_SyncInd(DPARAM param)
{
    ...
    if(MN_CALL_SYNC_RESOURCE_ASSIGNMENT == pSig->sync_cause && MMICC_IsCurrentCallIDValid())
    {
        ...
        else
        {
            ...
            //CC_OpenAudioTunnel(TRUE);
            #ifdef BLUETOOTH_SUPPORT
            SCI_TRACE_LOW("wangwp--ccapp.c: CC_SyncInd(): BT EventFlag = %d", GetCallTypeEventFlag());
            if((TRUE == MMISRVAUD_IsBTHeadSetActive()) && (FALSE == GetCallTypeEventFlag()))
            {
                SCI_TRACE_LOW("wangwp--ccapp.c: CC_SyncInd(): BT HeadSetActive");
                if (0 != s_bt_call_type_event_timer)
                {
                    SCI_TRACE_LOW("wangwp--ccapp.c: CC_SyncInd(): stop timer first");
                    MMK_StopTimer(s_bt_call_type_event_timer);
                    s_bt_call_type_event_timer = 0;
                }
                SCI_TRACE_LOW("wangwp--ccapp.c: CC_SyncInd(): create timer");
                s_bt_call_type_event_timer = MMK_CreateTimerCallback(MMICC_1SECOND, MMIAPICC_BtSetAudioTunnel, PNULL, FALSE);
            }
            else
            #endif
            {
                SCI_TRACE_LOW("wangwp--ccapp.c: CC_SyncInd(): NOT BT HeadSetActive");
                #ifdef BLUETOOTH_SUPPORT
                SetCallTypeEventFlag(FALSE);
                #endif
                CC_OpenAudioTunnel(TRUE);
                                                                ...
                                                                CC_WANT_MO_CALL_T *want_mo_call_ptr)
{
    ...
    SCI_TRACE_LOW("mmiccapp: want_mo_call_ptr->address.num_len = %d, tele_num = %s, tele_len = %d, call_type = %d", want_mo_call_ptr->address.num_len, tele_num, tele_len, 
		want_mo_call_ptr->call_type);
    ...
    {
        MMI_GetLabelTextByLang(STXT_EMERGENCY_ONLY, &name);
...
LOCAL void CC_SetupCompleteCnf(DPARAM param)
{
    ...
    //allow send DTMF for emergency call with 4G
    s_call_context.dtmf_state.allow_send_flag = (s_call_context.dtmf_state.allow_send_flag | DTMF_MN_PROGRESS_IND_BIT);
    ...
    if(!CC_IsCallIDValid(s_call_context.current_call))
    {
      ...
      //fix bug:1019416  If it is emergency call,Do not show option menu 
   if( is_ecc_call)
    {
          is_ecc_call =0;
          return;
...
//    MMICC_EnableMute(FALSE);//cr228324
...
#ifdef SOS_KEYLONGWEB
		 if( sos_times > 0 && sos_temp )
		 {
			sos_is_connected = TRUE; 
			sos_temp = FALSE ;				 //接通电话 退出 紧急拨号状态
			MMIAPICC_SwitchAudioDevice(MMICC_AUDIO_OPER_ENABLE, MMICC_AUDIO_DEV_HF);
		 }
#endif
        #ifdef ZDT_PRODUCT_Q98
               if(MMIAPISET_GetAutoHandFreeSetting())
		{
			MMIAPICC_SwitchAudioDevice(MMICC_AUDIO_OPER_ENABLE, MMICC_AUDIO_DEV_HF);
			//MMIAPICC_Q98RecordCC();
		}
    ...
    if (1 == s_call_context.call_number) 
    {
        ...
        s_call_context.call_time_count_start = s_call_context.call_info[s_call_context.current_call].call_time.start_time;
    }
...
LOCAL CC_RESULT_E CC_ReleaseCallReq(MN_DUAL_SYS_E dual_sys, uint8 call_index,MN_CALL_DISCONNECT_CALL_CAUSE_E cause)
{
    ...
    if(MN_RETURN_SUCCESS != MNCALL_DisconnectCallEx(s_call_context.dual_sys, s_call_context.call_info[call_index].call_id, cause, PNULL))
    {
                                ...
                                )
{
    ...
    case MN_SS_INCOMING_IS_FORWARED:
        forwared_type_textid= STR_INCOMING_FORWARDED;
        break;
                                  ...
                                  )
{
    ...
    {
        //MMICC_OpenAlertWarningWin(status_ind_textid);//do not show alert according to custom ui spec.
    }
}
...
//============== MET MEX end ===================    
    ...
    {
    	cause = CC_RELEASE_OPER;
		if (CFW_SSCheckTestSim(dual_sys))
		{
...
#ifdef MMI_SUPPORT_PROTECTION_CALL
static const char* const s_protection_number_list[]=
{
    "06"
};
LOCAL BOOLEAN IsProtectionNumber(uint8 *tele_num,uint8 tele_len)
{
    uint32 i = 0;
    uint8 temp_tele_num[MMICC_PHONE_NUM_MAX_LEN + 2] = {0};
    SCI_TRACE_LOW("IsProtectionNumber :tele_num=%s,lenth=%d,",tele_num,tele_len);
    SCI_MEMCPY(temp_tele_num, tele_num, tele_len);
    for(i = 0; i < ARR_SIZE(s_protection_number_list); i++)
    {
        if( (tele_len== strlen(s_protection_number_list[i]))
        && (0 == strncmp((char*)temp_tele_num, s_protection_number_list[i], tele_len)) )
        {
                                      ...
                                      )
{
    ...
    BOOL is_protection_number = FALSE;
    BOOLEAN isEmergencyCall = FALSE;
    ...
    if(MMICC_IsCCRecording())
    {
        ...
        MMIAPICC_StopRecordByCallId(call_id);
    }
#endif
...
//     MMIAPIBT_RecoveryBtStatus(MMIBT_MODULE_CC);
    ...
    SCI_TRACE_LOW("CC_DisconnectedCallByIndex call_index : %d,  call state: %d",call_index, s_call_context.call_info[call_index].call_state);
    if (!s_call_context.call_info[call_index].is_release_all)    //不是挂断所有电话
    {
                ...
                &&(RELEASE_ALL_ACTIVE_AND_RETRIEVE_HOLD != s_call_context.operate_CRSS_code)))
        {
            ...
            if((cause == MN_CAUSE_USER_BUSY) 
			&& (CC_INCOMING_CALL_STATE != s_call_context.call_info[ call_index ].call_state)
			&& (CC_WAITING_STATE != s_call_context.call_info[ call_index ].call_state)) //MT release with cause = 17
            {
                MMICC_HandleRemoteUserBusy();
            }
                ...
                &&(RELEASE_ALL_ACTIVE_AND_RETRIEVE_HOLD != s_call_context.operate_CRSS_code)))
        {
            ...
            {
                MMIAPICC_StopCCRecord(TRUE); //挂断的是后停止录音
            }
            ...
            MMIAPIIDLE_CloseDialMenuWin();
             ...
             MMK_CloseWin( MMICC_HOLDMENU_WIN_ID ); //fix bug 1014856
             Custom_CloseWaitIndicatorDialog();
    }    
    else    //挂断所有电话的一通
    {
    ...
    case CC_MT_CALL_TYPE:
            ...
            )
        {
            ...
            MMIAPICOM_StatusAreaSetMissedCallState(TRUE);
#ifdef PDA_UI_DROPDOWN_WIN
...
#endif
    {
        ...
        MMICC_GetCallNumStrByIndex(tele_num,&tele_num_len,call_index);
#ifdef MMI_SUPPORT_PROTECTION_CALL
        is_protection_number = IsProtectionNumber(tele_num, tele_num_len);
#endif
        if(!is_protection_number)
        {
        #if 0
		    MMICC_RemoveIdleMsg(tele_num,tele_num_len);
		    if(( MMIAPICC_GetMissedCallNumber() > 0) && MMICC_GetIsNewMissedCall())
		    {
			    MMICC_AppendIdleMsg(MMI_IDLE_PUSH_MSG_MISSED_CALL);
      ...
      {//bug1107824
        uint8 tmp_tele_num[CC_MAX_TELE_NUM_LEN + 2] = {0};
        uint32 tmp_tele_len = 0;
        tmp_tele_len = MMIAPICOM_GenDispNumber(s_call_context.want_mo_call.address.number_type, 
        (uint8)s_call_context.want_mo_call.address.num_len, 
        s_call_context.want_mo_call.address.party_num, 
        tmp_tele_num, 
        CC_MAX_TELE_NUM_LEN + 2);
            SCI_TRACE_LOW("CC_DisconnectedCallByIndex s_call_context.call_info[call_index].call_type=%d s_call_context.call_info[call_index].call_state=%d",s_call_context.call_info[call_index].call_type,s_call_context.call_info[call_index].call_state);
        if((CC_INCOMING_CALL_STATE != s_call_context.call_info[call_index].call_state)&&(CC_MO_CALL_TYPE == s_call_context.call_info[call_index].call_type))
        {
              isEmergencyCall = MMIAPICC_IsEmergencyNum(tmp_tele_num, tmp_tele_len, s_call_context.want_mo_call.dual_sys, FALSE);
         }
                    
        SCI_TRACE_LOW("CC_DisconnectedCallByIndex@@@@ isEmergencyCall=%d", isEmergencyCall);
        }
  ...
  //  }
...

    {
        ...
        if(!MMICC_GetIsPlayCallEndHintFlag() && !MMK_IsOpenWin(MMICC_DISCONNECT_WIN_ID) && !(isEmergencyCall))
        {
            MMICC_HandleOperAfterCallEnd();
        }
        else if(!MMK_IsOpenWin(MMICC_DISCONNECT_WIN_ID)
                &&!MMICC_IsExistActiveCall() && !MMICC_IsExistHoldCall()&& !(isEmergencyCall))
        {
            SCI_TRACE_LOW("CC_DisconnectedCallByIndex MMICC_HandleOperAfterCallEnd here!");
            MMICC_HandleOperAfterCallEnd();
        }
    }
#if defined(MMI_REJECT_CALL_BY_SMS_SUPPORT)
...
LOCAL BOOLEAN GetUriPhoneNumber(
            ...
            )
{
    ...
    {
        SCI_TraceLow("[szp]GetUriPhoneNumber wrong number head! uri_number_start_ptr = %s", uri_number_start_ptr);
        return FALSE;
    }
    ...
    {
        SCI_TraceLow("[szp]GetUriPhoneNumber wrong number semicolon!  uri_number_start_ptr = %s", uri_number_semicolon_ptr);
        return FALSE;
    }
...
LOCAL void mmicc_Changenumber(DPARAM param)
{
    ...
    {        
        ...
        if (strlen((char*)pSig->calling_num.calling_uri))
        {
            SCI_TraceLow("[szp]mmicc_Changenumber   pSig->calling_num.calling_uri = %s",pSig->calling_num.calling_uri);
            
            memset(&uri_number,0,sizeof(URI_NUMBER_T));
            if (GetUriPhoneNumber(pSig->calling_num.calling_uri, &uri_number))
            {
                if (uri_number.number[0] == '+')
                {
                    ...
                    SCI_TraceLow("[szp]mmicc_Changenumber+ len=%d num=%s",tel_num_len,tele_num);
                }
                else if ((uri_number.number[0] == '0') && (uri_number.number[1] == '0'))
                {
                    ...
                    tel_num_len = strlen((char*)tele_num);
                    SCI_TraceLow("[szp]mmicc_Changenumber00 len=%d num=%s",tel_num_len,tele_num);
                }
                else
                {
                    strcpy(tele_num,uri_number.number);
                    tel_num_len = strlen((char*)tele_num);
                    SCI_TraceLow("[szp]mmicc_Changenumber none len=%d num=%s",tel_num_len,tele_num);
                }
                memset(pSig->calling_num.party_num,0xff,MN_MAX_ADDR_BCD_LEN);
                MMIAPICOM_StrToBcd(PACKED_LSB_FIRST, tele_num, pSig->calling_num.party_num);
                pSig->calling_num.num_len = (strlen((char *)tele_num) + 1)/2;    
            }
        }
        else if (MN_NUM_TYPE_INTERNATIONAL == pSig->calling_num.number_type)
        {
            ...
            {
                memset(pSig->calling_num.party_num,0xff,MN_MAX_ADDR_BCD_LEN);
                MMIAPICOM_StrToBcd(PACKED_LSB_FIRST, &tele_num[1], pSig->calling_num.party_num);
                pSig->calling_num.num_len = (strlen((char *)&tele_num[1]) + 1)/2;    
            }
            else if (('0' == tele_num[0]) && ('0' == tele_num[1]))
            {
                memset(pSig->calling_num.party_num,0xff,MN_MAX_ADDR_BCD_LEN);
                MMIAPICOM_StrToBcd(PACKED_LSB_FIRST, &tele_num[2], pSig->calling_num.party_num);
                pSig->calling_num.num_len = (strlen((char *)&tele_num[2]) + 1)/2;    
            }
            
            SCI_TraceLow("[szp]mmicc_Changenumber tele_num = %s, len = %d, trans_len = %d",tele_num, pSig->calling_num.num_len, trans_len);
        }
        
        SCI_TraceLow("[szp]mmicc_Changenumber tele_num = %s tel_num_len=%d, number_type=%d",tele_num, tel_num_len, pSig->calling_num.number_type);
    }
...
LOCAL void CC_CallSetupInd(DPARAM param)
{
    ...
    {        
        ...
        SCI_TraceLow("[szp]CC_CallSetupInd tele_num = %s tel_num_len=%d, number_type=%d",tele_num, tel_num_len, pSig->calling_num.number_type);

    }
    //add for bug Bug 476861 
    /*if(MMICC_IsExistActiveAndHoldCalls())
    {
        if(MN_RETURN_SUCCESS != MNCALL_DisconnectCallEx(pSig->dual_sys, pSig->call_id, MN_CAUSE_NORMAL_CLEARING, PNULL))
        {
            //SCI_TRACE_LOW:"ccapp.c:CC_ReleaseCallReq() MNCALL_DiscCall return failure"
            SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMICC_APP_2375_112_2_18_2_10_11_61,(uint8*)"");
            return;
        }
        MMICC_AppendIdleMissCallMsg(pSig);
        return;
    }*/
...
LOCAL CC_RESULT_E CC_ConnectCallReq(void)
{
    ...
    if(!MMICC_IsCurrentCallIDValid() || MN_RETURN_SUCCESS != MNCALL_ConnectCallEx(
...
{
    uint8 dtmf_ch = 0;
    SCI_TRACE_LOW("[MMICC] CC_SendSingleDtmf Enter %d, msg_id=0x%x", MMICC_IsAllowSendDtmf(), msg_id);
    if (MMICC_IsAllowSendDtmf())
    {
...
LOCAL BOOLEAN CC_IsEmergencyOnly(MN_DUAL_SYS_E dual_sys)
{
    ...
    return (!is_sim_exist || SIM_NOT_INITED == MMIAPIPHONE_GetSimStatus(dual_sys));
}
...
LOCAL BOOLEAN CC_IsEmergencyOnlyStatus(MN_DUAL_SYS_E dual_sys)
{
    ...
    return (!is_sim_exist || SIM_NOT_INITED == MMIAPIPHONE_GetSimStatus(dual_sys) ||
		PLMN_EMERGENCY_ONLY == MMIAPIPHONE_GetPlmnStatus(dual_sys));
}
...
#ifdef MMI_FIX_LINK_ERROR  // zhigang.peng 2018.0915
PUBLIC BOOLEAN MMIAPICC_IsQaVersion(void)
{
#ifndef WIN32
    uint8 pcode[12] = {0};
...
PUBLIC BOOLEAN MMIAPICC_IsIndiaVersion(void)
{
#ifndef WIN32
	return FALSE;
#if 0
    ...
    if(0 == LP_CODE_Read(pcode))
    {
        if(strlen(pcode) > 0 && strncmp(pcode, "MH", 2) == 0)		
        {
            return TRUE;
        }
        else
...
/*****************************************************************************/
// 	Description :In normal status the phone make NORMAL call by 110,119,999 and 120 num
// but display ENMERGENCY CALL in window 
//	Global resource dependence : 
//	Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPICC_IsEmergencyPartNumByPrc(uint8 *tele_num,uint8 tele_len,MN_DUAL_SYS_E dual_sys){
    uint32 i = 0;
    ...
    if(SIM_STATUS_OK != MMIAPIPHONE_GetSimStatus(dual_sys))return FALSE;

    //if(MMIAPICC_IsQaVersion())
    if(1 == emergency_config)// 1 means China
    {
        temp_tele_len = tele_len;
        SCI_MEMCPY(temp_tele_num, tele_num, temp_tele_len);
        for(i = 0; i < ARR_SIZE(s_emergency_call_ui); i++)
        {
            if( (temp_tele_len <= strlen(s_emergency_call_ui[i]))
            && (0 == strncmp((char*)temp_tele_num, s_emergency_call_ui[i], temp_tele_len)) )
            {
                emergency_num_len = strlen(s_emergency_call_ui[i]);
                return TRUE;
            }
        }
    }
	...
	//bug 696460 MCC判断
	{
		...
		uint16 switzerlandMcc=  228;
		uint16 liechtensteinMcc = 295;//
		int sim_index = dual_sys; 
		int maxSimIndex = MMI_DUAL_SYS_MAX;
		MN_PLMN_T simPlmn = {0};
		uint16 mccValue = 0;
		int test = 0;
		//SCI_TRACE_ID(TRACE_TOOL_CONVERT,"Author lzk. bug 696460: %d.",(uint8*)"d", test);

		//get current PLMN Number.included MCC and MNC.
		MMIAPIPHONE_GetCurPLMN(&simPlmn, sim_index);
		
		mccValue = simPlmn.mcc;
		//SCI_TRACE_ID(TRACE_TOOL_CONVERT,"Author lzk. bug 696460:sim card index = %d, sim mcc = %d.",(uint8*)"dd", sim_index, mccValue);
		if (mccValue == switzerlandMcc || mccValue == liechtensteinMcc)
		{
			temp_tele_len = tele_len;
			SCI_MEMCPY(temp_tele_num, tele_num, temp_tele_len);

			//loop Judge. whether temp_tele_num is equal to s_emergency_call_ui_AX's item.
			for(i = 0; i < ARR_SIZE(s_emergency_call_ui_AX); i++)
			{
				if( (temp_tele_len <= strlen(s_emergency_call_ui_AX[i]))
				&& (0 == strncmp((char*)temp_tele_num, s_emergency_call_ui_AX[i], temp_tele_len)) )
				{
					emergency_num_len = strlen(s_emergency_call_ui_AX[i]);
					return TRUE;
				}
			}
		}
...
/*****************************************************************************/
// 	Description :999 should be an emergency number call out when DUT support CPHS. 
//	Global resource dependence : 
//	Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPICC_IsEmergencyCphs(uint8 *tele_num,uint8 tele_len,MN_DUAL_SYS_E dual_sys){
    uint32 i = 0;
    uint8 temp_tele_num[CC_MAX_TELE_NUM_LEN + 1] = {0};
    uint8 temp_tele_len = 0;

    if(SIM_STATUS_OK != MMIAPIPHONE_GetSimStatus(dual_sys))return FALSE;
    SCI_TRACE_LOW("MMIAPICC_IsEmergencyCphs----cphs_supported? %s--Valid? %s", MMIAPIPHONE_IsCPHSSupport()? "YES": "NO", 
		MMISET_IsCphsValid(dual_sys)?"YES":"NO");
    if(MMIAPIPHONE_IsCPHSSupport() && MMISET_IsCphsValid(dual_sys)){
        temp_tele_len = tele_len;
        SCI_MEMCPY(temp_tele_num, tele_num, temp_tele_len);
        if( (3 == temp_tele_len)&& (0 == strncmp((char*)temp_tele_num, "999", temp_tele_len)) )
        {
           return TRUE;
        }
    }
...
PUBLIC BOOLEAN MMIAPICC_IsEmergencyPartNumCphs(uint8 *tele_num,uint8 tele_len,MN_DUAL_SYS_E dual_sys){
    uint32 i = 0;
    uint8 temp_tele_num[CC_MAX_TELE_NUM_LEN + 1] = {0};
    uint8 temp_tele_len = 0;
...
LOCAL BOOLEAN MMIAPICC_IsPartFakeEmergencyNumByNdt(uint8 *tele_num,uint8 tele_len,MN_DUAL_SYS_E dual_sys)
{
	uint8 fake_ecc_list[10][32] = {0};
	NVITEM_ERROR_E nv_reslut = NVERR_NONE;
	char str[256] = {0};
	uint32 i = 0;
	uint32 lenth = 0;

	nv_reslut = EFS_NvitemRead(SETTINGS_NV_DEFAULT_FAKE_ECC,NVITEM_GetLength(SETTINGS_NV_DEFAULT_FAKE_ECC),(uint8*)fake_ecc_list);
	if (NVERR_NONE == nv_reslut)
	{
		for(i = 0;i < 10;i++){
			sprintf(str, "%s\n",fake_ecc_list[i]);
			lenth = strlen(str) - 1;
			SCI_TRACE_LOW("MMIAPICC_IsPartFakeEmergencyNumByNdt tele_num=%s,lenth=%d,str=%s",tele_num,lenth,str);
			if((tele_len <= lenth)&&(0 == strncmp((char*)tele_num, str, tele_len))){
                        emergency_num_len = lenth;
                        return TRUE;
			}		
		}
	}
...
LOCAL BOOLEAN MMIAPICC_IsFakeEmergencyNumByNdt(uint8 *tele_num,uint8 tele_len,MN_DUAL_SYS_E dual_sys)
{
    uint8 fake_ecc_list[10][32] = {0};
    NVITEM_ERROR_E nv_reslut = NVERR_NONE;
    char str[256] = {0};
	...
	if (NVERR_NONE == nv_reslut)
	{
		for(i = 0;i < 10;i++){
			sprintf(str, "%s\n",fake_ecc_list[i]);
			lenth = strlen(str) - 1;
			SCI_TRACE_LOW("MMIAPICC_IsFakeEmergencyNumByNdt tele_num=%s, lenth=%d, str=%s, dual_sys=%d",tele_num, lenth, str, dual_sys);
			if((tele_len == lenth)&&(0 == strncmp((char*)tele_num, str, tele_len)))
			return TRUE;
		}		
	}
...
/*****************************************************************************/
// 	Description :Get Emergency number form NV list
//	Global resource dependence : 
//	Note:
/*****************************************************************************/

PUBLIC BOOLEAN MMIAPICC_IsEmergencyNumByNdt(uint8 *tele_num,uint8 tele_len,MN_DUAL_SYS_E dual_sys)
{
	uint8 sim_ecc_list[10][32] = {0};
	uint8 no_sim_ecc_list[15][32] = {0};
	NVITEM_ERROR_E nv_reslut = NVERR_NONE;
	char str[256] = {0};
	uint32 i = 0;
	uint32 lenth = 0;

	if(!MMIAPIPHONE_GetSimExistedStatusEx(dual_sys)){
		nv_reslut = EFS_NvitemRead(SETTINGS_NV_DEFAULT_NOSIM_ECC,NVITEM_GetLength(SETTINGS_NV_DEFAULT_NOSIM_ECC),(uint8*)no_sim_ecc_list);
		if (NVERR_NONE == nv_reslut)
		{
			for(i = 0;i < 15;i++){
				sprintf(str, "%s\n",no_sim_ecc_list[i]);
                lenth = strlen(str) - 1;
                SCI_TRACE_LOW("MMIAPICC_IsEmergencyNumByNdt absent::tele_num=%s,lenth=%d,str=%s",tele_num,lenth,str);
				if((tele_len == lenth)&&(0 == strncmp((char*)tele_num, str, tele_len)))return TRUE;
			}		
		}
	}else{
		nv_reslut = EFS_NvitemRead(SETTINGS_NV_DEFAULT_SIM_ECC,NVITEM_GetLength(SETTINGS_NV_DEFAULT_SIM_ECC),(uint8*)sim_ecc_list);
		if (NVERR_NONE == nv_reslut)	
		{
			for(i = 0;i < 10;i++){
				sprintf(str, "%s\n",sim_ecc_list[i]);
                lenth = strlen(str) - 1;
                SCI_TRACE_LOW("MMIAPICC_IsEmergencyNumByNdt present::tele_num=%s,lenth=%d,str=%s",tele_num,lenth,str);
				if((tele_len == lenth)&&(0 == strncmp((char*)tele_num, str, tele_len)))return TRUE;
			}		
		}
	} 
...
PUBLIC BOOLEAN MMIAPICC_IsEmergencyPartNumByNdt(uint8 *tele_num,uint8 tele_len,MN_DUAL_SYS_E dual_sys)
{
    uint8 sim_ecc_list[10][32] = {0};
    uint8 no_sim_ecc_list[15][32] = {0};
    NVITEM_ERROR_E nv_reslut = NVERR_NONE;
    ...
    if(!MMIAPIPHONE_GetSimExistedStatusEx(dual_sys)){
        ...
        if (NVERR_NONE == nv_reslut)
        {
            for(i = 0;i < 15;i++){
                sprintf(str, "%s\n",no_sim_ecc_list[i]);
                lenth = strlen(str) - 1;
                SCI_TRACE_LOW("MMIAPICC_IsEmergencyPartNumByNdt absent::tele_len=%d,lenth=%d,str=%s",tele_len,lenth,str);
                if((tele_len <= lenth)&&(0 == strncmp((char*)tele_num, str, tele_len))){
                    emergency_num_len = lenth;
                    return TRUE;
                }
        ...
        if (NVERR_NONE == nv_reslut)	
        {
            for(i = 0;i < 10;i++){
                sprintf(str, "%s\n",sim_ecc_list[i]);
                lenth = strlen(str) - 1;
                SCI_TRACE_LOW("MMIAPICC_IsEmergencyPartNumByNdt present::tele_len=%d,lenth=%d,str=%s",tele_len,lenth,str);
                if((tele_len <= lenth)&&(0 == strncmp((char*)tele_num, str, tele_len))){
                    emergency_num_len = lenth;
                    return TRUE;
                }
...
PUBLIC BOOLEAN MMIAPICC_IsVoiceMailNum(
                                 uint8 *tele_num,
                                 uint8 tele_len                                 )
{
	uint32 i = 0;
	...
	// Bug 1350648, 以呼出的卡去寻找对应语音信箱号码
	if (MMIAPICC_GetCallingSIM() == MN_DUAL_SYS_1)
	{
		...
		if (vm1tele_len>0)
		{
			...
			SCI_TraceLow("mmicc: MMIAPICC_IsVoiceMailNum  vm1tele_len=%d  temp_tele_num=%s  temp_tele_num2=%s",vm1tele_len,temp_tele_num,temp_tele_num2);
			...
			//if (0 == strcmp(temp_tele_num,temp_tele_num2))
				//return TRUE;
		}
	}
	
	if (MMIAPICC_GetCallingSIM() == MN_DUAL_SYS_2)
	{
		...
		if (vm2tele_len > 0)
		{
			...
			SCI_TraceLow("mmicc: MMIAPICC_IsVoiceMailNum  vm2tele_len=%d  temp_tele_num=%s  temp_tele_num2=%s ",vm2tele_len,temp_tele_num,temp_tele_num2);
			MMIAPIPB_StringToBCDNumber(&temp_tele_num, &bcd_tele_num);
			MMIAPIPB_StringToBCDNumber(&temp_tele_num2, &bcd_vm_num);
			if(0 == MMIAPIPB_BcdSmartCmp(&bcd_tele_num, &bcd_vm_num))
			{
...
#else
	vm1tele_len = MMISET_GetIVoiceMailNumberToNV(s_call_context.dual_sys,vm1tele_str);
	if (vm1tele_len > 0)
	{
        MMIAPICOM_WstrToStr(vm1tele_str,temp_tele_num2);
        SCI_TraceLow("mmicc: MMIAPICC_IsVoiceMailNum  vm1tele_len=%d  temp_tele_num=%s  temp_tele_num2=%s ",vm2tele_len,temp_tele_num,temp_tele_num2);
        if (0==strncmp(temp_tele_num,temp_tele_num2,vm1tele_len)) return TRUE;
	}
#endif
                                 ...
                                 )
{
    ...
    if(MMIAPICC_IsEmergencyCphs(tele_num,tele_len,dual_sys))return TRUE;
#ifdef MMI_PLMN_LOAD_DEFAULT_SUPPORT
   is_emergency_call_in_local = MMIAPICC_IsEmergencyNumByNdt(tele_num,tele_len,dual_sys);
    ...
    {    
        ...
        {
            uint8 ecc_code[MN_MAX_ECC_LEN*2 + 2] = {0};
            uint8 tel_num_len = 0;
            
            SCI_MEMSET(ecc_code, 0x00, sizeof(ecc_code));
            //SCI_MEMCPY(ecc_code, sim_emc.call_codes[i].ecc_code, MN_MAX_ECC_LEN);
            tel_num_len = MMIAPICOM_GenNetDispNumber(
                MN_NUM_TYPE_NATIONAL, 
                (uint8)(MN_MAX_ECC_LEN),
                (uint8*)sim_emc.call_codes[i].ecc_code,
                ecc_code,
                (uint8)(MN_MAX_ECC_LEN*2+2)
                );
    ...
    if(MMIAPICC_IsFakeEmergencyNumByNdt(tele_num,tele_len, dual_sys))
        return TRUE;
...
// but display ENMERGENCY CALL in window 
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPICC_IsEmergencyPartNum(
                                 uint8 *tele_num, 
                                 uint8 tele_len,
                                 MN_DUAL_SYS_E dual_sys,
                                 BOOLEAN is_only_for_display
                                 )
{
    ...
    if(dual_sys >= MMI_DUAL_SYS_MAX)
    {
        SCI_TRACE_LOW("ccapp.c: MMIAPICC_IsEmergencyPartNum error dual_sys = %d", dual_sys);
        return FALSE;
    }

    if(PNULL == tele_num)
    {
        SCI_TRACE_LOW("ccapp.c: MMIAPICC_IsEmergencyPartNum tele_num == PNULL");
        return FALSE;
    }

    if(0 == tele_len)
    {
        SCI_TRACE_LOW("ccapp.c: MMIAPICC_IsEmergencyPartNum tele_len == 0");
        return FALSE;
    }
    ...
    if ('+' == tele_num[0])
    {
        ...
        if(0 == temp_tele_len)
        {
            SCI_TRACE_LOW("ccapp.c: MMIAPICC_IsEmergencyPartNum temp_tele_len == 0");
            return FALSE;
        }
    ...
    if(MMIAPICC_IsEmergencyPartNumCphs(tele_num,tele_len,dual_sys))
    {
        return TRUE;
    }

    if(MMIAPICC_IsEmergencyPartNumByPrc(tele_num,tele_len,dual_sys))
    {
        return TRUE;
    }

    is_emergency_call_in_local = MMIAPICC_IsEmergencyPartNumByNdt(tele_num,tele_len,dual_sys);
    ...
    if (s_cc_csc_sos.sos_num > 0)
    {
        if (IsPartCSCSosNum(temp_tele_num, temp_tele_len, &s_cc_csc_sos))
        {
            return TRUE;
        }
    }
    ...
    if(MMIAPIPHONE_GetSimExistedStatus(dual_sys))
    {
        ...
        SCI_TRACE_LOW("ccapp.c: MMIAPICC_IsEmergencyPartNum: emc num=%d", sim_emc.validate_num);
        ...
        if (0 < sim_emc.validate_num)
        {
            for (i = 0; i < sim_emc.validate_num; i++)
            {
                ...
                SCI_TRACE_LOW("ccapp.c: MMIAPICC_IsEmergencyPartNum: ecc_code=%s", ecc_code);

                if((tele_len <= tel_num_len)
                    && (0 == strncmp((char*)tele_num, (char *)ecc_code, tele_len)))
                {
                    SCI_TRACE_LOW("ccapp.c: MMIAPICC_IsEmergencyPartNum() is sim emc");
                    emergency_num_len = tel_num_len;
                    return TRUE;
                }
            }
        }
            ...
            && s_cc_network_emg_num_list_ptr[dual_sys]->mcc == curPlmn.mcc)
        {
            for (i = 0; i < s_cc_network_emg_num_list_ptr[dual_sys]->list_num; i++)
            {
                ...
                SCI_TRACE_LOW("ccapp.c: MMIAPICC_IsEmergencyPartNum: list_num=%s", ecc_code);
    ...
    if(MMIAPICC_IsPartFakeEmergencyNumByNdt(tele_num,tele_len, dual_sys))
        return TRUE;
    ...
    if (is_only_for_display || (!is_only_for_display && CC_IsEmergencyOnly(dual_sys)))/*lint !e774 */
    {
        if (s_cc_csc_no_sim_sos.sos_num > 0)
        {
            if (IsPartCSCSosNum(temp_tele_num, temp_tele_len, &s_cc_csc_no_sim_sos))
            {
                return TRUE;
            }
        }
...
PUBLIC MMICC_IS_ECC_T MMIAPICC_EmergencyCallCheckType(
                                 uint8 *tele_num, 
                                 uint8 tele_len,
                                 MN_DUAL_SYS_E dual_sys
                                 )
{
    ...
    MMICC_IS_ECC_T ecc;
    ecc.dual_sys = dual_sys;
    ecc.call_type = NOT_ECC;

    //check the param
    if((dual_sys >= MMI_DUAL_SYS_MAX) || (PNULL == tele_num) ||(0 == tele_len))
    {
	 return ecc;
    }
...
#ifdef MMI_PLMN_LOAD_DEFAULT_SUPPORT
    if(MMIAPICC_IsEmergencyNumByNdt(tele_num,tele_len,dual_sys))
    {
    ...
    if (PNULL != s_cc_network_emg_num_list_ptr[dual_sys]
        ...
        && s_cc_network_emg_num_list_ptr[dual_sys]->mcc == curPlmn.mcc)
    {
        SCI_TRACE_LOW("ccapp: MMIAPICC_EmergencyCallCheckType: emg[%d].mcc =%d  , curPlmn =%d", dual_sys, s_cc_network_emg_num_list_ptr[dual_sys]->mcc, curPlmn.mcc);
        ...
        for (i = 0; i < s_cc_network_emg_num_list_ptr[dual_sys]->list_num; i++)
        {
            ...
            SCI_TRACE_LOW("ccapp: MMIAPICC_EmergencyCallCheckType: network_emg_list_num=%s", ecc_code);
            
            if((tele_len == tel_num_len)
                && (0 == strncmp((char*)tele_num, (char *)ecc_code, tele_len)))
            {
                ecc.call_type = NETWORK_ECC;
                return ecc;
            }
        }
    }
    ...
    if(MMIAPICC_IsFakeEmergencyNumByNdt(tele_num,tele_len, dual_sys))
    {
        plmn_status = MMIAPIPHONE_GetPlmnStatus((MN_DUAL_SYS_E)dual_sys);
        is_ps_ready = MMIAPIPHONE_GetIsPsReady((MN_DUAL_SYS_E)dual_sys);
        if(PLMN_NO_SERVICE != plmn_status
        ...
        && PLMN_REGISTER_GPRS_ONLY != plmn_status
        && is_ps_ready){
        ecc.call_type = FAKE_ECC;
        ecc.dual_sys = (MN_DUAL_SYS_E)dual_sys;
        }
        else
        {
            for(i = MN_DUAL_SYS_1; i < MMI_DUAL_SYS_MAX; i++)
            {
                plmn_status = MMIAPIPHONE_GetPlmnStatus((MN_DUAL_SYS_E)i);
                is_ps_ready = MMIAPIPHONE_GetIsPsReady((MN_DUAL_SYS_E)i);
                if(PLMN_NO_SERVICE != plmn_status
                && PLMN_EMERGENCY_ONLY != plmn_status
                && PLMN_EMERGENCY_SIM_INVALID != plmn_status
                && PLMN_EMERGENCY_GPRS_ONLY != plmn_status
                ...
                && is_ps_ready){
                ecc.call_type = FAKE_ECC;
                ecc.dual_sys = (MN_DUAL_SYS_E)i;
                break;
                }
            }
        }
       SCI_TRACE_LOW("ccapp: MMIAPICC_EmergencyCallCheckType plmn_status[%d] =%d,is_ps_ready[%d] =%d", ecc.dual_sys, plmn_status, ecc.dual_sys, is_ps_ready);
    }
    return ecc;    
}
...
PUBLIC BOOLEAN MMICC_IsEmergencyCallConnecting(void)
{
    return is_ecc_call;
}

PUBLIC BOOLEAN MMICC_IsEmergencyCallAlerting(void)
{
  return is_ecc_call_alerting;
}
/*****************************************************************************/
// 	Description : if the SIM is ok
//	Global resource dependence : 
                              ...
                              )
{
    ...
    SCI_TRACE_LOW("ccapp: CC_OpenAudioTunnel is_enable=%d", is_enable);
    if (is_enable)
    {
...
LOCAL CC_RESULT_E CC_ReleaseActiveCall(void)
{
    ...
    //SCI_TRACE_LOW:"ccapp.c:enter function CC_ReleaseActiveCall()"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMICC_APP_4091_112_2_18_2_10_15_108,(uint8*)"");
    
    //search the call which state is active
    for( i = 0; i < s_call_context.call_number ; i++ )
    {
        if( CC_CALL_STATE == s_call_context.call_info[ i ].call_state )
        {
            ...
            if(MN_RETURN_SUCCESS != MNCALL_DisconnectCallEx(s_call_context.dual_sys, s_call_context.call_info[ i ].call_id, MN_CAUSE_NORMAL_CLEARING, PNULL))
            {
...
LOCAL CC_RESULT_E CC_HoldAndAnswerCW(void)
{
        ...
        ( CC_NO_NUM == s_call_context.exist_call.exist_hold_call_num ) )    //只有active
    {
        ...
        s_is_need_connect_call_after_hold = TRUE;
        SCI_TRACE_LOW("ccapp.c:enter function CC_HoldAndAnswerCW()  hold call=%d, incoming index =%d" , s_call_context.call_info[ index ].call_id,s_call_context.incoming_call_index);
...
LOCAL void CC_HandleRetrieveCnf( DPARAM param )
{
    ...
    {
        if(CC_HaveRelatedRecordCall(sig_ptr->call_id))
        {
            MMICC_ResumeCCRecord();
        }
                            ...
                            )
{
    ...
    if( sig_ptr->req_is_accepted )
    {
        ...
        else	//只有active
        {
                ...
                ( CC_NO_NUM == s_call_context.exist_call.exist_active_call_num ) )
            {
                ...
                {
                    s_call_context.current_call = call_index;
                    if(ERR_MNCALL_FAIL_BY_FDN != MNCALL_GetErrCodeEx(sig_ptr->dual_sys))
                    {
                    ...
                    else
                    {
                        ...
                        MMICC_OpenAlertFailWin(TXT_CC_FDN_ONLY);
            ...
            else
            {
				...
				// 收到HOLD CNF后，刷新一次通话介面
				MMK_PostMsg(MMICC_STATUS_WIN_ID, MSG_GET_FOCUS, PNULL, 0);
    ...
    else
    {
        ...
        s_call_context.current_call = FindCallInModule();
		if (CFW_SSCheckTestSim(sig_ptr->dual_sys) == FALSE)//IF enter GCF test, dont Display any warining 
		{
    ...
    s_is_need_connect_call_after_hold = FALSE;
                                       ...
                                       )
{
    ...
    tele_num, 
    CC_MAX_TELE_NUM_LEN + 2);
     SCI_TRACE_LOW("CC_HandleDisconnectedInMPTY s_call_context.call_info[call_index].call_type=%d s_call_context.call_info[call_index].call_state=%d",s_call_context.call_info[call_index].call_type,s_call_context.call_info[call_index].call_state);
     if((CC_INCOMING_CALL_STATE != s_call_context.call_info[call_index].call_state)&&(CC_MO_CALL_TYPE == s_call_context.call_info[call_index].call_type))
    {
        isEmergencyCall = MMIAPICC_IsEmergencyNum(tele_num, tele_len, s_call_context.want_mo_call.dual_sys, FALSE);
    }
    ...
    if(s_call_context.call_number > 1){ // bug1162871  挂断时先判断静音再删除通话信息
        MMICC_EnableMute(MMICC_IsCurrentCallMute());
         //删除相关CALL INFO
        CC_DeleteCallInfo( call_index );
    } else {
...
#ifdef MMI_VOLTE_SUPPORT
    SCI_TRACE_LOW("CC_HandleDisconnectedInMPTY: isEmergencyCall=%d", isEmergencyCall);
#endif
    if ((0 == s_call_context.exist_call.exist_active_call_num
        ...
        )
    {
        if (0 != s_bt_call_type_event_timer)
        {
            SCI_TRACE_LOW("CC_HandleDisconnectedInMPTY(), stop BT open audio timer");
            MMK_StopTimer(s_bt_call_type_event_timer);
            s_bt_call_type_event_timer = 0;
        }
        if ((!MMICC_IsExistIncommingCall())
    ...
    if(s_call_context.exist_call.exist_incoming_call)
    {
        SCI_TRACE_LOW("ccapp CC_HandleDisconnectedInMPTY existincoming call, incoming index=%d, operate_waiting_call=%d,  current_call=%d ", 
		s_call_context.incoming_call_index,
		s_call_context.operate_waiting_call,
		s_call_context.current_call);
    }
    if( CC_NO_NUM == s_call_context.call_number )
    {        
...
//STK hold failed without HOLD CNF MSG Begin
    if( s_is_need_connect_call_after_hold) //mt call
    {    		
    		SCI_TRACE_LOW("ccapp CC_HandleDisconnectedInMPTY  existincoming call %d , current_call=%d", s_call_context.incoming_call_index, s_call_context.current_call);
    		if(s_call_context.current_call == s_call_context.incoming_call_index)
    		{
	                s_call_context.incoming_call_index = CC_INVALID_INDEX;
	                CC_ConnectCallReq();
		}
    ...
    else if( ( s_is_need_call_after_hold ) && (s_call_context.want_mo_call.is_want_mo_call) && ( CC_NO_NUM == s_call_context.exist_call.exist_active_call_num) )//mo call
    {
                CC_RESULT_E cc_result = CC_RESULT_SUCCESS;
                SCI_TRACE_LOW("ccapp CC_HandleDisconnectedInMPTY with MO call tele_num = %s, tele_len = %d ", tele_num, tele_len);
                if (s_call_context.want_mo_call.is_save_name)//this call is from a record
                {
                    MMI_STRING_T name_str = {0};
                    wchar               str_buf[CC_MAX_PBNAME_LEN + 1] = {0};
                    ...
                    cc_result = MMIAPICC_MakeCall(
                        s_call_context.want_mo_call.dual_sys,
                        tele_num,
                        tele_len,
                        PNULL,
                        &name_str,
                        MMIAPICC_GetCallSimType(s_call_context.want_mo_call.dual_sys),
                        CC_CALL_NORMAL_CALL,
                        PNULL
                        );
                }
                ...
                SCI_TRACE_LOW("APP_MN_CALL_DISCONNECTED_IND with MO call  cc_result =%d ", cc_result);
                if (CC_RESULT_SUCCESS != cc_result)
                {
                    s_call_context.current_call = call_index;
                    if(ERR_MNCALL_FAIL_BY_FDN != MNCALL_GetErrCodeEx(s_call_context.want_mo_call.dual_sys))
                    {
                        MMICC_UpdateCallStatusDisplay(MMICC_CALL_AFTER_HOLD_FAIL_IND);
                    }
                    else
...
//STK hold failed without HOLD CNF MSG End
                              ...
                              )
{
    ...
    if( CC_NO_NUM != s_call_context.call_number )
    {
        ...
        else if( CC_NO_NUM != s_call_context.exist_call.exist_active_call_num )
        {
            ...
            s_call_context.current_call = active_index;
            hold_by_user[s_call_context.call_info[active_index].call_id] = 1;
            //hold the active call
        ...
        else
        {
            ...
            hold_by_user[s_call_context.call_info[hold_index].call_id] = 0;
            //retrieve the hold call
    ...
    else
    {
        ...
        MMIPUB_OpenAlertWarningWin(STR_OPERATION_FAIL_EXT01);
#if !defined(MMI_GUI_STYLE_TYPICAL)
        MMIPUB_SetWinTitleTextId( MMIPUB_ALERT_WIN_ID, STR_CALL_ON_HOLD, FALSE );
#endif
        return CC_RESULT_ERROR;
    }
...
{    
    return s_call_context.exist_call.exist_outgoing_call;
}
...
{    
    return s_call_context.exist_call.exist_incoming_call;
}
                                 ...
                                 )
{
    ...
    if( sig_ptr->req_is_accepted )
    {
        ...
        if(MMICC_IsTheCallRecording(sig_ptr->call_id))
        {
            MMICC_ResumeCCRecord();
        }
        else
                                 ...
                                 )
{
	...
	if (MMIAPISET_GetIsWhitelistVedioCall() || MMIAPISET_GetIsWhitelistAudioCall())
	   {
		   if (!MMIAPISET_IsCCWhitelist(&call_num, call_type))
		   {
			   result = TRUE;
		   }
	   }
	   else if (MMIAPISET_GetIsBlacklistVedioCall() || MMIAPISET_GetIsBlacklistAudioCall())
	   {
		   if (MMIAPISET_IsCCBlacklist(&call_num,call_type))
		   {
			   result = TRUE;
		   }
	   }
...
LOCAL CC_RESULT_E SendDtmfByChar(MN_DUAL_SYS_E dual_sys, uint8 dtmf_ch, BOOLEAN is_stop_immediately)
{
    ...
    if(CC_INVALID_DTMF_VALUE != dtmf_ch)
    {
	...
	#ifdef HERO_ENGINE_TTS_SUPPORT
		// 屏蔽默认按键音
		extern BOOLEAN hero_tts_dial_broadcast_is_open(void);
		if(hero_tts_dial_broadcast_is_open())
		{
			//
		}
		else
        ...
        //start send dtmf
        if(MN_RETURN_SUCCESS != MNCALL_StartDTMFEx(dtmf_dual_sys, dtmf_ch, s_call_context.call_info[s_call_context.current_call].call_id))
        {
...
PUBLIC BOOLEAN MMIAPICC_IsInState(CC_CALL_STATE_FOR_USING_E call_state)
{
    ...
    case CC_MO_CONNECTING_STATE:
        if ((0 != s_call_context.exist_call.exist_outgoing_call)
#ifdef VT_SUPPORT
    ...
    case CC_MT_CONNECTING_STATE:
        if ((0 != s_call_context.exist_call.exist_incoming_call)
#ifdef VT_SUPPORT
...
{
    if(MMICC_IsCurrentCallIDValid() && s_call_context.call_info[s_call_context.current_call].name_len > 0)
    {
...
LOCAL void GetCurrentCallNumber(MMIPB_BCD_NUMBER_T *bcd_number_ptr, BOOLEAN is_remove_ip_head, BOOLEAN is_want_mo_call)
{
    ...
    MMI_MEMCPY(
        ...
        sizeof(address_ptr->party_num)//address_ptr->num_len
        );	
}
/*****************************************************************************/
//  Description : get current call number
...
PUBLIC BOOLEAN MMIAPICC_ClearMissedCallNum(void)
{
    ...
    MMIAPICOM_StatusAreaSetMissedCallState(FALSE);
    MMIAPICC_SaveMissedCallNumToNv();
    //[when enter call log,remove idle missed call msg]
    MMIIDLE_RemovePushMsgByType(MMI_IDLE_PUSH_MSG_MISSED_CALL);
    //[end]
...
{	 
    ...
    if (MMICC_IsCurrentCallIDValid())
        cur_time_count = s_call_context.call_time_count - s_call_context.call_info[s_call_context.current_call].start_call_time_count;
...
PUBLIC CC_RESULT_E MMIAPICC_ReleaseAspCallReq(MN_CALL_DISCONNECT_CALL_CAUSE_E cause)
{
    ...
    if(MN_RETURN_SUCCESS != MNCALL_DisconnectCallEx(s_call_context.dual_sys, s_asp_call_id, cause, PNULL))
    {
...
PUBLIC BOOLEAN MMIAPICC_IsInputEmergencyCall(MMI_STRING_T *str_info_ptr, uint16 *emergency_call_len_ptr, MN_DUAL_SYS_E dual_sys)
{
    ...
    if(CC_MAX_TELE_NUM_LEN > str_info_ptr->wstr_len)
    {
        MMIAPICOM_WstrToStr(str_info_ptr->wstr_ptr, temp_tele_num);
        temp_tele_len = strlen(temp_tele_num);
    }
    else
    {
        SCI_TRACE_LOW("ccapp.c: MMIAPICC_IsEmergencyPartNum number len = %d",  str_info_ptr->wstr_len);
        return FALSE;
    }
    
    if(dual_sys >= MMI_DUAL_SYS_MAX)
    {
        SCI_TRACE_LOW("ccapp.c: MMIAPICC_IsEmergencyPartNum dual_sys = %d", dual_sys);
        call_dual_sys = MMIAPIPHONE_GetCurResponsePinSimID();
    }
    else
    {
    ...
    if(call_dual_sys >= MMI_DUAL_SYS_MAX)
    {
        call_dual_sys = MN_DUAL_SYS_1;
    }
    
    is_input_emergency_call = MMIAPICC_IsEmergencyPartNum(temp_tele_num, temp_tele_len, call_dual_sys, FALSE);
    if(is_input_emergency_call)
    {
        *emergency_call_len_ptr = emergency_num_len;
        emergency_num_len = 0;
    }
...
//	Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPICC_IsInSosString(char *str_info_ptr, MN_DUAL_SYS_E dual_sys)
{
	uint32 i = 0;
	uint32 j = 0;
	uint16 emergency_call_len = 0;
	BOOLEAN is_in_sos_str = FALSE;
	MNSIM_ECC_T sim_emc = {0};//记录sim卡存储的emc号码
	MN_DUAL_SYS_E call_dual_sys = MN_DUAL_SYS_1;
	...
	if (PNULL == str_info_ptr || 0 == strlen(str_info_ptr))
	{
		return FALSE;
	}
	...
	for (j = 0; j < MMI_DUAL_SYS_MAX; j++)
	{
		...
		if (PNULL != local_emergency_list_ptr)
		{
			for (i = 0; ; i++)
			{
				if (PNULL != local_emergency_list_ptr[i].emergency_call_ptr)
				{
					search_result = strstr(local_emergency_list_ptr[i].emergency_call_ptr, str_info_ptr);
					if (NULL != search_result && search_result == local_emergency_list_ptr[i].emergency_call_ptr)
					{
						is_in_sos_str = TRUE;
						return is_in_sos_str;
					}
					else
					{
						is_in_sos_str = FALSE;
	...
	if (s_cc_csc_sos.sos_num > 0)
	{
		...
		while (PNULL != cur_csc_sos_ptr)
		{
			...
			{
				search_result = strstr(cur_csc_sos_ptr->sos_num_ptr, str_info_ptr);
				if (NULL != search_result && search_result == cur_csc_sos_ptr->sos_num_ptr)
				{
					is_in_sos_str = TRUE;
					return is_in_sos_str;
				}
...
#endif
	{
...
#endif
		{
			...
			{
				search_result = strstr(s_emergency_call[i], str_info_ptr);
				if (NULL != search_result && search_result == s_emergency_call[i])
				{
					is_in_sos_str = TRUE;
					return is_in_sos_str;
				}
	...
	for (j = 0; j < MMI_DUAL_SYS_MAX; j++)
	{
		if (MMIAPIPHONE_GetSimExistedStatus(j))
		{
			...
			if (0 < sim_emc.validate_num)
			{
				...
				{
					uint8 ecc_code[MN_MAX_ECC_LEN*2 + 2] = {0};
					wchar ecc_code_wstr[MN_MAX_ECC_LEN*2 + 2] = {0};
					uint8 tel_num_len = 0;
					...
					search_result = strstr(ecc_code, str_info_ptr);
					if (NULL != search_result && search_result == ecc_code)
					{
						is_in_sos_str = TRUE;
						return is_in_sos_str;
					}
	...
	{    
			...
			|| SIM_NOT_INITED == MMIAPIPHONE_GetSimStatus(dual_sys))
		{
...
#endif
			{
				...
				for (i = 0; i < emergency_call_len; i++)
				{
					search_result = strstr(s_emergency_call_no_sim[i], str_info_ptr);
					if (NULL != search_result && search_result == s_emergency_call_no_sim[i])
					{
						is_in_sos_str = TRUE;
						return is_in_sos_str;
...
//     MMISET_DUALSYS_TYPE_E dual_setting = MMISET_DAULSYS_ONLY_SIM1_ON;
    ...
    else if(sim_num > 1)
    {
         ...
         else
         {
             ...
             {
                 if(MMISET_MAJOR_SIM_SEL_MANUAL_SEL > major_sim_sel_type)
                 {
...
LOCAL BOOLEAN IsNeedCRSS(void)
{
    if(s_call_context.current_call >= CC_MAX_CALL_NUM)
    {
        return FALSE;
    }
                                                           ...
                                                           )
{
    ...
    for( i = 0;  i < MMI_DUAL_SYS_MAX; i++)
    {
        ...
        if( MMIAPIPHONE_GetSimExistedStatusEx((MN_DUAL_SYS_E) i)
            //bug 186648 modify is_only_for_display(the last parameter of MMIAPICC_IsEmergencyNum) true to false,is_only_for_display is setted true is to some requirement,
            //but in different countries in normal status the phone make NORMAL call by 000, 08, 110, 999, 118 and 119 num(no sim emergency) ,
            //it is not normal display emergency call in window
   ...
   )
{
    ...
    if(MMK_IsOpenWin(PHONE_LANGUAGE_FIRST_POWER_WIN_ID))
     {
            SCI_TRACE_LOW("ccapp.c: PHONE_LANGUAGE_FIRST_POWER_WIN_ID is open,can not make call");
        return CC_RESULT_ERROR;
      }
...
#ifdef FLYMODE_ECC_SUPPORT	
    if (MMIAPISET_GetFlyMode() && (MMIAPICC_IsEmergencyNum(make_call_info.tele_num, make_call_info.tele_len, call_dual_sys, FALSE)))
    {
        if(OpenAlertWarningWinEmergency())
        {
            return CC_RESULT_ERROR;
        }
        else
        {
            MMIAPISET_SetFlyModeWholeFlow(FALSE);
            MMICC_EnterFlymodeEccWin((ADD_DATA)(&make_call_info));
        }
    }
    else
#endif
#if defined(MMI_SIM_LOCK_TYPE2_SUPPORT) && !defined(MMI_SIM_LOCK_SUPPORT)
	// 如果锁卡的情况下呼出紧急号码，需开启紧急协议栈
	if (MMIAPIPHONE_GetIsLegalState() == FALSE && MMIAPIPHONE_GetSimExistedNum(PNULL, 0) > 0 &&
		MMIAPICC_IsEmergencyNum(make_call_info.tele_num, make_call_info.tele_len, call_dual_sys, FALSE))
	{
		MN_DUAL_SYS_E i;
		for (i = MN_DUAL_SYS_1;i < MN_DUAL_SYS_MAX;i++)
		{
			if (MMIAPIPHONE_GetSimExistedStatusEx(i))
			{
				break;
			}
		}
		s_mmi_simlock_is_emergcall = TRUE;
		MMIAPIPHONE_PowerOnPsForEmerge(i);
		MMICC_EnterFlymodeEccWin((ADD_DATA)(&make_call_info));
	}
	else
#endif
...
LOCAL void CreateSimSelectionMenu2(MMI_WIN_ID_T win_id,MMI_CTRL_ID_T ctrl_id)
{
    ...
    MMI_GetLabelTextByLang(STR_SIM_DIFFERENT_SIM_EXT01,&name_str);
	GUIMENU_SetMenuText(&name_str,ctrl_id);
//    menu_ptr.item_text_ptr = &name_str;
//    GUIMENU_InsertNode(index++,CP_SIM_SEL_MENU_NOTE,CP_SIM_SEL_MENU_NODE_ROOT,&menu_ptr,ctrl_id);

    MMI_GetLabelTextByLang(STR_SIM_CURRENT_SIM_EXT01,&name_str);
    menu_ptr.item_text_ptr = &name_str;
    GUIMENU_InsertNode(index++,CP_SIM_SEL_MENU_SIM1,CP_SIM_SEL_MENU_NODE_ROOT,&menu_ptr,ctrl_id);

    MMI_GetLabelTextByLang(STR_SIM_PREFERRED_SIM_EXT01,&name_str);
    menu_ptr.item_text_ptr = &name_str;
...
//    GUIMENU_InsertNode(index++,CP_SIM_SEL_MENU_ALWAYS_ASK,CP_SIM_SEL_MENU_NODE_ROOT,&menu_ptr,ctrl_id);
   ...
   // MMI_GetLabelTextByLang(TXT_PHONE_SIM_ACTIVE, &stb_str);
...

{
		...
		case MSG_CTL_MIDSK:
			...
			{
			case 3:
				s_cur_dual_sys =  make_call_info_ptr->dual_sys;
				if(SIM_STATUS_OK == MMIAPIPHONE_GetSimStatus(s_cur_dual_sys)){
					MMICC_ProcessMakeCall(s_cur_dual_sys, make_call_info_ptr);
					MMK_CloseWin(win_id);
				}
				break;
			case 2:
				s_cur_dual_sys =1;
				if (make_call_info_ptr->dual_sys==0)
					{
					s_cur_dual_sys =1;
					}
				else
        ...
        case MSG_CLOSE_WINDOW:
            CC_FreeMakeCallInfo(make_call_info_ptr);
            break;
		default:
...
#endif
#if defined(MMI_DUALMODE_ENABLE)
   BOOLEAN  is_pin_input_flag = FALSE;
#endif
...
#if defined(MMI_DUALMODE_ENABLE) 
       //[MS]bug480187
        if (0 == strncmp((char *)make_call_info_ptr->tele_num, "**05*", 5)//**05*PUK*NEW_PIN*NEW_PIN#
            || 0 == strncmp((char *)make_call_info_ptr->tele_num, "**04*", 5)//**04*PIN*NEW_PIN*NEW_PIN#
            || 0 == strncmp((char *)make_call_info_ptr->tele_num, "**052*", 6)//**052*PUK2*NEW_PIN2*NEW_PIN2#
            || 0 == strncmp((char *)make_call_info_ptr->tele_num, "**042*", 6))//**042*PIN2*NEW_PIN2*NEW_PIN2#
        {
            is_pin_input_flag = TRUE;			
        }
        SCI_TRACE_LOW("[PIN] MMICC_ProcessMakeCall is_pin_input_flag = %d", is_pin_input_flag);//[MS]bug480187
//add by ljg
         if(MMIAPICC_IsEmergencyNum(make_call_info_ptr->tele_num, make_call_info_ptr->tele_len, dual_sys, FALSE)&&OpenAlertWarningWinEmergency())
         {
                return CC_RESULT_ERROR;
         }  

        if ((FALSE == is_pin_input_flag) && !CC_IsNetSupport(dual_sys))
        {
            ...
            //提示网络不可用
            //[for net is not available]
            //MMIPUB_OpenAlertWarningWin(/*TXT_NET_UNAVAIL_FOR_CALL*/STR_CL_NOTE_CALL_FAILED_EXT01);
            DisplayBannerWin(0,STR_CL_NOTE_CALL_FAILED_EXT01);
            //[end]
            return CC_RESULT_ERROR;
            }
#else
         ...
         //BUG 1119547,1119561 start
         if(!MMIAPICC_IsEmergencyNum(make_call_info_ptr->tele_num, make_call_info_ptr->tele_len, dual_sys, FALSE) && CC_IsEmergencyOnlyStatus(dual_sys))
         {
                SCI_TRACE_LOW("ccapp.c:MMICC_ProcessMakeCall() emegency call ONLY");
                DisplayBannerWin(0,STXT_EMERGENCY_ONLY);
                return CC_RESULT_ERROR;
         }
         //BUG 1119547,1119561 end
#endif
           ...
           || MMI_PARSE_NORMAL_CALL_TYPE_E == parse_result)
       {
                ...
                &&!CC_IsSimOk(dual_sys))
            {
                ...
                {
                    MMIPUB_OpenAlertWarningWin(STXT_EMERGENCY_ONLY);
                }
           ...
           if ((s_call_context.call_number > CC_NO_NUM) && (s_call_context.dual_sys != dual_sys))//cr128072
           {
               ...
               MMIPUB_OpenAlertWarningWin(STR_CL_SIM_IN_CALL);
               return CC_RESULT_ERROR;
           }
           if (CC_NO_NUM != s_call_context.exist_call.exist_active_call_num)//已有通话
           {
               ...
               else
               {
                   ...
                   //get the active call,because MNCALL_HoldCallEx need first active call_id
...
#endif
            {   //不再选卡直接呼叫 0 sim1 1 sim2
                //MMICC_ProcessMakeCall(0, call_info_ptr);
                ...
                wchar  str_buf[CC_MAX_PBNAME_LEN + 1] = {0};
                name_str.wstr_ptr=str_buf;
                MMIAPIPB_StringToBCDNumber(make_call_info_ptr->tele_num,&number);
                MMIAPIPB_GetNamePreferredSimByNumber(&number,&name_str,&pb_prefer_sim,10,TRUE);
                call_info_ptr->pb_prefer_sim=pb_prefer_sim;
#if defined UNISOC_VERSION && defined MMI_SUPPORT_CONTROLPANEL
                if (prefer_sim==ALWAYS_ASK)
#endif
                {
                    //check if there has a active calling 
                    if(MMIAPICC_GetCallingSIM() < MMI_DUAL_SYS_MAX)
                    {
                        MMICC_ProcessMakeCall(MMIAPICC_GetCallingSIM(), call_info_ptr);//use same SIM ID for calling
                        CC_FreeMakeCallInfo(call_info_ptr);                        
                    }
                    else
                    {
                ...
                {
                    switch(pb_prefer_sim)
                    {
                        case 0:
                            switch (prefer_sim)
                            {
                                case PREF_SIM_1:
                                    MMICC_ProcessMakeCall(PREF_SIM_1, call_info_ptr);
                                    CC_FreeMakeCallInfo(call_info_ptr);
                                    break;
                                case PREF_SIM_2:
                                    MMICC_ProcessMakeCall(PREF_SIM_2, call_info_ptr);
                                    CC_FreeMakeCallInfo(call_info_ptr);
                                    break;
                                default:
                                    MMICC_OpenSelectSimWin((ADD_DATA) call_info_ptr);
                            ...
                            break;
                        case 1:
                            if (prefer_sim==PREF_SIM_2)
                            {
                                call_info_ptr->dual_sys=PREF_SIM_1;
                                MMK_CreateWin((uint32*)MMICC_SELECT_PRE_SIM_WIN_TAB, (ADD_DATA) call_info_ptr);	
                            }
                            else
                            {
                                MMICC_ProcessMakeCall(PREF_SIM_1, call_info_ptr);
                            ...
                            break;
                        case 2:
                            if (prefer_sim==PREF_SIM_1)
                            {
                                call_info_ptr->dual_sys=PREF_SIM_2;
                                MMK_CreateWin((uint32*)MMICC_SELECT_PRE_SIM_WIN_TAB, (ADD_DATA) call_info_ptr);	  
                            }
                            else
                            {
...
LOCAL void StartLocalNetRing(void)
{
	...
	// Bug 1711674, 连接蓝牙情况下不播放local ring DSP tone音，避免播放失败后续无声
	if (MMISRVAUD_IsBTHeadSetActive())
	{
		SCI_TRACE_LOW("StartLocalNetRing: MMISRVAUD_IsBTHeadSetActive");
		return;
	}

    SCI_TRACE_LOW("StartLocalNetRing: IsIndiaVersion=%d, IsVolte=%d", MMIAPICC_IsIndiaVersion(), MMIAPICC_IsVolteNetworkStatus());
    if(MMIAPICC_IsIndiaVersion() && MMIAPICC_IsVolteNetworkStatus())
    {
        SCI_TRACE_LOW("StartIndiaLocalRing: s_india_ring_timer=%d", s_india_ring_timer);
        MMISRVAUD_PlayToneEx(MMISRVAUD_PRI_HIGH, MMISRVAUD_TYPE_TONE_GENERIC, MMI_NT_RING_CON_ID, MMI_400MSECONDS,MMISRVAUD_TONE_FREQ_DEFAULT);
        if (0 != s_india_ring_timer)
        {
            MMK_StopTimer(s_india_ring_timer);
            s_india_ring_timer = 0;
        }
        s_india_ring_timer = MMK_CreateTimerCallback(MMI_400MSECONDS, HandleIndiaRingToneTimer, PNULL, FALSE);
        s_india_ringtone_state = CC_RING_TONE_PAUSE_200MS_START;
    }
    else
    {
...
LOCAL void StopLocalNetRing(void)
{
    SCI_TRACE_LOW("StopIndiaLocalRing: s_india_ring_timer=%d, s_network_ring_timer=%d", s_india_ring_timer, s_network_ring_timer);
    if (0 != s_india_ring_timer)
    {
        MMK_StopTimer(s_india_ring_timer);
        s_india_ring_timer = 0;
    ...
    s_india_ringtone_state = CC_RING_TONE_NONE;
    if (0 != s_network_ring_timer)
    {
...
LOCAL void HandleIndiaRingToneTimer(uint8  timer_id, uint32 param)
{
    SCI_TRACE_LOW("MMIAPICC_HandleIndiaRingToneTimer: timer_id=%d, tone_state=%d", timer_id, s_india_ringtone_state);
    if (0 != s_india_ring_timer)
    {
        MMK_StopTimer(s_india_ring_timer);
        s_india_ring_timer = 0;
    }
    switch (s_india_ringtone_state)
    {
    case CC_RING_TONE_NONE:
        MMISRVAUD_StopTone(MMISRVAUD_TYPE_TONE_GENERIC);
        s_india_ringtone_state = CC_RING_TONE_NONE;
        break;

    case CC_RING_TONE_PLAY_400MS_START_1:
        s_india_ring_timer = MMK_CreateTimerCallback(MMI_400MSECONDS, HandleIndiaRingToneTimer, NULL, FALSE);
        MMISRVAUD_PlayToneEx(MMISRVAUD_PRI_HIGH, MMISRVAUD_TYPE_TONE_GENERIC, MMI_NT_RING_CON_ID, MMI_400MSECONDS,MMISRVAUD_TONE_FREQ_DEFAULT);
        s_india_ringtone_state = CC_RING_TONE_PAUSE_200MS_START;
        break;
        
    case CC_RING_TONE_PAUSE_200MS_START:
        s_india_ring_timer = MMK_CreateTimerCallback(MMI_200MSECONDS, HandleIndiaRingToneTimer, NULL, FALSE);
        MMISRVAUD_StopTone(MMISRVAUD_TYPE_TONE_GENERIC);
        s_india_ringtone_state = CC_RING_TONE_PLAY_400MS_START_2;
        break;
        
    case CC_RING_TONE_PLAY_400MS_START_2:
        s_india_ring_timer = MMK_CreateTimerCallback(MMI_400MSECONDS, HandleIndiaRingToneTimer, NULL, FALSE);
        MMISRVAUD_PlayToneEx(MMISRVAUD_PRI_HIGH, MMISRVAUD_TYPE_TONE_GENERIC, MMI_NT_RING_CON_ID, MMI_400MSECONDS,MMISRVAUD_TONE_FREQ_DEFAULT);
        s_india_ringtone_state = CC_RING_TONE_PAUSE_2600MS_START;
        break;
        
    case CC_RING_TONE_PAUSE_2600MS_START:
        s_india_ring_timer = MMK_CreateTimerCallback(MMI_2600MSECONDS, HandleIndiaRingToneTimer, NULL, FALSE);
        MMISRVAUD_StopTone(MMISRVAUD_TYPE_TONE_GENERIC);
        s_india_ringtone_state = CC_RING_TONE_PLAY_400MS_START_1;
        break;
                
    default:
        SCI_TRACE_LOW("MMIAPICC_HandleIndiaRingToneTimer: unknown state");
        break;
    }
}
...
PUBLIC BOOLEAN MMICC_IsMptyAndAnswerAvailable(void)
{
    BOOLEAN is_mpty_and_anwser_available = TRUE;
    uint8 call_index = 0;

    //bug 1253285: IMS can not build MPTY while there are 1 active, 1 hold and 1 incoming call
    if (MMIAPICC_IsVolteNetworkStatus() && s_call_context.call_number > 2)
    {
        for( call_index = 0; call_index < s_call_context.call_number ; call_index++ )
        {
            if (!s_call_context.call_info[ call_index ].is_in_mpty)
            {
                is_mpty_and_anwser_available = FALSE;
                break;
            }
        }
    }
    SCI_TRACE_LOW("ccapp:MMICC_IsMptyAndAnswerAvailable  %d",is_mpty_and_anwser_available);
    return is_mpty_and_anwser_available;
}
...
//	Note:2004.10.14
/*****************************************************************************/
PUBLIC CC_RESULT_E MMICC_ReleaseCallById(uint8 call_id);
...
{    
    ...
    if(!MMICC_IsExistIncommingCall()&&CustomGetCurrentCallIdInMuliCallList(MMICC_MULTI_CALL_ANIMATION_LIST_ID,&call_id))
    {
        return MMICC_ReleaseCallById(call_id);
    }
...
BOOLEAN FilterMsgFromATCmdCallEX(MMI_MESSAGE_ID_E    msg_id)
{
    return FilterMsgFromATCmdCall(msg_id);
}
...
LOCAL BOOLEAN IncomingCallName_To_Speech(void)
{
    ...
    uint16 play_len = 0;
	 uint16 play_num_len = 0;
    wchar* name_str_ptr = PNULL;
	 wchar* callnum_str_ptr = PNULL;
    ...
    //else
    {
        ...
        if(is_call_number_exist)
        {
            if (0 != tel_num_len)
            {
                ...
                callnum_str_ptr = callnum_wchar;
                MMICC_TTSValidNum(no_name_str.wstr_ptr, no_name_str.wstr_len, callnum_str_ptr, CC_MAX_PBNAME_LEN);
                play_num_len = MMIAPICOM_Wstrlen(callnum_str_ptr);            
	...
	play_times = ZDT_GetCCSwitch_type();
	 Trace_Log_Print("  IncomingCallName_To_Speech play_times  =%d",play_times );
	if(ZDT_GetCCSwitch_type()==2) //if(ZDT_GetCCSwitch_type()==3)
	{
		 return FALSE;
	}
	#endif
    MMIAPICOM_Wstrncat(play_text, callnum_str_ptr, play_num_len);
	...
	play_times = ZDT_GetCCSwitch_type();
	 ...
	 if (ZDT_GetCCSwitch_type() == 0)
	 {
		 play_param.play_times = 1;
	 }
	 else if(ZDT_GetCCSwitch_type() == 1)
	 {
		play_param.play_times = 0;/// play_param.play_times = 2;
	 }	
	// else if(ZDT_GetCCSwitch_type() == 2)
	 //{
    ...
    play_param.play_times = 0;
	#endif
	 Trace_Log_Print("  IncomingCallName_To_Speech play_param.play_times  =%d",play_param.play_times );
    play_param.tts_vol = ZDT_GetVol_Num();//    MMIAPIENVSET_GetActiveModeRingVol(MMISET_RING_TYPE_CALL);
    play_param.win_handle = MMICC_ANIMATION_WIN_ID;
    play_param.ring_type = TTS_RING_TYPE_CALL;
	play_param.tts_route =MMISRVAUD_ROUTE_SPEAKER ;/// MMISRVAUD_ROUTE_EARFREE;
                                                            ...
                                                            )
{
        ...
        for (i=0; i<instr_len; i++)
        {
        #if (defined(MMI_READ_INCOMING_CALL_NUM)&&defined(CCT_DIALPANEL_RING_SUPPORT))
            if ( '0' > innumber_str[i] || '9' < innumber_str[i])
            {
                continue;
            }
...
#ifdef __INKLEAK_TEST__ // 名字和号码都播报
LOCAL BOOLEAN IncomingCallName_To_Speech(void)
{
    MMI_STRING_T   no_name_str       = {0};
    //I_STRING_T   unknow_num_str    = {0}; //@fen.xie MS00176958
    ...
    wchar  play_text[CC_MAX_PBNAME_LEN + 13 + 15] = {0};
    uint16 play_len = 0;
    uint16 play_num_len = 0;
    wchar* name_str_ptr = PNULL;
    wchar* callnum_str_ptr = PNULL;
...
#if defined(MMI_READ_INCOMING_CALL_NUM)&&defined(CCT_DIALPANEL_RING_SUPPORT)
    if(!mmi_Is_ResFile_exist())
    {
        return MMIplay_incoming_call_num();
    }
#endif

    SCI_MEMSET(&incoming_call, 0, sizeof(MMI_STRING_T));
    MMI_GetLabelTextByLang(TXT_HERO_TTS_INCOMING_CALL_STRING, &incoming_call);
    MMIAPITTS_StopPlayTextByType(TTS_RING_TYPE_CALL);
    ...
    play_param.is_ucs2 = TRUE;
    play_param.play_times = 0;
    play_param.tts_vol = 1;
    play_param.ring_type = TTS_RING_TYPE_CALL;
	play_param.tts_route = MMISRVAUD_ROUTE_AUTO;//MMISRVAUD_ROUTE_EARFREE;
	play_param.digit_mode = TTS_DIGIT_NUMBER;
	play_param.punc_mode = TTS_PUNC_OFF;	
	hero_tts_printf("IncomingCallName_To_Speech play_len = %d", play_len);
    return MMIAPITTS_PlayText(&play_param);	
}
#else // 有名字则播报名字，没有名字播报号码
LOCAL BOOLEAN IncomingCallName_To_Speech(void)
{
    MMI_STRING_T   no_name_str       = {0};
    //I_STRING_T   unknow_num_str    = {0}; //@fen.xie MS00176958
	...
	play_param.digit_mode = TTS_DIGIT_NUMBER;
	play_param.punc_mode = TTS_PUNC_OFF;	
	hero_tts_incall_playing_stop(TRUE);//clark add 20220817
    return MMIAPITTS_PlayText(&play_param);	
}
#endif
/*****************************************************************************/
...
{
    //只在第一MO电话进行处理
    if( (CC_CALLING_STATE != s_call_context.call_info[s_call_context.current_call].call_state))
    {
...
//        if(APP_MN_SETUP_COMPLETE_CNF == msg_id)
//        {
//            StopLocalNetRing();
//        }        
        break;

    case APP_MN_CALL_PATH_IND://41576
        if ((0 != s_network_ring_timer) || (0 != s_india_ring_timer))
        {        
            SCI_TRACE_LOW("HandleMoMuteAndLocalRingState: APP_MN_CALL_PATH_IND");
            if(MO_CALL_STATE_SYNC == s_is_downlnk_mute_mocalling_flag)
            {
                MMICC_EnableNetRemoteMute(FALSE);
            }
    ...
    case APP_MN_ALERTING_IND:
        if ((MO_CALL_STATE_DONE != s_is_downlnk_mute_mocalling_flag) && (FALSE == s_is_call_audio_on))
        {
            StartLocalNetRing();
        }
        s_is_call_audio_on = FALSE;
                                   ...
                                   )
{
    ...
    if(!MMICC_IsCurrentCallIDValid() && CC_MAX_TELE_NUM_LEN + 2 > number_max_len)
    {
...
//bug 754242
PUBLIC MMI_RESULT_E handleConfirmationDialog(
...
PUBLIC BOOLEAN MMICC_AnswerCall(void)
{
    ...
    case CC_WAITING_STATE:
        ...
        else if(MMICC_GetCallNum() >= 3)
        {
            ...
            MMIPUB_OpenConfirmationDialogWinByTextIdEx(MMK_GetFirstAppletHandle(),
                PNULL,
                STR_CALL_DIAL_DROP,
                PNULL,
                PNULL,
                ...
                handleConfirmationDialog,(uint32)0);
#if 0
            //MMICC_OpenCCMenu();
                                                                                    ...
                                                                                    BOOLEAN is_need_modified)
{
    ...
    {
        wchar temp_wchar[MN_MAX_ADDR_BCD_LEN + 3] = {0};
        uint8 tel_num[MMICC_PHONE_NUM_MAX_LEN + 2] = {0};
        uint8 tel_num_len = 0;
        uint8 copy_len = 0;
        BOOLEAN b_search_result = FALSE;
        ...
        else
        {
            ...
            SCI_MEMSET(&prompt_str, 0x00, sizeof(prompt_str));
                ...
                copy_len = tel_num_len;
#else
                copy_len = ( tel_num_len > (MN_MAX_ADDR_BCD_LEN + 3) ) ? (MN_MAX_ADDR_BCD_LEN + 3) : tel_num_len;
#endif
...
PUBLIC void MMIAPICC_IsCallingAndInfoBt(void)
{
    ...
    if(MMICC_IsCurrentCallIDValid() && MMIAPICC_IsInState(CC_MT_CONNECTING_STATE))
    {
        cur_call_number = s_call_context.call_info[s_call_context.current_call].address;
...
PUBLIC BOOLEAN MMICC_GetCallNumStrByIndex(uint8* num_str_ptr, uint8* num_len_ptr, uint8 call_index)
{
    ...
    if((PNULL == num_str_ptr) || (PNULL == num_len_ptr) || !CC_IsCallIDValid(call_index))
    {
        return FALSE;
    }
...
{
    BOOLEAN is_name_exist = FALSE;
    uint8 call_number[60]                   = {0};
	if (s_call_context.call_info[call_index].address.num_len>0 && s_call_context.call_info[call_index].address.num_len<15)
	MMIAPICOM_BcdToStr(PACKED_LSB_FIRST,s_call_context.call_info[call_index].address.party_num,s_call_context.call_info[call_index].address.num_len<<1,call_number);
    if(!CC_IsCallIDValid(call_index))
    {
        return FALSE;
    }
    ...
    } else if (MMIAPICC_IsVoiceMailNum(call_number, strlen(call_number) ))
    {
    MMI_STRING_T    state_str = {0};
        ...
        if(PNULL != name_str_ptr)
        {
            name_str_ptr->wstr_ptr = state_str.wstr_ptr;
            name_str_ptr->wstr_len = state_str.wstr_len;
        }
    }
    else
    {
...
//add for msg_timer can not work exactly
PUBLIC void MMICC_CaliberationSetCallTimeCount(void)
{
     s_call_context.call_time_count= MMIAPICL_GetCallDurationTime(s_call_context.call_time_count_start, MMIAPICOM_GetCurTime());
}
...
PUBLIC BOOLEAN MMICC_IsCurrentCallMute(void)
{
    ...
    {
        SCI_TRACE_LOW("ccapp IsCurrentCallMute error: num  %d, cur %d ", s_call_context.call_number,s_call_context.current_call);
    ...
    //modify for Bug 494111,mute is for all calls.
    is_mute = MMICC_IsMute();
...
PUBLIC BOOLEAN MMICC_SetCurrentCallMute(BOOLEAN is_mute)
{
    ...
    if (result && MMICC_IsCurrentCallIDValid())
    {
        //if current call is in mpty
        if(s_call_context.call_info[s_call_context.current_call].is_in_mpty)
        {
...
//	Note:
/*****************************************************************************/
LOCAL BOOLEAN IsPartCSCSosNum(uint8 *tele_num_ptr, uint16 tele_len, MMICC_CSC_SOS_NUM_T *cc_csc_sos_ptr)
{
    MMICC_CSC_SOS_NUM_INFO_T *cur_csc_sos_ptr = PNULL;
    ...
    while (PNULL != cur_csc_sos_ptr)
    {
        if (PNULL != cur_csc_sos_ptr->sos_num_ptr
            ...
            && (0 == strncmp((char*)tele_num_ptr, (char *)cur_csc_sos_ptr->sos_num_ptr, tele_len)))
        {
            emergency_num_len = strlen((char *)cur_csc_sos_ptr->sos_num_ptr);
            return TRUE;
        }
...
LOCAL void HandleEmgNumListInd(DPARAM param)
{
    APP_MN_EMG_NUM_LIST_IND_T *emg_num_list_ptr = (APP_MN_EMG_NUM_LIST_IND_T *)param;
    MN_DUAL_SYS_E dual_sys = MN_DUAL_SYS_1;
...
#define NAME_SET_7BIT_WITH_INDICATION_DCS_LANG_LEN 3
#define NAME_SET_UCS2_WITH_INDICATION_DCS_LANG_LEN 2
//Only for GCF and SIM1
LOCAL wchar saved_call_name[CC_MAX_CALL_NUM][CC_MAX_PBNAME_LEN+1]={0};
LOCAL uint16 saved_call_name_len[CC_MAX_CALL_NUM]={0};

LOCAL void CC_ResetCallingName(uint8 index )
{
    if (index>= CC_MAX_CALL_NUM)
        {
            return;
        }
    if (!s_call_context.call_info[index].is_callingname_exist)
        {
            return;
        }
    //set calling name again
    s_call_context.call_info[index].is_name_exist= TRUE;
    s_call_context.call_info[index].name_len = sizeof(saved_call_name[index])/sizeof(wchar);
    if (s_call_context.call_info[index].name_len>CC_MAX_PBNAME_LEN)
        {
        s_call_context.call_info[index].name_len = CC_MAX_PBNAME_LEN;
        }
    SCI_MEMCPY(s_call_context.call_info[index].name,
        saved_call_name[index], s_call_context.call_info[index].name_len*sizeof(wchar));
    {
        BOOLEAN result = FALSE;
        MMI_STRING_T    name_str = {0};
        name_str.wstr_len =saved_call_name_len[index];
        name_str.wstr_ptr = saved_call_name[index];
        SCI_TRACE_LOW("ccapp.c: CC_ResetCallingName name_str.wstr_len=%d",name_str.wstr_len);
        if(s_call_context.call_info[index].is_name_exist)
        {
            result = GUILABEL_SetText(MMICC_CONNECTED_NAME_LABEL_CTRL_ID, &name_str, TRUE);
            SCI_TRACE_LOW("ccapp.c: CC_ResetCallingName  MMICC_CONNECTED_NAME_LABEL_CTRL_ID result=%d",result);
        }
        else
        {
            result = GUILABEL_SetText(MMICC_CONNECTED_NUMBER_LABEL_CTRL_ID, &name_str, TRUE);
            SCI_TRACE_LOW("ccapp.c: CC_ResetCallingName MMICC_CONNECTED_NUMBER_LABEL_CTRL_ID result=%d",result);
        }
    }
    return;
}
...
LOCAL void CC_SetCallingName(uint8 index)
{
    ...
    s_call_context.call_info[index].is_callingname_exist  = TRUE;
    //set name
...
LOCAL void CC_ClearCallingName(uint8 index)
{
    ...
    s_call_context.call_info[index].is_name_exist= FALSE;
    s_call_context.call_info[index].name_len = 0;
    SCI_MEMSET(s_call_context.call_info[index].name,0, (CC_MAX_PBNAME_LEN + 1)*sizeof(wchar));
    SCI_MEMSET(saved_call_name[index],0, (CC_MAX_PBNAME_LEN + 1)*sizeof(wchar));
    return;
}

LOCAL void  CC_HandleCallingNameInd(DPARAM param)
{
    APP_MN_CALLING_NAME_IND_T *sig_ptr      = (APP_MN_CALLING_NAME_IND_T*)param;
    uint8                  index   = CC_INVALID_INDEX;
    uint8                   i = 0;

    if(PNULL == sig_ptr)
    {
        SCI_TRACE_LOW("ccapp.c: CC_HandleCallingNameInd( param == NULL");
        return;
    }
    SCI_TRACE_LOW("ccapp.c:enter function CC_HandleCallingNameInd(), call_id = %d, calling_name = %d",
        sig_ptr->call_id,
        sig_ptr->calling_name
        );
    
    //get the index of call id
    index = CC_GetIndexOfCallId(sig_ptr->dual_sys, sig_ptr->call_id );

    if(index >= CC_MAX_CALL_NUM)
    {
        SCI_TRACE_LOW("ccapp.c: CC_HandleCallingNameInd( call_index >= CC_MAX_CALL_NUM");
        return ;
    }
    //get 
    if( (sig_ptr->calling_name ==MN_SS_PRESENT_RESTRICTED) ||
        (sig_ptr->calling_name ==MN_SS_NAME_UNAVAILABLE))
        {
            return;
        }
    
        
    switch(sig_ptr->name_set.data_code_scheme)
    {
        ...
        case MN_SS_7BIT_WITH_INDICATION_DCS:
            ...
            uint32 ret_val = 0;
            uint8 *temp_name_str = PNULL;
            uint16 temp_str_len = 0;
            uint16 len = 0;
            if (MN_SS_7BIT_WITH_INDICATION_DCS == sig_ptr->name_set.data_code_scheme
                && sig_ptr->name_set.name_str_len> NAME_SET_7BIT_WITH_INDICATION_DCS_LANG_LEN)
            {
                temp_name_str = (uint8 *)(sig_ptr->name_set.name_str +NAME_SET_7BIT_WITH_INDICATION_DCS_LANG_LEN);
                temp_str_len = sig_ptr->name_set.name_str_len-NAME_SET_7BIT_WITH_INDICATION_DCS_LANG_LEN;
                len = temp_str_len * 8 / 7 + 1;
            }
            else
            {
                temp_name_str = sig_ptr->name_set.name_str;
                temp_str_len = sig_ptr->name_set.name_str_len;
                len = temp_str_len * 8 / 7 + 1;
            }
            ...
            ret_val = SCI_Unpack7bitsTo8bits(temp_name_str,
                                                            (int16)temp_str_len,
                                                            0,
                                                            temp_buf,
                                                            (int16 *)&len);

            SCI_TRACE_LOW("ParseCallingNameData ret_val=%d, len=%d", ret_val, len);

            len = MIN(CC_MAX_PBNAME_LEN, len);

            len = MMIAPICOM_Default2Wchar(
                                            temp_buf,
                                            saved_call_name[index],
                                            len
                                            );
            saved_call_name_len[index] = len;
            CC_SetCallingName(index);
            SCI_FREE(temp_buf);
            temp_buf = PNULL;
        }
            break;
        case MN_SS_8BIT_NO_CLASS_DCS:
            saved_call_name_len[index]= sig_ptr->name_set.name_str_len;
            saved_call_name_len[index] = MMIAPICOM_Default2Wchar(
                                (uint8 *)sig_ptr->name_set.name_str,
                                saved_call_name[index],
                                sig_ptr->name_set.name_str_len
                                );
            CC_SetCallingName(index);
            break;
        ...
        case MN_SS_USC2_NO_CLASS_DCS:
        {
                uint8 *temp_name_str = PNULL;
                uint16 len = 0;
                if (MN_SS_UCS2_WITH_INDICATION_DCS == sig_ptr->name_set.data_code_scheme
                    && sig_ptr->name_set.name_str_len > NAME_SET_UCS2_WITH_INDICATION_DCS_LANG_LEN)
                {
                    temp_name_str = (uint8 *)(sig_ptr->name_set.name_str + NAME_SET_UCS2_WITH_INDICATION_DCS_LANG_LEN);
                    len = (uint16)(sig_ptr->name_set.name_str_len - NAME_SET_UCS2_WITH_INDICATION_DCS_LANG_LEN)/sizeof(wchar);
                }
                else
                {
                    temp_name_str = sig_ptr->name_set.name_str;
                    len = sig_ptr->name_set.name_str_len/sizeof(wchar);
                }

#if defined(__BIG_ENDIAN) || defined(__BigEndian)
                MMI_WSTRNCPY(
                    saved_call_name[index],
                    sizeof(saved_call_name[index]),
                    (wchar *)temp_name_str,
                    len,
                    len
                    );
#else
                GUI_UCS2L2UCS2B((uint8 *)saved_call_name[index],
                                                sizeof(saved_call_name[index]) * sizeof(wchar),
                                                (uint8 *)temp_name_str,
                                                len * sizeof(wchar));
#endif
                saved_call_name_len[index] = len;
    ...
    /*{
        ...
        if(s_call_context.call_info[index].is_name_exist)
            {
            GUILABEL_SetText(MMICC_CONNECTED_NAME_LABEL_CTRL_ID, &name_str, TRUE);
            }
        else
            {
            GUILABEL_SetText(MMICC_CONNECTED_NUMBER_LABEL_CTRL_ID, &name_str, TRUE);
            }
    }*/

    CC_ResetCallingName(index);

}


//[fu.ruilin, add for CALL UI]
PUBLIC BOOLEAN MMICC_IsValidCallState(CC_CALL_STATE_E call_state)
{
    if (call_state == CC_CALLING_STATE
                ||call_state == CC_CALL_STATE
                ||call_state == CC_INCOMING_CALL_STATE
                ||call_state == CC_HOLD_STATE
                ||call_state == CC_WAITING_STATE)
    {
        SCI_TRACE_LOW("MMICC_IsValidCallState: TRUE");
        return TRUE;
    }
        SCI_TRACE_LOW("MMICC_IsValidCallState: FALSE");
    return FALSE;
}

PUBLIC int MMICC_GetAllValidCallId(uint8 call_id[])
{
    int i = 0;
    int count = 0;
    for(i = 0; i < CC_MAX_CALL_NUM; i++)
    {
        if(MMICC_IsValidCallState(s_call_context.call_info[i].call_state))
        {
            call_id[count++] = s_call_context.call_info[i].call_id;
        }
    }
    return count;
}

PUBLIC int MMICC_GetAllOnGoingCallIndexs(uint8 *call_index)
{
    int i = 0;
    int count = 0;
    for(i = 0; i < CC_MAX_CALL_NUM; i++)
    {
        if(s_call_context.call_info[i].call_state == CC_HOLD_STATE
            || s_call_context.call_info[i].call_state == CC_CALL_STATE
            || s_call_context.call_info[i].call_state == CC_CALLING_STATE)
        {
            call_index[count++] = i;
        }
    }
...
PUBLIC CC_RESULT_E MMICC_ReleaseCallById(uint8 call_id)
{
    ...
    for( i = 0; i < s_call_context.call_number ; i++ )
    {
        if( call_id == s_call_context.call_info[ i ].call_id ||(release_mpty && s_call_context.call_info[i].is_in_mpty))
        {
            //whether the call ID is vaild
            if(  s_call_context.call_info[ i ].call_id >= CC_MAX_CALL_NUM )
            {
...
PUBLIC CC_RESULT_E MMICC_RetrieveCallByCallId(uint8 call_id)
{
    uint8	active_index = CC_INVALID_INDEX;
    uint8	hold_index = CC_INVALID_INDEX;
    ...
    if( CC_NO_NUM != s_call_context.call_number )
    {
                ...
                ( CC_NO_NUM != s_call_context.exist_call.exist_hold_call_num ) )
        {
            ...
            //get the hold call
            MMICC_GetIndexByCallId(call_id,&hold_index);//FindStateCall( CC_HOLD_STATE );
            if (hold_index >= s_call_context.call_number || hold_index>=CC_MAX_CALL_NUM)
            {
                //SCI_TRACE_LOW:"ccapp HoldCall: invalid hold idx: %d"
                //SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMICC_APP_5078_112_2_18_2_10_17_144,(uint8*)"d", hold_index);
                return CC_RESULT_ERROR;
...
PUBLIC BOOLEAN MMICC_GetIdByIndex(uint8 index,uint8 *call_id)
{
    BOOLEAN result  = FALSE;
    if(index < CC_MAX_CALL_NUM)
    {
        *call_id = s_call_context.call_info[index].call_id;
        result = TRUE;
    }
    return result;
}

PUBLIC BOOLEAN MMICC_GetIndexByCallId(uint8 call_id,uint8 *index)
{
    BOOLEAN result  = FALSE;
    uint16  i = 0;
    for( i = 0; i < CC_MAX_CALL_NUM ; i++ )
    {
        if( call_id == s_call_context.call_info[ i ].call_id )
        {
            *index = i;
            result  = TRUE;
            break;
...
PUBLIC MMIPB_BCD_NUMBER_T MMICC_GetPbNumByCallId(uint8 call_id)
{
    uint8 index = 0;
    MMIPB_BCD_NUMBER_T pb_num = {0};
    if(MMICC_GetIndexByCallId(call_id,&index))
    {
        return MMICC_GetPbNumByIndex(index);
    }
    return pb_num;
}

PUBLIC BOOLEAN MMICC_GetCallNameStrByCallId(MMI_STRING_T* name_str_ptr, uint8 call_id)
{
    uint8 index = 0;
    if(MMICC_GetIndexByCallId(call_id,&index))
    {
        return MMICC_GetCallNameStrByIndex(name_str_ptr,index);
    }
    return FALSE;
}

PUBLIC BOOLEAN MMICC_GetCallNumStrByCallId(uint8* num_str_ptr, uint8* num_len_ptr, uint8 call_id)
{
    uint8 index = 0;
    if(MMICC_GetIndexByCallId(call_id,&index))
    {
        return MMICC_GetCallNumStrByIndex(num_str_ptr,num_len_ptr,index);
    }
    return FALSE;
}

PUBLIC CC_CALL_STATE_E	MMICC_GetCallStatusByCallId(uint8 call_id)
{
    uint8 index = 0;
    if(MMICC_GetIndexByCallId(call_id,&index))
    {
        return MMICC_GetCallStatusByIndex(index);
    }
    return CC_NULL_STATE;
}

PUBLIC uint8 MMICC_GetMPTYCallIds(uint8 call_id[])
{
    int i = 0;
    uint8 count = 0;
    for(i = 0; i < CC_MAX_CALL_NUM; i++)
...
PUBLIC BOOLEAN MMICC_IsCallIdInMPTY(uint8 call_id)
{
    uint8 index;
    MMICC_GetIndexByCallId(call_id,&index);
    return MMICC_IsCallIndexInMPTY(index);
}

PUBLIC BOOLEAN CC_HaveRelatedRecordCall(uint8 call_id)
{
    uint8 call_ids[CC_MAX_CALL_NUM] = {CC_INVALID_ID};
    uint8 call_count = 0;
    uint8 i = 0;
    if(MMICC_IsTheCallRecording(call_id))
    {
        return TRUE;
    }
    else if(MMICC_IsCallIdInMPTY(call_id))
    {
        call_count = MMICC_GetMPTYCallIds(call_ids);
        for(i = 0; i < call_count; i++)
        {
            if(MMICC_IsTheCallRecording(call_ids[i]))
            {
                return TRUE;
            }
        }
...
PUBLIC void MMICC_SetAllCallsMute(BOOLEAN is_mute)
{
    uint8 i = 0;
    for(i = 0; i < s_call_context.call_number; i++)
    {
...
PUBLIC BOOLEAN MMICC_IsHoldByUser(uint8 call_id)
{
    if(call_id < CC_MAX_CALL_NUM)
    {
        return hold_by_user[call_id] == 1;
    }
    return FALSE;
}
...
PUBLIC void MMICC_HandleRemoteUserBusy()
{
    ...
    banner_info.banner_type = MMI_BANNER_TEXT;
    banner_info.text1_id = TXT_CC_USER_BUSY;
    banner_info.start_point = start_point;
    SCI_TRACE_LOW("ccapp MMICC_HandleRemoteUserBusy");
    MMIAPIPHONE_GetCurPLMN(&cur_Plmn, MMIAPICC_GetCallDualSys());
     if((cur_Plmn.mcc ==505) || (cur_Plmn.mcc ==530)) //fix bug704030, if disconnect cause is CAUSE_USER_BUSY, country AU&NZ need display TXT_CC_MT_CALL_REJECTED
     {
        banner_info.text1_id = STR_CALL_REJECTED;
    }
#ifdef PLAY_BUSY_TONE_FOR_UNISOC
    MMICC_SetIsPlayBusyToneFlag(TRUE);
#endif
    MMIAPICOM_OpenBannerWin(MMICC_USERBUSY_BANNER_WIN_ID, banner_info);
}
