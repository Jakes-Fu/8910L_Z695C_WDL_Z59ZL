...
/*****************************************************************************/
// 	Description : get the CC audio handle
//	Global resource dependence : 
//  Author:weipeng.wang
//	Note:
/*****************************************************************************/
PUBLIC MMISRV_HANDLE_T MMIAPICC_GetAudioHandle(void)
{
    return GetAudioHandle();
}
...
{
    BOOLEAN result = TRUE;
    BOOLEAN play_res = MMISRVAUD_RET_ERROR;
	...
	else
	{
		...
		SCI_TRACE_LOW("ccapp_a SwitchAudioDevice, pre_route=%d, target_route=%d", s_audio_status.pre_route, s_audio_status.target_route);
#ifdef BLUETOOTH_SUPPORT
	...
	if (result)
	{
        if(!MMISRVAUD_IsAudPlaying(GetAudioHandle()))//bug1084655
        {
            SCI_TRACE_LOW("ccapp_a SwitchAudioDevice, Play the CC audio handle");
            play_res = MMISRVAUD_Play(GetAudioHandle(), 0);

            if((s_audio_status.codec_handle == GetAudioHandle()) && MMISRVAUD_RET_OK == play_res)
            {
...
PUBLIC BOOLEAN MMICC_ConstructVoiceCodec (BOOLEAN is_vt_codec)
{
    ...
    //default system audio route is  speaker, need set handlehold for voice call
        ...
        {
             audio_srv.default_route = MMISRVAUD_ROUTE_HANDHOLD;
        }
        s_audio_status.pre_route = s_audio_status.default_route =  MMISRVAUD_ROUTE_HANDHOLD;
...
#ifdef BLUETOOTH_SUPPORT
	SetCallTypeEventFlag(FALSE); // Bug 1698250
#endif
}
...
PUBLIC void MMICC_EnableVoiceCodec ( BOOLEAN is_enable )
{
    ...
    if(is_enable)
    {
        ...
        {
			MMISRVAUD_SetDefaultRouteByHandle(handle, MMISRVAUD_ROUTE_HANDHOLD);			
        }
...
PUBLIC BOOLEAN MMICC_EnableMute ( BOOLEAN is_mute )
{
    ...
    if(result)
    {
        //MMICC_SetAllCallsMute(is_mute);
        s_audio_status.is_mute = is_mute;
    } else {
        SCI_TRACE_LOW("ccapp_a: MMISRVAUD_SetUpLinkMute false");
    }
    return result;
...
#ifdef HERO_ENGINE_TTS_SUPPORT
PUBLIC void HeroPlayCustomRing(void)
{
...
#elif defined(TTS_SUPPORT)		
	 Trace_Log_Print("MMIAPICC_StartRingOrVibrate   0000000 ");
        if(!MMICC_IncomingCallName_To_Speech())
         {
     	    Trace_Log_Print("MMIAPICC_StartRingOrVibrate   111111 ");
              PlayCustomRing(dual_sys, &pb_num);
         }
#else
        PlayCustomRing(dual_sys, &pb_num);
...
PUBLIC void MMIAPICC_StopRingOrVibrate(void)
{
    ...
    //stop vibrate
    if(MMIAPISET_IsVibratTimerActive())//dong.chunguang add
        MMIAPISET_StopVibratTimer();
    MMIAPISET_SetVibratorEx(FALSE,MMISET_RING_TYPE_CALL,FALSE,0);
	MMIAPISET_SetVibrator(FALSE,MMISET_RING_TYPE_NONE);//dong.chunguang add for stop vib
...
LOCAL void PlayCustomRing(MN_DUAL_SYS_E dual_sys, MMIPB_BCD_NUMBER_T *bcd_number_ptr)
{
    ...
    /* first play pb ring*/    
    {
        ...
        if(is_fixed_ring && fixed_ring_id==0)
        {
            if(s_audio_status.mt_ring_handle >0)
            {
                MMISRVMGR_Free(s_audio_status.mt_ring_handle);
        ...
        if(is_exist_in_pb)
        {
            is_ring_info_valid = ((is_fixed_ring) && (fixed_ring_id <= /*MMISET_RING_FIXED_DEFAUT_NUM*/g_ringtone_max))
                                ||((!is_fixed_ring) 
        ...
        if(is_ring_info_valid)
        {
            ...
            if(is_fixed_ring)
            {
                ...
                if( (fixed_ring_id - 1 )<MMISET_RING_FIXED_DEFAUT_NUM)
                {
                    MMI_GetRingInfo(
                        (fixed_ring_id + MMI_CALL_RING_START_ID - 1),
                        &ring_data);
                }
                else
                {
                    MMI_GetRingInfo(
#ifdef MMI_COMPILE_E
                        (fixed_ring_id + MMI_OPER_RING_START_ID - MMISET_RING_FIXED_DEFAUT_NUM- 1),
#else
                        (fixed_ring_id + MMI_CLOCK_RING_START_ID - MMISET_RING_FIXED_DEFAUT_NUM- 1),
#endif
                        &ring_data);
                }
...
//             }
    ...
    is_ring_info_valid = ((call_ring_info.call_ring_type == MMISET_CALL_RING_FIXED) && (call_ring_info.fix_ring_index <= /*MMISET_RING_FIXED_DEFAUT_NUM*/g_ringtone_max))
    ...
    if(is_ring_info_valid)
    {
        ...
        if(call_ring_info.call_ring_type == MMISET_CALL_RING_FIXED)
        {
            audio_srv.info.ring_buf.type = MMISRVAUD_TYPE_RING_BUF;
            MMI_GetRingInfo(
                (call_ring_info.fix_ring_index + MMI_CALL_RING_START_ID - 1),
                &ring_data);
...
#ifdef HERO_ENGINE_TTS_SUPPORT
extern BOOLEAN hero_tts_incall_playing(void);
#endif
/*****************************************************************************/
...
#ifdef HERO_ENGINE_TTS_SUPPORT
		|| MMIAPITTS_IsPlayRing(TTS_RING_TYPE_CALL)
		|| hero_tts_incall_playing()//clark add 20220817
#endif
        )
