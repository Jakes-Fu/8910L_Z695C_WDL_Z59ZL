...
PUBLIC void MMIAPICC_SendDtmf( uint32 msg_id)
{
    ...
    SCI_TRACE_LOW("[MMICC] MMIAPICC_SendDtmf Enter");
#if !defined(MMI_GUI_STYLE_TYPICAL)&&defined(TOUCH_PANEL_SUPPORT)
    ...
    SCI_TRACE_LOW("[MMICC] MMIAPICC_SendDtmf: is_process_win_focus=%d, is_dialWin_focus=%d", is_process_win_focus, is_dialWin_focus);
...
#ifdef HERO_ENGINE_TTS_SUPPORT
    {
                ...
                )
        {
			...
			{
				uint32 keystatus = msg_id & 0xFF00;
				uint32 keycode = msg_id & 0xFF;
				extern void hero_tts_printf(const char *format,...);
				hero_tts_printf("[inkleak.zhao] MMIAPICC_SendDtmf 333 msg_id = %d", msg_id);
				if(keystatus == KEY_PRESSED)
				{
					hero_tts_dial_broadcast(keycode);
				}
			}
        }
    }
...
//    win_create.win_priority = WIN_ONE_LEVEL;
    ...
    {
        win_create.win_priority = WIN_TWO_LEVEL;
    }
    else
    {
        win_create.win_priority = WIN_ONE_LEVEL;
...
LOCAL void PdaGetConferenceNumStr(MMI_STRING_T *conference_people, BOOLEAN is_hold_specific)
{
    ...
    MMI_WSTRNCPY(conference_people->wstr_ptr, (size_t)1, &temp_wchar, (size_t)1, (size_t)1);
    MMI_WSTRNCPY((conference_people->wstr_ptr + 1), (size_t)1, &space_ch, (size_t)1, (size_t)1);
    MMI_WSTRNCPY((conference_people->wstr_ptr + 2), (size_t)6, people_str.wstr_ptr, (size_t)6, (size_t)6);
}
                                             ...
                                             )
{
    ...
    if (!MMIAPICC_IsInputEmergencyCall(&prompt_str_array[CC_DISPALY_INFO_NUMBER], &emergency_call_len, MMI_DUAL_SYS_MAX))
    {
        ...
        {
            MMI_WSTRNCPY(display_str_ptr + sim_str_len, line_str_len, line_str.wstr_ptr, line_str_len, line_str_len);
            MMI_WSTRNCPY((display_str_ptr + sim_str_len + line_str_len), (size_t)space_ch_len, &space_ch, (size_t)space_ch_len, (size_t)space_ch_len);
                                          ...
                                          )
{
    ...
    case MSG_TTS_OVER: 
		 ...
		 Trace_Log_Print("MSG_TTS_OVER  PlayNewCallRing   00 0 0 0");
		cc_tts_over = TRUE;
	      PlayNewCallRing(MMIAPICC_GetCallDualSys());
	      s_waiting_call_ring_play_times ++;
		break;
...
LOCAL void CC_HandleAnimWinSoftkey( MMI_HANDLE_T win_handle, BOOLEAN is_need_fresh )
{
    ...
    if (MMICC_IsExistIncommingCall())
    {
         if( !MMICC_IsExistActiveCall() && !MMICC_IsExistHoldCall())
        {
            ...
            if (MMIAPICC_IsMtCallPlayingRing())
            {
                SetHandFreeMenu(win_handle,STXT_SOFTKEY_ANSWER_MK,TXT_NULL,TXT_RING_SILENT,is_need_fresh);
...
LOCAL void CC_OpenMissedCallWin(void)
{
    ...
    
    {
        ...
        if( PNULL != prompt_str.wstr_ptr)
        {
            ...
            MMI_WSTRNCPY((prompt_str.wstr_ptr + prompt_str.wstr_len), (size_t)blank_len, &blank_char, (size_t)blank_len, (size_t)blank_len);
...
LOCAL MMISRV_HANDLE_T s_dtmf_tone_handle=0;
LOCAL MMI_WIN_ID_T record_win_id=0;
PUBLIC void MMISRVAUD_StopTone_DTMF(void)
{
	SCI_TRACE_LOW("[MIXRING] MMISRVAUD_StopTone_DTMF");
	//MMK_StopDtmfPlayingTimer();
	if(0 < s_dtmf_tone_handle)
	{
		MMISRVAUD_Stop(s_dtmf_tone_handle);
		MMISRVMGR_Free(s_dtmf_tone_handle);
		s_dtmf_tone_handle=0;
	}
}
LOCAL BOOLEAN DTMFPlayCallback(MMISRV_HANDLE_T handle, 
MMISRVMGR_NOTIFY_PARAM_T *param)
{
	...
	MMIFILE_ERROR_E    fs_ret = SFS_ERROR_NONE;
	SCI_TRACE_LOW("[MIXRING] DTMFPlayCallback event:%d",param->event);
...
PUBLIC BOOLEAN DTMF_RequestHandle( 
...
PUBLIC void MMISRVAUD_PlayTone_DTMF(
			uint32 tone_id,
			uint16 play_times
                )
{
	...
	MMIRES_GetRingInfo(tone_id  + R_KEY_2,MMK_GetFirstAppletHandle(), &ring_info);
	...
	/* Create handle */
	DTMF_RequestHandle(&audio_handle, play_times,MMISRVAUD_ROUTE_AUTO_NO_BLUETOOTH, &audio_data,DTMFPlayCallback);

	if (audio_handle != 0)
	{
		s_dtmf_tone_handle=audio_handle;
	...
	{    
        ...
        /**
            ...
            **/
            ...
            MMISRVAUD_PlayTone_DTMF(0, 1);// play tone dtmf first
        }
        else
        {
            MMIAPIENG_AlertIQMode();
                                       ...
                                       )
{
    ...
    case MSG_FULL_PAINT:
        ...
        //MPTY and Anwer
        {
            BOOLEAN setup_mpty_and_answer_menu_gray = FALSE;
            if (!MMICC_IsMptyAndAnswerAvailable())
            {
                setup_mpty_and_answer_menu_gray = TRUE;
            }
            GUIMENU_SetItemGrayed(MMICC_MENUWIN_MENU_CTRL_ID,MENU_CC_RECEIVE_WC2, ID_CC_BUILDMPTY_ANSWER, setup_mpty_and_answer_menu_gray);
        }
    ...
    case MSG_APP_WEB:
	 ...
	 MMK_CloseWin(win_id);
        ...
        case CC_HOLDMENU_NODE_CL:
            ...
            {
                MMICC_OpenAlertWarningWin( STR_JAVA_BACKGROUND_RUNNING);
                MMK_CloseWin(win_id);
                break;
            }
#endif            
