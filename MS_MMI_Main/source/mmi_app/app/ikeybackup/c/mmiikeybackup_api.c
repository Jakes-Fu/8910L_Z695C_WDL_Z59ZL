...
{
	if(s_ikb_config.store_pos>MMIIKB_STORE_POS_MAX||s_ikb_config.store_pos==MMIIKB_STORE_POS_NONE){
		s_ikb_config.store_pos = MMIIKB_STORE_POS_SD_1;
	    //save config nv values.
	    MMIIKB_SetConfigNVInfo((MMIIKB_CONFIG_T *)&s_ikb_config);
	}
    return &s_ikb_config;
}
...
PUBLIC BOOLEAN MMIIKB_RecoverConfigInfo(MMIIKB_CONFIG_T *cfg_ptr)
{
    ...
    if (MMIIKB_STORE_POS_NONE == cfg_ptr->store_pos||cfg_ptr->store_pos>MMIIKB_STORE_POS_MAX)
    {
...
PUBLIC BOOLEAN MMIIKB_GetCurBaseDir(uint16 *path_ptr, uint16 *path_len_ptr)
{
    ...
    if (is_ret && is_find)
    {
		...
		MMIAPICOM_Wstrcat(path_ptr, L":\\Backups");
