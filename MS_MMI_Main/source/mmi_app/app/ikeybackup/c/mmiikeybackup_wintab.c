...
LOCAL MMIIKB_UI_SEL_ITEM_LIST_T s_ui_backup_list[] = 
{
    ...
    {MMIIKB_TYPE_PHONEBOOK, IMAGE_SECMENU_ICON_TOOL_CL,         STR_LIST_BACKUP_CONTACTS,       {0}, 0, 0, FALSE, FALSE},
    {MMIIKB_TYPE_SMS,       IMAGE_SECMENU_ICON_MESSAGE_INBOX,   STR_LIST_BACKUP_MESSAGES,                    {0}, 0, 0, FALSE, FALSE},
...
/*****************************************************************************/
//  Description : HandleBackupDataWinMsg
//  Global resource dependence :
//  Author: dong.chunguang
//  Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleBackupDataWinMsg(
    MMI_WIN_ID_T win_id,
    MMI_MESSAGE_ID_E msg_id,
    DPARAM param
);
/*****************************************************************************/
//  Description : HandleRestoreDataWinMsg
//  Global resource dependence :
//  Author: dong.chunguang
//  Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleRestoreDataWinMsg(
    MMI_WIN_ID_T win_id,
    MMI_MESSAGE_ID_E msg_id,
    DPARAM param
);
/*****************************************************************************/
//  Description : HandleErrorMsgWin
//  Global resource dependence : none
//  Author: dong.chunguang
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleErrorMsgWin(
                                               MMI_WIN_ID_T     win_id, 
                                               MMI_MESSAGE_ID_E msg_id,
                                               DPARAM           param
                                               );
/*****************************************************************************/
//  Description : Backup CheckListItem
//  Global resource dependence :
//  Author: dong.chunguang
//  Note:
/*****************************************************************************/
LOCAL void BackupCheckListItem(
    MMI_CTRL_ID_T ctrl_id,
    MMIIKB_UI_SEL_ITEM_LIST_T *ui_backup_list_ptr,
    uint16 list_num
);
...
/*****************************************************************************/
//  Description : HandleLoadRestoreDataWinMsg
//  Global resource dependence :
//  Author: dong.chunguang
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleLoadRestoreDataWinMsg(
    MMI_WIN_ID_T     win_id,
    MMI_MESSAGE_ID_E msg_id,
    DPARAM           param
);
...
WINDOW_TABLE(MMIIKB_ENCRYPT_DATA_WIN_TAB) = 
{
    ...
    WIN_SOFTKEY(STXT_SOFTKEY_SELECT_MK, TXT_NULL, TXT_COMPLETE),
    END_WIN
};

WINDOW_TABLE(MMIIKB_BACKUP_DATA_WIN_TAB) =
{
    WIN_FUNC((uint32)HandleBackupDataWinMsg),
    WIN_ID(MMIIKB_BACKUP_DATA_WIN_ID),
    CREATE_LISTBOX_CTRL(GUILIST_CHECKLIST_E, MMIIKB_BACKUP_CREATE_LIST_CTRL_ID),
    WIN_SOFTKEY(NULL, STXT_SOFTKEY_MARK_MK, TXT_COMMON_COMMON_BACK),
    END_WIN
};

WINDOW_TABLE(MMIIKB_RESTORE_DATA_WIN_TAB) =
{
    WIN_FUNC((uint32)HandleRestoreDataWinMsg),
    //WIN_ID(MMIIKB_RESTORE_DATA_WIN_ID),
    WIN_ID(MMIIKB_RESTORE_CHILD_WIN_ID),
    CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMIIKB_BACKUP_RESTORE_LIST_CTRL_ID),
    WIN_SOFTKEY(NULL, STXT_SOFTKEY_SELECT_MK, TXT_COMMON_COMMON_BACK),
    END_WIN
};
WINDOW_TABLE(MMIIKB_LOAD_RESTORE_DATA_WIN_TAB) =
{
    WIN_FUNC((uint32)HandleLoadRestoreDataWinMsg),
    WIN_ID(MMIIKB_WAIT_WIN_ID),
    CREATE_SPINNER_CTRL(MMIIKB_BACKUP_LOAD_SPINNER_CTRL_ID),
...
LOCAL void AddIKBTabsAndChildWindows(void)
{
    ...
    MMI_IMAGE_ID_T ikb_pda_tab[3][2] = {0};
...
)
{
	...
	MMI_WIN_ID_T win_id = MMIIKB_BANNER_WIN_ID;
    ...
    case MMIIKB_STATUS_SUCCESS:
		...
		if (MMIIKB_TASK_TYPE_BACKUP == task_type)
	    {
	        text_id2 = STR_NOTE_BACKUP_SUCCEED_EXT01;
	    }
	    else if (MMIIKB_TASK_TYPE_RESTORE == task_type)
	    {
	        text_id2 = STR_NOTE_RESTORE_SUCCEED_EXT01;
	    }
		break;
    case MMIIKB_STATUS_FAILED:
        text_id2 = STR_ERROR_BACKUP_EXT01;//TXT_ERROR;
        break;
    case MMIIKB_STATUS_SYSTEM_ERROR:
        text_id2 = STR_ERROR_BACKUP_EXT01;//TXT_ERROR;
        break;
    case MMIIKB_STATUS_NO_MEMORY:
        text_id2 = STR_BACKUP_NOTE_MEMORY_FULL_EXT02;
        break;
    case MMIIKB_STATUS_INVALID_PARAM:
        text_id2 = STR_ERROR_BACKUP_EXT01;//TXT_IKB_TIPS_INVALID_PARAM;
        break;
    case MMIIKB_STATUS_DISK_NO_SPACE:
        text_id2 = STR_BACKUP_NOTE_MEMORY_FULL_EXT02;//TXT_NO_SPACE;
        break;
    case MMIIKB_STATUS_NO_INIT_TASK:
        text_id2 = STR_ERROR_BACKUP_EXT01;//TXT_IKB_TIPS_NO_INIT_TASK;
        break;
    case MMIIKB_STATUS_WRONG_TASK:
        text_id2 = STR_ERROR_BACKUP_EXT01;//TXT_IKB_TIPS_WRONG_TASK;
        break;
    case MMIIKB_STATUS_WRONG_OP_TYPE:
        text_id2 = STR_ERROR_BACKUP_EXT01;//TXT_IKB_TIPS_WRONG_OP_TYPE;
        break;
    case MMIIKB_STATUS_CRC_ERROR:
        text_id2 = STR_ERROR_BACKUP_EXT01;//TXT_IKB_TIPS_WRONG_CRC;
        break;
    case MMIIKB_STATUS_FILE_ERROR:
        text_id2 = STR_ERROR_BACKUP_EXT01;//TXT_IKB_TIPS_FILE_OPERATE_ERROR;
        break;
    case MMIIKB_STATUS_SD_OPERATE_FAIL:
        text_id2 = STR_ERROR_BACKUP_EXT01;//TXT_IKB_TIPS_SD_OPERATE_FAIL;
        break;
    case MMIIKB_STATUS_SD_NOT_READY:
        text_id2 = STR_ERROR_BACKUP_EXT01;//TXT_COMMON_SD_FAIL;
        break;
    case MMIIKB_STATUS_SD_NOT_EXIST:
        text_id2 = STR_NOTE_NO_CARD;//TXT_PLEASE_INSERT_SD;
        break;
    case MMIIKB_STATUS_READ_PACK_ERROR:
        text_id2 = STR_ERROR_BACKUP_EXT01;//TXT_IKB_TIPS_READ_PACK_ERROR;
        break;
    case MMIIKB_STATUS_CANCEL_ACTION:
        text_id2 = TXT_CANCELED;
        break;
    case MMIIKB_STATUS_BACKUP_TIME_OUT:
    case MMIIKB_STATUS_RESTORE_TIME_OUT:
        text_id2 = STR_BACKUP_TIMEOUT;
        break;
    case MMIIKB_STATUS_PB_FULL:
        text_id2 = STR_ERROR_BACKUP_EXT01;//TXT_PB_NV_FULL;
        break;
    case MMIIKB_STATUS_ILLEGAL_VERSION:
        text_id2 = STR_ERROR_BACKUP_EXT01;//TXT_IKB_TIPS_ILLEGAL_VER;
        break;
    ...
    if (MMIIKB_STATUS_SD_NOT_EXIST == error_code)
    {
        ...
        //MMIPUB_OpenConfirmationDialogByTextId(PNULL,TXT_PLEASE_INSERT_SD, PNULL, IMAGE_PUBWIN_WARNING, PNULL,PNULL, MMIPUB_SOFTKEY_CUSTOMER,HandleErrorMsgWin);
		...
		MMIAPICOM_OpenBannerWin(MMIIKB_BANNER_WIN_ID, banner_info);

    }
    else
    {
		banner_info.banner_type = MMI_BANNER_TEXT;
		banner_info.text1_id = text_id2;
		banner_info.start_point = pin_alert_start_point;
		MMIAPICOM_OpenBannerWin(MMIIKB_BANNER_WIN_ID, banner_info);

    	//MMIPUB_OpenConfirmationDialogByTextId(PNULL,text_id1, text_id2, image_id, PNULL,PNULL, MMIPUB_SOFTKEY_CUSTOMER,HandleErrorMsgWin);
        //MMIPUB_OpenAlertWinByTextId(PNULL, text_id1, text_id2, image_id, 
        //    PNULL, PNULL, MMIPUB_SOFTKEY_CUSTOMER, HandleErrorMsgWin);
    }
    is_ret = TRUE;
	if(MMIIKB_TASK_TYPE_RESTORE == task_type&&MMIIKB_STATUS_SUCCESS==error_code){
		if(MMK_IsOpenWin(MMIIKB_RESTORE_CHILD_WIN_ID))
			delay_time_id = MMK_CreateWinTimer(MMIIKB_RESTORE_CHILD_WIN_ID, 200, FALSE);
		else if(MMIFMM_IsFmmMainWinOpen())
			delay_time_id = MMK_CreateWinTimer(MMIFMM_MAIN_WIN_ID, 1000, FALSE);
	}
    return is_ret;
}
...
#endif
/*****************************************************************************/
//  Description : modify GetOneListItemValue as ui requirement
//  Global resource dependence :
//  Author: dong.chunguang
//  Note:
/*****************************************************************************/
...
)
{
	...
	item_ptr->item_style    = GUIITEM_STYLE_MULTI_CHOICE_PICKER_TEXT;
    item_ptr->item_data_ptr = item_data_ptr;
    item_ptr->user_data = cur_list_ptr->user_data;

    item_data_ptr->item_content[0].item_data_type = GUIITEM_DATA_TEXT_ID;
    item_data_ptr->item_content[0].item_data.text_id = cur_list_ptr->text_id1;
}
/*****************************************************************************/
//  Description : AppendALLListItemCtrl
//  Global resource dependence :
...
)
{
    ...
    {
        //backup list dong.chunguang modify for new ui
        //if (MMIIKB_BACKUP_LIST_CTRL_ID == ctrl_id)
		if (MMIIKB_BACKUP_CREATE_LIST_CTRL_ID == ctrl_id)
        {
    ...
    {
    	//dong.chunguang modify for new ui
        //if (MMIIKB_BACKUP_LIST_CTRL_ID == ctrl_id)
		if (MMIIKB_BACKUP_CREATE_LIST_CTRL_ID == ctrl_id)
        {
            SCI_MEMSET(item_list_ptr->text2_buf, 0x0, 
...
)
{
    ...
    case MMIIKB_TYPE_BRO_LABEL:
        ...
        text_id = STR_ILIST_INTERNET_EXT01;
        break;
#endif
    ...
    if (MMIIKB_TASK_STATE_END == seq_task_item_ptr->task_state)
    {
	    ...
	    {
			SCI_MEMSET(&text_str, 0x00, sizeof(text_str));
			MMI_GetLabelTextByLang(STR_WAIT_CREATING_BACKUP, &text_str);
	        MMIPUB_SetProgressBarTextByPtr(win_id, &text_str, FALSE);
	    }
	    else if (MMIIKB_TASK_TYPE_RESTORE == seq_task_item_ptr->task_type)
	    {
			SCI_MEMSET(&text_str, 0x00, sizeof(text_str));
			MMI_GetLabelTextByLang(STR_NOTE_PROGRESS_RESTORE, &text_str);
	        MMIPUB_SetProgressBarTextByPtr(win_id, &text_str, FALSE);
	    }
    ...
    if(total_size>0)
    {
	    MMI_STRING_T    txt_str = {0};
		wchar blank_wchar[] = {0x20,0x00};

	    MMIPUB_SetProgressTotalSize(&win_id, total_size);
        txt_str.wstr_ptr = blank_wchar;
	    txt_str.wstr_len = MMIAPICOM_Wstrlen(txt_str.wstr_ptr);
		MMIPUB_SetProgressContentByTextPtr(&win_id,&txt_str,FALSE);
...
)
{
    ...
    case MSG_CLOSE_WINDOW:
		...
		CTRLLIST_SetAllSelected(MMIIKB_BACKUP_CREATE_LIST_CTRL_ID,FALSE);
        //MMIAPIIKB_SuspendTask();
        break;
...
)
{
	...
	case MSG_APP_RED:
        ...
        break;
	case MSG_CLOSE_WINDOW:
		if(MMK_IsOpenWin(MMIIKB_BACKUP_DATA_WIN_ID)){
			MMK_CloseWin(MMIIKB_BACKUP_DATA_WIN_ID);
		}
		break;
    default:
        result = MMIPUB_HandleProgressWinMsg(win_id, msg_id, param);
...
LOCAL MMIIKB_TYPE_T OptimizeBackupSelTypes(MMIIKB_TYPE_T types)
{
    ...
    for (i = 0; i < ARR_SIZE(s_ui_backup_list); i++)
    {
        ...
        {
            ret_types &= ~(0x01<<i/*cur_list_ptr->user_data*/);/*lint !e502*/
        }
    }
...
)
{
    ...
    if (0 == types)
    {
		...
		MMIPUB_OpenConfirmationDialogByTextId(PNULL,STR_BACKUP_NO_CONTENT, PNULL, IMAGE_PUBWIN_WARNING, PNULL,PNULL, MMIPUB_SOFTKEY_DIALOG_OK,HandleErrorMsgWin);            
    }
    else if (0 != (types & (0x01 << MMIIKB_TYPE_BRO_LABEL)))
    {
        //暂不支持浏览器书签
        //MMIPUB_OpenAlertWarningWin(TXT_IKB_NOT_SUPPORT_LABLE);
		//MMIPUB_OpenConfirmationDialogByTextId(PNULL,TXT_IKB_NOT_SUPPORT_LABLE, PNULL, IMAGE_PUBWIN_WARNING, PNULL,PNULL, MMIPUB_SOFTKEY_DIALOG_OK,HandleErrorMsgWin);            
    }
    else
    {
        //过滤掉０条的栏目
        *opt_types_ptr = OptimizeBackupSelTypes(types);
        ...
        if(MMIAPIIKB_IsBusy())
        {
			...
			MMIPUB_OpenConfirmationDialogByTextId(PNULL,STR_BACKUP_ONGOING, PNULL, IMAGE_PUBWIN_WARNING, PNULL,PNULL, MMIPUB_SOFTKEY_DIALOG_OK,HandleErrorMsgWin);            
            return is_ret;
        }
...
LOCAL void BackupSelItems(MMIIKB_PRE_BACKUP_DATA_T *pre_backup_data_ptr)
{
    ...
    if (MMIIKB_STATUS_SUCCESS == error_code)
    {
        ...
        MMIPUB_OpenProgressWinByTextId(STR_WAIT_CREATING_BACKUP, 
        ...
        txt_str.wstr_ptr = blank_wchar;
	    txt_str.wstr_len = MMIAPICOM_Wstrlen(txt_str.wstr_ptr);
		MMIPUB_SetProgressContentByTextPtr(&progress_win_id,&txt_str,FALSE);
    }
    else
    {
        ShowTips(MMIIKB_TASK_TYPE_BACKUP, error_code);
...
)
{
    ...
    //1: create pack dir
    if(pre_backup_data_ptr->types ==1)
	{
		sprintf(c_pack_dir_name, "Contacts-backup_%02d-%02d-%04d_%02d-%02d-%02d",
		cur_date.mday, cur_date.mon, cur_date.year,
		cur_time.hour, cur_time.min, cur_time.sec);
	}
	else if(pre_backup_data_ptr->types == 2){
		sprintf(c_pack_dir_name, "Text-messages-backup_%02d-%02d-%04d_%02d-%02d-%02d",
		cur_date.mday, cur_date.mon, cur_date.year,
		cur_time.hour, cur_time.min, cur_time.sec);
	}else{
		sprintf(c_pack_dir_name, "All-backup_%02d-%02d-%04d_%02d-%02d-%02d",
		cur_date.mday, cur_date.mon, cur_date.year,
		cur_time.hour, cur_time.min, cur_time.sec);
	}
    ...
    if (is_find_dir)
    {
        if (!MMIAPIFMM_IsFolderExist(backup_req.pack_dir, path_len))
        {
			...
			if(is_ret == FALSE){//if create fail return
            	error_code = MMIIKB_STATUS_SD_OPERATE_FAIL;
				return error_code;
			}
        }
        ...
        //dong.chunguang modify for don't hide
...
LOCAL void EndBackupItems(MMIIKB_SIG_BACKUP_RES_T *sig_backup_res_ptr)
{
    ...
    //针对结果的提示
	if(MMK_IsOpenWin(MMIIKB_RESTORE_CHILD_WIN_ID))
    	MMK_PostMsg(MMIIKB_RESTORE_CHILD_WIN_ID, MSG_MAINMENU_SHOW_BANNER, &error_code, sizeof(MMIIKB_ERROR_E));
    else
		ShowTips(MMIIKB_TASK_TYPE_BACKUP, error_code);
}
...
)
{
    ...
    case MSG_APP_FLIP:
        {
            //打开一个询问窗口，是否确认取消
            //dong.chunguang mark for can not cancel as ms spec
...
LOCAL BOOLEAN PreCheckRestoreItems(uint16 item_index, MMIIKB_TYPE_T types)
{
    ...
    else
    {
        if(MMIAPIIKB_IsBusy())
        {
            MMIPUB_OpenAlertWarningWin(STR_BACKUP_ONGOING);
...
LOCAL void RestoreSelItems(uint16 item_index, MMIIKB_TYPE_T types)
{
    ...
    if (MMIIKB_STATUS_SUCCESS == error_code)
    {
        ...
        MMIPUB_OpenProgressWinByTextId(STR_NOTE_PROGRESS_RESTORE, 
            &progress_win_id, PNULL, MMIPUB_SOFTKEY_NONE, 
...
)
{
    ...
    if (0 < total_num)
    {
        ...
        //set item
        item.item_style = GUIITEM_STYLE_ONE_LINE_TEXT_MS;
...
#endif
/*****************************************************************************/
//  Description : Append restore ListItem
//  Global resource dependence :
//  Author: dong.chunguang
//  Note:
/*****************************************************************************/
...
)
{
    ...
    if (MMIIKB_GetPackFileInfo(item_index, &file_info))
    {
            ...
            pack_buf_ptr))
        {
			...
			GUILIST_SetItemStyle(ctrl_id,GUIITEM_STYLE_ONE_LINE_TEXT_MS,item_index);
            if(MMIAPICOM_Wstrcmp(pack_buf_ptr->mod_name, MMIIKB_MODU_NAME))
            {
                return result;
            }
            ...
            item_data.item_content[0].item_data.text_buffer.wstr_ptr = 
                pack_buf_ptr->pack_name;
            item_data.item_content[0].item_data.text_buffer.wstr_len = pack_name_len;
...
)
{
    ...
    {    
    case MSG_OPEN_WINDOW:
        if(0==GUILIST_GetTotalItemNum(MMIIKB_BACKUP_RESTORE_LIST_CTRL_ID))
        {
...
LOCAL BOOLEAN IsVersionLegal(MMIIKB_PACK_INFO_T * pack_info_ptr)
{
	...
	if(strnicmp(ver1_10, s_str_ikb_version,MMIIKB_VERSION_NUM_LEN)>= 0)
    {
        is_legal = TRUE;
    }
    else
...
LOCAL void AbortCurAction(MMIIKB_TASK_TYPE_E task_type)
{
	...
	/*
    MMK_SetWinUserData(win_cancel_id,(uint32)task_type);
    MMIPUB_OpenConfirmationDialogByTextId(PNULL, TXT_IKB_ABORT_QUERY, TXT_NULL,
                                          IMAGE_NULL, &win_cancel_id, &win_priority_dialog, MMIPUB_SOFTKEY_DIALOG_OK_CLOSE,
                                          HandleCancelActionQueryMsg);
	*/
    return;
}
/*****************************************************************************/
//  Description : FormatListTotalValue
//  Global resource dependence :
//  Author: dong.chunguang
//  Note:
/*****************************************************************************/
LOCAL void FormatListTotalValue(
    MMI_CTRL_ID_T ctrl_id,
    MMIIKB_UI_SEL_ITEM_LIST_T *item_list_ptr
)
{
...
//  Note:
/*****************************************************************************/
LOCAL void GetCheckListItemValue(
    const MMIIKB_UI_SEL_ITEM_LIST_T *cur_list_ptr,
    GUILIST_ITEM_T *item_ptr,
    GUILIST_ITEM_DATA_T *item_data_ptr
)
...
/*****************************************************************************/
//  Description : backup confirm
//  Global resource dependence : none
//  Author: dong.chunguang
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleBackupConfirmWin(
                                               MMI_WIN_ID_T     win_id, 
                                               MMI_MESSAGE_ID_E msg_id,
                                               DPARAM           param
                                               )
{
    ...
    case MSG_APP_WEB:
	    ...
	    if (MMIAPIFMM_GetDeviceFreeSpace(
	        ...
	        MMIAPIFMM_GetDevicePathLen(MMI_DEVICE_SDCARD),
	        &free_high_word,
	        &free_low_word)){//low memory
			if ((0 < free_high_word) || (5*MMIIKB_SPACE_LIMIT_VALUE < free_low_word))
		    {
		    }else{
				if(MMK_IsOpenWin(MMIIKB_BACKUP_DATA_WIN_ID)){
					MMK_CloseWin(MMIIKB_BACKUP_DATA_WIN_ID);
				}
				ShowTips(MMIIKB_TASK_TYPE_BACKUP, MMIIKB_STATUS_NO_MEMORY);
				break;
			}
		}
		BackupCheckListItem(MMIIKB_BACKUP_CREATE_LIST_CTRL_ID, s_ui_backup_list,
            ARR_SIZE(s_ui_backup_list));
		OnClickBackupSoftkey();
        break;
...
//  Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleRestoreConfirmWin(
                                               MMI_WIN_ID_T     win_id, 
                                               MMI_MESSAGE_ID_E msg_id,
                                               DPARAM           param
                                               )
{
    MMI_RESULT_E        result = MMI_RESULT_TRUE;
    MMI_CTRL_ID_T 		list_ctrl_id = MMIIKB_BACKUP_RESTORE_LIST_CTRL_ID;
    uint16 				item_index = 0;
	...
	case MSG_APP_OK:
		break;
    case MSG_APP_WEB:
		MMK_CloseWin(win_id);//close win first
    	item_index = GUILIST_GetCurItemIndex(list_ctrl_id);
		InitRestoreSelItemList(item_index, s_ui_restore_list, 
            ARR_SIZE(s_ui_restore_list));
...
)
{
    ...
    //index = GUILIST_GetCurItemIndex(ctrl_id);
	for(index = 0; index<ARR_SIZE(s_ui_backup_list);index++)
	{
    ...
    cur_list_ptr = ui_backup_list_ptr + index;
    if (!cur_list_ptr->is_gray)
    {
        is_cur_line_state = cur_list_ptr->is_check;

		        cur_list_ptr->is_check = !is_cur_line_state;
		        FormatListTotalValue(ctrl_id, cur_list_ptr);
		        //FormatTotalValue(ctrl_id, cur_list_ptr);
                                         ...
                                         )
{
    ...
    MMI_TEXT_ID_T       str_id[] = {STR_LIST_BACKUP_CONTACTS,STR_LIST_BACKUP_MESSAGES};
    ...
    uint16  max_items = 2;
    uint16   cur_item = 0;
    MMI_CTRL_ID_T               ctrl_id = MMIIKB_BACKUP_CREATE_LIST_CTRL_ID;
	GUILIST_ITEM_T const *item_ptr = NULL;
	//CTRLLIST_ITEM_INFO_T *cur_item_info_ptr = PNULL;
	uint16   select_items = 0;
	BOOLEAN  is_has_check = FALSE;
	uint16              sel_items[2] = {0};
	uint16              sel_count = 0;
    switch(msg_id)
    {
    case MSG_OPEN_WINDOW: 
        ...
        //item_t.item_data_ptr = &item_data;
        InitListUIValue(s_ui_backup_list, ARR_SIZE(s_ui_backup_list));
        GUILIST_SetMaxItem(ctrl_id, ARR_SIZE(s_ui_backup_list), FALSE);
        GUILIST_SetMaxSelectedItem(ctrl_id,ARR_SIZE(s_ui_backup_list));
        AppendALLListItemCtrl(ctrl_id, s_ui_backup_list, 
            ARR_SIZE(s_ui_backup_list));
		/*
	...
	case MSG_FULL_PAINT:
		{//reset status bar
			...
			MMI_GetLabelTextByLang(STR_SETT_LIST_STORAGE_EXT01, &temp_str);
		...
		item_ptr = GUILIST_GetItemPtrByIndex(ctrl_id,cur_item);
		//cur_item_info_ptr = LIST_GetItemInfoPtr(item_ptr,cur_item);

		if(CTRLLIST_GetSelectedItemNum(ctrl_id)>0){
			GUIWIN_SeSoftkeytButtonTextId(win_id,STXT_SOFTKEY_MARK_MK,RIGHT_BUTTON,TRUE);
		}else{
			GUIWIN_SeSoftkeytButtonTextId(win_id,TXT_COMMON_COMMON_BACK,RIGHT_BUTTON,TRUE);
		}
		if(GUILIST_GetItemState(item_ptr, GUIITEM_STATE_SELECTED))
    ...
    case MSG_CTL_MIDSK:
		/*
		sel_count = GUILIST_GetSelectedItemIndex(ctrl_id,sel_items, 2);
        cur_item = GUILIST_GetCurItemIndex(ctrl_id);
		item_ptr = GUILIST_GetItemPtrByIndex(ctrl_id,cur_item);
		if(GUILIST_GetItemState(item_ptr,GUIITEM_STATE_SELECTED))
        {
			GUILIST_SetSelectedItem(ctrl_id, cur_item, FALSE);
        }else{
			GUILIST_SetSelectedItem(ctrl_id, cur_item, TRUE);
        }
        */
		/*
        if(sel_items[cur_item]==0){
			GUILIST_SetSelectedItem(ctrl_id, cur_item, TRUE);
		}else{
			GUILIST_SetSelectedItem(ctrl_id, cur_item, FALSE);
		}
		*/
		//BackupCheckListItem(ctrl_id, s_ui_backup_list,
        //   ARR_SIZE(s_ui_backup_list));
        //OnClickCheckListItem(ctrl_id, s_ui_backup_list,
        //    ARR_SIZE(s_ui_backup_list));
		//BackupCheckListItem(ctrl_id, s_ui_backup_list,
        //    ARR_SIZE(s_ui_backup_list));
        MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);
    ...
    case MSG_APP_CANCEL:
        ...
        {
			MMI_WIN_PRIORITY_E      win_priority_dialog = WIN_ONE_LEVEL;
			MMI_WIN_ID_T     win_id_confirm = MMIIKB_BACKUP_CONFIRM_WIN_ID;
			MMIPUB_OpenConfirmationDialogByTextId(PNULL,STR_NOTE_BACKUP_EXT01, PNULL, IMAGE_NULL, &win_id_confirm,&win_priority_dialog, MMIPUB_SOFTKEY_DIALOG_OK,HandleBackupConfirmWin);
...
/*****************************************************************************/
//  Description : enter backup
//  Global resource dependence :
//  Author: dong.chunguang
//  Note:
/*****************************************************************************/
PUBLIC void MMIIKB_EnterBackupWin(void)
{
    MMK_CreateWin((uint32 *)MMIIKB_BACKUP_DATA_WIN_TAB, PNULL);
}
/*****************************************************************************/
//  Description : HandleRestoreDataWinMsg
//  Global resource dependence :
...
)
{
    ...
    case MSG_IKB_LOAD_PACK_DATA_REQ:
        {
            ...
            //clear pack list
            ClearPackList(win_id, list_ctrl_id);
			MMK_CreateWin((uint32 *)MMIIKB_LOAD_RESTORE_DATA_WIN_TAB, PNULL);
            //MMIPUB_OpenAlertWinByTextId(&time_out, TXT_COMMON_WAITING, TXT_NULL,
            //    IMAGE_PUBWIN_WAIT, &wait_win_id, PNULL, MMIPUB_SOFTKEY_CUSTOMER,
            //    HandleLoadPackWinMsg);
        }
	...
	case MSG_MAINMENU_SHOW_BANNER:
		{
	        MMIIKB_ERROR_E error_code = *((MMIIKB_ERROR_E*)param);
			ShowTips(MMIIKB_TASK_TYPE_BACKUP, error_code);
		}
		break;
    case MSG_CTL_OK:
    ...
    case MSG_CTL_PENOK:
	    ...
	    if(0!=GUILIST_GetTotalItemNum(list_ctrl_id))
	    {
			...
			MMIPUB_OpenConfirmationDialogByTextId(PNULL,STR_NOTE_RESTORE_EXT01, PNULL, IMAGE_NULL, &win_id_confirm,&win_priority_dialog, MMIPUB_SOFTKEY_DIALOG_OK,HandleRestoreConfirmWin);
	    }
        break;
    case MSG_APP_CANCEL:
    case MSG_CTL_CANCEL:
        //MMK_CloseParentWin(MMIIKB_MAIN_WIN_ID);
        if(delay_time_id==0)//don't close when restore
		MMK_CloseWin(win_id);
        break;
	case MSG_TIMER:
...
/*****************************************************************************/
//  Description : enter restore
//  Global resource dependence :
//  Author: dong.chunguang
//  Note:
/*****************************************************************************/
PUBLIC void MMIIKB_EnterRestoreWin(void)
{
    MMK_CreateWin((uint32 *)MMIIKB_RESTORE_DATA_WIN_TAB, PNULL);
}
...
//  Note:
/*****************************************************************************/
LOCAL MMIIKB_ERROR_E StartRestoreItemsByFile(FILEARRAY_DATA_T *file_info,MMIIKB_TYPE_T types)
{
    MMIIKB_ERROR_E error_code = MMIIKB_STATUS_INVALID_PARAM;
    MMIIKB_RESTORE_REQ_T restore_req = {0};
    MMIIKB_PACK_INFO_T *pack_info_ptr = PNULL;
...
/*****************************************************************************/
//  Description : MMIIKB_RestoreItemsByFile
//  Global resource dependence :
//  Author: dong.chunguang
//  Note:
/*****************************************************************************/
PUBLIC void MMIIKB_RestoreItemsByFile(FILEARRAY_DATA_T *file_info)
{
    MMIIKB_ERROR_E error_code = MMIIKB_STATUS_INVALID_PARAM;
    MMI_WIN_ID_T progress_win_id = MMIIKB_RESTORE_PRG_WAIT_WIN_ID;
    uint32 total = 0;
	...
	wchar blank_wchar[] = {0x20,0x00};
	MMIIKB_GET_TOTAL_OUT_T param_out = {0};
	MMIIKB_TYPE_E types = MMIIKB_TYPE_MAX;

    error_code = IKeyBackup_GetTotalSizeByType(types, &param_out);
	total = param_out.total;
    //开始还原
    error_code = StartRestoreItemsByFile(file_info,types);
...
{
    MMI_RESULT_E          result  = MMI_RESULT_TRUE;
    static uint8        s_wait_load_timer_id = 0;
    ...
    spinner_ctr_id = MMK_GetCtrlHandleByWin(win_id, MMIIKB_BACKUP_LOAD_SPINNER_CTRL_ID);
    switch (msg_id)
	{
	case MSG_OPEN_WINDOW:
	case MSG_GET_FOCUS:
		if (s_wait_load_timer_id != 0)
		{
			MMK_StopTimer(s_wait_load_timer_id);
			s_wait_load_timer_id = 0;
		}
		s_wait_load_timer_id = MMK_CreateWinTimer(win_id,60000,TRUE);
		//set control info
		...
		StartLoadPack(win_id, msg_id, param);
		break;

	case MSG_IKB_GET_ALL_PACK_DATA_CNF:
		if(0 != s_wait_load_timer_id)
		{
			MMK_StopTimer(s_wait_load_timer_id);
			s_wait_load_timer_id = 0;
		}
