...
#define   MMIPB_TASK_QUEUE_NUM          20    //pb task queue num
#else
                        ...
                        )
{
    ...
    while (1)/*lint !e716*/
    {
        MMI_RECEIVE_SIGNAL((xSignalHeaderRec*)psig, g_mmipb_pb_task_id);/*lint !e63*/
        ...
        {
        case MMIPB_SIG_RESORT:
            SCI_TRACE_LOW("[MMIPB] MMIPB_SIG_RESORT");
            MMIPB_ReleasePhoneContactListCache();
            //排序
            MMIPB_InitEntryList();
            //排序结束，通知MMI
            if(pb_signal != PNULL)
            ...
            break;
        case MMIPB_SIG_SORT:
            SCI_TRACE_LOW("[MMIPB] MMIPB_SIG_SORT");
            //排序
            MMIPB_InitEntryList();
            //排序结束，通知MMI
            if(pb_signal != PNULL)
        ...
        case MMIPB_SIG_DELETE:
            ...
            break;
#ifdef MMIPB_SYNC_WITH_O365
		case MMIPBSYNC_SIG_LOGIN_CNF:
			SCI_TRACE_LOW("[MMIPBSYNC] MMIPBSYNC_SIG_LOGIN_CNF");
            MMIPB_SYNC_Login();
			break;
        case MMIPB_SYNC_START:
			SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_SYNC_START");
            MMIPB_SYNC_Start();
            break;
		case MMIPB_SYNC_ALL_O365_CONTACTS_TO_PHONE:
			SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_SYNC_ALL_O365_CONTACTS_TO_PHONE");
			MMIPB_SYNC_GetAllContacts();	
			break;
        case MMIPB_SYNC_TOTAL_CONTACTS_COUNT:
			SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_SYNC_TOTAL_CONTACTS_COUNT");
            MMIPB_SYNC_GetTotalContactsCount();
            break;
		case MMIPB_SYNC_ALL_ADDED_CONTACTS_COUNT:
			SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_SYNC_ALL_ADDED_CONTACTS_COUNT");
			MMIPB_SYNC_GetAllAddedContactsCount();
			break;
		case MMIPB_SYNC_ALL_UPDATED_CONTACTS_COUNT:
			SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_SYNC_ALL_UPDATED_CONTACTS_COUNT");
			MMIPB_SYNC_GetAllUpdatedContactsCount();
			break;
		case MMIPB_SYNC_CHECK_IF_HEAVY_SYNC_POSSIBLE:
			SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_SYNC_CHECK_IF_HEAVY_SYNC_POSSIBLE");
			MMIPB_SYNC_CheckIfHeavySyncPossible();
			break;
        case MMIPB_SYNC_ALL_O365_ADDED_TO_PHONE:
			SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_SYNC_ALL_O365_ADDED_TO_PHONE");
            MMIPB_SYNC_GetAllAddedContacts();
            break;
        case MMIPB_SYNC_ALL_O365_UPDATED_TO_PHONE:
			SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_SYNC_ALL_O365_UPDATED_TO_PHONE");
            MMIPB_SYNC_GetAllUpdatedContacts();
            break;
        case MMIPB_SYNC_ALL_ID_AND_MODIFIED_TIME:
			SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_SYNC_ALL_ID_AND_MODIFIED_TIME");
            MMIPB_SYNC_GetAllIdAndModiTime();
            break;
        case MMIPB_SYNC_ALL_O365_DELETED_TO_PHONE:
			SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_SYNC_ALL_O365_DELETED_TO_PHONE");
            MMIPB_SYNC_UpdateO365DeletedContactsToPB();
            break;
        case MMIPB_SYNC_PHONE_LIST_ITEM_TO_O365:
			SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_SYNC_PHONE_LIST_ITEM_TO_O365");
            MMIPB_SYNC_UpdatePBChangeToO365();
            break;
        case MMIPB_SYNC_COMPLETE:
			SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_SYNC_COMPLETE");
            MMIPB_SYNC_Complete();
            break;
		case MMIPBSYNC_SIG_LOGOUT_CNF:
			SCI_TRACE_LOW("[MMIPBSYNC] MMIPBSYNC_SIG_LOGOUT_CNF");
			MMIPB_SYNC_Logout();
			break;
#endif
#ifdef  SNS_SUPPORT            
        ...
        case MMIPB_SIG_LOAD_SIM_CONTACT:
           MMIPB_GenerateSimEntryList();
            ...
            cnf_msg.ret = MMIPB_ERROR_SUCCESS;
            MMIPB_SendSignalToMMITask(MMI_PB_LOAD_SIM_CONTACT_CNF, &cnf_msg);
            break;
        case MMIPB_SIG_RELEASE_SIM_CONTACT:
           MMIPB_FreeOnlySimcontactList();
            //排序结束，通知MMI
            if(pb_signal != PNULL)
            {
                cnf_msg.handle = pb_signal->handle;
            ...
            cnf_msg.ret = MMIPB_ERROR_SUCCESS;
            MMIPB_SendSignalToMMITask(MMI_PB_RELEASE_SIM_CONTACT_CNF, &cnf_msg);
            break;
