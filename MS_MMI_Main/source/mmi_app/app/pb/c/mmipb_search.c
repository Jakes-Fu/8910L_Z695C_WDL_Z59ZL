...
#endif
LOCAL uint8 s_cli_number = MIN_SEARCH_PHONE_NUMBER_COMPARE_LEN;
...
//Return: 
/*****************************************************************************/
LOCAL void RemoveZeroAndAreaCode2
(
const MMIPB_BCD_NUMBER_T  *sour_num_ptr, 
MMIPB_BCD_NUMBER_T *dest_num_ptr, 
uint32 *area_bcd 
);
                                     ...
                                     BOOLEAN is_protect)
{
    ...
    MMIPB_NAME_T            *pb_surname_ptr = PNULL;
    ...
    if (result)
    {
        ...
        else            
        {
            ...
            pb_surname_ptr = &base_string.surname;
#endif			
            if(PNULL != group_ptr)
            {
    ...
    if(!MMIAPIPB_IsSortBySurname())
    {
		if(pb_name_ptr != PNULL && (pb_name_ptr->wstr_len > 0))
		{
			...
			name_ptr->wstr_len  = MIN(pb_name_ptr->wstr_len, max_name_len);
			MMIAPICOM_Wstrncpy(name_ptr->wstr_ptr, pb_name_ptr->wstr, name_ptr->wstr_len);
			if(pb_surname_ptr != PNULL && (pb_surname_ptr->wstr_len > 0))
			{
				remain_len = max_name_len - name_ptr->wstr_len;
				if(remain_len > 2)
				{
					MMIAPICOM_Wstrncat(name_ptr->wstr_ptr, L" ", 1);
					remain_len--;
					MMIAPICOM_Wstrncat(name_ptr->wstr_ptr, pb_surname_ptr->wstr, MIN(pb_surname_ptr->wstr_len, remain_len));
					name_ptr->wstr_len  = MMIAPICOM_Wstrlen(name_ptr->wstr_ptr);
				}
			}
		...
		else
		{
			if(pb_surname_ptr != PNULL && (pb_surname_ptr->wstr_len > 0))
			{
				name_ptr->wstr_len  = MIN(pb_surname_ptr->wstr_len, max_name_len);
				MMIAPICOM_Wstrncpy(name_ptr->wstr_ptr, pb_surname_ptr->wstr, name_ptr->wstr_len);
			}
			else
			{
				name_ptr->wstr_len = 0;
		...
		if(pb_surname_ptr != PNULL && (pb_surname_ptr->wstr_len > 0))
		{
			...
			if(pb_name_ptr != PNULL && (pb_name_ptr->wstr_len > 0))
			{
				...
				if(remain_len > 2)
				{
					MMIAPICOM_Wstrncat(name_ptr->wstr_ptr, L" ", 1);
					remain_len--;
					MMIAPICOM_Wstrncat(name_ptr->wstr_ptr, pb_name_ptr->wstr, MIN(pb_name_ptr->wstr_len, remain_len));
					name_ptr->wstr_len  = MMIAPICOM_Wstrlen(name_ptr->wstr_ptr);
				}
			}
...
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMIPB_GetNamePreferredSimByNumber(MMIPB_BCD_NUMBER_T    *bcd_number,
                                     MMI_STRING_T *name_ptr,
                                     uint16 max_name_len,
                                     uint16* entry_id_ptr,
                                     uint16* storage_id_ptr,
                                     ...
                                     BOOLEAN is_protect)
{
    ...
    if (result)
    {
         ...
         
        {
            ...
            if(PNULL != preferred_sim)
            {
                *preferred_sim = MMIPB_PREFERRED_SIM_NONE;
            }
        }
        else            
        {
            ...
            if(PNULL != preferred_sim)
            {
                *preferred_sim = base_string.preferred_sim;
            }
    ...
    if(PNULL != storage_id_ptr)
    {
     ...
     SCI_TRACE_LOW("[mmipb] MMIPB_GetNamePreferredSimByNumber result(%d) name_len(%d) stoage_id(%d) entry_id(%d)",result,name_ptr->wstr_len, storage_id, entry_id);
    if(contact_ptr != PNULL)
    {
        SCI_FREE(contact_ptr);
    }
...
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMIPB_GetNameSurnamePreferredSimByNumber(MMIPB_BCD_NUMBER_T    *bcd_number,
                                     MMI_STRING_T *name_ptr,
                                     ...
                                     BOOLEAN is_protect)
{
    ...
    if(PNULL == bcd_number || PNULL == name_ptr || PNULL == name_ptr->wstr_ptr || PNULL == surname_ptr || PNULL == surname_ptr->wstr_ptr)
    {
        //SCI_TRACE_LOW:"[MMIPB]MMIPB_GetNameByNumber bcd_number %d name_ptr %d"
        SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMIPB_SEARCH_412_112_2_18_2_44_0_275,(uint8*)"dd", bcd_number, name_ptr);
        return FALSE;
    ...
    if(pb_surname_ptr != PNULL && (pb_surname_ptr->wstr_len > 0))
    {
        surname_ptr->wstr_len  = MIN(pb_surname_ptr->wstr_len, max_name_len);
        MMIAPICOM_Wstrncpy(surname_ptr->wstr_ptr, pb_surname_ptr->wstr, surname_ptr->wstr_len);
    }
    else
    {
        surname_ptr->wstr_len = 0;
        SCI_MEMSET(surname_ptr->wstr_ptr, 0x00, max_name_len);
    }

	if(PNULL != abbr_ptr && PNULL != abbr_ptr->wstr_ptr)
	{
		wchar abbr[3] = {0};
		uint8 pos = 0;
		...
		if(!MMIAPIPB_IsSortBySurname())
		{
			if(name_ptr->wstr_len != 0) //MMIAPIIM_IsPunct
			{
				abbr[pos] = name_ptr->wstr_ptr[0];
				pos++;
				if(surname_ptr->wstr_len != 0)
				{
					abbr[pos] =surname_ptr->wstr_ptr[0];
					pos++;
				}
			}	
			else
		...
		{
			if(surname_ptr->wstr_len != 0) //MMIAPIIM_IsPunct
			{
				abbr[pos] = surname_ptr->wstr_ptr[0];
				pos++;
		...
		if(0 != pos)
		{
			abbr_ptr->wstr_len  = MIN(MMIAPICOM_Wstrlen(abbr), max_name_len);
			MMIAPICOM_Wstrncpy(abbr_ptr->wstr_ptr, abbr, abbr_ptr->wstr_len);
		}
	}

    if (result && name_ptr->wstr_len == 0 && 0 == surname_ptr->wstr_len && is_use_default_name) 
    {
        MMI_GetLabelTextByLang(
            TXT_NO_NAME, 
            &noname_str
    ...
    if(PNULL != storage_id_ptr)
    {
     ...
     SCI_TRACE_LOW("[mmipb] MMIPB_GetNameSurnamePreferredSimByNumber result(%d) name_len(%d) stoage_id(%d) entry_id(%d)",result,name_ptr->wstr_len, storage_id, entry_id);
    if(contact_ptr != PNULL)
    {
        SCI_FREE(contact_ptr);
    }
...
)
{
	...
	/*20151212 wang.qinggang modify for 505015*/
	//RemoveZeroAndAreaCode(bcd_num_ptr, &new_number);
	RemoveZeroAndAreaCode2(bcd_num_ptr, &new_number, &(phone_number_ptr->area_bcd));
                        ...
                        )
{
    ...
    *next_pos_ptr = pos;
    if(is_find)
    {
        if(item_ptr->original_len < s_cli_number/*MIN_SEARCH_PHONE_NUMBER_COMPARE_LEN*/
          ||search_list_ptr->item_ptr[match_pos].original_len < s_cli_number/*MIN_SEARCH_PHONE_NUMBER_COMPARE_LEN*/
          )//两个匹配的号码中其中有个号码长度 < 6
        {
            //要求两个号码完全匹配
    ...
    /*20151212 wang.qinggang add for 505015*/
    if(is_find)
    {
        if(item_ptr->number_len < s_cli_number
          ||search_list_ptr->item_ptr[match_pos].number_len < s_cli_number
          )
        {
            //比较国家码
            if((item_ptr->number_len == search_list_ptr->item_ptr[match_pos].number_len)
            &&(item_ptr->phone_number.area_bcd  != search_list_ptr->item_ptr[match_pos].phone_number.area_bcd)
            )
            {
                //两个号码不是完全匹配
                is_find = FALSE;
...
//#endif
    ...
    //then search in sdn
    //if(0 < s_pb_number_list.list_inof.count)//wang.qinggang modify for sdn
    {
        for(dual_sys = MN_DUAL_SYS_1; dual_sys < MMI_DUAL_SYS_MAX && (!recode); dual_sys++)
        {
                                        ...
                                        )
{
    ...
    //bug714039,bug714021,将列表中无用的数据清空。
    for(j=cur_index; j<search_list_ptr->list_inof.count; j++)
    {
        SCI_MEMSET(&search_list_ptr->item_ptr[j], 0x00, sizeof(MMIPB_SEARCH_ITEM_T));
        j++;
    }
...
PUBLIC BOOLEAN MMIPB_IsExactMatchNumber(SEARCH_PHONE_NUMBER_T phone_number, uint16 num_len)
{
	...
	#if 0 //20151003 wang.qinggang for 481559
...
PUBLIC BOOLEAN MMIPB_IsNumberPartialMatch(MMIPB_SEARCH_ITEM_T* base_item_ptr, MMIPB_SEARCH_ITEM_T* table_item_ptr)
{
	...
	if(!MMIPB_IsExactMatchNumber(base_item_ptr->phone_number, base_item_ptr->number_len))
	{
        ...
        if(compare_num_len >= s_cli_number/*MIN_SEARCH_PHONE_NUMBER_COMPARE_LEN*/)
        {
            //compare_num_len的最小长度不能小于6
            compare_num_len = MIN(s_cli_number/*MIN_SEARCH_PHONE_NUMBER_COMPARE_LEN*/, compare_num_len);
                               ...
                               )
{
     ...
     else
     {
         ...
         //MMIAPIPB_PrintDebugMsTime("[mmipb_statistics] MMIPB_SearchList  MMIPB_SearchSubStringInList before");
         //count = MMIPB_SearchSubStringInList(wchar_lower.wstr_ptr,search_type,is_char, MMIPB_ALL_GROUP_MASK, 0);         
         count = MMIPB_SearchSubStringInList2(wchar_lower.wstr_ptr,search_type,is_char, 100);         
         //MMIAPIPB_PrintDebugMsTime("[mmipb_statistics] MMIPB_SearchList  MMIPB_SearchSubStringInList after");
         if(wchar_lower.wstr_ptr != PNULL)
         {
             SCI_FREE(wchar_lower.wstr_ptr);
         }
     ...
     SCI_TRACE_LOW("[mmipb_statistics] MMIPB_SearchList count=%d",count);
     //遍历所有符合条件的记录，并把它们的索引保存到表中
     if(count > 0 )
     {
         //MMIAPIPB_PrintDebugMsTime("[mmipb_statistics] MMIPB_SearchList  count > 0 before"); 
         if(MMIPB_ERROR_SUCCESS == MMIPB_FindFirstContact(MMIPB_ALL_GROUP_MASK, storage_id, &index, &valid_table, &contact_node, handle_ptr))
         {
             ...
             if(MMIPB_GetHandleContent(*handle_ptr, &handle_node))
             {
				 ...
				 SCI_TRACE_LOW("[mmipb_statistics] MMIPB_SearchList tab_ptr->max_count = %d",tab_ptr->max_count );
                 do                  
                 {
                     ...
                     {   
                         ...
                         if(MMIPB_LIST_FIELD_MAIL & field_type)
                         {
                             ...
                             if(tab_ptr->used_count< tab_ptr->max_count)
                             {
                                 // modified by Tong Yongli. According to Coverity CID 24636
                                 if(valid_table.is_mail_visible && base_str->mail.wstr_len > 0 && tab_ptr->index_table != PNULL)
                                 {
         ...
         //MMIAPIPB_PrintDebugMsTime("[mmipb_statistics] MMIPB_SearchList  count > 0 after"); 
     }
             ...
             if(index < select_list->used_count)
             {
                 ...
                 if(MMIPB_ERROR_SUCCESS == MMIPB_GetContactNameNumberByIndex(&base_string, contact_list_index))
                 {
                     ...
                     if(name_str != PNULL && name_str->wstr_ptr != PNULL)
                     {
						 ...
						 if(!MMIAPIPB_IsSortBySurname())
						 {
							 name_str->wstr_len  = MIN(base_string.name.wstr_len, max_name_len);
							 MMIAPICOM_Wstrncpy(name_str->wstr_ptr, base_string.name.wstr, name_str->wstr_len);
							 if(base_string.surname.wstr_len > 0)
							 {
								 ...
								 if(remain_len > 2)
								 {
									 ...
									 remain_len--;
									 MMIAPICOM_Wstrncat(name_str->wstr_ptr, base_string.surname.wstr, MIN(base_string.surname.wstr_len, remain_len));
						 ...
						 {
							 name_str->wstr_len  = MIN(base_string.surname.wstr_len, max_name_len);
							 MMIAPICOM_Wstrncpy(name_str->wstr_ptr, base_string.surname.wstr, name_str->wstr_len);
							 if(base_string.name.wstr_len > 0)
							 {
								 remain_len = max_name_len - name_str->wstr_len;
								 if(remain_len > 2)
								 {
									 MMIAPICOM_Wstrncat(name_str->wstr_ptr, L" ", 1);
									 remain_len--;
									 MMIAPICOM_Wstrncat(name_str->wstr_ptr, base_string.name.wstr, MIN(base_string.name.wstr_len, remain_len));
									 name_str->wstr_len  = MMIAPICOM_Wstrlen(name_str->wstr_ptr);
								 }
							 } 
						 }
...
/*****************************************************************************/
//  Description : 返回搜素到的第index（从0开始）个记录的姓名，姓名缩写，和号码,号码类型，头像路径
//  Global resource dependence : none
//  Author: 
//  Note: 
//RETURN: 
...
(
 ...
 uint8* total, //[OUT]  高4位是号码总个数，低4位是当前号码是第几个
 uint32 index
 )
{
	BOOLEAN result = FALSE;    
	...
	uint16  contact_list_index = 0;
	uint16  num_index = 0;
	uint8 valid_num_count = 0;
	uint8 cur_valid_num_index = 0;
		...
		if(select_list != PNULL)
		{
			if(index < select_list->used_count)
			{
				...
				if(MMIPB_ERROR_SUCCESS == MMIPB_GetContactNameNumberByIndex(&base_string, contact_list_index))
				{
					...
					for(i = 0; i < MMIPB_MAX_NV_PHONE_NUM; i++)
					{
						if(MMIPB_INVLID_NUMBER != base_string.num_type[i])
						{
							valid_num_count++;
							if(num_index == i)
							{
								cur_valid_num_index = valid_num_count;
							}
						}
					...
					if(PNULL != abbr_ptr && PNULL != abbr_ptr->wstr_ptr)
					{
						...
						if(base_string.name.wstr_len != 0)
						{
							abbr[pos] = base_string.name.wstr[0];
							pos++;
						}	
					...
					if(PNULL != abbr_ptr && PNULL != abbr_ptr->wstr_ptr)
					{
						...
						if(!MMIAPIPB_IsSortBySurname())
						{
							if(base_string.name.wstr_len != 0)
							{
								...
								if(base_string.surname.wstr_len != 0)
								{
									abbr[pos] =base_string.surname.wstr[0];
									pos++;
								}
							}	
							else
					...
					if(num_type_str != PNULL && num_type_str->wstr_ptr != PNULL && num_index < MMIPB_MAX_NV_PHONE_NUM)
					{
						MMI_TEXT_ID_T    type_text_id = TXT_NULL;
						MMI_STRING_T type_str = {0};
						if(MMIPB_ADN_NUMBER == base_string.num_type[num_index])
						{
							type_text_id = STXT_NUMBER_TYPE_MOBILE_EXT01;
						}
						else if(MMIPB_HOME_NUMBER == base_string.num_type[num_index])
						{
						    #ifndef MMIPB_RMVITEM_SUPPORT
							type_text_id = STXT_NUMBER_TYPE_HOME_EXT01;
							#else
							type_text_id = STXT_NUMBER_TYPE_OFFICE_EXT01;
							#endif
						}
						else if(MMIPB_OFFICE_NUMBER == base_string.num_type[num_index])
						{
						    #ifndef MMIPB_RMVITEM_SUPPORT
							type_text_id = STXT_NUMBER_TYPE_OFFICE_EXT01;
							#else
						...
						if(TXT_NULL != type_text_id)
						{
							MMI_GetLabelTextByLang(type_text_id, &type_str);
							num_type_str->wstr_len = MIN(max_name_len, type_str.wstr_len);
							MMIAPICOM_Wstrncpy(num_type_str->wstr_ptr, type_str.wstr_ptr, num_type_str->wstr_len);
						}
					}

					contact_node_info =  MMIPB_GetContactBaseInfo(contact_list_index);
					if(PNULL !=anim_path && PNULL !=anim_path->full_path_wstr_ptr)
					{
						MMIPB_CONTACT_T         *contact_ptr = PNULL;
						...
						if(PNULL == contact_ptr || PNULL == custom_photo_ptr)
						{
						...
						#if 0
						if(MMIPB_ERROR_SUCCESS == MMIPB_GetContactInfo(contact_node_info.contact_id, contact_node_info.storage_id, contact_ptr))
						{
							if((contact_ptr->photo_id_flag & MMIPB_PHOTO_ID_EXIST) && MMIPB_ReadPhotoFile(contact_node_info.contact_id, contact_node_info.storage_id, custom_photo_ptr))
							{
								if (MMIAPIFMM_IsFileExist((const uint16 *)custom_photo_ptr->file_name,(uint16)(custom_photo_ptr->file_name_len)))
								{
									...
									MMIAPICOM_Wstrncpy(anim_path->full_path_wstr_ptr, custom_photo_ptr->file_name, anim_path->full_path_wstr_len);
								}
							}
						}
						#else
						if((contact_node_info.photo_id_flag & MMIPB_PHOTO_ID_EXIST) && MMIPB_ReadPhotoFile(contact_node_info.contact_id, contact_node_info.storage_id, custom_photo_ptr))
						{
						    if (MMIAPIFMM_IsFileExist((const uint16 *)custom_photo_ptr->file_name,(uint16)(custom_photo_ptr->file_name_len)))
						    {
						        anim_path->full_path_wstr_len = custom_photo_ptr->file_name_len;
					...
					if(PNULL!=total)
					{
						*total = (uint8)(valid_num_count << 4) | cur_valid_num_index;
					}
...
/*****************************************************************************/
//  Description : 返回搜素到的第index（从0开始）个记录的姓名，Email
//  Global resource dependence : none
//  Author: 
//  Note: 
//RETURN: 
...
#endif /*MMI_PB_SURNAME_SUPPORT*/
					if(email_str != PNULL && email_str->wstr_ptr != PNULL)
					{
						email_str->wstr_len = MIN(max_email_len, base_string.mail.wstr_len);
						MMIAPICOM_Wstrncpy(email_str->wstr_ptr, base_string.mail.wstr, email_str->wstr_len);
					}
					result = TRUE;
				}
			}
...
)
{
       ...
       if(sour_num_ptr != PNULL && (sour_num_ptr->number_len > 0) && (dest_num_ptr != PNULL) && (sour_num_ptr->number_len <= MMIPB_BCD_NUMBER_MAX_LEN))
	   {
		   ...
		   if(start_pos - zero_len > 0)
		   {
		       int areaCode_start = 0;
		       uint8  area_bcd_len = 0;
		       for (areaCode_start=zero_len; areaCode_start < MIN(start_pos, (zero_len +6/*MMIPB_COUNTRY_AREA_CODE_MAX_LEN*/)); areaCode_start++)
		       {
		           uint8  area_bcdCode = 0;
		           area_bcdCode = (sour_num_ptr->number[areaCode_start/2] >> (((areaCode_start+1) & 1) ? 0 : 4)) & 0x0F;
		           ((uint8*)area_bcd)[area_bcd_len/2] |= area_bcdCode << (((area_bcd_len+1) & 1) ? 0 : 4);
		           area_bcd_len ++;
		       }
		   }
...
//        
/*****************************************************************************/
PUBLIC uint8 MMIPB_GetCliNumber()
{
	NVITEM_ERROR_E nv_reslut = NVERR_NONE;
	uint16 nv_len = 0;
	
	nv_len = NVITEM_GetLength(TELE_NV_CLI_NUMBER);
	nv_reslut = EFS_NvitemRead(TELE_NV_CLI_NUMBER,(uint16)sizeof(s_cli_number),(uint8*)&s_cli_number);
	if (NVERR_NONE != nv_reslut)
	{
		SCI_TRACE_LOW("MMIPB_GetCliNumber error");				
	}
	SCI_TRACE_LOW("MMIPB_GetCliNumber  %d", s_cli_number);				
	return s_cli_number;
}
...
// exclude_group:需要排除的group数组，如果要查找的记录在这个group数组中，不显示
/*****************************************************************************/
PUBLIC uint32 MMIPB_MultiSearchOnlySim
(
     MMI_STRING_T *str_input,//IN
     uint32 group,//IN
     uint32 exclude_group,
...
)
{
    ...
    MMIPB_CleanOnlySimVisibleTable(group);
    {
        ...
        MMIPB_SearchOnlySimQSortList(&wchar_lower, group, exclude_group, TRUE);
        count = MMIPB_SearchSubStringInOnlySimList(wchar_lower.wstr_ptr,search_type, TRUE, group, exclude_group);
        if(wchar_lower.wstr_ptr != PNULL)
        {
            SCI_FREE(wchar_lower.wstr_ptr);
        }
