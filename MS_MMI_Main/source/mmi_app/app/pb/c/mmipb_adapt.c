...
//shiwei 2015-7-14
    SCI_MEMCPY(phone_contact_ptr->o365_id, contact_ptr->o365_id, sizeof(phone_contact_ptr->o365_id));
    phone_contact_ptr->o365_modifiedTime = contact_ptr->o365_modifiedTime;
    phone_contact_ptr->phone_modifiedTime = contact_ptr->phone_modifiedTime;
#endif
    ...
    MMIPB_WstrToInternalString(contact_ptr->name.wstr,
        ...
        &phone_contact_ptr->name.is_ucs2,
            MMIPB_NAME_MAX_LEN*2
            );

#ifdef MMI_PB_SURNAME_SUPPORT
    //surname
    MMIPB_WstrToInternalString(contact_ptr->surname.wstr,
        contact_ptr->surname.wstr_len,
        phone_contact_ptr->surname.alpha,
        &phone_contact_ptr->surname.alpha_len,
        &phone_contact_ptr->surname.is_ucs2,
            MMIPB_NAME_MAX_LEN*2
            );
#endif /*MMI_PB_SURNAME_SUPPORT*/

#ifndef MMI_MULTI_SIM_SYS_SINGLE
    phone_contact_ptr->preferred_sim = contact_ptr->preferred_sim;
                                            ...
                                            )
{
    ...
    MMIPB_ERROR_E result = MMIPB_ERROR_SUCCESS;
    BOOLEAN office_num_flag = FALSE;
    BOOLEAN home_num_flag = FALSE;
    ...
    for(index = 0;index <MIN(MN_USIM_AAS_MAX_NUM, (MMIPB_MAX_NV_PHONE_NUM -1)); index++)/*lint !e506*/
    {
            ...
            contact_ptr->number[index+1].number_len))
        {
			...
			//aas id转化为号码类型
			num_type = MMIPB_GetNumTypeByAasID(add_contact_ptr->anr_number[index].aas_id);
			if(MMIPB_OFFICE_NUMBER == num_type)
			{
				if(!office_num_flag)
				{
					office_num_flag = TRUE;
			...
			else if(MMIPB_HOME_NUMBER == num_type)
			{
				if(!home_num_flag)
				{
					home_num_flag = TRUE;
			...
			contact_ptr->num_type[index+1] = num_type;
...
#ifdef MMIPB_SYNC_WITH_O365
	//shiwei 2015-7-14
    SCI_MEMCPY(contact_ptr->o365_id, phone_contact_ptr->o365_id, sizeof(phone_contact_ptr->o365_id));
    contact_ptr->o365_modifiedTime = phone_contact_ptr->o365_modifiedTime;
    contact_ptr->phone_modifiedTime = phone_contact_ptr->phone_modifiedTime;
#endif
...
#ifndef MMI_MULTI_SIM_SYS_SINGLE
    contact_ptr->preferred_sim = phone_contact_ptr->preferred_sim;
#endif /*MMI_MULTI_SIM_SYS_SINGLE*/
...
//  Note:
/*****************************************************************************/
PUBLIC void MMIPB_GetTextLetters(uint16 text_unicode, uint16 *letters_ptr)
{
	char pinyin = '#';
	MMIAPIIM_GetTextLetters(text_unicode, letters_ptr);
	pinyin = pinyinFirstLetter(*letters_ptr);
	if('#' != pinyin)
	{
		*letters_ptr = pinyin;
	}
	if(0x00E1 == *letters_ptr
		|| 0x0103 == *letters_ptr
		|| 0x00E2 == *letters_ptr
		|| 0x00E4 == *letters_ptr
		|| 0x00E6 == *letters_ptr
	...
	{
		*letters_ptr = 0x0061; //a
	}
	else if(0x0107 == *letters_ptr
		|| 0x00E7== *letters_ptr
		|| 0x0109== *letters_ptr
		|| 0x010B== *letters_ptr
		|| 0x010D== *letters_ptr
	...
	{
		*letters_ptr = 0x0063; //c
	}
	else if(0x0111 == *letters_ptr
		|| 0x010F== *letters_ptr
		)
	{
		*letters_ptr = 0x0064; //d
	}
	else if(0x00E9 == *letters_ptr
		|| 0x00EA== *letters_ptr
		|| 0x00EB== *letters_ptr
		|| 0x0117== *letters_ptr
		|| 0x00E8== *letters_ptr
	...
	{
		*letters_ptr = 0x0065; //e
	}
	else if(0x011F == *letters_ptr
		|| 0x0123== *letters_ptr
		|| 0x011D== *letters_ptr
		|| 0x0121== *letters_ptr
		)
	{
		*letters_ptr =0x0067; //g
	}
	else if(0x0127 == *letters_ptr
		|| 0x0125== *letters_ptr
		)
	{
		*letters_ptr = 0x0068; //h
	}
	else if(0x00ED == *letters_ptr
		|| 0x00EE== *letters_ptr
		|| 0x00EF== *letters_ptr
		|| 0x0131== *letters_ptr
		|| 0x00EC== *letters_ptr
	...
	{
		*letters_ptr = 0x0069; //i
	}
	else if(0x0135 == *letters_ptr
		)
	{
		*letters_ptr =0x006A; //j
	}
	else if(0x0137 == *letters_ptr
		)
	{
		*letters_ptr = 0x006B; //k
	}
	else if(0x013A == *letters_ptr
		|| 0x013C== *letters_ptr
		|| 0x013E== *letters_ptr
		|| 0x0142== *letters_ptr
		)
	{
		*letters_ptr = 0x006C; //l
	}
	else if(0x0144 == *letters_ptr
		|| 0x0146== *letters_ptr
		|| 0x0148== *letters_ptr
		|| 0x00F1== *letters_ptr
		)
	{
		*letters_ptr = 0x006E; //n
	}
	else if(0x00F3 == *letters_ptr
		|| 0x00F4== *letters_ptr
		|| 0x00F6== *letters_ptr
		|| 0x0151== *letters_ptr
		|| 0x0153== *letters_ptr
	...
	{
		*letters_ptr = 0x006F; //o
	}
	else if(0x0155 == *letters_ptr
		|| 0x0157== *letters_ptr
		|| 0x0159== *letters_ptr
		)
	{
		*letters_ptr = 0x0072; //r
	}
	else if(0x015B == *letters_ptr
		|| 0x015F== *letters_ptr
		|| 0x015D== *letters_ptr
		|| 0x0161== *letters_ptr
		|| 0x00DF== *letters_ptr
	...
	{
		*letters_ptr = 0x0073; //s
	}else if(0x0167 == *letters_ptr
		|| 0x0163== *letters_ptr
		|| 0x0165== *letters_ptr
		)
	{
		*letters_ptr = 0x0074; //t
	}else if(0x00FA == *letters_ptr
		|| 0x016D== *letters_ptr
		|| 0x00FB== *letters_ptr
		|| 0x00FC== *letters_ptr
		|| 0x0171== *letters_ptr
	...
	{
		*letters_ptr = 0x0075; //u
	}else if(0x00FD == *letters_ptr
		|| 0x00FF== *letters_ptr
		)
	{
		*letters_ptr = 0x0079; //y
	}else if(0x017A == *letters_ptr
		|| 0x017C== *letters_ptr
		|| 0x017E== *letters_ptr
		)
	{
		*letters_ptr = 0x007A; //z
	}		
}
...
/*****************************************************************************
	...
	Note:
*****************************************************************************/
BOOLEAN MMIPB_GetNameLettersWithSurname(
	const MMIPB_NAME_T 	*name_ptr,//IN
	const MMIPB_NAME_T 	*surname_ptr,//IN
	MMIPB_NAME_STRING_TYPE_T1  letter_type,//IN
	BOOLEAN                     is_transfer_letter,//IN
	MMIPB_NAME_LETTERS_T *letters_str//OUT
)
{
	wchar  letter_buf[40*2+1] = {0};
	wchar  head_letter_buf[2] = {0};
	uint16 i = 0;
	uint16 full_letter_len = 0;
	BOOLEAN pre_is_punct = TRUE;
	BOOLEAN is_punct = FALSE;
	wchar full_name_ptr[MMIPB_NAME_MAX_LEN*2 + 1 + 1] = {0};
	
	if(PNULL == name_ptr || PNULL == surname_ptr || letters_str == PNULL)
	{
		//SCI_TRACE_LOW:"MMIPB_GetNameLetters name_ptr %d "
		SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMIPB_ADAPT_2254_112_2_18_2_43_3_17,(uint8*)"d", name_ptr);
		return FALSE;
	}
	if(!g_mmipb_sort_by_surname)
	{
		MMIAPICOM_Wstrcpy(full_name_ptr, name_ptr->wstr);
		if(0 != name_ptr->wstr_len && 0 != surname_ptr->wstr_len)
		{
			MMIAPICOM_Wstrcat(full_name_ptr, L" ");
		}
		MMIAPICOM_Wstrcat(full_name_ptr, surname_ptr->wstr);
	}
	else
	{
		MMIAPICOM_Wstrcpy(full_name_ptr, surname_ptr->wstr);
		if(0 != name_ptr->wstr_len && 0 != surname_ptr->wstr_len)
		{
			MMIAPICOM_Wstrcat(full_name_ptr, L" ");
		}
		MMIAPICOM_Wstrcat(full_name_ptr, name_ptr->wstr);
	}
	
	for(i = 0; i < MIN(MMIAPICOM_Wstrlen(full_name_ptr), MMIPB_NAME_MAX_LEN*2 + 1); i++)
	{
		//get text letters
		SCI_MEMSET(letter_buf, 0x00, sizeof(wchar)*40);
		#ifndef MMI_PB_MULTILANG_SORT_SUPPORT
		//MMIAPIIM_GetTextLetters(full_name_ptr[i], letter_buf);
		MMIPB_GetTextLetters(full_name_ptr[i], letter_buf);
		#else /*MMI_PB_MULTILANG_SORT_SUPPORT*/
		*letter_buf=full_name_ptr[i];
		#endif /*MMI_PB_MULTILANG_SORT_SUPPORT*/
		full_letter_len = MMIAPICOM_Wstrlen(letter_buf);
		if(letter_type &MMIPB_NAME_FULL_CHAR
			||(letter_type &MMIPB_NAME_FULL_CHAR_SPACER)   
...
)
{
	...
	//MMIAPIPB_PrintDebugMsTime("[mmipb_statistics] MMIPB_GetNameLettersWithSurname for before");
	#ifdef MMI_PB_MULTILANG_SORT_SUPPORT
	full_name_len = MMIAPICOM_Wstrlen(full_name_ptr);
	if (!is_transfer_letter && 
	    (letter_type == MMIPB_NAME_FULL_CHAR || letter_type == MMIPB_NAME_FULL_CHAR_SPACER || (letter_type == (MMIPB_NAME_FULL_CHAR | MMIPB_NAME_FULL_CHAR_SPACER)))
	)   
	{
		uint16 min_len = MIN(full_name_len, MMIPB_NAME_MAX_LEN*2 + 1);
		for(i = 0; i < min_len; i++)
		{
			letter_buf[i] = MMIPB_MultiLang_GetFirstChar(full_name_ptr[i]);
		}
		MMIAPICOM_Wstrlower(letter_buf);
		MMIIM_GetWcharKeyValueByTable(letter_buf, &letters_str->full_letters[letters_str->full_letters_len], FALSE);
		letters_str->full_letters_len =+ min_len;
	}
	else
	#endif /*MMI_PB_MULTILANG_SORT_SUPPORT*/
	{
	...
	//MMIAPIPB_PrintDebugMsTime("[mmipb_statistics] MMIPB_GetNameLettersWithSurname for after");
...
PUBLIC uint8 MMIPB_GetFirstValidNumberIndex(MMIPB_BCD_NUMBER_T *number_t_ptr, uint32 num_count)
{
     ...
     uint8  i = 0;
     //added by Tong Yongli. According to Coverity CID 11763
     uint8  result = 0;//MMIPB_INDEX_OF_MAX; 
...
{
   BOOLEAN result = FALSE;
   SCI_TRACE_LOW("+[MMIPBSYNC] MMIPB_IsContactValid");
   if (PNULL != contact_ptr)
   {
       if(contact_ptr->name.wstr_len>0)
       {
           SCI_TRACE_LOW("+[MMIPBSYNC] MMIPB_IsContactValid contact_ptr->name.wstr_len>0");
           result = TRUE;
       }
       #ifdef MMI_PB_SURNAME_SUPPORT
       else if(contact_ptr->surname.wstr_len>0)
       {
           SCI_TRACE_LOW("+[MMIPBSYNC] MMIPB_IsContactValid contact_ptr->surname.wstr_len>0");
           result = TRUE;
       }
       #endif
       #ifdef MMIPB_MAIL_SUPPORT
       else if(contact_ptr->mail.wstr_len>0)
       {
           SCI_TRACE_LOW("+[MMIPBSYNC] MMIPB_IsContactValid contact_ptr->mail.wstr_len>0");
           result = TRUE;
       }
       #endif
       else if(MMIPB_IsPhoneContact(contact_ptr->storage_id) )
       {
           SCI_TRACE_LOW("+[MMIPBSYNC] MMIPB_IsContactValid MMIPB_IsPhoneContact yes !");
           //Nv entry must exist valid number or exist mail
           if(MMIPB_IsExistValidNumber(contact_ptr->number, MMIPB_MAX_NV_PHONE_NUM))
           {
               SCI_TRACE_LOW("+[MMIPBSYNC] MMIPB_IsContactValid MMIPB_IsExistValidNumber yes !");
               result = TRUE;
           }
       }
       else
       {
           SCI_TRACE_LOW("+[MMIPBSYNC] MMIPB_IsContactValid MMIPB_IsPhoneContact no !");
           //sim entry must exist ADN number
           if(MMIPB_IsExistValidNumber(contact_ptr->number, 1))
           {
               SCI_TRACE_LOW("+[MMIPBSYNC] MMIPB_IsContactValid MMIPB_IsPhoneContact no ! IsExistValidNumber yes!");
               result = TRUE;
           }
