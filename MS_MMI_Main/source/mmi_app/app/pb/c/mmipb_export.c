...
/*****************************************************************************/
//  Description : 根据输入的字符串查找号码中包含该串的记录和匹配数字
//  Global resource dependence : none
//  Author: 
//  Note: 
                               ...
                               )
{
    ...
    if(count_ptr != PNULL)
    {
        MMIAPIPB_PrintDebugMsTime("[mmipb_statistics] MMIAPIPB_SearchListAll before");
        *count_ptr = MMIPB_SearchList(search_str, field_type, MMIPB_SEARCH_NAME|MMIPB_SEARCH_NUMBER,&handle);
        MMIAPIPB_PrintDebugMsTime("[mmipb_statistics] MMIAPIPB_SearchListAll after");
    }
    return handle;
}
                               ...
                               )
{
    ...
    if(count_ptr != PNULL)
    {
        MMIPB_SEARCH_TYPE_T search_type= MMIPB_SEARCH_NAME | MMIPB_SEARCH_NUMBER;
        MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
        MMIAPISET_GetLanguageType(&lang_type);
        if(MMISET_LANGUAGE_TRAD_CHINESE_HK == lang_type && MMIAPICOM_IsNumberString(search_str->wstr_ptr, search_str->wstr_len))
        {
            search_type = MMIPB_SEARCH_NUMBER;
        }
        MMIAPIPB_PrintDebugMsTime("[mmipb_statistics] MMIAPIPB_SearchList before");
        *count_ptr = MMIPB_SearchList(search_str, field_type, search_type,&handle);
        MMIAPIPB_PrintDebugMsTime("[mmipb_statistics] MMIAPIPB_SearchList after");
    }
    return handle;
}
...
)
{
	...
	if(!MMIPB_IsPbReady())
	{
		...
		MMIPUB_OpenAlertWinByTextId(PNULL,STR_SIM_NOT_READY_EXT01,TXT_NULL,IMAGE_PUBWIN_WARNING,&alert_win_id,PNULL,MMIPUB_SOFTKEY_ONE,PNULL);
		#else
...
#ifdef MMIPB_SYNC_WITH_O365        
	else if (MMIPB_SYNC_IsSyncing())
	{
		MMIPB_OpenSyncConflictPromptWin(MMK_GetFirstAppletHandle());
	}
#endif
	else
	{
...
{
   SCI_TRACE_BUF("yanyan1029:MMIAPIPB_VcardToContact enter:data_ptr: ", 
                    data_ptr, data_len*sizeof(uint8));
...
//  Global resource dependence : 
//  Author:
//  Note:专供java pim使用，主要增加了设置和清除"允许号码包含非法字符的"flag标志
/*****************************************************************************/
                                    ...
                                    )
{
   ...
   MMIAPIVCARD_SetAllowInvalidNumberFlag();//设置flag
   
   ret = MMIPB_VcardToContact(data_ptr, data_len, contact_ptr);//vcard格式转换为联系人的格式

   MMIAPIVCARD_ClearAllowInvalidNumberFlag();//清除flag

   SCI_TRACE_BUF("yanyan1029:MMIAPIPB_VcardToContact out:name: ", 
                   contact_ptr->name.wstr, 
                   contact_ptr->name.wstr_len*sizeof(wchar));
   
   SCI_TRACE_BUF("yanyan1029:MMIAPIPB_VcardToContact out:number: ", 
                   contact_ptr->number[0].number, 
                   contact_ptr->number[0].number_len*sizeof(wchar)); 
...
PUBLIC MMIPB_ERROR_E MMIAPIPB_GetContactInfo(uint16 entry_id, uint16 storage_id, MMIPB_CONTACT_T* contact_ptr)
{
    ...
    MMIPB_ERROR_E   ret = MMIPB_ERROR_ERROR;
    const char      *tel_null = "f0f0";

    SCI_TRACE_LOW("yanyan1029:MMIAPIPB_GetContactInfo enter:entry_id=%d, strage_id=%d",
                                            entry_id, storage_id);
    ...
    if(PNULL != contact_ptr)
    {
        if((!strcmp(&contact_ptr->number[0].number, tel_null))
            && (0 == contact_ptr->name.wstr_len))
        {
            SCI_MEMSET(&contact_ptr->number[0], 0x00, sizeof(contact_ptr->number[0]));
        }
    }
    ...
    if(lower_string.wstr_ptr != PNULL)
    {
        ...
        for(i = 0; i < string1->wstr_len; i++)
        {
            ...
            else
            {
                ...
                {
                    wchar  temp_letter_buf[40*2+1] = {0};
                    MMIAPIIM_GetTextLetters(MMIPB_MultiLang_GetFirstChar(letter_buf[0]), temp_letter_buf);
                    MMIIM_GetWcharKeyValueByTable(temp_letter_buf, &letters_str->full_letters[letters_str->full_letters_len], FALSE);
                }
            ...
            #else /*MMI_PB_MULTILANG_SORT_SUPPORT*/
            if(MMIPB_MultiLang_Wstrncmp(&letters_str->full_letters[letters_str->full_letters_len], 
                &lower_string.wstr_ptr[full_cmp_len], 
                MIN(lower_string.wstr_len - full_cmp_len, full_letter_len)))
            #endif /*MMI_PB_MULTILANG_SORT_SUPPORT*/
            {
...
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPIPB_GetNameSurnameAvatarByNumber
(
...
{
	uint16 temp_entry_id = 0;
	uint16 temp_storage_id = 0;
	if(MMIPB_GetNameSurnamePreferredSimByNumber(bcd_number, name_ptr, surname_ptr, abbr_ptr, max_name_len, &temp_entry_id, &temp_storage_id,PNULL,PNULL,is_use_default_name, TRUE))
	{
		if(PNULL != entry_id)
		{
		...
		if(PNULL !=anim_path && PNULL !=anim_path->full_path_wstr_ptr)
		{
			...
			if(MMIPB_ERROR_SUCCESS == MMIPB_GetContactInfo(temp_entry_id, temp_storage_id, contact_ptr))
			{
				if((contact_ptr->photo_id_flag & MMIPB_PHOTO_ID_EXIST) && MMIPB_ReadPhotoFile(temp_entry_id, temp_storage_id, custom_photo_ptr))
				{
 ...
 )
{
	...
	MMI_STRING_T firstname = {0};
	MMI_STRING_T surname = {0};
	
	if(PNULL == bcd_number || PNULL == name_ptr || PNULL == name_ptr->wstr_ptr)
	{
	    return FALSE;
	}
	firstname.wstr_ptr = SCI_ALLOC_APP( (max_name_len + 1) * sizeof(wchar) );
	surname.wstr_ptr = SCI_ALLOC_APP( (max_name_len + 1) * sizeof(wchar) );
	if (PNULL == firstname.wstr_ptr || PNULL == surname.wstr_ptr)
	{
		if(PNULL != firstname.wstr_ptr)
		{
			SCI_FREE(firstname.wstr_ptr);
		}
		if(PNULL != surname.wstr_ptr)
		{
			SCI_FREE(surname.wstr_ptr);
		}
		
		return FALSE;
	}
	SCI_MEMSET(firstname.wstr_ptr, 0x00, (max_name_len + 1) * sizeof(wchar) );        
	SCI_MEMSET(surname.wstr_ptr, 0x00, (max_name_len + 1) * sizeof(wchar) );        
	
	if(MMIPB_GetNameSurnamePreferredSimByNumber(bcd_number, &firstname, &surname, abbr_ptr, max_name_len, entry_id, storage_id,PNULL,PNULL,is_use_default_name, TRUE))
	{
		uint16 remain_len = 0;
		if(!MMIAPIPB_IsSortBySurname())
		{
			MMIAPICOM_Wstrcpy(name_ptr->wstr_ptr, firstname.wstr_ptr);
			remain_len = max_name_len - MMIAPICOM_Wstrlen(name_ptr->wstr_ptr);
			if(0 != firstname.wstr_len && 0 != surname.wstr_len && remain_len > 2)
			{
				MMIAPICOM_Wstrcat(name_ptr->wstr_ptr, L" ");
				remain_len --;
			}
			MMIAPICOM_Wstrncat(name_ptr->wstr_ptr, surname.wstr_ptr, MIN(remain_len, MMIAPICOM_Wstrlen(surname.wstr_ptr)));
			name_ptr->wstr_len = MMIAPICOM_Wstrlen(name_ptr->wstr_ptr);
		}
		else
		{
			MMIAPICOM_Wstrcpy(name_ptr->wstr_ptr, surname.wstr_ptr);
			remain_len = max_name_len - MMIAPICOM_Wstrlen(name_ptr->wstr_ptr);
			if(0 != firstname.wstr_len && 0 != surname.wstr_len && remain_len > 2)
			{
				MMIAPICOM_Wstrcat(name_ptr->wstr_ptr, L" ");
			...
			MMIAPICOM_Wstrncat(name_ptr->wstr_ptr, firstname.wstr_ptr, MIN(remain_len, MMIAPICOM_Wstrlen(firstname.wstr_ptr)));
			name_ptr->wstr_len = MMIAPICOM_Wstrlen(name_ptr->wstr_ptr);
		}
		if(PNULL !=anim_path && PNULL !=anim_path->full_path_wstr_ptr)
		{
			...
			{
				if((contact_ptr->photo_id_flag & MMIPB_PHOTO_ID_EXIST) && MMIPB_ReadPhotoFile(*entry_id, *storage_id, custom_photo_ptr))
				{
					if (MMIAPIFMM_IsFileExist((const uint16 *)custom_photo_ptr->file_name,(uint16)(custom_photo_ptr->file_name_len)))
					{
						anim_path->full_path_wstr_len = custom_photo_ptr->file_name_len;
...
{
	if(MMIPB_GetNameSurnamePreferredSimByNumber(bcd_number, name_ptr, surname_ptr, PNULL, max_name_len, PNULL, PNULL, PNULL,PNULL,is_use_default_name, TRUE))
	{
		return TRUE;
	}
	else
...
{
	if(MMIPB_GetNameSurnamePreferredSimByNumber(bcd_number, name_ptr, surname_ptr, PNULL, max_name_len, PNULL, PNULL, PNULL,preferred_sim,is_use_default_name, TRUE))
	{
		return TRUE;
	}
	else
...
{
	return  MMIPB_GetNamePreferredSimByNumber(bcd_number, name_ptr, max_name_len, PNULL,PNULL,PNULL,preferred_sim,is_use_default_name, TRUE);
}
 ...
 )
{
	...
	{
	    SCI_TRACE_LOW("MMIAPIPB_GetNameAvatarByEmail pb is not ready");
	    return (FALSE);
	}

	if(PNULL == email_str || 0 == MMIAPICOM_Wstrlen(email_str)
		|| PNULL == name_ptr || PNULL == name_ptr->wstr_ptr 
		|| PNULL == abbr_ptr || PNULL == abbr_ptr->wstr_ptr 
		|| PNULL == anim_path || PNULL == anim_path->full_path_wstr_ptr
		|| PNULL == entry_id 
		|| PNULL == storage_id
		)
	{
	...
	if(MMIPB_GetContactWithEmail(email_str, entry_id, storage_id))
	{
		uint16 remain_len = 0;
		contact_ptr = SCI_ALLOC_APPZ(sizeof(MMIPB_CONTACT_T));
		custom_photo_ptr = SCI_ALLOC_APPZ(sizeof(MMIPB_CUSTOM_PHOTO_T));
		...
		if(MMIPB_ERROR_SUCCESS == MMIPB_GetContactInfo(*entry_id, *storage_id, contact_ptr))
		{
			...
			name_ptr->wstr_len  = MIN(contact_ptr->name.wstr_len, max_name_len);
			MMIAPICOM_Wstrncpy(name_ptr->wstr_ptr, contact_ptr->name.wstr, name_ptr->wstr_len);
			...
			if(!MMIAPIPB_IsSortBySurname())
			{
				if(contact_ptr->name.wstr_len > 0)
				{
					...
					if(contact_ptr->surname.wstr_len > 0)
					{
						...
						if(remain_len > 2)
						{
							...
							remain_len--;
							MMIAPICOM_Wstrncat(name_ptr->wstr_ptr, contact_ptr->surname.wstr, MIN(contact_ptr->surname.wstr_len, remain_len));
							name_ptr->wstr_len  = MMIAPICOM_Wstrlen(name_ptr->wstr_ptr);
				...
				else
				{
					if(contact_ptr->surname.wstr_len > 0)
					{
						name_ptr->wstr_len  = MIN(contact_ptr->surname.wstr_len, max_name_len);
						MMIAPICOM_Wstrncpy(name_ptr->wstr_ptr, contact_ptr->surname.wstr, name_ptr->wstr_len);
						abbr[pos] = contact_ptr->surname.wstr[0];
						pos++;
					}
					else
				...
				if(contact_ptr->surname.wstr_len > 0)
				{
					...
					if(contact_ptr->name.wstr_len > 0)
					{
						...
						if(remain_len > 2)
						{
							MMIAPICOM_Wstrncat(name_ptr->wstr_ptr, L" ", 1);
							remain_len--;
							MMIAPICOM_Wstrncat(name_ptr->wstr_ptr, contact_ptr->name.wstr, MIN(contact_ptr->name.wstr_len, remain_len));
							name_ptr->wstr_len  = MMIAPICOM_Wstrlen(name_ptr->wstr_ptr);
							abbr[pos] = contact_ptr->name.wstr[0];
							pos++;
						}
...
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPIPB_IsLoadSimContactReady(void) 
{
    return MMIPB_IsLoadSimContactReady();
}
