...
#else
#define MMIPB_CACHE_SIZE                        (1024*100)
#endif
#endif

#ifdef MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT
#ifdef PB_SUPPORT_LOW_MEMORY
#define MMIPB_ONLYSIM_CACHE_SIZE                        (1024*50)
#else
#define MMIPB_ONLYSIM_CACHE_SIZE                        (1024*100)
#endif
...
#ifdef MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT
LOCAL MMIPB_LOAD_SIM_CONTACT_STATUS_E s_mmpb_load_sim_contact_status = MMIPB_LOAD_SIM_CONTACT_INIT;
#endif /*MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT*/

BOOLEAN   g_mmipb_sort_by_surname = FALSE;
...
//  Note: 
/*****************************************************************************/
LOCAL void HandleLoadSimContactCnf(void);
...
//  Note: 
/*****************************************************************************/
LOCAL void HandleReleaseSimContactCnf(void);
#endif /*MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT*/
...
//  Description : 
//  Global resource dependence : 
//  Author: MMI_PB_SYNC_UPDATE_BROADCAST
//  Note: 
/*****************************************************************************/
LOCAL void HandlePBSyncUpdateBroadcast(MMIPB_SYNC_UPDATE_BROADCAST_MSG_T * update_ptr);

/*****************************************************************************/
//  Description : 
//  Global resource dependence : 
//  Author: MS_ACCOUNT_SIGNOFF_CNF
//  Note: 
/*****************************************************************************/
LOCAL void HandleMSAccountSignOffCnf(void);
#endif
...
//  Note:
/*****************************************************************************/
PUBLIC MMI_HANDLE_T MMIPB_StartBgApplet(MMIPB_INSTANCE_T *instance_ptr)
{
    MMI_APPLET_START_T start ={0};
    MMI_HANDLE_T applet_handle = 0;
    ...
    start.param_ptr = instance_ptr;
    start.size_of_param = sizeof(MMIPB_INSTANCE_T);
    start.state = MMI_APPLET_STATE_DYNAMIC|MMI_APPLET_STATE_BG;
...
/*****************************************************************************/
#ifdef MMI_VCARD_SUPPORT  
PUBLIC MMIPB_ERROR_E MMIPB_ExportBTVcardEnd(
                                        MMIPB_HANDLE_T handle 
                                        );
#endif
LOCAL MMI_RESULT_E  Applet_HandleEvent(    
                                       ...
                                       )
{
    ...
    //added by Tong Yongli. According to Coverity CID 12712
    if(PNULL == applet_ptr)
    {
        return result;
    }
    ...
    case MSG_APPLET_STOP: 
        ...
        if(MMIPB_OPC_TEMP_EXPORT == handle_node.cmd_type)
        {
        	MMIPB_ExportBTVcardEnd(applet_ptr->instance.handle);
        }
#endif
        }
...
PUBLIC BOOLEAN MMIPB_DeleteAllSelNode(MMI_HANDLE_T  handle)
{
    ...
    //else //20101005 wang.qinggang del
#endif
    {
...
//         }
        if(MMIPB_ERROR_SUCCESS == error)
        {
            ...
            if(error_count == 0)
            {
                if(applet_ptr->instance.update_callback != PNULL)
                {
                    applet_ptr->instance.update_callback(applet_handle ,MMIPB_OPERATE_SUCCESS, error,MMIPB_OPC_IMPORT,0);
                }
                //给applet发成功消息
            }
            else
        ...
        else
        {
            ...
            else
            {
                if(applet_ptr->instance.update_callback != PNULL)
                {
                    applet_ptr->instance.update_callback(applet_handle, MMIPB_OPERATE_FAIL, error,MMIPB_OPC_IMPORT,0);
                }
                //给applet发失败消息
            }
        }
...
//  Note:
/*****************************************************************************/
LOCAL void HandleExportBTContactResult(MMIPB_HANDLE_T handle, MMIPB_ERROR_E error)
{
    MMIPB_APPLET_T* applet_ptr = GetPBApplet(handle);
    MMI_HANDLE_T    applet_handle = NULL;
    MMIPB_HANDLE_NODE_T handle_node = {0};
    ...
    if(PNULL == applet_ptr || applet_ptr->instance.select_list == PNULL)
    {
        SCI_TRACE_LOW("[MMIPB] HandleExportBTContactResult PNULL ");
        return;
    }
    SCI_TRACE_LOW(" HandleExportBTContactResult error:%d", error);
    applet_handle = applet_ptr->caf_applet.app_handle;
	finished_count = applet_ptr->instance.select_list->opc_count; 
    applet_ptr->instance.select_list->opc_count++;
    MMIPB_DeleteSelHeadNode(handle);
    if(error == MMIPB_ERROR_NO_SPACE)
    {
        MMIPB_ExportBTVcardEnd(handle);
        MMIPB_DeleteAllSelNode(handle); 
	 SCI_TRACE_LOW(" HandleExportBTContactResult error:MMIPB_ERROR_NO_SPACE ");	
    ...
    else
    {
        ...
        if(MMIPB_ERROR_SUCCESS == error)
        {
            ...
            if(error_count == 0)
            {
                if(applet_ptr->instance.update_callback != PNULL)
                {
                    applet_ptr->instance.update_callback(applet_handle ,MMIPB_OPERATE_SUCCESS, error,MMIPB_OPC_TEMP_EXPORT,0);
                }
                SCI_TRACE_LOW(" HandleExportBTContactResult : MMIPB_ERROR_SUCCESS ");
            }
            else
            {
                if(applet_ptr->instance.update_callback != PNULL)
                {
                    applet_ptr->instance.update_callback(applet_handle, 
                        MMIPB_OPERATE_PARTLY_SUCCESS, 
                        error,
                        MMIPB_OPC_TEMP_EXPORT,
                        error_count);
                }
	         SCI_TRACE_LOW(" HandleExportBTContactResult : MMIPB_OPERATE_PARTLY_SUCCESS ");            
                //给applet发部分成功消息
            }
        ...
        else
        {
            ...
            else
            {
                if(applet_ptr->instance.update_callback != PNULL)
                {
                    applet_ptr->instance.update_callback(applet_handle, MMIPB_OPERATE_FAIL, error,MMIPB_OPC_TEMP_EXPORT,0);
                }
                //给applet发失败消息
            }
        }
...
LOCAL void HandleManageGroupResult(MMIPB_HANDLE_T handle, MMIPB_ERROR_E error)
{
    ...
    if(MMIPB_ERROR_SUCCESS != error)
    {
        applet_ptr->instance.select_list->error_count++;
        error_count++;
    }
    MMIPB_DeleteSelHeadNode(handle);
            ...
            ) //no entry
    {
        ...
        error_count = applet_ptr->instance.select_list->error_count;
        sel_count = applet_ptr->instance.select_list->sel_count;  
        applet_ptr->instance.all_count = applet_ptr->instance.select_list->opc_count + sel_count;
        applet_ptr->instance.finish_count = applet_ptr->instance.select_list->opc_count - applet_ptr->instance.select_list->error_count;
        MMIPB_DeleteAllSelNode(handle);
        ...
        {
            //error_count++;
            if(applet_ptr != PNULL && applet_ptr->instance.all_count > error_count)//出错的数目小于总共要删除的数目
            {
                if(applet_ptr->instance.update_callback != PNULL)
                {
                    applet_ptr->instance.update_callback(applet_handle, MMIPB_OPERATE_PARTLY_SUCCESS, error,MMIPB_OPC_MANAGE_GROUP,error_count);
...
LOCAL void HandleCopyContactResult(MMIPB_HANDLE_T handle, MMIPB_ERROR_E error)
{
    ...
    if(applet_ptr!= PNULL && applet_ptr->instance.select_list != PNULL)
    {
        SCI_TRACE_LOW("[MMIPBCOPY] HandleCopyContactResult opc_count = %d, error=%d",applet_ptr->instance.select_list->opc_count, error);
        ret = MMIPB_CopyMultiContactAsyn(handle, applet_ptr->instance.select_list->to_storage_id);
        SCI_TRACE_LOW("[MMIPBCOPY] HandleCopyContactResult2 ret = %d, opc_count = %d", ret, applet_ptr->instance.select_list->opc_count);
    }
    if(MMIPB_ERROR_PENDING !=ret)
    {
        //复制结束
        ...
        if(applet_ptr != PNULL && applet_ptr->instance.select_list != PNULL)
        {
            sel_count = applet_ptr->instance.select_list->sel_count;
            error_count = applet_ptr->instance.select_list->error_count;           
            applet_ptr->instance.all_count = applet_ptr->instance.select_list->opc_count -1 + sel_count;
            if(MMIPB_ERROR_PHONE_NO_SPACE == ret || MMIPB_ERROR_SIM1_NO_SPACE == ret || MMIPB_ERROR_SIM2_NO_SPACE == ret)
            {
                applet_ptr->instance.finish_count = applet_ptr->instance.select_list->opc_count;
            }
            else
            {
                applet_ptr->instance.finish_count = applet_ptr->instance.select_list->opc_count - 1;
            }
        }
        ...
        #endif
        {
            ...
            {
                error_count++;
                if(applet_ptr != PNULL && applet_ptr->instance.all_count  > error_count)//出错的数目小于总共要copy的数目
                {
                    if(applet_ptr != PNULL && applet_ptr->instance.update_callback != PNULL)
                    {
...
LOCAL void HandleDeleteContactResult(MMIPB_HANDLE_T handle, MMIPB_ERROR_E error)
{
    ...
    {
        applet_ptr->instance.del_entry_id = applet_ptr->instance.select_list->sel_table_head->entry_id;
        applet_ptr->instance.del_storage_id = applet_ptr->instance.select_list->sel_table_head->storage_id;
    }
    ...
    else
    {
        if(applet_ptr->instance.select_list != PNULL)
        {
            error_count = applet_ptr->instance.select_list->error_count;
            sel_count = applet_ptr->instance.select_list->sel_count;
            applet_ptr->instance.all_count = applet_ptr->instance.select_list->opc_count;
            applet_ptr->instance.finish_count = applet_ptr->instance.select_list->opc_count - applet_ptr->instance.select_list->error_count;
            MMIPB_DeleteAllSelNode(handle);
        }
...
LOCAL void MMIPB_HandleSelNodeFunc(MMIPB_HANDLE_T handle, MMIPB_ERROR_E error)
{
   ...
   if(MMIPB_ERROR_CANCEL == error)
   {
        ...
        if(MMIPB_OPC_TEMP_EXPORT == applet_ptr->instance.cmd)
        {
            MMIPB_ExportBTVcardEnd(handle);
            MMIPB_DeleteAllSelNode(handle);
        }
...
case MMIPB_OPC_TEMP_EXPORT:
    HandleExportBTContactResult(handle, error);
    break;
...
//  Note:
/*****************************************************************************/
PUBLIC void MMIPB_SetOpcType(MMIPB_HANDLE_T handle, MMIPB_OPC_E opc_type)
{
...
PUBLIC MMIPB_ERROR_E MMIPB_ModifyContact(MMIPB_CONTACT_T *contact_ptr)
{
    ...
    if((!MMIPB_IsPbReady()))
    {
     ...
     SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_ModifyContact MMIPB_IsPbReady No !");
     return MMIPB_ERROR_NO_INITIAL;
    }
    
    if(!MMIPB_IsContactValid(contact_ptr))
    {
    SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_ModifyContact MMIPB_IsContactValid No !");
        return MMIPB_ERROR_INVALID_FORMAT;
    }
    ...
    {
       SCI_TRACE_LOW("[MMIPBSYNC] MMIPB_ModifyContact MMIPB_UpdateContact");
       ret = MMIPB_UpdateContact(contact_ptr); 
    }
    SCI_TRACE_LOW("-[MMIPBSYNC] MMIPB_ModifyContact result = %d", ret);
                                            ...
                                            )   //RETURN: TRUE, 更新成功；FALSE, 更新失败
{
    ...
    // 生成phone修改时间，格式按照"DateTimeLastModified":"2015-06-16T02:20:23Z"
    /*sprintf(phoneModifiedTime, "%04d-%02d-%02dT%02d:%02d:%02dZ",
        file_time.tm_year,file_time.tm_mon,\
        file_time.tm_mday, file_time.tm_hour,\
        file_time.tm_min, file_time.tm_sec); shiwei 2015-7-14*/
    ...
    //shiwei 2015-7-15
    contact_ptr->phone_modifiedTime =  phoneModifiedTime;
    //shiwei 2015-7-8 for test
    //SCI_MEMCPY(contact_ptr->o365_id, contact_ptr->name.wstr, sizeof(contact_ptr->o365_id));
    //shiwei 2015-7-15 for test
    //contact_ptr->o365_modifiedTime =  contact_ptr->phone_modifiedTime;
    //test 
#endif  
#endif
...
LOCAL wchar TEMP_PATH[] = { 'T', 'E', 'M', 'P', 'V' , 'C' , 'A' , 'D' , '.' , 'v' , 'c' , 'f' , 0 };
LOCAL wchar vcard_dir_wstr[]   = { 'v', 'C', 'a', 'r', 'd' , 0 };
/*****************************************************************************/
// Description :异步导出联系人
// Global resource dependence :
// Author:mary.xiao
                                        ...
                                        )
{
    ...
    if(strncmp(path_name_ptr, "temp",4)==0)
    {
        MMIPB_SetOpcType(handle, MMIPB_OPC_TEMP_EXPORT);
     memset(full_path_name, 0, sizeof(wchar)*MMIFILE_FULL_PATH_MAX_LEN + 1);
     MMIAPIFMM_CombineFullPath( MMIAPIFMM_GetDevicePath(MMI_DEVICE_UDISK), MMIAPIFMM_GetDevicePathLen(MMI_DEVICE_UDISK),
        vcard_dir_wstr, 5, 
        TEMP_PATH, 12, 
    ...
    MMIAPICOM_WstrToStr(full_path_name, temp);     
                                        ...
                                        )
{
    ...
    MMIAPICOM_Wstrcpy(bt_file_info.filepath_name, MMIPB_GetCurVcardFileFullPath());
    ...
    bt_file_info.file_size = filesize;
                                  ...
                                  )
{
    ...
    MMIPB_ACCESS_POINT_T access_point = {0};
    BOOLEAN         result = FALSE;
    uint8          no_copied_field = src_field;
...
//        if(left_src_field_ptr != PNULL)
    ...
    if(no_copied_field > 0)
    {
        ...
        if(MMIPB_ERROR_SUCCESS == MMIPB_Read(MMIPB_ACCESS_CONTACT, access_point, sizeof(MMIPB_CONTACT_T), src_contact_ptr))
        {
            ...
            {
                if(no_copied_field & (MMIPB_FEILD_MASK_SINGLE_NUMBER << i))
                {
                    //源联系人信息中该域还没有被复制
                    ...
                    else
                    {
                        ...
                        *left_src_field_ptr &= (~(MMIPB_FEILD_MASK_SINGLE_NUMBER << i));/*lint !e502*///去掉该域
                        if(dest_usage_info.number_free_flag[dest_num_index])
                            {
                                dest_contact_ptr->num_type[dest_num_index] = MMIPB_INVLID_NUMBER;//src_contact_ptr->num_type[i]; //wang.qinggang for 508389
                                dest_num_index++;
                            }
                    }
                }
            ...
            *left_src_field_ptr &= (~MMIPB_FEILD_FLAG_MASK_NAME);//去掉姓名域
			...
			if(MMIPB_IsSIMContact(dest_storage_id))
			{
                uint16 remain_len = 0;
				wchar index_ary[4] = {0x005f , 0x00,0x00 , 0x00}; 
				MMIPB_NAME_T     tmp_name = {0};  //联系人姓名
				uint8 sim_contact_name_len = MMIPB_GetMaxAlphaLen(dest_storage_id, MMIPB_CONTACT_NAME);
								
                if(!MMIAPIPB_IsSortBySurname())
                {
                    dest_contact_ptr->name.wstr_len  = MIN(src_contact_ptr->name.wstr_len, MMIPB_NAME_MAX_LEN);
                    MMIAPICOM_Wstrncpy(dest_contact_ptr->name.wstr, src_contact_ptr->name.wstr, dest_contact_ptr->name.wstr_len);
                    if( src_contact_ptr->surname.wstr_len > 0)
                    {
                        remain_len = MMIPB_NAME_MAX_LEN - dest_contact_ptr->name.wstr_len;
                        if(remain_len > 2)
                        {
							if (src_contact_ptr->name.wstr_len > 0) 
							{
								MMIAPICOM_Wstrncat(dest_contact_ptr->name.wstr, L" ", 1);
								remain_len--;
                            }
							MMIAPICOM_Wstrncat(dest_contact_ptr->name.wstr, src_contact_ptr->surname.wstr, MIN(src_contact_ptr->surname.wstr_len, remain_len));
                            dest_contact_ptr->name.wstr_len  = MMIAPICOM_Wstrlen(dest_contact_ptr->name.wstr);
                        }
                    } 
                }
                else
                {
                    dest_contact_ptr->name.wstr_len  = MIN(src_contact_ptr->surname.wstr_len, MMIPB_NAME_MAX_LEN);
                    MMIAPICOM_Wstrncpy(dest_contact_ptr->name.wstr, src_contact_ptr->surname.wstr, dest_contact_ptr->name.wstr_len);
                    if( src_contact_ptr->name.wstr_len > 0)
                    {
                        remain_len = MMIPB_NAME_MAX_LEN - dest_contact_ptr->name.wstr_len;
                        if(remain_len > 2)
                        {
							...
							MMIAPICOM_Wstrncat(dest_contact_ptr->name.wstr, src_contact_ptr->name.wstr, MIN(src_contact_ptr->name.wstr_len, remain_len));
                            dest_contact_ptr->name.wstr_len  = MMIAPICOM_Wstrlen(dest_contact_ptr->name.wstr);
                        }
                    } 
                }
				
				index_ary[1] = 0x0031 + sim_postfix_index ; 
				SCI_MEMCPY((void*)&tmp_name , (void*)&dest_contact_ptr->name , sizeof(MMIPB_NAME_T));
				
				remain_len = (dest_contact_ptr->name.wstr_len + 2 > sim_contact_name_len) ? sim_contact_name_len -2 : dest_contact_ptr->name.wstr_len ;
				tmp_name.wstr[remain_len] = 0 ;
				MMIAPICOM_Wstrncat((wchar*)&tmp_name.wstr , index_ary , 2);
				tmp_name.wstr_len = remain_len + 2 ;      	
				SCI_MEMCPY(&dest_contact_ptr->name, &tmp_name, sizeof(dest_contact_ptr->name));
            ...
            *left_src_field_ptr &= (~MMIPB_FEILD_FLAG_MASK_MAIL);//去掉email域
            if((no_copied_field & MMIPB_FEILD_FLAG_MASK_MAIL)
...
{   
     ...
     )
    {
        ...
        if(applet_ptr->instance.is_set_preferred_sim)
        {
            uint16  phone_storage_id =  MMIPB_GetStorageID(MMIPB_STORAGE_PHONE, MMIPB_NORMAL_PHONE_CONTACT);
            uint16  sim1_storage_id =MMIPB_GetStorageID(MMIPB_STORAGE_SIM1, PHONEBOOK_SIM_STORAGE);
            uint16  sim2_storage_id =MMIPB_GetStorageID(MMIPB_STORAGE_SIM2, PHONEBOOK_SIM_STORAGE);
            if(dest_storage_id == phone_storage_id)
            {
                if(sim1_storage_id == applet_ptr->instance.select_list->sel_table_head->storage_id)
                {
                    dest_contact_ptr->preferred_sim = 1;
                }
                else if(sim2_storage_id == applet_ptr->instance.select_list->sel_table_head->storage_id)
                {
                    dest_contact_ptr->preferred_sim = 2;
                }
            }
        }
...
PUBLIC MMIPB_ERROR_E MMIPB_CopyMultiContactAsyn(MMIPB_HANDLE_T handle, uint16 to_storage_id)
{
    ...
    if(applet_ptr != PNULL && applet_ptr->instance.select_list != PNULL)
    {
        while(PNULL != applet_ptr->instance.select_list->sel_table_head)
        {
            ...
            SCI_TRACE_LOW("[MMIPBCOPY] MMIPB_CopyMultiContactAsyn ret=%d", ret);
            ...
            SCI_TRACE_LOW("[MMIPBCOPY] MMIPB_CopyMultiContactAsyn opc_count = %d",applet_ptr->instance.select_list->opc_count);
        }
    }
    if(ret == MMIPB_ERROR_SUCCESS || ret == MMIPB_ERROR_PENDING)
    {
          ...
          )
        {
            ...
            SCI_TRACE_LOW("[MMIPBCOPY] MMIPB_CopyMultiContactAsyn opc_count = 1");
        }
    }
    return ret;
}
    ...
    if(adn_update_cnf_ptr != PNULL)
    {
        ...
        if(adn_update_cnf_ptr->is_ok)
        {
            if(MMIPB_ERROR_SUCCESS == MMIPB_Read(MMIPB_ACCESS_CONTACT, access_point, sizeof(MMIPB_CONTACT_T), contact))
            {
                ...
                {
#ifdef MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT
                    MMIPB_RemoveContactFromOnlySimList(access_point.contact.contact_id, access_point.contact.storage_id);
                    MMIPB_InsertContactToOnlySimList(contact);
        ...
        //if(is_cur)
        {
            ...
            if(MMIPB_GetHandleContent(MMIPB_GetCurrentHandle(), &handle_node))
            {
...
)
{
...
#ifdef SIM_LOCK_CT_SUPPORT
    {
        ...
        dual_sys = MN_DUAL_SYS_1;
#endif
        if(FALSE == MMICDMA_CheckCard_OK(dual_sys))
        {
            return MMI_RESULT_FALSE;
        }
    }
...
//zouruoqi 20160324 PBSYNC error windows
	case MMI_PB_ERROR_NETWORK_OPEN_WIN:
		MMIPB_OpenSyncErrorPermissionDialogWin();
		SCI_TRACE_LOW("[MMIPB]MMI_PB_ERROR_NETWORK_OPEN_WIN: ");
		break;

	case MMI_PB_ERROR_PERMISSION_OPEN_WIN:
		MMIPB_OpenSyncErrorNetworkDialogWin();
		SCI_TRACE_LOW("[MMIPBSYNC]MMI_PB_ERROR_PERMISSION_OPEN_WIN: ");
		break;
	case MMIPB_CloseSyncResultwin:
		SCI_TRACE_LOW("[MMIPBSYNC]MMIPB_CloseSyncResultwin: ");
		MMIPB_CloseSyncResultDialog();
		break;
//zouruoqi
#endif

#ifdef MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT
    case MMI_PB_LOAD_SIM_CONTACT_CNF:
        SCI_TRACE_LOW("[MMIPBSYNC]HandlePBPsMsg MMI_PB_LOAD_SIM_CONTACT_CNF "); 
        HandleLoadSimContactCnf();
        break;
    case MMI_PB_RELEASE_SIM_CONTACT_CNF:
        SCI_TRACE_LOW("[MMIPB]HandlePBPsMsg MMI_PB_RELEASE_SIM_CONTACT_CNF "); 
        HandleReleaseSimContactCnf();
        break;
#endif /*MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT*/

#ifdef MMIPB_SYNC_WITH_O365
    case MMI_PB_SYNC_UPDATE_BROADCAST:
        SCI_TRACE_LOW("[MMIPBSYNC]HandlePBPsMsg MMI_PB_SYNC_UPDATE_BROADCAST"); 
        HandlePBSyncUpdateBroadcast((MMIPB_SYNC_UPDATE_BROADCAST_MSG_T*)param);
        break;
        
    case MSG_MS_ACCOUNT_SIGNOFF_CNF:
        SCI_TRACE_LOW("[MMIPBSYNC]HandlePBPsMsg MSG_MS_ACCOUNT_SIGNOFF_CNF"); 
        HandleMSAccountSignOffCnf();
...
#ifdef MMIPB_SYSTEM_GROUP_DELETE_SUPPORT
/*****************************************************************************/
//  Description : 
//  Global resource dependence : 
//  Author:
...
//  Note: 
/*****************************************************************************/
LOCAL BOOLEAN  MMIPB_WriteNeedCreateDefaultGroups(BOOLEAN value)
{
    BOOLEAN ret = FALSE;
    ERR_MNDATAMAG_CODE_E  nv_result   = ERR_MNDATAMAG_NO_ERR;   
    nv_result = MNNV_WriteItem(MMI_GetTrueNVItemId(MMINV_PHONEBOOK_NEED_CREATE_GROUP), sizeof(BOOLEAN), (void *)(&value));  
...
//  Note: 
/*****************************************************************************/
LOCAL BOOLEAN  MMIPB_NeedCreateDefaultGroups()
{
    ERR_MNDATAMAG_CODE_E  nv_result   = ERR_MNDATAMAG_NO_ERR;
    BOOLEAN value = TRUE;
    nv_result = MNNV_ReadItem(MMI_GetTrueNVItemId(MMINV_PHONEBOOK_NEED_CREATE_GROUP), sizeof(BOOLEAN), (void *)(&value));  
    if(ERR_MNDATAMAG_NO_ERR != nv_result)
    {
        MMIPB_WriteNeedCreateDefaultGroups(FALSE);
        return TRUE;
    }
    return FALSE;
}
...
//  Note: 
/*****************************************************************************/
LOCAL void  MMIPB_CreateDefaultGroups()
{
    MMIPB_GROUP_T      group_info = {0};
    uint16 *ring_id_ptr = PNULL;
    MMI_STRING_T       group_name_str = {0};
    
    MMIPB_GetGroupInfo(PB_GROUP_HOME, &group_info);
    if(!group_info.is_valid)
    {
        SCI_MEMSET(&group_info, 0x00, sizeof(MMIPB_GROUP_T));
        group_info.is_valid = TRUE;
        MMI_GetLabelTextByLang(STR_GROUP_LIST_FAMILY_EXT01,&group_name_str);
        group_info.group_name.wstr_len = MIN(group_name_str.wstr_len, MMIPB_NAME_MAX_LEN);
        MMIAPICOM_Wstrncpy(group_info.group_name.wstr, group_name_str.wstr_ptr, group_info.group_name.wstr_len);   
        MMIPB_SetGroupInfo(PB_GROUP_HOME, &group_info);
    }
    SCI_MEMSET(&group_info, 0x00, sizeof(MMIPB_GROUP_T));
    MMIPB_GetGroupInfo(PB_GROUP_FRIEND, &group_info);
    if(!group_info.is_valid)
    {
        SCI_MEMSET(&group_info, 0x00, sizeof(MMIPB_GROUP_T));
        group_info.is_valid = TRUE;
        MMI_GetLabelTextByLang(STR_GROUP_LIST_FRIEND_EXT01,&group_name_str);
        group_info.group_name.wstr_len = MIN(group_name_str.wstr_len, MMIPB_NAME_MAX_LEN);
        MMIAPICOM_Wstrncpy(group_info.group_name.wstr, group_name_str.wstr_ptr, group_info.group_name.wstr_len);   
        MMIPB_SetGroupInfo(PB_GROUP_FRIEND, &group_info);
    }
    SCI_MEMSET(&group_info, 0x00, sizeof(MMIPB_GROUP_T));
    MMIPB_GetGroupInfo(PB_GROUP_COLLEAGUE, &group_info);
    if(!group_info.is_valid)
    {
        SCI_MEMSET(&group_info, 0x00, sizeof(MMIPB_GROUP_T));
        group_info.is_valid = TRUE;
        MMI_GetLabelTextByLang(STR_GROUP_LIST_BUSINESS_EXT01,&group_name_str);
        group_info.group_name.wstr_len = MIN(group_name_str.wstr_len, MMIPB_NAME_MAX_LEN);
        MMIAPICOM_Wstrncpy(group_info.group_name.wstr, group_name_str.wstr_ptr, group_info.group_name.wstr_len);   
        MMIPB_SetGroupInfo(PB_GROUP_COLLEAGUE, &group_info);
    }   
}
                        ...
                        )
{
    ...
    #ifdef MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT
    mem_addr = SCI_ALLOC_CONST(MMIPB_ONLYSIM_CACHE_SIZE);
    if(mem_addr != PNULL)
    {
        SCI_MEMSET(mem_addr, 0x00, MMIPB_ONLYSIM_CACHE_SIZE);
        MMIPB_SetOnlySimMempoolHandle(SCI_CreateMemPool("MMIPB_ONLYSIM_POOL", mem_addr, MMIPB_ONLYSIM_CACHE_SIZE, &mem_cfg));      
    }
    #endif /*MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT*/
#endif
    MMIPB_InitInterface();
    ...
    MMIPB_SetContactSaveStatus(MMIPB_ReadContactSaveStatusFromNV());
#endif 
#ifdef MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT
    if( MMIPB_STATUS_SIM_NOT_READY == MMIPB_GetPBStatus())
    {
        ...
        SCI_TRACE_LOW("[MMIPB] MMIPB_InitApp MMIAPIPB_MultiLang_SetSortLang = %d",lang_type);
...
LOCAL void HandleSortCnf(void)
{
    ...
    SCI_TRACE_LOW("[MMIPB] HandleSortCnf status = %d",status);
    ...
    else if(MMIPB_STATUS_SORTING == status)
    {
        ...
        MMISET_LANGUAGE_TYPE_E lang_type = MMISET_LANGUAGE_ENGLISH;
        MMIAPISET_GetLanguageType(&lang_type);
        if ((MMIAPIPB_MultiLang_GetSortLang() != lang_type) && (MMISET_LANGUAGE_TEST != lang_type))
        {
            MMIAPIPB_MultiLang_SetSortLang(lang_type);
            SCI_TRACE_LOW("[MMIPB] HandleSortCnf MMIAPIPB_MultiLang_SetSortLang = %d",lang_type);
            MMIPB_BeginToReSort();
            return;
        }
        #endif /*MMI_PB_MULTILANG_SORT_SUPPORT*/
        ...
        if(IsAllSimReady())
        {
            if (MMIPB_MEMORY_INIT == enStatus)//nv读取状态为init
            {
                if((MMIAPIPHONE_GetSimExistedStatus(MN_DUAL_SYS_1))||
                   (MMIAPIPHONE_GetSimExistedStatus(MN_DUAL_SYS_2)))//sim1存在或者sim2存在
                {
            ...
            else if((MMIPB_MEMORY_SIM1 == enStatus)&&(!(MMIAPIPHONE_GetSimExistedStatus(MN_DUAL_SYS_1))))//nv状态为sim1，sim1不存在
            {
                enStatus = MMIPB_MEMORY_PHONE;
            }
            else if((MMIPB_MEMORY_SIM2 == enStatus)&&(!(MMIAPIPHONE_GetSimExistedStatus(MN_DUAL_SYS_2))))//nv状态为sim2，sim2不存在
            {
                enStatus = MMIPB_MEMORY_PHONE;
            }
            else if((MMIPB_MEMORY_PHONEANDSIM == enStatus) &&
            ...
            MMIPB_SetContactSaveStatus(enStatus);
        }
    ...
    //读USIM扩展信息
        ...
        //读取usim扩展信息
        {
            uint16 contact_id = 0;
            uint16 storage_id = 0; 
#ifndef MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT
...
//  Note: 
/*****************************************************************************/
PUBLIC void MMIPB_BeginToReSort(void)
{
    #ifdef MMIPB_SYNC_WITH_O365        
    ...
    //设置为sorting状态
    MMIPB_SetPBStatus(MMIPB_STATUS_SORTING);
    MMIPB_SendSignalToPBTask(MMIPB_SIG_RESORT, 0);
    //PB_SendInitSimToInitTask();
}
...
//调整此段请同时注意MMIPB_InitApp中分配内存池的地方
    ...
    SCI_FREE(mem_addr);
#endif
    //PB_Sync start, 默认开机后设置非同步状态,20150722
    //WritePbFlagToNV(MMINV_PHONEBOOK_SYNCING, 0);
    //PB_Sync end
}
...
PUBLIC void MMIPB_ReadUSIMExtCnf(APP_MN_READ_ADDITIONAL_ENTRY_CNF_T *usim_ext_ptr)
{
    ...
    if(usim_ext_ptr != PNULL && usim_ext_ptr->dual_sys < MMI_DUAL_SYS_MAX)
    {
...
#ifdef MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT
        if(MMIPB_GetOnlySimNextNotInitEntryID(&entry_id, &storage_id))
        {
             SCI_TRACE_LOW("[MMIPB] MMIPB_GetNextNotInitEntryID entry_id = %d",entry_id);
            //read next usim entry ans and email
        ...
        {
            SCI_TRACE_LOW("[MMIPB] s_mmipb_is_usim_addtion_ready TRUE");
            s_mmipb_is_usim_addtion_ready = TRUE;
        }
    }
}
...
PUBLIC void MMIPB_USIMDeleteCnf(APP_MN_UPDATE_USER_INFO_CNF_T *delete_result)
{
...
#ifdef MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT
        result = MMIPB_RemoveContactFromOnlySimList(contact_id , storage_id);
#else /*MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT*/
    ...
    //modified by Tong Yongli. According to Coverity CID 16078
    if(MMIPB_GetHandleContent(MMIPB_GetCurrentHandle(), &handle_node))
    {
...
PUBLIC BOOLEAN  MMIPB_GetSelectName(MMIPB_HANDLE_T handle,MMI_STRING_T *name_str, uint16 max_name_len,uint16 index)
{
    ...
    wchar            surname_wstr[MMIPB_MAX_STRING_LEN] = {0};
    uint16 remain_len = 0;
    ...
    if(MMIPB_GetSelectContact(handle, contact_info, index, PNULL))
    {
...
#ifdef MMI_PB_SURNAME_SUPPORT
        surname_str.wstr_ptr = surname_wstr;
        MMIPB_ConvertNameToString(
            &surname_str,
            &(contact_info->surname)
            );
        if(!MMIAPIPB_IsSortBySurname())
        {
            name_str->wstr_len  = MIN(str.wstr_len, max_name_len);
            MMIAPICOM_Wstrncpy(name_str->wstr_ptr, str.wstr_ptr, name_str->wstr_len);
            if(surname_str.wstr_len > 0)
            {
                remain_len = max_name_len - name_str->wstr_len;
                if(remain_len > 2)
                {
                    MMIAPICOM_Wstrncat(name_str->wstr_ptr, L" ", 1);
                    remain_len--;
                    MMIAPICOM_Wstrncat(name_str->wstr_ptr, surname_str.wstr_ptr, MIN(surname_str.wstr_len, remain_len));
        ...
        {
            name_str->wstr_len  = MIN(surname_str.wstr_len, max_name_len);
            MMIAPICOM_Wstrncpy(name_str->wstr_ptr, surname_str.wstr_ptr, name_str->wstr_len);
            if(str.wstr_len > 0)
            {
                remain_len = max_name_len - name_str->wstr_len;
                if(remain_len > 2)
                {
                    MMIAPICOM_Wstrncat(name_str->wstr_ptr, L" ", 1);
                    remain_len--;
                    MMIAPICOM_Wstrncat(name_str->wstr_ptr, str.wstr_ptr, MIN(str.wstr_len, remain_len));
                    name_str->wstr_len  = MMIAPICOM_Wstrlen(name_str->wstr_ptr);
                }
            } 
        }
...
PUBLIC BOOLEAN  MMIPB_GetSelectNumber(MMIPB_HANDLE_T handle, MMIPB_BCD_NUMBER_T *nameber_ptr, uint16 max_number_len,  uint16 index)
{
    ...
    if(MMIPB_GetSelectContact(handle, contact_ptr, index, &num_index))
    {
		...
		if(len == 0)
		{
			...
			{
				len = MIN(contact_ptr->number[num_index+1].number_len, max_number_len);
				nameber_ptr->number_len = len;
				SCI_MEMCPY(nameber_ptr, &contact_ptr->number[num_index+1], sizeof(MMIPB_BCD_NUMBER_T));
				ret = TRUE;
				SCI_FREE(contact_ptr);
				return ret;
			}
...
PUBLIC void MMIPB_AbortPB(BOOLEAN is_abort)
{
    ...
    if(is_abort)
    {
        ...
        if(applet_ptr != PNULL)
        {
            if(applet_ptr->instance.select_list != PNULL)
            {
                if(MMIPB_OPC_COPY ==  applet_ptr->instance.cmd)
                {
                    applet_ptr->instance.all_count = applet_ptr->instance.select_list->opc_count + applet_ptr->instance.select_list->sel_count - 1;
                    applet_ptr->instance.finish_count = applet_ptr->instance.select_list->opc_count  -  applet_ptr->instance.select_list->error_count;
                }
                else
                {
                    applet_ptr->instance.all_count = applet_ptr->instance.select_list->opc_count + 1 + applet_ptr->instance.select_list->sel_count;
                    applet_ptr->instance.finish_count = applet_ptr->instance.select_list->opc_count  + 1 -  applet_ptr->instance.select_list->error_count;
                }
            }
            MMIPB_DeleteAllSelNode(applet_ptr->instance.handle);
        }
                                              ...
                                              )
{
    ...
    if(MMIPB_IsInSIMGroup(group_id))
    {
        ...
        contact_node = MMIPB_GetOnlySimContactBaseInfo(index);
...
)
{
     ...
     CC_CALL_SIM_TYPE_E call_sim_type = MMIAPICC_GetCallSimType(dual_sys);
...
//  Note: 
/*****************************************************************************/
PUBLIC void MMIPB_CallNumberWithSurname
(
 MN_DUAL_SYS_E dual_sys,
 MMIPB_CALL_TYPE_E  interface_type, //IN:
 uint8                *call_number_ptr, //IN:
 MMI_STRING_T         *first_name_ptr, //IN:contact_name 
 MMI_STRING_T         *last_name_ptr //IN:contact_name 
 )
{
    //MN_NUMBER_TYPE_E  number_type = 0;//
    CC_CALL_SIM_TYPE_E call_sim_type = MMIAPICC_GetCallSimType(dual_sys);
    ...
    if(PNULL == first_name_ptr 
        ||  PNULL == first_name_ptr->wstr_ptr
        ||  PNULL == last_name_ptr
        ||  PNULL == last_name_ptr->wstr_ptr
        )
    {
        return;
    }
    ...
    if(!MMIAPIPB_IsSortBySurname())
    {
        MMIAPICOM_Wstrncpy(full_name_ptr, first_name_ptr->wstr_ptr, first_name_ptr->wstr_len);
        if(0 != first_name_ptr->wstr_len && 0 != last_name_ptr->wstr_len)
        {
            MMIAPICOM_Wstrcat(full_name_ptr, L" ");
        }
        MMIAPICOM_Wstrncat(full_name_ptr, last_name_ptr->wstr_ptr,last_name_ptr->wstr_len);
    }
    else
    {
        MMIAPICOM_Wstrncpy(full_name_ptr, last_name_ptr->wstr_ptr, last_name_ptr->wstr_len);
        if(0 != first_name_ptr->wstr_len && 0 != last_name_ptr->wstr_len)
        {
            MMIAPICOM_Wstrcat(full_name_ptr, L" ");
        }
        MMIAPICOM_Wstrncat(full_name_ptr, first_name_ptr->wstr_ptr, first_name_ptr->wstr_len);
    }
    call_name_str.wstr_ptr = full_name_ptr;
    call_name_str.wstr_len = MMIAPICOM_Wstrlen(full_name_ptr);
    if(PNULL == call_number_ptr)
    {
        //SCI_TRACE_LOW:"[MMIPB]MakeNumberCallByBCDNumber number_str_ptr == PNULL !"
        SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMIPB_APP_5389_112_2_18_2_43_15_153,(uint8*)"");
    ...
    )
{
    ...
    {
        MMI_STRING_T    surname_str = {0};
        wchar surname_buf[MMIPB_NAME_MAX_LEN] = {0};
        surname_str.wstr_ptr = surname_buf;
        surname_str.wstr_len = MIN(contact_ptr->surname.wstr_len, MMIPB_NAME_MAX_LEN);
        MMIAPICOM_Wstrncpy(surname_str.wstr_ptr, contact_ptr->surname.wstr,surname_str.wstr_len);
        MMIPB_CallNumberWithSurname(dual_sys, call_type,tele_num,&name_str, &surname_str);
    }
#endif /*MMI_PB_SURNAME_SUPPORT*/
...
#define MMIPB_SEND_BY_OUTLOOK_CONTENT_LEN (sizeof(wchar)*(MMIFILE_FULL_PATH_MAX_LEN+1 + 20))
#define MMIPB_SEND_BY_OUTLOOK_UTF8_CONTENT_LEN (sizeof(wchar)*(MMIFILE_FULL_PATH_MAX_LEN+1 + 20)*2)
...
//  Note: 
/***************************************************************************/
PUBLIC MMIPB_ERROR_E MMIPB_SendGroupEmailByOutlook(MMIPB_HANDLE_T handle)
{
	...
	wchar*  outlook_file_name = L"outlookcontactshare.txt";
	...
	uint8 unicode_head[3]={0xEF,0xBB,0xBF};
	...
	if(MMIPB_GetSelectCount(handle) > 0)
	{
		...
		if(0 != des_file_handle)
		{
			...
			uint32 rw_size  = 0;		
			content = SCI_ALLOCA(MMIPB_SEND_BY_OUTLOOK_CONTENT_LEN);
			utf8_content = SCI_ALLOCA(MMIPB_SEND_BY_OUTLOOK_UTF8_CONTENT_LEN);
			if(content == PNULL || utf8_content == PNULL)
			{
			...
			MMIAPIFMM_WriteFile(des_file_handle, unicode_head, sizeof(unicode_head), &rw_size, NULL);
			SCI_MEMSET(content, 0x00, MMIPB_SEND_BY_OUTLOOK_CONTENT_LEN);
			MMIAPICOM_Wstrcat(content, L"Contact\n");
			utf8_content_len = GUI_WstrToUTF8(utf8_content, MMIPB_SEND_BY_OUTLOOK_UTF8_CONTENT_LEN, content, MMIAPICOM_Wstrlen(content));
			des_file_ret = MMIAPIFMM_WriteFile(des_file_handle, utf8_content, utf8_content_len, &rw_size, NULL);
			if(SFS_ERROR_NONE == des_file_ret && rw_size == utf8_content_len)
			{
				for(i=0;i<MMIPB_GetSelectCount(handle); i++)
				{
					mail_str.wstr_ptr = mailbuf;
					if(MMIPB_GetSelectEmail(handle, &mail_str, MMIPB_MAX_MAIL_LEN, i))
					{
						SCI_MEMSET(content, 0x00, MMIPB_SEND_BY_OUTLOOK_CONTENT_LEN);
						SCI_MEMSET(utf8_content, 0x00, MMIPB_SEND_BY_OUTLOOK_UTF8_CONTENT_LEN);
						MMIAPICOM_Wstrcat(content,L"email://");
						MMIAPICOM_Wstrcat(content,mailbuf);
			...
			{								
			    send_error = MMIPB_GetErrorType(des_file_ret);								
			}
			else if(rw_size != utf8_content_len)
			{								
			    send_error = MMIPB_ERROR_ERROR;								
			}
			else
			{	
#ifdef JAVA_SUPPORT_MYRIAD
				char temp_path [MMIFILE_FULL_PATH_MAX_LEN+1] = {0};
				char path [MMIFILE_FULL_PATH_MAX_LEN+1 + 10] = {0};
				...
				MMIJAVA_SetAppProperty("com.microsoft.outlook.share",path);
...
//  Note: 
/***************************************************************************/
PUBLIC MMIPB_ERROR_E MMIPB_SendEmailByOutlook(MMIPB_MAIL_T *pb_email)
{
	MMIPB_ERROR_E send_error = MMIPB_ERROR_SUCCESS;
	MMIFILE_DEVICE_E file_dev = MMI_DEVICE_UDISK;
	MMIFILE_ERROR_E des_file_ret = 0;
	...
	if(PNULL == pb_email || 0 == pb_email->wstr_len)
	{
		return MMIPB_ERROR_INVALID_PARAM;
	}
	outlook_file_full_path = SCI_ALLOCA(sizeof(wchar)*(MMIFILE_FULL_PATH_MAX_LEN + 1));
	...
	if(0 != des_file_handle)
	{
		...
		MMIAPICOM_Wstrcat(content, L"Contact\n");
		MMIAPICOM_Wstrcat(content,L"email://");
		MMIAPICOM_Wstrcat(content,pb_email->wstr);
		utf8_content_len = GUI_WstrToUTF8(utf8_content, MMIPB_SEND_BY_OUTLOOK_UTF8_CONTENT_LEN, content, MMIAPICOM_Wstrlen(content));
...
//  Note: 
/***************************************************************************/
PUBLIC MMIPB_ERROR_E MMIPB_SendVcardByOutlook(MMIFILE_FILE_INFO_T *file_info_ptr)
{
	MMIPB_ERROR_E send_error = MMIPB_ERROR_SUCCESS;
	MMIFILE_ERROR_E des_file_ret = 0;
	MMIFILE_HANDLE  des_file_handle = 0;
	...
	if(0 != des_file_handle)
	{
		...
		wchar* vcard_full_path = PNULL;
		uint16 vcard_path_len = MMIFILE_FULL_PATH_MAX_LEN;
		...
		vcard_full_path = SCI_ALLOCA(sizeof(wchar)*(MMIFILE_FULL_PATH_MAX_LEN+1));
		if(vcard_full_path == PNULL)
		{
			if(PNULL != outlook_file_full_path)
			{
				SCI_FREE(outlook_file_full_path);
		...
		SCI_MEMSET(vcard_full_path, 0x00, sizeof(wchar)*(MMIFILE_FULL_PATH_MAX_LEN+1));
		
		MMIAPIFMM_CombineFullPath(file_info_ptr->device_name, file_info_ptr->device_name_len,
			file_info_ptr->dir_name, file_info_ptr->dir_name_len,
			file_info_ptr->file_name, file_info_ptr->file_name_len,
			vcard_full_path, &vcard_path_len);
		MMIAPICOM_WstrReplace(vcard_full_path,sizeof(wchar)*(MMIFILE_FULL_PATH_MAX_LEN+1), slash, new_split);
		MMIAPICOM_Wstrcat(content, L"Contact\n");
		MMIAPICOM_Wstrcat(content,L"file://");
		MMIAPICOM_Wstrcat(content,vcard_full_path);
		utf8_content_len = GUI_WstrToUTF8(utf8_content, MMIPB_SEND_BY_OUTLOOK_UTF8_CONTENT_LEN, content, MMIAPICOM_Wstrlen(content));
		...
		if(PNULL != vcard_full_path)
		{
			SCI_FREE(vcard_full_path);
		}
		if(SFS_ERROR_NONE != des_file_ret)
		{								
			send_error = MMIPB_GetErrorType(des_file_ret);								
    ...
    )
{
    ...
    if(MMIPB_SEND_BY_SMS != send_type)
    {
		...
		{
			SCI_TRACE_LOW("[SendVCARD]Ori %d dev is no space", file_dev);
			if(file_dev != MMI_DEVICE_NUM)
			{
				...
				else
				{
					...
					{
						send_error = MMIPB_ERROR_SUCCESS;
					}
					SCI_TRACE_LOW("[SendVCARD] Change to use sdcard and space is %s", (send_error == MMIPB_ERROR_SUCCESS)?"enough":"not enough");
				}
			}
	     }
	     ...
	     if(!send_error )
            {
                ...
                if(MMIPB_ERROR_SUCCESS == send_error)
                {
                    ...
                    else if (MMIPB_SEND_BY_EMAIL== send_type)
                    {
                        ...
                        {
                            send_error = MMIPB_SendVcardByOutlook(file_info_ptr);
...
//  Note: 
/***************************************************************************/
PUBLIC MMIPB_ERROR_E MMIPB_SendMultiVcard(MMIPB_HANDLE_T applet_handle, MMIPB_SEND_TYPE_E send_type)
{
    MMIPB_ERROR_E send_error = MMIPB_ERROR_SUCCESS;
    MMIFILE_ERROR_E file_ret = 0;
    MMIFILE_HANDLE  file_handle = 0;
    ...
    if(PNULL == applet_ptr || PNULL == applet_ptr->instance.select_list || PNULL == applet_ptr->instance.select_list->sel_table_head)
    {
        send_error = MMIPB_ERROR_INVALID_PARAM;
        return send_error;
    }
    ...
    if(MMIPB_SEND_BY_SMS != send_type)
    {
		...
		if(!CheckDiskSpaceForSaveVCard(file_dev))
		{
			SCI_TRACE_LOW("[SendMultiVCARD]Ori %d dev is no space", file_dev);
			if(file_dev != MMI_DEVICE_NUM)
			{
				file_dev = MMI_DEVICE_SDCARD;
				if(!MMIAPIFMM_GetDeviceTypeStatus((file_dev)))
				...
				else
				{
					...
					SCI_TRACE_LOW("[SendMultiVCARD] Change to use sdcard and space is %s", (send_error == MMIPB_ERROR_SUCCESS)?"enough":"not enough");
				}
			}
		}
	     ...
	     if(!send_error )
            {
                ...
                SCI_MEMSET(file_info_ptr, 0x00, sizeof(MMIFILE_FILE_INFO_T));
                SCI_MEMSET((void *)bt_file_info.filepath_name, 0, ((MMIFILE_FULL_PATH_MAX_LEN+1)*sizeof(uint16)));
                MMIPB_GetContactInfo(applet_ptr->instance.select_list->sel_table_head->entry_id, applet_ptr->instance.select_list->sel_table_head->storage_id, contact_ptr);
                ...
                if(SFS_ERROR_NONE == file_ret)
                {
                    ...
                    do
                    {
                        SCI_MEMSET(contact_ptr, 0x00, sizeof(MMIPB_CONTACT_T));
                        MMIPB_GetContactInfo(applet_ptr->instance.select_list->sel_table_head->entry_id, applet_ptr->instance.select_list->sel_table_head->storage_id, contact_ptr);
                        MMIPB_DeleteSelHeadNode(applet_ptr->instance.handle);
                        send_error= MMIPB_Write(MMIPB_ACCESS_VCARD, access_point, sizeof(MMIPB_CONTACT_T), contact_ptr);
                    }while(MMIPB_ERROR_SUCCESS == send_error && PNULL!=applet_ptr->instance.select_list->sel_table_head);
                    MMIFILE_CloseFile(file_handle);
                }
                else
                {
...
//  Note: 
/***************************************************************************/
PUBLIC MMIPB_ERROR_E MMIPB_SendMultiVcardByMMS(MMIPB_HANDLE_T applet_handle, MMIPB_SEND_TYPE_E send_type)
{
    MMIPB_ERROR_E send_error = MMIPB_ERROR_SUCCESS;
    MMIFILE_ERROR_E file_ret = 0;
    MMIFILE_HANDLE  file_handle = 0;
    MMIFILE_FILE_INFO_T      *file_info_ptr = PNULL;
    MMIFMM_BT_SENDFILE_INFO_T *file_info = PNULL;
    MMIPB_ACCESS_POINT_T      access_point = {0};
    MMIFILE_DEVICE_E file_dev = MMI_DEVICE_UDISK;
    MMIPB_CONTACT_T *contact_ptr = PNULL;
    MMIPB_APPLET_T*  applet_ptr= (MMIPB_APPLET_T*)MMK_GetAppletPtr(applet_handle);
    ...
    mark_num = MMIPB_GetSelectCount(applet_ptr->instance.handle);
    ...
    if(MMIPB_SEND_BY_SMS != send_type)
    {
        ...
        {
            unfinish_file_num = send_file_num = mark_num;
#if 0 // add if need to limit send vcard number
            ...
            {
                file_info_ptr = SCI_ALLOCA(send_file_num*sizeof(MMIFILE_FILE_INFO_T));
                file_info = SCI_ALLOCA(send_file_num*sizeof(MMIFMM_BT_SENDFILE_INFO_T));
                if(file_info_ptr == PNULL)
                {
                    SCI_FREE(contact_ptr);
                    return MMIPB_ERROR_NO_SPACE;
                }
                SCI_MEMSET(file_info_ptr, 0x00, send_file_num*sizeof(MMIFILE_FILE_INFO_T));
                SCI_MEMSET(file_info, 0x00, send_file_num*sizeof(MMIFMM_BT_SENDFILE_INFO_T));

                for(i = 0; i< send_file_num; i ++)
                {
                    MMIPB_GetContactInfo(applet_ptr->instance.select_list->sel_table_head->entry_id, applet_ptr->instance.select_list->sel_table_head->storage_id, contact_ptr);
                    MMIAPIVCARD_GetVcardFileName(contact_ptr, 
                        MMIPB_GET_CONTACT_TYPE(contact_ptr->storage_id), 
                        file_info_ptr[i].file_name);
                
                    MMIAPICOM_CombineDefaultFullPathByFilename(file_dev, 
                        file_info_ptr[i].file_name, 
                        MMIAPICOM_Wstrlen(file_info_ptr[i].file_name), 
                        (wchar *)(file_info[i].filepath_name), 
                        MMIFILE_FULL_PATH_MAX_LEN);

                    MMIAPICOM_GetNoExistFileName(file_info[i].filepath_name, MMIAPICOM_Wstrlen(file_info[i].filepath_name));        
                    file_ret = MMIAPIFMM_CreateFileAsyn(file_info[i].filepath_name, 
                         SFS_MODE_WRITE|SFS_MODE_CREATE_NEW|SFS_MODE_APPEND,
                        PNULL,
                        PNULL,
                        0,
                ...
                if(MMIPB_ERROR_SUCCESS == send_error)
                {
                    ...
                    for(i = 0; i < send_file_num; i ++)
                    {
                        file_info[i].filepath_len = MMIAPICOM_Wstrlen(file_info[i].filepath_name);
                        SCI_MEMSET(file_info_ptr, 0x00, sizeof(MMIFILE_FILE_INFO_T));
                        ...
                        if(MMIPB_SEND_BY_MMS == send_type)
                        {
                            if (send_file_num == 1)
                            {
                                MMIAPIFMM_GetFileInfoFormFullPath(file_info[i].filepath_name, file_info[i].filepath_len, file_info_ptr);
                                MMIAPIMMS_EditMMSFromFile(file_info_ptr, MMIFMM_FILE_TYPE_VCARD);                            
                            }
                            else
                            {
                                MMIAPIFMM_GetFileInfoFormFullPath(file_info[i].filepath_name, file_info[i].filepath_len, file_info_ptr);
                                MMIAPIMMS_EditMMSFromMultiFile(file_info_ptr, MMIFMM_FILE_TYPE_VCARD, unfinish_file_num, send_file_num);  
                                unfinish_file_num --;
                            }
                        }
#endif
...
LOCAL BOOLEAN is_same_contact(MMIPB_CONTACT_T *contact_ptr)
{
    ...
    for(i=0; i<total_count; i++)
    {
      ...
      if(MMIPB_IsPhoneContact(contact_node_info.storage_id)&& !memcmp(&contact_info.name, &(contact_ptr->name), sizeof(MMIPB_NAME_T))
          #ifdef MMI_PB_SURNAME_SUPPORT
          && !memcmp(&contact_info.surname, &(contact_ptr->surname), sizeof(MMIPB_NAME_T))
          #endif
...
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMIPB_IsAllSimReady(void)
{
    return IsAllSimReady();
}
...
//  author: 
/*****************************************************************************/
PUBLIC void MMIPB_SetLoadSimContactStatus(MMIPB_LOAD_SIM_CONTACT_STATUS_E status)
{
    SCI_DisableIRQ();
    s_mmpb_load_sim_contact_status = status;
    SCI_RestoreIRQ();
}
...
//  author:
/*****************************************************************************/
PUBLIC MMIPB_LOAD_SIM_CONTACT_STATUS_E MMIPB_GetLoadSimContactStatus(void)
{
    MMIPB_LOAD_SIM_CONTACT_STATUS_E status = 0;

    //SCI_DisableIRQ();
    status = s_mmpb_load_sim_contact_status;
    //SCI_RestoreIRQ();
    return status;
}
...
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMIPB_IsLoadSimContactReady(void)
{
    BOOLEAN result = FALSE;
    SCI_TRACE_LOW("[MMIPB] MMIPB_IsLoadSimContactReady1, MMIPB_GetLoadSimContactStatus() = %d", MMIPB_GetLoadSimContactStatus());
    if(MMIPB_LOAD_SIM_CONTACT_READY == MMIPB_GetLoadSimContactStatus())
    {
        SCI_TRACE_LOW("[MMIPB] MMIPB_IsLoadSimContactReady2");
#ifdef CMCC_VCARD_SUPPORT
        if(MMIPB_IsUSIMReady())
        {
            SCI_TRACE_LOW("[MMIPB] MMIPB_IsLoadSimContactReady3");
            result = TRUE;
        }
        else
        {
...
{
    SCI_TRACE_LOW("[MMIPB] MMIPB_BeginToLoadSimContact, IsAllSimReady()=%d, MMIPB_GetLoadSimContactStatus()=%d", IsAllSimReady(), MMIPB_GetLoadSimContactStatus());
    if(IsAllSimReady() && MMIPB_LOAD_SIM_CONTACT_INIT == MMIPB_GetLoadSimContactStatus())
    {
        MMIPB_SetLoadSimContactStatus(MMIPB_LOADING_SIM_CONTACT);
        MMIPB_SendSignalToPBTask(MMIPB_SIG_LOAD_SIM_CONTACT, 0);
        return TRUE;
    }
    return FALSE;
}
...
{
    SCI_TRACE_LOW("[MMIPB]MMIPB_ReleaseLoadSimContact");
    //if(MMIPB_LOAD_SIM_CONTACT_READY == MMIPB_GetLoadSimContactStatus())
    {
        MMIPB_SetLoadSimContactStatus(MMIPB_RELEASE_SIM_CONTACT);
        MMIPB_SendSignalToPBTask(MMIPB_SIG_RELEASE_SIM_CONTACT, 0);
    }
}
...
LOCAL void HandleLoadSimContactCnf(void)
{
    MMIPB_LOAD_SIM_CONTACT_STATUS_E status = MMIPB_GetLoadSimContactStatus();    
    MMIPB_HANDLE_T applet_handle = 0;
    MMIPB_APPLET_T*  applet_ptr= PNULL;

    SCI_TRACE_LOW("[MMIPB]HandleLoadSimContactCnf MMIPB_GetLoadSimContactStatus = %d", status);
    if(MMIPB_LOADING_SIM_CONTACT == status)
    {
        //排序完成，并且在排序中没有重新排序的请求，排序完成
        MMIPB_SetLoadSimContactStatus(MMIPB_LOAD_SIM_CONTACT_READY);
        MMIPB_GetCurAppletHandle(&applet_handle);
        applet_ptr = (MMIPB_APPLET_T* )MMK_GetAppletPtr(applet_handle);
        if(applet_ptr != PNULL)
        {
            if(0)//applet_ptr->instance.update_callback != PNULL)
            {
                applet_ptr->instance.update_callback(applet_handle, 
                    MMIPB_OPERATE_SUCCESS, 
                    MMIPB_ERROR_SUCCESS,
        ...
        //读取usim扩展信息
        {
            uint16 contact_id = 0;
            uint16 storage_id = 0; 
            if(MMIPB_GetOnlySimNextNotInitEntryID(&contact_id, &storage_id))
            {
                SCI_TRACE_LOW("[MMIPB] HandleLoadSimContactCnf MMIPB_GetOnlySimNextNotInitEntryID = %d", contact_id);
                //read next usim entry ans and email
                MMIPB_ReadUSIMAddtionalAsyn(MMIPB_GetDualSysByStorageId(storage_id), contact_id);
            }
            else
...
LOCAL void HandleReleaseSimContactCnf(void)
{
    ...
    MMIPB_HANDLE_T applet_handle = 0;
    MMIPB_APPLET_T*  applet_ptr= PNULL;
    SCI_TRACE_LOW("[MMIPB]HandleReleaseSimContactCnf MMIPB_GetLoadSimContactStatus = %d", status);
    if(MMIPB_RELEASE_SIM_CONTACT == status)
    {
        //排序完成，并且在排序中没有重新排序的请求，排序完成
        MMIPB_SetLoadSimContactStatus(MMIPB_LOAD_SIM_CONTACT_INIT);
        MMIPB_GetCurAppletHandle(&applet_handle);
        applet_ptr = (MMIPB_APPLET_T* )MMK_GetAppletPtr(applet_handle);
        if(applet_ptr != PNULL)
        {
    ...
    #ifdef CMCC_VCARD_SUPPORT
    SCI_TRACE_LOW("[MMIPB] s_mmipb_is_usim_addtion_ready FALSE");          
    s_mmipb_is_usim_addtion_ready = FALSE;
    #endif
}
...
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMIPB_GenerateSimEntryList(void)
{
    MN_DUAL_SYS_E           sim_type  = MN_DUAL_SYS_1;
    uint16                     i=0;
    uint16                  storage_id = 0;
    ...
    if(!MMIPB_AllocOnlySimcontactList())
    {
        return FALSE;
    }

    SCI_TRACE_LOW("[MMIPB] MMIPB_GenerateSimEntryList 1");
    contact_info = (MMIPB_CONTACT_T *)SCI_ALLOC_APP(sizeof(MMIPB_CONTACT_T));
    ...
    for(sim_type = MN_DUAL_SYS_1; sim_type < MMI_DUAL_SYS_MAX; sim_type++)
    {
        ...
        {
             storage_info = MMIPB_GetContactStorageInfo(MMIPB_GetStorageID(MMIPB_STORAGE_SIM1 + sim_type, PHONEBOOK_SIM_STORAGE));
             SCI_TRACE_LOW("[MMIPB] MMIPB_GenerateSimEntryList sim%d.max_record_num = %d",sim_type,storage_info.max_record_num);
            for (i=1; (i<= MIN(storage_info.max_record_num, MMIPB_SIM_MAX_RECORD_NUM)) && (MMIPB_LOADING_SIM_CONTACT == MMIPB_GetLoadSimContactStatus()); i++)
            {
                if(0 == (i%100))
                {
                    SCI_Sleep(1);
                ...
                if(MMIPB_ERROR_SUCCESS == ret && (MMIPB_IsContactValid(contact_info)))
                {
                    MMIPB_InsertOnlySimContactListNode(contact_info);
                }
            } 
        }
        else
        {
            SCI_TRACE_LOW("[MMIPB] MMIPB_GenerateSimEntryList sim%d not exist",sim_type);
        }
    }
    SCI_FREE(contact_info);
    SCI_TRACE_LOW("[MMIPB] MMIPB_GenerateSimEntryList 2");
    return TRUE;
}
#endif /*MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT*/
...
{
#ifdef MMIPB_SYSTEM_GROUP_DELETE_SUPPORT
    if(MMIPB_NeedCreateDefaultGroups())
    {
        MMIPB_CreateDefaultGroups();
    }
#endif /*MMIPB_SYSTEM_GROUP_DELETE_SUPPORT*/
}
...
{
    return g_mmipb_sort_by_surname;
}
...
//  Note: 
/*****************************************************************************/
PUBLIC uint32 MMIPB_DebugEmpty(const char *x_format, ...)
{
    return 0;
}
...
LOCAL void HandlePBSyncUpdateBroadcast(MMIPB_SYNC_UPDATE_BROADCAST_MSG_T * update_ptr)
{
    SCI_TRACE_LOW("+[MMIPBSYNC] HandlePBSyncUpdateBroadcast");

    if (PNULL != update_ptr)
    {
        SCI_TRACE_LOW("[MMIPBSYNC] entry_id=%d", update_ptr->entry_id);
        SCI_TRACE_LOW("[MMIPBSYNC] storage_id=%d", update_ptr->storage_id);
        SCI_TRACE_LOW("[MMIPBSYNC] opt=%d", update_ptr->opt);
        
        MMIAPIPB_SyncUpdateAllWin(update_ptr->entry_id, update_ptr->storage_id, update_ptr->opt);
    }
...
//  Note: 
/*****************************************************************************/
LOCAL void MMIPB_SYNC_Stop_Sync_Callback(void)
{
    SCI_TRACE_LOW("+[MMIPBSYNC] MMIPB_SYNC_Stop_Sync_Callback");

    MMIPB_SYNC_Remove_All_AccountData();
...
LOCAL void HandleMSAccountSignOffCnf(void)
{
    SCI_TRACE_LOW("+[MMIPBSYNC] HandleMSAccountSignOffCnf");

    if (MMIPB_SYNC_IsSyncing())
    {
        SCI_TRACE_LOW("[MMIPBSYNC] is pb syncing, stop sync first");
        MMIPB_SYNC_Stop_Sync(MMIPB_SYNC_Stop_Sync_Callback);
    }
    else
    {
        MMIPB_SYNC_Remove_All_AccountData();
