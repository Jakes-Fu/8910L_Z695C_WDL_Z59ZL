...
{                                                                       \
    ...
    /* 直接调用 EFS_NvitemWrite 可以绕开 MN 对 ID 的限制 */             \
    _ret_value = (MN_RETURN_RESULT_E)MNNV_ReadItem(                     \
    (MMI_GetTrueNVItemId(_item_id)),                                    \
...
{                                                                       \
    ...
    _ret_value = FALSE;                                                 \
                                                                        ...
                                                                        \
    if (MN_RETURN_SUCCESS != (MN_RETURN_RESULT_E)MNNV_WriteItem(        \
    (MMI_GetTrueNVItemId(_item_id )),                                   \
    sizeof (MMIPB_CONTACT_PHONE_T),                                     \
    (void *)(_addr)                                                     \
    )                                                                   \
    ...
    {                                                                   \
    SCI_TRACE_MID("mmipb_mnapi: MMINV_WRITE_PHONEBOOK FAILURE");        \
    _ret_value = MN_RETURN_FAILURE;                                                 \
    }                                                                   \
                                                                        \
    else                                                                \
    {                                                                   \
    _ret_value = MN_RETURN_SUCCESS;                                                  \
    }                                                                   \
}     

//PB_Sync start, 定义读写本地删除记录的宏,add by dufei,20150714
#define MMINV_READ_LOCALDELETED_CONTACT(_item_id, _addr, _ret_value)           \
{                                                                       \
    if ((MMINV_PHONEBOOK_LOCALDELETEO365_LAST_ENTRY < (_item_id))                  \
    || (MMINV_PHONEBOOK_LOCALDELETEO365_FIRST_ENTRY > (_item_id)))                 \
    {                                                                   \
    SCI_TRACE_MID("mmipb_mnapi: MMINV_READ_LOCALDELETED item_id = %d",     \
    (_item_id));                                                        \
    _ret_value = MN_RETURN_FAILURE;                                     \
    }                                                                   \
    /* 直接调用 EFS_NvitemWrite 可以绕开 MN 对 ID 的限制 */             \
    _ret_value = (MN_RETURN_RESULT_E)MNNV_ReadItem(                     \
    (MMI_GetTrueNVItemId(_item_id)),                                    \
    sizeof (MMIPB_CONTACT_LOCALDELETED_T),                                     \
    (void *)(_addr)                                                     \
    );                                                                  \
}  
                                                              
#define MMINV_WRITE_LOCALDELETED_CONTACT(_item_id, _addr,_ret_value)           \
{                                                                       \
    if ((MMINV_PHONEBOOK_LOCALDELETEO365_LAST_ENTRY < (_item_id))                  \
    || (MMINV_PHONEBOOK_LOCALDELETEO365_FIRST_ENTRY > (_item_id)))                 \
    {                                                                   \
    SCI_TRACE_MID("mmipb_mnapi: MMINV_WRITE_LOCALDELETED item_id = %d",    \
    (_item_id));                                                        \
    _ret_value = FALSE;                                                 \
    }                                                                   \
                                                                        \
...
{                                                                       \
    _ret_value= (MN_RETURN_RESULT_E)MNNV_WriteItem(                     \
    (MMI_GetTrueNVItemId(item_id )),                                    \
    sizeof (MMIPB_CONTACT_MOSTUSED_T),                                  \
    (void *)(_addr)                                                     \
    );                                                                  \
...
{                                                                   \
    ...
    {                                                               \
    SCI_TRACE_MID("mmipb: MMINV_WRITE_GROUP FAILURE");              \
    }                                                               \
} 
#endif
...
/*****************************************************************************/
PUBLIC uint32  ReadPbFlagFromNV(uint32 item_id, uint32 defalutVal)
{
...
#if 0
/*****************************************************************************/
// 	Description : get system time and trans to uint32
//	Global resource dependence : 
//  Author:Dufei
//  return 
//	Note:获取系统时间并转换为uint32类型
/*****************************************************************************/
PUBLIC uint32  GetPhoneTimeToUint32()
{
...
//PB_Sync start, add by dufei ,20150714
/*****************************************************************************/
...
//	Note: 向NV中写入flag
/*****************************************************************************/
PUBLIC BOOLEAN  WritePbFlagToNV(uint32 item_id, uint32  value)
{
    BOOLEAN ret = FALSE;
    ERR_MNDATAMAG_CODE_E  nv_result   = ERR_MNDATAMAG_NO_ERR;   
...
#if 0
/*****************************************************************************/
//  Description :清空所有本地删除记录在NV中的O365Id的数据
//  Global resource dependence :
//  Author: 
//  Note: 
/*****************************************************************************/
PUBLIC void MMIPB_ClearLocalDeletedO365IdFromNV()
{
	uint16 i=0;
	MMIPB_CONTACT_LOCALDELETED_T *pLocalDel = PNULL;
    	MN_RETURN_RESULT_E  nv_result   = MN_RETURN_FAILURE;
	pLocalDel = (MMIPB_CONTACT_LOCALDELETED_T *)SCI_ALLOCA(sizeof(MMIPB_CONTACT_LOCALDELETED_T));
    	SCI_MEMSET(pLocalDel, 0x00, sizeof(MMIPB_CONTACT_LOCALDELETED_T));
	for (i=0; i<MMINV_MAX_PHONEBOOK_RECORDS; i++){
		MMINV_WRITE_LOCALDELETED_CONTACT(
            			(PB_NV_ITEM_E)(MMINV_PHONEBOOK_LOCALDELETEO365_FIRST_ENTRY + i),
            			(void*)pLocalDel,
            			nv_result
            			);
	}
	SCI_FREE(pLocalDel);
	//设置删除记录数为0
	WritePbFlagToNV(MMINV_PHONEBOOK_LOCALDELETEDO365_NUMBER,0);
}

/*****************************************************************************/
//  Description 删除NV中手机删除的联系人记录通过O365Id查找
//  Global resource dependence :
//  Author: 
//  Note: 
/*****************************************************************************/
PUBLIC void MMIPB_RemoveLocalDeletedFromNVByO365Id(char *o365Id)
{
	uint16 i=0;
	uint32 iLocalDeletedNum = 0;
	MMIPB_CONTACT_LOCALDELETED_T *pLocalDel = PNULL;
	MMIPB_CONTACT_LOCALDELETED_T *pLocalDelIter = PNULL;
    	MN_RETURN_RESULT_E  nv_result   = MN_RETURN_FAILURE;
	pLocalDel = (MMIPB_CONTACT_LOCALDELETED_T *)SCI_ALLOCA(sizeof(MMIPB_CONTACT_LOCALDELETED_T));
    	SCI_MEMSET(pLocalDel, 0x00, sizeof(MMIPB_CONTACT_LOCALDELETED_T));
	for (i=0; i<MMINV_MAX_PHONEBOOK_RECORDS; i++){
		pLocalDelIter = (MMIPB_CONTACT_LOCALDELETED_T *)SCI_ALLOCA(sizeof(MMIPB_CONTACT_LOCALDELETED_T));
		if (pLocalDelIter != PNULL){
			SCI_MEMSET(pLocalDelIter, 0x00, sizeof(MMIPB_CONTACT_LOCALDELETED_T));
			if (MMIPB_GetLocalDeleted(i+1, pLocalDelIter)==MMIPB_ERROR_SUCCESS){
				if (MMIAPICOM_Wstrcmp(pLocalDelIter->o365_id, o365Id)==0){
					MMINV_WRITE_LOCALDELETED_CONTACT(
            					(PB_NV_ITEM_E)(MMINV_PHONEBOOK_LOCALDELETEO365_FIRST_ENTRY + i),
            					(void*)pLocalDel,
            					nv_result
					...
					//更新NV中删除记录的个数
					iLocalDeletedNum = ReadPbFlagFromNV(MMINV_PHONEBOOK_LOCALDELETEDO365_NUMBER, 0);
					if (iLocalDeletedNum > 0)
						WritePbFlagToNV(MMINV_PHONEBOOK_LOCALDELETEDO365_NUMBER, iLocalDeletedNum-1);
				}
			}
			SCI_FREE(pLocalDelIter);
		}
	}
	SCI_FREE(pLocalDel);
}
...
/*****************************************************************************/
//  Description :通过位置获取NV中本地删除的具有O365Id的记录
//  Global resource dependence :
//  Author: 
//  Note: pos从0开始到MMINV_MAX_PHONEBOOK_RECORDS-1
/*****************************************************************************/
PUBLIC MMIPB_ERROR_E MMIPB_GetLocalDeleted(uint16 entry_id,
                                            MMIPB_CONTACT_LOCALDELETED_T *pLocalDel//[OUT]
                                            )
{
    MN_RETURN_RESULT_E  nv_result   = MN_RETURN_FAILURE;
    MMIPB_ERROR_E       ret = MMIPB_ERROR_ERROR;
    ...
    if(entry_id <= MMINV_MAX_PHONEBOOK_RECORDS)
    {
        MMINV_READ_LOCALDELETED_CONTACT(
            			(PB_NV_ITEM_E)(MMINV_PHONEBOOK_LOCALDELETEO365_FIRST_ENTRY+entry_id -1),
            			(void*)pLocalDel,
            			nv_result
            			);
    }
                                             ...
                                             )
{
    ...
    if(contact_ptr != PNULL)
    {
        SCI_MEMSET(contact_ptr, 0x00, sizeof(MMIPB_CONTACT_PHONE_T));
        {
            ...
            SCI_TRACE_LOW("[PBSYNC] MMIPB_DeletePhoneContact check 1");
        }
        SCI_FREE(contact_ptr);
    }
    SCI_TRACE_LOW("[PBSYNC] MMIPB_DeletePhoneContact check 2");
    return ret;
}
#ifdef MMIPB_MOST_USED_SUPPORT
/*****************************************************************************/
...
//ARM下，获取3G下扩展号码长度
            ...
            if(PHONEBOOK_MSISDN_STORAGE == storage)//bug1255688, 1255805
            {
                phonebook_info.max_record_num   = 1;
                phonebook_info.used_record_num  = 1;
                SCI_TRACE_LOW("[MMIPB] MMIPB_GetContactStorage: storage is MSISDN");
            }
            else
#endif
            {
                           ...
                           )
{
     ...
     if(dual_sys < MMI_DUAL_SYS_MAX && name_ptr != PNULL)
     {
        ...
        // modified by Tong Yongli. According to Coverity CID 16171
#ifndef PB_SUPPORT_LOW_MEMORY
