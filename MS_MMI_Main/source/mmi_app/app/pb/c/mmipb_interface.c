...
PUBLIC void MMIPB_CreateEvent(void)
{
    ...
    MMIPB_CreateOnlySimDataListSemphore();
...
#ifdef MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT
        MMIPB_ReadUSIMAddtionalFromOnlySimContactList(access_point.contact.contact_id,
...
{
#ifdef MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT
   return MMIPB_SaveAddtionToOnlySimList(contact_id, storage_id, contact_ptr);
#else /*MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT*/
...
#ifdef MMIPB_SYNC_WITH_O365
/*****************************************************************************/
// 	Description : get system time and trans to UTC time
//	Global resource dependence : 
//  Author:Dufei
//  return 
//	Note:
/*****************************************************************************/
PUBLIC uint32 GetPhoneTimeToUtcTime(void)
{
	SCI_TM_T local_time = {0};
    SCI_DATE_T sys_date = {0};
    SCI_TIME_T sys_time = {0};
    uint32 utcTime = 0;
	float time_zone = 0.0f;
	float delta_seconds = 0.0f;

    TM_GetSysDate(&sys_date);
    TM_GetSysTime(&sys_time);
	
    local_time.tm_year=sys_date.year;
    local_time.tm_mday=sys_date.mday;
    local_time.tm_mon=sys_date.mon;
    
    local_time.tm_hour=sys_time.hour;
    local_time.tm_min=sys_time.min;
    local_time.tm_sec = sys_time.sec;

    utcTime = MMIAPICOM_Tm2Second(
                      local_time.tm_sec,
                      local_time.tm_min,
                      local_time.tm_hour,
                      local_time.tm_mday,
	...
	delta_seconds = time_zone * (float)3600;
	utcTime -= (uint32)(delta_seconds);

	SCI_TRACE_LOW(" [MMIPBSYNC] GetPhoneTimeToUtcTime, utcTime=%d", utcTime);
                            ...
                            )
{
	...
	{
		((MMIPB_CONTACT_T*)contact_ptr)->phone_modifiedTime = GetPhoneTimeToUtcTime();
	}
	else 
	{
		((MMIPB_CONTACT_T*)contact_ptr)->phone_modifiedTime = ((MMIPB_CONTACT_T*)contact_ptr)->o365_modifiedTime;
	}
#if 0
		if(0 != SCI_STRLEN(phonecontact_ptr->o365_id))
		{//o365id  exist, so this is modify operation.
			//shiwei 2015-7-30
			...
			//shiwei 2015-07-10
			MMIPB_OPC_E 	   modify_type = MMIPB_OPC_ADD; 
    ...
    /*sprintf(phoneModifiedTime, "%04d-%02d-%02dT%02d:%02d:%02dZ",
        ...
        file_time.tm_min, file_time.tm_sec); shiwei 2015-7-14*/
			...
			//shiwei 2015-7-30
			((MMIPB_CONTACT_T*)contact_ptr)->phone_modifiedTime =  phoneModifiedTime;
		}
	}
#endif
#endif
                ...
                if(MMIPB_ERROR_SUCCESS == MMIPB_ContactToPhoneContact(contact_ptr, phonecontact_ptr))
                {
                    ...
                    if(result == MMIPB_ERROR_SUCCESS)
                    {
                        ...
                        if (MMIPB_SYNC_TYPE_SUSPEND == MMIPB_SYNC_GetContactsSyncType())
                        {
                            if(0 != SCI_STRLEN(phonecontact_ptr->o365_id))
                            {//o365id  exist, so this is modify operation.
                                modify_type = MMIPB_OPC_EDIT;
                                //record this contact id in phone update list
                                SCI_TRACE_LOW("[PBSYNC] Record PATCH contact id = %d in phone_update_list", access_point.contact.contact_id);
                                MMIPB_SYNC_Phone_Update_List_Append(phonecontact_ptr->o365_id,
                                                            phonecontact_ptr->o365_modifiedTime,
                                                            phonecontact_ptr->phone_modifiedTime,
                                                            modify_type
                                                          );
                                //record this contact id in a backup file in case the power fails.
                                //write it in phone contacts update file
                            }
                        }
#endif
#endif
                            ...
                            )
{
    ...
    //char O365Id[MMIPB_SYNC_O365_ID_LEN] ="" ;
    MMIPB_CONTACT_LIST_NODE_T *node = PNULL;//node in pb_contacts_list

    MMIPB_ERROR_E error;
    uint16 index;
    MMIPB_SYNC_PHONE_UPDATE_LIST_T item;
#endif
...
#endif
#ifdef MMIPB_SYNC_WITH_O365
        SCI_TRACE_LOW("[PBSYNC] Record DELETE check 1");
        if(result == MMIPB_ERROR_SUCCESS)
        {
            SCI_TRACE_LOW("[PBSYNC] Record DELETE check 2");
            if ((MMIPB_SYNC_TYPE_SUSPEND == MMIPB_SYNC_GetContactsSyncType()) &&
				(TRUE == MMIPB_SYNC_NV_Is_MSA_Sign_In()))
            {
                SCI_TRACE_LOW("[PBSYNC] Record DELETE ContactsSyncType = %d", MMIPB_SYNC_GetContactsSyncType());
                if(MMIPB_SYNC_GetListNodeByPhoneId(access_point.contact.contact_id, 
                                                access_point.contact.storage_id,
                                                &node//[OUT]
                                                ))
                {//o365id  exist, so this is deleting operation.
                    SCI_TRACE_LOW("[PBSYNC] Record DELETE node found = %d");
                    if(0 != SCI_STRLEN(node->o365_id))
                    {
                        //modify_type = MMIPB_OPC_DELETE;
                        //record this contact id in phone update list
                        //record this contact id in a backup file in case the power fails.
                        //write it in phone contacts update file
                        SCI_TRACE_LOW("[PBSYNC] Record DELETE phone id = %d in phone_update_list", access_point.contact.contact_id);
						#if 0
                        MMIPB_SYNC_Phone_Update_List_Append(node->o365_id,
                                                                node->o365_modifiedTime,
                                                                phoneModifiedTime,
                                                                modify_type
                                                              );
						#endif
						//PB_SYNC FS wk
						SCI_MEMSET(&item, 0x00, sizeof(MMIPB_SYNC_PHONE_UPDATE_LIST_T));
						SCI_MEMCPY(&item.pb_data_change.o365_id, node->o365_id, sizeof(item.pb_data_change.o365_id));
						item.pb_data_change.o365_modifiedTime = node->o365_modifiedTime;
						item.pb_data_change.phone_modifiedTime = GetPhoneTimeToUtcTime();
						item.pb_data_change.modify_type = MMIPB_OPC_DELETE;
						error = MMIPB_SYNC_Write_Phone_Update_Item(&index, &item);
                    }
                    //MMIPB_SYNC_SetCurNodeToHead();//test 20150730
                }
            }
        }
#endif
    ...
    else if(dual_sys < MMI_DUAL_SYS_MAX)
    {
        //delete sim contact
#ifdef CMCC_VCARD_SUPPORT
SCI_TRACE_LOW("[PBSYNC] ???????MMIPB_IsUsim???????");
        if(MMIPB_IsUsim(access_point.contact.storage_id))
        {
            //usim
...
#ifdef CMCC_VCARD_SUPPORT // wang.qinggang Coverity for 26953
    #ifndef MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT
    sim_number_max_count += MMI_AAS_PHONE_NUM*MMIPB_SIM_MAX_RECORD_NUM;
    #else /*MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT*/
    sim_number_max_count = 0;
    #endif /*MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT*/
#elif defined(MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT)
    sim_number_max_count = 0;
#endif
                               ...
                               )
{
   ...
   if(is_valid)
   {
       if(handle_node.data_ptr)
       {       
           SCI_PutMutex(s_pb_asyn_mutex_p);
       
           SCI_TRACE_LOW("[MMIPB]MMIPB_WriteAsyn handle_node.data_ptr is not NULL !");
   ...
   if(MMIPB_IsSimRecord(storage_id) && !MMIPB_IsSIMContact(storage_id))
    ...
    else
    {
       ...
       if(MMIPB_IsSimRecord(storage_id))
       {
           ...
           count = storage_info.used_record_num;
       }
       else
       #endif /*MMI_PB_DYNAMIC_LOAD_SIM_CONTACT_SUPPORT*/
       {
...
)
{
	...
	if(type_str != PNULL)
	{
		...
		if(MMIPB_IsSimRecord(storage_id))
		{
			MMI_GetLabelTextByLang(STXT_NUMBER_TYPE_MOBILE,type_str);
			return;
		}
		...
		case MMIPB_ADN_NUMBER:
			default_text_id = STXT_NUMBER_TYPE_MOBILE;//TXT_PB_MOBLIE;			
            break;
		...
		case MMIPB_HOME_NUMBER:	
...
#endif
			{
				...
				default_text_id = STXT_NUMBER_TYPE_HOME;//TXT_PB_HOME;
				#else
				default_text_id = STXT_NUMBER_TYPE_OFFICE;//TXT_PB_WORK;
				#endif
			}
			break;
...
//  Note: 
/*****************************************************************************/
PUBLIC MMIPB_CONTACT_BASE_INFO_T MMIPB_GetOnlySimContactBaseInfo(uint16 index)
{
    MMIPB_CONTACT_BASE_INFO_T node = {0};

    MMIPB_ReadOnlySimContactSimpleInfo(index, &node);
...
//  Note:
/*****************************************************************************/
PUBLIC MMIPB_ERROR_E MMIPB_InsertContactToOnlySimList( 
                                               MMIPB_CONTACT_T *contact_ptr
                                               )
{
    MMIPB_ERROR_E result = MMIPB_ERROR_ERROR;
    ...
    if(contact_ptr != PNULL)
    {
            result = MMIPB_InsertOnlySimContactListNode((MMIPB_CONTACT_T *)contact_ptr);
    }
...
//  Note:
/*****************************************************************************/
PUBLIC MMIPB_ERROR_E MMIPB_RemoveContactFromOnlySimList(
                            uint16 contact_id,
                            uint16 storage_id
                            )
{
    ...
    {
        //remove from contact list
        result = MMIPB_DeleteOnlySimContactListNode(contact_id, storage_id);        
    }
    return result;
}
...
//  Note: 
/*****************************************************************************/
PUBLIC MMIPB_ERROR_E MMIPB_FindFirstOnlySimContact(
                                            uint32  group_mask,//[IN]
                                            uint16  storage_id, //要查找的联系人类型
                                            uint16  *cur_index_ptr,//[OUT]
                                            MMIPB_CONTACT_VALID_NUM_MAIL_INDEX_T *valid_index_array,//[OUT]
        ...
        *handle_ptr = MMIPB_CreateHandle(MMIPB_OPC_FIND, 0);
        if(*handle_ptr != 0)
        {
            ...
            if(MMIPB_GetHandleContent(*handle_ptr, &handle_node))
            {
                ...
                //find from first one
                handle_node.access_point.list_index = 0;
                result = MMIPB_FindOnlySimContact(handle_node.access_point.list_index, 
                                          group_mask,
                                          storage_id,
                                          &index,
                                          valid_index_array,
...
//  Note: 
/*****************************************************************************/
PUBLIC MMIPB_ERROR_E MMIPB_FindNextOnlySimContact( MMIPB_HANDLE_T handle, 
                                            uint32  group_mask,//[IN]
                                            uint16  storage_id, //要查找的联系人类型
                                            uint16  *cur_index_ptr,//[OUT]
                                            MMIPB_CONTACT_VALID_NUM_MAIL_INDEX_T *valid_index_array,//[OUT]
                                           ...
                                           )
{
    ...
    if(MMIPB_GetHandleContent(handle, &handle_node))
    {
        result = MMIPB_FindOnlySimContact(
            handle_node.access_point.list_index,   
            group_mask, 
            storage_id,
            &index,//[OUT]
