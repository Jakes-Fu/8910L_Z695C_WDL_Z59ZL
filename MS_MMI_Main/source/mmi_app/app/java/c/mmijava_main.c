...
#else
            txt_id = TXT_NO_SD_CARD_ALERT;//nor只提示sd卡不存在
#endif
        }
        else
        {
											...
											)
{
    ...
    case MSG_OPEN_WINDOW:        
        ...
        //GUIWIN_SetSoftkeyTextId(win_id, STXT_SOFTKEY_OK_MK, COMMON_TXT_NULL, STXT_RETURN, FALSE); //Softbutton
        break;
                                           ...
                                           )
{
    ...
    if (win_type == PAUSE_WIN) {
        ...
        MMIAPISET_AppendListItemByTextId(TXT_COMMON_COMMON_END, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
        item_num ++;
        ...
        if ((javacall_lifecycle_get_vm_state() == JAVACALL_LIFECYCLE_MIDLET_PAUSED)
            || (javacall_lifecycle_get_vm_state() == JAVACALL_LIFECYCLE_MIDLET_PAUSING)) {
            MMIAPISET_AppendListItemByTextId(TXT_COMMON_COMMON_CONTINUE, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
            item_num ++;
        }
