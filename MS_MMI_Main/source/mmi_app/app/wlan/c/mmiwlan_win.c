...
#else
#define MMIWIFI_LIST_PERMANENT_ITEM_NUM     (1)
#endif
//只通过ASCII认证
...
WINDOW_TABLE(MMIWIFI_EAP_ACCOUNT_WIN_TAB) = 
{
    ...
    WIN_SOFTKEY(STXT_SOFTKEY_OK_MK, TXT_NULL,TXT_NULL),
...
WINDOW_TABLE(MMIWIFI_WLAN_SETTING_WIN_TAB) = //WLAN设置
{
    WIN_FUNC((uint32)WifiHandleWlanSettingWin),
        ...
        WIN_SOFTKEY(STXT_SOFTKEY_SELECT_MK,TXT_NULL,STXT_RETURN),
        END_WIN
};
...
WINDOW_TABLE(MMIWIFI_AUTH_SETTING_WAPI_CERTIFICATE_FOR_CONNECT_WIN_TAB) = 
{
    WIN_FUNC((uint32)WifiHandleAuthSettingWAPICERTIFICATEForConnectWinMsg),    
        ...
        WIN_SOFTKEY(STXT_SOFTKEY_SAVE_MK,TXT_NULL,STXT_RETURN),
#else
        WIN_SOFTKEY(STXT_SOFTKEY_SAVE_MK,TXT_WIFI_SELECT_PATH,STXT_RETURN),
#endif /* MMI_PDA_SUPPORT */
        END_WIN
};
...
#ifdef MMI_PDA_SUPPORT
    uint32          wifi_menu[][3] ={
                                                        {MMIWIFI_POPMENU_ID_ADD, STXT_SOFTKEY_ADD_MK, IMAGE_COMMON_OPTMENU_ADD},
                                                        {MMIWIFI_POPMENU_ID_CLEAR, TXT_COMMON_CLEAR, IMAGE_COMMON_OPTMENU_REMOVE_ALL}
	};
#else
    uint32          wifi_menu[3][2] ={
                                                        {MMIWIFI_POPMENU_ID_ADD, STXT_SOFTKEY_ADD_MK},
...
PUBLIC MMI_IMAGE_ID_T WifiGetConnectedRssiImage(int32 rssi,BOOLEAN isEncrypt )
{
    ...
    {
        image = IMAGE_WIFI_CONNECT_RSSI0;
    }
    else if (rssi <= MMIWIFI_RSSI_80)
    {
        image = IMAGE_WIFI_CONNECT_RSSI1;
    }
    else 
    {
        image = IMAGE_WIFI_CONNECT_RSSI2;
    }
...
LOCAL void WifiAppendListItem(MMI_CTRL_ID_T ctrl_id, MMIWIFI_PROFILE_T *profile_ptr)
{
    ...
    wchar               item_text2[GUIRICHTEXT_TEXT_MAX_LEN + 1] = {0};
	...
	//set to richtext
    item_data.item_content[0].item_data_type = GUIITEM_DATA_TEXT_BUFFER;
    item_data.item_content[0].item_data.text_buffer.wstr_len = (uint16)MMIAPICOM_Wstrlen(item_text);
    item_data.item_content[0].item_data.text_buffer.wstr_ptr = (wchar *)item_text;
#endif
    ...
    if (profile_ptr->is_on_air)
    {
...
#ifdef WIFI_SUPPORT_UNISOC_RX
		SCI_MEMSET(item_text2, 0, sizeof(item_text2));
		...
		MMI_STRNTOWSTR(uint16_str, MMIWIFI_DETAIL_TEMP_LEN, (uint8 *)temp_buf, len, MIN(len,MMIWIFI_DETAIL_TEMP_LEN));
		//cat
		MMIAPICOM_Wstrncpy(item_text2,uint16_str,GUIRICHTEXT_TEXT_MAX_LEN);

        item_data.item_content[3].item_data_type    = GUIITEM_DATA_TEXT_BUFFER;		
		item_data.item_content[3].item_data.text_buffer.wstr_len = (uint16)MMIAPICOM_Wstrlen(item_text2);
		item_data.item_content[3].item_data.text_buffer.wstr_ptr = (wchar *)item_text2;
#else
...
#ifdef WIFI_SUPPORT_UNISOC_RX
	GUIWIN_SetSoftkeyTextId(MMIWIFI_LIST_WIN_ID,  TXT_NULL, TXT_NULL, STXT_RETURN, FALSE);
#endif
    ...
    case MSG_FULL_PAINT:
        {
            ...
            if (MMIWIFI_LABEL_WAPI_UNINSTALL_CTRL_ID != MMK_GetActiveCtrlId(win_id))
            {
                GUIWIN_SetSoftkeyTextId(win_id,STXT_SOFTKEY_SAVE_MK, TXT_WIFI_SELECT_PATH, STXT_RETURN, FALSE);
            }
            else
            {
                GUIWIN_SetSoftkeyTextId(win_id,STXT_SOFTKEY_SAVE_MK, TXT_WLAN_UNINSTALL, STXT_RETURN, FALSE);
            }
#endif /* MMI_GUI_STYLE_TYPICAL */
...
PUBLIC void MMIAPIWIFI_Init(void)
{
    ...
    // Initialize Spreadtrum supplicant Adapter layer
#ifndef WIN32
    WIFISUPP_Init();
#endif    
...
PUBLIC void MMIAPIWIFI_Reset(void)
{
    ...
    MMIWIFI_SetWlanAutoScanStatus(0);
...
LOCAL void  WifiCreateSearchMenu(MMI_WIN_ID_T win_id, MMI_CTRL_ID_T ctrl_id)
{
    MMI_TEXT_ID_T auto_scan_index[] = {STR_POC_ILIST_OFF, TXT_WIFI_PER_1_MIN, TXT_WIFI_PER_2_MIN, TXT_WIFI_PER_5_MIN, TXT_WIFI_PER_10_MIN};
...
#endif // MMI_WLAN_TAB_STYLE
    MMI_TEXT_ID_T           on_index[] = {STR_POC_ILIST_OFF, TXT_COMMON_ENABLE_ON};
...
LOCAL void WifiSetWlanWindow(void)
{
    ...
    MMI_TEXT_ID_T               wlan_index[2] = {STR_POC_ILIST_OFF,TXT_COMMON_ENABLE_ON};
    MMI_TEXT_ID_T               auto_connect_index[2] = {STR_POC_ILIST_OFF,TXT_COMMON_ENABLE_ON};
    MMI_TEXT_ID_T               auto_scan_index[5] = {STR_POC_ILIST_OFF,TXT_WIFI_PER_1_MIN,TXT_WIFI_PER_2_MIN,TXT_WIFI_PER_5_MIN,TXT_WIFI_PER_10_MIN};                                                                                //自动搜索:关闭，每1分钟，每2分钟，每5分钟，每10分钟，                                                                               
#ifdef WLAN_RESELECT_SUPPORT
...
LOCAL void WifiListAppendPermanentItem(MMI_CTRL_ID_T ctrl_id)
{
    ...
    {
        MMIRES_GetText(STR_POC_ILIST_OFF, PNULL, &string2);
    }
