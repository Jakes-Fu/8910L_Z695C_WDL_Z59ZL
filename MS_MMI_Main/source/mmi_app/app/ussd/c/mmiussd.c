                            ...
                            )
{
    ...
    case MN_SS_ERROR_INVAILD_DEFLECTED_NUMBER:
        *error_code_text_ptr = (MMI_TEXT_ID_T)STR_INVALID_NUM_EXT01;
        break;
                                ...
                                )
{
    ...
    if (!MMK_IsOpenWin(win_id)) 
    {
...

        {
            ...
            //bug 714785. begin
            softkey_data.rightsoft_id = TXT_COMMON_COMMON_BACK;
            //bug 714785. end
#endif
    ...
    //Set title
    MMI_GetLabelTextByLang(STR_ILIST_PUSH_NOTIF_EXT01, &title);
...
LOCAL void OpenUssdInputWin(void)
{
    ...
    //bug 714785. begin
    GUIWIN_SetTitleTextId(win_id, (MMI_TEXT_ID_T)STXT_SOFTKEY_REPLY, FALSE);
    //bug 714785. end
...
#else
    GUIEDIT_SetIm(MMIUSSD_INPUT_EDITBOX_CTRL_ID, GUIIM_TYPE_ENGLISH|GUIIM_TYPE_ABC|GUIIM_TYPE_DIGITAL, GUIIM_TYPE_DIGITAL);/*lint !e655*/
#endif
...
//        SCI_TRACE_LOW("mmiussd.c ParseUssdData: ussd_data_ptr->str_len = %d", ussd_data_ptr->str_len);
    ...
    if((ussd_data_ptr->data_code_scheme &0xf0)==0) 
    {
        SCI_TraceLow("ParseUssdData: change data_code_scheme  to MN_SS_DEFAULT_DCS_VALUE");
        ussd_data_ptr->data_code_scheme =MN_SS_DEFAULT_DCS_VALUE;
    }
    ...
    case MN_SS_UTF8_NO_CLASS_DCS:
        ...
        else if (MN_SS_UTF8_NO_CLASS_DCS == ussd_data_ptr->data_code_scheme)
        {
            len = ussd_data_ptr->str_len;

            len = MMIAPICOM_UTF8toUnicode(
                                            (uint8 *)ussd_data_ptr->ussd_str,
                                            ussd_data_ptr->str_len,
                                            ussd_str,
                                            MN_SS_MAX_USSD_STRING_LENGTH_BEFORE_ENCODE
                                            );
	    SCI_TraceLow("ParseUssdData utf8 len = %d",len);		
        }
    ...
    case MSG_CTL_RICHTEXT_SWITCHFOCUS_BY_TP:
        {
            ...
            {
            case GUIRICHTEXT_FOCUS_PHONENUM:
                GUIWIN_SetSoftkeyTextId(win_id, STXT_SOFTKEY_REPLY, TXT_COMMON_NUMBER, STXT_EXIT, is_need_update);
                break;

            case GUIRICHTEXT_FOCUS_URL:
                GUIWIN_SetSoftkeyTextId(win_id, STXT_SOFTKEY_REPLY, TXT_URL_ADRESS, STXT_EXIT, is_need_update);
                break;

            case GUIRICHTEXT_FOCUS_EMAIL:
                GUIWIN_SetSoftkeyTextId(win_id, STXT_SOFTKEY_REPLY, TXT_EMAIL_BOX, STXT_EXIT, is_need_update);
                break;

            default:
                GUIWIN_SetSoftkeyTextId(win_id , STXT_SOFTKEY_REPLY, TXT_NULL, STXT_EXIT, is_need_update);
                break;
            }
#endif
        }
