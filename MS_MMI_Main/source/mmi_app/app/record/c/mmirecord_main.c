...
PUBLIC void MMIRECORD_OpenRecordFileList(void)
{
    ...
    {
        SCI_TRACE_LOW("[RECORD] MMIRECORD_OpenRecordFileList file_dev = %d", file_dev);
    ...
    MMIAPIFMM_OpenExplorerRecord(full_path, full_path_len,PNULL,0,FALSE, file_suffix);
    SCI_FREE(full_path);
...
PUBLIC void  MMIRECORD_ShowErrorMsg(MMIFILE_ERROR_E fs_error)
{
    ...
    BOOLEAN  ret = TRUE;
   filedev =  MMIAPIFMM_GetFirstValidSD();
