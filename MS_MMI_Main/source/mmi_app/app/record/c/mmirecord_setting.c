...
LOCAL MMIRECORD_SETTING_INFO_T GetRecordSettingInfo(void)
{
    ...
    if(MN_RETURN_SUCCESS != return_value || !MMIAPIFMM_GetDeviceStatus(file_dev_ptr, file_dev_len))
    {
        ...
        record_setting_info.dev = MMIAPIFMM_GetValidateDevice(MMI_DEVICE_UDISK);
#else
        ...
        //else
        {
            //record_setting_info.file_formattype = MMIRECORD_WORK_FORMAT_AMR;
            record_setting_info.file_formattype = MMIRECORD_WORK_FORMAT_WAV;
        }	 
        MMINV_WRITE(MMINV_RECORD_SETTING, &record_setting_info);
    }
    SCI_TRACE_LOW("[RECORD] GetRecordSettingInfo dev = %d, file format:%d", record_setting_info.dev, record_setting_info.file_formattype);
