...
#endif
        //  更新状态栏的时间
        MMIAPICOM_StatusAreaSetDispTime();//update status bar
    ...
    case MMIPDP_ACTIVE_CNF:
        if(MMIPDP_RESULT_SUCC == msg_ptr->result)
        {
            ...
            if(0 == item_num)
            {
                ...
                if ((MMK_IsOpenWin(wait_win_id)) && (TRUE == s_is_user_update_time))
                {
                    MMIPUB_OpenAlertWarningWin(TXT_AUT_CURRENT_SERVICE_NO_TIME);
                    MMIAPIAUT_CloseManualUpdateWaitWin();
        ...
        else
        {
            ...
            else if (MMIPDP_RESULT_TIMEOUT == msg_ptr->result)
            {
                StartPdpActiveNextWaitTimer();
                break;
            }
...
PUBLIC BOOLEAN MMIAUT_GetSNTPUpdateFlag(void)
{
    SCI_TRACE_LOW("[szp]: MMIAUT_GetSNTPUpdateFlag s_is_user_update_time = %d",s_is_user_update_time);
    return s_is_user_update_time;
}
PUBLIC void MMIAUT_SetSNTPUpdateFlag(BOOLEAN is_user_update)
{
    SCI_TRACE_LOW("[szp]: MMIAUT_SetSNTPUpdateFlag s_is_user_update_time = %d",is_user_update);
    s_is_user_update_time = is_user_update;
    return;
}
/*****************************************************************************/
...
PUBLIC BOOLEAN MMIAUT_ActiveGPRS(MMICONNECTION_SETTING_DETAIL_ITEM_T detail_item, MMIPDP_MSG_CALLBACK pdp_callback)
{
    ...
    {    
        linkset_num = MMIAPICONNECTION_GetLinkSettingNum(detail_item.dual_sys);
        if (0 < linkset_num)
        {
        ...
        SCI_TRACE_LOW("[szp] dual_sys=%d,linkset_num=%d", detail_item.dual_sys, linkset_num);
    }
...
PUBLIC void MMIAPIAUT_UpdateNitzTime(MN_DUAL_SYS_E dual_sys)
{
    ...
    link_item.item_index = MMIAPICONNECTION_GetDefaultAPN(dual_sys);

    SCI_TRACE_LOW("MMIAPIAUT_UpdateNitzTime dual_sys = %d, sim_available_num = %d, GetLinkState = %d item_index=%d",
                  dual_sys, sim_available_num, MMIAPIPDP_GetLinkState(MMIAUT_GetModuleID()), link_item.item_index);
    ...
    if (MMIPDP_LINK_STATE_DEACTIVED == MMIAPIPDP_GetLinkState(MMIAUT_GetModuleID())
        ||MMIPDP_LINK_STATE_MAX == MMIAPIPDP_GetLinkState(MMIAUT_GetModuleID()))
    {            
        if (!MMIAUT_ActiveGPRS(link_item, HandleNetTimeServerPdpMsg))
        {
            StartPdpActiveNextWaitTimer();
        }
        s_active_sntp_pdp_count --;
    }
    else if (0 < s_active_sntp_pdp_count)
    {
        StartPdpActiveNextWaitTimer();
        SCI_TRACE_LOW("MMIAPIAUT_UpdateNitzTime s_active_sntp_pdp_count = %d",s_active_sntp_pdp_count);
    }
    else
    {
        SCI_TRACE_LOW("MMIAPIAUT_UpdateNitzTime Failed to update the time!!! s_active_sntp_pdp_count = %d",s_active_sntp_pdp_count);
    }
...
LOCAL void StartPdpActiveNextWaitTimer(void)
{
    ...
    s_active_next_pdp_timer = MMK_CreateTimerCallback(SNTP_TIMEOUT_20S, ActiveNextNitzPDP, PNULL, FALSE);
}
