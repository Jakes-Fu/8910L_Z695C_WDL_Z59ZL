...
WBXMLParser * g_wbxml_parser_ptr = PNULL;            //parser struct
...
LOCAL void StartParseWBXMLElement(void *user_data_ptr, 
                                  WBXMLTag *element_ptr, 
                                  WBXMLAttribute **atts_pptr, 
                                  BOOLEAN empty
                                  );
...
* Note:         
* ----------------------------------------------------------------------------*/ 
LOCAL void StartParseWBXMLElement(void *user_data_ptr, WBXMLTag *element_ptr, WBXMLAttribute **atts_pptr, BOOLEAN empty)
{   
...
* Note:         
* ----------------------------------------------------------------------------*/ 
LOCAL void EndParseWBXMLElement(void *user_data_ptr, WBXMLTag *element, BOOLEAN empty)
{
...
LOCAL BOOLEAN ParseOTAWbxml(OTA_MIME_TYPE_E mime_type, OTA_DATA_T  *ota_data_ptr)
{
    ...
    WBXMLLanguage language = WBXML_LANG_OTA_SETTINGS; 
...
PUBLIC BOOLEAN  MMIAPIOTA_ParseWBXML(const uint8  *push_data_ptr, uint32 push_data_len, MMIOTA_CBF provision_setting, uint8 *sender_ptr, MN_DUAL_SYS_E dual_sys)
{
    ...
    if (MIME_OTACONNECTIVITY == mime_type)
    {
        ...
        if((MMIAPISET_GetOTASecStatus(dual_sys)) && (sec<4) &&(sec>=0))        //if enable,don't handle msg with sec code 
            {
