...
#endif
LOCAL uint16  g_Show_push_id=0;//弹出浏览器消息提示的push的record id
/**--------------------------------------------------------------------------*
**                         STATIC DEFINITION                                *
**--------------------------------------------------------------------------*/
...
{  
    ...
    if (PNULL != sender_ptr)
    {
        ...
        if (0 < sender_sr_len && MMIAPICOM_GenPartyNumber(sender_ptr, sender_sr_len, &party_num))
        {
            ...
            black_bcd_num.number_len = (uint8)MIN(MMIPB_BCD_NUMBER_MAX_LEN -1, party_num.num_len);
...
LOCAL BOOLEAN  SaveOTASettingValue(MN_DUAL_SYS_E dual_sys, MMIOTA_SETTING_T *ota_setting_ptr)
{
    ...
    if(index == MMIAPICONNECTION_GetLinkSettingNum(dual_sys))
    {
        ...
        else
        {
            ...
            {
                //should use MMIOTA_SETTING_MAX_DNS_LEN instead of MMICONNECTION_MAX_DNS_LEN
                SCI_MEMCPY((char *)link_setting_info.dns,(char *)ota_setting_ptr->dnsaddress, MIN(ota_setting_ptr->dnsaddress_len, MMIOTA_SETTING_MAX_DNS_LEN));
            }            
            ...
            link_setting_info.visible = TRUE;
...
//                    else
            ...
            MMIPUB_CloseQuerytWin(PNULL); 
						...
						banner_info.text1_id = TXT_DELETED;
						MMIAPICOM_OpenBannerWin(MMK_GetFocusWinId(),banner_info); 
...
LOCAL BOOLEAN MMIOTA_DealWapPush(MMIOTA_SETTING_T  *mmi_setting_ptr, uint8 *sender_ptr, MN_DUAL_SYS_E dual_sys)
{
    ...
    {        
                ...
                for(i = 0; i < MMICMSBRW_MAX_ITEM_NUM; i++)
                {
                        ...
                        )
                    {
                        ...
                        if(i == g_Show_push_id)
                        {
                            ...
                            if(MMK_IsOpenWin(MMIOTA_EXTRACT_NUMBER_POP_WIN_ID))
                                MMK_CloseWin(MMIOTA_EXTRACT_NUMBER_POP_WIN_ID);
                            if(MMK_IsOpenWin(MMIOTA_EXTRACT_URL_POP_WIN_ID))
                                MMK_CloseWin(MMIOTA_EXTRACT_URL_POP_WIN_ID);                                
                            if(MMK_IsOpenWin(MMIOTA_EXTRACT_EMAIL_POP_WIN_ID))
                                MMK_CloseWin(MMIOTA_EXTRACT_EMAIL_POP_WIN_ID);
                        }
                        if(i == (uint16)order_id->record_id)
                        {
#ifdef    MMISMS_CHATMODE_ONLY_SUPPORT                        
                            if(MMK_IsOpenWin(MMISMS_PUSHMSG_MENUKEY_WIN_ID))
                                MMK_CloseWin(MMISMS_PUSHMSG_MENUKEY_WIN_ID);
#endif                            
        ...
        {//在idle直接阅读，其他界面提示，同时收件箱保存    
            ...
            {
                is_save = TRUE;
                is_alert = TRUE;//BUG 516300
                is_read_si = TRUE;//BUG 519610
                is_play_sound = TRUE;                
            }
            else
            {
    ...
    if (is_save)
    {
        ...
        if (MMIOTA_BRW_WAP_SL == mmi_setting_ptr->setting_type && MMIOTA_SL_EXECUTE_ACTION_CACHE == mmi_setting_ptr->wap_sl.execute_action)
        {
            SCI_TRACE_LOW("MMIOTA: MMIOTA_SL_EXECUTE_ACTION_CACHE");
            return result;
        }
...
/*wei.ren cr124504 add*/
    ...
    if (is_enter_broswer && MMIOTA_BRW_WAP_SL == mmi_setting_ptr->setting_type)
    {
        ...
        else if (MMISMS_SL_OPERATION_PROMPT == oper)
        {
            ...
            MMI_GetLabelTextByLang(STR_ONEDRIVE_ILIST_CONNECT_EXT01, &connect_string);
...
*/
            MMIPUB_OpenConfirmationDialogWinByTextPtrEx(
                                              MMK_GetFirstAppletHandle(),
                                               ...
                                               &prompt_string,         //text 1
                                               PNULL,         //text 2
                                               IMAGE_PUBWIN_QUERY,           //alert image id
                                               &prompt_win_id,        //win id,default is MMIPUB_ALERT_WIN_ID
                                               PNULL,  //win priority,default is WIN_ONE_LEVEL
                                               MMIPUB_SOFTKEY_DIALOG_OK_CLOSE,      //softkey style
                                               HandleSLPushPromptWinMsg,        //handle function
                                               (uint32)index           //user_data
                                               );
...
LOCAL MMI_RESULT_E HandleSLPushPromptWinMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param)
{
    ...
    {  
    case MSG_OPEN_WINDOW:
        GUIWIN_SetSoftkeyTextId(win_id, TXT_DRM_FORWARD_FBN_YES, TXT_NULL, TXT_DRM_FORWARD_FBN_NO, FALSE);
        break;
...
LOCAL void MMIOTA_InsertWapPushToSms(uint16 index)
{
    ...
    {
        SCI_TRACE_LOW("MMIAPIBRW_InsertWapPushToSms,index is invalid ,not save it");
        return;
    }
...
//             MMIPUB_SetWinSoftkey(win_id, TXT_NULL, STXT_EXIT, FALSE);
            MMIPUB_SetWinSoftkeyEx(win_id, TXT_NULL, STXT_EXIT, TXT_NULL, FALSE);
        }
        else
        {
//             MMIPUB_SetWinSoftkey(win_id, STXT_SOFTKEY_SELECT_MK, STXT_EXIT, FALSE);
            MMIPUB_SetWinSoftkeyEx(win_id, STXT_SOFTKEY_SELECT_MK, TXT_NULL, STXT_EXIT, FALSE);
        }
...
LOCAL void WapPushShowEntry(MMI_CTRL_ID_T ctrl_id, uint16 push_index)
{
    ...
    {
        MMI_GetLabelTextByLang(TXT_COMMUNICATION_MESSAGE, &temp_str);
...
//  Note:
/*****************************************************************************/
LOCAL void PushMsgGetPhoneNumber(uint8 *number_ptr, uint16 src_len, wchar *focus_num_ptr, uint16 dest_len)
{
    uint16 i = 0;
    uint16 new_len = 0;
...
//    Note:
/*****************************************************************************/
LOCAL void HandlePushMsgWinExtractNumberSaveContact(GUITEXT_TAG_INFO_T tag_info)
{
    ...
    PushMsgGetPhoneNumber(number_ptr, tag_info.str_len , tag_info.str_ptr, tag_info.str_len); //对-连接的号码处理
    // save to phonebook
                                           ...
                                           )
{
    ...
    case MSG_OPEN_WINDOW:
		...
		number.wstr_ptr = temp_arr;
		number.wstr_len = SCI_STRLEN(g_push_info_ptr->item_detail[i].sender);
        number.wstr_len = MIN(MMISMS_PBNAME_MAX_LEN, number.wstr_len);
        MMI_STRNTOWSTR(number.wstr_ptr,
                       number.wstr_len,
                       (uint8 *)g_push_info_ptr->item_detail[i].sender,
                       number.wstr_len,
                       number.wstr_len);
		MMIAPICOM_StatusAreaSetViewHeaderinfo(number.wstr_ptr, number.wstr_len);
    ...
    case MSG_CTL_RICHTEXT_SWITCHFOCUS_BY_TP:
        {
            ...
            case GUIRICHTEXT_FOCUS_URL:
                GUIWIN_SeSoftkeytButtonTextId(win_id, TXT_COMMON_COMMON_BACK, RIGHT_BUTTON, FALSE);
                GUIWIN_SeSoftkeytButtonTextId(win_id, STXT_SOFTKEY_GO_TO_MK, MIDDLE_BUTTON, TRUE);
                GUIWIN_SeSoftkeytButtonTextId(win_id, PNULL, LEFT_BUTTON, TRUE);
                break;

            case GUIRICHTEXT_FOCUS_EMAIL:
                GUIWIN_SeSoftkeytButtonTextId(win_id, TXT_COMMON_COMMON_BACK, RIGHT_BUTTON, FALSE);
                GUIWIN_SeSoftkeytButtonTextId(win_id, STR_LABEL_EMAIL_EXT01, MIDDLE_BUTTON, TRUE);
                GUIWIN_SeSoftkeytButtonTextId(win_id, STXT_SOFTKEY_MENU_MK, LEFT_BUTTON, TRUE);
                break;
    ...
    case MSG_APP_WEB:
        if (list_index < MMICMSBRW_MAX_ITEM_NUM)
        {
            ...
            case GUIRICHTEXT_FOCUS_PHONENUM:
                ...
                HandlePushMsgWinExtractNumberSaveContact(tag_info);
                if(PNULL != tag_info.str_ptr)
                {
...
/*****************************************************************************/
//  Description :Delete push message
//  Global resource dependence : 
//  Author:Yongli Tong
//  Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E HandlePushMessageQueryWinMsg(
                                            ...
                                            )
{
    MMI_RESULT_E  recode = MMI_RESULT_TRUE;
    uint16 list_index = 0;
    int16 i = 0, j = 0;//coverity 29070
    MMICMSBRW_ITEM_T *push_msg_list = PNULL;
    MMIPUB_INFO_T       *pubwin_info_ptr = PNULL;
    pubwin_info_ptr = (uint32)(MMIPUB_GetWinAddDataPtr(win_id));
    if(PNULL != pubwin_info_ptr)
    {	 
        list_index = pubwin_info_ptr->user_data;
    }
    i = list_index;
    push_msg_list = g_push_info_ptr;
    switch (msg_id)
                                           ...
                                           )
{
    ...
    case MSG_APP_WEB:
        ...
        case ID_CMSBRW_PUSHMSG_DELETE://删除       
            //MMIPUB_OpenQueryWinByTextId(STR_QUEST_DELETE_MARKED,IMAGE_PUBWIN_QUERY,PNULL,PNULL);//打开询问是否删除
            /*MMI_GetLabelTextByLang(STR_QUEST_DELETE_MARKED, &prompt_str);
           // MMIPUB_OpenConfirmationDialogByTextPtr(  PNULL,&prompt_str,PNULL, IMAGE_NULL,          
                                         &query_win_id, PNULL,MMIPUB_SOFTKEY_DIALOG_OK_CLOSE,  HandlePushMessageQueryWinMsg);*/
            MMIPUB_OpenAlertWinByTextIdEx(
                                            MMK_GetFirstAppletHandle(), 
                                            PNULL, 
                                            STR_QUEST_DELETE_MARKED,
                                            TXT_NULL, 
                                            IMAGE_NULL,
                                            &query_win_id,
                                            PNULL,
                                            MMIPUB_SOFTKEY_DIALOG_OK_CLOSE,
                                            HandlePushMessageQueryWinMsg,
                                            (uint32)list_index);
            MMK_CloseWin(MMICMSBRW_PUSHMSGMENU_WIN_ID);
            break;

        case ID_CMSBRW_PUSHMSG_FORWD://转发
        {
            MMI_STRING_T sms_msg = {0};
            wchar forward_content[OTA_MAX_CONTENT_LEN + OTA_MAX_URL_LEN + 1] = {0};
            if (0 != g_push_info_ptr->item_detail[list_index].content_len)
            {
                MMIAPICOM_Wstrncpy( 
                    forward_content,  
                    g_push_info_ptr->item_detail[list_index].content, 
                    g_push_info_ptr->item_detail[list_index].content_len 
                );
                sms_msg.wstr_len += g_push_info_ptr->item_detail[list_index].content_len;
            }
            if(0 != g_push_info_ptr->item_detail[list_index].url_len)
            {
                MMI_STRNTOWSTR(forward_content + sms_msg.wstr_len, OTA_MAX_URL_LEN,
					(uint8 *)g_push_info_ptr->item_detail[list_index].url, OTA_MAX_URL_LEN,
					g_push_info_ptr->item_detail[list_index].url_len);
                sms_msg.wstr_len += g_push_info_ptr->item_detail[list_index].url_len;
            }
            
            if(sms_msg.wstr_len > 0)
            {
                sms_msg.wstr_ptr = forward_content;
            }
