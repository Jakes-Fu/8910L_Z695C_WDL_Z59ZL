...
LOCAL BOOLEAN s_is_allow_invalid_number = FALSE;//记录是否支持包含非法字符的号码，默认为FALSE,主要供JavaPim使用
...
#if 1 //#ifdef MMIPB_SYNC_WITH_O365
LOCAL BOOLEAN s_num_TelTypes_flag[3]={0};
#endif /*MMIPB_SYNC_WITH_O365*/
...
// Note:
/*****************************************************************************/
LOCAL uint16 VCARD_GetSurname
(
    MMIPB_CONTACT_T  *contact_ptr,  //IN
    uint8 *surname_ptr,  //IN/OUT
    uint16 max_surname_length
)
{
	uint16 gbname_length = 0;
	
	if((PNULL == contact_ptr) || (PNULL == surname_ptr))
	{
		return gbname_length;
	}
	
	gbname_length = GUI_WstrToUTF8(surname_ptr,((MMIPB_NAME_MAX_LEN >> 1)* 3 + 1), contact_ptr->surname.wstr ,contact_ptr->surname.wstr_len);
	gbname_length = MIN(gbname_length, max_surname_length);
	if (1 <= gbname_length)
	{
	}
	else
...
// Note:
/*****************************************************************************/
LOCAL uint16 VCARD_Getname
(
    MMIPB_CONTACT_T  *contact_ptr,  //IN
    uint8 *name_ptr,  //IN/OUT
    uint16 max_name_length
)
{
	uint16 gbname_length = 0;
	
	if((PNULL == contact_ptr) || (PNULL == name_ptr))
	{
		return gbname_length;
	}
	gbname_length = GUI_WstrToUTF8(name_ptr,((MMIPB_NAME_MAX_LEN >> 1)* 3 + 1), contact_ptr->name.wstr ,contact_ptr->name.wstr_len);
	gbname_length = MIN(gbname_length, max_name_length);
	if (1 <= gbname_length)
	{
	}
	else
									 ...
									 )
{
	...
	{
		uint16 surnamelength = 0;
		uint16 namelength = 0;
		uint8 surname[((MMIPB_NAME_MAX_LEN >> 1)* 3 + 1)] = {0};
		uint8  name[((MMIPB_NAME_MAX_LEN >> 1)* 3 + 1)] = {0}; 
		
		surnamelength= VCARD_GetSurname(contact_ptr,
			surname,
			((MMIPB_NAME_MAX_LEN >> 1)* 3 + 1));   
		namelength= VCARD_Getname(contact_ptr,
			name,
			((MMIPB_NAME_MAX_LEN >> 1)* 3 + 1));   
		if(surnamelength > 0 && namelength > 0)
       	{
			strcat ((char*)vcard_ptr, "N;CHARSET=UTF-8;ENCODING=QUOTED-PRINTABLE:");
			vcard_length = strlen((char*)vcard_ptr);
			if(vcard_length >= vcard_max_len)
			...
			vf_vcard_qprint_encode(surname, strlen((char*)surname),(char*)(vcard_ptr+ vcard_length), (vcard_max_len - vcard_length), TRUE);
			strcat ((char*)vcard_ptr, ";");
			vcard_length = strlen((char*)vcard_ptr);
			vf_vcard_qprint_encode(name, strlen((char*)name),(char*)(vcard_ptr+ vcard_length), (vcard_max_len - vcard_length), TRUE);
			strcat ((char*)vcard_ptr, ";;;\r\n");
                                    ...
                                    wchar*file_name)
{
	...
	#ifdef MMI_PB_SURNAME_SUPPORT
	else if(0<contact_ptr->surname.wstr_len)
	{
		
		if (MMIAPICOM_IsIncludeInvalidChar(contact_ptr->surname.wstr,contact_ptr->surname.wstr_len))
		{
			name_len = MIN(strlen((char*)name_str), VCARD_NAME_MAX_LEN); /*lint !e666*/
			MMI_STRNTOWSTR((wchar *)file_name, VCARD_NAME_MAX_LEN, name_str, name_len, name_len);
		}
		...
		{
			valid_pos = MMIVC_GetFirstValidLetterPos(contact_ptr->surname.wstr);
			
			max_len = MIN(MMIAPICOM_Wstrlen(contact_ptr->surname.wstr),(VCARD_NAME_MAX_LEN - VCARD_DEFAULT_SUBFIX_LEN - MMIAPICOM_Wstrlen(MMIMULTIM_DIR_VCARD) -1));/*lint !e666*/
			max_len = MIN(max_len,MMIPB_NAME_MAX_LEN);/*lint !e831*/
			...
			{
				max_len = max_len - valid_pos;
				MMIAPICOM_Wstrncpy(file_name, &contact_ptr->surname.wstr[valid_pos], max_len);/*lint !e831*/
				MMI_STRNTOWSTR(&file_name[max_len], VCARD_DEFAULT_SUBFIX_LEN,(uint8*)p_ext, VCARD_DEFAULT_SUBFIX_LEN, VCARD_DEFAULT_SUBFIX_LEN);
				file_name[max_len + VCARD_DEFAULT_SUBFIX_LEN] = 0;
			}
		}
...
#if 1//#ifdef MMIPB_SYNC_WITH_O365
    SCI_MEMSET(s_num_TelTypes_flag, 0, sizeof(s_num_TelTypes_flag));
#endif /*MMIPB_SYNC_WITH_O365*/
    ...
    if (vf_get_property(&pProp, p_Object, VFGP_FIND, NULL, p_PropName, NULL))
    {
        do
        {
            ...
            if (pTel)
            {
                for(i=0;(pp_Array[i]!=PNULL);i++)
                {
                    for (n = 0;n < p_vprop->name.n_strings;n++)/*lint !e574 !e737*/
                    {
                        if(p_stristr(p_vprop->name.pp_strings[n],pp_Array[i]))
                        {
                            ...
                            #else /*MMIPB_SYNC_WITH_O365*/
                            if(cur_index<VCARD_MAX_NV_PHONE_NUM && !s_num_TelTypes_flag[i])
                            #endif /*MMIPB_SYNC_WITH_O365*/
                            {
                ...
                if((!is_set) && (VCARD_MAX_NV_PHONE_NUM > x_tel_num))  //bug 75026
                {
        ...
        for(i=0;i<x_tel_num;i++)
        {
            if(cur_index<VCARD_MAX_NV_PHONE_NUM)
            {
                #if 1//#ifdef MMIPB_SYNC_WITH_O365
                int32 j=0;
                for(j=0;j<VCARD_MAX_NV_PHONE_NUM;j++)
                {
                    if(!s_num_TelTypes_flag[j])
                    {
                        break;
                    }
                }
                if(j<VCARD_MAX_NV_PHONE_NUM)
                {
                    contact_ptr->num_type[cur_index] = s_num_TelephoneTypes[j];
                    s_num_TelTypes_flag[j] = TRUE;
                }
                else
                {
    ...
    for(i = j;i < length;i++)
    {
        switch( p_org_num[i] )
        {
          ...
          case 'X':
            ...
            //对非法字符的号码直接转换
          default:
            if(s_is_allow_invalid_number)
            {
                SCI_TRACE_LOW("[MMIVCARD]vc_cleanuptelnum s_is_allow_invalid_number is true");
        		p_dst_num[j] = p_org_num[i];
                break;
            }
            else
...
//                             SCI_FREE(photo_file);
				...
				//增加对name&tel都为空字符串的处理
                if((0 == contact_ptr->name.wstr_len)&&(0 == contact_ptr->number[0].number_len))
                {
                    const  char  *null_tel = "f0f0";
                    
                    contact_ptr->num_type[0] = MMIPB_UNKNOWN_NUMBER;
                    vc_setteltopb(null_tel, &contact_ptr->number[0]);
                }
                                    ...
                                    )
{
    ...
    SCI_TRACE_BUF("yanyan1029:MMIAPIVCARD_ContactToVcard enter:name: ", 
                    contact_ptr->name.wstr, 
                    contact_ptr->name.wstr_len*sizeof(wchar));

    SCI_TRACE_BUF("yanyan1029:MMIAPIVCARD_ContactToVcard enter:number: ", 
    ...
    if(data_len_ptr == PNULL || data_ptr == PNULL)
    {
    		return result; 
    }
    ...
    if (data_len_ptr != PNULL && data_ptr != PNULL)
    {
        SCI_TRACE_BUF("yanyan1029:MMIAPIVCARD_ContactToVcard out:data_ptr: ", 
                    data_ptr, *data_len_ptr*sizeof(uint32));   
    }
    
    return result;      
}
/****************************************************************************/
//  Description:set if allow number contains illegal characters's flag
//  Global resource dependence
//  Auther:
//  Note: default is FALSE, mainly used for java pim
/****************************************************************************/
PUBLIC void MMIAPIVCARD_SetAllowInvalidNumberFlag(void)
{
    SCI_TRACE_LOW("MMIAPIVCARD_SetAllowInvalidNumber: s_is_allow_invalid_number is true");
    s_is_allow_invalid_number = TRUE;
}
/****************************************************************************/
//  Description:clear allow number contains illegal characters's flag
//  Global resource dependence
//  Auther:
//  Note: 
/****************************************************************************/
PUBLIC void MMIAPIVCARD_ClearAllowInvalidNumberFlag(void)
{
    SCI_TRACE_LOW("MMIAPIVCARD_ClearAllowInvalidNumberFlag: s_is_allow_invalid_number is false");
    s_is_allow_invalid_number = FALSE;
}
/****************************************************************************/
//  Description:
//  Global resource dependence
 ...
 )
{
	...
	if((PNULL == p_name &&  PNULL == p_surname) || (PNULL == contact_ptr))
	{
		return;
	}
	...
	if(PNULL != p_name)
	{
		p_len    = &contact_ptr->name.wstr_len;
		p_text   = contact_ptr->name.wstr;
		p_datastr = (MMI_STRING_T *)p_name;
		if (p_len != NULL && p_text != PNULL && p_datastr->wstr_len > 0)
		{
			if (max_len < p_datastr->wstr_len)
			{
				*p_len = max_len;
	...
	if(PNULL != p_surname)
	{
		p_len    = &contact_ptr->surname.wstr_len;
		p_text   = contact_ptr->surname.wstr;
		p_datastr = (MMI_STRING_T *)p_surname;
		if (p_len != NULL && p_text != PNULL && p_datastr->wstr_len > 0)
		{
			if (max_len < p_datastr->wstr_len)
			{
...
LOCAL VCARD_ERROR_E vc_ObjectToContact(VF_OBJECT_T  *p_object,MMIPB_CONTACT_T *contact_ptr, MMIPB_CUSTOM_PHOTO_T *photo_ptr)
{
    ...
    VOBJECT_T * object=(VOBJECT_T *)p_object;
    #ifdef MMI_PB_SURNAME_SUPPORT
    BOOLEAN is_set_n=FALSE;
    MMI_STRING_T     surname_str = {NULL};
    #endif /*MMI_PB_SURNAME_SUPPORT*/
#ifdef MMI_CONTACT_PHOTO_SUPPORT
    char         *pBase64Prop = PNULL;
    MMIFILE_DEVICE_E    file_dev = MMI_DEVICE_SDCARD;
...
//                             SCI_FREE(photo_file);
                ...
                case VF_PROPERTY_N:
                    is_set_n = TRUE;
                    MMIVC_GetNValue(pProp, nValue, &tmp_str, &surname_str);
                    vc_setnametopb(MMIPB_NAME_MAX_LEN,(void *)&tmp_str, (void *)&surname_str,contact_ptr);
#ifdef MMI_CONTACT_PHOTO_SUPPORT
//                         if(PNULL != photo_file)
//                             SCI_FREE(photo_file);
                        photofilelen = (MMIAPICOM_Wstrlen(tmp_str.wstr_ptr)+MMIAPICOM_Wstrlen(photo_file_extension)+1) * sizeof(wchar);
