...
// Note: 
/*****************************************************************************/
PUBLIC void MMIVC_GetNValue(VF_PROP_T     *p_prop,
                           uint32         nValue,
                           MMI_STRING_T  *p_name,
                           MMI_STRING_T  *p_surname)
{
	char         PropNameValueBuffer[200] = "\0";
	char         PropSurNameValueBuffer[200] = "\0";
	uint16       buf_len = 0, buf_size = 0;
	uint16       n;
	char        *p_valuestr;
	uint16       value_len = 0;
	
	if((PNULL == p_prop) || (PNULL == p_name) || (PNULL == p_surname))
	{
		return;
	}
	//surname
	p_valuestr = vf_get_prop_value_string(p_prop, 0);
	if (NULL != p_valuestr)
	{
		buf_len   = p_strlen(PropSurNameValueBuffer);
		value_len = p_strlen(p_valuestr);
		if ((buf_len + value_len) < buf_size)
		{
			p_strcat(PropSurNameValueBuffer, p_valuestr);
		}
		else
		{
	...
	//name
	p_valuestr = vf_get_prop_value_string(p_prop, 1);
	if (NULL != p_valuestr)
	{
		buf_len   = p_strlen(PropNameValueBuffer);
		value_len = p_strlen(p_valuestr);
		if ((buf_len + value_len) < buf_size)
		{
			p_strcat(PropNameValueBuffer, p_valuestr);
		}
		else
		{
			if(buf_size > buf_len - 1 )
	...
	p_valuestr = PropSurNameValueBuffer;
	if ( VF_CHARSET_UTF8 == vc_getcharset(p_prop))
	{
		uint16    length;
		length = p_strlen(p_valuestr);
		//p_value->is_ucs2 = TRUE;
		p_surname->wstr_ptr = (wchar *)SCI_ALLOC_APP(length * sizeof(wchar));
		p_surname->wstr_len = GUI_UTF8ToWstr(p_surname->wstr_ptr,length,
			(const uint8*)p_valuestr, length);
	}
	else
	{
		MMIAPICOM_GBToSTRING((const uint8 *)p_valuestr, p_surname);
	}
	
	p_valuestr = PropNameValueBuffer;
	if ( VF_CHARSET_UTF8 == vc_getcharset(p_prop))
	{
		uint16    length;
		length = p_strlen(p_valuestr);
		//p_value->is_ucs2 = TRUE;
		p_name->wstr_ptr = (wchar *)SCI_ALLOC_APP(length * sizeof(wchar));
		p_name->wstr_len = GUI_UTF8ToWstr(p_name->wstr_ptr,length,
			(const uint8*)p_valuestr, length);
	}
	else
	{
		MMIAPICOM_GBToSTRING((const uint8 *)p_valuestr, p_name);
	}
	return;
}
...
// Added by sunhongzhe to handle datetime which format like 19960401T033000Z.
PUBLIC void MMIVC_UTCTimeToLocalTime(MMIVC_DATE_TIME_T *date_time)
{
    SCI_TM_T utc_time = {0};
    MMI_TM_T local_tm = {0};
    uint32 localTime = 0;
    ...
    if (date_time == PNULL) return;

    utc_time.tm_year= date_time->year;
    utc_time.tm_mday= date_time->mday;
    utc_time.tm_mon= date_time->mon;

    utc_time.tm_hour= date_time->hour;
    utc_time.tm_min= date_time->min;
    utc_time.tm_sec = date_time->sec;

    localTime = MMIAPICOM_Tm2Second(
                      utc_time.tm_sec,
                      utc_time.tm_min,
                      utc_time.tm_hour,
                      utc_time.tm_mday,
    ...
    localTime += (uint32)(delta_seconds);

    SCI_TRACE_LOW(" [MMIVCALENDAR] MMIVC_UTCTimeToLocalTime, time_zone=%f", time_zone);
    local_tm = MMIAPICOM_Second2Tm(localTime);

    date_time->year      = local_tm.tm_year;
    date_time->mday    = local_tm.tm_mday;
    date_time->mon      = local_tm.tm_mon;
    date_time->hour      = local_tm.tm_hour;
    date_time->min       = local_tm.tm_min;
    date_time->sec        = local_tm.tm_sec;
}

PUBLIC BOOLEAN MMIVC_UTCFormatStringToDateTime( char* string,
                                             MMIVC_DATE_TIME_T *date_time)
{
    BOOLEAN ret = TRUE;
    uint32 str_len = 0;
    ...
    if (PNULL!=string && PNULL!=date_time)
    {
        ...
        if (string[str_len - 1] != 'Z')
        {
            return MMIVC_StringToDateTime(string, date_time);
        }
        ...
        MMIVC_UTCTimeToLocalTime(date_time);
    }

    return ret;
}

PUBLIC void MMIVC_LocalTimeToUTCTime(MMIVC_DATE_TIME_T *date_time)
{
    ...
    local_time.tm_year= date_time->year;
    local_time.tm_mday= date_time->mday;
    local_time.tm_mon= date_time->mon;

    local_time.tm_hour= date_time->hour;
    local_time.tm_min= date_time->min;
    local_time.tm_sec = date_time->sec;
    ...
    SCI_TRACE_LOW(" [MMIVCALENDAR] MMIVC_LocalTimeToUTCTime, time_zone=%f", time_zone);
    utc_tm = MMIAPICOM_Second2Tm(utcTime);

    date_time->year      = utc_tm.tm_year;
    date_time->mday    = utc_tm.tm_mday;
    date_time->mon      = utc_tm.tm_mon;
    date_time->hour      = utc_tm.tm_hour;
    date_time->min       = utc_tm.tm_min;
    date_time->sec        = utc_tm.tm_sec;
}
