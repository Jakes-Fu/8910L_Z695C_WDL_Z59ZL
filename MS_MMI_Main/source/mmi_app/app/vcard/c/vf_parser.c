        ...
        case _VF_STATE_BASE64:
            {
                ...
                if (ISCRORNL(c))
                {
                    if (p_parse->p_b64buf)
                    {
                        uint16_t buf_len = p_strlen(p_parse->p_b64buf);
                        char *p_tail_b64buf = NULL;

                        ok = handle_base64_chars(p_parse, p_parse->p_b64buf, buf_len);

                        if(buf_len % 4)
                        {
                            uint16_t start_pos = (buf_len/4)*4;
                            uint16_t non_parse_len = buf_len - start_pos;

                            if(non_parse_len > 0)
                            {
                                ok = append_to_pointer(&p_tail_b64buf, NULL, &(p_parse->p_b64buf[start_pos]), non_parse_len);
                            }
                        }
                        vf_free(p_parse->p_b64buf);
                        p_parse->p_b64buf = NULL;

                        if(NULL != p_tail_b64buf)
                        {
                            buf_len = p_strlen(p_tail_b64buf);
                            ok = append_to_pointer(&(p_parse->p_b64buf), NULL, p_tail_b64buf, buf_len);
                            vf_free(p_tail_b64buf);
                            p_tail_b64buf = NULL;
                        }
                    }
                }
