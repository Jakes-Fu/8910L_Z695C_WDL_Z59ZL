...
LOCAL const MMIVCAL_FIELDINFO_T s_vcal_fields[] =
{
    ...
    // Added by sunhongzhe for location field.
    {MMIVCAL_PROPERTY_LOCATION, VFP_LOCATION},
    // End
                           ...
                           MMIVCALENDAR_INFO_T *vcal_info)
{
    ...
    case MMIVCAL_PROPERTY_CLASS:
        ...
        break;
    // Added by sunhongzhe for location field.
    case MMIVCAL_PROPERTY_LOCATION:
        vcal_info->location.wstr_len = p_datastr->wstr_len;
        if(vcal_info->location.wstr != PNULL)
        {
            SCI_FREE(vcal_info->location.wstr);
        }
        vcal_info->location.wstr = p_text;
        break;
    default:
        return FALSE;
    }
    ...
    if (vf_get_property(&pProp, pObject, VFGP_FIND, NULL, "*", NULL))
    {
        do
        {
            ...
            case MMIVCAL_PROPERTY_OBJECT_TYPE://对象类型，event or vtodo
                ...
                if(NULL == value)
                {
                    SCI_TRACE_LOW("MMIVCAL_ObjectToVcalInfo vf_get_prop_value_string NULL");
                    break;
                }
            ...
            case MMIVCAL_PROPERTY_DTSTART:
                ...
                if(PNULL!=date_time)
                {
                    ...
                    //Modified by sunhongzhe to handle datetime which format like 19960401T033000Z.
                    MMIVC_UTCFormatStringToDateTime(date_time, &vcal_info->dtstart.date_time);
                }
                break;
            case MMIVCAL_PROPERTY_DTEND:
                ...
                if(PNULL!=date_time)
                {
                    ...
                    //Modified by sunhongzhe to handle datetime which format like 19960401T033000Z.
                    MMIVC_UTCFormatStringToDateTime(date_time, &vcal_info->dtend.date_time);
                }
                break;
            case MMIVCAL_PROPERTY_COMPLETED:
                ...
                if(PNULL!=date_time)
                {
                    ...
                    //Modified by sunhongzhe to handle datetime which format like 19960401T033000Z.
                    MMIVC_UTCFormatStringToDateTime(date_time, &vcal_info->completed.date_time);
                }
                break;
            case MMIVCAL_PROPERTY_DUE:
                ...
                if(PNULL!=date_time)
                {
                    ...
                    //Modified by sunhongzhe to handle datetime which format like 19960401T033000Z.
                    MMIVC_UTCFormatStringToDateTime(date_time, &vcal_info->due.date_time);
                }
                break;
            ...
            case MMIVCAL_PROPERTY_AALARM:
                ...
                if(PNULL!=date_time)
                {
                    ...
                    //Modified by sunhongzhe to handle datetime which format like 19960401T033000Z.
                    MMIVC_UTCFormatStringToDateTime(date_time, &vcal_info->aalarm.run_date_time);
                }
                break;
            ...
            case MMIVCAL_PROPERTY_LOCATION:
                MMIVC_GetPropValue(pProp, nValue, &tmp_str);
                VcalSetDataToVcalInfo(MMIVCAL_PROPERTY_LOCATION,
                               (void *)&tmp_str, vcal_info);
                 MMIVC_freeData(&tmp_str);
                break;
            // End
										...
										MMIACC_SMART_EVENT_T *event_info_ptr)
{
    ...
    // Added by sunhongzhe for note and location fields.
    // Note
    if (0 < vcal_info_ptr->description.wstr_len)
    {
        content_len = MIN(MMIACC_EVENT_TEXT_MAX_LEN, vcal_info_ptr->description.wstr_len);
        SCI_MEMSET(event_info_ptr->event_content_info.w_text,
                0,
                sizeof(wchar)*(MMIACC_EVENT_TEXT_MAX_LEN+1));
        MMIAPICOM_Wstrncpy(event_info_ptr->event_content_info.w_text,
                vcal_info_ptr->description.wstr,
                content_len);
        event_info_ptr->event_content_info.w_text_length = content_len;
    }

    // location
    if (0 < vcal_info_ptr->location.wstr_len)
    {
        content_len = MIN(MMIACC_EVENT_TEXT_MAX_LEN, vcal_info_ptr->location.wstr_len);
        SCI_MEMSET(event_info_ptr->event_content_info.w_place,
                0,
                sizeof(wchar)*(MMIACC_EVENT_TEXT_MAX_LEN+1));
        MMIAPICOM_Wstrncpy(event_info_ptr->event_content_info.w_place,
                vcal_info_ptr->location.wstr,
                content_len);
        event_info_ptr->event_content_info.w_place_length = content_len;
    }
    // End
    ...
    if(NULL!=vcal_info_ptr->repeat_rule.rrule)
    {
        ...
        else
        {
            ...
            event_info_ptr->event_fast_info.fre_mode = ALM_MODE_ONCE;//ALM_MODE_NO;
        }
                                 ...
                                 uint16 *index_ptr)
{
    ...
    *index_ptr = 0;
    ...
    {
        if (MMIALM_AddOneEvent(&event_info, index_ptr))
        {
        }
        if (0 == (*index_ptr))
        {
...
LOCAL void DatetimeToVCalDateTime(uint16 year, uint8 month, uint8 mday,
    uint8 hour, uint8 min, uint8 sec, MMIVC_DATE_TIME_T *date_time)
{
    date_time->year      = year;
    date_time->mon      = month;
    date_time->mday    = mday;
												...
												MMIVCAL_OBJECT_TYPE_E type)
{
    ...
    strcat ((char*)vcal_ptr, MMIVCAL_COLON);
    /*
    if(MMIVCAL_STATUS_MAX==vcal_status)
    {
    ...
    // Added by sunhongzhe for note and location fields.
    // DESCRIPTION
    if(SCI_STRLEN((char*)vcal_ptr)+SCI_STRLEN(VFP_DESCRIPTION)+SCI_STRLEN(charset)>MMIVCAL_MAX_FILE_LEN)
    {
        goto Err_Exit;
    }
    strcat ((char*)vcal_ptr, VFP_DESCRIPTION);
    strcat ((char*)vcal_ptr, charset);
    ...
    temp_len = GUI_WstrToUTF8(temp_buf,
                ...
                MIN(event_ptr->event_content_info.w_text_length, MMIACC_EVENT_TEXT_MAX_LEN));
    vcal_length = SCI_STRLEN((char*)vcal_ptr);
    if(vcal_length + SCI_STRLEN(MMIVCAL_LFCR) >= MMIVCAL_MAX_FILE_LEN)
    {
        goto Err_Exit;
    ...
    // LOCATION
    if(SCI_STRLEN((char*)vcal_ptr)+SCI_STRLEN(VFP_LOCATION)+SCI_STRLEN(charset) > MMIVCAL_MAX_FILE_LEN)
    {
        goto Err_Exit;
    }
    strcat ((char*)vcal_ptr, VFP_LOCATION);
    strcat ((char*)vcal_ptr, charset);

    SCI_MEMSET(temp_buf, 0, MMIVCAL_SUMMARY_MAX_LEN);
    temp_len = GUI_WstrToUTF8(temp_buf,
                MMIVCAL_SUMMARY_MAX_LEN,
                event_ptr->event_content_info.w_place,
                MIN(event_ptr->event_content_info.w_place_length, MMIACC_EVENT_TEXT_MAX_LEN));
    vcal_length = SCI_STRLEN((char*)vcal_ptr);
    if(vcal_length + SCI_STRLEN(MMIVCAL_LFCR) >= MMIVCAL_MAX_FILE_LEN)
    {
        goto Err_Exit;
    ...
    if(MMIVCAL_OBJECT_TYPE_VEVENT== type)
    {
        ...
        // Added by sunhongzhe to use UTC time.
        SCI_MEMSET(&dt, 0, sizeof(dt));
        DatetimeToVCalDateTime(event_ptr->event_fast_info.start_year,
        ...
        // Added by sunhongzhe to use UTC time.
        SCI_MEMSET(&dt, 0, sizeof(dt));
        DatetimeToVCalDateTime(event_ptr->event_fast_info.end_year,
    ...
    else if(ALM_MODE_ONCE == event_ptr->event_fast_info.fre_mode)//一次
    {
        ...
        // Added by sunhongzhe to use UTC time.
        SCI_MEMSET(&dt, 0, sizeof(dt));
        DatetimeToVCalDateTime(event_ptr->event_fast_info.year,
