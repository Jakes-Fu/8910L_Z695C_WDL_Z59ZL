...
PUBLIC BOOLEAN  MMIAPISD_Init( void )
{
        ...
        SCI_TRACE_LOW( "MMIAPISD_Init line[%d] file[%s]", __LINE__, __FILE__ );

        error = SFS_RegisterDevice (MMIAPIFMM_GetDevicePath(MMI_DEVICE_UDISK), &format);

        // if (SFS_NO_ERROR == error || SFS_ERROR_INVALID_PARAM == error)
        if( SFS_ERROR_NONE == error ) { /*revised by yongwei for8801h format usdik*/
