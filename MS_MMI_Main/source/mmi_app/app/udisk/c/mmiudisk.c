...
PUBLIC MMI_RESULT_E MMIAPIUdisk_HandleUsbCablePlugIn(void)
{
    ...
    {//手机关机，进入U盘
		...
		#ifndef RELEASE_INFO
		SCI_TRACE_LOW("MMIAPIUdisk_HandleUsbCablePlugIn  SCI_ASSERT_DEBUG_MODE line=%d",__LINE__);
			// bug 973380  
			// avoid charging into usb log when power off ;
        ...
        {  
		#ifndef RELEASE_INFO
		SCI_TRACE_LOW("MMIAPIUdisk_HandleUsbCablePlugIn  line=%d",__LINE__);
			// bug 973380  
			// avoid charging into usb log when power off ;
        		// MMIAPIUdisk_OpenUsbOperWindow();       //Popup usb operation select window 
        result = MMIAPIUdisk_OpenUsbOperWindow();        //Popup usb operation select window
        #else
...
#if 0
   #ifndef RELEASE_INFO
			MMINV_READ(MMINV_PHONE_FIRST_POWER_ON, &is_first_power_on, return_value);//读NVif(MN_RETURN_SUCCESS != return_value||is_first_power_on == TRUE){
			if(MN_RETURN_SUCCESS != return_value||is_first_power_on == TRUE)
			{
				SCI_TRACE_LOW("MMIAPIUdisk_HandleUsbCablePlugIn Delay line=%d is_first_power_on =%d return_value =%d",__LINE__,is_first_power_on,return_value);
...
{  
 ...
 /*****************************************************************************/
//  Description : Start USM Service For Mass Storage, AT and Diag  
/*****************************************************************************/
PUBLIC BOOLEAN MMIUDISK_StartUMS(void)
{
    BOOLEAN             result      =   TRUE;
...
#endif
//     MMIUDISK_OpenWaitWin(USB_SERVICE_UCOM_UMS, MMIUDISK_USB_SERVICE_START_WIN_ID, TXT_UDISK_WAIT_START_UMS);//for bug497103
...
#endif 		
	if (USB_StartService(USB_SERVICE_UCOM_UMS))
	{
		MMIAPIUdisk_SetRunStatus(TRUE);
...
PUBLIC MMI_RESULT_E MMIAPIUdisk_HandleUsbCablePlugOut(void)
{
    ...
    else
    {
		...
		{
//            MMIUDISK_OpenWaitWin(USB_SERVICE_UDISK, MMIUDISK_STOP_WAIT_WIN_ID, TXT_UDISK_WAIT_STOP_UDISK);//for bug497103
...
#endif
     //进入等待界面
//     MMIUDISK_OpenWaitWin(USB_SERVICE_UDISK, MMIUDISK_USB_SERVICE_START_WIN_ID, TXT_UDISK_WAIT_START_UDISK);//for bug497103
...
PUBLIC void MMIAPIUdisk_HandleFormatCnf(BOOLEAN result)
{
	...
	MMIAPIVCAL_Init();
        ...
        //MMIPUB_DisplayTipsIDEx(PNULL, TXT_SUCCESS, MMIPUB_TIPS_DIS_BOTTOM, PNULL);//194364
        {
            ...
            banner_info.text1_id = STR_FORMAT_COMPLETE_EXT01;
    ...
    if(!result)
    {
        ...
        else
        {
            ...
            MMIPUB_OpenAlertWinByTextId(PNULL,
                TXT_NO_SD_CARD_ALERT, //nor只提示sd卡不存在
...
/*****************************************************************************/
//  Description : set sd/udisk memory detail info by MFST style
//  Global resource dependence :
//  Author:wen.dong
//  Note:
/*****************************************************************************/
PUBLIC void MMIUDISK_SetMemoryDetailByMicroStyle(uint32 device,MMI_CTRL_ID_T ctrl_id ,MMI_WIN_ID_T  win_id)
{
    uint32          free_space_low                              =   0;
    uint32          free_space_high                             =   0;
    uint32          used_space_low                              =   0;
	...
	//set name
    MMI_GetLabelTextByLang(MMIAPIFMM_GetDeviceMemStatusText(device), &name);
    AddOneDetailItem (ctrl_id, STR_MCD_DTAIL_COD_NAME, PNULL, &nIndex, name.wstr_ptr, name.wstr_len);

    //set size
    MMIUDISK_GetFileSizeString(total_space_low,total_space_high,total_wstr,GUILIST_STRING_MAX_NUM);
    AddOneDetailItem (ctrl_id, STR_MCD_DTAIL_COD_SIZE, PNULL, &nIndex, total_wstr, (uint16)MMIAPICOM_Wstrlen(total_wstr));

    //set free memory
    MMIUDISK_GetFileSizeString(free_space_low,free_space_high,unused_wstr,GUILIST_STRING_MAX_NUM);
    AddOneDetailItem (ctrl_id, STR_INFO_LIST_FREE_MEMORY_EXT01, PNULL, &nIndex, unused_wstr, (uint16)MMIAPICOM_Wstrlen(unused_wstr));

    //set used memory
    MMIUDISK_GetFileSizeString(used_space_low,used_space_high,used_wstr,GUILIST_STRING_MAX_NUM);
    AddOneDetailItem (ctrl_id, STR_INFO_LIST_USED_MEMORY_EXT01, PNULL, &nIndex, used_wstr, (uint16)MMIAPICOM_Wstrlen(used_wstr));
}
...
/*****************************************************************************/
//  Description : get sd/udisk memory detail info
//  Global resource dependence :
//  Author:wen dong
//  Note:
/*****************************************************************************/
PUBLIC void MMIUDISK_GetMemoryDetail(uint32 device,MMI_STRING_T *label_str)
{
    uint32          free_space_low                              =   0;
    uint32          free_space_high                             =   0;
    uint32          used_space_low                              =   0;
...
{    
    ...
    {
        sprintf( temp_str, "%ld.%ld TB", (hight_size/tb_size),(hight_size % tb_size)/(tb_size/10));/*lint !e737 */
        MMI_STRNTOWSTR( str_ptr, wstr_len, (uint8*)temp_str, MMIUDISK_SIZE_STR_LEN, strlen(temp_str) );
    }
    else if (hight_size > 0)
    {
        sprintf( temp_str, "%ld.%ld GB", ((hight_size<<2) + low_size/gb_size),(low_size % gb_size)/(gb_size/10));/*lint !e737 */
        MMI_STRNTOWSTR( str_ptr, wstr_len, (uint8*)temp_str, MMIUDISK_SIZE_STR_LEN, strlen(temp_str) );
    }
    else
    {
