...
LOCAL   uint32                s_current_device = MMI_DEVICE_UDISK;
 ...
 #ifndef RELEASE_INFO 
	LOCAL uint32  s_mmiudisk_choose_timer_id = 0;
 #endif
...
//    WIN_SOFTKEY(PNULL, STXT_SOFTKEY_OK_MK, TXT_COMMON_COMMON_BACK), 
    WIN_SOFTKEY(PNULL, STXT_SOFTKEY_SELECT_MK, TXT_COMMON_COMMON_BACK),  //for bug497103
...
PUBLIC BOOLEAN MMIAPIUdisk_OpenUsbOperWindow(void)
{
    ...
    if (MMK_CreateWin((uint32 *)MMIUDISK_USB_OPER_SELECT_WIN_TAB, PNULL))
    {
        if(banner_flag)
        {
               ...
               if(100 != p_chgmng_info->bat_remain_cap)
                {
...
/*****************************************************************************/
LOCAL void UDISK_ValidatePrivacyUMSOper(BOOLEAN is_permitted, DPARAM param_ptr)
{
    MMI_WIN_ID_T win_id = 0;
    ...
    if (is_permitted)
    {
        ...
        MMK_SendMsg(win_id, MSG_UMS_VALIDATE_PRIVACY_SUCCESS, PNULL);
    }
}
#endif
                                              ...
                                              )
{
    ...
    case MSG_OPEN_WINDOW:
		...
		if(REFPARAM_GetUsbServicesFlag()==1)
		{
			service_num =SetUSBServiceList(ctrl_id);
...
#endif
		{
...
#endif		
		    {
		        ...
		        txt_exit_WRE.wstr_len=4;
		        txt_exit_WRE.wstr_ptr=L"    ";
		        MMIAPICOM_StatusAreaSetViewHeaderinfo(txt_exit_WRE.wstr_ptr, txt_exit_WRE.wstr_len);
		    }

//ENG_USB_TEST_SET
#ifndef RELEASE_INFO
		is_SetOn = MMIAPIENGUSBTestSet_GetMode();	
		SCI_TRACE_LOW("HandleUsbOperSelectWindow %d",is_SetOn);
		 if(is_SetOn!=USB_SERVICE_MAX)
		 {
			 for(iSetOn=0;iSetOn<service_num;iSetOn++)
			 {
				GUILIST_GetItemData(ctrl_id, iSetOn, &service_type);
				if(service_type == is_SetOn)
				{
					GUILIST_SetSelectedItem(ctrl_id,iSetOn, TRUE);
				       GUILIST_SetCurItemIndex(ctrl_id, iSetOn);
					//MMK_PostMsg(win_id, MSG_APP_MENU,PNULL, 0);
					if (s_mmiudisk_choose_timer_id != 0)
					{
						MMK_StopTimer(s_mmiudisk_choose_timer_id);
						s_mmiudisk_choose_timer_id = 0;
					}
					s_mmiudisk_choose_timer_id = MMK_CreateWinTimer(win_id, 3000, FALSE);
					break;
				}
			 }
...
#ifndef RELEASE_INFO 
	case MSG_TIMER:    
		 if((*(uint8*)param) != s_mmiudisk_choose_timer_id)
		 {
		     break;
		 }
		 if (s_mmiudisk_choose_timer_id != 0)
		{
		 ...
		 if( USB_SERVICE_LOG == MMIAPIENGUSBTestSet_GetMode()	)
		 {
			SCI_TRACE_LOW("HandleUsbOperSelectWindow MMIUDISK_StartUsbLog ");
    ...
    case MSG_APP_MENU:
		...
		if(GUILIST_GetItemData(ctrl_id, cur_selection, &service_type))
		{
			  ...
			  case USB_SERVICE_UCOM_UMS:
				  SCI_TRACE_LOW("mmiudisk:start UMS");
#ifdef JAVA_SUPPORT
				  if (MMIAPIJAVA_IsJavaRuning())
				  {
					//GUILIST_SetSelectedItem(ctrl_id, cur_selection, FALSE);
				  ...
				  if( !MMIFILE_GetDeviceSDStatus() ){    
					MMIPUB_OpenAlertWarningWin(STR_USB_NO_MMC_EXT01);
				  }
				  ...
				  {
					//current condition can start udisk
					MMIAPISET_ValidatePrivacy(MMISET_PROTECT_USB, UDISK_ValidatePrivacyUMSOper, (void*)&win_id, sizeof(win_id));
				  }
				  else
				  {
					  //current app can not be exit, so can not start U disk
    ...
    case MSG_SET_VALIDATE_PRIVACY_SUCCESS:
        ...
        break;
#ifdef  UCOM_UMS_SUPPORT
		case MSG_UMS_VALIDATE_PRIVACY_SUCCESS:
			
		{
			GUILIST_SetSelectedItem(ctrl_id, cur_selection, TRUE);
			SCI_TRACE_LOW("mmiudisk:MMIUDISK_StartUMS");
			MMIUDISK_StartUMS();
		}
		break;
#endif
                                       ...
                                       )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        MMIUDISK_SetMemoryDetailByMicroStyle((MMIFILE_DEVICE_E)device, ctrl_id,win_id);
        //End
    ...
    case MSG_APP_WEB:
		...
		//MMIPUB_OpenQueryWinByTextId(STR_FORMAT_QUERY_EXT01,IMAGE_PUBWIN_QUERY,PNULL,PNULL);
        break;
                                  ...
                                  )
{
	...
	do
	{
		...
		if((REFPARAM_GetUsbServicesFlag()==1) && (MMI_DEVICE_NUM > MMIAPIFMM_GetFirstValidDevice()))
...
#ifdef  UCOM_UMS_SUPPORT
		else if((REFPARAM_GetUsbServicesFlag()==0)&&(USB_SERVICE_UCOM_UMS==serive_type))
		{
		// if sd card or u disk exist.
		if (MMI_DEVICE_NUM > MMIAPIFMM_GetFirstValidDevice() && (RESTART_BY_CHARGE != restart_condition))		
		{
			if(FALSE == MMIAPISET_IsDisableShareViaOption())
				is_service_support = TRUE;
		}
...
#ifdef USB_VUART
			if ((REFPARAM_GetUsbServicesFlag()==1)&&(RESTART_BY_CHARGE != restart_condition) &&(RESTART_BY_ALARM != restart_condition))
			{
				//is_service_support = TRUE; //bug 1377279 remove pc connect
			}
#endif	
		}
...
LOCAL MMI_TEXT_ID_T GetServiceNameTextID(USB_SERVICE_E type)
{
	...
	{
	case USB_SERVICE_UDISK:
		text_id = STR_CAM_ILIST_MEMORY_PHN_MEMORY;
		break;
	case USB_SERVICE_UCOM:
    ...
    case USB_SERVICE_CHARGE:
        text_id = STR_USB_LIST_CHARGING_EXT02;		
        break;
#ifdef UCOM_UMS_SUPPORT
	case USB_SERVICE_UCOM_UMS:
	    text_id = STR_USB_LISTQ_MASS_STOR_MODE;
	    break;
#endif
	default:
		break;
...
#ifdef UCOM_UMS_SUPPORT
	if ((MMI_USB_SERVICE_UMS== usb_srv_type))
	{
		MMIFW_PumpUdiskMountEvent(DEV_UDISK, TRUE);
	}
#endif
