...
BOOLEAN g_sms_local_time = FALSE;
...
//    Note:
/*****************************************************************************/
LOCAL BOOLEAN MMISMS_HandleSmsNumInd(DPARAM    param);
...
#if 0
/*****************************************************************************/
//     Description : Set AutoEnable Flag
//    Global resource dependence : 
//  Author: bingjie.chen
//    Note:
/*****************************************************************************/
PUBLIC void MMISMS_SetAutoEnableFlag(BOOLEAN AutoEnable);
/*****************************************************************************/
//     Description : Get AutoEnable Flag
//    Global resource dependence : 
//  Author: bingjie.chen
//    Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMISMS_GetAutoEnableFlag();

/*****************************************************************************/
//     Description : Set first startup phone Flag
//    Global resource dependence : 
//  Author: bingjie.chen
//    Note:
/*****************************************************************************/
PUBLIC void MMISMS_SetAutoFirstStartPhoneFlag(BOOLEAN firststart);

/*****************************************************************************/
//     Description : Get first startup phone Flag
//    Global resource dependence : 
//  Author: bingjie.chen
//    Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMISMS_GetAutoFirstStartPhoneFlag();

/*****************************************************************************/
//     Description : set auto sms timer
//    Global resource dependence : 
//  Author:bingjie.chen
//    Note:
/*****************************************************************************/
PUBLIC void MMISMS_SetAutoSmsTimer(uint8 time);

/*****************************************************************************/
//     Description : Set AutoSms Timer
//    Global resource dependence : 
//  Author: bingjie.chen
//    Note:
/*****************************************************************************/
PUBLIC void MMISMS_SaveAutoSmsTimer(uint8 time);

/*****************************************************************************/
//     Description : Get AutoSms Timer
//    Global resource dependence : 
//  Author: bingjie.chen
//    Note:
/*****************************************************************************/
PUBLIC uint8 MMISMS_GetAutoSmsTimer(void);
...
/*****************************************************************************/
//     Description : to handle mem-full ind ex
//    Global resource dependence : none
//  Author:louis.wei
//    Note:
/*****************************************************************************/
LOCAL void HandleMemFullIndEx(
                            DPARAM param    //IN:
                            );
...
//    Note:
/*****************************************************************************/
PUBLIC MN_RETURN_RESULT_E MMISMS_GetNDTNvSetting(
...
LOCAL void InitStatusReportFlag(void)
{
    ...
    if (MN_RETURN_SUCCESS != nv_value)
    {
        ...
        ret_value = MMISMS_GetNDTNvSetting(MESSAGES_NV_SMS_DELIVERY_REPORT,&flag);
...
PUBLIC void MMISMS_SaveAutoSmsTimer(uint8 time)
{  
	SCI_TRACE_LOW("MMI_AUTOSMS: MMISMS_SetAutoSmsTimer time = %d", time);

    MMINV_WRITE(MMINV_SMS_AUTO_TIMER, &time);
}
/*****************************************************************************/
//     Description : Get AutoSms Timer
//    Global resource dependence : 
...
{  
    ...
    MMINV_READ(MMINV_SMS_AUTO_TIMER, &time, nv_value);
	...
	SCI_TRACE_LOW("MMI_AUTOSMS: MMISMS_GetAutoSmsTimer time = %d", time);
...
PUBLIC void MMISMS_SetAutoEnableFlag(BOOLEAN AutoEnable)
{  
	SCI_TRACE_LOW("MMI_AUTOSMS: MMISMS_SetAutoEnableFlag AutoEnable = %d", AutoEnable);
    //MMINV_WRITE(MMINV_SMS_AUTO_FLAG, &AutoEnable);
    if ((s_auto_sms_timer_id != 0 || s_auto_sms_12h_timer_id != 0) && !AutoEnable)
    {
        if (0 != s_auto_sms_timer_id)
        {
            MMK_StopTimer(s_auto_sms_timer_id);
            
            s_auto_sms_timer_id = 0;
        }

        if (s_auto_sms_12h_timer_id != 0)
        {
            MMK_StopTimer(s_auto_sms_12h_timer_id);
            
            s_auto_sms_12h_timer_id = 0;
        }
        
    }
    else if (s_auto_sms_12h_timer_id == 0 && AutoEnable)
    {
        SCI_TRACE_LOW("MMI_AUTOSMS: 2 Minutes timer start...");
        s_auto_sms_timer_id = MMK_CreateTimerCallback(TIMEOUT_120S, MMIAUTOSMS_SendAliveMessage, NULL, FALSE);
        
        SCI_TRACE_LOW("MMI_AUTOSMS: 12 Hours timer start...");
        s_auto_sms_12h_timer_id = MMK_CreateTimerCallback(TIMEOUT_12HOUR, MMIAUTOSMS_Handle12HourTimerMsg, NULL, TRUE);
...
PUBLIC BOOLEAN MMISMS_GetAutoEnableFlag()
{
    ...
    MMINV_READ(MMINV_SMS_AUTO_FLAG, &AutoEnable, nv_value);
	...
	SCI_TRACE_LOW("MMI_AUTOSMS: MMISMS_GetAutoEnableFlag AutoEnable = %d", AutoEnable);
...
PUBLIC void MMISMS_SetAutoFirstStartPhoneFlag(BOOLEAN firststart)
{  
	SCI_TRACE_LOW("MMI_AUTOSMS: MMISMS_SetAutoFirstStartPhoneFlag firststart = %d", firststart);
    MMINV_WRITE(MMINV_SMSAUTO_FIRSTSTART_PHONE, &firststart);
}
...
PUBLIC BOOLEAN MMISMS_GetAutoFirstStartPhoneFlag()
{
    ...
    MMINV_READ(MMINV_SMSAUTO_FIRSTSTART_PHONE, &firststart, nv_value);
	...
	SCI_TRACE_LOW("MMI_AUTOSMS: MMISMS_GetAutoFirstStartPhoneFlag AutoEnable = %d", firststart);
...
PUBLIC void MMISMS_SetAutoSmsTimer(uint8 time)
{
    ...
    SCI_TRACE_LOW("MMI_AUTOSMS: %d min timer start...", time);
	...
	{
		timer = time * 1000 * 60 * 60;
	}

	if (MMISMS_GetAutoEnableFlag())
	{
		s_auto_sms_timer_id = MMK_CreateTimerCallback(timer, MMIAUTOSMS_SendAliveMessage, NULL, TRUE);
	}
}
#endif
...
{    
...
#endif
    {
        ...
        if(new_sms_info_ptr->cur_chat_order_id->flag.is_concatenate_sms)
        {
            long_sms_first_come_one = MMISMS_IsLongSmsFirstOneOrder(new_sms_info_ptr->cur_chat_order_id);
        }
      
        if(long_sms_first_come_one
...
#endif           
        {
            ...
            MMISMS_ShowNewMsgPrompt(new_sms_info_ptr->dual_sys, TRUE);
#else
			MMISMS_ShowNewMsgNotification(new_sms_info_ptr->dual_sys, TRUE, (void*)new_sms_info_ptr);
#endif
#endif//UNISOC_MMI_NEW
        }
    }
}
...
PUBLIC BOOLEAN MMISMS_GetLocalTime(MN_SMS_TIME_STAMP_T *time_stamp_ptr)
{
    SCI_TIME_T time = {0};
    SCI_DATE_T date = {0};
    if(PNULL == time_stamp_ptr || !g_sms_local_time)
    {
       return FALSE;
    }
    TM_GetSysTime(&time);
    TM_GetSysDate(&date);
    time_stamp_ptr->year = date.year - MMISMS_MT_OFFSERT_YEAR;  //MMISMS_MT_OFFSERT_YEAR is added in InsertNewMtToOrder , so must reduce here
    time_stamp_ptr->month = date.mon;
    time_stamp_ptr->day = date.mday;
    time_stamp_ptr->hour = time.hour;
    time_stamp_ptr->minute = time.min;
    time_stamp_ptr->second = time.sec;
    return TRUE;
}
...
PUBLIC void MMISMS_InitForPS(void)
{
    ...
    /*****************************START****************************************/
    EFS_NvitemRead(MESSAGES_NV_TIME_STAMP_TYPE, sizeof(BOOLEAN), &bl_time_type);
    if(0 == bl_time_type)
    {
        g_sms_local_time = TRUE;
    }
    ...
    if(g_sms_local_time)
    {       
        MNSMS_RegGetLocalTimeFunc(MMISMS_GetLocalTime);
    }
#endif
                               ...
                               )
{
    ...
    case APP_MN_SMS_NUM_IND:
		SCI_TRACE_LOW("Receive APP_MN_SMS_NUM_IND");
    ...
    case APP_MN_SMS_MEM_FULL_IND_EX:
        SCI_TRACE_LOW("MMISMS: receive APP_MN_SMS_MEM_FULL_IND_EX");
        HandleMemFullIndEx( param );        
        break;
                            ...
                            )
{
    ...
    APP_MN_SMS_MEM_FULL_IND_EX_T *sig_ptr = (APP_MN_SMS_MEM_FULL_IND_EX_T *)param;
    ...
    SCI_TRACE_LOW("mmisms APP_MN_SMS_MEM_FULL_IND_EX_T sig_ptr event %d",sig_ptr->event);
    //SCI_TRACE_LOW:"SMS: HandleMemFullIndEx mem_status = %d, unread_count = %d"
...
#ifdef MMI_SMS_CHAT_SUPPORT
/*****************************************************************************/
//     Description : draft operation order id, no need consider long sms, it already first node
//    Global resource dependence : g_mmisms_global
//  Author:rong.gu
//    Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMISMS_GetIsDraftDeleting(void)
{
    SCI_TRACE_LOW("[MMISMS] MMISMS_GetIsDraftDeleting = %d", g_mmisms_global.is_draft_deleting);
    return g_mmisms_global.is_draft_deleting;
}
...
//    Note:  
/*****************************************************************************/
PUBLIC  void MMISMS_SetIsDraftDeleting(BOOLEAN is_deleting)
{
   g_mmisms_global.is_draft_deleting = is_deleting;
}
...
//    Note:  
/*****************************************************************************/
PUBLIC MMISMS_ORDER_ID_T MMISMS_GetDraftOperationOrderId(void)
{
    return g_mmisms_global.draft_oper_order_id;
}
...
//    Note:  
/*****************************************************************************/
PUBLIC  void MMISMS_SetDraftOperationOrderId(MMISMS_ORDER_ID_T order_id)
{
   g_mmisms_global.draft_oper_order_id = order_id;
}
#endif
...
{
    MN_DUAL_SYS_E dual_sys   = MN_DUAL_SYS_MAX;
    APP_MN_SMS_NUM_IND_T *SmsNumInd = (APP_MN_SMS_NUM_IND_T *)param;
    
    dual_sys = SmsNumInd->dual_sys;
    if (dual_sys >=  MMI_DUAL_SYS_MAX)
    {
        SCI_TRACE_LOW("MMISMS_HandleSmsNumInd, dual_sys:%d", dual_sys);
        return FALSE;
    }
#ifdef SIM_LOCK_CT_SUPPORT
    if(FALSE == MMICDMA_CheckCard_OK(dual_sys))
    ...
    SCI_TRACE_LOW("MMISMS_HandleSmsNumInd, dual_sys:%d, num:%d", dual_sys,SmsNumInd->sms_num);
    MMIAPISMS_SetMaxSimSmsNum(dual_sys, SmsNumInd->sms_num);
...
//    Note:
/*****************************************************************************/
PUBLIC void MMISMS_InitAlphabetNDT(void)
{
    MN_RETURN_RESULT_E ret_value = MN_RETURN_SUCCESS;
    BOOLEAN mmisms_ndt_cfg = FALSE;
    MMISMS_ALPHABET_MANAGEMENT_E alpha_management = MMISMS_ALPHABET_MANAGEMENT_DEFAULT;
    ret_value = MMISMS_GetNDTNvSetting(MESSAGES_NV_SMS_CFG,&mmisms_ndt_cfg);
    if(ret_value != MN_RETURN_SUCCESS )
    {
    ...
    else
    {
       if(1 == mmisms_ndt_cfg)
       {
           alpha_management  = MMISMS_ALPHABET_MANAGEMENT_DEFAULT;  // this is full coding
       }
       else if(2 == mmisms_ndt_cfg)
       {
...
PUBLIC void MMISMS_SetAlphabetManagementEx(
                                MMISMS_ALPHABET_MANAGEMENT_E alpha_management
                                )
{
    g_mmisms_global.nv_param.alpha_management = alpha_management;
...
#else
    param_out_ptr->total = MMISMS_GetSmsNumByStorage(MN_DUAL_SYS_1, MN_SMS_STORAGE_ME)
    						+MMISMS_GetSmsNumByStorage(MN_DUAL_SYS_2, MN_SMS_STORAGE_ME);
#endif
