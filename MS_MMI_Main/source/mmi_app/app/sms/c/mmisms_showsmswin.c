...
#define MMISMS_MAX_CLASS0_STR_LENGTH 500
#define MMISMS_MAX_CLASS0_CONTACT_LENGTH 50
#define MMISMS_MAX_CLASS0_TIME_LENGTH 50
 ...
 **                         STATIC DEFINITION                                *
 **--------------------------------------------------------------------------*/
LOCAL wchar contact_str[MMISMS_MAX_CLASS0_CONTACT_LENGTH+1] = {0};
...
LOCAL void SetMtSmsDetail(MMISMS_SHOWSMS_WINDATA_T *win_data_ptr, MMI_CTRL_ID_T ctrl_id)
{
    ...
    else
    {
        ...
        MMI_GetLabelTextByLang(STR_ILIST_SIM_EXT01, &sim_num );
...
//  Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E  HandleConfirmClass0CodeWin(
                                        MMI_WIN_ID_T     win_id, 
                                        MMI_MESSAGE_ID_E msg_id, 
                                        DPARAM           param 
                                        )
...
//  Note: 
/*****************************************************************************/
PUBLIC void MMISMS_ShowClass0Alert(MMISMS_READ_MSG_T *read_msg_ptr)
{
    MMI_STRING_T   class0_text1 = {0};
    MMI_STRING_T   title_text1 = {0};

    wchar class0_str[MMISMS_MAX_CLASS0_STR_LENGTH+1] = {0};
    wchar time_string[MMISMS_MAX_CLASS0_TIME_LENGTH+1] = {0};
    wchar    *string = PNULL;
    uint8 i = 0;
    uint16 str_len = 0;
    wchar *temp = PNULL;
    uint16 total_len = 0;
    MMI_WIN_ID_T win_id = MMISMS_SHOWCLASS0_WIN_ID;
    class0_text1.wstr_ptr = class0_str;

    MMISMS_GetReadSmsDispTime(read_msg_ptr,time_string,&str_len);   //显示时间
    MMIAPICOM_Wstrncpy(class0_str,time_string,str_len);
    temp = class0_str+str_len;
    MMIAPICOM_Wstrcat(temp, L"\n");
    temp= temp + 1;
    ...
    total_len = temp - class0_str;
    MMISMS_GetReadSmsDispContent(read_msg_ptr,string,&str_len); 
    MMIAPICOM_Wstrncat(temp, string,MIN(MMISMS_MAX_CLASS0_STR_LENGTH -total_len, str_len ));
    class0_text1.wstr_len = MMIAPICOM_Wstrlen(class0_str);
    if(MMK_IsOpenWin(win_id))
    {
        MMK_CloseWin(win_id);
    }
    MMIPUB_OpenTextDialogByTextPtr(PNULL,&class0_text1,PNULL,PNULL,&win_id,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,HandleConfirmClass0CodeWin);   
    SCI_MEMSET(contact_str,0,sizeof(contact_str));
    MMISMS_GetReadSmsDispNum(read_msg_ptr,contact_str,&str_len);
	for(i = 0; i< str_len;i++)
	{
		SCI_TRACE_LOW("MMISMS_ShowClass0Alert %c",contact_str[i]);
	}
	
    SCI_TRACE_LOW("MMISMS_ShowClass0Alert %s ---str_len=%d",contact_str,str_len);
    
    title_text1.wstr_ptr = contact_str;
    title_text1.wstr_len = str_len;
    MMIPUB_SetTextDialogTitle(win_id, &title_text1, TRUE);
    ...
    case MSG_CTL_RICHTEXT_SWITCHFOCUS_BY_TP:
        {
            ...
            case GUIRICHTEXT_FOCUS_PHONENUM:
                GUIWIN_SetSoftkeyTextId(win_id, STXT_SOFTKEY_SAVE_MK, TXT_COMMON_NUMBER, STXT_RETURN, is_need_update);
                break;

            case GUIRICHTEXT_FOCUS_URL:
                GUIWIN_SetSoftkeyTextId(win_id, STXT_SOFTKEY_SAVE_MK, TXT_URL_ADRESS, STXT_RETURN, is_need_update);
                break;

            case GUIRICHTEXT_FOCUS_EMAIL:
                GUIWIN_SetSoftkeyTextId(win_id, STXT_SOFTKEY_SAVE_MK, TXT_EMAIL_BOX, STXT_RETURN, is_need_update);
                break;

            default:
                GUIWIN_SetSoftkeyTextId(win_id, STXT_SOFTKEY_SAVE_MK, TXT_NULL, STXT_RETURN, is_need_update);
                break;
            }
        }
        break;
                                       ...
                                       )
{
    ...
    case MSG_FULL_PAINT:
        ...
        /* Bug fix: cr00203127. If in call state, do not play SMS text */
		 ...
		 if( ! MMIAPICC_IsInState(CC_IN_CALL_STATE) && MMISMS_GetTTSInfo() &&( 0 == ZDT_GetSmsSwitch_type() ))		
	#else				
        if( ! MMIAPICC_IsInState(CC_IN_CALL_STATE) && MMISMS_GetTTSInfo())
	#endif
        {
            ...
            if (!is_tts_played)
            {
                ...
                if(PNULL != msg_content.wstr_ptr)
                {
                    ...
                    play_param.is_ucs2 = TRUE;
                    play_param.play_times = 1;
                    play_param.tts_vol = ZDT_GetVol_Num();;//MMIAPISET_GetMultimVolume();
                    play_param.ring_type = TTS_RING_TYPE_SMS;
                    play_param.win_handle = MMISMS_SHOWMSG_WIN_ID;
					play_param.tts_route = MMISRVAUD_ROUTE_AUTO;
					play_param.digit_mode = TTS_DIGIT_AUTO;
...
LOCAL BOOLEAN ShowSmsWinReadSmsCallback(SMSSRV_CALLBACK_STRUCT_T *callback_data)
{
    ...
    {        
        ...
        memcpy(&win_data_ptr->read_msg, read_msg, sizeof(MMISMS_READ_MSG_T));
