...
LOCAL BOOLEAN s_is_forward_sms = FALSE;
/**--------------------------------------------------------------------------*
 **                         STATIC DEFINITION                                *
 **--------------------------------------------------------------------------*/
...
PUBLIC void MMISMS_MarkChatItem(MMISMS_ORDER_ID_T order_id,MMI_CTRL_ID_T ctrl_id,uint32* mark_num)
{
    ...
    if (order_id != PNULL && !order_id->flag.is_marked)
    {
        if(MMK_GetActiveCtrlId(MMISMS_CHATBOX_CHILD_WIN_ID) != ctrl_id)
        {
            MMK_SetAtvCtrl(MMISMS_CHATBOX_CHILD_WIN_ID,ctrl_id);
            GUILIST_SetSelectedItem(ctrl_id,0,TRUE);
        }
        GUILIST_SetListState(ctrl_id, GUILIST_STATE_CYC_SCROLL_ITEM, TRUE);
        GUILIST_SetTextListMarkable(ctrl_id, TRUE);
        GUIFORM_SetChildDisplayOnly(MMISMS_CHATBOX_FORM_CTRL_ID,MMISMS_CHATBOX_CHILD_BUTTON_FORM_CTRL_ID,GUIFORM_CHILD_DISP_HIDE);
//         GUILIST_SetCheckIconLeftFlag(ctrl_id,TRUE);
        GUILIST_SetMaxSelectedItem(ctrl_id, MMISMS_GetMaxSmsNumInOrder());
        CTRLLIST_ChangeCheckListDefSoftkeyID(ctrl_id,STXT_SOFTKEY_UNMARK_MK,STXT_SOFTKEY_MARK_MK,STXT_SOFTKEY_OPEN_MK);
        CTRLLIST_SetNeedCustomRightSK(ctrl_id,TRUE);
        CTRLLIST_ChangeCheckListRightSoftkey(ctrl_id,STR_LIST_DELETE,IMAGE_COMMON_DELETE);

#if 0 // UNISOC_MMI_Delete
        GUILIST_SetSelectedItem(ctrl_id,item_pos,TRUE);
#endif
        //(*mark_num)++;   // it is not marked at first , so *mark_num should not ++
#if 0 // UNISOC_MMI_Delete
...
PUBLIC void  MMISMS_AddToPbNew(MMISMS_ORDER_ID_T order_id,uint8 *number_arr)
{
    ...
    wchar temp_mail_number[MMIPB_MAX_MAIL_LEN + 1] = {0};
    int32 temp_mail_len = 0;
    ...
    {
        pb_mail.wstr_len = MIN(SCI_STRLEN((char *)order_id->number.sender) , MMIMMS_MAX_ADDRESS_LEN);
        
        if(pb_mail.wstr_len > 0)
        {
            temp_mail_len = (int32)pb_mail.wstr_len;
            MMI_STRNTOWSTR(pb_mail.wstr, MMIMMS_MAX_ADDRESS_LEN, (uint8 *)order_id->number.sender, MMISMS_NUMBER_MAX_LEN, pb_mail.wstr_len);
            MMIAPICOM_Wstrncpy(temp_mail_number, pb_mail.wstr, pb_mail.wstr_len);
        }
        if(URL_IsMailtoDefault((URL_CHAR_T const *)temp_mail_number, (int32*)&temp_mail_len))
        {
...
PUBLIC void  MMISMS_AddToPbExist(MMISMS_ORDER_ID_T order_id,uint8 *number_arr)
{
    ...
    else if (MMISMS_TYPE_MMS == order_id->flag.msg_type)
    {
        ...
        if(pb_mail.wstr_len > 0)
        {
            ...
            MMI_STRNTOWSTR(pb_mail.wstr, MMIMMS_MAX_ADDRESS_LEN, (uint8 *)order_id->number.sender, MMISMS_NUMBER_MAX_LEN, pb_mail.wstr_len);
            MMIAPICOM_Wstrncpy(temp_mail_number, pb_mail.wstr, pb_mail.wstr_len);
            is_mail_address = URL_IsMailtoDefault((URL_CHAR_T const *)temp_mail_number, (int32*)&temp_mail_len);
        }
...
#ifdef MMI_SMS_CHAT_SUPPORT 
    if(MMK_IsOpenWin(MMISMS_SHOW_CHAT_SMS_WIN_ID) && !is_re_send && !MMISMS_GetIsForwardSms())
    {
...
)
{
    ...
    if (PNULL != sms_init_ptr)
    {
        ...
        {
            MMIPUB_OpenAlertWinByTextId(PNULL, STR_MSG_MAX_LENGTH, TXT_NULL, IMAGE_PUBWIN_WARNING, PNULL, PNULL, MMIPUB_SOFTKEY_NONE, PNULL);
        }
    }
#else
...
)
{
	...
	{
		MMISMS_ENTER_BY_MUSIC mmisms_enter_by_music = {0};
		
		mmisms_enter_by_music.dual_sys =dual_sys ;
		mmisms_enter_by_music.sms_init_ptr = sms_init_ptr;
		mmisms_enter_by_music.need_load_saved = need_load_saved;
		mmisms_enter_by_music.dest_addr_ptr = dest_addr_ptr;
		mmisms_enter_by_music.dest_addr_len = dest_addr_len;
		MMIAPISMS_OpenConfirmation((void *)(&mmisms_enter_by_music), MMISMS_ENTER_BY_WRITEMESSAGE);	
		return ;
	}
	else
	{
...
PUBLIC GUI_FONT_T MMISMS_GetWinFont(void)
{
    ...
    case MMISMS_FONT_BIG:
        font =SONG_FONT_32 ;// MMI_DEFAULT_BIG_FONT;
        break;
                                            ...
                                            MN_SMS_MEM_FULL_STATUS_E mem_status)
{
    ...
    if (!MMK_IsOpenWin(alert_win_id))
    {
        ...
        {
            MMI_GetLabelTextByLang(alert_txt_id, &prompt_str);
...
{
    s_sms_play_ring_info.is_need_play_ring = is_need_play;
}
...
PUBLIC  BOOLEAN MMISMS_GetIsForwardSms(void)
{    
    return s_is_forward_sms;
}

PUBLIC  void MMISMS_SetIsForwardSms(BOOLEAN is_forward_sms)
{    
    s_is_forward_sms = is_forward_sms;
}
