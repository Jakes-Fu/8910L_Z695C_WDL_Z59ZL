...
//                       )
    ...
    {
        windata_p->dual_sys = MMISMS_GetCurSaveDualSys();
    }
#else
    windata_p->dual_sys = MN_DUAL_SYS_1;
#endif
...
// no use
    ...
    switch (msg_id)
    {
        case MSG_OPEN_WINDOW:
            ...
            GUIEDIT_SetSoftkey(MMISMS_EDIT_SMS_PHONENUM_CTRL_ID, 1, 0, STXT_SMS_PB, TXT_NULL, PNULL);
#endif
    ...
    if (0 < num_count)
    {
        ...
        for (i = 0; i < num_count; i++)
        {
            ...
            else if (!is_delete_invalid)
            {
                ...
                {
                    MMIPUB_OpenAlertWarningWin(STR_MSG_INVALID_RECIPIENT);
                }
            }
            else
            {
