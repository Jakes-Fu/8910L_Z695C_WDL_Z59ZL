                ...
                if (!MMISMS_IsExistNextSMS(&send_info_ptr->send_content)) //是长短信的最后一条
                {
                    ...
                    send_info_ptr->cur_send_data.cur_send_oper = MMISMS_SAVE_SENDFAIL_SMS;
                        ...
                        if(MMISMS_NO_ERR != save_result)
                        {
                            ...
                            {
                                MMIPUB_OpenAlertWinByTextId(PNULL,STR_NO_SPACE_EXT01,TXT_NULL,IMAGE_PUBWIN_WARNING,PNULL,PNULL,MMIPUB_SOFTKEY_ONE, MMISMS_HandleSaveNextMessageAlertWin);
                                OperAfterSendCnf(dual_sys, is_send_success, is_finish);
                            }
                            else
                            {
                            ...
                            )
{
    ...
    {
#ifdef SFR_SUPPORT
	if(!send_info_ptr->send_content.is_sfr_sms)
#endif
                            ...
                            )
{
    ...
    {
        SCI_TRACE_LOW("mmisms OperAfterSendCnf maybe enter error case here");
                            ...
                            )
{
    ...
    else
    {
        ...
        SCI_TRACE_LOW("[MMIPHONE]:  SendMsgReqToMN get nettime %d",sec);
        if(sec == 0)
        {
		    	TM_GetSysTime(&time);
		    	TM_GetSysDate(&date);
        ...
        {
        	MMI_TM_T mtm    = {0};
        	mtm = MMIAPICOM_Second2Tm(sec);
		    	mo_sms.time_stamp_t.day = (uint8)mtm.tm_mday;
		    	mo_sms.time_stamp_t.hour = (uint8)mtm.tm_hour;
		    	mo_sms.time_stamp_t.minute = (uint8)mtm.tm_min;
		    	mo_sms.time_stamp_t.month = (uint8)mtm.tm_mon;
		    	mo_sms.time_stamp_t.second = (uint8)mtm.tm_sec;
		    	mo_sms.time_stamp_t.timezone = 0;    //待完善
		    	mo_sms.time_stamp_t.year = (uint16)mtm.tm_year - MMISMS_OFFSERT_YEAR;    //@cr116674         	
        }    	
    }
...
#ifdef SFR_SUPPORT
    SCI_TRACE_LOW("SendMsgReqToMN MO msg data len= %d, ",mo_sms.user_data_t.length);
    SCI_TraceBuf("SendMsgReqToMN MO msg data: ", mo_sms.user_data_t.user_data_arr,mo_sms.user_data_t.length);
#endif
                                    ...
                                    )
{
    ...
    if (dest_len > 0)
    {
        ...
        SCI_TRACE_LOW("MMISMS_dest_number_ptr = %s", dest_number_ptr->party_num);
        ret_val = TRUE;
    }
                                  ...
                                  )
{
    ...
    if (0 == fail_count)
    {
	   ...
	   if(sos_temp)
	   {
	   	 SCI_TRACE_MID(" zdthero000	MMISMS_ShowSendResult  0000");
		   SOS_EmergencyCallWin();
		 //  idle_long_WEBKEY = TRUE ;
		   //sos_temp =    FALSE ;
	   }
#endif
    ...
    else
    {
		...
		if(sos_temp)
		{
			 SCI_TRACE_MID(" zdthero000	MMISMS_ShowSendResult  1111");
			SOS_EmergencyCallWin();
		 // sos_temp =	FALSE ;
		}
#endif		
        ...
        banner_info.text1_id = STR_MSG_SENDING_FAIL_EXT01;
    ...
    for (i = 0; i < MMISMS_SPLIT_MAX_NUM; i++)
    {
        if (0 != msg_content_ptr->split_sms_length[i])
        {
            ...
            MMISMS_ConvertValidDataToDispStr(
                                       ...
                                       MIN(MMISMS_R8_LOCK_DEFAULT_CHAR_LEN,msg_content_ptr->split_sms_length[i]),
...
BOOLEAN is_data_need_resend(MMISMS_SEND_T * data,BOOLEAN resend)
{
    int index;
    MMISMS_SEND_T * sendlist;
    
    if(resend == FALSE || s_mmisms_sendlist.send_num == 0 || NULL == s_mmisms_sendlist.sendlist_head_ptr) return TRUE;

    SCI_TraceLow("is_data_need_resend send_num=%d",s_mmisms_sendlist.send_num);
    sendlist = s_mmisms_sendlist.sendlist_head_ptr;
    for(index = 0; index < s_mmisms_sendlist.send_num; index++)
    {
        if((0 == memcmp(&(data->send_content.send_message.data),&(sendlist->send_content.send_message.data),MMISMS_SPLIT_MAX_NUM * MMISMS_DEFAULT_CHAR_LEN)) && \
            (data->send_content.dual_sys == sendlist->send_content.dual_sys))
        {
            SCI_TraceLow("is_data_need_resend: data[0] = 0x%02x,dual_sys=%d",data->send_content.send_message.data[0],sendlist->send_content.dual_sys);
            return FALSE;
        }
                                              ...
                                              )
{
    ...
    else
    {
        ...
        if (MMK_IsOpenWin(MMISMS_SHOW_CHAT_SMS_WIN_ID))
        {
			...
			if (PNULL != resend_content.wstr_ptr)
			{
				SCI_FREE(resend_content.wstr_ptr);

				resend_content.wstr_len = 0;
			}
        }
        else
#endif
...
#endif            
    {
        ...
        {
            if(is_data_need_resend(send_info_ptr,is_re_send))
            {
                send_info_ptr->backsend_status = MMISMS_SENDSTATUS_WAITING;
            }
            else
...
PUBLIC MMISMS_OPER_ERR_E MMISMS_AddSendList( MMISMS_SEND_T *send_info_ptr )
{
    ...
    {//如果第一条消息不是sending状态或者saving就是不对的，发送中的消息应该是sending
            ...
            && MMISMS_SENDSTATUS_SAVING != s_mmisms_sendlist.sendlist_head_ptr->backsend_status)
		{
            ...
            SCI_TraceLow("[MMISMS] MMISMS_AddSendList s_mmisms_sendlist.sendlist_head_ptr->backsend_status=%d",s_mmisms_sendlist.sendlist_head_ptr->backsend_status);
    ...
    if(temp_sendinfo == send_info_ptr)
    {
        SCI_TraceLow("[MMISMS] MMISMS_AddSendList : error return empty box");
        return MMISMS_EMPTY_BOX;
    }
    ...
    SCI_TraceLow("[MMISMS] MMISMS_AddSendList s_mmisms_sendlist.send_num = %d!leave function",s_mmisms_sendlist.send_num);
    return send_result;
}
...
PUBLIC void ClearExpiredSmsInSendList( void )
{
    ...
    while (PNULL != cur_sendinfo)
    {
        ...
        else
        {
            ...
            {
                SCI_TraceLow("[MMISMS] ClearExpiredSmsInSendList one item expired, deleted!");
                s_mmisms_sendlist.send_num --;
            }    
...
PUBLIC BOOLEAN MMISMS_SetBackSendStatus( MMISMS_BACKSENDSTATUS_E send_status, MMISMS_SEND_T * send_info_ptr)
{
    ...
    if (MMISMS_BOX_SENDFAIL == send_info_ptr->cur_box_type || MMISMS_BOX_NOSEND == send_info_ptr->cur_box_type)
    {
        if(send_info_ptr->mosend_update_data.is_mosend_update && send_info_ptr->mosend_update_data.mosend_send_oper == MMISMS_UPDATE_SENDFAIL_SMS)
        {
            order_id = send_info_ptr->mosend_update_data.mosend_update_info.order_id_arr[0];
        }        
        else
        {
...
#ifdef SFR_SUPPORT
    if(send_data_ptr->is_sfr_sms)
    {
        send_info_ptr->send_content.is_sfr_sms = TRUE;
    }
#endif
