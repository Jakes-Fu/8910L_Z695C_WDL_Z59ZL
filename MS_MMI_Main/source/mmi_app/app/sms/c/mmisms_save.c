                                    ...
                                    )
{
    ...
    //============== MET MEX  end ===================
                    ...
                    {//已经收到cnf。发送成功或者失败后发送次数没有到 
                        BOOLEAN   is_exist_next_sms = FALSE;
                        ...
                        g_mmisms_global.update_chat_disp.is_update = TRUE;
                        is_exist_next_sms = MMISMS_IsExistNextSMS(&send_info_ptr->send_content);
                        if(!is_exist_next_sms)
                        {
                            g_mmisms_global.update_chat_disp.is_send_finish = TRUE;      
                        }
                        
                        if(send_info_ptr->mosend_update_data.is_mosend_update)
                        {
                            if(MMISMS_UPDATE_SENDFAIL_SMS == send_info_ptr->mosend_update_data.mosend_send_oper )
                            {
                                g_mmisms_global.update_chat_disp.update_dual_sys = send_info_ptr->mosend_update_data.mosend_update_info.order_id_arr[0]->flag.dual_sys;
                                g_mmisms_global.update_chat_disp.update_record_id = send_info_ptr->mosend_update_data.mosend_update_info.order_id_arr[0]->record_id;
                                g_mmisms_global.update_chat_disp.update_storage = send_info_ptr->mosend_update_data.mosend_update_info.order_id_arr[0]->flag.storage;
                            }
                        }
                        else
                        {
                            g_mmisms_global.update_chat_disp.update_dual_sys = send_info_ptr->send_content.dual_sys;
                            g_mmisms_global.update_chat_disp.update_record_id = send_info_ptr->backsend_update_data.save_send_record_id[0];
                            g_mmisms_global.update_chat_disp.update_storage = send_info_ptr->backsend_update_data.save_send_storage;
                        }

                        SCI_TRACE_LOW("mmisms save is_mosend_update %d, mosend_send_oper %d ",send_info_ptr->mosend_update_data.is_mosend_update,send_info_ptr->mosend_update_data.mosend_send_oper);
                        ...
                        SCI_TRACE_LOW("mmisms save is_update %d, backsend_status %d send_info_ptr->send_content.dual_sys %d",g_mmisms_global.update_chat_disp.is_update,send_info_ptr->backsend_status,send_info_ptr->send_content.dual_sys);
                        if (is_exist_next_sms || MMISMS_CheckNextDestAddr())
                        {//如果后面还有发送状态不变                        
                            send_info_ptr->backsend_status = MMISMS_SENDSTATUS_SENDING;
                        }
    ...
    //             }
                ...
                {                            
                    ...
                    if(PNULL != send_info_ptr)
                    {
                        g_mmisms_global.update_chat_disp.is_update = TRUE;
                        g_mmisms_global.update_chat_disp.is_send_finish = TRUE;                                                        
                        g_mmisms_global.update_chat_disp.is_success = FALSE;
                        g_mmisms_global.update_chat_disp.update_dual_sys = send_info_ptr->send_content.dual_sys;
                        g_mmisms_global.update_chat_disp.update_record_id = send_info_ptr->backsend_update_data.save_send_record_id[0];
                        g_mmisms_global.update_chat_disp.update_storage = send_info_ptr->backsend_update_data.save_send_storage;
                        SCI_TRACE_LOW("mmisms save is_update %d, backsend_status %d send_info_ptr->send_content.dual_sys %d,save_send_record_id %d",g_mmisms_global.update_chat_disp.is_update,
                            send_info_ptr->backsend_status,
                            send_info_ptr->send_content.dual_sys,
                            send_info_ptr->backsend_update_data.save_send_record_id[0]
                            );                    
                        MMISMS_SaveInfoToOrderByResult(
                            sig_ptr->dual_sys,  sig_ptr->storage, sig_ptr->record_id, 
                            save_status, is_exist_sc_addr, folder_type,  oper_status,  send_info_ptr, MMISMS_SENDSTATUS_SENDFAIL);
    ...
    //============== MET MEX  end ===================
           if (oper_status==MMISMS_SAVE_SMS)
           {
            ...
            else if (MMISMS_SENDSTATUS_SENDING == send_info_ptr->backsend_status)
            {
                ...
                is_exist_sc_addr = TRUE;
                save_status = MN_SMS_MO_TO_BE_SEND;
                if((MN_SMS_MN_BUSY !=  sig_ptr->cause)&&( MN_SMS_SIM_NOT_READY != sig_ptr->cause))
                 {
                ...
                {
                            SCI_TraceLow("mmisms:   MN_SMS_MN_BUSY, Do not MMISMS_SaveInfoToOrderByResult sig_ptr->cause is %d",  sig_ptr->cause);
                 }
                                           ...
                                           )
{
    ...
    else
    {
        ...
        SCI_TRACE_LOW("[MMISMS] SaveInfoToOrderByResult dest num len = %d", strlen((char*)edit_content_ptr->dest_info.dest_list.dest_addr[addr_id]));
        ...
        SCI_TRACE_LOW("[MMISMS] SaveInfoToOrderByResult number_len = %d, npi_ton = %d", order_info.number.bcd_number.number_len,g_mmisms_global.save_info.number.npi_ton);
        SCI_MEMCPY(order_info.number.bcd_number.number, party_number.bcd_num, party_number.num_len);
    }
                                                ...
                                                )
{
    ...
    if (total_num > 1)
    {
        ...
        else
        {
            ...
            SCI_TRACE_LOW("MMISMS_SaveNextMessage storage = %d, id = %d, addr len = %d", storage, id, strlen((char *)dest_addr_ptr));
        }
...
// no use
    ...
    || (MMISMS_UPDATE_NOSEND_SMS != MMISMS_GetCurrentOperStatus() && update_info_ptr->current_num < update_info_ptr->total_num))
    {
        ...
        if(MMISMS_SPLIT_MAX_NUM> update_info_ptr->current_num)
        {
