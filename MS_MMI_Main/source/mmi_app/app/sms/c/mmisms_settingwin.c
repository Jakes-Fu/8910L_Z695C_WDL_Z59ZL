...
#define  MMISMS_STRING_ALPHA  (255*0.6)
...
#endif
#if defined(HERO_ENGINE_SUPPORT) && defined(HERO_APP_SPAM_MSG_MENU_SUPPORT)
	MMISMS_PDA_SMS_SETTINGS_SPAM_MSG_CLEAN,
#endif
...
typedef enum
{
	MMISMS_MESSAGE_SETTING_SET_SMS,
	MMISMS_MESSAGE_SETTING_SET_MMS,
	MMISMS_MESSAGE_SETTING_SET_CELL_BROADCAST,
	MMISMS_MESSAGE_SETTING_SET_MAX
}MMISMS_MESSAGE_SETTING_SET;
/**--------------------------------------------------------------------------*
**                         EXTERN DECLARE                                   *
**--------------------------------------------------------------------------*/
#ifdef MMI_GUI_STYLE_TYPICAL
...
//    Note:设置信息服务中心号码
/*****************************************************************************/
LOCAL BOOLEAN MMISMS_AppSetSCAddr(                    //RETURN:
#ifdef MMI_SMS_MULTI_PARAM_SUPPORT
                                      MMISMS_SET_SIMMODE_T *set_mode_ptr,
#endif
                                      MN_DUAL_SYS_E   dual_sys,
...
/*****************************************************************************/
// 	Description : SIM message center
//	Global resource dependence :
//  Author:
//	Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleInputSimCenterWinMsg(
                                       MMI_WIN_ID_T        win_id,    //IN:
                                       MMI_MESSAGE_ID_E msg_id,    //IN:
                                       DPARAM            param    //IN:
                                       );
...
/*****************************************************************************/
//     Description : to handle Message Settting window
//    Global resource dependence :
//  Author: yuguang.liu
//    Note:
/*****************************************************************************/
LOCAL MMI_RESULT_E HandleMessageSettingWinMsg(
												 MMI_WIN_ID_T        win_id,        //IN:
												 MMI_MESSAGE_ID_E    msg_id,        //IN:
												 DPARAM                param        //IN:
                                                        );
...
{
    WIN_TITLE( STR_MSG_HDR_ALPHABET),
...
#ifndef MMI_PDA_SUPPORT
    WIN_SOFTKEY(TXT_COMMON_OK, STXT_SOFTKEY_SELECT_MK, TXT_COMMON_COMMON_BACK),
#endif
    END_WIN
};
#endif
...
WINDOW_TABLE( MMISMS_SELECTSIMMODE_WIN_TAB ) =
{
    ...
    WIN_SOFTKEY(STXT_SOFTKEY_SELECT_MK, TXT_EDIT, STXT_RETURN),
    END_WIN
};
...
#endif
    WIN_SOFTKEY(STXT_SOFTKEY_SAVE_MK, TXT_EDIT, STXT_RETURN),
    END_WIN
};
...
WINDOW_TABLE( MMISMS_MESSAGE_SETTING_WIN_TAB ) =
{
    WIN_TITLE( STXT_SMS_OPTIONS_MENU ),
    WIN_FUNC( (uint32)HandleMessageSettingWinMsg),
	WIN_ID(MMISMS_MESSAGE_SETTING_WIN_ID),
	WIN_STATUSBAR,
	CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMISMS_SMS_MESSAGE_SETTING_CTRL_ID),
	WIN_SOFTKEY(PNULL, STXT_SOFTKEY_SELECT_MK, TXT_COMMON_COMMON_BACK),
	END_WIN
};

#ifdef HERO_ENGINE_SUPPORT
#define MSG_SET_SPAM_MSG_CLEAN_TYPE_UPDATE   	0x3e20
int hero_get_clean_spam_msg_switch(void);
void hero_set_clean_spam_msg_switch(int onoff);
#ifdef HERO_APP_SPAM_MSG_MENU_SUPPORT
LOCAL MMI_RESULT_E HandleSpamMsgCleanMainMenuWinMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param);
WINDOW_TABLE( MMISMS_SET_SPAM_MSG_CLEAN_WIN_TAB ) = 
{
	WIN_TITLE(STR_HERO_SPAM_MSG_CLEAR),
	WIN_FUNC((uint32)HandleSpamMsgCleanMainMenuWinMsg),
	WIN_ID(MMISMS_SET_SPAM_MSG_CLEAN_WIN_ID),
	CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMISMS_SET_SPAM_MSG_CLEAN_CTRL_ID),
#ifndef MMI_PDA_SUPPORT
	WIN_SOFTKEY(TXT_COMMON_OK, TXT_NULL, STXT_RETURN),
#endif
    END_WIN  
};
LOCAL MMI_RESULT_E HandleSpamMsgCleanHelpWinMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param);
WINDOW_TABLE(MMISMS_SET_SPAM_MSG_CLEAN_HELP_WIN_TAB) = 
{
    WIN_FUNC((uint32)HandleSpamMsgCleanHelpWinMsg),
    WIN_ID(MMISMS_SET_SPAM_MSG_CLEAN_HELP_WIN_ID),
    WIN_TITLE(STR_HERO_SPAM_MSG_CLEAR_HELP_TITLE),
    CREATE_TEXT_CTRL(MMISMS_SET_SPAM_MSG_CLEAN_HELP_CTRL_ID),
#ifndef MMI_PDA_SUPPORT
    WIN_SOFTKEY(TXT_NULL, TXT_NULL, STXT_EXIT),
#endif
    END_WIN    
};
LOCAL MMI_RESULT_E HandleSpamMsgCleanSetCloseAckWinMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param);
WINDOW_TABLE(MMISMS_SET_SPAM_MSG_CLEAN_CLOSE_ACK_WIN_TAB) = 
{
    WIN_FUNC((uint32)HandleSpamMsgCleanSetCloseAckWinMsg),
    WIN_ID(MMISMS_SET_SPAM_MSG_CLEAN_SET_CLOSE_ACK_WIN_ID),
    WIN_TITLE(STR_HERO_SPAM_MSG_CLEAR_SET),
    CREATE_TEXT_CTRL(MMISMS_SET_SPAM_MSG_CLEAN_SET_CLOSE_ACK_CTRL_ID),
#ifndef MMI_PDA_SUPPORT
	WIN_SOFTKEY(TXT_COMMON_OK, TXT_NULL, STXT_RETURN),
#endif
    END_WIN    
};
LOCAL MMI_RESULT_E	HandleSpamMsgCleanSettingsWin(MMI_WIN_ID_T    win_id, MMI_MESSAGE_ID_E   msg_id, DPARAM param);
WINDOW_TABLE( MMISMS_SET_SPAM_MSG_CLEAN_SET_WIN_TAB ) = 
{
    WIN_FUNC((uint32)HandleSpamMsgCleanSettingsWin),
    WIN_ID( MMISMS_SET_SPAM_MSG_CLEAN_SET_WIN_ID ),
    WIN_TITLE(STR_HERO_SPAM_MSG_CLEAR_SET),
#ifndef MMI_PDA_SUPPORT
    WIN_SOFTKEY(STXT_SELECT, TXT_NULL, STXT_FINISH),
#endif
    CREATE_LISTBOX_CTRL(GUILIST_TEXTLIST_E, MMISMS_SET_SPAM_MSG_CLEAN_SET_LIST_CTRL_ID),
    END_WIN
};
LOCAL MMI_RESULT_E HandleSpamMsgCleanMainMenuWinMsg(
                                          MMI_WIN_ID_T win_id, 
                                          MMI_MESSAGE_ID_E msg_id, 
                                          DPARAM param             
                                          )
{
	...
	MMI_CTRL_ID_T               ctrl_id    =   MMISMS_SET_SPAM_MSG_CLEAN_CTRL_ID;
	uint16 cur_item = 0;
	GUILIST_ITEM_T      item_t = {0};
	GUILIST_ITEM_DATA_T item_data = {0};
	uint16 max_items = 2;
	MMI_TEXT_ID_T str_id[] = {
		STR_HERO_SPAM_MSG_CLEAR_SET,
		STR_HERO_SPAM_MSG_CLEAR_HELP_TITLE,
	};
	...
	switch(msg_id)
	{
		...
		case MSG_APP_WEB:
			...
			{
				case 0:
					MMK_CreateWin((uint32 *)MMISMS_SET_SPAM_MSG_CLEAN_SET_WIN_TAB, PNULL);
				break;
				case 1:
					MMK_CreateWin((uint32 *)MMISMS_SET_SPAM_MSG_CLEAN_HELP_WIN_TAB, PNULL);
				break;
				default:
				break;
			}
...
LOCAL MMI_RESULT_E HandleSpamMsgCleanHelpWinMsg(
								MMI_WIN_ID_T win_id, 
								MMI_MESSAGE_ID_E msg_id, 
								DPARAM param 
								)
{
	MMI_RESULT_E result = MMI_RESULT_TRUE;
	MMI_CTRL_ID_T ctrl_id = MMISMS_SET_SPAM_MSG_CLEAN_HELP_CTRL_ID;
	MMI_STRING_T content_str = {0};
	switch( msg_id ) 
	{
		case MSG_OPEN_WINDOW:
			MMI_GetLabelTextByLang(STR_HERO_SPAM_MSG_CLEAR_HELP_TITLE, &content_str);
			GUIWIN_SetTitleText(win_id, content_str.wstr_ptr, content_str.wstr_len, FALSE);
			MMI_GetLabelTextByLang(STR_HERO_SPAM_MSG_CLEAR_HELP, &content_str);
		...
		case MSG_CTL_CANCEL:
		case MSG_APP_CANCEL:
#if !defined(MMI_KEY_UP_RED_TO_IDLE)
		case MSG_APP_RED:
#endif
			MMK_CloseWin(win_id);
		break;
...
LOCAL MMI_RESULT_E HandleSpamMsgCleanSetCloseAckWinMsg(
								MMI_WIN_ID_T win_id, 
								MMI_MESSAGE_ID_E msg_id, 
								DPARAM param 
								)
{
	MMI_STRING_T content_str = {0};
	MMI_RESULT_E result = MMI_RESULT_TRUE;
	MMI_CTRL_ID_T ctrl_id = MMISMS_SET_SPAM_MSG_CLEAN_SET_CLOSE_ACK_CTRL_ID;
	switch( msg_id ) 
	{
		case MSG_OPEN_WINDOW:
			MMI_GetLabelTextByLang(STR_HERO_SPAM_MSG_CLEAR_SET, &content_str);
			GUIWIN_SetTitleText(win_id, content_str.wstr_ptr, content_str.wstr_len, FALSE);
			MMI_GetLabelTextByLang(STR_HERO_SPAM_MSG_CLEAR_CLOSE_ACK, &content_str);
			GUITEXT_SetString(ctrl_id,content_str.wstr_ptr,content_str.wstr_len,FALSE);
			MMK_SetAtvCtrl(win_id, ctrl_id);
		break;
		case MSG_GET_FOCUS:
        ...
        case MSG_APP_WEB:
        case MSG_CTL_MIDSK:
			hero_set_clean_spam_msg_switch(0);
			MMK_PostMsg(MMISMS_SET_SPAM_MSG_CLEAN_SET_WIN_ID, MSG_SET_SPAM_MSG_CLEAN_TYPE_UPDATE, PNULL, 0);
			MMK_CloseWin(win_id);
		break;
		case MSG_CLOSE_WINDOW:
		break;
...
LOCAL void InitSpamMsgCleanSettingsListCtrl(MMI_CTRL_ID_T ctrl_id)
{
	MMI_STRING_T  text_str = {0};
	GUILIST_SetMaxItem(ctrl_id, 1, FALSE );
	AppendPdaSettingsItem2Line(
		ctrl_id,
		STR_HERO_SPAM_MSG_CLEAR_AUTO,
		text_str,
		(hero_get_clean_spam_msg_switch() ? IMAGE_CHECK_SELECTED_ICON : IMAGE_CHECK_UNSELECTED_ICON),
		(uint32)0);
}
LOCAL MMI_RESULT_E	HandleSpamMsgCleanSettingsWin(
								MMI_WIN_ID_T    win_id, 
								MMI_MESSAGE_ID_E   msg_id, 
								DPARAM			  param
								)
{
	...
	MMI_CTRL_ID_T	ctrl_id 	=	MMISMS_SET_SPAM_MSG_CLEAN_SET_LIST_CTRL_ID;
	uint16 index = 0;
	switch(msg_id)
	{
	case MSG_OPEN_WINDOW:
		InitSpamMsgCleanSettingsListCtrl(ctrl_id);
		MMK_SetAtvCtrl(win_id, ctrl_id);
	break;
	case MSG_SET_SPAM_MSG_CLEAN_TYPE_UPDATE:
...
#endif
	{
		...
		{
			case 0:
				if(hero_get_clean_spam_msg_switch())
				{
					MMK_CreateWin((uint32 *)MMISMS_SET_SPAM_MSG_CLEAN_CLOSE_ACK_WIN_TAB, PNULL);
					return recode;
				}
				else
				{
					hero_set_clean_spam_msg_switch(!hero_get_clean_spam_msg_switch());
				}
			break;
			default:
			break;
		}
		MMK_PostMsg(win_id, MSG_SET_SPAM_MSG_CLEAN_TYPE_UPDATE, PNULL, 0);
...
#else
#define HERO_SPAM_MSG_CLEAR_OPERAT_MAX_COUNT    (5 - 1) //5次
enum {
	TYPE_SPAM_MSG_CLEAR_TIP = 0,
	TYPE_SPAM_MSG_CLEAR_FAIELD,
	TYPE_SPAM_MSG_CLEAR_SUCCESS,
	TYPE_SPAM_MSG_CLEAR_END,
};
typedef struct tagSpamMsgTipDataSt{
	int boxtype;
}SpamMsgTipDataSt;
void hero_get_operate_count(int *okcount, int *cancelcount);
void hero_set_operate_count(int okcount, int cancelcount);
static void hero_spam_msg_clear_ok(MMI_WIN_ID_T win_id);
static void hero_spam_msg_clear_cancel(MMI_WIN_ID_T win_id);
int hero_start_clean_spam_msg(int rmmaxcount, int nvmaxcount, int simmaxcount, int isauto, void *callback, void *userdata);
LOCAL MMI_RESULT_E HandleSpamMsgCleanTipWinMsg(MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param);
WINDOW_TABLE(MMISMS_SET_SPAM_MSG_CLEAN_WIN_TAB) = 
{
    WIN_FUNC((uint32)HandleSpamMsgCleanTipWinMsg),
    WIN_ID(MMISMS_SET_SPAM_MSG_CLEAN_WIN_ID),
    WIN_TITLE(STR_HERO_SPAM_MSG_CLEAR),
    CREATE_TEXT_CTRL(MMISMS_SET_SPAM_MSG_CLEAN_HELP_CTRL_ID),
#ifndef MMI_PDA_SUPPORT
...
static SpamMsgTipDataSt gSpamMsgTipData = {0};
LOCAL MMI_RESULT_E HandleSpamMsgCleanTipWinMsg( MMI_WIN_ID_T win_id, MMI_MESSAGE_ID_E msg_id, DPARAM param )
{
	MMI_STRING_T content_str = {0};
	MMI_RESULT_E result = MMI_RESULT_TRUE;
	...
	{
		case MSG_OPEN_WINDOW:
			gSpamMsgTipData.boxtype = TYPE_SPAM_MSG_CLEAR_TIP;
			MMI_GetLabelTextByLang(STR_HERO_SPAM_MSG_CLEAR, &content_str);
			GUIWIN_SetTitleText(win_id, content_str.wstr_ptr, content_str.wstr_len, FALSE);
			MMI_GetLabelTextByLang(SMS_SETT_HERO_SPAM_MSG_CLEAR_TIP, &content_str);
            GUITEXT_SetString(ctrl_id,content_str.wstr_ptr,content_str.wstr_len,FALSE);
			MMK_SetAtvCtrl(win_id, ctrl_id);
		...
		break;
		case MSG_SET_SPAM_MSG_CLEAN_TYPE_UPDATE:
			GUITEXT_SetDisplayRect(0, MMK_GetCtrlPtr(ctrl_id));
			if(gSpamMsgTipData.boxtype == TYPE_SPAM_MSG_CLEAR_TIP)
			{
				MMI_GetLabelTextByLang(SMS_SETT_HERO_SPAM_MSG_CLEAR_TIP, &content_str);
				GUITEXT_SetString(ctrl_id,content_str.wstr_ptr,content_str.wstr_len, TRUE);
				GUIWIN_SetSoftkeyTextId(win_id,  (MMI_TEXT_ID_T)TXT_COMMON_OK, (MMI_TEXT_ID_T)TXT_NULL, (MMI_TEXT_ID_T)STXT_RETURN, TRUE);
			}
			else if(gSpamMsgTipData.boxtype == TYPE_SPAM_MSG_CLEAR_FAIELD)
			{
				MMI_GetLabelTextByLang(SMS_SETT_HERO_SPAM_MSG_CLEAR_FAILED_TIP, &content_str);
				GUITEXT_SetString(ctrl_id,content_str.wstr_ptr,content_str.wstr_len, TRUE);
				GUIWIN_SetSoftkeyTextId(win_id,  (MMI_TEXT_ID_T)TXT_NULL, (MMI_TEXT_ID_T)TXT_NULL, (MMI_TEXT_ID_T)STXT_RETURN, TRUE);
			}
			else if(gSpamMsgTipData.boxtype == TYPE_SPAM_MSG_CLEAR_SUCCESS)
			{
				MMI_GetLabelTextByLang(SMS_SETT_HERO_SPAM_MSG_CLEANING, &content_str);
				GUITEXT_SetString(ctrl_id,content_str.wstr_ptr,content_str.wstr_len, TRUE);
				GUIWIN_SetSoftkeyTextId(win_id,  (MMI_TEXT_ID_T)TXT_NULL, (MMI_TEXT_ID_T)TXT_NULL, (MMI_TEXT_ID_T)STXT_RETURN, TRUE);
			}
			else if(gSpamMsgTipData.boxtype == TYPE_SPAM_MSG_CLEAR_END)
			{
				MMI_GetLabelTextByLang(SMS_SETT_HERO_SPAM_MSG_CLEAR_DONE, &content_str);
				GUITEXT_SetString(ctrl_id,content_str.wstr_ptr,content_str.wstr_len, TRUE);
				GUIWIN_SetSoftkeyTextId(win_id,  (MMI_TEXT_ID_T)TXT_NULL, (MMI_TEXT_ID_T)TXT_NULL, (MMI_TEXT_ID_T)STXT_RETURN, TRUE);
			}
		break;
		...
		case MSG_CTL_CANCEL:
		case MSG_APP_CANCEL:
	#if 0//!defined(MMI_KEY_UP_RED_TO_IDLE)
		case MSG_APP_RED:
	#endif
			hero_spam_msg_clear_cancel(win_id);
		break;
		case MSG_CLOSE_WINDOW:
		break;
		default:
...
static void box_clean_spam_msg_callback(void *userdata)
{
	if(MMK_IsOpenWin(MMISMS_SET_SPAM_MSG_CLEAN_WIN_ID))
	{
		gSpamMsgTipData.boxtype = TYPE_SPAM_MSG_CLEAR_END;
		MMK_PostMsg(MMISMS_SET_SPAM_MSG_CLEAN_WIN_ID, MSG_SET_SPAM_MSG_CLEAN_TYPE_UPDATE, PNULL, 0);
	}
}
static void hero_spam_msg_clear_ok(MMI_WIN_ID_T win_id)
{
	if(gSpamMsgTipData.boxtype == TYPE_SPAM_MSG_CLEAR_TIP)
	{
		int okcount = 0; int cancelcount = 0;
		MN_DUAL_SYS_E activeSim = MMIAPISET_GetActiveSim();
		uint16 nvMaxNum = 0; uint16 NvUsedNum = 0; uint16 NvSmsUsedNum = 0;
		uint16 simMaxNum = 0; uint16 simUsedNum = 0; uint16 simSmsUsedNum = 0;
		MMISMS_GetSMSUsedAndMax(activeSim, TRUE, &nvMaxNum, &NvUsedNum, &NvSmsUsedNum);
		MMISMS_GetSMSUsedAndMax(activeSim, FALSE, &simMaxNum, &simUsedNum, &simSmsUsedNum);
		hero_get_operate_count(&okcount, &cancelcount);
		if(hero_start_clean_spam_msg(nvMaxNum + simMaxNum, nvMaxNum, simMaxNum, 1, (void*)box_clean_spam_msg_callback, NULL) > 0)
		{
			gSpamMsgTipData.boxtype = TYPE_SPAM_MSG_CLEAR_SUCCESS;
			MMK_PostMsg(win_id, MSG_SET_SPAM_MSG_CLEAN_TYPE_UPDATE, PNULL, 0);
			hero_set_operate_count(0, 0);
		}
		else
		{
			gSpamMsgTipData.boxtype = TYPE_SPAM_MSG_CLEAR_FAIELD;
			MMK_PostMsg(win_id, MSG_SET_SPAM_MSG_CLEAN_TYPE_UPDATE, PNULL, 0);
			if(okcount >= HERO_SPAM_MSG_CLEAR_OPERAT_MAX_COUNT) {
				hero_set_clean_spam_msg_switch(0);
			}
			hero_set_operate_count(okcount + 1, 0);
		}
	}
}
static void hero_spam_msg_clear_cancel(MMI_WIN_ID_T win_id)
{
	if(gSpamMsgTipData.boxtype == TYPE_SPAM_MSG_CLEAR_TIP)
	{
		int okcount = 0; int cancelcount = 0;
		hero_get_operate_count(&okcount, &cancelcount);
		if(cancelcount >= HERO_SPAM_MSG_CLEAR_OPERAT_MAX_COUNT) {
			hero_set_clean_spam_msg_switch(0);
		}
		hero_set_operate_count(0, cancelcount + 1);
	}
	MMK_CloseWin(win_id);
}
void hero_spam_msg_clear_show_tip_win(void)
{
	if(!MMK_IsOpenWin(MMISMS_SET_SPAM_MSG_CLEAN_WIN_ID)) {
		MMK_CreateWin((uint32*)MMISMS_SET_SPAM_MSG_CLEAN_WIN_TAB, PNULL);
	}
}
#endif
#endif
...
)
{
	...
	MMI_CTRL_ID_T ctrl_id = MMISMS_SELECTVPWIN_LISTBOX_CTRL_ID;
#ifdef MMI_SMS_MULTI_PARAM_SUPPORT
    ...
    GUILIST_SetListState(ctrl_id, GUILIST_STATE_SPLIT_LINE, FALSE);
     for (i = 0; i < MMISMS_VP_ITEM; i++)
     {
		...
		item_data.item_content[0].item_data.text_id = (STXT_SMS_1_H + i);

 		item_data.softkey_id[LEFT_BUTTON] = PNULL;
 		item_data.softkey_id[MIDDLE_BUTTON] = STXT_SOFTKEY_SELECT_MK;
 		item_data.softkey_id[RIGHT_BUTTON] = TXT_COMMON_COMMON_BACK;
 		//ctrl_id=MMK_GetCtrlHandleByWin(MMISMS_SELECTVP_WIN_ID,MMISMS_SELECTVPWIN_LISTBOX_CTRL_ID );
        GUILIST_AppendItem( ctrl_id, &item_t );
     }

	 GUILIST_SetListState(MMISMS_SELECTVPWIN_LISTBOX_CTRL_ID, GUILIST_STATE_OWN_SOFTKEY, TRUE);
    ...
    if (5 == cur_selection)
    {
        GUILIST_SetTopItemIndex(MMISMS_SELECTVPWIN_LISTBOX_CTRL_ID, cur_selection);
    }
}
...
)
{
   ...
   MMK_CreateWin((uint32*) MMISMS_SELECTVP_WIN_TAB, PNULL );
...
#endif
	MMI_GetLabelTextByLang(STXT_HEADER_VALIDITY,&str_validity);
    switch (msg_id)
    {
    case MSG_OPEN_WINDOW:
		// set status bar
		GUISTBDATA_GetItemText(index,&stb_data_text);
        stb_data_text.wstr_len = str_validity.wstr_len;
        SCI_MEMCPY(stb_data_text.wstr_ptr, str_validity.wstr_ptr,
                                               ...
                                               )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        MMIAPISET_AppendListItemByTextId( STR_ILIST_SIM_EXT01, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
        MMIAPISET_AppendListItemByTextId( TXT_PHONE, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
...
#ifdef MMI_GUI_STYLE_TYPICAL
        //提示保存成功
        MMIPUB_OpenAlertWinByTextId(PNULL,STXT_SOFTKEY_SAVE_MK,TXT_NULL,IMAGE_PUBWIN_SUCCESS,PNULL,PNULL,MMIPUB_SOFTKEY_NONE,PNULL);
#endif
                                       ...
                                       )
{
	...
	MMI_CTRL_ID_T ctrl_list_id = MMISMS_SMS_SETTING_MENU_CTRL_ID;
	uint16 index = 0;
	MMI_RESULT_E recode = MMI_RESULT_TRUE;
    MMI_CTRL_ID_T ctrl_id= MMIPUB_INPUTFIELD_CTRL_ID;
    MMI_STRING_T num_string = {0};
#ifdef MMI_SMS_MULTI_PARAM_SUPPORT
#if defined (MMS_SUPPORT) || defined (BROWSER_SUPPORT)
    MMI_WIN_ID_T main_win_id = MMISMS_SETTINGS_TAB_WIN_ID;
    ...
    MMISMS_SETTING_MAIN_WIN_PARAM_T * param_to_create_setting_main_win = MMK_GetWinUserData(main_win_id);
#endif
	MN_DUAL_SYS_E dual_sys = CTRLLIST_GetCurItemIndex(ctrl_list_id);
    //SCI_TRACE_LOW:"MMISMS: HandleInputSCWinMsg msg_id = 0x%x"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMISMSAPP_WINTAB_6150_112_2_18_2_54_32_738,(uint8*)"d", msg_id);
    switch (msg_id)
    {
    case MSG_OPEN_WINDOW:
//		MMK_SetAtvCtrl(win_id, ctrl_id);
        GUIEDIT_SetMaxLen(ctrl_id, MMISMS_MAX_SC_NUMBER_LEN);
		MMIPUB_SetInputDialogEditorType(win_id,DIALOG_TEL_EDITOR);
			...
			)
		{
			if (sc_num_len > 0)
			{
				MMI_STRNTOWSTR( sc_num_wstr, MMISMS_STRNUM_MAX_LEN, (const uint8 *)sc_num, MMISMS_STRNUM_MAX_LEN, sc_num_len);
				GUIEDIT_SetString(MMIPUB_INPUTFIELD_CTRL_ID, sc_num_wstr, (uint16)MMIAPICOM_Wstrlen(sc_num_wstr));
			}
	   }
        break;
	case MSG_FULL_PAINT:
    ...
    case MSG_CTL_MIDSK:    
    case MSG_APP_WEB:
    //case MSG_KEYUP_WEB: for skip msg from dailog control
        //to get the input number
        SCI_MEMSET(&num_string, 0, sizeof (MMI_STRING_T));
        GUIEDIT_GetString(ctrl_id, &num_string);
        ...
        if (num_string.wstr_len > 0)
        {
            if (IsSCAddrValid(&num_string))
            {
                ...
                if((num_string.wstr_len == 1) && (num_str[0] == '+'))
                {
                     break;
                }
    ...
    default:
        ...
        // need keep not gray for empty prompt to make user know what happended(just can keep current softkey design
        {
            MMIPUB_INFO_T     *win_info_ptr = MMIPUB_GetWinAddDataPtr(win_id);
            MMI_HANDLE_T      edit_ctrl_handle = NULL;
            MMI_STRING_T text_str = {0};
            if(PNULL == win_info_ptr) //fix coverity 27152
            {
                break;
            }
...
LOCAL void MMISMS_OpenSetSCWin(MN_DUAL_SYS_E dual_sys)
{
...
#endif
#ifndef MMI_MULTI_SIM_SYS_SINGLE
	if ( (MN_DUAL_SYS_E)0 == dual_sys)
	{
		MMI_GetLabelTextByLang(STR_SIM1_MSG_CENTRE_EXT01,&content_str);
	}
	else if((MN_DUAL_SYS_E)1 == dual_sys)
	{
		MMI_GetLabelTextByLang(STR_SIM2_MSG_CENTRE_EXT01,&content_str);
	}
	else
	{
		return;
	...
	if ( (MN_DUAL_SYS_E)0 == dual_sys)
	{
		MMI_GetLabelTextByLang(STR_MSG_CENTRE_EXT01,&content_str);
	}
	else
	{
		return;
...
#endif
	MMIPUB_OpenDialogInputFieldWinByTextPtr(&content_str, PNULL, PNULL, MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN, HandleInputSimCenterWinMsg);
#if 0
                                 ...
                                 )
{
    ...
    item_t.item_data_ptr = &item_data;
    item_t.user_data     = user_data;
    if (item_style == GUIITEM_STYLE_TOGGLE_ONE_LINE_TEXT || item_style ==GUIITEM_STYLE_TOGGLE_TWO_LINE_TEXT_TEXT)
    {
    ...
    //text 2
    if(text_str.wstr_ptr != PNULL && text_str.wstr_len != 0 && GUIITEM_STYLE_TWO_LINE_TEXT_AND_TEXT_MS != item_style)
    {
    ...
    else if (GUIITEM_STYLE_TWO_LINE_TEXT_AND_TEXT_MS == item_style && text_str.wstr_ptr != PNULL && text_str.wstr_len != 0)
    {
        GUILIST_SetItemState(&item_t, GUIITEM_STATE_COSTMER_FONT_COLOR, TRUE);
        //设置第1个content的字体颜色、透明度,包括高亮状态下和正常状态下
        GUILIST_SetItemContentCustomFontColor(ctrl_id, 1, &color, MMISMS_STRING_ALPHA, &color,  MMISMS_STRING_ALPHA);//设置第1个content的字体颜色,包括高亮状态下和正常状态下
...
LOCAL void  InitPdaSmsSettingsListCtrl(BOOLEAN is_need_replace)
{
    ...
    {
#ifndef MMI_MULTI_SIM_SYS_SINGLE
			STR_SIM1_MSG_CENTRE_EXT01,
			STR_SIM2_MSG_CENTRE_EXT01,
#if defined(MMI_MULTI_SIM_SYS_TRI)||defined(MMI_MULTI_SIM_SYS_QUAD)
    ...
    if((is_need_replace && index == focus_index) || !is_need_replace)
    {
        ...
        MMI_GetLabelTextByLang(STXT_SMS_1_H + cur_selection, &text_str);
        AppendSmsPdaSettingsItem2Line(
            ctrl_id,
           //  TXT_SMS_SETTING_VP,
		   STXT_SMS_VALIDITY_PERIOD,
            text_str,
            IMAGE_COMMON_RIGHT_ARROW,
    ...
    if((is_need_replace && index == focus_index) || !is_need_replace)
    {
        ...
        #if 1//wuxx add 解决报读发送情况报告再报 --最长时限--的问题
        SCI_MEMSET(&text_str, 0x00, sizeof(text_str)); 
        #endif
        AppendSmsPdaSettingsItem2Line(
            ctrl_id,
            //TXT_SMS_STATUSREPORT,
			STXT_SMS_DELIVERY_REPORTS,
            text_str,
            image_id,
		...
		//	GUIITEM_STYLE_ONE_LINE_TEXT_ICON
		 ...
		 GUILIST_SetToggleState(ctrl_id,index,flag);
    }
    index++;
...
#if 0//def MMI_SMS_REPLY_PATH_SUPPORT
    if ((is_need_replace && index == focus_index) || !is_need_replace)
    {
    ...
    if ((is_need_replace && index == focus_index) || !is_need_replace)
    {
        ...
        case MMISMS_ALPHABET_MANAGEMENT_GSM_ALPHABET:
            MMI_GetLabelTextByLang(STR_MSG_ALPHABET_GSM, &text_str);
            break;

        case MMISMS_ALPHABET_MANAGEMENT_8BIT:
            //MMI_GetLabelTextByLang(TXT_SMS_ALPHABET_8BIT, &text_str);  // no such menu
            break;

        case MMISMS_ALPHABET_MANAGEMENT_UCS2:
            MMI_GetLabelTextByLang(STR_MSG_ALPHABET_UCS2, &text_str);
            break;

        case MMISMS_ALPHABET_MANAGEMENT_AUTO:
            MMI_GetLabelTextByLang(STR_MSG_ALPHABET_AUTO, &text_str);
            break;
    ...
    if((is_need_replace && index == focus_index) || !is_need_replace)
    {
        ...
        case MMINV_SMS_SETTING_SAVE:
            MMI_GetLabelTextByLang(STXT_SOFTKEY_SAVE_MK, &text_str);
            break;
    ...
    if((is_need_replace && index == focus_index) || !is_need_replace)
   	{
		text_str.wstr_len = 0; text_str.wstr_ptr = NULL;
		AppendSmsPdaSettingsItem2Line(
			ctrl_id,
			STR_HERO_SPAM_MSG_CLEAR,
			text_str,
			image_id,
			(uint32)MMISMS_PDA_SMS_SETTINGS_SPAM_MSG_CLEAN,
			is_need_replace,
			GUIITEM_STYLE_TWO_LINE_TEXT_AND_TEXT_MS);
    }
	index++;
                                              ...
                                              )
{
    ...
    static BOOLEAN mmisms_sc_editable = FALSE;
    ...
    case MSG_OPEN_WINDOW:
        ...
        ret_value = MMISMS_GetNDTNvSetting(MESSAGES_NV_SMSC_EDITABLE,&mmisms_sc_editable);
        if(ret_value != MN_RETURN_SUCCESS )
        {
            mmisms_sc_editable = TRUE;
            SCI_TRACE_LOW("mmisms setting ndt read error , sc editable default value set to %d",mmisms_sc_editable);
        }

        if(mmisms_sc_editable)
        {
            for(i = MN_DUAL_SYS_1; i<MMI_DUAL_SYS_MAX; i++)
            {
                ...
                #else
                    GUILIST_SetItemInvalid(ctrl_id, (i - MN_DUAL_SYS_1), TRUE);
...
#endif
#if defined(HERO_ENGINE_SUPPORT) && defined(HERO_APP_SPAM_MSG_MENU_SUPPORT)
		case MMISMS_PDA_SMS_SETTINGS_SPAM_MSG_CLEAN:
		{
            MMK_CreateWin((uint32*)MMISMS_SET_SPAM_MSG_CLEAN_WIN_TAB, PNULL);
		}
		break;
		...
		case MMISMS_PDA_SMS_SETTINGS_STATUS_REPORT:
            {
                BOOLEAN is_flag = !MMISMS_GetStatusReportFlag();
                MMISMS_SetStatusReportFlag(is_flag);
				GUILIST_SetToggleState(ctrl_id,index,is_flag);
            }
			break;
                         ...
                         )
{
    ...
    //append item
    MMIAPISET_AppendListItemByTextId(TXT_COMMON_OPEN,STXT_SOFTKEY_OK_MK,ctrl_id,GUIITEM_STYLE_ONE_LINE_RADIO);
    MMIAPISET_AppendListItemByTextId(TXT_COMMON_CLOSE,STXT_SOFTKEY_OK_MK,ctrl_id,GUIITEM_STYLE_ONE_LINE_RADIO);
                         ...
                         )
{
    ...
    //append item
    MMIAPISET_AppendListItemByTextId(TXT_COMMON_ALWAYS,STXT_SOFTKEY_SAVE_MK,ctrl_id,GUIITEM_STYLE_ONE_LINE_RADIO);
    MMIAPISET_AppendListItemByTextId(TXT_COMMON_PROMPT,STXT_SOFTKEY_SAVE_MK,ctrl_id,GUIITEM_STYLE_ONE_LINE_RADIO);
    MMIAPISET_AppendListItemByTextId(TXT_COMMON_NEVER,STXT_SOFTKEY_SAVE_MK,ctrl_id,GUIITEM_STYLE_ONE_LINE_RADIO);
                                               ...
                                               )
{
    ...
    case MSG_OPEN_WINDOW:
        // add the item to listbox
        GUILIST_SetMaxItem(ctrl_id, 2, FALSE );//max item 2
        MMIAPISET_AppendListItemByTextId( TXT_COMMON_ENABLE_ON, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
                                               ...
                                               )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        MMIAPISET_AppendListItemByTextId( STR_MSG_ALPHABET_GSM, TXT_COMMON_OK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
        //MMIAPISET_AppendListItemByTextId( TXT_SMS_ALPHABET_8BIT, TXT_COMMON_OK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
        MMIAPISET_AppendListItemByTextId( STR_MSG_ALPHABET_UCS2, TXT_COMMON_OK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
        MMIAPISET_AppendListItemByTextId( STR_MSG_ALPHABET_AUTO, TXT_COMMON_OK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
        ...
        {
             case MMISMS_ALPHABET_MANAGEMENT_GSM_ALPHABET:   //reduced coding
                cur_selection = 0;
                break;
             case MMISMS_ALPHABET_MANAGEMENT_UCS2:   //full coding
                cur_selection = 1;
                break;
             case MMISMS_ALPHABET_MANAGEMENT_AUTO:   //automatic
                cur_selection = 2;
                break;
              default:
                cur_selection = 2;
        ...
        GUILIST_SetCurItemIndex(ctrl_id, cur_selection);
        // set status bar
        MMI_GetLabelTextByLang(STR_MSG_HDR_ALPHABET,&str_alphabet);
        GUISTBDATA_GetItemText(index,&stb_data_text);
        stb_data_text.wstr_len = str_alphabet.wstr_len;
        SCI_MEMCPY(stb_data_text.wstr_ptr, str_alphabet.wstr_ptr,
        sizeof(wchar)*MIN(stb_data_text.wstr_len, MMISTATUSBAR_ITEM_TEXT_LEN));
        GUISTBDATA_SetItemText(0,&stb_data_text);
        MMK_SetAtvCtrl(win_id, ctrl_id);
        break;
    ...
    case MSG_CTL_MIDSK:
        ...
        {
             case 0:
                alpha_management = MMISMS_ALPHABET_MANAGEMENT_GSM_ALPHABET;
                break;
             case 1:
                alpha_management = MMISMS_ALPHABET_MANAGEMENT_UCS2;
                break;
             case 2:
                                               ...
                                               )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        MMIAPISET_AppendListItemByTextId(TXT_SMS_PREFERRED_GPRS, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
        MMIAPISET_AppendListItemByTextId(TXT_SMS_PREFERRED_GSM_ONLY, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
        MMIAPISET_AppendListItemByTextId(TXT_SMS_PREFERRED_GPRS_ONLY, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
                        ...
                        )
{
    ...
    // add the item to listbox
    MMIAPISET_AppendListItemByTextId( TXT_COMMON_ENABLE_ON, STXT_SOFTKEY_OK_MK, MMISMS_SELECTSRWIN_LISTBOX_CTRL_ID, GUIITEM_STYLE_ONE_LINE_RADIO);
    MMIAPISET_AppendListItemByTextId( STR_POC_ILIST_OFF, STXT_SOFTKEY_OK_MK, MMISMS_SELECTSRWIN_LISTBOX_CTRL_ID, GUIITEM_STYLE_ONE_LINE_RADIO);
                                        ...
                                        )
{
    ...
    case MSG_OPEN_WINDOW:
        ...
        MMIAPISET_AppendListItemByTextId(TXT_SMS_PROMPT, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
        MMIAPISET_AppendListItemByTextId(STXT_SOFTKEY_SAVE_MK, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
        MMIAPISET_AppendListItemByTextId(TXT_SMS_NOT_SAVE, STXT_SOFTKEY_OK_MK, ctrl_id, GUIITEM_STYLE_ONE_LINE_RADIO);
                                                        ...
                                                        )
{
	...
	ctrl_id=MMK_GetCtrlHandleByWin(win_id,MMISMS_SMS_MESSAGE_SETTING_CTRL_ID );
#ifdef MMI_ETWS_SUPPORT
    ...
    case MSG_OPEN_WINDOW:
		...
		item_t.item_data_ptr=&item_data;
		item_data.item_content[0].item_data_type=GUIITEM_DATA_TEXT_ID;
		item_data.item_content[0].item_data.text_id=STR_LIST_SMS_EXT01;
		item_t.user_data = 	MMISMS_MESSAGE_SETTING_SET_SMS;
		GUILIST_AppendItem(ctrl_id,&item_t);

#ifdef MMS_SUPPORT
		item_t.item_data_ptr=&item_data;
		item_data.item_content[0].item_data_type=GUIITEM_DATA_TEXT_ID;
		item_data.item_content[0].item_data.text_id=STR_SMSSE_LIST_MULTIMEDIA;
		item_t.user_data = 	MMISMS_MESSAGE_SETTING_SET_MMS;
		GUILIST_AppendItem(ctrl_id,&item_t);
#endif
		...
		item_data.item_content[0].item_data_type=GUIITEM_DATA_TEXT_ID;
                ...
                {
                    item_data.item_content[0].item_data.text_id = text_cb_ita_Public_setting;
                }
                else
#endif //MMI_ETWS_SUPPORT
                {
                    item_data.item_content[0].item_data.text_id=STR_CB_CELL_BROADCAST;
                }
		item_t.user_data = 	MMISMS_MESSAGE_SETTING_SET_CELL_BROADCAST;
		GUILIST_AppendItem(ctrl_id,&item_t);
		MMK_SetAtvCtrl(win_id, ctrl_id);

#ifdef MMI_ETWS_SUPPORT
            cb_config = MMISMSCB_GetCBDefaultConfig();
            if( CB_CONFIG_DISABLE == cb_config )
            {
                CTRLLIST_SetItemInvalid(ctrl_id,2,TRUE);
                CTRLLIST_SetItemGray(ctrl_id,2,TRUE);
            }
#endif
  ...
  //  case MSG_CTL_OK:
    ...
    case MSG_CTL_MIDSK:
        index = GUILIST_GetCurItemIndex(ctrl_id);
        GUILIST_GetItemData(ctrl_id, index, &user_data);
		switch((MMISMS_MESSAGE_SETTING_SET)user_data)
			{
			case MMISMS_MESSAGE_SETTING_SET_SMS:
				MMK_CreateWin((uint32 *)MMISMS_SMS_SETTING_MENUWIN_TAB, PNULL);
				break;
			case MMISMS_MESSAGE_SETTING_SET_MMS:
			...
			case MMISMS_MESSAGE_SETTING_SET_CELL_BROADCAST:
...
/*****************************************************************************/
//     Description : to enter the window for Setting SMS
//    Global resource dependence :
//  Author:yuguang.liu
//    Note:
/*****************************************************************************/
PUBLIC void MMISMS_OpenMessageSetting(void)
{

	MMK_CreateWin((uint32 *)MMISMS_MESSAGE_SETTING_WIN_TAB, PNULL);
}
