                                      ...
                                      )
{
   ...
   case MSG_CTL_LIST_NEED_ITEM_CONTENT:
    ...
    case MSG_FULL_PAINT:
        if(MMK_GetWinDirty(win_id))
        {
            ...
            else
            {
                ...
                GUIWIN_SeSoftkeytButtonIconId(win_id, PNULL, LEFT_BUTTON, TRUE);
            }
    ...
    case MSG_APP_MENU:
//#ifdef MMI_PDA_SUPPORT
        if (s_have_items && !GUILIST_GetListState(ctrl_id, GUILIST_STATE_ENABLE_MARK) && !GUILIST_GetItemState((GUILIST_ITEM_T*) GUILIST_GetItemPtrByIndex (ctrl_id, GUILIST_GetCurItemIndex(ctrl_id)), GUIITEM_STATE_LEFT_FOCUS))
//#endif
        { 
#ifdef MMI_PDA_SUPPORT            
            s_is_mark_all = FALSE;
			...
			selected_index = GUILIST_GetCurItemIndex(ctrl_id);
            MMISMS_SetPushMsgCurOperationOrderId(selected_index);
            win_data_ptr->cur_focus_order_id = MMISMS_GetPushMsgLinkedArrayPtrId(selected_index);
...
LOCAL void HandlePushMsgOpenMenuOpt(MMIPUB_MENU_NOTIFY_T *param )
{
    ...
    switch(menu_id)
    {
        case MMISMS_MENU_DEL_PUSH_ITEM_ID:
            ...
            MMISMS_OpenDelteQueryWin(delete_type, TRUE, win_data_ptr->cur_focus_order_id,MMISMS_BOX_MT,HandlePushMsgDelQueryWinMsg);
            break;
        case MMISMS_MENU_RETURN_MESSAGE_ITEM_ID:
            MMK_CloseWin(MMISMS_PUSH_MESSAGE_WIN_ID);
            break;
