...
#define MMISMS_MAX_LINE1_TITLE_LEN 50
extern MMI_VOICEMAIL_ITEM_T g_fix_voicemail_item[VOICEMAIL_MAX_COUNT];
...
//    Note: 2006-02-10
/*****************************************************************************/
LOCAL void OpenVMListWin(MN_DUAL_SYS_E dual_sys, MMISMS_VMN_RECORD_ID_E vmn_type);
/*****************************************************************************/
//  Description : 
//  Global resource dependence : 
//  Author: rong.gu 
...
//  Note: 
/*****************************************************************************/
LOCAL void SaveVMAddr2SimByType(MMISMS_VOICEMAIL_WINDATA_T *win_data_ptr,MN_DUAL_SYS_E dual_sys,MMI_STRING_T* num_string, MMISMS_VMN_RECORD_ID_E vmn_type);
#else
 /*****************************************************************************/
...
//    Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E  MMISMS_MakeVoicemailCall(uint32 dual_sys, BOOLEAN isReturnOK, DPARAM param);
...
//  Note:
/*****************************************************************************/
PUBLIC uint8 MMISMS_GetFixVoiceMailNum(void)
{
    uint8 i = 0;
    uint8 fix_voicemail_num = 0;
    
    for(i = 0; i < VOICEMAIL_MAX_COUNT; i++)
    {
        if(strlen(g_fix_voicemail_item[i].viocemail) != 0)
        {
            fix_voicemail_num++;
        }
    }
    return fix_voicemail_num;
}
                                 ...
                                 )
{
    ...
    item_data.softkey_id[0] = (MMI_TEXT_ID_T)TXT_EDIT;
    item_data.softkey_id[1] = (MMI_TEXT_ID_T)TXT_CALL;
...
//  Note: 
/*****************************************************************************/
LOCAL void AppendFixVoiceMailNumberItem2Line(
                                 MMI_CTRL_ID_T  ctrl_id,    //控件ID
                                 MMI_STRING_T  text_1,     //主文本
                                 MMI_STRING_T  text_str,     //副文本
                                 MMI_IMAGE_ID_T imgae_id1,   //图标ID
                                 MMI_IMAGE_ID_T imgae_id2,   //图标ID
                                 uint32         user_data   //用户数据
                                 )
{
    ...
    item_data.item_content[0].item_data.text_buffer = text_1;
...
//  Note: 
/*****************************************************************************/
LOCAL void AppendFixVoiceMailNumberListCtrl(void)
{
    uint8 i = 0;
    uint8 number_count = 0;
    MMI_STRING_T text_str= {0};
    MMI_STRING_T text_line1= {0};
    MMI_STRING_T text_fix_voice_num= {0};
    wchar num[VOICEMAIL_MAX_COUNT][3]= {L"1"};

    MMI_CTRL_ID_T       ctrl_id = MMISMS_VOICEMAIL_SETTING_MENU_CTRL_ID;
    wchar temp_wstr[VOICEMAIL_STRING_LEN + 1] = {0};
    wchar temp_line1[MMISMS_MAX_LINE1_TITLE_LEN] = {0};
    uint16 mail_len = 0;
    for(i = 0; i < VOICEMAIL_MAX_COUNT; i++)
    {        
        mail_len = strlen(g_fix_voicemail_item[i].viocemail);
        if(mail_len != 0)    
        {
            SCI_MEMSET(&text_str, 0 ,sizeof(MMI_STRING_T));
            SCI_MEMSET(&text_line1, 0 ,sizeof(MMI_STRING_T));
            SCI_MEMSET(temp_wstr, 0 ,sizeof(temp_wstr));
            SCI_MEMSET(temp_line1, 0 ,sizeof(temp_line1));

            MMI_GetLabelTextByLang(TXT_SMS_FIX_SIM_SEND_ONOFF, &text_fix_voice_num);
            MMI_WSTRNCPY(temp_line1,MMISMS_MAX_LINE1_TITLE_LEN,text_fix_voice_num.wstr_ptr,MMISMS_MAX_LINE1_TITLE_LEN,text_fix_voice_num.wstr_len);                
            MMIAPICOM_Wstrcat(temp_line1,num[number_count]);
            text_line1.wstr_ptr = temp_line1;
            text_line1.wstr_len =  MMIAPICOM_Wstrlen(temp_line1);
                      
            MMI_STRNTOWSTR(temp_wstr,
                                 VOICEMAIL_STRING_LEN ,
                                 (const uint8 *)g_fix_voicemail_item[i].viocemail,
                                 VOICEMAIL_STRING_LEN ,
                                 mail_len);    
            ...
            number_count++;
            AppendFixVoiceMailNumberItem2Line(
            ctrl_id,
            text_line1,
            text_str,
            IMAGE_NULL,
            IMAGE_NULL,
            (uint32)MMISMS_PDA_SMS_VOICEMAIL_DATA+number_count
            );
...
LOCAL void  InitPdaSmsVoiceMailListCtrl(MMISMS_VOICEMAIL_WINDATA_T *win_data_ptr)
{
    ...
    uint8 maxitemnum = 0;
    //set max item
    maxitemnum = MMISMS_PDA_SMS_VOICEMAIL_MAX+MMISMS_GetFixVoiceMailNum();
    
    GUILIST_SetMaxItem(ctrl_id, maxitemnum, FALSE );   
    ...
    AppendFixVoiceMailNumberListCtrl();
 ...
 case MSG_NOTIFY_LIST_CHECK_CONTENT:
    ...
    case MSG_CTL_OK:
        cur_index = GUILIST_GetCurItemIndex(ctrl_id);
        {
            ...
            if(user_data > MMISMS_PDA_SMS_VOICEMAIL_DATA)
            {
                SCI_TRACE_LOW("mmisms fix voice mail number should not open it");
                return recode;
            }
    ...
    case MSG_APP_WEB:
        cur_index = GUILIST_GetCurItemIndex(ctrl_id);
        {
            ...
            else
            {
                ...
                uint8 voice_mail[MMISMS_PBNUM_MAX_LEN + 2] = {0};
                uint8 mail_len = 0;
                sim_ok_num = MMIAPIPHONE_GetSimAvailableNum(&sim_sys, 1);
                ...
                else if (1 == sim_ok_num)
                {
                    ...
                    MMI_WSTRNTOSTR((uint8 *)voice_mail, 
                        ...
                        item_ptr->item_data_ptr->item_content[2].item_data.text_buffer.wstr_ptr,
                        item_ptr->item_data_ptr->item_content[2].item_data.text_buffer.wstr_len, 
                        item_ptr->item_data_ptr->item_content[2].item_data.text_buffer.wstr_len);
                    mail_len = strlen(voice_mail);
                    cc_result = MMIAPICC_MakeCall(
                                                                    ...
                                                                    PNULL,
                                                                    PNULL,
                                                                    MMIAPICC_GetCallSimType(sim_sys),
                                                                    CC_CALL_NORMAL_CALL,
                                                                    PNULL
                                                                    );
                ...
                {
                    MMISMS_SelectSIMFunc(MMISMS_MakeVoicemailCall,PNULL,FALSE);
                }
                #endif
            }			
        }
...
LOCAL MMI_RESULT_E  MMISMS_MakeVoicemailCall(uint32 dual_sys, BOOLEAN isReturnOK, DPARAM param)
{
    ...
    if (isReturnOK)
    {
        cur_index = GUILIST_GetCurItemIndex(MMISMS_VOICEMAIL_SETTING_MENU_CTRL_ID);
        item_ptr = GUILIST_GetItemPtrByIndex(MMISMS_VOICEMAIL_SETTING_MENU_CTRL_ID,  cur_index);	
        MMI_WSTRNTOSTR((uint8 *)voice_mail, 
            MMISMS_PBNUM_MAX_LEN,
            item_ptr->item_data_ptr->item_content[2].item_data.text_buffer.wstr_ptr,
            item_ptr->item_data_ptr->item_content[2].item_data.text_buffer.wstr_len, 
...
LOCAL MMI_RESULT_E  VoiceMailSelectSimCallback(uint32 dual_sys, BOOLEAN isReturnOK, DPARAM param)
{
    ...
    if (isReturnOK)
    {
#ifndef MMI_GUI_STYLE_TYPICAL
        {
            ...
            for(i = 0; i < (MMISMS_VMN_TYPE_MAX - 1); i++)
            {
...
LOCAL void SaveVMAddr2SimByType(MMISMS_VOICEMAIL_WINDATA_T *win_data_ptr,MN_DUAL_SYS_E dual_sys,MMI_STRING_T* num_string, MMISMS_VMN_RECORD_ID_E vmn_type)
{
    ...
    SCI_MEMCPY( voimess_addr_info.mailbox_number, party_num.bcd_num, voimess_addr_info.mailbox_number_len); // avoid copy overflow
                                       ...
                                       )
{
    ...
    case MMISMS_MSG_UPDATELIST:
        ...
        if(PNULL!=num_string.wstr_ptr)
        {
            ...
            SaveVMAddr2SimByType(win_data_ptr,MMISMS_GetCurVoiceMailDualSys(),&num_string,vmn_type);
