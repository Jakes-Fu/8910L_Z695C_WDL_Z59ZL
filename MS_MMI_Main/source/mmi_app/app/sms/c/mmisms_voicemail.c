...
MMI_VOICEMAIL_ITEM_T g_fix_voicemail_item[VOICEMAIL_MAX_COUNT] = {0};
...
/*****************************************************************************/
//     Description : to get multi variant value
//    Global resource dependence : none
//  Author:rong.gu
//    Note:
/*****************************************************************************/
LOCAL void MMISMS_InitVMMultiVariant(void);
                                           ...
                                           )
{
    ...
    else
    {
        ...
        case MMISMS_VMN_OPER_TYPE_SMS_VM_SETTING:
            ...
            case MMISMS_VMN_OPER_TYPE_IDLE_DIAL:
                ...
                get_vm_addr_info.line_id = s_vmn_type;
    ...
    case MMISMS_VMN_OPER_TYPE_SMS_VM_SETTING:
		...
		//warning,if PS modify, this part should be adjust. now only line1 and line2, no fax and data, so count can not 4
         if(count == 4 || (count >= 2 && (MN_SMS_SIM_READ_FAILED == get_vm_addr_info.cause || MN_SMS_OPERATE_SUCCESS == get_vm_addr_info.cause)))
         {
             count = 0;
             if(MMK_IsOpenWin(MAIN_WAIT_WIN_ID))
             {
    ...
    case MMISMS_VMN_OPER_TYPE_IDLE_DIAL:
        ...
        MMISMS_OpenCallVMWin(get_vm_addr_info.dual_sys, (MMISMS_VMN_RECORD_ID_E)get_vm_addr_info.line_id);
        break;
...
PUBLIC void MMISMS_UpdateVMFlagState(void)
{
    ...
    MMIAPICOM_StatusAreaSetVMFlagState( is_new_vm );
...
PUBLIC void MMISMS_InitVMInfo(void)
{
    ...
    MMISMS_InitVMMultiVariant();
}

/*****************************************************************************/
//     Description : Init Voice Mail multivariant value
//    Global resource dependence : none
//  Author:rong.gu
//    Note:
/*****************************************************************************/
LOCAL void MMISMS_InitVMMultiVariant(void)
{
    MMI_MULTIVAR_ReadVoiceMail(g_fix_voicemail_item);
}
