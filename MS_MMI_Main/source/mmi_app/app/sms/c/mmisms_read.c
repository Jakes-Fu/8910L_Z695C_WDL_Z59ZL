                               ...
                               )
{
    ...
    read_msg_ptr->read_content.is_single_shift = read_msg_ptr->is_single_shift;
...
//    MMISMS_BOX_TYPE_E box_type = MMISMS_BOX_NONE;
    ...
    do 
    {
            ...
            (MN_SMS_FREE_SPACE == sig_ptr->status))
        {
            ...
            SCI_TRACE_LOW("mmisms meet read error, maybe sms already deleted");
            MMK_CloseWin(MMISMS_READSMS_WAIT_WIN_ID);
        ...
        {    
            ...
            case MN_SMS_MO_SR_RECEIVED_AND_STORED:
                ...
                {
					if (MMISMS_MO_DRAFT == cur_order_id->flag.mo_mt_type)
					{
						g_mmisms_global.read_msg.mo_mt_type = MMISMS_MO_DRAFT;
					} 
					else
					{
    ...
    //============== MET MEX  end ===================
            //MMISMS_OpenMsgBox(TXT_SMS_READ_ERR, IMAGE_PUBWIN_WARNING);
            SCI_TRACE_LOW("mmisms is_exist_read_err, maybe sms already deleted ");            
            oper_error = MMISMS_SMS_READ_FAIL;
            break;
        }
...
//    uint16 content_r8 [MMIMESSAGE_DIAPLAY_LEN * sizeof(wchar) + 1] = {0};
    ...
    temp_year = mt_sms_ptr->time_stamp_t.year;
