...
static const MMISMS_GSM_LIB_UNICODE_CONVERT_STR mmisms_gsm_lib_unicode_convert_table[] =
{
    { 0x10, 0x03B4 }, //δΔ
    { 0x11, 0x005F }, // no need convert      //  _
    { 0x12, 0x03C6 }, //φΦ
    { 0x13, 0x03B3 }, //γΓ
    { 0x14, 0x03BB }, //λΛ
    { 0x15, 0x038F }, //NA  Ω
...
PUBLIC void MMISMS_SetDefaultPresetSms(BOOLEAN is_clear)
{
    ...
    else
    {
        ...
        for (i = 0; i < MMISMS_DEFAULT_WORD_NUM; i++)
        {
            ...
            {
                MMI_GetLabelTextByLang(STXT_SMS_TEMP_1_EXT01+i, &text_str);
                                        ...
                                        )
{
    ...
    if (MN_RETURN_SUCCESS == nv_value && MMISMS_FREQUENT_WORD_NUM > item_id)
    {
        ...
        if (item_id < MMISMS_DEFAULT_WORD_NUM)
        {
            ...
            MMI_GetLabelTextByLang(STXT_SMS_TEMP_1_EXT01+item_id, &ori_text_str);
                                    ...
                                    BOOLEAN *is_single_shift)
{
    ...
    return MMISMS_R8_LANGUAGE_LATIN;
...
/*****************************************************************************/
//  Description : 判断该字符是否属于GSM Lib 表中的字符
//  Global resource dependence : none
//  Author: Yongli Tong
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMISMS_IsGSMByteChar(uint16 ch)
{
    BOOLEAN is_default = FALSE;
    int32 i = 0;
    uint16 table_len = sizeof(mmisms_gsm_lib_unicode_convert_table)/sizeof(mmisms_gsm_lib_unicode_convert_table[0]);

    for(i=0;i<table_len;i++)
    {
        if(mmisms_gsm_lib_unicode_convert_table[i].orig_char == ch) 
        {
            is_default = TRUE;
            break;
        }
...
PUBLIC wchar MMISMS_ConvertChar2Alphbet(wchar ch)
{
    ...
    {
        total_length = sizeof(mmisms_gsm_lib_unicode_convert_table)/sizeof(MMISMS_GSM_LIB_UNICODE_CONVERT_STR);
        for(i = 0;i < total_length; i++)
        {
            if(ch == mmisms_gsm_lib_unicode_convert_table[i].orig_char) //orig_char 是对应的gsm alphabet的字符,还需要转成unicode字符
            {
                convert_ch = mmisms_gsm_lib_unicode_convert_table[i].convert_char;//gsm alphabet的字符
                convert_ch = sms_default_to_ascii_table[convert_ch];//gsm alphabet的字符对应的unicode
                break;
            }
        }
    }
