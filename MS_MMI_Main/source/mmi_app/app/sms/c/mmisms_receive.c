...
//    uint16 content_r8 [MMIMESSAGE_DIAPLAY_LEN * sizeof(wchar) + 1] = {0};
    ...
    temp_year = sc_time_ptr->year;
	SCI_TRACE_LOW("mmisms: InsertNewMtToOrder year:%d",temp_year);
    ...
    sc_time = MMIAPICOM_Tm2Second( 
        ...
        sc_time_ptr->day,
        sc_time_ptr->month,
        MMISMS_MT_OFFSERT_YEAR - (0x100 - temp_year)
        );      
    }
    else
    {
...
*/
        ...
        SCI_TRACE_LOW("[MMISMS] InsertNewMtToOrder number_len = %d", num_len);
    }
...
static char *hero_app_assemble_pdu(heroAppPudIndItemSt *pduItem, int *contentLen, char *numberBuff, int numberBufSize)
{
	...
	for(index = 0; index < HERO_APP_PDU_IND_LIST_MAX_COUNT; index++)
	{
		...
		if(sig_ptr != NULL)
		{
			...
			{
				uint16 dataLen = 0;
				uint8 user_data_arr[MN_SMS_MAX_USER_VALID_DATA_LENGTH + 1] = {0};
...
#ifdef SFR_SUPPORT
#define TELECOM_SMS_SELFREG_CNF_INFO_LEN 2
LOCAL BOOLEAN MMISFR_SmsIsSelfRegCnfInfo(DPARAM param)
{
    ...
    SCI_TRACE_LOW("MMISFR_SmsIsSelfRegCnfInfo NUM =%s", tele_string);
    if(0 == strncmp((char*)tele_string, "10659401", tele_string_len))
    {
		...
		if(sms_user_data_t.user_valid_data_t.length == TELECOM_SMS_SELFREG_CNF_INFO_LEN &&
			sms_user_data_t.user_valid_data_t.user_valid_data_arr[0] == 0x03 &&
			sms_user_data_t.user_valid_data_t.user_valid_data_arr[1] == 0x04)
...
extern BOOLEAN lebao_sms_captcha_filter(DPARAM param);
#endif
...
#endif
SCI_TRACE_LOW("MMISMS: receive APP_MN_SMS_IND111 sig_ptrstorage =%d",sig_ptr->storage);
#if 0
	EFS_NvitemRead(MESSAGES_NV_TIME_STAMP_TYPE, sizeof(BOOLEAN), &bl_time_type);//maryxiao
	if(bl_time_type == 0)
	{
		TM_GetSysTime(&time);
         TM_GetSysDate(&date);
         sig_ptr->sms_t.time_stamp_t.year = date.year - MMISMS_MT_OFFSERT_YEAR;  //MMISMS_MT_OFFSERT_YEAR is added in InsertNewMtToOrder , so must reduce here
         sig_ptr->sms_t.time_stamp_t.month = date.mon;
         sig_ptr->sms_t.time_stamp_t.day = date.mday;
         sig_ptr->sms_t.time_stamp_t.hour = time.hour;
         sig_ptr->sms_t.time_stamp_t.minute = time.min;
         sig_ptr->sms_t.time_stamp_t.second = time.sec;
            
         SCI_TRACE_LOW("MMISMS: HandleSMSMTInd  year = %d, month = %d, day = %d,hour = %d,minute = %d, second = %d"  ,date.year,date.mon,date.mday,time.hour,time.min,time.sec);       
    }
#endif
...
#if 0 //defined(TTS_SUPPORT_YOUNGTONE)
		{
...
#if defined(LEBAO_MUSIC_SUPPORT) || defined(LEBAO_FEATURE_PHONE_STYLE)
	if(lebao_sms_captcha_filter(param))
	{
	#if 1   //不屏蔽短信验证码
           #ifdef _USE_OLD_L4_API
		   MNSMS_UpdateSmsStatus(sig_ptr->storage, sig_ptr->record_id, MN_SMS_FREE_SPACE );
...
#endif
  SCI_TRACE_LOW("MMISMS: receive APP_MN_SMS_IND111 sig_ptrstorage111asas =%d",sig_ptr->storage);
    orig_num.number_len = sig_ptr->sms_t.origin_addr_t.num_len;
    orig_num.npi_ton = MMIAPICOM_ChangeTypePlanToUint8(sig_ptr->sms_t.origin_addr_t.number_type, MN_NUM_PLAN_UNKNOW );    
    SCI_MEMCPY(&orig_num.number,&sig_ptr->sms_t.origin_addr_t.party_num,orig_num.number_len);  
	...
	{
		if (!MMIAPISET_IsSMSWhitelist(&orig_num))
		{
			result = TRUE;
		}
	}
	else if (MMIAPISET_GetIsBlacklistMessage())
	{
...
#endif	
#endif
  SCI_TRACE_LOW("MMISMS: receive APP_MN_SMS_IND1assss");
#ifndef WIN32
...
#if defined(BROWSER_SUPPORT_WEBKIT) && ((!defined(WIN32)) || (defined(_MSC_VER) && (_MSC_VER >= 1400)))
    MMIAPIWK_HandleNewSMSInd(sig_ptr);
#endif    
#ifdef BROWSER_SUPPORT
    if (MMIAPIBROWSER_IsRunning())
    {
        MMIAPIBRW_HandleNewSMSInd(sig_ptr);
    }
#endif	
  ...
  SCI_TRACE_LOW("MMISMS: receive APP_MN_SMS_IND1a22222s");
#ifdef HERO_ENGINE_SUPPORT
...
#ifdef SFR_SUPPORT
        SCI_TRACE_LOW("MMISMS: MMISMS_HandleSMSMTInd msg head len= %d, data len =%d",sms_user_data_t.user_data_head_t.length, sms_user_data_t.user_valid_data_t.length);
        SCI_TraceBuf("MMISMS: MMISMS_HandleSMSMTInd msg head: ", sms_user_data_t.user_data_head_t.user_header_arr ,sms_user_data_t.user_data_head_t.length);
        SCI_TraceBuf("MMISMS: MMISMS_HandleSMSMTInd msg data: ", sms_user_data_t.user_valid_data_t.user_valid_data_arr ,sms_user_data_t.user_valid_data_t.length);
#endif
  SCI_TRACE_LOW("MMISMS: recseive MMISMS_HandleSMSMTInd");
#ifdef MRAPP_SUPPORT
...
#endif	
  SCI_TRACE_LOW("MMISMS: recseive MMISMS_HandleSMSMTIndsszxcccc");
    //bug 760037
    {
        uint16 err_code;
        BOOLEAN is_new_mms=FALSE;
        ...
        if (is_new_mms && ( (sig_ptr->sms_t.dcs.class_is_present) &&(MN_SMS_CLASS_0 == sig_ptr->sms_t.dcs.sms_class) ))
        {
            sig_ptr->sms_t.dcs.class_is_present=FALSE;
        }
    }
    //bug 760037
        ...
        (MN_SMS_CLASS_0 == sig_ptr->sms_t.dcs.sms_class) )
    {
...
#endif            
        {
            ...
            MMISMS_ShowClass0Alert(&g_mmisms_global.class0_msg);
        }
  SCI_TRACE_LOW("MMISMS: recseive MMISMS_HandleSMSMTIndsszxccxxxcbbbbcc");
#ifdef MMI_SMS_VOICE_MAIL_SUPPORT
...
//END @huangns_20111103z
        ...
        if (!is_finished)
        {
            is_finished = MMISMS_IsVoiceMailMessage(sig_ptr->dual_sys,
                                                                            ...
                                                                            sig_ptr->storage,
                                                                            sig_ptr->record_id);
			if (is_finished && sig_ptr->sms_t.dcs.is_save_wait_msg)
			{
				OperateMtIndAsSms(
					sig_ptr->dual_sys,
					&sms_user_data_t,
...
#endif
          SCI_TRACE_LOW("MMISMS: recseive MMISMS_HandleSMSMTIndsszxccxxxcbbbbcc  is_finished%d",is_finished);
        if (!is_finished)
        {
		 ...
		 SCI_TRACE_LOW("MMISMS: recseive MMISMS_HandleSMSMTIndsszxccxxxcbbbbcc 333 is_finished%d",is_finished);
	...
	if(is_finished)
	{
		...
		if(is_finished)
		{
			extern void hero_tts_new_sms_broadcast(void *mdata ,void *mcontent);
			hero_tts_new_sms_broadcast((void *)sig_ptr, (void *)&sms_user_data_t);
		}
#endif
                                      ...
                                      )
{
    ...
    //MMI_STRING_T            status_text_str = {0};
...
// no use
    ...
    if (MMISMS_IsOrderOk())
    {
        ...
        {
            text_id = STR_MSG_DELIVERY_SUCCESS_EXT01;
        }
        else
        {
            text_id = STR_MSG_DELIVERY_FAIL_EXT01;
        }

        MMI_GetLabelTextByLang(text_id, &status_str1);

		status_str1.wstr_len = status_str1.wstr_len - 2; //去除%U非法字符

#if 0 
        //Merge status_str1 and status_str to status_text_str
        status_text_str.wstr_len = status_str.wstr_len + status_str1.wstr_len;
        status_text_str.wstr_ptr = SCI_ALLOC_APP((status_text_str.wstr_len)*sizeof(wchar));
        SCI_MEMSET((uint8 *)status_text_str.wstr_ptr, 0, ((status_text_str.wstr_len)*sizeof(wchar)));

        //Copy string of delivery status
        MMI_WSTRNCPY(
            status_text_str.wstr_ptr,
            status_text_str.wstr_len,
            status_str1.wstr_ptr,
            status_str1.wstr_len,
            status_str1.wstr_len
            );
 
        //Copy string of transmitter info
        MMI_WSTRNCPY(
            status_text_str.wstr_ptr + status_str1.wstr_len,
            status_text_str.wstr_len,
            status_str.wstr_ptr,
            status_str.wstr_len,
            status_str.wstr_len
        ...
        MMIPUB_OpenAlertWinByTextPtr(&time_out,&status_str1,&status_str,IMAGE_PUBWIN_NEWMSG,&alert_win_id,&win_priority,MMIPUB_SOFTKEY_DIALOG_OK,MMISMS_HandleNewReportWin);        
        //MMIPUB_OpenAlertWinByTextPtr(&time_out,&status_text_str,PNULL,IMAGE_PUBWIN_NEWMSG,&alert_win_id,&win_priority,MMIPUB_SOFTKEY_ONE,MMISMS_HandleNewReportWin);
...
#if 0 
        if(PNULL != status_text_str.wstr_ptr)
        {
            SCI_FREE(status_text_str.wstr_ptr);
            status_text_str.wstr_ptr = PNULL;
        }
#endif         
                                     ...
                                     )
{
    ...
    if(MMISMS_READ_MTSMS == read_msg_ptr->read_type)   //MT time
    {
                                                                                ...
                                                                                )
{
    ...
    SCI_TRACE_LOW("SMS: MMISMS_IsVoiceMailMessage: msg_is_waiting = %d wait_type = %d is_active_wait_ind=%d is_save_wait_msg = %d", dcs.msg_is_waiting, dcs.wait_type, dcs.is_active_wait_ind, dcs.is_save_wait_msg);
                               ...
                               )
{
    ...
    if (buf_len > 0)
    {
        ...
        {
            //MMIPUB_OpenAlertWarningWin(TXT_MMS_STXT_UNSUPORTED_MESSAGE);
...
LOCAL uint32 GetSmsReceiveTimeStamp(void)
{
    ...
    if(bl_time_type == 0)
    {
    	sec = (TM_GetTotalSeconds() + (365*23+366*7)*24*3600);
    	/*1970年1月1日零点到1999年12月31日23点59分59秒以来经过的秒数*/
    }
    else
    {
        sec = MMIAPIPHONE_GetSysTimeWithNitzAndTickTime();
        SCI_TRACE_LOW("[MMIPHONE]:  GetSmsReceiveTimeStamp get nettime %d",sec);
        if(sec == 0)
        {
        		sec = (TM_GetTotalSeconds() + (365*23+366*7)*24*3600);
        }
