...
//============== MET MEX  end ===================
        ...
        if(!ret)
        {
            ...
            {                
                    ...
                    )
                {
                    ...
                    ERR_MNSMS_CODE_E   error_code            = ERR_MNSMS_NONE;
                    update_order_id = g_mmisms_global.update_info.order_id_arr[g_mmisms_global.update_info.current_num];
                    error_code = MNSMS_UpdateSmsStatusEx(update_order_id->flag.dual_sys, update_order_id->flag.storage, update_order_id->record_id, MN_SMS_FREE_SPACE);                
    ...
    else
    {
        ...
        {
            SCI_TRACE_LOW("mmisms delete error, maybe sms already not exist before");                
...
#endif
                {
                    next_longsms_ptr = del_order_id->next_long_sms_ptr;  //must do it before HandleDeleteCnf
                    if(PNULL == next_longsms_ptr )
                    {
                        SCI_TRACE_LOW("mmisms draft deleting is finished");
                    }
                }
                HandleDeleteCnf(sig_ptr->cause,sig_ptr->record_id,sig_ptr->storage,sig_ptr->dual_sys);
            
                if( PNULL != next_longsms_ptr && MMISMS_MO_DRAFT == next_longsms_ptr->flag.mo_mt_type)
                {
                    MMISMS_SetGlobalOperationOrderId(next_longsms_ptr);
...
// no use para
        ...
        if ((sig_ptr->record_id == record_id) && (sig_ptr->storage == storage))
        {
            ...
            if (MN_SMS_OPERATE_SUCCESS == sig_ptr->cause)
            {
                ...
                {
                    uint32 index = 0;         
                    if( g_mmisms_global.update_chat_disp.send_finish_order_id && (MMISMS_UPDATE_BACKSEND_SMS == oper_status || MMISMS_UPDATE_SENDFAIL_SMS == oper_status))
                    {
                        cur_order_id = g_mmisms_global.update_chat_disp.send_finish_order_id;
                    ...
                    {                        
                        SCI_TRACE_LOW("mmisms should not enter this case now");
                ...
                //MMISMS_TrigSmsDataChangeEvent(MMISMS_DATACHANGE_TYPE_DELETE);    //delete to fix chatwin forward error, before would delete the chat sms.
#ifdef MMI_SMS_CHAT_SUPPORT                
                if(is_need_update_chatwin && MMK_IsOpenWin(MMISMS_SHOW_CHAT_SMS_WIN_ID))
                {
                     if(MMISMS_UPDATE_BACKSEND_SMS == oper_status || MMISMS_UPDATE_SENDFAIL_SMS == oper_status)
                     {
                            g_mmisms_global.update_chat_disp.is_update = TRUE;  
                            g_mmisms_global.update_chat_disp.is_send_finish = TRUE;  
                            if(send_message_ptr->backsend_status == MMISMS_SENDSTATUS_SENDSUCC)
                            {
                                g_mmisms_global.update_chat_disp.is_success = send_message_ptr->backsend_status;                                        
                            }
                            SCI_TRACE_LOW("mmisms delete %d,is_send_finish %d, is_success %d",g_mmisms_global.update_chat_disp.is_update,g_mmisms_global.update_chat_disp.is_send_finish,g_mmisms_global.update_chat_disp.is_success);        
                     }
                     ...
                     {
                         MMISMS_SetIsNeedUpdateList(TRUE);
                     }
                }
#endif                
