...
**                         MACRO DEFINITION                                  *
**--------------------------------------------------------------------------*/
#define MMISMS_SMS_SUBJECT_ALPHA  (255*0.6)
...
uint8 del_fail_index = 0;
uint8 del_fail_flag = 0;
BOOLEAN second_search_fail_flag = 0;
uint8 second_del_fail_flag = 0;
#define SMS_DEBUG_ORDER_NUM 30
//监控删除或者插入的30个order,方便debug
MMISMS_STATE_T sms_delete_array[SMS_DEBUG_ORDER_NUM] = {0};
MMISMS_STATE_T sms_insert_array[SMS_DEBUG_ORDER_NUM] = {0};
MMISMS_STATE_T sms_delete_fail_array[10] = {0};
#endif
...
LOCAL MMISMS_CHAT_UNIT_T  s_mmisms_chat_blocklist_linked_order;
...
//    Note: 
/*****************************************************************************/
LOCAL MMI_RESULT_E MMISMS_HandleExceedUnreadWaitWinMsg(
...
/*****************************************************************************/
//  Function Name: AdjustDraftChatNode
//  Description  : delete draft chat node
//  Global resource dependence : s_mmisms_chat_linked_order
//  Author:rong.gu
//  Note:解决短信草稿编辑成彩信草稿后无法查看的问题
/*****************************************************************************/
LOCAL void AdjustDraftChatNode(
...
/*****************************************************************************/
//  Function Name: UpdateChatNodeTotalUnreadNum and notify to remove idle
...
//  Note:
/*****************************************************************************/
LOCAL uint32 UpdateChatNodeTotalUnreadNumAndRemoveIdleNotification(MMIPB_BCD_NUMBER_T *pb_bcd);
...
/*****************************************************************************/
//  Function Name: DeleteChatNodeByFindNumber
//  Description  : delete chat node
//  Global resource dependence : s_mmisms_chat_linked_order
//  Author:rong.gu
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN DeleteChatNodeByFindNumber(
                                MMISMS_CHAT_STATE_UNIT_T  *first_node_ptr,
                                MMISMS_CHAT_STATE_UNIT_T *head_node_ptr,
                                MMISMS_STATE_T *delete_state_ptr,
                                MMISMS_TYPE_E type
                                    ...
                                    MMISMS_ORDERTYPE_E sort_type)
{
    ...
    SCI_TRACE_LOW("[MMISMS] InsertNodeAsNormalSms");
    ...
    case MMISMS_BOX_CHAT:
        {
			...
			BOOLEAN is_blockmessage = MMISmsAPI_GetBlockedMessageWindowStatus();
		
            num_ptr = s_mmisms_chat_linked_order.head_ptr;
			
#if 0
           cmp_bcd = MMISMS_SmsBCDToPbBCD(num_ptr->bcd_num); 
		is_blocklist = MMIAPISET_IsSMSBlacklist(&cmp_bcd);
		while ((num_ptr != PNULL) && (is_blocklist))
		{
...
PUBLIC BOOLEAN MMISMS_InsertSmsToOrder (MMISMS_STATE_T *order_info_ptr)
{
    ...
    uint32 i = 0;
    MMISMS_FOLDER_TYPE_E folder_type = 0;
...
#ifdef BUSMONITOR_SUPPORT    
    SCI_MEMCPY(&sms_insert_array[insert_index++],(MMISMS_STATE_T *)order_info_ptr,sizeof(MMISMS_STATE_T));
    if(insert_index == SMS_DEBUG_ORDER_NUM)
    {
        insert_index = 0;
    }
#endif  
    ...
    SCI_TRACE_LOW("[MMISMS] MMISMS_InsertSmsToOrder record_id = %d, storage = %d, num_len = %d", new_state_ptr->record_id, new_state_ptr->flag.storage, new_state_ptr->number.bcd_number.number_len);
...
#ifdef DATACOUNTER_SUPPORT
    if ( order_info_ptr->flag.msg_type == MMISMS_TYPE_SMS && 
        order_info_ptr->flag.mo_mt_type == MMISMS_MO_SEND_SUCC )
   {
        MMIAPIDataCounter_smsAlertNotifaction(order_info_ptr->flag.dual_sys , DATACOUNTER_SENT_DATATYPE);
    }
    else if(order_info_ptr->flag.msg_type == MMISMS_TYPE_SMS && 
        (order_info_ptr->flag.mo_mt_type == MMISMS_MT_TO_BE_READ
        ||order_info_ptr->flag.mo_mt_type == MMISMS_MT_HAVE_READ))
    {

        MMIAPIDataCounter_smsAlertNotifaction(order_info_ptr->flag.dual_sys, DATACOUNTER_RECV_DATATYPE);
                                               ...
                                               )
{
	...
	while(PNULL != number_node)
    {
        ...
        {
			cmp_bcd = MMISMS_SmsBCDToPbBCD(number_node->bcd_num); 
			is_blocklist = MMIAPISET_IsSMSBlacklist(&cmp_bcd);
...
#ifdef BUSMONITOR_SUPPORT    
    SCI_MEMCPY(    &sms_delete_array[del_index++],(MMISMS_STATE_T *)order_id,sizeof(MMISMS_STATE_T));
    if(del_index == SMS_DEBUG_ORDER_NUM)
    {
        del_index = 0;
    }
#endif    
                                           ...
                                           
{
    ...
    {
#if 0
//yuguang.liu for bug 493746 
		if (!MMISmsAPI_GetBlockedMessageWindowStatus())
		{
...
//小屏下由于与之前的layout冲突，这里使用动态创建layout
#if defined(MAINLCD_SIZE_128X160) || defined(MAINLCD_SIZE_176X220)
{
    GUIITEM_STYLE_E mini_style_id = 0;
    //建立新的layout
    GUIITEM_LAYOUT_DATA_T mini_layout = {
        MMI_LIST_DOUBLE_LINE_ITEM_HEIGHT,
        //MMI_LIST_ITEM_HEIGHT,
        {
...
#else
            { MMI_LIST_ITEM_SELECT_ICON_WIDTH + MMI_LIST_ICON_LEFT_SPACE + MMI_LIST_TEXT_LEFT_SPACE,  MMI_LIST_ITEM_TOP_MARGIN,  MMI_LIST_ITEM_RIGHT-MMI_DEFAULT_FONT_ASCII_WIDTH*5-3, MMI_LIST_DOUBLE_LINE_ITEM_HEIGHT/2-1 },       /* 2 */
#endif
            { MMI_LIST_ITEM_RIGHT-MMI_DEFAULT_FONT_ASCII_WIDTH*5 - 2, MMI_LIST_ITEM_TOP_MARGIN,  MMI_LIST_ITEM_RIGHT - 2, MMI_LIST_DOUBLE_LINE_ITEM_HEIGHT/2-1 },       /* 3 */
            ...
            {0,  MMI_LIST_DOUBLE_LINE_ITEM_HEIGHT/2 +1, MMI_LIST_ITEM_SELECT_ICON_WIDTH + MMI_LIST_ICON_LEFT_SPACE, MMI_LIST_DOUBLE_LINE_ITEM_HEIGHT - MMI_LIST_ITEM_TOP_MARGIN},       /* 4 */                
            { 0 }
        },
        { MMI_LIST_ITEM_RIGHT - (MMI_LIST_ITEM_SELECT_ICON_WIDTH + MMI_LIST_ICON_LEFT_SPACE),  0,  MMI_LIST_ITEM_RIGHT,  MMI_LIST_DOUBLE_LINE_ITEM_HEIGHT - 1},
        { 0 },
        //{{ 0 },{ 0 }}
    };
    //注册自定义layout
    GUIITEM_LAYOUT_ID_E mini_layout_id = MMITHEME_CustomListItemLayoutReg(ctrl_id, &mini_layout, &mini_layout);
    //建立新的style
    GUIITEM_STYLE_T mini_style = {0};
    MMITHEME_GetItemStyleEx(GUIITEM_STYLE_TWO_LINE_ICON_TWO_TEXT_AND_TEXT_NORMAL, &mini_style);
    mini_style.layout_id = mini_layout_id;
    mini_style.main_index = 1;
    //注册自定义style
    mini_style_id = MMITHEME_CustomListItemStyleReg(ctrl_id, &mini_style, 0);
    //将动态style保存在item info中
    item_info.item_style = mini_style_id;
}
#endif
...
LOCAL BOOLEAN FormChatListFromLinkhead(MMI_CTRL_ID_T ctrl_id)
{
    ...
    while(PNULL != number_node)
    {
        ...
        {
#if 0        
            if(current_order_id->flag.mo_mt_type == MMISMS_MO_DRAFT)  //正在删除中的草稿不显示
            {
                MMISMS_ORDER_ID_T draft_operate_order = PNULL;
                draft_operate_order = MMISMS_GetDraftOperationOrderId();   
                if(draft_operate_order != PNULL && MMISMS_CompareSmsState(draft_operate_order,MMISMS_MO_TYPE,current_order_id) && PNULL == current_order_id->next_chat_ptr)
                {
                    number_node = number_node->next_number_ptr;
                    continue;
                }
...
#else
#ifndef MMI_MULTI_SIM_SYS_SINGLE
                item_info.item_style = GUIITEM_STYLE_TWO_LINE_2TEXT_ICON_AND_TEXT_ICON_MS;
#else
                item_info.item_style = GUIITEM_STYLE_TWO_LINE_2TEXT_AND_TEXT_ICON_MS;
#endif
                //Quick delete
                //item_info.item_style    = GUIITEM_STYLE_THREE_PARTS_2TEXT_AND_TEXT_MS;
                item_info.left_item_style = GUIITEM_STYLE_2TEXT_AND_TEXT_WITH_LEFT_FOCUS_MS;
                item_info.right_item_style = GUIITEM_STYLE_2TEXT_AND_TEXT_WITH_RIGHT_FOCUS_MS;
                //GUILIST_SetShowLRTriangleIcon(ctrl_id, TRUE, FALSE);
                GUILIST_SetQuickActionIcon(ctrl_id, IMAGE_COMMON_LIST_DISMISS, PNULL);
                GUILIST_SetHandleLeftRightEx(ctrl_id, TRUE, FALSE);
                //GUILIST_SetListState(ctrl_id, GUILIST_STATE_EFFECT_STR, TRUE);
                //设置颜色
                GUILIST_SetItemState(&item_info, GUIITEM_STATE_COSTMER_FONT_COLOR, TRUE);
                GUILIST_SetItemContentCustomFontColor(ctrl_id, 2, &color, MMISMS_SMS_SUBJECT_ALPHA, &color, MMISMS_SMS_SUBJECT_ALPHA);
#if 0 //MS_MMI_Delete               
                item_info.item_style = GUIITEM_STYLE_TWO_LINE_ICON_TWO_TEXT_AND_TEXT_NORMAL;
#endif
#endif
...
PUBLIC MMISMS_ORDER_ID_T MMISMS_GetChatLinkedArrayPtrIdInList(uint32 list_index)
{
    ...
    if(cur_number_ptr == PNULL)
    {
        return cur_order_id;
    }
    //SCI_TRACE_LOW:"head_order_id is 0x%x"
    SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMISMSAPP_ORDER_4348_112_2_18_2_54_2_641,(uint8*)"d",cur_number_ptr);
       item_totalnum= s_mmisms_chat_linked_order.uint_num;
	cmp_bcd = MMISMS_SmsBCDToPbBCD(cur_number_ptr->bcd_num); 
	is_blocklist = MMIAPISET_IsSMSBlacklist(&cmp_bcd);
...
#endif	
	for(;j<item_totalnum;j++)
	{
            if(is_blockmessage)
    	    {
    		if((list_index ==0) &&(is_blocklist))
    		{
    			break;
    		}
    		while ((cur_number_ptr != PNULL) && (!is_blocklist))
    		{
    			cur_number_ptr = cur_number_ptr->next_number_ptr;
                    if(cur_number_ptr==PNULL)
                        break;
            ...
            if(list_index>0)
            {
                if(cur_number_ptr != PNULL) //coverity CID26097
                {
                    cur_number_ptr = cur_number_ptr->next_number_ptr;
                }
                if(cur_number_ptr != PNULL)
...
/*****************************************************************************/
//  Description : 根据信箱list列表的index，获得信箱的链表指针id, 可以获取草稿
//  Global resource dependence : 
//  Author:fengming.huang
//  Note:
/*****************************************************************************/
PUBLIC MMISMS_ORDER_ID_T MMISMS_GetChatLinkedArrayPtrIdInListEx(uint32 list_index)
{
    MMISMS_CHAT_STATE_UNIT_T *cur_number_ptr  = PNULL;
    MMISMS_ORDER_ID_T cur_order_id  = PNULL;
    uint32 while_count = 0;
    ...
    if(PNULL != cur_number_ptr)
    {
        if(cur_number_ptr->is_has_draft && cur_number_ptr->draft_ptr != PNULL)
        {
            return cur_number_ptr->draft_ptr;
        }
        cur_order_id = cur_number_ptr->head_ptr;
...
PUBLIC BOOLEAN MMISMS_OrderInit(void)
{
    ...
    uint16 used_num = 0;
    uint16 max_sim_num = 0;
    uint16 max_mms_number = 0;
#ifdef MMS_SUPPORT    
    max_mms_number =  MMIMMS_GetMaxNumber();
#endif
...
#ifdef MMS_SUPPORT    
    s_mmisms_order.max_sms_num.total_msg_num += max_mms_number;// get the mms num
#endif    
                              ...
                              )                                    
{
    if (MMISMS_TYPE_SMS  != sms_state_info->flag.msg_type )  //mms,wap push, wap ota, mms ota
    {
        if(!MMISMS_GetNameByNumberFromPB(sms_state_info->number.sender, name, max_name_len) )
        {
            ...
            BOOLEAN is_email_contact = FALSE;
            wchar msg_sender[MMISMS_NUMBER_MAX_LEN+1] = {0};
            uint8  msg_sender_len = 0;
            MMIAPICOM_StrToWstr(sms_state_info->number.sender, msg_sender);
            msg_sender_len = MMIAPICOM_Wstrlen(msg_sender);
            if((URL_IsMailto((URL_CHAR_T const *)msg_sender, &msg_sender_len))
               ||(URL_IsMailtoDefault((URL_CHAR_T const *)msg_sender, &msg_sender_len)))
            {
                MMI_STRING_T abbr = {0};
                wchar temp_array[MMISMS_NUMBER_MAX_LEN + 1]  = {0};
                GUIANIM_FILE_INFO_T anim_path = {0};
                uint16 entry_id = 0;
                uint16 storage_id = 0;
                abbr.wstr_ptr = temp_array;
                anim_path.full_path_wstr_ptr = temp_array;
                if(MMIAPIPB_GetNameAvatarByEmail(msg_sender, name, &abbr, &anim_path, &entry_id, &storage_id, MMISMS_NUMBER_MAX_LEN))
                {
                    is_email_contact = TRUE;
                }            
            }
                                    ...
                                    )
{
    ...
    *sms_ori_used_num_ptr = *used_num_ptr;
    ...
    if (!is_nv)
    {
        if(dual_sys < MMI_DUAL_SYS_MAX)  //possibility: dual_sys == MMI_DUAL_SYS_MAX ? or MN_DUAL_SYS_MAX > MMI_DUAL_SYS_MAX?
        {
...
{
    if(0 == MMIAPIPB_BcdSmartCmp(compare_number_ptr1,compare_number_ptr2))
    {
        return TRUE;
    }
    else
...
PUBLIC MMISMS_ORDER_ID_T MMISMS_FindValidChatSMSMoveToSecurity(MMIPB_BCD_NUMBER_T pb_num)
{
    ...
    while(order_id != PNULL)//is  marked
    {
        ...
        {
            order_id->flag.is_marked = TRUE; // 2015 0715
                                    ...
                                    )
{
    ...
    //草稿箱和安全信箱不出现在聊天窗口中
    if((MMISMS_FOLDER_SECURITY == new_state_ptr->flag.folder_type) ||
    ...
    //The draft without number should be created as a new node
    if((MMISMS_MO_DRAFT == new_state_ptr->flag.mo_mt_type) &&
    (((MMISMS_TYPE_MMS == new_state_ptr->flag.msg_type) && (0 == strlen(new_state_ptr->number.sender))) ||   //coverity CID26066   
    ((MMISMS_TYPE_SMS == new_state_ptr->flag.msg_type) && (0 == new_state_ptr->number.bcd_number.number_len))))
    {
        first_number_ptr = PNULL;
    }
    ...
    if (PNULL != first_number_ptr)
    {
        ...
        {        
            ...
            {
                if(((MMISMS_TYPE_MMS == first_number_ptr->head_ptr->flag.msg_type) && (0 == strlen(first_number_ptr->head_ptr->number.sender)))||   //coverity CID26066   
                ((MMISMS_TYPE_SMS == first_number_ptr->head_ptr->flag.msg_type) && (0 == first_number_ptr->head_ptr->number.bcd_number.number_len)))
                {
                    SCI_TRACE_LOW("mmisms empty number case 1 ignore type: %d",first_number_ptr->head_ptr->flag.msg_type); //比较号码为空，认为不相等,new_state_ptr为空的情况，上面已经创建节点了
                }
                else
                {
        ...
        else
        {
            ...
            while(PNULL != first_number_ptr)
            {
                ...
                ((MMISMS_TYPE_SMS == first_number_ptr->head_ptr->flag.msg_type) && (0 == first_number_ptr->head_ptr->number.bcd_number.number_len)))
                {
                    SCI_TRACE_LOW("mmisms empty number case 2 ignore type: %d",first_number_ptr->head_ptr->flag.msg_type);   //被比较号码为空，认为不相等
                }
                else
                {            
    ...
    {
		//must consider long sms draft issue,  if not 479361 issue would come, only set seq 1 draft sms
		//judge it before LinkedChatAllocNode
		if (MMISMS_MO_DRAFT == new_state_ptr->flag.mo_mt_type) 
		{
			if(new_state_ptr->flag.is_concatenate_sms && 1 != new_state_ptr->longsms_seq_num)  //draft 短信默认按seq顺序存储
			{
				return result;
			}
		}
                                             ...
                                             )
{
    ...
    if(PNULL == head_ptr)//插到此号码列表末尾
    {
		...
		else if (MMISMS_MO_DRAFT == new_state_ptr->flag.mo_mt_type) 
            {
                number_head_ptr->is_has_draft = TRUE;
                number_head_ptr->draft_ptr = new_state_ptr;
            }
                         ...
                         )
{
    ...
    if(PNULL == number_node)
    {
        ...
        {
            unit_ptr->is_has_draft = TRUE;
            unit_ptr->draft_ptr = new_state_ptr;
        }
                                     ...
                                     )
{
    uint8 bcd_num_len = 0;
    BOOLEAN result = TRUE;
    uint8 sender_num[MMISMS_NUMBER_MAX_LEN+1] = {0};
    uint8 bcd_number[MMIPB_BCD_NUMBER_MAX_LEN] = {0};
    MMISMS_CHAT_STATE_UNIT_T  *first_number_ptr = s_mmisms_chat_linked_order.head_ptr;
    MMISMS_CHAT_STATE_UNIT_T  *first_node_ptr = PNULL;
    MMIPB_BCD_NUMBER_T pb_bcd1 = {0};
    MMIPB_BCD_NUMBER_T pb_bcd2 = {0};
    ...
    if((MMISMS_FOLDER_SECURITY == delete_state_ptr->flag.folder_type) ||
   ...
   if (PNULL != first_number_ptr)
    {
		//we should handle the special case: draft without number
		if((MMISMS_MO_DRAFT == delete_state_ptr->flag.mo_mt_type) &&
			( ((MMISMS_TYPE_MMS == delete_state_ptr->flag.msg_type) && (0 == strlen(delete_state_ptr->number.sender))) ||   //fix coverity CID26058
			((MMISMS_TYPE_SMS == delete_state_ptr->flag.msg_type) && (0 ==delete_state_ptr->number.bcd_number.number_len))))
		{
			while(PNULL != first_number_ptr)
			{
				if(delete_state_ptr->record_id== first_number_ptr->head_ptr->record_id)
				{
					break;
				}
				first_number_ptr = first_number_ptr->next_number_ptr;
        ...
        else if (MMISMS_TYPE_MMS == delete_state_ptr->flag.msg_type)
        {
            //比较号码
            while(PNULL != first_number_ptr)
            {
                ...
                ((MMISMS_TYPE_SMS == first_number_ptr->head_ptr->flag.msg_type) && (0 == first_number_ptr->head_ptr->number.bcd_number.number_len)))
                {
                    SCI_TRACE_LOW("mmisms empty number case 1 ignore type: %d",first_number_ptr->head_ptr->flag.msg_type);
                }
                else
                {            
                    if (MMISMS_TYPE_SMS == first_number_ptr->head_ptr->flag.msg_type)
        ...
        else
        {
            ...
            while(PNULL != first_number_ptr)
            {
                ...
                ((MMISMS_TYPE_SMS == first_number_ptr->head_ptr->flag.msg_type) && (0 == first_number_ptr->head_ptr->number.bcd_number.number_len)))
                {
                    SCI_TRACE_LOW("mmisms empty number case 2 ignore type: %d",first_number_ptr->head_ptr->flag.msg_type);
                }
                else
                {            
                    if (MMISMS_TYPE_SMS == first_number_ptr->head_ptr->flag.msg_type)
    ...
    {
        BOOLEAN result = TRUE;
        result = DeleteChatNode(first_node_ptr,first_number_ptr,delete_state_ptr,type);
        if(!result) //号码未找到的情况
        {
              DeleteChatNodeByFindNumber(first_node_ptr,first_number_ptr,delete_state_ptr,type);
        }        
        //接着重排s_mmisms_chat_linked_order
        SortChatLinkedOrderByTime();
     
        UpdateChatNodeTotalUnreadNumAndRemoveIdleNotification(&pb_bcd2);
                             ...
                             )
{
    ...
    uint32 total_msg_num = s_mmisms_order.max_sms_num.total_msg_num;  
    uint32 while_count = 0;

    if(PNULL == head_node_ptr)
    {
        return;
    }
    number_node_ptr = head_node_ptr->head_ptr;

    if(head_node_ptr->is_has_draft )
    {
        while(PNULL != number_node_ptr)
        { 
            if(MMISMS_MO_DRAFT == number_node_ptr->flag.mo_mt_type && delete_state_ptr != number_node_ptr)
            {
                 SCI_TRACE_LOW("mmisms find another draft");
                 head_node_ptr->is_has_draft = TRUE;
                 head_node_ptr->draft_ptr = number_node_ptr;
                 return;
            }        
            number_node_ptr = number_node_ptr->next_chat_ptr;       
            while_count++;
            if(number_node_ptr != PNULL && number_node_ptr == number_node_ptr->next_chat_ptr)
            {
                SCI_TRACE_LOW("MMISMS link array error AdjustDraftChatNode");
                break;
            } 

            if (while_count > total_msg_num)
            {
                SCI_TRACE_LOW("MMISMS link array error while_count exceed total_msg_num AdjustDraftChatNode");
                break;
            }       
        }
    }
    head_node_ptr->is_has_draft = FALSE;
    head_node_ptr->draft_ptr = PNULL;
}
/*****************************************************************************/
//  Function Name: DeleteChatNode
//  Description  : delete chat node
                             ...
                             )
{
    ...
    MMISMS_STATE_T *temp_long_ptr = PNULL;
    BOOLEAN is_found = FALSE;
    ...
    while(first_number_ptr)
    {        
        temp_ptr = first_number_ptr->head_ptr;
        while_count = 0;
        while(temp_ptr)
        {
           temp_long_ptr = temp_ptr;
           while(temp_long_ptr) 
           {
                if(temp_long_ptr == delete_state_ptr)
                {
                    is_found = TRUE;
                    break;
                }
                temp_long_ptr = temp_long_ptr->next_long_sms_ptr;
           }
           if(is_found)
           {
                break;
           }
           temp_ptr = temp_ptr->next_chat_ptr;
           while_count++;
           if(temp_ptr != PNULL && temp_ptr == temp_ptr->next_chat_ptr)
           {
               SCI_TRACE_LOW("MMISMS link array error DeleteChatNodeByFindNumber");
               break;
           } 
   
           if (while_count > total_msg_num)
           {
               SCI_TRACE_LOW("MMISMS link array error while_count exceed total_msg_num DeleteChatNodeByFindNumber");
               break;
           }  
    ...
    if(is_found && first_number_ptr)
    {
#ifdef BUSMONITOR_SUPPORT        
        del_fail_flag = 0;
        second_del_fail_flag = 1;
#endif  
        is_ret = DeleteChatNode(first_node_ptr,first_number_ptr,delete_state_ptr,type);
#ifdef BUSMONITOR_SUPPORT
        if(!is_ret)
        {
            second_del_fail_flag = 2;
        }
#endif  
		return TRUE;
    }
                             ...
                             )
{
        ...
        if(PNULL != number_node_ptr)
        {
            if (PNULL == number_node_ptr->prev_chat_ptr) //是链表的头
            {
                ...
                else
                {
                    ...
                    if (MMISMS_MO_DRAFT == delete_state_ptr->flag.mo_mt_type) 
                    {
                        AdjustDraftChatNode(head_node_ptr,delete_state_ptr);
                    }
                }
            }
            else  //不是链表的头
        ...
        if (PNULL != cur_ptr) //找到ref相同的位置
        {
                ...
                && (delete_state_ptr->flag.storage == cur_ptr->flag.storage))  //在主链表上
            {
                ...
                else //存在下条长短信
                {
                    ...
                    if (MMISMS_MO_DRAFT == delete_state_ptr->flag.mo_mt_type)
                    {
                        head_node_ptr->draft_ptr = cur_ptr->next_long_sms_ptr;   //长短信的下条作为草稿
                    } 
    ...
    if(!result)
    {
        SCI_MEMCPY(    &sms_delete_fail_array[del_fail_index++],(MMISMS_STATE_T *)delete_state_ptr,sizeof(MMISMS_STATE_T));   //记录删除失败的节点
        if(del_fail_index == 10)
        {
            del_fail_index = 0;
        }
    }
...
{
    uint32 unread_num = 0;
    uint32 undownload_num = 0;
    BOOLEAN undownload_del_flag = FALSE;
    MMISMS_CHAT_STATE_UNIT_T *number_node = PNULL;
    MMISMS_STATE_T *mt_undownloaded_ptr = PNULL;
    MMIPB_BCD_NUMBER_T mms_pb_bcd = {0};

    number_node = s_mmisms_chat_linked_order.head_ptr;

    mt_undownloaded_ptr = s_mmisms_order.mt_undownloaded_head_ptr;
    ...
    if((PNULL != pb_bcd) && (s_mmisms_chat_linked_order.unread_unit_num > unread_num))
    {
        //All messages send from current number had been changed to ALREADY_READ state
        // so that we should remove its notification on idle screen
        MMIIDLE_RemovePushMsgEx( MMI_IDLE_PUSH_MSG_SMS, PNULL, pb_bcd);
    }
    if (PNULL != mt_undownloaded_ptr)
    {
        MMIAPIPB_StringToBCDNumber(mt_undownloaded_ptr->number.sender, &mms_pb_bcd);
    }

    while(PNULL != mt_undownloaded_ptr)
    {
        if(!MMIAPIPB_BCDCmp(&mms_pb_bcd, pb_bcd))
        {
            undownload_del_flag = TRUE;
            break;
        }
        mt_undownloaded_ptr = mt_undownloaded_ptr->next_ptr;
    }
    if((s_mmisms_order.total_undownloaded_num.total_msg_num > 0) && undownload_del_flag)
    {
        MMIIDLE_RemovePushMsgEx( MMI_IDLE_PUSH_MSG_SMS, PNULL, pb_bcd);
    }
    s_mmisms_chat_linked_order.unread_unit_num = unread_num;
                                       ...
                                       )
{
    ...
    while (PNULL != cur_order_id)
    {
        ...
        if(cur_order_id != PNULL && cur_order_id == cur_order_id->next_chat_ptr)
        {
            SCI_TRACE_LOW("MMISMS link array error MMISMS_SetCurChatMsgMarked");
            break;
        } 
 
        if (while_count > total_msg_num)
        {
            SCI_TRACE_LOW("MMISMS link array error while_count exceed total_msg_num MMISMS_SetCurChatMsgMarked");
            break;
        }          
    }
}
...
//  Note:
/*****************************************************************************/
PUBLIC MMISMS_ORDER_ID_T MMISMS_GetChatOrderByNumber(
                                        MMIPB_BCD_NUMBER_T idle_bcd 
                                       )
{
    MMISMS_CHAT_STATE_UNIT_T *head_chat_id = PNULL;    
    BOOLEAN is_blocklist = FALSE;
    ...
    is_blocklist = MMIAPISET_IsSMSBlacklist(&idle_bcd);
    if(is_blocklist )
    {
        return PNULL;
    }

    while (head_chat_id != PNULL)
    {
        cmp_bcd = MMISMS_SmsBCDToPbBCD(head_chat_id->bcd_num); 
        if(CompareSMSNum(&cmp_bcd, &idle_bcd))
        {
            return head_chat_id->head_ptr;
        }
        head_chat_id = head_chat_id->next_number_ptr;
    }                  
    return PNULL;
                                       ...
                                       )
{
        ...
        for(;j<item_totalnum;j++)
	{
            if(is_blockmessage)
            {
                    if((index ==0) &&(is_blocklist))
                    {
                        break;
                    }
                    while ((head_chat_id != PNULL) && (!is_blocklist))
                    {
                        	head_chat_id = head_chat_id->next_number_ptr;
                            if(head_chat_id==PNULL)
                                break;
            ...
            else
            {
             ...
             if(index>0)
             {
                 if(head_chat_id != PNULL)  //fix coverity cid26084
                 {
                    head_chat_id = head_chat_id->next_number_ptr;
                 }
                 if(head_chat_id != PNULL)
    ...
    while (PNULL != cur_order_id)
    {
        ...
        if(cur_order_id != PNULL && cur_order_id == cur_order_id->next_chat_ptr)
        {
            SCI_TRACE_LOW("MMISMS link array error MMISMS_SetChatMsgMarkedInList");
            break;
        } 
 
        if (while_count > total_msg_num)
        {
            SCI_TRACE_LOW("MMISMS link array error while_count exceed total_msg_num MMISMS_SetChatMsgMarkedInList");
            break;
        }          
    }
}
                                       ...
                                       )
{
	...
	for(;j<item_totalnum;j++)
	{
            if(is_blockmessage)
                ...
                if(index>0)
                {
                    if(head_chat_id != PNULL)  //coverity 26113
                    {
                        head_chat_id = head_chat_id->next_number_ptr;
                    }
                    if(head_chat_id != PNULL)
...
/*****************************************************************************/
//  Description : get current chat massage number   
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC uint16 MMISMS_GetCurChatMsgNum(void)
{
    MMISMS_ORDER_ID_T order_id = PNULL;
...
PUBLIC uint16 MMISMS_GetCurChatMsgNumEx(void)
{
    MMISMS_ORDER_ID_T temp_order_id = PNULL;
    uint16 index = 0;
    MMIPB_BCD_NUMBER_T cmp_bcd = {0}; 
    ...
    temp_order_id = MMISMS_GetChatOrderByNumber(cmp_bcd);
...
LOCAL uint32 GetChatOrderLinkDeleteAvailNum(void)
{
    ...
    while (PNULL != num_ptr)
    {
        ...
        while(PNULL != order_id)
        {
            ...
            order_id = order_id->next_chat_ptr;
            while_count++;
            if(order_id != PNULL && order_id == order_id->next_chat_ptr)
            {
                SCI_TRACE_LOW("MMISMS link array error GetChatOrderLinkDeleteAvailNum");
                break;
            } 
    
            if (while_count > total_msg_num)
            {
                SCI_TRACE_LOW("MMISMS link array error while_count exceed total_msg_num GetChatOrderLinkDeleteAvailNum");
                break;
            }              
        }
...
PUBLIC MMISMS_ORDER_ID_T MMISMS_GetNumNodeByIndexInList(uint16 index)
{
	...
	for(;j<item_totalnum;j++)
	{
            ...
            if(index>0)
            {
                if(PNULL != head_chat_id)  //coverity CID26067
                {
                    head_chat_id = head_chat_id->next_number_ptr;
                }
                if(PNULL != head_chat_id)
...
//  Note: 
/*****************************************************************************/
PUBLIC MMISMS_ORDER_ID_T MMISMS_GetNumNodeByIndexInListEx(uint16 index, BOOLEAN *is_need_edit_draft)
{
    MMISMS_ORDER_ID_T current_order_id = PNULL;
    MMISMS_CHAT_STATE_UNIT_T *head_chat_id = PNULL;
    MMISMS_ORDER_ID_T temp_order_id = PNULL;
    ...
    uint16 item_totalnum=0;
	uint16 j=0;
    BOOLEAN  is_cur_number_all_draft =  FALSE;
    head_chat_id = s_mmisms_chat_linked_order.head_ptr;
    ...
    //优先找草稿
    if(head_chat_id != PNULL && head_chat_id->is_has_draft)
    {
        is_cur_number_all_draft = TRUE;
        temp_order_id = head_chat_id->head_ptr;
        while(temp_order_id)
        {
            if(MMISMS_MO_DRAFT != temp_order_id->flag.mo_mt_type)
            {
                is_cur_number_all_draft = FALSE;
                SCI_TRACE_LOW("mmisms meet a message not draft, no need edit draft");
                break;
            }
            temp_order_id = temp_order_id->next_chat_ptr;
        }
    ...
    if(head_chat_id != PNULL)  //coverity CID26087
    {
        if(is_cur_number_all_draft ||
        (head_chat_id->draft_ptr && (MMISMS_TYPE_MMS == head_chat_id->draft_ptr->flag.msg_type)))
        {
            *is_need_edit_draft = TRUE;
            return head_chat_id->draft_ptr;
        }
    }
...
/*****************************************************************************/
//  Description : get sms/mms total number
//  Global resource dependence : 
//  Author: bingjie.chen
//  Note: 
/*****************************************************************************/
PUBLIC uint16 MMISMS_GetChatMessageTotalNum(void)
{
    MMISMS_CHAT_STATE_UNIT_T *head_chat_id        = PNULL;
	uint16                   total_num            = 0;
    ...
    while (PNULL != head_chat_id)
    {
		total_num   += head_chat_id->total_num;

        head_chat_id = head_chat_id->next_number_ptr;
    }

	SCI_TRACE_LOW("MMISMS GetChatMessageTotalNum total_num = %d", total_num);
	
	return total_num;
}

/*****************************************************************************/
//  Description : get sms/mms number by some conditions
//  Global resource dependence : 
//  Author: bingjie.chen
//  Note: 
/*****************************************************************************/
PUBLIC uint16 MMISMS_GetChatMessageNumbyConditions(uint32 start_time, uint32 end_time, MMISMS_MSG_TYPE_E type, MMISMS_TYPE_E mo_mt_type, MN_DUAL_SYS_E dual_sys)
{
	MMISMS_ORDER_ID_T        current_order_id      = PNULL;
    MMISMS_CHAT_STATE_UNIT_T *head_chat_id         = PNULL;
	uint16                   send_counter          = 0;
	uint16					 receive_counter	   = 0;
	uint16					 counter			   = 0;
    ...
    while (PNULL != head_chat_id)
    {
        ...
        SCI_TRACE_LOW("MMISMS: current_order_id is 0x%x", current_order_id);
        SCI_TRACE_LOW("MMISMS: dual_sys %d ", dual_sys);
		if (PNULL != current_order_id)
		{
	        SCI_TRACE_LOW("MMISMS: current_order_id->flag.dual_sys = %d ", current_order_id->flag.dual_sys);
			while (PNULL != current_order_id)
			{
			        SCI_TRACE_LOW("MMISMS: current_order_id->time = %d", current_order_id->time);
                            SCI_TRACE_LOW("MMISMS: start_time = %d", start_time);
                            SCI_TRACE_LOW("MMISMS: end_time = %d", end_time);
                            SCI_TRACE_LOW("MMISMS: current_order_id->flag.msg_type = %d", current_order_id->flag.msg_type);
                            SCI_TRACE_LOW("MMISMS: type = %d", type);
				if (dual_sys == current_order_id->flag.dual_sys 
				    && current_order_id->time > start_time 
				    && current_order_id->time <= end_time 
				    && current_order_id->flag.msg_type == type)
				{
				        SCI_TRACE_LOW("MMISMS: mo_mt_type = %d", mo_mt_type);
                                   SCI_TRACE_LOW("MMISMS: current_order_id->flag.mo_mt_type = %d", current_order_id->flag.mo_mt_type);
					if (mo_mt_type == MMISMS_MO_TYPE && current_order_id->flag.mo_mt_type == MMISMS_MO_SEND_SUCC)
					{
						send_counter++;
					}
					else if (mo_mt_type == MMISMS_MT_TYPE && (current_order_id->flag.mo_mt_type == MMISMS_MT_TO_BE_READ || current_order_id->flag.mo_mt_type == MMISMS_MT_HAVE_READ))
					{
						receive_counter++;
					}
					else
	...
	if (mo_mt_type == MMISMS_MT_TYPE)
	{
		counter = receive_counter;
	} 
	else if (mo_mt_type == MMISMS_MO_TYPE)
	{
		counter = send_counter;
	} 
	else
	{
		SCI_TRACE_LOW("MMISMS: mo_mt_type is %d", mo_mt_type);
	}

	SCI_TRACE_LOW("MMISMS: MMISMS_GetChatMessageNumbyConditions mo_mt_type = %d, counter = %d", mo_mt_type, counter);
...
//    MMIPB_BCD_NUMBER_T cmp_bcd = {0};
	...
	for(;j<item_totalnum;j++)
	{
            ...
            if(index>0)
            {
                if(head_chat_id != PNULL)  // coverity cid26121
                {
                    head_chat_id = head_chat_id->next_number_ptr;
                }
                index--;
...
PUBLIC void MMISMS_SetExceedUnreadNeedUpdate(void)
{
        	...
        	if(is_blockmessage)
        	   {
                ...
                if(index>0)
                {
                    if(head_chat_id != PNULL)  //coverity cid26125
                    {
                        head_chat_id = head_chat_id->next_number_ptr;
                    }
                    if(head_chat_id != PNULL)
    ...
    while(temp_order_id)
    {
               ...
               if(MMISMS_MT_TO_BE_READ == cur_longsms_order_id->flag.mo_mt_type)
               {
                   ...
                   if(!MMK_IsOpenWin(MMISMS_WAITING_WIN_ID))
                   {
                       MMIPUB_OpenWaitWin(1, &wait_text, PNULL, PNULL, MMISMS_WAITING_WIN_ID, IMAGE_NULL,
                               ANIM_PUBWIN_WAIT, WIN_ONE_LEVEL, MMIPUB_SOFTKEY_NONE, MMISMS_HandleExceedUnreadWaitWinMsg);
                   }
       ...
       temp_order_id = temp_order_id->next_chat_ptr;
       while_count++;
       if(temp_order_id != PNULL && temp_order_id == temp_order_id->next_chat_ptr)
       {
           SCI_TRACE_LOW("MMISMS link array error AdjustDraftChatNode");
           break;
       } 
...
//    Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMISMS_IsLongSmsFirstOneOrder(MMISMS_ORDER_ID_T order_id)
{
    BOOLEAN long_sms_first_come_one = FALSE;
    MMISMS_ORDER_ID_T cur_order_id = PNULL;
    ...
    while (cur_order_id != PNULL)
    {
        if(order_id == cur_order_id)
        {
            long_sms_first_come_one = TRUE;
            break;
        }
        cur_order_id = cur_order_id->next_ptr;
    }
 
    return long_sms_first_come_one;
}        

/*****************************************************************************/
//  Function Name: Is draft message
//  Description  : 
//  Global resource dependence : s_mmisms_chat_linked_order
//  Author:arvin
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMISMS_IsDraftMessage(int32 record_id)
{
    BOOLEAN result = FALSE;
    MMISMS_ORDER_ID_T head_order_id = s_mmisms_order.mo_draft_head_ptr; 

   while(head_order_id)
   {
        if(record_id == head_order_id->record_id)
        {
            result = TRUE;
		 break;
        }
	  head_order_id = head_order_id->next_ptr;
   }
    return result;
}
/*****************************************************************************/
...
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMISMS_MarkAndRemoveDraftByOrderId(int32 record_id)
{
    BOOLEAN result = FALSE;
    MMISMS_ORDER_ID_T head_order_id = s_mmisms_order.mo_draft_head_ptr; 
