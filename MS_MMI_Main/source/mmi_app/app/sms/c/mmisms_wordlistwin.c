...
{
    WIN_TITLE( STXT_HEADER_TEMPLATES ),
...
)
{
    ...
    switch (msg_id)
    {
        case MSG_OPEN_WINDOW:
            ...
            //GUILIST_SetSoftkeyType(ctrl_id,GUISK_DATA_IMAGE_ID, GUISK_DATA_TEXT_ID, GUISK_DATA_IMAGE_ID);
            {
                list_rect = MMITHEME_GetClientRectEx(win_id);
                list_rect.top = MMITHEME_GetStatusBarHeight();
                GUILIST_SetRect(MMISMS_SELECTWORD_LIST_CTRL_ID,&list_rect);
            }
        ...
        case MSG_FULL_PAINT:
            {
                ...
                title_rect = MMITHEME_GetClientRectEx(win_id);
                title_rect.bottom = MMITHEME_GetStatusBarHeight() - 1;
                //draw rect
                LCD_FillRect(&lcd_dev_info, title_rect, GUI_RGB2RGB565(0,0,0));/*lint !e572*/
                //draw title
                MMI_GetLabelTextByLang(STXT_HEADER_TEMPLATES, &title_str);

                 //draw page tip
                tip_style.font = SONG_FONT_16;
                tip_style.font_color = GUI_RGB2RGB565(255,255,255);/*lint !e572*/
