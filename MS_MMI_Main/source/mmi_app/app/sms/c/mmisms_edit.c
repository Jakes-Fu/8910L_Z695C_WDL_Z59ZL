...
#ifdef MMS_SUPPORT
            if( MMIMMS_EDIT_FROM_SENDBOX_LIST_FARWARD != MMIAPIMMS_GetMMSEditEnterReason())
#endif
            {            
                MMISMS_DisplayUpdatePrompt(TRUE);            
                MMISMS_CloseEditWin();
                                     ...
                                     )
{
    ...
    case MMISMS_ALPHABET_MANAGEMENT_GSM_ALPHABET:        
        for (i = 0; i < length; i++)
        {
            convert_content_ptr[i] = MMISMS_ConvertChar2Alphbet(content_ptr[i]);
        }
        break;
    ...
    case MMISMS_ALPHABET_MANAGEMENT_8BIT:
        for (i = 0; i < length; i++)
        {
            if (!MMIAPICOM_IsUcs2Char(content_ptr[i]))
            {
                convert_content_ptr[i] = content_ptr[i];
            }
            else
            {
    ...
    default:
        MMI_WSTRNCPY(convert_content_ptr, length, content_ptr, length, length);
        break;
    }
...
#endif
    {
        is_ucs2 = !MMIAPICOM_IsDefaultString(content_ptr, length);

        if (is_ucs2)
        {
        language = MMISMS_R8_DecodeContentForLanguage(content_ptr, length,&is_single_shift);
        }
        ...
        is_ucs2 = FALSE;
        is_r8 = TRUE;
        length = MMISMS_NationalR82default(content_ptr,content_r8_ptr,sms_content_ptr->length,language);      
        }
    }
    
#ifdef MMI_SMS_ALPHABET_SETTING_SUPPORT
    if (MMISMS_ALPHABET_MANAGEMENT_AUTO == alpha_management  ) // full , reduce coding
#endif
    {
        sms_content_ptr->alphabet = (is_ucs2) ? MN_SMS_UCS2_ALPHABET : MN_SMS_DEFAULT_ALPHABET;
    }
    else if(MMISMS_ALPHABET_MANAGEMENT_GSM_ALPHABET == alpha_management )
    {
         if(!MMISMS_IsR8Language(language))
         {
