...
const uint16 sms_nv_len[] =
{
    ...
    sizeof( MMISMS_CB_CONFIG_T ) * MMI_DUAL_SYS_MAX,
#else
    sizeof( MMINV_SMSCB_ENABLE_T ) * MMI_DUAL_SYS_MAX,
#endif
    sizeof( MMINV_SMSCB_50_CHANNEL_T ) * MMI_DUAL_SYS_MAX,
    ...
    sizeof(BOOLEAN) * MMI_DUAL_SYS_MAX,
#endif
#endif
                              ...
                              )
{
    BOOLEAN ret_value = TRUE;
    uint16 true_id = 0;
    uint32 m_item_id = MMINV_SMSCB_MESSAGE_LIST_FIRST + rid;
    NVITEM_ERROR_E nv_result = NVERR_NONE;
                                ...
                                )
{
    ...
    BOOLEAN ret_value = TRUE;
    uint16 begin_true_id = 0;
    uint32 begin_m_item_id = MMINV_SMSCB_MESSAGE_LIST_FIRST;
    NVITEM_ERROR_E nv_result = NVERR_NONE;
    ...
    begin_true_id = MMI_GetTrueNVItemId(begin_m_item_id);
    nv_result = EFS_NvitemDelete(
        begin_true_id + start_index,
        (uint16)(begin_true_id + end_index )
        );
                                ...
                                )
{
    ...
    nv_result = EFS_NvitemDelete(
        begin_true_id,
        (uint16)(begin_true_id + smscb_in_me_valid_num - 1)
        );
