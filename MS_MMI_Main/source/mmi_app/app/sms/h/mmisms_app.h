...
#define     MMISMS_R8_MAX_SINGLE_DEF_LEN          155
...
#define MMISMS_ALPHABET_MANAGEMENT_DEFAULT MMISMS_ALPHABET_MANAGEMENT_UCS2
...
#define MMISMS_RESEND_MAX_COUNT            0    // 1 // 自动重发次数
#else
...
#define    MMISMS_DEFAULT_WORD_NUM            7
...
typedef struct MMISMS_CHAT_STATE_UNIT_T_tag
{
    ...
    BOOLEAN is_has_draft;//include draft message or not
	...
	MMISMS_STATE_T *draft_ptr;//the draft message of the current number
...
typedef enum
{
    MMISMS_ENTER_BY_MAINMENU = 1,  //。
    MMISMS_ENTER_BY_PB = 2,
    MMISMS_ENTER_BY_CL = 3,
    MMISMS_ENTER_BY_IDLE = 4,
    MMISMS_ENTER_BY_IDLE_Reply = 5,
    MMISMS_ENTER_BY_DIAL_IDLE = 6,
    MMISMS_ENTER_BY_DIAL_MENU = 7,
    MMISMS_ENTER_BY_WRITEMESSAGE = 8,
    MMISMS_ENTER_BY_MAX        
} MMISMS_ENTER_FOR_BACK_MUSIC_E;
...
typedef struct
{
    wchar  convert_char; //char of GSM 7 bit default alphabet
    wchar  orig_char;
}MMISMS_GSM_LIB_UNICODE_CONVERT_STR;
#endif
...
typedef struct
{
    BOOLEAN encoding_suport;
    uint8 encoding_table;  //0: reduced 1:full
}MMISMS_NDT_CFG_T;
#endif
...
{
    BOOLEAN is_update;
    BOOLEAN is_send_finish;
    BOOLEAN is_success;  //send success or fail
    MN_DUAL_SYS_E        update_dual_sys;
    MN_SMS_STORAGE_E            update_storage;
    MN_SMS_RECORD_ID_T        update_record_id;
    MMISMS_ORDER_ID_T          send_finish_order_id;
} MMISMS_SEND_UPDATE_CHAT_DISP_T;
...
typedef struct _MMI_SMS_GLOBAL_T
{
    ...
    MMISMS_SEND_UPDATE_CHAT_DISP_T update_chat_disp;
...
#endif    
    MMISMS_ORDER_ID_T       draft_oper_order_id;    
    BOOLEAN       is_draft_deleting;    
