...
BOOLEAN MMIAPIPARSE_ParseString(uint8 *str, uint8 str_len)
{
	...
	if (MMIMRAPP_ProcessEntrStr((uint8*)parsStrInfo.inputStr, parsStrInfo.strLen)) /*lint !e718 !e18*/
	{
        ...
        if (isStrParsered)
        {
            MMIAPISET_OpenSIMLockNetUnlockWin();
            MMIAPIIDLE_CloseDialWin();
...
MMI_PARSE_TYPE_E MMIAPIPARSE_ParseTeleNum(MN_DUAL_SYS_E dual_sys, uint8 *str, uint8 str_len, BOOLEAN is_video_call )
{
    ...
    if(MMIAPIPARSE_Is2DigitDial(parsStrInfo.inputStr,parsStrInfo.strLen))
    {
    ...
    if((parsStrInfo.strLen > 1)
        && ('#' == parsStrInfo.inputStr[parsStrInfo.strLen - 1])
        && (PNULL == MMIAPICOM_SearchChar((uint8 *)parsStrInfo.inputStr,'P'))
        &&(PNULL == MMIAPICOM_SearchChar((uint8 *)parsStrInfo.inputStr,'W'))
        &&(PNULL == MMIAPICOM_SearchChar((uint8 *)parsStrInfo.inputStr,'p'))
        &&(PNULL == MMIAPICOM_SearchChar((uint8 *)parsStrInfo.inputStr,'w')))
    {
        //ussd
...
PUBLIC BOOLEAN MMIAPIPARSE_Is2DigitDial(char str_ptr[],uint8 str_len)
{
	BOOLEAN get_status = FALSE;
    if(PNULL == str_ptr || str_len != 2)
    {
       return FALSE;
    }
    EFS_NvitemRead(TELE_NV_DIAL2_PROMPT,(uint16)sizeof(get_status),(uint8*)&get_status);
    if(TRUE == get_status)
    {
        if(str_ptr[0]=='*')
        {
            if(str_ptr[1]=='1'||str_ptr[1]=='2'||str_ptr[1]=='5'||str_ptr[1]=='9')
            {
                return TRUE;
            }
        }
