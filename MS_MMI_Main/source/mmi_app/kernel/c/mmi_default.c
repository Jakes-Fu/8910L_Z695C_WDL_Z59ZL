...
//20151209 liuzhicheng add for music.
...
#define MMI_BACKLIGHT_TIMER_VALUE           5000
#define MMI_BACKLIGHT_CHARGING_TIMER_VALUE      5000//     30000  //关机充电背景灯光时间30s，2015
...
#ifdef MMI_DEVICELOCK_SUPPORT
#define MMI_DEVICELOCK_DELAY_TIME    (1000*60*2)
#endif
...
LOCAL uint16 *s_popup_menu_bg_buf_ptr = PNULL;
...
LOCAL BOOLEAN s_is_keylock_by_star_key = FALSE;   // 当前是否允许keylock
...
)
{
	...
	/* 上下键关闭Banner */
    MMIAPICOM_CloseBannerWin(keystatus,keycode);
    /* 滑盖关闭时候的处理 */
...
//============== MET MEX  End ===================
    ...
    //add control panel
    if(keystatus == KEY_LONG_PRESSED && keycode == KEY_HASH){
        MMIAPICP_OpenMainWin();
        return TRUE;
    }
#endif
#ifdef MMI_INDIAN_SOS_SUPPORT
    if ((TRUE == is_indiasos_supported) && (keystatus == KEY_LONG_PRESSED) && (keycode == KEY_5))
    {
        return TRUE;
    }
#endif
#if  0//ZDT_PRODUCT_Z685S
    //长按*键锁屏
    if(keystatus == KEY_LONG_PRESSED && keycode == KEY_STAR)
    {
        //mainmenu spec: All the other keys are disabled except Navi, Back, End in move mode.
        if(
#ifdef IM_ENGINE_T9
			!MMIIM_GetImKerState() &&
#endif
			(!GUIMAINMENU_IsMoveMode(MAINMENUWIN_MENU_CTRL_ID))&&(!GUIMAINMENU_IsColorThemeMode(MAINMENUWIN_MENU_CTRL_ID)))
        {
            MMIAPIKL_LockPhone();
            MMIDEFAULT_CloseAllLight();
            MMIDEFAULT_SetKeylockByStarKey(TRUE);
			return TRUE;
        }
    }
#endif
...
extern BOOLEAN s_is_redkey_closelight;
/*==============================================================================
    ...
    case KEY_PRESSED:
        ...
        if (is_flip)
        {
            if (MMIAPIKL_IsKLWinOnTop())
            {
...
#elif defined(MMI_DEVICELOCK_SUPPORT)
                if (keycode == KEY_RED)
#else
                ...
                //KEY_RED和KEY_WEB键点亮锁屏屏幕2015-10-01
#ifdef MMI_UNISOC_IDLE_EMERGENCY_CALL_SUPPORT
                msg_id = (KEY_PRESSED | keycode);               
#endif
#ifdef ZDT_PRODUCT_BH
			 if (keycode == KEY_RED)
#else
                if ((keycode == KEY_WEB)||(keycode == KEY_RED)
				
#if 0
def MMI_UNISOC_IDLE_EMERGENCY_CALL_SUPPORT
                    ||MMIAPIIdle_IsEmergencyCallOnly(msg_id)
#endif                   
                    )
 #endif	                  
#endif
                {
                    ...
                    //if (keycode == KEY_RED || (keycode == KEY_WEB  && !is_back_light))//wuxx
                    if (keycode == KEY_RED )
#else
                ...
                else
                {
                    ...
                    {
                        need_light_up = FALSE;
                        GPIO_SetLcdBackLightSleepEnable();
                    }
                }
            }
            else
        ...
        case KEY_5:
            ...
            break;
				...
				case KEY_WEB:
					if (MMIAPIKL_IsKLWinOnTop())
					{
					#if defined( ZDT_WDL_Z35ZL_SUPPORT)
						SwitchTorchLight();
					#endif				
					}
				...
				case KEY_0:
					if (MMIAPIKL_IsKLWinOnTop())
					{
						Hourtime_To_Speech();//hero_tts_akey_time_broadcast();
					}
					break;
				//#ifndef ZDT_WDL_Z35ZL_SUPPORT	
				case KEY_STAR:
					if (MMIAPIKL_IsKLWinOnTop())
						{
							Bat_cap_To_Speech();
...
#endif			
#endif
#elif defined (HERO_ENGINE_TTS_SUPPORT)
		case KEY_5:
				case KEY_0:
					if (MMIAPIKL_IsKLWinOnTop())
					{
							hero_tts_akey_time_broadcast();;//hero_tts_akey_time_broadcast();
					}
					break;
			case KEY_STAR:
				if (MMIAPIKL_IsKLWinOnTop())
				{
					Bat_cap_To_Speech();///hero_tts_akey_dateweeklunar_broadcast();;//hero_tts_akey_time_broadcast();
				}
...
)
{
    ...
    */
	...
	{	// 屏蔽默认按键音
		...
		if(hero_tts_dial_broadcast_is_open()||hero_tts_menu_broadcast_is_open())
		{
			return;
		}
	}
...
)
{
    ...
    MMIAPICOM_StatusAreaUpdateEarPhoneState(); // 耳机插拔需要更新状态栏的状态。
...
)
{
    ...
    *halt_back_ptr = TRUE;
    ...
    {
        ex_alert_text = TXT_NO_SD_CARD_ALERT;
    }
...
)
{
    ...
    if (is_flip)
    {
        ...
        if (is_back_light)
        {
            ...
            {
              if((KEY_LONG_PRESSED==keystatus)&&(KEY_RED == keycode)&&(RESTART_BY_CHARGE==POWER_GetRestartCondition()))
              {
                  return TRUE;
              }
              else
...
)
{
    ...
    *halt_back_ptr = FALSE;
	...
	if ((keystatus == KEY_PRESSED) && ((KEY_UPSIDEKEY == keycode) || (KEY_DOWNSIDEKEY != keycode))) 
	{
		if (lebao_is_running() && lebao_player_is_playing()) 
		{
			int vol = lebao_get_volume();
...
PUBLIC void MMK_HandleRedKeyProcess(void)
{
    ...
    )
    {
        ...
        //若有打开的Active application, Red Key关App回到Mainmenu
        //Mainmenu界面Red Key锁屏
        if(MAIN_IDLE_WIN_ID == focus_win_id 
            ||MMI_DEVICELOCK_PROGRESS_WIN_ID == focus_win_id
            ||MMIIDLE_SECURITY_CODE_WIN_ID == focus_win_id
#ifdef MMI_UNISOC_IDLE_EMERGENCY_CALL_SUPPORT
            ||MMIIDLE_EMERGENCY_CALL_WIN_ID == focus_win_id
#endif
            ||MMIAPIKL_IsPhoneLocked()
            )
        {
            if(FALSE == MMIIDLE_IsAllowLockedOperation())
            {
                MMK_ReturnIdleWin();
                MMIAPIKL_LockPhone();
                MMIDEFAULT_CloseAllLight();
        ...
        else if((FALSE == MMIIDLE_IsAllowLockedOperation() && (TRUE == MMIAPIKL_IsPhoneLocked())))
        {
            MMK_ReturnIdleWin();
            MMIAPIKL_LockPhone();
            MMIDEFAULT_CloseAllLight();
        ...
        else if(MAIN_MAINMENU_WIN_ID == focus_win_id)
        {
            MMK_ReturnIdleWin();
            MMIAPIKL_LockPhone();
            MMIDEFAULT_CloseAllLight();
        ...
        else if(!MMK_IsOpenWin(MAIN_MAINMENU_WIN_ID))
        {
            MMK_ReturnIdleWin();
        ...
        if(MMK_IsOpenWin(MMISMS_DELETE_PROGRESS_WIN_ID))
        {
           return;
        }
        else if(MMK_IsOpenWin(MMIMP3_DEL_FILE_WAIT_WIN_ID))
        {
           return;
        } 
        else
...
#if 0 //defined (MAINLCD_SIZE_176X220) || defined  (MAINLCD_SIZE_128X160)
...
#endif
    if((  MMICC_STATUS_WIN_ID < MMK_GetWinId(MMK_ConvertIdToHandle(win_handle)) 
            && MMICC_MULTICALL_LIST_WIN_ID > MMK_GetWinId(MMK_ConvertIdToHandle(win_handle)) )
            ||(MAIN_IDLE_WIN_ID == MMK_GetWinId(MMK_ConvertIdToHandle(win_handle)))  )
    {
...
#endif
LOCAL MMI_RESULT_E Zdt_handleConfirmationDialog(
                                ...
                                )
{
    ...
    case MSG_KEYLONG_RED:
        {
            ...
            {
                //关机
                SCI_TRACE_LOW("MMK_DefaultMSGKbd: PowerOff by MSG_KEYLONG_RED");
#ifdef ZDT_PRODUCT_Z685S
		if (MMIAPIENG_RedkeyIsEnableInIdle()&& !MMIAPIKL_IsKLWinOnTop())
#else
		if (MMIAPIENG_RedkeyIsEnableInIdle())
#endif
                {
			 ...
			 ///  MMIPUB_OpenConfirmationDialogWinByTextIdEx(MMK_GetFirstAppletHandle(),PNULL,qtn_query_poweroff,PNULL,PNULL,&query_id,PNULL,MMIPUB_SOFTKEY_DIALOG_OK,Zdt_handleConfirmationDialog,PNULL);
                        MMIAPIPHONE_PowerOff(); //关机
                    }
                    else
                    {
    ...
    //避免关机和锁屏冲突, 改为响应keyup
#if defined MMI_PDA_SUPPORT || defined MMI_MSTYLE_SUPPORT
    case MSG_KEYUP_RED:
        if(MMIAPIKL_IsKLWinOnTop())
        {
             //pda按power, 一键灭屏, to do...
        ...
        else     
        {
            if (s_is_down_keystatus_backlight_on)
            {
                if(!MMK_IsOpenWin(PHONE_STARTUP_CHARGE_WIN_ID) && !MMK_IsOpenWin(PHONE_STARTUP_CHARGE_OK_WIN_ID))
                {
    ...
    case MSG_KEYDOWN_HEADSET_DETECT:
        ...
        MMIAPIENVSET_ReSetAllVolmue(MMISET_VOL_HENDSET_MAX);
        ...
        MMISRVAUD_ResetDurationTimer(PNULL);
        // 设置LCD和KeyBad的背景灯
        MMIDEFAULT_TurnOnBackLight();
...
//            MmiSendSignal(TASK_FL_ID, sendSignal);
    ...
    case MSG_KEYLONG_STAR:
        //mainmenu spec: All the other keys are disabled except Navi, Back, End in move mode.
        if((!GUIMAINMENU_IsMoveMode(MAINMENUWIN_MENU_CTRL_ID))&&(!GUIMAINMENU_IsColorThemeMode(MAINMENUWIN_MENU_CTRL_ID)))
        {
            //常按*键解锁
            MMIAPIKL_LockPhone();
            MMIDEFAULT_CloseAllLight();
...
PUBLIC void MMIDEFAULT_TurnOnBackLight(void)
{
	...
	BOOLEAN lcd_staus = FALSE;
	BOOLEAN kpd_satus = FALSE;
	GUI_RECT_T  full_screen_rect = MMITHEME_GetFullScreenRect();
#ifdef DYNAMIC_MAINMENU_SUPPORT
	MMI_BACKLIGHT_STATUS_E prev_backlight_status = s_backlight_status;
#endif

#ifndef MMI_TURNON_BACKLIGHT_EFFECT_SUPPORT //点亮屏幕动画 
	if((s_backlight_status == MMI_BACKLIGHT_OFF)&&((s_backlight_ctrol == MMI_BACKLIGHT_CTRL_NORMAL)||(s_backlight_ctrol == MMI_BACKLIGHT_CTRL_ALWAYS_ON )||(s_backlight_ctrol == MMI_BACKLIGHT_CTRL_NORMAL_HALF_ON )))
	{
		...
		//点亮背光	
		OS_TickDelay (30);
		MMIAPISET_UpdateLCDContrast(LCD_ID_0, MMIAPISET_GetCurrentContrast());
		BOOT_SetBLStatus(1);  //set bk flag to be 1
#ifndef WIN32		
		if (MMIDEFAULT_GetFlipStatus())
		{
			_GPIO_PROD_SetVal (GPIO_PROD_KEYPAD_BL_ID, kpd_satus);
		}
						
		_GPIO_PROD_SetVal (GPIO_PROD_LCD_BL_EN_ID, TRUE);
#else
		GPIO_SetLcdBackLight(TRUE);
#endif
		SCI_MMI_EnableDeepSleep( (uint32) !TRUE);
		SCI_LCDBacklightBrightless_EnableDeepSleep ( (uint32) !TRUE);
...
#ifdef DYNAMIC_MAINMENU_SUPPORT
	if (prev_backlight_status == MMI_BACKLIGHT_OFF)
	{
		MMIMENU_DyWakeUpSDCard();
	}
#endif
}
...
#else
            //SCI_SLEEP(20);   //should not use sleep here
            OS_TickDelay (20);
#endif
            {           
                //点亮背光
                                              ...
                                              )
{
    ...
    if(MMIDEFAULT_GetFlipStatus())
    {
		...
		else if (RESTART_BY_CHARGE == restart_condition)
		{
			switch(s_backlight_status)
            {
                case MMI_BACKLIGHT_ON:
                    MMK_StartWinTimer(
                        VIRTUAL_WIN_ID,
                        MMI_LCD_LIGHT_TIMER_ID,
                        MMI_BACKLIGHT_CHARGING_TIMER_VALUE,
                        FALSE);
                    break;
...
#ifdef MMI_DEVICELOCK_SUPPORT
                            //backlight_time = MMI_BACKLIGHT_TIMER_VALUE; 
#endif
...
BOOLEAN MMIDEFAULT_HandleKeyLock(BOOLEAN need_lock)
{
    POWER_RESTART_CONDITION_E   start_condition = POWER_GetRestartCondition();
    BOOLEAN is_lock = MMIAPIKL_IsPhoneLocked();
	if(MMIAPIKL_IsPhoneLocked())
	{
		need_lock = FALSE ;
	}
                             ...
                             )
{
    ...
    case APP_COPY_ONE_FILE:
        MMIFMM_CopyProgressCallBack(((MmiAppCopyMsgS*)sig_data_ptr)->data);
        break;
    case APP_FIND_FILES_END_IND:
        {
            ...
            if (PNULL != sig)
            {
               MMIAPIFMM_SearchFileNotify((MMIFMM_SRV_HANDLE)sig->handle);
    ...
    case APP_DEL_DIR_END_IND:
        ...
        break;
    case APP_DEL_ONE_FILE:
        MMIAPIFMM_DelProgressCallBack(((MmiAppCopyMsgS*)sig_data_ptr)->data);
        break;
#if 0
    case APP_MP3_INIT_PLAYER_FINISH:
    case APP_MP3_LOAD_ALL_MUSIC_FINISH:
        MMIAP_SaveCallBack(sig_code);
        break;
#endif
...
//  Note: TRUE success, FALSE fail
/*****************************************************************************/
PUBLIC BOOLEAN MMIDEFAULT_SavePopMenuBgBuf(
                              uint16 *bg_buf_ptr
                              )
{
   if(PNULL == s_popup_menu_bg_buf_ptr) 
   {
        s_popup_menu_bg_buf_ptr = SCI_ALLOC_APP(SCR_SIZE);
   }
   
    if(PNULL != bg_buf_ptr && PNULL != s_popup_menu_bg_buf_ptr)
    {
        SCI_MEMSET(s_popup_menu_bg_buf_ptr, 0, SCR_SIZE);
        SCI_MEMCPY(s_popup_menu_bg_buf_ptr, bg_buf_ptr, SCR_SIZE);
        return TRUE;
    }
...
//  Note: 
/*****************************************************************************/
PUBLIC uint16 * MMIDEFAULT_GetPopMenuBgBuf(void)
{
    return s_popup_menu_bg_buf_ptr;
}
...
//  
/*****************************************************************************/
PUBLIC void MMIDEFAULT_FreePopMenuBgBuf(void)
{
    if(PNULL != s_popup_menu_bg_buf_ptr)
    {
        SCI_FREE(s_popup_menu_bg_buf_ptr);
        s_popup_menu_bg_buf_ptr = PNULL;
    }
}
    ...
    if(MMI_IsPermit3D((BLOCK_MEM_ID_E)BLOCK_MEM_POOL_MMI_3D_DESKTOP))
    {
            ...
            if(GUIMENU_IsFirstDisp(handle) && MMITHEME_Is3DOpen())
            {
                if(0 == GUIMENU_GetLevel(handle) && MMK_GetWinMoveStyle(win_handle) != MOVE_FORBIDDEN)
                {
                    if(AllocTrans3DBuf(&old_buf_ptr, &new_buf_ptr))
                    {
                        ...
                        MMITHEME_PopupMenuEffect(PNULL, &menu_rect, 0, TRUE);
                    }
                }
        ...
        else if(s_is_last_closed_popup_window)
        {
            ...
            {          
                ...
                MMITHEME_PopupMenuEffect(PNULL, &s_last_popup_menu_rect, 0,  FALSE);
            }
...
PUBLIC BOOLEAN MMIDEFAULT_HandleAutoKeylockOver( void )
{
    ...
    SCI_TRACE_LOW("[lockscreen]:MMIDEFAULT_HandleAutoKeylockOver Enter==========");

#ifdef ZDT_PRODUCT_Z685S
	auto_keylock_timer = MMIAPISET_GetAutoBoardKey();  
#else
#ifdef MMI_MSTYLE_SUPPORT
    auto_keylock_timer = MMISET_KEYLOCK_1_MIN;
#else
    auto_keylock_timer = MMIAPISET_GetAutoBoardKey();  
#endif
#endif
    ...
    SCI_TRACE_LOW("[lockscreen]: s_is_allow_keylock(%d), auto_keylock_timer(%d)", s_is_allow_keylock, auto_keylock_timer);
    SCI_TRACE_LOW("[lockscreen]: IsPhoneLocked(%d)", MMIAPIKL_IsPhoneLocked());
    SCI_TRACE_LOW("[lockscreen]: start_condition(%d), IsKbdTpPress(%d)", start_condition, IsKbdTpPress());
#ifdef LCD_SLIDE_SUPPORT
    SCI_TRACE_LOW("[lockscreen]: MMIDEFAULT_GetSlideStatus(%d)", MMIDEFAULT_GetSlideStatus());
#endif
#ifdef MRAPP_SUPPORT
    SCI_TRACE_LOW("[lockscreen]: MMIMRAPP_mrappIsActived(%d)", MMIMRAPP_mrappIsActived());
#endif
#ifdef BROWSER_SUPPORT
    SCI_TRACE_LOW("[lockscreen]: MMIAPIBROWSER_IsRunning(%d)", MMIAPIBROWSER_IsRunning());
#endif
#ifdef JAVA_SUPPORT
    SCI_TRACE_LOW("[lockscreen]: MMIAPIJAVA_IsForegroundRuning(%d)", MMIAPIJAVA_IsForegroundRuning());
#endif
...
&&(!GetStartupLockStatus())
    ...
    {
        MMIAPIKL_LockPhone();       
        SCI_TRACE_LOW("[lockscreen]:MMIAPIKL_LockPhone ==========");
    }               

    SCI_TRACE_LOW("[lockscreen]:MMIDEFAULT_HandleAutoKeylockOver Left==========");
...
PUBLIC void MMIDEFAULT_StartAutoKeylockTimer(void)
{
    ...
    MMISET_KEYLOCK_TYPE_E auto_keylock_timer = {0};   
    SCI_TRACE_LOW("[lockscreen]:MMIDEFAULT_StartAutoKeylockTimer Enter");

#ifndef MMI_KEY_LOCK_BACKLIGHT_IN_1_SUPPORT

#ifdef MMI_MSTYLE_SUPPORT
//    auto_keylock_timer = MMISET_KEYLOCK_1_MIN;
    auto_keylock_timer = MMIAPISET_GetAutoBoardKeyMS();
#else
    auto_keylock_timer = MMIAPISET_GetAutoBoardKey();  
#endif
    ...
    SCI_TRACE_LOW("[lockscreen]: s_is_allow_keylock(%d), auto_keylock_timer(%d)", s_is_allow_keylock, auto_keylock_timer);
    SCI_TRACE_LOW("[lockscreen]: IsPhoneLocked(%d)", MMIAPIKL_IsPhoneLocked());
    SCI_TRACE_LOW("[lockscreen]: GetKeyguardSwitch(%d)", MMIAPISET_GetKeyguardSwitch());    

    //因为自动锁屏功能会根据up键来处理，平台收到MSG_KEYUP_STAR的时候判断
    //是否启动定时器，这个时候恰好还没有走到这里解锁，
    //导致自动锁屏定时器在解锁后不能开启；
    //因为很多应用处理按键的动作很不规范，解锁还是改回MSG_KEYUP_STAR
    //自动解锁的定时器再设定时去掉当前是否锁屏的判断，在定时器触发时再判断
    if (s_is_allow_keylock &&(MMISET_KEYLOCK_CLOSE != auto_keylock_timer)/* && !MMIAPIKL_IsPhoneLocked()*/ && MMIAPISET_GetKeyguardSwitch())
    {
        MMK_StartWinTimer(
        VIRTUAL_WIN_ID,
...
#else
    #ifdef MMI_DEVICELOCK_SUPPORT
    if (s_is_allow_keylock && !MMIAPIKL_IsPhoneLocked() && MMIAPISET_GetKeyguardSwitch())
    {
        MMK_StartWinTimer(
        VIRTUAL_WIN_ID,
        MMI_AUTO_KEYLOCK_TIMER_ID,
        MMI_DEVICELOCK_DELAY_TIME,
        FALSE);
    }
    #endif
#endif
    SCI_TRACE_LOW("[lockscreen]:MMIDEFAULT_StartAutoKeylockTimer Left");

}

/*****************************************************************************/
//  Description : check if the keylock by star key          
//  Global resource dependence : 
//  Author: cheney.wu
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMIDEFAULT_IsKeylockByStarKey(void)
{
    return s_is_keylock_by_star_key;
}

/*****************************************************************************/
//  Description : set the keylock by star key          
//  Global resource dependence : 
//  Author: cheney.wu
//  Note: 
/*****************************************************************************/
PUBLIC void MMIDEFAULT_SetKeylockByStarKey(BOOLEAN is_keylock)
{
    s_is_keylock_by_star_key = is_keylock;
}
...
PUBLIC BOOLEAN MMIDEFAULT_IsPermitPlayNewMsgRing(void)
{
    ...
    if( 
		...
		|| (MMIMP3_AUDIO_PLAYING == MMIAPIMP3_GetAudioState()) //MP3只在播放的状态不播放信息铃声
        || MMIAPIFM_IsFmActive()
//#ifdef MMI_BLUETOOTH
...
PUBLIC void MMI_HanldeEndKeyForTest(void)
{
   ...
   if(MMIAPIIDLE_IsPhoneLocked())
   {
		...
		sendSignal->keyCode = SCI_VK_STAR;
		sendSignal->Sender = KPDSVR;
		MmiSendSignal(TASK_FL_ID, (MmiSignalS*)sendSignal);
		SCI_Sleep(100);
    ...
    if(!(MAIN_MAINMENU_WIN_ID == focus_win_id) && (!(MAIN_IDLE_WIN_ID == focus_win_id)))
    {
		single_code = KPD_DOWN;
		MmiCreateSignal(single_code, sizeof(MmiKeyPress), (MmiSignalS**)&sendSignal);

		sendSignal->keyCode = SCI_VK_POWER;
		sendSignal->Sender = KPDSVR;
		MmiSendSignal(TASK_FL_ID, (MmiSignalS*)sendSignal);
		SCI_Sleep(100);
		single_code = KPD_UP;
		...
		i=0;
		while(MAIN_MAINMENU_WIN_ID != (MMK_GetFocusWinId()) )
		{
			if(i>100)
			{
				break;
    ...
    if(MAIN_MAINMENU_WIN_ID ==  (MMK_GetFocusWinId()) )
    {
		single_code = KPD_DOWN;
		MmiCreateSignal(single_code, sizeof(MmiKeyPress), (MmiSignalS**)&sendSignal);

		sendSignal->keyCode = SCI_VK_MENU_CANCEL;
		sendSignal->Sender = KPDSVR;
		MmiSendSignal(TASK_FL_ID, (MmiSignalS*)sendSignal);
		SCI_Sleep(100);
		single_code = KPD_UP;
		...
		SCI_Sleep(100);
		i=0;
		while(MAIN_IDLE_WIN_ID != (MMK_GetFocusWinId()) )
		{
			if(i>100)
			{
				break;
