...
/*****************************************************************************/
//  Description : Get English Label Text
//  Global resource dependence : 
//  Author:
//  Note:
/*****************************************************************************/
...
//    uint8* re_str_ptr = NULL;
    ...
    if( (MMI_SYS_RES_FLAG == ( label & MMI_SYS_RES_FLAG ) ) || (0 == label))
    {
        MMIRES_GetEnglishText(label, MMK_GetFirstAppletHandle(), str_ptr);
    }
    else
    {
        /* please use MMIRES_GetText(....) instead of MMI_GetLabelTextByLang() */
...
/*****************************************************************************/
//  Description : check the image whether is NULL
//  Global resource dependence : none
//  Author: 
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMI_IsImageNull(MMI_IMAGE_ID_T label)
{
    BOOLEAN is_image_null = FALSE;

    if( (IMAGE_NULL == label) || (COMMON_IMAGE_NULL == label) || (0 == MMICOM_GET_WORD_LB(label)))
    {
        is_image_null = TRUE;
    }

    return is_image_null;
}
...
uint32 getDataIdByName(const char* name){
	if (!name) return 0;
	if (!strcmp(name, "NotoSans-Regular.ttf")){
		return R_WEBKIT_FONT;
	}
	return 0;
}

BOOLEAN getFontDataFromRes(void** buffer, size_t* size, const char* name){
	MMIRES_DATA_INFO_T file_info = {0};
	uint32 id = getDataIdByName(name);
	if (id > 0){
		BOOLEAN result = MMI_GetDataInfo(id, &file_info);
		*buffer = file_info.data_ptr;
		*size = file_info.data_len;
		return result;
	}
	return FALSE;
}
...
PUBLIC uint16 MMI_GetModuleNVItemCount(uint16 module_id)
{
    return GetNVMAXItem(module_id);
}

#ifdef BT_LOG_MENU_SUPPORT
PUBLIC MN_RETURN_RESULT_E MMI_ReadNVItemAtInit(uint32 item_id, void * buffer_ptr)
{
    uint16 module_id = 0;
    uint16 nv_offset = 0;
    uint16 true_item_id = MN_NV_USER_BASE; //MMI_NV_USER_BASE;
    uint16 i = 0;
#ifndef MMI_MULTI_SIM_SYS_NONE
    ERR_MNDATAMAG_CODE_E reslut = ERR_MNDATAMAG_NO_ERR;
#else
...
#else
    true_item_id += 0x10 + 0 + 0x7 + 0x0 + 0x1 + 0x2 + 0x0 + 0x43 + 0x2A + 0x0 + 0x0;
#endif
...
#ifndef MMI_MULTI_SIM_SYS_NONE
    reslut = MNNV_ReadItem(true_item_id, s_mmi_nv_len[module_id][nv_offset], buffer_ptr); /*read NV*/
    if (ERR_MNDATAMAG_NO_ERR != reslut)
    {
        ret_value = MN_RETURN_FAILURE;
    }

#else
    reslut = EFS_NvitemRead(true_item_id,s_mmi_nv_len[module_id][nv_offset],(uint8 *)buffer_ptr);	
...
PUBLIC void MMI_WriteNVItem(uint32 item_id, void * buffer_ptr)
{
    ...
    SCI_ASSERT((true_item_id <= MMI_UINV_MAX_ID) && (true_item_id >= MMI_UINV_USER_BASE)); /*MAX NV item*/ /*assert verified*/
    SCI_ASSERT(s_mmi_nv_len[module_id][nv_offset] != 0); /*assert verified*/

    MNNV_WriteItem(true_item_id, s_mmi_nv_len[module_id][nv_offset], buffer_ptr);/*write nv*/
}
