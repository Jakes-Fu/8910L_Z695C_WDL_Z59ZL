...
#define MMICOM_PANEL_BG_COLOR MMI_BLACK_COLOR
#define MMICOM_PANEL_FG_COLOR MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_DEFAULT)
#define MMICOM_PANEL_FONT_COLOR MMI_WHITE_COLOR
#define MMICOM_PANEL_MAXVOL_FONT MMI_DEFAULT_NORMAL_FONT

#define MMICOM_PANEL_CURVOL_LENGTH    36
#define MMICOM_PANEL_SEPARATOR_LENGTH 10
#define MMICOM_PANEL_MAXVOL_MARGIN    37 
#define MMICOM_PANEL_MAXVOL_LENGTH 	  28
#define MMICOM_PANEL_VOLUME_HEIGHT_H    27
#if defined (MAINLCD_SIZE_128X160)
#define MMICOM_PANEL_CURVOL_MARGIN    48
#define MMICOM_PANEL_INCVOL_MARGIN     0
#define MMICOM_PANEL_VOLUME_HEIGHT    25
#define MMICOM_PANEL_DECVOL_MARGIN    18
#define MMICOM_PANEL_CURVOL_FONT SONG_FONT_22
#else
#define MMICOM_PANEL_CURVOL_MARGIN    45
#define MMICOM_PANEL_INCVOL_MARGIN    9
#define MMICOM_PANEL_VOLUME_HEIGHT    35
#define MMICOM_PANEL_DECVOL_MARGIN    8
#define MMICOM_PANEL_CURVOL_FONT SONG_FONT_27
#endif
...
typedef struct  
{
	...
	uint8                 control_fadeout_timer_id;
...
//  Note: 
/*****************************************************************************/
LOCAL void StartControlFadeoutTimer(void);
...
//  Note: 
/*****************************************************************************/
LOCAL void StopControlFadeoutTimer(void);
...
LOCAL float                 s_panel_opacity = 1.0;
...
LOCAL void StartControlFadeoutTimer(void)
{
    if (0 < s_panel_para.control_fadeout_timer_id)
    {
        MMK_StopTimer(s_panel_para.control_fadeout_timer_id);
    }
    s_panel_para.control_fadeout_timer_id = MMK_CreateWinTimer(s_panel_para.cur_win_handle/*MMICOMMON_PANEL_WIN_ID*/, MMI_100MSECONDS, TRUE);
}
...
LOCAL void StopControlFadeoutTimer(void)
{
    ...
    {
        MMK_StopTimer(s_panel_para.control_fadeout_timer_id);
        s_panel_para.control_fadeout_timer_id= 0;
    }
}
                                      ...
                                      )
{
...
#endif
		MMK_SetWinDisplayStyleState(s_panel_para.parent_win_id, WS_DISPATCH_TO_CHILDWIN, TRUE);
        InitDisplayPosition();
        StartControlPanelTimer();
    ...
    case MSG_TIMER:
        ...
        {
            s_panel_para.control_panel_timer_id = 0;
			StartControlFadeoutTimer();
        }
		else if(s_panel_para.control_fadeout_timer_id == *((uint8 *)param))
		{
			if(s_panel_opacity > 0.2)
			{
				s_panel_opacity -= 0.2;
...
//#endif
    ...
    default:
        recode = MMI_RESULT_FALSE;
        if(MMK_IsOpenWin(s_panel_para.parent_win_id) && 
            (s_panel_para.parent_win_id == MMIAPMAINWIN_GetMainPlayWinHandle()))
        {
            MMK_PostMsg(s_panel_para.parent_win_id, msg_id, PNULL, 0);
        }
        break;
    }
...
LOCAL void DrawControlPanel(void)
{
    ...
    UILAYER_ClearRect(lcd_dev_info_ptr, s_panel_rect.panel_rect);
    //背景
    LCD_FillArgbRect(lcd_dev_info_ptr, s_panel_rect.panel_rect, MMICOM_PANEL_BG_COLOR, (uint8)(255*0.8*s_panel_opacity));
#else
    //LCD_FillRect(lcd_dev_info_ptr, s_panel_rect.panel_rect, MMI_TRANSPARENCE_COLOR);
    //LCD_FillArgbRect(lcd_dev_info_ptr, s_panel_rect.panel_rect, MMI_TRANSPARENCE_COLOR, 255);
#endif
    IMG_EnableTransparentColor(TRUE);     
    dis_point.x = s_panel_rect.panel_rect.left;
    dis_point.y = s_panel_rect.panel_rect.top;
    ...
    dis_rect.left = s_panel_rect.progress_rect.left;
    dis_rect.top = s_panel_rect.progress_rect.top + s_panel_rect.progress_offset;
    dis_rect.right = s_panel_rect.progress_rect.left + s_panel_rect.progress_width - 1;	//s_panel_rect.progress_rect.left + 120; //
    dis_rect.bottom = s_panel_rect.progress_rect.bottom; 
    image_id = IMAGE_COMMON_PLAYER_PANEL_PROGRESS_BG;//IMAGE_COMMON_PLAYER_PANEL_PROGRESS_BG;
    GUIRES_DisplayImg(PNULL,
        &dis_rect,
        PNULL,
        win_id,
...
#ifdef UI_MULTILAYER_SUPPORT
    LCD_FillArgbRect(lcd_dev_info_ptr, dis_rect, MMICOM_PANEL_FG_COLOR, (uint8)(255*s_panel_opacity));
#else
    LCD_FillRect(lcd_dev_info_ptr, dis_rect, MMICOM_PANEL_FG_COLOR);
    //LCD_FillArgbRect(lcd_dev_info_ptr, dis_rect, MMICOM_PANEL_FG_COLOR, (uint8)(255*s_panel_opacity));
#endif

    //音量数字
    num_str.wstr_len =  sprintf((char*)char_num_str, "%d", s_panel_info.cur_value);
    MMIAPICOM_StrToWstr(char_num_str,wchar_num_str);
    num_str.wstr_ptr = wchar_num_str;
    text_style.align = ALIGN_HVMIDDLE;
    text_style.font = MMICOM_PANEL_CURVOL_FONT;
    text_style.font_color = MMICOM_PANEL_FONT_COLOR;
    GUISTR_DrawTextToLCDInRect( 
        lcd_dev_info_ptr,
        (const GUI_RECT_T *)&s_panel_rect.number_rect,       //the fixed display area
        (const GUI_RECT_T *)&s_panel_rect.number_rect,       //用户要剪切的实际区域
...
//分隔符("/")
    num_str.wstr_len =  1;
    num_str.wstr_ptr = &separator;
    text_style.font = MMICOM_PANEL_MAXVOL_FONT;
    text_style.begin_alpha = 255*0.4;
    text_style.end_alpha = text_style.begin_alpha;
#if 0
    GUISTR_DrawTextToLCDInRect( 
        lcd_dev_info_ptr,	
        (const GUI_RECT_T *)&s_panel_rect.icon_rect,       //the fixed display area
        (const GUI_RECT_T *)&s_panel_rect.icon_rect,       //用户要剪切的实际区域
        (const MMI_STRING_T *)&num_str,
        &text_style,
        state,
        GUISTR_TEXT_DIR_AUTO
...
#endif
    state |= GUISTR_STATE_GRADUAL_HOR; /*lint !e63*/
    state |= GUISTR_STATE_GRADUAL_VER; /*lint !e63*/
	////////////解决分隔符位置错乱的问题/////		
	/*GUISTR_DrawSingleLineGradualInRect(
		lcd_dev_info_ptr, 
		(const GUI_RECT_T *)&s_panel_rect.icon_rect,
		(const MMI_STRING_T *)&num_str, 
		&text_style, 
         state);*/
    GUISTR_DrawTextToLCDInRect(
...
//总音量
    num_str.wstr_len =  sprintf((char*)char_num_str, "%d", s_panel_info.max_value);
    MMIAPICOM_StrToWstr(char_num_str,wchar_num_str);
    num_str.wstr_ptr = wchar_num_str;
    ...
    GUISTR_DrawSingleLineGradualInRect(
		lcd_dev_info_ptr, 
		(const GUI_RECT_T *)&s_panel_rect.text_rect,
		(const MMI_STRING_T *)&num_str, 
		&text_style, 
         state);
    ...
    if(s_panel_info.cur_value == s_panel_info.max_value)
    {
        img_id = IMAGE_PRIMO_ARROW_UP;
    }
    else
    {
        img_id = IMAGE_PRIMO_ARROW_UP_PRESS;
    }
    GUIRES_DisplayImg(PNULL,
	        &s_panel_rect.inc_rect,
	        PNULL,
	        win_id,
	        img_id,
	        lcd_dev_info_ptr);
    ...
    if(s_panel_info.cur_value == s_panel_info.min_value)
    {
        img_id = IMAGE_PRIMO_ARROW_DOWN;
    }
    else
    {
        img_id = IMAGE_PRIMO_ARROW_DOWN_PRESS;
    }
    GUIRES_DisplayImg(PNULL,
	        &s_panel_rect.dec_rect,
	        PNULL,
	        win_id,
	        img_id,
	        lcd_dev_info_ptr);
...
LOCAL void InitDisplayPosition(void)
{
    ...
    s_panel_rect.panel_rect.left = 0;
    s_panel_rect.panel_rect.right = lcd_width - 1;
    s_panel_rect.panel_rect.top = 0;
    s_panel_rect.panel_rect.bottom = lcd_height - 1;
    
    s_panel_rect.number_rect.left = lcd_width/2 - MMICOM_PANEL_CURVOL_LENGTH/2;
    s_panel_rect.number_rect.right = lcd_width/2 + MMICOM_PANEL_CURVOL_LENGTH/2 - 1;
    s_panel_rect.number_rect.top = lcd_height - MMICOM_PANEL_CURVOL_MARGIN - MMICOM_PANEL_SEPARATOR_LENGTH - MMICOM_PANEL_INCVOL_MARGIN;
#if defined (MAINLCD_SIZE_128X160)	
    s_panel_rect.number_rect.bottom = s_panel_rect.number_rect.top + GUI_GetFontHeight(33, UNICODE_ASCII) - 1;
#else
    s_panel_rect.number_rect.bottom = s_panel_rect.number_rect.top + GUI_GetFontHeight(MMICOM_PANEL_CURVOL_FONT, UNICODE_ASCII) - 1;	
#endif    
    s_panel_rect.icon_rect.left = s_panel_rect.number_rect.right + 1;
    s_panel_rect.icon_rect.right = s_panel_rect.icon_rect.left + MMICOM_PANEL_SEPARATOR_LENGTH - 1;
    s_panel_rect.icon_rect.top = lcd_height - MMICOM_PANEL_MAXVOL_MARGIN - MMICOM_PANEL_SEPARATOR_LENGTH - MMICOM_PANEL_INCVOL_MARGIN;
    s_panel_rect.icon_rect.bottom = s_panel_rect.icon_rect.top + GUI_GetFontHeight(MMICOM_PANEL_MAXVOL_FONT, UNICODE_ASCII) - 1;
    
    s_panel_rect.text_rect = s_panel_rect.icon_rect;
    s_panel_rect.text_rect.left = s_panel_rect.icon_rect.right + 1;
    s_panel_rect.text_rect.right = s_panel_rect.text_rect.left + MMICOM_PANEL_MAXVOL_LENGTH - 1;
    
    GUIRES_GetImgWidthHeight(&image_width, &image_height, IMAGE_PRIMO_ARROW_UP_PRESS, win_id);
    s_panel_rect.inc_rect.top = lcd_height - MMICOM_PANEL_VOLUME_HEIGHT*3 + MMICOM_PANEL_INCVOL_MARGIN + MMICOM_PANEL_INCVOL_MARGIN + 6;
    s_panel_rect.inc_rect.bottom = s_panel_rect.inc_rect.top + image_height - 1 ;
    s_panel_rect.inc_rect.left = lcd_width/2 - image_width/2;
    s_panel_rect.inc_rect.right = s_panel_rect.inc_rect.left + image_width - 1;
    
    GUIRES_GetImgWidthHeight(&image_width, &image_height, IMAGE_PRIMO_ARROW_DOWN_PRESS, win_id);
    s_panel_rect.dec_rect.bottom = lcd_height - MMICOM_PANEL_DECVOL_MARGIN - 1- MMICOM_PANEL_SEPARATOR_LENGTH - MMICOM_PANEL_INCVOL_MARGIN;
    s_panel_rect.dec_rect.top = s_panel_rect.dec_rect.bottom - image_height + 1;
    s_panel_rect.dec_rect.left = lcd_width/2 - image_width/2;
    s_panel_rect.dec_rect.right = s_panel_rect.dec_rect.left + image_width - 1;
}
#endif
