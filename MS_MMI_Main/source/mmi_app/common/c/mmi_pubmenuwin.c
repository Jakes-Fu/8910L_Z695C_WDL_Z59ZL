    ...
    case MSG_APP_MENU:
        {
            ...
            GUIWIN_GetSoftkeyTextId(win_id,&left_softkey_id,PNULL,PNULL);
            if(TXT_NULL == left_softkey_id||IMAGE_NULL == left_softkey_id)
            {
                break;
            }
        }
                                  ...
                                  )
{
    ...
    if ( PNULL == param )
    {
        SCI_TRACE_LOW("MMIPUB_OpenPubMenuWin param is PNULL!!");
        return;
    }
#ifdef HERO_ENGINE_TTS_SUPPORT 
    ...
    {   
        ...
        {
            SCI_TRACE_LOW("MMIPUB_OpenPubMenuWin result is FALSE!!");
