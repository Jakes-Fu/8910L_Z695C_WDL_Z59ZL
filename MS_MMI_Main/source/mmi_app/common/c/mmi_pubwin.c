...
#define TXT_COMMON_OK STXT_SOFTKEY_OK_MK
...
//Dialog window type 
LOCAL uint32                      dialog_win_type               = NULL;
//alert_win 
LOCAL BOOLEAN                     alert_win                     = FALSE;

uint16     dialog_edit_height = 0;
...
/*****************************************************************************/
//    Description : Set win_type for dialog 
//    Global resource dependence : 
//    Author:   xiaoming.ren
//    Note:
/*****************************************************************************/
LOCAL void SetDialogWinType(
                            DIALOG_TYPE  dialog_type
                            );

/*****************************************************************************/
//  Description : Set dialog softkey type 
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL void SetDialogSoftkeyType(
                            MMI_HANDLE_T win_handle
                            );

/*****************************************************************************/
//  Description : Display alarm dialog window
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL void DisplayAlarmDialogWin(
                                 MMIPUB_INFO_T    *win_info_ptr
                                 );

/*****************************************************************************/
//  Description : Is type dialog window
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN IsTypeDialogWin(
                              MMI_HANDLE_T win_handle
                              );

/*****************************************************************************/
//  Description : create dialog progress control
//  Global resource dependence : 
//  Author: Jassmine
//  Note:
/*****************************************************************************/
LOCAL IGUICTRL_T* CreateDialogPrgCtrl(
                                      MMIPUB_INFO_T    *win_info_ptr
                                      );

/*****************************************************************************/
//  Description : display dialog progress window
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL void DisplayDialogProgressWin(
                              MMIPUB_INFO_T    *win_info_ptr,
                              uint32            total_size,
                              uint32            finished_size
                              );                              
...
/*****************************************************************************/
//  Description : Open dialog window with input field 
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL void OpenDialogInputFieldWin(
                        MMI_HANDLE_T                applet_handle,
                        MMI_STRING_T                *text1_str_ptr,     //text 1
                        MMI_WIN_ID_T                *win_id_ptr,        //win id,default is MMIPUB_ALERT_WIN_ID
                        MMI_WIN_PRIORITY_E          *win_priority_ptr,  //win priority,default is WIN_ONE_LEVEL
...
LOCAL void OpenDialogInputFieldWinLen(
                        MMI_HANDLE_T                applet_handle,
                        MMI_STRING_T                *text1_str_ptr,     //text 1
                        MMI_WIN_ID_T                *win_id_ptr,        //win id,default is MMIPUB_ALERT_WIN_ID
                        MMI_WIN_PRIORITY_E          *win_priority_ptr,  //win priority,default is WIN_ONE_LEVEL
...
/*****************************************************************************/
//  Description : Display dialog softkey
//  Global resource dependence : 
//  Author: Jassmine
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN DisplayDialogSoftkey(
                            MMI_TEXT_ID_T   *softkey_id,
                            MMIPUB_INFO_T   *win_info_ptr
                            );

/*****************************************************************************/
//  Description : Convert to dialog softkey
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL MMIPUB_SOFTKEY_STYLE_E ConvertDialogSoftkey(
                                MMIPUB_SOFTKEY_STYLE_E      softkey_style      //softkey style
                                );

/*****************************************************************************/
//  Description : creat and open progress window - waiting indicator 
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL void OpenProgressWaitingIndWin(
                        MMI_HANDLE_T                applet_handle,
                        MMI_STRING_T                *text1_str_ptr,     //text 1
                        MMI_WIN_ID_T                *win_id_ptr,        //win id,default is MMIPUB_ALERT_WIN_ID
                        MMI_WIN_PRIORITY_E          *win_priority_ptr,  //win priority,default is WIN_ONE_LEVEL
...
/*****************************************************************************/
//  Description : create dialog progress control for waiting indicator.
//  Global resource dependence : 
//  Author:
//  Note:
/*****************************************************************************/
LOCAL IGUICTRL_T* CreateDialogPrgWaitingIndCtrl(
                                      MMIPUB_INFO_T    *win_info_ptr,
                                      GUI_BOTH_RECT_T  *prog_rect                                      
                                      );

/*****************************************************************************/
//  Description : display dialog progress window (text only)
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
LOCAL void DisplayDialogProgressTextOnlyWin(
                              MMIPUB_INFO_T    *win_info_ptr
                              );
...
//  Note:
/*****************************************************************************/
LOCAL void OpenProgressTextOnlyWin(
                        MMI_HANDLE_T                applet_handle,
                        MMI_STRING_T                *text1_str_ptr,     //text 1
                        MMI_WIN_ID_T                *win_id_ptr,        //win id,default is MMIPUB_ALERT_WIN_ID
                        MMI_WIN_PRIORITY_E          *win_priority_ptr,  //win priority,default is WIN_ONE_LEVEL
...
{   
    DIALOG_TYPE dialog_type = DIALOG_WITH_CONFIRMATION;        //dialog type 
    
    SetDialogWinType(dialog_type);

    dialog_win_type = MMIPUBWIN_ALERT;
	if ((softkey_style == PNULL)  || (softkey_style == MMIPUB_SOFTKEY_CUSTOMER))
	{
      alert_win = TRUE;
    }
                                             ...
                                             )
{
    ...
    case MSG_APP_MENU:
    case MSG_APP_WEB:
        if ((win_info_ptr->softkey_style == MMIPUB_SOFTKEY_DIALOG_OK) || (win_info_ptr->softkey_style == MMIPUB_SOFTKEY_ONE))
		{
            SCI_TRACE_LOW("MMIPUB_HandleAlertWinMsg:  MSG_APP_WEB MMIPUB_SOFTKEY_ONE or MMIPUB_SOFTKEY_DIALOG_OK");
			if(TRUE == MMK_IsOpenWin(win_info_ptr->win_id))
		    {
                MMK_CloseWin(win_id);
			}
        ...
        /*
        in some cases, win_info_ptr data may be modified after  MSG_PROMPTWIN_OK msg prcessed, so use win_id instead
        for example:
            in calendar, when pressed delte all until today menu , confirm win will pop up, precess ok button, process win will show, occassonally 
            confirm win and process win have the same adddata_ex_ptr!!! so the following close action is to close process win,it is not right!!!
        */
         //if (MMK_IsOpenWin(win_id))
            if ((MMIPUB_SOFTKEY_NONE != win_info_ptr->softkey_style) && (TRUE == MMK_IsOpenWin(win_info_ptr->win_id)))
		    {
                MMK_CloseWin(win_id);
			}
        }
     ...
     case MSG_APP_CANCEL:
        if ((win_info_ptr->softkey_style == MMIPUB_SOFTKEY_DIALOG_OK) || (win_info_ptr->softkey_style == MMIPUB_SOFTKEY_ONE))
		{
            SCI_TRACE_LOW("MMIPUB_HandleAlertWinMsg: MSG_APP_CANCEL MMIPUB_SOFTKEY_ONE or MMIPUB_SOFTKEY_DIALOG_OK");
			if(TRUE == MMK_IsOpenWin(win_info_ptr->win_id))
		    {
                MMK_CloseWin(win_id);
			}
...
{
    OpenProgressTextOnlyWin(applet_handle, text1_str_ptr, win_id_ptr, win_priority_ptr, softkey_style, handle_func,user_data);    
}
                                 ...
                                 )
{
    ...
    OpenProgressWaitingIndWin( MMK_GetFirstAppletHandle(), &text1_str, win_id_ptr,win_priority_ptr,softkey_style,handle_func,0);
}
                                 ...
                                 )
{
    ...
    OpenProgressWaitingIndWin(applet_handle, &text1_str, win_id_ptr,win_priority_ptr,softkey_style,handle_func,user_data);
}
...
{
    OpenProgressWaitingIndWin(MMK_GetFirstAppletHandle(), text1_ptr, win_id_ptr,win_priority_ptr,softkey_style,handle_func,0);
}
...
{
    OpenProgressWaitingIndWin(applet_handle, text1_ptr, win_id_ptr,win_priority_ptr,softkey_style,handle_func,user_data);
}
                                                ...
                                                )
{
...
#endif
#endif
        if (FALSE == IsTypeDialogWin(win_info_ptr->win_handle))
        {
    ...
    case MSG_FULL_PAINT:
        ...
        if (TRUE == IsTypeDialogWin(win_info_ptr->win_handle)) 
        {           
            DisplayDialogProgressWin(win_info_ptr, s_total_size, s_finished_size);
        }
        else 
        {
...
//         }
        break;
     case MSG_PUB_PROGRESS_UPDATE_TIME_PERIOD:
        if (win_info_ptr != PNULL)
        {
            win_info_ptr->time_period =  (uint32)param;
        }
        break;
    default:
        result = MMIPUBHandleWinMsg( win_info_ptr, msg_id, param );
...
//  Note:
/*****************************************************************************/
LOCAL void UpdateDialogProgress(
                              MMIPUB_INFO_T    *win_info_ptr,
                              uint32            total_size,
                              uint32            finished_size
                              )
{
	...
	uint16          progress_x_point = 0;
	uint16          progress_y_point = 0;
	MMI_HANDLE_T    progbar_ctrl_handle = NULL;
	
	if (PNULL == win_info_ptr 
		|| !MMK_IsFocusWin(win_info_ptr->win_handle) 
		|| !IsTypeDialogWin(win_info_ptr->win_handle))
	{
		return ;
	}
	
	progbar_ctrl_handle =  MMK_GetCtrlHandleByWin( win_info_ptr->win_handle, MMIPUB_PROGRESSBAR_CTRL_ID );
	...
	//progress percentage     
	disp_rect.bottom = bg_rect.bottom - PROGBAR_DIALOG_BOTTOM_MARGIN; //16;
	disp_rect.top = disp_rect.bottom - PROGBAR_DIALOG_PERCENTAGE_HEIGHT; //30;
	disp_rect.left = PROGBAR_DIALOG_PERCENT_LEFT_MARGIN; //6;
	disp_rect.right = bg_rect.right - PROGBAR_DIALOG_PERCENT_RIGHT_MARGIN; //6;
	
	//rec progress 
	progress_x_point = PROGBAR_DIALOG_PROGRESS_LEFT_MARGIN; //0;
	progress_y_point = disp_rect.top - PROGBAR_DIALOG_PROGRESS_HEIGHT; // 15
	
	//progressbar 
	//bg display rect
	bg_rect.top = text_rect.top - PROGBAR_DIALOG_TEXT_TOP_MARGIN ; //8;
	GUI_FillRect( &lcd_dev_info, bg_rect, win_info_ptr->pubwin_theme.dialog_bg_color);               
	DisplayProgressText( win_info_ptr, disp_str, disp_rect, lcd_dev_info );
	CTRLPROGRESSBAR_SetRectBarPosition(progbar_ctrl_handle, progress_x_point, progress_y_point);
	CTRLPROGRESSBAR_SetParam( progbar_ctrl_handle, total_size, 2);    
	CTRLPROGRESSBAR_SetPos(TRUE,finished_size, 0, progbar_ctrl_handle);    
}
...
{
	UpdateDialogProgress(MMIPUB_GetWinAddDataPtr(win_id), s_total_size, s_finished_size);
}
                                                ...
                                                )
{
    ...
    case MSG_FULL_PAINT:
        ...
        DisplayDialogProgressTextOnlyWin(win_info_ptr);
...
/*****************************************************************************/
//  Description : set progress win TimePeriod
//                
//  Global resource dependence : 
//  Author: 
//  Note:
...
PUBLIC void MMIPUB_SetProgressTimePeriod(MMI_WIN_ID_T *win_id_ptr, uint32 TimePeriod)
{
    ...
    MMK_SendMsg(win_id, MSG_PUB_PROGRESS_UPDATE_TIME_PERIOD, (DPARAM)TimePeriod);
}
                                     ...
                                     )
{
    ...
    if (TRUE == IsTypeDialogWin(win_info_ptr->win_handle)) 
    {
        MMIPUB_SetTextDialogTitle(win_info_ptr->win_handle,&text_str,TRUE);
    }        
    else 
    {
                                   ...
                                   )
{
    ...
    {    
        if (TRUE == IsTypeDialogWin(win_info_ptr->win_handle)) 
        {
            MMIPUB_SetTextDialogTitle(win_info_ptr->win_handle,text_ptr,TRUE);
        }        
        else 
        {
                                  ...
                                  )
{
    ...
    DestroyPubWinDisplayText( &(win_info_ptr->text_dialog_title));
    MMIPUB_ReleaseLayer(win_info_ptr);
    ResetPubwinInfo( win_info_ptr );
                        ...
                        )
{
    ...
    MMIPUB_INFO_T       *win_info_ptr = PNULL;
    MMI_WIN_PRIORITY_E  win_priority = WIN_ONE_LEVEL;
    PROCESSMSG_FUNC     handle_function = PNULL;
    MMI_WINDOW_CREATE_T win_create = {0};
    MMI_ANIM_ID_T       anim_id = 0;
    uint32              type = MMIPUBWIN_ALERT;
	
    handle_function = (PROCESSMSG_FUNC)MMIPUB_HandleAlertWinMsg;
                       ...
                       )
{
    ...
    if (!( MMIPUBWIN_WAIT == type || MMIPUBWIN_WAITING == type || MMIPUBWIN_ALARM_DIALOG == type ))
    {
        return;
    }


    if (MMIPUBWIN_ALARM_DIALOG == type && NULL == win_id)
    {
        win_id = MMIPUB_ALARM_WIN_ID;
    }
    ...
    if ( !MMK_IsOpenWin( MMK_GetWinHandle( applet_handle, win_id ) ) )
    {
        ...
        if (MMIPUBWIN_ALARM_DIALOG == type)
        {
            win_info_ptr->pubwin_theme.bg_alpha = 255;
        }        
        win_info_ptr->pubwin_theme.font_color = MMI_BLACK_COLOR;        
        win_create.applet_handle = applet_handle;
        win_create.win_id = win_id;
        win_create.func = HandleDefaultWinMsg;
        win_info_ptr->user_func = handle_function;
                        ...
                        )
{
    ...
    if ( !MMK_IsOpenWin( MMK_GetWinHandle( applet_handle, win_id ) ) )
    {
        ...
        win_info_ptr->softkey_style = softkey_style;
        win_info_ptr->win_id = win_id;
        win_info_ptr->type = MMIPUBWIN_PROGRESSBAR_DIALOG;
        win_info_ptr->is_dialog_progbar_waitingind = TRUE;
                        ...
                        )
{
    ...
    MMIPUB_INFO_T       *win_info_ptr = PNULL;
    MMI_WIN_PRIORITY_E  win_priority = WIN_ONE_LEVEL;
    PROCESSMSG_FUNC     handle_function = (PROCESSMSG_FUNC)MMIPUB_HandleProgressTextOnlyWinMsg;
    MMI_WINDOW_CREATE_T win_create = {0};
    ...
    if ( !MMK_IsOpenWin( MMK_GetWinHandle( applet_handle, win_id ) ) )
    {
        ...
        win_info_ptr->win_id = win_id;
        win_info_ptr->type = MMIPUBWIN_PROGRESSBAR_DIALOG;
        win_info_ptr->is_dialog_progbar_waitingind = FALSE;
                         ...
                         )
{
    ...
    if ((MMIPUBWIN_WAITING == win_info_ptr->type) || (MMIPUBWIN_WAIT == win_info_ptr->type) || 
		(MMIPUBWIN_PROGRESSBAR_DIALOG == win_info_ptr->type) || (MMIPUBWIN_PROGRESS == win_info_ptr->type))
    {
    // 获取alert图片的宽高
    ...
    {
	GUI_BOTH_RECT_T both_client_rect = MMITHEME_GetWinClientBothRect(win_info_ptr->win_handle);
	if(MMITHEME_IsMainScreenLandscape())
	{
    ...
    else
    {
        ...
        else
        {
			...
			{
                line_num = win_info_ptr->line_num + 3;
			}
			else if (MMIPUBWIN_TEXT_DIALOG == win_info_ptr->type)
			{
            ...
            if (MMIPUBWIN_LIST == win_info_ptr->type)
            {
                ...
                MMI_CTRL_ID_T ctrl_id = MMK_GetCtrlId(win_info_ptr->ctrl_handle);
				
                const THEMELIST_ITEM_STYLE_T* layout_ptr = GUILIST_GetItemStyle(ctrl_id , 0);//THEMELIST_GetStyle(GUIITEM_LAYOUT_ONE_LINE_TEXT);
            ...
            if (MMIPUBWIN_PROGRESSBAR_DIALOG == win_info_ptr->type)
            {
                line_height = PROGBAR_DIALOG_PERCENTAGE_HEIGHT;
            }
			else if(MMIPUBWIN_ALARM_DIALOG == win_info_ptr->type)
			{
                line_height = ALARM_TEXT_HEIGHT;
			}
            else
            {
            ...
            if ( MMIPUBWIN_LIST == win_info_ptr->type  )
            {
                if(0 == list_normal_line_height)
                {
                     list_normal_line_height = line_height;
                }
                max_line = (
                            bg_rect.bottom - bg_rect.top + 1 
                            - top_height - bottom_height
                            - win_info_ptr->pubwin_theme.layout.client_v_margin * 2
                           ) / list_normal_line_height;
            }
            else
            {
            max_line = (
            ...
            //不能超过屏幕的高度
            if (MMIPUBWIN_ALARM_DIALOG == win_info_ptr->type){
               line_num = ALARM_TEXT_LINE_MAX;
            }
			else
			{
            ...
            {
                total_height += line_height/*title height*/+ win_info_ptr->pubwin_theme.layout.client_v_margin * 2;

                if (line_num > 1)
                {
                    //list_client_height = ((line_num - 1) * list_normal_line_height + line_height);
                    list_client_height = line_num * list_normal_line_height + win_info_ptr->pubwin_theme.line_space * 2;
                }
                else
                {
                    //list_client_height = line_num * line_height + win_info_ptr->pubwin_theme.line_space * 4;
                    list_client_height = line_num * list_normal_line_height + win_info_ptr->pubwin_theme.line_space * 4;
                }

                total_height += list_client_height;
            }
           else if (MMIPUBWIN_PROGRESSBAR_DIALOG == win_info_ptr->type)
            {
                total_height = line_num * line_height + PROGBAR_DIALOG_TEXT_TOP_MARGIN + PROGBAR_DIALOG_PROGRESS_HEIGHT + PROGBAR_DIALOG_PERCENTAGE_HEIGHT + PROGBAR_DIALOG_BOTTOM_MARGIN + SOFTKEY_BAR_HEIGHT;
            }
            else
            {
                total_height += line_num * line_height + win_info_ptr->pubwin_theme.layout.client_v_margin * 2;
            ...
            image_width = bg_rect.right - bg_rect.left + 1;
            image_height = total_height;
			if ((MMIPUBWIN_INPUTFIELD_DIALOG == win_info_ptr->type)
				|| (MMIPUBWIN_PASSWORD_DIALOG == win_info_ptr->type) || (MMIPUBWIN_DATE_EDITOR_DIALOG == win_info_ptr->type)) 
			{
				image_height =total_height+INPUT_DIALOG_CLIENT_HEIGHT;
				
				if ((MMIPUBWIN_INPUTFIELD_DIALOG == win_info_ptr->type) || (MMIPUBWIN_PASSWORD_DIALOG == win_info_ptr->type)) 
				{
					GUI_RECT_T vk_rect      = {0};
					MMI_HANDLE_T      edit_ctrl_handle = MMK_GetCtrlHandleByWin( win_info_ptr->win_handle, MMIPUB_INPUTFIELD_CTRL_ID );
					im_handle = GUIEDIT_GetIMHandle(edit_ctrl_handle);
					if (GUIIM_INPUT_TYPE_TOUCHPEN == CTRLIM_GetInput (im_handle))
					{
						GUIIM_GetImRect(im_handle,&vk_rect);
						bg_rect.bottom = vk_rect.top-1;
					}
				}				
			}
        }
    ...
    if ( 0 != image_width && 0 != image_height )
    {
        ...
        if (MMIPUBWIN_PROGRESSBAR_DIALOG == win_info_ptr->type
            && FALSE == win_info_ptr->is_dialog_progbar_waitingind )
        {
    ...
    else
    {
        ...
        else
        {
			if (is_RTL)
			{
				alert_rect.right -= win_info_ptr->pubwin_theme.layout.title_start_margin;
				alert_rect.left = alert_rect.right - alert_width + 1;
			}
			else
			{
    ...
    if ( MMIPUBWIN_WAITING != win_info_ptr->type )
    {
		...
		if (is_RTL)
		{
			title_rect.right -= win_info_ptr->pubwin_theme.layout.title_start_margin;
			if ( MMIPUBWIN_PROGRESS != win_info_ptr->type )
			{
				title_rect.right -= alert_width + win_info_ptr->pubwin_theme.layout.title_image_text_space;
			}
		}
		else
		{
...
//				title_rect.left += alert_width + win_info_ptr->pubwin_theme.layout.title_image_text_space;
...
#endif
	{
			uint8 title_line_height = GUI_ConvertGUIFontToSize(win_info_ptr->pubwin_theme.big_font)
                        + win_info_ptr->pubwin_theme.line_space;
			if (is_RTL)
			{
				title_rect.left += win_info_ptr->pubwin_theme.layout.title_start_margin; 
				title_rect.top = bg_rect.top + v_margin;
				title_rect.bottom = title_rect.top + top_height - 1;
				title_rect.top = title_rect.top - (alert_rect.bottom - title_rect.bottom);
			}
			else
			{
				...
				title_rect.top = bg_rect.top + v_margin;
				title_rect.bottom = (0 < win_info_ptr->title_string.wstr_len)? title_rect.top + title_line_height - 1:title_rect.top + win_info_ptr->pubwin_theme.layout.title_start_margin;
			}
        }
	}
    //title_rect   = PNULL;
    ...
    if ( MMIPUBWIN_WAITING != win_info_ptr->type )
    {
...
#endif
        {
            ...
            {
                //prgwin需要为 0/0 和图片预留两行
                    client_rect.bottom = bg_rect.bottom - SOFTKEY_BAR_HEIGHT;
            }
            else if (MMIPUBWIN_LIST == win_info_ptr->type)
            {
                client_rect.top = title_rect.bottom + win_info_ptr->pubwin_theme.layout.client_v_margin;
			...
			else
            {
				...
				client_rect.top = title_rect.bottom + win_info_ptr->pubwin_theme.layout.client_v_margin; 
                client_rect.bottom = client_rect.top + line_num * line_height + win_info_ptr->pubwin_theme.layout.client_v_margin * 2 - 1;
			}
...
//                MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);
    ...
    {
        if (win_info_ptr->time_period != 0)
        {
            u32TimePeriod = win_info_ptr->time_period;
        }
        else
        {
            u32TimePeriod = MMI_500MSECONDS;
        }
         switch( msg_id )
        {
        case MSG_FULL_PAINT:
            ...
            //开启延迟刷新定时器
            win_info_ptr->timer_id = MMK_CreateWinTimer( win_info_ptr->win_handle, u32TimePeriod, FALSE );
            break;
...
//                MMK_SendMsg(win_id, MSG_FULL_PAINT, PNULL);
    ...
    if((MSG_FULL_PAINT == msg_id || MSG_OPEN_WINDOW == msg_id || MSG_NOTIFY_EDITBOX_UPDATE_STRNUM == msg_id || MSG_CLOSE_WINDOW == msg_id)
       && (MMIPUBWIN_INPUTFIELD_DIALOG == win_info_ptr->type)
       || (MMIPUBWIN_PASSWORD_DIALOG == win_info_ptr->type)       
	   || (MMIPUBWIN_DATE_EDITOR_DIALOG == win_info_ptr->type)
        )
    {
    ...
    //switch page
    if (MMIPUBWIN_TEXT_DIALOG == win_info_ptr->type)
    {
        switch( msg_id )
        {
            case MSG_OPEN_WINDOW:
                MMK_SetAtvCtrl(win_info_ptr->win_handle, MMK_GetCtrlHandleByWin(win_info_ptr->win_handle,MMIPUB_TEXT_CTRL_ID));            
                break;

            case MSG_NOTIFY_MIDSK:
		        if (MMK_IsOpenWin(win_info_ptr->win_handle) && 
					((MMIPUB_SOFTKEY_DIALOG_OK == win_info_ptr->softkey_style) || (MMIPUB_SOFTKEY_DIALOG_DISMISS == win_info_ptr->softkey_style)
                    || (MMIPUB_SOFTKEY_DIALOG_HIDE == win_info_ptr->softkey_style)) )
                {
            ...
            case MSG_KEYDOWN_RED:
                if( MMIPUB_SOFTKEY_DIALOG_NONE_CANNOT_CLOSE ==win_info_ptr->softkey_style )
                {
                    break;
                }
                if (MMK_IsOpenWin(win_info_ptr->win_handle))
            ...
            case MSG_KEYREPEAT_DOWN:        
                //MMK_SendMsg(MMK_GetCtrlHandleByWin(win_info_ptr->win_handle,MMIPUB_TEXT_CTRL_ID),MSG_APP_DOWN,(DPARAM)&(win_info_ptr->win_id));        
                break;

            case MSG_APP_UP:
            case MSG_KEYREPEAT_UP:        
                //MMK_SendMsg(MMK_GetCtrlHandleByWin(win_info_ptr->win_handle,MMIPUB_TEXT_CTRL_ID),MSG_APP_UP,(DPARAM)&(win_info_ptr->win_id));
                break;
        }
    }
        
    if ( MSG_CTL_UPDATE_COLOR_THEME == msg_id )
    {
        win_info_ptr->pubwin_theme.dialog_bg_color = MMI_WHITE_COLOR;//MMITHEME_GetAccentColorByOffset(MMITHEME_OFFSET_COLOR_DEFAULT);
    }
    return result;
}
                                       ...
                                       )
{
    ...
    case MSG_FULL_PAINT:
        ...
        {                
            ...
            DisplayAlarmDialogWin( win_info_ptr);                        
        }
        else 
        {
                                      ...
                                      )
{
   if ((MMIPUB_SOFTKEY_OKEXIT == softkey_style) || 
		...
		(MMIPUB_SOFTKEY_DIALOG_OK == softkey_style) ||
        (MMIPUB_SOFTKEY_DIALOG_ALARM == softkey_style) ||
        (MMIPUB_SOFTKEY_DIALOG_CLOSE == softkey_style) ||
        ((MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN == softkey_style) && (FALSE == is_right_softkey)) || 
        ((MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN_VMAIL == softkey_style) && (FALSE == is_right_softkey)) || 
        (MMIPUB_SOFTKEY_DIALOG_OK_CLOSE == softkey_style))
    { 
        MMK_CloseWin(win_id);
    }    
}
...
//     ClearPubWinLayer(lcd_dev_info);
    ...
    if ( MMIPUBWIN_WAITING != win_info_ptr->type )
    {
        ...
        else
        {
...
#if defined (MAINLCD_SIZE_128X160)
            image_rect.left=image_rect.left+5;
            image_rect.right=image_rect.right-5;
#else
            ...
            // 顶上的title图
			if (( MMIPUBWIN_CONFIRM_DIALOG == win_info_ptr->type )|| (MMIPUBWIN_PASSWORD_DIALOG == win_info_ptr->type)
		|| (MMIPUBWIN_INPUTFIELD_DIALOG == win_info_ptr->type)|| (MMIPUBWIN_DATE_EDITOR_DIALOG == win_info_ptr->type)
		|| (MMIPUBWIN_QUERY == win_info_ptr->type) || (MMIPUBWIN_TEXT_DIALOG== win_info_ptr->type))
			{
//SCI_TRACE_LOW("[mmmmmm] line[%d] file[%s] func[%s] win_info_ptr->type=%d win_info_ptr->softkey_style=%d", __LINE__, __FILE__, __FUNCTION__,__FUNCTION__,win_info_ptr->type,win_info_ptr->softkey_style);
...
{
    MMI_IMAGE_ID_T   icon = win_info_ptr->prompt_image_id;
    GUISTR_STYLE_T   text_style = {0};       
    GUI_LCD_DEV_INFO lcd_dev_info = {0};    
    ...
    lcd_dev_info = GetPubWinLayer(win_info_ptr);    
    GUILCD_GetLogicWidthHeight(lcd_dev_info.lcd_id, &logic_width, &logic_height);          
    if(TRUE == IsExistSoftkey(win_info_ptr))
    {
        bg_rect.top = logic_height - ALARM_DIALOG_HEIGHT - SOFTKEY_BAR_HEIGHT;   
    }
    else 
    {
        bg_rect.top = logic_height - ALARM_DIALOG_HEIGHT;   
    }

	bg_rect.top =bg_rect.top/2;
	if(TRUE == IsExistSoftkey(win_info_ptr))
    {
    bg_rect.bottom = bg_rect.top + ALARM_DIALOG_HEIGHT+SOFTKEY_BAR_HEIGHT;
	}
	else
    {
     bg_rect.bottom = bg_rect.top + ALARM_DIALOG_HEIGHT;
    }
#if defined (MAINLCD_SIZE_128X160)
	bg_rect.left = 3;
#else
    bg_rect.left = 10;
#endif
    bg_rect.right = logic_width -bg_rect.left;
    //GUI_FillRect( &lcd_dev_info, bg_rect, win_info_ptr->pubwin_theme.dialog_bg_color);  
    LCD_FillRoundedRect(&lcd_dev_info, bg_rect, bg_rect ,MMI_WHITE_COLOR);

    //Draw alarm icon 
    GUIRES_GetImgWidthHeight(&img_width,&img_height,icon,win_info_ptr->win_handle);
    image_rect.top = bg_rect.top + ALARM_ICON_TOP_MARGIN;   
    image_rect.bottom = image_rect.top + img_height;
    image_rect.left = 0 + ((logic_width - img_width) /2);    
    image_rect.right = image_rect.left + img_width;      
    ...
    //Draw text 
    client_rect.top    = image_rect.bottom + ALARM_ICON_TEXT_SPACE; 
    client_rect.bottom = client_rect.top + ALARM_TEXT_HEIGHT;   
    client_rect.left   = bg_rect.left + ALARM_TEXT_LEFT_MARGIN;  
    client_rect.right  = bg_rect.right - ALARM_TEXT_RIGHT_MARGIN;     

    text_style.align      = ALIGN_HVMIDDLE;
    text_style.font       = win_info_ptr->pubwin_theme.big_font;
                              ...
                              )
{
    ...
    if ( PNULL != win_info_ptr->string[1].wstr_ptr )
       {
           SCI_FREE( win_info_ptr->string[1].wstr_ptr );
           win_info_ptr->string[1].wstr_ptr = PNULL;
           win_info_ptr->string[1].wstr_len = 0;
       }
       ...
       {
           win_info_ptr->string[1].wstr_ptr = SCI_ALLOC_APP( ( text_str_ptr->wstr_len + 1 ) * sizeof(wchar) );
           
           SCI_MEMSET( win_info_ptr->string[1].wstr_ptr, 0, (( text_str_ptr->wstr_len + 1 ) * sizeof(wchar) ));
           
           MMIAPICOM_Wstrncpy( win_info_ptr->string[1].wstr_ptr, text_str_ptr->wstr_ptr, text_str_ptr->wstr_len );
           
           win_info_ptr->string[1].wstr_len = text_str_ptr->wstr_len;
       }
                              ...
                              )
{
    ...
    uint16          max_line = 4;
    uint16          total_height =0;    
    MMI_HANDLE_T    text_ctrl_handle = NULL;  
    
    if (PNULL == win_info_ptr) 
    {
        return ;
    }
    text_ctrl_handle = MMK_GetCtrlHandleByWin( win_info_ptr->win_handle, MMIPUB_TEXT_CTRL_ID );    
    ...
    GUILCD_GetLogicWidthHeight(lcd_dev_info.lcd_id, &logic_width, &logic_height);        
    bg_rect.top = 0;
    bg_rect.bottom = bg_rect.top + logic_height - 1;
    bg_rect.left = 0;
    bg_rect.right = bg_rect.left + logic_width - 1;

    is_exist_skb = IsExistSoftkey( win_info_ptr );
    
    if (TRUE == IsExistSoftkey(win_info_ptr))
    {
        bg_rect.bottom = bg_rect.bottom - SOFTKEY_BAR_HEIGHT;
    }
    //else 
    //{
    //    bg_rect.bottom = bg_rect.bottom;
    //}    
    
    //rec progress 
    progress_x_point = PROGBAR_DIALOG_PROGRESS_LEFT_MARGIN; //0;
    progress_y_point = bg_rect.bottom - 61; // 15

    //progress title
    text_rect.bottom = progress_y_point;
    text_rect.left = bg_rect.left + PROGBAR_DIALOG_TEXT_LEFT_MARGIN; //6;
    text_rect.right = bg_rect.right - PROGBAR_DIALOG_TEXT_RIGHT_MARGIN; //6;
  
    line_num = win_info_ptr->line_num;
    line_num = (line_num <= max_line)?line_num: max_line;
	line_height = GUI_ConvertGUIFontToSize(20) + 4;

    total_height = line_num * line_height;
    
    text_rect.top = text_rect.bottom - total_height;       
    GUITEXT_SetRect( text_ctrl_handle, &text_rect );         
    GUITEXT_SetMargin( text_ctrl_handle, 0 );    
    GUITEXT_SetAlign( text_ctrl_handle, ALIGN_LVMIDDLE);
...
//Craete progressbar control and set parameters        
    if (FALSE == win_info_ptr->is_dialog_progbar_waitingind)
    {
        CreateDialogPrgCtrl(win_info_ptr);
    }
    else 
    {        
        GUI_BOTH_RECT_T prog_rect = {0};
        prog_rect.v_rect.top = progress_y_point;
        prog_rect.v_rect.bottom = prog_rect.v_rect.top + 15;
        prog_rect.v_rect.left = progress_x_point;
        prog_rect.v_rect.right = prog_rect.v_rect.left + 240;            
        CreateDialogPrgWaitingIndCtrl(win_info_ptr, &prog_rect);
    }
    ...
    if (TRUE == IsExistSoftkey(win_info_ptr))
    {
        disp_rect.bottom = bg_rect.bottom - SOFTKEY_BAR_HEIGHT;
    }
    disp_rect.bottom = disp_rect.bottom - PROGBAR_DIALOG_BOTTOM_MARGIN; //16;
    disp_rect.top = disp_rect.bottom - PROGBAR_DIALOG_PERCENTAGE_HEIGHT; //30;
    disp_rect.left = bg_rect.left + PROGBAR_DIALOG_PERCENT_LEFT_MARGIN; //6;
    disp_rect.right = bg_rect.right - PROGBAR_DIALOG_PERCENT_RIGHT_MARGIN; //6;

    //rec progress 
    if (FALSE == win_info_ptr->is_dialog_progbar_waitingind)
    {
        progress_x_point = PROGBAR_DIALOG_PROGRESS_LEFT_MARGIN + 10; //0;
    }
    progress_y_point = disp_rect.top - PROGBAR_DIALOG_PROGRESS_HEIGHT; // 15

    //progress title
    text_rect.bottom = progress_y_point;
    text_rect.left = bg_rect.left; //6;
    text_rect.right = bg_rect.right; //6;
  
    line_num = win_info_ptr->line_num;
    line_num = (line_num <= max_line)?line_num: max_line;
    line_height = GUI_ConvertGUIFontToSize(win_info_ptr->pubwin_theme.big_font) + win_info_ptr->pubwin_theme.line_space;      
    total_height = line_num * line_height;
    
    text_rect.top = text_rect.bottom - total_height - PROGBAR_DIALOG_TEXT_TOP_MARGIN ;       
    GUITEXT_SetRect( text_ctrl_handle, &text_rect );         
	
	//progressbar 
    //bg display rect
    LCD_FillRoundedRect( &lcd_dev_info, bg_rect, bg_rect, win_info_ptr->pubwin_theme.dialog_bg_color);               


    //Display progressbar percentage (progress content)    
    if (FALSE == win_info_ptr->is_dialog_progbar_waitingind)
    {
        if ( PNULL != win_info_ptr->string[1].wstr_ptr )
        {    
            DisplayProgressText( win_info_ptr, win_info_ptr->string[1], disp_rect, lcd_dev_info );
        }
        else
        {       
            DisplayProgressText( win_info_ptr, disp_str, disp_rect, lcd_dev_info );
    ...
    MMK_SetWinDialogProgbarCtrlId(win_info_ptr->win_handle,MMIPUB_PROGRESSBAR_CTRL_ID);

    //Get progressbar control handle by window handle and control id 
    CTRLPROGRESSBAR_SetIsRectBarInDialog(progbar_ctrl_handle, TRUE);
    
    if (TRUE == win_info_ptr->is_dialog_progbar_waitingind)
    {    
        CTRLPROGRESSBAR_SetRectBarPosition(progbar_ctrl_handle, progress_x_point, progress_y_point);
    }
    else 
...
//     ClearPubWinLayer(lcd_dev_info);
    ...
    disp_rect.top = disp_rect.bottom
            - GUI_ConvertGUIFontToSize( win_info_ptr->pubwin_theme.big_font)
            - win_info_ptr->pubwin_theme.line_space
            + 1;
#else
    ...
    disp_rect.bottom = disp_rect.top + line_num  * line_height;
                                      ...
                                      )
{
    ...
    init_data.style  = GUIPRGBOX_STYLE_HORIZONTAL_PROCESS;
    init_data.is_forbid_paint = FALSE;
    
    //create progress
    ctrl_create.guid               = SPRD_GUI_PRGBOX_ID;
    ctrl_create.init_data_ptr      = &init_data;
    ctrl_create.ctrl_id            = MMIPUB_PROGRESSBAR_CTRL_ID;
    ctrl_create.parent_ctrl_handle = 0;
    ctrl_create.parent_win_handle  = win_info_ptr->win_handle;
                                      ...
                                      )
{
    ...
    init_data.lcd_id = lcd_dev_info.lcd_id;
    init_data.style  = GUIPRGBOX_STYLE_RECT_WAITING_INDICATOR_PROCESS;
    init_data.is_forbid_paint = FALSE;
    init_data.both_rect = *prog_rect;
                           ...
                           )
{
    ...
    if (MMIPUBWIN_WAITING == win_info_ptr->type)
    {   
	GUI_FillRect(&win_info_ptr->pubwin_bg_dev, win_info_ptr->client_rect, 0x0);
        return;        
    } 
                            ...
                            )
{
    ...
    title_style.font       = win_info_ptr->pubwin_theme.big_font;
                             ...
                             )
{
    ...
    uint16 line_num = ( MMIPUBWIN_PROGRESSBAR_DIALOG == win_info_ptr->type ) ? MMIPUB_MULTILINE_MAX - 1 : MMIPUB_MULTILINE_MAX;
                                    ...
                                    )
{
	...
	if ( PNULL != win_info_ptr )
    {
        ...
        GUITEXT_SetFont( ctrl_handle, &win_info_ptr->pubwin_theme.big_font, &win_info_ptr->pubwin_theme.font_color );
        GUITEXT_SetLineSpace( ctrl_handle, win_info_ptr->pubwin_theme.line_space);
        
        switch (win_info_ptr->type)
        {
            case MMIPUBWIN_PROGRESSBAR_DIALOG: 
            case MMIPUBWIN_TEXT_DIALOG: 
            case MMIPUBWIN_INPUTFIELD_DIALOG:  
            case MMIPUBWIN_PASSWORD_DIALOG:
            case MMIPUBWIN_DATE_EDITOR_DIALOG:    
    	        GUITEXT_SetAlign( ctrl_handle, ALIGN_LEFT );
                break;

            case MMIPUBWIN_CONFIRM_DIALOG: 
                GUITEXT_SetAlign( ctrl_handle, ALIGN_HVMIDDLE );                                  
                break;    
                
            case MMIPUBWIN_ALARM_DIALOG: 
		case MMIPUBWIN_WAITING:
                GUITEXT_SetAlign( ctrl_handle, ALIGN_HVMIDDLE );   
                break;
        ...
        if (MMIPUBWIN_TEXT_DIALOG == win_info_ptr->type) 
        {
            ...
            GUITEXT_SetMarginEx(ctrl_handle,text_theme.margin_left_space,text_theme.margin_right_space,text_theme.margin_top_space,text_theme.margin_bottom_space);
            GUITEXT_SetIsTextDialog(ctrl_handle,TRUE);
        }       
        else if (MMIPUBWIN_PROGRESSBAR_DIALOG == win_info_ptr->type) 
        {
            GUITEXT_IsDisplayPrg(FALSE, ctrl_handle);   
            GUITEXT_SetMarginEx(ctrl_handle,text_theme.margin_left_space,text_theme.margin_right_space,text_theme.margin_top_space,text_theme.margin_bottom_space);
            GUITEXT_SetAlign( ctrl_handle, ALIGN_LVMIDDLE);
        }
        else if (MMIPUBWIN_CONFIRM_DIALOG == win_info_ptr->type) 
        {
            GUITEXT_SetMarginEx(ctrl_handle,text_theme.margin_left_space,text_theme.margin_right_space,text_theme.margin_top_space,text_theme.margin_bottom_space);
            GUITEXT_IsDisplayPrg(FALSE, ctrl_handle);   
        }        
        else if (MMIPUBWIN_PASSWORD_DIALOG == win_info_ptr->type) 
        {
            GUITEXT_SetMarginEx(ctrl_handle,text_theme.margin_left_space,text_theme.margin_right_space,text_theme.margin_top_space,text_theme.margin_bottom_space);
        }
        else 
...
//fixed by hongjun.jia for bugzilla 2443/2444
        ...
        && MMIPUBWIN_WAITING != win_info_ptr->type )
    {
        ...
        case MMIPUB_SOFTKEY_DIALOG_OK:
        case MMIPUB_SOFTKEY_DIALOG_HIDE:
        case MMIPUB_SOFTKEY_DIALOG_CLOSE:
        case MMIPUB_SOFTKEY_OKEXIT:
        case MMIPUB_SOFTKEY_OKCANCEL:
        case MMIPUB_SOFTKEY_CUSTOMER:
        case MMIPUB_SOFTKEY_DIALOG_ALARM:	
        case MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN:    
        case MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN_VMAIL:
        case MMIPUB_SOFTKEY_DIALOG_OK_CLOSE:
        case MMIPUB_SOFTKEY_DIALOG_TURNOFF_CLOSE:
        case MMIPUB_SOFTKEY_DIALOG_NONE_CANNOT_CLOSE:
        ...
        for ( i = 0; i < GUISOFTKEY_BUTTON_NUM; i++ )
        {
                ...
                && TXT_NULL != softkey_id[i] )
            {
                if ( 0 == handle_array[i] )
                {
                    ...
                    init_data.bg.color   = win_info_ptr->pubwin_theme.dialog_bg_color;
                    init_data.bg.img_id  = win_info_ptr->pubwin_theme.layout.button_img_id;
        ...
        if ( IsExistSoftkey( win_info_ptr ) )
        {
            ...
            gray_bg.bg_type = GUI_BG_COLOR;
            gray_bg.color   = win_info_ptr->pubwin_theme.dialog_bg_color;
            gray_bg.img_id  = win_info_ptr->pubwin_theme.layout.button_img_id;
...
{
    BOOLEAN         result = TRUE;
    //GUI_FONT_T      little_font = SONG_FONT_16;
    ...
    //Set default dialog softkey
    switch (win_info_ptr->softkey_style)
    {        
    case MMIPUB_SOFTKEY_DIALOG_OK_CLOSE:   
		win_info_ptr->softkey_id[0] = PNULL;
		win_info_ptr->softkey_id[1] = STXT_SOFTKEY_OK_MK;  
        win_info_ptr->softkey_id[2] = STXT_CANCEL;
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,PNULL,0,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_SOFTKEY_OK_MK,1,TRUE);
    ...
    case MMIPUB_SOFTKEY_DIALOG_DISMISS:
        win_info_ptr->softkey_id[1] = STXT_SOFTKEY_DISMISS_MK;               
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,PNULL,0,TRUE);
			GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_SOFTKEY_DISMISS_MK,1,TRUE);        
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,PNULL,2,TRUE);
        break;
    ...
    case MMIPUB_SOFTKEY_DIALOG_HIDE:
        win_info_ptr->softkey_id[2] = STXT_CANCEL;
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,PNULL,0,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_SOFTKEY_HIDE_MK,1,TRUE);        
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_CANCEL,2,TRUE);
        break;
        
    case MMIPUB_SOFTKEY_DIALOG_ALARM:
        win_info_ptr->softkey_id[0] = STXT_SOFTKEY_OPEN_LSK;
        win_info_ptr->softkey_id[1] = STXT_SOFTKEY_SLEEP_MK;
        win_info_ptr->softkey_id[2] = STXT_SOFTKEY_STOP_RSK;
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,win_info_ptr->softkey_id[0],0,TRUE);
        GUIWIN_SetSoftKeyButtonfont(win_info_ptr->win_handle,0,&win_info_ptr->pubwin_theme.normal_font);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,win_info_ptr->softkey_id[1],1,TRUE);
		GUIWIN_SetSoftKeyButtonfont(win_info_ptr->win_handle,1,&win_info_ptr->pubwin_theme.big_font);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,win_info_ptr->softkey_id[2],2,TRUE);
        GUIWIN_SetSoftKeyButtonfont(win_info_ptr->win_handle,2,&win_info_ptr->pubwin_theme.normal_font);
        break;
    ...
    case MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN:         
	 case MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN_VMAIL:
        win_info_ptr->softkey_id[0] = STXT_CANCEL;
        win_info_ptr->softkey_id[1] = STXT_SOFTKEY_OK_MK;        
        win_info_ptr->softkey_id[2] = TXT_DELETE;
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_CANCEL,0,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_SOFTKEY_OK_MK,1,TRUE);        
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,TXT_DELETE,2,TRUE);
        break;

    case MMIPUB_SOFTKEY_DIALOG_TURNOFF_CLOSE:             
        win_info_ptr->softkey_id[1] = STXT_SOFTKEY_TURN_OFF_MK;        
        win_info_ptr->softkey_id[2] = STXT_CANCEL;        
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,PNULL,0,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_SOFTKEY_TURN_OFF_MK,1,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_CANCEL,2,TRUE);        
        break;
...
#if 0
    if (TRUE == IsTypeDialogWin(win_handle) && TRUE == IsExistSoftkey(win_info_ptr))
    {
        GUISOFTKEY_INIT_DATA_T  softkey_data = {0};
        MMI_CONTROL_CREATE_T softkey_create = {0};
        MMI_CTRL_ID_T   softkey_ctrl_id = 0;
                              ...
                              )
{
    ...
    MMIPUB_SetWinTitleText(win_id, &title_str, is_fresh);
}
                              ...
                              )
{
    ...
    if ( PNULL != win_info_ptr->text_dialog_title.wstr_ptr )
    {
        SCI_FREE( win_info_ptr->text_dialog_title.wstr_ptr);
        win_info_ptr->text_dialog_title.wstr_ptr = PNULL;
        win_info_ptr->text_dialog_title.wstr_len = 0;
    }
    if ( PNULL != text_str_ptr)
    {
	    title_str_len = text_str_ptr->wstr_len;
		
        win_info_ptr->text_dialog_title.wstr_ptr = SCI_ALLOC_APP( ( title_str_len + 1 ) * sizeof(wchar) );
        
        SCI_MEMSET( win_info_ptr->text_dialog_title.wstr_ptr, 0, (( title_str_len + 1 ) * sizeof(wchar) ));
        
        MMIAPICOM_Wstrncpy( win_info_ptr->text_dialog_title.wstr_ptr, text_str_ptr->wstr_ptr, title_str_len);
        
        win_info_ptr->text_dialog_title.wstr_len = title_str_len;
    }
    GUITEXT_SetTextDialogTitle(ctrl_handle, win_info_ptr->text_dialog_title);
...
#if 0
	else if(!((MMIPUBWIN_DATE_EDITOR_DIALOG ==  win_info_ptr->type) ||())
	{
		MMI_HANDLE_T edit_ctrl_handle = MMK_GetCtrlHandleByWin( win_info_ptr->win_handle, MMIPUB_INPUTFIELD_CTRL_ID );
		MMI_HANDLE_T im_handle = GUIEDIT_GetIMHandle(edit_ctrl_handle);

		text_rect.top = win_info_ptr->bg_rect.top + CONFIRM_DIALOG_TEXT_TOP_MARGIN;

		if (PNULL != im_handle)
		{
			//text_rect.bottom = win_info_ptr->client_rect.top - win_info_ptr->pubwin_theme.layout.client_v_margin;
		}
	}
#endif
                                    ...
                                    )
{
    ...
    {    
        switch (dialog_type)
        {
        case DIALOG_WITH_CONFIRMATION: 
             win_id = MMIPUB_ALERT_TEXT_WIN_ID;
             break;

        case DIALOG_WITH_TEXT:         
             win_id = MMIPUB_ALERT_TEXT_WIN_ID;
             break;
             
        case DIALOG_WITH_ALARM:         
             win_id = MMIPUB_ALARM_WIN_ID;
             break;

        case DIALOG_WITH_PROGRESSBAR:         
             win_id =  MMIPUB_PROGRESS_WIN_ID;
             break;
             
        case DIALOG_WITH_INPUTFIELD:         
             win_id = MMIPUB_INPUTFIELD_WIN_ID;
             break;      
    ...
    if ( PNULL != win_info_ptr )
    {
        win_info_ptr->dialog_softkey.index = dialog_softkey->index;
        win_info_ptr->dialog_softkey.content = dialog_softkey->content;
        win_info_ptr->dialog_softkey.type = dialog_softkey->type;
        win_info_ptr->softkey_id[win_info_ptr->dialog_softkey.index] = win_info_ptr->dialog_softkey.content;
		
	    //Draw icon 
        if (DIALOG_SOFTKEY_IMAGE_ID == win_info_ptr->dialog_softkey.type)
        {
            GUIWIN_SeSoftkeytButtonIconId(win_info_ptr->win_handle,dialog_softkey->content,dialog_softkey->index,TRUE);
        }
		//Draw text 
        else if (DIALOG_SOFTKEY_TEXT_ID == win_info_ptr->dialog_softkey.type)
        {
            GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,dialog_softkey->content,dialog_softkey->index,TRUE);
        }              
    }
}
                            ...
                            )
{
    ...
    case MMIPUB_SOFTKEY_ONE:
        softkey_id[1] = STXT_EXIT; //STXT_EXIT;
		win_info_ptr->softkey_id[0] = PNULL;
		win_info_ptr->softkey_id[1] = STXT_EXIT;  
        win_info_ptr->softkey_id[2] = PNULL;
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,PNULL,0,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_EXIT,1,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,PNULL,2,TRUE);
        is_null_softkey = FALSE;
        break;

    case MMIPUB_SOFTKEY_OKEXIT:
        softkey_id[1] = TXT_COMMON_OK;
        softkey_id[2] = STXT_EXIT;
        win_info_ptr->softkey_id[0] = TXT_COMMON_OK;
        win_info_ptr->softkey_id[1] = PNULL;  
        win_info_ptr->softkey_id[2] = STXT_EXIT;
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,TXT_COMMON_OK,0,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,PNULL,1,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_EXIT,2,TRUE);
        is_null_softkey = FALSE;
        break;
    ...
    case MMIPUB_SOFTKEY_DIALOG_DISMISS:
        softkey_id[1] = TXT_COMMON_OK;               
		win_info_ptr->softkey_id[0] = PNULL;
		win_info_ptr->softkey_id[1] = TXT_COMMON_OK;  
        win_info_ptr->softkey_id[2] = PNULL;
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,PNULL,0,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,TXT_COMMON_OK,1,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,NULL,2,TRUE);
        is_null_softkey = FALSE;
        break;
        
    case MMIPUB_SOFTKEY_DIALOG_ALARM:
        softkey_id[0] = STXT_SOFTKEY_OPEN_LSK;
        softkey_id[1] = STXT_SOFTKEY_SLEEP_MK;
        softkey_id[2] = STXT_SOFTKEY_STOP_MK;
        win_info_ptr->softkey_id[0] = STXT_SOFTKEY_OPEN_LSK;
        win_info_ptr->softkey_id[1] = STXT_SOFTKEY_SLEEP_MK;
        win_info_ptr->softkey_id[2] = STXT_SOFTKEY_STOP_RSK;
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,win_info_ptr->softkey_id[0],0,TRUE);
       // GUIWIN_SetSoftKeyButtonfont(win_info_ptr->win_handle,0,&little_font);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,win_info_ptr->softkey_id[1],1,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,win_info_ptr->softkey_id[2],2,TRUE);
        //GUIWIN_SetSoftKeyButtonfont(win_info_ptr->win_handle,2,&little_font);
	    is_null_softkey = FALSE;
        break;

    case MMIPUB_SOFTKEY_DIALOG_OK_CLOSE:   
        softkey_id[0] = STXT_SOFTKEY_OK_MK;
		softkey_id[1] = PNULL;
		softkey_id[2] = STXT_CANCEL;
		win_info_ptr->softkey_id[0] = STXT_SOFTKEY_OK_MK;
		win_info_ptr->softkey_id[1] = PNULL;  
        win_info_ptr->softkey_id[2] = STXT_CANCEL;
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_SOFTKEY_OK_MK,0,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,PNULL,1,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_CANCEL,2,TRUE);
        break;

    case MMIPUB_SOFTKEY_DIALOG_OK:          
	   	softkey_id[1] = STXT_SOFTKEY_OK_MK;
	    win_info_ptr->softkey_id[0] = PNULL;
		win_info_ptr->softkey_id[1] = STXT_SOFTKEY_OK_MK;  
        win_info_ptr->softkey_id[2] = PNULL;
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,PNULL,0,TRUE);
    ...
    case MMIPUB_SOFTKEY_DIALOG_HIDE:
	   	softkey_id[0] = STXT_SOFTKEY_HIDE_MK;
	   	softkey_id[2] = STXT_CANCEL;
        win_info_ptr->softkey_id[0] = STXT_SOFTKEY_HIDE_MK;
        win_info_ptr->softkey_id[2] = STXT_CANCEL;
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_SOFTKEY_HIDE_MK,0,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,PNULL,1,TRUE);        
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_CANCEL,2,TRUE);
        break;
    ...
    case MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN:     
	case MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN_VMAIL:
	   	softkey_id[0] = STXT_CANCEL;
	   	softkey_id[1] = STXT_SOFTKEY_OK_MK;
	   	softkey_id[2] = TXT_DELETE;
        win_info_ptr->softkey_id[0] = STXT_CANCEL;
        win_info_ptr->softkey_id[1] = STXT_SOFTKEY_OK_MK;        
        win_info_ptr->softkey_id[2] = TXT_DELETE;
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_CANCEL,0,TRUE);
    ...
    case MMIPUB_SOFTKEY_DIALOG_TURNOFF_CLOSE:             
	   	softkey_id[0] = STXT_SOFTKEY_TURN_OFF_MK;
	   	softkey_id[2] = STXT_CANCEL;
        win_info_ptr->softkey_id[0] = STXT_SOFTKEY_TURN_OFF_MK;        
        win_info_ptr->softkey_id[2] = STXT_CANCEL;        
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_SOFTKEY_TURN_OFF_MK,0,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,1,PNULL,TRUE);
        GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STXT_CANCEL,2,TRUE);        
        break;
                             ...
                             )
{
    if ( PNULL != win_info_ptr )
    {
        ...
        if (IsTypeDialogWin(win_info_ptr->win_handle))
        {
            if ( (MMIPUB_SOFTKEY_DIALOG_NONE == win_info_ptr->softkey_style )
                || (MMIPUB_SOFTKEY_DIALOG_NONE_CANNOT_CLOSE == win_info_ptr->softkey_style) )
            {
                return FALSE;
            }
            else 
                         ...
                         )
{
    ...
    if ( PNULL != win_info_ptr )
    {
        ...
        /*if (TRUE == IsTypeDialogWin(win_info_ptr->win_handle) && TRUE == IsExistSoftkey(win_info_ptr))
        {
            //win_info_ptr->softkey_style = ConvertDialogSoftkey(win_info_ptr->softkey_style);
			//
            //SetDialogSoftkeyType(win_info_ptr->win_handle);   
            DisplayDialogSoftkey(softkey_id, (MMIPUB_INFO_T*)win_info_ptr);        
        }
        else */
        {

            if (win_info_ptr->softkey_style ==PNULL) 
            	{ 
                   SetDialogSoftkeyType(win_info_ptr->win_handle);   
            	}
			else if (win_info_ptr->softkey_style == MMIPUB_SOFTKEY_CUSTOMER)
				{
	                if ((win_info_ptr->softkey_id[0]==PNULL) &&  (win_info_ptr->softkey_id[1]==PNULL)
			           && (win_info_ptr->softkey_id[2]==PNULL))
			           {
    	 	             SetDialogSoftkeyType(win_info_ptr->win_handle);  
		               }
...
//fixed by hongjun.jia for bugzilla 2443/2444
    ...
    space_h = MMITHEME_GetSoftkeyHorSpace();
    space_w = MMITHEME_GetSoftkeyButtonWidth();

        rect_ptr[LEFT_BUTTON] = GUI_CreateRect(
                (uint16)(bg_rect.left),
                (uint16)(bg_rect.top),
                (uint16)(bg_rect.left + space_w - 1),
                (uint16)(bg_rect.bottom)
                );
        rect_ptr[MIDDLE_BUTTON] = GUI_CreateRect(
                (uint16)(bg_rect.left + space_h + space_w -1),
                (uint16)(bg_rect.top),
                (uint16)(bg_rect.right - space_h -space_w + 1),
                (uint16)(bg_rect.bottom)
                );
        rect_ptr[RIGHT_BUTTON] = GUI_CreateRect(
                    (uint16)(bg_rect.right - space_w + 1),
                    (uint16)(bg_rect.top),
                    (uint16)(bg_rect.right),
                    (uint16)(bg_rect.bottom)
                    );
    
#if defined MAINLCD_SIZE_128X160
	if ( 0 == win_info_ptr->softkey_id[MIDDLE_BUTTON] || TXT_NULL == win_info_ptr->softkey_id[MIDDLE_BUTTON])
    {
		rect_ptr[LEFT_BUTTON].right += (rect_ptr[MIDDLE_BUTTON].right - rect_ptr[MIDDLE_BUTTON].left) / 2 - 1; 
		rect_ptr[RIGHT_BUTTON].left -= (rect_ptr[MIDDLE_BUTTON].right - rect_ptr[MIDDLE_BUTTON].left) / 2 + 1; 
    }
	else
    {
		MMI_STRING_T     text_str = {0};
		MMI_GetLabelTextByLang(win_info_ptr->softkey_id[MIDDLE_BUTTON] , &text_str);
		button_width = GUI_CalculateStringPiexlNum(text_str.wstr_ptr, text_str.wstr_len, win_info_ptr->pubwin_theme.big_font, 0)+ MMI_SOFTKEY_TEXT_HOR_SPACE * 2;
		rect_ptr[MIDDLE_BUTTON] = GUI_GetCenterRect( rect_ptr[MIDDLE_BUTTON], button_width, button_height );
		rect_ptr[LEFT_BUTTON].right = rect_ptr[MIDDLE_BUTTON].left - 1; 
		rect_ptr[RIGHT_BUTTON].left = rect_ptr[MIDDLE_BUTTON].right + 1; 
    }
#endif
}
...
{     
    DIALOG_TYPE     dialog_type = DIALOG_WITH_TEXT;        //dialog type     
    MMI_STRING_T    text1_str = {0};
    MMI_STRING_T    text2_str = {0};    
    ...
    OpenAlertWin( MMK_GetFirstAppletHandle(), time_period_ptr,&text1_str,&text2_str,
        IMAGE_NULL,win_id_ptr,win_priority_ptr,softkey_style,handle_func, 0 );
}
...
{     
    ...
    OpenAlertWin( applet_handle, time_period_ptr,&text1_str,&text2_str,
        IMAGE_NULL,win_id_ptr,win_priority_ptr,softkey_style,handle_func, user_data );
}
...
/*****************************************************************************/
//  Description : Set dialog softkey type (set to default)
//  Global resource dependence : 
//  Author: 
//  Note:
/*****************************************************************************/
    ...
    if (PNULL == win_info_ptr)
    {
        ...
        case MMIPUBWIN_CONFIRM_DIALOG:      
			if (alert_win == TRUE)
				...
				{
                 win_info_ptr->softkey_style = MMIPUB_SOFTKEY_DIALOG_OK_CLOSE;
				}
            break;
            
        case MMIPUBWIN_TEXT_DIALOG:         
			if ( !(win_info_ptr->softkey_style < MMIPUB_SOFTKEY_MAX) )
				win_info_ptr->softkey_style = MMIPUB_SOFTKEY_DIALOG_OK;//Candice
            break;

         case MMIPUBWIN_ALARM_DIALOG: 
            win_info_ptr->softkey_style = MMIPUB_SOFTKEY_DIALOG_ALARM;        
            break;

         case MMIPUBWIN_PROGRESSBAR_DIALOG: 
            win_info_ptr->softkey_style = MMIPUB_SOFTKEY_DIALOG_CLOSE;                
            break;

         case MMIPUBWIN_INPUTFIELD_DIALOG:         
         case MMIPUBWIN_PASSWORD_DIALOG:
         case MMIPUBWIN_DATE_EDITOR_DIALOG:   
            win_info_ptr->softkey_style = MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN;        
            break;
...
//    Note:
/*****************************************************************************/
LOCAL void SetDialogWinType(DIALOG_TYPE  dialog_type)
{
    switch (dialog_type)
    {
    case DIALOG_WITH_CONFIRMATION: 
         dialog_win_type = MMIPUBWIN_CONFIRM_DIALOG;
         break;

    case DIALOG_WITH_TEXT:         
         dialog_win_type = MMIPUBWIN_TEXT_DIALOG;
         break;
         
    case DIALOG_WITH_ALARM:         
         dialog_win_type = MMIPUBWIN_ALARM_DIALOG;
         break;

    case DIALOG_WITH_PROGRESSBAR:         
         dialog_win_type =  MMIPUBWIN_PROGRESSBAR_DIALOG;
         break;
         
    case DIALOG_WITH_INPUTFIELD:         
         dialog_win_type = MMIPUBWIN_INPUTFIELD_DIALOG;
         break;      

     case DIALOG_WITH_PASSWORD:        
        dialog_win_type = MMIPUBWIN_PASSWORD_DIALOG;
        break;

     case DIALOG_WITH_DATE_EDITOR:        
        dialog_win_type = MMIPUBWIN_DATE_EDITOR_DIALOG;        
        break;
...
/*****************************************************************************/
//  Description : Open password dialog window with call barring input field 
//  Global resource dependence : 
//  Note:
/*****************************************************************************/
PUBLIC void MMIPUB_OpenPasswordDialogInputFieldWinByTextPtr(
                        MMI_STRING_T                *text1_str_ptr,     //text 1
                        MMI_WIN_ID_T                *win_id_ptr,        //win id,default is MMIPUB_ALERT_WIN_ID
                        MMI_WIN_PRIORITY_E          *win_priority_ptr,  //win priority,default is WIN_ONE_LEVEL
                        MMIPUB_SOFTKEY_STYLE_E      softkey_style,      //softkey style
...
{
    BOOLEAN             result = FALSE;
    MMI_WIN_ID_T        win_id = MMIPUB_INPUTFIELD_WIN_ID;
    MMIPUB_INFO_T       *win_info_ptr = PNULL;
	MMI_WIN_PRIORITY_E  win_priority = WIN_ONE_LEVEL;
    PROCESSMSG_FUNC     handle_function = (PROCESSMSG_FUNC)MMIPUB_HandleDialogInputFieldWinMsg;
    MMI_WINDOW_CREATE_T win_create = {0};
    MMI_HANDLE_T        win_handle = 0;
    uint16                  str_max_len = 4;
    MMI_CTRL_ID_T           ctrl_id = 0;
    GUIEDIT_INIT_DATA_T     init_data = {0};
    MMI_CONTROL_CREATE_T    ctrl_create = {0};
	uint32                  user_data = 0;

	SetDialogWinType(DIALOG_WITH_INPUTFIELD);
	win_handle = MMK_GetFirstAppletHandle();
    //set window id
    if (PNULL != win_id_ptr)
    {
        win_id = *win_id_ptr;
    ...
    //judge window is already open
    if ( !MMK_IsOpenWin( MMK_GetWinHandle( win_handle, win_id ) ) )
    {
        ...
        win_info_ptr->win_id = win_id;        
        win_info_ptr->user_data = user_data;
        win_info_ptr->type = MMIPUBWIN_INPUTFIELD_DIALOG;
        
        //get pubwin theme
        MMITHEME_GetPubTheme( win_info_ptr->type, &win_info_ptr->pubwin_theme );
        
        win_create.applet_handle = win_handle;
        win_create.win_id = win_id;
        win_create.func = HandleDefaultWinMsg;
        win_info_ptr->user_func = handle_function;
        win_create.win_priority = WIN_ONE_LEVEL;
        ...
        {
            init_data.both_rect   = MMITHEME_GetWinClientBothRect(win_info_ptr->win_handle);
            init_data.type        = edit_type;
            init_data.str_max_len = str_max_len;
            
            //set control param
            ctrl_create.ctrl_id           = MMIPUB_INPUTFIELD_CTRL_ID;
            ctrl_create.guid              = SPRD_GUI_EDITBOX_ID;
            ctrl_create.init_data_ptr     = &init_data;
            ctrl_create.parent_win_handle = win_info_ptr->win_handle;

            //creat control and set active
            MMK_CreateControl(&ctrl_create);            
    		MMK_SetAtvCtrl(win_info_ptr->win_handle, MMIPUB_INPUTFIELD_CTRL_ID);
        }
                        ...
                        )
{
    ...
    if ( !MMK_IsOpenWin( MMK_GetWinHandle( applet_handle, win_id ) ) )
    {
        ...
        switch (dialog_win_type)
        {
            case MMIPUBWIN_INPUTFIELD_DIALOG:             
                win_info_ptr->type = MMIPUBWIN_INPUTFIELD_DIALOG;
                win_info_ptr->allow_inputmethod = GUIIM_TYPE_SET_ALL;
                win_info_ptr->default_inputmethod = GUIIM_TYPE_SMART;                                  
                win_info_ptr->tag_inputmethod = GUIIM_TAG_NONE;     
                break;   
                
            case MMIPUBWIN_PASSWORD_DIALOG:
                win_info_ptr->type = MMIPUBWIN_PASSWORD_DIALOG;
                win_info_ptr->allow_inputmethod = GUIIM_TYPE_SET_ALL;
                win_info_ptr->default_inputmethod = GUIIM_TYPE_SMART;                                  
                win_info_ptr->tag_inputmethod = GUIIM_TAG_NONE;     
                break;
            ...
            case MMIPUBWIN_DATE_EDITOR_DIALOG:                 
                win_info_ptr->type = MMIPUBWIN_DATE_EDITOR_DIALOG;
                win_info_ptr->allow_inputmethod = GUIIM_TYPE_DIGITAL;
                win_info_ptr->default_inputmethod = GUIIM_TYPE_DIGITAL;                                  
                win_info_ptr->tag_inputmethod = GUIIM_TAG_PURE_NUM;     
                break;
                
            default:         
                win_info_ptr->type = NULL;
                return;
        }
                
         if (MMIPUB_SOFTKEY_CUSTOMER==win_info_ptr->softkey_style)
         	{
         //   win_info_ptr->softkey_style = MMIPUB_SOFTKEY_DIALOG_OK_CLOSE; 
         	}
        ...
        //Create input field control              
        {
            ...
            switch (win_info_ptr->type) 
            {
                ...
                case MMIPUBWIN_INPUTFIELD_DIALOG:
                    init_data.type        = GUIEDIT_TYPE_TEXT;
                        //GUIEDIT_TYPE_LIST;//GUIEDIT_TYPE_PHONENUM;//GUIEDIT_TYPE_TEXT;
                    break;
            ...
            //creat control and set active
            MMK_CreateControl(&ctrl_create);        
    		//MMK_SetAtvCtrl(win_info_ptr->win_handle, MMK_GetCtrlHandleByWin(win_info_ptr->win_handle,MMIPUB_INPUTFIELD_CTRL_ID));            
        }
...
{   
    ...
    win_info_ptr->pubwin_theme.bg_alpha = alpha_value;
}
...
{   
    ...
    switch (bg_style)
    {    
        case DIALOG_HALF_BG_SCREEN:            
            win_info_ptr->pubwin_theme.bg_alpha = 0;
            break;
        
        case DIALOG_FULL_BG_SCREEN:            
            win_info_ptr->pubwin_theme.bg_alpha = 255;
            break;
            
        case DIALOG_MASK_BG_SCREEN:            
            win_info_ptr->pubwin_theme.bg_alpha = 120;
            break;
                            ...
                            )
{
    ...
    if (MMIPUBWIN_DATE_EDITOR_DIALOG == win_info_ptr->type)
        return;

    win_info_ptr->allow_inputmethod = allow_inputmethod;
    win_info_ptr->default_inputmethod = default_inputmethod;                                
    win_info_ptr->tag_inputmethod = tag_inputmethod;                                
}
                            ...
                            )
{
    ...
    switch(editor_type){
        case DIALOG_TEXT_EDITOR:
            win_info_ptr->allow_inputmethod = GUIIM_TYPE_SET_ALL;
            win_info_ptr->default_inputmethod = GUIIM_TYPE_SMART;                                  
            win_info_ptr->tag_inputmethod = GUIIM_TAG_NONE;            
            break;
            
        case DIALOG_PASSWORD_EDITOR:                            
            win_info_ptr->allow_inputmethod = GUIIM_TYPE_ABC | GUIIM_TYPE_DIGITAL;
            win_info_ptr->default_inputmethod = GUIIM_TYPE_ABC;
            win_info_ptr->tag_inputmethod = GUIIM_TAG_NONE;                
            break;

        case DIALOG_TEL_EDITOR:            
            win_info_ptr->allow_inputmethod = GUIIM_TYPE_DIGITAL;
            win_info_ptr->default_inputmethod = GUIIM_TYPE_DIGITAL;
            win_info_ptr->tag_inputmethod = GUIIM_TAG_PHONE_NUM;
            break;

        case DIALOG_EMAIL_EDITOR:
            win_info_ptr->allow_inputmethod = GUIIM_TYPE_ENGLISH | GUIIM_TYPE_ABC | GUIIM_TYPE_DIGITAL;
            win_info_ptr->default_inputmethod = GUIIM_TYPE_ABC;
            win_info_ptr->tag_inputmethod = GUIIM_TAG_EMAIL;
            break;
                                          ...
                                          )
{
    ...
    GUI_RECT_T        text_rect    = {0};
    MMI_HANDLE_T      text_ctrl_handle = NULL;
    MMI_HANDLE_T      edit_ctrl_handle = NULL;
    uint16           line_num = 0;    
    uint16           line_height = 0;
    uint16           total_height = 0;        
    BOOLEAN          dialog_create_ok = FALSE;
    MMI_STRING_T text_str = {0};
    GUI_RECT_T       bg_rect = {0};   
    ...
    lcd_dev_info_ptr = GetPubWinLayer(win_info_ptr);
    text_ctrl_handle = MMK_GetCtrlHandleByWin( win_info_ptr->win_handle, MMIPUB_TEXT_CTRL_ID );    
    edit_ctrl_handle = MMK_GetCtrlHandleByWin( win_info_ptr->win_handle, MMIPUB_INPUTFIELD_CTRL_ID );
    
SCI_TRACE_LOW("[mmmmmm] MMIPUB_HandleDialogInputFieldWinMsg line[%d] file[%s] func[%s] msg_id=0x%x", __LINE__, __FILE__, __FUNCTION__, msg_id);
    switch(msg_id)
    {
      case MSG_OPEN_WINDOW:    
        {
            GUIEDIT_SetIm(MMIPUB_INPUTFIELD_CTRL_ID,win_info_ptr->allow_inputmethod,win_info_ptr->default_inputmethod);                               
            GUIEDIT_SetImTag(MMIPUB_INPUTFIELD_CTRL_ID, win_info_ptr->tag_inputmethod);

            if (MMIPUBWIN_INPUTFIELD_DIALOG == win_info_ptr->type ||
                MMIPUBWIN_PASSWORD_DIALOG == win_info_ptr->type
                )
            {
                BOOLEAN isDateEditor = MMIPUBWIN_DATE_EDITOR_DIALOG ==  win_info_ptr->type;
                if (!isDateEditor)
                {
                    GUIEDIT_GetString(edit_ctrl_handle, &text_str);
                }
                if (0 == text_str.wstr_len)
                {
                    if (!isDateEditor)
                    {
						// GUIWIN_SetSoftkeyBtnState(win_info_ptr->win_handle,1,TRUE,FALSE);
                        //GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,PNULL,2,TRUE);
						// GUIEDIT_SetSoftkey(MMIPUB_INPUTFIELD_CTRL_ID, 1, 1, TXT_NULL, STXT_SOFTKEY_OK_MK, PNULL);
						if (win_info_ptr->softkey_style == MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN)
                       	{
							GUIEDIT_SetSoftkey(MMIPUB_INPUTFIELD_CTRL_ID, 1, 1, TXT_NULL, STXT_SOFTKEY_OK_MK, PNULL);
							GUIEDIT_SetSoftkey(MMIPUB_INPUTFIELD_CTRL_ID, 2, 1, TXT_NULL, TXT_DELETE, PNULL);
						}
						else if (win_info_ptr->softkey_style == MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN_VMAIL)
						{
							GUIEDIT_SetSoftkey(MMIPUB_INPUTFIELD_CTRL_ID, 2, 1, TXT_NULL, TXT_DELETE, PNULL);
                       	}
						else
      ...
      case MSG_FULL_PAINT:  
            {
                ...
                //Set the type of input editbox 
                //date editor 
                if (MMIPUBWIN_DATE_EDITOR_DIALOG ==  win_info_ptr->type)
                {
SCI_TRACE_LOW("[mmmmmm] MMIPUBWIN_DATE_EDITOR_DIALOG line[%d] file[%s] func[%s]", __LINE__, __FILE__, __FUNCTION__);
                    edit_both_rect.v_rect.left = 0;
                    edit_both_rect.v_rect.right = logic_width;

                    if (TRUE == IsExistSoftkey(win_info_ptr)) 
                    {
            		    edit_both_rect.v_rect.bottom = logic_height - SOFTKEY_BAR_HEIGHT - 6;
                    }
                    else 
                    {
                        edit_both_rect.v_rect.bottom = logic_height;
                    }
                    
            		edit_both_rect.v_rect.top = edit_both_rect.v_rect.bottom - 64;
                    GUIAPICTRL_SetBothRect(edit_ctrl_handle, &edit_both_rect);
                    
    				CTRLDATEEDIT_SetDateEditorDisplayType(edit_ctrl_handle,GUIEDIT_DATE_DIALOG_DISPLAYTYPE);
                    CTRLDATEEDIT_SetTitle(edit_ctrl_handle, &win_info_ptr->string[0]);                
                    
                    win_rect.top = edit_both_rect.v_rect.top;                
                }
                //text input field / password input field 
                else
                {
				  MMI_HANDLE_T im_handle = GUIEDIT_GetIMHandle(edit_ctrl_handle);
SCI_TRACE_LOW("[mmmmmm] line[%d] file[%s] func[%s]", __LINE__, __FILE__, __FUNCTION__);
                   
				  win_info_ptr->is_layout = 0;
					MMIPUBLayout( win_info_ptr );

				    edit_bg.bg_type = GUI_BG_COLOR;
                    edit_bg.color   = win_info_ptr->pubwin_theme.dialog_bg_color;
                    
                    edit_bg.shape   = GUI_SHAPE_RECT;        
                    GUIEDIT_SetBg(edit_ctrl_handle,&edit_bg);
                    GUIEDIT_SetThemeColor(edit_ctrl_handle,MMI_WHITE_COLOR);
            		GUIEDIT_SetStyle(edit_ctrl_handle,GUIEDIT_STYLE_SINGLE_UP);   //GUIEDIT_STYLE_MULTI_DYN_UP 
                   
                    //Display input editbox       
                    edit_both_rect.v_rect.left = win_info_ptr->pubwin_theme.layout.client_h_margin;
                    edit_both_rect.v_rect.right = logic_width-win_info_ptr->pubwin_theme.layout.client_h_margin;
                    edit_both_rect.h_rect.left = win_info_ptr->pubwin_theme.layout.client_h_margin;
                    edit_both_rect.h_rect.right = logic_width-win_info_ptr->pubwin_theme.layout.client_h_margin;

				  //edit_both_rect.v_rect.top = win_info_ptr->client_rect.top;//dialog_edit_height;
				  edit_both_rect.v_rect.top = win_info_ptr->client_rect.bottom;//dialog_edit_height;
				  edit_both_rect.v_rect.bottom = edit_both_rect.v_rect.top+CONFIRM_DIALOG_CLIENT_HEIGHT;//logic_height-250;                    

                    GUIAPICTRL_SetBothRect(edit_ctrl_handle, &edit_both_rect);
            MMIPUBSetButton( win_info_ptr, FALSE );
				  SetPubWinTextBoxRect( win_info_ptr );  // set rect of title text
        ...
        case MSG_CTL_OK:
            {
                if ((MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN == win_info_ptr->softkey_style) || (MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN_VMAIL== win_info_ptr->softkey_style))
                {
                    if (MMK_IsOpenWin(win_info_ptr->win_id))
                    {
        ...
        case MSG_NOTIFY_CANCEL:
            {
                if ((MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN == win_info_ptr->softkey_style) || (MMIPUB_SOFTKEY_DIALOG_CLOSE_OK_RETURN_VMAIL== win_info_ptr->softkey_style))
                {
                    GUIEDIT_GetString(edit_ctrl_handle, &text_str);                
                    if ((0 != text_str.wstr_len) &&
                       (TRUE == MMK_IsOpenWin(win_info_ptr->win_id))
                       )
                    {
                        MMK_CloseWin(win_info_ptr->win_id);
                    }            
...
SCI_TRACE_LOW("[mmmmmm] line[%d] file[%s] func[%s]", __LINE__, __FILE__, __FUNCTION__);
                ...
                {
                    GUIEDIT_GetString(edit_ctrl_handle, &text_str);
SCI_TRACE_LOW("[mmmmmm] line[%d] file[%s] func[%s] wstr_len=%d", __LINE__, __FILE__, __FUNCTION__, text_str.wstr_len);
                    ...
                    {                       
        	            //GUIWIN_SetSoftkeyBtnState(win_info_ptr->win_handle,1,FALSE,FALSE);            
        	            //GUIWIN_SeSoftkeytButtonTextId(win_info_ptr->win_handle,STR_LIST_DELETE,2,TRUE);            
        			}
                    //MMK_SendMsg(win_info_ptr->win_handle, MSG_FULL_PAINT, PNULL);     
                }
                break;
		...
		case MSG_CLOSE_WINDOW:
			if (MMIPUBWIN_DATE_EDITOR_DIALOG ==  win_info_ptr->type)
		    {
				CTRLDATEEDIT_FreeTitle(edit_ctrl_handle,PNULL);
		    }
			break;
...
{
    DIALOG_TYPE dialog_type = DIALOG_WITH_INPUTFIELD;        //dialog type 
    
    SetDialogWinType(dialog_type);
    OpenDialogInputFieldWin(MMK_GetFirstAppletHandle(), text1_ptr, win_id_ptr,win_priority_ptr,softkey_style,handle_func,0);
}
PUBLIC void MMIPUB_OpenDialogInputFieldWinByTextPtrLen(
                                  MMI_STRING_T              *text1_ptr,         //text 1
                                  MMI_WIN_ID_T              *win_id_ptr,        //win id,default is MMIPUB_ALERT_WIN_ID
                                  MMI_WIN_PRIORITY_E        *win_priority_ptr,  //win priority,default is WIN_ONE_LEVEL
                                  MMIPUB_SOFTKEY_STYLE_E    softkey_style,      //softkey style
                                  ...
                                  )
{
    ...
    SetDialogWinType(dialog_type);
    OpenDialogInputFieldWinLen(MMK_GetFirstAppletHandle(), text1_ptr, win_id_ptr,win_priority_ptr,softkey_style,handle_func,str_max_len,0);
}
                                  ...
                                  )
{
    ...
    SetDialogWinType(dialog_type);
    OpenDialogInputFieldWin(applet_handle, text1_ptr, win_id_ptr,win_priority_ptr,softkey_style,handle_func,user_data);
}
...
{
    DIALOG_TYPE dialog_type = DIALOG_WITH_PASSWORD;        //dialog type 
...
{
    DIALOG_TYPE dialog_type = DIALOG_WITH_DATE_EDITOR;        //dialog type 
                                             ...
                                             )
{
    ...
    return MMK_GetActiveCtrlId(win_info_ptr->win_handle);    
}
                                ...
                                )
{
    ...
    case MMIPUB_SOFTKEY_DIALOG_NONE:
    case MMIPUB_SOFTKEY_DIALOG_OK:
    case MMIPUB_SOFTKEY_DIALOG_DISMISS:
    case MMIPUB_SOFTKEY_DIALOG_ALARM:
    case MMIPUB_SOFTKEY_DIALOG_CLOSE:
