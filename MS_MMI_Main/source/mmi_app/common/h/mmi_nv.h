...
#endif        
	MMINV_CONNECTION_VODAFONE_SWITCH,
#endif
...
// end main menu nv data, 请不要在这之间加入其他数据
    ...
    MMI_NV_THEME_ACCENT_ID, //用于记录当前主题accent值的NV项
      MMINV_PHONE_FIRST_POWER_ON,
      MMINV_PHONE_FIRST_VODAFONE_SIM,  //用于开机vodafone sim卡选择信息记录
    MMI_NV_INSERT_SIM_TIME,
#ifdef UNISOC_MMI_SIM_LOCK_SUPPORT      
    MMI_NV_SIM_LOCK_INFO_ID,//用于记录当前sim卡锁卡信息
#endif
    MMINV_MAINMENU_MENU_ID_ARRAY,
    MMIMV_MAINMENU_IS_APPEND_PREINSTALL_ITEM,
    MMINV_IDLE_PUSH_NOTIFICATION,
    MMINV_CAMERAROLL_SYNC_STATE,
    MMINV_SETTING_PREFER_DISK,//prefer disk
    MMINV_CAMERAROLL_IS_CONNECT_ONEDRIVE,
    MMINV_NETWORK_ALL_NAME,
    MMINV_CAMERAROLL_ONDRIVE_INFO,
#ifdef MMI_GPRS_SUPPORT//dong.chunguang add for apn requirement
    MMINV_CONNECTION_SIM1_APN_DEFAULT,
    MMINV_CONNECTION_SIM2_APN_DEFAULT,
    MMINV_CONNECTION_VODAFONE1_APN_DEFAULT,
    MMINV_CONNECTION_VODAFONE2_APN_DEFAULT,
#endif
#ifdef MMI_BT_PBAP_SUPPORT
    MMINV_BT_ADDRESS_INFO,    //connected device address
#endif
    MMINV_CONNECTION_MVNO_CHECK,	
    MMI_NV_MAX_ITEM_NUM,
