...
{
	AddVelocityItemByTracker(tracker_ptr, x, y, MMI_TP_NONE);
}
...
{
    AddVelocityItemByTracker(&s_velocity_tracker, x, y, tp_action);
}
...
{
    AddVelocityItemByTracker(velocity_tracker_ptr, x, y, tp_action);
}
/*****************************************************************************/
// 	Description : 重置速度点
//	Global resource dependence :
    ...
    while(tmpptr != &dbg_head)
    {
        ...
        if ( flag != MEM_DEBUG_END_FLAG )
        {
            SCI_PAssert("MMI_CHECK_MEMINFO", file_name_ptr, line, 
                SCI_SaveAssertInfo("Memory 0x%x malloced by %s on %d line, size is %d, has been destroied!",
                tmpptr,
                tmpptr->file_name,
                tmpptr->line,
                tmpptr->size ) ); /*assert verified*/
        }
