...
PUBLIC BOOLEAN MMK_IsControlNeedTopmostShow(MMI_HANDLE_T win_handle)
{
	uint32 guid = MMK_GetGuidByCtrlNode(MMK_GetCtrlNodeByHandle (win_handle));

	if(guid == SPRD_GUI_IM_ID)
		return TRUE;
	else
		return FALSE;

    //return (SPRD_GUI_IM_ID == MMK_GetGuidByCtrlNode(MMK_GetCtrlNodeByHandle (win_handle)));
}
...
)
{
    ...
    if (PNULL == ctrl_node_ptr)
    {
    	SCI_TRACE_LOW("ControlTreeNodeDestruct: ctrl_node_ptr NULL");
        return FALSE;
    }
    ...
    {
        SCI_TRACE_LOW("ControlTreeNodeDestruct: tree_node_ptr is already PNULL");
    }
