...
/*MMI_DEVICELOCK_SUPPORT*/
/*****************************************************************************/
// 	Description : return to the devicelock
//	Global resource dependence : 
//  Author:Great.Tian
//	Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMK_ReturnToDevicelockWin(void)
{
#if 0
	extern MMI_WIN_ID_T g_devicelock_win;
	MMI_APPLET_NODE_T*   applet_node_ptr   = PNULL;
	MMI_HANDLE_T         devicelock_handle = MMK_GetWinHandle( MMK_GetFirstAppletHandle(), g_devicelock_win );
	
	applet_node_ptr = GetAppletNodePtr( MMK_GetAppletHandleByWin(MMK_ConvertIdToHandle(devicelock_handle)));
	MMK_RemoveToAppointTreeNode(&applet_node_ptr->win_tree_root_ptr, MMK_WindowTreeNodeDestruct,MMK_GetWinTreeNode(devicelock_handle));
	ReturnFirstApplet();
	UpdateAppletSystem();
#endif
	return TRUE;
...
/*****************************************************************************/
//  Description : return to the mainmenu window
//  Global resource dependence : 
//  Author: 
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMK_ReturnMainmenuWin(
                                 void
                                 )
{
    extern MMI_WIN_ID_T  g_mainmenu_win;
    MMI_HANDLE_T         mainmenu_handle = MMK_GetWinHandle( MMK_GetFirstAppletHandle(), g_mainmenu_win );
    
    ReturnAppointedWin( mainmenu_handle );
    ReturnFirstApplet();
...
/*****************************************************************************/
// 	Description : update color theme 
//	Global resource dependence : 
//    Author:Pan.Yang
//	Note:
/*****************************************************************************/
PUBLIC void MMK_UpdateColorTheme()
{
    MMI_APPLET_NODE_T*   applet_node_ptr   = PNULL;
    MMI_TREE_TRAVEL_INFO_T* win_tree_travel_ptr = PNULL;
    MMI_TREE_TRAVEL_INFO_T* ctrl_tree_travel_ptr = PNULL;
    ...
    applet_node_ptr = GetAppletNodePtr(MMK_GetFirstAppletHandle());
    if (PNULL == applet_node_ptr)
    {
        SCI_TRACE_LOW("MMK_UpdateColorTheme PNULL == applet_node_ptr");
        return;
    }

    win_tree_travel_ptr = MMK_CreateTreeTravelInfo();
    ctrl_tree_travel_ptr = MMK_CreateTreeTravelInfo();
    if ((PNULL != win_tree_travel_ptr) && (PNULL != ctrl_tree_travel_ptr))
    {
        MMK_TravelTree(win_tree_travel_ptr, applet_node_ptr->win_tree_root_ptr, MMI_TREE_TRAVEL_ORDER | MMI_TREE_TRAVEL_CHILD);

        for (i = 0; i < win_tree_travel_ptr->total_num; i++)
        {
            win_node_ptr = MMK_GetWinNodeByHandle( win_tree_travel_ptr->data_ptr[i] );
            if (PNULL != win_node_ptr)
            {
                ctrl_tree_root_ptr = MMK_GetCtrlTreeByWinNode(win_node_ptr);
                if (PNULL != ctrl_tree_root_ptr)
                {
                    MMK_TravelTree(ctrl_tree_travel_ptr, ctrl_tree_root_ptr, MMI_TREE_TRAVEL_ORDER | MMI_TREE_TRAVEL_CHILD);
                    for (j = 0; j < ctrl_tree_travel_ptr->total_num; j++)
                    {
                        ctrl_ptr = MMK_GetCtrlPtr(ctrl_tree_travel_ptr->data_ptr[j]);
                        if (PNULL != ctrl_ptr)
                        {
                            IGUICTRL_HandleEvent ((IGUICTRL_T*)ctrl_ptr, MSG_CTL_UPDATE_COLOR_THEME, PNULL);
                        }
                    }//travel ctrl tree
                }
            }
    ...
    {
       SCI_TRACE_LOW("MMK_UpdateColorTheme PNULL == travel_ptr");
    }

    if (PNULL != win_tree_travel_ptr)
    {
        MMK_DestroyTreeTravelInfo(win_tree_travel_ptr);
    }

    if (PNULL != ctrl_tree_travel_ptr)
    {
        MMK_DestroyTreeTravelInfo(ctrl_tree_travel_ptr);
    }
