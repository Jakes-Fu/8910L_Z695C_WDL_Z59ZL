...
{	
    ...
    //Progressbar for dialog     
    MMI_CTRL_ID_T				progbar_ctrl_id;
...
LOCAL MMI_WINDOW_HIST s_mmi_window_hist_tag={0};
                           ...
                           )
{
    ...
    BOOLEAN is_tool_win = FALSE;

    SCI_ASSERT( PNULL != win_node_ptr );/*assert verified*/

    is_tool_win = MMK_GetWinDisplayStyleState( win_handle, WS_TOOL_WIN );
    ...
    else
    {
        ...
        if ( (!is_tool_win) && win_node_ptr->zorder_node.is_got_topmost )
        {
...
PUBLIC BOOLEAN MMK_ZorderSystemUpdate( void )
{
    ...
    MMI_WINDOW_NODE_T* hd = NULL;

    SCI_ASSERT(PNULL != zorder_system_ptr);/*assert verified*/

    hd = win_node_ptr;

    while( MMK_GetWinDisplayStyleState(focus_handle, WS_TOOL_WIN ))
    {
        win_node_ptr = win_node_ptr->zorder_node.prev_ptr->win_node_ptr;
        focus_handle = win_node_ptr->base_node.handle;

	 if (win_node_ptr == hd) break;
    }
...
/*****************************************************************************/
//     Description : set window has statusbar
//    Global resource dependence : 
//  Author:
//    Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMK_SetWinHasStb( 
                                   MMI_HANDLE_T         win_handle,
                                   BOOLEAN              is_true
                                   )
{
    ...
    if( PNULL != ( win_node_ptr = GetWindowNodePtr( win_handle ) ) )
    {
        win_node_ptr->win_ptr->is_has_statusbar = is_true;
                                         ...
                                         )
{
    ...
    case MMI_TREE_NODE_FIND_ID:
        {          
            if((GetWindowNodePtr( handle )!= PNULL) && condition2 == GetWindowNodePtr( handle )->win_id )
            {
                result = TRUE;
            }
        }
                            ...
                            )
{
    ...
    if ( PNULL != win_node_ptr )
    {
        ...
        s_mmi_window_hist_tag.handle=win_handle;
        s_mmi_window_hist_tag.win_id=win_node_ptr->win_id;
        s_mmi_window_hist_tag.msg_id=msg_id;
        SCI_TRACE_ID(TRACE_TOOL_CONVERT,MMK_WINDOW_4253_112_2_18_3_23_31_54,(uint8*)"ddsd", win_handle, win_node_ptr->win_id, win_id_name?win_id_name:"NULL", msg_id );
#endif
                                             ...
                                             )
{
    ...
    if( PNULL != win_node_ptr )
    {
        ...
        else
        {
            ...
            if ( MSG_KEYDOWN <= msg_id && MSG_KEYDOWN_MAX_NUM >= msg_id || MSG_KEYREPEAT_DOWN == msg_id)
            {
                ctrl_node_ptr = win_node_ptr->active_control_ptr;
                              ...
                              )
{
    BOOLEAN             result       = TRUE;
    MMI_WINDOW_NODE_T   *win_node_ptr = GetWindowNodePtr( win_handle );
	SCI_TRACE_LOW("MMK_ProcSpecialWinMsg\n");
    ...
    if( PNULL != win_node_ptr )
    {
		...
		SCI_TRACE_LOW("MMK_ProcSpecialWinMsg msg_id=%d\n",msg_id);
        switch(msg_id)
        {
        case MSG_CLOSE_WINDOW:
			SCI_TRACE_LOW("MMK_ProcSpecialWinMsg MSG_CLOSE_WINDOW\n");
...
/*****************************************************************************/
// 	Description : set the window progbar ctrl id for dialog
//	Global resource dependence : 
//  Author:Great.Tian
//	Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMK_SetWinDialogProgbarCtrlId(
									   MMI_HANDLE_T		win_handle,
									   MMI_CTRL_ID_T	dialog_progbar_ctrl_id
									   )
{
    MMI_WINDOW_T*		win_ptr = PNULL;
    BOOLEAN				result  = FALSE;
	...
	if (NULL != win_ptr)
	{
		win_ptr->progbar_ctrl_id = dialog_progbar_ctrl_id;
	}
                                     ...
                                     )
{
    ...
    if (root_pptr != PNULL && (*root_pptr) == node_ptr )
    {
        //only one node
    ...
    else
    {
        ...
        if ( (!node_ptr->is_hide) && root_pptr != PNULL &&(*root_pptr) != PNULL
			&& MMK_GetWinDisplayStyleState((*root_pptr)->win_node_ptr->base_node.handle, WS_TOOL_WIN ))
        {
            SetZorderSystemFocusType(MMI_ZORDER_SYSTEM_FOCUS_NORMAL);
        }
    }
...
//	Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMK_WinResumeWindowFocus(
                           MMI_HANDLE_T win_handle
                           )
{
    BOOLEAN            result             = FALSE;
