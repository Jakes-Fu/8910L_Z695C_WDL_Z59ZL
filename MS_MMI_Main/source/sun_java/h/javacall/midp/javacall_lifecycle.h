 ...
 *
 */
void javanotify_install_midlet_from_browser(const javacall_utf16* browserUrl,
                                               int browserUrlLen, const javacall_utf16* localResPath, int localResPathLen,
                                               javacall_bool silentInstall, javacall_bool forceUpdate);

void javanotify_install_midlet_from_filesystem(const javacall_utf16* jadFilePath,
                                               int jadFilePathLen, javacall_bool silentInstall, javacall_bool forceUpdate);
 ...
 */
typedef enum {
    /** VM is starting, but MIDlet is not created */
    JAVACALL_LIFECYCLE_MIDLET_CREATING           =9,
    ...
    /** MIDlet pausing, pausing in progress */
    JAVACALL_LIFECYCLE_MIDLET_PAUSING           =19,
} javacall_lifecycle_state;
typedef enum {
    JAVACALL_LIFECYCLE_MIDLET_BACKGROUND        =101,
    JAVACALL_LIFECYCLE_MIDLET_FOREGROUND        =102,
    JAVACALL_LIFECYCLE_MIDLET_SWITCHING_TO_BACKGROUND         =103,
    JAVACALL_LIFECYCLE_MIDLET_SWITCHING_TO_FOREGROUND         =104,
} javacall_lifecycle_ground_state;
...
struct javacall_lifecycle_push_entry_info {
    char* suiteId;
    char* className;
};

typedef enum {
    /** Request to get push entry */
    JAVACALL_OP_GET_PUSH_ENTRY
} javacall_operation;
...
javacall_result javacall_get_string_from_local_file_browser(javacall_utf16* fileUrl, int* fileUrlLength);

javacall_result javacall_lifecycle_launchNativeWebBrowser(/*IN*/javacall_utf16* url, /*IN*/int urlLength);

char * javacall_lifecycle_get_wapUrlPointer(void);
void javacall_lifecycle_release_url_memory(void);
void javacall_lifecycle_set_ams_auto_start(int autoStart);
int javacall_lifecycle_is_ams_auto_start(void);
javacall_lifecycle_state javacall_lifecycle_get_vm_state(void);
...
int javacall_lifecycle_get_running_suiteId(void);
/**
 * external event loop
 */
void JavaTask(void);
...
void javanotify_file_browser_complete(javacall_result status);
 ...
 */
void javanotify_start_midlet(char* suiteId, char* className);
void javanotify_start_suite(char* suiteId);
void javanotify_remove_suite(char* suiteId);

/**
 * Checks the integrity of the suite storage database and remove the uninstalled suite.
 */
void javanotify_check_suites_integrity(void);
void javanotify_background(void);

void javanotify_foreground(void);
void javanotify_get_suite_info(int suiteId);

const char* JAVA_GetVersion(void);
/** @} */
void javacall_lifecycle_set_vm_state(javacall_lifecycle_state state);
/** @} */

javacall_lifecycle_ground_state javacall_lifecycle_get_ground_state(void);
