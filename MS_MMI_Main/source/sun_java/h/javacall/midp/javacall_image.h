...
/**
 * @brief Image data return type
 */
typedef struct {
    /** Pointer to decoding target buffer */
    javacall_pixel* decodeBuf;
    /** Size of decodeBuf in bytes */
    long decodeBufSize;
    /** Pointer to alpha data buffer, 
          It could be a NULL if there is no need to get alpha data */
    char* alphaBuf;
    /** Size of alphaBuf in bytes */
    long alphaBufSize;
} javacall_image_data;
