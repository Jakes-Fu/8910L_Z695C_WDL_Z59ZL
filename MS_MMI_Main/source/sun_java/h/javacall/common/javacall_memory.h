 ...
 */
//below is copy from spreadtrum to record all memory alloc location and size
//#define TRACK_MEMORY_ALLOC
#ifdef TRACK_MEMORY_ALLOC
#define   DYNAMIC_HEAP_APP_MEMORY           0x44444444UL 
#define javacall_malloc(_SIZE)    SCI_MallocEx(_SIZE, DYNAMIC_HEAP_APP_MEMORY, __FILE__, __LINE__)      	        
#else
...
void* pcsl_mem_malloc_javacall(unsigned int size);
void pcsl_mem_free_javacall(void *ptr);
/** @} */
