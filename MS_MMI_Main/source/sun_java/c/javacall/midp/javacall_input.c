...
typedef struct {
    javacall_utf16 *textBuffer;
    int *textBufferLength;
    int *caretPos;
    int textBufferMaxLength;
    javacall_textfield_constraint constraint;
    javacall_textfield_modifier modifier;	
} INPUT_METHOD_T;

LOCAL INPUT_METHOD_T textFieldData;
...
javacall_result javacall_input_get_locale(/*OUT*/ char locale[6]) {
    ...
    switch (language_type) {
        case MMISET_LANGUAGE_SIMP_CHINESE:
            SCI_STRCPY(locale, "zh-CN");    /*lint !e779*/
            break;
        case MMISET_LANGUAGE_TRAD_CHINESE:    
            SCI_STRCPY(locale, "zh-TW");    /*lint !e779*/
            break;
        case MMISET_LANGUAGE_FRENCH:        
            SCI_STRCPY(locale, "fr-FR");        /*lint !e779*/
            break;
        case MMISET_LANGUAGE_PORTUGUESE:        
            SCI_STRCPY(locale, "pt-BR");        /*lint !e779*/
            break;
        case MMISET_LANGUAGE_RUSSIAN:      
            SCI_STRCPY(locale, "ru-RU");        /*lint !e779*/
            break;
        case MMISET_LANGUAGE_SPANISH:
            SCI_STRCPY(locale, "es-ES");        /*lint !e779*/
            break;
        case MMISET_LANGUAGE_ITALIAN:
            SCI_STRCPY(locale, "it-IT");        /*lint !e779*/
            break;
        case MMISET_LANGUAGE_GERMAN:
            SCI_STRCPY(locale, "de-DE");        /*lint !e779*/
            break;
        default:
            SCI_STRCPY(locale, "en-US");        /*lint !e779*/
            break;
    }
...
javacall_textfield_status javacall_input_textfield_launch(
        ...
        /*IN*/ int					            modifiers,
        /*IN*/ int                              keyCode) {
    textFieldData.textBuffer = textBuffer;
    textFieldData.textBufferLength = textBufferLength;
    textFieldData.caretPos = caretPos;
    textFieldData.textBufferMaxLength = textBufferMaxLength;
    textFieldData.constraint = constraint;
    textFieldData.modifier = modifiers;

    javacall_printf(LOG_STR("textBuffer = %d, textBufferLength =%d"),textBuffer, textBufferLength );
    javacall_printf(LOG_STR("caretPos = %d, textBufferMaxLength =%d"),caretPos, textBufferMaxLength );  
	
    MMIJAVA_SendJVMMessage2MMI(APP_JAVA_LAUNCH_INPUT_METHOD_IND, LAUNCH_INPUT_METHOD);
    return JAVACALL_TEXTFIELD_WOULDBLOCK;
}

void javacall_prepare_inputmethod_data(MMIJAVA_TEXTEDITBOX_INIT_DATA_T *storeData) {
    storeData->max_str_len = (uint16)textFieldData.textBufferMaxLength;
    storeData->wstr_len = (uint16)(*textFieldData.textBufferLength);
    storeData->cusor_pos = (uint16)(*textFieldData.caretPos);
    storeData->wstr_ptr = (wchar*)textFieldData.textBuffer;
    storeData->rect = MMIJAVA_GetInputMethodRect();    

    javacall_printf(LOG_STR("constraint = %d, modifier= %d"),textFieldData.constraint, textFieldData.modifier);  

    switch (textFieldData.constraint) {
        case JAVACALL_TEXTFIELD_CONSTRAINT_ANY:        
            storeData->allow_inputmethod = GUIIM_TYPE_SET_ALL;
            storeData->cur_inputmethod = GUIIM_TYPE_SMART;		
	     	break;

	case JAVACALL_TEXTFIELD_CONSTRAINT_EMAILADDR:
        case JAVACALL_TEXTFIELD_CONSTRAINT_URL:
            storeData->allow_inputmethod = GUIIM_TYPE_ENGLISH | GUIIM_TYPE_ABC | GUIIM_TYPE_DIGITAL;     /*lint !e655*/
            storeData->cur_inputmethod = GUIIM_TYPE_ENGLISH;
            break;
			
        case JAVACALL_TEXTFIELD_CONSTRAINT_NUMERIC:
            storeData->allow_inputmethod = GUIIM_TYPE_DIGITAL;
            storeData->cur_inputmethod = GUIIM_TYPE_DIGITAL;
            break;
       case JAVACALL_TEXTFIELD_CONSTRAINT_PHONENUMBER:
            break;
        case JAVACALL_TEXTFIELD_CONSTRAINT_DECIMAL:
            storeData->allow_inputmethod = GUIIM_TYPE_DIGITAL;
            storeData->cur_inputmethod = GUIIM_TYPE_DIGITAL;
            break;
    }
    switch (textFieldData.modifier){
	case JAVACALL_TEXTFIELD_MODIFIER_PASSWORD:
	 	storeData->edit_type = GUIEDIT_TYPE_PASSWORD;
		break;
	case JAVACALL_TEXTFIELD_MODIFIER_UNEDITABLE:
	case JAVACALL_TEXTFIELD_MODIFIER_SENSITIVE:				
	case JAVACALL_TEXTFIELD_MODIFIER_NON_PREDICTIVE:	
	case JAVACALL_TEXTFIELD_MODIFIER_INITIAL_CAPS_WORD:		 
	case JAVACALL_TEXTFIELD_MODIFIER_INITIAL_CAPS_SENTENCE:
		storeData->edit_type = GUIEDIT_TYPE_TEXT;
		break;
	default:
		storeData->edit_type = GUIEDIT_TYPE_TEXT;
		break;
    	}
    if (textFieldData.constraint == JAVACALL_TEXTFIELD_CONSTRAINT_PHONENUMBER) {
            storeData->allow_inputmethod = GUIIM_TYPE_DIGITAL;
            storeData->cur_inputmethod = GUIIM_TYPE_DIGITAL;
            storeData->edit_type = GUIEDIT_TYPE_PHONENUM;
    }    
 
}

void javacall_get_inputmethod_data(MMI_STRING_T *inputData,uint16 pos) {    /*lint -e{765}*/
    if (inputData->wstr_len == 0 || inputData->wstr_ptr == PNULL) {
        *textFieldData.textBufferLength = 0;
        return;
    }
    *textFieldData.caretPos = pos;
    *textFieldData.textBufferLength = inputData->wstr_len;

    javacall_printf(LOG_STR("wstr_len = %d"),inputData->wstr_len);
   
    SCI_MEMCPY(textFieldData.textBuffer, inputData->wstr_ptr, inputData->wstr_len*sizeof(wchar));
}
 ...
 */
javacall_bool javacall_input_pointer_support() {
    return (javacall_bool)(mmijava_input_pointer_support());
}
 ...
 */
javacall_bool javacall_input_motion_support() {
    return (javacall_bool)(mmijava_input_motion_support());
}
#ifdef __cplusplus
}
#endif
