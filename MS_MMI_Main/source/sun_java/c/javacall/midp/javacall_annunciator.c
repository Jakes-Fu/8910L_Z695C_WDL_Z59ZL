...
{
	//GPIO_SetVibrator(enableVibrate);
	if( enableVibrate == JAVACALL_TRUE )
	{
	    //MMIAPISET_SetVibrator(1,MMISET_RING_TYPE_OTHER);
...
javacall_result javacall_annunciator_play_audible_tone(javacall_audible_tone_type soundType) {
    switch (soundType) {
	 ...
	 case JAVACALL_AUDIBLE_TONE_ALARM:
			//MMESET_PlayRing(1, MMISET_RING_TYPE_ERROR, SCI_NULL);
			MMIAPISET_PlayRing(0, FALSE, 0, 1, MMISET_RING_TYPE_BAT, PNULL);/*lint !e64 */
			// comment for core dump if called several times
            break;
