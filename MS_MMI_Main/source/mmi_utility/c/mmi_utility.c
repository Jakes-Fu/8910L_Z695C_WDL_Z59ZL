		...
		&& IMG_USER_GRAPH_DATA == *(src_data_ptr + 4))
    {
            ...
            && PNULL != data_ptr->graph_array )
        {
            ...
            if ( PNULL != image_rect_ptr )
            {
                ...
                if(!GUI_IntersectRect( &clip_rect, dst_rect, clip_rect ))
                {
                ...
                case MMI_GRAPH_RECT:
                    {
                        ...
                        {
                            if(!GUI_IntersectRect( &dst_rect, dst_rect, *((GUI_RECT_T*)data_ptr->graph_array[i].data_ptr ) ))
                            {
                                SCI_MEMSET(&dst_rect,0,sizeof(GUI_RECT_T));
                                break;
                            }
                        }
