                                                        ...
                                                        )
{
    ...
    travel_ptr = SCI_ALLOC_APP( sizeof( MMI_TREE_TRAVEL_INFO_T ) );/*lint !e26 !e64*/
    if(PNULL == travel_ptr)
    {
        SCI_TRACE_LOW("MMK_CreateTreeTravelInfo: travel_ptr is PNULL, malloc size is %d", sizeof(MMI_TREE_TRAVEL_INFO_T));    
        SCI_ASSERT(PNULL != travel_ptr);/*assert verified*/
    }
    SCI_MEMSET(travel_ptr, 0x00, sizeof(MMI_TREE_TRAVEL_INFO_T));
    ...
    if(PNULL == travel_ptr->data_ptr)
    {
        
        SCI_TRACE_LOW("MMK_CreateTreeTravelInfo: travel_ptr->data_ptr is PNULL, malloc size is %d", travel_ptr->max_num * sizeof(uint32));    
        SCI_FREE(travel_ptr);
                                ...
                                )
{
    ...
    if( travel_ptr->total_num == travel_ptr->max_num )
    {
        ...
        if(PNULL == new_data_ptr)
        {
            SCI_TRACE_LOW("AppendTreeTravelInfo: new_data_ptr is PNULL, malloc size is %d", travel_ptr->max_num * sizeof(uint32));
            SCI_ASSERT(PNULL != new_data_ptr);/*assert verified*/
        }
        SCI_MEMSET(new_data_ptr, 0x00, travel_ptr->max_num * sizeof(uint32));
                                      ...
                                      )
{
    ...
    if (msg_id == MSG_CTL_PAINT)
    {
        for (i = 0 ; i < travel_ptr->total_num; i++ )
        {
            b_topmost = MMK_IsControlNeedTopmostShow (travel_ptr->data_ptr[i]);
            if (b_topmost)
            {
                // move to the last show item
                int j;
		  uint32 tmp = travel_ptr->data_ptr[i];
		  for (j = i; j < travel_ptr->total_num - 1; j++ )
		  {
		      travel_ptr->data_ptr[j] = travel_ptr->data_ptr[j + 1];
		  }
		  travel_ptr->data_ptr[travel_ptr->total_num - 1] = tmp;
            }
        }
    }
#endif
                                  ...
                                  )
{
    ...
    *root_pptr = PNULL;
    ...
    MMK_DestroyTreeTravelInfo( travel_ptr );
    SCI_TRACE_LOW("MMK_RemoveAllTreeNode: Exit");
    return;
}
...
#if 1 /*0->1 for MMI_DEVICELOCK_SUPPORT*/
/*****************************************************************************/
                                   ...
                                   )
{
    ...
    if(PNULL == node_ptr)
    {
        SCI_TRACE_LOW("TreeNodeNew: node_ptr is PNULL, malloc size is %d", sizeof(MMI_TREE_NODE_T));
