...
{    
    ...
    //对非法字符的号码，重新计算length
    if(!MMIAPICOM_IsValidNumberString((char*)( tele_num + offset), tele_len - offset))
    {
        party_num->num_len = MIN(tele_len - offset + 1, MN_MAX_ADDR_BCD_LEN);//bug501073       
    }
                                     ...
                                     )
{
    ...
    //check the tele_num is internation tele
    if(( MN_NUM_TYPE_INTERNATIONAL == number_type )&&(party_num[0]!=0) )
    {
        offset = 1;
        tele_num[0] = '+';
    }
                              ...
                              )
{
    ...
    uint32      j = 0;
    uint16      offset_len = 1; //offset len for slash_char and dot_char
    ...
    if (dir_len > 0 && !is_extname)
    {
        ...
        MMI_WSTRNCPY((full_path_ptr + j),offset_len, &slash_char, offset_len,  offset_len );
        j = j + 1;
    }
    else if (dir_len > 0 && is_extname)
    {
        MMI_WSTRNCPY((full_path_ptr + j),  offset_len, &dot_char,offset_len,  offset_len );
        j = j + 1;
    }
...
PUBLIC uint32 MTK_GetImgID(uint16   unicode)
{
      uint32 img_id = 0;
      
      if(unicode >= 0x30 && unicode <= 0x39)
      {
        img_id = IMAGE_NUMBER_BIG_DIAL_0 + unicode - 0x30;
      }
      else if(unicode == 0x23)
      {
         img_id = IMAGE_NUMBER_BIG_DIAL_HUSH;
      }
      else if(unicode == 0x2A)
      {
        img_id = IMAGE_NUMBER_BIG_DIAL_STAR;
      }
      else if(unicode == 0x2B)
      {
        img_id = IMAGE_NUMBER_BIG_DIAL_ADD;
      }
      else if(unicode == 0x50)
      {
        img_id = IMAGE_NUMBER_BIG_DIAL_P;
      }
      else if(unicode == 0x57)
      {
        img_id = IMAGE_NUMBER_BIG_DIAL_W;
      }
	return img_id;
}

#if 1
void DrawMTKImgBuf( GUI_COLOR_T *lcd_buffer_ptr, uint16 lcdwidth,  uint16 lcdheight,  int16 x0, int16 y0,GUI_RECT_T rect, uint8 fontwidth,  uint8 fontheight, uint32 img_id, GUI_COLOR_T color, GUI_COLOR_T edge_color )
{
    GUI_LCD_DEV_INFO    lcd_dev_info = {0};
//    GUI_RECT_T img_rect = {0,0,46-1,65-1};
    GUI_RECT_T img_rect = {0,0,56-1,88-1};
    GUI_POINT_T        point = {0};
    point.x = x0;
    point.y = y0;
...
//	Parameters:  osname:osname ptr,Input/Output
//               str_max_len:the max lenth of osname ptr 
//  Author:
//	Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPICOM_GetOsName(char *osname,uint8 str_max_len)
{
    char testdata[] = "ARM926EJ-S";
    
    if(PNULL != osname)
    {
        if(SCI_STRLEN(testdata) < str_max_len)
        {
            SCI_MEMCPY(osname, testdata, SCI_STRLEN(testdata));
            return TRUE;
        }
...
/*****************************************************************************/
// 	Description : Get processor model
//	Parameters:  processor_model:processor_model ptr,Input/Output
//               str_max_len:the max lenth of processor_model ptr 
//  Author:
//	Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPICOM_GetProcessorModel(char *processor_model,uint8 str_max_len)
{
    char testdata[] = "Threadx4.0";
    
    if(PNULL != processor_model)
    {
        if(SCI_STRLEN(testdata) < str_max_len)
        {
            SCI_MEMCPY(processor_model, testdata, SCI_STRLEN(testdata));
            return TRUE;
        }
...
/*****************************************************************************/
// 	Description : Get language pack ID
//	Parameters:  language_pack:language pack ID ptr,Input/Output
//               str_max_len:the max lenth of language_pack ptr 
//  Author:
//	Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMIAPICOM_GetLanguagePackID(char *language_pack,uint8 str_max_len)
{
    char testdata[] = "en-UN";
    
    if(PNULL != language_pack)
    {
        if(SCI_STRLEN(testdata) < str_max_len)
        {
            SCI_MEMCPY(language_pack, testdata, SCI_STRLEN(testdata));
            return TRUE;
        }
...
char* MMI_UnicodeToAnsii(char *pOutBuffer, char *pInBuffer )
{
    char * orgptr = pOutBuffer;
