...
{   /*BCD format - LSB first (1234 = 0x21, 0x43)*/
    ...
    uint8 bcdCode = 0;
    uint8 ascCode = 0;
	BOOLEAN is_direct_convert = FALSE;
    uint8   upBcdLen = 0;

    //对非法字符的号码，直接进行转换,如Test、空字符""
...
{   /*BCD format - LSB first (1234 = 0x21, 0x43)*/
    ...
    for(i = 0;i < bcdLen; i++)
    {
        ...
        str[i] = ascCode;
        SCI_TRACE_LOW("[szp]MMIAPICOM_NetBcdLfToStr: i = %d, str[i] = %c",i,str[i]);
    }
...
#if 0
/* For mobile side, according to "4.1.1 Initiating action at the Mobile Station (MS)" as described belows, it should treat 
     USSD MMI string according to 22.030 where '#" is not prohibitted to be used as seperator. */
		//#出现中间无效
		if ( pStr[i] == '#')
		{
			i++;
                   ...
                   )
{
    ...
    //对非法字符的号码，直接进行转换
    if(!MMIAPICOM_IsValidNumberString((char*)pStr, strLen))
    {
        is_direct_convert = TRUE;
    }
    if(is_direct_convert)
    {
        pBcd[0] = 0xff;//非法号码标志位
        for(i = 0; i < strLen; i++)//bug494365
        {
		if (i+1 < MN_MAX_ADDR_BCD_LEN)
		{
			pBcd[i+1] = pStr[i];
		}
		else
...
//  Note:
/*****************************************************************************/
PUBLIC void MMIAPICOM_BinaryToHexCharForDac(uint8* src_ptr, uint16 src_len, uint8* dest_ptr, uint16* dest_len)
{
    uint8 szValue[ 4 ] = { 0 };
    int32 i = 0;    
    ...
    {
        sprintf((char*)szValue, "%02x", src_ptr[i]);
        *dest_ptr = szValue[0];
        dest_ptr++;
        *dest_ptr = szValue[1];
        dest_ptr++;
