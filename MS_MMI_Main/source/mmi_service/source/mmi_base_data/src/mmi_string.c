...
/*****************************************************************************/
//  Description : 字符串替换(源字符串, 旧字符串, 新字符串) 函数
//  Global resource dependence : none
//  Author: grant.peng
//  Note:
/*****************************************************************************/
PUBLIC wchar *MMIAPICOM_WstrReplace(wchar *src_buf, uint32 buf_size, const wchar *oldstr, const wchar *newstr)
{
    wchar *dst = PNULL;
    wchar *needle = PNULL;
    wchar *tmp = PNULL;
    ...
    if(PNULL == src_buf || PNULL == oldstr || PNULL == newstr)
    {
        return src_buf;
    }
    
    if (MMIAPICOM_Wstrlen(oldstr) == MMIAPICOM_Wstrlen(newstr) 
        && MMIAPICOM_Wstrcmp(oldstr, newstr) == 0)
    { 
        return src_buf;
    }

    while ((needle = MMIAPICOM_Wstrstr(src_buf, oldstr)))
    {
        size = (MMIAPICOM_Wstrlen(src_buf) + (MMIAPICOM_Wstrlen(newstr) - MMIAPICOM_Wstrlen(oldstr)) +1) * sizeof(wchar);
        if(size < buf_size)
        {
            tmp = (wchar*)SCI_ALLOCA(size);
            if(PNULL != tmp)
            {
                SCI_MEMSET(tmp, 0 , size);
                MMIAPICOM_Wstrncpy(tmp, src_buf, needle-src_buf);
                tmp[needle-src_buf] = '\0';
                MMIAPICOM_Wstrcat(tmp, newstr);
                MMIAPICOM_Wstrcat(tmp, needle + MMIAPICOM_Wstrlen(oldstr)); 
                SCI_MEMCPY(src_buf, tmp, size);
                SCI_Free(tmp);
                tmp = PNULL;
            }
        }
...
/********************************************************************************
 ...
 DATE:          2004.09.14
********************************************************************************/
PUBLIC void MMIAPICOM_Int2String(uint32 value, uint8* buf, uint8 buf_len)
{
    int32 i = 0, j = 0;
    int32 semi_len = 0;
    int32 offset = 0;
...
//     Note:
/*****************************************************************************/
PUBLIC BOOLEAN  MMIAPICOM_CustomGenerateReplaceStrByTextId
(
...
)
{
       ...
       return MMIAPICOM_CustomGenerateReplaceStr(&src_str, find_str, replace_str, result_max_len, result_str);
}
...
PUBLIC uint16 MMIAPICOM_GetStrNumForDisplayWidth(
                                    const wchar     *src_str_ptr,       //in:
                                    uint16          *src_str_len_ptr,   //in
                                    uint16          str_space,      //in:
                                    uint16          display_width,  //in:
                                    GUI_FONT_T      font_type       //in:
                                    )
{
    ...
    if (font_type >= GUI_MAX_FONT_NUM  || src_str_ptr == NULL || src_str_len_ptr == NULL)
    {
        //SCI_TRACE_LOW:"MMIAPICOM_GetStrWidth PNULL == str_ptr || PNULL == str_len_ptr || font_type >= GUI_MAX_FONT_NUM"
        SCI_TRACE_LOW("[%s: %d] invalide paramter \n",__FUNCTION__,__LINE__);
        return 0;
    }
	
    for (i=0; i<*src_str_len_ptr && str_width < display_width; i++)
    {
        //add string space
        if (0 != i)
        {
    ...
    if (str_width > display_width && i > 0)
    {
        i--;
    }
