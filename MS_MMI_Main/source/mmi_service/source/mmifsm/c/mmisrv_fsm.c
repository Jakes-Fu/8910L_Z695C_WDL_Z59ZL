									  ...
									  )
{
    ...
    if (PNULL == fsm_ptr)
    {
        SCI_TRACE_LOW("FSMSRV_Construct: fsm_ptr is PNULL");
        return 0;
    }
    SCI_MEMSET(fsm_ptr,0,sizeof(FSMSRV_T)+fsm_object_size);
    SCI_TRACE_LOW("FSMSRV_Construct: fsm_object=0x%x, size=0x%x, fsm_ptr=0x%x", fsm_object, fsm_object_size, fsm_ptr);
    //初始化状态机信息    
	...
	else
	{
		fsm_ptr->object_data = (FSM_OBJECT)((uint32)fsm_ptr+sizeof(FSMSRV_T));
              SCI_TRACE_LOW("FSMSRV_Construct: object_data=0x%x", fsm_ptr->object_data);
...
{ 
    ...
    {
        SCI_TRACE_LOW("FSMSRV_Destruct: fsm_handle_ptr=0x%x, *fsm_handle_ptr=0x%x", fsm_handle_ptr, *fsm_handle_ptr);
