...
LOCAL MMISRV_HANDLE_T s_mmsrvaud_current_handle = 0;
...
{
   return s_mmsrvaud_current_handle;
}
/*****************************************************************************/
...
PUBLIC MMISRVAUD_RET_E MMISRVAUD_Play(MMISRV_HANDLE_T audio_handle, uint32 offset)
{
    ...
    s_mmsrvaud_current_handle = audio_handle;
...
LOCAL MMISRV_HANDLE_T ReqToneSrvVolume(
                uint16 pri,
                MMISRVAUD_TYPE_E tone_type,
                uint32 tone_id,
                uint32 duration,
                ...
                )
{
    ...
    case MMISRVAUD_TYPE_TONE_GENERIC:
        ...
        type_info.tone_generic.type = tone_type;
        type_info.tone_generic.id = tone_id;
        type_info.tone_generic.volume = volume;
        break;
    case MMISRVAUD_TYPE_TONE_CUSTOM:
        type_info.tone_custom.duation = duration;
        type_info.tone_generic.type = tone_type;
                ...
                )
{
    ...
    handle = ReqToneSrvVolume(pri, tone_type, tone_id, duration, freq, volume, FALSE);
...
PUBLIC uint16 MMISRVAUD_GetAudPriorityInfo(MMISRV_HANDLE_T audio_handle)
{
	...
	SCI_TRACE_LOW("MMISRVAUD_GetAudPriorityInfo...audio_handle=%d",audio_handle);
    if(audio_handle != 0)
    {
        data.event = MMISRVAUD_EVENT_GET_SRV_PRI;
        res = MMISRVMGR_IoCtrl(audio_handle, (uint32)&data, PNULL);
