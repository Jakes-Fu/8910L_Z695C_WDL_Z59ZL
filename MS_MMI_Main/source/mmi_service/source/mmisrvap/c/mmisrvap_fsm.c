...
)
{
    ...
    do
    {
        ...
        if((MMISRVAUD_RET_OK == audio_ret) || (MMISRVAUD_RET_PENDING == audio_ret) )
        {
			...
			else //MMISRVAUD_RET_PENDING == audio_ret
            {
...
)
{
    ...
    error_code = *((uint32 *)msg_ptr->msg_body);

    if(error_code != MMISRVAP_ERROR_NONE)
    {
	    APUpdateMsgID(fsm_ptr, msg_ptr->msg_id);
	    AP_SetErrorCode(fsm_ptr, error_code);
	    APSetCallerCallbackFlag(TRUE, fsm_ptr);
...
)
{
    ...
    SCI_TRACE_LOW("[MMISRVAP] APHandleResumeReq error_code = %d", \
                  error_code);
...
)
{
    ...
    MMISRVAP_TRACE_LOW("[MMISRVAP] APHandlePeriodTimer timer_id=%d, param=0x%x", \
                       timer_id, param);
    ...
    */
    ...
    {
        MMISRVAP_TRACE_LOW("[MMISRVAP] APHandlePeriodTimer fsm_ptr=0x%x, ap_handle=0x%x, state=0x%x", \
                       fsm_ptr,  fsm_ptr->ap_handle, fsm_ptr->state);
