...
/*                          TYPE AND CONSTANT                                */
/*---------------------------------------------------------------------------*/
#if 0//move to mmisrvrecord_export.h
typedef enum
{
									 ...
									 MMISRVAUD_RECORD_FMT_E formattype)
{
	...
	if(MMISRVAUD_RECORD_FMT_MP3 == formattype)
    {
        GetFileIDString(temp_photo_name, prefix_ptr, record_file_id, RECORD_FILE_NAME_ID_MAX, "mp3");
    }
    else if(MMISRVAUD_RECORD_FMT_AMR == formattype) 
    {
        GetFileIDString(temp_photo_name, prefix_ptr, record_file_id, RECORD_FILE_NAME_ID_MAX, "amr");
    }
     else 
	{
        GetFileIDString(temp_photo_name, prefix_ptr, record_file_id, RECORD_FILE_NAME_ID_MAX, "wav");
    }
	SCI_MEMSET(record_srv_info_ptr->record_file_name_ptr,0,((MMIRECORD_SRV_MAX_FILE_NAME_LEN + 1)* sizeof(wchar)));
   ...
   if (PNULL != file_name_info.record_name_ptr)
	...
	{			
		*file_name_info.record_dev_ptr = (*file_name_info.record_dev_ptr < MMI_DEVICE_NUM ? *file_name_info.record_dev_ptr : MMI_DEVICE_UDISK);
		...
		MMIAPIFMM_CombineFullPath(file_dev_ptr, file_dev_len, MMIMULTIM_DIR_RECORDING, MMIAPICOM_Wstrlen(MMIMULTIM_DIR_RECORDING),
								...
								)
{
    ...
    {      				
		...
		else if(PNULL != file_name_info.record_file_id_ptr)
		{
			...
			while(1)  /*lint !e716*/
			{           
				*file_name_info.record_file_id_ptr += step;
				step = step * 2;
				if(*file_name_info.record_file_id_ptr >= RECORD_FILE_NAME_ID_MAX)
				{
					circle ++;
					if(circle >= RECORD_FILE_NAME_ID_MAX)
					{
						circle = 1;
					}
					*file_name_info.record_file_id_ptr = circle;
					step = 1;
				}		
