...
#define BT_CLOSE_DELAY_TIME     (3 * 1000)
...
LOCAL BOOLEAN IsNewSrvCoexist(MMISRVAUD_TYPE_U* type_ptr)
{
    ...
    const uint32 co_table[] = 
    {
        ...
        FL(MMISRVAUD_TYPE_VOICE) |  FL(MMISRVAUD_TYPE_VIBERATE),   /* Candice */
#else
...
/*****************************************************************************/
//  Description :get audio type by audio handle
//  Global resource dependence : 
//  Author:
//  Note:
/*****************************************************************************/
PUBLIC MMISRVAUD_TYPE_E MMISRVAUD_GetAudioType(MMISRV_HANDLE_T audio_handle)
{
	SRVAUD_ENTITY_INFO_T *entity_ptr = GetEntityAddrByHandle(audio_handle);

    if(entity_ptr != NULL)
    {
       return entity_ptr->audio.audio_info.info.type;
    }
    return MMISRVAUD_TYPE_NONE;
}
/*****************************************************************************/
//  Description :need check high volume  
//  Global resource dependence : 
//  Author:
//  Note:
/*****************************************************************************/
LOCAL BOOLEAN IsNeedCheckHighVolume(SRVAUD_ENTITY_INFO_T *entity_ptr)
{
    BOOLEAN res = FALSE;
    if(entity_ptr != PNULL)
    {
...
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMISRVAUD_IsRestartHighVolumeDetect(MMISRV_HANDLE_T handle)
{
	if(IsNeedCheckHighVolume(GetEntityAddrByHandle(handle)) && MMISRVAUD_IsAudioPlaying())
	{
	    SCI_TRACE_LOW("[baokun] MMISRVAUD_IsRestartHighVolumeDetect true");
		return TRUE;
	}
    SCI_TRACE_LOW("[baokun] MMISRVAUD_IsRestartHighVolumeDetect false");
	return FALSE;
}
/*****************************************************************************/
...
LOCAL MMISRVAUD_ROUTE_T GetShouldBeRoute(SRVAUD_ENTITY_INFO_T *entity_ptr)
{
    ...
    SCI_TRACE_LOW("[MMISRV]: Audio GetShouldBeRoute() entity_ptr->audio.audio_info.info.type %d", entity_ptr->audio.audio_info.info.type);	
    ...
    else
    {
        if(MMISRVAUD_ROUTE_AUTO == entity_ptr->audio.audio_info.all_support_route)
        {
            ...
            if(IsTypeSupportBt(entity_ptr) && MMISRVAUD_IsBTHeadSetActive() && (MMIBT_IsExitedActiveA2dp() || MMICC_IsValidCallState(MMICC_GetCurrentCallStatus())))//bug1062084
            {
                route = MMISRVAUD_ROUTE_BLUETOOTH;
            }            
            else 
        ...
        else
        {
            ...
            {
                //SCI_TRACE_LOW(" fuc[%s] line[%d] default_route is[%d]  ", __FUNCTION__,__LINE__,entity_ptr->audio.audio_info.default_route);
#ifdef VIRTUAL_VIBRATE_FUNC
                 if((MMISRVAUD_IsHeadSetPlugIn()) && (entity_ptr->audio.audio_info.info.type==MMISRVAUD_TYPE_VIBERATE))//MMISRVAUD_SearchHandleByType(MMISRVAUD_TYPE_VIBERATE) != 0)
                   {
                     	route = MMISRVAUD_ROUTE_EARFREE;
                        SCI_TRACE_LOW(" earfree and vib should set status  [%d]  ", __FUNCTION__,__LINE__,route);
                   }
                 else
#endif
	                 ...
	                 if(entity_ptr->audio.audio_info.info.type==MMISRVAUD_TYPE_VIBERATE)//MMISRVAUD_SearchHandleByType(MMISRVAUD_TYPE_VIBERATE) != 0)
	                   {
                        ...
                        case MMISRVAUD_ROUTE_BLUEPHONE:
                            if(MMISRVAUD_IsBTHeadSetActive() && (MMIBT_IsExitedActiveA2dp() || MMICC_IsValidCallState(MMICC_GetCurrentCallStatus())))//bug1062084
                            {
                                route = MMISRVAUD_ROUTE_BLUETOOTH;
                            }
                            break;
    ...
    while(i < MMISRVAUD_SERVICE_NUM_MAX && num < s_handle_box.num)
    {
        if(s_handle_box.entity_addr[i] != 0)
        {
            ...
            if(entity_ptr != PNULL)
            {
                ...
                SCI_TRACE_LOW("[baokun] SetVolumeByType %d ", entity_ptr->handle);
			    MMISRVAUD_ResetDurationTimer(entity_ptr->handle);
                        flag = TRUE;
                    }
                }
            }
...
LOCAL void HandleAudioTimer(uint8 timer, uint32 param)
{
    ...
    if(entity_ptr != PNULL)
    {
		...
		if(entity_ptr->audio.audio_info.info.vibrate.cb !=PNULL)
		{
             entity_ptr->audio.audio_info.info.vibrate.cb();
		}
...
LOCAL MMISRV_HANDLE_T SrvCreate(MMISRVMGR_SERVICE_REQ_T *req_data, uint32 ext_data)
{
    ...
    {   
        ...
        if(num > 0 && MMISRVAUD_IsAudioPlaying())
        {   
            UNLOCK_SERVICE;
            return 0;
        }
...
//  Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMISRVAUD_IsAudioPlaying(void)
{
        ...
        if(entity_ptr->audio.audio_info.volume!= (MMISRVAUD_VOLUME_T)event_data->data1)
        {
            ...
            if(!result)
            {
                ...
                {        
                    ...
                    if(result)
                    {
                        SCI_TRACE_LOW("[baokun] MMISRVAUD_EVENT_SET_VOLUME %d ", entity_ptr->handle);
						MMISRVAUD_ResetDurationTimer(entity_ptr->handle);
...
LOCAL BOOLEAN SrvCmdFunc(MMISRVMGR_SERVICE_CMD_PARAM_T *param, uint32 *res_data)
{
    ...
    case MMISRVMGR_SERVICE_CMD_EXT:
        ...
        case MMISRVAUD_EXT_CMD_SET_VOLUME:
	     ...
	     if(res)
	     {
	       SCI_TRACE_LOW("[baokun] MMISRVAUD_EXT_CMD_SET_VOLUME ");
		    MMISRVAUD_ResetDurationTimer(0);
	     }
            break;
...
LOCAL BOOLEAN BtPreStop(SRVAUD_ENTITY_INFO_T *entity_ptr, MMISRVAUD_EVENT_T *data)
{
    ...
    _DBG(    //MMISRV_TRACE_LOW:"[MMISRV]: Audio BtPreStop() entry, entity_ptr=0x%x, data=0x%x, s_bt_info.state=%d"
    ...
    _DBG(    //MMISRV_TRACE_LOW:"[MMISRV]: Audio BtPreStop() exit, result=%d"
...
LOCAL MMISRVAUD_RET_E AudPlay(SRVAUD_ENTITY_INFO_T *entity_ptr, MMISRVAUD_EVENT_T *event_ptr)
{
    ...
    if(IS_SRV_SUSPEND(entity_ptr))
    {
        ...
        if(result && IS_AUD_PLAY(entity_ptr))
        {
	     ...
	     if(IsNeedCheckHighVolume(entity_ptr))  MMISRVAUD_StartDurationTimer(entity_ptr->handle);
            return ret_val;
        }
        //result = AudResume(entity_ptr, event_ptr);
    }
    ...
    if(ret_val == MMISRVAUD_RET_OK && IsNeedCheckHighVolume(entity_ptr))
	{
		MMISRVAUD_StartDurationTimer(entity_ptr->handle);
	}
...
LOCAL BOOLEAN AudResume(SRVAUD_ENTITY_INFO_T *entity_ptr, MMISRVAUD_EVENT_T *event_ptr)
{
    ...
    MMISRV_TRACE_LOW("[MMISRV]: Audio AudResume entry, is_standing=%d, is_suspend=%d", entity_ptr->audio.audio_info.is_standing, IS_SRV_SUSPEND(entity_ptr));
    if(entity_ptr->audio.audio_info.is_standing)
    {
        if(MMISRVAUD_IsAudioPlaying())
        {
    ...
    {
        MMISRV_TRACE_LOW("[MMISRV]: Audio AudResume 1");
        if(IsNeedSetRoute(entity_ptr) && IS_SHOULD_BT_ROUTE(entity_ptr))
        {
            result = BtPreResume(entity_ptr, event_ptr);
            MMISRV_TRACE_LOW("[MMISRV]: Audio AudResume 2 result=%d", result);
        }
        else
        {
            SetAudParam(entity_ptr);
            ...
            {
                result = DRVAUD_Resume(entity_ptr->drv_handle, &entity_ptr->audio.audio_info.info);
                MMISRV_TRACE_LOW("[MMISRV]: Audio AudResume 3 result=%d", result);
            }
            else if(IS_AUD_PENDING(entity_ptr) || IS_AUD_STOP(entity_ptr))
            {
                result = DRVAUD_Play(entity_ptr->drv_handle, &entity_ptr->audio.audio_info.info, 0, entity_ptr->audio.cur_route);                
                MMISRV_TRACE_LOW("[MMISRV]: Audio AudResume 4 result=%d", result);
            }
            if(result)
            {
                entity_ptr->audio.audio_state = AUDIO_STATE_PLAY;
    ...
    if(result == TRUE && IsNeedCheckHighVolume(entity_ptr))
	{
		MMISRVAUD_ResumeDurationTimer();
	}
...
LOCAL BOOLEAN AudPause(SRVAUD_ENTITY_INFO_T *entity_ptr, MMISRVAUD_EVENT_T *event_ptr, BOOLEAN is_change_state)
{
    ...
    if((IS_SRV_ACTIVE(entity_ptr) || IS_SRV_STANDING(entity_ptr))&& IS_AUD_PLAY(entity_ptr))
    {
        ...
        {
            MMISRV_TRACE_LOW("[MMISRV]: Audio AudPause(), current is bt route, result=%d", result);//1120282
            if(result)
            {
...
PUBLIC void MMISRVAUD_WaitBtCloseDelayTimer(void)
{
    SCI_TRACE_LOW("[mmmmmm] line[%d] file[%s] func[%s] %d, %d", __LINE__, __FILE__, __FUNCTION__, MMIAPIBT_IsBtOn(), s_bt_info.timer);
    if(MMIAPIBT_IsBtOn())
    {
        uint32 total_wait_time = 0;
        while((0 != s_bt_info.timer) && (total_wait_time < BT_CLOSE_DELAY_TIME))
        {
            SCI_Sleep(20);//sleep 20ms
            total_wait_time +=20;
        }
        if((0 != s_bt_info.timer) && (BT_CLOSE_DELAY_TIME <= total_wait_time))
        {
             HandleBTCloseDelayTimer(s_bt_info.timer, 0);
             BT_CLEAR_TIMER;
        }
        SCI_TRACE_LOW("[mmmmmm] line[%d] file[%s] func[%s] total_wait_time=%d, %d", __LINE__, __FILE__, __FUNCTION__, total_wait_time, BT_CLOSE_DELAY_TIME);
    }
}
