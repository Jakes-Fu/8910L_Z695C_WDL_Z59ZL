...
//FOR MS EUT-detect 
#define AUD_DURATION_SKIP   10*1000 /*10s*/
#define AUD_MAX_PLAY_DURATION   20*60*60*1000 /*20 hour*/
#define AUD_ALERT_AUD_VOLUME_THRESHOLD 7
#define AUD_ALERT_AUD_MIN_VOLUME   6

#define AUD_ALERT_FM_VOLUME_THRESHOLD 6
#define AUD_ALERT_FM_MIN_VOLUME   5

LOCAL uint8 s_aud_play_duration_timer = 0; /*the accumlative duration of audio playing during 85db*/
LOCAL uint32 s_alert_start = 0;
/*****************************************************************************/
...
PUBLIC BOOLEAN MMISRVAUD_LinkTravel(SRVAUD_LINK_E link_type, MMISRVAUD_LINK_TRAVEL_FUNC func, uint32 param)
{
    ...
    tmp_node = link_head->prev_ptr;
    while((tmp_node != link_head) && (link_head ==GetLinkHeadByType(link_type)) && IsExistInLink(tmp_node, link_head))
    {
...
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMISRVAUD_CheckAudioVolumeExcessThreshold(MMISRV_HANDLE_T audio_handle, BOOLEAN only_with_earphone)
{
    BOOLEAN result = FALSE;
    BOOLEAN headset_plugin = FALSE;
    uint32 audio_level = AUD_ALERT_AUD_VOLUME_THRESHOLD;
    
    //SCI_TRACE_LOW("[baokun]MMISRVAUD_CheckAudioVolumeExcessThreshold only_with_earphone %d audiorount 0x%x audio type %d volume %d", 
	//	only_with_earphone, MMISRVAUD_GetRoute(audio_handle), MMISRVAUD_GetAudioType(audio_handle), MMISRVAUD_GetVolume(audio_handle));  

    if(MMISRVAUD_TYPE_FM == MMISRVAUD_GetAudioType(audio_handle))
    {
        audio_level = AUD_ALERT_FM_VOLUME_THRESHOLD;
    }
#if defined(WIN32)
    headset_plugin = TRUE;
#else
    headset_plugin = GPIO_CheckHeadsetStatus();
#endif
    if(only_with_earphone && (!headset_plugin))
    {
       result = FALSE;
    }
	else if(audio_handle != PNULL)
	{
		if(MMISRVAUD_GetVolume(audio_handle) >= audio_level)
		{
			result = TRUE;
		}
	}
	else if(MMISRVAUD_GetVolumeNoHandle() >= audio_level)
	{
		result = TRUE;
	}
    SCI_TRACE_LOW("[baokun] MMISRVAUD_CheckAudioVolumeExcessThreshold  only_with_earphone %d audio_handle %d result %d", only_with_earphone, audio_handle, result);
	return result;
...
//  Note: 
/*****************************************************************************/
LOCAL void AudioTooHighAlert(uint8 timer_id, uint32 param) 
{
    BOOLEAN needalert = MMISRVAUD_CanEUAlert();
    SCI_TRACE_LOW("[baokun]AudioTooHighAlert enter needalert %d", needalert);
    if(timer_id == s_aud_play_duration_timer)
    {
        MMIAPIMP3_SetVolume(AUD_ALERT_AUD_MIN_VOLUME);
#ifdef VIDEO_PLAYER_SUPPORT
        MMIAPIVP_FSMUpdateCurrentVolNoHandle(AUD_ALERT_AUD_MIN_VOLUME);
#endif
#ifdef FM_SUPPORT
        MMIFM_AdjustVolumeTP(AUD_ALERT_FM_MIN_VOLUME);
#endif
    }
    if(needalert)
    {
        MMK_PostMsg (VIRTUAL_WIN_ID, MSG_AUDIO_EU_ALERT_NOTIFY, PNULL, 0);
    }
}
...
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMISRVAUD_InitDurationTimer(void)
{
	BOOLEAN result = FALSE;    
	s_aud_play_duration_timer = MMK_CreateTimerCallback(AUD_MAX_PLAY_DURATION, AudioTooHighAlert,0, TRUE);
	if(s_aud_play_duration_timer > 0)
	{
         result = MMK_PauseTimer(s_aud_play_duration_timer);
	}
	return result;
}

/*****************************************************************************/
//  Description:  start duration timer .
//  Global resource dependence: 
//  Author: 
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMISRVAUD_StartDurationTimer(MMISRV_HANDLE_T audio_handle)
{   
	BOOLEAN result = FALSE;
       //SCI_TRACE_LOW("[baokun]MMISRVAUD_StartDurationTimer audiorount %d isPermit %d", 
       // MMISRVAUD_GetRoute(audio_handle), MMISRVAUD_CheckAudioVolumeExcessThreshold(audio_handle, TRUE));
	if(MMISRVAUD_CheckAudioVolumeExcessThreshold(audio_handle, TRUE))
	{
		result = MMK_ResumeTimer(s_aud_play_duration_timer);
...
/*****************************************************************************/
//  Description:  Pause duration timer .
//  Global resource dependence: 
//  Author: 
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMISRVAUD_PauseDurationTimer(void)
{
	return MMK_PauseTimer(s_aud_play_duration_timer);
}

/*****************************************************************************/
//  Description:  resume duration timer .
//  Global resource dependence: 
//  Author: 
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMISRVAUD_ResumeDurationTimer(MMISRV_HANDLE_T audio_handle)
{
    //SCI_TRACE_LOW("[baokun]MMISRVAUD_ResumeDurationTimer audiorount %d ", MMISRVAUD_GetRoute(audio_handle));
#ifndef WIN32
    if(MMISRVAUD_ROUTE_EARPHONE == MMISRVAUD_GetRoute(audio_handle))
#endif
    {
	    return MMISRVAUD_StartDurationTimer(audio_handle);
    }
    return FALSE;
}

/*****************************************************************************/
//  Description:  reset duration timer .
//  Global resource dependence: 
//  Author: 
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMISRVAUD_ResetDurationTimer(MMISRV_HANDLE_T audio_handle)
{
    //SCI_TRACE_LOW("[baokun] MMISRVAUD_ResetDurationTimer enter audio_handle %d", audio_handle);
    if(PNULL == audio_handle)
    {
    	//get current playing audio handle
		if(MMISRVAUD_IsAudioPlaying())
        {
			audio_handle = MMISRVAUD_GetCurrentHandle();
        }
    }
    if(audio_handle != PNULL)
    {
		if(!MMISRVAUD_CheckAudioVolumeExcessThreshold(audio_handle, FALSE))
		{
			MMK_StopTimer(s_aud_play_duration_timer);
			MMISRVAUD_InitDurationTimer();		
					
		}else if(MMISRVAUD_IsRestartHighVolumeDetect(audio_handle))
		{		
			MMISRVAUD_ResumeDurationTimer(audio_handle);
		}
    }
	return (s_aud_play_duration_timer > 0);
}

/*****************************************************************************/
//  Description:  get s_aud_need_alert .
//  Global resource dependence: 
//  Author: 
//  Note: 
/*****************************************************************************/
PUBLIC BOOLEAN MMISRVAUD_CanEUAlert(void)
{
    uint32 curtick = SCI_GetTickCount();
    if(0 == s_alert_start 
    ||((AUD_MAX_PLAY_DURATION - AUD_DURATION_SKIP) <=(curtick - s_alert_start))
    )
    {
       return TRUE;
    }
...
/*****************************************************************************/
//  Description:  set  MMISRVAUD_StartEUAlertCount.
//  Global resource dependence: 
//  Author: 
//  Note: 
/*****************************************************************************/
PUBLIC void MMISRVAUD_StartEUAlertCount(void)
{
   
   s_alert_start = SCI_GetTickCount();
}