...
#ifdef ARM_TONE_SUPPORT
#define MMISRVAUD_ARM_TONE_SUPPORT
#endif

#ifdef MMISRVAUD_ARM_TONE_SUPPORT
...
{    
    ...
    HAUDIODEV hDevice = 0;
    const uint16* pusDevName = PNULL;
    const uint32 record_nb_frame_length[MMISRVAUD_RECORD_FRAME_LEN_MAX] = {32,27,21,20,18,16,14,13}; 
    const uint32 record_wb_frame_length[MMISRVAUD_RECORD_WB_FRAME_LEN_MAX] = {61,59,51,47,41,37,33,24,18}; 
    MP3_ENC_CFG_T mp3_record_param = {AUD_REC_MP3,RECORD_OUTPUT_SAVE_TO_FILE,0,8000,32000,1,0,0,4000};
    RECORD_PARAM_T record_param = {AUD_REC_PCM, RECORD_OUTPUT_SAVE_TO_FILE, 0, 160, 0,0,0,0,0};
    AMR_SW_ENC_PARAM_T amr_sw_record = {AUD_REC_AMR_SW, RECORD_OUTPUT_SAVE_TO_FILE, 0, 320, 0,0,0,8000,0,8,2};
    ...
    case MMISRVAUD_RECORD_FMT_AMR:
        ...
        {
            amr_sw_record.format = AUD_REC_AMR_SW;
            amr_sw_record.amr_wb_mode = MMISRVAUD_RECORD_FRAME_LEN_MAX - 1 - frame_len;
        }
...
#endif
        {
            is_support = FALSE;/*lint !e831*/
        }

        record_param.frame_length = record_nb_frame_length[frame_len];
        break;
    case MMISRVAUD_RECORD_FMT_AMR_WB:
#ifdef RECORD_SUPPORT
        if(Record_QueryEncoderCapability(AUD_REC_AMR_WB_SW))
        {
            amr_sw_record.format = AUD_REC_AMR_WB_SW;
            amr_sw_record.samplerate = 16000;
        }
        else
#endif
        {
        ...
        record_param.frame_length = record_wb_frame_length[frame_len];
        break;
    case MMISRVAUD_RECORD_FMT_PCM:
        ...
        if(Record_QueryEncoderCapability(AUD_REC_PCM))
        {
            ...
            record_param.samplerate = 16000;// 
            _DBG(MMISRV_TRACE_LOW("[MMISRV]aud drv: CreateRecordFileHandle() entry, record_param.samplerate = 16000"));            
#endif
        }
        else
#endif
...
#ifdef ARMVB_RECORD_SUPPORT
            record_param.samplerate = 8000; //change to 8k for  

            _DBG(MMISRV_TRACE_LOW("[MMISRV]aud drv: CreateRecordFileHandle() entry, record_param.samplerate = 8000"));             
#endif
        }
        else
#endif
 ...
 handle = AUDIO_CreateRecordFileHandle(
    ...
    if(handle > 0)
    {
...
#ifdef ARMVB_RECORD_SUPPORT
        else if((MMISRVAUD_RECORD_FMT_AMR_WB == fmt) ||(MMISRVAUD_RECORD_FMT_AMR == fmt))
        {
            AUDIO_ExeCodecExtOpe(
                handle,
                ANSI2UINT16("SET_RECORDPARAM"),//扩展操作的标志 
                RECORD_AMR_PARAM_SET_COUNT,
                &amr_sw_record,
                &param_result
                );
        }
...
#ifdef ARMVB_RECORD_SUPPORT
#if 1
        if(MMISRVAUD_RECORD_FMT_ADPCM == fmt)
        {
        ...
        if((MMISRVAUD_RECORD_FMT_ADPCM == fmt) || (MMISRVAUD_RECORD_FMT_PCM == fmt) || (MMISRVAUD_RECORD_FMT_AMR_WB == fmt))
        {
        	  _DBG(MMISRV_TRACE_LOW("[MMISRV]aud drv: CreateRecordFileHandle() entry, AUDIO_SetOutputSampleRate = 16000"));
...
LOCAL BOOLEAN PlayTone(DRVAUD_HANDLE handle, MMISRVAUD_TYPE_U *type_ptr)
{
    ...
    if(hArmToneCodec == codec)
    {
        ...
        case MMISRVAUD_TYPE_TONE_GENERIC:
            ...
            tArmTonePara.tone_para_u.generic_para.volume = 16384;
            break;
        case MMISRVAUD_TYPE_TONE_DTMF:
            ...
            tArmTonePara.tone_para_u.dtmf_para.volume = 4096;
            break;
        case MMISRVAUD_TYPE_TONE_CUSTOM:
            ...
            tArmTonePara.tone_para_u.custom_para.freq2 = 0;
            tArmTonePara.tone_para_u.custom_para.freq3 = 0;
            tArmTonePara.tone_para_u.custom_para.volume = 16384;
            break;
        default:
            break;
        }
        ...
        case MMISRVAUD_TYPE_TONE_GENERIC:
            {
               ...
               if ( tone->volume !=2048)
               {
                   tGenericPara.volume = 65535;  // Change 16384 to 65535; for fix bug 505963.
               }
               else
               {
                    tGenericPara.volume = tone->volume;
                }
...
PUBLIC DRVAUD_HANDLE DRVAUD_CreateHandle(MMISRVAUD_TYPE_U *audio_data, BOOLEAN is_a2dp, uint32 *buff, uint32 buff_size)
{
    ...
    case MMISRVAUD_TYPE_FM:    
        ...
        handle = 0xffff;
#else
        handle = AUDIO_CreateNormalHandle(hNullCodec, PNULL, hFMDev, PNULL, AudioDrvCallback);
#endif
        break;
    case MMISRVAUD_TYPE_VIRTUAL:
#ifndef VIRTUAL_VIBRATE_FUNC

      case MMISRVAUD_TYPE_VIBERATE:  // 2013 0827
 #endif
...
PUBLIC BOOLEAN DRVAUD_CloseHandle(DRVAUD_HANDLE handle, MMISRVAUD_TYPE_U *type_ptr)
{
    ...
    if(handle > 0)
    {
        ...
        case MMISRVAUD_TYPE_VIRTUAL:
#ifndef VIRTUAL_VIBRATE_FUNC
        case MMISRVAUD_TYPE_VIBERATE:  // 2013 0828
#endif
            result = AUDIO_NO_ERROR;            
            break;
        case MMISRVAUD_TYPE_IQ:
...
PUBLIC BOOLEAN DRVAUD_Play(DRVAUD_HANDLE handle, MMISRVAUD_TYPE_U *type_ptr, uint32 offset, MMISRVAUD_ROUTE_T route)
{
    ...
    case MMISRVAUD_TYPE_FM:
        ...
        break;
    case MMISRVAUD_TYPE_VOICE_LOOPBACK:
        result = AUD_EnableVoiceLoopback(TRUE, 1, 0, 100);
        break;
#ifdef MMISRVAUD_ARM_TONE_SUPPORT        
    case MMISRVAUD_TYPE_TONE_DTMF:
    case MMISRVAUD_TYPE_TONE_GENERIC:
...
PUBLIC BOOLEAN DRVAUD_GetContentInfo(DRVAUD_HANDLE handle, MMISRVAUD_CONTENT_INFO_T *content_info)
{
    ...
    {
        content_info->codec = fmt_info.hCodecHandle;
...
PUBLIC MMISRVAUD_RING_FMT_E MMISRVAUD_GetAudioTypeAccordingCodec(uint32 audio_codec)
{
    return GetAudioType((HAUDIOCODEC)audio_codec);
}
