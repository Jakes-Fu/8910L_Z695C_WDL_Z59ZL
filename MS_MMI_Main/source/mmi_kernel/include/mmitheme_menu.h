...
typedef struct
{
    ...
    uint16               toggle_text_width;
    uint16               radio_text_width;
#ifdef GUISUBPOPMENU_CONTEXT_SUPPORT
    MMITHEME_PUBWIN_LAYOUT_T    layout;     // layout
#endif

    BOOLEAN             is_centered;        //是否居中显示，对应toolkit spec中的selection menu
    uint8               item_max_num;       //最多显示的item条数，显示不下则显示滚动条
    uint8               gray_mask_alpha;    //gray状态菜单项蒙版透明度
    GUI_COLOR_T         accent_color;       //normal状态菜单项图标颜色
    GUI_COLOR_T         focus_color;        //focus状态菜单项图标颜色accent-3
    
    MMI_IMAGE_ID_T      toggle_on_focus_img[MMITHEME_ACCENT_COLOR_MAX];
    MMI_IMAGE_ID_T      toggle_on_normal_img[MMITHEME_ACCENT_COLOR_MAX];
    MMI_IMAGE_ID_T      toggle_off_focus_img[MMITHEME_ACCENT_COLOR_MAX];
    MMI_IMAGE_ID_T      toggle_off_normal_img[MMITHEME_ACCENT_COLOR_MAX];

    GUISK_DATA_TYPE_E   sk_type[GUISOFTKEY_BUTTON_NUM]; //softkey类型，可lsk、msk、rsk设置不同类型
    uint32              sk_id[GUISOFTKEY_BUTTON_NUM];   //传入的id类型需要与sk_type保持一致

    MMI_TEXT_ID_T       toggle_msk_id;
} MMITHEME_POPMENU_T;
