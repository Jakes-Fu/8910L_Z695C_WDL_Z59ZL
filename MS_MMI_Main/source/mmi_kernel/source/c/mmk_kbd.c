...
#if 1
#define SDK_KEY_DEBOUCE_ENABLE 1
#else
#define SDK_KEY_DEBOUCE_ENABLE 0
#endif
#if SDK_KEY_DEBOUCE_ENABLE
#define y_abs(x)  ((x) >= 0 ? (x) : (-(x)))
#define SDK_KEY_DEBOUCE_TIME_MS  150
static BOOLEAN s_is_camera_test_mode = FALSE;
static BOOLEAN is_mp3_key_down = FALSE;
static uint32 cur_mp3_key_down_time = 0;
LOCAL uint8  s_hall_int_check_timer_id      = 0;
LOCAL void HALL_Int_HandleCheckTimer(
                                            uint8 timer_id,
                                            uint32 param
                                            )
{
     MMIDEFAULT_HandleKeyLock(FALSE);
    is_mp3_key_down = FALSE;
}
LOCAL void HALL_Int_StartCheckTimer(void)
{
    if(0 != s_hall_int_check_timer_id)
    {
        MMK_StopTimer(s_hall_int_check_timer_id);
        s_hall_int_check_timer_id = 0;
    }
    s_hall_int_check_timer_id = MMK_CreateTimerCallback(SDK_KEY_DEBOUCE_TIME_MS, 
                                                                        HALL_Int_HandleCheckTimer, 
                                                                        PNULL, 
                                                                        FALSE);
}
LOCAL void HALL_Int_StopCheckTimer(void)
{
    if(0 != s_hall_int_check_timer_id)
    {
        MMK_StopTimer(s_hall_int_check_timer_id);
...
)
{
    ...
    //SCI_TICK_TIME_T cur_aud_tick_time_info = {0};
    ...
    case KPD_DOWN:
        ...
        if(KEY_SLIDE == key_code)
        {
#if SDK_KEY_DEBOUCE_ENABLE
            HALL_Int_StopCheckTimer();
            cur_mp3_key_down_time = SCI_GetTickCount();
            if(is_mp3_key_down == FALSE)
            {
		  MMIDEFAULT_HandleKeyLock(TRUE);
                is_mp3_key_down = TRUE;
            }
#else
#endif
            return;
        ...
        //SCI_GetTickTime(&cur_aud_tick_time_info);
        //SCI_TRACE_LOW("[xx:key] func[%s] time:[%d][%d]",__FUNCTION__,cur_aud_tick_time_info.second, cur_aud_tick_time_info.milliseconds);
    ...
    case KPD_UP:
        ...
        if(KEY_SLIDE == key_code)
        {
            ...
            if(is_mp3_key_down)
            {
                ...
                if(y_abs(cur_ms - cur_mp3_key_down_time) < SDK_KEY_DEBOUCE_TIME_MS)
                {
                    HALL_Int_StartCheckTimer();
                    return;
                }
		  MMIDEFAULT_HandleKeyLock(FALSE);
                is_mp3_key_down = FALSE;
...
)
{
    ...
    if(KEY_NONE==key_code)
    {
	    SCI_TRACE_LOW("signal_keycode=0x%x, is invalid key",signal_keycode);
	    return FALSE;
    }
    else
    {
...
#else
    if (KEY_HEADSET_DETECT == key_code
#ifdef Z695_KEYLOCK_INT_SUPPORT
        || KEY_FLIP == key_code
        || KEY_SLIDE == key_code
#endif
...
PUBLIC void MMK_InitKBD(void)
{
    ...
    for (i = 0; i < MMI_MAX_MULTI_KEYS; i++)
    {
        s_kbd_down_bak_info.kbd_down_bak[i].timer_id = MMI_KBD_TIMER_ID_1+i;
    }
...
/*****************************************************************************/
// 	Description : is navi up key press
//	Global resource dependence : none
//  Author: 
//	Note:
/*****************************************************************************/
PUBLIC BOOLEAN MMK_IsNaviUpKeyPress(void)
{
    BOOLEAN  result = FALSE;
    uint32   i = 0;
    ...
    {
        if (s_kbd_down_bak_info.kbd_down_bak[i].is_valid
                && KEY_UP == s_kbd_down_bak_info.kbd_down_bak[i].key_code)
        {
            result = TRUE;
            break;
        }
