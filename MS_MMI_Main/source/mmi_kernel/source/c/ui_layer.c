                               ...
                               )
{
    ...
    if (PNULL != layer)
    {
        ...
        if ((PNULL != src_buf) && (PNULL != dst_buf))
        {
            ...
            src_bytes_per_pixel = GetBytesPerPixel(src_data_type);
            dst_bytes_per_pixel = GetBytesPerPixel(dst_data_type);
            if ((rect.left < layer_rect.left) || (rect.top < layer_rect.top))
            {
                return;
            }
                               ...
                               )
{
    ...
    if (PNULL != layer)
    {
        ...
        if ((PNULL != src_buf) && (PNULL != dst_buf))
        {
            ...
            src_bytes_per_pixel = GetBytesPerPixel(src_data_type);
            dst_bytes_per_pixel = GetBytesPerPixel(dst_data_type);
            if ((rect.left < layer_rect.left) || (rect.left < param_ptr->rect.left) 
                || (rect.top < layer_rect.top)|| (rect.top < param_ptr->rect.top))
            {
                return;
            }
