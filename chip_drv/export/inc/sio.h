...
#define VIR_COM3         (9) 
#define VIR_COM4          (10) 
#define MAX_SIO_PORT_NUM (11)
/* added @eddie.li end */

#define MAX_LOGICAL_SIO_PORT_NUM  COM_MAX_PORT_NUM             //'COM_DATA', 'COM_DEBUG'  ''COM_DATA2" COM_DEBUG2"
...
#ifdef PPP_USB
typedef void (* ATC_CALLBACK) (uint32,uint8);
#else
...
typedef struct
{
    ...
    uint32 (*get_txfifocnt) (uint32 phy_port);                     /* get tx fifo cnt */
    ...
    void (*put_chars) (uint32 phy_port, uint8* dataptr, int32 len); /* send chars*/
    uint32 (*get_rxfifocnt) (uint32 phy_port);                     /* get tx fifo cnt */ 
} sio_operation_st;
...
/*output single char per time*/
PUBLIC void    SIO_DumpPutChar (
    uint32 port,
    char   nchar
);

/*output chars per time*/
PUBLIC void SIO_DumpPutChars (
    uint32 port,
    uint8* data_ptr,
    int32 len
    );
...
PUBLIC uint32 SIO_Debug2_PortHdl (uint32 event);
