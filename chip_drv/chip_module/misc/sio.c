...
LOCAL uint8 v_modem_debug_phy_port_flag = 0xFF;
...
#define PPP_RXBUF_SIZE 16384 //16k
...
)
{
    ...
    {
	DPSLP_UART_EnableApbSleep(sio_port[COM_DATA].phy_port, ENABLE_APB_SLEEP);
    }

    if (sio_port[COM_DEBUG].phy_port ==UART0_E ||sio_port[COM_DEBUG].phy_port ==UART1_E)
    {
	DPSLP_UART_EnableApbSleep(sio_port[COM_DEBUG].phy_port, ENABLE_APB_SLEEP);
    }              
}
...
LOCAL  void SIO_UartDma_WriteData (uint32 com_id, uint8 *start_addr, uint32 length)
{
...
#endif
    {
        ...
        UART_DmaWriteData (com_id, start_addr, length, channel);
...
*/
LOCAL uint32 SIO_WritePPPFrameToBuf (uint8 *src, uint32 size,uint32 *written_size)
{
...
)
{
    ...
    if ((POWER_GetMsStatus() != POWER_MS_STATUS_POWER_ON)&&( !(POWER_IsCalibrationResetMode())))
    {
        char format_str[BOOTUP_LOG_MAX_LEN + 11 + 5];
        uint8*buf = format_str;
        uint32 start=0,buf_size =0;
        buf_size=BOOTUP_LOG_MAX_LEN + 11 +5;
        ...
        if (len > BOOTUP_LOG_MAX_LEN)
        {
            len = BOOTUP_LOG_MAX_LEN;
        }
...
{
	SIO_SetModemTracePort(VIR_COM4); 
...
{
     if(v_modem_debug_logic_port != port)
     {

         if(modem_cp_log_semaphore)
	 {
	       v_modem_debug_phy_port_flag = 1;  //port changed
	       v_modem_debug_logic_port = port;

	      if(v_modem_debug_logic_port <= COM3)
	      {
	        v_modem_debug_phy_port=SIO_MODEM_UART;
	      }
	     else if((VIR_COM0 == v_modem_debug_logic_port) || (VIR_COM1 == v_modem_debug_logic_port) \
		 	|| (VIR_COM3 == v_modem_debug_logic_port) \
		 	|| (VIR_COM4 == v_modem_debug_logic_port)) //usb port
	     {
	        v_modem_debug_phy_port=SIO_MODEM_AP_USB;
	      }	
	     else if((DSK_COM0 == v_modem_debug_logic_port) || (DSK_COM1 == v_modem_debug_logic_port)) //sdcord port
	     {
	        v_modem_debug_phy_port=SIO_MODEM_AP_DSK;
	      }
		
		SCI_TraceLow("SIO_SetModemTracePort:logic and phy port 0x%x and 0x%x", v_modem_debug_logic_port,v_modem_debug_phy_port);	 
		   
	 	SCI_PutSemaphore(modem_cp_log_semaphore);
         }
	  else
	  	v_modem_debug_logic_port=0xFF;
     }
}
...
{
   uint8 v_port = SIO_MODEM_PORT_NULL;
    ...
    if(v_modem_debug_phy_port_flag)
   {
	...
	SCI_TraceLow("SIO_ModemTracePortNotify:logic and phy port 0x%x and 0x%x", v_modem_debug_logic_port,v_modem_debug_phy_port);	 
   }
...
void SIO_SendRemainCPLog (void)
{
	...
	if( p_log_buff_info)
	{
		...
		{
			i = 0;
			p_head = (uint8 *)(p_log_buff_info->head << 1);
...
PUBLIC int SIO_SendPacket (void *src, int size)
//PUBLIC int32 SIO_SendWholePacket(void * src, int32 size)
{
    DATA_FRAME_T   *list_ptr = s_sio_ppp_ctrl.frame_list;
    ...
    while (cnt >= max_size)
    {
        ...
        if ( (last_tick - cur_tick) > s_sio_send_wait_time) /*1s*/
        {
			...
			s_sio_packet_lost_count++;
			if(s_sio_packet_series_lost >= SIO_SERIES_LOST_THLD)
			{
				s_sio_send_wait_time = 1;
			}
			else
			{
				s_sio_packet_series_lost++;
			}
			SCI_TRACE_LOW("SIO_SendPacket is full, lost_count=%d,series_lost=%d,wait_time=%d",s_sio_packet_lost_count, s_sio_packet_series_lost,s_sio_send_wait_time);
            return 0;
#endif
        }

        SCI_Sleep (1); //to release some mips for other task
    }

	s_sio_packet_series_lost = 0;
	s_sio_send_wait_time = SIO_DFAULT_WAIT_TIME;
#else
    /*write head flag until we got free space*/
    while (cnt >= max_size)
    {
...
#endif
        ret_val = SIO_WritePPPFrameToBuf ( ( (uint8 *) src+cur_pos) , (uint) size- cur_pos, &written_len);
        cur_pos += written_len;
#ifdef _SIO_DEBUG_
...
PUBLIC int SIO_SendFrame (void *src, int size)
{
    ...
    /*encode , write data, return true size written*/
    written_len = 0;
    ret_val = SIO_WritePPPFrameToBuf ( ( (uint8 *) src) , (uint) size, &written_len);
...
extern void *g_ppp_data_buf;
extern uint32 g_ppp_send_status;
void sio_ppp_tx_enable(int enable_flag)
{
    uint32 *ppp_len;

    if (UCOM_TxFifoIsEmpty(COM_DATA2) && 
        (g_pppbuf_alloc != g_pppbuf_free) &&
        (g_ppp_send_status == 0))
    {
        g_ppp_send_status = 1;
        g_ppp_data_buf = &s_ppp_dma_txbuf[g_pppbuf_free][0];
        ppp_len = (uint32 *)(&s_ppp_dma_txbuf[g_pppbuf_free][2044]);
        udc_hw_start_xfer_(0x82, 512, g_ppp_data_buf, *ppp_len);
    }
}
...
#endif
{
   ...
   uint8 port_index=COM_DATA;
	...
	if(fifo_cnt > sizeof (p_data_txbuff))
	{
		fifo_cnt = sizeof(p_data_txbuff);
	}
	if (fifo_cnt)
	{
	#ifdef PPP_USB
	    ...
	    SCI_PPP_TRACE_1("ppp:SIO_Tx_Send,port %d,num%d,32K 0x%x",port_index,num,SLEEP_GetCur32kNum());
           if (num > 0)
	    {
...
LOCAL uint32 SIO_RxForUserPort (uint8 *data_buf,uint32 rec_num,uint8 port_index)
{
   ...
   //SCI_PPP_TRACE("ppp:SIO_RxForUserPort,port%d",port_index);
	...
	{
          #if 0
	    if (MIX_MODE == user_port_mode[port_index])
	    {
#ifdef _U0_FLOW_CONTROL

	        if (ring_space (& (sio_port[port_index].rx_buf)) <= EMPTY_SPACE_WATER_LOW_MARK)
	        {
          ...
          #endif
	    {
	        ...
	        if ( (MAX_PLUS_NUMBER >= rec_num)
	                && (MODE_CHANGE_DELAY < (cur_rx_time - last_rx_time_2))
	                && DATA_MODE == user_port_mode[port_index])
	        {
	            uint32 i;
	            ...
	            for (i = 0; i < MAX_PLUS_NUMBER; i++)
	            {
	                ...
	                if (plus_char[i] == XON)
	                {
	                    gRemoteXon_on[port_index] = 1;
	                }
	                else if (plus_char[i] == XOFF)
	                {
	                    gRemoteXon_on[port_index]= 0;
	                }
	            ...
	            if (MAX_PLUS_NUMBER == plus_num_temp)
	            {
	                ...
	                SCI_DeactiveTimer (ATC_timer[port_index]);
	                SCI_ChangeTimer (ATC_timer[port_index], ATC_CheckPPP, MODE_CHANGE_DELAY + 100); // Add 100 to make the timer response longer, so that the timeout logic will be OK.
	                SCI_ActiveTimer (ATC_timer[port_index]);
	            }
	            else
	            {
        ...
        case EVENT_WRITE_COMPLETE: //send data to uart or vcom
            {
                ...
                if (fifo_cnt)
                {
                    ...
                    //add log for bug1006386
			SCI_PPP_TRACE("ppp:SIO_DataPortHdl,num%d ",num);			
                    if (num > 0)
                    {
...
LOCAL uint32 SIO_DataPortHdl_2 (uint32 event)
{
    ...
    SCI_PPP_TRACE_1("ppp:SIO_DataPortHdl_2,event%d,mode %d,32K 0x%x",
    ...
    switch (event)
    {
        case EVENT_DATA_TO_READ:   //receive data from uart or vcom
            {
                ...
                while (fifo_cnt)
                {
                    if (user_port_mode_2 == ATC_MODE)
                    {
                        num = (sio_port[COM_DATA2].sio_op.read)(port_num, s_uart_data2_rxbuf, 1);
                        if (num > 0)
                        {
                ...
                if (fifo_cnt && (user_port_mode_2 == DATA_MODE))
                {
                    ppp_rxbuf = &s_ppp_rx_buf[g_ppp_rx_widx];
                    ...
                    else
                    {
                        ...
                        if (num > 0)
                        {
                            ...
                            num = (sio_port[COM_DATA2].sio_op.read)(port_num, s_ppp_rx_buf, fifo_cnt - ppp_rxbuf_tail);
                            if (num > 0)
                            {
                                g_ppp_rx_widx += num;
                                g_ppp_rx_widx &= PPP_RXBUF_MASK;
        ...
        case EVENT_WRITE_COMPLETE: //send data to uart or vcom
            {
                fifo_cnt = SIO_GetTxFifoCnt (COM_DATA2);
                if (fifo_cnt > sizeof (s_uart_data2_txbuf))
                {
                	fifo_cnt = sizeof(s_uart_data2_txbuf);
                }
                if (fifo_cnt)
                {
                    num = SIO_TxForUserPort (s_uart_data2_txbuf,fifo_cnt,COM_DATA2);

                    //add log for bug1006386		
                    if (num > 0)
                    {   
                        SCI_PPP_TRACE_1("ppp: 111 write num %d,32K 0x%x", num, SLEEP_GetCur32kNum());
                        (sio_port[COM_DATA2].sio_op.write) (SIO_PhyPortfromIndex (phy_port),s_uart_data2_txbuf,num);
                    }
                    else
                    {
                        //when using usb virtual com, then should disable it(not closing intbits in usb isr handle)
                        //it will affect nothing to disable double times for uart,
                        SIO_Tx_Int_Enable (COM_DATA2,FALSE);
                    }
                }
                else
                {
                    ...
                    //is more than max UART_FIFO_SIZE,then only enable uart TX
                    //if current is usb, fifo_cnt will not be zero
                    SIO_Tx_Int_Enable (COM_DATA2,TRUE);
                }
            }
            break;
        case EVENT_INIT_COMPLETE:
...
#elif defined(CHIP_VER_UIX8910) || defined(CHIP_VER_UIS8910C) || defined(CHIP_VER_UIS8910A) || defined(CHIP_VER_UIS8910FF) 
    uart_st.tx_watermark  = TX_WATER_MARK; // 0~127B
    uart_st.rx_watermark  = UART_RxWaterMark(); // 0~127B
