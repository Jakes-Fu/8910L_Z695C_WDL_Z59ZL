...
/*****************************************************************************/
//  Description:    The function is used to save assert Info to interal shared memrory.
//  Global resource dependence:
//  Author:         Johnson.sun
//  Note:
/*****************************************************************************/
PUBLIC void  _TXAS_SaveAssertToIRAM (const char *file, int line, const char *exp)
{
    unsigned char  buf[MAX_ASSERT_BUF];
    uint16  len;
    uint16         CurTaskID;
    ...
    IRAM_buf *shared_mem = (IRAM_buf *) CHIP_GetAssertInfoBufBase();

#if !defined(PLATFORM_UIX8910)
    TM_GetTime (&time);
#endif
    VersionInfo = COMMON_GetProjectVersionInfo();
    CurTaskID = SCI_IdentifyThread();
    ...
    file = _TXAS_ParseFileName (file);

#if !defined(PLATFORM_UIX8910)
    //Build the assert information.
    len = sprintf ( (char *) buf, "%s\r\nAssert:%s(%d)(%s)\r\nCurrent Task ID:%2d\r\nDay:%4d-%2d-%2d\r\nTime:%2d-%2d-%2d",
                    VersionInfo,          //software build information.
                    file, line, exp,    //assert location and the express.
                    CurTaskID,         //current task ID
                    (uint16) time.tm_year, (uint8) time.tm_mon, (uint8) time.tm_mday, //  day
                    (uint8) time.tm_hour, (uint8) time.tm_min, (uint8) time.tm_sec);//lint !e144 //   time
#else
    //Build the assert information.
    len = sprintf ( (char *) buf, "%s\r\nAssert:%s(%d)(%s)\r\nCurrent Task ID:%2d\r\n",
                    VersionInfo,          //software build information.
    ...
    curNVID = _TXAS_SearchCurAssertID();  //curNVID is next NV id number.

    if (curNVID > 999)
    {
        curNVID = 0;  //begin a new cycle.
    }

    shared_mem->assert_start_flag = (uint16) ASSERT_START_FLAG;
    shared_mem->cur_nv_id = (uint16) curNVID;
    shared_mem->len = (uint16) len;

    for (i=0, j=0; i < len;)
    {
        data = (uint16) (buf[i]) | ( (uint16) (buf[i+1]) << 8);
        shared_mem->buf[j++] = data;
        i += 2;
    }
}
...
)
{
    ...
    if ( (ASSERT_MAX_COUNT < s_assert_cnt) && (SCI_ASSERT_DEBUG_MODE == assert_mode))
    {
    ...
    if (OSPARAM_GetRecordAssertInforToFlashFlag()) //open the switch.
    {
        _TXAS_SaveAssertToIRAM (file, line, exp);    //@Johnson.sun Save assert Info to shared memory 2006/11/13
    }
...
#if defined(PLATFORM_UIX8910)     //need to set some addr to MEM_TYPE_DEVICE before dump
...
/*****************************************************************************/
//  Description:    The function is used to save watchdog reset Info to
//                  interal shared memrory.
//  Author:         binggo zhou
//  Global resource dependence:
//  Note:
/*****************************************************************************/
PUBLIC void  SCI_SaveWtddogResetFlagToIRAM (void)
{
    IRAM_buf *shared_mem = (IRAM_buf *) CHIP_GetAssertInfoBufBase();

    //Just Record Watchdog Flag
    shared_mem->assert_start_flag = (uint16) WATCHDOG_RESET_FLAG;
}

/*****************************************************************************/
//  Description:    The function is used to write assert Info from shared memory to NV.
//  Global resource dependence:
//  Author:         Johnson.sun
//  Note:
/*****************************************************************************/
PUBLIC void  SCI_WriteAssertInfoToNv (void)
{
    char   *buf;
    uint16 len;
    uint16 test_code;
    ...
    test_code = shared_mem->assert_start_flag;
    curNVID = shared_mem->cur_nv_id;
    len = shared_mem->len + 2; //Add 2 for saving curNVID value.

    if (test_code != (uint16) ASSERT_START_FLAG)
    {
        return;
    }

    if (len > 1024)  //used for defending some dirty data.
    {
        return;
    }

    if (len == 0)  //used for defending some dirty data.
    {
        return;
    }
    ...
    i = 0;
    j = 0;
    buf = (char *) SCI_ALLOC_APP (MAX_ASSERT_BUF+2);
    buf[i++] = curNVID & 0xff;
    buf[i++] = (curNVID & 0xff00) >> 8;

    do
    {
        data = shared_mem->buf[j++];
        data_low = data & 0xff;
    ...
    //Add end-charater of string.
    buf[i++] = '\0';
    len = len + 1;

    // Wrtite to Nvitem.
    _TXAS_WriteAssertInfoToNV ( (uint8 *) buf, len);
    SCI_FREE (buf);
}
...
/*****************************************************************************/
//  Description:    The function is used to write watchdog reset Info to NV.
//  Global resource dependence:
//  Author:         binggo zhou
//  Note:
/*****************************************************************************/
PUBLIC void  SCI_WriteWatchdogResetInfoToNv (void)
{
    char   *nvbuf;
    const  char    *VersionInfo;
    uint16 len;
    ...
    if (test_code != (uint16) WATCHDOG_RESET_FLAG)
    {
        return;
    }
    ...
    nvbuf = (char *) SCI_ALLOC_APP (MAX_ASSERT_BUF+3);

    //Build the watchdog reset information.
    len = snprintf ( (char *) (nvbuf+2), (uint32) "%s\r\nWatchdogReset:\r\nTime:%4d-%2d-%2d",
                     VersionInfo,          //software build information.
                     (uint16) time.tm_year, (uint8) time.tm_mon, (uint8) time.tm_mday); //time
    ...
    SCI_ASSERT (len <= MAX_ASSERT_BUF);/*assert verified*/
    ...
    len = len + 2; //Add 2 for saving curNVID value.

    nvbuf[0] = curNVID & 0xff;
    nvbuf[1] = (curNVID & 0xff00) >> 8;
    ...
    // Wrtite to Nvitem.
    _TXAS_WriteAssertInfoToNV ( (uint8 *) nvbuf, len);
    SCI_FREE (nvbuf);
}
