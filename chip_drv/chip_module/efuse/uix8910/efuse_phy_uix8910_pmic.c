...
#define IS_ERR_VALUE(x)		((x) >= (unsigned long)-MAX_ERRNO)?1:0
#define DIV_ROUND_UP(n,d)	(((n) + (d) - 1) / (d))

/* registers definitions for controller ANA_REGS_EFUSE */
#define ANA_REG_EFUSE_GLB_CTRL          (ANA_REGS_EFUSE_BASE + 0x0000)
#define ANA_REG_EFUSE_DATA_RD           (ANA_REGS_EFUSE_BASE +0x0004)
#define ANA_REG_EFUSE_DATA_WR           (ANA_REGS_EFUSE_BASE + 0x0008)
#define ANA_REG_EFUSE_BLOCK_INDEX      (ANA_REGS_EFUSE_BASE +0x000c)
#define ANA_REG_EFUSE_MODE_CTRL         (ANA_REGS_EFUSE_BASE +0x0010)
#define ANA_REG_EFUSE_STATUS            (ANA_REGS_EFUSE_BASE + 0x0014)
#define ANA_REG_EFUSE_WR_TIMING_CTRL    (ANA_REGS_EFUSE_BASE + 0x0020)
#define ANA_REG_EFUSE_RD_TIMING_CTRL    (ANA_REGS_EFUSE_BASE + 0x0024)
#define ANA_REG_EFUSE_EFUSE_DEB_CTRL    (ANA_REGS_EFUSE_BASE + 0x0028)
...
#define ANA_EFS_BASE		(SPRD_ADISLAVE_PHYS + 0x300)

#define ANA_REGS_EFUSE_BASE ANA_EFS_BASE
#define EFUSE_BLOCK_MAX                 ( 32 )
#define EFUSE_BLOCK_WIDTH               ( 16 )	/* bit counts */
#define VALUE_TYPE	uint16

LOCAL VALUE_TYPE dummy_cached_otp[EFUSE_BLOCK_MAX] = {
...
LOCAL void pmic_efuse_power_on(void)
{
	ANA_REG_MSK_OR(ANA_REG_GLB_MODULE_EN0, BIT_ANA_EFS_EN,BIT_ANA_EFS_EN);
}

LOCAL void pmic_efuse_power_off(void)
{
	ANA_REG_MSK_OR(ANA_REG_GLB_MODULE_EN0, ~BIT_ANA_EFS_EN,BIT_ANA_EFS_EN);
}

LOCAL __inline int pmic_efuse_wait_clear(uint32 bits)
{
	int ret = 0;
	...
	SCI_TRACE_LOW("pmic efuse wait %x\n", ANA_REG_GET(ANA_REG_EFUSE_STATUS));
	SCI_Sleep(2);
	//udelay(2000);
	/* wait for maximum of 3000 msec */
	timeout = jiffies + msecs_to_jiffies(300000);
	while (bit_status) {
		val = ANA_REG_GET(ANA_REG_EFUSE_STATUS);
		bit_status = (~val) & bits;

		if (time_after(jiffies, timeout)) {
			SCI_TRACE_LOW("pmic efuse wait clear timeout !!!\n");
			ret = -1;
			break;
		}
	}
...
PUBLIC uint32 sprd_pmic_efuse_read(int blk_index)
{
	uint32 val = 0;
	...
	ANA_REG_SET(ANA_REG_EFUSE_BLOCK_INDEX,
...
out:
	...
	SCI_TRACE_LOW("pmic efuse read %d,val=0x%x\n", blk_index,val);
	return val;
}

PUBLIC uint32 sprd_pmic_efuse_read_bits(int bit_index, int length)
{
	...
	val &= BIT(length) - 1;
	SCI_TRACE_LOW("pmic efuse read bits %d ++ %d 0x%08x\n\n", bit_index, length, val);
	return val;
}
