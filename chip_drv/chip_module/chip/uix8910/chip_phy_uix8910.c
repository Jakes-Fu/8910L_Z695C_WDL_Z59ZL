...
volatile  uint32 a_auto_sms_buff[256]={0,};
...
#define    APLL_FREQ    (1000000000)      /*APLL freq,relation to hwp_analogReg->SDM_APLL_reg1*/
/*****************************************************************************/
...
uint32 CHIP_GetArmClk (void)
{
    uint32 div;
    uint32 arm_clk;
    uint16 regdiv_div[16] = {240, 160, 120, 80, 70, 60, 55, 50, 45, 40, 35, 30, 25, 20, 240, 10};
    
    div = hwp_sysCtrl->CFG_PLL_AP_CPU_DIV & 0xF;
    arm_clk = (APLL_FREQ / regdiv_div[div]) * 10;
...
{
    uint32 div;
    uint32 axiclk, ahb_clk;
    uint16 regdiv_div[16] = {240, 160, 120, 80, 70, 60, 55, 50, 45, 40, 35, 30, 25, 20, 240, 10};
    
    div = hwp_sysCtrl->CFG_PLL_SYS_AXI_DIV & 0xF;
    axiclk = (APLL_FREQ / regdiv_div[div]) * 10;

    div = hwp_sysCtrl->CFG_PLL_SYS_AHB_APB_DIV & 0xF;
    ahb_clk = (axiclk / regdiv_div[div]) * 10;
...
PUBLIC const char *CHIP_GetChipInfo (void)
{
	...
	if(chip_info[i].chip_id == CHIP_ID_UIS8910 )
	{
		if(chip_info[i].chip_id == (chip_id&0xffff0000))
		{
			 info = (const char *) chip_info[i].info;
            		break;
		}
...
PUBLIC void CHIP_MODEM_CP_Start(void)
{
       ...
       hwp_idle->IDLE_RES2 = 0xC8DACAFE;
	   hwp_idle->IDLE_RES3 = v_cp_jump_addr;   
       hwp_sysCtrl->CPCPU_RST_CLR = 0xF; //release cp core
#endif
