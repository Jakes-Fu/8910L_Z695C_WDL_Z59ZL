...
//添加标志 
//指示是CP睡眠时间太短不进入深睡眠还是CP真实未睡眠不进入深睡流程
...
LOCAL void SLEEP_WCNOSC_EnableDeepSleep(void)
{
    ...
    if (BT_IsEnabled())
    {
        ...
        curr_32k_tick = hwp_idle->IDL_32K_REF;//��ǰ��AP��32K����//���������ڲ�ѯ��(hwp_pwrctrl->WCN_LPS & CP_PWRCTRL_WCN2SYS_OSC_EN) == 0���ٻ�ȡ32K���п���32K�պñ��趨��˯��ʱ�䳤��       //g_bt_sleep_time��1��32K�ļ��޳�����
        ...
        {
            curr_32k_tick = (uint32)(((uint64)curr_32k_tick + IDL_REF_32K_MASK - (uint64)g_bt_sleep_time_ap32k)%IDL_REF_32K_MASK);//����Ѿ���ȥ��ʱ��

            {
                if ((g_bt_sleep_time - curr_32k_tick) > 230)// 230 32K_ticks means about 6.8ms //����Ƿ�С��6.8ms,С��6.9ms��AP��˯��
                {
                    sleep_active_modules &= (~SLEEP_DRV_WCN_OSC);
...
#if 0     /*��lowpower enable�����ŵ�bootloader�л��ɿ�ʱ��֮ǰ*/
