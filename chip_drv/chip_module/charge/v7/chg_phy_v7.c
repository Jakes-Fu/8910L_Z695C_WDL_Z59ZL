...
{
    ANA_REG_OR (ANA_CHGR_CTL0, (CHGR_CC_EN));
}
...
PUBLIC void CHG_PHY_SetSwitchoverPoint (CHG_SWITPOINT_E eswitchpoint)
{
    ...
    ANA_REG_MSK_OR (ANA_CHGR_CTL0, (eswitchpoint<<CHGR_SW_POINT_SHIFT), CHGR_SW_POINT_MSK);
}
...
PUBLIC uint32 CHG_PHY_UpdateSwitchoverPoint (BOOLEAN up_or_down)
{
    ...
    chg_switchpoint = (ANA_REG_GET (ANA_CHGR_CTL0) & CHGR_SW_POINT_MSK) >> CHGR_SW_POINT_SHIFT;
    ...
    ANA_REG_MSK_OR (ANA_CHGR_CTL0, (chg_switchpoint<<CHGR_SW_POINT_SHIFT), CHGR_SW_POINT_MSK);
...
PUBLIC void CHG_PHY_SetChargeEndVolt(uint32 mv)
{
    ...
    ANA_REG_MSK_OR(ANA_CHGR_CTL0, (b << CHGR_END_V_SHIFT), CHGR_END_V_MSK);
}
...
PUBLIC void CHG_PHY_SetVChgOvpVolt(uint32 mv)
{
    ...
    ANA_REG_MSK_OR(ANA_CHGR_CTL1, (b << VCHG_OVP_V_SHIFT), VCHG_OVP_V_MSK);
}

extern uint32 SCI_Get_HW_VERSION(void);
...
PUBLIC void CHG_PHY_SetChgCurrent (uint16 current)
{
    ...
    if(0x02 == (SCI_Get_HW_VERSION()&0x02)) // the HW is different, the DVT phone's current is 1/2.2 of the 8910FF
    {
...
//current = CHARGER_CURRENT_700MA;
    ...
    {
        case CHARGER_CURRENT_300MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0x0 << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_350MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0x1 << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_400MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0x2 << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_450MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0x3 << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_500MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0x4 << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_550MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0x5 << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_600MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0x6 << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_650MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0x7 << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_700MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0x8 << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_750MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0x9 << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_800MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0xA << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_900MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0xB << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_1000MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0xC << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_1100MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0xD << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_1200MA:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0xE << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        case CHARGER_CURRENT_MAX:
            ANA_REG_MSK_OR (ANA_CHGR_CTL1, (0xF << CHGR_CHG_CUR_SHIFT), CHGR_CHG_CUR_MSK);
            break;
        default:
            SCI_ASSERT (0);/*assert verified*/
    }
...
PUBLIC void CHG_PHY_SetChargeEndCurrent(uint32 ma)
{
    ...
    b = (ANA_REG_GET(ANA_CHGR_CTL1) & CHGR_CHG_CUR_MSK) >> CHGR_CHG_CUR_SHIFT;
    ...
    ANA_REG_MSK_OR (ANA_CHGR_CTL0, (b << CHGR_ITERM_SHIFT), CHGR_ITERM_MSK);
}
...
{
    return ((ANA_REG_GET(ANA_MIXED_CTRL1) & BATDET_OK_MASK) >> BATDET_OK_SHIFT);
}
...
PUBLIC CHGMNG_ADAPTER_TYPE_E CHG_PHY_IdentifyAdpType (void)
{
    ...
    CHGMNG_ADAPTER_TYPE_E               type = CHGMNG_ADP_UNKNOW;

    if (  0 == CHG_PHY_IsChargerPresent() )
    {
        return CHGMNG_ADP_UNKNOW;
    }

    //enable USB LDO
    if ( !LDO_IsLDOOn(LDO_LDO_VDD33) )
    {
...
//    hwp_analogReg->usb_reg2 = 0X478;
    hwp_analogReg->usb_reg2 |=  (ANALOG_REG_USB_PU_IREF | ANALOG_REG_USB_PU_PKD | ANALOG_REG_USB_PU_OTG
                                | ANALOG_REG_USB_PU_1V2 | ANALOG_REG_USB_PU_1V8);

    hwp_analogReg->usb_reg2 &=  (~ANALOG_REG_USB_PU_USB_DEV)
                              & (~ANALOG_REG_USB_PWR_ON)
                              & (~ANALOG_REG_USB_PU_HSRX)
                              & (~ANALOG_REG_USB_PU_HSTX)
                              & (~ANALOG_REG_USB_PU_LPTX);

    /* PU 1.2/ 1.8 need delay at least 1us */
    OS_TickDelay(1);



    hwp_analogReg->usb_reserved   = 0x0028;

    hwp_analogReg->usb_reg4 &= (~(ANALOG_REG_USB_RES_TERM_BIT(3)));

    hwp_analogReg->usb_reg3 &=  (~ANALOG_REG_USB_EN_PATTERN);

    hwp_analogReg->usb_reg3 |= (ANALOG_REG_USB_LOOPBACK);

    hwp_analogReg->usb_reg3 |= (ANALOG_REG_USB_DET_EN);

    /* det en need delay at least 120us, or usb mon wont be right */
    OS_TickDelay(1);


    usb_mon         = hwp_analogReg->usb_mon;

    /* dp_chr == 1 && dm_chr == 1, is pc;
     * dp_chr == 0 && dm_chr == 0, is ac charge */

    dp_chr = (usb_mon & ANALOG_REG_USB_DP_CHR);
    dm_chr = (usb_mon & ANALOG_REG_USB_DM_CHR);

    if  ( (0 == dp_chr)
       && (0 == dm_chr) )
    {
        type = CHGMNG_ADP_STANDARD;
    }
    else if  ( (0 != dp_chr)
            && (0 != dm_chr) )
    {
...
//    SCI_TRACE_LOW("CHG_PHY_IdentifyAdpType usb_reg2=%x, usb_reserved=%x, usb_reg4=%x usb_reg3=%x",
//        hwp_analogReg->usb_reg2, hwp_analogReg->usb_reserved, hwp_analogReg->usb_reg4, hwp_analogReg->usb_reg3);

    SCI_TRACE_LOW("CHG_PHY_IdentifyAdpType type=%d, dp_chr=%d, dm_chr=%d", type, dp_chr, dm_chr);
