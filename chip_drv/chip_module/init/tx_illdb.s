...
;// Check whether unexpectly pc reset
    ...
    LDR     r12, [pc, #EXCEPTION_MODE_FLAG-.-8] 
    LDR     r12, [r12]                      ; Load EXCEPTION_MODE_FLAG into r12
    LDR     r10, =0xFFF
    AND     r12, r12, r10                   ; Compare all flags, r12 &= 0xFFF

; This operation may not right if the cause of assert is ADI or Analog Die error
; ADI is ready to be used after reset.
    LDR     r11, [pc, #HWRST_ADD-.-8]
    LDR     r11, [r11]				        ; get HWRST status reg addr
    LDR     r10, [pc, #ADI_ARM_RD_CMD-.-8]
    LDR     r10, [r10]                      ; get adi read cmd reg addr
    STR     r11, [R10]				        ; send read command through cmd reg

    LDR     r11, [pc, #ADI_RD_DATA-.-8]
    LDR     r10, [r11]                      ; get adi read data reg addr

__adi_read_loop          
    LDR     r11, [r10]                      ; read status
    TST     r11, #0x80000000                ; check the MSB(Rd_cmd_busy) status
    BNE     __adi_read_loop                 ; wait while Rd_cmd_busy == 0
    
    LDR     r10, =0xF1F
    AND     r11, r11, r10                   ; All flags
...
ARM9_CORE_PD_FLAG   EQU     0x10000
    ...
    CMP    r12, r11
    BNE    __arm_mode_error
 
    IMPORT  RECOVERY_ARM_REG    
    
    LDR     R5, =AHB_SYS_CTL2
    LDR     R3, [R5]
    AND     R4, R3, #ARM9_CORE_PD_FLAG   
    CMP     R4, #ARM9_CORE_PD_FLAG
    BEQ     RECOVERY_ARM_REG
    ...
    LDR     r12, [r12]                      ; Load EXCEPTION_MODE_FLAG into r12
    LDR     r10, =0xFF
    AND     r12, r12, r10                   ; Compare all flags, r12 &= 0xFF
...
; ADI is ready to be used after reset.
    ...
    LDR     r10, =0xFFF
    AND     r11, r11, r10
    LDR     r10, [pc, #ADI_ARM_RD_CMD_ADDR-.-8]
    LDR     r10, [r10]                      ; get adi read cmd reg addr
    STR     r11, [R10]                      ; send read command through cmd reg

    LDR     r11, [pc, #ADI_RD_DATA_ADDR-.-8]
    LDR     r10, [r11]                      ; get adi read data reg addr
...
ARM9_CORE_PD_FLAG   EQU     0x1
    ...
    MOV     r10, #0                          ; Clear stack limit register
...
__pure_assert
    ...
    B       __PureAssert                     ; Software reset caused by setting
                                             ; PC to 0 unexpectly


    IF (:DEF:ARM_CORE_POWER_DOMAIN) 
    
    IMPORT   __ArmModeAssert
...
__skip_clock_set
	IF (:DEF:SECURE_BOOT_SUPPORT)
	;;LDR  R1, =JTAG_REENABLE_CTL
	;;LDR  R2, [R1]
	;;ORR  R2, R2, #0x1
	;;STR  R2, [R1, #0]
	ENDIF	
...
;// Init SVC mode stack at the first time
    ...
    ; Only for 6530 dual cs spi-flash CS1 space config 
    IF (:DEF:FLASH_TYPE_SPI)
    BL  SPIFLASH_SetCS1StartAddr
    ENDIF
...
;// Init SVC mode stack at the first time
    MOV     a1, #SVC_MODE                   ; Build SVC mode CPSR
    MSR     CPSR_fsxc, a1                   ; Enter SVC mode
    LDR     a3, [pc, #TXILLDB_SVC_STACK-.-8]; Pickup SVC mode stack address
    LDR     a2, [a3, #0]                    ; Pick the const value from memory
    BIC     a2, a2, #7                      ; Ensure 8 byte  alignment
    SUB     sp, a2, #8
...
;   Save all current registers.
    ...
    IF (:DEF:PLATFORM_SC6530) :LOR: (:DEF:PLATFORM_SC6531EFM) :LOR: (:DEF:PLATFORM_UIX8910)
    LDR     r12, =_tx_abort_stack_start
    LDR     r12, [r12, #0]
    CMP     sp, r12
    BLS     __reset_prefetch_handle_stack
    LDR     r12, =_tx_abort_stack_end
    LDR     r12, [r12, #0]
    CMP     sp, r12
    BLS     __do_prefetch_exception_handle
__reset_prefetch_handle_stack  
    LDR     r12, =_tx_abort_stack_end    ;switch SVC Stack To Exception Handle Stack
    LDR     sp, [r12, #0]
    ENDIF
...
;   Save all current registers.
	...
	IF (:DEF:PLATFORM_SC8800H) :LOR: (:DEF:PLATFORM_SC6800H) :LOR: (:DEF:PLATFORM_SC8800G) :LOR: (:DEF:PLATFORM_SC6530) :LOR: (:DEF:PLATFORM_SC6531EFM) :LOR: (:DEF:PLATFORM_UIX8910)
    ...
    LDR     r12, [r12, #0]
    CMP     sp, r12
    BLS     __reset_abort_handle_stack
    LDR     r12, =_tx_abort_stack_end
    LDR     r12, [r12, #0]
    CMP     sp, r12
    BLS     __do_abort_exception_handle
__reset_abort_handle_stack  
    LDR     r12, =_tx_abort_stack_end    ;switch SVC Stack To Exception Handle Stack
    LDR     sp, [r12, #0]
    ENDIF
