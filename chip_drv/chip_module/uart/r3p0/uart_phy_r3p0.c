...
#define UART2_TX_REQ          (28)
#define UART2_RX_REQ          (29)
...
#define UART2_TX_REQ_ADDR     (DMA_CTL_REG_BASE+0x2000+(UART2_TX_REQ-1)*0x04)
#define UART2_RX_REQ_ADDR     (DMA_CTL_REG_BASE+0x2000+(UART2_RX_REQ-1)*0x04)
        ...
        case UART_COM2:
            {
                if (enable)
                {
                    CHIP_REG_SET (APB_EB0_SET, UART2_EB_SET);
                }
                else
                {
                    CHIP_REG_SET (APB_EB0_CLR, UART2_EB_CLR);
                }
                                  ...
                                  UART_INIT_PARA_T *para)
{
    ...
    {
        p->flow_ctl = (para->rx_watermark-1) | RECEIVE_TIMEOUT  // Receive time out value
...
void UART_PHY_SetBaudSpeed (uint32 port, uint32 baudrate)
{
    ...
    if (UART_COM2 == port)
    {
        REG32 (APB_CLK_CFG0) |= CLK_UART2_SEL;
        apbclk = ARM_CLK_104M;
        divider = apbclk/baudrate;
    }
    else
    {
