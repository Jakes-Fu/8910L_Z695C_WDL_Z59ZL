...
#include "sc6531efm_reg_analog.h"
...
void LDO_TestInit(void)
{
    ...
    LDO_TurnOffLDO(LDO_LDO_SD);
    LDO_TurnOffLDO(LDO_LDO_SDIO);
    LDO_SetVoltValue(LDO_LDO_SD, 3000);
    LDO_SetVoltValue(LDO_LDO_SDIO, 3000);
...
#endif
#if defined(CHIP_VER_UIS8910C) || defined(CHIP_VER_UIS8910FF)     //psram ldo_vddr12 not used,so close
#if !defined(MEM_TYPE_DDR)
    LDO_TurnOffLDO(LDO_LDO_DDR12); 
#endif
#endif
    ANA_REG_OR(ANA_CHGR_DET_FGU_CTRL, 0x1000);    //FGU pd mode
}

void LDO_DcdcCoreDeepSleepSet(void)
{
	ANA_REG_OR(ANA_SLP_DCDC_PD_CTRL, 0x8);
	ANA_REG_SET(ANA_LDO_CORE_SLP_CTRL0, (LDO_CORE_SLP_IN_STEP_EN | (0x10 << LDO_CORE_SLP_STEP_VOL_SHIFT) | (4 << LDO_CORE_SLP_STEP_NUM_SHIFT) | (0x3 << LDO_CORE_SLP_STEP_DELAY_SHIFT)));
	ANA_REG_SET(ANA_LDO_CORE_SLP_CTRL1, (0xE0 << LDO_CORE_V_DS_SHIFT));
...
PUBLIC LDO_ERR_E LDO_TurnOnLDO (LDO_ID_E ldo_id)
{
	LDO_CTL_PTR ctl = NULL;
#if defined(PLATFORM_UIX8910)
     if(((ldo_id>=LDO_LDO_SIM0) &&(ldo_id<=LDO_LDO_SIM3)))
...
#if defined(PLATFORM_SC6531EFM)  //   || defined(PLATFORM_UIX8910)
	CAL_LDO_PTR ldo_cali_info = Cal_GetLdoCtl(ldo_id);
#endif
	...
	#if  !defined(PLATFORM_SC6531EFM) && !defined(PLATFORM_UIX8910)
		LDO_REG_OR(ctl->bp_rst_reg,ctl->bp_rst);
	#endif
#endif
	}
...
#if defined(PLATFORM_SC6531EFM)  //  || defined(PLATFORM_UIX8910)
// after turn on ldo, calibrate ldo according to cali_done && cali_en && adc_cali_build
#if defined(LDO_CALIBRATION_SUPPORT)
	if(adc_cali_build)
	{
		if(ldo_cali_info->if_cali_en && !(ldo_cali_info->cali_done))
		{
...
#else
	#if  !defined(PLATFORM_SC6531EFM) && !defined(PLATFORM_UIX8910)
		LDO_REG_AND(ctl->bp_rst_reg, ~ctl->bp_rst);
	#endif
...
PUBLIC void LDO_DeepSleepInit (void)
{
    ...
    LDO_REG_OR (ANA_SLP_CTRL, SLP_LDO_PD_EN);    //all ldo/dcdc powerdown enable in deep sleep mode
    ...
    for (i=0; slp_ldo_ctl_tab[i].id != SLP_LDO_MAX; i++)
    {
	 ...
	 ANA_REG_SET(ANA_SLP_LDO_LP_CTRL0, 0x7fff);
	 ANA_REG_SET(ANA_SLP_LDO_LP_CTRL1, 0xf);
	 ANA_REG_SET(ANA_SLP_DCDC_LP_CTRL, 0x13);
     
#if defined(POP_MDY_SUPPORT)
       ANA_REG_OR (0x180, (0xC000));    /*output by soft*/
       ANA_REG_SET (0xdf8, (0xba0b));    /*kbled_ctrl0*/
       ANA_REG_SET (0xdfc, (0x8040));    /*kbled_ctrl1*/
       OS_TickDelay(600);
       ANA_REG_AND(0xdfc, (0x7fff));   /*ldo on*/
#endif
#endif
}
...
LOCAL void __calibrate_ldo(CAL_LDO_PTR cal_ldo_tab)
{
	...
	{
		if(LDO_REG_GET(ANA_LDO_SF_REG0) & LDO_SF_OPTION_O)
		{
			//if ldo sf is 3v, modify ldo sf configure table. the default configuration is 1.8v
			ldo_cali_info->default_gear = 0xB4;
			ldo_cali_info->default_vol = VDD3V0_DEFAULT;
			ldo_cali_info->current_vol = VDD3V0_DEFAULT;
			
			ANA_REG_MSK_OR(ldo_cali_info->reg, (ldo_cali_info->default_gear << ldo_cali_info->shift_bit), ldo_cali_info->mask_bit);
		}
	}
