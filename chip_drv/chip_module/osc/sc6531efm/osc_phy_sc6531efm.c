	...
	if(isEnable)
	{
		switch(clk_src)
		{
			case CLOCK_SELF_32K:
				...
				break;
			case CLOCK_DCXO_32K:
				OSC_REG_AND(ANA_POWER_PD_SW,~LDO_DCXO_PD);	//open DCXO 32K LDO
				OSC_Delay(3);
				break;						
			case CLOCK_EXTE_32K:
	...
	else
	{
		switch(clk_src)
		{
			case CLOCK_SELF_32K:
				...
				break;
			case CLOCK_DCXO_32K:
				OSC_REG_OR(ANA_POWER_PD_SW,LDO_DCXO_PD);	//disable DCXO 32K LDO
				OSC_Delay(3);
				break;						
			case CLOCK_EXTE_32K:
				SCI_ASSERT(0);//do not support external 32k crystal
...
PUBLIC void OSC_PHY_SELF_CaliEnable (BOOLEAN isEnable)
{
    if (isEnable)
    {
        ...
        OSC_Delay (3);
        OSC_REG_OR (ANA_ARM_CLK_EN0,ANA_CLK_CAL_EN);
        OSC_Delay (3);
    ...
    else
    {
        ...
        OSC_Delay (3);
        OSC_REG_AND (ANA_ARM_CLK_EN0,~ANA_CLK_CAL_EN);
        OSC_Delay (3);
