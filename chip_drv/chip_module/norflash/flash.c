...
void FLASH_GetRfParaAsCrystalAgeing(void)
{
    ...
    {
        g_flash_rfpara_addr = (uint32)pblock2 + 0xFFE0;     /*设置当前地址为block2最后一个数据地址*/
    }
    else
    {
        pfront = pblock1;
...
void FLASH_WriteRfParaAsCrystalAgeing(uint32 *src_addr)
{
    ...
    if ((g_flash_rfpara_addr + FLASH_RFPARA_TOTAL_SIZE) & 0xFFFF)   //next write_addr still at current block
    {
        ...
        if ((g_flash_rfpara_addr & 0xFFFF0000) == (uint32)pblock1)
        {
            pblock = pblock2;
        }
        else
        ...
        if ((*pblock != 0xFFFFFFFF) &&  (*((uint32 *)((uint32)pblock + 0xFFFC)) != 0xFFFFFFFF))    /*之前擦除并没有完成*/
        {
...
PUBLIC void FLASH_RfParaStoreThreadInit()
{
       ...
       gp_flash_rfpara_block2_addr = (uint32 *)(SPIFLASH_GetCrystalAgingAddress() + 0x10000);
