...
#define REG_TIMER4_HW_CUR_VAL            0x0880E01c


uint32 timer_Get20MRealTimer4(void) // 0.5us per uint
{    
    return (*(VOLATILE uint32 *)REG_TIMER4_HW_CUR_VAL);
}
void timer_Delayus (int us_delay)
{

   uint32 wait_time = 0;
   uint32 wait_time2 = 0;
   wait_time2 = us_delay*2;
    wait_time = timer_Get20MRealTimer4();  //TIMER_2
    while(1)
    {
        if((timer_Get20MRealTimer4() - wait_time) > wait_time2)
        {
            return;
        }
    }
...
{    
    uint32 timer_regs_protect_table[TIMER_SAVE_REG_NUM] = 
    {    
        REG_TIMER2_BASE+0x08,     
        REG_TIMER2_BASE+0x10,    
