...
PUBLIC void __vbc_iis_high_for_da0(uint32 en)
{
    ...
    _vbc_en_op(0,VBIISSEL,VBIISSEL_DA_LRCK);
}

PUBLIC void __vbc_iis_high_for_ad0(uint32 en)
{
    //MUST BE 0. OR SOME TEST PERFORMANCE WILL BE FAIL!!
    // IIS timeing :0 High(right channel) for DA0, Low(left channel) for DA1
    _vbc_en_op(0,VBIISSEL,VBIISSEL_AD_LRCK);
}

LOCAL void audio_clock_init()
{
#if defined(PLATFORM_SC6531EFM) || defined(PLATFORM_UIX8910)
    // On NemoG platform, set audio clock to "TUNED 26M" which shares the
    // same source CPLL as RF modules to avoid unexpected mismatch.

    // set aud cfg to CLK_26M_TUNED
    CHIP_REG_AND(CGM_AUD_CFG, ~CGM_AUD_SEL_MSK);
    CHIP_REG_OR(CGM_AUD_CFG, 2<<CGM_AUD_SEL_SHIFT);

    // set aud if cfg to CLK_26M_TUNED
    CHIP_REG_AND(CGM_AUDIF_CFG, ~CGM_AUDIF_SEL_MSK);
    CHIP_REG_OR(CGM_AUDIF_CFG, 2<<CGM_AUDIF_SEL_SHIFT);

    //switch audio analog clock to 26M tuned
    CHIP_REG_OR(ANALOG_BB_TOP0,CLK26M_AUD_IN_SEL);
#endif
}
...
LOCAL void _vbc_pt_tone_en(int16 enable)
{
    if(enable)
        VB_WRITE_BITS(DANCHCTL,(BIT_7),(BIT_7)); 
    else
        VB_WRITE_BITS(DANCHCTL,(~BIT_7),(BIT_7)); 
}
LOCAL void _vbc_pt_tone_on(uint32 on)
{
    if(on)   
        VB_WRITE_BITS(DANCHCTL,(BIT_8),(BIT_8));
    else
        VB_WRITE_BITS(DANCHCTL,(~BIT_8),(BIT_8));
}
LOCAL void _vbc_pt_para_set(uint32 fs)
{
    switch(fs){
    case 44100:{
    REG32(DANCH_PTSINMW)=0xfe67;
    REG32(DANCH_PTSINLW)=0x891a;
    REG32(DANCH_PTCOSMW)=0x3ffa;
    REG32(DANCH_PTCOSLW)=0xe856;
    }
    break;
   case 32000:{
    REG32(DANCH_PTSINMW)=0xfdcd;
    REG32(DANCH_PTSINLW)=0x232b;
    REG32(DANCH_PTCOSMW)=0x3ff6;
    REG32(DANCH_PTCOSLW)=0x5428;
    }
    break;
    case 16000:
    break;
    case 48000:{
    REG32(DANCH_PTSINMW)=0xFE7D;
    REG32(DANCH_PTSINLW)=0xFF2F;
    REG32(DANCH_PTCOSMW)=0x3FFB;
    REG32(DANCH_PTCOSLW)=0x73CA;
    }
    break;
    default:{
    REG32(DANCH_PTSINMW)=0xfe67;
    ...
    SCI_TRACE_LOW("_vbc_pt_para_set fs error!"); 
    break;
    }
    //REG32(DANCH_PTGL)=0x733;
    //REG32(DANCH_PTGR)=0x733;
    REG32(DANCH_PTGR)=0x395;
    REG32(DANCH_PTGL)=0x395;
    REG32(DANCH_PTGD)=0x8;
    REG32(DANCH_PTGU)=0x2;
}
LOCAL void _vbc_pt_tone_hold_time()
{
    REG32(DANCH_PTH)=0x0100;
}
PUBLIC void _vbc_notch_enable(uint32 en,uint32 fs)
{
    if(en){
        _vbc_pt_para_set(fs);
        _vbc_pt_tone_hold_time();
        _vbc_pt_tone_en(1);
        _vbc_pt_tone_on(1);
    }
    else{
...
PUBLIC void _vbc_set_notchpara(NothPara* para)
{
    SCI_TRACE_LOW("para->ptgl=0x%x,para->ptgr=0x%x,para->gd=0x%x",para->ptgl,para->ptgr,para->gd);

    REG32(DANCH_PTSINMW)=para->ptsinM;
    REG32(DANCH_PTSINLW)=para->ptsinL;
    REG32(DANCH_PTCOSMW)=para->ptcosM;
    REG32(DANCH_PTCOSLW)=para->ptcosL;
    REG32(DANCH_PTGL)=para->ptgl;
    REG32(DANCH_PTGR)=para->ptgr;
    REG32(DANCH_PTGD)=para->gd;
    REG32(DANCH_PTGU)=para->gu;
    REG32(DANCH_PTH)=para->holdtime;
}
/**---------------------------------------------------------------------------*
 **                         Compiler Flag                                     *
 **---------------------------------------------------------------------------*/
