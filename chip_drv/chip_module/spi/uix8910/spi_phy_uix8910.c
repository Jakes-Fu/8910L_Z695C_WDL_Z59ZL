...
)
{
    ...
    while (write_bytes_cnt < len)
    {
        ...
        {
            SCI_PASSERT (0, ("cnt=%d,len=%d,ctrl=0x%x,cfg=0x%x,status=0x%x",write_bytes_cnt,len,/*assert verified*/
            ...
            else
            {
                switch (bitlen)
                {
                    ...
                    case BITLEN_32:     // 32bit mode
                        ...
                        break;
                    default:
                        SCI_PASSERT (0,("bitlen:%d",bitlen));/*assert to do*/
                        break;
                }
            }
        }
        if(p_hwp_spi->status & SPI_TX_OVF )p_hwp_spi->status |= SPI_TX_OVF;
    }
...
)
{
    ...
    while (read_bytes_cnt < len)
    {
        ...
        if (time_out_cnt > SPI_PHY_POLL_TIMEOUT)
        {
            SCI_PASSERT (0, ("cnt=%d,len=%d,ctl=0x%x,cfg=0x%x,status=0x%x,txrx=0x%x,pa=0x%x",read_bytes_cnt,/*assert verified*/
                              len,p_hwp_spi->ctrl,p_hwp_spi->cfg, p_hwp_spi->status,p_hwp_spi->rxtx_buffer,p_hwp_spi->pattern));
        }
            ...
            else
            {
                switch (bitlen)
                {
                    case BITLEN_8:     // 8bit mode
                        ...
                        if (RW_CHECK == rw_chk_mode)
                        {
                            * ( (uint8 *) (&din_buffer_ptr[read_bytes_cnt])) = p_hwp_spi->rxtx_buffer;
                            read_bytes_cnt +=INCREMENT_BYTE;
                        }
                        else if (RW_NOCHECK == rw_chk_mode)
                        {
                    ...
                    case BITLEN_16:    // 16bit mode
                        ...
                        if (RW_CHECK == rw_chk_mode)
                        {
                            * ( (uint16 *) (&din_buffer_ptr[read_bytes_cnt])) = p_hwp_spi->rxtx_buffer;
#ifdef CHIP_ENDIAN_LITTLE
                    ...
                    case BITLEN_32:     // 32bit mode
                        ...
                        if (RW_CHECK == rw_chk_mode)
                        {
                            * ( (uint32 *) (&din_buffer_ptr[read_bytes_cnt])) = p_hwp_spi->rxtx_buffer;
#ifdef CHIP_ENDIAN_LITTLE
        ...
        if(p_hwp_spi->status & SPI_RX_OVF )p_hwp_spi->status |= SPI_RX_OVF;
        if(p_hwp_spi->status & SPI_RX_UDF )p_hwp_spi->status |= SPI_RX_UDF;
    }
...
)
{
    ...
    {
        case 0:
            phy_inst->ctrl |= SPI_CLOCK_DELAY(1) |SPI_DO_DELAY(0) | SPI_DI_DELAY(3) | SPI_CS_DELAY(3);
            break;
        case 1:
    ...
    /*set bit len*/
    SPI_PHY_SetBitLen(phy_id,tx_bit_length);
    ...
    p_hwp_spi->ctrl &= ~SPI_CTRL_DATA_MUX_SEL_MASK;

    p_hwp_spi->ctrl |= SPI_INPUT_SEL(1);

    p_hwp_spi->ctrl |= SPI_ENABLE;
...
LOCAL int32 SPI_PHY_WaitForBusIdle (uint32 spi_add)
{
    ...
    while ((p_hwp_spi->status & SPI_TX_SPACE_MASK) != 0x100000)
    {
        time_out_cnt++;

        if (time_out_cnt > (uint32) SPI_PHY_POLL_TIMEOUT)
        {
            SCI_PASSERT (0, ("ctrl=0x%x,cfg=0x%x,status=0x%x,pctrl=0x%x",/*assert verified*/
                             p_hwp_spi->ctrl, p_hwp_spi->cfg, p_hwp_spi->status,p_hwp_spi->pin_control));
...
)
{
    ...
    p_hwp_spi->pin_control |=SPI_CS0_CTRL_FORCE_0_CTRL;
    p_hwp_spi->ctrl &= ~(SPI_OE_DELAY_MASK);
    p_hwp_spi->cfg &= ~(SPI_CLOCK_LIMITER_MASK);

    p_hwp_spi->status |= SPI_FIFO_FLUSH;
    /*set transmit only write mode*/
    p_hwp_spi->ctrl |= SPI_ENABLE;
    p_hwp_spi->ctrl &= (~SPI_INPUT_MODE);


    while (len)
    {
        txspace = p_hwp_spi->status;
        txspace &= SPI_TX_SPACE_MASK;
        txspace = txspace >> SPI_TX_SPACE_SHIFT;
        wrlen = (len > txspace) ? txspace : len;
        __SPI_PHY_WBytes (phy_id, buffer_ptr, wrlen, RW_CHECK);
...
)
{
    ...
    while (read_bytes_cnt < len)
    {
        ...
        rxlevel = p_hwp_spi->status;
        rxlevel &= SPI_RX_LEVEL_MASK;
        rxlevel = rxlevel >> SPI_RX_LEVEL_SHIFT;
        rxlevel = SPI_FIFO_SIZE - rxlevel; 
        cur_cnt = ((len - read_bytes_cnt) > rxlevel) ? rxlevel : (len - read_bytes_cnt);
...
)
{
    ...
    phy_inst->cfg &= ~SPI_CLOCK_DIVIDER_MASK;
    phy_inst->cfg |= SPI_CLOCK_DIVIDER(spi_clkd);
...
)
{
    ...
    spi_clk = source_clk / (2 * (((p_hwp_spi->cfg & SPI_CLOCK_DIVIDER_MASK) >> SPI_CLOCK_DIVIDER_SHIFT) + 1));
...
)
{
    ...
    bitlen =  ((p_hwp_spi->ctrl & SPI_FRAME_SIZE_MASK) >> SPI_FRAME_SIZE_SHIFT) + 1;
...
)
{
    ...
    SPI_PHY_WaitForBusIdle (spi_addr);
    SCI_PASSERT (7 == bit_len ||8 == bit_len || 9 == bit_len ||
                10 == bit_len || 11 == bit_len || 12 == bit_len || 16 == bit_len ||
                32 == bit_len,("bitlen:%d",bit_len));/*assert to do*/
