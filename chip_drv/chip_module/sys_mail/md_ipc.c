...
{
    g_mdev->regs->SYSMAIL87 = SM_BASE + g_mdev->bt_rf_sm_off;
}
...
static int32 ipc_init_ch(struct smd_ch *ch)
{
	...
	case SMD_CH_PS:
        ...
        mdev->regs->SYSMAIL67 = SM_BASE + mdev->delta_nv_off;
        mdev->regs->SYSMAIL68 = SM_BASE + mdev->ims_nv_info_off;
        mdev->regs->SYSMAIL87 = SM_BASE + mdev->bt_rf_sm_off;
...
void ipc_hisr_func (uint32 i, void *d)
{
 ...
 // not support?
	...
	if (sysmail_irq_status & RISC_BT_ASSERT_NOTIFY)
    {
       ...
       if (sysmail_irq_status & CP_NOTIFY_AP_STORE_RFPARA) {
		SCI_DisableIRQ();
		sysmail_irq_status &= ~CP_NOTIFY_AP_STORE_RFPARA;
		SCI_RestoreIRQ();
 ...
 int ipc_init(void)
{
	...
	mdev->md_version_off	= MD_VERSION_OFF;
	mdev->md_exec_cause_off = MD_EXEC_CAUSE_OFF;
	
	rda_sm_base = SM_BASE;
    sram_sm_base = 0x803000;
    memset(sram_sm_base, 0, 0x1000);
	memset(rda_sm_base, 0, SM_LEN);
