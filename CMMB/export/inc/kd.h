/* Reference KD/kd.h for OpenKODE Core 1.0  */
#ifndef __kd_h_
#define __kd_h_
...
/* kdThreadAttrSetPriority: Set priority attribute. */
KD_API KDint KD_APIENTRY kdThreadAttrSetPriority(KDThreadAttr *attr, KDint priority);
...
/* kdThreadMutexTyeLock: Try to Lock a mutex. */
KD_API KDint KD_APIENTRY kdThreadMutexTryLock(KDThreadMutex *mutex);
...
KD_API KD_NORETURN void KD_APIENTRY kdMsDelay(KDuint32 ms);
...
struct KDEvent {
    ...
    /* FIXME: This is not part of the reference OpenKode 1.0 spec
       Pointer to the related operating system event (if it exists). (for now, it's only implemented by MAC OSX port) */
    void *osEvent;
};
...
/*
	data & funcs added for debugging and testing
 */

char *kdStrcpy(char *dest, const char *src);

char *kdStrncpy(char *dest, const char *src, int count);

char *kdStrcat(char *dest, const char *src);

int kdStrcmp(const char *cs, const char *ct);

KD_API KD_NORETURN void KD_APIENTRY kdSleep(KDuint32 ms);
