/*=====================================================================================
    ...
    GENERAL DESCRIPTION:
        this file defined several functions to manage and deal with the session, peer and master 
        secret  information.
...
=======================================================================================
    ...
    ---------- -------------- --------- --------------------------------------
    2006-07-22        Bird                                          create sec_wimsp.c
...
void Sec_WimPeerRemove (WE_HANDLE hSecHandle, WE_INT32 iTargetID, WE_INT32 iMasterSecretId)
{
    ...
    if (NULL == hSecHandle)
    {
        Sec_WimDeletePeerLinkSessionResponse(hSecHandle, iTargetID, M_SEC_ERR_INVALID_PARAMETER);
        return;
    }
    ...
    Sec_WimDeletePeerLinkSessionResponse(hSecHandle, iTargetID, M_SEC_ERR_OK);
}
            ...
            if (iMatch)
            {
                ...
                if (uiTimeDiff <= uiMaxAge)
                {
                    ...
                    Sec_WimPeerSearchResponse(hSecHandle,iWid, M_SEC_ERR_OK, ucConnectionType, 
                                                  pstPeerCache[iIndexI].iMasterSecretId);
                    return;
                }
                /*
                else
                {
                    ...
                    Sec_WimClrPeerRec(&(pstPeerCache[iIndexI]));
                }*/ /* modified by tang 070613*/

            }
        }
    }
    
    Sec_WimPeerSearchResponse(hSecHandle,iWid, M_SEC_ERR_NOT_FOUND, ucConnectionType, 0);
}
...
void Sec_WimSessionStart (WE_HANDLE hSecHandle)
{
    ...
    if (M_SEC_ERR_OK == Sec_WimCheckFileExist(hSecHandle,M_SEC_SH_NAME))
    {
        ...
        eRes = WE_FILE_OPEN(SEC_WE_HANDLE, M_SEC_SH_NAME, E_WE_FILE_SET_RDWR, &hFileHandle);
        if(eRes != E_WE_OK )
        {
            return;
        }
...
void Sec_WimWriteCacheToFile(WE_HANDLE hSecHandle,WE_UINT32 uiSessionCount, WE_UINT32 uiPeerCount)
{
    ...
    /*create S_H file*/
    eRes = WE_FILE_OPEN(SEC_WE_HANDLE, M_SEC_SH_NAME, E_WE_FILE_SET_CREATE , &hFileHandle);
    if (eRes != E_WE_OK)
    {
        return;
    }
    ...
    /*write head 'S'*/
    eRes = WE_FILE_OPEN(SEC_WE_HANDLE,M_SEC_SH_NAME, E_WE_FILE_SET_WRONLY, &hFileHandle);
    ...
    iResult = Sec_WimEncryptDataToFile(hSecHandle,hFileHandle, (void*)pstSessionCache,
        (WE_INT32)uiPos, (WE_INT32)uiSessionCount, &iWriteNum);
    
    if ((iWriteNum < (WE_INT32)uiSessionCount) ||(iResult != M_SEC_ERR_OK))
    {    
        eRes = WE_FILE_CLOSE(hFileHandle);
        eRes = WE_FILE_REMOVE(SEC_WE_HANDLE,M_SEC_SH_NAME);
        return;
    }
    uiPos = (WE_UINT32)iWriteNum;
    
    /*write length file head */
    eRes = WE_FILE_SEEK(hFileHandle, 0, E_WE_FILE_SEEK_SET,&iNewPos);    
    eRes = WE_FILE_WRITE(hFileHandle,&uiPos, M_FILE_LENGTH_BITS,&iWriteNum);
    if ((iWriteNum < (WE_INT32)M_FILE_LENGTH_BITS) || (eRes != E_WE_OK))
    {    
        eRes = WE_FILE_CLOSE(hFileHandle);
        eRes = WE_FILE_REMOVE(SEC_WE_HANDLE,M_SEC_SH_NAME);
        return;
...
void Sec_WimSessionStop (WE_HANDLE hSecHandle)
{
    ...
    if ( (WE_INT32)E_SEC_WAP == SEC_CALLER_TYPE) /*is wtls*/
    {
                           ...
                           WE_INT32 *piMasterSecretId, WE_INT32 *piRes)
{
    ...
    /*Sec_WimClrSessionRec(&(pstSessionCache[*piMasterSecretId]));*/ /*modified by tang 070613*/
    for (iIndexI=0; iIndexI<iMasterSecretLen; iIndexI++) 
    {
...
AUTHOR:
    Bird Zhang
DESCRIPTION: 
    get session and peer data length, and check the length of file. if length is wrong, 
        delete the file.    
ARGUMENTS PASSED:
                                 ...
                                 WE_UINT32* puiViewLength,WE_LONG *plReadNum)
{
    ...
    if((NULL == hSecHandle) || (NULL == puiSessionLength) ||(NULL == puiPeerLength) 
       || (NULL == puiViewLength) || (NULL == plReadNum) || (NULL == hFileHandle))
    {
        return -1;
    }
    
    eResult = WE_FILE_GETSIZE(SEC_WE_HANDLE, M_SEC_SH_NAME, &(iFileSize));
    if (eResult != E_WE_OK)
    {
        return -1;
    }
    if (iFileSize < iDataLen)
    {
        return -1;
    }
    ...
    eResult = WE_FILE_SEEK(hFileHandle, 0, E_WE_FILE_SEEK_SET,&iNewPos);
    eResult = WE_FILE_READ(hFileHandle, pucData, iDataLen, &iReadNum);
    if(eResult != E_WE_OK || iReadNum < iDataLen)
    {
        WE_FREE(pucData);
        return -1;
    }
    ...
    *pucViewTemp = *pucTemp;
    ...
    if ((WE_UINT32)iFileSize != (*puiSessionLength + *puiPeerLength + *puiViewLength + 3 * M_FILE_LENGTH_BITS))
    {
        return -1;
    }
    return 0 ;
