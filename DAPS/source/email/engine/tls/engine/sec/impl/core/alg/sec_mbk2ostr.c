...
E_Result We_MbkRegPrep(St_Reg * pstReg, WE_UINT32  uiPrec, E_Result ePreserve)
{
    ...
    if(pstReg->uiAllocPrec < uiPrec) 
    {
        ...
        else 
        {
            ...
            {
                (WE_VOID)WE_MEMCPY(puiNewAlloc, pstReg->stNormVal.puiDig,pstReg->stNormVal.uiPrec * sizeof(WE_UINT32));
            }
            else 
            {
...
WE_UCHAR  *We_MbkAsn1Berinteger2s32(WE_UCHAR *pucSrc, WE_INT32 *piNum)
{
    WE_UINT32    uiType = 0;
    WE_LONG      lSize = 0;
    ...
    if(lSize > 4) 
    {
        /*ASN.1 integer too large */
        return NULL;
    }
    *piNum = *pucSrc & 128 ? -1L : 0L;
    while(lSize--)
    {
        *piNum = (*piNum << 8) + (WE_INT32) *pucSrc++;
    }
    return pucSrc;
}
#endif
