...
*******************************************************************************/
#define M_SEC_LIB_HANDLE_DH            (((St_PublicData *)(((ISec*)hSecHandle)->hPrivateData))->pvAlgHandle)
...
/*for dh add by tang 070410*/
static WE_INT32 We_MpmSqrtInit(pst_Reg pstRoot, pst_Reg pstRem, pst_Reg pstOp);
#endif
...
static E_Result We_MpmExpmAbs(St_Reg * pstRes, St_Reg * pstOp, St_Reg * pstExp, St_Reg * pstMod)
{
    ...
    pstTmp->puiDig =(WE_ULONG *) WE_MALLOC((uiModPrec > uiOpPrec ? uiModPrec : uiOpPrec) * 2 * sizeof(WE_ULONG));
    if(pstTmp->puiDig == NULL)
    {
        for(usIIndex = 0; usIIndex < POW_M; usIIndex++)
        {
    ...
    pstIres->puiDig =(WE_ULONG *) WE_MALLOC(uiModPrec * sizeof(WE_ULONG));
    if(pstIres->puiDig == NULL)
    {
        for(usIIndex = 0; usIIndex < POW_M; usIIndex++)
        {
            if(astBk[usIIndex].puiDig != 0)
...
==================================================================================================*/
#ifdef M_SEC_CFG_DHE_RSA
WE_VOID We_MpmRegNorm(pst_Reg pstReg, WE_UINT32 uiPrec)
{  
...
E_Result We_MpmRegPrep(pst_Reg pstReg, WE_UINT32 uiPrec, E_Result ePreserve)
{ 
    if(pstReg->uiAllocPrec < uiPrec) 
    {
        ...
        else 
        {
            ...
            {
                pstReg->stNormVal.uiPrec =(WE_UINT32)0;
            }
...
static WE_INT32 We_MpmSqrtInit(pst_Reg pstRoot, pst_Reg pstRem, pst_Reg pstOp)
{
    ...
    if(!We_MpmRegPrep(pstRoot, (pstOp->stNormVal.uiPrec + 1) >> 1, E_FALSE))
    {
        return -1;
    }
    if(pstRem && !We_MpmRegPrep(pstRem, pstOp->stNormVal.uiPrec + 1, E_FALSE))
    {
        return -1;
    }
    
    iRval = We_MpaSqrt(&pstRoot->stNormVal, pstRem ? &pstRem->stNormVal : 0,
        &pstOp->stNormVal);
    We_MpmRegNorm(pstRoot, (pstOp->stNormVal.uiPrec + 1) >> 1);
    if(pstRem)
    {
        We_MpmRegNorm(pstRem, (pstOp->stNormVal.uiPrec + 3) >> 1);
    }    
    return iRval;
}
/*==================================================================================================
...
IMPORTANT NOTES:
    None
==================================================================================================*/
WE_VOID We_MpmSqrt(WE_VOID * pvMid, E_MpmReg eRootRid, E_MpmReg eRemRid, E_MpmReg eOpRid)
{
    pst_Mach    pstMach = (pst_Mach)pvMid;
    pst_Reg     pstOpReg = NULL; 
    pst_Reg     pstRootReg = NULL; 
    pst_Reg     pstRemReg = NULL; 
    WE_INT32    iRval = 0;    
    ...
    if(REG_NEG(*pstOpReg)) 
    {
        pstMach->stFlag.uiNegative = (WE_UINT32)E_TRUE;
        return;
    }
    
    pstRootReg = eRootRid == eOpRid ? &pstMach->stExtraReg
        : &pstMach->stRegister[eRootRid];
    pstRemReg = eRemRid == eRootRid ? 0 : eRemRid == eOpRid ? &pstMach->stExtraReg
        : &pstMach->stRegister[eRemRid];
    if((iRval = We_MpmSqrtInit(pstRootReg, pstRemReg, pstOpReg)) < 0) 
    {
        pstMach->stFlag.uiOverFlow = (WE_UINT32)E_TRUE;
        return;
    }
    ...
    if(eRootRid == eOpRid) 
    { 
        pst_Reg actual_root_reg_p = &pstMach->stRegister[eRootRid];
        
        (void) We_MpmRegMoveAbs2(actual_root_reg_p, pstRootReg, E_FALSE);
        pstRootReg = actual_root_reg_p;
    } 
    else if(eRemRid == eOpRid) 
    { 
        pst_Reg actual_rem_reg_p = &pstMach->stRegister[eRemRid];
        
        (void) We_MpmRegMoveAbs2(actual_rem_reg_p, pstRemReg, E_FALSE);
        pstRemReg = actual_rem_reg_p;
    }
    
    pstRootReg->eSign = E_MPM_POS;
    if(pstRemReg)
    {
        pstRemReg->eSign = E_MPM_POS;
    }
    pstMach->stFlag.uiZero = iRval == 0 ? (WE_UINT32)E_TRUE : (WE_UINT32)E_FALSE;
    pstMach->stFlag.uiNegative = (WE_UINT32)E_FALSE;
}
/*==================================================================================================
FUNCTION: 
...
IMPORTANT NOTES:
    None
==================================================================================================*/
WE_UINT16 We_MpmSizeOf(WE_VOID * pvMid, E_MpmReg eOpRid)
{
    pst_Mach pstMach = (pst_Mach)pvMid;
    
    return We_MpmRegPlace(&pstMach->stRegister[eOpRid]);
}
/*==================================================================================================
FUNCTION: 
    We_MpmRegPlace
...
IMPORTANT NOTES:
    None
==================================================================================================*/
WE_UINT16 We_MpmRegPlace(pst_Reg pstReg)
{
    WE_UINT16  usPrec = 0;
    WE_UINT32  uiMsPlace = 0;     
    ...
    {       
        ...
        if((usPrec - 1) /((WE_UINT16)REP_RATIO) != (WE_UINT16)(pstReg->stNormVal.uiPrec - 1) )
        {
            /*Internal Function Failure*/
            return 0;
        }        
        ...
        while(uiMsPlace >= BASE_MPM_DIG) 
        {
            if(++usPrec == 0) 
            {
                /*Memory Overflow in We_MpmRegPlace*/
                return 0;
            }
...
IMPORTANT NOTES:
    None
==================================================================================================*/
WE_VOID We_MpmLoad(WE_VOID * pvMid, E_MpmReg eDestRid, E_MpmSign eSrcSign, WE_UINT16 usSrcPrec,
                   WE_UCHAR *pucSrcPlaces)
{
    pst_Mach          pstMach = (pst_Mach)pvMid;
    WE_UINT32       uiDestPrec = 0;
    WE_ULONG       *puiDestPlaces = NULL;
    pst_Reg            pstDestReg = NULL; 
    WE_UINT16       usILoop = 0; 
    WE_UINT16       usJLoop = 0; 
    ...
    if(!We_MpmRegPrep(pstDestReg, uiDestPrec, E_FALSE)) 
    {
        pstMach->stFlag.uiOverFlow = (WE_UINT32)E_TRUE;
        return;
    } 
    puiDestPlaces = pstDestReg->stNormVal.puiDig;
    for(usILoop = (WE_UINT16) uiDestPrec; usILoop > 0; usILoop--) 
    {
        WE_UINT16       usBlkSz = 0;
        WE_UCHAR        *pucSrcBlk = NULL;
        WE_UINT32       uiHolder = 0;
        
        usBlkSz = (WE_UINT16) (usILoop != 1 ? REP_RATIO
            : usSrcPrec - ((uiDestPrec - 1) * REP_RATIO));
        pucSrcBlk = pucSrcPlaces += usBlkSz;
        uiHolder = 0;
        for(usJLoop = usBlkSz; usJLoop > 0; usJLoop--)
        {
            uiHolder = uiHolder * BASE_MPM_DIG + *(--pucSrcBlk);
        }
        *puiDestPlaces++ = uiHolder;
    }    
    
    pstDestReg->eSign = eSrcSign;
    We_MpmRegNorm(pstDestReg, uiDestPrec);
}
/*==================================================================================================
FUNCTION: 
    We_MpmTransfer
...
ARGUMENTS PASSED:
    WE_VOID * pvDestMid[IN]:
    E_MpmReg eDestRid[IN]:
    E_MpmSign eSrcSign[IN]:
    WE_UINT16 eSrcRid[IN]:
    E_Result ePreserve[IN]:
RETURN VALUE:
...
IMPORTANT NOTES:
    None
==================================================================================================*/
WE_VOID We_MpmTransfer(WE_VOID * pvDestMid, E_MpmReg eDestRid, WE_VOID * pvSrcMid, E_MpmReg eSrcRid, E_Result ePreserve)
{
    pst_Mach        pstDestMach = (pst_Mach)pvDestMid;
    pst_Mach        pstSrcMach = (pst_Mach)pvSrcMid;
    pst_Reg         pstSrcReg = NULL; 
    pst_Reg         pstDestReg = NULL; 
    E_MpmSign       eDestSign = (E_MpmSign)0;    
    
    pstDestReg = &pstDestMach->stRegister[eDestRid];
    pstSrcReg = &pstSrcMach->stRegister[eSrcRid];
    if(pvDestMid == pvSrcMid && eDestRid == eSrcRid)
    {
        return; 
    }
    eDestSign = REG_SIGN(*pstSrcReg);
    if(!We_MpmRegMoveAbs2(pstDestReg, pstSrcReg, ePreserve)) 
    {
        pstDestMach->stFlag.uiOverFlow = (WE_UINT32)E_TRUE;
        return;
    }
    pstDestReg->eSign = eDestSign;
}
...
IMPORTANT NOTES:
    None
==================================================================================================*/
E_Result We_MpmRegMoveAbs2(pst_Reg pstDest, pst_Reg pstSrc, E_Result ePreserve)
{
    if(!pstDest||!pstSrc)
    {
        return E_FALSE;
    ...
    if(ePreserve) 
    { 
        if(!We_MpmRegPrep(pstDest, pstSrc->stNormVal.uiPrec, E_FALSE))
        {
            return E_FALSE;
        }
        pstDest->stNormVal.uiPrec = pstSrc->stNormVal.uiPrec;
        (WE_VOID)WE_MEMCPY(pstDest->stNormVal.puiDig, pstSrc->stNormVal.puiDig,
            pstSrc->stNormVal.uiPrec * sizeof(WE_INT32));
        return E_TRUE;
    } 
    else 
    {
...
ARGUMENTS PASSED:
    ...
    WE_VOID     *pvUml[IN]:Pointer to the uml.
    WE_INT32    iDest[IN]:dest.
    WE_INT32    iLow[IN]:low.
    WE_INT32    iHigh[IN]:high.
RETURN VALUE:
...
IMPORTANT NOTES:
    None
==================================================================================================*/
WE_INT32 We_MpmPrng2Mpm(WE_HANDLE   hSecHandle,WE_VOID *pvUml, WE_INT32 iDest, WE_INT32 iLow, WE_INT32 iHigh)
{
    WE_VOID         *pvUm = (WE_VOID *) pvUml;
    WE_VOID         *pvLm = We_MpmNewMach();
    WE_INT32        iRval = -1;
    WE_UINT16       usDigits = 0;
    WE_UCHAR        *pucInput = NULL;
    WE_UINT32       uiDigits32 = 0;
    WE_INT32        iRes = 0;
    
    const E_MpmReg eLLow = E_MPM_R0, eLHigh = E_MPM_R1, eLDiff = E_MPM_RMOD, eLRes = E_MPM_R2;
    
    if(!pvLm) 
    {
        /*failed to create new MPM pvMachine*/
        return -1;
    }
    
    We_MpmTransfer(pvLm, eLLow, pvUm, (E_MpmReg)iLow, E_FALSE);
    We_MpmLToMpm(pvLm, E_MPM_R3, 1);
    We_MpmAdd(pvLm, eLLow, E_MPM_R3, eLLow);
    We_MpmTransfer(pvLm, eLHigh, pvUm, (E_MpmReg)iHigh, E_FALSE);
    We_MpmSub(pvLm, eLDiff, eLHigh, eLLow);
    if(We_MpmFlagSet(pvLm, E_MPM_OVERFLOW)) 
    {
        We_MpmTransfer(pvUm, (E_MpmReg)iLow, pvLm, eLLow, E_FALSE);
        We_MpmTransfer(pvUm, (E_MpmReg)iHigh, pvLm, eLHigh, E_FALSE);
        We_MpmDisposeMach(pvLm);
        return iRval;
    }   
    
    uiDigits32 = We_MpmSizeOf(pvLm, eLDiff) + 4;
    usDigits = (WE_UINT16) uiDigits32;
    pucInput = WE_MALLOC(usDigits * sizeof(*pucInput));
    if(!pucInput) 
    {
        We_MpmTransfer(pvUm, (E_MpmReg)iLow, pvLm, eLLow, E_FALSE);
        We_MpmTransfer(pvUm, (E_MpmReg)iHigh, pvLm, eLHigh, E_FALSE);
    ...
    iRes = WE_CRPT_GENERATE_RANDOM(M_SEC_LIB_HANDLE_DH,(WE_INT8*)pucInput,usDigits);
    if(iRes)
    {
        WE_FREE(pucInput);
        We_MpmTransfer(pvUm, (E_MpmReg)iLow, pvLm, eLLow, E_FALSE);
    ...
    We_MpmLoad(pvLm, eLRes, E_MPM_POS, usDigits, pucInput);
    if(We_MpmFlagSet(pvLm, E_MPM_OVERFLOW)) 
    {
        WE_FREE(pucInput);
        We_MpmTransfer(pvUm, (E_MpmReg)iLow, pvLm, eLLow, E_FALSE);
    ...
    We_MpmAdd(pvLm, eLRes, eLLow, eLRes);
    if(We_MpmFlagSet(pvLm, E_MPM_OVERFLOW)) 
    {
        WE_FREE(pucInput);
        We_MpmTransfer(pvUm, (E_MpmReg)iLow, pvLm, eLLow, E_FALSE);
    ...
    We_MpmTransfer(pvUm, (E_MpmReg)iLow, pvLm, eLLow, E_FALSE);
    We_MpmTransfer(pvUm, (E_MpmReg)iHigh, pvLm, eLHigh, E_FALSE);
    We_MpmTransfer(pvUm, (E_MpmReg)iDest, pvLm, eLRes, E_FALSE);
    We_MpmDisposeMach(pvLm);
    return iRval;
}
#endif
