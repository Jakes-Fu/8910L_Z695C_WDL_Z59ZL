...
#define TABLE_GB_LEN          7445        /* total chars defined in GB2312 */
static const WE_UINT16  ausGB2312[] = 
{
...
//give a unicode, return its GB code, if this unicode has no GB code, return 0xffff
static WE_UINT16 LookGBTable(WE_UINT16  unicode)
{
   WE_UINT16  i = 0;
   WE_UINT16  temp = 0, data = 0;

   /*modify the data by rachel*/
//   temp = (unicode << 8) | (unicode >> 8);
//   unicode = temp;
   
   for (i = 0; i < TABLE_GB_LEN; i ++)
   {
      if(ausGB2312[i*2+1] == unicode)
      {
         temp =  ausGB2312[i*2] + 0x8080;
         data = (temp << 8) | (temp>>8); 
         return data;
      }
   }
...
WE_BOOL  UnicodeToGb2312(const WE_UINT16 *pusSrc, WE_UINT8 *pucDes, WE_INT32 *plDstLen)
{
   ...
   if (NULL == pusSrc || NULL == plDstLen) 
   {
      return FALSE;
   }
 
   usLength = (WE_UINT16)WE_CSC_WSTRLEN((unsigned short *)pusSrc);
   /*if the pusSrc is ascii,usLength may be 1 */
   if (usLength < 1) 
   {
      return FALSE;
   }
   ...
   {        
      ...
      if( NULL != pucDes)
      {
         ...
         *pucDes++ = (usMultiCode & 0xFF);
         *pucDes++ = ((usMultiCode >> 8) & 0xFF);
      }
      iDindex += 2;
   }
...
static WE_UINT16  UnicodeFromGBTable(WE_UINT16  srcWord)
{
   WE_UINT16  i;
   
   for (i = 0; i < TABLE_GB_LEN; i ++)
   {
      if(ausGB2312[i*2] == srcWord)
      {
         return ausGB2312[i*2+1];
      }
   }
...
{    
   ...
   if (NULL == pucSrc || NULL == piSrcLen || *piSrcLen <= 0 || NULL == piDstLen) 
   {
      return FALSE;
   }
