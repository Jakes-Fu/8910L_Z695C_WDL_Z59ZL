...
#define TABLE_BIG5_LEN  13710       //total chars defined in BIG5
static const WE_UINT16 ausBig5[]=
{
...
//when convert from GB/BIG5 into unicode, if there is no corresponding unicode found (impossible),
//the 0x003f ('?') will be used.
#define UNICODE_ERR_CHAR    0x003F 

static WE_UINT16  UnicodeFromBIG5Table(WE_UINT16  srcWord)
{
   WE_UINT16  i;
   
   for (i = 0; i < TABLE_BIG5_LEN; i ++)
   {
      if(ausBig5[i*2] == srcWord)
      {
         return ausBig5[i*2+1];
      }
   }
   return UNICODE_ERR_CHAR;
}
...
WE_BOOL  Big5ToUnicode(const WE_UCHAR *pucBig5, WE_UINT32 *puiBig5Byte, WE_UINT16 *pusUnicode, WE_UINT32 *puiUnicodeWord)
{
   ...
   if(NULL == pucBig5 ||  NULL == puiBig5Byte || *puiBig5Byte <= 1 || NULL == puiUnicodeWord)
   {
      return FALSE;
   }

   while(0x00 != (ucSrcByte = *pucBig5++) )
   {    /*ANSI Char?*/
      if(ucSrcByte <= 0x7F)     
      {  /*just convert to 16bit.*/
         *pusUnicode++ = (WE_UINT16) ucSrcByte; 
         usLen ++;            
      }
      else
      {
         usSrcWord = (*pucBig5++) | (((WE_UINT16)ucSrcByte)<<8);
         ...
         *pusUnicode++ = usUnicode;                
         
         usLen  ++;
      }
   }
   
   *puiUnicodeWord = usLen;

   return TRUE;
}//   Big5ToUnicode

//give a unicode, return its BIG5 code, if this unicode has no BIG5 code, return 0xffff
static WE_UINT16  LookBIG5Table(WE_UINT16  unicode)
{
   WE_UINT16   i     = 0;
   WE_UINT16   temp  = 0;
   WE_UINT16   data  = 0;
...
{   
   ...
   if(NULL == pusUnicode || NULL == puiUnicodeWord || *puiUnicodeWord < 1 || NULL == puiBig5Byte)
   {
      return FALSE;
   }
   
   while(puiUnicodeWord--)
   {        
      usUnicode = *pusUnicode++;
      ...
      {  /*discard the high byte */     
         ...
         if( NULL != pucBig5)
         {      
            if (uiDindex + 1 > *puiBig5Byte)
               break;   
            *pucBig5++ = (WE_UINT8)(usUnicode);
         }
         uiDindex += 1;
         continue;
      }
      ...
      if( NULL != pucBig5)
      {
         if (uiDindex + 2 > *puiBig5Byte)
            break; 
         
         *pucBig5++   = (usMultiCode) & 0xFF;
         *pucBig5++   = (usMultiCode >> 8) & 0xFF;
      }
      uiDindex += 2;
   }
   //    if(NULL != pucDes)
