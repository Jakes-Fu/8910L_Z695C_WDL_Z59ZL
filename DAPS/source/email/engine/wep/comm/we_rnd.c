...
#define BBS_LO(x) ((x) & (((WE_UINT32) 1 << MPA_SHORT_BIT) - 1))
...
WE_VOID We_LibMpaMul(St_WeLibMpaNum *pstRes, St_WeLibMpaNum *pstOp1, St_WeLibMpaNum *pstOp2)
{
    ...
    (WE_VOID)WE_MEMSET(pstRes->puiDig, 0, uiOp1Prec * sizeof( WE_UINT32));
    
    for(uiJLoop = 0; uiJLoop < uiOp2Prec; uiJLoop++) 
    {
        WE_UINT32 *puiDst = (WE_UINT32 *)(pstRes->puiDig + uiJLoop);
        WE_UINT32 *puiSrc = (WE_UINT32 *)pstOp1->puiDig;
        WE_UINT32  uiCur = 0;
        WE_UINT32 *puiEnd = puiSrc + uiOp1Prec;
        WE_UINT32  uiML = pstOp2->puiDig[uiJLoop];
        WE_UINT32  uiMH = BBS_HI(uiML);
        
        uiML = BBS_LO(uiML);
        while(puiSrc < puiEnd) 
        { 
            WE_UINT32 uiL1 = *puiSrc++;
            WE_UINT32 uiH1 = BBS_HI(uiL1);
            WE_UINT32 uiNXT = uiCur;
            ...
            uiL1 += uiH1; 
            uiNXT += ( WE_UINT32) (uiL1 < uiH1) << MPA_SHORT_BIT; 
...
{
    WE_UINT32 *pulLastPlaceP = NULL;
    
    if(!pstNum)
    {
        return ;
    }    
    pulLastPlaceP = (WE_UINT32 *)(&pstNum->puiDig[uiPrec]);
...
WE_INT32 We_LibMpaBitLen(St_WeLibMpaNum *pstNum,WE_INT32 *piRval)
{
    ...
    if ((uiPrec * sizeof ( WE_UINT32) * BIT_CHAR + 1) > (WE_UINT32) ~1) 
    {
        /*"number too big");*/
        return M_WE_LIB_ERR_INVALID_PARAMETER;
    }
    
    uiNbits = uiPrec * sizeof( WE_UINT32) * BIT_CHAR + 1;
...
{
    WE_UINT8        ucRShift=0;
    WE_UINT32        *puiSrc = NULL;
    WE_UINT32        *puiDst = NULL;
    WE_UINT32        *puiEnd = NULL;
    WE_UINT32        uiShifted=0;
    WE_UINT32        uiOShifted=0;
    ...
    if(!ucShift) 
    {
        ...
        {
            (WE_VOID)WE_MEMCPY(pstDest->puiDig, pstSrc->puiDig, pstSrc->uiPrec * sizeof( WE_UINT32));
        }
        *puiShifted=0;
        // return 0;
        return ;
...
static WE_UINT32 We_LibMpaSingleDiv( WE_UINT32 uiTHI,  WE_UINT32 uiTLO,  WE_UINT32 uiBLO,  WE_UINT32 *pulQuo)
{
    WE_UINT32 uiRem=0;
    WE_UINT32 uiQuo=0;
...
static WE_INT32 We_LibMpaGuessQuo( WE_UINT32 *puiMsNumer,  WE_UINT32 *puiMsDenom,WE_UINT32 *puiQuo)
{
    WE_UINT32 *puiU = NULL;
    WE_UINT32 *puiV = NULL;
    WE_UINT32  uiRem=0;
...
{
    WE_UINT32   *puiRes = NULL;
    WE_UINT32   *puiOp1 = NULL;
    WE_UINT32   *puiOp2 = NULL;
    WE_UINT32   *puiEnd = NULL;
    WE_UINT32   uiOp = 0;
    WE_UINT32   uiSum = 0;
    WE_UINT32      uiCarry = 0;
    ...
    if(!uiCarry) 
    {
        if(puiRes != puiOp1)
        {
            (WE_VOID)WE_MEMCPY(puiRes, puiOp1, (WE_UINT32) (puiEnd - puiOp1) * sizeof( WE_UINT32));
        }
        *piRval=0;
        return M_WE_LIB_ERR_OK;
    } 
...
static WE_INT32 We_LibMpaMulSub(St_WeLibMpaNum *pstNumer, St_WeLibMpaNum *pstDenom,  WE_UINT32 uiQuoGuess,WE_UINT32 *puiQuo)
{
    WE_UINT32 *puiDst = NULL;
    WE_UINT32 *puiSrc = NULL;
    WE_UINT32  uiMH = 0;
...
{
    WE_UINT32        auiNormNumberHolder[MPA_TMP_PLACES]={0};
    WE_UINT32        auiNormDenomHolder[MPA_TMP_PLACES]={0};
    WE_UINT32        uiTmp=0;
    ...
    WE_UINT32        *puiMsRem=NULL;   
    WE_UINT32        *puiMsDenom=NULL;   
    WE_INT32        iRes=0;
    WE_UINT32        uiShifted=0;
    ...
    puiTmpBuf1 = (WE_UINT32 *)WE_MALLOC((pstNumer->uiPrec + 1) * sizeof( WE_UINT32));	
    puiTmpBuf2 =  (WE_UINT32 *)WE_MALLOC((pstDenom->uiPrec + 1) * sizeof( WE_UINT32));
    ...
    WE_MEMSET(puiTmpBuf1,0,(pstNumer->uiPrec + 1) * sizeof( WE_UINT32));	
    WE_MEMSET(puiTmpBuf2,0,(pstDenom->uiPrec + 1) * sizeof( WE_UINT32));
    //eDiscardQuo = !pstQuo;
    ...
    if(uiShift) 
    {
        ...
        WE_UINT32  *puiPtr = stNormRem.puiDig + uiIdx;        
...
static WE_VOID We_LibMpaShortDiv(St_WeLibMpaNum *pstQuo,  WE_UINT32 *puiRem, St_WeLibMpaNum *pstNumer,  WE_UINT32 uiDenom)
{
    WE_UINT32 uiPrec=0;
    WE_UINT32  uiRemainder = 0;
    if(!puiRem||!pstNumer)
    {
        return ;
    }    
