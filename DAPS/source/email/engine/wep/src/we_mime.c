...
St_WeSCLStrTableEntry *WeMime_GetExtStringsEx(WE_HANDLE hMem, WE_BOOL bMemMgrr)
{
    ...
    if(TRUE == bMemMgrr)
	{
		pstMimeExtStrings = BrwMem_Alloc(hMem,sizeof(St_WeSCLStrTableEntry)*WE_EXT_STRING_TABLE_SIZE);		
	}
    else
	{
...
void WeMime_FreeExtStringsEx(St_WeSCLStrTableEntry *pstMimeExtStrings,WE_HANDLE hMem, WE_BOOL bMemMgr)
{
	...
	if(TRUE == bMemMgr)
	{
		BrwMem_Free(hMem,pstMimeExtStrings);
	}
	else
	{
...
St_WeSCLStrTableEntry *WeMime_GetTypeStringsEx(WE_HANDLE hMem, WE_BOOL bMemMgr)
{
    ...
    if(TRUE == bMemMgr)
	{	
		pstWeMimeTypeStrings = BrwMem_Alloc(hMem,sizeof(St_WeSCLStrTableEntry)*WE_MIME_TYPE_STRING_TABLE_SIZE);
	}
    else
	{
...
void WeMime_FreeTypeStringsEx(St_WeSCLStrTableEntry *pstWeMimeTypeStrings,WE_HANDLE hMem, WE_BOOL bMemMgr)
{
    ...
    if(TRUE == bMemMgr)
	{
		BrwMem_Free(hMem, pstWeMimeTypeStrings);
	}
    else
	{
...
St_WeSCLStrtableInfo *WeMime_GetMimeExtensionsEx(WE_HANDLE hMem, WE_BOOL bMemMgr)
{
    ...
    if(TRUE == bMemMgr)
	{
		pstWeMimeExtensions = BrwMem_Alloc(hMem,sizeof(stWeMimeExtensions));	
	}
	else
	{
...
void WeMime_FreeMimeExtensionsEx(St_WeSCLStrtableInfo *pstWeMimeExtensions,WE_HANDLE hMem,WE_BOOL bMemMgr)
{
    ...
    if(TRUE == bMemMgr)
	{
		...
		WeMime_FreeExtStringsEx(pstWeMimeExtensions->pstStringTable,hMem,TRUE);
		BrwMem_Free(hMem,pstWeMimeExtensions);
	}
    else
	{
		WeMime_FreeExtHashTable(pstWeMimeExtensions->pucHashTable);
...
St_WeSCLStrtableInfo *WeMime_GetMimeTypesEx(WE_HANDLE hMem, WE_BOOL bMemMgr)
{
    ...
    if(TRUE ==	bMemMgr)
	{
		pstWeMimeTypes = BrwMem_Alloc(hMem,sizeof(St_WeSCLStrtableInfo));
	}
	else
	{
...
void WeMime_FreeMimeTypesEx(St_WeSCLStrtableInfo *pstWeMimeTypes,WE_HANDLE hMem,WE_BOOL bMemMgr)
{
    ...
    if(TRUE == bMemMgr)
	{
		...
		WeMime_FreeTypeStringsEx(pstWeMimeTypes->pstStringTable,hMem,TRUE);
		BrwMem_Free(hMem,pstWeMimeTypes);
	}
    else
	{
		WeMime_FreeTypeHashTable(pstWeMimeTypes->pucHashTable);
...
const WE_CHAR * WeMime_Ext2Mime_Ex (const WE_CHAR *pcExt,WE_HANDLE hMem,WE_BOOL bMemMgr)
{
	...
	if(iIndx >= 0)
    {
        if(TRUE == bMemMgr)
		{
			...
			pcStr = Brw_StrDup(hMem,WE_SCL_INT2STR(iIndx, pstWeMimeTypes));
...
const WE_CHAR * WeMime_Mime2Ext_Ex (const WE_CHAR *pcMime,WE_HANDLE hMem, WE_BOOL bMemMgr)
{
    ...
    if(iIndx >= 0)
    {
        if(TRUE == bMemMgr)
		{
			const St_WeSCLStrtableInfo *pstWeMimeExtensions = WeMime_GetMimeExtensionsEx(hMem,TRUE);
			pcStr = Brw_StrDup(hMem,WE_SCL_INT2STR (iIndx, pstWeMimeExtensions));
			WeMime_FreeMimeExtensionsEx((St_WeSCLStrtableInfo *)pstWeMimeExtensions,hMem,TRUE);
		}
		else
		{
...
const WE_CHAR * WeMime_Int2MimeEx (WE_INT32 iKnownVal,WE_HANDLE hMem , WE_BOOL bMemMgr)
{
	...
	{
		pstWeMimeTypes = WeMime_GetMimeTypesEx(hMem,TRUE);
		pcMimeType = Brw_StrDup(hMem,WE_SCL_INT2STR (iKnownVal, pstWeMimeTypes));
		WeMime_FreeMimeTypesEx((St_WeSCLStrtableInfo *)pstWeMimeTypes,hMem,TRUE);
	}
	else
	{
		pstWeMimeTypes = WeMime_GetMimeTypesEx(NULL,FALSE);
...
WE_CHAR ** WeMime_Mime2ExtExEx (const WE_CHAR *pcMime,WE_HANDLE hMem , WE_BOOL bMemMgr) 
{
    ...
    if(iIndx >= 0)
    {
		...
		if(TRUE == bMemMgr)
		{
			...
			{
				WE_INT i = 0;
				pcStr = (WE_CHAR **)BrwMem_Alloc(hMem,CMMN_MEM_SIZEOF(WE_CHAR *) * (iNum+1));
				if(NULL == pcStr)
					return NULL;
				TF3RD_MEMSET(pcStr,0x00,CMMN_MEM_SIZEOF(WE_CHAR *) * (iNum+1));
				for (iCount = 0; iCount < iStringTableSize; iCount++) 
				{
					if (pstStringTable[iCount].iValue == iIndx)
					{
...

{
    ...
    if(pcExt == NULL)
    {
        if(TRUE == bMemMgr)
		{
			pcStr = (WE_CHAR **)BrwMem_Alloc(hMem,sizeof(WE_CHAR *) * 2);
			if(NULL == pcStr)
				return NULL;
			TF3RD_MEMSET(pcStr,0x00,sizeof(WE_CHAR *) * 2);
	...
	if(iIndx >= 0)
    {
		...
		if(TRUE == bMemMgr)
		{
			...
			pcStr = (WE_CHAR **)BrwMem_Alloc(hMem,sizeof(WE_CHAR *) * (iNum+1));
			if(NULL == pcStr)
				return NULL;
			
			TF3RD_MEMSET(pcStr,0x00,sizeof(WE_CHAR *) * (iNum+1));
			for (iCount = 0; iCount < iStringTableSize; iCount++) 
			{
				if (pstStringTable[iCount].iValue == iIndx)
				{
