...
#define CONFIG_DHCPD_LEASES_FILE "D:\\System\\Dhcps\\lease.dat"  //by lisa
//#define bb_common_bufsiz1 100    //by lisa

/* Defaults you may want to tweak */
/* Default max_lease_sec */
#define DEFAULT_LEASE_TIME      (60*60*24*7)
...
// if more than N seconds have passed after last use.
	...
	uint16_t port;
#endif
	uint8_t server_mac[MAC_ADDR_LEN];          /* our MAC address (used only for ARP probing) */
...
/*modified by lisa@2011-7-24*/
//#define server_config (*(struct server_config_t*))
...
struct dyn_lease {
	 ...
	 */
	uint8_t lease_mac[MAC_ADDR_LEN];
	char hostname[20];
	uint8_t pad[2];
