...
extern const uint8_t MAC_BCAST_ADDR[MAC_ADDR_LEN]; /* six all-ones */
#define CONFIG_UDHCPC_SLACK_FOR_BUGGY_SERVERS 40    // to be determined ... lisalin

/*DHCP file related configuration*/
//#define   DHCPS_FILE_FOLDER  "DHCPS"
//lease file name
//#define   DHCPS_LEASE_FILE   "lease"
...
extern const uint8_t dhcp_option_lengths[][2];

extern char* safe_strncpy(char *dst, const char *src, size_t size);
extern int  index_in_strings(const char *strings, const char *key);

extern int udhcp_option_idx(const char *name);

extern uint8_t* udhcp_get_option(struct dhcp_packet *packet, int code);
extern uint32_t udhcp_end_option(uint8_t* option_ptr);
extern void udhcp_add_binary_option(struct dhcp_packet *packet, uint8_t *addopt);
extern void udhcp_add_simple_option(struct dhcp_packet *packet, uint8_t code, uint32_t data);
#if ENABLE_FEATURE_UDHCP_RFC3397
...
#endif
extern struct option_set *udhcp_find_option(struct option_set *opt_list, uint8_t code);
...
extern void udhcp_dump_packet(struct dhcp_packet *packet);
#endif
...
extern uint8_t udhcp_get_optionlen(uint8_t flag);
