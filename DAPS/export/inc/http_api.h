...
#define HTTP_RESPONSE_CODE_UNPROCESSABLE_REQUEST				422
...
typedef enum
{
    HTTP_CONNECTION_DEFAULT = 0,//the default connection value, in this http, the default value is "keep-alive"
...
typedef struct _HTTP_CONTENT_TYPE_HEADER
{
    char*   content_type_ptr;       //the content-type header value content-type:
}HTTP_CONTENT_TYPE_HEADER_T;     //example: application/json

typedef struct _HTTP_IF_MATCH_HEADER
{
    char*   if_match_ptr;      //the IF-MATCH header value if_match:
}HTTP_IF_MATCH_HEADER_T;     //example: "123456"
...
typedef struct _HTTP_PATCH_FILE
{
    HTTP_UNICODE_T  file_name[HTTP_MAX_FILE_NAME_LEN + 1];  //the file name that the data will patch
    uint32          file_size;                              //the file size    
}HTTP_PATCH_FILE_T;

typedef struct _HTTP_DELETE_FILE
{
    HTTP_UNICODE_T  file_name[HTTP_MAX_FILE_NAME_LEN + 1];  //the file name that the data will delete
    uint32          file_size;                              //the file size    
}HTTP_DELETE_FILE_T;
...
typedef struct _HTTP_PATCH_BUFFER
{
    uint8*  buffer_ptr;         //the PATCH data pointer
    uint32  buffer_len;         //the PATCH data length
    BOOLEAN is_copied_by_http;  //if this flag is set TRUE, HTTP will copy the data of buffer_ptr, 
}HTTP_PATCH_BUFFER_T;        //and app can free buffer_ptr after using the relative interface; 
                                //if this flag is set FALSE, HTTP will not copy the data of buffer_ptr,
                                //and app can not free buffer_ptr until APP receives HTTP_SIG_PATCH_CNF

typedef struct _HTTP_DELETE_BUFFER
{
    uint8*  buffer_ptr;         //the delete data pointer
    uint32  buffer_len;         //the delte data length
    BOOLEAN is_copied_by_http;  //if this flag is set TRUE, HTTP will copy the data of buffer_ptr, 
}HTTP_DELETE_BUFFER_T;        //and app can free buffer_ptr after using the relative interface; 
                                //if this flag is set FALSE, HTTP will not copy the data of buffer_ptr,
...
typedef struct _HTTP_PATCH_BODY
{
    ...
    union
    {
        HTTP_PATCH_FILE_T    patch_file;      //the PATCH data use file style
        HTTP_PATCH_BUFFER_T  patch_buffer;    //the PATCH data use buffer style
    }u;
}HTTP_PATCH_BODY_T;

typedef struct _HTTP_DELETE_BODY
{
    ...
    union
    {
        HTTP_DELETE_FILE_T    delete_file;      //the delete data use file style
        HTTP_DELETE_BUFFER_T  delete_buffer;    //the delete data use buffer style
    }u;
}HTTP_DELETE_BODY_T;
...
typedef struct _HTTP_PATCH_PARAM
{
    HTTP_URI_T                      uri;                    //the resource uri
    HTTP_ACCEPT_HEADER_T            accept;                 //accept header value
    HTTP_ACCEPT_CHARSET_HEADER_T    accept_charset;         //accept charset header value
    HTTP_ACCEPT_LANGUAGE_HEADER_T   accept_language;        //accept language header value
    HTTP_IF_MATCH_HEADER_T          if_match;               //if match header value
    HTTP_CONNECTION_E               connection;             //connection header value
    HTTP_USER_AGENT_HEADER_T        user_agent;             //user agent header value
    HTTP_UA_PROFILE_HEADER_T        ua_profile;             //ua profile header value
    HTTP_REFERER_HEADER_T           referer;                //referer header value
    ...
    HTTP_AUTH_PARAM_T               auth;                   //auth param
    HTTP_GET_DATA_T                 get_data;               //get data style: file or buffer
    HTTP_PATCH_BODY_T				patch_body;			    //patch body style:  file or buffer
    HTTP_ENCODING_TYPE_E            accept_encoding;        //accept encoding header value
    BOOLEAN                         need_net_prog_ind;      //if the app need net progress indication, set this flag TRUE
    BOOLEAN                         is_use_relative_url;    //if use relative url 
...
typedef struct _HTTP_DELETE_PARAM
{
	HTTP_URI_T                      uri;                    //the resource uri
    HTTP_ACCEPT_HEADER_T            accept;                 //accept header value
    HTTP_ACCEPT_CHARSET_HEADER_T    accept_charset;         //accept charset header value
    ...
    HTTP_AUTH_PARAM_T               auth;                   //auth param
    HTTP_GET_DATA_T                 get_data;               //get data style: file or buffer
    HTTP_DELETE_BODY_T				delete_body;			//delete body style:  file or buffer
    HTTP_ENCODING_TYPE_E            accept_encoding;        //accept encoding header value
    BOOLEAN                         need_net_prog_ind;      //if the app need net progress indication, set this flag TRUE
    BOOLEAN                         is_use_relative_url;    //if use relative url 
	uint32                          recv_timeout;           //ms
...
typedef struct _HTTP_PATCH_REQ_SIG
{
    _SIGNAL_VARS
    HTTP_CONTEXT_ID_T   context_id;     //context id that http assigned for app
    HTTP_PATCH_PARAM_T   patch_param;   //patch parameter that app set
    HTTP_APP_INSTANCE_T app_instance;   //the app instance
}HTTP_PATCH_REQ_SIG_T;
...
typedef struct _HTTP_DELETE_REQ_SIG
{
    _SIGNAL_VARS
    HTTP_CONTEXT_ID_T   context_id;     //context id that http assigned for app
    HTTP_DELETE_PARAM_T   delete_param;     //options parameter that http set
    HTTP_APP_INSTANCE_T app_instance;   //the app instance
...
typedef struct _HTTP_PATCH_CNF_SIG
{
    _SIGNAL_VARS
    HTTP_CONTEXT_ID_T       context_id;         //context id that http assigned for app
    HTTP_REQUEST_ID_T       request_id;         //the request id that http assigned for app request
...
/*****************************************************************************/
//  Description : patch HTTP data request
//  Global resource dependence : 
//  Author: mingzhe.jin
//  Note: 
/*****************************************************************************/
PUBLIC HTTP_ERROR_E HTTP_PatchRequest(
                                    HTTP_CONTEXT_ID_T          context_id,
                                    const HTTP_PATCH_PARAM_T*  patch_param_ptr,
                                    HTTP_APP_INSTANCE_T        app_instance
                                    );
...
//  Note:
/*****************************************************************************/
PUBLIC HTTP_ERROR_E HttpDeleteCookieFile();
